...
input_list_list, Input_list = create_model("Input_list", {"pvilanguage":int, "from_date":date, "to_date":date, "ci_date":date, "create_inhouse":bool, "sorttype":int, "modetype":int, "min_stay":int})

def repeat_glist_webbl (1)(input_list_list:[Input_list]):
    g_list_list = []
    repeat_list_list = []
    cur_date_list = []
    pvilanguage:int = 0
    ...
    g_list_list, G_list = create_model("G_list", {"resnr":int, "gastnr":int, "name":string, "ankunft":date, "abreise":date, "zinr":string, "reslinnr":int, "zipreis":Decimal, "currency":string, "argt":string, "erwachs":int, "kind1":int, "gratis":int, "arrflag":bool, "resname":string, "lodging":Decimal})
    repeat_list_list, Repeat_list = create_model("Repeat_list", {"flag":int, "gastnr":int, "name":string, "nation":string, "birthdate":date, "email":string, "telefon":string, "vip":string, "city":string, "stay":int, "rmnite":int, "ankunft":date, "arrflag":bool, "zinr":string, "remark":string, "resname":string, "lodging":Decimal, "pax":int, "mobil_telefon":string})
    cur_date_list, Cur_date = create_model("Cur_date", {"curr_date":date})
    output_rlist_list, Output_rlist = create_model_like(Repeat_list)
    output_glist_list, Output_glist = create_model_like(G_list)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal g_list_list, repeat_list_list, cur_date_list, pvilanguage, from_date, to_date, ci_date, create_inhouse, sorttype, modetype, min_stay


        nonlocal input_list, g_list, repeat_list, cur_date, output_rlist, output_glist
        nonlocal g_list_list, repeat_list_list, cur_date_list, output_rlist_list, output_glist_list
    ...
    def disp_repeatlist():
        ...
        if sorttype == 0:
            ...
            for output_rlist in query(output_rlist_list, sort_by=[("stay",True),("name",False)]):
            ...
            if sorttype == 1:
                ...
                output_rlist_list.append(output_rlist)
    ...
    def disp_guesthistory():
        ...
        for output_glist in query(output_glist_list, filters=(lambda output_glist: output_glist.gastnr == sorttype), sort_by=[("ankunft",False)]):
            g_list = G_list()
    ...
    output_glist_list, output_rlist_list, cur_date_list = get_output(repeat_glist_1bl(pvilanguage, from_date, to_date, ci_date, create_inhouse))
