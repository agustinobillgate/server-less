...
def mast_memberbl(resno:int, billno:int):
    ...
    prepare_cache ([Bill, Res_line, Waehrung])
    ...
    def generate_output():
        nonlocal tot_adult, tot_room, b1_list_data, bill, res_line, waehrung
        nonlocal resno, billno
        nonlocal mbill


        nonlocal b_list, b1_list, mbill
        nonlocal b_list_data, b1_list_data

        return {"billno": billno, "tot_adult": tot_adult, "tot_room": tot_room, "b1-list": b1_list_data}
    ...
    for res_line in db_session.query(Res_line).filter(
             ((Res_line.resnr == resno) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.l_zuordnung[inc_value(2)] == 0)) | ((Res_line.l_zuordnung[inc_value(4)] == resno) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.active_flag <= 1) & (Res_line.resnr != resno))).order_by(Res_line.zinr, Res_line.name).all():

        b_list = query(b_list_data, filters=(lambda b_list: b_list.resnr == res_line.resnr and b_list.reslinnr == res_line.reslinnr), first=True)
