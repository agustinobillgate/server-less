...
def mk_resline_discounted_ratebl(res_mode:string, r_arrangement:string, r_betriebsnr:int, r_ankunft:date, r_zikatnr:int, r_erwachs:int, r_kind1:int, r_kind2:int, ci_date:date):
    ...
    t_katpreis_data, T_katpreis = create_model_like(Katpreis)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal exrate1, exrate2, t_katpreis_data, datum, wd_array, katpreis, arrangement, htparam, waehrung
        nonlocal res_mode, r_arrangement, r_betriebsnr, r_ankunft, r_zikatnr, r_erwachs, r_kind1, r_kind2, ci_date


        nonlocal t_katpreis
        nonlocal t_katpreis_data

        return {"exrate1": exrate1, "exrate2": exrate2, "t-katpreis": t_katpreis_data}
    ...
    if not katpreis:
        ...
        t_katpreis_data.append(t_katpreis)
