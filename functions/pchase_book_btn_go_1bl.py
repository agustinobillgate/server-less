...
def pchase_book_btn_go_1bl(sorttype:int, f_date:date, t_date:date, to_dt:date, mi_ch:string, mi_call:string, s_artnr:int):
    ...
    pchase_list_data, Pchase_list = create_model("Pchase_list", {"bestelldatum":date, "firma":string, "docu_nr":string, "traubensort":string, "lief_einheit":Decimal, "betriebsnr":int, "anzahl":Decimal, "einzelpreis":Decimal, "warenwert":Decimal, "remark":string, "artnr":int, "bezeich":string})
    ...
    def generate_output():
        nonlocal pchase_list_data, tmpart, l_artikel, l_pprice, l_lieferant, l_order
