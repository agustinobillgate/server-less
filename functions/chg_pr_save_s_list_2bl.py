...
tt_app_id_data, Tt_app_id = create_model("Tt_app_id", {"i_counter":int, "app_id":string})

def chg_pr_save_s_list_2bl(s_list_data:[S_list], tt_app_id_data:[Tt_app_id], rec_id:int, deptnr:int, comments_screen_value:string, rej_id:string, lieferdatum:date, rej_flag:bool, user_init:string):
    ...
    old_bestellart:string = ""
    new_bestellart:string = ""
    ...
    def generate_output():
        nonlocal sfdate1, sfdate2, sfdate3, stdate1, stdate2, stdate3, app_id, old_bestellart, new_bestellart, l_order, l_lieferant, l_orderhdr, queasy, bediener, res_history
        ...
        nonlocal t_l_order_data, t_l_lieferant_data

        return {}


    for tt_app_id in query(tt_app_id_data):
    ...
    res_history.aenderung = "Change Supplier name from: " + chr_unicode(10) + chr_unicode(10) +\
            to_string(old_bestellart) + "*** Changed to:" + chr_unicode(10) + chr_unicode(10) +\
