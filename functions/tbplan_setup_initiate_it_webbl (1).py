...
def tbplan_setup_initiate_it_webbl (1)(location:int, floor:int):
    t_queasy_list = []
    queasy = None
    ...
    for queasy in db_session.query(Queasy).filter(
             (Queasy.key == 31) & (Queasy.number1 == location) & (Queasy.betriebsnr == 0) & (Queasy.deci3 == floor)).order_by(Queasy.number2).all():
        t_queasy = T_queasy()
