...
def additional_custom_programbl(prog_name:string, prog_title:string, prog_description:string, prog_type:int, prog_flag:bool, case_type:int, prog_number:int, prog_hotel_code:string, prog_password:string):
    mess_result = ""
    program_list_data = []
    record_number:int = 0
    queasy = None
    ...
    program_list_data, Program_list = create_model("Program_list", {"prog_number":int, "prog_hotel_code":string, "prog_name":string, "prog_title":string, "prog_description":string, "prog_type":int, "prog_flag":bool, "prog_password":string, "prog_hashpassword":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal mess_result, program_list_data, record_number, queasy
        ...
        nonlocal program_list_data

        return {"mess_result": mess_result, "program-list": program_list_data}
    ...
    def load_data():
        ...
        for queasy in db_session.query(Queasy).filter(
                 (Queasy.key == 268) & (Queasy.char1 == (prog_hotel_code).lower())).order_by(Queasy._recid).all():
            ...
            program_list_data.append(program_list)
    ...
    if case_type == 1:
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 268)],"char1": [(eq, prog_hotel_code)],"char2": [(eq, prog_name)]})
        ...
        if queasy:
            ...
            for queasy in db_session.query(Queasy).filter(
                     (Queasy.key == 268) & (Queasy.char1 == (prog_hotel_code).lower())).order_by(Queasy.number1.desc()).yield_per(100):
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 268)],"char1": [(eq, prog_hotel_code)],"number1": [(eq, prog_number)]})
