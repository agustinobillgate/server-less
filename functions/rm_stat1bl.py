...
def rm_stat1bl(pvilanguage:int, segmcode:int, jan_date:date, to_date:date):
    ...
    t_list_data, T_list = create_model("T_list", {"flag":bool, "gastnr":int, "gname":string, "dlodge":Decimal, "mlodge":Decimal, "ylodge":Decimal, "dnite":int, "mnite":int, "ynite":int, "drate":Decimal, "mrate":Decimal, "yrate":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_list_data, lvcarea, genstat, guest
    ...
    def create_list():
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.gastnr == genstat.gastnr), first=True)
