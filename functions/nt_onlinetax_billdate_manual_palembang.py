...
from models import H_bill_line, Htparam, Billjournal, Artikel, Hoteldpt, H_artikel, Nitehist

def nt_onlinetax_billdate_manual_palembang(fr_date:date, to_date:date):
    avail_flag = False
    n:int = 1
    do_it:bool = False
    ...
    datum:date = None
    h_bill_line = htparam = billjournal = artikel = hoteldpt = h_artikel = nitehist = None
    ...
    vat_list_list, Vat_list = create_model("Vat_list", {"artnr":int, "rechnr":int, "zeit":int, "bill_datum":date, "service_amt":decimal, "departement":int})
    j_list_list, J_list = create_model("J_list", {"rechnr":int, "bill_datum":str, "cdtrx":str, "amount":decimal, "bezeich":str})
    t_list_list, T_list = create_model("T_list", {"departement":int, "rechnr":int, "bill_datum":date, "sysdate":date, "zeit":int, "fb":decimal, "other":decimal, "fb_service":decimal, "other_service":decimal, "pay":decimal, "compli":decimal, "artnr":int, "bezeich":str})
    ...
    def generate_output():
        nonlocal avail_flag, n, do_it, vat_artnr, revcode, serv_taxable, serv, vat, netto, serv_betrag, outstr, outstr1, rechnr_nottax, htl_name, qtystr, failed, lvi, lvcleft, lvctemp, nopd, logname, versioninfo, lvctmp, lvitmp, lnfeed, npause, check_duplicate, it_exist, request_sent, debug_mode, debug_code, counter, under_line, i_counter, progname, night_type, reihenfolge, line_nr, service_code, vat_proz, service_proz, datum, h_bill_line, htparam, billjournal, artikel, hoteldpt, h_artikel, nitehist
        ...
        return {"avail_flag": avail_flag}

    def datetime2char(datum:date, zeit:int):

        nonlocal avail_flag, n, do_it, vat_artnr, revcode, serv_taxable, serv, vat, netto, serv_betrag, outstr, outstr1, rechnr_nottax, htl_name, qtystr, failed, lvi, lvcleft, lvctemp, nopd, logname, versioninfo, lvctmp, lvitmp, lnfeed, npause, check_duplicate, it_exist, request_sent, debug_mode, debug_code, counter, under_line, i_counter, progname, night_type, reihenfolge, line_nr, service_code, vat_proz, service_proz, h_bill_line, htparam, billjournal, artikel, hoteldpt, h_artikel, nitehist
        nonlocal fr_date, to_date
        nonlocal hbill_buff
    ...
    def step_1(bill_date:date):
        ...
        for billjournal in db_session.query(Billjournal).filter(
            ...
            if do_it:
                ...
                if serv_betrag != 0:
                    ...
                    vat_list = query(vat_list_list, filters=(lambda vat_list: vat_list.artnr == billjournal.artnr and vat_list.rechnr == billjournal.rechnr and vat_list.bill_datum == billjournal.bill_datum), first=True)
        ...
        for fo_list in query(fo_list_list, filters=(lambda fo_list: fo_list.amount != 0)):
            ...
            outstr = chr(34) + to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") + to_string(get_day(bill_date) , "99") + to_string(i_counter, "999999") + chr(34) + "|" + chr(34) + to_string(fo_list.rechnr) + chr(34) + "|" + chr(34) + fo_list.cdtrx + chr(34) + "|" + chr(34) + datetime2char (fo_list.bill_datum, fo_list.zeit) + chr(34) + "|" + chr(34) + to_string(fo_list.artnr) + chr(34) + "|" + chr(34) + fo_list.bezeich + chr(34) + "|" + chr(34) + dec2char (fo_list.amount) + chr(34) + "|" + chr(34) + "1" + chr(34) + chr(2) + to_string(fo_list.departement)
            add_line(outstr, bill_date)

        for vat_list in query(vat_list_list, filters=(lambda vat_list: vat_list.service_amt != 0)):
            i_counter = i_counter + 1
            outstr = chr(34) + to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") + to_string(get_day(bill_date) , "99") + to_string(i_counter, "999999") + chr(34) + "|" + chr(34) + to_string(vat_list.rechnr) + chr(34) + "|" + chr(34) + "ATM1" + chr(34) + "|" + chr(34) + datetime2char (vat_list.bill_datum, vat_list.zeit) + chr(34) + "|" + chr(34) + to_string(vat_list.artnr) + chr(34) + "|" + chr(34) + "Service Charge" + chr(34) + "|" + chr(34) + dec2char (vat_list.service_amt) + chr(34) + "|" + chr(34) + "1" + chr(34) + chr(2) + to_string(vat_list.departement)
            add_line(outstr, bill_date)
        ...
        for hbill_list in query(hbill_list_list, filters=(lambda hbill_list: hbill_list.do_it and hbill_list.i_fact != 0)):
            ...
            for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).filter(
                ...
                t_list = query(t_list_list, filters=(lambda t_list: t_list.artnr == h_bill_line.artnr and t_list.rechnr == h_bill_line.rechnr and t_list.departement == h_bill_line.departement and t_list.bill_datum == h_bill_line.bill_datum), first=True)
                ...
                if not t_list:
                    ...
                    t_list.artnr = h_bill_line.artnr
        ...
        for t_list in query(t_list_list, filters=(lambda t_list: t_list.fb != 0 or t_list.other != 0)):
            ...
            if t_list.fb != 0:
                ...
                outstr = chr(34) + to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") +\
                        ...
                        chr(34) + to_string(t_list.artnr) + chr(34) + "|" +\
                        chr(34) + t_list.bezeich + chr(34) + "|" +\
            ...
            if t_list.fb_service != 0:
                ...
                outstr1 = chr(34) + to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") +\
                        ...
                        chr(34) + datetime2char (t_list.bill_datum, t_list.zeit) + chr(34) + "|" +\
                        chr(34) + to_string(t_list.artnr) + chr(34) + "|" +\
                        chr(34) + "Service Charge - " + t_list.bezeich + chr(34) + "|" +\
                        chr(34) + dec2char (t_list.fb_service) + chr(34) + "|" +\
                        chr(34) + "1" + chr(34) + chr(2) + to_string(t_list.departement)


                add_line(outstr1, bill_date)
    ...
    def remove_revenue(dept:int, billno:int, amount:decimal):
        ...
        for h_bill_line, hbuff in db_session.query(H_bill_line, Hbuff).join(Hbuff,(Hbuff.artnr == H_bill_line.artnr) & (Hbuff.departement == H_bill_line.departement) & (Hbuff.artart == 0)).filter(
            ...
            if htparam and htparam.fdecimal != 0:
                ...
                if serv_taxable:
                    vat =  to_decimal(vat) * to_decimal((1) + to_decimal(serv))
            netto =  to_decimal(p_sign) * to_decimal(h_bill_line.betrag) / to_decimal((1) + to_decimal(serv) + to_decimal(vat) )
            serv_betrag =  to_decimal(netto) * to_decimal(serv)
