...
def read_exrate_webbl(currencyno:int, from_date:date, to_date:date):
    ...
    waehrung_his_data, Waehrung_his = create_model("Waehrung_his", {"waehrungsnr":int, "wabkurz":string, "bezeich":string, "betrag":Decimal, "datum":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal avg_rate, waehrung_his_data, days, tot_betrag, waehrung, exrate
        nonlocal currencyno, from_date, to_date


        nonlocal waehrung_his
        nonlocal waehrung_his_data

        return {"avg_rate": avg_rate, "waehrung-his": waehrung_his_data}
    ...
    if waehrung:
        ...
        for exrate in db_session.query(Exrate).filter(
            ...
            waehrung_his = query(waehrung_his_data, filters=(lambda waehrung_his: waehrung_his.betrag == exrate.betrag), first=True)

            if not waehrung_his:
                waehrung_his = Waehrung_his()
                waehrung_his_data.append(waehrung_his)
