...
payload_list_data, Payload_list = create_model("Payload_list", {"frm_grp":int, "to_grp":int, "storage_no":int, "frm_date":date, "to_date":date, "case_type":int})

def incoming_print_summary_by_date_webbl(payload_list_data:[Payload_list]):
    ...
    print_list_data, Print_list = create_model("Print_list", {"datum":date, "t_amount":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal print_list_data, frm_grp, to_grp, storage_no, frm_date, to_date, tot_amount, temp_date, l_artikel, l_lieferant, l_op, l_untergrup, l_ophis


        nonlocal payload_list, print_list
        nonlocal print_list_data

        return {"print-list": print_list_data}
