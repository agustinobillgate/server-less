...
import re
from models import Guest, Artikel, Hoteldpt, Billjournal, Bill, H_bill, Bk_veran, Reservation

def neo_fo_journalbl(from_art:int, to_art:int, from_dept:int, to_dept:int, from_date:date, to_date:date, sorttype:int, exclude_artrans:bool, long_digit:bool, foreign_flag:bool, mi_onlyjournal:bool, mi_excljournal:bool, mi_post:bool):

    prepare_cache ([Guest, Artikel, Hoteldpt, Billjournal, Bill, H_bill, Bk_veran, Reservation])
    ...
    ind:int = 0
    gdelimiter:string = ""
    guest = artikel = hoteldpt = billjournal = bill = h_bill = bk_veran = reservation = None

    output_list = None

    output_list_data, Output_list = create_model("Output_list", {"bezeich":string, "c":string, "ns":string, "mb":string, "shift":string, "dept":string, "str":string, "remark":string, "gname":string, "descr":string, "voucher":string, "gst":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal gtot, output_list_data, curr_date, descr1, voucher_no, ind, gdelimiter, guest, artikel, hoteldpt, billjournal, bill, h_bill, bk_veran, reservation
    ...
    def journal_list():
        ...
        nonlocal gtot, output_list_data, descr1, voucher_no, ind, gdelimiter, guest, artikel, hoteldpt, billjournal, bill, h_bill, bk_veran, reservation
        nonlocal from_art, to_art, from_dept, to_dept, from_date, to_date, sorttype, exclude_artrans, long_digit, foreign_flag, mi_onlyjournal, mi_excljournal, mi_post
        ...
        sub_gst:Decimal = to_decimal("0.0")
        ggst:Decimal = to_decimal("0.0")
        curr_date:date = None
        last_dept:int = -1
        it_exist:bool = False
        lviresnr:int = -1
        ...
        for artikel in db_session.query(Artikel).filter(
                    ...
                    for billjournal in db_session.query(Billjournal).filter(
                        ...
                        if do_it:
                            ...
                            if (billjournal.bediener_nr == 0 and mi_onlyjournal == False) or (billjournal.bediener_nr != 0 and mi_excljournal == False):
                                ...
                                if billjournal.bill_datum > 08/31/18:

                                    if artikel.artart == 0 or artikel.artart == 1 or artikel.artart == 8 or artikel.artart == 9:

                                        if artikel.artart == 1:
                                            output_list.gst = ( to_decimal(billjournal.betrag) / to_decimal(1.06)) * to_decimal((6) / to_decimal("100") )
                                        ...
                                        sub_gst =  to_decimal(sub_gst) + to_decimal(output_list.gst)
                                        ggst =  to_decimal(ggst) + to_decimal(output_list.gst)
            ...
            if it_exist:
                ...
                output_list.gst =  to_decimal(sub_gst)
        ...
        gtot =  to_decimal(tot)
        output_list.gst =  to_decimal(ggst)
