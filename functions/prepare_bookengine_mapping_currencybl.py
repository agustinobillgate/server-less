...
def prepare_bookengine_mapping_currencybl(bookengid:int):
    ...
    t_mapping_currency_data = []
    waehrung = queasy = None

    t_mapping_currency = None

    t_mapping_currency_data, T_mapping_currency = create_model("T_mapping_currency", {"currencyvhp":string, "currencybe":string, "descr":string, "nr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_mapping_currency_data, waehrung, queasy
        nonlocal bookengid


        nonlocal t_mapping_currency
        nonlocal t_mapping_currency_data

        return {"t-mapping-currency": t_mapping_currency_data}

    for waehrung in db_session.query(Waehrung).order_by(Waehrung._recid).all():
        t_mapping_currency = T_mapping_currency()
        t_mapping_currency_data.append(t_mapping_currency)
