...
def if_country_add_costlistbl(zone1:string, grace1:int, wday1:int, ftime1:int, ttime1:int, tdura1:int, dura1:int, cost1:Decimal):
            ...
            if substring(parameters.vstring, n - 1, 1) == (";").lower() :
                ...
                if i == 1:
                    ...
                    for k in range(m,(n - 3)  + 1) :
                        ...
                        if substring(parameters.vstring, k - 1, 1) == (".").lower()  or substring(parameters.vstring, k - 1, 1) == (",").lower() :
                            s = s + ","
                        else:
                            s = s + substring(parameters.vstring, k - 1, 1)
                    cost_list.cost =  to_decimal(to_decimal(s))
