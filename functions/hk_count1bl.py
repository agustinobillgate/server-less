...
def hk_count1bl(aswaittime:int, price_decimal:int):
    ...
    prepare_cache ([Htparam, Zimmer, Res_line, Reservation, Segment, Waehrung])
    ...
    tot_arrive1 = 0
    tot_arrive2 = 0
    avrg_rate1 = to_decimal("0.0")
    avrg_rate2 = to_decimal("0.0")
    ...
    def generate_output():
        ...
        nonlocal aswaittime, price_decimal
    ...
    def init_var():
        ...
        res_line = Res_line()
        zimmer = Zimmer()
        for res_line.erwachs, res_line.gratis, res_line.kind1, res_line.kind2, res_line.l_zuordnung, res_line.abreise, res_line.zipreis, res_line.zinr, res_line.resstatus, res_line.zimmeranz, res_line.reserve_dec, res_line.betriebsnr, res_line.ankunft, res_line._recid, zimmer.sleeping, zimmer.zistatus, zimmer._recid in db_session.query(Res_line.erwachs, Res_line.gratis, Res_line.kind1, Res_line.kind2, Res_line.l_zuordnung, Res_line.abreise, Res_line.zipreis, Res_line.zinr, Res_line.resstatus, Res_line.zimmeranz, Res_line.reserve_dec, Res_line.betriebsnr, Res_line.ankunft, Res_line._recid, Zimmer.sleeping, Zimmer.zistatus, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Res_line.zinr)).filter(
        ...
        for res_line.erwachs, res_line.gratis, res_line.kind1, res_line.kind2, res_line.l_zuordnung, res_line.abreise, res_line.zipreis, res_line.zinr, res_line.resstatus, res_line.zimmeranz, res_line.reserve_dec, res_line.betriebsnr, res_line.ankunft, res_line._recid, zimmer.sleeping, zimmer.zistatus, zimmer._recid, reservation.segmentcode, reservation._recid in db_session.query(Res_line.erwachs, Res_line.gratis, Res_line.kind1, Res_line.kind2, Res_line.l_zuordnung, Res_line.abreise, Res_line.zipreis, Res_line.zinr, Res_line.resstatus, Res_line.zimmeranz, Res_line.reserve_dec, Res_line.betriebsnr, Res_line.ankunft, Res_line._recid, Zimmer.sleeping, Zimmer.zistatus, Zimmer._recid, Reservation.segmentcode, Reservation._recid).join(Zimmer,(Zimmer.zinr == Res_line.zinr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
                 (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
            if res_line_obj_list.get(res_line._recid):
                continue
            else:
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            if (res_line.resstatus == 1 or res_line.resstatus == 2 or res_line.resstatus == 5 or res_line.resstatus == 11):
                ...
                if rm_active:
                    ...
                    cal_local_rmrate()
        avrg_rate1 =  to_decimal(rm_rate) / to_decimal(occ_rm1)
        avrg_rate2 =  to_decimal(rm_rate) / to_decimal(occ_rm2)
        ...
        zimmer_obj_list = {}
        for zimmer, zimkateg in db_session.query(Zimmer, Zimkateg).join(Zimkateg,(Zimkateg.zikatnr == Zimmer.zikatnr)).order_by(Zimmer._recid).all():
    ...
    def cal_local_rmrate():
        ...
        rm_rate = to_decimal(rm_rate + round(res_line.zipreis * frate , price_decimal) * res_line.zimmeranz)
