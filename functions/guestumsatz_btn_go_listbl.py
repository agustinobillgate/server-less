...
def guestumsatz_btn_go_listbl(from_date:string, cardtype:int, hide_zero:bool, disptype:int):
    ...
    glodging_turnover_data = []
    gfb_turnover_data = []
    gother_turnover_data = []
    guest = guestat = guestbud = None

    glodging_turnover = gfb_turnover = gother_turnover = to_list = None

    glodging_turnover_data, Glodging_turnover = create_model("Glodging_turnover", {"cust_name":string, "t_o":Decimal, "budget_to":Decimal, "percnt_to":Decimal, "ytd":Decimal, "budget_ytd":Decimal, "percnt_ytd":Decimal, "lodg_to":Decimal, "budget_lodg_to":Decimal, "percnt_lodg_to":Decimal, "lodg_ytd":Decimal, "budget_lodg_ytd":Decimal, "percnt_lodg_ytd":Decimal, "rmnite":Decimal, "budget_rmnite":Decimal, "percnt_rmnite":Decimal, "rmnite_ytd":Decimal, "budget_rmnite_ytd":Decimal, "percnt_rmnite_ytd":Decimal})
    gfb_turnover_data, Gfb_turnover = create_model("Gfb_turnover", {"cust_name":string, "t_o":Decimal, "fb_to":Decimal, "budget_fb_to":Decimal, "percnt_fb_to":Decimal, "fb_ytd":Decimal, "budget_fb_ytd":Decimal, "percnt_fb_ytd":Decimal})
    gother_turnover_data, Gother_turnover = create_model("Gother_turnover", {"cust_name":string, "t_o":Decimal, "other_to":Decimal, "budget_other_to":Decimal, "percnt_other_to":Decimal, "other_ytd":Decimal, "budget_other_ytd":Decimal, "percnt_other_ytd":Decimal})
    to_list_data, To_list = create_model("To_list", {"gastnr":int, "name":string, "flag":[bool,12], "gesamt":Decimal, "b_gesamt":Decimal, "proz1":Decimal, "ygesamt":Decimal, "b_ygesamt":Decimal, "proz2":Decimal, "logis":Decimal, "b_logis":Decimal, "proz3":Decimal, "ylogis":Decimal, "b_ylogis":Decimal, "proz4":Decimal, "fb":Decimal, "b_fb":Decimal, "proz5":Decimal, "yfb":Decimal, "b_yfb":Decimal, "proz6":Decimal, "sonst":Decimal, "b_sonst":Decimal, "proz7":Decimal, "ysonst":Decimal, "b_ysonst":Decimal, "proz8":Decimal, "room":Decimal, "b_room":Decimal, "proz9":Decimal, "yroom":Decimal, "b_yroom":Decimal, "proz10":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal glodging_turnover_data, gfb_turnover_data, gother_turnover_data, guest, guestat, guestbud
        nonlocal from_date, cardtype, hide_zero, disptype


        nonlocal glodging_turnover, gfb_turnover, gother_turnover, to_list
        nonlocal glodging_turnover_data, gfb_turnover_data, gother_turnover_data, to_list_data

        return {"glodging-turnover": glodging_turnover_data, "gfb-turnover": gfb_turnover_data, "gother-turnover": gother_turnover_data}
    ...
    def create_umsatz():
        ...
        mm:int = 0
        yy:int = 0
        glodging_turnover_data.clear()
        gfb_turnover_data.clear()
        gother_turnover_data.clear()
        to_list_data.clear()
        ...
        for guestat.gastnr, guestat.monat, guestat.jahr, guestat.gesamtumsatz, guestat.argtumsatz, guestat.f_b_umsatz, guestat.sonst_umsatz, guestat.room_nights, guestat._recid, guest.gastnr, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest._recid in db_session.query(Guestat.gastnr, Guestat.monat, Guestat.jahr, Guestat.gesamtumsatz, Guestat.argtumsatz, Guestat.f_b_umsatz, Guestat.sonst_umsatz, Guestat.room_nights, Guestat._recid, Guest.gastnr, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest._recid).join(Guest,(Guest.gastnr == Guestat.gastnr) & (Guest.karteityp == cardtype)).filter(
            ...
            to_list = query(to_list_data, filters=(lambda to_list: to_list.gastnr == guest.gastnr), first=True)
        ...
        if hide_zero :
            ...
            for to_list in query(to_list_data, filters=(lambda to_list: to_list.gesamt != 0)):

                if disptype == 1:
                    glodging_turnover = Glodging_turnover()
                    glodging_turnover_data.append(glodging_turnover)
                    ...
                    gfb_turnover_data.append(gfb_turnover)
                    ...
                    gother_turnover_data.append(gother_turnover)
