#using conversion tools version: 1.0.0.117

from functions.additional_functions import *
from decimal import Decimal
from models import Briefzei, Brief, Htparam

def prepare_fo_wordvibl(briefnr:int):

    prepare_cache ([Htparam])

    t_briefzei_data = []
    htp_list_data = []
    briefzei = brief = htparam = None

    htp_list = t_briefzei = None

    htp_list_data, Htp_list = create_model("Htp_list", {"nr":int, "fchar":string, "bezeich":string})
    t_briefzei_data, T_briefzei = create_model_like(Briefzei)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_briefzei_data, htp_list_data, briefzei, brief, htparam
        nonlocal briefnr


        nonlocal htp_list, t_briefzei
        nonlocal htp_list_data, t_briefzei_data

        return {"t-briefzei": t_briefzei_data, "htp-list": htp_list_data}

    def create_list():

        nonlocal t_briefzei_data, htp_list_data, briefzei, brief, htparam
        nonlocal briefnr


        nonlocal htp_list, t_briefzei
        nonlocal htp_list_data, t_briefzei_data

        keychar:string = ""

        brief = get_cache (Brief, {"briefnr": [(eq, briefnr)]})

        htparam = get_cache (Htparam, {"paramnr": [(eq, 600)]})
        keychar = htparam.fchar

        for htparam in db_session.query(Htparam).filter(
                 (Htparam.paramgruppe == 8) & (Htparam.bezeichnung != ("Not Used").lower())).order_by(Htparam.fchar).all():
            htp_list = Htp_list()
            htp_list_data.append(htp_list)

            htp_list.nr = htparam.paramnr

            if substring(htparam.fchar, 0, 1) == (".").lower() :
                htp_list.fchar = htparam.fchar
            else:
                htp_list.fchar = keychar + htparam.fchar
            htp_list.bezeich = htparam.bezeichnung
        htp_list = Htp_list()
        htp_list_data.append(htp_list)

        htp_list.nr = 9092
        htp_list.fchar = keychar + "SourceRev"
        htp_list.bezeich = "Logding Revenue of selected Source"


        htp_list = Htp_list()
        htp_list_data.append(htp_list)

        htp_list.nr = 9813
        htp_list.fchar = keychar + "SourceRoom"
        htp_list.bezeich = "Rooms of selected Source"


        htp_list = Htp_list()
        htp_list_data.append(htp_list)

        htp_list.nr = 9814
        htp_list.fchar = keychar + "SourcePers"
        htp_list.bezeich = "Number of Pax of selected Source"


    def fill_words():

        nonlocal t_briefzei_data, htp_list_data, briefzei, brief, htparam
        nonlocal briefnr


        nonlocal htp_list, t_briefzei
        nonlocal htp_list_data, t_briefzei_data

        for briefzei in db_session.query(Briefzei).filter(
                 (Briefzei.briefnr == briefnr)).order_by(Briefzei.briefzeilnr).all():
            t_briefzei = T_briefzei()
            t_briefzei_data.append(t_briefzei)

            buffer_copy(briefzei, t_briefzei)


    create_list()
    fill_words()

    return generate_output()