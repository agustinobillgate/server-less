...
def sall_onhandhis_btn_go_webbl(anf_date:date, end_date:date, all_flag:bool, show_price:bool, zero_flag:bool, from_grp:int, sub_grp:int, from_lager:int, to_lager:int, sorttype:int, mattype:int):
    ...
    out_list_data, Out_list = create_model("Out_list", {"artnr":int, "bezeich":string, "unit":string, "qty":Decimal, "val":Decimal, "content":Decimal, "d_unit":string, "d_content":Decimal, "last_price":Decimal, "act_price":Decimal, "avrg_price":Decimal, "flag":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal done, out_list_data, long_digit, vk_preis, htparam, l_besthis, l_lager, l_artikel, l_untergrup
        ...
        return {"done": done, "out-list": out_list_data}
    ...
    def create_list():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            if tt_val == 0:
                ...
                out_list = query(out_list_data, filters=(lambda out_list: out_list.bezeich.lower()  == (s_bezeich).lower()), first=True)
