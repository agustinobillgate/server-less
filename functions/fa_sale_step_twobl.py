...
def fa_sale_step_twobl(nr:int, qty:int, amt:Decimal, user_init:string):
    ...
    buff_g_list_data, Buff_g_list = create_model("Buff_g_list", {"nr":int, "jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "duplicate":bool, "acct_fibukonto":string, "acct_bezeich":string}, {"sysdate": get_current_date(), "chgdate": None, "duplicate": True})
    ...
    def generate_output():
        nonlocal fa_wert, depn_wert, book_wert, curr_anz, debits, credits, remains, buff_g_list_data, s_list_data, profit, credit_betrag, debit_betrag, gl_acct, gl_jouhdr, fa_artikel, mathis, fa_grup, htparam
        ...
        nonlocal g_list_data, s_list_data, buff_g_list_data

        return {"fa_wert": fa_wert, "depn_wert": depn_wert, "book_wert": book_wert, "curr_anz": curr_anz, "debits": debits, "credits": credits, "remains": remains, "buff-g-list": buff_g_list_data, "s-list": s_list_data}
    ...
    if depn_wert != 0:
        ...
        if gl_acc1:
            ...
            if num_entries(fa_artikel.credit_fibu, ".") > 1:

                gl_acc1 = get_cache (Gl_acct, {"fibukonto": [(eq, entry(0, fa_artikel.credit_fibu, "."))]})
    ...
    for gl_acct1 in db_session.query(Gl_acct1).filter(
             ((Gl_acct1.fibukonto.in_(list(set([g_list.fibukonto for g_list in g_list_data])))))).order_by(Gl_acct1._recid).all():
