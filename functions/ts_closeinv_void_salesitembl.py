...
def ts_closeinv_void_salesitembl(t_b_list_data:[T_b_list], language_code:int, v_type:int, bl_recid:int, rec_id:int, bill_no:int, curr_dept:int, tischnr:int, pax:int, price_decimal:int, curr_waiter:int, kreditlimit:Decimal, exchg_rate:Decimal, gname:string, user_init:string, cancel_str:string, double_currency:bool, foreign_rate:bool, balance:Decimal):
    ...
    ordered_item_data, Ordered_item = create_model_like(H_bill_line, {"rec_id":int, "tax":Decimal, "service":Decimal})
    t_void_list_data, T_void_list = create_model_like(H_bill_line, {"rec_id":int})
    ...
    tmp_hartikel_data, Tmp_hartikel = create_model_like(H_artikel, {"rec_id":int})
    ...
    def generate_output():
        ...
        nonlocal ordered_item_data, t_void_list_data, t_h_artsales_data, b_list_data, tmp_hartikel_data, tp_bediener_data, submenu_list_data, t_h_bill_data, t_kellner1_data, summary_bill_data

        return {"balance": balance, "mess_result": mess_result, "v_success": v_success, "t-void-list": t_void_list_data, "summary-bill": summary_bill_data}

    def check_permission(user_init:string, array_nr:int, expected_nr:int):

        nonlocal mess_result, v_success, t_void_list_data, summary_bill_data, vcorrect, cancel_flag, answer, zugriff, fl_code, fl_code1, fl_code2, fl_code3, sales_art, rec_id_h_art, total_qty, hoga_resnr, hoga_reslinnr, qty, f_disc, b_disc, o_disc, rechnr, bcol, add_second, ci_date, bill_date, description, msg_str, printed, deptname, price, do_it, s, f_log, p_88, closed, mwst_sales, mwst_foreign_sales, balance_sales, balance_foreign_sales, amount_foreign, amount, netto_betrag, rec_id_artikel, service_code, h_bill_line, h_artikel, bediener, h_bill, kellner, htparam, hoteldpt, artikel
    ...
    def recalculate_summarybill():
        ...
        for t_b_list in query(t_b_list_data):
            ordered_item = Ordered_item()
            ordered_item_data.append(ordered_item)

            buffer_copy(t_b_list, ordered_item)

        for ordered_item in query(ordered_item_data, filters=(lambda ordered_item: ordered_item.rec_id == bl_recid)):
        ...
        for buff_hart in db_session.query(Buff_hart).filter(
                 ((Buff_hart.artnr.in_(list(set([ordered_item.artnr for ordered_item in ordered_item_data])))) & (Buff_hart.departement == ordered_item.departement))).order_by(Buff_hart._recid).all():
    ...
    t_b_list = query(t_b_list_data, first=True)
    ...
    for t_b_list in query(t_b_list_data, filters=(lambda t_b_list: t_b_list.rec_id == bl_recid)):
        b_list = B_list()
    ...
    if v_type == 1:
        ...
        t_h_artsales = query(t_h_artsales_data, filters=(lambda t_h_artsales: t_h_artsales.artnr == b_list.artnr and t_h_artsales.departement == b_list.departement and b_list.anzahl > 0), first=True)

        if t_h_artsales and balance != 0:

            t_b_list = query(t_b_list_data, filters=(lambda t_b_list: t_b_list.artnr == f_disc or t_b_list.artnr == b_disc or t_b_list.artnr == o_disc), first=True)
        ...
        fl_code, fl_code1, fl_code2, qty, answer, cancel_flag, sales_art, description, price, rec_id_h_art, anz, tmp_hartikel_data = get_output(ts_closeinv_cancel_orderbl(bl_recid))

        tmp_hartikel = query(tmp_hartikel_data, first=True)
        ...
        if qty != 0:
            ...
            if price != 0:
                ...
                if do_it:
                    ...
                    t_void_list_data.append(t_void_list)
