...
def prepare_eg_staffbl(user_init:string):
    ...
    queasy_19_data, Queasy_19 = create_model("Queasy_19", {"number1":int, "char3":string})
    dept_data, Dept = create_model("Dept", {"dept_nr":int, "dept_nm":string})
    userskill_data, Userskill = create_model("Userskill", {"categ_nr":int, "categ_nm":string, "categ_sel":bool})
    t_eg_staff_data, T_eg_staff = create_model_like(Eg_staff, {"rec_id":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal engid, groupid, dept_data, userskill_data, t_eg_staff_data, queasy_19_data, eg_staff, queasy, htparam, bediener
        nonlocal user_init


        nonlocal queasy_19, dept, userskill, t_eg_staff
        nonlocal queasy_19_data, dept_data, userskill_data, t_eg_staff_data

        return {"engid": engid, "groupid": groupid, "Dept": dept_data, "UserSkill": userskill_data, "t-eg-staff": t_eg_staff_data, "queasy-19": queasy_19_data}
    ...
    def create_skill():
        ...
        userskill_data.clear()

        for queasy in db_session.query(Queasy).filter(
                 (Queasy.key == 132)).order_by(Queasy.number1).all():
            userskill = Userskill()
            userskill_data.append(userskill)
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        queasy_19 = Queasy_19()
        queasy_19_data.append(queasy_19)
