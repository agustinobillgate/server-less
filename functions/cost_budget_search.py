...
def cost_budget_search(deptno:int, artno:int, from_date:date):
    ...
    cost_list_data, Cost_list = create_model("Cost_list", {"month_i":int, "year_i":int, "budget":Decimal, "actual":Decimal, "ytd_budget":Decimal, "ytd_actual":Decimal})
    alloc_list_data, Alloc_list = create_model("Alloc_list", {"fibu":string})
    ...
    def generate_output():
        nonlocal cost_list_data, to_date, curr_date, inv_close_date, parameters, gl_acct, l_artikel, htparam, costbudget, l_op, l_ophis
    ...
    def create_costlist():
        ...
        cost_list = query(cost_list_data, filters=(lambda cost_list: cost_list.month_i == get_month(curr_date) and cost_list.year_i == get_year(curr_date)), first=True)
        ...
        if get_month(curr_date) == get_month(inv_close_date) and get_year(curr_date) == get_year(inv_close_date):
            ...
            for l_op in db_session.query(L_op).filter(
                ...
                alloc_list = query(alloc_list_data, filters=(lambda alloc_list: alloc_list.fibu == l_op.stornogrund), first=True)
            ...
            for l_ophis in db_session.query(L_ophis).filter(
                ...
                alloc_list = query(alloc_list_data, filters=(lambda alloc_list: alloc_list.fibu == l_ophis.fibukonto), first=True)
    ...
    def calc_ytd():
        ...
        Cbuff = Cost_list
        cbuff_data = cost_list_data

        for cost_list in query(cost_list_data):

            for cbuff in query(cbuff_data, filters=(lambda cbuff:((cbuff.month_i <= cost_list.month_i) and (cbuff.year_i == cost_list.year_i)) or (cbuff.year_i < cost_list.year_i))):
