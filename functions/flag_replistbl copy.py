...
def flag_replistbl(s_resnr:int, s_reslinnr:int, s_ind:int, s_done:bool, s_recid:int):
    ...
    if not reslin_queasy or not(reslin_queasy.key.lower()  == ("flag").lower()  and reslin_queasy.resnr == s_resnr and reslin_queasy.reslinnr == s_reslinnr and reslin_queasy._recid == s_recid):
        reslin_queasy = db_session.query(Reslin_queasy).filter(
            (func.lower(Reslin_queasy.key) == ("flag").lower()) &  (Reslin_queasy.resnr == s_resnr) &  (Reslin_queasy.reslinnr == s_reslinnr) &  (Reslin_queasy._recid == s_recid)).first()
