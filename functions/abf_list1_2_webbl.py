...
from models import Res_line, Genstat, Arrangement, Artikel, Argt_line, Reslin_queasy, Fixleist, Guest, Reservation, Segment, Ratecode, Mc_guest, Mc_types, Mealcoup, Bill, Master

zikat_list_data, Zikat_list = create_model("Zikat_list", {"selected":bool, "zikatnr":int, "kurzbez":string, "bezeich":string})

def abf_list1_2_webbl(fdate:date, bfast_artnr:int, bfast_dept:int, zikat_list_data:[Zikat_list]):

    prepare_cache ([Res_line, Arrangement, Argt_line, Fixleist, Guest, Reservation, Segment, Ratecode, Mc_guest, Mc_types, Mealcoup])

    abf_list_data = []
    diffcidate:int = 0
    p_87:date = None
    num_of_day:int = 0
    res_line = genstat = arrangement = artikel = argt_line = reslin_queasy = fixleist = guest = reservation = segment = ratecode = mc_guest = mc_types = mealcoup = bill = master = None
    ...
    def generate_output():
        nonlocal abf_list_data, diffcidate, p_87, num_of_day, res_line, genstat, arrangement, artikel, argt_line, reslin_queasy, fixleist, guest, reservation, segment, ratecode, mc_guest, mc_types, mealcoup, bill, master
