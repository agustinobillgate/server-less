...
def salesboardbl(fdate:date, curr_task:int):
    ...
    month_list_data, Month_list = create_model("Month_list", {"gastnr":int, "name":string, "occ":int, "rmrev":Decimal, "fbrev":Decimal, "otherrev":Decimal})
    mtd_list_data, Mtd_list = create_model_like(Month_list)
    tguest_data, Tguest = create_model("Tguest", {"gastnr":int, "name":string, "adresse1":string, "adresse2":string, "wohnort":string, "plz":string, "land":string, "telefon":string, "segment1":string})
    reslist_data, Reslist = create_model("Reslist", {"gastnr":int, "name":string, "rnight":int, "rrev":Decimal, "datum":date})
    year_list_data, Year_list = create_model("Year_list", {"gastnr":int, "name":string, "rnight":int, "rrev":Decimal, "curr_month":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal month_list_data, tguest_data, mtd_list_data, year_list_data, fdate1, first_day, frdate, todate, date1, date2, startdate, enddate, ci_date, dummy_indv, dummy_walkin, bfast_art, service, vat, htparam, guest, guestseg, segment, genstat, reservation, res_line
        nonlocal fdate, curr_task


        nonlocal month_list, mtd_list, tguest, reslist, year_list
        nonlocal month_list_data, mtd_list_data, tguest_data, reslist_data, year_list_data

        return {"month-list": month_list_data, "tguest": tguest_data, "mtd-list": mtd_list_data, "year-list": year_list_data}

    def create_genstat(date1:date, date2:date):

        nonlocal month_list_data, tguest_data, mtd_list_data, year_list_data, fdate1, first_day, frdate, todate, startdate, enddate, ci_date, dummy_indv, dummy_walkin, bfast_art, service, vat, htparam, guest, guestseg, segment, genstat, reservation, res_line
        nonlocal fdate, curr_task
        ...
        for genstat.datum, genstat.gastnr, genstat.logis, genstat.res_deci, genstat.gratis, genstat._recid, guest.name, guest.anredefirma, guest.gastnr, guest.adresse1, guest.adresse2, guest.adresse3, guest.wohnort, guest.plz, guest.land, guest.telefon, guest._recid in db_session.query(Genstat.datum, Genstat.gastnr, Genstat.logis, Genstat.res_deci, Genstat.gratis, Genstat._recid, Guest.name, Guest.anredefirma, Guest.gastnr, Guest.adresse1, Guest.adresse2, Guest.adresse3, Guest.wohnort, Guest.plz, Guest.land, Guest.telefon, Guest._recid).join(Guest,(Guest.gastnr == Genstat.gastnr) & ((Guest.karteityp == 1) | (Guest.karteityp == 2))).filter(
            ...
            year_list = query(year_list_data, filters=(lambda year_list: year_list.curr_month == get_month(genstat.datum) and year_list.gastnr == genstat.gastnr), first=True)

            if not year_list:
                year_list = Year_list()
                year_list_data.append(year_list)
    ...
    def create_rline(date1:date, date2:date):
        ...
        for res_line, reservation, guest in db_session.query(Res_line, Reservation, Guest).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Res_line.gastnr) & ((Guest.karteityp == 1) | (Guest.karteityp == 2))).filter(
            ...
            for datum in date_range(dat1,dat2) :
                ...
                year_list = query(year_list_data, filters=(lambda year_list: year_list.curr_month == get_month(datum) and year_list.gastnr == res_line.gastnr), first=True)
    ...
    for genstat.datum, genstat.gastnr, genstat.logis, genstat.res_deci, genstat.gratis, genstat._recid, guest.name, guest.anredefirma, guest.gastnr, guest.adresse1, guest.adresse2, guest.adresse3, guest.wohnort, guest.plz, guest.land, guest.telefon, guest._recid in db_session.query(Genstat.datum, Genstat.gastnr, Genstat.logis, Genstat.res_deci, Genstat.gratis, Genstat._recid, Guest.name, Guest.anredefirma, Guest.gastnr, Guest.adresse1, Guest.adresse2, Guest.adresse3, Guest.wohnort, Guest.plz, Guest.land, Guest.telefon, Guest._recid).join(Guest,(Guest.gastnr == Genstat.gastnr) & ((Guest.karteityp == 1) | (Guest.karteityp == 2))).filter(
        ...
        mtd_list = query(mtd_list_data, filters=(lambda mtd_list: mtd_list.gastnr == genstat.gastnr), first=True)

        if not mtd_list:
            mtd_list = Mtd_list()
            mtd_list_data.append(mtd_list)
