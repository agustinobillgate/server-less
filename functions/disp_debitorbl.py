...
def disp_debitorbl():
    ...
    dlist_data, Dlist = create_model("Dlist", {"gastnr":int, "gname":string, "city":string, "gtype":int, "aging":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal dlist_data, debitor, artikel, guest


        nonlocal dlist
        nonlocal dlist_data

        return {"dlist": dlist_data}
    ...
    def create_list():
        ...
        for debitor, artikel in db_session.query(Debitor, Artikel).join(Artikel,(Artikel.artnr == Debitor.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter(
            ...
            dlist = query(dlist_data, filters=(lambda dlist: dlist.gastnr == debitor.gastnr), first=True)

            if not dlist:
                dlist = Dlist()
                dlist_data.append(dlist)
