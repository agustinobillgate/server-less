...
def prepare_ts_restinv_close_shiftbl(pvilanguage:int, curr_dept:int, curr_waiter:int):
    ...
    def generate_output():
        nonlocal billdate, msg_str, shift_list_data, lvcarea, h_bill, h_bill_line, htparam
        nonlocal pvilanguage, curr_dept, curr_waiter
        nonlocal hbill, hbline


        nonlocal shift_list, hbill, hbline
        nonlocal shift_list_data

        return {"billdate": billdate, "msg_str": msg_str, "shift-list": shift_list_data}
