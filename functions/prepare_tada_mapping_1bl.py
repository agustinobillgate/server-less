...
def prepare_tada_mapping_1bl(dept:int):
    ...
    t_dept = t_queasy270 = vhp_art = t_article = crd_list = None

    t_dept_data, T_dept = create_model("T_dept", {"nr":int, "dept":string})
    t_queasy270_data, T_queasy270 = create_model_like(Queasy)
    vhp_art_data, Vhp_art = create_model("Vhp_art", {"art_dept":int, "art_nr":int, "art_name":string})
    t_article_data, T_article = create_model("T_article", {"vhpdept":int, "vhpartnr":int, "vhpbezeich":string, "vhpflag":bool, "tadadept":int, "tadaartnr":int, "tadabezeich":string, "tadaflag":bool, "tadasku":string})
    crd_list_data, Crd_list = create_model("Crd_list", {"deptno":int, "uname":string, "pass":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_dept_data, t_queasy270_data, vhp_art_data, t_article_data, crd_list_data, queasy, hoteldpt, h_artikel
        nonlocal dept


        nonlocal t_dept, t_queasy270, vhp_art, t_article, crd_list
        nonlocal t_dept_data, t_queasy270_data, vhp_art_data, t_article_data, crd_list_data

        return {"t-dept": t_dept_data, "t-queasy270": t_queasy270_data, "vhp-art": vhp_art_data, "t-article": t_article_data, "crd-list": crd_list_data}
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        t_queasy270_data.append(t_queasy270)
