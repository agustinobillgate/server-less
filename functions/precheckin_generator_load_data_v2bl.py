...
def precheckin_generator_load_data_v2bl(resno:int, reslinno:int):
    ...
    arrive_list_data, Arrive_list = create_model("Arrive_list", {"resnr":int, "reslinnr":int, "rsv_name":string, "zinr":string, "guest_name":string, "guest_email":string, "phone_no":string, "arrival":date, "departure":date, "hotelcode":string, "mail_eng":string, "mail_oth":string, "hotel_name":string, "hotel_telp":string, "hotel_mail":string, "link_pci_eng":string, "link_pci_oth":string})
    ...
    def generate_output():
        nonlocal arrive_list_data, en_hotelencrip, oth_hotelencrip, cpersonalkey, rkey, mmemptrout, precheckinurl, hotelcode, hotelcode_ok, guest, res_line, reservation, queasy
        nonlocal resno, reslinno
        nonlocal gmember


        nonlocal arrive_list, gmember
        nonlocal arrive_list_data

        return {"arrive-list": arrive_list_data}

    def encryptedtext(input_string:string, task_mode:string):
        ...
        if task_mode.lower()  == ("E").lower() :
            OUTPUT STREAM s1 TO VALUE (filebat)
            OUTPUT STREAM s1 CLOSE

        elif task_mode.lower()  == ("D").lower() :
            OUTPUT STREAM s1 TO VALUE (filebat)
            OUTPUT STREAM s1 CLOSE
        OS_COMMAND SILENT VALUE ("sudo bash " + filebat)
        foundit = False
        loopint = 5
        while True:

            if SEARCH ("/usr1/vhp/etc/output.txt") != None:
                foundit = True
                loopint = 99
            loopint = loopint + 1
        ...
        if foundit:
            filestr = FILE "/usr1/vhp/etc/output.txt"
            mess_result = to_string(filestr)
        else:
            mess_result = "process failed"

        return generate_inner_output()

    arrive_list_data.clear()
    ...
    if res_line:
        ...
        arrive_list_data.append(arrive_list)
        ...
        if not hotelcode_ok:
            ...
            en_hotelencrip = chr_unicode(34) + "ENG|" + hotelcode + "|" + to_string(arrive_list.arrival) + "|" + to_string(arrive_list.resnr) + chr_unicode(34)
            oth_hotelencrip = chr_unicode(34) + "IDN|" + hotelcode + "|" + to_string(arrive_list.arrival) + "|" + to_string(arrive_list.resnr) + chr_unicode(34)
            en_hotelencrip = encryptedtext(en_hotelencrip, "E")
            ...
            oth_hotelencrip = encryptedtext(oth_hotelencrip, "E")
