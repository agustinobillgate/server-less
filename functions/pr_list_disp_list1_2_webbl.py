...
def pr_list_disp_list1_2_webbl(char1:string, billdate:date, from_date:date, to_date:date, outstand_flag:bool, expired_flag:bool, approve_flag:bool, reject_flag:bool, artnumber:int, deptno:int, sorttype:int):
    ...
    def disp_list(pr_nr:string):
        ...
        if pr_nr != "":
            ...
            if l_orderhdr:
                ...
                l_order = get_cache (L_order, {"docu_nr": [(eq, l_orderhdr.docu_nr)],"loeschflag": [(le, 1)],"pos": [(gt, 0)],"lief_nr": [(eq, 0)]})
                while None != l_order:
        ...
        for l_orderhdr in db_session.query(L_orderhdr).filter(
            ...
            l_order = get_cache (L_order, {"docu_nr": [(eq, l_orderhdr.docu_nr)],"pos": [(eq, 0)],"lief_nr": [(eq, 0)],"artnr": [(eq, artnumber)]})
            do_it = True
    ...
    for s_list in query(s_list_data, filters=(lambda s_list: s_list.pos == 0)):
        ...
        p_list = query(p_list_data, filters=(lambda p_list: p_list.pos > 0 and p_list.docu_nr == s_list.docu_nr), first=True)
