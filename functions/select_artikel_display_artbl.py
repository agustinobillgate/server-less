...
def select_artikel_display_artbl(departement:int, sub_group:int):
    ...
    q2_artikel_data, Q2_artikel = create_model("Q2_artikel", {"artnr":int, "bezeich":string, "epreis":Decimal, "anzahl":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal q2_artikel_data, artikel
        nonlocal departement, sub_group


        nonlocal q2_artikel
        nonlocal q2_artikel_data

        return {"q2-artikel": q2_artikel_data}
    ...
    for artikel in db_session.query(Artikel).filter(
        ...
        q2_artikel_data.append(q2_artikel)
