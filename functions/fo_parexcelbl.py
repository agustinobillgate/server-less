...
def fo_parexcelbl(pvilanguage:int, ytd_flag:bool, jan1:date, ljan1:date, lfrom_date:date, lto_date:date, from_date:date, to_date:date, start_date:date, lytd_flag:bool, lmtd_flag:bool, pmtd_flag:bool, pfrom_date:date, pto_date:date, lytoday_flag:bool, lytoday:date, foreign_flag:bool, budget_flag:bool, w1_data:[W1], w2_data:[W2]):
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, ytd_flag, jan1, ljan1, lfrom_date, lto_date, from_date, to_date, start_date, lytd_flag, lmtd_flag, pmtd_flag, pfrom_date, pto_date, lytoday_flag, lytoday, foreign_flag, budget_flag, w1_data, w2_data
        nonlocal segmbuff
        ...
        return {"error_nr": error_nr, "msg_str": msg_str, "w1": w1_data, "w2": w2_data}
    ...
    def fill_cover(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 552 and w11.dept == w1.dept), first=True)
    ...
    def fill_rmocc(rec_w1:int):
        ...
        w22 = query(w22_data, filters=(lambda w22: w22.main_code == 183), first=True)
        ...
        for segment in db_session.query(Segment).order_by(Segment._recid).all():
            ...
            w11 = query(w11_data, filters=(lambda w11: w11.main_code == 92 and w11.artnr == segment.segmentcode), first=True)

            if w11 and w11.done:
                pass

            w12 = query(w12_data, filters=(lambda w12: w12.main_code == 813 and w12.artnr == segment.segmentcode), first=True)

            if w12 and w12.done:
                pass

            w13 = query(w13_data, filters=(lambda w13: w13.main_code == 814 and w13.artnr == segment.segmentcode), first=True)
    ...
    def fill_segment(rec_w1:int, main_nr:int):
        ...
        if main_nr == 92:
            ...
            w12 = query(w12_data, filters=(lambda w12: w12.main_code == 813 and w12.artnr == segm and not w12.done), first=True)

            w13 = query(w13_data, filters=(lambda w13: w13.main_code == 814 and w13.artnr == segm and not w13.done), first=True)

            w756 = query(w756_data, filters=(lambda w756: w756.main_code == 756 and w756.artnr == segm and not w756.done), first=True)

            w757 = query(w757_data, filters=(lambda w757: w757.main_code == 757 and w757.artnr == segm and not w757.done), first=True)

            w758 = query(w758_data, filters=(lambda w758: w758.main_code == 758 and w758.artnr == segm and not w758.done), first=True)
            ...
            w11 = query(w11_data, filters=(lambda w11: w11.main_code == 92 and w11.artnr == segm and not w11.done), first=True)
    ...
    def fill_zinrstat(rec_w1:int, main_nr:int):
        ...
        if main_nr == 800:
            ...
            w12 = query(w12_data, filters=(lambda w12: w12.main_code == 180 and w12.artnr == rmno), first=True)

            w13 = query(w13_data, filters=(lambda w13: w13.main_code == 181 and w13.artnr == rmno), first=True)
            ...
            w11 = query(w11_data, filters=(lambda w11: w11.main_code == 800 and w11.artnr == rmno), first=True)
