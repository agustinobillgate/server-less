...
def abf_listbl(fdate:date, bfast_artnr:int, bfast_dept:int):
    ...
    prepare_cache ([Arrangement, Argt_line, Fixleist, Guest, Reservation, Zimkateg])
    ...
    abf_list_data, Abf_list = create_model("Abf_list", {"zinr":string, "name":string, "segmentcode":int, "ankunft":date, "anztage":int, "abreise":date, "kurzbez":string, "arrangement":string, "zimmeranz":int, "erwachs":int, "kind1":int, "gratis":int, "resnr":int, "bemerk":string, "gastnr":int, "resstatus":int, "resname":string, "address":string, "city":string, "comments":string, "datum":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal abf_list_data, res_line, arrangement, artikel, argt_line, reslin_queasy, fixleist, guest, reservation, zimkateg, bill, master
    ...
    def disp_arlist():
        ...
        for res_line in db_session.query(Res_line).filter(
                 ((Res_line.resstatus != 3) & (Res_line.resstatus != 4) & (Res_line.resstatus != 8) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12)) & (Res_line.active_flag <= 1) & (Res_line.ankunft < fdate) & (Res_line.abreise >= fdate) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line.zinr).all():
            do_it = False
            roflag = True
            qty = 0
