...
def exportgcf_3bl(fdate:date, tdate:date):
    ...
    def generate_output():
        nonlocal tguest_data, end_date, tmp_date, guest, genstat, nation, res_line, reservation, sourccod, history
    ...
    def create_genstat(date1:date, date2:date):
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            if guest:
                ...
                tguest = query(tguest_data, filters=(lambda tguest: tguest.gastnr == guest.gastnr), first=True)
    ...
    for tguest in query(tguest_data):
