...
def vhp_prepare_transactionbl(ci_date:date):
    ...
    flist_data, Flist = create_model("Flist", {"dept":int, "rechnr":int, "saldo":string, "discount":string, "created":string, "checkin":string, "checkout":string, "pax":int, "usr":string, "id":int, "reslinnr":string, "service":int, "resnr":int, "breslin":int, "gastnr":int, "gastpay":int})
    fline_list_data, Fline_list = create_model("Fline_list", {"rechnr":int, "dept":int, "bezeich":string, "price":string, "rtcode":string, "rmtype":string, "resnr":int, "reslinnr":string, "breslin":int, "gastnr":int, "gastpay":int})
    ...
    def generate_output():
        nonlocal allocated_point, avail_data, record__count, flist_data, fline_list_data, reihenfolge, progname, nitehist, nightaudit, htparam
        nonlocal ci_date
        nonlocal buffnite


        nonlocal flist, fline_list, buffnite
        nonlocal flist_data, fline_list_data

        return {"allocated_point": allocated_point, "avail_data": avail_data, "record__count": record__count, "flist": flist_data, "fline-list": fline_list_data}
    ...
    for nitehist in db_session.query(Nitehist).filter(
        ...
        if allocated_point == 2 or allocated_point == 3:
            ...
            for buffnite in db_session.query(Buffnite).filter(
                ...
                fline_list_data.append(fline_list)
