...
import math,re
from sqlalchemy import Column, Integer, String, Float, Boolean, Date, DateTime, ForeignKey, ARRAY, Text
...
def create_model(model_name: str, create_fields: Dict[str, any] = None, default_values: Optional[Dict[str, any]] = None, like=None):
    from models.base import Base
    if default_values is None:
        default_values = {}

    # attributes = {}
    attributes = {'__tablename__': model_name.lower()}  # Add __tablename__ attribute
    attributes['__table_args__'] = {'extend_existing': True}

    # If 'like' parameter is provided, copy its columns
    if like:
        for column in like.__table__.columns:
            attributes[column.name] = column.copy()

    # Dynamically create columns based on the fields dictionary
    if create_fields:
        for field_name, field_type in create_fields.items():
            column_type = None

            # Check if field_type is a list for array type
            if isinstance(field_type, list) and len(field_type) == 2 and isinstance(field_type[1], int):
                element_type, length = field_type
                if element_type == int:
                    column_type = ARRAY(Integer, dimensions=1)
                    default_value = [0] * length
                elif element_type == float:
                    column_type = ARRAY(Float, dimensions=1)
                    default_value = [0.0] * length
                elif element_type == str:
                    column_type = ARRAY(Text, dimensions=1)
                    default_value = [''] * length
                elif element_type == bool:
                    column_type = ARRAY(Boolean, dimensions=1)
                    default_value = [False] * length
                elif element_type == datetime.date:
                    column_type = ARRAY(Date, dimensions=1)
                    default_value = [None] * length
                elif element_type == datetime.datetime:
                    column_type = ARRAY(DateTime, dimensions=1)
                    default_value = [None] * length
            ...
            if column_type:
                attributes[field_name] = Column(column_type, default=default_value)

    # Create a new class (model)
    model = type(model_name, (Base,), attributes)
    return [], model

def convert_to_dict_list(model):
    """
    Convert a SQLAlchemy query result to a list of dictionaries.

    :param query_result: The result of a SQLAlchemy query.
    :return: A list of dictionaries, where each dictionary represents a row from the query result.
    """

    query_result = local_storage.db_session.query(model).all()
    dict_list = []
    for row in query_result:
        row_dict = {column.key: getattr(row, column.key) for column in row.__table__.columns}
...
def query(data: List[Type], 
    ...
    if (first or last) and not sort_by:
        ...
        data_gen = (item for item in data if filters(item))
        if last:
