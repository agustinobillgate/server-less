...
def slow_moving_btn_go_3bl(storeno:int, main_grp:int, tage:int, show_price:bool, disptype:string):
    ...
    s_list_data, S_list = create_model("S_list", {"artnr":int, "name":string, "min_oh":Decimal, "curr_oh":Decimal, "avrgprice":Decimal, "ek_aktuell":Decimal, "datum":date, "content":Decimal, "zwkum":int, "endkum":int, "unit":string, "datum2":date})
    ...
    def generate_output():
        nonlocal str_flag, s_list_data, l_ophis, l_op, l_verbrauch, htparam, l_artikel, l_bestand
        ...
        return {"str_flag": str_flag, "s-list": s_list_data}
