...
def prepare_ts_hbline_1bl(user_init:string, dept:int, curr_rechnr:int):
    ...
    grp_list_data, Grp_list = create_model("Grp_list", {"pos":int, "dept":int, "zknr":int, "bezeich":string, "grp_bgcol":int, "grp_fgcol":int}, {"grp_bgcol": 1, "grp_fgcol": 15})
    ...
    def generate_output():
        nonlocal zero_flag, billdate, balance, max_gpos, cashless_license, cashless_minsaldo, menu_list_data, grp_list_data, curr_zwkum, fgcol_array, kellner, htparam, bediener, h_artikel, h_bill_line, wgrpdep
        nonlocal user_init, dept, curr_rechnr
        nonlocal kbuff


        nonlocal menu_list, grp_list, kbuff
        nonlocal menu_list_data, grp_list_data

        return {"zero_flag": zero_flag, "billdate": billdate, "balance": balance, "max_gpos": max_gpos, "cashless_license": cashless_license, "cashless_minsaldo": cashless_minsaldo, "menu-list": menu_list_data, "grp-list": grp_list_data}
