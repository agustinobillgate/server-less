...
def gcjour_listbl(case_type:int, from_refno:string, sorttype:int, journaltype:int, jtype1:int, from_date:date, to_date:date):
    ...
    gl_jouhdr_list_data, Gl_jouhdr_list = create_model("Gl_jouhdr_list", {"datum":date, "refno":string, "bezeich":string, "debit":Decimal, "credit":Decimal, "remain":Decimal, "jnr":int, "activeflag":int})
    b2_list_data, B2_list = create_model("B2_list", {"fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal gl_jouhdr_list_data, b2_list_data, yy, gl_jouhdr, gl_journal, gl_acct
