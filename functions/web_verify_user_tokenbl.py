...
def web_verify_user_tokenbl(user_init:str, license_nr:str, user_token:str):
    new_user_token = ""
    i_result = 0
    errmess = ""
    headerstring:str = ""
    ...
    if curr_i < max_counter:
        ...
        delta_time = get_current_time_in_seconds() + (get_current_date() - last_date) * 86400 - last_time
    ...
    if i_result == 1:
        ...
        if tot_counter >= 4:
            ...
            gqbuff = db_session.query(Gqbuff).filter(
                     (func.lower(Gqbuff.key) == ("userToken").lower()) & (func.lower(Gqbuff.char1) == (user_init).lower()) & (Gqbuff.number3 <= 1)).first()
            while None != gqbuff:

                gqbuff1 = db_session.query(Gqbuff1).filter(
                             (Gqbuff1._recid == gqbuff._recid)).first()
                gqbuff1_list.remove(gqbuff1)
                pass


                curr_recid = gqbuff._recid
                gqbuff = db_session.query(Gqbuff).filter(
                         (func.lower(Gqbuff.key) == ("userToken").lower()) & (func.lower(Gqbuff.char1) == (user_init).lower()) & (Gqbuff.number3 <= 1) & (Gqbuff._recid > curr_recid)).first()

            guest_queasy = db_session.query(Guest_queasy).filter(
                     (func.lower(Guest_queasy.key) == ("userToken").lower()) & (func.lower(Guest_queasy.char1) == (user_init).lower())).first()
            ...
            if guest_queasy:
                ...
                while not gqbuff:
                    ...
                    for guest_queasy in db_session.query(Guest_queasy).filter(
                             (func.lower(Guest_queasy.key) == ("userToken").lower()) & (func.lower(Guest_queasy.char1) == (user_init).lower())).order_by(Guest_queasy._recid).all():
