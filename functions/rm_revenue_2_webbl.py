...
def rm_revenue_2_webbl(m_ftd:bool, m_ytd:bool, f_date:date, t_date:date, to_date:date, rm_no:string, sorttype:int, lod__rev:bool, excl_compl:bool):

    prepare_cache ([Htparam, Zimmer, Reservation, Arrangement, Res_line, Waehrung, Argt_line, Zinrstat, Genstat, Zimkateg])
    ...
    output_list_data, Output_list = create_model("Output_list", {"rmno":string, "flag":string, "str":string, "rmno1":string, "rmtype":string, "rm":int, "pax":int, "rm_rev":Decimal, "percent":Decimal, "mtdrm":int, "pax1":int, "rm_rev1":Decimal, "percent1":Decimal, "ftdrm":int, "pax2":int, "rm_rev2":Decimal, "percent3":Decimal})
    ...
    def generate_output():
        nonlocal output_list_data, i, anz, manz, yanz, pax, mpax, ypax, mnet, ynet, net, t_anz, t_manz, t_yanz, t_pax, t_mpax, t_ypax, t_net, t_mnet, t_ynet, from_bez, to_bez, price_decimal, from_date, curr_zeit, ci_date, do_it, htparam, zimmer, reservation, arrangement, res_line, waehrung, segment, argt_line, artikel, zinrstat, genstat, zimkateg
    ...
    def create_resline():
        ...
        nonlocal output_list_data, i, anz, manz, yanz, pax, mpax, ypax, mnet, ynet, net, t_anz, t_manz, t_yanz, t_pax, t_mpax, t_ypax, t_net, t_mnet, t_ynet, from_bez, to_bez, from_date, curr_zeit, ci_date, do_it, htparam, zimmer, reservation, arrangement, res_line, waehrung, segment, argt_line, artikel, zinrstat, genstat, zimkateg
        nonlocal m_ftd, m_ytd, f_date, t_date, to_date, rm_no, sorttype, lod__rev, excl_compl
