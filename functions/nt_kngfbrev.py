...
def nt_kngfbrev():
    ...
    map_list_list, Map_list = create_model("Map_list", {"deptnr":int, "food":[int,4], "bev":[int,4]})
    ...
    def generate_output():
        ...
        nonlocal s_list_list, map_list_list
    ...
    def get_anzahl():
        ...
        hb_buff_obj_list = []
        for hb_buff, h_art1, artikel in db_session.query(Hb_buff, H_art1, Artikel).join(H_art1,(H_art1.artnr == Hb_buff.artnr) & (H_art1.departement == Hb_buff.departement) & (H_art1.artart == 0)).join(Artikel,(Artikel.artnr == H_art1.artnrfront) & (Artikel.departement == H_art1.departement)).filter(
            ...
            if hb_buff._recid in hb_buff_obj_list:
                continue
            else:
                hb_buff_obj_list.append(hb_buff._recid)
    ...
    for i in range(1,num_entries(str_par, sep2) - 1 + 1) :
        ...
        map_list = Map_list()
        map_list_list.append(map_list)

        map_list.deptnr = to_int(substring(entry(0, c1, sep1) , 1, len(entry(0, c1, sep1)) - 1))
    ...
    for map_list in query(map_list_list):

        hoteldpt = db_session.query(Hoteldpt).filter(
                 (Hoteldpt.num == map_list.deptnr)).first()
    ...
    h_bill_obj_list = []
    for h_bill, h_bill_line in db_session.query(H_bill, H_bill_line).join(H_bill_line,(H_bill_line.rechnr == H_bill.rechnr) & (H_bill_line.bill_datum == ci_date) & (H_bill_line.departement == H_bill.departement)).filter(
        ...
        hb_buff_obj_list = []
        for hb_buff, hart_buff in db_session.query(Hb_buff, Hart_buff).join(Hart_buff,(Hart_buff.artnr == Hb_buff.artnr) & (Hart_buff.departement == Hb_buff.departement) & ((Hart_buff.artart == 11) | (Hart_buff.artart == 12))).filter(
        ...
        if do_it:
            ...
            hb_buff_obj_list = []
            for hb_buff, hart_buff in db_session.query(Hb_buff, Hart_buff).join(Hart_buff,(Hart_buff.artnr == Hb_buff.artnr) & (Hart_buff.departement == Hb_buff.departement) & (Hart_buff.artart == 0)).filter(
                     (Hb_buff.rechnr == h_bill.rechnr) & (Hb_buff.departement == h_bill.departement) & (Hb_buff.bill_datum == ci_date)).order_by(Hb_buff._recid).all():
                if hb_buff._recid in hb_buff_obj_list:
                    continue
                else:
                ...
                if shift == 0:
                    ...
                    s_list = query(s_list_list, filters=(lambda s_list: s_list.hb_buff.zeit >= s_list.ftime and hb_buff.zeit <= s_list.ttime), first=True)
                ...
                map_list = query(map_list_list, filters=(lambda map_list: map_list.deptnr == h_bill.departement), first=True)
                ...
                if shift_art != 0:
                    ...
                    umsatz = db_session.query(Umsatz).filter(
                             (Umsatz.artnr == hart_buff.artnrfront) & (Umsatz.departement == hart_buff.departement) & (Umsatz.datum == hb_buff.bill_datum)).first()
                    ...
                    if umsatz:
                        ...
                        ubuff = db_session.query(Ubuff).filter(
                                 (Ubuff.artnr == shift_art) & (Ubuff.departement == hart_buff.departement) & (Ubuff.datum == hb_buff.bill_datum)).first()
