...
def stock_hohlist1_webbl(from_lager:int, to_lager:int, from_art:int, to_art:int, to_date:date, move_art:bool, end_notzero:bool, global_oh:bool):
    ...
    stockoh_list = art_bestand = t_vkpreis = None
    ...
    def generate_output():
        nonlocal stockoh_list_data, do_it, curr_art, curr_lager, out_bestand, zwkum, t_val, bezeich, j, tot_prev, tot_value, tot_in, tot_out, total_in, total_out, total_prev, total_value, from_date, long_digit, htparam, l_artikel, l_besthis, l_lager, l_ophis, l_untergrup
        nonlocal from_lager, to_lager, from_art, to_art, to_date, move_art, end_notzero, global_oh


        nonlocal stockoh_list, art_bestand, t_vkpreis
        nonlocal stockoh_list_data, art_bestand_data, t_vkpreis_data
    ...
    def create_stock1():
        ...
        l_besthis = L_besthis()
        l_artikel = L_artikel()
        for l_besthis.lager_nr, l_besthis.artnr, l_besthis.anz_anf_best, l_besthis.anz_eingang, l_besthis.anz_ausgang, l_besthis.val_anf_best, l_besthis.wert_eingang, l_besthis.wert_ausgang, l_besthis._recid, l_artikel.zwkum, l_artikel.bezeich, l_artikel.vk_preis, l_artikel._recid in db_session.query(L_besthis.lager_nr, L_besthis.artnr, L_besthis.anz_anf_best, L_besthis.anz_eingang, L_besthis.anz_ausgang, L_besthis.val_anf_best, L_besthis.wert_eingang, L_besthis.wert_ausgang, L_besthis._recid, L_artikel.zwkum, L_artikel.bezeich, L_artikel.vk_preis, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_besthis.artnr)).filter(
                 (L_besthis.lager_nr == 0) & (L_besthis.anf_best_dat == from_date) & (L_besthis.artnr >= from_art) & (L_besthis.artnr <= to_art)).order_by(L_artikel.zwkum, L_artikel.artnr).all():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            for l_besthis in db_session.query(L_besthis).filter(
                     (L_besthis.lager_nr == l_lager.lager_nr) & (L_besthis.anf_best_dat == from_date) & (L_besthis.artnr >= from_art) & (L_besthis.artnr <= to_art)).order_by(t_vkpreis.zwkum, t_vkpreis.artnr).all():
                t_vkpreis = query(t_vkpreis_data, (lambda t_vkpreis: t_vkpreis.artnr == l_besthis.artnr), first=True)
                if not t_vkpreis:
                    continue
                ...
                if do_it:
                    ...
                    art_bestand = query(art_bestand_data, filters=(lambda art_bestand: art_bestand.artnr == l_besthis.artnr and art_bestand.lager == l_lager.lager_nr), first=True)
                    ...
                    for l_ophis in db_session.query(L_ophis).filter(
                             (L_ophis.lager_nr == l_lager.lager_nr) & (L_ophis.artnr == l_besthis.artnr) & ((L_ophis.datum >= from_date) & (L_ophis.datum <= to_date)) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.datum).all():

                        if l_ophis.op_art == 1 or l_ophis.op_art == 2:
                            art_bestand.incoming =  to_decimal(art_bestand.incoming) + to_decimal(l_ophis.anzahl)
                            art_bestand.in_val =  to_decimal(art_bestand.in_val) + to_decimal(l_ophis.anzahl) * to_decimal(t_vkpreis.vk_preis)
                            ...
                            art_bestand.out_val =  to_decimal(art_bestand.out_val) + to_decimal(l_ophis.anzahl) * to_decimal(t_vkpreis.vk_preis)
                            art_bestand.actqty =  to_decimal(art_bestand.actqty) - to_decimal(l_ophis.anzahl)
                    art_bestand.actval =  to_decimal(art_bestand.actqty) * to_decimal(t_vkpreis.vk_preis)
                ...
                if (move_art and end_notzero) and (art_bestand.incoming != 0 or art_bestand.outgoing != 0 or art_bestand.actqty != 0):
                    ...
                    if art_bestand.avrg_pr == 0:
                        ...
                        if art_bestand.actqty != 0:
                            ...
                            art_bestand.avrg_pr =  to_decimal(t_vkpreis.vk_preis)
    ...
    def create_stock2():
        ...
        l_besthis_obj_list = {}
        for l_besthis in db_session.query(L_besthis).filter(
                 (L_besthis.anf_best_dat == from_date) & (L_besthis.lager_nr == 0) & (L_besthis.artnr >= from_art) & (L_besthis.artnr <= to_art)).order_by(t_vkpreis.zwkum, t_vkpreis.artnr).all():
            t_vkpreis = query(t_vkpreis_data, (lambda t_vkpreis: t_vkpreis.artnr == l_besthis.artnr), first=True)
            if not t_vkpreis:
                continue
        ...
        for art_bestand in query(art_bestand_data, sort_by=[("zwkum",False),("artnr",False)]):
            t_vkpreis = query(t_vkpreis_data, (lambda t_vkpreis: t_vkpreis.artnr == art_bestand.artnr), first=True)
            if not t_vkpreis:
                continue


            for l_ophis in db_session.query(L_ophis).filter(
                     (L_ophis.artnr == art_bestand.artnr) & ((L_ophis.datum >= from_date) & (L_ophis.datum <= to_date)) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.datum).all():
