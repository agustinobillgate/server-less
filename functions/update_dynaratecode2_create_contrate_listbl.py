...
def update_dynaratecode2_create_contrate_listbl(currcode:string, rmtype:string):
    ...
    t_list_data, T_list = create_model("T_list", {"mesvalue":string})
    ratecode_list_data, Ratecode_list = create_model("Ratecode_list", {"rcode":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal iftask, tokcounter, mestoken, mesvalue, rcode, ratecode_list_data, t_list_data, doit, ratecode, queasy
        nonlocal currcode, rmtype


        nonlocal t_list, ratecode_list
        nonlocal t_list_data, ratecode_list_data

        return {"ratecode-list": ratecode_list_data, "t-list": t_list_data}
    ...
    for ratecode in db_session.query(Ratecode).filter(
        ...
        if rcode.lower()  == (rmtype).lower() :
            ...
            if doit:
                ...
                ratecode_list = query(ratecode_list_data, filters=(lambda ratecode_list: ratecode_list.rcode == mesvalue), first=True)
