...
def print_pchase_lnl_webbl(pvilanguage:int, lnldelimeter:string, docunr:string, stattype:int, curr_status:string):
    ...
    str3_list_data, Str3_list = create_model("Str3_list", {"str":string})
    esign_print_data, Esign_print = create_model("Esign_print", {"sign_nr":int, "sign_name":string, "sign_img":bytes, "sign_date":string, "sign_position":string})
    str1_data, Str1 = create_model("Str1", {"bill_recv":string, "address1":string, "address2":string, "cp_name":string, "telp":string, "fax_no":string, "bill_no":string, "bill_date":date, "refer":string, "po_source":string, "po_number":string, "dep_date":int, "arr_date":date, "created_by":string, "delivery_date":date, "remark":string, "globaldisc":Decimal, "bank_name":string, "account":string, "rekening":string, "companytitle":string, "vat_code":string, "afterdisc":Decimal, "htl_name":string, "htl_adr":string, "htl_tel":string})
    str3_data, Str3 = create_model("Str3", {"bl_descript":string, "arr_date":string, "bl_qty":string, "d_unit":string, "bl_price":string, "bl_amount":string, "c_exrate":string, "bl_balance":string, "remark":string, "konto":string, "disc":Decimal, "disc2":Decimal, "vat":Decimal, "disc_value":Decimal, "disc2_value":Decimal, "epreis0":Decimal, "bl_vat":string, "artnr":int, "brutto":Decimal, "po_nr":string, "po_source":string, "vat1":Decimal, "vat2":Decimal, "add_vat":Decimal, "bl_amount_add_vat":string})
    op_list_data, Op_list = create_model("Op_list", {"artnr":int, "anzahl":Decimal, "bezeich":string, "bez_aend":bool, "disc":Decimal, "disc2":Decimal, "vat":Decimal, "epreis":Decimal, "epreis0":Decimal, "warenwert":Decimal, "konto":string, "warenwert0":Decimal, "remark":string, "disc_value":Decimal, "disc2_value":Decimal, "brutto":Decimal, "vat_value":Decimal, "po_nr":string, "vat_code":string, "vat1":Decimal, "vat2":Decimal, "add_vat":Decimal, "warenwert_add_vat":Decimal})
    ...
    def generate_output():
        ...
        return {"str3-list": str3_list_data, "esign-print": esign_print_data, "str1": str1_data, "str3": str3_data}
            ...
            for l_order in db_session.query(L_order).filter(
                ...
                if l_art:
                    ...
                    if l_art.jahrgang == 0 or length(l_order.stornogrund) <= 12:
                        ...
                        op_list = query(op_list_data, filters=(lambda op_list: op_list.artnr == l_order.artnr and op_list.epreis == l_order.einzelpreis and op_list.bezeich == l_art.bezeich and op_list.disc == disc and op_list.disc2 == disc2 and op_list.konto == l_order.stornogrund and op_list.remark == l_order.besteller), first=True)
                    else:
                    ...
                    if not op_list or create_it:
                        ...
                        if queasy:
                            op_list.add_vat = ( to_decimal(1.0) + to_decimal((queasy.deci1) / to_decimal(100)))
                        else:
    ...
    if tmp_liefnr != None:
        ...
        if l_lieferant:
            ...
            str1.account = substring(l_lieferant.bank, i + 4 - 1, length(l_lieferant.bank))
    ...
    for op_list in query(op_list_data):
        ...
        str3.add_vat = ( to_decimal((op_list.add_vat) - to_decimal(1)) * to_decimal("100") )
