...
def close_rsv_list_journal_list_webbl(from_date:date, to_date:date):
    ...
    output_list_data, Output_list = create_model("Output_list", {"datum":date, "engager":string, "contact_person":string, "ba_event":string, "venue":string, "ba_time":string, "pax":int, "room_rev":Decimal, "fb_rev":Decimal, "other_rev":Decimal, "total_rev":Decimal, "salesid":string, "cmid":string, "resnr":int, "resline":int, "tablesetup":string, "remark":string, "ba_status":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal troomrev, tfbrev, tothrev, ttrev, output_list_data, total_rev, b_history, bk_rart, bk_stat, bk_veran, bediener, guest
