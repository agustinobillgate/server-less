...
def phone_listbl(case_type:int, name1:string, dept1:string, phone_nr:string, pn:string, mobil_nr:string, lvcoldmobilnr:string):
    ...
    t_phone_list_data, T_phone_list = create_model("T_phone_list", {"dept":string, "name":string, "telephone":string, "ext":string, "mobil_telefon":string, "fax":string, "adresse1":string, "wohnort":string, "prefix":string, "land":string, "vorname":string, "telex":string, "adresse2":string, "rec_id":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_phone_list_data, telephone
        nonlocal case_type, name1, dept1, phone_nr, pn, mobil_nr, lvcoldmobilnr


        nonlocal t_phone_list
        nonlocal t_phone_list_data

        return {"t-phone-list": t_phone_list_data}
    ...
    def assign_it():
        ...
        t_phone_list_data.append(t_phone_list)
