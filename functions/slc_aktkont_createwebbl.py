...
def slc_aktkont_createwebbl(gastnr:int, gname:string):
    ...
    t_akt_kont1_data, T_akt_kont1 = create_model_like(Akt_kont)
    t_akt_kont_data, T_akt_kont = create_model_like(Akt_kont)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal kontnr, t_akt_kont1_data, curr_gastnr, success_flag, akt_kont
        nonlocal gastnr, gname


        nonlocal t_akt_kont1, t_akt_kont
        nonlocal t_akt_kont1_data, t_akt_kont_data

        return {"kontnr": kontnr, "t-akt-kont1": t_akt_kont1_data}

    t_akt_kont_data = get_output(read_akt_kontbl(6, gastnr, None, ""))

    t_akt_kont = query(t_akt_kont_data, last=True)
    ...
    t_akt_kont1_data.append(t_akt_kont1)
