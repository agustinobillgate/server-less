...
def odoo_journal_historybl(from_date:date, to_date:date):
    ...
    history_list_data, History_list = create_model("History_list", {"datum":date, "refno":string, "bezeich":string, "timestamp":int})
    ...
    def generate_output():
        nonlocal history_list_data, queasy, gl_jouhdr
        nonlocal from_date, to_date
        nonlocal bqueasy


        nonlocal history_list, bqueasy
        nonlocal history_list_data

        return {"history-list": history_list_data}


    history_list_data.clear()
