...
def auto_room_maidlistbl(loc_combo:string, stat1:string, stat2:string, stat3:string, stat4:string, fr_floor:int, to_floor:int, pax:int):
    ...
    temp_loc_data, Temp_loc = create_model("Temp_loc", {"t_loc":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal rmlist_data, credit, resbemerk, count_i
        nonlocal loc_combo, stat1, stat2, stat3, stat4, fr_floor, to_floor, pax


        nonlocal rmlist, temp_loc
        nonlocal rmlist_data, temp_loc_data

        return {"rmlist": rmlist_data}
    ...
    def create_rmlist():
        ...
        credit, rmlist_data = get_output(hk_rmboy_rmlist_1bl(loc_combo, stat1, stat2, stat3, stat4, fr_floor, to_floor))
    ...
    def create_browse():
        ...
        for rmlist in query(rmlist_data, filters=(lambda rmlist: rmlist.zinr != ''), sort_by=[("floor",False),("zinr",False)]):
