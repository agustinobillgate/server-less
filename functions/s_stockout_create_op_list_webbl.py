...
def s_stockout_create_op_list_webbl(s_artnr:int, curr_lager:int, lscheinnr:string, transdate:date, qty:Decimal, cost_acct:string, a_bez:string, transfered:bool, tp_bediener_nr:int):
    ...
    temp_l_artikel_data, Temp_l_artikel = create_model("Temp_l_artikel", {"artnr":int, "bezeich":string, "betriebsnr":int, "endkum":int, "masseinheit":string, "vk_preis":Decimal, "inhalt":Decimal, "lief_einheit":Decimal, "traubensort":string})
    op_list_data, Op_list = create_model_like(L_op, {"fibu":string, "a_bezeich":string, "a_lief_einheit":Decimal, "a_traubensort":string})
    op_list1_data, Op_list1 = create_model_like(L_op, {"a_lief_einheit":Decimal, "a_traubensort":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal msg_str, op_list_data, amount, t_amount, price, description, l_bestand, h_rezlin, l_op, l_artikel
        ...
        nonlocal t_l_bestand_data, t_h_rezlin_data, temp_l_artikel_data, op_list_data, op_list1_data

        return {"msg_str": msg_str, "op-list": op_list_data}
    ...
    def create_op_list():
        ...
        if l_artikel.betriebsnr > 0:
            op_list1_data.clear()
            t_l_bestand_data, t_h_rezlin_data = get_output(s_stockout_h_rezlinbl(l_artikel.betriebsnr, curr_lager))
    ...
    def create_op_list1(p_artnr:int, menge:Decimal, oh_ok:bool):
        ...
        l_art_data = temp_l_artikel_data
        temp_l_artikel_data.clear()
        temp_l_artikel_data = get_output(s_stockout_create_op_list1bl(p_artnr))
        ...
        for t_h_rezlin in query(t_h_rezlin_data):
            ...
            if t_h_rezlin.recipe_flag :
                ...
                l_art = query(l_art_data, filters=(lambda l_art: l_art.artnr == t_h_rezlin.artnrlager), first=True)

                t_l_bestand = query(t_l_bestand_data, filters=(lambda t_l_bestand: t_l_bestand.lager_nr == curr_lager and t_l_bestand.artnr == t_h_rezlin.artnrlager), first=True)
