...
def ts_restinv_btn_ccard2bl(billart:int, rec_id:int, curr_dept:int, balance:Decimal, paid:Decimal, balance_foreign:Decimal, exchg_rate:Decimal, full_paid:bool, transdate:date, disc_art1:int, disc_art2:int, disc_art3:int, kellner_kellner_nr:int):
    ...
    def generate_output():
        nonlocal qty, price, description, amount_foreign, amount, bill_date, t_h_artikel_data, t_h_bill_data, h_artikel, h_bill, tisch, htparam, h_umsatz, artikel, h_bill_line, queasy
        nonlocal billart, rec_id, curr_dept, balance, paid, balance_foreign, exchg_rate, full_paid, transdate, disc_art1, disc_art2, disc_art3, kellner_kellner_nr


        nonlocal t_h_artikel, t_h_bill
        nonlocal t_h_artikel_data, t_h_bill_data

        return {"billart": billart, "qty": qty, "price": price, "description": description, "amount_foreign": amount_foreign, "amount": amount, "bill_date": bill_date, "t-h-artikel": t_h_artikel_data, "t-h-bill": t_h_bill_data}
