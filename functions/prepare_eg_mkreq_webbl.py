...
def prepare_eg_mkreq_webbl(pvilanguage:int, user_init:string):
    ...
    def generate_output():
        nonlocal engid, groupid, status_str, open_str, flag, dept_str, ci_date, request1_data, t_eg_location_data, t_queasy_data, t_zimmer_data, lvcarea, guestname, eg_request, eg_location, queasy, zimmer, res_line, guest, htparam, bediener, counters
        nonlocal pvilanguage, user_init
        nonlocal resline1, guest1


        nonlocal request1, t_eg_location, t_queasy, t_zimmer, resline1, guest1
        nonlocal request1_data, t_eg_location_data, t_queasy_data, t_zimmer_data

        return {"engid": engid, "groupid": groupid, "status_str": status_str, "open_str": open_str, "flag": flag, "dept_str": dept_str, "ci_date": ci_date, "request1": request1_data, "t-eg-location": t_eg_location_data, "t-queasy": t_queasy_data, "t-zimmer": t_zimmer_data}
