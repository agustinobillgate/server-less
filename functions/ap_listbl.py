...
def ap_listbl(from_date:date, to_date:date, lastname:string, sorttype:int, price_decimal:int):

    prepare_cache ([L_kredit, L_lieferant])
    ...
    output_list_data, Output_list = create_model("Output_list", {"betriebsnr":int, "ap_recid":int, "curr_pay":Decimal, "lscheinnr":string, "str":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list_data, t_ap, t_pay, t_bal, tot_ap, tot_pay, tot_bal, i, l_kredit, l_lieferant
    ...
    def disp_it():
        ...
        if lastname == "":
            ...
            for l_kredit.counter, l_kredit.betriebsnr, l_kredit._recid, l_kredit.rgdatum, l_kredit.name, l_kredit.lscheinnr, l_kredit.netto, l_kredit.ziel, l_kredit.saldo, l_lieferant.segment1, l_lieferant.firma, l_lieferant.lief_nr, l_lieferant._recid in db_session.query(L_kredit.counter, L_kredit.betriebsnr, L_kredit._recid, L_kredit.rgdatum, L_kredit.name, L_kredit.lscheinnr, L_kredit.netto, L_kredit.ziel, L_kredit.saldo, L_lieferant.segment1, L_lieferant.firma, L_lieferant.lief_nr, L_lieferant._recid).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr)).filter(
