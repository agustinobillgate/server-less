...
def eg_repmaintain_all_categorybl(all_room:bool, tcategory_data:[Tcategory], tlocation_data:[Tlocation], troom_data:[Troom]):

    prepare_cache ([Queasy, Eg_property, Eg_location])

    tmaintask_data = []
    tproperty_data = []
    queasy = eg_property = eg_location = None

    tproperty = troom = tlocation = tmaintask = tcategory = comcategory = comlocat = commain = comroom = None
    ...
    def generate_output():
        nonlocal tmaintask_data, tproperty_data, queasy, eg_property, eg_location
        nonlocal all_room


        nonlocal tproperty, troom, tlocation, tmaintask, tcategory, comcategory, comlocat, commain, comroom
        nonlocal tproperty_data, tmaintask_data

        return {"tMaintask": tmaintask_data, "tproperty": tproperty_data}
