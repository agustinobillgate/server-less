...
def eg_maincalendar_btn_gobl(user_init:string, all_room:bool):
    ...
    def generate_output():
        nonlocal groupid, engid, troom_data, tproperty_data, bediener, htparam, queasy, eg_staff, eg_location, zimmer, eg_property
        ...
        nonlocal tmaintask_data, tpic_data, tstatus_data, dept_link_data, tproperty_data, troom_data, tlocation_data, tcategory_data

        return {"groupid": groupid, "engid": engid, "troom": troom_data, "tproperty": tproperty_data}
    ...
    def create_property():
        ...
        qbuff_obj_list = {}
        for qbuff in db_session.query(Qbuff).filter(
                 ((Qbuff.location.in_(list(set([comlocat.loc_nr for comlocat in comlocat_data if comlocat.loc_selected])))))).order_by(Qbuff._recid).all():
            if qbuff_obj_list.get(qbuff._recid):
                continue
            else:
                qbuff_obj_list[qbuff._recid] = True

            comlocat = query(comlocat_data, (lambda comlocat: (qbuff.location == comlocat.loc_nr)), first=True)

            if comlocat.loc_guest :

                commain = query(commain_data, filters=(lambda commain: commain.main_nr == qbuff.maintask and commain.main_selected), first=True)

                if commain:

                    comroom = query(comroom_data, filters=(lambda comroom: comroom.room_nm == qbuff.zinr and comroom.room_selected), first=True)
