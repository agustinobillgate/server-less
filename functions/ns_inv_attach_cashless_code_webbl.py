...
def ns_inv_attach_cashless_code_webbl(sorttype:int, gastname:string, dept:int, rechnr:int):
    resname = ""
    address = ""
    city = ""
    comments = ""
    ...
    fr_name:string = ""
    to_name:string = ""
    stat:List[string] = ["Fix", "Tentative", "", "", "", "", "", "Closed", "Cancelled"]
    ...
    def generate_output():
        nonlocal resname, address, city, comments, b1_list_data, ba_dept, fr_name, to_name, stat, reservation, bk_reser, bk_veran
        nonlocal sorttype, gastname, dept, rechnr


        nonlocal b1_list, t_reservation, t_bk_reser, t_bk_veran
        nonlocal b1_list_data, t_reservation_data, t_bk_reser_data, t_bk_veran_data

        return {"resname": resname, "address": address, "city": city, "comments": comments, "b1-list": b1_list_data}

    ba_dept = get_output(htpint(900))
    b1_list_data = get_output(ns_inv_attach_cashless_codebl(0, sorttype, gastname, dept, ba_dept, rechnr))
    ...
    if b1_list:
        ...
        if dept == ba_dept and b1_list.rechnr != 0:
            t_bk_veran_data = get_output(read_bk_veranbl(2, None, None, b1_list.rechnr, None))

            t_bk_veran = query(t_bk_veran_data, first=True)

            if t_bk_veran:
                comments = "RefNo: " + to_string(t_bk_veran.veran_nr) + chr_unicode(10)
                t_bk_reser_data = get_output(read_bk_reserbl(5, t_bk_veran.veran_nr, None, 9, None))

                for t_bk_reser in query(t_bk_reser_data):
                    comments = comments + to_string(t_bk_reser.veran_resnr) + ": " + stat[t_bk_reser.resstatus - 1] + " " + to_string(t_bk_reser.datum) + chr_unicode(10) + t_bk_reser.raum + " " + to_string(t_bk_reser.von_zeit, "99:99") + " - " + to_string(t_bk_reser.bis_zeit, "99:99") + chr_unicode(10)
        comments = comments + b1_list.bemerk
        t_reservation_data = get_output(read_reservationbl(1, b1_list.resnr, None, ""))

        t_reservation = query(t_reservation_data, first=True)

        if t_reservation and t_reservation.bemerk != "":
            comments = comments + chr_unicode(10) + t_reservation.bemerk
