...
def web_interface_print_fa_pchasebl(pvilanguage:int, docunr:str, lnldelimeter:str):
    str1 = ""
    str2 = ""
    tmp_tbl_data_list = []
    lvcarea:str = "print-fa-pchase-lnl"
    ...
    def str_header():
        ...
        if fa_ordheader:
            ...
            parameters = db_session.query(Parameters).filter(
                     (func.lower(Parameters.progname) == ("CostCenter").lower()) & (func.lower(Parameters.section) == ("Name").lower()) & (to_int(Parameters.varname) == fa_ordheader.dept_nr)).first()
    ...
    def str_data():
        ...
        for fa_order in db_session.query(Fa_order).filter(
                 (func.lower(Fa_order.order_nr) == (docunr).lower()) & (Fa_order.activeflag == 0)).order_by(Fa_order.fa_pos).all():
            create_it = False
            bez_aend = False
