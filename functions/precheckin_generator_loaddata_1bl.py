...
def precheckin_generator_loaddata_1bl(show_rate:bool, last_sort:int, lresnr:int, long_stay:int, ci_date:date, grpflag:bool, room:string, lname:string, sorttype:int, fdate1:date, fdate2:date, fdate:date, segm1_list_data:[Segm1_list], argt_list_data:[Argt_list], zikat_list_data:[Zikat_list]):
    ...
    def generate_output():
        ...
        nonlocal show_rate, last_sort, lresnr, long_stay, ci_date, grpflag, room, lname, sorttype, fdate1, fdate2, fdate, segm1_list_data
        ...
        return {"fdate1": fdate1, "fdate2": fdate2, "rmlen": rmlen, "mail_eng": mail_eng, "mail_oth": mail_oth, "hotel_name": hotel_name, "hotel_telp": hotel_telp, "hotel_mail": hotel_mail, "arl-list": arl_list_data}

    def get_toname(lname:string):

        nonlocal rmlen, mail_eng, mail_oth, hotel_name, hotel_telp, hotel_mail, arl_list_data, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, stay, en_hotelencrip, oth_hotelencrip, cpersonalkey, rkey, mmemptrout, precheckinurl, hotelcode, hotelcode_ok, guest, paramtext, htparam, res_line, queasy, waehrung, reservation, zimkateg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer
        nonlocal show_rate, last_sort, lresnr, long_stay, ci_date, grpflag, room, sorttype, fdate1, fdate2, fdate, segm1_list_data
        nonlocal gbuff, gmember
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                    ...
                    argt_list = query(argt_list_data, (lambda argt_list: argt_list.argt == res_line.arrangement and argt_list.selected), first=True)
                    if not argt_list:
                        continue
    ...
    if not hotelcode_ok:
        ...
        for arl_list in query(arl_list_data):
