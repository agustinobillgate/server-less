...
from models import Htparam, Bediener, Hoteldpt, Queasy, H_artikel, H_bill_line, H_bill, Artikel

def selforder_prepare_sendbillbl(user_init:str, dept:int, table_no:int, session_parameter:str):
    mess_result = ""
    user_name = ""
    dept_name = ""
    guest_name = ""
    ...
    billno:int = 0
    do_it:bool = False
    htparam = bediener = hoteldpt = queasy = h_artikel = h_bill_line = h_bill = artikel = None

    ordered_item = merge_item = bqsy = bposted = qrqsy = queasyorderbill = orderbill = orderbill_line = orderbill_posted = bufforder = searchbill = hbuff = abuff = None
    ...
    merge_item_list, Merge_item = create_model_like(Ordered_item)
    ...
    def query_order():
        ...
        if searchbill:
            ...
            for orderbill in db_session.query(Orderbill).filter(
                    (Orderbill.key == 225) &  (Orderbill.number1 == dept) &  (func.lower(Orderbill.char1) == "orderbill") &  (func.lower(Orderbill.char3) == (session_parameter).lower()) &  (Orderbill.logi1 == False)).all():

                if orderbill.deci1 != 0:
    ...
    for ordered_item in query(ordered_item_list):
        ...
        if re.match(".*wait for.*",ordered_item.order_status):
            ordered_item_list.remove(ordered_item)

        elif re.match(".*ORDER CANCELED.*",ordered_item.order_status):
            ordered_item_list.remove(ordered_item)
        else:

            if ordered_item.article_type.lower()  == "VOID":
                ordered_item.subtotal = ordered_item.price

            merge_item = query(merge_item_list, filters=(lambda merge_item :merge_item.artnr == ordered_item.artnr), first=True)

            if not merge_item:
                merge_item = Merge_item()
                merge_item_list.append(merge_item)

                buffer_copy(ordered_item, merge_item)
            else:
                merge_item.qty = merge_item.qty + ordered_item.qty


                merge_item.subtotal = merge_item.subtotal + ordered_item.subtotal
    ordered_item._list.clear()

    for merge_item in query(merge_item_list):
        ordered_item = Ordered_item()
        ordered_item_list.append(ordered_item)

        buffer_copy(merge_item, ordered_item)
    merge_item._list.clear()
    ...
    for ordered_item in query(ordered_item_list, filters=(lambda ordered_item :ordered_item.count_amount)):
        ...
        if (re.match(".*PAYMENT.*",ordered_item.article_type) or re.match(".*VOUCHER.*",ordered_item.article_type)):
            ordered_item.tax = 0
            ordered_item.service = 0
