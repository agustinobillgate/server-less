...
def eg_received_budget_fromgl_webbl(resources_char1:string, tbudget_data:[Tbudget], sbudget_data:[Sbudget]):
    ...
    def generate_output():
        ...
        return {"tbudget": tbudget_data, "sbudget": sbudget_data, "msg_str": msg_str}
    ...
    if resources_char1 != "":
        ...
        if not buf_gl_acct:
            ...
            for i in range(1,12 + 1) :
                ...
                tbudget = query(tbudget_data, filters=(lambda tbudget: tbudget.month == i), first=True)
                tbudget.amount =  to_decimal(buf_gl_acct.budget[i - 1])
                pass

                sbudget = query(sbudget_data, filters=(lambda sbudget: sbudget.month == i), first=True)
