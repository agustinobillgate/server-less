...
from models import Htparam, L_op, L_artikel, L_ophdr, L_lager, Parameters, L_bestand, Bediener, Gl_acct, Queasy

def storereq_list_create_list_3_webbl(from_date:date, to_date:date, from_dept:int, to_dept:int, curr_lschein:string, show_price:bool, filter:string):
    ...
    sr_remark_list_data, Sr_remark_list = create_model("Sr_remark_list", {"lscheinnr":string, "sr_remark":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal it_exist, t_list_data, sr_remark_list_data, appflag, long_digit, htparam, l_op, l_artikel, l_ophdr, l_lager, parameters, l_bestand, bediener, gl_acct, queasy
        ...
        return {"it_exist": it_exist, "t-list": t_list_data, "sr-remark-list": sr_remark_list_data}
    ...
    def create_list():
        ...
        if filter.lower()  == ("ALL").lower() :
            ...
            for l_op.lscheinnr, l_op.op_art, l_op.loeschflag, l_op.lager_nr, l_op.pos, l_op.reorgflag, l_op._recid, l_op.herkunftflag, l_op.datum, l_op.anzahl, l_op.deci1, l_op.einzelpreis, l_op.warenwert, l_op.artnr, l_op.stornogrund, l_op.fuellflag, l_artikel.bezeich, l_artikel.masseinheit, l_artikel.inhalt, l_artikel.lief_einheit, l_artikel.traubensorte, l_artikel.zwkum, l_artikel.endkum, l_artikel._recid in db_session.query(L_op.lscheinnr, L_op.op_art, L_op.loeschflag, L_op.lager_nr, L_op.pos, L_op.reorgflag, L_op._recid, L_op.herkunftflag, L_op.datum, L_op.anzahl, L_op.deci1, L_op.einzelpreis, L_op.warenwert, L_op.artnr, L_op.stornogrund, L_op.fuellflag, L_artikel.bezeich, L_artikel.masseinheit, L_artikel.inhalt, L_artikel.lief_einheit, L_artikel.traubensorte, L_artikel.zwkum, L_artikel.endkum, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
                ...
                lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername = create_list_data(lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername)

        elif filter.lower()  == ("NO-APPROVE").lower() :
        ...
        elif filter.lower()  == ("APPROVE").lower() :
        ...
        elif filter.lower()  == ("OUTGOING").lower() :
            ...
            for l_op.lscheinnr, l_op.op_art, l_op.loeschflag, l_op.lager_nr, l_op.pos, l_op.reorgflag, l_op._recid, l_op.herkunftflag, l_op.datum, l_op.anzahl, l_op.deci1, l_op.einzelpreis, l_op.warenwert, l_op.artnr, l_op.stornogrund, l_op.fuellflag, l_artikel.bezeich, l_artikel.masseinheit, l_artikel.inhalt, l_artikel.lief_einheit, l_artikel.traubensorte, l_artikel.zwkum, l_artikel.endkum, l_artikel._recid in db_session.query(L_op.lscheinnr, L_op.op_art, L_op.loeschflag, L_op.lager_nr, L_op.pos, L_op.reorgflag, L_op._recid, L_op.herkunftflag, L_op.datum, L_op.anzahl, L_op.deci1, L_op.einzelpreis, L_op.warenwert, L_op.artnr, L_op.stornogrund, L_op.fuellflag, L_artikel.bezeich, L_artikel.masseinheit, L_artikel.inhalt, L_artikel.lief_einheit, L_artikel.traubensorte, L_artikel.zwkum, L_artikel.endkum, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
                ...
                tl_op = db_session.query(Tl_op).filter(
                         (Tl_op.lscheinnr == l_op.lscheinnr) & ((Tl_op.op_art == 3) | (Tl_op.op_art == 4)) & (Tl_op.loeschflag <= 1)).first()

                if tl_op:
                    pass
                    lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername = create_list_data(lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername)

        elif filter.lower()  == ("DELETE").lower() :
    ...
    def create_list_data(lscheinnr:string, qty:Decimal, qty1:Decimal, val:Decimal, amount:Decimal, t_qty:Decimal, t_qty1:Decimal, t_val:Decimal, t_amount:Decimal, appflag:bool, deptno:int, curr_centername:string):
        ...
        def generate_inner_output():
            return (lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername)
        ...
        if do_it:
            ...
            sr_remark_list = query(sr_remark_list_data, filters=(lambda sr_remark_list: sr_remark_list.lscheinnr == l_op.lscheinnr), first=True)

            if not sr_remark_list:
                sr_remark_list = Sr_remark_list()
                sr_remark_list_data.append(sr_remark_list)
