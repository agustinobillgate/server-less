#using conversion tools version: 1.0.0.110
...
def gacct_balance_btn_go_listbl(pvilanguage:int, bill_alert_list:[Bill_alert], heute:date, billdate:date, ank_flag:bool, sorttype:int, fact1:int, price_decimal:int, short_flag:bool):

    prepare_cache ([Bill_line, Bill, Res_line, Guest, Artikel, Uebertrag])
    ...
    bill_line = bill = res_line = guest = artikel = uebertrag = None

    output_list = m_list = sum_list = gacct_balance_list = s_list = ns_list = bill_alert = bill_list = bline = None

    output_list_list, Output_list = create_model("Output_list", {"flag":int, "artart":int, "str":string, "ankunft":date, "ankzeit":string})
    m_list_list, M_list = create_model("M_list", {"resnr":int, "zinr":string, "abreise":date})
    sum_list_list, Sum_list = create_model("Sum_list", {"bezeich":string, "debit":Decimal, "credit":Decimal, "balance":Decimal})
    ...
    bill_list_list, Bill_list = create_model("Bill_list", {"rechnr":int, "billnr":int, "resnr":int, "reslinnr":int, "ankzeit":int, "ankunft":date, "abreise":date, "first_date":date, "last_date":date, "zinr":string, "gname":string, "billtype":string, "betrag":Decimal}, {"first_date": None, "last_date": None})
    ...
    def generate_output():
        nonlocal msg_str, msg_str2, gacct_balance_list_list, i, prevbal, debit, credit, balance, current_counter, t_prevbal, t_debit, t_credit, t_balance, tot_bline, curr_rechnr, curr_flag, jl, lvcarea, bill_line, bill, res_line, guest, artikel, uebertrag
        ...
        nonlocal output_list_list, m_list_list, sum_list_list, gacct_balance_list_list, s_list_list, ns_list_list, bill_list_list
    ...
    def create_bill_list():
        ...
        for bill in db_session.query(Bill).filter(
                 (Bill.rechnr > 0) & ((Bill.flag == 0) | ((Bill.datum >= billdate) & (Bill.flag == 1)))).order_by(Bill._recid).all():
            last_date = None
            first_date = None
            ...
            if bill.flag == 1 and last_date == None:
                last_date = bill.datum
    ...
    def create_umsatz():
        ...
        if ank_flag:
            ...
            if sorttype == 1:
                ...
                for bill_list in query(bill_list_list, filters=(lambda bill_list: bill_list.billtype.lower()  == ("G").lower()), sort_by=[("ankunft",False),("ankzeit",False),("gname",False),("zinr",False),("billnr",False)]):
                    create_data2()

            else:

                for bill_list in query(bill_list_list, filters=(lambda bill_list: bill_list.billtype.lower()  == ("G").lower()), sort_by=[("ankunft",False),("ankzeit",False),("zinr",False),("gname",False),("billnr",False)]):
                    create_data2()
            ...
            if sorttype == 1:
                ...
                for bill_list in query(bill_list_list, filters=(lambda bill_list: bill_list.billtype.lower()  == ("G").lower()), sort_by=[("gname",False),("zinr",False),("billnr",False)]):
                    create_data2()

            else:

                for bill_list in query(bill_list_list, filters=(lambda bill_list: bill_list.billtype.lower()  == ("G").lower()), sort_by=[("zinr",False),("gname",False),("billnr",False)]):
                    create_data2()


        for bill_list in query(bill_list_list, filters=(lambda bill_list: bill_list.billtype.lower()  == ("M").lower()), sort_by=[("rechnr",False)]):
        ...
        for bill_list in query(bill_list_list, filters=(lambda bill_list: bill_list.billtype.lower()  == ("N").lower()), sort_by=[("gname",False)]):
            prevbal =  to_decimal("0")
            debit =  to_decimal("0")
            credit =  to_decimal("0")
            balance =  to_decimal("0")
        ...
        for s_list in query(s_list_list, sort_by=[("i_counter",False)]):
        ...
        for sum_list in query(sum_list_list, sort_by=[("bezeich",False)]):
            gacct_balance_list = Gacct_balance_list()
            gacct_balance_list_list.append(gacct_balance_list)
