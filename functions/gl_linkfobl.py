...
def gl_linkfobl(trans_dept_data:[Trans_dept], from_date:date, to_date:date, user_init:string, refno:string, curr_anz:int):
    ...
    def generate_output():
        nonlocal debits, credits, acct_error, remains, art_dpt, art_artnr, art_bezeich, buf_g_list_data, curr_date, artart_list, price_decimal, credit_betrag, debit_betrag, serv_acctno, vat_acctno, fibukonto, lastdate, gl_acct, gl_jouhdr, htparam, artikel, hoteldpt, umsatz
        ...
        return {"curr_anz": curr_anz, "debits": debits, "credits": credits, "acct_error": acct_error, "remains": remains, "art_dpt": art_dpt, "art_artnr": art_artnr, "art_bezeich": art_bezeich, "buf-g-list": buf_g_list_data}
    ...
    def step_two():
        ...
        for curr_date in date_range(from_date,to_date) :
            ...
            for hoteldpt in db_session.query(Hoteldpt).order_by(Hoteldpt.num).all():
                trans_dept = query(trans_dept_data, (lambda trans_dept: trans_dept.nr == hoteldpt.num), first=True)
                if not trans_dept:
                    continue
                    ...
                    for umsatz in db_session.query(Umsatz).filter(
                        ...
                        if artikel.artart == 0 or artikel.artart == 5 or artikel.artart == 8:
                            ...
                            if service > 0:
                                ...
                                g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.datum == curr_date), first=True)
                            ...
                            if sales > 0:
                                ...
                                g_list = query(g_list_data, filters=(lambda g_list: g_list.artnr == artikel.artnr and g_list.dept == artikel.departement and g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.datum == curr_date), first=True)
                            ...
                            if umsatz.betrag <= 0:
                                ...
                                g_list = query(g_list_data, filters=(lambda g_list: g_list.artnr == artikel.artnr and g_list.dept == artikel.departement and g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.datum == curr_date and g_list.flag != 9), first=True)
                    ...
                    for umsatz in db_session.query(Umsatz).filter(
                        ...
                        if artikel.artart == 13:
                            ...
                            if sales > 0:
                                ...
                                g_list = query(g_list_data, filters=(lambda g_list: g_list.artnr == bartikel.artnr and g_list.dept == bartikel.departement and g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.datum == curr_date), first=True)
    ...
    def check_dept():
        ...
        if htparam.fchar != "":
            for i in range(1,num_entries(htparam.fchar, ",")  + 1) :
                ...
                trans_dept = query(trans_dept_data, filters=(lambda trans_dept: trans_dept.nr == to_int(entry(i - 1, htparam.fchar, ","))), first=True)

                if not trans_dept:
                    trans_dept = Trans_dept()
                    trans_dept_data.append(trans_dept)
