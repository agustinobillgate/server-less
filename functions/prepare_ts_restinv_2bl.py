...
def prepare_ts_restinv_2bl(pvilanguage:int, curr_dept:int, curr_printer:int, user_init_str:str, transdate:date):
    ...
    def generate_output():
        nonlocal mealcoupon_cntrl, must_print, zero_flag, multi_cash, cancel_exist, msg_str, disc_art1, disc_art2, disc_art3, mi_ordertaker, price_decimal, curr_local, curr_foreign, double_currency, foreign_rate, exchg_rate, b_title, deptname, p_223, curr_waiter, fl_code, pos1, pos2, cashless_flag, c_param870, activate_deposit, interval_time, hbill_list, t_kellner_list, lvcarea, user_init, from_acct, guest, printer, htparam, queasy, waehrung, hoteldpt, kellner, h_bill, bediener
        ...
        return {"mealcoupon_cntrl": mealcoupon_cntrl, "must_print": must_print, "zero_flag": zero_flag, "multi_cash": multi_cash, "cancel_exist": cancel_exist, "msg_str": msg_str, "disc_art1": disc_art1, "disc_art2": disc_art2, "disc_art3": disc_art3, "mi_ordertaker": mi_ordertaker, "price_decimal": price_decimal, "curr_local": curr_local, "curr_foreign": curr_foreign, "double_currency": double_currency, "foreign_rate": foreign_rate, "exchg_rate": exchg_rate, "b_title": b_title, "deptname": deptname, "p_223": p_223, "curr_waiter": curr_waiter, "fl_code": fl_code, "pos1": pos1, "pos2": pos2, "cashless_flag": cashless_flag, "c_param870": c_param870, "activate_deposit": activate_deposit, "interval_time": interval_time, "hbill": hbill_list, "t-kellner": t_kellner_list}
    ...
    htparam = db_session.query(Htparam).filter(
             (Htparam.paramnr == 870)).first()
    c_param870 = htparam.fchar

    htparam = db_session.query(Htparam).filter(
             (Htparam.paramnr == 588)).first()
    activate_deposit = htparam.flogical
    ...
    queasy = db_session.query(Queasy).filter(
             (Queasy.key == 305) & (Queasy.number1 == 1) & (Queasy.number2 == 1) & (Queasy.number3 == 1) & (Queasy.betriebsnr == curr_dept)).first()
    ...
    kellner = db_session.query(Kellner).filter(
             (Kellner.kellner_nr == curr_waiter) & (Kellner.departement == curr_dept)).first()
