...
from models import Res_line, Reservation, Bill, Billhis, Bill_line, Artikel, Arrangement

def co_guest_1bl(case_type:int, pvilanguage:int, fr_date:date, to_date:date, price_decimal:int):

    prepare_cache ([Res_line, Reservation, Bill, Billhis, Bill_line, Artikel, Arrangement])
    ...
    res_line = reservation = bill = billhis = bill_line = artikel = arrangement = None
    ...
    def generate_output():
        nonlocal cl_list_data, tot_rm, tot_deposit, tot_cash, tot_cc, tot_cl, tot_amt, tot_bb, lvcarea, res_line, reservation, bill, billhis, bill_line, artikel, arrangement
    ...
    def disp_billbalance():
        ...
        for res_line.resnr, res_line.resstatus, res_line.reslinnr, res_line.zinr, res_line.zipreis, res_line.ankunft, res_line.abreise, res_line.abreisezeit, res_line.name, res_line.arrangement, res_line._recid, reservation.resnr, reservation.name, reservation._recid in db_session.query(Res_line.resnr, Res_line.resstatus, Res_line.reslinnr, Res_line.zinr, Res_line.zipreis, Res_line.ankunft, Res_line.abreise, Res_line.abreisezeit, Res_line.name, Res_line.arrangement, Res_line._recid, Reservation.resnr, Reservation.name, Reservation._recid).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
            ...
            if do_it :
                ...
                if bill:
                    ...
                    billhis = get_cache (Billhis, {"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, res_line.reslinnr)]})
