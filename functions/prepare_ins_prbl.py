...
def prepare_ins_prbl(docu_nr:string):
    ...
    def generate_output():
        nonlocal billdate, comments, deptname, pos, t_l_orderhdr_data, ins_list_data, l_artikel, htparam, l_orderhdr, parameters, l_order
        nonlocal docu_nr
        nonlocal l_art
    ...
    for l_order, l_art in db_session.query(L_order, L_art).join(L_art,(L_art.artnr == L_order.artnr)).filter(
        ...
        s_list = query(s_list_data, (lambda s_list: s_list.artnr == l_order.artnr), first=True)
        if not s_list:
            continue
