...
def get_db_url(hotelCode):
    ...
    # session = get_database_session("postgresql://vhpadmin:VHPLogin2023@vhp-dev.cjxtrsmbui3n.ap-southeast-1.rds.amazonaws.com:5432/tenants")
    DATABASE_URL = f"postgresql+psycopg2cffi://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}?application_name=aws_lambda_serverless"
    session = get_database_session(DATABASE_URL)
    #TODO: testing only
    if not test_database_connection(session):
        return "postgresql+psycopg2cffi://postgres:password@localhost:5432/vhp_rental"
        # return get_database_session("postgresql+psycopg2cffi://postgres:VHPLite#2023@localhost:5432/vhp_rental")
        # return "postgresql+psycopg2cffi://vhpadmin:VHPLogin2023@/vhp-dev.cjxtrsmbui3n.ap-southeast-1.rds.amazonaws.com:5432/vhp_rental"
    ...
    return "postgresql+psycopg2cffi://" + username + ":" + enc_pass + "@" + ip + ":" + str(port) + "/" + db_name
