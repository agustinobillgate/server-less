...
def nt_onlinetax_billdate_manual(fr_date:date, to_date:date):
    avail_flag = False
    n:int = 1
    do_it:bool = False
    ...
    def step_1(bill_date:date):
        ...
        for hbill_list in query(hbill_list_list, filters=(lambda hbill_list: hbill_list.do_it  and hbill_list.tot_sales != 0)):
            ...
            for h_bill_line in db_session.query(H_bill_line).filter(
                     (H_bill_line.departement == hbill_list.dept) & (H_bill_line.rechnr == hbill_list.rechnr) & (H_bill_line.bill_datum == bill_date)).order_by(H_bill_line.zeit).all():
