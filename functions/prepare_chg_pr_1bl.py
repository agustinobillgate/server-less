...
def prepare_chg_pr_1bl(docu_nr:string):
    ...
    def generate_output():
        nonlocal billdate, comments, deptnr, lieferdatum, deptname, hierarchie, tot, s_list_data, t_l_orderhdr_data, t_l_artikel_data, t_parameters_data, t_waehrung_data, local_curr, amt, def_curr, tot_anz, anz_anf_best, anz_eingang, anz_ausgang, l_order, l_orderhdr, waehrung, bediener, htparam, parameters, l_artikel, l_lieferant, gl_acct, l_pprice, l_bestand
        nonlocal docu_nr
        nonlocal usr, b_list


        nonlocal s_list, t_parameters, t_l_orderhdr, t_waehrung, t_l_artikel, usr, b_list
        nonlocal s_list_data, t_parameters_data, t_l_orderhdr_data, t_waehrung_data, t_l_artikel_data

        return {"billdate": billdate, "comments": comments, "deptnr": deptnr, "lieferdatum": lieferdatum, "deptname": deptname, "hierarchie": hierarchie, "tot": tot, "s-list": s_list_data, "t-l-orderhdr": t_l_orderhdr_data, "t-l-artikel": t_l_artikel_data, "t-parameters": t_parameters_data, "t-waehrung": t_waehrung_data}
    ...
    def reload_s_list():
        ...
        for l_order, l_artikel in db_session.query(L_order, L_artikel).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
            ...
            while None != s_list:
                ...
                s_list = query(s_list_data, next=True)
                pass
