#using conversion tools version: 1.0.0.117

from functions.additional_functions import *
from decimal import Decimal
from models import Waehrung, Hoteldpt, Artikel, Htparam

def prepare_money_exchgbl():

    prepare_cache ([Htparam])

    local_nr = 0
    local_code = ""
    price_decimal = 0
    err_code = 0
    t_waehrung_data = []
    t_hoteldpt_data = []
    art1_data = []
    waehrung = hoteldpt = artikel = htparam = None

    t_waehrung = t_hoteldpt = art1 = art2 = None

    t_waehrung_data, T_waehrung = create_model_like(Waehrung)
    t_hoteldpt_data, T_hoteldpt = create_model_like(Hoteldpt)
    art1_data, Art1 = create_model_like(Artikel)

    Art2 = create_buffer("Art2",Artikel)


    db_session = local_storage.db_session

    def generate_output():
        nonlocal local_nr, local_code, price_decimal, err_code, t_waehrung_data, t_hoteldpt_data, art1_data, waehrung, hoteldpt, artikel, htparam
        nonlocal art2


        nonlocal t_waehrung, t_hoteldpt, art1, art2
        nonlocal t_waehrung_data, t_hoteldpt_data, art1_data

        return {"local_nr": local_nr, "local_code": local_code, "price_decimal": price_decimal, "err_code": err_code, "t-waehrung": t_waehrung_data, "t-hoteldpt": t_hoteldpt_data, "art1": art1_data}

    htparam = get_cache (Htparam, {"paramnr": [(eq, 112)]})

    art2 = db_session.query(Art2).filter(
             (Art2.artnr == htparam.finteger) & (Art2.departement == 0) & (Art2.artart == 6) & not_ (Art2.pricetab)).first()

    if not art2:
        err_code = 1

        return generate_output()
    local_nr = art2.artnr
    art1 = Art1()
    art1_data.append(art1)

    buffer_copy(art2, art1)

    htparam = get_cache (Htparam, {"paramnr": [(eq, 152)]})
    local_code = htparam.fchar

    waehrung = get_cache (Waehrung, {"wabkurz": [(eq, local_code)]})

    if not waehrung:
        err_code = 2

        return generate_output()

    htparam = get_cache (Htparam, {"paramnr": [(eq, 491)]})
    price_decimal = htparam.finteger

    for waehrung in db_session.query(Waehrung).order_by(Waehrung._recid).all():
        t_waehrung = T_waehrung()
        t_waehrung_data.append(t_waehrung)

        buffer_copy(waehrung, t_waehrung)

    for hoteldpt in db_session.query(Hoteldpt).order_by(Hoteldpt._recid).all():
        t_hoteldpt = T_hoteldpt()
        t_hoteldpt_data.append(t_hoteldpt)

        buffer_copy(hoteldpt, t_hoteldpt)

    return generate_output()