...
def main_fs_readequipment_webbl():
    ...
    text_p2_list_data, Text_p2_list = create_model("Text_p2_list", {"nr":int, "bezeich":string, "bg_col":int, "fg_col":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal text_p2_list_data, lvcval, lvi, lvicnt, dept, cdelimiter, htparam, zwkum


        nonlocal text_p2_list
        nonlocal text_p2_list_data

        return {"text-p2-list": text_p2_list_data}
    ...
    for lvi in range(1,lvicnt + 1) :
        ...
        if lvcval != "":
            ...
            if zwkum and (lvi <= 6):
                ...
                text_p2_list_data.append(text_p2_list)
