...
def bqt_religionbl(int:int, from_mm:int, from_dd:int, to_mm:int, to_dd:int):
    ...
    religion_data, Religion = create_model("Religion", {"gastnr":int, "company":string, "name":string, "pers_bez":string, "telephone":string, "email":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal religion_data, from_date, to_date, count, stopped, bk_veran, guest, akt_kont, queasy
        nonlocal int, from_mm, from_dd, to_mm, to_dd


        nonlocal religion
        nonlocal religion_data
