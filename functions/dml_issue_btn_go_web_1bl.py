...
from models import L_op, Gl_acct, Bediener, L_ophdr, Queasy, Dml_art, Dml_artdep, L_artikel, L_lieferant, L_liefumsatz, Reslin_queasy, Htparam, L_kredit, Ap_journal, L_pprice, L_bestand

op_list_data, Op_list = create_model_like(L_op, {"bezeich":string, "username":string, "dml_code":string})
op_list_data, Op_list = create_model_like(L_op, {"bezeich":string, "username":string, "dml_code":string})

def dml_issue_btn_go_web_1bl(op_list_data:[Op_list], billdate:date, closedate:date, curr_lager:int, curr_dept:int, cost_acct:string, lief_nr:int, lscheinnr:string, user_init:string, t_amount:Decimal, op_list_data:[Op_list], billdate:date, closedate:date, curr_lager:int, curr_dept:int, cost_acct:string, lief_nr:int, lscheinnr:string, user_init:string, t_amount:Decimal):

    prepare_cache ([L_op, Bediener, L_ophdr, L_artikel, L_liefumsatz, Reslin_queasy, Htparam, L_kredit, Ap_journal, L_pprice, L_bestand])
    ...
    def generate_output():
        nonlocal curr_pos, s_artnr, qty, price, created, err_code, gl_notfound, dml_code, curr_pos, s_artnr, qty, price, created, err_code, gl_notfound, dml_code, l_op, gl_acct, bediener, l_ophdr, queasy, dml_art, dml_artdep, l_artikel, l_lieferant, l_liefumsatz, reslin_queasy, htparam, l_kredit, ap_journal, l_pprice, l_bestand
        nonlocal billdate, closedate, curr_lager, curr_dept, cost_acct, lief_nr, lscheinnr, user_init, t_amount, billdate, closedate, curr_lager, curr_dept, cost_acct, lief_nr, lscheinnr, user_init, t_amount


        nonlocal op_list

        return {"op-list": op_list_data, "curr_pos": curr_pos, "s_artnr": s_artnr, "qty": qty, "price": price, "created": created, "err_code": err_code, "op-list": op_list_data, "curr_pos": curr_pos, "s_artnr": s_artnr, "qty": qty, "price": price, "created": created, "err_code": err_code}
    ...
    def create_purchase_book(s_artnr:int, price:Decimal, qty:Decimal, datum:date, lief_nr:int):
        ...
        nonlocal curr_pos, err_code, gl_notfound, dml_code, curr_pos, err_code, gl_notfound, dml_code, l_op, gl_acct, bediener, l_ophdr, queasy, dml_art, dml_artdep, l_artikel, l_lieferant, l_liefumsatz, reslin_queasy, htparam, l_kredit, ap_journal, l_pprice, l_bestand
        nonlocal billdate, closedate, curr_lager, curr_dept, cost_acct, lscheinnr, user_init, t_amount, billdate, closedate, curr_lager, curr_dept, cost_acct, lscheinnr, user_init, t_amount
