...
def mk_resline_view_guest_remarkbl(inp_gastnr:int):
    ...
    t_guest_remark_data, T_guest_remark = create_model_like(Guest_remark)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal guest_name, t_guest_remark_data, guest_remark, guest
        nonlocal inp_gastnr


        nonlocal t_guest_remark
        nonlocal t_guest_remark_data

        return {"guest_name": guest_name, "t-guest-remark": t_guest_remark_data}
    ...
    for guest_remark in db_session.query(Guest_remark).filter(
        ...
        t_guest_remark_data.append(t_guest_remark)
