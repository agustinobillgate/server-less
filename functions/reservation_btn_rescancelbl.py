...
def reservation_btn_rescancelbl(pvilanguage:int, curr_select:string, resno:int, reslinno:int):

    prepare_cache ([Reservation, Res_line, Htparam, Guest_pr, Ratecode])
    ...
    def generate_output():
        nonlocal msg_str, cancel_str, error_flag, pswd_str, max_comp, com_rm, queasy_flag, deposit, may_delete, lvcarea, reservation, res_line, htparam, queasy, guest, arrangement, guest_pr, reslin_queasy, ratecode
        nonlocal pvilanguage, curr_select, resno, reslinno

        return {"msg_str": msg_str, "cancel_str": cancel_str, "error_flag": error_flag, "pswd_str": pswd_str, "max_comp": max_comp, "com_rm": com_rm, "queasy_flag": queasy_flag}
    ...
    def cancel_resline():
        ...
        if res_line.l_zuordnung[2] == 1:
            msg_str = translateExtended ("Accompanying Guest can not be cancelled.", lvcarea, "")

            return

        if res_line.active_flag == 1 or res_line.resstatus == 6 or res_line.resstatus == 13:
            msg_str = translateExtended ("Cancellation of Inhouse-Guests is not possible.", lvcarea, "")

            return

        htparam = get_cache (Htparam, {"paramnr": [(eq, 437)]})

        if htparam.flogical == False and res_line.betrieb_gast > 0:
        ...
        if reservation:
            cancel_str = reservation.vesrdepot2
        error_flag = False
        queasy_flag = None != queasy
        msg_str = "&Q" +\
                translateExtended ("Do you really want to CANCEL the reservation:", lvcarea, "") +\
                chr_unicode(10) +\
    ...
    def cancel_mainres():
        ...
        if res_line:
            msg_str = translateExtended ("Cancellation not possible, In-House guest exists.", lvcarea, "")
        ...
        if res_line:
            msg_str = translateExtended ("KeyCard has been generated for", lvcarea, "") + " " + res_line.name + " - " + translateExtended ("RmNo", lvcarea, "") + " " + res_line.zinr + chr_unicode(10) + translateExtended ("Cancellation for all members no longer possible.", lvcarea, "")
        ...
        if guest:
            msg_str = "&Q" +\
                    translateExtended ("Do you really want to CANCEL the main reservation of", lvcarea, "") +\
                    chr_unicode(10) +\
                    guest.name + ", " + guest.vorname1 + guest.anredefirma +\
                    " " + guest.anrede1 +\
                    chr_unicode(10) +\
                    translateExtended ("including it's all reservation members ?", lvcarea, "")
    ...
    def check_deposit():
        ...
        if deposit and reservation1.bestat_datum == None:
            ...
            for resline1 in db_session.query(Resline1).filter(
                     (Resline1.resnr == resno) & (Resline1.active_flag == 0) & (Resline1.resstatus <= 6)).order_by(Resline1._recid).yield_per(100):
                anzahl = anzahl + 1

                if (resline1.resstatus == 11 or resline1.resstatus == 13 and resline1.l_zuordnung[2] == 1) or (resline1.resstatus == 11 or resline1.resstatus == 13 and resline1.l_zuordnung[2] == 0):
                    anzahl = 1
                    break
