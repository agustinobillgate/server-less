...
def outstand_polist_btn_go_webbl(from_date:date, to_date:date):

    prepare_cache ([L_order, L_artikel, Htparam, L_lieferant, L_orderhdr])

    exp_polist_data = []
    exp_polist_items_data = []
    billdate:date = None
    ...
    def generate_output():
        nonlocal exp_polist_data, exp_polist_items_data, billdate, l_order, l_artikel, htparam, l_lieferant, l_orderhdr
        nonlocal from_date, to_date
        nonlocal l_order1, l_art


        nonlocal exp_polist, exp_polist_items, l_order1, l_art
        nonlocal exp_polist_data, exp_polist_items_data

        return {"exp-polist": exp_polist_data, "exp-polist-items": exp_polist_items_data}
    ...
    l_order1 = L_order()
    for l_orderhdr.bestelldatum, l_orderhdr.docu_nr, l_orderhdr.lieferdatum, l_orderhdr.lief_fax, l_orderhdr.bestellart, l_orderhdr.gedruckt, l_orderhdr.besteller, l_orderhdr._recid, l_lieferant.firma, l_lieferant._recid, l_order1.docu_nr, l_order1.artnr, l_order1.anzahl, l_order1.einzelpreis, l_order1.warenwert, l_order1.lief_fax, l_order1.geliefert, l_order1.rechnungspreis, l_order1.txtnr, l_order1.lieferdatum_eff, l_order1.angebot_lief, l_order1.pos, l_order1.besteller, l_order1._recid, l_order1.rechnungswert, l_order1.lieferdatum in db_session.query(L_orderhdr.bestelldatum, L_orderhdr.docu_nr, L_orderhdr.lieferdatum, L_orderhdr.lief_fax, L_orderhdr.bestellart, L_orderhdr.gedruckt, L_orderhdr.besteller, L_orderhdr._recid, L_lieferant.firma, L_lieferant._recid, L_order1.docu_nr, L_order1.artnr, L_order1.anzahl, L_order1.einzelpreis, L_order1.warenwert, L_order1.lief_fax, L_order1.geliefert, L_order1.rechnungspreis, L_order1.txtnr, L_order1.lieferdatum_eff, L_order1.angebot_lief, L_order1.pos, L_order1.besteller, L_order1._recid, L_order1.rechnungswert, L_order1.lieferdatum).join(L_lieferant,(L_lieferant.lief_nr == L_orderhdr.lief_nr)).join(L_order1,(L_order1.docu_nr == L_orderhdr.docu_nr) & (L_order1.loeschflag == 0) & (L_order1.pos >= 0) & (L_order1.geliefert < L_order1.anzahl)).filter(
        ...
        for l_order.docu_nr, l_order.artnr, l_order.anzahl, l_order.einzelpreis, l_order.warenwert, l_order.lief_fax, l_order.geliefert, l_order.rechnungspreis, l_order.txtnr, l_order.lieferdatum_eff, l_order.angebot_lief, l_order.pos, l_order.besteller, l_order._recid, l_order.rechnungswert, l_order.lieferdatum, l_art.bezeich, l_art.masseinheit, l_art.jahrgang, l_art._recid in db_session.query(L_order.docu_nr, L_order.artnr, L_order.anzahl, L_order.einzelpreis, L_order.warenwert, L_order.lief_fax, L_order.geliefert, L_order.rechnungspreis, L_order.txtnr, L_order.lieferdatum_eff, L_order.angebot_lief, L_order.pos, L_order.besteller, L_order._recid, L_order.rechnungswert, L_order.lieferdatum, L_art.bezeich, L_art.masseinheit, L_art.jahrgang, L_art._recid).join(L_art,(L_art.artnr == L_order.artnr)).filter(
            ...
            exp_polist_items_data.append(exp_polist_items)
