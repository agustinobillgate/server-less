...
def stock_hmovelist_create_list_webbl(pvilanguage:int, s_artnr:int, mm:int, yy:int, from_lager:int, to_lager:int, show_price:bool):
    ...
    art_list_data, Art_list = create_model("Art_list", {"datum":string, "devnote":string, "qty":string, "val":string, "in_qty":string, "in_val":string, "out_qty":string, "out_val":string, "note":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal anfdate, enddate, art_list_data
        nonlocal pvilanguage, s_artnr, mm, yy, from_lager, to_lager, show_price


        nonlocal str_list, art_list
        nonlocal str_list_data, art_list_data

        return {"anfdate": anfdate, "enddate": enddate, "art-list": art_list_data}

    anfdate, enddate, str_list_data = get_output(stock_hmovelist_create_listbl(pvilanguage, s_artnr, mm, yy, from_lager, to_lager, show_price))
