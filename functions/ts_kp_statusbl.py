...
def ts_kp_statusbl(pvilanguage:int, billno:int, depart:int):
    ...
    tlist_data, Tlist = create_model("Tlist", {"artqty":int, "artname":string, "send_print":date, "send_tprint":bool, "printed":bool})
    print_list_data, Print_list = create_model("Print_list", {"artno":int, "dept_no":int, "art_desc":string, "art_desc2":string, "bill_no":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal msg_str, tlist_data, lvcarea, lvdelimiter1, lvdelimiter2, loopi, loopn, curr_str, str1, dept_name, hoteldpt, h_artikel, h_bill_line, h_bill, queasy
        nonlocal pvilanguage, billno, depart


        nonlocal tlist, print_list
        nonlocal tlist_data, print_list_data

        return {"msg_str": msg_str, "tlist": tlist_data}
    ...
    for h_bill_line.rechnr, h_bill_line.departement, h_bill_line.artnr, h_bill_line.bezeich, h_bill_line._recid, h_artikel.bezeich, h_artikel._recid in db_session.query(H_bill_line.rechnr, H_bill_line.departement, H_bill_line.artnr, H_bill_line.bezeich, H_bill_line._recid, H_artikel.bezeich, H_artikel._recid).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == depart) & (H_artikel.artart == 0)).filter(
        ...
        print_list = query(print_list_data, filters=(lambda print_list: print_list.bill_no == h_bill_line.rechnr and print_list.dept_no == h_bill_line.departement and print_list.artno == h_bill_line.artnr and print_list.art_desc == h_artikel.bezeich and print_list.art_desc2 == h_bill_line.bezeich), first=True)
        ...
        for queasy in db_session.query(Queasy).filter(
            ...
            for loopi in range(1,num_entries(queasy.char3, lvdelimiter1)  + 1) :
                ...
                if curr_str != "":
                    ...
                    print_list = query(print_list_data, filters=(lambda print_list:(matches(print_list.art_desc,r"*" + trim(substring(curr_str, 5)) + r"*)) or matches(print_list.art_desc2,r"*" + trim(substring(curr_str, 5)) + r"*")) and print_list.dept_no == h_bill.departement), first=True
