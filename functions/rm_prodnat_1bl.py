...
from models import Nation, Guest, Genstat

def rm_prodnat_1bl(nation_name:string, ytd_flag:bool, incl_comp:bool, fr_date:date, to_date:date):

    prepare_cache ([Nation, Guest, Genstat])
    ...
    nation = guest = genstat = None

    output_list = t_list = None

    output_list_data, Output_list = create_model("Output_list", {"str":string, "gname":string, "flag":bool, "resrv_name":string, "dlodge":Decimal, "mlodge":Decimal, "ylodge":Decimal, "dnite":int, "mnite":int, "ynite":int, "drate":Decimal, "mrate":Decimal, "yrate":Decimal})
    t_list_data, T_list = create_model("T_list", {"natnr":int, "nat":string, "counts":int, "tot_nat":int, "flag":bool, "gastnr":int, "name":string, "gname":string, "dlodge":Decimal, "mlodge":Decimal, "ylodge":Decimal, "dnite":int, "mnite":int, "ynite":int, "drate":Decimal, "mrate":Decimal, "yrate":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal flag, output_list_data, tot_dlodge, tot_mlodge, tot_ylodge, tot_dnite, tot_mnite, tot_ynite, tot_drate, tot_mrate, tot_yrate, tot1_dlodge, tot1_mlodge, tot1_ylodge, tot1_dnite, tot1_mnite, tot1_ynite, tot1_drate, tot1_mrate, tot1_yrate, nation, guest, genstat
        nonlocal nation_name, ytd_flag, incl_comp, fr_date, to_date


        nonlocal output_list, t_list
        nonlocal output_list_data, t_list_data

        return {"flag": flag, "output-list": output_list_data}
    ...
    def create_list1():
        ...
        for genstat in db_session.query(Genstat).filter(
                 (Genstat.datum >= fr_date) & (Genstat.datum <= to_date) & (Genstat.nationnr == nation.nationnr) & (Genstat.resstatus != 13) & (Genstat.segmentcode != 0) & (Genstat.res_logic[inc_value(1)])).order_by(Genstat._recid).all():
            do_it = True
            ...
            if do_it:
                ...
                t_list = query(t_list_data, filters=(lambda t_list: t_list.gastnr == genstat.gastnr and t_list.natnr == genstat.source), first=True)
        ...
        output_list.gname = fill("-", 32)
        output_list.str = fill("-", 139)


        output_list = Output_list()
        output_list_data.append(output_list)

        str = to_string("Total", "X(24)") +\
                to_string(tot_dlodge, "->,>>>,>>>,>>9.99") +\
                to_string(tot_mlodge, "->,>>>,>>>,>>9.99") +\
                to_string(tot_ylodge, "->,>>>,>>>,>>9.99") +\
                to_string(tot_dnite, ">>9") +\
                to_string(tot_mnite, ">,>>9") +\
                to_string(tot_ynite, ">,>>9") +\
                to_string(tot_drate, "->,>>>,>>>,>>9.99") +\
                to_string(tot_mrate, "->,>>>,>>>,>>9.99") +\
                to_string(tot_yrate, "->,>>>,>>>,>>9.99")
        output_list.resrv_name = to_string("Total", "X(24)")
    ...
    def count_sub():
        ...
        for t_list in query(t_list_data, sort_by=[("natnr",False)]):
            output_list = Output_list()
            output_list_data.append(output_list)

            output_list.gname = t_list.gname
            output_list.str = to_string(t_list.name, "X(24)") +\
                    to_string(t_list.dlodge, "->,>>>,>>>,>>9.99") +\
                    to_string(t_list.mlodge, "->,>>>,>>>,>>9.99") +\
                    to_string(t_list.ylodge, "->,>>>,>>>,>>9.99") +\
                    to_string(t_list.dnite, ">>9") +\
                    to_string(t_list.mnite, ">,>>9") +\
                    to_string(t_list.ynite, ">,>>9") +\
                    to_string(t_list.drate, "->,>>>,>>>,>>9.99") +\
                    to_string(t_list.mrate, "->,>>>,>>>,>>9.99") +\
                    to_string(t_list.yrate, "->,>>>,>>>,>>9.99")
            output_list.resrv_name = to_string(t_list.name, "X(24)")
            output_list.dlodge =  to_decimal(t_list.dlodge)
    ...
    nation = get_cache (Nation, {"bezeich": [(eq, nation_name)],"natcode": [(eq, 0)]})

    if not nation:

        nation = db_session.query(Nation).filter(
                 (substring(Nation.bezeich, 0, length((nation_name).lower() )) == (nation_name).lower()) & (Nation.natcode == 0)).first()
