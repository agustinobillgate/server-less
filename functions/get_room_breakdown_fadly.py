...
def get_room_breakdown_fadly(recid_resline:int, datum:date, curr_i:int, curr_date:date):
    ...
    def calc_lodging2():
        ...
        if arrangement:
            ...
            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                     (Reslin_queasy.key == ("arrangement").lower()) & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr)).order_by(Reslin_queasy._recid).yield_per(100):

                if datum >= reslin_queasy.date1 and datum <= reslin_queasy.date2:
                    fixed_rate = True
    ...
    def get_argtline_rate(curr_date:date, contcode:string, argt_recid:int):
        ...
        if add_it:
            ...
            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                     (Reslin_queasy.key == ("fargt-line").lower()) & (Reslin_queasy.char1 == "") & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number1 == argtline.departement) & (Reslin_queasy.number2 == argtline.argtnr) & (Reslin_queasy.number3 == argtline.argt_artnr)).order_by(Reslin_queasy._recid).all():

                if curr_date >= reslin_queasy.date1 and curr_date <= reslin_queasy.date2:
