...
def import_check_rest_artikelxls_tzhbl(rest_article_list_data:[Rest_article_list], rest_maingroup_list_data:[Rest_maingroup_list], rest_subgroup_list_data:[Rest_subgroup_list]):
    output_list_data = []
    ...
    output_list_data, Output_list = create_model("Output_list", {"dept_no":int, "dept_name":string, "artnr":int, "art_desc":string, "maingroup":int, "subgroup":int, "str":string, "key":string, "flag":int})
    ...
    def generate_output():
        nonlocal output_list_data, h_rezept, h_artikel
    ...
    for rest_article_list in query(rest_article_list_data):
        ...
        b_maingroup_list = query(b_maingroup_list_data, filters=(lambda b_maingroup_list:(b_maingroup_list.maingroup_desc == rest_article_list.main_group_desc) or (b_maingroup_list.new_maingroup_desc == rest_article_list.main_group_desc)), first=True)
        ...
        b_subgroup_list = query(b_subgroup_list_data, filters=(lambda b_subgroup_list:(b_subgroup_list.subgroup_desc == rest_article_list.sub_group_desc) or (b_subgroup_list.new_subgroup_desc == rest_article_list.sub_group_desc)), first=True)
        ...
        b_article_list = query(b_article_list_data, filters=(lambda b_article_list: b_article_list.dept_no == rest_article_list.dept_no and b_article_list.artnr == rest_article_list.artnr), first=True)
        ...
        if length(to_string(rest_article_list.artnr)) > 8:
            ...
            rest_maingroup_list = query(rest_maingroup_list_data, filters=(lambda rest_maingroup_list: rest_maingroup_list.new_maingroup_no == to_int(substring(to_string(rest_article_list.artnr) , 0, 1))), first=True)
            ...
            rest_subgroup_list = query(rest_subgroup_list_data, filters=(lambda rest_subgroup_list: rest_subgroup_list.new_subgroup_no == to_int(substring(to_string(rest_article_list.artnr) , 3, 2)) and rest_subgroup_list.dept_no == rest_article_list.dept_no), first=True)
    ...
    output_list = query(output_list_data, filters=(lambda output_list: output_list.key.lower()  != ("completed").lower()), first=True)
