...
def prepare_gl_detailoutgoingbl(fibu:string, from_date:date, bemerk:string):
    ...
    s_list_data, S_list = create_model("S_list", {"datum":date, "lager_nr":int, "artnr":int, "bezeich":string, "einzelpreis":Decimal, "anzahl":Decimal, "warenwert":Decimal, "stornogrund":string, "lscheinnr":string, "lflag":bool})
    t_gl_acct_data, T_gl_acct = create_model_like(Gl_acct)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal close_date, t_gl_acct_data, s_list_data, gl_acct, htparam, l_ophdr, l_artikel, l_untergrup, l_op, l_ophis
        ...
        return {"close_date": close_date, "t-gl-acct": t_gl_acct_data, "s-list": s_list_data}
