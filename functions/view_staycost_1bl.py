...
def view_staycost_1bl(pvilanguage:int, resnr:int, reslinnr:int, contcode:string, repeat_charge:bool):
    ...
    def generate_output():
        nonlocal output_list_data, ci_date, new_contrate, wd_array, bonus_array, periode, loopi, curr_diff, lvcarea, reservation, htparam, res_line, arrangement, guest_pr, waehrung, genstat, reslin_queasy, queasy, katpreis, argt_line, artikel, fixleist
    ...
    def calc_periode():
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 301)],"number1": [(eq, res_line.resnr)],"logi1": [(eq, repeat_charge)]})
