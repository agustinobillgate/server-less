...
def comp_statisticbl(pvilanguage:int, from_date:date, to_date:date, all_occ:bool, check_ftd:bool):
    ...
    output_list_data, Output_list = create_model("Output_list", {"hno":int, "htlname":string, "occ_rm":string, "saleable":string, "rmrev":string, "avrgrate":string, "nat_mark":string, "mark_share":string, "yield_":string, "occ_rm1":string, "saleable1":string, "rmrev1":string, "avrgrate1":string, "nat_mark1":string, "mark_share1":string, "yield1":string, "occ_rm2":string, "saleable2":string, "rmrev2":string, "avrgrate2":string, "nat_mark2":string, "mark_share2":string, "yield2":string, "hotel1":string, "hotel2":string, "hotel3":string, "occ_proz":string, "occ_proz1":string, "occ_proz2":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal tocc_rm, tocc_rm1, tocc_rm2, tsale, tsale1, tsale2, tocc_proz, tocc_proz1, tocc_proz2, hotel1, hotel2, hotel3, hotela, hotelb, hotelc, output_list_data, trmrev, trmrev1, trmrev2, f_date, t_date, lvcarea, zinrstat, akt_code
    ...
    def to_date1():
        ...
        for cl_list in query(cl_list_data, sort_by=[("hno",False)]):
        ...
        for cl_list in query(cl_list_data, sort_by=[("hno",True)]):
