...
from models import Res_line, Kontline, Queasy, Artikel, Htparam, Waehrung, Outorder, Zimmer, Guest, Zimkateg, Segment, Genstat, Exrate, Fixleist, Reservation, Arrangement, Bill_line, Reslin_queasy, Argt_line, Guestseg, Zinrstat, Zkstat, Umsatz
...
def cr_occfcast1_2_webbl_sy(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):

    prepare_cache ([Res_line, Artikel, Htparam, Waehrung, Zimmer, Genstat, Exrate, Fixleist, Reservation, Arrangement, Reslin_queasy, Argt_line, Zinrstat, Zkstat, Umsatz])
    ...
    allotment_exist:bool = False
    global_allotment_exist:bool = False
    qsy14_exist:bool = False
    ooo_exist:bool = False
    ooo_frdate:date = None
    curr_time:int = 0
    tmpint:int = 0
    res_line = kontline = queasy = artikel = htparam = waehrung = outorder = zimmer = guest = zimkateg = segment = genstat = exrate = fixleist = reservation = arrangement = bill_line = reslin_queasy = argt_line = guestseg = zinrstat = zkstat = umsatz = None

    room_list = segm_list = argt_list = zikat_list = outlook_list = print_list = print_list2 = print_list3 = rline1 = active_rm_list = dayuse_list = art_list = s_list = a_list = z_list = o_list = bsegm = bargt = broom = s_list = a_list = z_list = o_list = s_list = z_list = None
    ...
    def generate_output():
        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, datum, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, allotment_exist, global_allotment_exist, qsy14_exist, ooo_exist, ooo_frdate, curr_time, tmpint, res_line, kontline, queasy, artikel, htparam, waehrung, outorder, zimmer, guest, zimkateg, segment, genstat, exrate, fixleist, reservation, arrangement, bill_line, reslin_queasy, argt_line, guestseg, zinrstat, zkstat, umsatz
        ...
        nonlocal room_list, segm_list, argt_list, zikat_list, outlook_list, print_list, print_list2, print_list3, rline1, active_rm_list, dayuse_list, art_list, s_list, a_list, z_list, o_list, bsegm, bargt, broom, s_list, a_list, z_list, o_list, s_list, z_list
        nonlocal room_list_list, print_list_list, print_list2_list, print_list3_list, active_rm_list_list, dayuse_list_list, art_list_list

        return {"room-list": room_list_list}

    def create_browse():

        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, allotment_exist, global_allotment_exist, qsy14_exist, ooo_exist, ooo_frdate, curr_time, tmpint, res_line, kontline, queasy, artikel, htparam, waehrung, outorder, zimmer, guest, zimkateg, segment, genstat, exrate, fixleist, reservation, arrangement, bill_line, reslin_queasy, argt_line, guestseg, zinrstat, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
                            ...
                            if datum == res_line.abreise:
                                ...
                                if arrangement:
                                    ...
                                    for argt_line.argt_artnr, argt_line.departement, argt_line.fakt_modus, argt_line.intervall, argt_line.betriebsnr, argt_line.argtnr, argt_line.betrag, argt_line._recid in db_session.query(Argt_line.argt_artnr, Argt_line.departement, Argt_line.fakt_modus, Argt_line.intervall, Argt_line.betriebsnr, Argt_line.argtnr, Argt_line.betrag, Argt_line._recid).filter(
                                        ...
                                        post_it = check_fixargt_posted(argt_line.argt_artnr, argt_line.departement, argt_line.fakt_modus, argt_line.intervall, res_line.ankunft, res_line.abreise)
                                        service =  to_decimal("0")
                                        vat =  to_decimal("0")
                                        fcost =  to_decimal("0")
        ...
        if qsy14_exist:
            for i in range(1,tmpint + 1) :
                ...
                room_list = query(room_list_list, filters=(lambda room_list: room_list.datum == datum), first=True)
                if Queasy.date1 and Queasy.date2:
                    for queasy in db_session.query(Queasy).filter(
                            (Queasy.key == 14) & (Queasy.date1 <= datum) & (Queasy.date2 >= datum)).order_by(Queasy.number3).all():
        ...
        for room_list in query(room_list_list, filters=(lambda room_list: room_list.wd != 0)):
            ...
            if room_list.datum < ci_date:
                ...
                if zinrstat:
                    ...
                    if not all_zikat:
                        if zinrstat:
                            z_list = query(z_list_list, filters=(lambda z_list: z_list.zikatnr == zinrstat.zinr and z_list.selected), first=True)
        ...
        if do_it and incl_oth:
            calculate_other_revenue()
        room_list = Room_list()
        room_list_list.append(room_list)
    ...
    def room_code_name():
        ...
        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, datum, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, allotment_exist, global_allotment_exist, qsy14_exist, ooo_exist, ooo_frdate, tmpint, res_line, kontline, queasy, artikel, htparam, waehrung, outorder, zimmer, guest, zimkateg, segment, genstat, exrate, fixleist, reservation, arrangement, bill_line, reslin_queasy, argt_line, guestseg, zinrstat, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
    ...
    def create_browse1():
        ...
        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, allotment_exist, global_allotment_exist, qsy14_exist, ooo_exist, ooo_frdate, curr_time, tmpint, res_line, kontline, queasy, artikel, htparam, waehrung, outorder, zimmer, guest, zimkateg, segment, genstat, exrate, fixleist, reservation, arrangement, bill_line, reslin_queasy, argt_line, guestseg, zinrstat, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
    ...
    def calculate_other_revenue():
        ...
        othrev:Decimal = to_decimal("0.0")
        curr_exrate:Decimal = to_decimal("0.0")
        ...
        for art_list in query(art_list_list):
            ...
            umsatz = Umsatz()
            for umsatz.datum, umsatz.departement, umsatz.artnr, umsatz.betrag, umsatz._recid in db_session.query(Umsatz.datum, Umsatz.departement, Umsatz.artnr, Umsatz.betrag, Umsatz._recid).filter(
                ...
                if curr_datum != umsatz.datum:
                    ...
                    if curr_datum != "01/01/2000" and curr_exrate !=0:
                        room_list.lodg[4] = room_list.lodg[4] + othrev
                        if exchg_rate and exchg_rate > 0:
                            room_list.lodg[6] = room_list.lodg[6] + othrev / exchg_rate

                        if room_list.wd != 0 and curr_exrate !=0:
                            t_lodg[4] = t_lodg[4] + othrev
                            t_lodg[6] = t_lodg[6] + othrev / curr_exrate
                    ...
                    if waehrung:
                        ...
                        if exrate:
                            curr_exrate =  to_decimal(exrate.betrag)
                        else:
                            curr_exrate =  to_decimal(exchg_rate)
                    else:
                        curr_exrate =  to_decimal(exchg_rate)
                    service, vat = get_output(calc_servvat(umsatz.departement, umsatz.artnr, umsatz.datum, art_list.service_code, art_list.mwst_code))
            ...
            room_list.lodg[4] = room_list.lodg[4] + othrev
            room_list.lodg[6] = room_list.lodg[6] + othrev / curr_exrate
    ...
    kontline = db_session.query(Kontline).filter(
             ((Kontline.betriebsnr == 0) & (Kontline.kontstatus == 1)) & (((Kontline.ankunft <= curr_date) & (Kontline.abreise >= curr_date)) | ((Kontline.ankunft >= curr_date) & (Kontline.ankunft <= to_date)))).first()
    allotment_exist = None != kontline

    kontline = db_session.query(Kontline).filter(
             ((Kontline.betriebsnr == 1) & (Kontline.kontstatus == 1)) & (((Kontline.ankunft <= curr_date) & (Kontline.abreise >= curr_date)) | ((Kontline.ankunft >= curr_date) & (Kontline.ankunft <= to_date)))).first()
    global_allotment_exist = None != kontline

    queasy = db_session.query(Queasy).filter(
             (Queasy.key == 14) & (((Queasy.date1 <= curr_date) & (Queasy.date2 >= curr_date)) | ((Queasy.date1 >= curr_date) & (Queasy.date1 <= to_date)))).first()
    qsy14_exist = None != queasy
    ...
    if to_date >= ci_date:
        ...
        if curr_date < ci_date:
            ...
            ooo_frdate = curr_date

        outorder = get_cache (Outorder, {"gespende": [(lt, ooo_frdate)],"gespstart": [(gt, to_date)],"betriebsnr": [(le, 1)]})
        ooo_exist = None != outorder
