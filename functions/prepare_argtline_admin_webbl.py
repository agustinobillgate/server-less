...
def prepare_argtline_admin_webbl(argtno:int):
    ...
    t_argt_line_data, T_argt_line = create_model_like(Argt_line, {"bezeich":string})
    t_arrangement_data, T_arrangement = create_model_like(Arrangement)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_argt_line_data, t_arrangement_data, argt_line, arrangement, artikel
        nonlocal argtno


        nonlocal t_argt_line, t_arrangement
        nonlocal t_argt_line_data, t_arrangement_data

        return {"t-argt-line": t_argt_line_data, "t-arrangement": t_arrangement_data}
