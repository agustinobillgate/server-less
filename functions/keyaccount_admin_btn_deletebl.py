...
def keyaccount_admin_btn_deletebl(number1:int, user_init:string):
    ...
    queasy = get_cache (Queasy, {"key": [(eq, 211)],"number1": [(eq, number1)]})

    if queasy:
        pass

        q212 = db_session.query(Q212).filter(
                 (Q212.key == 212) & (Q212.number1 == queasy.number1)).first()

        if q212:
            str_msg = "Member entries exist, deleting KeyAccount not possible"
        else:
            str_msg = ""
