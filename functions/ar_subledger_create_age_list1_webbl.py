...
def ar_subledger_create_age_list1_webbl(incl:bool, t_artnr:int, t_dept:int, from_name:string, to_name:string, fr_date:date, to_date:date, bdate:date, gastnr:int):
    ...
    t_age_list_data, T_age_list = create_model_like(Age_list)
    ...
    def generate_output():
        nonlocal tot_debt, tot_paid, t_age_list_data, curr_rechnr, opart, opart1, opart2, rechnr, nr, curr_saldo, ankunft, abreise, voucherno, gname, do_it, t_resnr, t_name, guest, artikel, debitor, bill, reservation, res_line, h_bill, waehrung, bediener
        ...
        nonlocal ar_list_data, age_list_data, t_age_list_data

        return {"tot_debt": tot_debt, "tot_paid": tot_paid, "t-age-list": t_age_list_data}
            ...
            if debitor.betriebsnr == 0:
                ...
                if debitor.rechnr != 0:
                    ...
                    if bill:
                        ...
                        res_line = get_cache (Res_line, {"resnr": [(eq, bill.resnr)],"reslinnr": [(eq, bill.parent_nr)],"zipreis": [(gt, 0)]})
    ...
    for guest1 in db_session.query(Guest1).filter(
             ((Guest1.gastnr.in_(list(set([age_list.gastnrmember for age_list in age_list_data])))))).order_by(age_list.company, age_list.datum, age_list.counter, age_list.billname.desc()).all():
        if guest1_obj_list.get(guest1._recid):
            continue
        else:
            guest1_obj_list[guest1._recid] = True

        age_list = query(age_list_data, (lambda age_list: (guest1.gastnr == age_list.gastnrmember)), first=True)

        guest = get_cache (Guest, {"gastnr": [(eq, age_list.gastnr)]})
        t_age_list = T_age_list()
        t_age_list_data.append(t_age_list)
