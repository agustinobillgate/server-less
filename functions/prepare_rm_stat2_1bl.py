...
def prepare_rm_stat2_1bl(gastno:int, segmcode:int, from_date:date, to_date:date, mi_ftd:bool):
    ...
    t_list_data, T_list = create_model("T_list", {"gastnr":int, "mgastnr":int, "gname":string, "resnr":int, "reslinnr":int, "fdate":date, "tdate":date, "ankunft":date, "abreise":date, "zimmeranz":int, "dlodge":Decimal, "dnite":int, "drate":Decimal, "mlodge":Decimal, "mnite":int, "mrate":Decimal, "ylodge":Decimal, "ynite":int, "yrate":Decimal}, {"fdate": get_current_date(), "tdate": date_mdy(1, 1, 1900), "ankunft": None, "abreise": None})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal avail_guest, f_tittle, t_list_data, ci_date, guest, segment, htparam, genstat, reservation, res_line
        ...
        return {"avail_guest": avail_guest, "f_tittle": f_tittle, "t-list": t_list_data}
    ...
    def create_list(f_date:date, t_date:date):
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.resnr == genstat.resnr), first=True)
    ...
    def create_list2(f_date:date, t_date:date):
        ...
        for res_line, reservation in db_session.query(Res_line, Reservation).join(Reservation,(Reservation.resnr == Res_line.resnr) & (Reservation.segmentcode == segmcode)).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.resnr == res_line.resnr), first=True)
