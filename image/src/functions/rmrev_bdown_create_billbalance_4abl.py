...
def rmrev_bdown_create_billbalance_4abl(exc_taxserv:bool, pvilanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, curr_date:date, sorttype:int):
    ...
    sum_list = currency_list = cl_list = s_list = argt_list = waehrung1 = cc_list = member1 = rguest = artikel1 = argtline = None
    ...
    def generate_output():
        nonlocal msg_str, msg_warning, cl_list_list, currency_list_list, sum_list_list, s_list_list, argt_list_list, exchg_rate, frate, post_it, total_rev, rm_rate, lvcarea, loop2, j, m, curr_code, curr_rate, curr_local, curr_bfast, curr_lodge, curr_lunch, curr_dinner, curr_misc, curr_fcost, curr_trev, curr_pax, curr_com, curr_rm, curr_adult, curr_ch1, curr_ch2, curr_comch, waehrung, guest, artikel, htparam, res_line, zimmer, arrangement, reservation, bill, zimkateg, reslin_queasy, guest_pr, pricecod, argt_line, fixleist
        nonlocal waehrung1, cc_list, member1, rguest, artikel1, argtline


        nonlocal sum_list, currency_list, cl_list, s_list, argt_list, waehrung1, cc_list, member1, rguest, artikel1, argtline
    ...
    def create_billbalance():
        ...
        if sorttype == 2:
            ...
            for res_line in db_session.query(Res_line).filter(
                    ((Res_line.active_flag <= 1)) &  ((Res_line.resstatus <= 2) |  (Res_line.resstatus == 5) |  (Res_line.resstatus == 6)) &  (not (Res_line.ankunft > curr_date)) &  (not (Res_line.abreise < curr_date))).all():
                do_it = True
                ...
                if do_it:
                    ...
                    if res_line.active_flag == 1:
                        ...
                        if not bill:
                            ...
                            if not bill:
                                DO: msg_warning = "&W" + translateExtended ("Bill not found: RmNo ", lvcarea, "") + res_line.zinr + " - " + res_line.name
                ...
                if foreign_rate and price_decimal == 0 and not res_line.adrflag:
                    ...
                    if htparam.finteger != 0:
                        ...
                        for j in range(1,finteger + 1) :
                            m = m * 10
                        lodge_betrag = round(lodge_betrag / m, 0) * m
