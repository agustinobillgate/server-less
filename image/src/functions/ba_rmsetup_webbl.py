...
def ba_rmsetup_webbl():
    ...
    bk_rset_obj_list = []
    for bk_rset, bk_raum, bk_setup in db_session.query(Bk_rset, Bk_raum, Bk_setup).join(Bk_raum,(Bk_raum.raum == Bk_rset.raum)).join(Bk_setup,(Bk_setup.setup_id == Bk_rset.setup_id)).all():
