...
def mobile_prepare_hk_statadminbl(zinr:str, floor:int, dispsort:int, statsort:int):
    ci_date = None
    z_list_list = []
    om_list_list = []
    bline_list_list = []
    ...
    def create_list():
        ...
        for outorder in db_session.query(Outorder).filter(
            ...
            if not om_list or not(om_list.zinr == outorder.zinr):
    ...
    if not htparam or not(paramnr == 87):
