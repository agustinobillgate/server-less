...
def get_data_print_confirmletter_webbl(resnumber:int, user_init:str):
    ...
    for res_line in db_session.query(Res_line).filter(
            (Res_line.resnr == resnumber) &  (Res_line.active_flag <= 1) &  (Res_line.resstatus != 11)).all():
        ...
        if waehrung:
            ...
            print_list.depositgef = reservation.depositgef
            print_list.depositbez = reservation.depositbez + reservation.depositbez2
            ...
            if fixleist:
                print_list.bedsetup_rate = fixleist.betrag
            else:
                print_list.bedsetup_rate = 0

            t_guest = db_session.query(T_guest).filter(
                    (T_guest.gastnr == res_line.gastnr)).first()

            if t_guest.karteityp == 2:
                print_list.roomrate = 0
                print_list.total_rate = 0
            else:
                print_list.roomrate = res_line.zipreis
                print_list.total_rate = (res_line.zipreis * print_list.room_night) + print_list.bedsetup_rate
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (Reslin_queasy.resnr == res_line.resnr) &  (Reslin_queasy.reslinnr == res_line.reslinnr) &  (Reslin_queasy.deci1 != 0)).all():
            print_list.tot_pay = print_list.tot_pay + reslin_queasy.deci1

            if (reslin_queasy.date2 - reslin_queasy.date1) == 0:
                night = 1
            else:
                night = (reslin_queasy.date2 - reslin_queasy.date1) + 1
            pricerm = pricerm + to_string(decimal.Decimal(reslin_queasy.deci1) , "->,>>>,>>>,>>>,>>9.99") + ":"
            tot_rm = tot_rm + to_string(decimal.Decimal(reslin_queasy.deci1 * night) , "->,>>>,>>>,>>>,>>9.99") + ":"
            all_total = all_total + (reslin_queasy.deci1 * night)

        for t_zim in db_session.query(T_zim).filter(
                (T_zim.zikatnr == res_line.zikatnr)).all():
            rmtype = rmtype + t_zim.bezeichnung + ":"

        for t_guest in db_session.query(T_guest).filter(
                (T_guest.gastnr == res_line.gastnr)).all():
        ...
        for t_res in db_session.query(T_res).filter(
                (T_res.resnr == res_line.resnr)).all():
            allgst = allgst + t_res.name + " : "

        if pricerm != "":
            print_list.all_rmrate = pricerm
        else:
            print_list.all_rmrate = to_string(decimal.Decimal(print_list.roomrate) , "->,>>>,>>>,>>>,>>9.99")

        if tot_rm != "":
            print_list.all_totrate = tot_rm
        else:
            print_list.all_totrate = to_string(decimal.Decimal(print_list.total_rate) , "->,>>>,>>>,>>>,>>9.99")
        print_list.all_total = all_total
