...
from models import H_bill_line, H_artikel, Htparam, Hoteldpt, Wgrpgen, Artikel
...
def ts_restinv_disp_bill_line_web_1bl(double_currency:bool, rechnr:int, curr_dept:int, avail_t_h_bill:bool, hbill_recid:int):
    ...
    h_bill_line = h_artikel = htparam = hoteldpt = wgrpgen = artikel = None

    t_h_bill_line = ordered_item = summary_bill = t_h_artikel = h_art2 = None

    t_h_bill_line_list, T_h_bill_line = create_model_like(H_bill_line, {"rec_id":int, "request_str":str, "flag_code":int, "kds_stat":int, "kds_stat_str":str}, {"kds_stat": -1})
    ...
    def generate_output():
        ...
        return {"t-h-bill-line": t_h_bill_line_list, "summary-bill": summary_bill_list, "t-h-artikel": t_h_artikel_list, "amount_bill": amount_bill, "depo_flag": depo_flag, "rsvtable_text": rsvtable_text}
    ...
    for h_artikel in db_session.query(H_artikel).filter(
            (H_artikel.departement == curr_dept) &  ((H_artikel.artart == 2) |  (H_artikel.artart == 6) |  (H_artikel.artart == 7) |  (H_artikel.artart == 11) |  (H_artikel.artart == 12) |  (H_artikel.artart == 5))).all():
