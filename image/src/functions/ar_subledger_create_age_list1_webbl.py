...
def ar_subledger_create_age_list1_webbl(incl:bool, t_artnr:int, t_dept:int, from_name:str, to_name:str, fr_date:date, to_date:date, bdate:date, gastnr:int):
    ...
    for debitor, gast in db_session.query(Debitor, Gast).join(Gast,(Gast.gastnr == Debitor.gastnr)).filter(
            (Debitor.artnr == artikel.artnr) &  (Debitor.gastnr == gastnr) &  (Debitor.opart >= opart) &  (Debitor.opart <= opart2) &  (Debitor.rgdatum >= fr_date) &  (Debitor.rgdatum <= to_date) &  (Debitor.zahlkonto == 0)).all():
    ...
    for age_list in query(age_list_list):
        guest1 = db_session.query(Guest1).filter((Guest1.gastnr == age_list.gastnrmember)).first()
        if not guest1:
            continue
