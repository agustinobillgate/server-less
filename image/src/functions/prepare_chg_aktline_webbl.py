...
def prepare_chg_aktline_webbl(akt_line_gastnr:int, akt_line_aktionscode:int, akt_line_prioritaet:int, akt_line_zeit:int, akt_line_dauer:int, akt_line_bemerk:str):
    lname = ""
    aktion = ""
    prior = ""
    zeit = ""
    ...
    def generate_output():
        nonlocal lname, aktion, prior, zeit, dauer, comment, t_akt_code_list, akt_line, akt_code, guest


        nonlocal akt_line1, t_akt_code, t_guest
        nonlocal akt_line1_list, t_akt_code_list, t_guest_list
        return {"lname": lname, "aktion": aktion, "prior": prior, "zeit": zeit, "dauer": dauer, "comment": comment, "t-akt-code": t_akt_code_list}
    ...
    t_akt_code_list = get_output(read_akt_codebl(1, "", None))

    t_akt_code = query(t_akt_code_list, filters=(lambda t_akt_code :t_akt_code.aktiongrup == 1 and t_akt_code.aktionscode == akt_line_aktionscode), first=True)
