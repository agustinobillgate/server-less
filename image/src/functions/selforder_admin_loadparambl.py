...
def selforder_admin_loadparambl(dept_no:int):
    ...
    t_param_list, T_param = create_model("T_param", {"dept":int, "grup":int, "number":int, "bezeich":str, "typ":int, "logv":bool, "val":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_param_list, queasy
    ...
    queasy = db_session.query(Queasy).filter(
            (Queasy.key == 222) &  (Queasy.number1 == 1) &  (Queasy.betriebsnr == dept_no)).first()
    ...
    for queasy in db_session.query(Queasy).filter(
            (Queasy.key == 222) &  (Queasy.number1 == 1) &  (Queasy.betriebsnr == dept_no)).all():
        t_param = T_param()
