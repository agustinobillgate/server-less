...
def connect_group_join_itbl(resno:int, selected_resnr:int, user_init:str, res_list:[Res_list]):
    res_line = bediener = reservation = res_history = None

    res_list = rline = mbuff = None
    ...
    def generate_output():
        ...
        nonlocal rline, mbuff


        nonlocal res_list, rline, mbuff
    ...
    def join_it():
        ...
        for res_line in db_session.query(Res_line).filter(
                    (Res_line.resnr == selected_resnr) &  (Res_line.l_zuordnung[4] == 0)).all():
        ...
        for res_list in query(res_list_list, filters=(lambda res_list :(res_list.prev_join != res_list.join_flag) or (res_list.prev_mbill != res_list.mbill_flag))):
            ...
            for res_line in db_session.query(Res_line).filter(
                        (Res_line.resnr == res_list.resnr) &  (Res_line.kontakt_nr == res_list.reslinnr) &  (Res_line.l_zuordnung[2] == 1)).all():
