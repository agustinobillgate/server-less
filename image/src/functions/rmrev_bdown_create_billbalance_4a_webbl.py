...
from models import Waehrung, Guest, Artikel, Htparam, Res_line, Zimmer, Arrangement, Reservation, Bill, Zimkateg, Segment, Reslin_queasy, Guest_pr, Pricecod, Argt_line, Fixleist
...
def rmrev_bdown_create_billbalance_4a_webbl(exc_taxserv:bool, pvilanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, curr_date:date, sorttype:int):
    ...
    curr_ch2:int = 0
    curr_comch:int = 0
    waehrung = guest = artikel = htparam = res_line = zimmer = arrangement = reservation = bill = zimkateg = segment = reslin_queasy = guest_pr = pricecod = argt_line = fixleist = None
    ...
    def generate_output():
        nonlocal msg_str, msg_warning, cl_list_list, currency_list_list, sum_list_list, s_list_list, argt_list_list, exchg_rate, frate, post_it, total_rev, rm_rate, lvcarea, loop2, j, m, curr_code, curr_rate, curr_local, curr_bfast, curr_lodge, curr_lunch, curr_dinner, curr_misc, curr_fcost, curr_trev, curr_pax, curr_com, curr_rm, curr_adult, curr_ch1, curr_ch2, curr_comch, waehrung, guest, artikel, htparam, res_line, zimmer, arrangement, reservation, bill, zimkateg, segment, reslin_queasy, guest_pr, pricecod, argt_line, fixleist
