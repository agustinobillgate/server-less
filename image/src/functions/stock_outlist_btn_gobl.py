...
def stock_outlist_btn_gobl(trans_code:str, from_grp:int, mi_alloc:bool, mi_article:bool, mi_docu:bool, mi_date:bool, mattype:int, from_lager:int, to_lager:int, from_date:date, to_date:date, from_art:int, to_art:int, show_price:bool, cost_acct:str, deptno:int):
    ...
    str_list = s_list = gl_acct1 = usr = None
    ...
    def generate_output():
        ...
        nonlocal str_list, s_list, gl_acct1, usr
    ...
    def create_list_trans():
        ...
        for l_lager in db_session.query(L_lager).all():
            ...
            l_op_obj_list = []
            for l_op, l_ophdr, gl_acct, l_artikel in db_session.query(L_op, L_ophdr, Gl_acct, L_artikel).join(L_ophdr,(func.lower(L_ophdr.op_typ) == "STT") &  (L_ophdr.lscheinnr == L_op.lscheinnr) &  (L_ophdr.fibukonto != "")).join(Gl_acct,(Gl_acct.fibukonto == L_op.stornogrund)).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
                    (L_op.lager_nr == l_lager.lager_nr) &  (L_op.anzahl != 0) &  (L_op.op_art == 3) &  (func.lower(L_op.lscheinnr) == (trans_code).lower()) &  (L_op.loeschflag <= 1)).all():
    ...
    def create_list():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            l_op_obj_list = []
            for l_op, l_ophdr, gl_acct, l_artikel in db_session.query(L_op, L_ophdr, Gl_acct, L_artikel).join(L_ophdr,(func.lower(L_ophdr.op_typ) == "STT") &  (L_ophdr.lscheinnr == L_op.lscheinnr) &  (L_ophdr.fibukonto != "")).join(Gl_acct,(Gl_acct.fibukonto == L_op.stornogrund)).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
                    (L_op.lager_nr == l_lager.lager_nr) &  (L_op.datum >= from_date) &  (L_op.datum <= to_date) &  (L_op.artnr >= from_art) &  (L_op.artnr <= to_art) &  (L_op.anzahl != 0) &  (L_op.op_art == 3) &  (L_op.loeschflag <= 1)).all():
                if l_op._recid in l_op_obj_list:
                    continue
                else:
                    l_op_obj_list.append(l_op._recid)
