...
def gcfs_deletebl(deltype:int, sorttype:int, adr1:str, city:str, cntry:str, email:str, last_stay:date, min_sales:decimal, age_history:int):
    f_anz = 0
    d_anz = 0
    error_code:int = 0
    ...
    def del_deltype1():
        ...
        for guest in db_session.query(Guest).filter(
                (Guest.karteityp == sorttype) &  (Guest.gastnr > 0)).all():
            f_anz = f_anz + 1
            do_it = True
    ...
    def del_deltype2():
        ...
        for guest in db_session.query(Guest).filter(
            ...
            history = db_session.query(History).filter(
                    (History.gastnr == guest.gastnr)).first()
    ...
    def del_deltype4():
        ...
        Hbuff = History
        last_history = date_mdy(get_month(get_current_date()) , get_day(get_current_date()) , (get_year(get_current_date()) - age_history))

        history = db_session.query(History).filter(
                (History.gastnr > 0) &  (History.abreise < last_history)).first()
        while None != history:
            f_anz = f_anz + 1
