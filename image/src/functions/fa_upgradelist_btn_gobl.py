from functions.additional_functions import *
import decimal
from datetime import date
from models import Mathis, Fa_artikel

def fa_upgradelist_btn_gobl(fdate:date, tdate:date):
    q1_list_list = []
    mathis = fa_artikel = None

    mat_buff = q1_list = None

    q1_list_list, Q1_list = create_model("Q1_list", {"mathis_name":str, "mathis_asset":str, "location":str, "warenwert":decimal, "nr":int, "matbuff_name":str, "matbuff_asset":str, "deleted":date, "did":str, "recid_fa_artikel":int})

    Mat_buff = Mathis

    db_session = local_storage.db_session

    def generate_output():
        nonlocal q1_list_list, mathis, fa_artikel
        nonlocal mat_buff


        nonlocal mat_buff, q1_list
        nonlocal q1_list_list
        return {"q1-list": q1_list_list}

    fa_artikel_obj_list = []
    for fa_artikel, mathis, mat_buff in db_session.query(Fa_artikel, Mathis, Mat_buff).join(Mathis,(Mathis.nr == Fa_artikel.nr) &  (Mathis.flag == 2)).join(Mat_buff,(Mat_buff.nr == Fa_artikel.p_nr)).filter(
            (Fa_artikel.deleted >= fdate) &  (Fa_artikel.deleted <= tdate)).all():
        if fa_artikel._recid in fa_artikel_obj_list:
            continue
        else:
            fa_artikel_obj_list.append(fa_artikel._recid)


        q1_list = Q1_list()
        q1_list_list.append(q1_list)

        q1_list.mathis_name = mathis.name
        q1_list.mathis_asset = mathis.asset
        q1_list.location = mathis.location
        q1_list.warenwert = fa_artikel.warenwert
        q1_list.nr = mat_buff.nr
        q1_list.matbuff_name = mat_buff.name
        q1_list.matbuff_asset = mat_buff.asset
        q1_list.deleted = fa_artikel.deleted
        q1_list.did = fa_artikel.did
        q1_list.recid_fa_artikel = fa_artikel._recid

    return generate_output()