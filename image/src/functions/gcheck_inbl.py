...
def gcheck_inbl(input_resnr:int, ci_date:date):
    ...
    def disp_arlist():
        ...
        res_line_obj_list = []
        for res_line, waehrung, zimkateg, guest, reservation in db_session.query(Res_line, Waehrung, Zimkateg, Guest, Reservation).join(Waehrung,(Waehrungsnr == Res_line.betriebsnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Guest,(Guest.gastnr == Res_line.gastnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
                ((Res_line.resstatus <= 5) |  (Res_line.resstatus == 11)) &  (Res_line.resnr == input_resnr) &  (Res_line.ankunft == ci_date)).all():
            ...
            gcheck_in.zipreis = res_line.zipreis
