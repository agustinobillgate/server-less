...
def closeyear2bl(curr_yr:int, curr_date:date, user_init:str):
    ...
    while None != gl_acct:
        ...
        gl_acc1 = db_session.query(Gl_acc1).first()


        gl_acct = db_session.query(Gl_acct).first()
    last_2yr = date_mdy(1, 1, (curr_yr - 1))
    ...
    while None != gl_jouhdr:
        ...
        gl_jhdrhis = db_session.query(Gl_jhdrhis).first()
    ...
    if htparam.flogical:
        ...
        gl_acct1 = db_session.query(Gl_acct1).first()

        gbuff = db_session.query(Gbuff).first()

        gbuff1 = db_session.query(Gbuff1).first()
