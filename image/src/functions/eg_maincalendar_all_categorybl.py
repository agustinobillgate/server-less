...
def eg_maincalendar_all_categorybl(all_room:bool, tcategory:[Tcategory], tlocation:[Tlocation], troom:[Troom]):
    tmaintask_list = []
    queasy = eg_property = eg_location = None

    tproperty = troom = tlocation = tcategory = tmaintask = qbuff = comcategory = comlocat = commain = comroom = ques = None
    ...
    def generate_output():
        nonlocal tmaintask_list, queasy, eg_property, eg_location
        nonlocal qbuff, comcategory, comlocat, commain, comroom, ques


        nonlocal tproperty, troom, tlocation, tcategory, tmaintask, qbuff, comcategory, comlocat, commain, comroom, ques
        nonlocal tproperty_list, troom_list, tlocation_list, tcategory_list, tmaintask_list
        return {"tmaintask": tmaintask_list}
