...
def eg_maincalendar_all_maintaskbl(all_room:bool, tlocation:[Tlocation], tmaintask:[Tmaintask], troom:[Troom]):
    tproperty_list = []
    eg_property = queasy = eg_location = None

    tproperty = troom = tmaintask = tlocation = qbuff = comlocat = commain = comroom = ques = None
    ...
    def generate_output():
        nonlocal tproperty_list, eg_property, queasy, eg_location
        nonlocal qbuff, comlocat, commain, comroom, ques


        nonlocal tproperty, troom, tmaintask, tlocation, qbuff, comlocat, commain, comroom, ques
        nonlocal tproperty_list, troom_list, tmaintask_list, tlocation_list
        return {"tproperty": tproperty_list}
