...
def prepare_if_country1bl(fl_run_proc:bool):
    ...
    def cleanup_zonelist():
        ...
        if not parameters or not(parameters.progname.lower()  == ("if-internal").lower()  and parameters.section.lower()  == ("dcode").lower()  and parameters.varname > ""):
            parameters = db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == ("if-internal").lower()) &  (func.lower(Parameters.section) == ("dcode").lower()) &  (Parameters.varname > "")).first()

        if parameters:
            for parameters in db_session.query(Parameters).filter(
                    (func.lower(Parameters.progname) == ("if-internal").lower()) &  (func.lower(Parameters.section) == ("dcode").lower()) &  (Parameters.varname > "")).order_by(Parameters._recid).all():
    ...
    def create_costlist():
        ...
        for parameters in db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == ("if-internal").lower()) &  (func.lower(Parameters.section) == ("zone").lower()) &  (Parameters.varname > "")).order_by(Parameters._recid).all():
            cost_list = Cost_list()
    ...
    def create_zonelist():
        ...
        m:int = 1
        n:int = 0
        ifname:str = "if-internal"

        if substring(proversion(), 0, 1) >= ("9").lower()  or substring(proversion(), 0, 1) == ("1").lower() :
            ifname = "interface"

        for parameters in db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == (ifname).lower()) &  (func.lower(Parameters.section) == ("dcode").lower()) &  (Parameters.varname > "")).order_by(Parameters._recid).all():
            zone_list = Zone_list()
    ...
    for parameters in db_session.query(Parameters).filter(
            (func.lower(Parameters.progname) == ("interface").lower()) &  (func.lower(Parameters.section) == ("zone").lower())).order_by(Parameters._recid).all():
