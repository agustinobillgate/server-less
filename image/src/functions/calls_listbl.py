...
def calls_listbl(case_type:int, last_sort:int, from_date:date, to_date:date, from_ext:str, to_ext:str, stattype:int, price_decimal:int, double_currency:bool, fr_number:str, to_number:str, dialed_nr:str):
    ...
    def create_list():
        ...
        nonlocal amount1, amount2, tot_pulse, ext_amt1, ext_amt2, i, d, last_ext, prstr, calls, bediener
        nonlocal case_type, last_sort, from_date, to_date, from_ext, to_ext, stattype, price_decimal, double_currency, fr_number, to_number, dialed_nr
        nonlocal str_list
        ...
        if last_sort == 1:
            ...
            if from_date == to_date and from_ext.lower()  == (to_ext).lower() :
                ...
                if from_ext.lower()  == ("0").lower()  and to_ext.lower()  == ("99999").lower() :
                    ...
                    for calls in db_session.query(Calls).filter(
                            (Calls.key == 1) &  (Calls.buchflag == stattype) &  (func.lower(Calls.nebenstelle) >= (from_ext).lower()) &  (func.lower(Calls.nebenstelle) <= (to_ext).lower()) &  (Calls.datum == from_date) &  (Calls.zeit >= 0)).order_by(Calls.zeit.desc()).all():
                        create_record()
                ...
                for d in date_range(to_date,from_date) :
                    for calls in db_session.query(Calls).filter(
                            (Calls.key == 1) &  (Calls.buchflag == stattype) &  (func.lower(Calls.nebenstelle) >= (from_ext).lower()) &  (func.lower(Calls.nebenstelle) <= (to_ext).lower()) &  (Calls.datum == d) &  (Calls.zeit >= 0)).order_by(Calls.zeit.desc()).all():
                        create_record()
