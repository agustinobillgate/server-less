...
def disc_listbl(to_date:date):
    ...
    for hoteldpt in db_session.query(Hoteldpt).all():
        ...
        for artikel in db_session.query(Artikel).filter(
                (Artikel.departement == hoteldpt.num) &  (Artikel.artart == 0) &  (Artikel.endkum == ekumnr)).all():
            art_list = Art_list()
    ...
    for art_list in query(art_list_list):
        ...
        for umsatz in db_session.query(Umsatz).filter(
                (Umsatz.artnr == art_list.artnr) &  (Umsatz.departement == art_list.dept) &  (Umsatz.datum >= from_date) &  (Umsatz.datum <= to_date)).all():

            if umsatz.datum == to_date:
                disc_list1.day_disc = - umsatz.betrag
                dnet = dnet - umsatz.betrag

            if get_month(umsatz.datum) == mm:
                disc_list1.mtd_disc = disc_list1.mtd_disc - umsatz.betrag
                mnet = mnet - umsatz.betrag
            disc_list1.ytd_disc = disc_list1.ytd_disc - umsatz.betrag
            ynet = ynet - umsatz.betrag

    for disc_list1 in query(disc_list1_list):

        if dnet != 0:
            disc_list1.day_percent = disc_list1.day_disc / dnet * 100

        if mnet != 0:
            disc_list1.mtd_percent = disc_list1.mtd_disc / mnet * 100

        if ynet != 0:
            disc_list1.ytd_percent = disc_list1.ytd_disc / ynet * 100
    disc_list1 = Disc_list1()
    disc_list1_list.append(disc_list1)
    ...
    disc_list1.day_disc = dnet

    if dnet != 0:
        disc_list1.day_percent = 100
    disc_list1.mtd_disc = mnet

    if mnet != 0:
        disc_list1.mtd_percent = 100
    disc_list1.ytd_disc = ynet

    if ynet != 0:
        disc_list1.ytd_percent = 100
