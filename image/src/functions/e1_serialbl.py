...
def e1_serialbl(inp_serial:str):
    ...
    def extent_vhp_license_date():
        ...
        if get_month(get_current_date()) == 2 and get_day(get_current_date()) >= 28:
            max_ext_date = date_mdy(2, 28, get_year(get_current_date()) + 3)


        else:
            max_ext_date = date_mdy(get_month(get_current_date()) , get_day(get_current_date()) , get_year(get_current_date()) + 3)
    ...
    def encode_serial_str(in_str:str, prefix:bool):
        ...
        for len_ in range(1,len(in_str)  + 1) :
            len1 = (13 * len_) % 5 + 6
            out_str = out_str + chr (ord(substring(in_str, len_ - 1, 1)) + j - len1)
    ...
    def char_number(in_char:str):
        ...
        if ord(substring(in_char, 0, 1)) > 96 and ord(substring(in_char, 0, 1)) < 123:
            out_int = ord(substring(in_char, 0, 1)) - 60

        elif ord(substring(in_char, 0, 1)) > 64 and ord(substring(in_char, 0, 1)) < 91:
            out_int = ord(substring(in_char, 0, 1)) - 64

        elif ord(substring(in_char, 0, 1)) > 48 and ord(substring(in_char, 0, 1)) < 58:
            out_int = ord(substring(in_char, 0, 1)) - 21
