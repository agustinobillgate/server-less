...
def akt_sales1_disp_allbl(aktnr:int):
    ...
    q2_list_list, Q2_list = create_model_like(Akt_line, {"recid_akt_line":int, "akt_code_bezeich":str, "bediener_username":str})
    ...
    for akthdr, guest, akt_code, akt_kont in db_session.query(Akthdr, Guest, Akt_code, Akt_kont).join(Guest,(Guest.gastnr == Akthdr.gastnr)).join(Akt_code,(Akt_code.aktiongrup == 2) &  (Akt_code.aktionscode == Akthdr.stufe)).join(Akt_kont,(Akt_kont.gastnr == Akthdr.gastnr)).filter(
            (Akthdr.aktnr == aktnr) &  (Akthdr.flag != 0)).all():
    ...
    for akt_line, akt_code1, bediener in db_session.query(Akt_line, Akt_code1, Bediener).join(Akt_code1,(Akt_code1.aktionscode == Akt_line.aktionscode)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
            (Akt_line.aktnr == aktnr) &  (Akt_line.flag != 2)).all():
