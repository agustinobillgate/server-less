...
def res_list_1bl(case_type:int, from_date:date, to_date:date, resnr:int, gastnr:int, search_resno:int):
    ...
    res_listmember_list, Res_listmember = create_model("Res_listmember", {"name":str, "ankunft":date, "abreise":date, "zinr":str, "kurzbez":str, "zipreis":decimal, "arrangement":str, "erwachs":int, "gratis":int, "zimmeranz":int, "anztage":int, "changed_id":str, "changed":str, "gastnr":int, "resstatus":int, "zikatnr":int, "resnr":int, "bemerk":str, "active_flag":int, "l_zuordnung":[int], "reslinnr":int, "res_recid":int, "vip":str, "nat":str, "rate_code":str, "segment":str, "sp_req":str, "usr_id":str, "ratecode":str, "voucher_no":str, "email":str, "phone_no":str, "sob":str})
        ...
        for res_line, zimkateg in db_session.query(Res_line, Zimkateg).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
            ...
            if reservation:
                ...
                sourccod = db_session.query(Sourccod).filter(
                        (Sourccod.source_code == reservation.resart) &  (Sourccod.betriebsnr == reservation.betriebsnr)).first()
