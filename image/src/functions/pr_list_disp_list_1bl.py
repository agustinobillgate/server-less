...
def pr_list_disp_list_1bl(char1:str, billdate:date, from_date:date, to_date:date, outstand_flag:bool, expired_flag:bool, approve_flag:bool, reject_flag:bool):
    s_list_list = []
    ...
    def disp_list(pr_nr:str):
        ...
        if pr_nr != "":
            ...
            for l_order, l_artikel in db_session.query(L_order, L_artikel).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                    (L_order.docu_nr == l_orderhdr.docu_nr) &  (L_order.loeschflag <= 1) &  (L_order.pos > 0) &  (L_order.lief_nr == 0)).all():
                ...
                l_op = db_session.query(L_op).filter(
                        (L_op.op_art == 1) &  (L_op.artnr == l_artikel.artnr)).first()

                if l_op:
                    s_list.last_pprice = l_artikel.ek_letzter

                l_pprice = db_session.query(L_pprice).filter(
                        (L_pprice.artnr == l_artikel.artnr)).first()

                if l_pprice:

                    t_lieferant = db_session.query(T_lieferant).filter(
                            (T_lieferant.lief_nr == l_pprice.lief_nr)).first()
                    s_list.last_pdate = l_pprice.bestelldatum
                    s_list.last_pprice = l_pprice.einzelpreis
        ...
        for l_orderhdr in db_session.query(L_orderhdr).filter(
            ...
            if do_it:
                ...
                l_order_obj_list = []
                for l_order, l_artikel in db_session.query(L_order, L_artikel).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                            (L_order.docu_nr == l_orderhdr.docu_nr) &  (L_order.pos > 0) &  (L_order.lief_nr == 0)).all():
                    if l_order._recid in l_order_obj_list:
                        continue
                    else:
                        l_order_obj_list.append(l_order._recid)
