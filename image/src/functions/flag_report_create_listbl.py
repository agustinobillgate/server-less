...
def flag_report_create_listbl(user_init:str, f_date:date, t_date:date, sorttype:int, alldept_flag:bool):
    ...
    def create_list():
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == "flag") &  (Reslin_queasy.date1 >= f_date)).all():
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == "flag") &  (Reslin_queasy.date2 >= f_date)).all():
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == "flag") &  (Reslin_queasy.date3 >= f_date)).all():
    ...
    def check_do_it(inp_char:str, todate:date, done_flag:bool):
        ...
        if re.match(".*," + usrdept_str + ".*",dept_str):
