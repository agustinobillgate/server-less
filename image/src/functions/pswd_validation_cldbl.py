...
def pswd_validation_cldbl(user_name:str, inp_pswd:str):
    print("pswd_validation_cldbl, U/P:", user_name, inp_pswd)
    pswd_ok = False
    pswd_level = 0
    heute:date = None
    sindata_pswd:str = ""
    ...
    def check_pswd_strength(ipass:str):
        nonlocal pswd_ok, pswd_level, heute, sindata_pswd, dd_str, mm_str, yy_str, nr, dd, tot_digit, tot_char, curr_i, month_str, bediener
        nonlocal user_name, inp_pswd

        pswd_level = 0
        upper_case:List[str] = ["A", "B", "C", "D", "E", "F", "G", "H", "i", "j", "K", "L", "M", "N", "O", "P", "Q", "R", "s", "T", "U", "V", "W", "X", "Y", "Z"] 
        lower_case:List[str] = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"] 
    ...
    def decode_usercode():
        ...
        if not usr or not(usr.username.lower()  == user_name.lower()  and usr.flag == 0 and usr.betriebsnr == 1):
        ...
        while None != usr and not found:
            ...
            if passwd.lower()  == inp_pswd.lower() :
                ...
                usr = db_session.query(Usr).filter(
                        (func.lower(Usr.username) == user_name.lower()) &  (Usr.flag == 0) &  (Usr.betriebsnr == 1)).filter(Usr._recid > curr_recid).first()
    ...
    for curr_i in range(1,len(inp_pswd)  + 1) :
        if substring(inp_pswd, curr_i - 1, 1) >= "0"  and substring(inp_pswd, curr_i - 1, 1) <= "9" :
            tot_digit = tot_digit + 1

        elif substring(inp_pswd, curr_i - 1, 1) >= "a"  and substring(inp_pswd, curr_i - 1, 1) <= "z" :
            tot_char = tot_char + 1
        else:
