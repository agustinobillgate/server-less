from functions.additional_functions import *
import decimal
from datetime import date
from models import Queasy

def prepare_config_setup_save_horekabl():
    custid = ""
    username = ""
    password = ""
    url_push = ""
    url_notif = ""
    storage = 0
    push_dml = False
    art_deposit = 0
    api_key = ""
    supplier = ""
    coa_apdeposit = ""
    coa_apclearence = ""
    artikel_apclearence = 0
    usr_id = 0
    tlist_list = []
    loopi:int = 0
    str:str = ""
    queasy = None

    tlist = None

    tlist_list, Tlist = create_model("Tlist", {"datum":date, "depart":int, "approved":bool, "send_horeka":bool, "user_name":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal custid, username, password, url_push, url_notif, storage, push_dml, art_deposit, api_key, supplier, coa_apdeposit, coa_apclearence, artikel_apclearence, usr_id, tlist_list, loopi, str, queasy


        nonlocal tlist
        nonlocal tlist_list
        return {"custid": custid, "username": username, "password": password, "url_push": url_push, "url_notif": url_notif, "storage": storage, "push_dml": push_dml, "art_deposit": art_deposit, "api_key": api_key, "supplier": supplier, "coa_apdeposit": coa_apdeposit, "coa_apclearence": coa_apclearence, "artikel_apclearence": artikel_apclearence, "usr_id": usr_id, "tlist": tlist_list}

    queasy = db_session.query(Queasy).filter(
            (Queasy.key == 253)).first()

    if queasy:
        for loopi in range(1,num_entries(queasy.char1, ";")  + 1) :
            str = entry(loopi - 1, queasy.char1, ";")

            if substring(str, 0, 8) == "$custid$":
                custid = substring(str, 8)

            elif substring(str, 0, 10) == "$username$":
                username = substring(str, 10)

            elif substring(str, 0, 10) == "$password$":
                password = substring(str, 10)

            elif substring(str, 0, 9) == "$urlpush$":
                url_push = substring(str, 9)

            elif substring(str, 0, 10) == "$urlnotif$":
                url_notif = substring(str, 10)

            elif substring(str, 0, 9) == "$storage$":
                storage = to_int(substring(str, 9))

            elif substring(str, 0, 9) == "$pushdml$":
                push_dml = logical(substring(str, 9))

            elif substring(str, 0, 12) == "$artdeposit$":
                art_deposit = to_int(substring(str, 12))

            elif substring(str, 0, 8) == "$apikey$":
                api_key = substring(str, 8)

            elif substring(str, 0, 10) == "$supplier$":
                supplier = substring(str, 10)

            elif substring(str, 0, 11) == "$apdeposit$":
                coa_apdeposit = substring(str, 11)

            elif substring(str, 0, 13) == "$apclearence$":
                coa_apclearence = substring(str, 13)

            elif substring(str, 0, 11) == "$artikelap$":
                artikel_apclearence = to_int(substring(str, 11))

            elif substring(str, 0, 8) == "$userID$":
                usr_id = to_int(substring(str, 8))

    for queasy in db_session.query(Queasy).filter(
            (Queasy.key == 254) &  (Queasy.logi2) &  (Queasy.logi3)).all():
        tlist = Tlist()
        tlist_list.append(tlist)

        tlist.datum = queasy.date1
        tlist.depart = queasy.number1
        tlist.approved = queasy.logi1
        tlist.send_horeka = queasy.logi2
        tlist.user_name = queasy.char1

    return generate_output()