...
def availability1bl(pvilanguage:int, printer_nr:int, call_from:int, txt_file:str, curr_date:date, incl_tentative:bool):
    ...
    rmcat_list = room_list = sum_list = tmp_resline = tmp_extra = temp_art = zkbuff = rlist = gmember = None
    ...
    def generate_output():
        ...
        nonlocal rmcat_list, room_list, sum_list, tmp_resline, tmp_extra, temp_art, zkbuff, rlist, gmember
    ...
    def create_browse():
        ...
        for i in range(1,21 + 1) :
            ...
            for res_line in db_session.query(Res_line).filter(
                    (Res_line.active_flag <= 1) &  (Res_line.resstatus <= 6) &  (Res_line.resstatus != 4) &  (Res_line.ankunft <= datum) &  (Res_line.abreise > datum) &  (Res_line.l_zuordnung[2] == 0)).all():
                do_it = True
