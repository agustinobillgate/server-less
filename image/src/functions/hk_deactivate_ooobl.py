...
def hk_deactivate_ooobl(userinit:str, rec_id:int):
    ...
    if bediener and outorder:
        ...
        for queasy in db_session.query(Queasy).filter(
                (Queasy.key == 195) &  
                (Queasy.char1 == "ooo;room == " + outorder.zinr + ";from == " + to_string(get_day(outorder.gespstart) , "99") + "/" + to_string(get_month(outorder.gespstart) , "99") + "/" + to_string(get_year(outorder.gespstart) , "9999") + ";to == " + to_string(get_day(outorder.gespende) , "99") + "/" + to_string(get_month(outorder.gespende) , "99") + "/" + to_string(get_year(outorder.gespende) , "9999"))
                ).all():
        ...
        get_output(deactivate_ooo2_cldbl(user_nr, oos_flag, ci_date, ooo_list2))
