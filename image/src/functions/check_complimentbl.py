...
def check_complimentbl(pvilanguage:int, resnr:int, reslinnr:int, gastnr:int, datum:date, marknr:int, zikatnr:int, argt:str, qty:int, rate:decimal):
    ...
    if new_contrate:
        ...
        pricecod = db_session.query(Pricecod).filter(
                (func.lower(Pricecod.code) == (contcode).lower()) &  (Pricecod.marknr == marknr) &  (Pricecod.argtnr == arrangement.argtnr) &  (Pricecod.zikatnr == zikatnr) &  (Pricecod.datum >= Pricecod.startperiode) &  (Pricecod.datum <= Pricecod.endperiode)).first()
