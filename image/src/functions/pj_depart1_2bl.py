...
def pj_depart1_2bl(pvilanguage:int, case_type:int, disptype:int, fdate:date, tdate:date, froom:str, troom:str, setup_list:[Setup_list]):
    tot_rm = 0
    tot_a = 0
    tot_c = 0
    tot_co = 0
    ...
    str_list = s_list = cl_list = setup_list = gmember = None
    ...
    def generate_output():
        ...
        nonlocal str_list, s_list, cl_list, setup_list, gmember
    ...
    def create_departure0():
        ...
        if disptype == 1:
            ...
            for res_line, zimkateg, reservation, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                    ((Res_line.resstatus <= 2) |  (Res_line.resstatus == 5) |  (Res_line.resstatus == 6) |  (Res_line.resstatus == 13) |  (Res_line.resstatus == 8)) &  (Res_line.abreise >= fdate) &  (Res_line.abreise <= tdate) &  (((func.lower(Res_line.zinr) >= (froom).lower()) &  (func.lower(Res_line.zinr) <= troom)) |  ((func.lower(Res_line.zinr) >= (froom).lower() )))).all():
