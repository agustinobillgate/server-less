...
def eg_maincalendardel_gobl(from_date:date, to_date:date, user_init:str, all_room:bool):
    groupid = 0
    engid = 0
    p_992 = False
    maintain_list = []
    ...
    t_zimmer = maintain = tproperty = troom = tmaintask = tpic = tstatus = dept_link = tlocation = tcategory = t_eg_maintain = qbuff = qbuff1 = comcategory = comlocat = commain = comroom = ques = quesbuff = quesbuff1 = None
    ...
    def generate_output():
        ...
        nonlocal qbuff, qbuff1, comcategory, comlocat, commain, comroom, ques, quesbuff, quesbuff1


        nonlocal t_zimmer, maintain, tproperty, troom, tmaintask, tpic, tstatus, dept_link, tlocation, tcategory, t_eg_maintain, qbuff, qbuff1, comcategory, comlocat, commain, comroom, ques, quesbuff, quesbuff1
    ...
    def create_maintain():
        ...
        for qbuff in db_session.query(Qbuff).filter(
                (Qbuff.delete_flag) &  (Qbuff.cancel_date >= from_date) &  (Qbuff.cancel_date <= to_date)).all():
            ...
            if qbuff.propertynr != 0:
                ...
                if tproperty:
                    ...
                    quesbuff = db_session.query(Quesbuff).filter(
                            (Quesbuff.key == 133) &  (Quesbuff.number1 == tproperty.pmain_nr)).first()
