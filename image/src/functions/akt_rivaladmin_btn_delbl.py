...
def akt_rivaladmin_btn_delbl(pvilanguage:int, aktionscode:int, aktiongrup:int, bezeich:str):
    ...
    akthdr = db_session.query(Akthdr).filter(
            (Akthdr.mitbewerber[0] == aktionscode) |  (Akthdr.mitbewerber[1] == aktionscode) |  (Akthdr.mitbewerber[2] == aktionscode)).first()
    ...
    if akthdr:
        ...
        akt_code = db_session.query(Akt_code).filter(
                (Akt_code.aktiongrup == aktiongrup) &  (Akt_code.aktionscode == aktionscode) &  (func.lower(Akt_code.(bezeich).lower()) == (bezeich).lower())).first()
