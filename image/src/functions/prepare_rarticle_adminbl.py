...
def prepare_rarticle_adminbl(dept:int):
    ...
    # Line 36, dipecah bbrp step
    query_results = db_session.query(H_artikel).filter((H_artikel.departement == dept)).all()
    zwkum_list = set(h_artikel.zwkum for h_artikel in query_results)
    departement_list = set(h_artikel.departement for h_artikel in query_results)
    wbuff_list_filtered = query(wbuff_list,(lambda wbuff:wbuff.zknr in zwkum_list and wbuff.departement in departement_list))
    wbuff_list_indexed = indexed_list(wbuff_list_filtered, ["zknr", "departement"])
    for h_artikel in query_results:
