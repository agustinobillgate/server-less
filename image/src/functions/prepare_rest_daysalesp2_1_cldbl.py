...
def prepare_rest_daysalesp2_1_cldbl():
    ...
    if htparam.fchar != "":
        for i in range(1,num_entries(htparam.fchar, ";")  + 1) :
            ...
            if substring(str_tmp, 0, 1) == "D":
                dpt_str = substring(str_tmp, 1, (len(str_tmp) - 1))
            elif substring(str_tmp, 0, 1) == "A":
                art_str = substring(str_tmp, 1, (len(str_tmp) - 1))
            elif substring(str_tmp, 0, 1) == "G":
                oth_str = substring(str_tmp, 1, (len(str_tmp) - 1))
    else:
        err_flag = 1
    ...
    for i in range(1,num_entries(dpt_str, ",")  + 1) :
        ...
        if to_int(entry(i - 1, dpt_str, ",")) != 0:
            ...
            hoteldpt = db_session.query(Hoteldpt).filter(
                    (Hoteldpt.num == to_int(entry(i - 1, dpt_str, ",")))).first()
    ...
    for i in range(1,anzahl + 1) :
        ...
        for artikel in db_session.query(Artikel).filter(
                (Artikel.artnr == artnr_list[i - 1])).all():
            buf_art = Buf_art()
    ...
    if htparam.finteger > 0:
        ...
        h_artikel = db_session.query(H_artikel).filter(
                (H_artikel.artnr == htparam.finteger) &  (H_artikel.artart == 6)).first()

        if h_artikel:
            voucher_art = h_artikel.artnr

            h_artikel_obj_list = []
            for h_artikel, artikel in db_session.query(H_artikel, Artikel).join(Artikel,(Artikel.artnr == H_Artikel.artnrfront) &  (Artikel.departement == 0) &  (Artikel.pricetab)).filter(
                    (H_artikel.artart == 6)).all():
