# --- update Rd
# add: lvCAREA =  "PJ-arrive"
# update: zimkateg.bezeich => zimkateg.bezeichnung
# update: guest.bemerkung => guest.bemerkungung
...
def pj_arrive1_webbl(pvilanguage:int, from_date:date, to_date:date, ci_date:date, disptype:int, incl_tentative:bool, sorttype:int, comment_type:int, incl_accompany:bool, split_rsv_print:bool, total_flag:bool):
    tot_rm = 0
    tot_a = 0
    tot_c = 0
    tot_co = 0
    ...
    cl_list_list, Cl_list = create_model("Cl_list", {"ci_id":str, "stat_flag":str, "datum":date, "flag":int, "nr":int, "vip":str, "gastnr":int, "resnr":int, "name":str, "groupname":str, "zimmeranz":int, "rmno":str, "qty":int, "zipreis":str, "arrival":str, "depart":str, "rmcat":str, "kurzbez":str, "bezeich":str, "a":int, "c":int, "co":int, "pax":str, "nat":str, "nation":str, "argt":str, "company":str, "flight":str, "etd":str, "stay":int, "segment":str, "rate_code":str, "eta":str, "email":str, "bemerk":str, "bemerk01":str, "bemerk02":str, "bemerk03":str, "bemerk04":str, "bemerk05":str, "bemerk06":str, "bemerk07":str, "bemerk08":str, "spreq":str, "memberno":str, "resdate":str, "sob":str, "created_by":str, "ci_time":str, "city":str, "res_stat":int, "res_stat_str":str, "nation2":str, "birthdate":date, "rsv_comment":str, "other_comment":str, "g_comment":str, "zinr_bez":str, "flag_guest":int})
    t_cl_list_list, T_cl_list = create_model("T_cl_list", {"ci_id":str, "stat_flag":str, "datum":date, "flag":int, "nr":int, "vip":str, "gastnr":int, "resnr":int, "name":str, "groupname":str, "zimmeranz":int, "rmno":str, "qty":int, "zipreis":str, "arrival":str, "depart":str, "rmcat":str, "kurzbez":str, "bezeich":str, "a":int, "c":int, "co":int, "pax":str, "nat":str, "nation":str, "argt":str, "company":str, "flight":str, "etd":str, "stay":int, "segment":str, "rate_code":str, "eta":str, "email":str, "bemerk":str, "bemerk01":str, "bemerk02":str, "bemerk03":str, "bemerk04":str, "bemerk05":str, "bemerk06":str, "bemerk07":str, "bemerk08":str, "spreq":str, "memberno":str, "resdate":str, "sob":str, "created_by":str, "ci_time":str, "phonenum":str, "member_typ":str, "repeat_guest":str, "night":int, "city":str, "res_stat":int, "res_stat_str":str, "nation2":str, "birthdate":date, "rsv_comment":str, "other_comment":str, "g_comment":str, "zinr_bez":str, "flag_guest":int})
    s_list_list, S_list = create_model("S_list", {"rmcat":str, "bezeich":str, "nat":str, "anz":int, "adult":int, "proz":decimal, "child":int})
    t_list_list, T_list = create_model("T_list", {"gastnr":int, "company":str, "counter":str, "int_counter":int, "anzahl":int, "erwachs":int, "kind":int})
    ...
    def generate_output():
        ...
        nonlocal setup_list_list, cl_list_list, t_cl_list_list, s_list_list, t_list_list
        return {"tot_rm": tot_rm, "tot_a": tot_a, "tot_c": tot_c, "tot_co": tot_co, "s-list": s_list_list, "table": table}

    def create_arrival(curr_date:date):

        nonlocal tot_rm, tot_a, tot_c, tot_co, s_list_list, table, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, ol_gastnr, sms_gastnr, wg_gastnr, indi_gastnr, nr, vip_flag, i, dummy_flag, do_it, last_gcf, tentres, all_remark, stat_list, guest, htparam, zimkateg, reservation, sourccod, res_line, guestseg, segment, zimmer, nation, mc_guest, mc_types, queasy, reslin_queasy, history, paramtext
        nonlocal pvilanguage, from_date, to_date, ci_date, disptype, incl_tentative, sorttype, comment_type, incl_accompany, split_rsv_print, total_flag
        nonlocal gmember, gbuff
        ...
        if sorttype == 1:
            ...
            res_line_obj_list = []
            for res_line, zimkateg, reservation, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                ...
                if not incl_accompany:
                    ...
                    if not cl_list or not(cl_list.rmno == res_line.zinr and cl_list.resnr == res_line.resnr and date_mdy(cl_list.arrival) == res_line.ankunft and to_decimal(cl_list.zipreis) == 0 and cl_list.co < 1):
    ...
    def add_cllist():
        ...
        if not setup_list or not(setup_list.nr == res_line.setup + 1):
            setup_list = query(setup_list_list, filters=(lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
        nr = nr + 1
        vip_flag = ""

        if not guestseg or not(guestseg.gastnr == gmember.gastnr and (guestseg.segmentcode == vipnr1 or guestseg.segmentcode == vipnr2 or guestseg.segmentcode == vipnr3 or guestseg.segmentcode == vipnr4 or guestseg.segmentcode == vipnr5 or guestseg.segmentcode == vipnr6 or guestseg.segmentcode == vipnr7 or guestseg.segmentcode == vipnr8 or guestseg.segmentcode == vipnr9)):
        ...
        if guestseg:
            ...
            if not segment or not(segment.segmentcode == guestseg.segmentcode):
        ...
        if cl_list.nat == "":
            ...
            if not nation or not(nation.kurzbez == cl_list.nat):
        ...
        if not nation or not(nation.kurzbez == gmember.nation2 and nation.natcode > 0):
            nation = db_session.query(Nation).filter(
        ...
        if not mc_guest or not(mc_guest.gastnr == gmember.gastnr):
        ...
        if mc_guest:
            ...
            if not mc_types or not(mc_types.nr == mc_guest.nr):
        ...
        if comment_type == 0:
            ...
            if not split_rsv_print:
                ...
                if not gbuff or not(gbuff.gastnr == res_line.gastnrmember):
                ...
                if not queasy or not(queasy.key == 267 and queasy.number1 == res_line.resnr and queasy.number2 == res_line.reslinnr):
                    queasy = db_session.query(Queasy).filter(
                        (Queasy.key == 267) &  (Queasy.number1 == res_line.resnr) &  (Queasy.number2 == res_line.reslinnr)).first()
            ...
            if dummy_flag or guest.bemerkung == "":
                ...
                if gbuff:
                    for i in range(1,len(gbuff.bemerkung)  + 1) :

                        if substring(gbuff.bemerkung, i - 1, 1) == chr (10):
                            cl_list.bemerk = cl_list.bemerk + " "
                        else:
                            cl_list.bemerk = cl_list.bemerk + substring(gbuff.bemerkung, i - 1, 1)
            else:
                for i in range(1,len(guest.bemerkung)  + 1) :

                    if substring(guest.bemerkung, i - 1, 1) == chr (10):
                        cl_list.bemerk = cl_list.bemerk + " "
                    else:
        ...
        if not reslin_queasy or not(reslin_queasy.key.lower()  == ("specialRequest").lower()  and reslin_queasy.resnr == res_line.resnr and reslin_queasy.reslinnr == res_line.reslinnr):
    ...
    def create_summary():
        ...
        if (tot_a + tot_co) != 0:
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.nat != "")):
                ...
                if not nation or not(nation.kurzbez == s_list.nat):
                    nation = db_session.query(Nation).filter(
    ...
    def create_arrival1(curr_date:date):
        ...
        if sorttype == 1:
            ...
            res_line_obj_list = []
            for res_line, zimkateg, reservation, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                    ((Res_line.Res_line.Res_line.resstatus == 6) |  (Res_line.Res_line.resstatus == 8) |  (Res_line.Res_line.resstatus == 13)) &  (Res_line.ankunft == curr_date)).order_by(Reservation.name, Reservation.groupname, Res_line.name, Res_line.zinr).all():
                if res_line._recid in res_line_obj_list:
                    continue
                else:
                    res_line_obj_list.append(res_line._recid)
                ...
                if (res_line.ankunft == res_line.abreise) and res_line.resstatus == 8:
                    ...
                    if not history or not(history.resnr == res_line.resnr and history.reslinnr == res_line.reslinnr and history.gesamtumsatz > 0):
            ...
            res_line_obj_list = []
            for res_line, zimkateg, reservation, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                    ((Res_line.Res_line.Res_line.resstatus == 6) |  (Res_line.Res_line.resstatus == 8) |  (Res_line.Res_line.resstatus == 13)) &  (Res_line.ankunft == curr_date)).order_by(Res_line.zinr, Reservation.name, Reservation.groupname, Res_line.name).all():
                if res_line._recid in res_line_obj_list:
                    continue
                else:
                    res_line_obj_list.append(res_line._recid)
            ...
            res_line_obj_list = []
            for res_line, zimkateg, reservation, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                    ((Res_line.Res_line.Res_line.resstatus == 6) |  (Res_line.Res_line.resstatus == 8) |  (Res_line.Res_line.resstatus == 13)) &  (Res_line.ankunft == curr_date)).order_by(Reservation.resdat, Res_line.zinr).all():
                if res_line._recid in res_line_obj_list:
                    continue
                else:
                    res_line_obj_list.append(res_line._recid)
            ...
            res_line_obj_list = []
            for res_line, zimkateg, reservation, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                    ((Res_line.Res_line.Res_line.resstatus == 6) |  (Res_line.Res_line.resstatus == 8) |  (Res_line.Res_line.resstatus == 13)) &  (Res_line.ankunft == curr_date)).order_by(Res_line.arrangement, Reservation.name, Reservation.groupname, Res_line.name).all():
                if res_line._recid in res_line_obj_list:
                    continue
                else:
                    res_line_obj_list.append(res_line._recid)
            ...
            res_line_obj_list = []
            for res_line, zimkateg, reservation, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                    ((Res_line.Res_line.Res_line.resstatus == 6) |  (Res_line.Res_line.resstatus == 8) |  (Res_line.Res_line.resstatus == 13)) &  (Res_line.ankunft == curr_date)).order_by(Res_line.resstatus, Reservation.name, Reservation.groupname, Res_line.name).all():
                if res_line._recid in res_line_obj_list:
                    continue
                else:
                    res_line_obj_list.append(res_line._recid)
    ...
    def create_browse():
        ...
        for t_cl_list in query(t_cl_list_list, sort_by=[("nr",False)]):

            if total_flag and t_cl_list.gastnr > 0:

                if not t_list or not(t_list.gastnr == t_cl_list.gastnr):
    ...
    if not htparam or not(htparam.paramnr == 700):
    ...
    if not htparam or not(htparam.paramnr == 701):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 702):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 703):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 704):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 705):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 706):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 707):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 708):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 39):
        htparam = db_session.query(Htparam).filter(
            (Htparam.paramnr == 39)).first()
    ol_gastnr = htparam.finteger

    if not htparam or not(htparam.paramnr == 109):
        htparam = db_session.query(Htparam).filter(
            (Htparam.paramnr == 109)).first()
    wg_gastnr = htparam.finteger

    if not htparam or not(htparam.paramnr == 123):
        htparam = db_session.query(Htparam).filter(
            (Htparam.paramnr == 123)).first()
    indi_gastnr = htparam.finteger

    if not htparam or not(htparam.paramnr == 577):
        htparam = db_session.query(Htparam).filter(
