...
def sls_bystatus_btn_go_listbl(pvilanguage:int, next_date:date, to_date:date, all_flag:bool, usr_init:str):
    slstatus_list_list = []
    s_flag:[str] = ["", "", "", "", ""]
    lvcarea:str = "sls_bystatus"
    akt_code = guest = akthdr = akt_kont = None

    p_list = slstatus_list = akt_code1 = guest1 = akthdr1 = akt_kont1 = buf_akthdr = None

    p_list_list, P_list = create_model("P_list", {"pnr":int, "sflag":int, "pcomp":str, "pcont":str, "pname":str, "pntot":str, "pnam1":str, "pnam2":str, "pnam3":str, "pnam4":str, "pamt":decimal, "pamt1":decimal, "pamt2":decimal, "pamt3":decimal, "pamt4":decimal, "patot":decimal, "pamt_str":str, "stnr":int, "stage":str, "proz":str, "popen":date, "pfnsh":date, "pmain1":str, "pmain2":str, "pmain3":str, "reason":str, "refer":str, "pid":str, "pcid":str, "ctotal":int, "stat_flag":str})
    slstatus_list_list, Slstatus_list = create_model("Slstatus_list", {"pnr":int, "sflag":int, "pcomp":str, "pcont":str, "pname":str, "pntot":str, "pnam1":str, "pnam2":str, "pnam3":str, "pnam4":str, "pamt":decimal, "pamt1":decimal, "pamt2":decimal, "pamt3":decimal, "pamt4":decimal, "patot":decimal, "pamt_str":str, "stnr":int, "stage":str, "proz":str, "popen":date, "pfnsh":date, "pmain1":str, "pmain2":str, "pmain3":str, "reason":str, "refer":str, "pid":str, "pcid":str, "ctotal":int, "stat_flag":str})
    ...
    def generate_output():
        ...
        nonlocal akt_code1, guest1, akthdr1, akt_kont1, buf_akthdr


        nonlocal p_list, slstatus_list, akt_code1, guest1, akthdr1, akt_kont1, buf_akthdr
    ...
    def browse_open1():
        ...
        if all_flag:
            ...
            for i in range(1,4 + 1) :
                ...
                akthdr1 = db_session.query(Akthdr1).filter(
                        (Akthdr1.flag == i) &  (func.lower(Akthdr1.userinit) == (usr_init).lower())).first()
                ...
                if akthdr1:
                    ...
                    akthdr1_obj_list = []
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.flag == i) &  (func.lower(Akthdr1.userinit) == (usr_init).lower())).all():
                        if akthdr1._recid in akthdr1_obj_list:
                            continue
                        else:
                            akthdr1_obj_list.append(akthdr1._recid)
        ...
        if all_flag:
            ...
            for i in range(1,4 + 1) :
                ...
                if akthdr1:
                    ...
                    akthdr1_obj_list = []
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.flag == i) &  (Akthdr1.next_datum >= next_date) &  (Akthdr1.next_datum <= to_date)).all():
                        if akthdr1._recid in akthdr1_obj_list:
                            continue
                        else:
                            akthdr1_obj_list.append(akthdr1._recid)
            ...
            for i in range(1,4 + 1) :
                ...
                akthdr1 = db_session.query(Akthdr1).filter(
                        (Akthdr1.flag == i) &  (func.lower(Akthdr1.userinit) == (usr_init).lower()) &  (Akthdr1.next_datum >= next_date) &  (Akthdr1.next_datum <= to_date)).first()
                ...
                if akthdr1:
                    ...
                    akthdr1_obj_list = []
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.flag == i) &  (func.lower(Akthdr1.userinit) == (usr_init).lower()) &  (Akthdr1.next_datum >= next_date) &  (Akthdr1.next_datum <= to_date)).all():
                        if akthdr1._recid in akthdr1_obj_list:
                            continue
                        else:
                            akthdr1_obj_list.append(akthdr1._recid)
