    ...
    def detail_sixpm():
        ...
        for zimkateg in db_session.query(Zimkateg).all():
            ...
            for res_line in db_session.query(Res_line).filter(
                    (Res_line.active_flag == 0) &  (Res_line.resstatus == 2) &  (Res_line.zikatnr == zimkateg.zikatnr) &  (Res_line.ankunft <= datum) &  (Res_line.abreise > datum) &  (Res_line.l_zuordnung[2] == 0)).all():
                do_it = True
