...
def bqt_reportrev_create_listbl(from_date:date, to_date:date):
    ...
    def create_list():
        ...
        for bk_stat, bk_veran, guest in db_session.query(Bk_stat, Bk_veran, Guest).join(Bk_veran,(Bk_veran.veran_nr == Bk_stat.resnr)).join(Guest,(Guest.gastnr == bk_veran.gastnr)).filter(
                (Bk_stat.datum >= from_date) &  (Bk_stat.datum <= to_date)).all():
            ...
            if bk_stat:
                ...
                output_list.rm_rev = bk_stat.rm_rev
                output_list.fb_rev = bk_stat.fb_rev
                output_list.other_rev = bk_stat.other_rev
                output_list.tot_rev = bk_stat.rm_rev + bk_stat.fb_rev + bk_stat.other_rev
