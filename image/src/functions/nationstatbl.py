...
def nationstatbl(printer_nr:int, call_from:int, txt_file:str, from_month:str, hide_zero:bool, mi_arrival:bool, mi_comp:bool):
    room_list_list = []
    tot_all:int = 0
    from_date:date = None
    to_date:date = None
    ...
    room_list_list, Room_list = create_model("Room_list", {"name":str, "bezeich":str, "summe":int, "room":[int, 31], "proz":decimal})
    ...
    def create_browse():
        ...
        for nation in db_session.query(Nation).filter(
            ...
            for datum in range(from_date,to_date + 1) :
                ...
                for nationstat in db_session.query(Nationstat).filter(
                        (Nationstat.nationnr == nationnr) &  (Nationstat.datum == datum)).all():
        ...
        if tot_all > 0:
            ...
            for room_list in query(room_list_list, filters=(lambda room_list :room_list.summe != 0)):
                room_list.proz = room_list.summe / tot_all * 100
    ...
    def create_browse1():
        ...
        for nation in db_session.query(Nation).filter(
            ...
            for curr_datum in range(from_date,to_date + 1) :
                i = get_day(curr_datum)

                for nationstat in db_session.query(Nationstat).filter(
                        (Nationstat.nationnr == nationnr) &  (Nationstat.datum == curr_datum)).all():
