...
def arl_list_disp_arlist2bl(inp_gastnr:int, inp_grpname:str, ci_date:date, show_rate:bool, long_stay:int):
    arl_list_list = []
    ...
    def generate_output():
        ...
        nonlocal gbuff, gmember, arlbuff
    ...
    def disp_grp_all_rline_all_rstatus():
        ...
        res_line_obj_list = []
        for res_line, gbuff, waehrung, reservation, zimkateg, setup_list in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg, Setup_list).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) &  (func.lower(Reservation.groupname) == (inp_grpname).lower())).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Setup_list,(Setup_list.nr == Res_line.setup + 1)).filter(
                (Res_line.gastnr == inp_gastnr) &  (Res_line.resstatus != 9) &  (Res_line.resstatus != 10) &  (Res_line.resstatus != 12) &  (Res_line.resstatus != 99)).all():
    ...
    def create_it():
        ...
        if reservation.bemerk != "":
            comments = reservation.bemerk + chr (10)
