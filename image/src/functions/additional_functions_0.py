...
def get_first_matching_record(df, conditions):
    """
...
def get_db_url(hotelCode):
    ...
    print("get_db_url:", hotelCode, session)
    ...
    session.close()
    print("DB/host/user:", db_name, ip, username, decrypt(enc_pass), encrypt('bali2000'))
...
def set_db_and_schema(hotelCode):
    ...
    db_session.execute(sa.text("SET SEARCH_PATH TO  'vhpweb';"))
    result = db_session.execute(sa.text("SELECT current_schema();"))
    current_schema = result.scalar()
    print("HotelCode:", hotelCode)
    print("AddFunc.py, 626.Current Database/Schema:", current_database, current_schema)
    local_storage.db_session = db_session
...
def create_db_session(hotelCode):
    ...
    print("Add.Function, Session:", db_session)
    current_database = db_session.bind.url.database
    current_schema = db_session.bind.dialect.default_schema_name
    print("AddFunc.py, 658.Current Database/schema:", current_database, current_schema)
