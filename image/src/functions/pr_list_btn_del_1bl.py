...
def pr_list_btn_del_1bl(s_list:[S_list], s_list_artnr:int, billdate:date, user_init:str):
    del_cur_row = False
    docu_nr:str = ""
    ...
    if s_list_artnr == 0:
        ...
        l_order = db_session.query(L_order).filter(
                (L_order.lief_nr == 0) &  (L_order.pos == 0) &  (L_order.artnr == 0) &  (func.lower(L_order.(docu_nr).lower()) == (docu_nr).lower())).first()
        ...
        for s1_list in query(s1_list_list, filters=(lambda s1_list :s1_list.(docu_nr).lower().lower()  == (docu_nr).lower())):
        ...
        s1_list = query(s1_list_list, filters=(lambda s1_list :s1_list.(docu_nr).lower().lower()  == (docu_nr).lower()  and s1_list.artnr > 0 and s1_list.loeschflag <= 1), first=True)
