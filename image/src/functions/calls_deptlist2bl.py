...
def calls_deptlist2bl(cost_list_list:[Cost_list], sorttype:int, cost_center:int, to_cc:int, price_decimal:int, from_date:date, to_date:date, double_currency:bool, pr_summary:bool):
    ...
    output_list_list = []
    print_list_list = []
    read_parameter_str1:str = ""
    read_parameter_str2:str = ""
    substrtr:str = ""
    parameters = None
    ...
    print_list_list, Print_list = create_model("Print_list", {"flag":int, "ext":str, "datum":str, "zeit":str, "dialed":str, "dest":str, "pabx_rate":str, "guest_rate":str, "duration":str, "local":decimal, "ldist":decimal, "ovsea":decimal})
    ...
    def generate_output():
        ...
        nonlocal str_list_list, cost_list_list, output_list_list, print_list_list, t_parameters_list
    ...
    for str_list in query(str_list_list, filters=(lambda str_list: not str_list.zero_rate)):
        ...
        if sorttype == 0:
            ...
            if substring(trim(str_list.s) , 0, 10) == ("TOTAL DEPT").lower()  or substring(trim(str_list.s) , 0, 5) == "GRAND":
            ...
            if not pr_summary:
            ...
            elif pr_summary and (substring(trim(str_list.s) , 0, 5) == ("TOTAL").lower()  or substring(trim(str_list.s) , 0, 5) == "GRAND"):
                ...
                if substring(trim(str_list.s) , 0, 10) == ("TOTAL EXT.").lower() :
                    ...
                    if not cost_list or not(cost_list.num == to_int(substring(trim(str_list.s) , 13, 4))):
