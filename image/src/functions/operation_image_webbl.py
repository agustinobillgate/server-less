...
def operation_image_webbl(vkey:str, case_type:int, user_init:str, image_number:int, location:int, floor:int, char1:str, base64image:str):
    result_message = ""
    pointer:bytes = None
    info_str:str = ""
    pic_number:int = 0
    ...
    def load_dataimage():
        ...
        if not guestbook:
            ...
            pointer = guestbook.imagefile            base64image = BASE64_ENCODE (pointer)
    ...
    if vkey.lower()  == "upload":
        upload_image()

    elif vkey.lower()  == "load":
        load_dataimage()

    elif vkey.lower()  == "delete":
        delete_image()
