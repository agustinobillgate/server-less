...
def res_cancelledbl(case_type:int, show_rate:bool, curr_rmtype:str, do_it:bool, gname:str, tname:str, res_status:int, fdate:date, tdate:date, kart:int):
    tot_rm = 0
    tot_pax = 0
    tot_ch1 = 0
    tot_com = 0
    ...
    def generate_output():
        nonlocal tot_rm, tot_pax, tot_ch1, tot_com, tot_nite, tot_rm_reactive, tot_pax_reactive, tot_ch1_reactive, tot_com_reactive, tot_nite_reactive, res_cancelled_list_list, reservation, zimkateg, guest, res_line, history
                ...
                if fdate != None and tdate != None:
                    ...
                    if substring(gname, 0, 1) == "*" and substring(gname, len(gname) - 1, 1) == "*":
                        ...
                        if res_status == 1:
                            ...
                            res_line_obj_list = []
                            for res_line, reservation, zimkateg, guest in db_session.query(Res_line, Reservation, Zimkateg, Guest).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr) &  (Zimkateg.kurzbez == curr_rmtype)).join(Guest,(Guest.gastnr == Res_line.gastnr) &  (Guest.karteityp == kart)).filter(
                                    (Res_line.resstatus == 9) &  (Res_line.active_flag == 2) &  (Res_line.betrieb_gastpay != 3) &  (Res_line.cancelled >= fdate) &  (Res_line.cancelled <= tdate) &  (func.lower(Res_line.name) >= (gname).lower()) &  (func.lower(Res_line.name) <= (tname).lower()) &  (Res_line.l_zuordnung[2] == 0)).all():
                                if res_line._recid in res_line_obj_list:
                                    continue
                                else:
                                    res_line_obj_list.append(res_line._recid)
    ...
    def assign_it():
        ...
        res_cancelled_list.deposit = reservation.depositgef
        res_cancelled_list.depositpay = reservation.depositbez
