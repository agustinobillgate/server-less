...
def merged_guestbl(t_gastnr:int, s_gastnr:int, tg_karteityp:int, user_init:str):
    mess_str = ""
    answer:bool = False
    flag1:bool = False
    flag2:bool = False
    ...
    gast_list, Gast = create_model_like(Guest)
    ...
    def generate_output():
        ...
        nonlocal t_mc_guest_list, t_cl_member_list, t_bk_veran_list, gast_list, t_guest_pr_list, t_res_line_list
        return {"mess_str": mess_str}

    t_mc_guest_list = get_output(read_mc_guestbl(1, t_gastnr, ""))

    t_mc_guest = query(t_mc_guest_list, first=True)

    if t_mc_guest:
        mess_str = "Merging not for t_guest with active membership card."

        return generate_output()
    t_cl_member_list = get_output(read_cl_memberbl(1, t_gastnr))
    ...
    if t_cl_member:
        ...
        return generate_output()
    t_bk_veran_list = get_output(read_bk_veranbl(1, t_gastnr, 5, None, None))
    ...
    if s_gastnr > 0:
        ...
        if s_gastnr == t_gastnr:
            mess_str = "The selected t_guest cards must be different to the first."

            return generate_output()
        gast_list = get_output(read_guestbl(1, s_gastnr, "", ""))

        gast = query(gast_list, first=True)

        if gast.karteityp != tg_karteityp:
            mess_str = "The selected t_guest Card Type must have the same."

            return generate_output()
        t_guest_pr_list = get_output(read_guest_prbl(1, t_gastnr, ""))

        t_guest_pr = query(t_guest_pr_list, first=True)
        flag1 = None != t_guest_pr
        t_guest_pr_list = get_output(read_guest_prbl(1, s_gastnr, ""))
        ...
        if flag1 and flag2:
            ...
            return generate_output()
        t_res_line_list = get_output(read_res_linebl(18, None, None, None, None, "", None, None, t_gastnr, None, ""))
