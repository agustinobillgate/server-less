...
def bfast_kcard_parsingroomdatabl(inproomnumber:str, inpmealtime:str):
    ...
    if res_line:
        ...
        if arrangement and guest:
            ...
            argt_bez = " " + arrangement + " : " + arrangement.argt_bez
            ...
            for guestseg in db_session.query(Guestseg).filter(
                    (Guestseg.gastnr == res_line.gastnrmember)).all():
            ...
            for argt_line in db_session.query(Argt_line).filter(
                ...
                if argt_line.argt_artnr == artgrp_abf and inpmealtime.lower()  == "Breakfast":
                    ...
                    for i in range(1,num_entries(res_line.zimmer_wunsch, ";")  + 1) :
                        ...
                        if re.match(".*ChAge.*",entry(i - 1, res_line.zimmer_wunsch, ";")):
                ...
                elif argt_line.argt_artnr == artgrp_lunch and inpmealtime.lower()  == "Lunch":
                    pass

                elif argt_line.argt_artnr == artgrp_dinner and inpmealtime.lower()  == "Dinner":
                    pass
    else:
    ...
    if num_of_day == 0:
        outstr = "If guest checked_in today, a breakfast voucher will be available on the next day"
    ...
    if totadult + totchild + totcompli > 0 and res_line:
        ...
        for mealcoup in db_session.query(Mealcoup).filter(
                (Mealcoup.resnr == res_line.resnr) &  (Mealcoup.zinr == res_line.zinr) &  (Mealcoup.name == inpmealtime)).all():
            totconsumed = mealcoup.verbrauch[num_of_day - 1]

    for b_res_line in db_session.query(B_res_line).filter(
            (B_res_line.resnr == res_line.resnr) &  (B_res_line.zinr == res_line.zinr) &  (B_res_line.resstatus > 6) &  (B_res_line.kontakt_nr != 0)).all():
