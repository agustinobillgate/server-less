...
def ratecode_adm_check_plist_webbl(pvilanguage:int, curr_select:str, prcode:str, market:str, prlist_zikatnr:int, prlist_argtnr:int, market_nr:int, tb3_srecid:int, p_list:[P_list]):
    msg_str = ""
    child_error = False
    error_msg = ""
    lvcarea:str = "ratecode_adm_check_plistbl"
    curr_i:int = 0
    mesval:str = ""
    ...
    p_list_list, P_list = create_model_like(Ratecode, {"s_recid":int, "rmcat_str":str, "wday_str":str, "adult_str":str, "child_str":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal msg_str, child_error, error_msg, lvcarea, curr_i, mesval, error_flag, ratecode
    ...
    def proc_checka():
        ...
        for curr_i in range(1,num_entries(p_list.wday_str, ",")  + 1) :
            ...
            if mesval != "":
                ...
                if ord(mesval) < 48 or ord(mesval) > 55:
        ...
        for curr_i in range(1,num_entries(p_list.adult_str, ",")  + 1) :
            ...
            if mesval != "":
                ...
                if ord(mesval) < 48 or ord(mesval) > 57 or re.match(".*-.*",mesval):
        ...
        for curr_i in range(1,num_entries(p_list.child_str, ",")  + 1) :
            ...
            if mesval != "":
                ...
                if ord(mesval) < 48 or ord(mesval) > 57:
    ...
    def check_overlapping(curr_mode:str, f_date:date, t_date:date, w_day:int, adult:int, child1:int, child2:int, prcode:str, market:str, zikatnr:int, argtnr:int, zipreis:decimal):
        ...
        if curr_mode.lower()  == "insert" or curr_mode.lower()  == "copy_rate":
    ...
    if p_list.startperiode == None or p_list.endperiode == None or p_list.startperiode > p_list.endperiode:
        msg_str = translateExtended ("Start- and/or End_periode incorrect", lvcarea, "")
    ...
    if curr_select.lower()  == "insert" or curr_select.lower()  == "update":
        proc_checka()
