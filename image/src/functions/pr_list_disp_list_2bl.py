...
def pr_list_disp_list_2bl(char1:str, billdate:date, from_date:date, to_date:date, outstand_flag:bool, expired_flag:bool, approve_flag:bool, reject_flag:bool, sorttype:int, sort_app:str):
    s_list_list = []
    estimated:int = 0
    testimated:int = 0
    ...
    def generate_output():
        nonlocal s_list_list, estimated, testimated, bediener, l_lieferant, l_orderhdr, parameters, l_order, l_artikel, l_pprice, gl_acct
    ...
    def disp_list(pr_nr:str):
        ...
        if pr_nr != "":
            ...
            l_order = db_session.query(L_order).filter(
                    (L_order.docu_nr == l_orderhdr.docu_nr) &  (L_order.pos > 0) &  (L_order.lief_nr == 0) &  (L_order.loeschflag == sorttype)).first()
            while None != l_order:
        ...
        if sort_app.lower()  == "ALL" or sort_app.lower()  == "":
            ...
            for l_orderhdr in db_session.query(L_orderhdr).filter(
                ...
                if do_it:
                    ...
                    if entry(0, l_orderhdr.lief_fax[1], ";") == " " and entry(1, l_orderhdr.lief_fax[1], ";") == " " and entry(2, l_orderhdr.lief_fax[1], ";") == " " and entry(3, l_orderhdr.lief_fax[1], ";") == " ":
                        ...
                        s_list.app_rej = entry(0, entry(0, l_orderhdr.lief_fax[1], ";") , " ") + ";" + entry(0, entry(1, l_orderhdr.lief_fax[1], ";") , " ") + ";" + entry(0, entry(2, l_orderhdr.lief_fax[1], ";") , " ") + ";" + entry(0, entry(3, l_orderhdr.lief_fax[1], ";") , " ")
                    IF1 + get_index(l_orderhdr.lief_fax[1], "|") > 0 THEN
                    s_list.rej_reason = trim(entry(1, l_orderhdr.lief_fax[1], "|"))
                    s_list.app_rej = trim(entry(3, entry(0, l_orderhdr.lief_fax[1], "|") , ";"))
            ...
            for l_orderhdr in db_session.query(L_orderhdr).filter(
                        (L_orderhdr.bestelldatum >= from_date) &  (L_orderhdr.bestelldatum <= to_date) &  (L_orderhdr.betriebsnr >= 9) &  (entry(0, L_orderhdr.lief_fax[1], ";") == " ") &  (entry(1, L_orderhdr.lief_fax[1], ";") == " ") &  (entry(2, L_orderhdr.lief_fax[1], ";") == " ") &  (entry(3, L_orderhdr.lief_fax[1], ";") == " ")).all():
                app_flag = (l_orderhdr.lief_fax[1] != "" AND1 + get_index(l_orderhdr.lief_fax[1], "|") == 0) and entry(0, l_orderhdr.lief_fax[1], ";") != " " and entry(1, l_orderhdr.lief_fax[1], ";") != " " and entry(2, l_orderhdr.lief_fax[1], ";") != " " and entry(3, l_orderhdr.lief_fax[1], ";") != " "
                rej_flag = (l_orderhdr.lief_fax[1] != "" AND1 + get_index(l_orderhdr.lief_fax[1], "|") > 0)
