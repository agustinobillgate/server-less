...
def verify_user_tokenbl(user_init:str, license_nr:str, user_token:str):
    new_user_token = ""
    i_result = 0
    errmess = ""
    headerstring:str = ""
    ...
    for guest_queasy in db_session.query(Guest_queasy).filter(
            (func.lower(Guest_queasy.key) == "userToken") &  (func.lower(Guest_queasy.char1) == (user_init).lower())).all():
    ...
    if i_result == 1:
        ...
        if tot_counter >= 4:
            ...
            gqbuff = db_session.query(Gqbuff).filter(
                    (func.lower(Gqbuff.key) == "userToken") &  (func.lower(Gqbuff.char1) == (user_init).lower()) &  (Gqbuff.number3 <= 1)).first()
            while None != gqbuff:
            ...
            guest_queasy = db_session.query(Guest_queasy).filter(
                    (func.lower(Guest_queasy.key) == "userToken") &  (func.lower(Guest_queasy.char1) == (user_init).lower())).first()
            ...
            if guest_queasy:
                ...
                while not gqbuff:
                    ...
                    for guest_queasy in db_session.query(Guest_queasy).filter(
                        ...
                        gqbuff1 = db_session.query(Gqbuff1).first()
