...
def copy_bares_overlapping_check1bl(pvilanguage:int, resnr:int, reslinnr:int, datum:date, raum:str, von_i:int, bis_i:int):
    its_ok = False
    msg_str = ""
    ...
    resline = db_session.query(Resline).filter(
            (Resline.datum == datum) &  (func.lower(Resline.(raum).lower()) == (raum).lower()) &  (((Resline.von_i >= von_i) &  (Resline.von_i <= bis_i)) |  ((Resline.bis_i >= von_i) &  (Resline.bis_i <= bis_i)) |  ((Resline.von_i >= Resline.von_i) &  (Resline.bis_i <= Resline.bis_i))) &  (Resline.resstatus == bk_reser.resstatus)).first()
