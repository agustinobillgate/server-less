...
from models import Printer, Artikel, Htparam, H_bill, Hoteldpt, Queasy, H_queasy, Kellner, H_bill_line, H_artikel, Kontplan, Paramtext, Bediener, Tisch, Res_line, Mc_guest, H_mjourn, H_artcost, Guest, Printcod, H_journal, Waehrung

def print_hbilllnlbl(pvilanguage:int, session_parameter:str, user_init:str, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):
    filename = ""
    msg_str = ""
    ...
    printer = artikel = htparam = h_bill = hoteldpt = queasy = h_queasy = kellner = h_bill_line = h_artikel = kontplan = paramtext = bediener = tisch = res_line = mc_guest = h_mjourn = h_artcost = guest = printcod = h_journal = waehrung = None

    art_list = t_printer = vat_list = output_list = artbuff = abuff = None
    ...
    def generate_output():
        ...
        nonlocal art_list, t_printer, vat_list, output_list, artbuff, abuff
    ...
    def cal_servat(depart:int, h_artnr:int, service_code:int, mwst_code:int, inpdate:date):
        ...
        if bill_date < bill_date110 and (service_code != 0 or mwst_code != 0):
            ...
            if not hbuff or not(hbuff.artnr == h_artnr and hbuff.departement == depart):
                hbuff = db_session.query(Hbuff).filter(
                    (Hbuff.artnr == h_artnr) &  (Hbuff.departement == depart)).first()

            if not abuff or not(abuff.artnr == hbuff.artnrfront and abuff.departement == depart):
                abuff = db_session.query(Abuff).filter(
                    (Abuff.artnr == hbuff.artnrfront) &  (Abuff.departement == depart)).first()

            if not kontplan or not(kontplan.betriebsnr == depart and kontplan.kontignr == abuff.artnr and kontplan.datum == inpdate):
        ...
        if service_code != 0:
            ...
            if not htparam or not(htparam.paramnr == service_code):
        ...
        if mwst_code != 0:
            ...
            if not htparam or not(htparam.paramnr == mwst_code):
                htparam = db_session.query(Htparam).filter(
    ...
    def add_unitprice_text():
        ...
        if not htparam or not(htparam.paramnr == 412):
            htparam = db_session.query(Htparam).filter(
    ...
    def check_pages():
        ...
        if not art_list or not(art_list.artart == 0):
        ...
        if h_bill.saldo == 0 or print_all:
            ...
            if not paramtext or not(paramtext.txtnr == 712):
            ...
            if not paramtext or not(paramtext.txtnr == 711):
                paramtext = db_session.query(Paramtext).filter(
        ...
        if printed_line == 0 or print_all:
            ...
            if h_bill.bilname != "":
                ...
                if prtabledesc:
                    ...
                    if not tisch or not(tisch.tischnr == h_bill.tischnr and tisch.departement == h_bill.departement):
                        tisch = db_session.query(Tisch).filter(
                            (Tisch.tischnr == h_bill.tischnr) &  (Tisch.departement == h_bill.departement)).first()
                ...
                if h_bill.resnr > 0 and h_bill.reslinnr > 0:
                    ...
                    if not res_line or not(res_line.resnr == h_bill.resnr and res_line.reslinnr == h_bill.reslinnr):
                        res_line = db_session.query(Res_line).filter(
                            (Res_line.resnr == h_bill.resnr) &  (Res_line.reslinnr == h_bill.reslinnr)).first()

                    if res_line:

                        if not mc_guest or not(mc_guest.gastnr == res_line.gastnrmember and mc_guest.activeflag):
                    ...
                    if not mc_guest or not(mc_guest.gastnr == h_bill.resnr and mc_guest.activeflag):
                        mc_guest = db_session.query(Mc_guest).filter(
                ...
                if gst_logic:
                    ...
                    output_list.str_pos = 1
                    output_list.str = output_list.str +\
                        " " + to_string(bill_date) + " " + to_string(get_current_time_in_seconds(), "HH:MM") +\
                        " " + translateExtended ("BillNo", lvcarea, "") + " " +\
                        rechnr_str
    ...
    def print_billine():
        ...
        for h_mjourn in db_session.query(H_mjourn).filter(
            ...
            if not h_artcost or not(h_mjourn.h_artnr.artnr == h_mjourn.artnr and h_mjourn.h_artnr.departement == dept):
                h_artcost = db_session.query(H_artcost).filter(
                    (H_artcost.artnr == h_mjourn.artnr) &  (H_artcost.departement == dept)).first()

            if h_mjourn.h_artnr:
                bezeich = h_mjourn.h_artnr.bezeich
    ...
    def print_overhead2(prall_flag:int):
        ...
        if not htparam or not(htparam.paramnr == 846):
            htparam = db_session.query(Htparam).filter(
    ...
    def cut_it():
        ...
        if not printcod or not(printcod.emu == printer.emu and printcod.code.lower()  == ("cut").lower()):
    ...
    def cal_totalfb():
        ...
        if not h_bill_line or not(h_bill_line.rechnr == h_bill.rechnr and h_bill_line.departement == h_bill.departement):
        ...
        for h_journal, h_artikel in db_session.query(H_journal, H_artikel).join(H_artikel,(H_artikel.artnr == H_journal.artnr) &  (H_artikel.departement == H_journal.departement) &  (H_artikel.artart == 0)).filter(
            ...
            if h_artikel.artnr == disc_art1:
                ...
                if not artikel or not(artikel.artnr == h_artikel.artnrfront and artikel.departement == h_artikel.departement):
    ...
    def print_totalfb():
        ...
        if not h_bill_line or not(h_bill_line.rechnr == h_bill.rechnr and h_bill_line.departement == h_bill.departement and h_bill_line.artnr == f_discart and substring(h_bill_line.bezeich, len(h_bill_line.bezeich) - 1) == ("*").lower()):
            h_bill_line = db_session.query(H_bill_line).filter(
                (H_bill_line.rechnr == h_bill.rechnr) &  (H_bill_line.departement == h_bill.departement) &  (H_bill_line.artnr == f_discart) &  (substring(H_bill_line.bezeich, len(H_bill_line.bezeich) - 1) == ("*").lower())).first()
    ...
    def multi_currency():
        ...
        if not htparam or not(htparam.paramnr == 830):
            htparam = db_session.query(Htparam).filter(
        ...
        for i in range(1,num_entries(htparam.fchar, ";")  + 1) :
            ...
            if not waehrung or not(waehrung.wabkurz.lower()  == (mesval).lower()):
                waehrung = db_session.query(Waehrung).filter(
                    (func.lower(Waehrung.wabkurz) == (mesval).lower())).first()

            if waehrung:
                exchg_rate =  to_decimal(waehrung.ankauf) / to_decimal(waehrung.einheit)
                foreign_amt =  to_decimal(round(tot_amount) / to_decimal(exchg_rate , 2))
    ...
    def print_net_vat():
        ...
        if not htparam or not(htparam.paramnr == 376):
            htparam = db_session.query(Htparam).filter(
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) &  (H_artikel.departement == H_bill_line.departement) &  (H_artikel.artart == 0) &  (H_artikel.mwst_code != 0)).filter(
            ...
            if not htparam or not(htparam.paramnr == h_artikel.mwst_code):
                htparam = db_session.query(Htparam).filter(
                    (Htparam.paramnr == h_artikel.mwst_code)).first()

            if htparam.fdecimal != 0:

                if not vat_list or not(vat_list.vat_amt == htparam.fdecimal):
        ...
        if not htparam or not(htparam.paramnr == 872):
            htparam = db_session.query(Htparam).filter(
    ...
    def print_in_word():
        ...
        if not htparam or not(htparam.paramnr == 825):
            htparam = db_session.query(Htparam).filter(
        ...
        if not htparam or not(htparam.paramnr == 829):
            htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 468):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 469):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 557):
        htparam = db_session.query(Htparam).filter(
    ...
    if htparam:
        ...
        if not htparam.flogic and entry(0, htparam.fchar, ";") == ("GST(MA)").lower() :
            gst_logic = True

    if printnr > 0:

        if not printer or not(printer.nr == printnr):
    ...
    if printnr != 0:
        ...
        if not h_bill or not(h_bill._recid == hbrecid):
            h_bill = db_session.query(H_bill).filter(
                (H_bill._recid == hbrecid)).first()

        if not htparam or not(htparam.paramnr == 132):
            htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == 132)).first()

        if not artikel or not(artikel.artnr == htparam.finteger and artikel.departement == 0):
            artikel = db_session.query(Artikel).filter(
                (Artikel.artnr == htparam.finteger) &  (Artikel.departement == 0)).first()

        if artikel:

            if not artbuff or not(artbuff.artnr == artikel.artnr and artbuff.departement == h_bill.departement):
        ...
        if not htparam or not(htparam.paramnr == 133):
            htparam = db_session.query(Htparam).filter(
        ...
        if not htparam or not(htparam.paramnr == 850):
            htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == 850)).first()
        hmargin = htparam.finteger

        if not htparam or not(htparam.paramnr == 858):
            htparam = db_session.query(Htparam).filter(
        ...
        if not htparam or not(htparam.paramnr == 851):
            htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == 851)).first()
        lpage = htparam.finteger

        if not htparam or not(htparam.paramnr == 871):
            htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == 871)).first()
        nbezeich = 25

        if not htparam or not(htparam.paramnr == 831):
            htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == 831)).first()
        nwidth = htparam.finteger

        if not htparam or not(htparam.paramnr == 890):
            htparam = db_session.query(Htparam).filter(
        ...
        if not htparam or not(htparam.paramnr == 253):
            htparam = db_session.query(Htparam).filter(
        ...
        if not htparam or not(htparam.paramnr == 246):
            htparam = db_session.query(Htparam).filter(
        ...
        if not htparam or not(htparam.paramnr == 874):
            htparam = db_session.query(Htparam).filter(
        ...
        if not hoteldpt or not(hoteldpt.num == h_bill.departement):
        ...
        if not use_h_queasy:
            ...
            if not queasy or not(queasy.key == 4 and queasy.number1 == (h_bill.departement + h_bill.rechnr * 100) and queasy.number2 == 0 and queasy.deci2 == billnr):
            ...
            if not h_queasy or not(h_queasy.number1 == (h_bill.departement + h_bill.rechnr * 100) and h_queasy.number2 == 0 and h_queasy.billno == billnr):
        ...
        if not kellner or not(kellner.departement == h_bill.departement and kellner.kellner_nr == h_bill.kellner_nr):
            kellner = db_session.query(Kellner).filter(
                (Kellner.departement == h_bill.departement) &  (Kellner.kellner_nr == h_bill.kellner_nr)).first()
        ...
        if not htparam or not(htparam.paramnr == 824):
            htparam = db_session.query(Htparam).filter(
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if h_bill_line.artnr != 0:
                ...
                if not h_artikel or not(h_artikel.artnr == h_bill_line.artnr and h_artikel.departement == h_bill_line.departement):
            ...
            if not use_h_queasy:
                ...
                if not queasy or not(queasy.key == 4 and queasy.number1 == (h_bill.departement + h_bill.rechnr * 100) and queasy.number2 == to_int(h_bill_line._recid) and queasy.deci2 == billnr):
                    queasy = db_session.query(Queasy).filter(
                ...
                if not h_queasy or not(h_queasy.number1 == (h_bill.departement + h_bill.rechnr * 100) and h_queasy.number2 == to_int(h_bill_line._recid) and h_queasy.billno == billnr):
                    h_queasy = db_session.query(H_queasy).filter(
            ...
            if do_it:
                ...
                if not art_list or not(art_list.artnr == h_bill_line.artnr and art_list.dept == h_bill_line.departement and art_list.bezeich == h_bill_line.bezeich and art_list.price == h_bill_line.epreis and art_list.condiment == 0 and not art_list.printed):
        ...
        for abuff in query(abuff_list, filters=(lambda abuff:(abuff.zwkum == disc_zwkum) and re.match((".*-.*"),abuff.bezeich, re.IGNORECASE))):
            disc_bezeich = replace_str(abuff.bezeich, "-", "")

            if not art_list or not(art_list.artnr == abuff.artnr and art_list.bezeich.lower()  == (disc_bezeich).lower()  and art_list.amount == - abuff.amount):
        ...
        if not art_list or not(art_list.printed == False):
        ...
        if not art_list and printnr >= 0 and not bline_exist:
            msg_str = msg_str + chr(2) + translateExtended ("No new bill-lines was found.", lvcarea, "")
        ...
        if prdisc_flag and tot_ndisc_line >= 1 and tot_disc_line >= 1:
            ...
            if not art_list or not(art_list.artart == 0 and not art_list.disc_flag and not art_list.printed):
            ...
            if not art_list or not(art_list.artart == 0 and not art_list.printed):
