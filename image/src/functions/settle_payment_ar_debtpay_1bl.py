...
def settle_payment_ar_debtpay_1bl(age_list:[Age_list], pay_list:[Pay_list], pvilanguage:int, outstand1:decimal, foutstand1:decimal, outstand:decimal, curr_art:int, rundung:int, foutstand:decimal, pay_date:date, balance:decimal, fbalance:decimal, user_init:str):
    f_flag = 0
    msg_str = ""
    ...
    pay_list_list, Pay_list = create_model("Pay_list", {"artnr":int, "bezeich":str, "proz":decimal, "betrag":decimal, "f_amt":decimal, "currency":int, "curr_str":str, "bemerk":str, "remain_amt":decimal, "fremain_amt":decimal, "balance":decimal})
    ...
    def check_rounding():
        ...
        for age_list in query(age_list_list):
            ...
            for debitor in db_session.query(Debitor).filter(
                    (Debitor.zahlkonto > 0) &  (Debitor.rechnr == age_list.rechnr) &  (Debitor.gastnr == age_list.gastnr) &  (Debitor.gastnrmember == age_list.gastnrmember)).all():
                balance = balance + debitor.saldo

            debitor = db_session.query(Debitor).filter(
                    (Debitor.zahlkonto == 0) &  (Debitor.rechnr == age_list.rechnr) &  (Debitor.gastnr == age_list.gastnr) &  (Debitor.gastnrmember == age_list.gastnrmember)).first()

            if debitor:
                balance1 = debitor.saldo
            ...
            if balance2 == 0.01:
                ...
                debt = db_session.query(Debt).filter(
                        (Debt.zahlkonto > 0) &  (Debt.rechnr == age_list.rechnr) &  (Debt.gastnr == age_list.gastnr) &  (Debt.gastnrmember == age_list.gastnrmember)).first()

                if debt:
                    debt.saldo = debt.saldo - balance2
