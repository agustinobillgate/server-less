...
def rest_daysalesp2_btn_go_4_cldbl(bline_list:[Bline_list], buf_art:[Buf_art], disc_art1:int, disc_art2:int, disc_art3:int, curr_dept:int, all_user:bool, shift:int, from_date:date, to_date:date, art_str:str, voucher_art:int, zero_vat_compli:bool, show_fbodisc:bool, htl_dept_dptnr:int, incl_move_table:bool):
    t_betrag = 0
    t_foreign = 0
    exchg_rate = 0
    tot_serv = None
    ...
    other_art = temp = t_tot_betrag = t_nt_betrag = bline_list = outstand_list = pay_list = pay_listbuff = turnover = summ_list = buf_art = t_artnr = buf_hbline = buf_artikel = tlist = h_bline = b_hbline = b_hbill = kellner1 = None
    ...
    def generate_output():
        ...
        nonlocal pay_listbuff, buf_hbline, buf_artikel, tlist, h_bline, b_hbline, b_hbill, kellner1


        nonlocal other_art, temp, t_tot_betrag, t_nt_betrag, bline_list, outstand_list, pay_list, pay_listbuff, turnover, summ_list, buf_art, t_artnr, buf_hbline, buf_artikel, tlist, h_bline, b_hbline, b_hbill, kellner1
            ...
            while available h_bill_line:
                ...
                if h_bill_line.artnr == 0:
                    ...
                    b_hbline = db_session.query(B_hbline).filter(
                            (B_hbline.bill_datum == h_bill_line.bill_datum) &  (B_hbline.rechnr == h_bill_line.rechnr) &  (B_hbline.departement == h_bill_line.departement) &  (B_hbline.artnr == h_bill_line.artnr) &  (B_hbline.bezeich != h_artikel.bezeich)).first()
        ...
        if incl_move_table:
            for curr_date in range(from_date,to_date + 1) :
                ...
                for h_journal in db_session.query(H_journal).filter(
                        (H_journal.bill_datum == curr_date) &  (H_journal.departement == curr_dept)).all():
                    ...
                    if not b_hbill:
                        ...
                        turnover.rechnr = to_string(h_journal.rechnr)
                        turnover.belegung = 0
                        turnover.qty_fpax = 0
                        turnover.qty_bpax = 0
                        turnover.betrag[0] = 0
