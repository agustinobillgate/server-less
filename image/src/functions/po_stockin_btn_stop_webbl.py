...
from models import L_op, L_order, L_artikel, Bediener, L_orderhdr, L_bestand, L_liefumsatz, L_ophdr, Htparam, Gl_acct, L_kredit, Ap_journal, L_pprice

def po_stockin_btn_stop_webbl(user_init:str, l_orderhdr_recid:int, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, lief_nr:int, billdate:date, docu_nr:str, lscheinnr:str, crterm:int, curr_lager:int, t_amount:decimal, op_list:[Op_list], t_l_order:[T_l_order]):
    fl_code = 0
    tot_wert:decimal = 0
    tot_anz:decimal = 0
    curr_pos:int = 0
    l_op = l_order = l_artikel = bediener = l_orderhdr = l_bestand = l_liefumsatz = l_ophdr = htparam = gl_acct = l_kredit = ap_journal = l_pprice = None

    op_list = t_l_order = l_art = l_order1 = l_op1 = l_od = l_price1 = None

    op_list_list, Op_list = create_model_like(L_op, {"rec_id":int})
    t_l_order_list, T_l_order = create_model_like(L_order, {"rec_id":int, "art_bezeich":str, "jahrgang":int, "alkoholgrad":decimal, "lief_einheit":decimal, "curr_disc":int, "curr_disc2":int, "curr_vat":int})
    ...
    def generate_output():
        nonlocal fl_code, tot_wert, tot_anz, curr_pos, l_op, l_order, l_artikel, bediener, l_orderhdr, l_bestand, l_liefumsatz, l_ophdr, htparam, gl_acct, l_kredit, ap_journal, l_pprice
        nonlocal l_art, l_order1, l_op1, l_od, l_price1


        nonlocal op_list, t_l_order, l_art, l_order1, l_op1, l_od, l_price1
    ...
    def create_l_op():
        ...
        if l_art.ek_aktuell != op_list.einzelpreis:

            l_art = db_session.query(L_art).first()
            l_art.ek_letzter = l_art.ek_aktuell
            l_art.ek_aktuell = op_list.einzelpreis
        ...
        if ((l_art.endkum == f_endkum or l_art.endkum == b_endkum) and billdate <= fb_closedate) or (l_art.endkum >= m_endkum and billdate <= m_closedate):
            ...
            if tot_anz != 0:
                l_art.vk_preis = tot_wert / tot_anz
        ...
        l_liefumsatz.gesamtumsatz = l_liefumsatz.gesamtumsatz + op_list.warenwert
    ...
    for op_list in query(op_list_list):
        ...
        if t_l_order:
            ...
            l_art = db_session.query(L_art).filter(
                    (L_art.artnr == t_l_order.artnr)).first()
