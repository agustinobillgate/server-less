...
def view_staycostbl(pvilanguage:int, resnr:int, reslinnr:int, contcode:str):
    output_list_list = []
    ci_date:date = None
    ...
    def generate_output():
        nonlocal output_list_list, ci_date, new_contrate, wd_array, bonus_array, lvcarea, reservation, htparam, res_line, arrangement, guest_pr, waehrung, genstat, reslin_queasy, queasy, katpreis, argt_line, artikel, fixleist
        nonlocal w1
    ...
    def cal_revenue():
        ...
        ratecode_qsy:str = ""
        count_break:decimal = 0
        fixcost_rate:decimal = 0
        W1 = Waehrung
        n = 0
        ...
        if res_line.abreise > res_line.ankunft:
            co_date = res_line.abreise - 1
        else:
            co_date = res_line.abreise
        rm_rate = res_line.zipreis
        for datum in range(res_line.ankunft,co_date + 1) :
            curr_i = curr_i + 1
            bill_date = datum
            argt_rate = 0
            daily_rate = 0
            ...
            if datum < ci_date:
                ...
                genstat = db_session.query(Genstat).filter(
                        (Genstat.datum == datum) &  (Genstat.resnr == res_line.resnr) &  (Genstat.res_int[0] == res_line.reslinnr)).first()

                if genstat:
                    rm_rate = genstat.zipreis

                    reslin_queasy = db_session.query(Reslin_queasy).filter(
                            (func.lower(Reslin_queasy.key) == "arrangement") &  (Reslin_queasy.resnr == genstat.resnr) &  (Reslin_queasy.reslinnr == genstat.res_int[0]) &  (Reslin_queasy.datum >= Reslin_queasy.date1) &  (Reslin_queasy.datum <= Reslin_queasy.date2)).first()
                    ...
                    if add_it:
                        ...
                        if reslin_queasy:
                            ...
                            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                    (func.lower(Reslin_queasy.key) == "fargt_line") &  (Reslin_queasy.char1 == "") &  (Reslin_queasy.number1 == argt_line.departement) &  (Reslin_queasy.number2 == argt_line.argtnr) &  (Reslin_queasy.resnr == res_line.resnr) &  (Reslin_queasy.reslinnr == res_line.reslinnr) &  (Reslin_queasy.number3 == argt_line.argt_artnr) &  (Reslin_queasy.bill_date >= Reslin_queasy.date1) &  (Reslin_queasy.bill_date <= Reslin_queasy.date2)).all():
                                argt_defined = True

                                if reslin_queasy.char2.lower()  != "" and reslin_queasy.char2.lower()  != "0":
                                    argt_rate = rm_rate * to_int(reslin_queasy.char2) / 100
                                else:
                                ...
                                if argt_rate > 0:
                                    ...
                                    argt_rate = (rm_rate * (- argt_rate / 100)) * qty
                                ...
                                if argt_rate != 0:
                                    ...
                                    count_break = count_break + argt_rate
                        ...
                        if guest_pr and not argt_defined:
                            ...
                            if reslin_queasy:
                                ...
                                for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                        (func.lower(Reslin_queasy.key) == "argt_line") &  (func.lower(Reslin_queasy.char1) == (contcode).lower()) &  (Reslin_queasy.number1 == res_line.reserve_int) &  (Reslin_queasy.number2 == arrangement.argtnr) &  (Reslin_queasy.reslinnr == res_line.zikatnr) &  (Reslin_queasy.number3 == argt_line.argt_artnr) &  (Reslin_queasy.resnr == argt_line.departement) &  (Reslin_queasy.bill_date >= Reslin_queasy.date1) &  (Reslin_queasy.bill_date <= Reslin_queasy.date2)).all():
                                    argt_defined = True
                        ...
                        if argt_rate2 > 0:
                            argt_rate2 = argt_rate2 * qty
                        else:
                            argt_rate2 = (rm_rate * (- argt_rate2 / 100)) * qty
                        ...
                        if argt_rate2 != 0 and argt_rate == 0:
                            ...
                            count_break = count_break + argt_rate2
            ...
            for fixleist in db_session.query(Fixleist).filter(
                ...
                if argt_rate != 0:
                    ...
                    fixcost_rate = fixcost_rate + argt_rate
