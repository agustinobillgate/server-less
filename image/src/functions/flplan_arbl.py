...
def flplan_arbl(pvilanguage:int):
    ...
    def generate_output():
        ...
        nonlocal rbuff, rsvbuff


        nonlocal cr_list, rbuff, rsvbuff
    ...
    def fill_res_info(i_case:int, g_info:str, r_info:str):
        ...
        if i_case == 1:
            ...
            if not rbuff:
                ...
                rbuff = db_session.query(Rbuff).filter(
                        (Rbuff.active_flag == 0) &  (Rbuff.ankunft == ci_date) &  (Rbuff.resstatus == 11) &  (Rbuff.l_zuordnung[2] == 0) &  (Rbuff.zinr == res_line.zinr)).first()
            ...
            if rbuff.betriebsnr > 0:
                ...
                waehrung = db_session.query(Waehrung).filter(
                        (Waehrungsnr == rbuff.betriebsnr)).first()
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                    (func.lower(Reslin_queasy.key) == "specialRequest") &  (Reslin_queasy.resnr == rbuff.resnr) &  (Reslin_queasy.reslinnr == rbuff.reslinnr)).first()
    ...
    for res_line in db_session.query(Res_line).filter(
            (Res_line.active_flag <= 1) &  (Res_line.resstatus != 12) &  (Res_line.l_zuordnung[2] == 0) &  (Res_line.zinr != "")).all():
