...
def gcf_list_webbl(case_type:int, sorttype:int, lname:str, fname:str, num1:int):
    ...
    def case2a():
        ...
        if guest:
            ...
            if fname != " ":
                ...
                for guest in db_session.query(Guest).filter(
                        ((Guest.gastnr > 0) &  (Guest.karteityp == sorttype) &  (func.lower(Guest.name) == lname))).all():
                    t_guest = T_guest()
                    t_guest_list.append(t_guest)
    ...
    def case2c():
        ...
        for guest in db_session.query(Guest).filter(
                ((Guest.gastnr > 0) &  (Guest.karteityp == sorttype) &  ((Guest.vorname1 + Guest.anredefirma) MATCHES "*" + fname + "*"))).all():
            counter = counter + 1
    ...
    def case7():
        ...
        for guest in db_session.query(Guest).filter(
                ((Guest.karteityp == sorttype) &  (Guest.name >= trim(lname)) &  ((Guest.vorname1 + Guest.anredefirma) >= ""))).all():
