...
def eg_reglist1_btn_go_webbl(location:int, rmno:str, main_nr:int, reqstatus:int, sguestflag:bool, from_date:date, to_date:date, pvilanguage:int, tpic:[Tpic]):
    copyrequest_list = []
    lvcarea:str = "eg_reglist1"
    int_str:[str] = ["", "", "", ""]
    zimmer = queasy = eg_subtask = eg_location = eg_property = eg_request = None

    tpic = copyrequest = tcategory = tsource = tsubtask = tmaintask = tlocation = troom = tstatus = qbuff = qbuff1 = ques = None
    ...
    def generate_output():
        nonlocal copyrequest_list, lvcarea, int_str, zimmer, queasy, eg_subtask, eg_location, eg_property, eg_request
        nonlocal qbuff, qbuff1, ques


        nonlocal tpic, copyrequest, tcategory, tsource, tsubtask, tmaintask, tlocation, troom, tstatus, qbuff, qbuff1, ques
        ...
        if sguestflag :
            ...
            if rmno != "" and main_nr != 0:
                ...
                if reqstatus == 0:
                    ...
                    eg_request_obj_list = []
                    for eg_request, tstatus, tsource, tcategory, tMaintask, eg_location, eg_property in db_session.query(Eg_request, Tstatus, Tsource, Tcategory, TMaintask, Eg_location, Eg_property).join(Tstatus,(Tstatus.stat_nr == Eg_request.reqstatus)).join(Tsource,(Tsource.source_nr == Eg_request.SOURCE)).join(Tcategory,(Tcategory.categ_nr == Eg_request.category)).join(TMaintask,(tmaintask.main_nr == Eg_request.maintask)).join(Eg_location,(Eg_location.nr == Eg_request.reserve_int)).join(Eg_property,(Eg_property.nr == Eg_request.property)).filter(
            ...
            if location == 0:
                ...
                if main_nr != 0:
                    ...
                    if reqstatus == 0:
                        ...
                        eg_request_obj_list = []
                        for eg_request, tstatus, tsource, tcategory, tMaintask, eg_location, eg_property in db_session.query(Eg_request, Tstatus, Tsource, Tcategory, TMaintask, Eg_location, Eg_property).join(Tstatus,(Tstatus.stat_nr == Eg_request.reqstatus)).join(Tsource,(Tsource.source_nr == Eg_request.SOURCE)).join(Tcategory,(Tcategory.categ_nr == Eg_request.category)).join(TMaintask,(tmaintask.main_nr == Eg_request.maintask)).join(Eg_location,(Eg_location.nr == Eg_request.reserve_int)).join(Eg_property,(Eg_property.nr == Eg_request.property)).filter(
                                (Eg_request.deptnum >= 0) &  (Eg_request.delete_flag == False) &  (Eg_request.reqstatus == reqstatus) &  (Eg_request.opened_date >= from_date) &  (Eg_request.opened_date <= to_date)).all():
                            if eg_request._recid in eg_request_obj_list:
                                continue
                            else:
                                eg_request_obj_list.append(eg_request._recid)
    ...
    def create_copy1():
        ...
        copyRequest.property = eg_request.property
        ...
        eg_property = db_session.query(Eg_property).filter(
                (Eg_property.nr == eg_request.property)).first()

        if eg_property:
            copyRequest.property_nm = eg_property.bezeich + "(" + trim (to_string(eg_request.property , ">>>>>>9")) + ")"


        else:
            copyRequest.property_nm = eg_property.bezeich

        tpic = query(tpic_list, filters=(lambda tpic :tpic.pic_nr == eg_request.assign_to), first=True)

        if tpic:
            copyRequest.pic_str = tpic.pic_nm

        tsubtask = query(tsubtask_list, filters=(lambda tsubtask :tsubtask.sub_nr == eg_request.sub_task), first=True)
