...
def read_htparambl(case_type:int, paramno:int, paramgrup:int):
    ...
    if case_type == 1:
        ...
        for htparam in db_session.query(Htparam).filter(
                (Htparam.paramgruppe == paramgrup)).all():
            t_htparam = T_htparam()
            t_htparam_list.append(t_htparam)
        ...
        for htparam in db_session.query(Htparam).filter(
                (Htparam.paramnr >= 1) &  (Htparam.paramnr <= 16)).all():
            t_htparam = T_htparam()
            t_htparam_list.append(t_htparam)
        ...
        htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == paramno) &  (func.lower(Htparam.bezeich) != "Not used")).first()
