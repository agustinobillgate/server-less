...
def prepare_ts_tbplanbl(dept:int, curr_waiter:int):
    ...
    t_queasy = t_queasy33 = t_queasy31 = t_tisch = t_h_bill = t_kellner = t_mc_guest = t_zimmer = qsy = None
    ...
    def generate_output():
        nonlocal ci_date, pos1, pos2, mc_flag, mc_pos1, mc_pos2, vpos_flag, t_queasy_list, t_tisch_list, t_h_bill_list, t_kellner_list, t_mc_guest_list, t_zimmer_list, t_queasy31_list, t_queasy33_list, s, curr_n, queasy, tisch, h_bill, kellner, mc_guest, htparam, zimmer, h_bill_line
        nonlocal qsy


        nonlocal t_queasy, t_queasy33, t_queasy31, t_tisch, t_h_bill, t_kellner, t_mc_guest, t_zimmer, qsy
    ...
    def initiate_it():
        ...
        for queasy in db_session.query(Queasy).filter(
                (Queasy.key == 31) &  (Queasy.number1 == dept) &  (Queasy.betriebsnr == 0)).all():

            tisch = db_session.query(Tisch).filter(
                    (Tisch.departement == queasy.number1) &  (Tischnr == queasy.number2)).first()
            do_it = None != tisch
    ...
    for h_bill in db_session.query(H_bill).filter(
            (H_bill.departement == dept) &  (H_bill.flag == 0)).all():
        t_h_bill = T_h_bill()
    ...
    for queasy in db_session.query(Queasy).filter(
            (Queasy.key == 33) &  (Queasy.number1 == dept) &  (Queasy.date1 == ci_date) &  (Queasy.logi3)).all():
