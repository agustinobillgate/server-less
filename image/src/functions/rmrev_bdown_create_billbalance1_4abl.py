...
def rmrev_bdown_create_billbalance1_4abl(exc_taxserv:bool, pvilanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, curr_date:date, srttype:int):
    ...
    def generate_output():
        nonlocal cl_list_list, currency_list_list, sum_list_list, s_list_list, argt_list_list, exchg_rate, frate, post_it, total_rev, temptotjur, lvcarea, argt_line, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, billjournal, bill, zimkateg, reslin_queasy
    ...
    def create_billbalance1():
        ...
        if srttype == 2:
            ...
            for genstat, res_line, zimmer in db_session.query(Genstat, Res_line, Zimmer).join(Res_line,(Res_line.resnr == Genstat.resnr) &  (Res_line.reslinnr == Genstat.res_int[0]) &  (Res_line.l_zuordnung[2] == 0)).join(Zimmer,(Zimmer.zinr == Genstat.zinr)).filter(
                    (Genstat.zinr != "") &  (Genstat.datum == curr_date) &  (Genstat.res_logic[1])).all():
                ...
                for billjournal in db_session.query(Billjournal).filter(
                    ...
                    bill = db_session.query(Bill).filter(
                            (Bill.rechnr == billjournal.rechnr) &  (Bill.resnr == genstat.resnr) &  (Bill.reslinnr == 0)).first()
                    ...
                    for argt_line, artikel in db_session.query(Argt_line, Artikel).join(Artikel,(Artikel.artnr == Argt_line.argt_artnr) &  (Artikel.departement == Argt_line.departement)).filter(
                        ...
                        if artikel.zwkum == bfast_art and (artikel.umsatzart == 3 or artikel.umsatzart >= 5):
                            ...
                            if not exc_taxserv:
                                ...
                                cl_list.bfast = genstat.res_deci[1]
                                sum_list.bfast = sum_list.bfast + cl_list.bfast
                            ...
                            if not exc_taxserv:
                                ...
                                cl_list.lunch = genstat.res_deci[2]
                                sum_list.lunch = sum_list.lunch + cl_list.lunch
                            ...
                            if not exc_taxserv:
                                ...
                                cl_list.dinner = genstat.res_deci[3]
                                sum_list.dinner = sum_list.dinner + cl_list.dinner
                        ...
                        for billjournal, artikel in db_session.query(Billjournal, Artikel).join(Artikel,(Artikel.artnr == Billjournal.artnr) &  (Artikel.departement == Billjournal.departement) &  (Artikel.artart != 9)).filter(
                            ...
                            if billjournal.artnr != deposit_art:
                                ...
                                if lcur == waehrung1.wabkurz:
                                    s_list.f_betrag = s_list.f_betrag + billjournal.betrag
