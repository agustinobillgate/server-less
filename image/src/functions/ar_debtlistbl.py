...
def ar_debtlistbl(from_name:str, to_name:str, from_date:date, to_date:date, from_art:int, to_art:int, tot_flag:bool, lesspay:bool, show_inv:bool, case_type:int):
    ...
    def create_list():
        ...
        for debitor, artikel, guest in db_session.query(Debitor, Artikel, Guest).join(Artikel,(Artikel.artnr == Debitor.artnr) &  (Artikel.departement == 0)).join(Guest,(Guest.gastnr == Debitor.gastnr) &  (func.lower(Guest.name) >= (from_name).lower()) &  (func.lower(Guest.name) <= (to_name).lower())).filter(
            ...
            if curr_gastnr != debitor.gastnr:
                ...
                if tot_saldo != 0 and tot_flag:
                    ...
                    output_list.saldo = tot_saldo
                    output_list.fsaldo = tf_saldo
            ...
            if debitor.counter > 0 and lesspay:
                ...
                for debt in db_session.query(Debt).filter(
                        (Debt.counter == debitor.counter) &  (Debt.opart >= 1) &  (Debt.zahlkonto > 0)).all():
                ...
                if artnr != artikel.artnr:
                    ...
                    if artnr != 0:
                        ...
                        if tot_flag:
                            ...
                            output_list.saldo = t_debit
                            output_list.fsaldo = tf_debit
        ...
        output_list.saldo = tot_debit
        output_list.fsaldo = ttf_debit
