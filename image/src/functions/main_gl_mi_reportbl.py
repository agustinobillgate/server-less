...
def main_gl_mi_reportbl(reportnr:int):
    ...
    brief = db_session.query(Brief).filter((Brief.briefnr == reportnr)).first()
    t_brief = T_brief()
    ...
    parameters = db_session.query(Parameters).filter(
            (func.lower(Parameters.progname) == "GL_macro") and  (Parameters.SECTION == to_string(reportnr))).first()
