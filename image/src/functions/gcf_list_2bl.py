...
def gcf_list_2bl(case_type:int, sorttype:int, lname:str, fname:str, num1:int):
    first_gastnr = 0
    curr_lname = ""
    curr_fname = ""
    t_guest_list = []
    ...
    def generate_output():
        nonlocal first_gastnr, curr_lname, curr_fname, t_guest_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, guest, mc_guest, segment, guestseg, cl_member, guest_pr, ratecode, htparam


        nonlocal t_guest
        nonlocal t_guest_list
        return {"first_gastnr": first_gastnr, "curr_lname": curr_lname, "curr_fname": curr_fname, "t-guest": t_guest_list}
    ...
    def case2a():
        ...
        if guest:
            ...
            for guest in db_session.query(Guest).filter(
                    ((Guest.gastnr > 0) &  (Guest.karteityp == sorttype) &  (func.lower(Guest.name) == (lname).lower()) &  ((Guest.vorname1 + Guest.anredefirma) >= fname))).all():
                t_guest = T_guest()
    ...
    def case2b():
        ...
        for guest in db_session.query(Guest).filter(
                ((Guest.gastnr > 0)) &  ((Guest.karteityp == sorttype)) &  ((Guest.name + Guest.vorname1) MATCHES (lname))).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
    ...
    def case3a():
        ...
        for guest in db_session.query(Guest).filter(
                ((Guest.gastnr > 0) &  (Guest.karteityp == sorttype) &  (substring(Guest.name, 0, len((lname).lower() )) == (lname).lower()) &  ((Guest.vorname1 + Guest.anredefirma) >= ""))).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
    ...
    def case3b():
        ...
        for guest in db_session.query(Guest).filter(
                ((Guest.gastnr > 0) &  (Guest.karteityp == sorttype) &  ((Guest.name + Guest.vorname1) MATCHES (lname)) &  ((Guest.vorname1 + Guest.anredefirma) >= fname))).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
    ...
    def case5():
        ...
        for guest in db_session.query(Guest).filter(
                (Guest.karteityp == 0) &  (Guest.gastnr > 0) &  (func.lower(Guest.ausweis_nr1) == (lname).lower())).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
    ...
    def case6():
        ...
        for guest in db_session.query(Guest).filter(
                (Guest.karteityp == sorttype) &  (Guest.gastnr >= from_gastnr)).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
    ...
    def case7():
        ...
        for guest in db_session.query(Guest).filter(
                ((Guest.gastnr <= num1) &  (Guest.karteityp == sorttype) &  (Guest.name >= trim(lname)) &  ((Guest.vorname1 + Guest.anredefirma) >= ""))).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
    ...
    def case8():
        ...
        guest = db_session.query(Guest).filter(
                (Guest.gastnr == to_int(lname))).first()
    ...
    if case_type == 1:
        ...
        elif len(lname) >= 2 and substring(lname, 0, 1) == "*":
            case2b()
    elif case_type == 3:
    ...
    for t_guest in query(t_guest_list):
        ...
        for guestseg, segment in db_session.query(Guestseg, Segment).join(Segment,(Segment.segmentcode == Guestseg.segmentcode) &  (Segment.betriebsnr == 4)).filter(
                (Guestseg.gastnr == t_guest.gastnr)).all():
