...
def load_artikel_1bl(case_type:int, deptno:int):
    artikel_list_list = []
    t_artikel_list = []
    ...
    def create_abuff(inp_dept:int):
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            if substring(abuff.bezeich, len(abuff.bezeich) - 1 - 1) == ("01").lower() :
        ...
        if not ekum or not(ekum.eknr == 100 + hoteldpt.num):
    ...
    if case_type == 1:
        ...
        if not hoteldpt or not(hoteldpt.num == deptno):
        ...
        if not artikel or not(artikel.departement == deptno):
