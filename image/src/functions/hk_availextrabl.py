...
def hk_availextrabl(pvilanguage:int, art_nr:int, fdate:date, tdate:date):
    ...
    tmp_extra_list, Tmp_extra = create_model("Tmp_extra", {"reihe":int, "typ_pos":str, "pos_from":str, "cdate":date, "room":str, "qty":int, "rsvno":int})
    ...
    def create_data():
        ...
        for tmp_resline in query(tmp_resline_list, sort_by=[("resnr",False)]):
            ...
            if not arrangement or not(arrangement.arrangement == tmp_resline.arrangement):
    ...
    def create_tmpextra(reihe:int, typ_pos:str, pos_from:str, cdate:date, room:str, qty:int):
