...
def query(data: List[Type], 
    ...
    if (first or last) and filters == [] and not sort_by:
        if len(data) == 1:
            return data[0]
...
# from typing import List, TypeVar, Callable, Optional, Union, Tuple
...
def query(data: List[Type], 
    ...
    if sort_by:
        for field, descending in reversed(sort_by):
            data.sort(key=lambda x: getattr(x, field), reverse=descending)
