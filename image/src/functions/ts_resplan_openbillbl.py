...
def ts_resplan_openbillbl(pvilanguage:int, s_recid:int, curr_dept:int, curr_date:date, recid_hbill:int, user_init:str, pos_printer:int, curr_flag:str):
    err_flag = False
    result_msg = ""
    lvcarea:str = "ts_resplan"
    ...
    mealcoupon_cntrl, must_print, zero_flag, multi_cash, cancel_exist, msg_str, disc_art1, disc_art2, disc_art3, mi_ordertaker, price_decimal, curr_local, curr_foreign, double_currency, foreign_rate, exchg_rate, b_title, deptname, p_223, curr_waiter, fl_code, pos1, pos2, cashless_flag, hbill_list, t_kellner_list = get_output(prepare_ts_restinvbl(pvilanguage, curr_dept, pos_printer, user_init, None))
    ...
    deposit_foreign = deposit_amount / exchg_rate
    foreign_payment = - deposit_amount / exchg_rate
    local_payment = - deposit_amount


    add_zeit = add_zeit + 1
    bill_date, cancel_flag, fl_code, mwst, mwst_foreign, rechnr, balance, bcol, balance_foreign, fl_code1, fl_code2, fl_code3, p_88, closed, t_h_bill_list, kellner1_list = get_output(ts_restinv_update_bill_1bl(pvilanguage, recid_hbill, hart_recid, deptname, None, h_depoart_type, False, service_code, local_payment, foreign_payment, local_payment, double_currency, 1, exchg_rate, price_decimal, user_init, table_no, curr_dept, curr_waiter, gname, pax, 0, add_zeit, h_depoart, h_depobez, "", voucher_depo, "", "", voucher_str, "", True, False, h_depoart_front, 0, gastno, "", False, foreign_rate, "", user_init, 0, 0, False, 0, "", t_submenu_list))
