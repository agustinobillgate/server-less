...
def ap_age1bl(pvilanguage:int, to_date:date, from_name:str, to_name:str, detailed:bool, mi_bill:bool, segm:int, curr_disp:int):
    apage_list_list = []
    billdate:date = None
    curr_lief_nr:int = 0
    curr_art:int = 0
    ...
    apage_list_list, Apage_list = create_model("Apage_list", {"docu_nr":str, "lschein":str, "rgdatum":str, "str":str})
    age_list_list, Age_list = create_model("Age_list", {"artnr":int, "rechnr":str, "lschein":str, "counter":int, "lief_nr":int, "rgdatum":date, "firma":str, "p_bal":decimal, "debit":decimal, "credit":decimal, "saldo":decimal, "debt0":decimal, "debt1":decimal, "debt2":decimal, "debt3":decimal, "tot_debt":decimal})
        ...
        for l_kredit, l_lieferant in db_session.query(L_kredit, L_lieferant).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr) &  (func.lower(L_lieferant.firma) >= (from_name).lower()) &  (func.lower(L_lieferant.firma) <= (to_name).lower())).filter(
            ...
            if do_it:
                ...
                if (l_kredit.counter == 0) or (not age_list):
                    ...
                    if len(l_kredit.lscheinnr) <= 10:
                        age_list.lschein = l_kredit.lscheinnr
                    else:
                        age_list.lschein = substring(l_kredit.lscheinnr, 10, len(l_kredit.lscheinnr) - 10)
    ...
    def fill_in_list(fill_billno:bool, curr_po:str, curr_lschein:str, curr_rgdatum:str):
    ...
    if not detailed:
        ...
        for age_list in query(age_list_list):
            ...
            t_prev = t_prev + round(age_list.p_bal , 0)
            t_debit = t_debit + round(age_list.debit , 0)
            t_credit = t_credit + round(age_list.credit , 0)
