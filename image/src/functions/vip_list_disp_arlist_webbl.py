    ...
    def disp_arlist1():
        ...
        if sorttype == 1:
            ...
            if fdate == None:
                ...
                res_line_obj_list = []
                recs = (db_session.query(Res_line, Zimkateg, Guestseg, Segment)
                        .join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr))
                        .join(Guestseg,(Guestseg.gastnr == Res_line.gastnrmember) &  
                              ((Guestseg.segmentcode == vip_nr[0]) |  
                               (Guestseg.segmentcode == vip_nr[1]) |  
                               (Guestseg.segmentcode == vip_nr[2]) |  
                               (Guestseg.segmentcode == vip_nr[3]) |  
                               (Guestseg.segmentcode == vip_nr[4]) |  
                               (Guestseg.segmentcode == vip_nr[5]) |  
                               (Guestseg.segmentcode == vip_nr[6]) |  
                               (Guestseg.segmentcode == vip_nr[7]) |  
                               (Guestseg.segmentcode == vip_nr[8]) |  
                               (Guestseg.segmentcode == vip_nr[9])))
                        .join(Segment,(Segment.segmentcode == Guestseg.segmentcode))
                        .filter(
                        ((Res_line.resstatus <= 5) |  (Res_line.resstatus == 11)) &  
                        (Res_line.name.op("~")(".*" + lname + ".*"))).all())
                for res_line, zimkateg, guestseg, segment in recs:
                ...
                res_line_obj_list = []
                recs = (
                    db_session.query(Res_line, Zimkateg, Guestseg, Segment)
                    .join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr))
                    .join(Guestseg,(Guestseg.gastnr == Res_line.gastnrmember) &  
                          ((Guestseg.segmentcode == vip_nr[0]) |  
                           (Guestseg.segmentcode == vip_nr[1]) |  
                    ...
                    .filter(
                        ((Res_line.resstatus <= 5) |  (Res_line.resstatus == 11)) &  
                        (Res_line.name.op("~")(".*" + lname + ".*")) &  
                        (Res_line.ankunft >= fdate) &  
                        (Res_line.ankunft <= tdate))
                    .all()
                )
                for res_line, zimkateg, guestseg, segment in recs:
                    if res_line._recid in res_line_obj_list:
