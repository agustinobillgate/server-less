...
def delacct_allowedbl(pvilanguage:int, from_acct:str, mess_it:bool):
    do_it = False
    msg_str = ""
    lvcarea:str = "gl_export_import_journal"
    ...
    def delacct_allowed():
        ...
        if artikel:
            ...
            if mess_it:
                msg_str = msg_str + chr(2) + translateExtended ("Front_office Article exists, deleting not possible", lvcarea, "") + chr(10) + to_string(artikel.artnr) + " - " + artikel.bezeich

            return

        parameters = db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == "CostCenter") &  (func.lower(Parameters.section) == "Alloc") &  (Parameters.vtype == 1) &  (Parameters.vstring == gl_acct.fibukonto)).first()
