...
def prepare_sel_egsub_webbl(main_nr:int, dept_nr:int):
    ...
    duration_list, Duration = create_model("Duration", {"dur_nr":int, "dur_str":str})
    q_list_list, Q_list = create_model("Q_list", {"sub_code":str, "bezeich":str, "dur_str":str, "dur_nr":int, "days":str, "hours":str, "minutes":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal q_list_list, eg_subtask, eg_duration


        nonlocal duration, q_list
        nonlocal duration_list, q_list_list
    ...
    if main_nr == 0 and dept_nr != 0:
        ...
        for eg_subtask in db_session.query(Eg_subtask).filter(
                (Eg_subtask.dept_nr == dept_nr)).all():
            create_list()
        ...
        for eg_subtask in db_session.query(Eg_subtask).filter(
                (Eg_subtask.dept_nr == dept_nr) &  (Eg_subtask.main_nr == main_nr)).all():
            create_list()
