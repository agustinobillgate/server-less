    ...
    def disp_it():
        ...
        if ((usrid != None)) and (action == None):
            ...
            for res_history, ubuff in db_session.query(Res_history, Ubuff).join(Ubuff,(Ubuff.nr == Res_history.nr)).filter(
                    (Res_history.datum >= from_date) &  (Res_history.datum <= to_date) &  (Res_history.nr == bediener.nr)).all():
        ...
        if ((usrid == None)) and (action != None):
            ...
            res_history_obj_list = []
            for res_history, ubuff in db_session.query(Res_history, Ubuff).join(Ubuff,(Ubuff.nr == Res_history.nr)).filter(
                    (Res_history.datum >= from_date) &  (Res_history.datum <= to_date) &  (func.lower(Res_history.(action).lower()) == (action).lower())).all():
                if res_history._recid in res_history_obj_list:
                    continue
                else:
                    res_history_obj_list.append(res_history._recid)
        ...
        if ((usrid != None)) and (action != None):
            ...
            res_history_obj_list = []
            for res_history, ubuff in db_session.query(Res_history, Ubuff).join(Ubuff,(Ubuff.nr == Res_history.nr)).filter(
                    (Res_history.datum >= from_date) &  (Res_history.datum <= to_date) &  (Res_history.nr == bediener.nr) &  (func.lower(Res_history.(action).lower()) == (action).lower())).all():
                if res_history._recid in res_history_obj_list:
                    continue
                else:
                    res_history_obj_list.append(res_history._recid)
        ...
        for res_history in db_session.query(Res_history).filter(
                (Res_history.datum >= from_date) &  (Res_history.datum <= to_date)).all():
