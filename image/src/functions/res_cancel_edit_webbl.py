...
def res_cancel_edit_webbl(res_mode:str, done:bool, inp_resno:int, tot_qty:int, reslinno:int, grpflag:bool):
    successflag = False
    ...
    def cancel_edit():
        ...
        if res_mode.lower()  == "new" or res_mode.lower()  == "insert":
            ...
            res_line = db_session.query(Res_line).filter(
                    (Res_line.resnr == inp_resno) &  (Res_line.reslinnr == reslinno) &  (Res_line.resstatus == 12)).first()
