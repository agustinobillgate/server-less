...
def bookengine_ratecodepull_ratecode_listbl(prcode:str):
    ...
    ratecode_detail_list_list, Ratecode_detail_list = create_model("Ratecode_detail_list", {"id":int, "marknr":int, "rcode":str, "rmtype":str, "argmt":str, "cstr":str, "flag":int})
    ...
    for prtable, prmarket, prtable0 in db_session.query(Prtable, Prmarket, Prtable0).join(Prmarket,(Prmarket.nr == Prtable.marknr)).join(Prtable0,(Prtable0.marknr == Prtable.marknr) &  (Prtable0.prcode == "")).filter(
            (func.lower(Prtable.(prcode).lower()) == (prcode).lower())).all():
            ...
            if zimkateg:
                for j in range(1,99 + 1) :
                    ...
                    if prtable0.argtnr[j - 1] != 0:
                        ...
                        ratecode_detail_list = query(ratecode_detail_list_list, filters=(lambda ratecode_detail_list :ratecode_detail_list.rmType.lower()  == (roomtype).lower()  and ratecode_detail_list.argmt == arrangement and ratecode_detail_list.flag == 1), first=True)
                        ...
                        if not ratecode_detail_list:
                            ...
                            ratecode_detail_list.argmt = arrangement
