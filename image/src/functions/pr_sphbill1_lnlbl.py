...
def pr_sphbill1_lnlbl(pvilanguage:int, hbrecid:int, printnr:int, use_h_queasy:bool, session_parameter:str, user_init:str, billnr:int, print_all:bool):
    winprinterflag = False
    filename = ""
    msg_str = ""
    output_list_list = []
    t_printer_list = []
    lvcarea:str = "print_sphbill1"
    disc_bezeich:str = ""
    amount:decimal = 0
    sort_i:int = 0
    order_id:str = ""
    ...
    art_list = t_printer = vat_list = output_list = bline_vatlist = artbuff = abuff = hbuff = h_art = None
    ...
    def generate_output():
        ...
        nonlocal art_list, t_printer, vat_list, output_list, bline_vatlist, artbuff, abuff, hbuff, h_art
        nonlocal art_list_list, t_printer_list, vat_list_list, output_list_list, bline_vatlist_list
        return {"winprinterflag": winprinterflag, "filename": filename, "msg_str": msg_str, "output-list": output_list_list, "t-printer": t_printer_list}
    ...
    def print_overhead2(prall_flag:int):
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) &  (H_artikel.departement == H_bill_line.departement) &  (H_artikel.artart == 0)).filter(
                (H_bill_line.rechnr == h_bill.rechnr) &  (H_bill_line.departement == h_bill.departement) &  (H_bill_line.waehrungsnr == billnr)).all():
