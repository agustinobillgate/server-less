...
def prepare_mk_ins_po_webbl(docu_nr:str, pvilanguage:int, lief_nr:int, pr_deptnr:int, po_type:int, potype:int, bediener_username:str, ordername_screen_value:str, crterm:int):
    local_nr = 0
    billdate = None
    ...
    t_parameters = t_waehrung = t_l_order = t_l_orderhdr = art_supp = l_orderhdr1 = None
    ...
    art_supp_list, Art_supp = create_model("Art_supp", {"rec_id":int, "artnr":int, "bezeich":str, "ek_aktuell":decimal, "ek_letzter":decimal, "traubensort":str, "lief_einheit":decimal, "lief_nr1":int, "lief_nr2":int, "lief_nr3":int, "jahrgang":int, "alkoholgrad":decimal})
    ...
    def generate_output():
        nonlocal local_nr, billdate, zeroprice_flag, deptname, supplier, curr_liefnr, p_222, p_234, p_266, pos, t_amount, currency_add_first, currency_screen_value, msg_str, p_1093, p_464, p_220, globaldisc, t_waehrung_list, t_l_order_list, t_l_orderhdr_list, t_parameters_list, art_supp_list, l_order, l_orderhdr, htparam, waehrung, l_lieferant, parameters, l_artikel
        nonlocal l_orderhdr1


        nonlocal t_parameters, t_waehrung, t_l_order, t_l_orderhdr, art_supp, l_orderhdr1
    ...
    if not waehrung:
        msg_str = msg_str + chr(2) + "Local Currency Code incorrect! (Param 152 / Grp 7)"
    ...
    l_order = db_session.query(L_order).filter(
            (L_order.pos == 0) &  (func.lower(L_order.(docu_nr).lower()) == (docu_nr).lower()) &  (L_order.loeschflag == 0)).first()
    ...
    if p_234:
        ...
        for l_artikel in db_session.query(L_artikel).filter(
            ...
            art_supp.ek_aktuell = l_artikel.ek_aktuell
            art_supp.ek_letzter = l_artikel.ek_letzter
            art_supp.traubensort = l_artikel.traubensort
            art_supp.lief_einheit = l_artikel.lief_einheit
            ...
            art_supp.alkoholgrad = l_artikel.alkoholgrad
