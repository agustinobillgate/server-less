...
def sls_byrefer_btn_go_listbl(pvilanguage:int, next_date:date, to_date:date, all_flag:bool, usr_init:str):
    slrefer_list_list = []
    lvcarea:str = "sls_byrefer"
    akt_code = guest = akthdr = akt_kont = None

    p_list = slrefer_list = akt_code1 = buf_aktcode = guest1 = akthdr1 = akt_kont1 = buf_akthdr = None

    p_list_list, P_list = create_model("P_list", {"pnr":int, "sflag":int, "refer_name":str, "pcomp":str, "pcont":str, "pname":str, "pntot":str, "pnam1":int, "pnam2":int, "pnam3":int, "pnam4":int, "pamt":decimal, "pamt1":decimal, "pamt2":decimal, "pamt3":decimal, "pamt4":decimal, "patot":decimal, "pamt_str":str, "stnr":int, "stage":str, "proz":str, "popen":date, "pfnsh":date, "pmain1":str, "pmain2":str, "pmain3":str, "reason":str, "refer":int, "pid":str, "pcid":str, "ctotal":int})
    slrefer_list_list, Slrefer_list = create_model("Slrefer_list", {"pnr":int, "sflag":int, "refer_name":str, "pcomp":str, "pcont":str, "pname":str, "pntot":str, "pnam1":int, "pnam2":int, "pnam3":int, "pnam4":int, "pamt":decimal, "pamt1":decimal, "pamt2":decimal, "pamt3":decimal, "pamt4":decimal, "patot":decimal, "pamt_str":str, "stnr":int, "stage":str, "proz":str, "popen":date, "pfnsh":date, "pmain1":str, "pmain2":str, "pmain3":str, "reason":str, "refer":int, "pid":str, "pcid":str, "ctotal":int})
    ...
    def generate_output():
        nonlocal slrefer_list_list, lvcarea, akt_code, guest, akthdr, akt_kont
        nonlocal akt_code1, buf_aktcode, guest1, akthdr1, akt_kont1, buf_akthdr


        nonlocal p_list, slrefer_list, akt_code1, buf_aktcode, guest1, akthdr1, akt_kont1, buf_akthdr
        nonlocal p_list_list, slrefer_list_list
        return {"slrefer-list": slrefer_list_list}
    ...
    def browse_open1():
        ...
        if all_flag:
            ...
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.referred == Akt_code1.aktionscode) &  (Buf_akthdr.referred != 0)).filter(
                ...
                if akt_code1:
                    ...
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.referred == akt_code1.aktionscode)).all():
            ...
            akt_code1_obj_list = []
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.referred == Akt_code1.aktionscode) &  (Buf_akthdr.referred != 0) &  (func.lower(Buf_akthdr.userinit) == (usr_init).lower())).filter(
                    (Akt_code1.aktiongrup == 6)).all():
                if akt_code1._recid in akt_code1_obj_list:
                    continue
                else:
                ...
                if akt_code1:
                    ...
                    akthdr1_obj_list = []
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.referred == akt_code1.aktionscode) &  (func.lower(Akthdr1.userinit) == (usr_init).lower())).all():
                        if akthdr1._recid in akthdr1_obj_list:
                            continue
                        else:
                            akthdr1_obj_list.append(akthdr1._recid)
        ...
        if all_flag:
            ...
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.referred == Akt_code1.aktionscode) &  (Buf_akthdr.referred != 0) &  (Buf_akthdr.next_datum >= next_date) &  (Buf_akthdr.next_datum <= to_date)).filter(
                ...
                if akt_code1:
                    ...
                    akthdr1_obj_list = []
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.referred == akt_code1.aktionscode) &  (Akthdr1.next_datum >= next_date) &  (Akthdr1.next_datum <= to_date)).all():
                        if akthdr1._recid in akthdr1_obj_list:
                            continue
                        else:
                            akthdr1_obj_list.append(akthdr1._recid)
            ...
            akt_code1_obj_list = []
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.referred == Akt_code1.aktionscode) &  (Buf_akthdr.referred != 0) &  (func.lower(Buf_akthdr.userinit) == (usr_init).lower()) &  (Buf_akthdr.next_datum >= next_date) &  (Buf_akthdr.next_datum <= to_date)).filter(
                    (Akt_code1.aktiongrup == 6)).all():
                if akt_code1._recid in akt_code1_obj_list:
                    continue
                else:
                ...
                if akt_code1:
                    ...
                    akthdr1_obj_list = []
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == guest1.gastnr) &  (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                            (Akthdr1.referred == akt_code1.aktionscode) &  (func.lower(Akthdr1.userinit) == (usr_init).lower()) &  (Akthdr1.next_datum >= next_date) &  (Akthdr1.next_datum <= to_date)).all():
                        if akthdr1._recid in akthdr1_obj_list:
                            continue
                        else:
                            akthdr1_obj_list.append(akthdr1._recid)
    ...
    def fill_refer():
        ...
        for p_list in query(p_list_list):
            ...
            slrefer_list_list.append(slrefer_list)
