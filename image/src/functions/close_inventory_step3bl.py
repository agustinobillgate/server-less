...
def close_inventory_step3bl(inv_type:int, m_endkum:int, closedate:date):
    ...
    next_fdate = closedate + 1

    for l_ophis in db_session.query(L_ophis).filter(
            (L_ophis.datum >= startdate) &  (L_ophis.datum <= closedate) &  ((L_ophis.op_art >= 1) &  (L_ophis.op_art <= 4))).all():
        ...
        if l_ophis.op_art == 1:
            l_besthis.anz_eingang = l_besthis.anz_eingang + l_ophis.anzahl
            lbuff.anz_eingang = lbuff.anz_eingang + l_ophis.anzahl
            lbuff.wert_eingang = lbuff.wert_eingang + l_ophis.warenwert
            ...
            l_besthis.anz_eingang = l_besthis.anz_eingang + l_ophis.anzahl
            ltrans.anz_ausgang = ltrans.anz_ausgang + l_ophis.anzahl

        elif l_ophis.op_art == 3:
            l_besthis.anz_ausgang = l_besthis.anz_ausgang + l_ophis.anzahl
            lbuff.anz_ausgang = lbuff.anz_ausgang + l_ophis.anzahl
            lbuff.wert_ausgang = lbuff.wert_ausgang + l_ophis.warenwert

        elif l_ophis.op_art == 4:
            lbuff.anz_ausgang = lbuff.anz_ausgang + l_ophis.anzahl

    for l_besthis in db_session.query(L_besthis).filter(
                (L_besthis.anf_best_dat >= startdate) &  (L_besthis.anf_best_dat <= closedate) &  (L_besthis.lager_nr == 0) &  (L_besthis.anz_anf_best == 0)).all():

        for lbuff in db_session.query(Lbuff).filter(
                    (Lbuff.artnr == l_besthis.artnr) &  (Lbuff.anf_best_dat == l_besthis.anf_best_dat)).all():
            lbuff.anz_anf_best = 0
            lbuff.val_anf_best = 0

    curr_artnr = 0

    for l_besthis in db_session.query(L_besthis).filter(
            (L_besthis.anf_best_dat >= startdate) &  (L_besthis.anf_best_dat <= closedate)).all():
        ...
        if (inv_type == 1 and l_artikel.endkum < m_endkum) or (inv_type == 2 and l_artikel.endkum >= m_endkum) or (inv_type == 3):
            ...
            l_bestand.anz_anf_best = l_besthis.anz_anf_best +\
                    l_besthis.anz_eingang - l_besthis.anz_ausgang
            l_bestand.val_anf_best = l_besthis.val_anf_best +\
                    l_besthis.wert_eingang - l_besthis.wert_ausgang
            ...
            if l_bestand.lager_nr > 0 and l_bestand.anz_anf_best != 0:
                ...
                if l_bestand1.anz_anf_best != 0:
                    l_bestand.val_anf_best = l_bestand1.val_anf_best *\
                        l_bestand.anz_anf_best / l_bestand1.anz_anf_best
