...
from sqlalchemy import not_, func, text, Function
...
def substring(input_str, start:int, length:int = None):
    ...
    elif isinstance(input_str, InstrumentedAttribute) or isinstance(input_str, BinaryExpression):
...
def entry(entry_num, input_str, delimiter,str_value=""):
    if isinstance(input_str, InstrumentedAttribute) or isinstance(input_str, BinaryExpression):
...
def to_decimal(input_value):
    ...
    if isinstance(input_value, InstrumentedAttribute) or isinstance(input_value, BinaryExpression):
...
def to_int(input_str):
    try:
        if isinstance(input_str, InstrumentedAttribute) or isinstance(input_str, BinaryExpression):
            return sa.cast(input_str, sa.Integer)
...
def get_day(input_date):
    ...
    elif isinstance(input_date, InstrumentedAttribute) or isinstance(input_date, BinaryExpression):
...
def length(input_data):
    if isinstance(input_data, InstrumentedAttribute) or isinstance(input_data, Function) or isinstance(input_data, BinaryExpression):
...
def get_db_url(hotelCode):
    ...
    try:
        ...
        return ""        
    session.close()
    print(f"Masuk EC2:postgresql://{username}:{enc_pass}@{ip}:{port}/{db_name}")
