...
def prepare_print_gcpi2_1bl(docu_nr:str, user_init:str, printer_nr:int):
    output_list_list = []
    t_gc_pibline_list = []
    ...
    output_list_list, Output_list = create_model("Output_list", {"docu_nr":str, "u_username":str, "b_username":str, "bezeich":str, "bemerk":str, "betrag":decimal, "path":str, "avail_gc_pitype":bool})
    ...
    for gc_pibline in db_session.query(Gc_pibline).filter(
            (gc_PIbline.docu_nr == gc_pi.docu_nr)).all():
