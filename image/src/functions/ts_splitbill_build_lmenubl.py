...
def ts_splitbill_build_lmenubl(rec_id:int, dept:int):
    ...
    def generate_output():
        ...
        return {"max_lapos": max_lapos, "MENU": menu_list, "Lhbline": lhbline_list}
    ...
    def build_lmenu():
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
                (H_bill_line.rechnr == h_bill.rechnr) &  (H_bill_line.departement == dept) &  (H_bill_line.waehrungsnr == 0)).all():
            i = i + 1
            menu = Menu()
            ...
            MENU.bezeich = to_string(h_bill_line.anzahl) + " " + h_bill_line.bezeich
            lhbline = Lhbline()
