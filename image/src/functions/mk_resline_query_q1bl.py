...
def mk_resline_query_q1bl(pvilanguage:int, newflag:bool, chgnameflag:bool, inp_resno:int, rline_list:[Rline_list], reschanged_list:[Reschanged_list]):
    ...
    def generate_output():
        ...
        global rline_list_list, reschanged_list_list
        return {}
    ...
    if newflag:
        ...
        for rline in db_session.query(Rline).filter(
                (Rline.resnr == inp_resno) &  (Rline.active_flag <= 1) &  (Rline.resstatus != 12) &  (Rline.l_zuordnung[2] == 0)).all():
