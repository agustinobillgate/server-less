...
def sourcestat_listbl(from_month:str, ci_date:date, sorttype:int, hide_zero:bool):
    room_list_list = []
    mm:int = 0
    yy:int = 0
    datum:date = None
    ...
    room_list_list, Room_list = create_model("Room_list", {"flag":int, "bezeich":str, "summe":int, "room":[int, 31]})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal room_list_list, mm, yy, datum, i, j, from_date, to_date, sourccod, sources
    ...
    for sourccod in db_session.query(Sourccod).all():
        ...
        for datum in range(from_date,to_date + 1) :
            ...
            for sources in db_session.query(Sources).filter(
                    (Sources.source_code == sourccod.source_code) &  (Sources.datum == datum)).all():
