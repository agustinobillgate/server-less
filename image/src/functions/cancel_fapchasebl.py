...
def cancel_fapchasebl(lief_nr:int, docu_nr:str, user_init:str):
    ...
    for fa_op in db_session.query(Fa_op).filter(
            (Fa_op.lief_nr == lief_nr) &  (func.lower(Fa_op.(lscheinnr).lower()) == (lscheinnr).lower()) &  (Fa_op.loeschflag <= 1)).all():
        ...
        if fa_artikel.p_nr != 0:
            ...
            if fa_art:
                fa_art.warenwert = fa_art.warenwert - fa_artikel.warenwert
                fa_art.book_wert = fa_art.book_wert - fa_artikel.warenwert

                fa_art = db_session.query(Fa_art).first()
        fa_artikel.posted = False
        fa_artikel.next_depn = None
        fa_artikel.anzahl = 0
        fa_artikel.warenwert = 0
        fa_artikel.book_wert = 0

        fa_artikel = db_session.query(Fa_artikel).first()
        fa_op.loeschflag = 2
        t_amount = t_amount - fa_op.warenwert
    update_ap()
