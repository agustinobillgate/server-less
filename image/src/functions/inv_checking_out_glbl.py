    ...
    def out_gl():
        ...
        for d in range(d1,d2 + 1) :
            ...
            for l_op in db_session.query(L_op).filter(
                    (L_op.artnr >= frnr) &  (L_op.artnr <= tonr) &  (L_op.datum == d) &  (L_op.op_art == 3) &  (substring(L_op.stornogrund, 0, 8) != "00000000") &  (L_op.loeschflag <= 1)).all():
                ...
                art_list4.saldo1 = l_op.warenwert

            l_ophis = db_session.query(L_ophis).filter(
                    (L_ophis.artnr >= frnr) &  (L_ophis.artnr <= tonr) &  (L_ophis.datum == d) &  (L_ophis.op_art == 3)).first()

            if l_ophis:

                for l_ophis in db_session.query(L_ophis).filter(
                        (L_ophis.artnr >= frnr) &  (L_ophis.artnr <= tonr) &  (L_ophis.datum == d) &  (substring(L_ophis.fibukonto, 0, 8) != "00000000") &  (L_ophis.op_art == 3)).all():
                    ...
                    art_list4.artname = l_artikel.bezeich
                    art_list4.saldo1 = l_ophis.warenwert


        for coa_list in query(coa_list_list):

            for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
                    (Gl_jouhdr.datum == coa_list.datum) &  (Gl_jouhdr.jtyp == 3) &  (substring(Gl_jouhdr.refNo, 0, len(refNo)) == refNo)).all():

                for gl_journal in db_session.query(Gl_journal).filter(
                        (Gl_journal.jnr == gl_jouhdr.jnr) &  (Gl_journal.fibukonto == coa_list.fibukonto) &  (substring(Gl_journal.bemer, 0, 1) != "*")).all():
                ...
                if art_list4:
                    art_list4.saldo2 = coa_list.debit - coa_list.credit
        ...
        for coa_list in query(coa_list_list):
            ...
            coa_list3.wert2 = coa_list.wert
            coa_list3.fibu = coa_list.fibukonto
            coa_list3.creditdebit = coa_list.credit - coa_list.debit
            coa_list3.diff = coa_list.wert - coa_list.credit + coa_list.debit


    out_gl()

    for coa_list3 in query(coa_list3_list):
