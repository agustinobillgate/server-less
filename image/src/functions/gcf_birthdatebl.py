...
def gcf_birthdatebl(from_date:int, to_date:int, from_age:int, to_age:int, sorttype:int):
    ...
    birth_list_list, Birth_list = create_model("Birth_list", {"name2":str, "geburtdatum":date, "ankunft1":date, "abreise1":date, "zinr":str, "adresse":str, "wohnort":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal birth_list_list, guest, res_line
    ...
    def age_list():
        ...
        if sorttype == 1:
            ...
            for guest in db_session.query(Guest).filter(
                    (Guest.karteityp == 0) &  (Guest.gastnr > 0) &  (Guest.name > "") &  (Guest.vorname1 >= "") &  (Guest.geburtdatum1 != None)).all():
                ...
                if (get_month(guest.geburtdatum1) * 100 + get_day(guest.geburtdatum1)) >= from_date and (get_month(guest.geburtdatum1) * 100 + get_day(guest.geburtdatum1)) <= to_date and (- get_year(guest.geburtdatum1) + get_year(get_current_date())) >= from_age and (- get_year(guest.geburtdatum1) + get_year(get_current_date())) <= to_age:
                    ...
                    birth_list.adresse = guest.adresse1 + " " + guest.adresse2 +\
                            " " + guest.adresse3 + chr(3) + guest.email_adr + chr(3) + guest.telefon + chr(3) + guest.mobil_telefon
            ...
            for res_line in db_session.query(Res_line).filter(
                ...
                guest = db_session.query(Guest).filter(
                        (Guest.gastnr == res_line.gastnrmember) &  (Guest.geburtdatum1 != None)).first()
