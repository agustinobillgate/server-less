...
def gcjour_listbl(case_type:int, from_refno:str, sorttype:int, journaltype:int, jtype1:int, from_date:date, to_date:date):
    ...
    def disp_it2():
        ...
        gl_journal_obj_list = []
        for gl_journal, gl_acct, note_list in db_session.query(Gl_journal, Gl_acct, Note_list).join(Gl_acct,(Gl_acct.fibukonto == Gl_journal.fibukonto)).join(Note_list,(Note_list.s_recid == to_int(Gl_journal._recid))).filter(
                (Gl_journal.jnr == gl_jouhdr_list.jnr)).all():
