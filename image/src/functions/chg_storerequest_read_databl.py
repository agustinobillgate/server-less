...
def chg_storerequest_read_databl(t_lschein:str, t_datum:date, t_amount:decimal, lscheinnr:str):
    ...
    def generate_output():
        ...
        return {"curr_lager": curr_lager, "deptno": deptno, "transfered": transfered, "out_type": out_type, "to_stock": to_stock, "deptname": deptname, "lager_bezeich": lager_bezeich, "lager_bez1": lager_bez1, "curr_pos": curr_pos, "op-list": op_list_list}
    ...
    def read_data():
        ...
        for l_op in db_session.query(L_op).filter(
                (L_op.datum == t_datum) &  (func.lower(L_op.lscheinnr) == (t_lschein).lower()) &  (L_op.pos > 0) &  (L_op.loeschflag == 0)).all():
            op_list = Op_list()
