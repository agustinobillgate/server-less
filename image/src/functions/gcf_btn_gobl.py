...
def gcf_btn_gobl(icase:int, pvilanguage:int, user_init:str, t_guest:[T_guest]):
    msg_str = ""
    error_number = 0
    gastno:int = 0
    def_natcode:str = ""
        ...
        if priscilla_active:
            ...
            if (t_guest.name != guest.name) or (t_guest.vorname1 != guest.vorname1) or (t_guest.anrede1 != guest.anrede1) or (t_guest.email_adr != guest.email_adr):
                ...
                for res_line in db_session.query(Res_line).filter(
                        (Res_line.gastnrmember == guest.gastnr) &  (Res_line.active_flag <= 1) &  (Res_line.resstatus != 12) &  (Res_line.l_zuordnung[2] != 1)).all():
    ...
    if tlist and t_guest.karteityp == 0:
        ...
        for tlist in query(tlist_list):
            ...
            if (tlist.tfield == "ttl" and t_guest.anrede1 == "") or (tlist.tfield == "dob" and t_guest.geburtdatum1 == None) or (tlist.tfield == "sex" and t_guest.geschlecht == "") or (tlist.tfield == "adr" and t_guest.adresse1 == "" and t_guest.adresse2 == "" and t_guest.adresse3 == ""):
                error_number = 7

            if tlist.tfield.lower()  == "ttl":
                str1 = "Title"

            elif tlist.tfield.lower()  == "dob":
                str1 = "Birthdate"

            elif tlist.tfield.lower()  == "sex":
                str1 = "Sex"

            elif tlist.tfield.lower()  == "adr":
                str1 = "Address"
