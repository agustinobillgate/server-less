...
def reservation_btn_rescancelbl(pvilanguage:int, curr_select:str, resno:int, reslinno:int):
    msg_str = ""
    cancel_str = ""
    error_flag = False
    pswd_str = ""
    ...
    def cancel_resline():
        ...
        if res_line.active_flag == 1 or res_line.resstatus == 6 or res_line.resstatus == 13:
            msg_str = translateExtended ("Cancellation of Inhouse_Guests is not possible.", lvcarea, "")
    ...
    def cancel_mainres():
        ...
        if res_line:
            msg_str = translateExtended ("Cancellation not possible, In_House guest exists.", lvcarea, "")
        ...
        if res_line:
            msg_str = translateExtended ("KeyCard has been generated for", lvcarea, "") + " " + res_line.name + " - " + translateExtended ("RmNo", lvcarea, "") + " " + res_line.zinr + chr(10) + translateExtended ("Cancellation for all members no longer possible.", lvcarea, "")
        ...
        msg_str = "&Q" + translateExtended ("Do you really want to CANCEL the main reservation of", lvcarea, "") + chr(10) + guest.name + ", " + guest.vorname1 + guest.anredefirma + " " + guest.anrede1 + chr(10) + translateExtended ("including it's all reservation members ?", lvcarea, "")
    ...
    def check_deposit():
        ...
        if deposit and reservation1.bestat_dat == None:
            ...
            for resline1 in db_session.query(Resline1).filter(
                    (Resline1.resnr == resno) &  (Resline1.active_flag == 0) &  (Resline1.resstatus <= 6)).all():
                anzahl = anzahl + 1
