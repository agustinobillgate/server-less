...
def prepare_view_sitem1bl(user_init:str, docu_nr:str):
    show_price = None
    comments = ""
    sitem_list_list = []
    ...
    if show_price:
        ...
        for l_order, l_artikel in db_session.query(L_order, L_artikel).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                (func.lower(L_order.(docu_nr).lower()) == (docu_nr).lower()) &  (L_order.pos > 0) &  (L_order.loeschflag >= 1)).all():
    ...
    l_orderhdr = db_session.query(L_orderhdr).filter(
            (func.lower(L_orderhdr.(docu_nr).lower()) == (docu_nr).lower()) &  (L_orderhdr.lief_nr == l_order.lief_nr)).first()
