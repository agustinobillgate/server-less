...
t_data_list, T_data = create_model("T_data", {"gastnr":int, "infostr":str, "orig_infostr":str})

def guestbook_opr_listbl(instr:str, base64file:str, t_data_list:[T_data]):
    outstr = ""
    mess_result = ""
    tcase:int = 0
    tgastnr:int = 0
    ...
    def generate_output():
        ...
        nonlocal t_data
        global t_data_list
        return {"outstr": outstr, "mess_result": mess_result}
    ...
    if tcase == 1:
        ...
        if t_data:
            guestbook = db_session.query(Guestbook).filter(
                    (Guestbook.gastnr == t_data.gastnr)).first()
        ...
        guestbook = db_session.query(Guestbook).filter(
                (Guestbook.gastnr == tgastnr)).first()
        ...
        if guestbook:
            ...
            base64file = base64_decode(pointer)
