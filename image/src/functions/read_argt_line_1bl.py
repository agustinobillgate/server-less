...
def read_argt_line_1bl(case_type:int, argtno:int, artno:int, dept:int, vt_percnt:int):
    ...
    if case_type == 1:
        ...
        if artno == None:
            ...
            for argt_line in db_session.query(Argt_line).filter(
                    (Argt_line.argtnr == argtno) &  (Argt_line.vt_percnt == vt_percnt)).all():
                t_argt_line = T_argt_line()
                t_argt_line_list.append(t_argt_line)
        ...
        argt_line = db_session.query(Argt_line).filter(
                (Argt_line.argtnr == argtno) &  (Argt_line.argt_artnr == artno) &  (Argt_line.departement == dept) &  (Argt_line.vt_percnt == vt_percnt)).first()
