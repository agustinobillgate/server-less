...
def ar_subledger_update_debt_gastnrbl(age_list:[Age_list], t_artnr:int, t_dept:int, gastpay:int, a_gastnr:int, a_rechnr:int, user_init:str):
    billname = ""
    temp_billnr:int = 0
    bediener = artikel = debitor = guest = bill = res_line = res_history = None

    age_list = debt = gbuff = None
    ...
    def update_debt_gastnr():
        ...
        for debitor in db_session.query(Debitor).filter(
                (Debitor.rechnr == a_rechnr) &  (Debitor.gastnr == a_gastnr) &  (Debitor.artnr == t_artnr) &  (Debitor.opart >= 0)).all():
