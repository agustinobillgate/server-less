...
def tbplan_setup_save_table_location_webbl(case_type:int, t_queasy:[T_queasy]):
    success_flag = False
    queasy = None
    ...
    if case_type == 1:
        ...
        while None != t_queasy:
            ...
            queasy = db_session.query(Queasy).filter(
                    (Queasy.key == t_Queasy.key) &  (Queasy.number1 == t_Queasy.number1) &  (Queasy.number2 == t_Queasy.number2) &  ((Queasy.deci1 != t_Queasy.deci1) |  (Queasy.deci2 != t_Queasy.deci2)) &  (Queasy.betriebsnr == 0)).first()
