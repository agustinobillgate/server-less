...
from models import Nation, Htparam, Outorder, Res_line, Reservation, Guest, Guestseg
...
def check_pguestbl(pvilanguage:int):
    ...
    t_nation_list = []
    c_list_list = []
    lvcarea:str = "check_puest"
    nation = htparam = outorder = res_line = reservation = guest = guestseg = None
    ...
    def generate_output():
        nonlocal msg_str, tot_room, ext_char, bill_date, t_nation_list, c_list_list, lvcarea, nation, htparam, outorder, res_line, reservation, guest, guestseg
    ...
    def check_it():
        ...
        for outorder in db_session.query(Outorder).filter(
                (Outorder.gespende == bill_date)).all():
            c_list = C_list()
        ...
        for res_line in db_session.query(Res_line).filter(
                (Res_line.active_flag == 1) &  ((Res_line.resstatus == 6) |  (Res_line.resstatus == 13)) &  (Res_line.l_zuordnung[2] == 0)).all():
            tot_room = tot_room + 1
            segmentcode = 0
            ...
            c_list.zipreis = res_line.zipreis
