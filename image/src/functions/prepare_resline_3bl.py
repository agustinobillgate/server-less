...
from models import Res_line, History, Zimkateg, Ratecode, Zimmer, Guest, Htparam, Queasy, Nation, Bediener, Master, Reslin_queasy, Reservation, Kontline, Gentable, Outorder, Arrangement, Guest_pr, Pricecod, Prmarket, Bresline, Fixleist, Paramtext, Waehrung, Katpreis
...
def prepare_resline_3bl(pvilanguage:int, res_mode:str, session_date:str, user_init:str, inp_gastnr:int, inp_resnr:int, inp_reslinnr:int, rate_readonly:bool, qci_zinr:str, res_dynarate:[Res_dynarate]):
    ...
    new_reslinnr = 1
    curr_time = 0
    res_line = history = zimkateg = ratecode = zimmer = guest = htparam = queasy = nation = bediener = master = reslin_queasy = reservation = kontline = gentable = outorder = arrangement = guest_pr = pricecod = prmarket = bresline = fixleist = paramtext = waehrung = katpreis = None

    rline_list = reslin_list = curr_resline = t_history = currency_list = res_dynarate = reschanged_list = f_resline = nation_list = resline = resbuff = zimkateg1 = rbuff = qci_zimmer = bresline = bguest = qsy = m_queasy = m_leist = resmember = rline = genbuff = bline = prline = waehrung1 = gbuff = w1 = None
    ...
    def generate_output():
        nonlocal msg_str, error_flag, record_use, init_time, init_date, avail_gdpr, avail_mark, avail_news, save_gdpr, curr_date, serv_date, f_resline_list, curr_resline_list, reslin_list_list, reschanged_list_list, t_history_list, rline_list_list, weekdays, i, str, loopi, loopj, str1, foreign_nr, tokcounter, iftask, mestoken, mesvalue, rcode, prevcode, do_it, do_it1, flag_ok, dayuse_flag, split_modify, logic_p1109, priscilla_active, loopk, resbemerk, lvcarea, new_reslinnr, curr_time, res_line, history, zimkateg, ratecode, zimmer, guest, htparam, queasy, nation, bediener, master, reslin_queasy, reservation, kontline, gentable, outorder, arrangement, guest_pr, pricecod, prmarket, bresline, fixleist, paramtext, waehrung, katpreis
        nonlocal resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy, m_queasy, m_leist, resmember, rline, genbuff, bline, prline, waehrung1, gbuff, w1

        global currency_list_list, res_dynarate_list, nation_list_list
        nonlocal rline_list, reslin_list, curr_resline, t_history, currency_list, res_dynarate, reschanged_list, f_resline, nation_list, resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy, m_queasy, m_leist, resmember, rline, genbuff, bline, prline, waehrung1, gbuff, w1
        nonlocal rline_list_list, reslin_list_list, curr_resline_list, t_history_list, reschanged_list_list, f_resline_list
    ...
    def split_resline():
        ...
        for rline in db_session.query(Rline).filter(
            ...
            res_line = db_session.query(Res_line).filter((Res_line._recid == rline._recid)).first()
            ...
            if res_line.resstatus == 11:
                for prline in db_session.query(Prline).filter(
                        (Prline.resnr == res_line.resnr) &  (Prline.resstatus == 11) &  (Prline.kontakt_nr == 1)).all():
    ...
    def check_dynarate():
        ...
        nonlocal rline_list, reslin_list, curr_resline, t_history, currency_list, res_dynarate, reschanged_list, f_resline, nation_list, resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy, m_queasy, m_leist, resmember, rline, genbuff, bline, prline, waehrung1, gbuff, w1
        nonlocal rline_list_list, reslin_list_list, curr_resline_list, t_history_list, reschanged_list_list, f_resline_list
        print("res_dynarate_list:", res_dynarate_list)
    ...
    if htparam.flogical  and htparam.bezeichnung.lower()  != "not used":
    ...
    if res_mode.lower()  == "new" or res_mode.lower()  == "qci":
        ...
        if res_line.code != "" and res_line.code.lower()  != "0":
            ...
            # update Rd
            queasy = db_session.query(Queasy).filter(
                    (Queasy.key == 9) &  (Queasy.number1 == f_resline.bill_instruct)).first() 
        ...
        if ((res_line.resstatus == 11) | (res_line.resstatus == 13)):
            f_resline.sharer = True
        else:
            f_resline.sharer = False
    ...
    for loopk in range(1,len(f_resline.res_bemerk)) :
        if  (   ord(substring(f_resline.res_bemerk, loopk, 1)) == 0 or 
                ord(substring(f_resline.res_bemerk, loopk, 1)) > 255
            ) and (
                f_resline.res_bemerk.strip() != ""
            ):
            pass
        else:
    ...
    if len(f_resline.res_bemerk) < 3:
        f_resline.res_bemerk = replace_str(f_resline.res_bemerk, chr(32) , "")

    if len(f_resline.res_bemerk) == None:
        f_resline.res_bemerk = ""
    resbemerk = ""


    for loopk in range(1,len(f_resline.rline_bemerk)  + 1) :

        if ord(substring(f_resline.rline_bemerk, loopk - 1, 1)) == 0 or ord(substring(f_resline.rline_bemerk, loopk, 1)) > 255:
            pass
        else:
            resbemerk = resbemerk + substring(f_resline.rline_bemerk, loopk - 1, 1)
    f_resline.rline_bemerk = resbemerk

    if len(f_resline.rline_bemerk) < 3:
        f_resline.rline_bemerk = replace_str(f_resline.rline_bemerk, chr(32) , "")

    if len(f_resline.rline_bemerk) == None:
    ...
    if res_mode.lower()  != "split":
        ...
        reslin_list.bemerk = replace_str(reslin_list.bemerk, chr(10) , "")


        reslin_list.bemerk = replace_str(reslin_list.bemerk, chr(13) , "")
        ...
        reslin_list.bemerk = replace_str(reslin_list.bemerk, chr(10) + chr(13) , "")
        resbemerk = " "


        for loopk in range(1,len(reslin_list.bemerk)  + 1) :

            if ord(substring(reslin_list.bemerk, loopk - 1, 1)) == 0 or ord(substring(reslin_list.bemerk, loopk, 1)) > 255:
                1
            else:
                resbemerk = resbemerk + substring(reslin_list.bemerk, loopk - 1, 1)
        reslin_list.bemerk = resbemerk

        if len(reslin_list.bemerk) < 3:
            reslin_list.bemerk = replace_str(reslin_list.bemerk, chr(32) , "")

        if len(reslin_list.bemerk) == None:
            reslin_list.bemerk = ""
    ...
    if ((res_mode.lower()  == "modify" or res_mode.lower().lower()  == "inhouse") and 
        substring(bediener.permissions, 42, 2) < "2"
        ):
        ...
        if guest_pr:
            ...
            for guest_pr, queasy in db_session.query(Guest_pr, Queasy).join(Queasy,(Queasy.key == 2) &  (Queasy.char1 == Guest_pr.code)).filter(
                ...
                if queasy.char1 != prevcode:
                    ...
                    # elif f_resline.ci_date > queasy.date2:
                    #     do_it = False
                    if f_resline.ci_date is not None and queasy.date1 is not None and f_resline.ci_date < queasy.date1:
                        do_it = False
                    elif f_resline.ci_date is not None and queasy.date2 is not None and f_resline.ci_date > queasy.date2:
                        do_it = False
                    else:
                        do_it = True
    ...
    if (res_line.resstatus == 11 or res_line.resstatus == 13) and f_resline.contcode == " ":
        ...
        bresline = db_session.query(Bresline).filter(
                (Bresline.resnr == res_line.resnr) &  (Bresline.reslinnr != res_line.reslinnr) &  (Bresline.kontakt_nr == res_line.kontakt_nr)).first()
    ...
    # print("RLine:", rline_list)
    # print("ResC:", reschanged_list)
    if not split_modify:
        if (rline_list is not None) and (reschanged_list is not None):
            local_storage.debugging = local_storage.debugging + ",mk_resline_query_q1bl" 
            ...
            local_storage.debugging = local_storage.debugging + ",rline_list:None,skip:mk_resline_query_q1bl" 
