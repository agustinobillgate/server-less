...
def prepare_chg_gljourn_1bl(jnr:int):
    ...
    if not gl_jouhdr or not(gl_jouhdr.jnr == jnr):
    ...
    if not htparam or not(htparam.paramnr == 221):
        htparam = db_session.query(Htparam).filter(
            (Htparam.paramnr == 221)).first()
    closedate = htparam.fdate

    if not htparam or not(htparam.paramnr == 224):
        htparam = db_session.query(Htparam).filter(
    ...
    if not htparam or not(htparam.paramnr == 1012):
        htparam = db_session.query(Htparam).filter(
    ...
    if not l_lieferant or not(l_lieferant.firma.lower()  == ("GST").lower()):
