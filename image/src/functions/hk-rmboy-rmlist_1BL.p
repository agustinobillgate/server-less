...
DEF INPUT PARAMETER stat8     AS CHAR    NO-UNDO.
...
DEFINE VARIABLE stat-list AS CHAR EXTENT 8 FORMAT "x(20)" NO-UNDO. 
ASSIGN
    stat-list[8] = stat8
    stat-list[2] = stat2
    stat-list[3] = stat3
...
PROCEDURE create-rmlist:
    ...
    FOR EACH room WHERE (room.zistatus GE 2 AND room.zistatus LE 4)
        OR room.zistatus = 8  NO-LOCK:
        IF loc-combo = "ALL" THEN do-it = YES.
        ELSE do-it = room.CODE = loc-combo.
        IF do-it THEN
        DO:
            IF fr-floor NE 0 OR to-floor NE 0 THEN
                do-it = room.etage GE fr-floor AND room.etage LE to-floor.
        END.
        IF do-it THEN
        DO:
            CREATE rmlist.
