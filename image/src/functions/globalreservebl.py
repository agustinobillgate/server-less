...
def globalreservebl(case_type:int, k_list:[K_list], rmcat:str, gastnr:int, curr_mode:str, last_code:str, argt:str, comments:str, user_init:str):
    msg_int = 0
    ...
    def create_allotment():
        ...
        for datum in range(k_list.ankunft,k_list.abreise + 1) :
            ...
            if not counters:
                ...
                counters.counter_bez = "Allotment counters"
            counters = counters + 1
            kontline = Kontline()
            db_session.add(kontline)

            kontline.kontignr = counters
    ...
    def check_allotment():
        ...
        for res_line, kline in db_session.query(Res_line, Kline).join(Kline,(Kline.kontignr == - Res_line.kontignr) &  (Kline.kontcode == k_list.kontcode) &  (Kline.kontstat == 1)).filter(
                (Res_line.kontignr < 0) &  (Res_line.gastnr == gastnr) &  (Res_line.active_flag < 2) &  (Res_line.resstatus < 11) &  (not (Res_line.ankunft > k_list.abreise)) &  (not (Res_line.abreise < k_list.ankunft))).all():
    ...
    def open_query():
        ...
        kontline_obj_list = []
        for kontline, z_list, bediener in db_session.query(Kontline, Z_list, Bediener).join(Z_list,(Z_list.zikatnr == Kontline.zikatnr)).join(Bediener,(Bediener.nr == Kontline.bediener_nr)).filter(
                (Kontline.gastnr == gastnr) &  (Kontline.kontignr > 0) &  (Kontline.betriebsnr == 1) &  (Kontline.kontstat == 1)).all():
