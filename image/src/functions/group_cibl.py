...
def group_cibl(pvilanguage:int, ci_date:date):
    mainres_list_list = []
    lvcarea:str = "group_ci"
    reservation = res_line = guest = None

    mainres_list = None

    mainres_list_list, Mainres_list = create_model("Mainres_list", {"resnr":int, "gastnr":int, "name":str, "zimanz":int, "ci":int, "co":int, "arr":int, "abreise":date, "segm":int, "deposit":decimal, "until":date, "paid":decimal, "groupname":str, "res_address":str, "res_city":str, "res_bemerk":str}, {"abreise": 01/01/1998, "res_address": "", "res_city": "", "res_bemerk": ""})
    ...
    def fill_mainres():
        ...
        for reservation in db_session.query(Reservation).filter(
                (Reservation.activeflag == 0) &  (Reservation.grpflag) &  (Reservation.name > "") &  (Reservation.gastnr > 0)).all():
    ...
    def update_mainres():
        ...
        for res_line in db_session.query(Res_line).filter(
                (Res_line.resnr == reservation.resnr) &  (Res_line.grpflag) &  (Res_line.ankunft == ci_date)).all():
