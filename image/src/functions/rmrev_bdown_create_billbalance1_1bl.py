    ...
    def get_argtline_rate(contcode:str, argt_recid:int):
        ...
        for genstat, res_line, zimmer in db_session.query(Genstat, Res_line, Zimmer).join(Res_line,(Res_line.resnr == Genstat.resnr) &  (Res_line.reslinnr == Genstat.res_int[0]) &  (Res_line.l_zuordnung[2] == 0)).join(Zimmer,(Zimmer.zinr == Genstat.zinr)).filter(
            ...
            for argt_line, artikel in db_session.query(Argt_line, Artikel).join(Artikel,(Artikel.artnr == Argt_line.argt_artnr) &  (Artikel.departement == Argt_line.departement)).filter(
                ...
                cl_list.misc = genstat.zipreis - (cl_list.lodging + cl_list.bfast + cl_list.lunch + cl_list.dinner)
            ...
            for cl_list in query(cl_list_list, filters=(lambda cl_list :cl_list.flag.lower()  != "*")):
                ...
                if exc_taxserv:
                    cl_list.zipreis = round((cl_list.zipreis / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.localrate = round((cl_list.localrate / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.lodging = round((cl_list.lodging / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.bfast = round((cl_list.bfast / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.lunch = round((cl_list.lunch / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.dinner = round((cl_list.dinner / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.misc = round((cl_list.misc / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.fixcost = round((cl_list.fixcost / (1 + vat1 + vat2 + serv1)) , price_decimal)
                    cl_list.t_rev = round((cl_list.t_rev / (1 + vat1 + vat2 + serv1)) , price_decimal)
