...
def prepare_eg_propertybl(user_init:str):
    engid = 0
    groupid = 0
    maintask_list = []
    location_list = []
    ...
    queasy_133 = maintask = location = t_eg_property = t_eg_location = t_zimmer = qbuff = None

    queasy_133_list, Queasy_133 = create_model_like(Queasy)
    maintask_list, Maintask = create_model("Maintask", {"main_nr":int, "main_nm":str})
    location_list, Location = create_model("Location", {"loc_nr":int, "loc_nm":str})
    ...
    def generate_output():
        ...
        nonlocal queasy_133, maintask, location, t_eg_property, t_eg_location, t_zimmer, qbuff
