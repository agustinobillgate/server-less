...
def po_stockin_mi_all1bl(user_init:str, l_order_recid:int, l_orderhdr_recid:int, docu_nr:str, exchg_rate:decimal, price_decimal:int, billdate:date, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, curr_lager:int, lief_nr:int, lscheinnr:str, t_amount:decimal):
    created = False
    s_list_list = []
    t_l_order_list = []
    s_artnr:int = 0
    ...
    t_l_order_list, T_l_order = create_model_like(L_order, {"rec_id":int, "art_bezeich":str, "jahrgang":int, "alkoholgrad":decimal})
