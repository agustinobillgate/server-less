...
def nt_bapostbill():
    ...
    def generate_output():
        nonlocal zugriff, veran_nr, invnr, curr_resnr, banquet_dep, bill_date, price, amount, amount_foreign, room_amount, fb_amount, deposit_amount, exchg_rate, double_currency, foreign_rate, i, bk_rart, htparam, waehrung, bk_reser, bk_veran, guest, counters, bill, bk_func, artikel, bill_line, umsatz, billjournal
        nonlocal rbuff
    ...
    for bk_reser in db_session.query(Bk_reser).filter(
                (Bk_reser.datum == bill_date) &  (func.lower(Bk_reser.von_zeit) >= "00:00") &  (Bk_reser.resstatus == 1) &  (Bk_reser.fakturiert == 0)).all():
        ...
        if curr_resnr != bk_reser.veran_nr:
            ...
            for i in range(1,9 + 1) :
                deposit_amount = deposit_amount + bk_veran.deposit_payment[i - 1]
        ...
        for bk_rart in db_session.query(Bk_rart).filter(
                    (Bk_rart.veran_nr == bk_reser.veran_nr) &  (Bk_rart.veran_seite == bk_reser.veran_seite)).all():
