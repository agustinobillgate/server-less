...
def hk_ooobl(om_list:[Om_list], fdate:date, tdate:date, disptype:int, sorttype:int, user_init:str):
    ci_date = None
    ooo_list_list = []
    ...
    def disp_it():
        ...
        if disptype == 0:
            ...
            if sorttype == 0:
                ...
                outorder_obj_list = []
                for outorder, om_list, zimmer in db_session.query(Outorder, Om_list, Zimmer).join(Om_list,(Om_list.zinr == Outorder.zinr) &  (Om_list.gespstart == Outorder.gespstart) &  (Om_list.gespende == Outorder.gespende)).join(Zimmer,(Zimmer.zinr == Outorder.zinr)).all():
            ...
            if sorttype == 0:
                ...
                outorder_obj_list = []
                for outorder, om_list, zimmer in db_session.query(Outorder, Om_list, Zimmer).join(Om_list,(Om_list.zinr == Outorder.zinr) &  (Om_list.ind != 3) &  (Om_list.gespstart == Outorder.gespstart) &  (Om_list.gespende == Outorder.gespende)).join(Zimmer,(Zimmer.zinr == Outorder.zinr)).all():
                    if outorder._recid in outorder_obj_list:
                        continue
                    else:
                        outorder_obj_list.append(outorder._recid)
