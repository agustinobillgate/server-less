...
def chg_gljourn_fill_gl_journalbl(case_type:int, jnr:int, comment:str, user_init:str, jou_recid:int, b1_recid:int, t_bezeich:str, t_refno:str, g_list:[G_list]):
    debits = 0
    credits = 0
    remains = 0
    b1_list_list = []
    ...
    if case_type == 1:
        ...
        if gl_journal.(fibukonto).lower().lower()  != (fibukonto).lower() :
        ...
        if gl_journal.(bemerk).lower().lower()  != (bemerk).lower() :
