...
def date_range(start_date, end_end):
    res_date = start_date
    while res_date <= end_end:
        yield res_date
...
def add_interval(start_date, interval_value, interval_unit):
    ...
    if interval_unit in ['day', 'week', 'hour', 'minute', 'second']:
...
def get_db_url(hotelCode):
    ...
    #TODO: db for tenant
    # session = get_database_session("postgresql://postgres:bali2000@localhost:5432/vhp")
...
def gget_db_url(hotelCode):
    ...
    if not test_database_connection(session):
        # return "postgresql://postgres:bali2000@localhost:5432/vhp"
        # return get_database_session("postgresql://postgres:VHPLite#2023@localhost:5432/vhp_rental")
        return "postgresql://vhpadmin:bFdq8QsQoxH1vAvO@/vhp-dev.cjxtrsmbui3n.ap-southeast-1.rds.amazonaws.com:5432/vhp"
    ...
    session.close()
    print(enc_pass,  decrypt(enc_pass))
    dburl_access = f"postgresql://" + username + ":" + enc_pass + "@" + ip + ":" + str(port) + "/" + db_name
...
def set_db_and_schema(hotelCode):
    ...
    # print("1-Session (set_db_and_schema):", db_session)
    local_storage.db_session = db_session
    local_storage.hotelCode = hotelCode
    local_storage.pvILanguage = 1
    # print("2-Session (set_db_and_schema), Local Storage:", local_storage.db_session)
...
def set_combo_session(hotelCode,var2=None,var3=None,var4=None):
    ...
    if db_session:
        ...
        # print("Session (set_combo_session), Local Storage:", local_storage.combo_db_session)
        return True
...
def create_db_session(hotelCode):
    ...
    # print("Session (create_db_session):", db_url, db_session)
    return db_session
...
def print_beautify(obj):
    ...
    print(pd.DataFrame.from_dict(obj))
