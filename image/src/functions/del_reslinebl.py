...
def del_reslinebl(pvilanguage:int, res_mode:str, resnr:int, reslinnr:int, user_init:str, cancel_str:str):
    ...
    qsy = zbuff = rline = res_line1 = res_line2 = None
    ...
    def generate_output():
        ...
        nonlocal qsy, zbuff, rline, res_line1, res_line2
            ...
            if rline.resstatus == 6 and (rline.zinr != new_zinr):
                ...
                if res_line1:
                    ...
                    for res_line2 in db_session.query(Res_line2).filter(
                            (Res_line2.resnr == resnr) &  (Res_line2.zinr == rline.zinr) &  (Res_line2.resstatus == 13)).all():
                        ...
                        for bill in db_session.query(Bill).filter(
                                (Bill.resnr == resnr) &  (Bill.parent_nr == parent_nr) &  (Bill.flag == 0) &  (Bill.zinr == res_line2.zinr)).all():
                            bill.zinr = new_zinr
    ...
    if (res_mode.lower()  == "cancel" or res_mode.lower().lower()  == "delete"):
    ...
    if res_line.zinr != "":
        ...
        rline = db_session.query(Rline).filter(
                (Rline.active_flag == 0) &  (Rline.memozinr.op("~")(".*;.*")) &  (entry(1, Rline.memozinr, ";") == res_line.zinr) &  (not Rline.ankunft >= res_line.abreise) &  (not Rline.abreise <= res_line.ankunft) &  (Rline.resnr != res_line.resnr) &  (Rline.zinr != res_line.zinr)).first()

        if not rline:

            rline = db_session.query(Rline).filter(
                    (Rline.active_flag == 1) &  (Rline.memozinr.op("~")(".*;.*")) &  (entry(1, Rline.memozinr, ";") == res_line.zinr) &  (Rline.resnr != res_line.resnr) &  (Rline.zinr != res_line.zinr)).first()

        if rline:
            msg_str = msg_str + chr(2) + "&W" + "Reservation found with Memo RmNo  == " + " " + res_line.zinr

    if (res_mode == "cancel") and (res_line.resstatus <= 2 or res_line.resstatus == 5):

        zinrstat = db_session.query(Zinrstat).filter(
                (func.lower(Zinrstat.zinr) == "CancRes") &  (Zinrstat.datum == ci_date)).first()
    ...
    if res_mode.lower()  == "delete":
        ...
        for rline in db_session.query(Rline).filter(
                (Rline.resnr == res_line.resnr) &  (Rline.l_zuordnung[2] == 1) &  (Rline.kontakt_nr == reslinnr)).all():
            rline.zinr = ""
    ...
    if not res_line:
        ...
        for mast_art in db_session.query(Mast_art).filter(
                (Mast_art.resnr == resnr) &  (Mast_art.reslinnr == 1)).all():
