...
def check_approval_pobl(user_init:str):
    ...
    while None != l_orderhdr:
        ...
        while None != queasy:
            ...
            qsy_list = query(qsy_list_list, filters=(lambda qsy_list :trim(qsy_list.docu_nr) == trim(queasy.char1)), first=True)
        ...
        while None != qsy_list:
            ...
            if l_orderhdr:
                ...
                for queasy in db_session.query(Queasy).filter(
                        (Queasy.key == 245) &  (Queasy.char1 == po_list.docu_nr)).all():
