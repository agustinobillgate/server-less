...
from models import Kellner, H_bill, H_bill_line, H_mjourn, H_artikel, Hoteldpt, Artikel, Htparam, Queasy, Guest, Counters, H_umsatz, H_journal, Umsatz, Interface, Arrangement, Argt_line, Billjournal

def ts_restinv_update_bill_1bl(pvilanguage:int, rec_id:int, rec_id_h_artikel:int, deptname:str, transdate:date, h_artart:int, cancel_order:bool, h_artikel_service_code:int, amount:decimal, amount_foreign:decimal, price:decimal, double_currency:bool, qty:int, exchg_rate:decimal, price_decimal:int, order_taker:int, tischnr:int, curr_dept:int, curr_waiter:int, gname:str, pax:int, kreditlimit:decimal, add_zeit:int, billart:int, description:str, change_str:str, cc_comment:str, cancel_str:str, req_str:str, voucher_str:str, hoga_card:str, print_to_kitchen:bool, from_acct:bool, h_artnrfront:int, pay_type:int, guestnr:int, transfer_zinr:str, curedept_flag:bool, foreign_rate:bool, curr_room:str, user_init:str, hoga_resnr:int, hoga_reslinnr:int, incl_vat:bool, get_price:int, mc_str:str, submenu_list:[Submenu_list]):
    bill_date = None
    cancel_flag = False
    fl_code = 0
    mwst = 0
    ...
    curr_time:int = 0
    kellner = h_bill = h_bill_line = h_mjourn = h_artikel = hoteldpt = artikel = htparam = queasy = guest = counters = h_umsatz = h_journal = umsatz = interface = arrangement = argt_line = billjournal = None

    t_kellner1 = t_h_bill = submenu_list = h_bline = kellner1 = bbill = mjou = bill_guest = artikel1 = paramqsy = searchbill = genparamso = orderbill = orderbilline = orderbill_close = pickup_table = qpayment_gateway = buffq33 = None
    ...
    def generate_output():
        nonlocal bill_date, cancel_flag, fl_code, mwst, mwst_foreign, rechnr, balance, bcol, balance_foreign, fl_code1, fl_code2, fl_code3, p_88, closed, t_h_bill_list, t_kellner1_list, lvcarea, tax, serv, h_service, unit_price, nett_amount_foreign, h_mwst, h_mwst_foreign, h_service_foreign, nett_amount, subtotal, subtotal_foreign, service, service_foreign, serv_code, vat_code, servtax_use_foart, recid_h_bill_line, recid_hbill, sysdate, zeit, condiment, succed, active_deposit, serv_vat, ct, l_deci, vat, vat2, tax_vat, fact_scvat, get_rechnr, get_amount, curr_time, kellner, h_bill, h_bill_line, h_mjourn, h_artikel, hoteldpt, artikel, htparam, queasy, guest, counters, h_umsatz, h_journal, umsatz, interface, arrangement, argt_line, billjournal
        nonlocal h_bline, kellner1, bbill, mjou, bill_guest, artikel1, paramqsy, searchbill, genparamso, orderbill, orderbilline, orderbill_close, pickup_table, qpayment_gateway, buffq33


        nonlocal t_kellner1, t_h_bill, submenu_list, h_bline, kellner1, bbill, mjou, bill_guest, artikel1, paramqsy, searchbill, genparamso, orderbill, orderbilline, orderbill_close, pickup_table, qpayment_gateway, buffq33
    ...
    if h_artart == 0:
        ...
        if htparam.flogical:
            tax = 0
        h_service = unit_price * service

        if double_currency:
            h_service_foreign = round(h_service, 4)
            h_service = round(h_service * exchg_rate, 4)
            service_foreign = service_foreign + h_service_foreign * qty


        h_service = round(h_service, 4)
        ...
        if double_currency:
            h_mwst_foreign = round(h_mwst, 4)
            h_mwst = tax * unit_price * exchg_rate
            h_mwst = round(h_mwst, 4)


        else:
            h_mwst_foreign = round(h_mwst / exchg_rate, 4)
            h_mwst = round(h_mwst, 4)
    ...
    if not incl_vat:
        unit_price = unit_price * fact_scvat
    else:
        price = unit_price / fact_scvat
        nett_amount = nett_amount / fact_scvat
