...
def print_hbill1_phbl(pvilanguage:int, session_parameter:str, user_init:str, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):
    filename = ""
    msg_str = ""
    ...
    is_scd:bool = False
    scd_val:decimal = 0
    sub_scd_val:decimal = 0
    serv_scd_val:decimal = 0
    serv_art:int = 0
    vat_art:int = 0
    sc_art:int = 0
    sc_art1:int = 0
    ...
    art_list = t_printer = vat_list = output_list = fb_vatlist = t_htp = artbuff = abuff = hbuff = h_art = None

    art_list_list, Art_list = create_model("Art_list", {"printed":bool, "disc_flag":bool, "dept":int, "artnr":int, "bezeich":str, "artart":int, "zwkum":int, "qty":int, "price":decimal, "amount":decimal, "betrag":decimal, "happyhr":str, "datum":date, "sysdate":date, "zeit":int, "condiment":int, "i_group":int})
    ...
    output_list_list, Output_list = create_model("Output_list", {"str":str, "pos":int, "flag_popup":bool, "npause":int, "sort_i":int})
    fb_vatlist_list, Fb_vatlist = create_model("Fb_vatlist", {"i_type":int, "artno":int, "vatno":int, "bezeich":str, "vatvalue":decimal, "vatamt":decimal})
    ...
    def generate_output():
        ...
        nonlocal art_list, t_printer, vat_list, output_list, fb_vatlist, t_htp, artbuff, abuff, hbuff, h_art
    ...
    def print_overhead1():
        ...
        if printed_line == 0 or print_all:
            ...
            if h_bill.bilname != "":
                ...
                if gst_logic:
                    output_list.str = output_list.str + to_string("     " + to_string(bill_date) + " " + to_string(time, "HH:MM") + " " + translateExtended ("Tax Invoice No", lvcarea, "") + " " + to_string(rechnr_str, "x(43)"))
                else:
                    output_list.str = output_list.str + to_string("     " + to_string(bill_date) + " " + to_string(time, "HH:MM") + " " + translateExtended ("Bill No", lvcarea, "") + " " + to_string(rechnr_str, "x(33)"))
                output_list = Output_list()
                output_list_list.append(output_list)
    ...
    def print_billine():
        ...
        if prtwoline:
            ...
            if art_list.artnr == serv_art:
                serv_scd_val = ((serv% / 100) * subtotal)
    ...
    def print_overhead2(prall_flag:int):
        ...
        for art_list in query(art_list_list):
            ...
            if art_list.artnr == f_discart or art_list.artnr == f_discart1 or art_list.artnr == f_discart2:
                ...
                h_bill_line = db_session.query(H_bill_line).filter(
                        (H_bill_line.rechnr == h_bill.rechnr) &  (H_bill_line.artnr == f_discart)).first()
                for i in range(1,nbez1 + 1) :

                    if i > len(h_bill_line.bezeich):
                        output_list.str = output_list.str + to_string(" ")
                    else:
        ...
        if sc_art != 0:
            ...
            h_bill_line = db_session.query(H_bill_line).filter(
                    (H_bill_line.rechnr == h_bill.rechnr) &  (H_bill_line.departement == h_bill.departement) &  (H_bill_line.artnr == sc_art)).first()

            if h_bill_line:
                scd_val = subtotal * (scd% / 100) * -1
        for i in range(1,pos + 1) :
            output_list.str = output_list.str + to_string(" ")
        ...
        if service != 0 or mwst != 0:
            ...
            for fb_vatlist in query(fb_vatlist_list, filters=(lambda fb_vatlist :FB_vatlist.i_type >= 3)):
                ...
                if FB_vatlist.vatAmt != 0:
                    ...
                    output_list.str = output_list.str +\
                            substring(s, 0, nbez1) +\
                            fill(" ", nbez1 - len(s))
                    tot_amount = tot_amount + FB_vatlist.vatAmt
            ...
            if art_list:
                ...
                output_list.str = output_list.str +\
                        ...
                        fill(" ", nbez1 - len(art_list.bezeich))
            ...
            output_list.str = output_list.str +\
                    substring(s, 0, nbez1) +\
                    fill(" ", nbez1 - len(s))
            subtotal2 = subtotal + tot_amount + scd_val
    ...
    for loopi in range(1,num_entries(str452, ";")  + 1) :
        ...
        if bartnr != 0:
            ...
            h_artikel = db_session.query(H_artikel).filter(
                    (H_artikel.artnr == bartnr) &  (H_artikel.departement == h_bill.departement)).first()

            htparam = db_session.query(Htparam).filter(
                    (Htparam.paramgruppe == 4) &  (Htparam.bezeich == h_artikel.bezeich)).first()
            ...
            FB_vatlist.vatValue = htparam.fdecimal * 0.01

            if num_entries(htparam.fchar, " ") >= 2:
                FB_vatlist.vatValue = decimal.Decimal(entry(1, htparam.fchar, " ")) * 0.0001
    scd% = get_output(htpdec(462))
    ...
    if printnr != 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not art_list or h_bill_line.betriebsnr == 1:
                ...
                if h_artikel and h_artikel.artart == 0:
                    ...
                    if h_artikel.artnr == sc_art:
                        ...
                        fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist :FB_vatlist.artNo == art_list.artnr), first=True)

                        if FB_vatlist:
                            art_list.i_group = FB_vatlist.i_type
                            FB_vatlist.vatAmt = FB_vatlist.vatAmt + h_bill_line.betrag
            ...
            if art_list.artart == 0:
                serv%, mwst%, vat2%, fact = cal_servat(h_artikel.departement, h_artikel.artnr, h_artikel.service_code, h_artikel.mwst_code, h_bill_line.bill_datum)

                if art_list.i_group == 0:
                    is_bev, is_food = inv_cat(h_bill_line.artnr)

                    fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist :FB_vatlist.i_type == 2), first=True)

                    if is_food:
                        FB_vatlist.vatAmt = FB_vatlist.vatAmt +\
                            round(h_bill_line.betrag / fact * mwst%, price_decimal)

                    fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist :FB_vatlist.i_type == 3), first=True)
                    FB_vatlist.vatAmt = FB_vatlist.vatAmt +\
                            round(h_bill_line.betrag / fact * serv%, price_decimal)
                    ...
                    if artikel.prov_code != 0:
                        ...
                        htparam = db_session.query(Htparam).filter(
                                    (Htparam.paramgruppe == 4) &  (Htparam.paramnr == artikel.prov_code)).first()

                        fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist :FB_vatlist.bezeich == htparam.bezeich), first=True)
                        FB_vatlist.vatAmt = FB_vatlist.vatAmt + round(h_bill_line.betrag / fact * vat2%, price_decimal)
                ...
                if not vat_disc and h_bill_line.artnr == f_discart:
                    ...
                    h_vat2 = h_bill_line.betrag / fact * vat2%
                    h_vat2 = round(h_vat2, 2)
