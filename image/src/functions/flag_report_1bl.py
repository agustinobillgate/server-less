...
def flag_report_1bl(case_type:int, n:int, resnr:int, reslinnr:int, user_init:str, s_list_list:[S_list]):
    i:int = 0
    k:int = 0
    ...
    local_storage.debugging = local_storage.debugging + "19"
    ...
    if case_type == 1:
        local_storage.debugging = local_storage.debugging + ",Case1"
        if res_line:
        ...
        local_storage.debugging = local_storage.debugging + ",Case2"
        if not s_list or not(s_list.datum != None):
        ...
        if not s_list:
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                ...
                (Reslin_queasy.betriebsnr == n)).first()
            ...
            if not reslin_queasy:
                ...
                local_storage.debugging = local_storage.debugging + ",Add"
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.datum != None and i < (k + 3)), sort_by=[("datum",False)]):
                i = i + 1
                ...
                if i == k:
                    ...
                    reslin_queasy.char1 = reslin_queasy.char1 +\
                            ...
                            chr(2) + to_string(to_int(s_list.rsv_detail)) +\
                            chr(2) + to_string(to_int(s_list.bill_flag))
