...
def mk_resline_go_2bl(pvilanguage:int, accompany_tmpnr1:int, accompany_tmpnr2:int, accompany_tmpnr3:int, accompany_gastnr:int, accompany_gastnr2:int, accompany_gastnr3:int, comchild:int, rm_bcol:int, marknr:int, bill_instruct:int, restype:int, restype0:int, restype1:int, contact_nr:int, cutoff_days:int, segm__purcode:int, deposit:decimal, limitdate:date, wechsel_str:str, origcontcode:str, groupname:str, guestname:str, main_voucher:str, resline_comment:str, mainres_comment:str, purpose_svalue:str, letter_svalue:str, segm_svalue:str, source_svalue:str, res_mode:str, prev_zinr:str, memo_zinr:str, voucherno:str, contcode:str, child_age:str, flight1:str, flight2:str, eta:str, etd:str, user_init:str, currency_changed:bool, fixed_rate:bool, grpflag:bool, memozinr_readonly:bool, group_enable:bool, init_fixrate:bool, oral_flag:bool, pickup_flag:bool, drop_flag:bool, ebdisc_flag:bool, kbdisc_flag:bool, restricted:bool, sharer:bool, coder_exist:bool, gname_chged:bool, earlyci:bool, gdpr_flag:bool, mark_flag:bool, news_flag:bool, temp_segment:str, reslin_list:[Reslin_list], res_dynarate:[Res_dynarate], tot_qty:int):
    ...
    res_dynarate = reslin_list = nation_list = resline = bbuff = rline = rpbuff = res_line2 = rline2 = res_line1 = new_zkat = mbuff = waehrung1 = guest1 = rgast = qsy = accguest = b_receiver = gmember = bqsy = zbuff = zbuff1 = qsy_buff = None
    ...
    def generate_output():
        nonlocal update_kcard, msg_str, waehrungnr, reserve_dec, dyna_rmrate, accompany_tmpnr, ci_date, dynarate_created, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, max_resline, ind_gastnr, wig_gastnr, source_changed, priscilla_active, avail_gdpr, curr_nat, list_region, list_nat, loopi, avail_mark, avail_news, lvcarea, move_str, res_line, bill, htparam, queasy, nation, arrangement, reservation, bediener, outorder, zimmer, mealcoup, reslin_queasy, resplan, zimkateg, messages, waehrung, guest_pr, guest, res_history, master, brief, segment, sourccod, counters, mc_guest, interface, guestseg
        nonlocal resline, bbuff, rline, rpbuff, res_line2, rline2, res_line1, new_zkat, mbuff, waehrung1, guest1, rgast, qsy, accguest, b_receiver, gmember, bqsy, zbuff, zbuff1, qsy_buff


        nonlocal res_dynarate, reslin_list, nation_list, resline, bbuff, rline, rpbuff, res_line2, rline2, res_line1, new_zkat, mbuff, waehrung1, guest1, rgast, qsy, accguest, b_receiver, gmember, bqsy, zbuff, zbuff1, qsy_buff
        nonlocal res_dynarate_list, reslin_list_list, nation_list_list
        return {"update_kcard": update_kcard, "msg_str": msg_str, "waehrungnr": waehrungnr, "reserve_dec": reserve_dec, "dyna_rmrate": dyna_rmrate}
            ...
            while curr_date >= beg_datum and curr_date < rline.abreise:
                ...
                if resplan:
                    ...
                    if rpbuff:
                        ...
                        rpbuff = db_session.query(Rpbuff).first()

            curr_date = curr_date + 1

    def rmchg_ressharer(act_zinr:str, new_zinr:str, main_nr:int):
        ...
        res_line2 = db_session.query(Res_line2).filter(
                (Res_line2.resnr == reslin_list.resnr) &  (Res_line2.kontakt_nr == main_nr) &  (Res_line2.resstatus == 11) &  (((func.lower(Res_line2.zinr) != "") &  (func.lower(Res_line2.zinr) == act_zinr)) |  (func.lower(Res_line2.zinr) == ""))).first()
    ...
    def rmchg_sharer(act_zinr:str, new_zinr:str):
        ...
        if res_line.resstatus == 6 and res_recid1 != 0:
            ...
            res_line2 = db_session.query(Res_line2).filter(
                        (Res_line2.resnr == reslin_list.resnr) &  (func.lower(Res_line2.zinr) == (act_zinr).lower()) &  (Res_line2.resstatus == 13) &  (Res_line2.l_zuordnung[2] == 0)).first()
            while None != res_line2:
                ...
                for bill in db_session.query(Bill).filter(
                            (Bill.resnr == reslin_list.resnr) &  (Bill.parent_nr == res_line2.reslinnr)).all():
        ...
        if queasy:
            ...
            if waehrung1 and waehrung1.waehrungsnr != res_line.betriebsnr:
                ...
                if reslin_list.reserve_dec != 0:
                    reserve_dec = waehrung1.ankauf / waehrung1.einheit
                    res_line.reserve_dec = reserve_dec
                    reslin_list.reserve_dec = reserve_dec
    ...
    def update_mainres():
        ...
        if to_int(substring(ct, 0,1 + get_index(ct, " "))) != reservation.segmentcode:
        ...
        if to_int(substring(ct, 0,1 + get_index(ct, " "))) != reservation.resart:
    ...
    def update_resline():
        ...
        if accompany_gastnr > 0 or accompany_tmpnr[0] > 0:
            ...
            resline = db_session.query(Resline).filter(
                    (Resline.resnr == res_line.resnr) &  (Resline.active_flag <= 1) &  (Resline.kontakt_nr == res_line.reslinnr) &  (Resline.gastnrmember == accompany_gastnr) &  (Resline.l_zuordnung[2] == 1)).first()
        ...
        if accompany_gastnr2 > 0 or accompany_tmpnr[1] > 0:
            ...
            resline = db_session.query(Resline).filter(
                    (Resline.resnr == res_line.resnr) &  (Resline.active_flag <= 1) &  (Resline.kontakt_nr == res_line.reslinnr) &  (Resline.gastnrmember == accompany_gastnr2) &  (Resline.l_zuordnung[2] == 1)).first()
        ...
        if accompany_gastnr3 > 0 or accompany_tmpnr[2] > 0:
            ...
            resline = db_session.query(Resline).filter(
                    (Resline.resnr == res_line.resnr) &  (Resline.active_flag <= 1) &  (Resline.kontakt_nr == res_line.reslinnr) &  (Resline.gastnrmember == accompany_gastnr3) &  (Resline.l_zuordnung[2] == 1)).first()
        ...
        rline = db_session.query(Rline).filter(
                (Rline.resnr == reslin_list.resnr) &  (Rline.l_zuordnung[2] == 1) &  (Rline.kontakt_nr == reslin_list.reslinnr)).first()
        while None != rline:
            ...
            if avail_gdpr:
                ...
                if guest:
                    ...
                    mc_guest = db_session.query(Mc_guest).filter(
                            (Mc_guest.gastnr == reslin_list.gastnrmember)).first()
                    ...
                    if do_it:
                        ...
                        if not re.match(".*GDPR.*",reslin_list.zimmer_wunsch):
                            ct = ct + "GDPRyes;"

                        elif re.match(".*GDPRyes.*",reslin_list.zimmer_wunsch):
                            ct = ct + "GDPRno;"
                ...
                if re.match(".*GDPRyes.*",reslin_list.zimmer_wunsch):
                    ct = ct + "GDPRno;"

        if not gdpr_flag and re.match(".*GDPRyes.*",res_line.zimmer_wunsch):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        if gdpr_flag and (re.match(".*GDPRno.*",res_line.zimmer_wunsch) or not re.match(".*GDPR.*",res_line.zimmer_wunsch)):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        if not mark_flag and re.match(".*MARKETINGyes.*",res_line.zimmer_wunsch):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        if mark_flag and (re.match(".*MARKETINGno.*",res_line.zimmer_wunsch) or not re.match(".*MARKETING.*",res_line.zimmer_wunsch)):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        if not news_flag and re.match(".*NEWSLETTERyes.*",res_line.zimmer_wunsch):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        if news_flag and (re.match(".*NEWSLETTERno.*",res_line.zimmer_wunsch) or not re.match(".*NEWSLETTER.*",res_line.zimmer_wunsch)):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        for n in range(1,num_entries(reslin_list.zimmer_wunsch, ";") - 1 + 1) :
            ...
            elif substring(st, 0, 8) == "segm__pur":
                1

            elif substring(st, 0, 6) == "pickup":
                1

            elif substring(st, 0, 14) == "drop_passanger":
                1
        ...
        if segm__purcode != 0:
            ct = ct + "SEGM__PUR" + to_string(segm__purcode) + ";"

            resline = db_session.query(Resline).filter(
                    (Resline.resnr == res_line.resnr) &  (Resline.reslinnr != res_line.reslinnr) &  (Resline.active_flag <= 1) &  (Resline.resstatus != 12) &  (Resline.l_zuordnung[2] == 0)).first()
            while None != resline:

                if not re.match(".*SEGM__PUR.*",resline.zimmer_wunsch):
        ...
        if not pickup_flag and re.match(".*pickup.*",res_line.zimmer_wunsch):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        if not drop_flag and re.match(".*drop_passanger.*",res_line.zimmer_wunsch):
            res_history = Res_history()
            db_session.add(res_history)
        ...
        reslin_queasy = db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == "arrangement") &  (Reslin_queasy.resnr == reslin_list.resnr) &  (Reslin_queasy.reslinnr == reslin_list.reslinnr) &  (Reslin_queasy.date1 <= ratecode_date) &  (Reslin_queasy.date2 >= ratecode_date)).first()
        ...
        if not memozinr_readonly and res_line.memozinr.lower()  != (";" + memo_zinr + ";"):
        ...
        if restricted:
            ...
            if guest_pr:
                for datum in range(reslin_list.ankunft,(reslin_list.abreise - 1)  + 1) :

                    reslin_queasy = db_session.query(Reslin_queasy).filter(
                            (func.lower(Reslin_queasy.key) == "arrangement") &  (Reslin_queasy.resnr == reslin_list.resnr) &  (Reslin_queasy.reslinnr == reslin_list.reslinnr) &  (Reslin_queasy.date1 <= datum) &  (Reslin_queasy.date2 >= datum)).first()
    ...
    def res_dyna_rmrate():
        ...
        if reslin_list.ankunft != arrival_date or reslin_list.abreise != res_dynaRate.date2 + 1 or reslin_list.erwachs != res_dynaRate.adult or reslin_list.kind1 != res_dynaRate.child or reslin_list.arrangement != res_dynaRate.argt or zikatstr != res_dynaRate.rmcat or marknr != res_dynaRate.markNo:
            ...
            if reslin_list.ankunft == arrival_date and reslin_list.abreise == reslin_list.ankunft:
                ...
                msg_str = msg_str + chr(2) + "&W" + translateExtended ("Reservation data was changed,", lvcarea, "") + chr(10) + translateExtended ("Please re_check the rates.", lvcarea, "") + chr(2)
        dynarate_created = True

        for res_dynarate in query(res_dynarate_list):

            reslin_queasy = db_session.query(Reslin_queasy).filter(
                    (func.lower(Reslin_queasy.key) == "arrangement") &  (Reslin_queasy.resnr == reslin_list.resnr) &  (Reslin_queasy.reslinnr == reslin_list.reslinnr) &  (Reslin_queasy.date1 == res_dynaRate.date1) &  (Reslin_queasy.date2 == res_dynaRate.date2)).first()
    ...
    def update_qsy171():
        ...
        if res_line.zikatnr != reslin_list.zikatnr:
            ...
            if reslin_list.ankunft == reslin_list.abreise:
                ...
                upto_date = reslin_list.abreise - 1
            for datum in range(reslin_list.ankunft,upto_date + 1) :
            ...
            if res_line.ankunft == reslin_list.ankunft and res_line.abreise == reslin_list.abreise and res_line.resstatus != reslin_list.resstatus:
                ...
                if res_line.abreise > reslin_list.abreise:
                    for datum in range(reslin_list.abreise,res_line.abreise - 1  + 1) :
                    ...
                    for datum in range(res_line.abreise,reslin_list.abreise - 1 + 1) :
                ...
                if res_line.ankunft > reslin_list.ankunft:
                    for datum in range(reslin_list.ankunft,res_line.ankunft - 1 + 1) :
                    ...
                    for datum in range(res_line.ankunft,reslin_list.ankunft - 1 + 1) :
    ...
    if rm_bcol != 15 and reslin_list.ankunft != res_line.ankunft and reslin_list.resstatus == 1:
        ...
        if outorder:
            ...
            outorder.gespstart = reslin_list.ankunft - outorder.gespende +\
                    outorder.gespstart
    ...
    check_currency()
    get_output(res_changesbl(pvilanguage, res_mode, guestname, mainres_comment, resline_comment, user_init, earlyci, fixed_rate, reslin_list))
