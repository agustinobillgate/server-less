...
def to_string(input_value, format_spec=""):
    ...
    if format_spec.startswith("x("):
        ...
        minimal_datetime = datetime(1, 1, 1)
        # Create a timedelta object from the seconds
        td = timedelta(seconds=input_value)
        
        # Create a minimal datetime object (date doesn't matter here)
        minimal_datetime = datetime(1, 1, 1)
        
        # Add the timedelta to the minimal datetime
        time_datetime = minimal_datetime + td
        date_format = format_spec.replace("HH","%H").replace("MM","%M").replace("SS","%S")
        return time_datetime.strftime(date_format)            
    else:
