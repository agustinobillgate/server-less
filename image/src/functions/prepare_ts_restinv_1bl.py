...
def prepare_ts_restinv_1bl(pvilanguage:int, curr_dept:int, curr_printer:int, user_init_str:str, transdate:date):
    ...
    htparam = db_session.query(Htparam).filter(
            (htpara.paramnr == 870)).first()
    c_param870 = fchar

    htparam = db_session.query(Htparam).filter(
            (htpara.paramnr == 588)).first()
    activate_deposit = flogical
