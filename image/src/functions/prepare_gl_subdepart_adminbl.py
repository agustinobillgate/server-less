from functions.additional_functions import *
import decimal
from models import Queasy, Gl_department

def prepare_gl_subdepart_adminbl():
    t_queasy_list = []
    t_gl_depart_list = []
    queasy = gl_department = None

    t_queasy = t_gl_depart = None

    t_queasy_list, T_queasy = create_model_like(Queasy)
    t_gl_depart_list, T_gl_depart = create_model_like(Gl_department)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_queasy_list, t_gl_depart_list, queasy, gl_department
        nonlocal t_queasy, t_gl_depart
        nonlocal t_queasy_list, t_gl_depart_list
        return {"t-queasy": t_queasy_list, "t-gl-depart": t_gl_depart_list}

    pass

    for queasy in db_session.query(Queasy).filter((Queasy.key == 155)).all():
        t_queasy = T_queasy()
        t_queasy_list.append(t_queasy)
        buffer_copy(queasy, t_queasy)

    for gl_department in db_session.query(Gl_department).all():
        t_gl_depart = T_gl_depart()
        t_gl_depart_list.append(t_gl_depart)
        buffer_copy(gl_department, t_gl_depart)

    return generate_output()