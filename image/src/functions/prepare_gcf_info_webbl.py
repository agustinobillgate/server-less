...
from models import Guest, Artikel, Res_line, Hoteldpt, Guest_queasy, Reslin_queasy, Zimmer, Zimkateg, History

def prepare_gcf_info_webbl(pvilanguage:int, gastnr:int):
    payment = ""
    t_guest_list = []
    gcfinfo_list_list = []
    guest = artikel = res_line = hoteldpt = guest_queasy = reslin_queasy = zimmer = zimkateg = history = None

    gcfinfo_list = t_guest = None

    gcfinfo_list_list, Gcfinfo_list = create_model("Gcfinfo_list", {"nr":int, "dept":int, "rechnr":int, "datum":date, "str":str, "dept_str":str, "amount_food":str, "amount_bev":str, "amount_other":str, "room_number":str, "room_type":str, "arrival":str, "departure":str})
    t_guest_list, T_guest = create_model_like(Guest)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal payment, t_guest_list, gcfinfo_list_list, guest, artikel, res_line, hoteldpt, guest_queasy, reslin_queasy, zimmer, zimkateg, history
    ...
    def create_list():
        ...
        # reslin_queasy.reslinnr = None
        # reslin_queasy.reslinnr = 0
        htldpt = None
        ginfo = None
        str1:str = ""
        i:int = 0
        ...
        for ginfo in db_session.query(Ginfo).filter(
                (func.lower(Ginfo.key) == ("gast-info").lower()) &  (Ginfo.gastnr == gastnr)).order_by(Ginfo.number1, Ginfo.date1).all():
            ...
            if not htldpt or not(htldpt.num == ginfo.number1):
            ...
            if not reslin_queasy or not(reslin_queasy.reslinnr.resnr == ginfo.number2 and reslin_queasy.reslinnr.reslinnr == ginfo.number3):
                reslin_queasy = db_session.query(Reslin_queasy).filter(
                    (Reslin_queasy.resnr == ginfo.number2) &  (Reslin_queasy.reslinnr == ginfo.number3)).first()

            if reslin_queasy.reslinnr:
                cistr = to_string(reslin_queasy.reslinnr.ankunft, "99/99/99")
                costr = to_string(reslin_queasy.reslinnr.abreise, "99/99/99")
                rmno = reslin_queasy.reslinnr.zinr

                if not zimmer or not(zimmer.zinr.lower()  == (rmno).lower()):
                ...
                if not history or not(history.gastnr == gastnr and history.resnr == ginfo.number2 and history.reslinnr == ginfo.number3):
    ...
    def add_line(nr:int, deptname:str, amt_food:str, amt_bev:str, amt_other:str, room_no:str, rm_type:str, dept:int, rechnr:int, datum:date, arrival:str, departure:str):
    ...
    if not guest or not(guest.gastnr == gastnr):
    ...
    if guest:
        ...
        if guest.zahlungsart > 0:
            ...
            if not artikel or not(guest.departement == 0 and artikel.artnr == guest.zahlungsart):
