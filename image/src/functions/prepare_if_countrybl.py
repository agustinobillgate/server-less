...
zone_list_list, Zone_list = create_model("Zone_list", {"rec_id":int, "zone":str, "city":str, "acode":str, "info":str})

def prepare_if_countrybl(fl_run_proc:bool):
    global cost_list_list, zone_list_list, t_parameters_list, parameters
    cost_list_list = []
    zone_list_list = []
    t_parameters_list = []
    parameters = None
    ...
    def generate_output():
        ...
        print("nZOne:", len(zone_list_list))
    ...
    def cleanup_zonelist():
        ...
        parameters = db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == "if_internal") &  (func.lower(Parameters.section) == "dcode") &  (Parameters.varname > "")).first()

        if parameters:
            for parameters in db_session.query(Parameters).filter(
                    (func.lower(Parameters.progname) == "if_internal") &  (func.lower(Parameters.section) == "dcode") &  (Parameters.varname > "")).all():
    ...
    def create_costlist():
        ...
        nonlocal t_parameters, cost_list, zone_list
        # nonlocal t_parameters_list, cost_list_list, zone_list_list
        print("CreateCostList")
        i:int = 0
        m:int = 1
        n:int = 0

        for parameters in db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == "interface") &  (func.lower(Parameters.section) == "zone") &  (Parameters.varname > "")).all():
            cost_list = Cost_list()
            ...
            n = 0
            m = 1
            while i <= 8 and n < len(parameters.vstring) :
                n = n + 1
                ...
                if substring(parameters.vstring, n - 1, 1) == ";":
                    ...
                    if i == 1:
                        ...
                        if substring(parameters.vstring, n - 3 - 1, 1) == "." or substring(parameters.vstring, n - 3 - 1, 1) == ",":
                            cost_list.cost = decimal.Decimal(substring(parameters.vstring, m - 1, n - m - 3))
                            cost_list.cost = cost_list.cost + decimal.Decimal(substring(parameters.vstring, n - 2 - 1, 2)) / 100
                        else:
                            cost_list.cost = decimal.Decimal(substring(parameters.vstring, m - 1, n - m - 2))
                            cost_list.cost = cost_list.cost + decimal.Decimal(substring(parameters.vstring, n - 2 - 1, 2)) / 100
    ...
    def create_zonelist():
        ...
        nonlocal t_parameters, cost_list, zone_list
        # nonlocal t_parameters_list, cost_list_list, zone_list_list
        print("CreateZoneList")
        i:int = 0
        m:int = 1
        n:int = 0

        for parameters in db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == "interface") &  (func.lower(Parameters.section) == "dcode") &  (Parameters.varname > "")).all():
            zone_list = Zone_list()
            zone_list_list.append(zone_list)
            
            zone_list.rec_id = parameters._recid
            zone_list.zone = parameters.varname
            # print("Zone.1", parameters.vstring, zone_list)
            i = 1
            n = 0
            m = 1
            while i <= 3 and n < len(parameters.vstring) :
                n = n + 1
    ...
    for parameters in db_session.query(Parameters).filter(
            (func.lower(Parameters.progname) == "if_internal") &  (func.lower(Parameters.section) == "zone")).all():
    ...
    local_storage.debugging = f"nZone: {len(zone_list_list)}, nCost: {len(cost_list_list)}, nPar: {len(t_parameters_list)}"
