...
from models import Bk_reser, Guest, Htparam, Bk_veran, Bill, Bill_line

def mb_web_open_billbl(bil_recid:int, foreign_rate:bool, double_currency:bool, ba_dept:int):
    invno = ""
    gname = ""
    resname = ""
    rescomment = ""
    ...
    tot_room:int = 0
    room_no:str = ""
    bk_reser = guest = htparam = bk_veran = bill = bill_line = None

    t_bk_reser = t_guest = t_htparam = t_bk_veran = t_bill = t_bill_line = spbill_list = b_list = b1_list = None
    ...
    def generate_output():
        nonlocal invno, gname, resname, rescomment, printed, rechnr, balance, balance_foreign, kreditlimit, enbtn_bareserve, t_bill_line_list, t_bill_list, curr_select, telbill_flag, babill_flag, curr_gname, curr_invno, curr_b_recid, art_no, res_no, tot_adult, tot_room, room_no, bk_reser, guest, htparam, bk_veran, bill, bill_line


        nonlocal t_bk_reser, t_guest, t_htparam, t_bk_veran, t_bill, t_bill_line, spbill_list, b_list, b1_list
        nonlocal t_bk_reser_list, t_guest_list, t_htparam_list, t_bk_veran_list, t_bill_list, t_bill_line_list, spbill_list_list, b_list_list, b1_list_list
        return {"invno": invno, "gname": gname, "resname": resname, "rescomment": rescomment, "printed": printed, "rechnr": rechnr, "balance": balance, "balance_foreign": balance_foreign, "kreditlimit": kreditlimit, "enbtn_bareserve": enbtn_bareserve, "t-bill-line": t_bill_line_list, "t-bill": t_bill_list}
    ...
    resname = t_guest.name + ", " + t_guest.vorname1 + t_guest.anredefirma +\
            " " + t_guest.anrede1 +\
            chr (10) + t_guest.adresse1 +\
            chr (10) + t_guest.wohnort + " " + t_guest.plz +\
            chr (10) + t_guest.land
    rescomment = t_guest.bemerk
    art_no = t_guest.zahlungsart

    if t_bill.bilname != "" and t_bill.name != t_bill.bilname:
        rescomment = "Guest Name: " + t_bill.bilname + chr(10) + rescomment

    if t_bill.vesrdepot != "":
        rescomment = rescomment + chr(10) + t_bill.vesrdepot + chr(10)
    ...
    if t_bill.flag == 0 and t_bill.rechnr > 0 and t_billtyp == ba_dept and (t_bill.rechnr != int (invno)):
