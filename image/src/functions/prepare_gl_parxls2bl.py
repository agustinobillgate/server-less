...
def prepare_gl_parxls2bl(pvilanguage:int, briefnr:int, to_date:date, close_month:int):
    ...
    t_gl_acct_list, T_gl_acct = create_model("T_gl_acct", {"fibukonto":str, "bezeich":str, "acc_type":int, "main_nr":int, "deptnr":int, "actual":[decimal], "budget":[decimal], "last_yr":[decimal], "ly_budget":[decimal], "debit":[decimal]})
    t_exrate_list, T_exrate = create_model_like(Exrate)
    t_exrate1_list, T_exrate1 = create_model_like(Exrate)
    t_gl_accthis_list, T_gl_accthis = create_model("T_gl_accthis", {"fibukonto":str, "year":int, "actual":[decimal], "budget":[decimal], "last_yr":[decimal], "ly_budget":[decimal], "debit":[decimal]})
        ...
        for briefzei in db_session.query(Briefzei).filter(
            ...
            for i in range(1,len(briefzei.texte)  + 1) :
                ...
                if ord(substring(briefzei.texte, i - 1, 1)) == 10:
                    ...
                    if l > len(keycont) and substring(c, l - len(keycont) + 1 - 1, len(keycont)) == keycont:
                        continued = True
    ...
    def check_batch():
        ...
        for brief_list in query(brief_list_list):
            ...
            if batch_file:
                correct = check_to_int(check_to_int(texte)
    ...
    for exrate in db_session.query(Exrate).filter(
            (Exrate.artnr == 99999) |  (Exrate.artnr == 99998)).all():
        t_exrate = T_exrate()
    ...
    if c_param144 != "":
        ...
        waehrung = db_session.query(Waehrung).filter(
                (Waehrung.wabkurz == c_param144)).first()

        if waehrung:
            c_foreign_nr = waehrungsnr

    if c_foreign_nr != 0:

        for exrate in db_session.query(Exrate).filter(
                (Exrate.artnr == c_foreign_nr)).all():
            t_exrate1 = T_exrate1()
    ...
    for gl_accthis in db_session.query(Gl_accthis).filter(
            (Gl_accthis.YEAR == get_year(to_date))).all():
