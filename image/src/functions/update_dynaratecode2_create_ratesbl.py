...
def update_dynaratecode2_create_ratesbl(dynarate_list:[Dynarate_list], rmtype:str, currcode:str, from_date:date, to_date:date, market_number:int):
    rate_list1_list = []
    curr_date:date = None
    i:int = 0
    curr_i:int = 0
            ...
            if dynarate_list.w_day > 0 and dynarate_list.w_day != w_day:
                ...
                if zimkateg:
                    ...
                    ratecode = db_session.query(Ratecode).filter(
                            (Ratecode.CODE == dynarate_list.rCode) &  (Ratecode.startperiode <= to_date) &  (Ratecode.endperiode >= from_date) &  (Ratecode.zikatnr == inp_zikatnr) &  (Ratecode.marknr == market_number)).first()
                else:
