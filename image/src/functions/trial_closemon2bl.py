...
def trial_closemon2bl():
    ...
    def process_journal(jnr:int, datum:date):
        ...
        #         (Gl_journal.jnr == jnr) and  (Gl_journal.activeflag == 0)).first()
        for gl_journal in db_session.query(Gl_journal).filter(
                (Gl_journal.jnr == jnr) and  (Gl_journal.activeflag == 0)).all():
        # while None != gl_journal:
            # print("GL:", gl_journal.fibukonto, gl_journal._recid)
                ...
                if gl_acct.acc_type == 1:
                    ...
                    if wahrno != 0:
                        ...
                        exrate = db_session.query(Exrate).filter(
                                (Exrate.artnr == wahrno) and  (Exrate.datum == datum)).first()
    ...
    for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
            (Gl_jouhdr.activeflag == 0) and  (Gl_jouhdr.datum >= first_date) and  (Gl_jouhdr.datum <= curr_date)).all():
        process_journal(gl_jouhdr.jnr, gl_jouhdr.datum)

    exrate = db_session.query(Exrate).filter(
                (Exrate.artnr == 99999) and  (Exrate.datum == curr_date)).first()
