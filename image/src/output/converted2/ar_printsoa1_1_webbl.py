...
def ar_printsoa1_1_webbl(show_type:int, bof_month:date, eof_month:date, param_ar_list:[Param_ar], guestno:int, curr_day:date):
    ...
    soa_list_list, Soa_list = create_model("Soa_list", {"counter":int, "debref":int, "done_step":int, "artno":int, "vesrdep":Decimal, "to_sort":int, "outlet":bool, "datum":date, "ankunft":date, "abreise":date, "gastnr":int, "name":string, "inv_str":string, "rechnr":int, "refno":int, "voucherno":string, "voucherno1":string, "voucherno2":string, "saldo":Decimal, "fsaldo":Decimal, "printed":bool, "selected":bool, "printdate":date, "dptnr":int, "debt":Decimal, "credit":Decimal, "fdebt":Decimal, "fcredit":Decimal, "remarks":string, "arrecid":int, "newpayment":Decimal, "newfpayment":Decimal, "zinr":string, "erwachs":int, "child1":int, "child2":int, "roomrate":Decimal, "tot_amount":Decimal, "tot_balance":Decimal, "exrate":Decimal, "tot_exrate":Decimal, "gst_tot_non_taxable":Decimal, "gst_amount":Decimal, "gst_tot_sales":Decimal, "zimmeranz":int, "bill_datum":date, "resv_name":string, "rm_category":string, "gst_amount_str":string}, {"printdate": get_current_date()})
    ...
    def create_soalist():
        ...
        if show_type == 0:
            ...
            for debitor, guest in db_session.query(Debitor, Guest).join(Guest,(Guest.gastnr == Debitor.gastnr)).filter(
                     ((to_int(Debitor._recid).in_(list(set([soa_list.arrecid for soa_list in soa_list_list])))))).order_by(Debitor._recid).all():
                if debitor_obj_list.get(debitor._recid):
                    continue
                else:
                    debitor_obj_list[debitor._recid] = True

                soa_list = query(soa_list_list, (lambda soa_list: (to_int(debitor._recid) == soa_list.arrecid)), first=True)
                crdt = 0 debt == 0 fcrdt == 0 fdebt == 0
