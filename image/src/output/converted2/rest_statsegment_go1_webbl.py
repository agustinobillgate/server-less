...
input_list_list, Input_list = create_model("Input_list", {"pvilanguage":int, "from_date":date, "to_date":date, "sorttype":int, "excl_tax":bool})

def rest_statsegment_go1_webbl(input_list_list:[Input_list]):
    ...
    output_list_list, Output_list = create_model("Output_list", {"flag":string, "segm_no":string, "g_segm":string, "pax":string, "proz_pax":string, "t_rev":string, "proz_trev":string, "m_pax":string, "proz_mpax":string, "m_rev":string, "proz_mrev":string, "y_pax":string, "proz_ypax":string, "y_rev":string, "proz_yrev":string, "segm_gr":string})
    t_list_list, T_list = create_model("T_list", {"flag":string, "segm_no":int, "g_segm":string, "dept":int, "pax":int, "proz_pax":Decimal, "t_rev":Decimal, "proz_trev":Decimal, "m_pax":int, "proz_mpax":Decimal, "m_rev":Decimal, "proz_mrev":Decimal, "y_pax":int, "proz_ypax":Decimal, "y_rev":Decimal, "proz_yrev":Decimal, "segm_gr":int})
    segm_list_list, Segm_list = create_model("Segm_list", {"segmnr":int, "bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list_list, pvilanguage, price_decimal, black_list, start_year, long_digit, f_endkum, m_endkum, b_endkum, vat, vat2, service, fact, lvcarea, htparam, h_journal, h_bill, h_artikel, artikel, res_line, guestseg, segment
                ...
                if h_artikel:
                    ...
                    if artikel:
                        ...
                        if segm_no != 0:
                            ...
                            t_list = query(t_list_list, filters=(lambda t_list: t_list.segm_no == segm_no), first=True)
