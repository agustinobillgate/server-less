...
from models import History, Paramtext

def gcf_history_1bl(gastnr:int, fdate:date, tdate:date):

    prepare_cache ([History, Paramtext])

    ghistory_list = []
    summ_list_list = []
    htl_name:string = ""
    history = paramtext = None

    ghistory = summ_list = hist1 = None

    ghistory_list, Ghistory = create_model_like(History, {"hname":string, "gname":string, "address":string, "s_recid":int})
    ...
    def generate_output():
        nonlocal ghistory_list, summ_list_list, htl_name, history, paramtext
