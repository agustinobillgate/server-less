...
def over_crlimit_listbl(from_date:date, to_date:date, from_dept:int, to_dept:int, from_art:int, to_art:int):
    ...
    c_list_list, C_list = create_model("C_list", {"name":string, "rechnr":int, "p_artnr":int, "datum":date, "dept":int, "betrag":Decimal, "f_betrag":Decimal})
    output_list_list, Output_list = create_model("Output_list", {"guest_name":string, "artnr":int, "art_desc":string, "card_no":string, "credit_limit":Decimal, "amount":Decimal, "balanced":Decimal})
    out_list_list, Out_list = create_model("Out_list", {"guest_name":string, "artnr":int, "art_desc":string, "card_no":string, "credit_limit":Decimal, "amount":Decimal, "balanced":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal out_list_list, foreign_nr, h_artikel, htparam, waehrung, hoteldpt, h_compli, h_bill, h_journal, queasy, h_bill_line, exrate
    ...
    def create_list():
        ...
        for queasy in db_session.query(Queasy).filter(
            ...
            output_list = query(output_list_list, filters=(lambda output_list: output_list.guest_name.lower()  == (bezeich).lower()), first=True)
        ...
        for output_list in query(output_list_list, filters=(lambda output_list: output_list.artnr >= from_art and output_list.artnr <= to_art), sort_by=[("guest_name",False)]):
            out_list = Out_list()
