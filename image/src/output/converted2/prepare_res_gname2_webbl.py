...
def prepare_res_gname2_webbl(resnr:int):
    ...
    nation1_list, Nation1 = create_model("Nation1", {"kurzbez":string})
    grprline_list_list, Grprline_list = create_model("Grprline_list", {"s_recid":int, "zinr":string})
    ...
    zimkateg_list_list, Zimkateg_list = create_model("Zimkateg_list", {"zikatnr":int, "kurzbez":string})
    ...
    zimmer_list_list, Zimmer_list = create_model("Zimmer_list", {"zinr":string})
    active_roomlist_list, Active_roomlist = create_model_like(Zimmer, {"room_selected":bool, "res_recid":int, "arrival_date":date, "departure_date":date})
    room_resline_list, Room_resline = create_model("Room_resline", {"zinr":string, "zikatnr":int, "setup":int})
    room_ooo_list, Room_ooo = create_model_like(Outorder, {"zikatnr":int, "setup":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal if_flag, ci_date, default_nat, htparam_feldtyp, htparam_flogical, master_exist, htparam_flogical2, troom, grprline_list_list, s_list_list, zimkateg_list_list, resline_list_list, zimmer_list_list, nation1_list, active_roomlist_list, do_it, zimmer, outorder, master, htparam, reservation, guest, zimkateg, nation, res_line
        nonlocal resnr


        nonlocal nation1, grprline_list, s_list, zimkateg_list, resline_list, zimmer_list, active_roomlist, room_resline, room_ooo
        nonlocal nation1_list, grprline_list_list, s_list_list, zimkateg_list_list, resline_list_list, zimmer_list_list, active_roomlist_list, room_resline_list, room_ooo_list

        return {"if_flag": if_flag, "ci_date": ci_date, "default_nat": default_nat, "htparam_feldtyp": htparam_feldtyp, "htparam_flogical": htparam_flogical, "master_exist": master_exist, "htparam_flogical2": htparam_flogical2, "troom": troom, "grprline-list": grprline_list_list, "s-list": s_list_list, "zimkateg-list": zimkateg_list_list, "resline-list": resline_list_list, "zimmer-list": zimmer_list_list, "nation1": nation1_list, "active-roomlist": active_roomlist_list}
    ...
    for zimmer in db_session.query(Zimmer).filter(
        ...
        resline_list = query(resline_list_list, (lambda resline_list: resline_list.zikatnr == zimmer.zikatnr), first=True)
        if not resline_list:
            continue
        ...
        if not outorder:
            ...
            if zimmer.zistatus == 6 and resline_list.ankunft <= ci_date:
                ...
                active_roomlist_list.append(active_roomlist)
    ...
    for res_line in db_session.query(Res_line).filter(
        ...
        room_resline_list.append(room_resline)

        room_resline.zinr = res_line.zinr
        room_resline.zikatnr = res_line.zikatnr
        room_resline.setup = res_line.setup

    for active_roomlist in query(active_roomlist_list):

        room_resline = query(room_resline_list, filters=(lambda room_resline: room_resline.zinr == active_roomlist.zinr and room_resline.zikatnr == active_roomlist.zikatnr), first=True)

        if room_resline:
            active_roomlist_list.remove(active_roomlist)
