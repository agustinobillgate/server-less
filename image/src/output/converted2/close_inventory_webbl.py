...
def close_inventory_webbl(inv_type:int, user_init:str, port:str):
    its_ok = False
    msg_str = ""
    loopi:int = 0
    curr_folder:str = ""
    ...
    def cqueasy(bezeich:str, str_process:str):
        ...
        queasy = db_session.query(Queasy).filter(
                 (Queasy.key == 279) & (func.lower(Queasy.char2) == (bezeich).lower()) & (Queasy.date1 == get_current_date())).first()
    ...
    if inv_type == 1:
        ...
        if fb_close_date == last_journ_transgl:
            ...
            for ophis_fnb in db_session.query(Ophis_fnb).filter(
                     (get_month(Ophis_fnb.datum) == get_month(fb_close_date)) & (get_year(Ophis_fnb.datum) == get_year(fb_close_date))).order_by(Ophis_fnb._recid).all():

                l_artikel = db_session.query(L_artikel).filter(
                         (L_artikel.artnr == ophis_fnb.artnr) & (L_artikel.endkum <= 2)).first()
        ...
        if mat_close_date == last_journ_transgl:
            ...
            for ophis_mat in db_session.query(Ophis_mat).filter(
                     (get_month(Ophis_mat.datum) == get_month(mat_close_date)) & (get_year(Ophis_mat.datum) == get_year(mat_close_date))).order_by(Ophis_mat._recid).all():

                l_artikel = db_session.query(L_artikel).filter(
                         (L_artikel.artnr == ophis_mat.artnr) & (L_artikel.endkum > 2)).first()
