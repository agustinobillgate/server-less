...
def pj_inhouse4_summary_webbl(sorttype:int, from_date:date, to_date:date, froom:string, troom:string, exc_depart:bool, incl_gcomment:bool, incl_rsvcomment:bool, disp_accompany:bool):
    ...
    output_list_list, Output_list = create_model("Output_list", {"resnr":int, "qty":int, "rmcat":string, "rmno":string, "nation":string, "arrive":date, "depart":date, "zipreis":Decimal, "kurzbez":string, "bezeich":string, "a":int, "c":int, "co":int, "nat":string, "lodging":Decimal, "breakfast":Decimal, "lunch":Decimal, "dinner":Decimal, "otherev":Decimal, "curr":string, "paym":int, "segm":string})
    ...
    def generate_output():
        nonlocal tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, inactive, tot_keycard, summary_roomtype_list, summary_nation_list, summary_revenue_list, summary_segment_list, curr_date, curr_gastnr, prog_name, tot_qty, tot_rev, company, counter, queasy, zimkateg
        ...
        nonlocal inhouse_guest_list_list, output_list_list, tmplist_list, s_list_list, summary_roomtype_list, summary_nation_list, summary_revenue_list, summary_segment_list, t_buff_queasy_list, sum_list_list, zikat_list_list

        return {"tot_payrm": tot_payrm, "tot_rm": tot_rm, "tot_a": tot_a, "tot_c": tot_c, "tot_co": tot_co, "tot_avail": tot_avail, "inactive": inactive, "tot_keycard": tot_keycard, "summary-roomtype": summary_roomtype_list, "summary-nation": summary_nation_list, "summary-revenue": summary_revenue_list, "summary-segment": summary_segment_list}
    ...
    tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, inactive, tot_keycard, output_list_list, s_list_list, t_buff_queasy_list = get_output(pj_inhouse4_summary_cldbl(1, from_date, to_date, curr_date, curr_gastnr, froom, troom, exc_depart, incl_gcomment, incl_rsvcomment, "PJ-inhouse2", disp_accompany, zikat_list_list))
    sum_list_list.clear()
