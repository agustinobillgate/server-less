...
from models import Bediener, L_lieferant, L_orderhdr, Parameters, L_order, L_artikel, L_op, L_pprice, Gl_acct

def pr_list_disp_list_1bl(char1:string, billdate:date, from_date:date, to_date:date, outstand_flag:bool, expired_flag:bool, approve_flag:bool, reject_flag:bool):

    prepare_cache ([Bediener, L_lieferant, L_orderhdr, Parameters, L_order, L_artikel, L_pprice, Gl_acct])

    s_list_list = []
    bediener = l_lieferant = l_orderhdr = parameters = l_order = l_artikel = l_op = l_pprice = gl_acct = None

    s_list = usrbuff = t_lieferant = sbuff = tbuff = None
    ...
    def generate_output():
        nonlocal s_list_list, bediener, l_lieferant, l_orderhdr, parameters, l_order, l_artikel, l_op, l_pprice, gl_acct
        nonlocal char1, billdate, from_date, to_date, outstand_flag, expired_flag, approve_flag, reject_flag
        nonlocal usrbuff, t_lieferant


        nonlocal s_list, usrbuff, t_lieferant, sbuff, tbuff
    ...
    def disp_list(pr_nr:string):
        ...
        if pr_nr != "":
            ...
            l_order = L_order()
            l_artikel = L_artikel()
            for l_order.loeschflag, l_order.lief_fax, l_order._recid, l_order.docu_nr, l_order.pos, l_order.anzahl, l_order.stornogrund, l_order.bestelldatum, l_order.lieferdatum_eff, l_order.besteller, l_order.angebot_lief, l_order.einzelpreis, l_order.warenwert, l_order.txtnr, l_order.zeit, l_order.bestellart, l_order.lieferdatum, l_artikel.artnr, l_artikel.bezeich, l_artikel.traubensorte, l_artikel.lief_einheit, l_artikel.min_bestand, l_artikel.anzverbrauch, l_artikel.masseinheit, l_artikel.ek_letzter, l_artikel._recid in db_session.query(L_order.loeschflag, L_order.lief_fax, L_order._recid, L_order.docu_nr, L_order.pos, L_order.anzahl, L_order.stornogrund, L_order.bestelldatum, L_order.lieferdatum_eff, L_order.besteller, L_order.angebot_lief, L_order.einzelpreis, L_order.warenwert, L_order.txtnr, L_order.zeit, L_order.bestellart, L_order.lieferdatum, L_artikel.artnr, L_artikel.bezeich, L_artikel.traubensorte, L_artikel.lief_einheit, L_artikel.min_bestand, L_artikel.anzverbrauch, L_artikel.masseinheit, L_artikel.ek_letzter, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                     (L_order.docu_nr == l_orderhdr.docu_nr) & (L_order.loeschflag <= 1) & (L_order.pos > 0) & (L_order.lief_nr == 0)).order_by(L_artikel.bezeich).all():
                ...
                l_op = db_session.query(L_op).filter(
                         (L_op.op_art == 1) & (L_op.artnr == l_artikel.artnr)).order_by(L_op._recid.desc()).first()

                if l_op:
                    s_list.last_pprice =  to_decimal(l_artikel.ek_letzter)

                l_pprice = db_session.query(L_pprice).filter(
                         (L_pprice.artnr == l_artikel.artnr)).order_by(L_pprice._recid.desc()).first()

                if l_pprice:

                    t_lieferant = get_cache (L_lieferant, {"lief_nr": [(eq, l_pprice.lief_nr)]})
                    s_list.last_pdate = l_pprice.bestelldatum
                    s_list.last_pprice =  to_decimal(l_pprice.einzelpreis)
        ...
        for l_orderhdr in db_session.query(L_orderhdr).filter(
            ...
            if do_it:
                ...
                l_artikel = L_artikel()
                for l_order.loeschflag, l_order.lief_fax, l_order._recid, l_order.docu_nr, l_order.pos, l_order.anzahl, l_order.stornogrund, l_order.bestelldatum, l_order.lieferdatum_eff, l_order.besteller, l_order.angebot_lief, l_order.einzelpreis, l_order.warenwert, l_order.txtnr, l_order.zeit, l_order.bestellart, l_order.lieferdatum, l_artikel.artnr, l_artikel.bezeich, l_artikel.traubensorte, l_artikel.lief_einheit, l_artikel.min_bestand, l_artikel.anzverbrauch, l_artikel.masseinheit, l_artikel.ek_letzter, l_artikel._recid in db_session.query(L_order.loeschflag, L_order.lief_fax, L_order._recid, L_order.docu_nr, L_order.pos, L_order.anzahl, L_order.stornogrund, L_order.bestelldatum, L_order.lieferdatum_eff, L_order.besteller, L_order.angebot_lief, L_order.einzelpreis, L_order.warenwert, L_order.txtnr, L_order.zeit, L_order.bestellart, L_order.lieferdatum, L_artikel.artnr, L_artikel.bezeich, L_artikel.traubensorte, L_artikel.lief_einheit, L_artikel.min_bestand, L_artikel.anzverbrauch, L_artikel.masseinheit, L_artikel.ek_letzter, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                             (L_order.docu_nr == l_orderhdr.docu_nr) & (L_order.pos > 0) & (L_order.lief_nr == 0)).order_by(L_order.docu_nr, L_artikel.bezeich).all():
                    if l_order_obj_list.get(l_order._recid):
                        continue
                    else:
                        l_order_obj_list[l_order._recid] = True
