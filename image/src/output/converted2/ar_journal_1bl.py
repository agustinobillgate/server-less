...
def ar_journal_1bl(from_art:int, to_art:int, from_dept:int, to_dept:int, from_date:date, to_date:date):
    ...
    output_list_list, Output_list = create_model("Output_list", {"datum":date, "room_no":string, "bill_no":int, "art_no":int, "description":string, "voucher_no":string, "departement":string, "qty":int, "amount":Decimal, "zeit":string, "id":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list_list, long_digit, qty, sub_tot, tot, curr_date, last_dept, it_exist, descr1, voucher_no, ind, gdelimiter, cnt, i, artikel, billjournal, bill, hoteldpt
