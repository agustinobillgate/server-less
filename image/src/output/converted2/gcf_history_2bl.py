...
from models import History, Paramtext, Res_line

def gcf_history_2bl(gastnr:int, fdate:date, tdate:date):

    prepare_cache ([History, Paramtext, Res_line])
    ...
    str:string = ""
    i:int = 0
    history = paramtext = res_line = None

    ghistory = summ_list = hist1 = None

    ghistory_list, Ghistory = create_model_like(History, {"hname":string, "gname":string, "address":string, "s_recid":int, "vcrnr":string})
    ...
    def generate_output():
        nonlocal ghistory_list, summ_list_list, htl_name, str, i, history, paramtext, res_line
    ...
    def create_ghistory():
        ...
        for history in db_session.query(History).filter(
            ...
            if hist1:
                ...
                ghistory.reslinnr = hist1.reslinnr
