...
def mobileweb_self_cibl(co_date:date, book_code:string, ch_name:string, early_ci:bool, max_room:int, citime:string, groupflag:bool):
    ...
    arrival_guest_list, Arrival_guest = create_model("Arrival_guest", {"i_counter":int, "gastno":int, "resnr":int, "reslinnr":int, "gast":string, "ci":date, "co":date, "rmtype":string, "zinr":string, "argt":string, "adult":string, "child":string, "rmtype_str":string, "room_sharer":bool, "pre_checkin":bool, "argt_str":string, "preference":string, "new_zinr":bool, "zikatnr":int, "l_selected":bool, "kontakt_nr":int, "room_stat":int, "res_status":int})
    ...
    if not arrival_guest:
        ...
        return generate_output()
    arrival_guest_list, err_code = get_output(iproom_assignmentbl(arrival_guest_list))
