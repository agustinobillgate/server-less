...
def rm_atproduct_create_umsatz1_webbl(ci_date:date, tdate:date, to_date:date, show_ftd:bool, curr_id_screenvalue:string, currency_type:int, cardtype:int, fdate:date):
    ...
    rm_atproduct_list, Rm_atproduct = create_model("Rm_atproduct", {"name":string, "room":string, "pax":string, "lodg":string, "proz":string, "rm_rate":string, "mtd_room":string, "mtd_pax":string, "mtd_lodg":string, "mtdproz":string, "mtd_rm_rate":string, "ytd_room":string, "ytd_pax":string, "ytd_lodg":string, "ytd_proz":string, "ytd_rm_rate":string})
    sales_list_list, Sales_list = create_model("Sales_list", {"sales_id":string, "sales_name":string})
    to_list_list, To_list = create_model("To_list", {"sales_id":string, "gastnr":int, "name":string, "room":int, "croom":int, "pax":int, "logis":Decimal, "proz":Decimal, "avrgrate":Decimal, "m_room":int, "mc_room":int, "m_pax":int, "m_logis":Decimal, "m_proz":Decimal, "m_avrgrate":Decimal, "y_room":int, "yc_room":int, "y_pax":int, "y_logis":Decimal, "y_proz":Decimal, "y_avrgrate":Decimal})
    ...
    def create_umsatz1():
        ...
        for to_list in query(to_list_list, sort_by=[("sales_id",False),("name",False)]):
