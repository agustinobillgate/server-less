...
def read_zimmerbl(case_type:int, rmno:string, zikatno:int, setupno:int):
    ...
    def generate_output():
        nonlocal t_zimmer_list, counter, str, zimmer
    ...
    if case_type == 1:
        ...
        if num_entries(rmno, ";") > 1:
            for counter in range(1,num_entries(rmno, ";")  + 1) :
                ...
                if zimmer:
                    ...
                    t_zimmer = query(t_zimmer_list, filters=(lambda t_zimmer: t_zimmer.zinr.lower()  == (str).lower()), first=True)
        ...
        zimmer = get_cache (Zimmer, {"setup": [(eq, setupno - 9200)]})
