...
scost_list, Scost = create_model_like(Eg_cost, {"strmonth":string})

def eg_cost_btn_okbl(case_type:int, scost_list:[Scost], blframe:int, fdate1:date, tdate1:date, intres:int, intres4:int, fyear1:int, rmonth:int, fusage:int, fprice:Decimal, fval:Decimal):
    eg_cost = None
    ...
    if case_type == 1:
        ...
        if blframe == 0:
            ...
            for eg_cost in db_session.query(Eg_cost).filter(
                     (Eg_cost.datum >= fdate1) & (Eg_cost.datum <= tdate1) & (Eg_cost.resource_nr == intres)).order_by(Eg_cost._recid).all():
            ...
            for eg_cost in db_session.query(Eg_cost).filter(
                     (Eg_cost.datum >= fdate1) & (Eg_cost.datum <= tdate1) & (Eg_cost.resource_nr == intres4)).order_by(Eg_cost._recid).all():
                db_session.delete(eg_cost)
