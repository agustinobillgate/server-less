...
from models import H_artikel, H_bill_line, Queasy, H_bill, Wgrpdep, Hoteldpt, Kellner, Printcod, H_journal, H_mjourn
...
def ts_restinv_print_orderticket_webbl(submenu_list_list:[Submenu_list], pvilanguage:int, close_it:bool, reprint_it:bool, h_bill_rechnr:int, curr_dept:int, disc_art1:int, disc_art2:int, disc_art3:int, prorder:int, desclength:int):

    prepare_cache ([H_artikel, H_bill_line, Queasy, H_bill, Wgrpdep, Hoteldpt, Kellner, Printcod, H_journal, H_mjourn])
    ...
    h_artikel = h_bill_line = queasy = h_bill = wgrpdep = hoteldpt = kellner = printcod = h_journal = h_mjourn = None

    submenu_list = header_list = output_list = abuff = hbuff = hbline = qsy = None

    header_list_list, Header_list = create_model("Header_list", {"k":int, "depart":string, "tableno":string, "datum":date, "pax":string, "zeit":string, "ordertaker":string, "waiter":string, "bill_no":string, "guest_name":string})
    output_list_list, Output_list = create_model("Output_list", {"bill_no":int, "bezeich":string, "pos":int, "subgrp":string, "submenu_request":string, "printcod_concod":string, "aendertext":string, "pax":string, "stornogrund":string, "submenu_bezeich":string, "bezeich2":string})
    ...
    def generate_output():
        nonlocal k, header_list_list, output_list_list, prev_zknr, do_it, recid_h_bill_line, lvcarea, h_artikel, h_bill_line, queasy, h_bill, wgrpdep, hoteldpt, kellner, printcod, h_journal, h_mjourn
        ...
        nonlocal submenu_list, header_list, output_list, abuff, hbuff, hbline, qsy
        nonlocal header_list_list, output_list_list

        return {"k": k, "header-list": header_list_list, "output-list": output_list_list}
    ...
    def write_article(s_recid:int, prev_zknr:int):
        ...
        nonlocal k, header_list_list, output_list_list, do_it, recid_h_bill_line, lvcarea, h_artikel, h_bill_line, queasy, h_bill, wgrpdep, hoteldpt, kellner, printcod, h_journal, h_mjourn
        nonlocal pvilanguage, close_it, reprint_it, h_bill_rechnr, curr_dept, disc_art1, disc_art2, disc_art3, prorder, desclength
        nonlocal abuff, hbuff, hbline, qsy
        ...
        if h_artikel.betriebsnr > 0:
            ...
            for submenu_list in query(submenu_list_list, filters=(lambda submenu_list: submenu_list.nr == h_artikel.betriebsnr and submenu_list.zeit == hbuff.zeit)):
                created = True
            ...
            if not created:
                ...
                if h_journal:
                    ...
                    for h_mjourn in db_session.query(H_mjourn).filter(
                        ...
                        if h_art:
                            created = True
                            output_list.submenu_bezeich = "-> " + to_string(h_art.bezeich)
