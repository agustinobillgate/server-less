...
def pr_sphbill1_lnl_1bl(pvilanguage:int, hbrecid:int, printnr:int, use_h_queasy:bool, session_parameter:string, user_init:string, billnr:int, print_all:bool):
    ...
    art_list = t_printer = vat_list = output_list = bline_vatlist = artbuff = abuff = None

    art_list_list, Art_list = create_model("Art_list", {"printed":bool, "disc_flag":bool, "dept":int, "artnr":int, "bezeich":string, "artart":int, "zwkum":int, "qty":int, "price":Decimal, "amount":Decimal, "betrag":Decimal, "happyhr":string, "datum":date, "sysdate":date, "zeit":int, "condiment":int, "barcode":string})
    ...
    output_list_list, Output_list = create_model("Output_list", {"str":string, "str_pos":int, "pos":int, "flag_popup":bool, "npause":int, "sort_i":int})
    ...
    def generate_output():
        ...
        nonlocal art_list_list, t_printer_list, vat_list_list, output_list_list, bline_vatlist_list

        return {"print_all": print_all, "winprinterflag": winprinterflag, "filename": filename, "msg_str": msg_str, "output-list": output_list_list, "t-printer": t_printer_list}
    ...
    def print_overhead1():
        ...
        if printed_line == 0 or print_all:
            ...
            if h_bill.bilname != "":
                ...
                output_list.str = output_list.str + " " + translateExtended ("Table", lvcarea, "") + " " + to_string(h_bill.tischnr, ">>>>>9") + "/" + to_string(h_bill.belegung, ">>9 ") + "(" + to_string(billnr, ">>>9") + ") " + to_string(kname, "x(32)") + order_id
                output_list = Output_list()
                output_list_list.append(output_list)
