...
def ar_age1_2_list_webbl(pvilanguage:int, case_type:int, mtd_flag:bool, to_date:date, fdate:date, tdate:date, from_art:int, to_art:int, from_name:string, to_name:string, disptype:int, mi_bill:bool, day1:int, day2:int, day3:int, detailed:bool, cvt_flag:bool, dollar_rate:Decimal):
    ...
    arage_balance_list_list, Arage_balance_list = create_model("Arage_balance_list", {"strdate":string, "curr":string, "gastnr":int, "artnr":int, "age1":string, "age2":string, "age3":string, "age4":string, "rechnr":string, "num":string, "cust_nm":string, "bill_nm":string, "p_bal":string, "debit":string, "credit":string, "end_bal":string, "creditlimit_str":string})
    arage_list_list, Arage_list = create_model("Arage_list", {"strdate":string, "curr":string, "gastnr":int, "artnr":int, "age1":string, "age2":string, "age3":string, "age4":string, "rechnr":string, "str":string, "num":string, "cust_nm":string, "bill_nm":string, "p_bal":string, "debit":string, "credit":string, "end_bal":string, "creditlimit_str":string})
    age_list_list, Age_list = create_model("Age_list", {"fcurr":string, "artnr":int, "rechnr":int, "counter":int, "gastnr":int, "rgdatum":date, "gastname":string, "billname":string, "p_bal":Decimal, "debit":Decimal, "credit":Decimal, "saldo":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal, "opart":int, "credit_limit":Decimal})
    ledger_list, Ledger = create_model("Ledger", {"artnr":int, "bezeich":string, "p_bal":Decimal, "debit":Decimal, "credit":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal})
    ...
    def create_output():
        ...
        if curr_gastnr == 0:
            ...
            if p_bal != 0 or debit != 0 or credit != 0:
                ...
                if not long_digit:
                    outlist = " " + to_string(counter, ">>>9 ") + to_string(gastname, "x(30)") + to_string(billname, "x(30)") + to_string(tot_debt, "->>,>>>,>>>,>>9.99") + to_string(p_bal, "->>,>>>,>>>,>>9.99") + to_string(debit, "->>,>>>,>>>,>>9.99") + to_string(credit, "->>,>>>,>>>,>>9.99") + to_string(debt0, "->>,>>>,>>>,>>9.99") + to_string(debt1, "->>,>>>,>>>,>>9.99") + to_string(debt2, "->>,>>>,>>>,>>9.99") + to_string(debt3, "->>,>>>,>>>,>>9.99") + to_string(credit_limit, "->>,>>>,>>>,>>9.99")
                else:
                    outlist = " " + to_string(counter, ">>>9 ") + to_string(gastname, "x(30)") + to_string(billname, "x(30)") + to_string(tot_debt, "->,>>>,>>>,>>>,>>9") + to_string(p_bal, "->,>>>,>>>,>>>,>>9") + to_string(debit, "->,>>>,>>>,>>>,>>9") + to_string(credit, "->,>>>,>>>,>>>,>>9") + to_string(debt0, "->,>>>,>>>,>>>,>>9") + to_string(debt1, "->,>>>,>>>,>>>,>>9") + to_string(debt2, "->,>>>,>>>,>>>,>>9") + to_string(debt3, "->,>>>,>>>,>>>,>>9") + to_string(credit_limit, "->,>>>,>>>,>>>,>>9")
    ...
    def fill_in_list(fill_billno:bool, currency:string, curr_rgdatum:date):
        ...
        nonlocal arage_balance_list_list, curr_art, counter, t_saldo, t_prev, t_debit, t_credit, t_debt0, t_debt1, t_debt2, t_debt3, tmp_saldo, curr_name, curr_gastnr, gastname, billname, p_bal, debit, credit, debt0, debt1, debt2, debt3, credit_limit, curr_counter, curr_fcurr, i, tot_debt, from_date, default_fcurr, outlist, long_digit, bill_number, inp_gastnr, inp_artnr, tot_debtall, tot_debt0, tot_debt1, tot_debt2, tot_debt3, curr_time, lvcarea, debitor, htparam, artikel, guest, waehrung, h_bill, bill, res_line
        nonlocal pvilanguage, case_type, mtd_flag, to_date, fdate, tdate, from_art, to_art, from_name, to_name, disptype, mi_bill, day1, day2, day3, detailed, cvt_flag, dollar_rate
        nonlocal debtrec, debt
        ...
        if fill_billno:
            ...
            arage_list.num = to_string(counter, ">>>9 ")
            arage_list.cust_nm = to_string(gastname, "x(30)")
            arage_list.bill_nm = to_string(billname, "x(30)")
            arage_list.end_bal = to_string(tot_debt, "->>,>>>,>>>,>>9.99")
            arage_list.p_bal = to_string(p_bal, "->>,>>>,>>>,>>9.99")
            arage_list.debit = to_string(debit, "->>,>>>,>>>,>>9.99")
            arage_list.credit = to_string(credit, "->>,>>>,>>>,>>9.99")
            arage_list.age1 = to_string(debt0, "->>,>>>,>>>,>>9.99")
            arage_list.age2 = to_string(debt1, "->>,>>>,>>>,>>9.99")
            arage_list.age3 = to_string(debt2, "->>,>>>,>>>,>>9.99")
            arage_list.age4 = to_string(debt3, "->>,>>>,>>>,>>9.99")
            arage_list.creditlimit_str = to_string(credit_limit, "->>,>>>,>>>,>>9.99")

            if curr_rgdatum != None:
                arage_list.strdate = to_string(curr_rgdatum, "99/99/99")

        if matches(outlist,r"*T O T A L A/R*") and fill_billno == False:
            arage_list.cust_nm = "T O T A L A/R"
            arage_list.bill_nm = "T O T A L A/R"
            arage_list.end_bal = to_string(t_saldo, "->,>>>,>>>,>>>,>>9")
            arage_list.p_bal = to_string(t_prev, "->>,>>>,>>>,>>9.99")
            arage_list.debit = to_string(t_debit, "->>,>>>,>>>,>>9.99")
            arage_list.credit = to_string(t_credit, "->>,>>>,>>>,>>9.99")
        ...
        elif matches(outlist,r"*T o t a l*") and fill_billno == False:
            arage_list.cust_nm = "T o t a l"
            arage_list.bill_nm = "T o t a l"
            arage_list.end_bal = to_string(ledger.tot_debt, "->,>>>,>>>,>>>,>>9")
            arage_list.p_bal = to_string(ledger.p_bal, "->>,>>>,>>>,>>9.99")
            arage_list.debit = to_string(ledger.debit, "->>,>>>,>>>,>>9.99")
            arage_list.credit = to_string(ledger.credit, "->>,>>>,>>>,>>9.99")
            arage_list.age1 = to_string(ledger.debt0, "->>,>>>,>>>,>>9.99")
            arage_list.age2 = to_string(ledger.debt1, "->>,>>>,>>>,>>9.99")
            arage_list.age3 = to_string(ledger.debt2, "->>,>>>,>>>,>>9.99")
            arage_list.age4 = to_string(ledger.debt3, "->>,>>>,>>>,>>9.99")

        elif matches(outlist,r"*Statistic Percentage*") and fill_billno == False:
            arage_list.cust_nm = "Statistic Percentage (%) :"
            arage_list.bill_nm = "Statistic Percentage (%) :"
            arage_list.p_bal = " 100.00"
            arage_list.age1 = to_string(tot_debt0, " ->,>>9.99")
            arage_list.age2 = to_string(tot_debt1, " ->,>>9.99")
            arage_list.age3 = to_string(tot_debt2, " ->,>>9.99")
            arage_list.age4 = to_string(tot_debt3, " ->,>>9.99")

        elif outlist != "" and num_entries(outlist, "-") >= 2 and fill_billno == False:
            arage_list.num = to_string(entry(0, outlist, "-"))
            arage_list.cust_nm = "-" + to_string(entry(1, outlist, "-"))
        ...
        arage_list.age2 = substring(arage_list.str, 157, 18)
        arage_list.age3 = substring(arage_list.str, 175, 18)
        arage_list.age4 = substring(arage_list.str, 193, 18)
    ...
    def age_list1():
        ...
        for artikel in db_session.query(Artikel).filter(
                 ((Artikel.artart == 2) | (Artikel.artart == 7)) & (Artikel.artnr >= from_art) & (Artikel.artnr <= to_art) & (Artikel.departement == 0)).order_by(Artikel.artart, Artikel.artnr).all():
            ledger = Ledger()
        ...
        debitor = Debitor()
        artikel = Artikel()
        for debitor.counter, debitor.gastnr, debitor.artnr, debitor.rechnr, debitor.rgdatum, debitor.opart, debitor.betrieb_gastmem, debitor.saldo, debitor.vesrdep, debitor.zahlkonto, debitor.gastnrmember, debitor.name, debitor._recid, artikel.artnr, artikel.bezeich, artikel.artart, artikel._recid in db_session.query(Debitor.counter, Debitor.gastnr, Debitor.artnr, Debitor.rechnr, Debitor.rgdatum, Debitor.opart, Debitor.betrieb_gastmem, Debitor.saldo, Debitor.vesrdep, Debitor.zahlkonto, Debitor.gastnrmember, Debitor.name, Debitor._recid, Artikel.artnr, Artikel.bezeich, Artikel.artart, Artikel._recid).join(Artikel,(Artikel.artnr == Debitor.artnr) & (Artikel.departement == 0)).filter(
                ...
                if disptype == 0:
                    ...
                    if not cvt_flag:
                        ...
                        if age_list.fcurr.lower()  == (default_fcurr).lower() :
                            ...
                            age_list.tot_debt =  to_decimal(age_list.tot_debt) + to_decimal((debitor.saldo) / to_decimal(dollar_rate))
                    ...
                    if disptype == 0:
                        ...
                        if not cvt_flag:
                            ...
                            if age_list.fcurr.lower()  == (default_fcurr).lower() :
                                ...
                                age_list.p_bal =  to_decimal(age_list.p_bal) + to_decimal((debitor.saldo) / to_decimal(dollar_rate))
                    ...
                    if disptype == 0:
                        ...
                        if not cvt_flag:
                            ...
                            if age_list.fcurr.lower()  == (default_fcurr).lower() :
                                ...
                                age_list.debit =  to_decimal(age_list.debit) + to_decimal((debitor.saldo) / to_decimal(dollar_rate))
                        ...
                        if disptype == 0:
                            ...
                            if not cvt_flag:
                                ...
                                if age_list.fcurr.lower()  == (default_fcurr).lower() :
                                    ...
                                    age_list.credit =  to_decimal(age_list.credit) - to_decimal((debitor.saldo) / to_decimal(dollar_rate))
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            for debitor in db_session.query(Debitor).filter(
                     (Debitor.artnr == artikel.artnr) & (Debitor.rgdatum <= to_date) & (Debitor.opart == 2) & (Debitor.zahlkonto == 0) & (Debitor.artnr >= from_art) & (Debitor.artnr <= to_art)).order_by(Debitor._recid).all():
                            ...
                            if disptype == 0:
                                ...
                                if not cvt_flag:
                                    ...
                                    if age_list.fcurr.lower()  == (default_fcurr).lower() :
                                        ...
                                        age_list.p_bal =  to_decimal(age_list.p_bal) + to_decimal((debtrec.saldo) / to_decimal(dollar_rate))
                            ...
                            if disptype == 0:
                                ...
                                if not cvt_flag:
                                    ...
                                    if age_list.fcurr.lower()  == (default_fcurr).lower() :
                                        ...
                                        age_list.credit =  to_decimal(age_list.credit) - to_decimal((debtrec.saldo) / to_decimal(dollar_rate))
        ...
        for ledger in query(ledger_list, sort_by=[("artnr",False)]):
            ...
            age_list = query(age_list_list, filters=(lambda age_list: age_list.artnr == ledger.artnr), first=True)
            ...
            if mi_bill:
                ...
                for age_list in query(age_list_list, filters=(lambda age_list: age_list.artnr == ledger.artnr), sort_by=[("rgdatum",False),("gastname",False),("rechnr",False)]):
                    create_output()

            else:

                for age_list in query(age_list_list, filters=(lambda age_list: age_list.artnr == ledger.artnr), sort_by=[("gastname",False),("rechnr",False)]):
                    create_output()
            ...
            if not long_digit:
                outlist = " " + to_string(translateExtended ("T o t a l", lvcarea, "") , "x(30)") + to_string(translateExtended ("T o t a l", lvcarea, "") , "x(30)") + to_string(ledger.tot_debt, "->>,>>>,>>>,>>9.99") + to_string(ledger.p_bal, "->>,>>>,>>>,>>9.99") + to_string(ledger.debit, "->>,>>>,>>>,>>9.99") + to_string(ledger.credit, "->>,>>>,>>>,>>9.99") + to_string(ledger.debt0, "->>,>>>,>>>,>>9.99") + to_string(ledger.debt1, "->>,>>>,>>>,>>9.99") + to_string(ledger.debt2, "->>,>>>,>>>,>>9.99") + to_string(ledger.debt3, "->>,>>>,>>>,>>9.99")
            else:
                outlist = " " + to_string(translateExtended ("T o t a l", lvcarea, "") , "x(30)") + to_string(translateExtended ("T o t a l", lvcarea, "") , "x(30)") + to_string(ledger.tot_debt, "->,>>>,>>>,>>>,>>9") + to_string(ledger.p_bal, "->,>>>,>>>,>>>,>>9") + to_string(ledger.debit, "->,>>>,>>>,>>>,>>9") + to_string(ledger.credit, "->,>>>,>>>,>>>,>>9") + to_string(ledger.debt0, "->,>>>,>>>,>>>,>>9") + to_string(ledger.debt1, "->,>>>,>>>,>>>,>>9") + to_string(ledger.debt2, "->,>>>,>>>,>>>,>>9") + to_string(ledger.debt3, "->,>>>,>>>,>>>,>>9")
            fill_in_list(False, "", None)
            outlist = " " + to_string(translateExtended ("Statistic Percentage (%) :", lvcarea, "") , "x(30)") + to_string(translateExtended ("Statistic Percentage (%) :", lvcarea, "") , "x(30)") + " 100.00"
        ...
        if not long_digit:
            outlist = " " + to_string(translateExtended ("T O T A L A/R:", lvcarea, "") , "x(30)") + to_string(translateExtended ("T O T A L A/R:", lvcarea, "") , "x(30)") + to_string(t_saldo, "->>,>>>,>>>,>>9.99") + to_string(t_prev, "->>,>>>,>>>,>>9.99") + to_string(t_debit, "->>,>>>,>>>,>>9.99") + to_string(t_credit, "->>,>>>,>>>,>>9.99") + to_string(t_debt0, "->>,>>>,>>>,>>9.99") + to_string(t_debt1, "->>,>>>,>>>,>>9.99") + to_string(t_debt2, "->>,>>>,>>>,>>9.99") + to_string(t_debt3, "->>,>>>,>>>,>>9.99")
        else:
            outlist = " " + to_string(translateExtended ("T O T A L A/R:", lvcarea, "") , "x(30)") + to_string(translateExtended ("T O T A L A/R:", lvcarea, "") , "x(30)") + to_string(t_saldo, "->,>>>,>>>,>>>,>>9") + to_string(t_prev, "->,>>>,>>>,>>>,>>9") + to_string(t_debit, "->,>>>,>>>,>>>,>>9") + to_string(t_credit, "->,>>>,>>>,>>>,>>9") + to_string(t_debt0, "->,>>>,>>>,>>>,>>9") + to_string(t_debt1, "->,>>>,>>>,>>>,>>9") + to_string(t_debt2, "->,>>>,>>>,>>>,>>9") + to_string(t_debt3, "->,>>>,>>>,>>>,>>9")
        fill_in_list(False, "", None)
        outlist = " " + to_string(translateExtended ("Statistic Percentage (%) :", lvcarea, "") , "x(30)") + to_string(translateExtended ("Statistic Percentage (%) :", lvcarea, "") , "x(30)") + " 100.00"
        for i in range(1,54 + 1) :
            outlist = outlist + " "
    ...
    def age_list2():
        ...
        for debitor in db_session.query(Debitor).filter(
            ...
            if guest:
                bill_name = guest.name + ", " + guest.vorname1 + guest.anredefirma + " " + guest.anrede1
            else:
                bill_name = ""
        ...
        for ledger in query(ledger_list, sort_by=[("artnr",False)]):
            ...
            if mi_bill:
                ...
                for age_list in query(age_list_list, filters=(lambda age_list: age_list.artnr == ledger.artnr), sort_by=[("rgdatum",False),("gastname",False)]):
                    create_output()
    ...
    def get_bill_receiver_and_bill_name(p_rechnr:int):
        ...
        if p_rechnr == 0:
            age_list.gastname = guestname + ", " + guestvorname1 + guestandrefirma + " " + guestanrede1

            guest = get_cache (Guest, {"gastnr": [(eq, debitor.gastnrmember)]})

            if guest:
                age_list.billname = guestname + ", " + guestvorname1 + guestandrefirma + " " + guestanrede1
            else:
                age_list.billname = ""
        else:
            age_list.gastname = guestname + ", " + guestvorname1 + guestandrefirma + " " + guestanrede1
    ...
    for arage_list in query(arage_list_list):

        if substring(arage_list.str, 0, 7) != ("-------").lower() :
            ...
            arage_balance_list.num = arage_list.num
            arage_balance_list.cust_nm = arage_list.cust_nm
            arage_balance_list.bill_nm = arage_list.bill_nm
            arage_balance_list.p_bal = arage_list.p_bal
            arage_balance_list.debit = arage_list.debit
            arage_balance_list.credit = arage_list.credit
            arage_balance_list.end_bal = arage_list.end_bal
            arage_balance_list.creditlimit_str = arage_list.creditlimit_str

    arage_balance_list = query(arage_balance_list_list, first=True)
