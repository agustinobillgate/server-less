...
def mn_del_old_debtbl():
    ...
    def del_old_debt():
        ...
        debitor = get_cache (Debitor, {"opart": [(eq, 2)],"zahlkonto": [(gt, 0)],"rgdatum": [(lt, ci_date - anz)]})
        while None != debitor:

            debt1 = get_cache (Debitor, {"counter": [(eq, debitor.counter)],"zahlkonto": [(gt, 0)],"_recid": [(ne, debitor._recid)],"rgdatum": [(ge, ci_date - anz)]})
