...
def black_listbl():
    ...
    for c_list in query(c_list_list):
        ...
        for history in db_session.query(History).filter(
                 (History.gastnr == c_list.gastnr) & (not History.zi_wechsel)).order_by(History.abreise.desc()).all():
        ...
        for res_line in db_session.query(Res_line).filter(
                 (Res_line.gastnrmember == c_list.gastnr) & (Res_line.active_flag == 0)).order_by(Res_line.ankunft).all():
