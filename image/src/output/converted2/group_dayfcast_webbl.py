...
def group_dayfcast_webbl(input_param_list:[Input_param]):
    ...
    last_day = get_day(date_mdy(get_month(input_param.input_date) + 1, 1, get_year(input_param.input_date)) - 1)
    ...
    for tt_reslin in query(tt_reslin_list, sort_by=[("resnr",False)]):

        res_output = query(res_output_list, filters=(lambda res_output: res_output.resnr == tt_reslin.resnr), first=True)
        ...
        if not res_output:
            ...
            res_output_list.append(res_output)
    ...
    summary_status_list.clear()

    for res_output in query(res_output_list, filters=(lambda res_output: res_output.resstatus != 0), sort_by=[("resstatus",False)]):

        summary_status = query(summary_status_list, filters=(lambda summary_status: summary_status.resstatus == res_output.resstatus), first=True)

        if not summary_status:
            summary_status = Summary_status()
            summary_status_list.append(summary_status)
