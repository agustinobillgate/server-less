...
def gcf_list_2bl(case_type:int, sorttype:int, lname:string, fname:string, num1:int):
    ...
    def case7():
        ...
        for guest in db_session.query(Guest).filter(
                 ((Guest.gastnr <= num1) & (Guest.karteityp == sorttype) & (Guest.name >= trim(lname)) & ((Guest.vorname1 + Guest.anredefirma) >= ""))).order_by(Guest.name).all():
            t_guest = T_guest()
            t_guest_list.append(t_guest)
