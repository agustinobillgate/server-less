...
def ts_splitbill_btn_transfer_paytypegt1bl(rec_id_h_bill:int, bilrecid:int, curr_select:int, multi_vat:bool, balance:Decimal, pay_type:int, transdate:date, price_decimal:int, exchg_rate:Decimal, foreign_rate:bool, dept:int, change_str:string, add_zeit:int, hoga_card:string, cancel_str:string, curr_waiter:int, curr_room:string, user_init:string, cc_comment:string, guestnr:int, tischnr:int, double_currency:bool, amount_foreign:Decimal):
    ...
    def generate_output():
        ...
        return {"amount_foreign": amount_foreign, "err_flag": err_flag, "billart": billart, "qty": qty, "price": price, "amount": amount, "description": description, "bill_date": bill_date, "fl_code": fl_code, "t-h-bill-line": t_h_bill_line_list}
    ...
    def cal_vat_amount():
        ...
        nonlocal err_flag, billart, qty, price, description, bill_date, fl_code, t_h_bill_line_list, bname, payment_found, h_bill_line, kellner, h_bill, bill, h_artikel, counters, htparam, artikel, bill_line, billjournal, h_journal, queasy
        nonlocal rec_id_h_bill, bilrecid, curr_select, multi_vat, balance, pay_type, transdate, price_decimal, exchg_rate, foreign_rate, dept, change_str, add_zeit, hoga_card, cancel_str, curr_waiter, curr_room, user_init, cc_comment, guestnr, tischnr, double_currency, amount_foreign
        nonlocal kellner1
    ...
    def cal_vatamt(vatproz:Decimal):
        ...
        nonlocal err_flag, billart, price, description, bill_date, fl_code, t_h_bill_line_list, bname, payment_found, h_bill_line, kellner, h_bill, bill, h_artikel, counters, htparam, artikel, bill_line, billjournal, h_journal, queasy
        nonlocal rec_id_h_bill, bilrecid, curr_select, multi_vat, balance, pay_type, transdate, price_decimal, exchg_rate, foreign_rate, dept, change_str, add_zeit, hoga_card, cancel_str, curr_waiter, curr_room, user_init, cc_comment, guestnr, tischnr, double_currency, amount_foreign
        nonlocal kellner1
