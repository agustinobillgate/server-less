...
t_kartentext_list, T_kartentext = create_model("T_kartentext", {"kartentext":string, "curr_i":int})
t_sonstiges_list, T_sonstiges = create_model("T_sonstiges", {"sonstiges":string, "curr_i":int})

def main_fs_assign_page3bl(t_kartentext_list:[T_kartentext], t_sonstiges_list:[T_sonstiges], resnr:int, resline:int):
    ...
    for t_kartentext in query(t_kartentext_list):
        kartentext[t_kartentext.curr_i - 1] = t_kartentext.kartentext

    for t_sonstiges in query(t_sonstiges_list):
