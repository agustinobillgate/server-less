...
def rest_usrjournal_btn_go1bl(sumflag:bool, from_date:date, to_date:date, usr_init:string, curr_dept:int, price_decimal:int):
    ...
    output_list_list, Output_list = create_model("Output_list", {"bezeich":string, "str":string})
    rest_jour_list_list, Rest_jour_list = create_model("Rest_jour_list", {"datum":date, "tabelno":string, "billno":int, "artno":int, "descr":string, "qty":int, "amount":Decimal, "depart":string, "zeit":string, "id":string, "tb":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal rest_jour_list_list, monthpart, daypart, yearpart, fullyear, date_flag
    ...
    output_list_list = get_output(rest_usrjournal_btn_cldbl(sumflag, from_date, to_date, usr_init, curr_dept, price_decimal))
