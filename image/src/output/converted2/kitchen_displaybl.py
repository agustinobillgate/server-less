...
def kitchen_displaybl(casetype:int, dept_number:int, kp_number:int):
    ...
    kds_list_list, Kds_list = create_model("Kds_list", {"count_pos":int, "curr_flag":string, "qhead_recid":int, "qline_recid":int, "recid_hbline":int, "bill_no":int, "dept_no":int, "table_no":int, "user_post_id":string, "user_name":string, "artikel_no":int, "artikel_qty":int, "artikel_name":string, "sp_request":string, "post_date":date, "post_time":int, "post_timestr":string, "status_order":string, "void_menu":bool, "remain_qty":int, "dept_name":string, "system_date":date, "served_time":string})
    summary_artlist_list, Summary_artlist = create_model("Summary_artlist", {"artikel_no":int, "artikel_qty":int, "artikel_name":string, "artikel_dept":int, "subgroup_no":int, "subgroup_name":string, "void_menu":bool})
    summ_list_list, Summ_list = create_model("Summ_list", {"artikel_no":int, "artikel_qty":int, "artikel_name":string, "artikel_dept":int, "subgroup_no":int, "subgroup_name":string})
    header_list_list, Header_list = create_model("Header_list", {"rechnr":int, "tablenr":int, "zeit":int, "waiters":string, "datum":date, "rec_id":int, "status_no":int, "dept_no":int, "dept_name":string, "sysdate":date, "sysdatetime":datetime})
    order_list_list, Order_list = create_model("Order_list", {"rechnr":int, "tablenr":int, "zeit":int, "datum":date, "prod":string, "note":string, "qty":int, "flag":bool, "status_no":int, "subgrpno":int, "artno":int, "rec_id":int, "header_recid":int, "sysdate":date, "served_time":string, "dept_no":int})
    done_list_list, Done_list = create_model_like(Summary_artlist)
    summary_list_list, Summary_list = create_model_like(Summary_artlist)
    orderlist_list, Orderlist = create_model_like(Order_list)
    hdr_list_list, Hdr_list = create_model_like(Header_list)

    Bsumlist = Summary_artlist
    bsumlist_list = summary_artlist_list

    Bsummary_list = Summary_artlist
    bsummary_list_list = summary_artlist_list

    Order = Order_list
    order_list = order_list_list

    db_session = local_storage.db_session

    def generate_output():
        nonlocal header_list_list, orderlist_list, summary_list_list, subgrp, qheadrecid, tbno, billno, h_artikel, wgrpdep, hoteldpt
        ...
        nonlocal kds_list_list, summary_artlist_list, summ_list_list, header_list_list, order_list_list, done_list_list, summary_list_list, orderlist_list, hdr_list_list

        return {"header-list": header_list_list, "orderlist": orderlist_list, "summary-list": summary_list_list}


    kds_list_list, summary_artlist_list, done_list_list = get_output(kitchen_display_getdata_cld_1bl(1, kp_number))

    for bsummary_list in query(bsummary_list_list, filters=(lambda bsummary_list: bsummary_list.void_menu == False), sort_by=[("subgroup_name",False)]):

        if subgrp != bsummary_list.subgroup_no:
            subgrp = bsummary_list.subgroup_no

        summary_list = query(summary_list_list, filters=(lambda summary_list: summary_list.subgroup_no == subgrp), first=True)
        ...
        for bsumlist in query(bsumlist_list, filters=(lambda bsumlist: bsumlist.subgroup_no == subgrp and bsumlist.void_menu == False), sort_by=[("artikel_qty",True)]):

            summary_list = query(summary_list_list, filters=(lambda summary_list: summary_list.artikel_no == bsumlist.artikel_no), first=True)
    ...
    for kds_list in query(kds_list_list):
    ...
    for order_list in query(order_list_list, sort_by=[("subgrp",False),("status_no",False)]):
        ...
        orderlist = query(orderlist_list, filters=(lambda orderlist: orderlist.header_recid == order_list.header_recid and orderlist.subgrp == subgrp), first=True)

        if not orderlist:

            wgrpdep = get_cache (Wgrpdep, {"departement": [(eq, order_list.dept_no)],"zknr": [(eq, subgrp)]})
            orderlist = Orderlist()
            orderlist_list.append(orderlist)
        ...
        order_list_list.remove(order_list)

    for header_list in query(header_list_list, sort_by=[("sysdate",True),("zeit",False)]):

        orderlist = query(orderlist_list, filters=(lambda orderlist: orderlist.header_recid == header_list.rec_id and orderlist.artno != 0), first=True)
    ...
    for hdr_list in query(hdr_list_list, sort_by=[("sysdatetime",False)]):
        header_list = Header_list()
        header_list_list.append(header_list)
