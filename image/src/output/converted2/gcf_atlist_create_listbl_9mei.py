...
def gcf_atlist_create_listbl_9mei():
    ...
    s_list_list, S_list = create_model("S_list", {"ktype":int, "gastnr":int, "segm":int, "code":string, "name":string, "kname":string, "phone":string, "fax":string, "adresse":string, "city":string, "land":string, "zip":string, "sales":string, "email":string, "guest_pr_code":string, "avail_guest_pr":bool, "flag":int, "segmbez":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal s_list_list, bediener, guest, akt_kont, guestseg, segment, queasy, guest_pr, htparam
    ...
    def create_list():
        ...
        for guest in db_session.query(Guest).filter(
            ...
            for akt_kont in db_session.query(Akt_kont).filter(
                     (Akt_kont.gastnr == guest.gastnr)).order_by(Akt_kont.abteilung.desc(), Akt_kont.hauptkontakt.desc()).all():
                ...
                if substring(akt_kont.abteilung, 0, 3) != ("Acc").lower() :
                    break
