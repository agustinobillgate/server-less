...
def add_reprint_kitchprbl(pvilanguage:int, session_parameter:str, dept:int, rechnr:int, billdate:date, user_init:str):
    error_str = ""
    lvcarea:str = "add-reprint-kitchpr"
            ...
            if num_entries(printer.path, "$") > 1:
                for loopi in range(1,num_entries(printer.path, "$")  + 1) :
                    ...
                    bqsy = db_session.query(Bqsy).filter(
                             (Bqsy.key == 3) & (func.lower(Bqsy.char3).op("~")((("*" + to_string(h_bill.rechnr) + "*").lower().replace("*",".*")))) & (Bqsy.number3 == printer.nr)).first()
    ...
    if h_bill:
        ...
        queasy = db_session.query(Queasy).filter(
                 (Queasy.key == 233) & (func.lower(Queasy.char3).op("~")((("*" + to_string(h_bill.rechnr) + "*".lower().replace("*",".*")))))).first()

        if queasy:

            for buf_queasy in db_session.query(Buf_queasy).filter(
                     (Buf_queasy.key == 233) & (func.lower(Buf_queasy.char3).op("~")((("*" + to_string(h_bill.rechnr) + "*".lower().replace("*",".*")))))).order_by(Buf_queasy._recid).all():
                buf_queasy_list.remove(buf_queasy)
            pass
