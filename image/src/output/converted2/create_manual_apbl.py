...
def create_manual_apbl(language_code:int, docu_nr:string, lief_nr:int, firma:string, invoice:string, rgdatum:date, saldo:Decimal, disc:Decimal, d_amount:Decimal, netto:Decimal, ziel:int, ap_other:string, comments:string, balance:Decimal, s_list_fibukonto:string, s_list_debit:Decimal, journ_flag:bool, nr:int, user_init:string, tax_code:string, tax_amt:string, s_list_list:[S_list]):
    ...
    def generate_output():
        ...
        return {"s-list": s_list_list, "msg_str": msg_str, "fl_code": fl_code, "avail_gl": avail_gl, "err_code": err_code}
    ...
    if l_kredit:
        ...
        return generate_output()
    s_list_list, msg_str, fl_code, avail_gl = get_output(mk_ap_btn_ok_1bl(s_list_list, language_code, invoice, journ_flag, balance, avail_sbuff, docu_nr, rgdatum, lief_nr, disc, saldo, d_amount, ziel, bediener_nr, comments, netto, user_init, ap_other, user_init, firma, s_list_fibukonto, s_list_debit, tax_code, tax_amt))
