...
def inv_netconsumpbl(sorttype:int, from_date:date, to_date:date, main_grp:int):
    ...
    t_list_list, T_list = create_model("T_list", {"nr":int, "f_bezeich":string, "t_bezeich":string, "artnr":string, "bezeich":string, "qty":Decimal, "price":Decimal, "val":Decimal, "mess_unit":string, "deliv_unit":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_list_list, htparam, l_artikel, l_ophis, gl_acct, l_lager, l_op
    ...
    def create_list():
        ...
        for tbuff in query(tbuff_list, sort_by=[("f_bezeich",False)]):
            tbuff.nr = i
