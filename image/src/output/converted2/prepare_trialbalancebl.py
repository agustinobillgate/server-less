...
def prepare_trialbalancebl(lvcarea:string):
    ...
    t_gl_depart_list, T_gl_depart = create_model("T_gl_depart", {"nr":int, "bezeich":string})
    ...
    close_year = date_mdy(get_month(close_year) , get_day(close_year) , get_year(close_year))
    close_year = close_year + timedelta(days=1)
