...
def get_messages_webbl(gastnr:int, resnr:int, reslinnr:int, v_key:str, v_deactive:bool):
    v_success = False
    t_messages_list = []
    mess_list_list = []
    mess_data_list = []
    ...
    mess_data_list, Mess_data = create_model("Mess_data", {"gname":str, "arrival":date, "depart":date, "zinr":str, "pguest":bool, "nr":int, "tot":int, "username":str, "messtext":str, "currtime":str, "caller":str, "phoneno":str, "currdate":date})
    ...
    def init_var(if_flag:bool):
        ...
        elif re.match(r".*NEXT.*",v_key, re.IGNORECASE):
            num = to_int(substring(v_key, 4))
            get_messages(num + 1)
            nr = num + 1

        elif re.match(r".*PREV.*",v_key, re.IGNORECASE):
            num = to_int(substring(v_key, 4))
            get_messages(num - 1)
            nr = num - 1

        elif re.match(r".*DEACTIVE.*",v_key, re.IGNORECASE):
