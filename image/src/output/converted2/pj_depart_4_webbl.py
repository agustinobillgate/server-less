...
def pj_depart_4_webbl(pvilanguage:int, case_type:int, disptype:int, curr_date:date, froom:string, troom:string, disp_accompany:bool, ota_only:bool, setup_list_list:[Setup_list], zikat_list_list:[Zikat_list]):
    ...
    str_list_list, Str_list = create_model("Str_list", {"flag":int, "line1":string, "line2":string, "address":string})
    ...
    def generate_output():
        nonlocal tot_rm, tot_a, tot_c, tot_co, str_list_list, s_list_list, cl_list_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, lvcarea, all_remark, curr_zinr, htparam, guest, reservation, res_line, history, guestseg, segment, reslin_queasy, nation, bill, mc_guest, mc_types
        ...
        nonlocal str_list_list, s_list_list, cl_list_list
    ...
    def create_departure():
        ...
        if disptype == 1:
            ...
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                ...
                if not disp_accompany:
                    ...
                    cl_list = query(cl_list_list, filters=(lambda cl_list: cl_list.rmno == res_line.zinr and cl_list.resnr == res_line.resnr and date_mdy(cl_list.arrive) == res_line.ankunft and cl_list.zipreis == 0 and (cl_list.a + cl_list.c) < 1 and cl_list.co < 1), first=True)
        ...
        for cl_list in query(cl_list_list, filters=(lambda cl_list:(cl_list.a + cl_list.co) > 0), sort_by=[("nation",False),("bezeich",False)]):
