...
def ba_plan_create_wlist_prevbl(rml_list:[Rml], from_date:date):
    ...
    def generate_output():
        nonlocal gastname, bk_raum, guest, bk_veran, bk_reser
        nonlocal from_date
            ...
            if bk_reser.resstatus == 2:
                ...
                if bk_buff:
                    ...
                    if (i % 4 == 1) and bk_buff.von_i < 13:
                        ...
                        rml.gstatus[i - 1] = -1
            rml.room[i - 1] = substring(gastname, l - 1, 1)
    ...
    for bk_reser, bk_veran, gast in db_session.query(Bk_reser, Bk_veran, Gast).join(Bk_veran,(Bk_veran.veran_nr == Bk_reser.veran_nr)).join(Gast,(Gast.gastnr == Bk_veran.gastnr)).filter(
             ((Bk_reser.datum >= from_date) & (Bk_reser.datum <= from_date + timedelta(days=11))) & ((Bk_reser.resstatus == 8) | (Bk_reser.resstatus <= 2))).order_by(Bk_reser._recid).all():
