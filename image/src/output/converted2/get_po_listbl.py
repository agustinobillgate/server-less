...
from functions.po_list_btn_gocldbl import po_list_btn_gocldbl
from functions.po_list_btn_go2cldbl import po_list_btn_go2cldbl
from models import L_order, Gl_acct, L_artikel

def get_po_listbl(usrname:string, po_number:string, last_doc_nr:string, app_sort:string, dml_only:bool, t_liefno:int, deptnr:int, all_supp:bool, stattype:int, sorttype:int, from_date:date, to_date:date, billdate:date):
    ...
    t_list_list, T_list = create_model("T_list", {"bestelldatum":date, "bezeich":string, "firma":string, "docu_nr":string, "l_orderhdr_lieferdatum":date, "wabkurz":string, "bestellart":string, "gedruckt":date, "l_orderhdr_besteller":string, "l_order_gedruckt":date, "zeit":int, "lief_fax_2":string, "l_order_lieferdatum":date, "lief_fax_3":string, "lieferdatum_eff":date, "lief_fax_1":string, "lief_nr":int, "username":string, "del_reason":string, "tot_amount":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal p_267, first_docu_nr, curr_docu_nr, last_docu_nr, q2_list_list, param267, l_order, gl_acct, l_artikel
        nonlocal usrname, po_number, last_doc_nr, app_sort, dml_only, t_liefno, deptnr, all_supp, stattype, sorttype, from_date, to_date, billdate


        nonlocal q2_list, t_list
        nonlocal q2_list_list, t_list_list

        return {"p_267": p_267, "first_docu_nr": first_docu_nr, "curr_docu_nr": curr_docu_nr, "last_docu_nr": last_docu_nr, "q2-list": q2_list_list}
    ...
    if po_number != "":
        param267, t_list_list = get_output(po_list_btn_gocldbl(usrname, po_number, dml_only))
    else:
        first_docu_nr, curr_docu_nr, param267, last_docu_nr, t_list_list = get_output(po_list_btn_go2cldbl(t_liefno, last_doc_nr, sorttype, deptnr, all_supp, stattype, usrname, from_date, to_date, billdate, dml_only, app_sort))
    p_267 = to_int(param267)

    for t_list in query(t_list_list, sort_by=[("docu_nr",False)]):
        q2_list = Q2_list()
        q2_list_list.append(q2_list)

        buffer_copy(t_list, q2_list)

        l_order = get_cache (L_order, {"docu_nr": [(eq, t_list.docu_nr)],"loeschflag": [(eq, 0)],"pos": [(gt, 0)]})
        while None != l_order:
            q2_list = Q2_list()
            q2_list_list.append(q2_list)
            ...
            l_order = db_session.query(L_order).filter(
                     (L_order.docu_nr == t_list.docu_nr) & (L_order.loeschflag == 0) & (L_order.pos > 0) & (L_order._recid > curr_recid)).first()
