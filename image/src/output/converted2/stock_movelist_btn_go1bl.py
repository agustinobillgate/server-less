...
def stock_movelist_btn_go1bl(pvilanguage:int, s_artnr:int, show_price:bool, from_lager:int, to_lager:int):
    ...
    str_list_list, Str_list = create_model("Str_list", {"s":string, "id":string, "m_unit":string})
    str_list2_list, Str_list2 = create_model("Str_list2", {"datum":string, "lscheinnr":string, "init_qty":string, "init_val":string, "in_qty":string, "in_val":string, "out_qty":string, "out_val":string, "note":string, "id":string, "m_unit":string})
    stock_movelist_list, Stock_movelist = create_model("Stock_movelist", {"datum":date, "lscheinnr":string, "init_qty":Decimal, "init_val":Decimal, "in_qty":Decimal, "in_val":Decimal, "out_qty":Decimal, "out_val":Decimal, "note":string, "id":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal stock_movelist_list, lvcarea, tot_anz, tot_val, t_anz, t_val, long_digit, t_incoming, t_inval, t_outgoing, t_outval, htparam, l_artikel, bediener, l_bestand, l_op, l_ophdr, l_lager, l_lieferant, gl_acct, l_ophis
        nonlocal pvilanguage, s_artnr, show_price, from_lager, to_lager


        nonlocal str_list, str_list2, stock_movelist
        nonlocal str_list_list, str_list2_list, stock_movelist_list
    ...
    def create_list(pvilanguage:int, s_artnr:int, show_price:bool, from_lager:int, to_lager:int):
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            if l_bestand:
                ...
                str_list2 = Str_list2()
                str_list2_list.append(str_list2)
                ...
                if show_price:
                    ...
                    if not long_digit:
                        str_list2.datum = ""
                        str_list2.lscheinnr = ""
                        str_list2.init_qty = to_string("DEBUG")
    ...
    str_list2_list = create_list(pvilanguage, s_artnr, show_price, from_lager, to_lager)
    ...
    for str_list in query(str_list_list):
        ...
        stock_movelist.datum = date_mdy(substring(str_list.s, 0, 8))
        stock_movelist.lscheinnr = substring(str_list.s, 8, 16)
        stock_movelist.init_qty = to_decimal(substring(str_list.s, 24, 12))
        stock_movelist.init_val = to_decimal(substring(str_list.s, 36, 15))
        stock_movelist.in_qty = to_decimal(substring(str_list.s, 51, 14))
        stock_movelist.in_val = to_decimal(substring(str_list.s, 65, 14))
        stock_movelist.out_qty = to_decimal(substring(str_list.s, 79, 14))
        stock_movelist.out_val = to_decimal(substring(str_list.s, 93, 14))
        stock_movelist.note = substring(str_list.s, 117, 13)
