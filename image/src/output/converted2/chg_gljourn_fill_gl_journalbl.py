...
g_list_list, G_list = create_model("G_list", {"jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "duplicate":bool}, {"sysdate": get_current_date(), "chgdate": None, "duplicate": True})

def chg_gljourn_fill_gl_journalbl(case_type:int, jnr:int, comment:string, user_init:string, jou_recid:int, b1_recid:int, t_bezeich:string, t_refno:string, g_list_list:[G_list]):
    ...
    b1_list_list, B1_list = create_model("B1_list", {"fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "bezeich":string, "chginit":string, "chgdate":date, "sysdate":date, "zeit":int, "activeflag":int, "rec_gl_journ":int, "tax_code":string, "tax_amount":string, "tot_amt":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal debits, credits, remains, b1_list_list, fibukonto, bemerk, debit, credit, datum, gl_jouhdr, gl_journal, gl_acct, bediener, res_history, queasy
    ...
    def update_queasy_345(jnr:int, datum:date, bezeich:string):
        ...
        nonlocal debits, credits, remains, b1_list_list, fibukonto, bemerk, debit, credit, gl_jouhdr, gl_journal, gl_acct, bediener, res_history, queasy
