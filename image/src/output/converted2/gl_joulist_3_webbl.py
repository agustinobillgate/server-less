...
def gl_joulist_3_webbl(vmode:int, from_date:date, to_date:date, last_2yr:date, close_year:date, journaltype:int, excl_other:bool, other_dept:bool, summ_date:bool, from_fibu:string, to_fibu:string, sorttype:int, from_dept:int, journaltype1:int, cashflow:bool, f_note:string, from_main:int, srt_no:bool, curr_srecid:int, curr_of_coa:string, curr_date:date, curr_tdebit:Decimal, curr_tcredit:Decimal, curr_refno:string):
    ...
    def generate_output():
        nonlocal last_srecid, last_of_coa, last_date, last_tdebit, last_tcredit, last_refno, out_list_list, datum1, datum2, count_i, counter, out_flag, eor_refno, gl_jhdrhis, gl_jourhis, gl_jouhdr, gl_journal, gl_acct, gl_accthis, htparam
        ...
        return {"last_srecid": last_srecid, "last_of_coa": last_of_coa, "last_date": last_date, "last_tdebit": last_tdebit, "last_tcredit": last_tcredit, "last_refno": last_refno, "out-list": out_list_list}
    ...
    def create_hlist():
        ...
        if sorttype == 2:
            ...
            if srt_no == False:
                ...
                gl_jourhis_obj_list = {}
                for gl_jourhis, gl_jhdrhis, gl_acct in db_session.query(Gl_jourhis, Gl_jhdrhis, Gl_acct).join(Gl_jhdrhis,(Gl_jhdrhis.jnr == Gl_jourhis.jnr)).join(Gl_acct,(Gl_acct.fibukonto == Gl_jourhis.fibukonto)).filter(
                             (Gl_jourhis._recid.in_(list(set([g_list.grecid for g_list in g_list_list]))))).order_by(Gl_jourhis.fibukonto, Gl_jourhis.datum, Gl_jhdrhis.jnr).all():
                    if gl_jourhis_obj_list.get(gl_jourhis._recid):
                        continue
                    else:
                        gl_jourhis_obj_list[gl_jourhis._recid] = True
    ...
    def create_list():
        ...
        if sorttype == 2:
            ...
            if srt_no == False:
                ...
                gl_journal_obj_list = {}
                for gl_journal, gl_jouhdr, gl_acct in db_session.query(Gl_journal, Gl_jouhdr, Gl_acct).join(Gl_jouhdr,(Gl_jouhdr.jnr == Gl_journal.jnr)).join(Gl_acct,(Gl_acct.fibukonto == Gl_journal.fibukonto)).filter(
                         (Gl_journal._recid.in_(list(set([g_list.grecid for g_list in g_list_list]))))).order_by(Gl_journal.fibukonto, Gl_jouhdr.datum, Gl_jouhdr.jnr).all():
                    if gl_journal_obj_list.get(gl_journal._recid):
                        continue
                    else:
                        gl_journal_obj_list[gl_journal._recid] = True
