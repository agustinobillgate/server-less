...
def bil_selectbl(pvilanguage:int, sorttype:int, zinr:str, bil_int:int, curr_gastnr:int, ci_date:date, gastname:str, to_name:str, rechnr:int):
    ...
    lvcarea:str = "bil-select"
    res_line = guest = bill = bill_line = bediener = reservation = history = None

    b1_list = resline = rline = guestmember = mbill = bbuff = tbuff = None

    b1_list_list, B1_list = create_model("B1_list", {"zinr":str, "billnr":int, "rechnr":int, "name":str, "saldo":decimal, "resnr":int, "reslinnr":int, "datum":date, "printnr":int, "vesrcod":str, "rec_id":int, "fg_col":bool, "resname":str, "address":str, "city":str, "b_comments":str, "guest_name":str})
        ...
        if sorttype == 1:
            ...
            if bil_int == 0:
                ...
                for res_line in db_session.query(Res_line).filter(
                         (Res_line.active_flag == 1) & (func.lower(Res_line.zinr) == (zinr).lower()) & (Res_line.resstatus != 12) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
            ...
            for bill, rline in db_session.query(Bill, Rline).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                     (func.lower(Bill.zinr) >= (zinr).lower()) & (func.lower(Bill.zinr) != "") & (Bill.flag == bil_int)).order_by(Bill.zinr, Bill.billnr).all():
            ...
            if curr_gastnr == 0:
                ...
                bill_obj_list = []
                for bill, rline in db_session.query(Bill, Rline).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                         (Bill.zinr > "") & (Bill.flag == bil_int) & (func.lower(Bill.name) >= (fr_name).lower()) & (func.lower(Bill.name) <= (to_name).lower()) & (Bill.billtyp == 0)).order_by(Bill.name, Bill.zinr).all():
                    if bill._recid in bill_obj_list:
                        continue
                    else:
                        bill_obj_list.append(bill._recid)
                ...
                if not b1_list:
                    ...
                    bill_obj_list = []
                    for bill, rline in db_session.query(Bill, Rline).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                             (Bill.zinr > "") & (Bill.flag == bil_int) & (func.lower(Bill.name) >= (fr_name).lower()) & (Bill.billtyp == 0)).order_by(Bill.name, Bill.zinr).all():
                        if bill._recid in bill_obj_list:
                            continue
                        else:
                            bill_obj_list.append(bill._recid)
    ...
    def disp_bill_list1():
        ...
        if sorttype == 1:
            ...
            if zinr != "":
                ...
                bill_obj_list = []
                for bill, rline in db_session.query(Bill, Rline).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                         (func.lower(Bill.zinr) == (zinr).lower()) & (Bill.flag == bil_int) & (Bill.rechnr != 0)).order_by(Bill.rechnr.desc()).all():
                    if bill._recid in bill_obj_list:
                        continue
                    else:
                        bill_obj_list.append(bill._recid)
            ...
            if curr_gastnr == 0:
                ...
                bill_obj_list = []
                for bill, rline in db_session.query(Bill, Rline).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                         (Bill.zinr > "") & (Bill.flag == bil_int) & (func.lower(Bill.name) >= (gastname).lower()) & (func.lower(Bill.name) <= (to_name).lower()) & (Bill.billtyp == 0) & (Bill.rechnr != 0)).order_by(Bill.name, Bill.rechnr.desc()).all():
                    if bill._recid in bill_obj_list:
                        continue
                    else:
                        bill_obj_list.append(bill._recid)
                ...
                if not b1_list:
                    ...
                    bill_obj_list = []
                    for bill, rline in db_session.query(Bill, Rline).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                             (Bill.zinr > "") & (Bill.flag == bil_int) & (func.lower(Bill.name) >= (gastname).lower()) & (Bill.billtyp == 0) & (Bill.rechnr != 0)).order_by(Bill.name, Bill.rechnr.desc()).all():
                        if bill._recid in bill_obj_list:
                            continue
                        else:
                            bill_obj_list.append(bill._recid)
    ...
    def assign_it():
        ...
        while None != bill_line:
            ...
            bill_line = db_session.query(Bill_line).filter(
                     (Bill_line.rechnr == bill.rechnr)).filter(Bill_line._recid > curr_recid).first()
