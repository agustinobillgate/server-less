...
pay_list_list, Pay_list = create_model("Pay_list", {"artnr":int, "bezeich":string, "proz":Decimal, "betrag":Decimal, "f_amt":Decimal, "currency":int, "curr_str":string, "bemerk":string, "remain_amt":Decimal, "fremain_amt":Decimal, "balance":Decimal})

def settle_payment_ar_debtpay_1bl(age_list_list:[Age_list], pay_list_list:[Pay_list], pvilanguage:int, outstand1:Decimal, foutstand1:Decimal, outstand:Decimal, curr_art:int, rundung:int, foutstand:Decimal, pay_date:date, balance:Decimal, fbalance:Decimal, user_init:string):
    ...
    def settle_payment():
        ...
        if ok:
            ...
            for age_list in query(age_list_list, filters=(lambda age_list: age_list.selected  and age_list.tot_debt != 0), sort_by=[("rechnr",False)]):
                ...
                if count == 0:
                    ...
                    for debit in db_session.query(Debit).filter(
                             (Debit.opart >= 1) & (Debit.counters == count) & (Debit.rechnr == age_list.rechnr) & (Debit.artnr == curr_art) & (Debit.zahlkonto > 0)).order_by(Debit._recid).all():
                        debit.opart = 2
                ...
                if (pay_count == 0) and anzahl > 1:
                    ...
                    counters = get_cache (Counters, {"counter_no": [(eq, 31)]})
    ...
    def settle_pay1():
        ...
        for age_list in query(age_list_list, filters=(lambda age_list: age_list.selected  and age_list.tot_debt != 0), sort_by=[("tot_debt",False),("rechnr",False)]):
