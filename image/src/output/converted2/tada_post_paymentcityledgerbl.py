...
def tada_post_paymentcityledgerbl(bill_no:int, dept_no:int, pay_type:int, tischnr:int, gname:string, pax:int, payment:Decimal, resno:int, reslinno:int, curr_room:string, user_init:string, vhp_note:string):
    ...
    def generate_output():
        ...
        nonlocal hbill_list, t_kellner_list, t_h_artikel_list, t_submenu_list_list, t_h_bill_list, kellner1_list, p_list_list
    ...
    if balance != saldo:
        ...
        return generate_output()
    mealcoupon_cntrl, must_print, zero_flag, multi_cash, cancel_exist, msg_str, disc_art1, disc_art2, disc_art3, mi_ordertaker, price_decimal, curr_local, curr_foreign, double_currency, foreign_rate, exchg_rate, b_title, deptname, p_223, curr_waiter, fl_code, pos1, pos2, cashless_flag, c_param870, activate_deposit, hbill_list, t_kellner_list = get_output(tada_post_payment_preparebl(language_code, dept_no, 1, user_init, None))
    ...
    billart, qty, description, price, amount_foreign, amount, bill_date, fl_code, fl_code1, msg_str, var_testing, t_h_artikel_list = get_output(tada_post_payment_btn_transferbl(language_code, rec_id, gastnr, dept_no, payment, exchg_rate, price_decimal, balance, None, disc_art1, disc_art2, disc_art3, t_kellner.kellner_nr))
    ...
    if fl_code == 2:
        ...
        p_list_list = get_output(tada_post_payment_delqueasybl(p_list_list, t_h_bill.rec_id, False))
        vsuccess = True
