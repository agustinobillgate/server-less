...
def fo_load_macro_webbl(briefnr:int, excel_list_list:[Excel_list]):
    error_list_list = []
    error_flag = False
    n:int = 0
    l:int = 0
    ...
    mbuff:List[str] = ["jan", "feb", "mar", "apr", "mei", "jun", "jul", "aug", "sep", "oct", "nov", "des"]
    ...
    def create_colom(i:int):
        ...
        nonlocal error_list_list, error_flag, n, l, continued, c, ct, ch, curr_str, j, counter, counter_i, chcol, mbuff, parameters, briefzei, error_list
        nonlocal briefnr
        nonlocal parambuff
    ...
    if not error_flag:
        ...
        parameters = db_session.query(Parameters).filter(
                 (func.lower(Parameters.progname) == ("FO-Macro").lower()) & (Parameters.section == to_string(briefnr))).first()
        while None != parameters:

            parambuff = db_session.query(Parambuff).filter(
                         (Parambuff._recid == parameters._recid)).first()
            parambuff_list.remove(parambuff)


            curr_recid = parameters._recid
            parameters = db_session.query(Parameters).filter(
                     (func.lower(Parameters.progname) == ("FO-Macro").lower()) & (Parameters.section == to_string(briefnr))).filter(Parameters._recid > curr_recid).first()
