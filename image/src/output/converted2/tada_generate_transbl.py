...
def tada_generate_transbl(from_date:date, to_date:date):
    ...
    output_list_list, Output_list = create_model("Output_list", {"timestamp":string, "billnumber":int, "amount":Decimal, "terminalid":string, "phone":string, "programid":string, "walletid":string, "sku":string, "itemname":string, "qty":string, "price":Decimal, "price_str":string, "paymentmethod":string, "zinr":string, "resnr":int, "reslinnr":int, "gastnrmember":int, "segment":string, "sob":string, "send_date":date, "data_date":date})
    ...
    def generate_output():
        nonlocal sftp_localpath, filetimestamp, terminalid, output_list_list, deptnr, programid, walletid, tada_sku, tada_item, segment_list, prefix, phnumb, bill_line, queasy, bill, res_line, reservation, segment, sourccod, guest, zimmer
        ...
        return {"sftp_localpath": sftp_localpath, "filetimestamp": filetimestamp, "terminalid": terminalid, "output-list": output_list_list}
    ...
    for bill_line in db_session.query(Bill_line).filter(
        ...
        if res_line:
            ...
            output_list = query(output_list_list, filters=(lambda output_list: output_list.billnumber == bill.rechnr), first=True)
    ...
    for output_list in query(output_list_list, filters=(lambda output_list: output_list.amount == 0)):
    ...
    if segment_list != "":
        ...
        for output_list in query(output_list_list, filters=(lambda output_list: not matches(segment_list,r"*" + output_list.segment + r"*"))):
            output_list_list.remove(output_list)
