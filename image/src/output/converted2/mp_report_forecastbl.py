...
def mp_report_forecastbl(reporttype:int):
    ...
    t_raum_list, T_raum = create_model("T_raum", {"nr":int, "raum":string, "bezeich":string, "forecast":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_raum_list, forecast, daycount, bk_raum
        nonlocal reporttype


        nonlocal t_raum
        nonlocal t_raum_list

        return {"t-raum": t_raum_list}

    if reporttype == 3:

        for bk_raum in db_session.query(Bk_raum).order_by(Bk_raum._recid).all():
            t_raum = T_raum()
            t_raum_list.append(t_raum)

            t_raum.raum = bk_raum.raum
            t_raum.bezeich = bk_raum.bezeich

        for bk_event_detail in query(bk_event_detail_list):
            daycount = bk_event_detail.end_date - bk_event_detail.start_date + 1

            t_raum = query(t_raum_list, filters=(lambda t_raum: t_raum.bezeich == bk_event_detail.venue), first=True)
        ...
        for bk_queasy in query(bk_queasy_list, filters=(lambda bk_queasy: bk_queasy.key == 5)):
            t_raum = T_raum()
            t_raum_list.append(t_raum)
        ...
        for t_raum in query(t_raum_list):
