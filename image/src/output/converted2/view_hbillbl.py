...
def view_hbillbl(dept:int, inp_rechnr:int, datum:date):
    ...
    t_h_journal_list, T_h_journal = create_model("T_h_journal", {"artnr":int, "bezeich":string, "anzahl":int, "epreis":Decimal, "betrag":Decimal, "rechnr":int, "departement":int, "bill_datum":date, "sysdate":date, "zeit":int, "rec_id":int})
