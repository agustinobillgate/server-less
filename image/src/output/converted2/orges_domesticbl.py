...
from models import Nation, Genstat, Queasy, Reservation

def orges_domesticbl(fdate:date, to_date:date, show_ytd:bool):

    prepare_cache ([Nation, Genstat, Reservation])
    ...
    def create_list():
        ...
        out_list_list.clear()
        out_list1_list.clear()
        orges_list_list.clear()
        tot_b_individual = 0
        tot_b_grup = 0
        tot_l_individual = 0
        tot_l_grup = 0
        ...
        genstat = Genstat()
        nation = Nation()
        for genstat.res_char, genstat.datum, genstat.resnr, genstat._recid, nation.nationnr, nation.bezeich, nation._recid in db_session.query(Genstat.res_char, Genstat.datum, Genstat.resnr, Genstat._recid, Nation.nationnr, Nation.bezeich, Nation._recid).join(Nation,(Nation.nationnr == Genstat.domestic)).filter(
                 (Genstat.datum >= from_date) & (Genstat.datum <= to_date) & (Genstat.resstatus != 13) & (Genstat.segmentcode != 0) & (Genstat.domestic != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)]) & (matches(Genstat.res_char[inc_value(1)],("*SEGM_PUR*")))).order_by(Genstat._recid).all():
            if genstat_obj_list.get(genstat._recid):
                continue
            else:
                genstat_obj_list[genstat._recid] = True

            orges_list = query(orges_list_list, filters=(lambda orges_list: orges_list.nationnr == nation.nationnr), first=True)

            if not orges_list:
                orges_list = Orges_list()
                orges_list_list.append(orges_list)
        ...
        for orges_list in query(orges_list_list, sort_by=[("nationnr",False)]):
    ...
    def create_list1():
        ...
        for orges_list in query(orges_list_list, sort_by=[("nationnr",False)]):
            ...
            out_list1 = Out_list1()
            out_list1_list.append(out_list1)
