...
menu_list_list, Menu_list = create_model("Menu_list", {"request":string, "krecid":int, "posted":bool, "nr":int, "artnr":int, "bezeich":string, "anzahl":int, "price":Decimal, "betrag":Decimal, "voucher":string}, {"voucher": ""})

def ts_restinv_add_kdsbl(menu_list_list:[Menu_list], tischnr:int, curr_dept:int, curr_waiter:int, rechnr:int, user_init:string):
    ...
    for menu_list in query(menu_list_list, sort_by=[("nr",False)]):
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            queasy = get_cache (Queasy, {"key": [(eq, 255)],"char1": [(eq, "kds-line")],"number3": [(eq, to_int(h_bill_line._recid))]})
