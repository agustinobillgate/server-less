...
def release_ar_chk_btn_okbl(pvilanguage:int, artnr:int, bill_no:int, datum:date, saldo:Decimal):
    ...
    pay_list_list, Pay_list = create_model("Pay_list", {"artnr":int, "paynr":int, "datum":date, "saldo":Decimal, "bezeich":string, "s_recid":int})
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, artnr, bill_no, datum, saldo
        nonlocal debt, art1
    ...
    debitor = get_cache (Debitor, {"artnr": [(eq, artnr)],"rechnr": [(eq, bill_no)],"rgdatum": [(eq, datum)],"saldo": [(eq, saldo)],"opart": [(eq, 2)],"zahlkonto": [(eq, 0)]})
    ...
    if not debitor:
        ...
        if debt:
            ...
            debitor = get_cache (Debitor, {"artnr": [(eq, artnr)],"rechnr": [(eq, bill_no)],"rgdatum": [(eq, datum)],"saldo": [(eq, saldo)],"opart": [(eq, 0)],"zahlkonto": [(eq, 0)]})
