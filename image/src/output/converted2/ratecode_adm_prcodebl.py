...
def ratecode_adm_prcodebl(curr_select:string, prcode:string, bezeich:string, segmentcode:string, minstay:int, maxstay:int, minadvance:int, maxadvance:int, frdate:date, todate:date, user_init:string, foreign_rate:bool, local_flag:bool, drate_flag:bool, gastnr:int, local_nr:int, foreign_nr:int):
    ...
    tb1_list, Tb1 = create_model_like(Queasy, {"waehrungsnr":int, "wabkurz":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal tb1_list, ct, queasy, guest, guest_pr, bediener, res_history, waehrung
        nonlocal curr_select, prcode, bezeich, segmentcode, minstay, maxstay, minadvance, maxadvance, frdate, todate, user_init, foreign_rate, local_flag, drate_flag, gastnr, local_nr, foreign_nr
