...
def close_shiftbl(user_init:str, from_date:date, shift:int):
    ...
    for artikel in db_session.query(Artikel).filter(
        ...
        for billjournal in db_session.query(Billjournal).filter(
                 (func.lower(Billjournal.userinit) == (user_init).lower()) & (Billjournal.artnr == artikel.artnr) & (Billjournal.anzahl != 0) & (Billjournal.departement == artikel.departement) & (Billjournal.bill_datum == from_date) & (Billjournal.betriebsnr == 0)).order_by(Billjournal._recid).all():
