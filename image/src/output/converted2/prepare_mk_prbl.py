...
def prepare_mk_prbl(docu_nr:string, tp_bediener_user_group:int, tp_bediener_username:string, dml_flag:bool, dml_grp:int, dml_datum:date, cost_acct:string):
    ...
    t_l_artikel_list, T_l_artikel = create_model("T_l_artikel", {"rec_id":int, "artnr":int, "bezeich":string, "betriebsnr":int, "traubensort":string, "lief_einheit":Decimal, "jahrgang":int, "inhalt":Decimal, "masseinheit":string, "zwkum":int})
    s_list_list, S_list = create_model_like(L_order, {"s_recid":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal billdate, eng_dept, pos, dml_created, t_l_orderhdr_list, s_list_list, t_l_artikel_list, t_parameters_list, l_orderhdr, l_order, htparam, l_artikel, parameters, dml_art
        ...
        return {"docu_nr": docu_nr, "billdate": billdate, "eng_dept": eng_dept, "pos": pos, "dml_created": dml_created, "t-l-orderhdr": t_l_orderhdr_list, "s-list": s_list_list, "t-l-artikel": t_l_artikel_list, "t-parameters": t_parameters_list}
