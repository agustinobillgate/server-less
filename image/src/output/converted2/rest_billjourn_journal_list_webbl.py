...
def rest_billjourn_journal_list_webbl(from_art:int, from_dept:int, to_dept:int, from_date:date, to_date:date, price_decimal:int):
    ...
    booking_journbill_list_list, Booking_journbill_list = create_model("Booking_journbill_list", {"datum":date, "tabelno":string, "billno":int, "artno":int, "descr":string, "qty":int, "sales":Decimal, "payment":Decimal, "depart":string, "id":string, "zeit":string, "gname":string, "rmno":string, "st_optable":string, "ct_optable":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal booking_journbill_list_list, disc_art1, disc_art2, disc_art3, htparam, h_journal, h_artikel, hoteldpt, h_bill, res_line, guest
