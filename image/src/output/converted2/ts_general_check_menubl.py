...
payload_list_list, Payload_list = create_model("Payload_list", {"v_mode":int, "art_number":int, "dept_number":int, "art_quantity":int})

def ts_general_check_menubl(payload_list_list:[Payload_list]):
    ...
    response_list_list, Response_list = create_model("Response_list", {"result_msg":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal response_list_list, bill_date, max_soldout_qty, current_qty, remain_qty, posted_qty, soldout_flag, art_desc, htparam, h_artikel, queasy, h_journal
    ...
    if payload_list.v_mode == 1:
        ...
        for payload_list in query(payload_list_list):
            current_qty = 0
            posted_qty = 0
