...
s_order_list, S_order = create_model_like(Fa_order, {"nr_budget":int})

def fa_chgpo_save_detail_webbl(s_order_list:[S_order], order_nr:string, credit_term:int, curr:int, dept_nr:int, order_date:date, supplier_nr:int, expected_delivery:date, order_type:string, order_name:string, comments:string, user_init:string, billdate:date, appr_1:bool):
