...
def 3monthly_forecastbl(fmonth:string, tmonth:string):
    ...
    output_list_list, Output_list = create_model("Output_list", {"datum":date, "smonth":int, "rmsold":int, "ooo":int, "comp":int, "houseuse":int, "rmrevenue":Decimal, "percent_occ":Decimal})
    output_list1_list, Output_list1 = create_model_like(Output_list, {"counter":int, "sdate":string, "avrgrevenue":Decimal})
    ...
    def generate_output():
        nonlocal output_list1_list, bill_date, fdate, tdate, fmm, fyy, tmm, tyy, datum, datum1, datum2, datum3, curr_i, net_lodg, fnet_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, actual_tot_room, curr_month, tot_rmsold, tot_ooo, tot_comp, tot_houseuse, tot_rmrevenue, tot_avrgrevenue, tot_room, tot_zinr, htparam, genstat, segment, res_line, reservation, zinrstat, zimmer, outorder, zkstat
        nonlocal fmonth, tmonth
        nonlocal boutput


        nonlocal output_list, output_list1, active_rm_list, boutput
        nonlocal output_list_list, output_list1_list, active_rm_list_list
    ...
    if fdate < bill_date:
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            output_list = query(output_list_list, filters=(lambda output_list: output_list.datum == genstat.datum), first=True)
            ...
            for res_line in db_session.query(Res_line).filter(
                ...
                for datum3 in date_range(datum1,datum2) :
                    ...
                    if datum3 == res_line.abreise:
                        ...
                        output_list = query(output_list_list, filters=(lambda output_list: output_list.datum == datum3), first=True)
    ...
    for datum in date_range(fdate,tdate) :
        ...
        if datum < bill_date:
            ...
            if zinrstat:
                ...
                output_list = query(output_list_list, filters=(lambda output_list: output_list.datum == datum), first=True)
        ...
        boutput = query(boutput_list, filters=(lambda boutput: boutput.datum == datum), first=True)
