...
def supply_inlist_btn_go_webbl(pvilanguage:int, last_artnr:int, lieferant_recid:int, l_kredit_recid:int, ap_recid:int, long_digit:bool, show_price:bool, store:int, all_supp:bool, sorttype:int, from_grp:int, to_grp:int, from_date:date, to_date:date, taxcode_list_list:[Taxcode_list]):
    ...
    def generate_output():
        nonlocal first_artnr, curr_artnr, last_artnr1, unit_price, str_list_list, lvcarea, tot_anz, tot_amount, tot_amountexcl, tot_tax, tot_amt, i, counter, loopi, l_lieferant, l_kredit, l_artikel, l_op, l_ophdr, queasy, l_untergrup, htparam, l_ophis, l_ophhis, bediener, gl_acct
    ...
    def assign_create_list11a(t_anz:Decimal, t_amt:Decimal, t_tax:Decimal, t_inv:Decimal, lscheinnr:string, t_amountexcl:Decimal):
        ...
        str_list = query(str_list_list, filters=(lambda str_list: str_list.docu_nr == l_op.docu_nr and str_list.artnr == l_op.artnr and str_list.lager_nr == l_op.lager_nr and str_list.lscheinnr == l_op.lscheinnr and str_list.epreis == l_op.einzelpreis and str_list.billdate == l_op.datum), first=True)
    ...
    def convert_fibu(konto:string):
        ...
        nonlocal first_artnr, curr_artnr, last_artnr1, unit_price, str_list_list, lvcarea, tot_anz, tot_amount, tot_amountexcl, tot_tax, tot_amt, counter, loopi, l_lieferant, l_kredit, l_artikel, l_op, l_ophdr, queasy, l_untergrup, htparam, l_ophis, l_ophhis, bediener, gl_acct
        nonlocal pvilanguage, last_artnr, lieferant_recid, l_kredit_recid, ap_recid, long_digit, show_price, store, all_supp, sorttype, from_grp, to_grp, from_date, to_date
        nonlocal buff_l_kredit
    ...
    for str_list in query(str_list_list, filters=(lambda str_list: str_list.description.lower()  != ("T O T A L").lower()  and str_list.description.lower()  != ("GRAND TOTAL").lower())):
