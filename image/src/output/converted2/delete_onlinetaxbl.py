...
def delete_onlinetaxbl(fr_date:date, to_date:date):
    ...
    nightaudit = db_session.query(Nightaudit).filter(
             (func.lower(Nightaudit.programm) == ("nt-onlinetax.p").lower())).first()
    ...
    for curr_date in date_range(fr_date,to_date) :
        ...
        nitehist = db_session.query(Nitehist).filter(
                 (Nitehist.datum == curr_date) & (Nitehist.reihenfolge == nightaudit.reihenfolge) & (Nitehist.line == "END-OF-RECORNitehist.D")).first()
        ...
        nitehist = db_session.query(Nitehist).filter(
                 (Nitehist.datum == curr_date) & (Nitehist.reihenfolge == nightaudit.reihenfolge)).first()
        while None != nitehist:

            nbuff = db_session.query(Nbuff).filter(
                     (Nbuff._recid == nitehist._recid)).first()
            nbuff_list.remove(nbuff)
            pass

            curr_recid = nitehist._recid
            nitehist = db_session.query(Nitehist).filter(
                     (Nitehist.datum == curr_date) & (Nitehist.reihenfolge == nightaudit.reihenfolge)).filter(Nitehist._recid > curr_recid).first()
