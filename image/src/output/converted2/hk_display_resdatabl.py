...
def hk_display_resdatabl(i:int, curr_date:date, zinr:string, gstatus:int, recid1:int):
    ...
    if (i > 17) or (i < 0):
        ...
        if i == 0:
            n_edit, c_edit, fgcol_n, fgcol_c, t_res_line_list = get_output(hk_roomplan_disp_resdatabl(i, curr_date, zinr, None, None))
        else:
            n_edit, c_edit, fgcol_n, fgcol_c, t_res_line_list = get_output(hk_roomplan_disp_resdatabl(i, curr_date, zinr, gstatus, recid1))
