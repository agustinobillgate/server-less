...
def arl_list_run_mi_invoice_22bl(resnr:int, curr_resnr:int, arl_list_reslinnr:int, t_active_flag:int, printtype:int, split_bill:bool):
    ...
    s_list_list, S_list = create_model("S_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "datum":date, "qty":int, "erwachs":int, "kind1":int, "kind2":int, "zinr":string})
    t_list_list, T_list = create_model("T_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "tage":int, "date1":date, "date2":date, "qty":int, "betrag":Decimal, "erwachs":int, "kind1":int, "kind2":int, "vat":Decimal, "svc":Decimal, "zinr":string, "depo_billjour":bool, "resno_billjour":int})
    t_reservation_list, T_reservation = create_model_like(Reservation, {"bill_receiver":string})
    ...
    def generate_output():
        nonlocal err_flag, avail_master, avail_bill, reslinnr, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_list, t_res_line_list, t_guest_list, t_list_list, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, serv1, vat1, vat3, fact1, netto, bill_receiver, argt_options, t_char, t_resnr, reservation, res_line, guest, bill, htparam, master, waehrung, bill_line, artikel, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, billjournal, genstat, exrate
    ...
    def read_proforma_inv(resnr:int, rechnr:int):
        ...
        for resline in db_session.query(Resline).filter(
            ...
            for datum in date_range(resline.ankunft,co_date) :
                ...
                s_list = query(s_list_list, filters=(lambda s_list: s_list.bezeich.lower()  == (argt_rgbez_temp).lower()  and s_list.rmcat.lower()  == (zimkateg_kurzbez_tmp).lower()  and s_list.preis == rm_rate and s_list.datum == datum and s_list.ankunft == resline.ankunft and s_list.abreise == resline.abreise and s_list.erwachs == pax and s_list.kind1 == resline.kind1 and s_list.kind2 == resline.kind2), first=True)
        ...
        if rechnr > 0:
            ...
            for t_list in query(t_list_list, filters=(lambda t_list: t_list.depo_billjour)):
        ...
        if count_heritage < count_night:
            ...
            for t_list in query(t_list_list, filters=(lambda t_list: matches(t_list.bezeich,r"Heritage Fee*"))):
    ...
    def read_proforma_inv1(resnr:int, reslinnr:int, rechnr:int):
        ...
        nonlocal err_flag, avail_master, avail_bill, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_list, t_res_line_list, t_guest_list, t_list_list, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, serv1, vat1, vat3, fact1, netto, bill_receiver, argt_options, t_char, t_resnr, reservation, res_line, guest, bill, htparam, master, waehrung, bill_line, artikel, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, billjournal, genstat, exrate
        nonlocal curr_resnr, arl_list_reslinnr, t_active_flag, printtype, split_bill
        nonlocal rline, mbill, mainres, b_bill
