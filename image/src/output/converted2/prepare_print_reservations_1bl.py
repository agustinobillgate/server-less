...
def prepare_print_reservations_1bl(lresnr:int, ta_gastnr:int):
    ...
    t_list_list, T_list = create_model_like(Res_line, {"bed_setup":string, "str_zipreis":string, "mstr":string, "zimkateg_kurzbez":string, "sharer_no":int})
    ...
    def generate_output():
        nonlocal t_guest_list, t_reservation_list, t_list_list, res_line, guest, reservation, zimkateg, paramtext, waehrung, messages
        nonlocal lresnr, ta_gastnr
        nonlocal bresline


        nonlocal t_list, t_guest, t_reservation, bresline
        nonlocal t_list_list, t_guest_list, t_reservation_list

        return {"t-guest": t_guest_list, "t-reservation": t_reservation_list, "t-list": t_list_list}
