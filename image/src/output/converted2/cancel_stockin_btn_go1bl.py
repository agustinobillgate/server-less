...
def cancel_stockin_btn_go1bl(pvilanguage:int, all_supp:bool, sorttype:int, from_grp:int, store:int, from_date:date, to_date:date, show_price:bool, from_supp:string):
    ...
    cancel_stockin_list_list, Cancel_stockin_list = create_model("Cancel_stockin_list", {"datum":date, "lager":string, "lief_nr":int, "lief":string, "art":string, "bezeich":string, "unit":string, "epreis":Decimal, "in_qty":Decimal, "amount":Decimal, "docunr":string, "dlvnote":string, "note":string, "reason":string, "invnr":string})
