...
payload_list_list, Payload_list = create_model("Payload_list", {"ns_rechnr":int, "payment_type":int, "rechnr_remark":int, "input_remark":string, "rechnr_due_date":int, "due_date":date, "mode":int, "user_init":string, "bill_type":string})

def write_outstandfolio_webbl(payload_list_list:[Payload_list]):

    prepare_cache ([Queasy, Bill, Bediener, Res_history])

    guarantee_payment_list = []
    queasy = bill = bediener = res_history = None

    payload_list = guarantee_payment = buff_queasy = None

    guarantee_payment_list, Guarantee_payment = create_model("Guarantee_payment", {"payment_type":string, "payment_number":int})
    ...
    def generate_output():
        nonlocal guarantee_payment_list, queasy, bill, bediener, res_history
        nonlocal buff_queasy


        nonlocal payload_list, guarantee_payment, buff_queasy
        nonlocal guarantee_payment_list

        return {"guarantee-payment": guarantee_payment_list}
    ...
    if payload_list.mode == 1:
        ...
        for queasy in db_session.query(Queasy).filter(
            ...
            guarantee_payment_list.append(guarantee_payment)
