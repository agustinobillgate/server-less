...
def fo_journal_cldbl(from_art:int, to_art:int, from_dept:int, to_dept:int, from_date:date, to_date:date, sorttype:int, exclude_artrans:bool, long_digit:bool, foreign_flag:bool, mi_onlyjournal:bool, mi_excljournal:bool, mi_post:bool, mi_showrelease:bool):
    ...
    def generate_output():
        nonlocal gtot, output_list_list, curr_date, descr1, voucher_no, ind, gdelimiter, roomnumber, zinrdate, billnumber, curr_str, curr_resnr, lvcs, billjournal, guest, artikel, hoteldpt, bill, h_bill, bk_veran, reservation, arrangement, res_line, genstat, segment, argt_line
        ...
        nonlocal output_list_list, t_billjournal_list

        return {"gtot": gtot, "output-list": output_list_list}

    def journal_list():

        nonlocal gtot, output_list_list, descr1, voucher_no, ind, gdelimiter, roomnumber, zinrdate, billnumber, curr_str, curr_resnr, billjournal, guest, artikel, hoteldpt, bill, h_bill, bk_veran, reservation, arrangement, res_line, genstat, segment, argt_line
        nonlocal from_art, to_art, from_dept, to_dept, from_date, to_date, sorttype, exclude_artrans, long_digit, foreign_flag, mi_onlyjournal, mi_excljournal, mi_post, mi_showrelease
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            for curr_date in date_range(from_date,to_date) :
                ...
                if sorttype == 0:
                    ...
                    for t_billjournal in query(t_billjournal_list, filters=(lambda t_billjournal: t_billjournal.artnr == artikel.artnr and t_billjournal.departement == artikel.departement and bill_datum == curr_date and t_billjournal.anzahl != 0), sort_by=[("sysdate",False),("zeit",False),("zinr",False)]):
                        it_exist = True
                        do_it = True
                    ...
                    for t_billjournal in query(t_billjournal_list, filters=(lambda t_billjournal: t_billjournal.artnr == artikel.artnr and t_billjournal.departement == artikel.departement and t_billjournal.bill_datum == curr_date), sort_by=[("sysdate",False),("zeit",False),("zinr",False)]):
                        it_exist = True
                        do_it = True
                    ...
                    if mi_post :
                        ...
                        for t_billjournal in query(t_billjournal_list, filters=(lambda t_billjournal: t_billjournal.artnr == artikel.artnr and t_billjournal.departement == artikel.departement and t_billjournal.bill_datum == curr_date and t_billjournal.anzahl == 0), sort_by=[("sysdate",False),("zeit",False),("zinr",False)]):
                            it_exist = True
                            do_it = True
                        ...
                        for t_billjournal in query(t_billjournal_list, filters=(lambda t_billjournal: t_billjournal.artnr == artikel.artnr and t_billjournal.departement == artikel.departement and t_billjournal.sysdate == curr_date and t_billjournal.anzahl == 0), sort_by=[("sysdate",False),("zeit",False),("zinr",False)]):
                            it_exist = True
                            do_it = True
    ...
    for billjournal in db_session.query(Billjournal).filter(
        ...
        t_billjournal_list.append(t_billjournal)
