...
def reprint_nabl():
    ...
    b1_list_list, B1_list = create_model("B1_list", {"reihenfolge":int, "hogarest":int, "bezeichnun":string, "rec_id":int})
    ...
    for nightaudit in db_session.query(Nightaudit).filter(
             (Nightaudit.selektion)).order_by((1 - hogarest), Nightaudit.reihenfolge).all():
        b1_list = B1_list()
