...
def artikel_adminbl(pvilanguage:int, case_type:int, dept:int, artno:int):
    ...
    def check_argt():
        ...
        if argt_line:
            ...
            if arrangement:
                msg_str = msg_str + chr(2) + translateExtended ("Wrong Article type as arrangement found using this article", lvcarea, "") + " " + arrangement.arrangement
            else:
                msg_str = msg_str + chr(2) + translateExtended ("argt-line record exists for this article number!", lvcarea, "")
