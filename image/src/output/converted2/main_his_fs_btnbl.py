#using conversion tools version: 1.0.0.111

from functions.additional_functions import *
from decimal import Decimal
from models import B_storno

def main_his_fs_btnbl(resnr:int, resline:int, curr_gastnr:int):
    t_b_storno_list = []
    b_storno = None

    t_b_storno = None

    t_b_storno_list, T_b_storno = create_model_like(B_storno)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_b_storno_list, b_storno
        nonlocal resnr, resline, curr_gastnr


        nonlocal t_b_storno
        nonlocal t_b_storno_list

        return {"t-b-storno": t_b_storno_list}

    if curr_gastnr < 1:

        b_storno = get_cache (B_storno, {"bankettnr": [(eq, resnr)],"breslinnr": [(eq, resline)]})

        if b_storno:
            t_b_storno = T_b_storno()
            t_b_storno_list.append(t_b_storno)

            buffer_copy(b_storno, t_b_storno)
    else:

        b_storno = get_cache (B_storno, {"bankettnr": [(eq, resnr)],"breslinnr": [(eq, resline)],"gastnr": [(eq, curr_gastnr)]})

        if b_storno:
            t_b_storno = T_b_storno()
            t_b_storno_list.append(t_b_storno)

            buffer_copy(b_storno, t_b_storno)

    return generate_output()