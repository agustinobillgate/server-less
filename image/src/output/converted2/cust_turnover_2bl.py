...
def cust_turnover_2bl(cardtype:int, sort_type:int, curr_sort1:int, fdate:date, tdate:date, check_ftd:bool, currency:string, excl_other:bool, curr_sort2:int):
    ...
    found_it:bool = False
    exratenr:int = 0
    exrate_tmp:Decimal = to_decimal("0.0")
    curr_resnr2:int = 0
    ...
    def create_forecast():
                ...
                for datum in date_range(datum1,datum2) :
                    ...
                    if currency != " ":
                        cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal(((net_lodg) + to_decimal(tot_breakfast) +\
                                tot_lunch + to_decimal(tot_dinner) + to_decimal(tot_other)) / to_decimal(exrate_tmp) )
                        cust_list.argtumsatz =  to_decimal(cust_list.argtumsatz) + to_decimal((net_lodg) / to_decimal(exrate_tmp) )
                        cust_list.f_b_umsatz =  to_decimal(cust_list.f_b_umsatz) + to_decimal(((tot_breakfast) + to_decimal(tot_lunch) + to_decimal(tot_dinner)) / to_decimal(exrate_tmp) )
                        cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal((tot_other) / to_decimal(exrate_tmp) )
        ...
        fd_dd = get_day(fdate)
        fd_mm = get_month(fdate)
        fd_yyyy = get_year(fdate)
        td_dd = get_day(tdate)
        td_mm = get_month(tdate)
        td_yyyy = get_year(tdate)

        if fd_dd == 29 and fd_mm == 2:
            ly_fdate = date_mdy(fd_mm, 28, fd_yyyy - 1)
        else:
            ly_fdate = date_mdy(fd_mm, fd_dd, fd_yyyy - 1)

        if td_dd == 29 and td_mm == 2:
            ly_tdate = date_mdy(td_mm, 28, td_yyyy - 1)
        else:
            ly_tdate = date_mdy(td_mm, td_dd, td_yyyy - 1)
    ...
    if currency != "":
        ...
        if waehrung:
            ...
            exrate_tmp =  to_decimal(waehrung.ankauf)
