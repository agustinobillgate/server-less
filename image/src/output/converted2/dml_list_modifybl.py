...
def dml_list_modifybl(user_init:string, dml_no:string, curr_dept:int, selected_date:date):
    ...
    def generate_output():
        nonlocal c_list_list, counter, fill_dml_nr, reslin_queasy, dml_artdep, dml_art, l_artikel, l_untergrup, l_bestand, queasy, l_lieferant
    ...
    def modify_dml():
        ...
        if curr_dept == 0:
            ...
            for l_artikel.artnr, l_artikel.bezeich, l_artikel.ek_letzter, l_artikel.traubensorte, l_artikel.inhalt, l_artikel.ek_aktuell, l_artikel._recid, l_untergrup.bezeich, l_untergrup.zwkum, l_untergrup._recid in db_session.query(L_artikel.artnr, L_artikel.bezeich, L_artikel.ek_letzter, L_artikel.traubensorte, L_artikel.inhalt, L_artikel.ek_aktuell, L_artikel._recid, L_untergrup.bezeich, L_untergrup.zwkum, L_untergrup._recid).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).filter(
                ...
                c_list = query(c_list_list, filters=(lambda c_list: c_list.artnr == l_artikel.artnr), first=True)
