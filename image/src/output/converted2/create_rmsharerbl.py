...
def create_rmsharerbl(s_list_list:[S_list], user_init:str, lname:str, fname:str, ftitle:str, mresnr:int, mreslinnr:int):
    sh_created = False
    ...
    def create_resline():
        ...
        for resline in db_session.query(Resline).filter(
                 (Resline.resnr == res_line.resnr)).order_by(Resline.reslinnr.desc()).all():
