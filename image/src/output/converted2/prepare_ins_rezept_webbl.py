...
def prepare_ins_rezept_webbl(fr_artnr:int, to_artnr:int, h_artnr:int):
    ...
    s_rezlin_list, S_rezlin = create_model("S_rezlin", {"new_created":bool, "h_recid":int, "pos":int, "artnr":int, "bezeich":string, "masseinheit":string, "inhalt":Decimal, "vk_preis":Decimal, "cost":Decimal, "menge":Decimal, "lostfact":Decimal, "recipe_flag":bool, "s_unit":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal katnr, katbezeich, h_bezeich, portion, price_type, amount, cost_percent, poten_sell_price, t_l_artikel_list, t_h_rezept_list, s_rezlin_list, curr_pos, l_artikel, h_rezept, htparam, h_rezlin, queasy
        ...
        return {"katnr": katnr, "katbezeich": katbezeich, "h_bezeich": h_bezeich, "portion": portion, "price_type": price_type, "amount": amount, "cost_percent": cost_percent, "poten_sell_price": poten_sell_price, "t-l-artikel": t_l_artikel_list, "t-h-rezept": t_h_rezept_list, "s-rezlin": s_rezlin_list}
