...
def arl_list_add_keycardbl(pvilanguage:int, recid_resline:int, keyint:int):
    ...
    t_keycard_list, T_keycard = create_model("T_keycard", {"msg_str":str, "rline_betrieb_gast":int, "rline_resstatus":int, "resline_resnr":int, "resline_reslinnr":int, "rline_recid":int})
    ...
    if res_line.resstatus == 11 or res_line.resstatus == 13:
        ...
        if resline:
            err_flag = 1
            msg_str = msg_str + chr(2) + translateExtended ("Keycard is being used for other guest:", lvcarea, "") + " " + resline.name
        ...
        if resline.betrieb_gast >= maxkey:
            err_flag = 2
            msg_str = msg_str + chr(2) + translateExtended ("Number of given KeyCard =", lvcarea, "") + " " + to_string(resline.betrieb_gast) + chr(10) + translateExtended ("No more KeyCard can be generated for room", lvcarea, "") + " " + resline.zinr
        ...
        if (resline.resstatus == 1 or resline.resstatus == 6) and replaced:
            ...
            if rline:
                msg_str = msg_str + chr(2) + "&W" + translateExtended ("Room Shrer found. Replace the KeyCard too.", lvcarea, "")
        ...
        for resline in db_session.query(Resline).filter(
                 (Resline.resnr == resno) & (Resline.active_flag == 0) & (Resline.zinr != "") & (Resline.betrieb_gast == 0)).order_by(Resline.zinr, Resline.resstatus).all():
            ...
            if not do_it:
                t_keycard.msg_str = msg_str + chr(2) +\
                        translateExtended ("RmNo", lvcarea, "") + " " + rline.zinr +\
                        ": " +\
                        translateExtended ("Keycard is being used for other guest:", lvcarea, "") +\
                        " " + rline.name
