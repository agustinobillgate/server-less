...
def fa_incomingpobl(fromdate:date, todate:date, searchby:int, devnote_no:string, po_no:string, supp_no:int):
    ...
    prepare_cache ([L_lieferant, Mathis, Bediener, Fa_order])

    fa_ordheader = l_lieferant = mathis = fa_op = bediener = fa_order = None

    op_list = fa_ordheaderlist = None

    op_list_list, Op_list = create_model("Op_list", {"lscheinnr":string, "name":string, "location":string, "einzelpreis":Decimal, "anzahl":int, "warenwert":Decimal, "firma":string, "datum":date, "docu_nr":string, "lief_nr":int, "rec_id":int})
    fa_ordheaderlist_list, Fa_ordheaderlist = create_model_like(Fa_ordheader, {"create_name":string, "modify_name":string, "total_amount1":Decimal})
    ...
    def distinct_op():
        ...
        if (devnote_no == "" and po_no == "" and supp_no == 0) or searchby == 0 or (searchby == 1 and devnote_no == "") or (searchby == 2 and po_no == "") or (searchby == 3 and supp_no == 0):
            ...
            for fa_op, l_lieferant, mathis in db_session.query(Fa_op, L_lieferant, Mathis).join(L_lieferant,(L_lieferant.lief_nr == Fa_op.lief_nr)).join(Mathis,(Mathis.nr == Fa_op.nr)).filter(
                ...
                fa_ordheaderlist = query(fa_ordheaderlist_list, (lambda fa_ordheaderlist: fa_ordheaderlist.order_nr == fa_op.docu_nr), first=True)
            ...
            for fa_op, l_lieferant, mathis in db_session.query(Fa_op, L_lieferant, Mathis).join(L_lieferant,(L_lieferant.lief_nr == Fa_op.lief_nr)).join(Mathis,(Mathis.nr == Fa_op.nr)).filter(
                ...
                fa_ordheaderlist = query(fa_ordheaderlist_list, (lambda fa_ordheaderlist: fa_ordheaderlist.order_nr.lower()  == fa_op.docu_nr and fa_ordheaderlist.order_nr.lower()  == (po_no).lower()), first=True)
                if not fa_ordheaderlist:
                    continue
            ...
            for fa_op, l_lieferant, mathis in db_session.query(Fa_op, L_lieferant, Mathis).join(L_lieferant,(L_lieferant.lief_nr == Fa_op.lief_nr)).join(Mathis,(Mathis.nr == Fa_op.nr)).filter(
                     (Fa_op.loeschflag <= 1) & (Fa_op.warenwert > 0) & (Fa_op.datum >= fromdate) & (Fa_op.datum <= todate) & (Fa_op.lscheinnr != "")).order_by(Fa_op.docu_nr, Fa_op.lscheinnr, Fa_op.zeit).all():
                fa_ordheaderlist = query(fa_ordheaderlist_list, (lambda fa_ordheaderlist: fa_ordheaderlist.order_nr == fa_op.docu_nr and fa_ordheaderlist.supplier_nr == supp_no), first=True)
                if not fa_ordheaderlist:
                    continue
