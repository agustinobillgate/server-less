...
def produce_signbl(pvilanguage:int, resno:int, reslino:int, gastno:int):
    ...
    def generate_output():
        nonlocal ct, term_condition, gdpractivated, newsactivated, gdpr_flag, euro_flag, member_flag, marketing_flag, newsletter_flag, print_rc_list_list, output_list_list, temp_zimmerwusch, zimmerwunsch, anz, wi_gastnr, ind_gastnr, cc_str, cc_nr, mm, yy, cc_valid, contcode, tmp_str, bb, image_data, i, str, strgdpr, strmark, strnews, err_flag, guest, archieve, res_line, reservation, queasy, zimkateg, htparam, briefzei, guest_pr, mc_guest
        nonlocal pvilanguage, resno, reslino, gastno
        nonlocal rsvguest


        nonlocal print_rc_list, output_list, rsvguest
        nonlocal print_rc_list_list, output_list_list

        return {"ct": ct, "term_condition": term_condition, "gdpractivated": gdpractivated, "newsactivated": newsactivated, "gdpr_flag": gdpr_flag, "euro_flag": euro_flag, "member_flag": member_flag, "marketing_flag": marketing_flag, "newsletter_flag": newsletter_flag, "print-rc-list": print_rc_list_list, "output-list": output_list_list}
    ...
    if res_line:
        ...
        print_rc_list_list.append(print_rc_list)
        ...
        if newsactivated:
            ...
            if strnews.lower()  == ("YES").lower() :
                ...
                newsletter_flag = False
        output_list_list = get_output(view_staycostbl(pvilanguage, resno, reslino, contcode))
