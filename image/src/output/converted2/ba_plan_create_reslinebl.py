...
def ba_plan_create_reslinebl(curr_resnr:int, guest_gastnr:int, bkl_ftime:int, bkl_ttime:int, bkl_raum:string, bkl_datum:date, bkl_tdatum:date, bediener_nr:int, ba_dept:int, curr_resstatus:int, user_init:string):
    ...
    t_bk_reser1_list, T_bk_reser1 = create_model("T_bk_reser1", {"veran_nr":int, "resstatus":int, "datum":date, "bis_datum":date, "raum":string, "von_zeit":string, "bis_zeit":string, "veran_resnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal guest_name, reslinnr, main_exist, t_bk_reser1_list, name_contact, telefon_contact, email_contact, ftime, ttime, v_zeit, b_zeit, week_list, ci_date, htparam, guest, counters, bk_reser, bk_raum, akt_kont, bk_func, bk_veran
        ...
        return {"curr_resnr": curr_resnr, "guest_name": guest_name, "reslinnr": reslinnr, "main_exist": main_exist, "t-bk-reser1": t_bk_reser1_list}
