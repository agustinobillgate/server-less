...
t_request_list_list, T_request_list = create_model("T_request_list", {"billart":int, "curr_dept":int, "user_init":string})

def ts_restinv_billart_webbl(t_request_list_list:[T_request_list]):
    ...
    def generate_output():
        nonlocal price, error_message, t_h_artikel_list, perm, loopn, h_artikel, bediener, wgrpdep, paramtext


        nonlocal t_request_list, t_h_artikel, tp_bediener
        nonlocal t_h_artikel_list, tp_bediener_list

        return {"price": price, "error_message": error_message, "t-h-artikel": t_h_artikel_list}
    ...
    t_request_list = query(t_request_list_list, first=True)
