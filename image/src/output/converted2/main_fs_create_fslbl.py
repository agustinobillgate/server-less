...
def main_fs_create_fslbl(b1_resnr:int, b1_resline:int, bk_veran_recid:int, rsvsort:int, curr_gastnr:int):
    ...
    bkf_list, Bkf = create_model("Bkf", {"veran_nr":int, "veran_seite":int, "zweck":[string,6], "datum":date, "uhrzeit":string, "resstatus":int, "r_resstatus":[int,8], "c_resstatus":[string,8], "raeume":[string,6], "uhrzeiten":[string,6], "rpersonen":[int,8], "tischform":[string,6], "rpreis":[Decimal,8], "dekoration":[string,6], "begin_time":string, "ending_time":string, "begin_i":int, "ending_i":int, "bezeich":string})
    bstorno_list, Bstorno = create_model_like(B_storno)
    zinr_list_list, Zinr_list = create_model("Zinr_list", {"room_cat":string, "jml_room":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal resnr, resline, sales_id, sob, segcode, rmtype, rmno, roomrate, ci_date, co_date, sum_room, sum_room_cat, venue, fsl_list, bkf_list, bstorno_list, bk_func, b_storno, bk_veran, bk_reser, bk_raum, queasy, guest, bediener, res_line, zimkateg
        nonlocal b1_resnr, b1_resline, bk_veran_recid, rsvsort, curr_gastnr


        nonlocal fsl, bkf, bstorno, zinr_list
        nonlocal fsl_list, bkf_list, bstorno_list, zinr_list_list

        return {"resnr": resnr, "resline": resline, "sales_id": sales_id, "sob": sob, "segcode": segcode, "rmtype": rmtype, "rmno": rmno, "roomrate": roomrate, "ci_date": ci_date, "co_date": co_date, "sum_room": sum_room, "sum_room_cat": sum_room_cat, "venue": venue, "fsl": fsl_list, "bkf": bkf_list, "bstorno": bstorno_list}
    ...
    for res_line in db_session.query(Res_line).filter(
        ...
        if zimkateg:
            ...
            zinr_list = query(zinr_list_list, filters=(lambda zinr_list: zinr_list.room_cat == zimkateg.kurzbez), first=True)
    ...
    for zinr_list in query(zinr_list_list):
