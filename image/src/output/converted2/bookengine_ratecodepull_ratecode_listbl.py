...
def bookengine_ratecodepull_ratecode_listbl(prcode:string):
    ...
    ratecode_detail_list_list, Ratecode_detail_list = create_model("Ratecode_detail_list", {"id":int, "marknr":int, "rcode":string, "rmtype":string, "argmt":string, "cstr":[string,2], "flag":int})
    ...
    def generate_output():
        nonlocal ratecode_detail_list_list, i, j, k, zikatnr, argtnr, found1, pr_str, curr_id, iftask, tokcounter, mestoken, mesvalue, dyna_flag, prev_prcode, has_contract_rate, cat_flag, roomtype, prtable, queasy, ratecode, prmarket, zimkateg, arrangement
        nonlocal prcode
