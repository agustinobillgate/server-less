...
from models import Dml_artdep, Reslin_queasy, Dml_art, L_artikel, L_untergrup, L_bestand, Queasy, L_lieferant

def dml_list_create_it_2bl(curr_dept:int, selected_date:date, dml_no:string):
    ...
    dml_counter:int = 0
    fill_dml_nr:string = ""
    dml_artdep = reslin_queasy = dml_art = l_artikel = l_untergrup = l_bestand = queasy = l_lieferant = None
    ...
    def generate_output():
        nonlocal supply_list_list, c_list_list, dml_counter, fill_dml_nr, dml_artdep, reslin_queasy, dml_art, l_artikel, l_untergrup, l_bestand, queasy, l_lieferant
        nonlocal curr_dept, selected_date, dml_no


        nonlocal supply_list, c_list, dml_list
        nonlocal supply_list_list, c_list_list, dml_list_list
