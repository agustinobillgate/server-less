...
def availability1_1bl(pvilanguage:int, printer_nr:int, call_from:int, txt_file:string, curr_date:date, incl_tentative:bool, zikat_list_list:[Zikat_list]):
    ...
    def generate_output():
        nonlocal msg_str, room_list_list, sum_list_list, lnldelimeter, ttl_room, occ_room, ooo_room, anz_setup, tot_room, isetup_array, csetup_array, ci_date, datum, week_list, i, j, curr_day, lvcarea, res_line, zimkateg, htparam, zimmer, genstat, zinrstat, kontline, artikel, fixleist, arrangement, reslin_queasy, paramtext, outorder, guest
    ...
    def backdate_create_browse():
        ...
        rmcat_list = query(rmcat_list_list, first=True)
                ...
                if anz_setup > 0:
                    ...
                    for genstat.zikatnr, genstat.resstatus, genstat.resnr, genstat._recid, zimkateg.zikatnr, zimkateg.kurzbez, zimkateg.bezeichnung, zimkateg._recid, zimmer.setup, zimmer.sleeping, zimmer.zikatnr, zimmer.zinr, zimmer._recid in db_session.query(Genstat.zikatnr, Genstat.resstatus, Genstat.resnr, Genstat._recid, Zimkateg.zikatnr, Zimkateg.kurzbez, Zimkateg.bezeichnung, Zimkateg._recid, Zimmer.setup, Zimmer.sleeping, Zimmer.zikatnr, Zimmer.zinr, Zimmer._recid).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).join(Zimmer,(Zimmer.zinr == Genstat.zinr)).filter(
                        ...
                        if do_it:
                            ...
                            room_list = query(room_list_list, filters=(lambda room_list: room_list.zikatnr == genstat.zikatnr and room_list.setup == zimmer.setup), first=True)
                            room_list.room[m - 1] = room_list.room[m - 1] - 1
                            occ_room[m - 1] = occ_room[m - 1] + 1

                            room_list = query(room_list_list, filters=(lambda room_list: room_list.zikatnr == genstat.zikatnr and room_list.glores), first=True)

                            if not room_list:

                                room_list = query(room_list_list, filters=(lambda room_list: room_list.zikatnr == genstat.zikatnr), first=True)
                            room_list.room[m - 1] = room_list.room[m - 1] + 1
            else:
            ...
            if datum_browse < ci_date:
                ...
                for genstat.zikatnr, genstat.resstatus, genstat.resnr, genstat._recid, zimkateg.zikatnr, zimkateg.kurzbez, zimkateg.bezeichnung, zimkateg._recid, zimmer.setup, zimmer.sleeping, zimmer.zikatnr, zimmer.zinr, zimmer._recid in db_session.query(Genstat.zikatnr, Genstat.resstatus, Genstat.resnr, Genstat._recid, Zimkateg.zikatnr, Zimkateg.kurzbez, Zimkateg.bezeichnung, Zimkateg._recid, Zimmer.setup, Zimmer.sleeping, Zimmer.zikatnr, Zimmer.zinr, Zimmer._recid).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).join(Zimmer,(Zimmer.zinr == Genstat.zinr)).filter(
                    ...
                    if genstat.resstatus != 11 and genstat.resstatus != 13:
                        ...
                        room_list = query(room_list_list, filters=(lambda room_list: room_list.zikatnr == genstat.zikatnr and room_list.setup == zimmer.setup and not room_list.glores), first=True)

                        if not room_list:

                            room_list = query(room_list_list, filters=(lambda room_list: room_list.zikatnr == genstat.zikatnr and room_list.haupt  and not room_list.glores), first=True)
    ...
    def create_rmcat_list():
        ...
        if anz_setup > 0:
            ...
            for zimkateg in db_session.query(Zimkateg).filter(
                ...
                zikat_list = query(zikat_list_list, (lambda zikat_list: zikat_list.zikatnr == zimkateg.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
