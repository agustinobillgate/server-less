...
arrangement_list_list, Arrangement_list = create_model("Arrangement_list", {"selected":bool, "argtnr":int, "argt":string, "bezeich":string})

def pj_arrive2_webbl(pvilanguage:int, from_date:date, to_date:date, ci_date:date, disptype:int, incl_tentative:bool, sorttype:int, comment_type:int, incl_accompany:bool, split_rsv_print:bool, total_flag:bool, zikat_list_list:[Zikat_list], segm1_list_list:[Segm1_list], arrangement_list_list:[Arrangement_list]):
    ...
    setup_list = cl_list = t_cl_list = s_list = t_list = zikat_list = segm1_list = arrangement_list = gmember = gbuff = None
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, from_date, to_date, ci_date, disptype, incl_tentative, sorttype, comment_type, incl_accompany, split_rsv_print, total_flag, segm1_list_list
    ...
    def create_arrival(curr_date:date):
        ...
        if sorttype == 1:
            ...
            for res_line, reservation, sourccod, guest, gmember in db_session.query(Res_line, Reservation, Sourccod, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                ...
                arrangement_list = query(arrangement_list_list, (lambda arrangement_list: arrangement_list.argt == res_line.arrangement and arrangement_list.selected), first=True)
