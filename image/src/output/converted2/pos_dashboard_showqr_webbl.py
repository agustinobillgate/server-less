...
def pos_dashboard_showqr_webbl(asroom_service:bool, dynamic_qr:bool, outlet_number:int, table_nr:int, guest_name:string, pax:int, urlws:string, licensenr:int):
    ...
    if not asroom_service:
        ...
        if table_taken:
            success_taken, pick_table_list = get_output(pos_dashboard_taken_tablebl(1, sessionid, table_nr, guest_name, pax, outlet_number, dataqr, to_datetime(get_current_date(), MTIME), dynamic_qr))
