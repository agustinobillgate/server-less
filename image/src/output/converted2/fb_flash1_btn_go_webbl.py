...
def fb_flash1_btn_go_webbl(pvilanguage:int, from_grp:int, food:int, main_storage:int, f_store:int, t_store:int, date1:date, date2:date, foreign_nr:int, exchg_rate:Decimal, double_currency:bool):
    ...
    s_list_list, S_list = create_model("S_list", {"nr":int, "reihenfolge":int, "lager_nr":int, "fibukonto":string, "bezeich":string, "flag":int, "betrag":Decimal, "t_betrag":Decimal}, {"reihenfolge": 1, "flag": 2})
    fb_flash_list, Fb_flash = create_model("Fb_flash", {"flag":int, "bezeich":string, "c_alloc":string, "t_consumed":Decimal, "mtd_consumed":Decimal})
    ...
    def create_food():
        ...
        for l_store in db_session.query(L_store).filter(
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 1 and s_list.reihenfolge == 1), sort_by=[("nr",False)]):
    ...
    def create_bev():
        ...
        for l_store in db_session.query(L_store).filter(
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 2 and s_list.reihenfolge == 1), sort_by=[("nr",False)]):
                betrag =  to_decimal(betrag) + to_decimal(s_list.betrag)
                t_betrag1 =  to_decimal(t_betrag1) + to_decimal(s_list.t_betrag)
                d_betrag =  to_decimal(d_betrag) + to_decimal(s_list.betrag)
                m_betrag =  to_decimal(m_betrag) + to_decimal(s_list.t_betrag)
