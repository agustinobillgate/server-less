...
def roomplan2_1_webbl(from_room:string, curr_date:date, ci_date:date):
    ...
    t_res_line2_list, T_res_line2 = create_model("T_res_line2", {"resnr":int, "zinr":string, "name":string, "rec_id":int, "ziwech_zeit":int, "ankunft":date, "abreise":date})
    t_res_line3_list, T_res_line3 = create_model_like(Res_line, {"rec_id":int})
    room_list_list, Room_list = create_model("Room_list", {"location":string, "zistatus":int, "ststr":string, "build":string, "build_flag":string, "etage":int, "zinr":string, "c_char":string, "i_char":string, "zikatnr":int, "rmcat":string, "bezeich":string, "connec":string, "avtoday":string, "recid1":[int,28], "gstatus":[int,28], "bcol":[int,28], "fcol":[int,28], "room":[string,28], "off_mkt":[bool,28], "resnr":[int,28]})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal do_it, to_date, t_res_line_list, t_res_line2_list, t_outorder_list, t_outorder1_list, room_list_list, t_res_line3_list, stat_list, a, item_fgcol, res_line, outorder, zimmer, zimkateg, history
        ...
        return {"do_it": do_it, "to_date": to_date, "t-res-line": t_res_line_list, "t-res-line2": t_res_line2_list, "t-outorder": t_outorder_list, "t-outorder1": t_outorder1_list, "room-list": room_list_list, "t-res-line3": t_res_line3_list}
    ...
    def assign_room_list():
        ...
        for t_res_line in query(t_res_line_list):
            ...
            t_res_line3 = query(t_res_line3_list, filters=(lambda t_res_line3: t_res_line3.resnr == t_res_line.resnr and t_res_line3.reslinnr == t_res_line.reslinnr and t_res_line3.zinr == t_res_line.zinr), first=True)

            if t_res_line3:

                room_list = query(room_list_list, filters=(lambda room_list: room_list.zinr == t_res_line3.zinr), first=True)
                ...
                if t_res_line3.resstatus == 13 and room_list.recid1[i - 1] != 0:
                    ...
                    if (t_res_line3.resstatus <= 2 or t_res_line3.resstatus == 5):
                        ...
                        if t_res_line3.ankunft == curr_date:
                            ...
                            t_outorder1 = query(t_outorder1_list, filters=(lambda t_outorder1: t_outorder1.zinr == t_res_line3.zinr and t_outorder1.betriebsnr == t_res_line3.resnr), first=True)
