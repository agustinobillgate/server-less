...
def ts_restinv_run_help_webbl(kpr_time:int, kpr_recid:int, bill_date:date, tischnr:int, curr_dept:int, amount:Decimal):
    ...
    t_h_artikel1_list, T_h_artikel1 = create_model_like(H_artikel, {"rec_id":int, "amount_taxserv":Decimal, "max_soldout_qty":int, "soldout_flag":bool, "posted_qty":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal fl_code, t_h_artikel1_list, t_h_bill_list, ct, l_deci, serv_vat, tax_vat, tax, serv, service, vat, vat2, fact_scvat, servtax_use_foart, service_foreign, serv_code, vat_code, f_disc, b_disc, o_disc, price_decimal, curr_qty_posted, h_bill, h_artikel, hoteldpt, htparam, queasy, artikel, h_journal
