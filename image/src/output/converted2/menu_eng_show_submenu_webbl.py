...
def menu_eng_show_submenu_webbl(article_no:int, dept_no:int):
    ...
    t_h_menu_list, T_h_menu = create_model_like(H_menu, {"art_desc":string})
    ...
    def generate_output():
        nonlocal t_h_menu_list, h_menu, h_artikel
        nonlocal article_no, dept_no
        nonlocal buff_hart


        nonlocal t_h_menu, buff_hart
        nonlocal t_h_menu_list

        return {"t-h-menu": t_h_menu_list}
