...
def prepare_roomplanbl(from_room:string):
    ...
    room_list_list, Room_list = create_model("Room_list", {"location":string, "zistatus":int, "ststr":string, "build":string, "build_flag":string, "etage":int, "zinr":string, "c_char":string, "i_char":string, "zikatnr":int, "rmcat":string, "connec":string, "avtoday":string, "recid1":[int,28], "gstatus":[int,28], "bcol":[int,28], "fcol":[int,28], "room":[string,28], "off_mkt":[bool,28]})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal to_date, curr_date, ci_date, do_it, t_res_line_list, t_res_line2_list, t_outorder_list, t_outorder1_list, room_list_list, stat_list, a, res_line, outorder, htparam, zimmer, zimkateg, history
        nonlocal from_room
