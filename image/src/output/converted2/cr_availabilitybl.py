...
def cr_availabilitybl(pvilanguage:int, vhp_limited:bool, op_type:int, printer_nr:int, call_from:int, adult_child_str:string, statsort:int, dispsort:int, curr_date:date, incl_tentative:bool, mi_inactive:bool, show_rate:bool, indgastnr:int, qci_zinr:string):
    ...
    room_avail_list_list, Room_avail_list = create_model("Room_avail_list", {"avail_flag":bool, "allot_flag":bool, "zikatnr":int, "i_typ":int, "sleeping":bool, "allotment":[int,30], "bezeich":string, "room":[int,30], "coom":[string,30]}, {"sleeping": True})
    ...
    rate_list_list, Rate_list = create_model("Rate_list", {"ratecode":string, "segmentcode":string, "dynaflag":bool, "expired":bool, "room_type":int, "argtno":int, "statcode":[string,30], "rmrate":[Decimal,30], "minstay":int, "maxstay":int, "minadvance":int, "maxadvance":int, "frdate":date, "todate":date, "adult":int, "child":int, "currency":int, "wabkurz":string, "occ_rooms":int, "marknr":int, "i_counter":int}, {"frdate": None, "todate": None})
    created_list_list, Created_list = create_model("Created_list", {"ratecode":string, "marknr":int, "rmcateg":int, "argtno":int, "statcode":[string,30], "rmrate":[Decimal,30]})
    ...
    def generate_output():
        ...
        nonlocal sum_list_list, room_avail_list_list, date_list_list, room_list_list, rate_list_list, created_list_list, t_kontline_list, rmcat_list_list, tmp_resline_list, tmp_extra_list, temp_art_list

        return {"indgastnr": indgastnr, "room-list": room_list_list, "sum-list": sum_list_list}
    ...
    def create_rate_list():
        ...
        if tdate != None and co_date != None:
        ...
        for room_avail_list in query(room_avail_list_list):
            ...
            indgastnr, created_list_list, rate_list_list = get_output(available_ratesbl(f_date, tdate, room_avail_list.zikatnr, curr_i, adult_child_str, indgastnr, created_list_list))

            for rate_list in query(rate_list_list, sort_by=[("i_counter",False)]):
                ...
                if not qci_zimmer or (qci_zimmer and qci_zimmer.zikatnr == room_avail_list.zikatnr):
                    ...
                    looprate = 0
                    for datum in date_range(f_date,tdate) :
                        looprate = looprate + 1
    ...
    def calc_extra(f_date:date):
        ...
        tdate = f_date + timedelta(days=num_day)
        ...
        for res_line in db_session.query(Res_line).filter(
                 (not_ (Res_line.abreise < f_date)) & (not_ (Res_line.ankunft > tdate)) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line.resnr).all():
    ...
    def create_browse():
        ...
        if to_date != None and from_date != None:
            ...
            for kontline in db_session.query(Kontline).filter(
                     (Kontline.betriebsnr == 0) & (Kontline.ankunft <= to_date) & (Kontline.abreise >= from_date) & (Kontline.kontstatus == 1)).order_by(Kontline._recid).all():
        ...
        for res_line in db_session.query(Res_line).filter(
                 (Res_line.resstatus <= 6) & (Res_line.resstatus != 4) & (Res_line.active_flag <= 1) & (Res_line.kontignr > 0) & not_ (Res_line.ankunft > tdate) & not_ (Res_line.abreise <= f_date) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
        ...
        if not incl_tentative:
            ...
            if statsort == 1:
                ...
                zimkateg_obj_list = {}
                for zimkateg in db_session.query(Zimkateg).filter(
                         ((Zimkateg.zikatnr.in_(list(set([rmcat_list.zikatnr for rmcat_list in rmcat_list_list if ~rmcat_list.sleeping])))))).order_by(Zimkateg.typ, Zimkateg.zikatnr).all():
                ...
                if to_date != None and co_date != None:
                ...
                for datum in date_range(from_date,to_date) :
                    ...
                    for t_kontline in query(t_kontline_list, filters=(lambda t_kontline: t_kontline.betriebsnr == 0 and t_kontline.ankunft <= datum and t_kontline.abreise >= datum and t_kontline.kontstat == 1)):

                        if datum >= (ci_date + timedelta(days=t_kontline.ruecktage)):
                            sum_list.summe[i - 1] = sum_list.summe[i - 1] + t_kontline.zimmeranz

                            room_avail_list = query(room_avail_list_list, filters=(lambda room_avail_list: room_avail_list.zikatnr == t_kontline.zikatnr and room_avail_list.sleeping), first=True)
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            if do_it:
                ...
                if zimmer.sleeping:
                    ...
                    if datum1 != None and co_date != None:
                    ...
                    for datum in date_range(curr_date,datum1) :
                        ...
                        tmp_list[i - 1] = tmp_list[i - 1] - 1
                        i = i + 1
                    i = (res_line.abreise - curr_date).days
                    i = i + 1
    ...
    if num_entries(adult_child_str, ",") > 2:
        ...
        tmp_date = curr_date + timedelta(days=29)

        if tmp_date > co_date:

            if get_month(co_date) == get_month(curr_date) and get_year(co_date) == get_year(curr_date):
                num_day = get_day(co_date) - get_day(curr_date)
        else:
