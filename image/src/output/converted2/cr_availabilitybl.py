...
def cr_availabilitybl(pvilanguage:int, vhp_limited:bool, op_type:int, printer_nr:int, call_from:int, adult_child_str:str, statsort:int, dispsort:int, curr_date:date, incl_tentative:bool, mi_inactive:bool, show_rate:bool, indgastnr:int, qci_zinr:str):
    ...
    logstr:str = ""
    cdstr:str = ""
    col_label:List[str] = create_empty_list(31,"")
    curr_day:int = 0
    datum:date = None
    tot_room:int = 0
    i:int = 0
    ...
    week_list:List[str] = create_empty_list(7,"")
    rpt_title:str = ""
    ...
    def create_rate_list():
        ...
        for room_avail_list in query(room_avail_list_list):
            ...
            indgastnr, created_list_list, rate_list_list = get_output(available_ratesbl(fdate, tdate, room_avail_list.zikatnr, curr_i, adult_child_str, indgastnr, created_list_list))
                    ...
                    if artikel:
                        ...
                        for tmp_resline in query(tmp_resline_list, sort_by=[("resnr",False)]):
                            ...
                            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                     (func.lower(Reslin_queasy.key) == ("fargt-line").lower()) & (Reslin_queasy.resnr == tmp_resline.resnr) & (Reslin_queasy.reslinnr == tmp_resline.reslinnr) & (Reslin_queasy.number1 == 0) & (Reslin_queasy.number3 == art_nr) & (Reslin_queasy.number2 == argtnr)).order_by(Reslin_queasy._recid).all():
    ...
    def create_browse():
        ...
        for res_line in db_session.query(Res_line).filter(
                 (Res_line.resstatus <= 6) & (Res_line.resstatus != 4) & (Res_line.active_flag <= 1) & (Res_line.kontignr > 0) & (Res_line.ankunft <= tdate) & (not Res_line.abreise <= htparam.fdate) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
                ...
                for i in range(1,30 + 1) :
                    ...
                    if datum >= res_line.ankunft and datum < res_line.abreise:
                        ...
                        kontline = db_session.query(Kontline).filter(
                                 (Kontline.kontcode == kline.kontcode) & (Kontline.betriebsnr == 0) & (Kontline.kontstatus == 1)).first()
