...
def bqt_cutoff_update_resstatusbl(r_status:int, c_status:str, o_resnr:int, o_reslinnr:int):
    t_output_list_list = []
    bk_func = bk_reser = bk_veran = None

    t_output_list = None

    t_output_list_list, T_output_list = create_model("T_output_list", {"resnr":int, "reslinnr":int, "resstatus":int, "datum":date, "crdate":date, "cutoff":date, "str":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_output_list_list, bk_func, bk_reser, bk_veran
        nonlocal r_status, c_status, o_resnr, o_reslinnr


        nonlocal t_output_list
        nonlocal t_output_list_list
        return {"t-output-list": t_output_list_list}
    ...
    def update_resstatus():
        ...
        bk_func = db_session.query(Bk_func).filter(
                     (Bk_func.veran_nr == o_resnr) & (Bk_func.veran_seite == o_reslinnr)).first()
        ...
        t_output_list_list.append(t_output_list)
        ...
        bk_reser = db_session.query(Bk_reser).filter(
                     (Bk_reser.veran_nr == bk_veran.veran_nr)).first()
