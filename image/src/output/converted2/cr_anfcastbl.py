...
def cr_anfcastbl(pvilanguage:int, vhp_limited:bool, dlist:string, op_type:int, printer_nr:int, call_from:int, txt_file:string, monthdayselect:int, roompaxselect:int, nationselect:string, all_segm:bool, all_argt:bool, all_zikat:bool, from_month:string, show_rmrev:bool, incl_tent:bool, incl_wait:bool, incl_glob:bool, segm1_list_list:[Segm1_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list]):
    ...
    room_list_list, Room_list = create_model("Room_list", {"nr":int, "tag":int, "bezeich":string, "room":[int,12], "pax":[int,12], "coom":[string,12], "rstat":int})
    sum_list_list, Sum_list = create_model("Sum_list", {"bezeich":string, "summe":[int,12]})
    segm_list_list, Segm_list = create_model("Segm_list", {"segmentcode":int, "bezeich":string, "bezeich1":string, "segm":[int,12]})
    rev_list_list, Rev_list = create_model("Rev_list", {"bezeich":string, "str1":[string,6]})
    tt_month_str_list, Tt_month_str = create_model("Tt_month_str", {"i_counter":int, "month_str":string})
    ...
    def generate_output():
        ...
        nonlocal room_list_list, sum_list_list, segm_list_list, rev_list_list, tt_month_str_list
    ...
    def create_browse11():
        ...
        r8_list = query(r8_list_list, filters=(lambda r8_list: r8_list.tag == 39), first=True)
