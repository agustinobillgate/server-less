...
def fa_print_receiving_webbl(pvilanguage:int, user_init:string, lief_nr:int, po_nr:string, store:int, to_date:date, deliv_nr:string):
    ...
    str_list_list, Str_list = create_model("Str_list", {"artnr":int, "qty":Decimal, "warenwert":Decimal, "munit":string, "bezeich":string, "nr":int, "price":Decimal, "lscheinnr":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal printer_nr, show_price, crterm, d_purchase, unit_price, firma, str_list_list, tot_anz, tot_amount, lvcarea, htparam, bediener, fa_ordheader, l_lieferant, mathis, fa_op
        ...
        return {"printer_nr": printer_nr, "show_price": show_price, "crterm": crterm, "d_purchase": d_purchase, "unit_price": unit_price, "firma": firma, "str-list": str_list_list}
