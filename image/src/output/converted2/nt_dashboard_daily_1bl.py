...
def nt_dashboard_daily_1bl(from_date:date, to_date:date, exclude_article:string):
    ...
    temp_res_list, Temp_res = create_model_like(His_res, {"create_date":date, "hotel_code":string, "adult":int, "argt":string, "guest_id":int, "gname":string, "nat_code":string, "nat_group":string, "local_region":string})
    ...
    def generate_output():
        ...
        nonlocal his_res_list, his_inv_list, temp_res_list, future_res_list, future_inv_list, non_room_list, t_list_list, daily_his_res_list, hbill_list_list

        return {"his-res": his_res_list, "his-inv": his_inv_list, "future-res": future_res_list, "future-inv": future_inv_list, "non-room": non_room_list, "daily-his-res": daily_his_res_list}
            ...
            if do_it:
                ...
                if netto != 0:
                    ...
                    if (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.zinr == "") or (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.comment == ""):
                        ...
                        daily_his_res = query(daily_his_res_list, filters=(lambda daily_his_res: daily_his_res.staydate == billjournal.bill_datum and daily_his_res.resnr == bill_resnr and daily_his_res.reslinnr == 0), first=True)

                        if not daily_his_res:
                            daily_his_res = Daily_his_res()
                            daily_his_res_list.append(daily_his_res)
                        ...
                        if bill_reslinnr != 0:
                            ...
                            daily_his_res = query(daily_his_res_list, filters=(lambda daily_his_res: daily_his_res.staydate == billjournal.bill_datum and bill_resnr == daily_his_res.resnr and bill_reslinnr == daily_his_res.reslinnr), first=True)
                            ...
                            daily_his_res = query(daily_his_res_list, filters=(lambda daily_his_res: daily_his_res.staydate == billjournal.bill_datum and bill_resnr == daily_his_res.resnr and to_int(entry(1, billjournal.comment, ";")) == daily_his_res.reslinnr), first=True)
            ...
            if zimmer and zimmer.sleeping :
                for datum2 in date_range(outorder.gespstart,outorder.gespende) :
                    ...
                    if datum2 >= ci_date and datum2 <= ci_date + timedelta(days=365):
                        ...
                        future_inv = query(future_inv_list, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.zikatnr == zimmer.zikatnr and future_inv.room_status.lower()  == ("OUTOFORDER").lower()), first=True)
    ...
    for future_inv in query(future_inv_list, filters=(lambda future_inv: future_inv.room_status.lower()  == ("AVAILABLE").lower())):
        ...
        buffuture_inv = query(buffuture_inv_list, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("OCCUPIED").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.zikatnr == future_inv.zikatnr), first=True)

        if buffuture_inv:
            future_inv.qty = future_inv.qty - buffuture_inv.qty

        buffuture_inv = query(buffuture_inv_list, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("OUTOFORDER").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.zikatnr == future_inv.zikatnr), first=True)

        if buffuture_inv:
            future_inv.qty = future_inv.qty - buffuture_inv.qty

        buffuture_inv = query(buffuture_inv_list, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("COMPLIMENT").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.zikatnr == future_inv.zikatnr), first=True)
