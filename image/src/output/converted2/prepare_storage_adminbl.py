...
def prepare_storage_adminbl():
    ...
    t_l_lager_list, T_l_lager = create_model("T_l_lager", {"lager_nr":int, "bezeich":string, "betriebsnr":int})
    t_hoteldpt_list, T_hoteldpt = create_model("T_hoteldpt", {"num":int, "depart":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_l_lager_list, t_hoteldpt_list, l_lager, hoteldpt


        nonlocal t_l_lager, t_hoteldpt
        nonlocal t_l_lager_list, t_hoteldpt_list

        return {"t-l-lager": t_l_lager_list, "t-hoteldpt": t_hoteldpt_list}
