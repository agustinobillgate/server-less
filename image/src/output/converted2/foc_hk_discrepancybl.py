...
def foc_hk_discrepancybl():
    ...
    rmplan_list, Rmplan = create_model("Rmplan", {"nr":int, "str":string})
    hkdiscrepancy_list_list, Hkdiscrepancy_list = create_model("Hkdiscrepancy_list", {"zinr":string, "features":string, "etage":int, "bezeich":string, "house_status":int, "zistatus":int, "userinit":string, "nr":int})
    hk_discrepancy_list_list, Hk_discrepancy_list = create_model("Hk_discrepancy_list", {"roomno":string, "fo_status":string, "fo_adult":int, "fo_child":int, "hk_status":string, "hk_adult":int, "hk_child":int, "explanation":string, "times":string, "id":string, "floor":int, "room_descr":string})
