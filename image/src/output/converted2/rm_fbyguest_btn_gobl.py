...
def rm_fbyguest_btn_gobl(sum_month:bool, fr_date:date, to_date:date, to_year:int, ex_tent:bool, ex_comp:bool, guest_type:string):
    ...
    output_list_list, Output_list = create_model("Output_list", {"counter":int, "guest_type":int, "guest_name":string, "guest_gastnr":int, "datum":date, "str_datum":string, "room":int, "revenue":Decimal, "avrg_rev":Decimal, "str_room":string, "str_revenue":string, "str_avrg":string, "check_flag":bool})
    output_list1_list, Output_list1 = create_model("Output_list1", {"counter":int, "guest_type":int, "guest_name":string, "guest_gastnr":int, "room":[int,12], "revenue":[Decimal,12], "avrg_rev":[Decimal,12], "check_flag":bool})
    ...
    def generate_output():
        nonlocal output_list_list, output_list1_list, black_list, counter, ci_date, curr_date, tot_rm, trm, tot_rev, trev, do_it, trev1, trm1, loopi, htparam, zimkateg, guest, genstat, reservation, res_line, arrangement, bill_line, zimmer, queasy
    ...
    def create_browse():
        ...
        for genstat, zimkateg, guest in db_session.query(Genstat, Zimkateg, Guest).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).join(Guest,(Guest.gastnr == Genstat.gastnr) & (matches(to_string(Guest.karteityp),guest_type_char))).filter(
            ...
            if genstat.resstatus != 11 and genstat.resstatus != 13:
                ...
                output_list = query(output_list_list, filters=(lambda output_list: output_list.guest_gastnr == guest.gastnr and output_list.datum == genstat.datum), first=True)
                    ...
                    for datum in date_range(datum1,datum2) :
                        ...
                        if datum == res_line.abreise:
                            ...
                            if res_line.resstatus != 11 and res_line.resstatus != 13 and not res_line.zimmerfix:
                                ...
                                output_list = query(output_list_list, filters=(lambda output_list: output_list.guest_gastnr == guest.gastnr and output_list.datum == datum), first=True)
    ...
    def create_browse2():
        ...
        for genstat, zimkateg, guest in db_session.query(Genstat, Zimkateg, Guest).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).join(Guest,(Guest.gastnr == Genstat.gastnr) & (matches(to_string(Guest.karteityp),guest_type_char))).filter(
            ...
            if genstat.resstatus != 11 and genstat.resstatus != 13:
                ...
                output_list1 = query(output_list1_list, filters=(lambda output_list1: output_list1.guest_gastnr == guest.gastnr), first=True)
    ...
    if sum_month == False:
        ...
        for output_list in query(output_list_list, filters=(lambda output_list: output_list.check_flag), sort_by=[("datum",False),("guest_type",False),("guest_name",False)]):
        ...
        for output_list1 in query(output_list1_list, filters=(lambda output_list1: output_list1.check_flag), sort_by=[("guest_type",False),("guest_name",False)]):
