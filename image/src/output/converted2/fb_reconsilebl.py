...
from models import H_artikel, Htparam, Waehrung, L_bestand, L_besthis, Gl_acct, L_lager, L_artikel, L_op, L_ophdr, Hoteldpt, H_compli, Exrate, Artikel, Gl_main, H_cost, Umsatz, H_bill_line
...
def fb_reconsilebl(pvilanguage:int, case_type:int, from_date:date, to_date:date, from_grp:int, mi_opt:bool, date1:date, date2:date):
    ...
    prepare_cache ([H_artikel, Htparam, Waehrung, L_bestand, Gl_acct, L_lager, L_artikel, L_op, Hoteldpt, H_compli, Exrate, Artikel, Gl_main, H_cost, Umsatz, H_bill_line])
    ...
    h_artikel = htparam = waehrung = l_bestand = l_besthis = gl_acct = l_lager = l_artikel = l_op = l_ophdr = hoteldpt = h_compli = exrate = artikel = gl_main = h_cost = umsatz = h_bill_line = None

    fbreconsile_list = s_list = h_art = None

    fbreconsile_list_list, Fbreconsile_list = create_model("Fbreconsile_list", {"nr":int, "code":int, "bezeich":string, "col1":string, "col2":string, "col3":string, "col4":string, "col5":string})
    ...
    def generate_output():
        nonlocal done, fbreconsile_list_list, curr_nr, curr_reihe, ldry, dstore, long_digit, foreign_nr, exchg_rate, double_currency, type_of_acct, counter, coa_format, f_date, lvcarea, h_artikel, htparam, waehrung, l_bestand, l_besthis, gl_acct, l_lager, l_artikel, l_op, l_ophdr, hoteldpt, h_compli, exrate, artikel, gl_main, h_cost, umsatz, h_bill_line
        nonlocal pvilanguage, case_type, from_date, to_date, from_grp, mi_opt, date1, date2
        nonlocal h_art


        nonlocal fbreconsile_list, s_list, h_art
    ...
    def create_list():
        ...
        l_oh = L_bestand()
        l_artikel = L_artikel()
        for l_bestand.anz_anf_best, l_bestand.anz_eingang, l_bestand.anz_ausgang, l_bestand.val_anf_best, l_bestand._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid, l_oh.anz_anf_best, l_oh.anz_eingang, l_oh.anz_ausgang, l_oh.val_anf_best, l_oh._recid, l_artikel.artnr, l_artikel.endkum, l_artikel._recid in db_session.query(L_bestand.anz_anf_best, L_bestand.anz_eingang, L_bestand.anz_ausgang, L_bestand.val_anf_best, L_bestand._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid, L_oh.anz_anf_best, L_oh.anz_eingang, L_oh.anz_ausgang, L_oh.val_anf_best, L_oh._recid, L_artikel.artnr, L_artikel.endkum, L_artikel._recid).join(L_lager,(L_lager.lager_nr == L_bestand.lager_nr)).join(L_oh,(L_oh.artnr == L_bestand.artnr) & (L_oh.lager_nr == 0)).join(L_artikel,(L_artikel.artnr == L_bestand.artnr) & ((L_artikel.endkum == fl_eknr) | (L_artikel.endkum == bl_eknr))).order_by(L_bestand._recid).all():
        ...
        l_lager = L_lager()
        l_artikel = L_artikel()
        for l_op.warenwert, l_op.stornogrund, l_op.lscheinnr, l_op.anzahl, l_op._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid, l_artikel.artnr, l_artikel.endkum, l_artikel._recid in db_session.query(L_op.warenwert, L_op.stornogrund, L_op.lscheinnr, L_op.anzahl, L_op._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid, L_artikel.artnr, L_artikel.endkum, L_artikel._recid).join(L_lager,(L_lager.lager_nr == L_op.lager_nr)).join(L_artikel,(L_artikel.artnr == L_op.artnr) & ((L_artikel.endkum == fl_eknr) | (L_artikel.endkum == bl_eknr))).filter(
                 (L_op.datum >= from_date) & (L_op.datum <= to_date) & (L_op.op_art == 1) & (L_op.loeschflag <= 1)).order_by(L_op.lscheinnr).all():
        ...
        l_op_obj_list = {}
        for l_op, l_ophdr, gl_acct, l_lager, l_artikel in db_session.query(L_op, L_ophdr, Gl_acct, L_lager, L_artikel).join(L_ophdr,(L_ophdr.lscheinnr == L_op.lscheinnr) & (L_ophdr.op_typ == ("STT").lower()) & (L_ophdr.fibukonto != "")).join(Gl_acct,(Gl_acct.fibukonto == L_ophdr.fibukonto)).join(L_lager,(L_lager.lager_nr == L_op.lager_nr)).join(L_artikel,(L_artikel.artnr == L_op.artnr) & ((L_artikel.endkum == fl_eknr) | (L_artikel.endkum == bl_eknr))).filter(
                 (L_op.datum >= from_date) & (L_op.datum <= to_date) & (L_op.loeschflag <= 1) & (L_op.op_art == 3)).order_by(L_op.lscheinnr).all():
            if l_op_obj_list.get(l_op._recid):
                continue
            else:
                l_op_obj_list[l_op._recid] = True
        ...
        hoteldpt = Hoteldpt()
        h_art = H_artikel()
        for h_compli.datum, h_compli.departement, h_compli.artnr, h_compli.anzahl, h_compli.epreis, h_compli.rechnr, h_compli._recid, hoteldpt.num, hoteldpt._recid, h_art.artnrfront, h_art.departement, h_art.prozent, h_art.epreis1, h_art.artart, h_art._recid in db_session.query(H_compli.datum, H_compli.departement, H_compli.artnr, H_compli.anzahl, H_compli.epreis, H_compli.rechnr, H_compli._recid, Hoteldpt.num, Hoteldpt._recid, H_art.artnrfront, H_art.departement, H_art.prozent, H_art.epreis1, H_art.artart, H_art._recid).join(Hoteldpt,(Hoteldpt.num == H_compli.departement)).join(H_art,(H_art.departement == H_compli.departement) & (H_art.artnr == H_compli.p_artnr) & (H_art.artart == 11)).filter(
                 (H_compli.datum >= from_date) & (H_compli.datum <= to_date) & (H_compli.betriebsnr == 0) & (H_compli.departement != ldry) & (H_compli.departement != dstore)).order_by(H_compli.departement, H_compli.rechnr).all():
        ...
        if from_grp == 0 or from_grp == 1:
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 0 and s_list.reihenfolge == 1 and s_list.lager_nr != 9999 and s_list.anf_wert != 0), sort_by=[("lager_nr",False)]):
                i = i + 1
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 11 and s_list.reihenfolge == 1), sort_by=[("lager_nr",False)]):
                i = i + 1
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 12 and s_list.reihenfolge == 1), sort_by=[("lager_nr",False)]):
                i = i + 1
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 0 and s_list.reihenfolge == 1 and s_list.lager_nr != 9999 and s_list.end_wert != 0), sort_by=[("lager_nr",False)]):
                i = i + 1
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 4 and s_list.reihenfolge == 1 and s_list.betrag != 0), sort_by=[("bezeich",False)]):
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 5 and s_list.reihenfolge == 1 and s_list.betrag != 0), sort_by=[("bezeich",False)]):
                betrag6 =  to_decimal(betrag6) + to_decimal(s_list.betrag)
                counter = counter + 1
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 0 and s_list.reihenfolge == 2 and s_list.lager_nr != 9999 and s_list.anf_wert != 0), sort_by=[("lager_nr",False)]):
            i = i + 1
            betrag1 =  to_decimal(betrag1) + to_decimal(s_list.anf_wert)
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 11 and s_list.reihenfolge == 2), sort_by=[("lager_nr",False)]):
            i = i + 1
            betrag2 =  to_decimal(betrag2) + to_decimal(s_list.betrag)
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 12 and s_list.reihenfolge == 2), sort_by=[("lager_nr",False)]):
            i = i + 1
            betrag3 =  to_decimal(betrag3) + to_decimal(s_list.betrag)
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 0 and s_list.reihenfolge == 2 and s_list.lager_nr != 9999 and s_list.end_wert != 0), sort_by=[("lager_nr",False)]):
            i = i + 1
            betrag5 =  to_decimal(betrag5) + to_decimal(s_list.end_wert)
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 4 and s_list.reihenfolge == 2 and s_list.betrag != 0), sort_by=[("bezeich",False)]):
            betrag6 =  to_decimal(betrag6) + to_decimal(s_list.betrag)
            counter = counter + 1
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 5 and s_list.reihenfolge == 2 and s_list.betrag != 0), sort_by=[("lager_nr",False)]):
            counter = counter + 1
            betrag6 =  to_decimal(betrag6) + to_decimal(s_list.betrag)
