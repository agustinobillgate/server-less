...
def masterplan_viewbl(mode:string, curr_date:date):
    ...
    t_mplan_view_list, T_mplan_view = create_model("T_mplan_view", {"block_id":string, "event_name":string, "resno":int, "gname":string, "guest_no":int, "room":string, "room_desc":string, "stat_no":int, "stat_code":string, "invno":int, "fr_date":date, "to_date":date, "fr_time":string, "to_time":string, "amount":Decimal, "setup":string, "rsize":int, "person":int, "preparation":int, "ext":string, "flag":string})
    t_mplan_list_list, T_mplan_list = create_model("T_mplan_list", {"block_id":string, "event_name":string, "resno":int, "gname":string, "guest_no":int, "room":string, "room_desc":string, "stat_no":int, "stat_code":string, "invno":int, "fr_date":date, "to_date":date, "fr_time":string, "to_time":string, "amount":Decimal, "setup":string, "rsize":int, "person":int, "preparation":int, "ext":string, "flag":string})
    buff_mplan_list_list, Buff_mplan_list = create_model("Buff_mplan_list", {"block_id":string, "event_name":string, "resno":int, "gname":string, "guest_no":int, "room":string, "room_desc":string, "stat_no":int, "stat_code":string, "invno":int, "fr_date":date, "to_date":date, "fr_time":string, "to_time":string, "amount":Decimal, "setup":string, "rsize":int, "person":int, "preparation":int, "ext":string, "flag":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_mplan_view_list, curr_week, ftdate, logi1, bk_raum, bk_rset
        nonlocal mode, curr_date


        nonlocal t_mplan_view, t_mplan_list, buff_mplan_list
        nonlocal t_mplan_view_list, t_mplan_list_list, buff_mplan_list_list

        return {"t-mplan-view": t_mplan_view_list}
    ...
    def daily_view():
        ...
        if logi1 == False:
            ...
            for bk_raum in db_session.query(Bk_raum).order_by(Bk_raum.bezeich).all():
                ...
                t_mplan_list_list.append(t_mplan_list)
            ...
            for bk_event_detail in query(bk_event_detail_list, filters=(lambda bk_event_detail: bk_event_detail.start_date == curr_date and bk_event_detail.end_date == curr_date), sort_by=[("venue",False)]):
                t_mplan_list = T_mplan_list()
                t_mplan_list_list.append(t_mplan_list)
            ...
            for t_mplan_list in query(t_mplan_list_list):
                buff_mplan_list = Buff_mplan_list()
                buff_mplan_list_list.append(buff_mplan_list)

                buffer_copy(t_mplan_list, buff_mplan_list)

            for bk_raum in db_session.query(Bk_raum).order_by(Bk_raum.raum).all():

                buff_mplan_list = query(buff_mplan_list_list, filters=(lambda buff_mplan_list: buff_mplan_list.room == bk_raum.raum), first=True)
        ...
        for t_mplan_list in query(t_mplan_list_list, sort_by=[("room",False)]):
            t_mplan_view = T_mplan_view()
            t_mplan_view_list.append(t_mplan_view)
    ...
    def weekly_view():
        ...
        for bk_event_detail in query(bk_event_detail_list, filters=(lambda bk_event_detail: bk_event_detail.start_date >= curr_date and bk_event_detail.start_date <= curr_week)):
        ...
        if logi1 == False:
            ...
            for bk_event_detail in query(bk_event_detail_list, filters=(lambda bk_event_detail: bk_event_detail.start_date >= curr_date and bk_event_detail.start_date <= curr_week), sort_by=[("venue",False)]):
                t_mplan_list = T_mplan_list()
                t_mplan_list_list.append(t_mplan_list)
