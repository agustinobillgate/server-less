        ...
        if sorttype == 1:
            ...
            if input_resnr == 0:
                ...
                for res_line, zimkateg, guest, reservation in db_session.query(Res_line, Zimkateg, Guest, Reservation).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Guest,(Guest.gastnr == Res_line.gastnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
                         ((Res_line.resstatus == 6) | (Res_line.resstatus == 13)) & (func.lower(Res_line.zinr) >= (lzinr).lower()) & (Res_line.abreise == ci_date)).order_by((Res_line.zinr + Res_line.name)).all():
                ...
                res_line_obj_list = []
                for res_line, zimkateg, guest, reservation in db_session.query(Res_line, Zimkateg, Guest, Reservation).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Guest,(Guest.gastnr == Res_line.gastnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
                         ((Res_line.resstatus == 6) | (Res_line.resstatus == 13)) & (func.lower(Res_line.zinr) >= (lzinr).lower()) & (Res_line.resnr == input_resnr)).order_by((Res_line.zinr + Res_line.name)).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
            ...
            if input_resnr == 0:
                ...
                for res_line, zimkateg, guest, reservation in db_session.query(Res_line, Zimkateg, Guest, Reservation).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Guest,(Guest.gastnr == Res_line.gastnrmember) & (func.lower(Guest.name) >= (lname).lower())).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
                         ((Res_line.resstatus == 6) | (Res_line.resstatus == 13)) & (func.lower(Res_line.zinr) >= (lzinr).lower()) & (Res_line.abreise == ci_date)).order_by(Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
                ...
                res_line_obj_list = []
                for res_line, zimkateg, guest, reservation in db_session.query(Res_line, Zimkateg, Guest, Reservation).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Guest,(Guest.gastnr == Res_line.gastnrmember) & (func.lower(Guest.name) >= (lname).lower())).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter(
                         ((Res_line.resstatus == 6) | (Res_line.resstatus == 13)) & (func.lower(Res_line.zinr) >= (lzinr).lower()) & (Res_line.resnr == input_resnr)).order_by(Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
