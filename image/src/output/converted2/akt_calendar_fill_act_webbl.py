...
def akt_calendar_fill_act_webbl(user_init:string, all_flag:bool, curr_month:int, curr_year:int):
    ...
    act_list, Act = create_model("Act", {"linenr":int, "datum":date, "ftime":int, "ttime":int, "aktion":string, "lname":string, "kontakt":string, "regard":string, "sales":string, "priority":int, "flag":int})
