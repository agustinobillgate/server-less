...
def res_memozinrbl(memo_zinr:string):
    ...
    t_memozinr_list, T_memozinr = create_model("T_memozinr", {"gastnr":int, "memozinr":string, "memodatum":date, "name":string, "ankunft":date, "abreise":date, "anztage":int, "arrangement":string, "zipreis":Decimal, "erwachs":int, "kind1":int, "resstatus":int, "resnr":int, "gastnrmember":int, "zinr":string, "active_flag":int, "reslinnr":int})
