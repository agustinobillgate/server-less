...
def telop_webbl(sorttype:int, room:str, fdate1:date, fdate2:date, ci_date:date, lname:str, last_sort:int, lnat:str, lresnr:int):
    ...
    def disp_arl1():
        ...
        if lname != "" and substring(lname, 0, 1) != ("*").lower() :
            to_name = chr (asc(substring(lname, 0, 1)) + 1)
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                         (Res_line.active_flag == 0) & (Res_line.ankunft >= fdate1) & (Res_line.ankunft <= fdate2) & (func.lower(Res_line.name) >= (lname).lower()) & (func.lower(Res_line.name) <= (to_name).lower())).order_by(Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
        ...
        if last_sort == 1:
            ...
            if room != "":
                ...
                if lname == "":
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) & (func.lower(Res_line.name).op("~")(((lname).lower().replace("*",".*")))) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date)).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
    ...
    def disp_arl5():
        ...
        if last_sort == 1 and room != "":
            ...
            if lresnr == 0:
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                         (Res_line.active_flag == 2) & (Res_line.resstatus == 8) & (Res_line.abreise == fdate2)).order_by(Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
        ...
        if last_sort == 1:
            ...
            if room != "":
                ...
                if lname == "":
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                             ((Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (func.lower(Res_line.name).op("~")(((lname).lower().replace("*",".*")))) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date)) | ((Res_line.active_flag == 0) & (func.lower(Res_line.name).op("~")(((lname).lower().replace("*",".*")))) & (Res_line.ankunft == ci_date)) | ((Res_line.active_flag == 2) & (Res_line.resstatus == 8) & (func.lower(Res_line.name).op("~")(((lname).lower().replace("*",".*")))) & (Res_line.abreise == ci_date))).order_by(Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
    ...
    def count_al1():
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 0) & (Rline.ankunft >= fdate1) & (Rline.ankunft <= fdate2) & (func.lower(Rline.name) >= (lname).lower()) & (func.lower(Rline.name) <= (to_name).lower())).order_by(Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 0) & (Rline.ankunft >= fdate1) & (Rline.ankunft <= fdate2) & (func.lower(Rline.name).op("~")(((lname.lower().replace("*",".*")))))).order_by(Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 0) & (Rline.ankunft == fdate2) & (func.lower(Rline.name) >= (lname).lower()) & (func.lower(Rline.name) <= (to_name).lower())).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 0) & (Rline.ankunft == fdate2) & (func.lower(Rline.name).op("~")(((lname.lower().replace("*",".*")))))).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember) & (func.lower(Gme.nation1) == (lnat).lower())).filter(
    ...
    def count_all2():
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 1) & (Rline.resstatus != 12) & (func.lower(Rline.resname).op("~")(((lname).lower().replace("*",".*")))) & (Rline.ankunft <= ci_date) & (Rline.abreise >= ci_date)).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 1) & (Rline.resstatus != 12) & (func.lower(Rline.resname) >= (lname).lower()) & (func.lower(Rline.resname) <= (to_name).lower()) & (Rline.ankunft <= ci_date) & (Rline.abreise >= ci_date)).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
    ...
    def count_all4():
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember) & (func.lower(Gme.nation1) == (lnat).lower())).filter(
                         (Rline.active_flag == 1) & (Rline.resstatus != 12) & (func.lower(Rline.name).op("~")(((lname).lower().replace("*",".*")))) & (Rline.abreise == ci_date)).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.active_flag == 1) & (Rline.resstatus != 12) & (func.lower(Rline.name) >= (lname).lower()) & (func.lower(Rline.name) <= (to_name).lower()) & (Rline.abreise == ci_date)).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
    ...
    def count_all5():
        ...
        if last_sort == 1 and room != "":
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.resstatus == 8) & (Rline.abreise == fdate2) & (func.lower(Rline.zinr) >= (room).lower())).order_by(Rline.zinr, Reserv.name, Rline.resnr, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr) & (func.lower(Reserv.name) >= (lname).lower()) & (func.lower(Reserv.name) <= (to_name).lower())).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.resstatus == 8) & (func.lower(Rline.resname).op("~")(((lname).lower().replace("*",".*")))) & (Rline.abreise == fdate2)).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gme in db_session.query(Rline, Whrg, Reserv, Zk, Gme).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reserv,(Reserv.resnr == Rline.resnr)).join(Zk,(Zk.zikatnr == Rline.zikatnr)).join(Gme,(Gme.gastnr == Rline.gastnrmember)).filter(
                         (Rline.resstatus == 8) & (func.lower(Rline.resname) >= (lname).lower()) & (func.lower(Rline.resname) <= (to_name).lower()) & (Rline.abreise == fdate2)).order_by(Gme.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
