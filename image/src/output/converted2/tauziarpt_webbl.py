...
def tauziarpt_webbl(curr_date:date, occrpt:bool, occ_month:int, occ_year:int, linkgsheet:string, filenm:string, guestrpt:bool, fdate:date, tdate:date, revrpt:bool, fdate1:date, tdate1:date, otbmseg:bool, otbmonth:int, otbyear:int):
    ...
    s_list_list, S_list = create_model("S_list", {"gastnr":int, "datum":string, "name":string, "address":string, "city":string, "zimmeranz":int, "lodging":Decimal})
    t_list_list, T_list = create_model_like(Guest, {"cp1":string, "cp2":string, "cp3":string, "guest_name":string, "guest_adr":string})
    u_list_list, U_list = create_model("U_list", {"rmsegmt":int, "rmrev":Decimal})
    occupancy_forecast_list, Occupancy_forecast = create_model("Occupancy_forecast", {"datum":string, "days":string, "group_onhand_definite":int, "group_onhand_tentative":int, "booking_onhand_fit":int, "total_otb_group_fit":int, "total_room_avail":int, "arr_otb":int, "total_otb_percent":string, "leadtime_booking":int, "samedate_lastyear":int, "samedate_lastyear_percent":string, "arr_last_year":int})
    ...
    def generate_output():
        nonlocal occupancy_forecast_list, t_list_list, s_list_list, u_list_list, week_list, month_list, guest, segment
        ...
        nonlocal s_list_list, t_list_list, u_list_list, occupancy_forecast_list

        return {"occupancy-forecast": occupancy_forecast_list, "t-list": t_list_list, "s-list": s_list_list, "u-list": u_list_list}
        ...
        if linkgsheet != "":
            ...
            for i in range(0,2  + 1) :
                ...
                for j in range(1,lastday  + 1) :
                    ...
                    occupancy_forecast_list.append(occupancy_forecast)
    ...
    def generate_clientdata():
        ...
        t_list_list = get_output(nt_tauziarpt_gen_clientdatabl(2, curr_date, fdate, tdate))
    ...
    def generate_revenuedata():
        ...
        s_list_list = get_output(nt_tauziarpt_gen_revdatabl(fdate1, tdate1))
    ...
    def generate_otbmseg():
        ...
        nonlocal occupancy_forecast_list, t_list_list, s_list_list, u_list_list, month_list, guest, segment
        nonlocal curr_date, occrpt, occ_month, occ_year, linkgsheet, filenm, guestrpt, fdate, tdate, revrpt, fdate1, tdate1, otbmseg, otbmonth, otbyear
        nonlocal segtmp
        ...
        tmpseg:string = ""
        week_list:List[string] = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
        totroom, u_list_list = get_output(nt_tauziarpt_gen_otbmseg_gsheetbl(None))
