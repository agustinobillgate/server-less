...
from models import H_bill_line, H_artikel, Htparam, Hoteldpt, H_journal, Wgrpgen, Artikel

def ts_restinv_disp_bill_line_web_1bl(double_currency:bool, rechnr:int, curr_dept:int, avail_t_h_bill:bool, hbill_recid:int):

    prepare_cache ([H_artikel, Htparam, Hoteldpt, H_journal, Wgrpgen, Artikel])
    ...
    h_bill_line = h_artikel = htparam = hoteldpt = h_journal = wgrpgen = artikel = None

    t_h_bill_line = ordered_item = summary_bill = t_h_artikel = None

    t_h_bill_line_list, T_h_bill_line = create_model_like(H_bill_line, {"rec_id":int, "request_str":string, "flag_code":int, "kds_stat":int, "kds_stat_str":string, "posting_id":int, "t_time":string}, {"kds_stat": -1})
