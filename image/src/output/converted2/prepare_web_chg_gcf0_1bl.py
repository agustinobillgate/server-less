...
def prepare_web_chg_gcf0_1bl(gastnr:int, chg_gcf:bool, master_gastnr:int):
    ...
    q2_history_list, Q2_history = create_model("Q2_history", {"ankunft":date, "abreise":date, "zinr":string, "zipreis":Decimal, "kurzbez":string, "bemerk":string, "arrangement":string})
    t_segment1_list, T_segment1 = create_model("T_segment1", {"bezeich":string})
    t_guest_list, T_guest = create_model_like(Guest)
    forecast_list, Forecast = create_model("Forecast", {"ankunft":date, "abreise":date, "zinr":string, "kurzbez":string, "arrangement":string, "zipreis":Decimal})
