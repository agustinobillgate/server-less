...
def avail_roombl(mi_clean:bool, mi_clean1:bool, mi_dirty:bool, mi_depart:bool, mi_occ:bool, mi_ooo:bool, mi_off:bool, ankunft:date, abreise:date):
    ...
    room_list_list, Room_list = create_model("Room_list", {"flag":int, "build":string, "build_flag":string, "zistatus":int, "infochar":string, "inactive":string, "zinr":string, "bezeich":string, "zikatnr":int, "rmcat":string, "etage":int, "outlook":string, "setup":string, "verbindung":[string,2], "infonum":int, "recid1":int, "recid2":int, "infostr":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal room_list_list, do_it, zikatnr, zimmer, outorder, res_line, zimkateg, paramtext, zimplan, guest
