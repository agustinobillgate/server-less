...
def walkin_list_1webbl(case_type:int, fdate:date, tdate:date, walk_in:int, wi_grp:int, walkin_flag:bool):
    ...
    t_walkin_list_list, T_walkin_list = create_model("T_walkin_list", {"resnr":int, "zinr":string, "name":string, "ankunft":date, "anztage":int, "abreise":date, "zimmeranz":int, "kurzbez":string, "erwachs":int, "kind1":int, "gratis":int, "resstatus":int, "arrangement":string, "zipreis":Decimal, "ankzeit":int, "abreisezeit":int, "bezeich":string, "bemerk":string, "gastnr":int, "res_address":string, "res_city":string, "res_bemerk":string})
    summary_list_list, Summary_list = create_model("Summary_list", {"arrangement":string, "rooms":int, "pax":int, "comp":int, "ch1":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_walkin_list_list, summary_list_list, wi_int, tot_rm, tot_pax, tot_comp, tot_ch1, htparam, reservation, segment, zimkateg, res_line, guest
        nonlocal case_type, fdate, tdate, walk_in, wi_grp, walkin_flag


        nonlocal t_walkin_list, summary_list
        nonlocal t_walkin_list_list, summary_list_list

        return {"t-walkin-list": t_walkin_list_list, "summary-list": summary_list_list}
    ...
    def create_walkin_list():
        ...
        summary_list = query(summary_list_list, filters=(lambda summary_list: summary_list.arrangement == t_walkin_list.arrangement), first=True)
