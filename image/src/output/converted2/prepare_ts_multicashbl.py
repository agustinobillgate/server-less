...
def prepare_ts_multicashbl(dept:int, transdate:date, amt:Decimal):
    ...
    grp_list_list, Grp_list = create_model("Grp_list", {"pos":int, "num":int, "bezeich":string, "artname":string, "curr":string, "exrate":Decimal, "exrate1":Decimal}, {"exrate": 1, "exrate1": 1})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal price_decimal, billdate, curr_local, billart, max_gpos, art_exrate, amount, paid, lpaid, change, lchange, err_flag, grp_list_list, htparam, waehrung, h_artikel, artikel, exrate
        ...
        return {"price_decimal": price_decimal, "billdate": billdate, "curr_local": curr_local, "billart": billart, "max_gpos": max_gpos, "art_exrate": art_exrate, "amount": amount, "paid": paid, "lpaid": lpaid, "change": change, "lchange": lchange, "err_flag": err_flag, "grp-list": grp_list_list}
    ...
    grp_list = query(grp_list_list, first=True)
