...
def ts_restinv_posting_menu_webbl(case_type:int, pvilanguage:int, tischnr:int, curr_dept:int, cancel_reason:string, double_currency:bool, exchg_rate:Decimal, price_decimal:int, transdate:date, foreign_rate:bool, deptname:string, cancel_order:bool, order_taker:int, curr_waiter:int, gname:string, pax:int, kreditlimit:Decimal, change_str:string, cc_comment:string, hoga_card:string, print_to_kitchen:bool, from_acct:bool, pay_type:int, guestnr:int, transfer_zinr:string, curedept_flag:bool, curr_room:string, user_init:string, hoga_resnr:int, hoga_reslinnr:int, incl_vat:bool, get_price:int, mc_str:string, segment_code:int, tp_bediener_list:[Tp_bediener], submenu_list_list:[Submenu_list], cancel_flag:bool, menu_list_list:[Menu_list], t_h_bill_list:[T_h_bill]):
    ...
    t_menu_list_list, T_menu_list = create_model("T_menu_list", {"request":string, "krecid":int, "posted":bool, "nr":int, "artnr":int, "bezeich":string, "anzahl":int, "price":Decimal, "betrag":Decimal, "voucher":string}, {"anzahl": 1, "voucher": ""})
    bmenu_list, Bmenu = create_model("Bmenu", {"request":string, "krecid":int, "posted":bool, "nr":int, "artnr":int, "bezeich":string, "anzahl":int, "price":Decimal, "betrag":Decimal, "voucher":string, "rec_menu":int}, {"anzahl": 1, "voucher": ""})
    t_h_artikel_list, T_h_artikel = create_model_like(H_artikel, {"rec_id":int})
    kellner1_list, Kellner1 = create_model_like(Kellner)
    t_h_bill_tmp_list, T_h_bill_tmp = create_model_like(H_bill, {"rec_id":int})
    ...
    def generate_output():
        nonlocal avail_bill, not_access, not_access1, bill_date, mwst, mwst_foreign, rechnr, balance, bcol, balance_foreign, fl_code, fl_code1, fl_code2, fl_code3, fl_code4, fl_code5, p_88, closed, amount, kellner1_list, t_h_artikel_list, menurecid, add_zeit, billart, req_str, voucher_str, request_str, perm, zugriff, loopn, description, qty, price, cancel_str, amount_foreign, curr_zeit, krecid, hbill_recid, long_time, first_flag, menulist_nr, menulist_artno, menulist_bez, h_artikel, h_bill, kellner, bediener, queasy, wgrpdep
        ...
        nonlocal t_menu_list_list, bmenu_list, t_h_artikel_list, kellner1_list, t_h_bill_tmp_list, t_submenu_list_list, tp_bediener1_list

        return {"cancel_flag": cancel_flag, "menu-list": menu_list_list, "t-h-bill": t_h_bill_list, "avail_bill": avail_bill, "not_access": not_access, "not_access1": not_access1, "bill_date": bill_date, "mwst": mwst, "mwst_foreign": mwst_foreign, "rechnr": rechnr, "balance": balance, "bcol": bcol, "balance_foreign": balance_foreign, "fl_code": fl_code, "fl_code1": fl_code1, "fl_code2": fl_code2, "fl_code3": fl_code3, "fl_code4": fl_code4, "fl_code5": fl_code5, "p_88": p_88, "closed": closed, "amount": amount, "kellner1": kellner1_list, "t-h-artikel": t_h_artikel_list}

    def update_bill(h_artart:int, h_artnrfront:int):

        nonlocal avail_bill, not_access, not_access1, bill_date, mwst, mwst_foreign, rechnr, balance, bcol, balance_foreign, fl_code, fl_code1, fl_code2, fl_code3, fl_code4, fl_code5, p_88, amount, kellner1_list, t_h_artikel_list, menurecid, add_zeit, billart, req_str, voucher_str, request_str, perm, zugriff, loopn, description, qty, price, cancel_str, amount_foreign, curr_zeit, krecid, hbill_recid, long_time, first_flag, menulist_nr, menulist_artno, menulist_bez, h_artikel, h_bill, kellner, bediener, queasy, wgrpdep
        nonlocal case_type, pvilanguage, tischnr, curr_dept, cancel_reason, double_currency, exchg_rate, price_decimal, transdate, foreign_rate, deptname, cancel_order, order_taker, curr_waiter, gname, pax, kreditlimit, change_str, cc_comment, hoga_card, print_to_kitchen, from_acct, pay_type, guestnr, transfer_zinr, curedept_flag, curr_room, user_init, hoga_resnr, hoga_reslinnr, incl_vat, get_price, mc_str, segment_code, cancel_flag
        ...
        if zugriff:
            ...
            t_h_bill_tmp = query(t_h_bill_tmp_list, first=True)
    ...
    if first_flag:
        ...
        for t_h_bill in query(t_h_bill_list):
            t_h_bill_tmp = T_h_bill_tmp()
            t_h_bill_tmp_list.append(t_h_bill_tmp)
        ...
        for menu_list in query(menu_list_list, filters=(lambda menu_list: menu_list.nr > 0), sort_by=[("nr",False)]):
        ...
        for t_h_bill_tmp in query(t_h_bill_tmp_list):
            t_h_bill = T_h_bill()
