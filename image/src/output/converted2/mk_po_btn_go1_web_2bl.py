...
t_l_order_list, T_l_order = create_model_like(L_order, {"rec_id":int, "a_bezeich":string, "price0":Decimal, "brutto":Decimal, "disc":Decimal, "disc2":Decimal, "vat":Decimal, "disc_val":Decimal, "disc2_val":Decimal, "vat_val":Decimal, "addvat_no":int, "addvat_value":Decimal})

def mk_po_btn_go1_web_2bl(t_l_orderhdr_list:[T_l_orderhdr], t_l_order_list:[T_l_order], docu_nr:string, lief_nr:int, billdate:date, create_new:bool, pr:string, globaldisc:Decimal, currency_screen_value:string, zeroprice_flag:bool, user_init:string):
    ...
    for t_l_order in query(t_l_order_list, filters=(lambda t_l_order: t_l_order.pos > 0 and t_l_order.docu_nr.lower()  == (docu_nr).lower()  and t_l_order.betriebsnr >= 98)):

        if t_l_order.betriebsnr == 99:
            t_l_order.geliefert =  to_decimal("0")
        t_l_order.betriebsnr = 2

    t_l_order = query(t_l_order_list, filters=(lambda t_l_order: t_l_order.pos > 0 and t_l_order.docu_nr.lower()  == (docu_nr).lower()  and t_l_order.betriebsnr == 2), first=True)
    ...
    t_l_order = query(t_l_order_list, filters=(lambda t_l_order: t_l_order.docu_nr.lower()  == (docu_nr).lower()  and t_l_order.pos > 0 and t_l_order.einzelpreis == 0 and t_l_order.betriebsnr == 2), first=True)
