...
def chg_sarticle_btn_chgnrbl(t_l_zwkum:int, t_l_endkum:int, t_l_artnr:int, l_zwkum:int, l_endkum:int, l_artnr:int, artnr:int):
    ...
    def generate_output():
        ...
        nonlocal t_l_zwkum, t_l_endkum, t_l_artnr, l_zwkum, l_endkum, l_artnr, artnr

        return {"artnr": artnr, "artnr_ok": artnr_ok}
    ...
    def find_new_artnr(update_flag:bool):
        ...
        for l_art1 in db_session.query(L_art1).filter(
                 (L_art1.zwkum == l_zwkum) & (L_art1.endkum == l_endkum)).order_by(L_art1.artnr.desc()).all():
