...
t_amount_list_list, T_amount_list = create_model("T_amount_list", {"nr":int, "anzahl":int, "menu_recid":int, "amount":Decimal, "orig_amount":Decimal, "unit_price":Decimal, "artnr":int})

def ts_hbline_get_taxservice3_webbl(vmode:int, hbill_recid:int, menu_nr:int, dept:int, artnr:int, price:Decimal, anzahl:int, incl_vat:bool, balance:Decimal, t_amount_list_list:[T_amount_list]):
    ...
    t_out_list_list, T_out_list = create_model("T_out_list", {"amount":Decimal, "orig_amount":Decimal, "amt_balance":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal amount, t_out_list_list, fact_scvat, curr_nr, curr_balance, price_decimal, h_bill, htparam
        nonlocal vmode, hbill_recid, menu_nr, dept, artnr, price, anzahl, incl_vat, balance


        nonlocal menu_list, t_amount_list, t_out_list
        nonlocal menu_list_list, t_out_list_list

        return {"balance": balance, "t-amount-list": t_amount_list_list, "amount": amount, "t-out-list": t_out_list_list}
    ...
    if vmode == 1:
        ...
        t_out_list_list.append(t_out_list)
        ...
        if not incl_vat:
            ...
            for t_amount_list in query(t_amount_list_list):
                amount =  to_decimal(amount) + to_decimal(t_amount_list.amount)
        ...
        for t_amount_list in query(t_amount_list_list, filters=(lambda t_amount_list: t_amount_list.nr > 0), sort_by=[("nr",False)]):
            curr_nr = curr_nr + 1
