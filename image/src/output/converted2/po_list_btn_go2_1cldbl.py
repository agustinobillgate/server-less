...
def po_list_btn_go2_1cldbl(t_liefno:int, last_docu_nr:string, sorttype:int, deptnr:int, all_supp:bool, stattype:int, usrname:string, from_date:date, to_date:date, billdate:date, dml_only:bool, app_sort:string, pr_only:bool, excl_dml_pr:bool):
    ...
    q_list_list, Q_list = create_model_like(Q2_list, {"to_sort":int})
    ...
    def generate_output():
        nonlocal first_docu_nr, curr_docu_nr, p_267, last_docu_nr1, q2_list_list, counter, curr_counter, last_to_sort, temp_docu_nr, temp_counter_nr, approval, p_71, loop, app_lvl, cost_list_bezeich, w_list_wabkurz, l_order, l_lieferant, l_artikel, queasy, htparam, l_orderhdr, parameters, waehrung
        ...
        nonlocal w_list_list, cost_list_list, q2_list_list, q_list_list

        return {"first_docu_nr": first_docu_nr, "curr_docu_nr": curr_docu_nr, "p_267": p_267, "last_docu_nr1": last_docu_nr1, "q2-list": q2_list_list}
        ...
        if stattype == 0 and usrname == "":
            ...
            if last_docu_nr != "":
                ...
                for l_orderhdr, l_lieferant, l_order1 in db_session.query(L_orderhdr, L_lieferant, L_order1).join(L_lieferant,(L_lieferant.lief_nr == L_orderhdr.lief_nr)).join(L_order1,(L_order1.docu_nr == L_orderhdr.docu_nr) & (L_order1.loeschflag == 0) & (L_order1.pos == 0)).filter(
                    ...
                    w_list = query(w_list_list, filters=(lambda w_list: w_list.nr == l_orderhdr.angebot_lief[2]), first=True)

                    cost_list = query(cost_list_list, filters=(lambda cost_list: cost_list.nr == l_orderhdr.angebot_lief[0]), first=True)
    ...
    def disp_list1a():
        ...
        if stattype == 0 and usrname == "":
            ...
            if last_docu_nr != "":
                ...
                for q_list in query(q_list_list, filters=(lambda q_list: q_list.to_sort > temp_counter_nr)):
