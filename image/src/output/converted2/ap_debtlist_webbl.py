...
def ap_debtlist_webbl(duedate_flag:bool, from_lief:int, to_lief:int, paid_flag:bool, excl_apmanual:bool, from_date:date, to_date:date, from_supp:string, to_supp:string, only_apmanual:bool, taxcode_list_list:[Taxcode_list]):
    ...
    output_list_list, Output_list = create_model("Output_list", {"paid":string, "ap_recid":int, "str":string, "lscheinnr":string, "gstid":string, "tax_code":string, "tax_amount":string, "tot_amt":string, "lief":string, "datum":string, "docu_nr":string, "name":string, "amount":string, "id":string, "due_date":string, "remark":string})
    ...
    def create_list0a():
        ...
        for l_kredit.rgdatum, l_kredit.ziel, l_kredit._recid, l_kredit.name, l_kredit.netto, l_kredit.lscheinnr, l_kredit.bediener_nr, l_kredit.bemerk, l_kredit.steuercode, l_kredit.opart, l_lieferant.segment1, l_lieferant.firma, l_lieferant.plz, l_lieferant._recid in db_session.query(L_kredit.rgdatum, L_kredit.ziel, L_kredit._recid, L_kredit.name, L_kredit.netto, L_kredit.lscheinnr, L_kredit.bediener_nr, L_kredit.bemerk, L_kredit.steuercode, L_kredit.opart, L_lieferant.segment1, L_lieferant.firma, L_lieferant.plz, L_lieferant._recid).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr)).filter(
            ...
            if do_it:
                ...
                output_list.datum = to_string(l_kredit.rgdatum)
                ...
                if bediener:
                    ...
                    output_list.id = " "
                output_list.due_date = to_string(due_date)
