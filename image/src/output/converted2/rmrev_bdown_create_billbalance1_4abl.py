...
from models import Argt_line, Billjournal, Waehrung, Guest, Artikel, Htparam, Res_line, Zimmer, Genstat, Arrangement, Exrate, Reservation, Bill, Zimkateg, Reslin_queasy

def rmrev_bdown_create_billbalance1_4abl(exc_taxserv:bool, pvilanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, curr_date:date, srttype:int):

    prepare_cache ([Waehrung, Guest, Artikel, Htparam, Res_line, Zimmer, Genstat, Arrangement, Exrate, Bill, Zimkateg, Reslin_queasy])
    ...
    argt_line = billjournal = waehrung = guest = artikel = htparam = res_line = zimmer = genstat = arrangement = exrate = reservation = bill = zimkateg = reslin_queasy = None

    sum_list = currency_list = cl_list = s_list = argt_list = t_argt_line = t_billjournal = waehrung1 = cc_list = None
    ...
    argt_list_list, Argt_list = create_model("Argt_list", {"argtnr":int, "argtcode":string, "bezeich":string, "room":int, "pax":int, "qty":int, "bfast":Decimal})
    t_argt_line_list, T_argt_line = create_model_like(Argt_line)
    t_billjournal_list, T_billjournal = create_model_like(Billjournal)
    ...
    def generate_output():
        nonlocal cl_list_list, currency_list_list, sum_list_list, s_list_list, argt_list_list, exchg_rate, frate, post_it, total_rev, temptotjur, lvcarea, argt_line, billjournal, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, bill, zimkateg, reslin_queasy
        nonlocal exc_taxserv, pvilanguage, new_contrate, foreign_rate, price_decimal, curr_date, srttype
        nonlocal waehrung1, cc_list


        nonlocal sum_list, currency_list, cl_list, s_list, argt_list, t_argt_line, t_billjournal, waehrung1, cc_list
        nonlocal sum_list_list, currency_list_list, cl_list_list, s_list_list, argt_list_list, t_argt_line_list, t_billjournal_list
        ...
        if srttype == 2:
            ...
            for genstat.argt, genstat.datum, genstat.zinr, genstat.resnr, genstat.res_int, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.kind3, genstat.resstatus, genstat.zipreis, genstat.ratelocal, genstat.logis, genstat.res_deci, genstat.zikatnr, genstat.res_char, genstat._recid, res_line.betriebsnr, res_line.reserve_dec, res_line.gastnrpay, res_line.gastnrmember, res_line.resnr, res_line.l_zuordnung, res_line.zikatnr, res_line._recid, res_line.name, res_line.ankunft, res_line.abreise, res_line.resname, res_line.zimmer_wunsch, res_line.reslinnr, res_line.adrflag, res_line.zinr, res_line.zipreis, res_line.reserve_int, zimmer.sleeping, zimmer._recid in db_session.query(Genstat.argt, Genstat.datum, Genstat.zinr, Genstat.resnr, Genstat.res_int, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.kind3, Genstat.resstatus, Genstat.zipreis, Genstat.ratelocal, Genstat.logis, Genstat.res_deci, Genstat.zikatnr, Genstat.res_char, Genstat._recid, Res_line.betriebsnr, Res_line.reserve_dec, Res_line.gastnrpay, Res_line.gastnrmember, Res_line.resnr, Res_line.l_zuordnung, Res_line.zikatnr, Res_line._recid, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.resname, Res_line.zimmer_wunsch, Res_line.reslinnr, Res_line.adrflag, Res_line.zinr, Res_line.zipreis, Res_line.reserve_int, Zimmer.sleeping, Zimmer._recid).join(Res_line,(Res_line.resnr == Genstat.resnr) & (Res_line.reslinnr == Genstat.res_int[inc_value(0)]) & (Res_line.l_zuordnung[inc_value(2)] == 0)).join(Zimmer,(Zimmer.zinr == Genstat.zinr)).filter(
                ...
                if not argt_list:
                    ...
                    argt_list.room = 1
                    argt_list.pax = cl_list.pax


                else:
                    argt_list.room = argt_list.room + 1
                    argt_list.pax = argt_list.pax + cl_list.pax
                ...
                if reslin_queasy:
                    ...
                    if cl_list.zipreis != round(reslin_queasy.deci1, price_decimal):
                        cl_list.diffrate = True
                    cl_list.fix_rate = "F"
                    ...
                    if bill_flag2.lower()  == ("Guest Bill").lower() :
                        ...
                        for billjournal, artikel in db_session.query(Billjournal, Artikel).join(Artikel,(Artikel.artnr == Billjournal.artnr) & (Artikel.departement == Billjournal.departement) & (Artikel.artart != 9)).filter(
                            ...
                            if billjournal.artnr != deposit_art:
                                ...
                                t_billjournal = query(t_billjournal_list, filters=(lambda t_billjournal: t_billjournal.rechnr == billjournal.rechnr and t_billjournal.artnr == billjournal.artnr and t_billjournal.zinr == billjournal.zinr), first=True)

                                if not t_billjournal:

                                    if lcur == waehrung1.wabkurz:
                                        s_list.f_betrag =  to_decimal(s_list.f_betrag) + to_decimal(billjournal.betrag)
    ...
    def get_genstat_argt_betrag(frate:Decimal, argt_betrag:Decimal):
        ...
        nonlocal cl_list_list, currency_list_list, sum_list_list, s_list_list, argt_list_list, exchg_rate, post_it, total_rev, temptotjur, lvcarea, argt_line, billjournal, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, bill, zimkateg, reslin_queasy
        nonlocal exc_taxserv, pvilanguage, new_contrate, foreign_rate, price_decimal, curr_date, srttype
        nonlocal waehrung1, cc_list
