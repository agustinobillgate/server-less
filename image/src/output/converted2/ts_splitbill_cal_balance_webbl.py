...
def ts_splitbill_cal_balance_webbl(curr_dept:int, curr_select:int, t_rechnr:int):
    ...
    def generate_output():
        ...
        return {"balance": balance, "t-h-bill-line": t_h_bill_line_list, "summary-bill": summary_bill_list, "t-h-artikel": t_h_artikel_list}
    ...
    if curr_select > 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
                 (H_bill_line.rechnr == t_rechnr) & (H_bill_line.waehrungsnr == curr_select)).order_by(H_bill_line._recid).all():
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
                 (H_bill_line.rechnr == t_rechnr)).order_by(H_bill_line._recid).all():
            t_h_bill_line = T_h_bill_line()
            t_h_bill_line_list.append(t_h_bill_line)
