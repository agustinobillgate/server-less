...
from functions.hk_statadmin_chg_zistatus_cldbl import hk_statadmin_chg_zistatus_cldbl
from models import Zimkateg, Bediener

room_list_list, Room_list = create_model("Room_list", {"nr":string})

def hk_change_room_status_webbl(pvilanguage:int, chgsort:int, user_init:string, room_list_list:[Room_list], remark:string):
    ...
    t_bline_list_list, T_bline_list = create_model("T_bline_list", {"zinr":string, "selected":bool, "bl_recid":int})
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, chgsort, user_init, remark
    ...
    if chgsort != 4 and chgsort != 5:
        ...
        if flag == 6:
            ...
            if t_zistatus == 6:
                ...
                bline_list_list, om_list_list, curr_zinr, curr_stat, z_list_list = get_output(hk_statadmin_chg_zistatus_cldbl(bline_list_list, om_list_list, 0, ci_date, chgsort, "", user_init, bediener.nr, remark))
                msg_str = translateExtended ("Status Changed Success.", lvcarea, "")
        else:
