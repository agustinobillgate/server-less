...
def top10_incoming_outgoing_webbl(from_date:date, to_date:date, from_grp:int, to_grp:int, store:int, sorttype:int):
    ...
    str_list_list, Str_list = create_model("Str_list", {"artnr_no":int, "artikel_name":string, "qty":Decimal, "unit_price":Decimal, "tot_price":Decimal, "tot_incoming":Decimal, "tot_outgoing":Decimal})
    str_list_output_list, Str_list_output = create_model("Str_list_output", {"artnr_no":int, "artikel_name":string, "qty":Decimal, "unit_price":Decimal, "tot_price":Decimal, "turnover":Decimal, "tot_outgoing":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal str_list_output_list, tot_qty, qty, counter, unit_price, tot_price, tot_incoming, tot_outgoing, l_artikel, l_lieferant, l_ophis, l_op, l_ophhis, l_lager, gl_acct, gl_department, l_ophdr
        nonlocal from_date, to_date, from_grp, to_grp, store, sorttype


        nonlocal str_list, str_list_output
        nonlocal str_list_list, str_list_output_list

        return {"str-list-output": str_list_output_list}
    ...
    def incoming_procedure():
        ...
        if store == 0:
            ...
            for l_ophis, l_artikel, l_lieferant in db_session.query(L_ophis, L_artikel, L_lieferant).join(L_artikel,(L_artikel.artnr == L_ophis.artnr) & (L_artikel.endkum >= from_grp) & (L_artikel.endkum <= to_grp)).join(L_lieferant,(L_lieferant.lief_nr == L_ophis.lief_nr)).filter(
                ...
                str_list = query(str_list_list, filters=(lambda str_list: str_list.artnr_no == l_ophis.artnr), first=True)
            ...
            for l_op, l_artikel, l_lieferant in db_session.query(L_op, L_artikel, L_lieferant).join(L_artikel,(L_artikel.artnr == L_op.artnr) & (L_artikel.endkum >= from_grp) & (L_artikel.endkum <= to_grp)).join(L_lieferant,(L_lieferant.lief_nr == L_op.lief_nr)).filter(
                ...
                str_list = query(str_list_list, filters=(lambda str_list: str_list.artnr_no == l_op.artnr), first=True)
                ...
                if not str_list:
                    ...
                    str_list.tot_incoming =  to_decimal(str_list.tot_incoming) + to_decimal("1")
            str_list_output_list.clear()

            for str_list in query(str_list_list, sort_by=[("tot_incoming",True)]):
                counter = counter + 1
                str_list_output = Str_list_output()
                str_list_output_list.append(str_list_output)
    ...
    def outgoing_procedure():
        ...
        if store == 0:
            ...
            for str_list in query(str_list_list, sort_by=[("tot_outgoing",True)]):
                counter = counter + 1
                str_list_output = Str_list_output()
                str_list_output_list.append(str_list_output)
