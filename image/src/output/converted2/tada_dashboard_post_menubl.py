...
menu_list_list, Menu_list = create_model("Menu_list", {"artnr":int, "description":string, "qty":int, "price":Decimal, "special_request":string})

def tada_dashboard_post_menubl(user_init:string, tablenumber:int, outletnumber:int, guestname:string, pax:int, orderid:int, menu_list_list:[Menu_list]):
    ...
    crd_list_list, Crd_list = create_model("Crd_list", {"deptno":int, "uname":string, "pass":string, "tada_outlet_id":int})

    Mn_list = Menu_list
    mn_list_list = menu_list_list
    ...
    def generate_output():
        ...
        nonlocal hbill_list, t_kellner_list, t_h_artikel_list, t_h_bill_list, t_submenu_list_list, kellner1_list, crd_list_list
    ...
    crd_list = query(crd_list_list, filters=(lambda crd_list: crd_list.deptno == curr_dept), first=True)

    if crd_list:
        deptno = crd_list.tada_outlet_id
    mealcoupon_cntrl, must_print, zero_flag, multi_cash, cancel_exist, msg_str, disc_art1, disc_art2, disc_art3, mi_ordertaker, price_decimal, curr_local, curr_foreign, double_currency, foreign_rate, exchg_rate, b_title, deptname, p_223, curr_waiter, fl_code, pos1, pos2, cashless_flag, hbill_list, t_kellner_list = get_output(prepare_tada_post_menubl(language_code, curr_dept, 0, user_init, None))
    ...
    for orderline in db_session.query(Orderline).filter(
        ...
        mn_list = query(mn_list_list, filters=(lambda mn_list: mn_list.artnr == orderline.number1), first=True)
    ...
    for menu_list in query(menu_list_list):
        ...
        bill_date, cancel_flag, fl_code, mwst, mwst_foreign, billnumber, balance, bcol, balance_foreign, fl_code1, fl_code2, fl_code3, p_88, closed, t_h_bill_list, kellner1_list = get_output(tada_update_billbl(language_code, record_id, h_artikel._recid, "", None, h_artikel.artart, False, h_artikel.service_code, amount, 0, price, double_currency, menu_list.qty, exchg_rate, price_decimal, user_init, tischnr, curr_dept, user_init, gname, pax, 0, add_zeit, h_artikel.artnr, menu_list.DESCRIPTION, "", "", "", menu_list.special_request, "", "", True, False, h_artikel.artnrfront, 0, guestnr, "", False, foreign_rate, curr_room, user_init, resnr, reslinnr, t_submenu_list_list))
