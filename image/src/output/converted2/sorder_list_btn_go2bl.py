...
def sorder_list_btn_go2bl(user_init:string, sorttype:int, s_artnr:int, from_date:date, to_date:date, from_sup:string, to_sup:string, closepo:bool):
    ...
    str_list_list, Str_list = create_model("Str_list", {"docu_nr":string, "s":string, "dunit":string, "content":int, "lief_nr":int, "warenwert":Decimal})
    str_list2_list, Str_list2 = create_model("Str_list2", {"docu_nr":string, "l_order_bestelldatum":string, "l_order_docu_nr":string, "l_order_artnr":string, "l_artikel_bezeich":string, "l_order_anzahl":string, "l_order_preis":string, "l_order_warenwert":string, "l_order_geliefert":string, "l_order_angebot_lief1":string, "l_order_rechnungswert":string, "l_orderhdr_lieferdatum":string, "l_lieferant_firma":string, "cost_list_bezeich":string, "dunit":string, "content":int, "lief_nr":int, "warenwert":Decimal})
    po_list_list, Po_list = create_model("Po_list", {"datum":date, "document_no":string, "artno":int, "artdesc":string, "orderqty":int, "unit_price":Decimal, "amount1":Decimal, "delivered":int, "s_unit":int, "amount2":Decimal, "delivdate":date, "supplier":string})
    cost_list_list, Cost_list = create_model("Cost_list", {"nr":int, "bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal po_list_list, delidate, amount1, amount2, long_digit, show_price, price_decimal, htparam, bediener, l_orderhdr, l_lieferant, l_artikel, l_order, parameters
        ...
        nonlocal str_list_list, str_list2_list, po_list_list, cost_list_list
    ...
    str_list2_list = sorder_list_btn_gobl(user_init, sorttype, s_artnr, from_date, to_date, from_sup, to_sup, closepo)
    po_list_list.clear()

    for str_list2 in query(str_list2_list):
        ...
        if matches(str_list2.l_order_preis,r"*T O T A L*"):
            ...
            artdesc = "T O T A L"
            amount1 =  to_decimal(to_decimal(str_list2.l_order_warenwert) )
            ...
            datum = date_mdy(str_list2.l_order_bestelldatum)
            document_no = str_list2.l_order_docu_nr
            artno = to_int(str_list2.l_order_artnr)
            artdesc = str_list2.l_artikel_bezeich
            orderqty = to_int(str_list2.l_order_anzahl)
            unit_price = to_decimal(str_list2.l_order_preis)
            amount1 =  to_decimal(to_decimal(str_list2.l_order_warenwert) )
            delivered = to_int(str_list2.l_order_geliefert)
            s_unit = to_int(str_list2.l_order_angebot_lief1)
            amount2 =  to_decimal(to_decimal(str_list2.l_order_rechnungswert) )
            delivdate = date_mdy(str_list2.l_orderhdr_lieferdatum)
            supplier = str_list2.l_lieferant_firma
