...
def bookengine_ratecodepush_btn_exit_1bl(t_push_list_list:[T_push_list], bookengid:int, user_init:string):
    ...
    outlist_list, Outlist = create_model("Outlist", {"key":int, "number1":int, "char1":string})
    ...
    for t_push_list in query(t_push_list_list):
        str = t_push_list.rcodevhp + ";" + t_push_list.rcodebe + ";" + t_push_list.rmtypevhp + ";" + t_push_list.rmytpebe + ";" + t_push_list.argtvhp
        bufq = Queasy()
        db_session.add(bufq)
        ...
        for outlist in query(outlist_list, filters=(lambda outlist: outlist.key == 161 and outlist.number1 == bookengid)):
