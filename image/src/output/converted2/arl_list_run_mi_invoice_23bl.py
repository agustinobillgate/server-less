...
def arl_list_run_mi_invoice_23bl(resnr:int, curr_resnr:int, arl_list_reslinnr:int, t_active_flag:int, printtype:int, split_bill:bool):
    ...
    prepare_cache ([Reservation, Res_line, Bill, Htparam, Master, Waehrung, Bill_line, Artikel, Zimkateg, Arrangement, Reslin_queasy, Guest_pr, Queasy, Fixleist, Billjournal, Genstat, Exrate])
    ...
    def generate_output():
        ...
        nonlocal s_list_list, t_list_list, periode_list_list, t_reservation_list, t_res_line_list, t_guest_list
    ...
    def read_proforma_inv(resnr:int, rechnr:int):
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 301)],"number1": [(eq, resnr)],"logi1": [(eq, True)]})
        ...
        if queasy:
            ...
            for s_list in query(s_list_list, sort_by=[("ankunft",False),("datum",False),("bezeich",False),("erwachs",False)]):
                ...
                periode_list = query(periode_list_list, filters=(lambda periode_list: periode_list.s_list.datum >= periode_list.periode1 and s_list.datum <= periode_list.periode2), first=True)

                if periode_list:

                    t_list = query(t_list_list, filters=(lambda t_list: t_list.bezeich == s_list.bezeich and t_list.rmcat == s_list.rmcat and t_list.date1 == periode_list.periode1 and t_list.date2 == periode_list.periode2), first=True)
                    ...
                    t_list.preis =  to_decimal(t_list.preis) + to_decimal(s_list.preis)
    ...
    def calc_periode():
        ...
        if queasy:
            periode_rsv1 = queasy.date1
            periode_rsv2 = queasy.date2
