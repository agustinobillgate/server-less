...
def prepare_fo_transdetail_webbl(bill_no:int, billdate:date, artno:int, amount:Decimal, systdate:date, systtime:int):
    ...
    temp_bjournal_list, Temp_bjournal = create_model_like(Billjournal, {"item_name":string, "from_roomno":string, "from_billno":int, "from_qty":int}, {"item_name": ""})
    t_bill_list, T_bill = create_model_like(Bill, {"rec_id":int})
    ...
    def generate_output():
        nonlocal created, found, err, p_2314, p_497, avail_brief497, temp_bjournal_list, t_bill_list, currdate, ci_dt, co_dt, old_room, old_billno, tf_billno, zeit, fr_date, to_date, billjournal, bill, htparam, brief, res_line, artikel
        nonlocal bill_no, billdate, artno, amount, systdate, systtime
        nonlocal buf_bill


        nonlocal temp_bjournal, t_bill, buf_bill
        nonlocal temp_bjournal_list, t_bill_list

        return {"created": created, "found": found, "err": err, "p_2314": p_2314, "p_497": p_497, "avail_brief497": avail_brief497, "temp-bjournal": temp_bjournal_list, "t-bill": t_bill_list}
    ...
    for temp_bjournal in query(temp_bjournal_list):
