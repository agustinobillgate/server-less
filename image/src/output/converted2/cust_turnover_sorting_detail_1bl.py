...
def cust_turnover_sorting_detail_1bl(cardtype:int, sort_type:int, fdate:date, tdate:date, check_ftd:bool, currency:string, excl_other:bool, curr_sort2:int):

    prepare_cache ([Genstat, Guest, Htparam, Waehrung, Nation, Queasy, exrate, Res_line, Guest_queasy, Reservation, Arrangement])
    ...
    def generate_output():
        ...
        return {"curr_sort2": curr_sort2, "b-list": b_list_list}
                            ...
                            for bill in db_session.query(Bill).filter(
                                ...
                                for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & (Artikel.artart == 0)).filter(
                                    ...
                                    if currency != " ":
                                        ...
                                        if exrate:
                                            cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + (bill_line.betrag / exrate.betrag) , "->>>,>>>,>>>,>>9.99")
                                            cust_list_detail.gesamtumsatz = to_string(to_decimal(cust_list_detail.gesamtumsatz) + (bill_line.betrag / exrate.betrag) , "->>>,>>>,>>>,>>9.99")


                                    else:
                                        cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + bill_line.betrag, "->>>,>>>,>>>,>>9.99")
                                        cust_list_detail.gesamtumsatz = to_string(to_decimal(cust_list_detail.gesamtumsatz) + bill_line.betrag, "->>>,>>>,>>>,>>9.99")
                        ...
                        if genstat:
                            ...
                            if cust_list_detail:
                                ...
                                if currency != " ":
                                    ...
                                    if exrate:
                                        cust_list_detail.f_b_umsatz = to_string(to_decimal(cust_list_detail.f_b_umsatz) + ((guest_queasy.deci1 + guest_queasy.deci2) / exrate.betrag) , "->>>,>>>,>>>,>>9.99")
                                        cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + (guest_queasy.deci3 / exrate.betrag) , "->>>,>>>,>>>,>>9.99")
                                    ...
                                    cust_list_detail.f_b_umsatz = to_string(to_decimal(cust_list_detail.f_b_umsatz) + guest_queasy.deci1 + guest_queasy.deci2, "->>>,>>>,>>>,>>9.99")
                                    cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + guest_queasy.deci3, "->>>,>>>,>>>,>>9.99")
                                    cust_list_detail.gesamtumsatz = to_string(to_decimal(cust_list_detail.gesamtumsatz) + guest_queasy.deci1 + guest_queasy.deci2 +\
    ...
    def cr_ftd():
        ...
        nonlocal b_list_list, bfast_art, lunch_art, dinner_art, lundin_art, service, vat, datum, end_date, net_lodg, fnet_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, curr_i, i, ly_fdate, ly_tdate, ci_date, pos, curr_gastnr, curr_gname, curr_gastnr2, t_logiernachte, t_fb_umsatz, t_sonst_umsatz, t_ba_umsatz, t_gesamtumsatz, t_lyear, t_nofrm, tot_logiernachte, tot_argtumsatz, tot_fb_umsatz, tot_sonst_umsatz, tot_ba_umsatz, tot_gesamtumsatz, tot_stayno, tot_lyear, tot_nofrm, gt_logiernachte, gt_argtumsatz, gt_fb_umsatz, gt_sonst_umsatz, gt_ba_umsatz, gt_gesamtumsatz, gt_stayno, gt_lyear, gt_nofrm, curr_resnr1, curr_reslinnr1, found1, loopj, exratenr, exrate, genstat, guest, bill_line, bill, htparam, waehrung, nation, queasy, exrate, res_line, artikel, guest_queasy, reservation, sourccod, segment, arrangement, zimmer
        nonlocal cardtype, sort_type, fdate, tdate, check_ftd, currency, excl_other, curr_sort2
        nonlocal t_genstat, blist, glist, bguest, clist, bline, bbuf
        ...
        guest = Guest()
        for genstat.gastnrmember, genstat.gastnr, genstat.resnr, genstat.res_int, genstat.res_date, genstat.datum, genstat.res_deci, genstat.logis, genstat.resstatus, genstat._recid, guest.phonetik3, guest.name, guest.anredefirma, guest.vorname1, guest.plz, guest.land, guest.wohnort, guest.anrede1, guest._recid in db_session.query(Genstat.gastnrmember, Genstat.gastnr, Genstat.resnr, Genstat.res_int, Genstat.res_date, Genstat.datum, Genstat.res_deci, Genstat.logis, Genstat.resstatus, Genstat._recid, Guest.phonetik3, Guest.name, Guest.anredefirma, Guest.vorname1, Guest.plz, Guest.land, Guest.wohnort, Guest.anrede1, Guest._recid).join(Guest,(Guest.gastnr == Genstat.gastnr) & (Guest.karteityp == cardtype)).filter(
                 (Genstat.datum >= fdate) & (Genstat.datum <= tdate) & (Genstat.segmentcode != 0) & (Genstat.nationnr != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)])).order_by(Genstat.gastnr, Genstat.resnr, Genstat.res_int[0], Guest.land).all():
            if genstat_obj_list.get(genstat._recid):
                continue
            else:
                genstat_obj_list[genstat._recid] = True
    ...
    def create_forecast():
        ...
        nonlocal b_list_list, bfast_art, lunch_art, dinner_art, lundin_art, service, vat, end_date, net_lodg, fnet_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, curr_i, i, ly_fdate, ly_tdate, ci_date, pos, curr_gastnr, curr_gname, curr_gastnr2, t_logiernachte, t_argtumsatz, t_fb_umsatz, t_sonst_umsatz, t_ba_umsatz, t_gesamtumsatz, t_lyear, t_nofrm, tot_logiernachte, tot_argtumsatz, tot_fb_umsatz, tot_sonst_umsatz, tot_ba_umsatz, tot_gesamtumsatz, tot_stayno, tot_lyear, tot_nofrm, gt_logiernachte, gt_argtumsatz, gt_fb_umsatz, gt_sonst_umsatz, gt_ba_umsatz, gt_gesamtumsatz, gt_stayno, gt_lyear, gt_nofrm, curr_resnr1, curr_reslinnr1, found1, loopj, exratenr, exrate, genstat, guest, bill_line, bill, htparam, waehrung, nation, queasy, exrate, res_line, artikel, guest_queasy, reservation, sourccod, segment, arrangement, zimmer
        nonlocal cardtype, sort_type, fdate, tdate, check_ftd, currency, excl_other, curr_sort2
        nonlocal t_genstat, blist, glist, bguest, clist, bline, bbuf
        ...
        for res_line.zimmeranz, res_line.resnr, res_line.arrangement, res_line.reslinnr, res_line.zinr, res_line.gastnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.gastnrmember, res_line._recid, res_line.resstatus, res_line.zimmerfix, guest.phonetik3, guest.name, guest.anredefirma, guest.vorname1, guest.plz, guest.land, guest.wohnort, guest.anrede1, guest._recid in db_session.query(Res_line.zimmeranz, Res_line.resnr, Res_line.arrangement, Res_line.reslinnr, Res_line.zinr, Res_line.gastnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.gastnrmember, Res_line._recid, Res_line.resstatus, Res_line.zimmerfix, Guest.phonetik3, Guest.name, Guest.anredefirma, Guest.vorname1, Guest.plz, Guest.land, Guest.wohnort, Guest.anrede1, Guest._recid).join(Guest,(Guest.gastnr == Res_line.gastnr) & (Guest.karteityp == cardtype)).filter(
            ...
            if do_it:
                ...
                for datum in date_range(datum1,datum2) :
                        ...
                        if excl_other == False:
                            ...
                            for bill in db_session.query(Bill).filter(
                                ...
                                for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & (Artikel.artart == 0)).filter(
                                    ...
                                    if currency != " ":
                                        cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + (bill_line.betrag / exrate) , "->>>,>>>,>>>,>>9.99")
                                        cust_list_detail.gesamtumsatz = to_string(to_decimal(cust_list_detail.gesamtumsatz) + (bill_line.betrag / exrate) , "->>>,>>>,>>>,>>9.99")
                    ...
                    if currency != " ":
                        cust_list_detail.f_b_umsatz = to_string(to_decimal(cust_list_detail.f_b_umsatz) + ((tot_breakfast + tot_lunch + tot_dinner) / exrate) , "->>>,>>>,>>>,>>9.99")
                        cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + (tot_other / exrate) , "->>>,>>>,>>>,>>9.99")
                        cust_list_detail.argtumsatz = to_string(to_decimal(cust_list_detail.argtumsatz) + (net_lodg / exrate) , "->>>,>>>,>>>,>>9.99")
            ...
            for guest_queasy in db_session.query(Guest_queasy).filter(
                ...
                if res_line:
                    ...
                    if cust_list_detail:
                        ...
                        if currency != " ":
                            cust_list_detail.f_b_umsatz = to_string(to_decimal(cust_list_detail.f_b_umsatz) + ((guest_queasy.deci1 + guest_queasy.deci2) / exrate) , "->>>,>>>,>>>,>>9.99")
                            cust_list_detail.sonst_umsatz = to_string(to_decimal(cust_list_detail.sonst_umsatz) + (guest_queasy.deci3 / exrate) , "->>>,>>>,>>>,>>9.99")
                            cust_list_detail.gesamtumsatz = to_string(to_decimal(cust_list_detail.gesamtumsatz) + ((guest_queasy.deci1 + guest_queasy.deci2 +\
    ...
    def create_forecast1():
        ...
        for res_line.zimmeranz, res_line.resnr, res_line.arrangement, res_line.reslinnr, res_line.zinr, res_line.gastnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.gastnrmember, res_line._recid, res_line.resstatus, res_line.zimmerfix, guest.phonetik3, guest.name, guest.anredefirma, guest.vorname1, guest.plz, guest.land, guest.wohnort, guest.anrede1, guest._recid in db_session.query(Res_line.zimmeranz, Res_line.resnr, Res_line.arrangement, Res_line.reslinnr, Res_line.zinr, Res_line.gastnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.gastnrmember, Res_line._recid, Res_line.resstatus, Res_line.zimmerfix, Guest.phonetik3, Guest.name, Guest.anredefirma, Guest.vorname1, Guest.plz, Guest.land, Guest.wohnort, Guest.anrede1, Guest._recid).join(Guest,(Guest.gastnr == Res_line.gastnr)).filter(
            ...
            if do_it:
                ...
                for cust_list_detail in query(cust_list_detail_list, filters=(lambda cust_list_detail: cust_list_detail.resno == 4523)):

                cust_list_detail = query(cust_list_detail_list, filters=(lambda cust_list_detail: cust_list_detail.gastnr == res_line.gastnr and cust_list_detail.gname == res_line.NAME), first=True)

                if not cust_list_detail:

                    if res_line.resnr == 4523:
                        cust_list_detail = Cust_list_detail()
                        cust_list_detail_list.append(cust_list_detail)
    ...
    if sort_type == 0:
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("gesamtumsatz",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("logiernachte",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("cust_name",False)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("argtumsatz",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("f_b_umsatz",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("sonst_umsatz",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("wohnort",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("plz",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("land",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("resnr",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("ly_rev",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("count_room",True)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("region",False)]):
        ...
        for cust_list_detail in query(cust_list_detail_list, sort_by=[("gastnr",False),("ba_umsatz",True)]):
