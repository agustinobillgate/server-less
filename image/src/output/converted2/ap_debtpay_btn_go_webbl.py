        ...
        for l_kredit, l_lieferant in db_session.query(L_kredit, L_lieferant).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr)).filter(
            ...
            if create_it:
                ...
                for i in range(1,len(l_lieferant.bank)  + 1) :
                    ...
                    if re.match(r"a/n",substring(l_lieferant.bank, i - 1, 3), re.IGNORECASE):
                        break
            ...
            if gl_jouhdr:
                ...
                for gl_journal, gl_acct in db_session.query(Gl_journal, Gl_acct).join(Gl_acct,(Gl_acct.fibukonto == Gl_journal.fibukonto) & ((Gl_acct.acc_type == 2) | (Gl_acct.acc_type == 3) | (Gl_acct.acc_type == 4) | (Gl_acct.acc_type == 5))).filter(
