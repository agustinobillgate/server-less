...
def arl_list_run_mi_invoice_22_webbl(resnr:int, curr_resnr:int, arl_list_reslinnr:int, t_active_flag:int, printtype:int, split_bill:bool):
    ...
    s_list_list, S_list = create_model("S_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "datum":date, "qty":int, "erwachs":int, "kind1":int, "kind2":int, "zinr":string, "wabkurz":string}, {"wabkurz": " "})
    t_list_list, T_list = create_model("T_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "tage":int, "date1":date, "date2":date, "qty":int, "betrag":Decimal, "erwachs":int, "kind1":int, "kind2":int, "vat":Decimal, "svc":Decimal, "zinr":string, "depo_billjour":bool, "resno_billjour":int, "wabkurz":string}, {"wabkurz": " "})
    ...
    def generate_output():
        nonlocal err_flag, avail_master, avail_bill, reslinnr, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_list, t_res_line_list, t_guest_list, t_list_list, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, serv1, vat1, vat3, fact1, netto, bill_receiver, currency, local_currency, frate, waehrungsnr, price_decimal, tot_deposit, t_char, t_resnr, reservation, res_line, guest, bill, htparam, master, exrate, waehrung, bill_line, artikel, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, billjournal
    ...
    def read_proforma_inv(resnr:int, rechnr:int):
        ...
        if rechnr > 0:
            ...
            for t_list in query(t_list_list, filters=(lambda t_list: t_list.depo_billjour), sort_by=[("nr",False)]):
                ...
                if billjournal:
                    ...
                    if (t_list.betrag + billjournal.betrag) == 0:
                        ...
                        for buff_t in query(buff_t_list, filters=(lambda buff_t: buff_t.depo_billjour  and buff_t.NR <= t_list.nr), sort_by=[("nr",False)]):
                            buff_t_list.remove(buff_t)
                        tot_deposit =  to_decimal("0")
    ...
    def read_proforma_inv1(resnr:int, reslinnr:int, rechnr:int):
        ...
        nonlocal err_flag, avail_master, avail_bill, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_list, t_res_line_list, t_guest_list, t_list_list, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, serv1, vat1, vat3, fact1, netto, bill_receiver, currency, local_currency, frate, waehrungsnr, price_decimal, tot_deposit, t_char, t_resnr, reservation, res_line, guest, bill, htparam, master, exrate, waehrung, bill_line, artikel, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, billjournal
        nonlocal curr_resnr, arl_list_reslinnr, t_active_flag, printtype, split_bill
        nonlocal rline, mbill, mainres, b_bill
    ...
    if printtype == 2 or printtype == 3:
        ...
        if currency.lower()  != (local_currency).lower() :
            ...
            for t_list in query(t_list_list, filters=(lambda t_list: t_list.trim(t_list.wabkurz.lower() ) == "" or t_list.wabkurz.lower()  == (local_currency).lower())):
            ...
            for t_list in query(t_list_list, filters=(lambda t_list: t_list.trim(t_list.wabkurz) == "")):
