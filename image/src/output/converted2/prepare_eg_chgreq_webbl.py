...
def prepare_eg_chgreq_webbl(pvilanguage:int, reqno:int, view_first:bool, user_init:string):

    prepare_cache ([Eg_property, Eg_subtask, Res_line, Guest, Htparam, Bediener, Eg_vendor, Eg_alert, Eg_staff, Eg_reqdetail])
    ...
    t_zimmer = t_queasy = t_queasy130 = t_eg_property = t_eg_request = treqdetail = svendor = rbuff = request1 = send_alert = tvendor = t_eg_location = subtask1 = resline1 = guest1 = None
    ...
    treqdetail_list, Treqdetail = create_model("Treqdetail", {"reqnr":int, "actionnr":int, "action":string, "create_date":date, "create_time":int, "create_str":string, "create_by":string, "flag":bool})
    svendor_list, Svendor = create_model("Svendor", {"nr":string, "vendor_nr":int, "docno":string, "reqnr":int, "startdate":date, "estfinishdate":date, "finishdate":date, "price":Decimal, "bezeich":string, "pic":string, "created_by":string, "created_date":date, "created_time":int, "perform_nr":string, "stat":string})
    ...
    tvendor_list, Tvendor = create_model("Tvendor", {"ven_nr":int, "ven_nm":string})
    ...
    def generate_output():
        ...
        nonlocal rbuff, subtask1, resline1, guest1


        nonlocal t_zimmer, t_queasy, t_queasy130, t_eg_property, t_eg_request, treqdetail, svendor, rbuff, request1, send_alert, tvendor, t_eg_location, subtask1, resline1, guest1
    ...
    def create_request():
        ...
        if not rbuff:
            ...
            gbuff = get_cache (Guest, {"gastnr": [(eq, request1.gastnr)]})
    ...
    for queasy in db_session.query(Queasy).filter(
             (Queasy.key == 132) | (Queasy.key == 19) | (Queasy.key == 135)).order_by(Queasy._recid).all():
        t_queasy = T_queasy()
