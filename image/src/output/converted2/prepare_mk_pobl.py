...
def prepare_mk_pobl(docu_nr:string, pvilanguage:int, lief_nr:int, pr_deptnr:int, po_type:int, potype:int, bediener_username:string, ordername_screen_value:string, crterm:int):
    ...
    disc_list = t_parameters = t_waehrung = t_l_order = t_l_orderhdr = None
    ...
    def create_disclist():
        ...
        for l_order in db_session.query(L_order).filter(
                 (L_order.docu_nr == (docu_nr).lower()) & (L_order.pos > 0) & (L_order.loeschflag == 0) & (L_order.lief_nr == lief_nr)).order_by(L_order._recid).all():
            pos = pos + 1
            disc_list = Disc_list()
