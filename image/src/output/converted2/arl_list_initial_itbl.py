...
def arl_list_initial_itbl(inp_resname:str, user_init:str):
    ...
    guest = db_session.query(Guest).filter(
             (Guest.karteityp >= 1) & (Guest.gastnr > 0) & (Guest.firmen_nr > 0) & (Guest.steuernr != "")).first()
    while None != guest:
        t_guest = T_guest()
        ...
        guest = db_session.query(Guest).filter(
                 (Guest.karteityp >= 1) & (Guest.gastnr > 0) & (Guest.firmen_nr > 0) & (Guest.steuernr != "")).filter(Guest._recid > curr_recid).first()
