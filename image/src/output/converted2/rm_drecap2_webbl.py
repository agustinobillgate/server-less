...
def rm_drecap2_webbl(pvilanguage:int, opening_date:date, from_date:date, to_date:date, fdate:date, tdate:date, segmtype_exist:bool, mi_mtd_chk:bool, mi_ftd_chk:bool, mi_exchu_chk:bool, mi_exccomp_chk:bool, long_digit:bool):
    ...
    cl_list_list, Cl_list = create_model("Cl_list", {"segm":int, "betriebsnr":int, "compli":bool, "bezeich":string, "droom":int, "proz1":Decimal, "mroom":int, "proz2":Decimal, "dpax":int, "mpax":int, "drate":Decimal, "mrate":Decimal, "drev":Decimal, "mrev":Decimal, "yroom":int, "proz3":Decimal, "ypax":int, "yrate":Decimal, "yrev":Decimal, "zero_flag":bool})
    ...
    def generate_output():
        ...
        nonlocal cl_list_list, om_list_list
        ...
        for datum in date_range(from_date,to_date) :
            ...
            if datum >= ci_date:
                ...
                for outorder, zimmer in db_session.query(Outorder, Zimmer).join(Zimmer,(Zimmer.zinr == Outorder.zinr)).filter(
                    ...
                    om_list = query(om_list_list, filters=(lambda om_list: om_list.zinr == outorder.zinr and om_list.gespstart == outorder.gespstart and om_list.gespende == outorder.gespende), first=True)
    ...
    for cl_list in query(cl_list_list, filters=(lambda cl_list: cl_list.bezeich.lower()  == ("Deleted").lower())):
