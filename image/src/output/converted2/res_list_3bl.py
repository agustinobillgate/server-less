...
from models import Htparam, Zimkateg, Res_line, Reslin_queasy, Guest, Reservation, Bediener, Segment, Sourccod, Guestseg

def res_list_3bl(case_type:int, from_date:date, to_date:date, resnr:int, gastnr:int, search_resno:int, search_voucher:string):

    prepare_cache ([Htparam, Zimkateg, Res_line, Reslin_queasy, Guest, Reservation, Bediener, Segment, Sourccod, Guestseg])

    curr_select:string = ""
    res_listmain_list = []
    res_listmember_list = []
    vip_nr:List[int] = create_empty_list(10,0)
    htparam = zimkateg = res_line = reslin_queasy = guest = reservation = bediener = segment = sourccod = guestseg = None

    res_listmain = res_listmember = revenue_list = None

    res_listmain_list, Res_listmain = create_model("Res_listmain", {"resdat":date, "resnr":int, "name":string, "groupname":string, "depositgef":Decimal, "limitdate":date, "depositbez":Decimal, "zahldatum":date, "depositbez2":Decimal, "zahldatum2":date, "useridanlage":string, "mutdat":date, "useridmutat":string, "gastnr":int, "bemerk":string, "grpflag":bool, "activeflag":int, "resname":string, "address":string, "city":string, "sob":string})
    res_listmember_list, Res_listmember = create_model("Res_listmember", {"name":string, "ankunft":date, "abreise":date, "zinr":string, "kurzbez":string, "zipreis":Decimal, "arrangement":string, "erwachs":int, "gratis":int, "zimmeranz":int, "anztage":int, "changed_id":string, "changed":date, "gastnr":int, "resstatus":int, "zikatnr":int, "resnr":int, "bemerk":string, "active_flag":int, "l_zuordnung":[int,5], "reslinnr":int, "res_recid":int, "vip":string, "nat":string, "rate_code":string, "segment":string, "sp_req":string, "usr_id":string, "ratecode":string, "voucher_no":string, "child":int, "email":string, "phone_no":string, "sob":string, "excp_rev":string, "tot_rev":Decimal, "created_by":string})
    revenue_list_list, Revenue_list = create_model("Revenue_list", {"flag":int, "str":string, "str1":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal curr_select, res_listmain_list, res_listmember_list, vip_nr, htparam, zimkateg, res_line, reslin_queasy, guest, reservation, bediener, segment, sourccod, guestseg
        nonlocal case_type, from_date, to_date, resnr, gastnr, search_resno, search_voucher


        nonlocal res_listmain, res_listmember, revenue_list
        nonlocal res_listmain_list, res_listmember_list, revenue_list_list
    ...
    def calc_br1():
        ...
        res_line = Res_line()
        zimkateg = Zimkateg()
        for res_line.resstatus, res_line.zimmeranz, res_line.abreise, res_line.ankunft, res_line.resnr, res_line.reslinnr, res_line.name, res_line.zinr, res_line.zipreis, res_line.arrangement, res_line.erwachs, res_line.gratis, res_line.changed_id, res_line.changed, res_line.gastnr, res_line.l_zuordnung, res_line.zikatnr, res_line.bemerk, res_line.active_flag, res_line._recid, res_line.kind1, res_line.gastnrmember, res_line.zimmer_wunsch, res_line.anztage, res_line.zimmerfix, zimkateg.kurzbez, zimkateg._recid in db_session.query(Res_line.resstatus, Res_line.zimmeranz, Res_line.abreise, Res_line.ankunft, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.zinr, Res_line.zipreis, Res_line.arrangement, Res_line.erwachs, Res_line.gratis, Res_line.changed_id, Res_line.changed, Res_line.gastnr, Res_line.l_zuordnung, Res_line.zikatnr, Res_line.bemerk, Res_line.active_flag, Res_line._recid, Res_line.kind1, Res_line.gastnrmember, Res_line.zimmer_wunsch, Res_line.anztage, Res_line.zimmerfix, Zimkateg.kurzbez, Zimkateg._recid).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 (Res_line.resnr == resnr) & (Res_line.resstatus != 12)).order_by(Res_line.zinr, Res_line.ankunft, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, func.substring(Res_line.name, 0, 32)).all():
            ...
            if not res_line.zimmerfix and not res_line.resstatus == 11 and not res_line.resstatus == 13:
                qty = res_line.zimmeranz
            ...
            res_listmember.zimmeranz = qty
            res_listmember.anztage = rmnite
            ...
            if reservation:
                ...
                bediener = get_cache (Bediener, {"userinit": [(eq, reservation.useridanlage)]})

                if bediener:
                    res_listmember.created_by = bediener.username


                else:
                    res_listmember.created_by = "User Not Found"


            else:
                res_listmember.created_by = "Reservation Not Found"
            ...
            if guest:
                ...
                revenue_list_list = get_output(view_staycostbl(0, res_listmember.resnr, res_listmember.reslinnr, ""))

                revenue_list = query(revenue_list_list, last=True)

                if revenue_list:
                    res_listmember.excp_rev = to_string(trim(entry(1, revenue_list.str, "=")))
                    res_listmember.tot_rev =  to_decimal(to_decimal(res_listmember.excp_rev)) * to_decimal(res_line.zimmeranz)
