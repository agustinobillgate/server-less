...
def ts_tbplan_check_table_webbl(pvilanguage:int, resnr:int, reslinnr:int, tischnr:int, curr_waiter:int, tkellner_masterkey:bool, dept_no:int, t_h_bill_list:[T_h_bill], t_tisch_list:[T_tisch]):
    ...
    def generate_output():
        ...
        nonlocal t_queasy33_list, t_queasy31_list
        ...
        if not p_1342:
            ...
            if not tkellner_masterkey:
                ...
                if t_h_bill and t_h_bill.kellner_nr != curr_waiter:
                    ...
                    t_tisch = query(t_tisch_list, filters=(lambda t_tisch: t_tisch.tischnr == tischnr), first=True)
