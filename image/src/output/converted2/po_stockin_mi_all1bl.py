...
def po_stockin_mi_all1bl(user_init:string, l_order_recid:int, l_orderhdr_recid:int, docu_nr:string, exchg_rate:Decimal, price_decimal:int, billdate:date, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, curr_lager:int, lief_nr:int, lscheinnr:string, t_amount:Decimal):
    ...
    t_l_order_list, T_l_order = create_model_like(L_order, {"rec_id":int, "art_bezeich":string, "jahrgang":int, "alkoholgrad":Decimal})
    ...
    def generate_output():
        ...
        return {"t_amount": t_amount, "created": created, "s-list": s_list_list, "t-l-order": t_l_order_list}
    ...
    def create_purchase_book():
        ...
        nonlocal s_list_list, t_l_order_list, s_artnr, qty, s_qty, price, curr_disc, curr_vat, curr_disc2, curr_vat1, epreis, amount, l_order, bediener, l_orderhdr, l_artikel, l_bestand, l_liefumsatz, l_op, queasy, l_ophdr, l_pprice, htparam
        nonlocal user_init, l_order_recid, l_orderhdr_recid, docu_nr, exchg_rate, price_decimal, billdate, f_endkum, b_endkum, m_endkum, fb_closedate, m_closedate, curr_lager, lief_nr, lscheinnr, t_amount
        nonlocal l_od
