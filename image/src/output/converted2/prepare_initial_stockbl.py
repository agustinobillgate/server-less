...
def prepare_initial_stockbl():
    ...
    temp_l_artikel_list, Temp_l_artikel = create_model("Temp_l_artikel", {"artnr":int, "bezeich":string, "ek_aktuell":Decimal, "masseinheit":string, "inhalt":Decimal})
    t_l_lager_list, T_l_lager = create_model_like(L_lager)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal m_endkum, fb_date, m_date, t_l_lager_list, temp_l_artikel_list, l_lager, htparam, l_artikel


        nonlocal temp_l_artikel, t_l_lager
        nonlocal temp_l_artikel_list, t_l_lager_list

        return {"m_endkum": m_endkum, "fb_date": fb_date, "m_date": m_date, "t-l-lager": t_l_lager_list, "temp-l-artikel": temp_l_artikel_list}
