...
def close_inventory_step1bl(pvilanguage:int, closedate:date, inv_type:int, m_endkum:int):
    ...
    while None != l_bestand:
        ...
        if not l_artikel:
            ...
            l_onhand_list.remove(l_onhand)
            pass

        curr_recid = l_bestand._recid
        l_bestand = db_session.query(L_bestand).filter(
                 ((L_bestand.anf_best_dat <= closedate)) | ((L_bestand.anf_best_dat >= closedate)) | ((L_bestand.anf_best_dat == None))).filter(L_bestand._recid > curr_recid).first()
