...
op_list_list, Op_list = create_model_like(L_op, {"a_bezeich":string, "vat_no":int, "vat_value":Decimal, "disc_amount":Decimal, "disc_amount2":Decimal, "tax_percent":Decimal, "discamt_flag":bool, "addvat_amount":Decimal})

def s_stockiss_btn_go_1bl(op_list_list:[Op_list], f_endkum:int, b_endkum:int, m_endkum:int, lscheinnr:string, billdate:date, curr_lager:int, jobnr:int, lief_nr:int, fb_closedate:date, m_closedate:date, user_init:string, t_amount:Decimal):
    ...
    def generate_output():
        ...
        return {"op-list": op_list_list, "created": created, "s_artnr": s_artnr, "qty": qty, "price": price, "cost_acct": cost_acct, "err_code": err_code}
