...
g_list_list, G_list = create_model("G_list", {"nr":int, "jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "duplicate":bool}, {"sysdate": get_current_date(), "chgdate": None, "duplicate": True})

def fa_upgrade_btn_gobl(g_list_list:[G_list], p_nr:int, nr:int, amt:Decimal, user_init:string, qty:int, refno:string, datum:date, bezeich:string, debits:Decimal, credits:Decimal, remains:Decimal):
