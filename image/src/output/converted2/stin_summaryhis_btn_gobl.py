...
def stin_summaryhis_btn_gobl(sorttype:int, from_lager:int, to_lager:int, from_date:date, to_date:date, from_art:int, to_art:int, from_grp:int, to_grp:int):
    ...
    tt_list_list, Tt_list = create_model("Tt_list", {"flag":int, "subgroup":int, "subbez":string, "f_lager":int, "f_bezeich":string, "artnr":string, "bezeich":string, "einheit":string, "qty":Decimal, "val":Decimal, "t_qty":Decimal, "t_val":Decimal, "grand_flag":bool})
    t_list_list, T_list = create_model("T_list", {"flag":int, "subgroup":int, "subbez":string, "f_lager":int, "f_bezeich":string, "artnr":string, "bezeich":string, "einheit":string, "qty":Decimal, "val":Decimal, "t_qty":Decimal, "t_val":Decimal, "grand_flag":bool})
    big_total_list, Big_total = create_model("Big_total", {"total":Decimal, "lager_nr":int, "f_bezeich":string})
    temp_list_list, Temp_list = create_model_like(Tt_list)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_list_list, l_lager, l_artikel, l_untergrup, l_ophis
        ...
        nonlocal tt_list_list, t_list_list, big_total_list, temp_list_list
    ...
    def create_list1():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            for l_artikel in db_session.query(L_artikel).filter(
                     ((L_artikel.artnr.in_(list(set([l_ophis.artnr for l_ophis in l_ophis_list if l_ophishis.lager_nr == l_lager.lager_nr] & (l_ophis.datum >= from_date) & (l_ophis.datum <= to_date) & (l_ophis.artnr >= from_art) & (l_ophis.artnr <= to_art) & (l_ophis.anzahl != 0) & (l_ophis.op_art == 1))))) & (L_artikel.zwkum >= from_grp) & (L_artikel.zwkum <= to_grp))).order_by(L_artikel.artnr).all():
    ...
    def create_list4():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            for l_ophis.datum, l_ophis.artnr, l_ophis.anzahl, l_ophis.op_art, l_ophis.warenwert, l_ophis._recid, l_artikel.artnr, l_artikel.bezeich, l_artikel.masseinheit, l_artikel.zwkum, l_artikel._recid in db_session.query(L_ophis.datum, L_ophis.artnr, L_ophis.anzahl, L_ophis.op_art, L_ophis.warenwert, L_ophis._recid, L_artikel.artnr, L_artikel.bezeich, L_artikel.masseinheit, L_artikel.zwkum, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_ophis.artnr) & (L_artikel.zwkum >= from_grp) & (L_artikel.zwkum <= to_grp)).filter(
                ...
                tt_list = query(tt_list_list, filters=(lambda tt_list: tt_list.f_lager == l_lager.lager_nr and to_int(tt_list.artnr) == l_artikel.artnr), first=True)
    ...
    def create_output():
        ...
        for tt_list in query(tt_list_list):
            temp_list = Temp_list()
            temp_list_list.append(temp_list)

            buffer_copy(tt_list, temp_list)

        for tt_list in query(tt_list_list, filters=(lambda tt_list: tt_list.bezeich.lower()  == ("TOTAL").lower()  and tt_list.f_lager != 9999), sort_by=[("t_val",True)]):

            temp_list = query(temp_list_list, filters=(lambda temp_list: temp_list.f_lager == tt_list.f_lager and temp_list.f_bezeich != ""), first=True)
            ...
            for temp_list in query(temp_list_list, filters=(lambda temp_list: temp_list.f_lager == tt_list.f_lager and temp_list.bezeich.lower()  != ("TOTAL").lower()), sort_by=[("t_val",True)]):
                t_list = T_list()
                t_list_list.append(t_list)
            ...
            temp_list = query(temp_list_list, filters=(lambda temp_list: temp_list.f_lager == t_list.f_lager and temp_list.bezeich.lower()  == ("TOTAL").lower()), first=True)
        ...
        temp_list = query(temp_list_list, filters=(lambda temp_list: temp_list.f_lager == 9999), first=True)
