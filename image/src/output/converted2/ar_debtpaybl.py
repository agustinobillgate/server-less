...
def ar_debtpaybl(artnr:int, userinit:str, bill_nr:int, temp_art2:int, bill_date:date, bill_name:str, to_name:str, bill_saldo:decimal, art_selected:int, foutstand:decimal, outstand:decimal):
    ...
    def create_age_list():
        ...
        if bill_nr != 0:
            ...
            for debitor in db_session.query(Debitor).filter(
                         (Debitor.artnr == temp_art2) & (func.lower((func.lower(Debitor.name)).op("~")((("*" + bill_name + "*").lower().replace("*",".*")))) & (func.lower(Debitor.name) <= (to_name).lower())) & (Debitor.rgdatum <= bill_date) & (Debitor.opart < 2)).order_by(Debitor.zahlkonto, Debitor.debref, Debitor.rgdatum).all():
                ...
                if not age_list or debitor.counter == 0:
                    ...
                    if (re.match(r".*;.*",debitor.vesrcod, re.IGNORECASE)):
            ...
            for debitor in db_session.query(Debitor).filter(
                         (Debitor.artnr == temp_art2) & (func.lower(Debitor.name).op("~")(((bill_name).lower().replace("*",".*")))) & (Debitor.rgdatum <= bill_date) & (Debitor.opart < 2)).order_by(Debitor.zahlkonto, Debitor.debref, Debitor.rgdatum).all():
    ...
    for debthis in db_session.query(Debt).filter(
        ...
        if debthis._recid in debt_obj_list:
            continue
        else:
            debt_obj_list.append(debthis._recid)
