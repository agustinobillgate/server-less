...
def deposit_list2_webbl(fdate:date, sortype:int):
    ...
    depo_list_list, Depo_list = create_model("Depo_list", {"resnr":int, "reserve_name":string, "grpname":string, "guestname":string, "ankunft":date, "deposit_type":int, "depositgef":Decimal, "bal":Decimal, "depo1":Decimal, "datum1":date, "depo2":Decimal, "datum2":date, "deposit_type2":int, "deposit_paid":Decimal})
    out_list_list, Out_list = create_model("Out_list", {"deposit":Decimal, "deposit2":Decimal, "datum":date, "datum2":date, "resno":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal depo_list_list, total_saldo, depo_foreign, depo_curr, depo_artnr, str_resnr, resnr, bill_date, exchg_rate, found_it, resno, tot_depo1, tot_depo2, tot, curr_name, curr_date, subtotal, str_resnr2, str_resnr3, grpstr, htparam, artikel, billjournal, res_line, reservation
        nonlocal fdate, sortype


        nonlocal depo_list, out_list
        nonlocal depo_list_list, out_list_list

        return {"depo-list": depo_list_list, "total_saldo": total_saldo}
    ...
    def create_depo1():
        ...
        for billjournal in db_session.query(Billjournal).filter(
            ...
            out_list = query(out_list_list, filters=(lambda out_list: out_list.resno == resno), first=True)
        ...
        for res_line, reservation in db_session.query(Res_line, Reservation).join(Reservation,(Reservation.resnr == out_list.resno)).filter(
                 ((Res_line.resnr.in_(list(set([out_list.resno for out_list in out_list_list])))))).order_by(Res_line._recid).all():
            if res_line_obj_list.get(res_line._recid):
                continue
            else:
                res_line_obj_list[res_line._recid] = True

            out_list = query(out_list_list, (lambda out_list: (res_line.resnr == out_list.resno)), first=True)

            if out_list.deposit2 >= 0:
                depo_list = Depo_list()
                depo_list_list.append(depo_list)

                depo_list.resnr = reservation.resnr depo_list.reserve_name == reservation.name depo_list.grpname == reservation.groupname depo_list.guestname == res_line.name depo_list.ankunft == res_line.ankunft depo_list.deposit_type == reservation.deposit_type depo_list.depositgef == reservation.depositgef depo_list.depo1 == out_list.deposit depo_list.datum1 == out_list.datum depo_list.depo2 == out_list.deposit2 depo_list.datum2 == out_list.datum2 depo_list.deposit_type2 == reservation.deposit_type depo_list.deposit_paid == out_list.deposit + out_list.deposit2
        ...
        for res_line, reservation in db_session.query(Res_line, Reservation).join(Reservation,(Reservation.resnr == out_list.resno)).filter(
            ...
            if out_list.deposit2 >= 0:
                ...
                if curr_name != "" and curr_name != reservation.name:
                    ...
                    depo_list.resnr = None depo_list.reserve_name == "" depo_list.grpname == "TOTAL" depo_list.guestname == "" depo_list.ankunft == None depo_list.deposit_type == None depo_list.depositgef == subtotal depo_list.depo1 == None depo_list.datum1 == None depo_list.depo2 == None depo_list.datum2 == None depo_list.deposit_type2 == None depo_list.deposit_paid == None
