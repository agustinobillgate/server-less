...
from models import Guest, Paramtext, Htparam, Res_line, Queasy, Waehrung, Reservation, Zimkateg, Master, Messages, Kontline, Gentable, Mc_guest, Reslin_queasy, Zimmer
...
def precheckin_generator_loaddata_1bl(show_rate:bool, last_sort:int, lresnr:int, long_stay:int, ci_date:date, grpflag:bool, room:string, lname:string, sorttype:int, fdate1:date, fdate2:date, fdate:date, segm1_list_list:[Segm1_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list]):
    ...
    def generate_output():
        nonlocal rmlen, mail_eng, mail_oth, hotel_name, hotel_telp, hotel_mail, arl_list_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, stay, en_hotelencrip, oth_hotelencrip, cpersonalkey, rkey, mmemptrout, precheckinurl, hotelcode, hotelcode_ok, guest, paramtext, htparam, res_line, queasy, waehrung, reservation, zimkateg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer
        nonlocal show_rate, last_sort, lresnr, long_stay, ci_date, grpflag, room, lname, sorttype, fdate1, fdate2, fdate, segm1_list_list
        nonlocal gbuff, gmember


        nonlocal setup_list, gbuff, arl_list, segm1_list, argt_list, zikat_list, gmember
        nonlocal setup_list_list, arl_list_list

        return {"fdate1": fdate1, "fdate2": fdate2, "rmlen": rmlen, "mail_eng": mail_eng, "mail_oth": mail_oth, "hotel_name": hotel_name, "hotel_telp": hotel_telp, "hotel_mail": hotel_mail, "arl-list": arl_list_list}

    def get_toname(lname:string):

        nonlocal rmlen, mail_eng, mail_oth, hotel_name, hotel_telp, hotel_mail, arl_list_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, stay, en_hotelencrip, oth_hotelencrip, cpersonalkey, rkey, mmemptrout, precheckinurl, hotelcode, hotelcode_ok, guest, paramtext, htparam, res_line, queasy, waehrung, reservation, zimkateg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer
        nonlocal show_rate, last_sort, lresnr, long_stay, ci_date, grpflag, room, sorttype, fdate1, fdate2, fdate, segm1_list_list
        nonlocal gbuff, gmember
        ...
        if sorttype == 1:
            ...
            if last_sort == 1:
                ...
                if room != "":
                    ...
                    if lname == "":
                        ...
                        res_line_obj_list = {}
                        for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                        ...
                        res_line_obj_list = {}
                        for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                                 (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (Res_line.name >= (lname).lower()) & (Res_line.name <= (to_name).lower()) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date)).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                            setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                            if not setup_list:
                                continue
                    ...
                    if lname == "":
                        ...
                        res_line_obj_list = {}
                        for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                                 (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.name >= (lname).lower()) & (Res_line.name <= (to_name).lower()) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date)).order_by(Res_line.name).all():
                            setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                            if not setup_list:
                                continue
            ...
            if last_sort == 1:
                ...
                if lname == "":
                    ...
                    res_line_obj_list = {}
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft == ci_date) & (Res_line.name >= (lname).lower()) & (Res_line.name <= (to_name).lower()) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Res_line.name, Res_line.zinr).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
            ...
            if last_sort == 1:
                ...
                if lname == "":
                    ...
                    res_line_obj_list = {}
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == fdate) & (Res_line.name >= (lname).lower()) & (Res_line.name <= (to_name).lower()) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Res_line.name, Res_line.zinr).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                    ...
                    segm1_list = query(segm1_list_list, (lambda segm1_list: segm1_list.segm == reservation.segmentcode and segm1_list.selected), first=True)
                    if not segm1_list:
                        continue

                    argt_list = query(argt_list_list, (lambda argt_list: argt_list.argt == res_line.arrangement and argt_list.selected), first=True)
                    if not argt_list:
                        continue
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft >= fdate1) & (Res_line.ankunft <= fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (Res_line.name >= (lname).lower()) & (Res_line.name <= (to_name).lower())).order_by(Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            if lname == "":
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft >= fdate1) & (Res_line.ankunft <= fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (Res_line.resname >= (lname).lower()) & (Res_line.resname <= (to_name).lower())).order_by(Reservation.name, Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
    ...
    def sqry2():
        ...
        if sorttype == 2:
            ...
            if lname == "":
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (Reservation.name >= (lname).lower()) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (Reservation.name >= (lname).lower()) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date) & (Res_line.resname >= (lname).lower()) & (Res_line.resname <= (to_name).lower())).order_by(Reservation.name, Reservation.groupname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date) & (matches(Res_line.resname,(lname)))).order_by(Reservation.name, Reservation.groupname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            if lname == "":
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 0) & (Res_line.ankunft == ci_date) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (matches(Res_line.resname,(lname))) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Reservation.name, Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 0) & (Res_line.ankunft == ci_date) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (Res_line.resname >= (lname).lower()) & (Res_line.resname <= (to_name).lower()) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Reservation.name, Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            if lname == "":
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == fdate) & (matches(Res_line.resname,(lname))) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Reservation.name, Reservation.groupname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == fdate) & (Res_line.resname >= (lname).lower()) & (Res_line.resname <= (to_name).lower()) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Reservation.name, Reservation.groupname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
    ...
    def sqry5():
        ...
        if sorttype == 1:
            ...
            if fdate != None:
                ...
                res_line_obj_list = {}
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (length(Reservation.groupname) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.ankunft >= fdate) & (Res_line.abreise >= ci_date) & (Res_line.zinr >= (room).lower())).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
    ...
    def create_it():
        ...
        arl_list.segmentcode = segm1_list.segm
        ...
        arl_list.arrangement = argt_list.argt
        ...
        arl_list.phonenumber = gmember.mobil_tel
        arl_list.email_address = gmember.email_adr
        ...
        if not res_line:
            ...
            if res_line.active_flag == 1 and res_mode.lower()  == ("modify").lower() :
                ...
                for res_line in db_session.query(Res_line).filter(
                         (Res_line.resnr == curr_resnr) & (Res_line.active_flag <= 1)).order_by(Res_line._recid).yield_per(100):
    ...
    if not hotelcode_ok:
        ...
        for arl_list in query(arl_list_list):
            ...
            arl_list.link_pci_eng = precheckinurl + "?" + "hc=" + hotelcode + "&ec=" + en_hotelencrip
            ...
            arl_list.link_pci_oth = precheckinurl + "?" + "hc=" + hotelcode + "&ec=" + oth_hotelencrip
