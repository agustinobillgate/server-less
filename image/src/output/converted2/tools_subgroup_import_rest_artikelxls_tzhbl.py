...
def tools_subgroup_import_rest_artikelxls_tzhbl(rest_article_list_list:[Rest_article_list], rest_maingroup_list_list:[Rest_maingroup_list], rest_subgroup_list_list:[Rest_subgroup_list]):
    ...
    def fix_subgroup():
        ...
        rest_subgroup_list = query(rest_subgroup_list_list, filters=(lambda rest_subgroup_list: rest_subgroup_list.dept_no == rest_article_list.dept_no and ((rest_subgroup_list.subgroup_desc == rest_article_list.sub_group_desc) or (rest_subgroup_list.new_subgroup_desc == rest_article_list.sub_group_desc))), first=True)
        ...
        for h_artikel in db_session.query(H_artikel).order_by(H_artikel._recid).all():
            ...
            b_article_list = query(b_article_list_list, filters=(lambda b_article_list: b_article_list.artnr == h_artikel.artnr and b_article_list.dept_no == h_artikel.departement and b_article_list.sub_group != h_artikel.zwkum), first=True)
