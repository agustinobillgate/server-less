...
def copy_bares_overlapping_checkbl(pvilanguage:int, resnr:int, reslinnr:int, datum:date, von_i:int, bis_i:int):
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, resnr, reslinnr, datum, von_i, bis_i
        nonlocal resline
    ...
    if bk_reser:
        ...
        resline = db_session.query(Resline).filter(
                 (Resline.datum == datum) & (Resline.raum == bk_reser.raum) & (((Resline.von_i >= von_i) & (Resline.von_i <= bis_i)) | ((Resline.bis_i >= von_i) & (Resline.bis_i <= bis_i)) | ((von_i >= Resline.von_i) & (bis_i <= Resline.bis_i))) & (Resline.resstatus == bk_reser.resstatus)).first()
