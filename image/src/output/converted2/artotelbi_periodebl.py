...
def artotelbi_periodebl(start_date:date, end_date:date):
    ...
    his_res_list, His_res = create_model("His_res", {"resnr":int, "reslinnr":int, "staydate":date, "ci_date":date, "co_date":date, "nbrofroom":int, "room_type_name":string, "rm_rev":Decimal, "fb_rev":Decimal, "other_rev":Decimal, "resstatus":int, "reserveid":int, "reservename":string, "bookdate":date, "booktime":string, "rate_code":string, "nationality":string, "segmentcode":string, "cancel_date":date, "zinr":string, "memozinr":string, "food_rev":Decimal, "beverage_rev":Decimal, "card_type":string, "company_code":string, "source_code":string, "adult":int, "child":int, "infant":int, "compliment":int, "compliment_ch":int, "age":string, "argtnr":int, "res_logi":bool, "rtc":string, "arrangement":string, "voucher_nr":string, "rm_rate":Decimal, "fixed_rate":bool, "currency":string, "guestname":string, "bill_receiver":string, "bill_instruction":string, "purpose":string, "grpname":string, "letterno":int, "localregion":string, "cancel_reason":string, "code":string, "early_booking":string, "bona_fide":string, "tot_tax":Decimal, "tot_svc":Decimal}, {"res_logi": True})
    temp_res_list, Temp_res = create_model_like(His_res)
    future_res_list, Future_res = create_model_like(His_res)
    rev_list_list, Rev_list = create_model("Rev_list", {"dept":int, "s_dept":string, "datum":date, "s_grp":string, "subgrp":int, "s_subgrp":string, "revart":int, "s_revart":string, "amount":Decimal})
    b1_list_list, B1_list = create_model("B1_list", {"datum":date, "zinr":string, "betriebsnr":int, "bezeich":string, "zimmeranz":int, "personen":int, "argtumsatz":Decimal, "logisumsatz":Decimal})
    budget_umsatz_list, Budget_umsatz = create_model("Budget_umsatz", {"datum":date, "department":int, "s_dept":string, "umsatzart":int, "s_umsatzart":string, "zwkum":Decimal, "budget":Decimal})
    ...
    def generate_output():
        ...
        nonlocal his_res_list, temp_res_list, future_res_list, rev_list_list, b1_list_list, budget_umsatz_list

        return {"htl_name": htl_name, "success_flag": success_flag, "his-res": his_res_list, "future-res": future_res_list, "rev-list": rev_list_list, "budget-umsatz": budget_umsatz_list, "b1-list": b1_list_list}
