...
def fo_inv_update_voucher_webbl(new_voucher:string, bline_recid:int, bline_desc:string, art_num:int, dept_num:int, bill_num:int, bill_date:date):
    ...
    bill_line = get_cache (Bill_line, {"_recid": [(eq, bline_recid)]})
    ...
    for billjournal in db_session.query(Billjournal).filter(
             (Billjournal.rechnr == bill_num) & (Billjournal.artnr == art_num) & (Billjournal.departement == dept_num) & (Billjournal.bill_datum == bill_date) & (Billjournal.zinr == (room_num).lower())).order_by(Billjournal._recid).yield_per(100):
    ...
    if art_type == 2 or art_type == 7:
        ...
        for debitor in db_session.query(Debitor).filter(
                 (Debitor.rechnr == bill_num) & (Debitor.rgdatum == bill_date) & (Debitor.artnr == art_num) & (Debitor.zinr == (room_num).lower())).order_by(Debitor._recid).yield_per(100):
