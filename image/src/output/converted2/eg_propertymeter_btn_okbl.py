...
def eg_propertymeter_btn_okbl(temp_rec_list:[Temp_rec], user_init:string, blframe:int, frd:date, tod:date, intres:int):
    eg_propmeter = None
    ...
    if blframe == 0:
        ...
        for eg_propmeter in db_session.query(Eg_propmeter).filter(
                 (Eg_propmeter.rec_date >= frd) & (Eg_propmeter.rec_date <= tod) & (Eg_propmeter.propertynr == intres)).order_by(Eg_propmeter._recid).all():
