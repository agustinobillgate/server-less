...
def nt_onlinetax_etrapbl(fdate:date, tdate:date):
    ...
    tlist_list, Tlist = create_model("Tlist", {"invno":string, "datum_trx":string, "subtotal":Decimal, "diskon":Decimal, "service":Decimal, "other":Decimal, "pajak":Decimal, "amount":Decimal, "depart":int})
    ...
    def generate_output():
        nonlocal hotel_name, hotel_id, tlist_list, bill_date, do_it, serv, vat, netto, serv_betrag, vat_betrag, serv_taxable, service_code, vat_proz, service_proz, count_int, datum, disc_art1, disc_art2, disc_art3, h_bill_line, queasy, htparam, paramtext, billjournal, artikel, hoteldpt, h_artikel
        nonlocal fdate, tdate
        nonlocal hbill_buff, bline, bqueasy, tqueasy


        nonlocal tlist, hbill_list, hbill_buff, bline, bqueasy, tqueasy
        nonlocal tlist_list, hbill_list_list

        return {"hotel_name": hotel_name, "hotel_id": hotel_id, "tlist": tlist_list}

    def datetime2char(datum:date, zeit:int):

        nonlocal hotel_name, hotel_id, tlist_list, bill_date, do_it, serv, vat, netto, serv_betrag, vat_betrag, serv_taxable, service_code, vat_proz, service_proz, count_int, disc_art1, disc_art2, disc_art3, h_bill_line, queasy, htparam, paramtext, billjournal, artikel, hoteldpt, h_artikel
        nonlocal fdate, tdate
        nonlocal hbill_buff, bline, bqueasy, tqueasy
    ...
    def step_1(bill_date:date):
        ...
        nonlocal hotel_name, hotel_id, tlist_list, do_it, serv, vat, netto, serv_betrag, vat_betrag, serv_taxable, service_code, vat_proz, service_proz, count_int, datum, disc_art1, disc_art2, disc_art3, h_bill_line, queasy, htparam, paramtext, billjournal, artikel, hoteldpt, h_artikel
        nonlocal fdate, tdate
        nonlocal hbill_buff, bline, bqueasy, tqueasy
        ...
        for billjournal in db_session.query(Billjournal).filter(
            ...
            if do_it:
                ...
                if netto != 0:
                    ...
                    tlist = query(tlist_list, filters=(lambda tlist: tlist.invno == to_string(billjournal.rechnr) and tlist.depart == billjournal.departement), first=True)
                ...
                if h_artikel.artart == 0 or h_artikel.artart == 8:
                    ...
                    if artikel:
                        ...
                        if netto != 0:
                            ...
                            tlist = query(tlist_list, filters=(lambda tlist: tlist.invno == to_string(h_bill_line.rechnr) and tlist.depart == h_bill_line.departement), first=True)
