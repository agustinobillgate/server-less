...
def argt_frate_fill_adhocbl(pvilanguage:int, adhoc_code:str, argtnr:int, resnr:int, reslinnr:int, from_date:date, to_date:date, adhoc_list:[Adhoc]):
    argt_list_list = []
    post_type:List[str] = create_empty_list(6,"")
    pers_type:List[str] = create_empty_list(3,"")
    lvcarea:str = "argt-frate"
    ...
    def create_adhoc_argt():
        ...
        reslin_queasy = db_session.query(Reslin_queasy).filter(
                 (func.lower(Reslin_queasy.key) == ("fargt-line").lower()) & (Reslin_queasy.char1 == "") & (Reslin_queasy.resnr == resnr) & (Reslin_queasy.number2 == argtnr) & (Reslin_queasy.reslinnr == reslinnr)).first()
        while None != reslin_queasy:
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                     (func.lower(Reslin_queasy.key) == ("fargt-line").lower()) & (Reslin_queasy.char1 == "") & (Reslin_queasy.resnr == resnr) & (Reslin_queasy.number2 == argtnr) & (Reslin_queasy.reslinnr == reslinnr)).filter(Reslin_queasy._recid > curr_recid).first()
    ...
    def create_argt_list():
        ...
        for reslin_queasy, argt_line, artikel in db_session.query(Reslin_queasy, Argt_line, Artikel).join(Argt_line,(Argt_line.argtnr == Reslin_queasy.number2) & (Argt_line.argt_artnr == Reslin_queasy.number3) & (Argt_line.departement == Reslin_queasy.number1)).join(Artikel,(Artikel.artnr == Argt_line.argt_artnr) & (Artikel.departement == Argt_line.departement)).filter(
                 (func.lower(Reslin_queasy.key) == ("fargt-line").lower()) & (Reslin_queasy.char1 == "") & (Reslin_queasy.resnr == resnr) & (Reslin_queasy.number2 == argtnr) & (Reslin_queasy.reslinnr == reslinnr)).order_by(Argt_line.argt_artnr, Reslin_queasy.date1).all():
