...
def allotm_reviewbl(pvilanguage:int, from_name:string, to_name:string, from_date:date, to_date:date, resflag:bool, gflag:bool, cflag:bool, detailflag:bool, curr_rmtype:string):
    ...
    k_list_list, K_list = create_model("K_list", {"gastnr":int, "bediener_nr":int, "kontcode":string, "global_flag":bool, "global_str":string, "ankunft":date, "zikatnr":int, "argt":string, "zimmeranz":[int,31], "erwachs":int, "kind1":int, "ruecktage":int, "overbooking":int, "abreise":date, "useridanlage":string, "resdate":date, "bemerk":string}, {"global_str": ""})
    res_list_list, Res_list = create_model("Res_list", {"resno":int, "reslinnr":int, "flag":string, "count":int, "s1":string})
    output_list_list, Output_list = create_model("Output_list", {"reihe":int, "resno":int, "reslinnr":int, "str":string})
