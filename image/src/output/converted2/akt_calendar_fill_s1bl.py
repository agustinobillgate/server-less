...
def akt_calendar_fill_s1bl(user_init:str, curr_month:int, curr_year:int, all_flag:bool):
    tt_s1_list = []
    tt_s2_list = []
    s1:List[str] = create_empty_list(42,"")
    s2:List[str] = create_empty_list(42,"")
    i:int = 0
    j:int = 0
    start_ind:int = 0
    anz_day:int = 0
    ...
    while j <= get_day(last_day) :
        ...
        if all_flag == False:
            ...
            for akt_line, akt_code in db_session.query(Akt_line, Akt_code).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).filter(
                     (Akt_line.flag == 0) & (func.lower(Akt_line.userinit) == (user_init).lower()) & (get_day(Akt_line.datum) == j) & (get_month(Akt_line.datum) == curr_month) & (get_year(Akt_line.datum) == curr_year)).order_by(Akt_line._recid).all():
