...
g_list_list, G_list = create_model("G_list", {"jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "duplicate":bool, "bemerk":string, "jou_recid":int, "b1_recid":int, "flag":int}, {"sysdate": get_current_date(), "chgdate": None, "duplicate": True})

def chg_gljourn_fill_gl_journal_vhpwebbl(jnr:int, user_init:string, t_bezeich:string, t_refno:string, g_list_list:[G_list]):
    ...
    def generate_output():
        nonlocal debits, credits, remains, b1_list_list, debitval, creditval, tmp_activeflag, gl_jouhdr, gl_journal, gl_acct
