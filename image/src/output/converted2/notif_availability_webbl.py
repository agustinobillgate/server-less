...
def notif_availability_webbl():
    ...
    room_summary_list, Room_summary = create_model("Room_summary", {"room_number":int, "room_type_name":string, "overbook":int, "availability":int, "ooo_rooms":int, "total_rooms":int, "max_avail":int, "date":string})
    room_type_list_list, Room_type_list = create_model("Room_type_list", {"room_number":int, "room_count":int, "is_sleeping_room":bool}, {"is_sleeping_room": True})
    room_availability_list_list, Room_availability_list = create_model("Room_availability_list", {"avail_flag":bool, "allot_flag":bool, "room_number":int, "type_index":int, "is_sleeping_room":bool, "allotment":[int,30], "descriptions":string, "available_rooms":[int,30], "coom":[string,30], "sort_priority":int}, {"is_sleeping_room": True})
    ...
    def generate_output():
        nonlocal room_summary_list, show_notif, start_date, end_date, i, check_date, process_room, billing_date, ooo_count_list, day_names, current_max_avail, show_all_data, total_room_count, current_type_number, queasy, htparam, zimmer, zimkateg, outorder, res_line, kontline
        nonlocal avail_config


        nonlocal room_summary, room_type_list, room_availability_list, avail_config
        nonlocal room_summary_list, room_type_list_list, room_availability_list_list

        return {"room-summary": room_summary_list, "show_notif": show_notif}
    ...
    while check_date < end_date and not show_all_data:
        room_type_list_list.clear()
        room_availability_list_list.clear()
        ...
        for zimmer in db_session.query(Zimmer).filter(
            ...
            if zimkateg.verfuegbarkeit:
                ...
                if current_type_number != zimkateg.zikatnr:
                    ...
                    room_type_list_list.append(room_type_list)
        ...
        for zimkateg in db_session.query(Zimkateg).filter(
            ...
            room_type_list = query(room_type_list_list, filters=(lambda room_type_list: room_type_list.room_number == zimkateg.zikatnr and room_type_list.is_sleeping_room), first=True)

            if room_type_list:
                room_availability_list = Room_availability_list()
                room_availability_list_list.append(room_availability_list)
        ...
        for outorder.gespstart, outorder.gespende, outorder._recid, zimmer.zikatnr, zimmer.sleeping, zimmer._recid in db_session.query(Outorder.gespstart, Outorder.gespende, Outorder._recid, Zimmer.zikatnr, Zimmer.sleeping, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Outorder.zinr) & (Zimmer.sleeping)).filter(
            ...
            if zimkateg.verfuegbarkeit:
                ...
                room_availability_list = query(room_availability_list_list, filters=(lambda room_availability_list: room_availability_list.room_number == zimmer.zikatnr), first=True)
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            room_availability_list = query(room_availability_list_list, filters=(lambda room_availability_list: room_availability_list.is_sleeping_room and room_availability_list.room_number == res_line.zikatnr), first=True)
        ...
        for kontline in db_session.query(Kontline).filter(
            ...
            room_availability_list = query(room_availability_list_list, filters=(lambda room_availability_list: room_availability_list.is_sleeping_room and room_availability_list.room_number == kontline.zikatnr), first=True)

            if room_availability_list:
                room_availability_list.available_rooms[0] = room_availability_list.available_rooms[0] - kontline.zimmeranz

        for room_availability_list in query(room_availability_list_list, filters=(lambda room_availability_list: room_availability_list.is_sleeping_room)):
            ...
            room_summary_list.append(room_summary)
