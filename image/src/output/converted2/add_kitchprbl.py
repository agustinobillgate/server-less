...
def add_kitchprbl(pvilanguage:int, session_parameter:string, dept:int, rechnr:int, billdate:date, user_init:string):
    ...
    def generate_output():
        ...
        nonlocal t_queasy_list, submenu_list_list, t_h_artikel_list
    ...
    def write_article():
        ...
        if t_h_artikel.betriebsnr > 0:
            ...
            for submenu_list in query(submenu_list_list, filters=(lambda submenu_list: submenu_list.nr == t_h_artikel.betriebsnr and submenu_list.zeit == h_bill_line.zeit)):
                created = True
    ...
    if sort_subgrp :
        ...
        for h_bill_line, wgrpdep in db_session.query(H_bill_line, Wgrpdep).join(Wgrpdep,(Wgrpdep.departement == dept) & (Wgrpdep.zknr == t_h_artikel.zwkum)).filter(
            ...
            t_h_artikel = query(t_h_artikel_list, (lambda t_h_artikel: t_h_artikel.artnr == h_bill_line.artnr and t_h_artikel.departement == dept and t_h_artikel.artart == 0), first=True)
            if not t_h_artikel:
                continue
