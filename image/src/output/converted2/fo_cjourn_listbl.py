...
def fo_cjourn_listbl(from_art:int, to_art:int, from_dept:int, to_dept:int, from_date:date, to_date:date, foreign_flag:bool, long_digit:bool):
    ...
    cjourn_list_list, Cjourn_list = create_model("Cjourn_list", {"artnr":int, "bezeich":string, "dept":string, "datum":date, "zinr":string, "rechnr":int, "canc_reason":string, "qty":int, "amount":Decimal, "zeit":string, "id":string})
    htl_list_list, Htl_list = create_model("Htl_list", {"num":int, "depart":string})
    art_list_list, Art_list = create_model("Art_list", {"artnr":int, "departement":int, "endkum":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal cjourn_list_list, htparam, artikel, hoteldpt, billjournal
        ...
        nonlocal output_list_list, cjourn_list_list, htl_list_list, art_list_list
    ...
    def journal_list():
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            htl_list = query(htl_list_list, filters=(lambda htl_list: htl_list.num == artikel.departement), first=True)
        ...
        for billjournal in db_session.query(Billjournal).filter(
            ...
            art_list = query(art_list_list, filters=(lambda art_list: art_list.artnr == billjournal.artnr and art_list.departement == billjournal.departement), first=True)
            ...
            if art_list:
                ...
                htl_list = query(htl_list_list, filters=(lambda htl_list: htl_list.num == billjournal.departement), first=True)
