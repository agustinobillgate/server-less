...
def prepare_update_restrictionbl():
    ...
    prepare_cache ([Queasy, Htparam, Zimkateg, Guest_pr, Guest])
    ...
    ota_list_list, Ota_list = create_model("Ota_list", {"bezeich":string})
    ...
    for guest in db_session.query(Guest).filter(
             (Guest.karteityp == 2) & (Guest.steuernr != "")).order_by(Guest._recid).all():
