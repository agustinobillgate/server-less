...
op_list_list, Op_list = create_model_like(L_op, {"bezeich":string, "username":string, "dml_code":string})

def dml_issue_btn_go_web_1bl(op_list_list:[Op_list], billdate:date, closedate:date, curr_lager:int, curr_dept:int, cost_acct:string, lief_nr:int, lscheinnr:string, user_init:string, t_amount:Decimal):
    ...
    def create_l_op():
        ...
        if curr_dept == 0:
            ...
            d_art1 = db_session.query(D_art1).filter(
                     (D_art1.artnr == s_artnr) & (D_art1.datum == billdate) & (D_art1.departement == curr_dept) & (num_entries(d_art.chginit, ";") > 1)).first()
            ...
            if d_art1:
                ...
                reslin_queasy = db_session.query(Reslin_queasy).filter(
                         (Reslin_queasy.key == ("DML").lower()) & (Reslin_queasy.date1 == billdate) & (to_int(entry(0, Reslin_queasy.char1, ";")) == s_artnr) & (to_int(entry(1, Reslin_queasy.char1, ";")) == curr_dept) & (num_entries(Reslin_queasy.char3, ";") > 1)).first()
