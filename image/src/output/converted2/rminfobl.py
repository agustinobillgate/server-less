...
def rminfobl(pvilanguage:int, rmno:string):
    ...
    room_list, Room = create_model_like(Zimmer, {"rmtype":string})
    inhouse_list, Inhouse = create_model("Inhouse", {"gname":string, "arrival":date, "depart":date, "adult":int, "child":int})
    arrival_list, Arrival = create_model("Arrival", {"gname":string, "arrival":date, "depart":date, "adult":int, "child":int, "resstatus":string})
