...
def ktrans_report_journal_listbl(sorttype:int, from_dept:int, to_dept:int, from_date:date, to_date:date):
    ...
    c_list_list, C_list = create_model("C_list", {"nr":int, "s_recid":int, "datum":date, "dept":int, "p_artnr":int, "dept1":string, "dept2":string, "artnr":int, "bezeich":string, "f_cost":Decimal, "b_cost":Decimal})
