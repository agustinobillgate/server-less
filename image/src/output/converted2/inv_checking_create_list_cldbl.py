...
from models import Htparam, L_untergrup, Gl_acct, L_artikel, L_besthis, L_bestand

def inv_checking_create_list_cldbl(invtype:int, d1:date):

    prepare_cache ([Htparam, Gl_acct, L_artikel, L_besthis, L_bestand])
    ...
    inv_date:date = None
    htparam = l_untergrup = gl_acct = l_artikel = l_besthis = l_bestand = None
    ...
    def generate_output():
        nonlocal saldo, s_list2_list, art_list2_list, mon, art1, art2, frnr, tonr, saldo1, saldo2, inv_date, htparam, l_untergrup, gl_acct, l_artikel, l_besthis, l_bestand
        nonlocal invtype, d1
    ...
    def create_listhis():
        ...
        l_artikel_obj_list = {}
        for l_artikel, l_untergrup, gl_acct in db_session.query(L_artikel, L_untergrup, Gl_acct).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).join(Gl_acct,(Gl_acct.fibukonto == L_untergrup.fibukonto)).filter(
