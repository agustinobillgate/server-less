...
def argt_frate_copybl(s_recid:int):
    ...
    for rline in db_session.query(Rline).filter(
        ...
        r_queasy = db_session.query(R_queasy).filter(
                     (func.lower(R_queasy.key) == ("fargt-line").lower()) & (R_queasy.number1 == reslin_queasy.number1) & (R_queasy.number2 == reslin_queasy.number2) & (R_queasy.number3 == reslin_queasy.number3) & (R_queasy.resnr == rline.resnr) & (R_queasy.reslinnr == rline.reslinnr)).first()
