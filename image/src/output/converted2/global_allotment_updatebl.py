...
def global_allotment_updatebl(gastno:int, inp_kontcode:str, g_list_list:[G_list]):
    kontcode_str = ""
    ...
    for kontline in db_session.query(Kontline).filter(
             (Kontline.gastnr == gastno) & (func.lower(Kontline.kontcode) == (inp_kontcode).lower())).order_by(Kontline._recid).all():
