...
def stock_translist1_btn_go_1bl(trans_code:string, m_grp:int, sorttype:int, m_str:int, mattype:int, from_art:int, to_art:int, from_date:date, to_date:date, show_price:bool, expense_amt:bool):
    ...
    t_list_list, T_list = create_model("T_list", {"datum":date, "lager_nr":int, "pos":int, "op_art":int, "lscheinnr":string, "f_bezeich":string, "t_bezeich":string, "artnr":string, "bezeich":string, "einheit":string, "content":Decimal, "price":string, "qty":Decimal, "val":Decimal, "id":string, "cat_bez":string, "main_bez":string, "subgrp_bez":string})
