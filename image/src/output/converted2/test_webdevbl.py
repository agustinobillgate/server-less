...
def test_webdevbl(from_date:date, to_date:date):
    ...
    outlet_transaction_list, Outlet_transaction = create_model("Outlet_transaction", {"trans_date":date, "department":int, "waiter_name":string, "table_no":int, "bill_no":int, "guest_name":string, "pax":int, "total_amount":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal outlet_transaction_list, curr_dept, waiter, h_bill, h_bill_line, kellner
        nonlocal from_date, to_date


        nonlocal outlet_transaction
        nonlocal outlet_transaction_list

        return {"outlet-transaction": outlet_transaction_list}
    ...
    for h_bill in db_session.query(H_bill).filter(
        ...
        if h_bill_line:
            ...
            outlet_transaction_list.append(outlet_transaction)
