...
def availability1bl(pvilanguage:int, printer_nr:int, call_from:int, txt_file:str, curr_date:date, incl_tentative:bool):
    msg_str = ""
    room_list_list = []
    sum_list_list = []
    lnldelimeter:str = ""
    ...
    csetup_array:List[str] = create_empty_list(99,"")
    ci_date:date = None
    datum:date = None
                    ...
                    if artikel:
                        ...
                        for tmp_resline in query(tmp_resline_list, sort_by=[("resnr",False)]):
                            ...
                            arrangement = db_session.query(Arrangement).filter(
                                     (Arrangement.arrangement == tmp_resline.arrangement)).first()
    ...
    def count_rmcateg():
        ...
        zimmer_obj_list = []
        for zimmer, zkbuff in db_session.query(Zimmer, Zkbuff).join(Zkbuff,(Zkbuff.zikatnr == Zimmer.zikatnr)).filter(
    ...
    def create_browse():
        ...
        for zkbuff in db_session.query(Zkbuff).filter(
            ...
            if zkbuff._recid in zkbuff_obj_list:
                continue
            else:
                zkbuff_obj_list.append(zkbuff._recid)
