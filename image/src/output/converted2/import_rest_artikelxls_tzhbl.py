...
def import_rest_artikelxls_tzhbl(rest_article_list_list:[Rest_article_list], rest_maingroup_list_list:[Rest_maingroup_list], rest_subgroup_list_list:[Rest_subgroup_list]):
    ...
    def cek_artikel():
        ...
        for b_article_list in query(b_article_list_list):

            b_maingroup_list = query(b_maingroup_list_list, filters=(lambda b_maingroup_list:(b_maingroup_list.maingroup_desc == b_article_list.main_group_desc) or (b_maingroup_list.new_maingroup_desc == b_article_list.main_group_desc)), first=True)
            ...
            b_subgroup_list = query(b_subgroup_list_list, filters=(lambda b_subgroup_list:(b_subgroup_list.subgroup_desc == b_article_list.sub_group_desc) or (b_subgroup_list.new_subgroup_desc == b_article_list.sub_group_desc)), first=True)
            ...
            rest_article_list = query(rest_article_list_list, filters=(lambda rest_article_list: rest_article_list.dept_no == b_article_list.dept_no and rest_article_list.artnr == b_article_list.artnr), first=True)
    ...
    def import_artikel():
        ...
        for h_artikel in db_session.query(H_artikel).order_by(H_artikel._recid).all():
            ...
            rest_article_list = query(rest_article_list_list, filters=(lambda rest_article_list: rest_article_list.artnr == h_artikel.artnr and rest_article_list.dept_no == h_artikel.departement), first=True)

            if rest_article_list:
                h_artikel.activeflag = rest_article_list.active_art

            rest_maingroup_list = query(rest_maingroup_list_list, filters=(lambda rest_maingroup_list: rest_maingroup_list.maingroup_no == h_artikel.endkum), first=True)

            if rest_maingroup_list:
                h_artikel.endkum = rest_maingroup_list.new_maingroup_no

            rest_subgroup_list = query(rest_subgroup_list_list, filters=(lambda rest_subgroup_list: rest_subgroup_list.subgroup_no == h_artikel.zwkum and rest_subgroup_list.dept_no == h_artikel.departement), first=True)
