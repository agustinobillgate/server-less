...
def hk_update_add_lostfoundbl(rec_id:int, zinr:string, item_description:string, from_date:date, zeit:string, dept:int, remark:string, claimby:string, claim_date:date, expired_date:date, reportedby:string, report_date:date, phoneno:string, refno:string, foundby:string, location:string, submitted:string, user_init:string):
    ...
    if s_list:
        s_list_list = get_output(hk_lostfound_chgbl(zinr, from_date, zeit, dept, item_description + chr_unicode(2) + remark, reportedby + chr_unicode(2) + claimby + chr_unicode(2) + claim_date_str + chr_unicode(2) + expired_str, report_date, phoneno, refno, foundby, location, submitted, user_init, s_list_list))
    else:
        s_list_list = get_output(hk_lostfound_addbl(zinr, from_date, zeit, dept, item_description + chr_unicode(2) + remark, reportedby + chr_unicode(2) + expired_str, report_date, phoneno, refno, foundby, location, submitted, user_init))
