...
def ghs_x1_forecastbl(datum:date, propid:string):
    ...
    def generate_output():
        nonlocal x_list_list, birthdate, str_rsv, contcode, loop_i, curr_i, rsv_date, to_date, datum1, flodging, lodging, breakfast, lunch, dinner, others, rmrate, net_vat, net_service, vat, service, t_rmrev, t_fbrev, t_others, ci_date, co_date, guest, reservation, segment, sourccod, zimkateg, queasy, res_line
        nonlocal datum, propid
        nonlocal gmember, gcomp


        nonlocal x_list, gmember, gcomp
        nonlocal x_list_list

        return {"x-list": x_list_list}
