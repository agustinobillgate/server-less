...
def rest_rsvplan2_paymentdetailbl(reservationid:int):
    ...
    payment_list_list, Payment_list = create_model("Payment_list", {"paymentdate":date, "invoicenumber":string, "depositamount":Decimal, "paymentamount":Decimal, "chgdate":date, "chgtime":string, "userinit":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal payment_list_list, queasy
        nonlocal reservationid


        nonlocal payment_list
        nonlocal payment_list_list

        return {"payment-list": payment_list_list}
