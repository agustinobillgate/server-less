...
def check_gledger_btn_show1bl(currdate:date):
    ...
    s1_list_list, S1_list = create_model("S1_list", {"flag":int, "nr":int, "artnr":int, "bezeich":string, "artart":int, "dept":int, "amt":Decimal, "ums":Decimal})
    ...
    def show_bill_vs_rev():
        ...
        for s1_list in query(s1_list_list, filters=(lambda s1_list: s1_list.flag == 0), sort_by=[("dept",False),("artnr",False)]):
            curr_i = curr_i + 1
