...
input_list_list, Input_list = create_model("Input_list", {"from_date":date, "to_date":date, "usrid":string})

def keycard_history_webbl(input_list_list:[Input_list]):
    ...
    output_list_list, Output_list = create_model("Output_list", {"datum":date, "aenderung":string, "username":string, "zeit":string})
    ...
    def generate_output():
        nonlocal output_list_list, err_msg, bediener, res_history
        nonlocal ubuff
        ...
        return {"output-list": output_list_list, "err_msg": err_msg}
    ...
    input_list = query(input_list_list, filters=(lambda input_list: input_list.from_date != None and input_list.to_date != None), first=True)
