            ...
            if curr_gastnr == 0:
                ...
                if gastname.lower()  == ("*").lower() :
                    ...
                    for bill, reservation, res_line, guest1 in db_session.query(Bill, Reservation, Res_line, Guest1).join(Reservation,(Reservation.resnr == Bill.resnr)).join(Res_line,(Res_line.resnr == Bill.resnr)).join(Guest1,(Guest1.gastnr == Bill.gastnr)).filter(
                             (Bill.zinr == "") & (Bill.flag == bil_flag) & (func.lower(Bill.name) >= (fr_name).lower()) & (func.lower(Bill.name) <= (to_name).lower()) & (Bill.resnr > 0) & (Bill.reslinnr == 0) & (Bill.billtyp == bill_type)).order_by(Bill.name, Bill.rechnr.desc()).all():
                    ...
                    bill_obj_list = []
                    for bill, reservation, res_line, guest1 in db_session.query(Bill, Reservation, Res_line, Guest1).join(Reservation,(Reservation.resnr == Bill.resnr)).join(Res_line,(Res_line.resnr == Bill.resnr)).join(Guest1,(Guest1.gastnr == Bill.gastnr)).filter(
                             (Bill.zinr == "") & (Bill.flag == bil_flag) & (func.lower(Bill.name) >= (gastname).lower()) & (func.lower(Bill.name) <= (to_name).lower()) & (Bill.resnr > 0) & (Bill.reslinnr == 0) & (Bill.billtyp == bill_type)).order_by(Bill.name, Bill.rechnr.desc()).all():
                        if bill._recid in bill_obj_list:
                            continue
                        else:
                            bill_obj_list.append(bill._recid)
            ...
            if curr_gastnr == 0:
                ...
                if gastname == "":
                    ...
                    for bill, reservation, res_line, guest1 in db_session.query(Bill, Reservation, Res_line, Guest1).join(Reservation,(Reservation.resnr == Bill.resnr)).join(Res_line,(Res_line.resnr == Bill.resnr) & (Res_line.abreise == ci_date)).join(Guest1,(Guest1.gastnr == Bill.gastnr)).filter(
                             (Bill.zinr == "") & (Bill.flag == bil_flag) & (func.lower(Bill.name) >= (gastname).lower()) & (Bill.resnr > 0) & (Bill.reslinnr == 0) & (Bill.billtyp == bill_type)).order_by(Bill.name, Bill.rechnr.desc()).all():
                        if bill._recid in bill_obj_list:
                            continue
                        else:
                            bill_obj_list.append(bill._recid)
