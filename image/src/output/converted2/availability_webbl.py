...
def availability_webbl(pvilanguage:int, printer_nr:int, call_from:int, txt_file:string, curr_date:date, to_date:date, incl_tentative:bool, zikat_list_list:[Zikat_list]):
    ...
    def generate_output():
        nonlocal msg_str, room_list_list, sum_list_list, lnldelimeter, ttl_room, occ_room, ooo_room, anz_setup, tot_room, isetup_array, csetup_array, ci_date, datum, week_list, i, j, curr_day, tmp_date, lvcarea, res_line, zimkateg, htparam, artikel, fixleist, arrangement, reslin_queasy, paramtext, zimmer, kontline, outorder, guest
    ...
    def calc_extra(fdate:date):
        ...
        for temp_art in query(temp_art_list):
            ...
            if artikel:
                ...
                ndate = htparam.fdate
                for i in range(1,(to_date - tmp_date)  + 1) :
    ...
    def count_rmcateg():
        ...
        zimmer_obj_list = {}
        for zimmer, zkbuff in db_session.query(Zimmer, Zkbuff).join(Zkbuff,(Zkbuff.zikatnr == zikat_list.zikatnr)).filter(
    ...
    def create_browse():
        ...
        for zkbuff in db_session.query(Zkbuff).filter(
            ...
            m = 1
            while m <= (to_date - timedelta(days=tmp_date)) :
        ...
        for outorder.gespende, outorder.gespstart, outorder._recid, zimmer.zikatnr, zimmer.setup, zimmer.sleeping, zimmer.zinr, zimmer._recid in db_session.query(Outorder.gespende, Outorder.gespstart, Outorder._recid, Zimmer.zikatnr, Zimmer.setup, Zimmer.sleeping, Zimmer.zinr, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Outorder.zinr) & (Zimmer.sleeping)).filter(
            ...
            if not room_list:
                ...
                for m in range(1,(to_date - tmp_date)  + 1) :
    ...
    def create_rmcat_list():
        ...
        if anz_setup > 0:
            ...
            n = 99
        while n <= (to_date - timedelta(days=tmp_date)) :
