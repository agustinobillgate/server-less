...
payload_list_list, Payload_list = create_model("Payload_list", {"pr":string, "mode":int, "s_artnr":int})

def pr_pbook_list_webbl(payload_list_list:[Payload_list]):
    ...
    lart_list_list, Lart_list = create_model("Lart_list", {"artnr":int, "bezeich":string, "traubensorte":string, "lief_einheit":Decimal})
    output_list_list, Output_list = create_model("Output_list", {"long_digit":bool})
    q1_list_list, Q1_list = create_model_like(L_pprice, {"a_firma":string, "traubensorte":string, "lief_einheit":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list_list, q1_list_list, lart_list_list, l_pprice, htparam, bediener, l_artikel, l_order, l_lieferant


        nonlocal lart_list, payload_list, output_list, q1_list
        nonlocal lart_list_list, output_list_list, q1_list_list

        return {"output-list": output_list_list, "q1-list": q1_list_list, "lart-list": lart_list_list}
    ...
    def create_lart_list():
        ...
        for l_order, l_artikel in db_session.query(L_order, L_art).join(L_art,(L_art.artnr == L_order.artnr)).filter(
            ...
            lart_list = Lart_list()
            lart_list_list.append(lart_list)
