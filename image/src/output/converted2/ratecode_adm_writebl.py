...
p_list_list, P_list = create_model_like(Tb3, {"rmcat_str":string, "wday_str":string, "adult_str":string, "child_str":string})
...
def ratecode_adm_writebl(mode_str:string, markno:int, prcode:string, argtno:int, user_init:string, book_room:int, comp_room:int, max_room:int, p_list_list:[P_list], early_discount_list:[Early_discount], kickback_discount_list:[Kickback_discount], stay_pay_list:[Stay_pay]):
    ...
    q_list_list, Q_list = create_model("Q_list", {"rcode":string, "dcode":string})
    ...
    product_list_list, Product_list = create_model("Product_list", {"market":int, "i_product":[int,99], "prcode":string})
    ...
    def generate_output():
        nonlocal tb3_list, error_flag, tb3buff_list, zikatno, wday, adult, child1, curr_1, curr_2, curr_3, curr_4, mesval, ci_date, round_betrag, round_method, length_round, adjust_value, in_percent, tax_included, curr_time, ratecode, queasy, htparam, bediener, res_history, zimkateg, guest_pr, guest, prtable, arrangement, artikel, waehrung
        ...
        nonlocal tb3_list, tb3buff_list, t_ratecode_list, q_list_list, child_list_list, child_ratecode_list, product_list_list
