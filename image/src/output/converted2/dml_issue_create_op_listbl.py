...
def dml_issue_create_op_listbl(op_list_list:[Op_list]):
    ...
    def generate_output():
        ...
        nonlocal buf_op_list_list

        return {"op-list": op_list_list}
    ...
    for l_artikel, sys_user in db_session.query(L_art, Sys_user).join(Sys_user,(Sys_user.nr == buf_op_list.fuellflag)).filter(
             ((L_art.artnr.in_(list(set([buf_op_list.artnr for buf_op_list in buf_op_list_list])))))).order_by(buf_op_list.datum.desc(), buf_op_list.zeit.desc()).all():
