...
from models import H_rezept, L_artikel, Htparam, H_rezlin, Queasy

def prepare_chg_rezeipt_webbl(pvilanguage:int, h_artnr:int, description:string):
    ...
    h_rezept = l_artikel = htparam = h_rezlin = queasy = None
    ...
    s_rezlin_list, S_rezlin = create_model("S_rezlin", {"h_recid":int, "artnr":int, "bezeich":string, "masseinheit":string, "s_unit":string, "menge":Decimal, "cost":Decimal, "inhalt":Decimal, "vk_preis":Decimal, "lostfact":Decimal, "recipe_flag":bool})
    ...
    def generate_output():
        nonlocal h_bezeich, katbezeich, katnr, portion, price_type, amount, msg_str, msg_str2, msg_str3, record_use, init_time, init_date, cost_percent, poten_sell_price, r_list_list, s_rezlin_list, t_l_artikel_list, t_h_rezept_list, curr_i, flag_ok, lvcarea, h_rezept, l_artikel, htparam, h_rezlin, queasy
    ...
    def cal_cost(p_artnr:int, menge:Decimal, cost:Decimal):
        ...
        hrecipe = None
        i:int = 0
        artikel:Decimal = to_decimal("0.0")
        ...
        for h_rezln in db_session.query(H_rezln).filter(
            ...
            if h_rezln.recipe_flag :
                ...
                if price_type == 0 or l_artikel.ek_aktuell == 0:
                    artikel =  to_decimal(inh) / to_decimal(l_artikel.inhalt) * to_decimal(l_artikel.vk_preis)
                    cost =  to_decimal(cost) + to_decimal(inh) / to_decimal(l_artikel.inhalt) * to_decimal(l_artikel.vk_preis)
                else:
                    artikel =  to_decimal(inh) / to_decimal(l_artikel.inhalt) * to_decimal(l_artikel.ek_aktuell)
