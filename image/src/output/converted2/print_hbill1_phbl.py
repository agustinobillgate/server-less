...
def print_hbill1_phbl(pvilanguage:int, session_parameter:string, user_init:string, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):
    ...
    art_list_list, Art_list = create_model("Art_list", {"printed":bool, "disc_flag":bool, "dept":int, "artnr":int, "bezeich":string, "artart":int, "zwkum":int, "qty":int, "price":Decimal, "amount":Decimal, "betrag":Decimal, "happyhr":string, "datum":date, "sysdate":date, "zeit":int, "condiment":int, "i_group":int})
    ...
    output_list_list, Output_list = create_model("Output_list", {"str":string, "pos":int, "flag_popup":bool, "npause":int, "sort_i":int})
    fb_vatlist_list, Fb_vatlist = create_model("Fb_vatlist", {"i_type":int, "artno":int, "vatno":int, "bezeich":string, "vatvalue":Decimal, "vatamt":Decimal})
    t_htp_list, T_htp = create_model_like(Htparam)

    Artbuff = create_buffer("Artbuff",Artikel)
    Abuff = Art_list
    abuff_list = art_list_list
    ...
    def generate_output():
        ...
        nonlocal art_list_list, t_printer_list, vat_list_list, output_list_list, fb_vatlist_list, t_htp_list

        return {"print_all": print_all, "filename": filename, "msg_str": msg_str, "winprinterflag": winprinterflag, "output-list": output_list_list, "t-printer": t_printer_list}
    ...
    def print_billine():
        ...
        t_htp_list = get_output(read_htparambl(5, None, None))

        t_htp = query(t_htp_list, filters=(lambda t_htp: t_htp.paramnr == 10), first=True)
        ...
        if service != 0 or mwst != 0:
            ...
            if sc_art != 0:
                ...
                if h_bill_line:
                    ...
                    art_list = query(art_list_list, filters=(lambda art_list: art_list.i_group == 2), first=True)
            ...
            for fb_vatlist in query(fb_vatlist_list, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.i_type >= 3), sort_by=[("i_type",False)]):
            ...
            art_list = query(art_list_list, filters=(lambda art_list: art_list.i_group == 1), first=True)
    ...
    for loopi in range(1,num_entries(str452, ";")  + 1) :
        ...
        if bartnr != 0:
            ...
            fb_vatlist_list.append(fb_vatlist)
    ...
    if printnr != 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not art_list or h_bill_line.betriebsnr == 1:
                ...
                if h_artikel and h_artikel.artart == 0:
                    ...
                    if h_artikel.artnr == sc_art:
                        ...
                        fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.artNo == art_list.artnr), first=True)
            ...
            if art_list.artart == 0:
                ...
                if art_list.i_group == 0:
                    ...
                    fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.i_type == 2), first=True)

                    if is_food:
                        fb_vatlist.vatamt = to_decimal(fb_vatlist.vatamt +\
                            round(h_bill_line.betrag / fact * mwst_perc , price_decimal))

                    fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.i_type == 3), first=True)
                    fb_vatlist.vatamt = to_decimal(fb_vatlist.vatamt +\
                    ...
                    if artikel.prov_code != 0:
                        ...
                        fb_vatlist = query(fb_vatlist_list, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.bezeich == htparam.bezeich), first=True)
        ...
        for abuff in query(abuff_list, filters=(lambda abuff:(abuff.zwkum == disc_zwkum) and matches(abuff.bezeich,r"*-*"))):
        ...
        if prdisc_flag and tot_ndisc_line >= 1 and tot_disc_line >= 1:
            ...
            art_list = query(art_list_list, filters=(lambda art_list: art_list.artart == 0 and not art_list.disc_flag and art_list.i_group == 0 and not art_list.printed), first=True)
            n = 0
            curr_j = printed_line
            for i in range(1,npage + 1) :
                ...
                while None != art_list and (curr_j <= lpage) :
                    ...
                    art_list = query(art_list_list, filters=(lambda art_list: art_list.artart == 0 and not art_list.disc_flag and art_list.i_group == 0 and not art_list.printed), next=True)
                ...
                if i == npage:
                    ...
                    for art_list in query(art_list_list, filters=(lambda art_list: art_list.artart == 0 and not art_list.disc_flag and art_list.i_group == 0 and not art_list.printed)):
                        subtotal =  to_decimal(subtotal) + to_decimal(art_list.amount)
                        print_billine()
