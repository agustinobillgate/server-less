...
def glores_controlbl(pvilanguage:int, del_flag:bool, from_date:date, to_date:date, from_name:string, to_name:string):
    ...
    glores_control_list_list, Glores_control_list = create_model("Glores_control_list", {"datum":date, "gastnr":int, "firma":string, "kontcode":string, "zikatnr":int, "kurzbez":string, "erwachs":int, "kind1":int, "gloanz":int, "gresanz":int, "resanz":int, "resnrstr":string})
    ...
    for k_list in query(k_list_list, sort_by=[("ankunft",False)]):
