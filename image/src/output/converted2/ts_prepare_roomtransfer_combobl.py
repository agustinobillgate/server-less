...
def ts_prepare_roomtransfer_combobl(language_code:int, h_recid:int, balance:Decimal, pf_file1:string, pf_file2:string):
    ...
    roomtf_list_list, Roomtf_list = create_model("Roomtf_list", {"resnr":int, "zinr":string, "code":string, "resstatus":int, "erwachs":int, "kind1":int, "gratis":int, "bemerk":string, "billnr":int, "g_name":string, "vorname1":string, "anrede1":string, "anredefirma":string, "bill_name":string, "ankunft":date, "abreise":date, "nation1":string, "parent_nr":int, "reslinnr":int, "resname":string, "name_bg_col":int, "name_fg_col":int, "bill_bg_col":int, "bill_fg_col":int}, {"name_bg_col": 15, "bill_bg_col": 15})
    ...
    for q1_list in query(q1_list_list, sort_by=[("zinr",False),("parent_nr",False),("reslinnr",False),("resname",False)]):
