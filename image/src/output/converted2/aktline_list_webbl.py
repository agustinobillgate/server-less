...
def aktline_list_webbl(inp_gastnr:int, next_date:date, all_flag:bool, prior:int, act_combo:str, acttype:int, user_init:str, to_date:date):
    aktline_list_list = []
    prioritaet:List[str] = create_empty_list(3,"")
        ...
        if all_flag:
            ...
            if prior == 0:
                ...
                if act_combo.lower()  == ("ALL").lower() :
                    ...
                    akt_line_obj_list = []
                    for akt_line, akt_code, guest, bediener in db_session.query(Akt_line, Akt_code, Guest, Bediener).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).join(Guest,(Guest.gastnr == Akt_line.gastnr)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
                             (Akt_line.flag == acttype) & (func.lower(Akt_line.userinit) == (user_init).lower())).order_by(Akt_line.datum, Akt_line.zeit, Akt_line.prioritaet.desc()).all():
                        if akt_line._recid in akt_line_obj_list:
                            continue
                        else:
                            akt_line_obj_list.append(akt_line._recid)
                ...
                if act_combo.lower()  == ("ALL").lower() :
                    ...
                    akt_line_obj_list = []
                    for akt_line, akt_code, guest, bediener in db_session.query(Akt_line, Akt_code, Guest, Bediener).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).join(Guest,(Guest.gastnr == Akt_line.gastnr)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
                             (Akt_line.flag == acttype) & (func.lower(Akt_line.userinit) == (user_init).lower()) & (Akt_line.prioritaet == prior)).order_by(Akt_line.datum, Akt_line.zeit, Akt_line.prioritaet.desc()).all():
                        if akt_line._recid in akt_line_obj_list:
                            continue
                        else:
                            akt_line_obj_list.append(akt_line._recid)
    ...
    def disp_all2():
        ...
        if all_flag:
            ...
            if act_combo.lower()  == ("ALL").lower() :
                ...
                akt_line_obj_list = []
                for akt_line, akt_code, guest, bediener in db_session.query(Akt_line, Akt_code, Guest, Bediener).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).join(Guest,(Guest.gastnr == Akt_line.gastnr)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
                             (Akt_line.flag == acttype) & (func.lower(Akt_line.userinit) == (user_init).lower()) & (Akt_line.gastnr == inp_gastnr)).order_by(Akt_line.datum, Akt_line.zeit, Akt_line.prioritaet.desc()).all():
                    if akt_line._recid in akt_line_obj_list:
                        continue
                    else:
                        akt_line_obj_list.append(akt_line._recid)
        ...
        if all_flag:
            ...
            if prior == 0:
                ...
                if act_combo.lower()  == ("ALL").lower() :
                    ...
                    akt_line_obj_list = []
                    for akt_line, akt_code, guest, bediener in db_session.query(Akt_line, Akt_code, Guest, Bediener).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).join(Guest,(Guest.gastnr == Akt_line.gastnr)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
                                 (Akt_line.flag == acttype) & (func.lower(Akt_line.userinit) == (user_init).lower()) & (Akt_line.datum >= next_date) & (Akt_line.datum <= to_date)).order_by(Akt_line.datum, Akt_line.zeit, Akt_line.prioritaet.desc()).all():
                        if akt_line._recid in akt_line_obj_list:
                            continue
                        else:
                            akt_line_obj_list.append(akt_line._recid)
                ...
                if act_combo.lower()  == ("ALL").lower() :
                    ...
                    akt_line_obj_list = []
                    for akt_line, akt_code, guest, bediener in db_session.query(Akt_line, Akt_code, Guest, Bediener).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).join(Guest,(Guest.gastnr == Akt_line.gastnr)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
                                 (Akt_line.flag == acttype) & (func.lower(Akt_line.userinit) == (user_init).lower()) & (Akt_line.datum >= next_date) & (Akt_line.datum <= to_date) & (Akt_line.prioritaet == prior)).order_by(Akt_line.datum, Akt_line.zeit, Akt_line.prioritaet.desc()).all():
                        if akt_line._recid in akt_line_obj_list:
                            continue
                        else:
                            akt_line_obj_list.append(akt_line._recid)
    ...
    def disp_it2():
        ...
        if all_flag:
            ...
            if act_combo.lower()  == ("ALL").lower() :
                ...
                akt_line_obj_list = []
                for akt_line, akt_code, guest, bediener in db_session.query(Akt_line, Akt_code, Guest, Bediener).join(Akt_code,(Akt_code.aktionscode == Akt_line.aktionscode)).join(Guest,(Guest.gastnr == Akt_line.gastnr)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
                             (Akt_line.flag == acttype) & (func.lower(Akt_line.userinit) == (user_init).lower()) & (Akt_line.datum >= next_date) & (Akt_line.datum <= to_date) & (Akt_line.gastnr == inp_gastnr)).order_by(Akt_line.datum, Akt_line.zeit, Akt_line.prioritaet.desc()).all():
                    if akt_line._recid in akt_line_obj_list:
                        continue
                    else:
                        akt_line_obj_list.append(akt_line._recid)
