...
def inv_update_billbl(pvilanguage:int, bil_flag:int, invoice_type:string, transdate:date, r_recid:int, deptno:int, billart:int, qty:int, price:Decimal, amount:Decimal, amount_foreign:Decimal, description:string, voucher_nr:string, cancel_str:string, user_init:string, billno:int, master_str:string, master_rechnr:string, balance:Decimal, balance_foreign:Decimal):
    ...
    def generate_output():
        ...
        return {"billno": billno, "master_str": master_str, "master_rechnr": master_rechnr, "balance": balance, "balance_foreign": balance_foreign, "master_flag": master_flag, "msg_str": msg_str, "success_flag": success_flag, "t-bill-line": t_bill_line_list}
    ...
    def inv_ar(curr_art:int, zinr:string, gastnr:int, gastnrmember:int, rechnr:int, saldo:Decimal, saldo_foreign:Decimal, bill_date:date, billname:string, userinit:string, voucher_nr:string, dept_nr:int):
        ...
        nonlocal master_flag, msg_str, success_flag, t_bill_line_list, price_decimal, exchg_rate, currzeit, curr_room, lvcarea, bill_line, res_line, htparam, waehrung, artikel, bill, arrangement, counters, umsatz, billjournal, argt_line, master, mast_art, debitor, reservation, guest, bediener
