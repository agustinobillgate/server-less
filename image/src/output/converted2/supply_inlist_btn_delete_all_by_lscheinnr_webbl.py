...
def supply_inlist_btn_delete_all_by_lscheinnr_webbl(pvilanguage:int, str_list_billdate:date, str_list_lief_nr:int, str_list_docu_nr:string, str_list_lscheinnr:string, str_list_l_recid:int, str_list_artnr:int):
    ...
    prepare_cache ([L_artikel, L_untergrup, Gl_acct, Gl_jouhdr, Htparam, L_bestand])

    may_delete = False
    msg_str = ""
    msg_str1 = ""
    msg_str2 = ""
    l_op_not_eligible_list = []
    art_fibu:string = ""
    lvcarea:string = "supply-inlist"
    l_op = l_kredit = l_artikel = l_untergrup = gl_acct = gl_jouhdr = gl_journal = htparam = l_bestand = None

    l_op_not_eligible = incoming_op = l_op = None

    l_op_not_eligible_list, L_op_not_eligible = create_model("L_op_not_eligible", {"lscheinnr":string, "artnr":int, "bezeich":string, "reason":string})

    Incoming_op = create_buffer("Incoming_op",L_op)
    L_op = create_buffer("L_op",L_op)


    db_session = local_storage.db_session

    def generate_output():
        nonlocal may_delete, msg_str, msg_str1, msg_str2, l_op_not_eligible_list, art_fibu, lvcarea, l_op, l_kredit, l_artikel, l_untergrup, gl_acct, gl_jouhdr, gl_journal, htparam, l_bestand
        ...
        nonlocal l_op_not_eligible, incoming_op, l_op
        nonlocal l_op_not_eligible_list

        return {"may_delete": may_delete, "msg_str": msg_str, "msg_str1": msg_str1, "msg_str2": msg_str2, "l-op-not-eligible": l_op_not_eligible_list}
    ...
    def check_onhand_after_cancel_receiving():
        ...
        for l_op, l_artikel in db_session.query(L_op, L_artikel).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
            ...
            if l_op._recid == 0:
                ...
                l_op_not_eligible_list.append(l_op_not_eligible)
        ...
        l_op_not_eligible = query(l_op_not_eligible_list, first=True)
    ...
    if art_fibu != "":
        ...
        for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
                 (Gl_jouhdr.datum == str_list_billdate) & (Gl_jouhdr.jtyp == 6) & (matches(Gl_jouhdr.refno,"RCV*"))).order_by(Gl_jouhdr._recid).all():
        ...
        for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
                 (Gl_jouhdr.datum == str_list_billdate) & (Gl_jouhdr.jtyp == 3) & (matches(Gl_jouhdr.refno,"OUT*"))).order_by(Gl_jouhdr._recid).all():
