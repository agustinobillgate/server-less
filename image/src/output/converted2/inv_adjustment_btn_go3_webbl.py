...
def inv_adjustment_btn_go3_webbl(from_grp:int, transdate:date, curr_lager:int, sorttype:int, user_init:string, c_list_list:[C_list]):
    ...
    def generate_output():
        ...
        return {"c-list": c_list_list, "err_code": err_code}
    ...
    def do_adjustment():
        ...
        for c_list in query(c_list_list, filters=(lambda c_list: c_list.qty != c_list.qty1), sort_by=[("fibukonto",False)]):
    ...
    for c_list in query(c_list_list, filters=(lambda c_list: c_list.fibukonto != "" and c_list.fibukonto != None)):
