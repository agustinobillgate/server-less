...
payload_list_list, Payload_list = create_model("Payload_list", {"curr_dept":int, "selected_date":string, "dml_no":string})

def dml_list_create_it_3_webbl(payload_list_list:[Payload_list]):
    ...
    dml_list_list, Dml_list = create_model("Dml_list", {"counter":int, "dept":string, "id":string, "approved":bool})
    response_list_list, Response_list = create_model("Response_list", {"dml_hdr_remark":string, "dml_grand_total":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal supply_list_list, c_list_list, response_list_list, curr_dept, selected_date, dml_no, dml_hdr_remark, dml_grand_total, dml_counter, fill_dml_nr, dml_no_remark, dml_artdep, reslin_queasy, queasy, dml_art, l_artikel, l_untergrup, l_bestand, l_lieferant


        nonlocal supply_list, c_list, dml_list, payload_list, response_list
        nonlocal supply_list_list, c_list_list, dml_list_list, response_list_list

        return {"supply-list": supply_list_list, "c-list": c_list_list, "response-list": response_list_list}
    ...
    if payload_list:
        ...
        selected_date = date_mdy(to_int(substring(payload_list.selected_date, 0, 2)) , to_int(substring(payload_list.selected_date, 3, 2)) , 2000 + timedelta(days=to_int(substring(payload_list.selected_date, 6, 2))))
