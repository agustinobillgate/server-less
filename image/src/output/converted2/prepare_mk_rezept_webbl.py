...
t_artikel_list, T_artikel = create_model("T_artikel", {"artnr_no":string})

def prepare_mk_rezept_webbl(t_artikel_list:[T_artikel]):
    ...
    def generate_output():
        nonlocal price_type, t_l_artikel_list, t_h_rezept_list, cost_list_list, curr_i, l_artikel, h_rezept, htparam, h_rezlin


        nonlocal t_l_artikel, t_h_rezept, cost_list, t_artikel, h_recipe, hrecipe
        nonlocal t_l_artikel_list, t_h_rezept_list, cost_list_list

        return {"price_type": price_type, "t-l-artikel": t_l_artikel_list, "t-h-rezept": t_h_rezept_list, "cost-list": cost_list_list}
    ...
    def create_list():
        ...
        for h_rezlin, h_rezept in db_session.query(H_rezlin, H_rezept).join(H_rezept,(H_rezept.artnrrezept == H_rezlin.artnrrezept)).filter(
            ...
            if h_rezlin.recipe_flag:
                ...
                t_h_rezept = query(t_h_rezept_list, filters=(lambda t_h_rezept: t_h_rezept.artnrrezept == h_rezlin.artnrlager), first=True)
    ...
    def calculate_cost():
        ...
        for t_h_rezept in query(t_h_rezept_list):
            ...
            cost_list = query(cost_list_list, filters=(lambda cost_list: cost_list.artnrrezept == t_h_rezept.artnrrezept), first=True)
    ...
    def cal_cost(p_artnr:int, menge:Decimal, cost:Decimal):
        ...
        H_recipe = T_h_rezept
        h_recipe_list = t_h_rezept_list
        Hrecipe = T_h_rezept
        hrecipe_list = t_h_rezept_list

        h_recipe = query(h_recipe_list, filters=(lambda h_recipe: h_recipe.artnrrezept == p_artnr), first=True)
        ...
        if h_recipe:
            ...
            for h_rezlin in db_session.query(H_rezlin).filter(
                ...
                if h_rezlin.recipe_flag :
                    ...
                    hrecipe = query(hrecipe_list, filters=(lambda hrecipe: hrecipe.artnrrezept == h_rezlin.artnrlager), first=True)
