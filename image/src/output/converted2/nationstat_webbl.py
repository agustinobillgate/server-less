...
def nationstat_webbl(printer_nr:int, call_from:int, txt_file:string, from_month:string, hide_zero:bool, mi_arrival:bool, mi_comp:bool, mi_show_d_percent:bool):
    ...
    room_list_list, Room_list = create_model("Room_list", {"nationnr":int, "name":string, "bezeich":string, "summe":int, "room":[int,31], "proz":Decimal, "d_percent":[Decimal,31]})
    total_per_day_list, Total_per_day = create_model("Total_per_day", {"date_day":int, "total_nat":int})
    ...
    def generate_output():
        ...
        nonlocal room_list_list, total_per_day_list
    ...
    def create_browse():
        ...
        for nationstat in db_session.query(Nationstat).filter(
                 ((Nationstat.nationnr.in_(list(set([room_list.nationnr for room_list in room_list_list])))) & (Nationstat.datum >= from_date) & (Nationstat.datum <= to_date))).order_by(room_list.bezeich, Nationstat.datum).all():            room_list = query(room_list_list, (lambda room_list: (nationstat.nationnr == room_list.nationnr)), first=True)
            i = get_day(nationstat.datum)

            total_per_day = query(total_per_day_list, filters=(lambda total_per_day: total_per_day.date_day == i), first=True)

            if not total_per_day:
                total_per_day = Total_per_day()
                total_per_day_list.append(total_per_day)
        ...
        for room_list in query(room_list_list, filters=(lambda room_list: room_list.summe != 0)):
            ...
            if mi_show_d_percent:
                ...
                for total_per_day in query(total_per_day_list):
