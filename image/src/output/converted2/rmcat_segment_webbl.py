...
def rmcat_segment_webbl(pvilanguage:int, cardtype:int, to_date:date, f_date:date, t_date:date, mi_ftd:bool, incl_comp:bool):
    ...
    def create_output():
        ...
        for s_list in query(s_list_list, sort_by=[("cat_bez",False),("segment",False)]):
            i = i + 1
    ...
    rmcat_segm_list = query(rmcat_segm_list_list, first=True)

    if rmcat_segm_list:

        for rmcat_segm_list in query(rmcat_segm_list_list):
            count_i = count_i + 1
            queasy = Queasy()
