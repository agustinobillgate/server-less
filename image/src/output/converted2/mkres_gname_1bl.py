...
def mkres_gname_1bl(case_type:int, temp_flag:int, create_guestseg:bool, gastno:int, sorttype:int, famname:string, inp_compno:int, wiguestflag:bool, adult:int):
    ...
    resline_list_list, Resline_list = create_model("Resline_list", {"ankunft":date, "abreise":date, "kurzbez":string, "zimmeranz":int, "zipreis":Decimal, "arrangement":string, "resnr":int, "reslinnr":int, "resstatus":int, "groupname":string, "bemerk":string, "active_flag":int})
    guest_list_list, Guest_list = create_model("Guest_list", {"firmen_nr":int, "steuernr":string, "full_name":string, "nation1":string, "wohnort":string, "land":string, "gastnr":int, "karteityp":int, "telefon":string, "overcredit":bool})
    ...
    def generate_output():
        ...
        return {"adult": adult, "guest-list": guest_list_list, "resline-list": resline_list_list}
