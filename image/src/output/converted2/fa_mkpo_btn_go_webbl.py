...
tfa_order_list, Tfa_order = create_model_like(Fa_order, {"nr_budget":int})

def fa_mkpo_btn_go_webbl(tfa_order_list:[Tfa_order], cmb_curr_screen_value:string, local_nr:int, order_nr:string, pr_nr:string, order_date:date, order_type:string, order_name:string, comments:string, supplier_nr:int, dept_nr:int, credit_term:int, paymentdate:date, expected_delivery:date, user_init:string, billdate:date, t_amount:Decimal, appr_1:bool, answer:bool):
    ...
    def generate_output():
        ...
        nonlocal cmb_curr_screen_value, local_nr, order_nr, pr_nr, order_date, order_type, order_name, comments, supplier_nr, dept_nr, credit_term, paymentdate, expected_delivery, user_init, billdate, t_amount, appr_1, answer
    ...
    for tfa_order in query(tfa_order_list, sort_by=[("fa_pos",False)]):
        repos = repos + 1
        fa_order = Fa_order()
