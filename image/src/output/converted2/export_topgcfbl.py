...
def export_topgcfbl(cardtype:int, from_date:date, to_date:date, recordcount:int, crm_flag:bool, his_flag:bool):
    ...
    def generate_output():
        nonlocal guest_list_list, sort_type, check_ftd, currency, excl_other, curr_sort, nr, mastername, sales_name, mainsegm, bediener, guest, guestseg, segment, akt_kont, queasy
        ...
        nonlocal guest_list_list, cust_list_list, usr_list
    ...
    excl_other = False
    sort_type = 1
    curr_sort[1], cust_list_list = get_output(export_topgcf_getturnoverbl(cardtype, sort_type, curr_sort[0], from_date, to_date, check_ftd, currency, excl_other, curr_sort[1]))

    if his_flag:

        for cust_list in query(cust_list_list, filters=(lambda cust_list: cust_list.gesamtumsatz != 0), sort_by=[("gesamtumsatz",True)]):
            nr = nr + 1
        ...
        for cust_list in query(cust_list_list, sort_by=[("gesamtumsatz",True)]):
            nr = nr + 1
            guest_list = Guest_list()
            guest_list_list.append(guest_list)
        ...
        while None != guest:
            ...
            guest_list = query(guest_list_list, filters=(lambda guest_list: guest_list.guest_nr == guest.gastnr), first=True)
