...
def prepare_guest_depositpay_webbl(pvilanguage:int, user_init:string, guest_number:int, guest_name:string):
    ...
    guest_deposit_list_list, Guest_deposit_list = create_model("Guest_deposit_list", {"guest_number":int, "article_number":int, "article_desc":string, "trans_amount":Decimal, "trans_date":string, "trans_remark":string, "res_number":string, "bill_number":int, "bill_type":string, "user_init":string, "flag":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal deposit_balance, error_desc, guest_deposit_list_list, lvcarea, depoart_guest, depobez_guest, depoart_rsv, depoart_bqt, depoart_pos, str, uniq_str, htparam, artikel, billjournal, bill_line, bill
        nonlocal pvilanguage, user_init, guest_number, guest_name


        nonlocal guest_deposit_list
        nonlocal guest_deposit_list_list

        return {"deposit_balance": deposit_balance, "error_desc": error_desc, "guest-deposit-list": guest_deposit_list_list}
    ...
    while None != billjournal:
        ...
        guest_deposit_list_list.append(guest_deposit_list)
