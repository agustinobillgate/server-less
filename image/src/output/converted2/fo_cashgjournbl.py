...
def fo_cashgjournbl(pvilanguage:int, case_type:int, curr_shift:int, summary_flag:bool, from_date:date, bline_list_list:[Bline_list]):
    ...
    sum_list_list, Sum_list = create_model("Sum_list", {"artnr":int, "artart":int, "bezeich":string, "f_amt":Decimal, "amt":Decimal})
    output_list_list, Output_list = create_model("Output_list", {"flag":string, "amt_foreign":Decimal, "str_foreign":string, "str":string, "gname":string})
    ...
    def generate_output():
        ...
        nonlocal sum_list_list, output_list_list

        return {"from_date": from_date, "double_currency": double_currency, "foreign_curr": foreign_curr, "bline-list": bline_list_list, "output-list": output_list_list}
    ...
    def journal_list():
                ...
                for artikel in db_session.query(Artikel).filter(
                    ...
                    if bediener:
                        ...
                        for billjournal in db_session.query(Billjournal).filter(
                                 (Billjournal.userinit == bediener.userinit) & (Billjournal.artnr == artikel.artnr) & (Billjournal.anzahl != 0) & (Billjournal.departement == artikel.departement) & (Billjournal.bill_datum == from_date)).order_by(Billjournal.zeit, Billjournal.rechnr).all():
        ...
        if sum_list:
            ...
            for sum_list in query(sum_list_list, filters=(lambda sum_list:(sum_list.f_amt != 0 or sum_list.amt != 0) and sum_list.artart == 6), sort_by=[("artnr",False)]):
        ...
        if sum_list:
            ...
            for sum_list in query(sum_list_list, filters=(lambda sum_list: sum_list.f_amt != 0 or sum_list.amt != 0), sort_by=[("artnr",False)]):
                output_list = Output_list()
                output_list_list.append(output_list)
        ...
        for bline_list in query(bline_list_list, filters=(lambda bline_list: bline_list.selected), sort_by=[("name",False)]):
            ...
            if bediener:
                ...
                for artikel in db_session.query(Artikel).filter(
                    ...
                    for billjournal in db_session.query(Billjournal).filter(
                             (Billjournal.userinit == bediener.userinit) & (Billjournal.artnr == artikel.artnr) & (Billjournal.anzahl != 0) & (Billjournal.departement == artikel.departement) & (Billjournal.bill_datum == from_date) & (Billjournal.betriebsnr == curr_shift)).order_by(Billjournal.zeit, Billjournal.rechnr).all():
