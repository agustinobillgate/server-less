...
def main_fs_proc_btn_page2bl(b1_resnr:int, b1_resline:int, curr_gastnr:int):
    ...
    tt_grund_list, Tt_grund = create_model("Tt_grund", {"curr_i":int, "grund":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal avail_b_storno, tt_grund_list, i, b_storno
        nonlocal b1_resnr, b1_resline, curr_gastnr


        nonlocal tt_grund
        nonlocal tt_grund_list

        return {"avail_b_storno": avail_b_storno, "tt-grund": tt_grund_list}
