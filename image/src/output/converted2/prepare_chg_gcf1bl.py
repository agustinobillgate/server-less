...
from models import Guest, Bediener, Mc_guest, Artikel, Gentable, Htparam, Akt_kont, Guestseg, Segment, Guest_pr, Queasy, Sourccod, History, Genlayout

def prepare_chg_gcf1bl(gastnr:int, chg_gcf:bool):

    prepare_cache ([Guest, Bediener, Artikel, Htparam, Akt_kont, Guestseg, Segment, Guest_pr, Queasy, Sourccod, History])
    ...
    sourccod_chg_list = []
    flag_ok:bool = False
    guest = bediener = mc_guest = artikel = gentable = htparam = akt_kont = guestseg = segment = guest_pr = queasy = sourccod = history = genlayout = None

    segment1 = sourccod1 = sourccod_chg = t_guest = q1_akt_kont = q2_history = guest0 = usr = None

    segment1_list, Segment1 = create_model("Segment1", {"bezeich":string})
    sourccod1_list, Sourccod1 = create_model("Sourccod1", {"source_code":int, "bezeich":string})
    sourccod_chg_list, Sourccod_chg = create_model("Sourccod_chg", {"source_code":int, "bezeich":string})
    t_guest_list, T_guest = create_model_like(Guest)
    q1_akt_kont_list, Q1_akt_kont = create_model("Q1_akt_kont", {"name":string, "vorname":string, "anrede":string, "hauptkontakt":bool})
    q2_history_list, Q2_history = create_model("Q2_history", {"ankunft":date, "abreise":date, "zinr":string, "zipreis":Decimal, "bemerk":string, "arrangement":string})
