...
payload_list_list, Payload_list = create_model("Payload_list", {"sorttype":int, "from_month":string})

def res_groupsummary_webbl(payload_list_list:[Payload_list]):
    ...
    res_grplist_list, Res_grplist = create_model("Res_grplist", {"res_month":string, "res_no":int, "resline_no":int, "group_name":string, "arrival_date":string, "depart_date":string, "night":int, "pax":int, "room_stay":int, "room_night":int, "ta_company":string, "purpose_stay":string, "nationality":string})
    output_list_list, Output_list = create_model("Output_list", {"month_report":int, "year_report":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal res_grplist_list, output_list_list, grp_name, room, str, purpose_no, mm, yy, i, res_no, night_stay, tot_pax, tot_room, tot_rmnight, ci_date, month_int, year_int, htparam, reservation, genstat, res_line, queasy, guest, nation


        nonlocal res_grplist, payload_list, output_list
        nonlocal res_grplist_list, output_list_list

        return {"res-grplist": res_grplist_list, "output-list": output_list_list}
    ...
    if sorttype == 1:
        ...
        for genstat.res_char, genstat.res_date, genstat.erwachs, genstat.gratis, genstat.kind1, genstat.kind2, genstat.resnr, genstat.gastnr, genstat.zinr, genstat._recid, reservation.name, reservation.groupname, reservation._recid in db_session.query(Genstat.res_char, Genstat.res_date, Genstat.erwachs, Genstat.gratis, Genstat.kind1, Genstat.kind2, Genstat.resnr, Genstat.gastnr, Genstat.zinr, Genstat._recid, Reservation.name, Reservation.groupname, Reservation._recid).join(Reservation,(Reservation.resnr == Genstat.resnr)).filter(
            ...
            if grp_name != genstat.res_char[2]:
                ...
                res_grplist_list.append(res_grplist)
    ...
    for res_grplist in query(res_grplist_list):
    ...
    res_grplist = query(res_grplist_list, first=True)
