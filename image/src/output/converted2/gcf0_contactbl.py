...
aktkont_list_list, Aktkont_list = create_model_like(Akt_kont, {"nat":str, "email":str, "rec_id":int})

def gcf0_contactbl(aktkont_list_list:[Aktkont_list], case_type:int, gastnr:int):
    main_kont = ""
    maincontact = ""
    kont_nr:int = 0
    akt_kont = None
    ...
    def generate_output():
        ...
        return {"aktkont-list": aktkont_list_list, "main_kont": main_kont, "maincontact": maincontact}
    ...
    if case_type == 1:
        ...
        akt_kont = db_session.query(Akt_kont).filter(
                 (Akt_kont._recid == aktkont_list.rec_id)).first()
