...
def comp_productbl_ori(pvilanguage:int, curr_date:string):
    ...
    c_list_list, C_list = create_model_like(Compproduct_list, {"gastnr":int})
    ...
    def generate_output():
        nonlocal compproduct_list_list, mm, yy, i, from_date, to_date, datum, jml, lvcarea, curr_guest, curr_name, do_it, created, counter, troom, tytd, tlytd, lfdate, ltdate, guest, guestat1
    ...
    for compproduct_list in query(compproduct_list_list, filters=(lambda compproduct_list: compproduct_list.ytd != 0 or compproduct_list.lytd != 0), sort_by=[("ytd",True),("bezeich",False)]):
        i = i + 1
