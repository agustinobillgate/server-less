...
def ts_restinv_transfer_to_guestbill_webbl(pvilanguage:int, rec_id:int, curr_dept:int, bilrecid:int, balance_foreign:Decimal, balance:Decimal, pay_type:int, transdate:date, double_currency:bool, exchg_rate:Decimal, kellner1_kcredit_nr:int, foreign_rate:bool, user_init:string, price_decimal:int, rec_id_h_artikel:int, deptname:string, h_artart:int, cancel_order:bool, h_artikel_service_code:int, order_taker:int, tischnr:int, curr_waiter:int, pax:int, kreditlimit:Decimal, add_zeit:int, print_to_kitchen:bool, from_acct:bool, h_artnrfront:int, guestnr:int, curedept_flag:bool, curr_room:string, hoga_resnr:int, hoga_reslinnr:int, incl_vat:bool, get_price:int, mc_str:string, disc_art1:int, disc_art2:int, disc_art3:int, t_submenu_list_list:[T_submenu_list]):
    ...
    def generate_output():
        ...
        nonlocal t_h_bill_list, t_kellner_list

        return {"bill_date": bill_date, "cancel_flag": cancel_flag, "mwst": mwst, "mwst_foreign": mwst_foreign, "rechnr": rechnr, "bcol": bcol, "p_88": p_88, "closed": closed, "avail_bill": avail_bill, "billno_str": billno_str, "cancel_str": cancel_str, "error_message": error_message, "t-h-bill": t_h_bill_list, "t-kellner": t_kellner_list}
    ...
    amount, amount_foreign, bill_date, billart_bfo, qty_bfo, description_bfo, cancel_str, t_h_bill_list = get_output(ts_restinv_update_bill1bl(rec_id, transdate, double_currency, exchg_rate, t_kellner.kcredit_nr, bilrecid, foreign_rate, user_init, gname, hoga_resnr, hoga_reslinnr, price_decimal, amount, amount_foreign))
    pass
    h_bill.flag = 0
    pass
    bill_date, cancel_flag, fl_code, mwst, mwst_foreign, rechnr, balance, bcol, balance_foreign, fl_code1, fl_code2, fl_code3, p_88, closed, t_h_bill_list, t_kellner_list = get_output(ts_restinv_update_bill_1bl(pvilanguage, rec_id, rec_id_h_artikel, deptname, transdate, h_artart, cancel_order, h_artikel_service_code, amount, amount_foreign, price, double_currency, qty, exchg_rate, price_decimal, order_taker, tischnr, curr_dept, curr_waiter, gname, pax, kreditlimit, add_zeit, billart, description, "", "", "", "", "", "", print_to_kitchen, from_acct, h_artnrfront, pay_type, guestnr, transfer_zinr, curedept_flag, foreign_rate, curr_room, user_init, hoga_resnr, hoga_reslinnr, incl_vat, get_price, mc_str, t_submenu_list_list))
    ...
    if h_bill:
        ...
        h_bill.rgdruck = 1
        pass
    bill_date, flag_code, t_h_bill_list = get_output(ts_restinv_btn_transfer_paytypegt1_2bl(rec_id, True, transdate, curr_dept, disc_art1, disc_art2, disc_art3, curr_waiter))
