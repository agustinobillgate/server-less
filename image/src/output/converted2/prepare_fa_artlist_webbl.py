...
from models import Fa_grup, Fa_artikel, Mathis, Fa_lager, Fa_kateg, Gl_acct

sortir_list_list, Sortir_list = create_model("Sortir_list", {"from_date":date, "to_date":date, "location":int, "show_all":bool})

def prepare_fa_artlist_webbl(sortir_list_list:[Sortir_list]):

    prepare_cache ([Fa_grup, Fa_artikel, Mathis, Fa_lager, Fa_kateg, Gl_acct])

    p_881 = None
    q1_list_list = []
    fibu_list_list = []
    sort_loc:string = ""
    fa_grup = fa_artikel = mathis = fa_lager = fa_kateg = gl_acct = None

    q1_list = fibu_list = sortir_list = bfa_grup = None

    q1_list_list, Q1_list = create_model("Q1_list", {"name":string, "asset":string, "datum":date, "price":Decimal, "anzahl":int, "warenwert":Decimal, "depn_wert":Decimal, "book_wert":Decimal, "katnr":int, "bezeich":string, "location":string, "first_depn":date, "next_depn":date, "last_depn":date, "id":string, "created":date, "cid":string, "changed":date, "remark":string, "mathis_nr":int, "fname":string, "supplier":string, "posted":bool, "fibukonto":string, "faartikel_nr":int, "credit_fibu":string, "debit_fibu":string, "recid_fa_artikel":int, "recid_mathis":int, "avail_glacct1":bool, "avail_glacct2":bool, "avail_glacct3":bool, "subgroup":int, "model":string, "gnr":int, "flag":int, "grp_bez":string, "sgrp_bez":string, "rate":Decimal, "mark":string, "spec":string, "anz_depn":int, "category":int, "lager_nr":int})
    fibu_list_list, Fibu_list = create_model("Fibu_list", {"flag":int, "fibukonto":string, "bezeich":string, "credit":Decimal, "debit":Decimal})
    ...
    def generate_output():
        nonlocal p_881, q1_list_list, fibu_list_list, sort_loc, fa_grup, fa_artikel, mathis, fa_lager, fa_kateg, gl_acct
        nonlocal bfa_grup


        nonlocal q1_list, fibu_list, sortir_list, bfa_grup
    ...
    sortir_list = query(sortir_list_list, first=True)

    if sortir_list.location != None and sortir_list.show_all == False:

        mathis_obj_list = {}
        mathis = Mathis()
        fa_artikel = Fa_artikel()
        fa_grup = Fa_grup()
        for mathis.location, mathis.name, mathis.asset, mathis.datum, mathis.price, mathis.remark, mathis.nr, mathis.fname, mathis.supplier, mathis._recid, mathis.model, mathis.flag, mathis.mark, mathis.spec, fa_artikel.anzahl, fa_artikel.warenwert, fa_artikel.depn_wert, fa_artikel.book_wert, fa_artikel.katnr, fa_artikel.first_depn, fa_artikel.next_depn, fa_artikel.last_depn, fa_artikel.id, fa_artikel.created, fa_artikel.cid, fa_artikel.changed, fa_artikel.posted, fa_artikel.fibukonto, fa_artikel.nr, fa_artikel.credit_fibu, fa_artikel.debit_fibu, fa_artikel._recid, fa_artikel.subgrp, fa_artikel.gnr, fa_artikel.anz_depn, fa_grup.bezeich, fa_grup.fibukonto, fa_grup.credit_fibu, fa_grup.debit_fibu, fa_grup._recid in db_session.query(Mathis.location, Mathis.name, Mathis.asset, Mathis.datum, Mathis.price, Mathis.remark, Mathis.nr, Mathis.fname, Mathis.supplier, Mathis._recid, Mathis.model, Mathis.flag, Mathis.mark, Mathis.spec, Fa_artikel.anzahl, Fa_artikel.warenwert, Fa_artikel.depn_wert, Fa_artikel.book_wert, Fa_artikel.katnr, Fa_artikel.first_depn, Fa_artikel.next_depn, Fa_artikel.last_depn, Fa_artikel.id, Fa_artikel.created, Fa_artikel.cid, Fa_artikel.changed, Fa_artikel.posted, Fa_artikel.fibukonto, Fa_artikel.nr, Fa_artikel.credit_fibu, Fa_artikel.debit_fibu, Fa_artikel._recid, Fa_artikel.subgrp, Fa_artikel.gnr, Fa_artikel.anz_depn, Fa_grup.bezeich, Fa_grup.fibukonto, Fa_grup.credit_fibu, Fa_grup.debit_fibu, Fa_grup._recid).join(Fa_artikel,(Fa_artikel.nr == Mathis.nr) & (Fa_artikel.loeschflag == 0)).join(Fa_grup,(Fa_grup.gnr == Fa_artikel.subgrp) & (Fa_grup.flag == 1)).filter(
                 (Mathis.datum >= sortir_list.from_date) & (Mathis.datum <= sortir_list.to_date)).order_by(Mathis.name).all():
            if mathis_obj_list.get(mathis._recid):
                continue
            else:
                mathis_obj_list[mathis._recid] = True

            fa_lager = get_cache (Fa_lager, {"lager_nr": [(eq, sortir_list.location)],"bezeich": [(eq, mathis.location)]})
    ...
    elif sortir_list.location == None and sortir_list.show_all == False:
        ...
        for mathis.location, mathis.name, mathis.asset, mathis.datum, mathis.price, mathis.remark, mathis.nr, mathis.fname, mathis.supplier, mathis._recid, mathis.model, mathis.flag, mathis.mark, mathis.spec, fa_artikel.anzahl, fa_artikel.warenwert, fa_artikel.depn_wert, fa_artikel.book_wert, fa_artikel.katnr, fa_artikel.first_depn, fa_artikel.next_depn, fa_artikel.last_depn, fa_artikel.id, fa_artikel.created, fa_artikel.cid, fa_artikel.changed, fa_artikel.posted, fa_artikel.fibukonto, fa_artikel.nr, fa_artikel.credit_fibu, fa_artikel.debit_fibu, fa_artikel._recid, fa_artikel.subgrp, fa_artikel.gnr, fa_artikel.anz_depn, fa_grup.bezeich, fa_grup.fibukonto, fa_grup.credit_fibu, fa_grup.debit_fibu, fa_grup._recid in db_session.query(Mathis.location, Mathis.name, Mathis.asset, Mathis.datum, Mathis.price, Mathis.remark, Mathis.nr, Mathis.fname, Mathis.supplier, Mathis._recid, Mathis.model, Mathis.flag, Mathis.mark, Mathis.spec, Fa_artikel.anzahl, Fa_artikel.warenwert, Fa_artikel.depn_wert, Fa_artikel.book_wert, Fa_artikel.katnr, Fa_artikel.first_depn, Fa_artikel.next_depn, Fa_artikel.last_depn, Fa_artikel.id, Fa_artikel.created, Fa_artikel.cid, Fa_artikel.changed, Fa_artikel.posted, Fa_artikel.fibukonto, Fa_artikel.nr, Fa_artikel.credit_fibu, Fa_artikel.debit_fibu, Fa_artikel._recid, Fa_artikel.subgrp, Fa_artikel.gnr, Fa_artikel.anz_depn, Fa_grup.bezeich, Fa_grup.fibukonto, Fa_grup.credit_fibu, Fa_grup.debit_fibu, Fa_grup._recid).join(Fa_artikel,(Fa_artikel.nr == Mathis.nr) & (Fa_artikel.loeschflag == 0)).join(Fa_grup,(Fa_grup.gnr == Fa_artikel.subgrp) & (Fa_grup.flag == 1)).order_by(Mathis.name).all():
            ...
            fa_lager = get_cache (Fa_lager, {"bezeich": [(eq, mathis.location)]})
            q1_list = Q1_list()
            q1_list_list.append(q1_list)
