...
def fo_parxls_gs_list_outputbl(pvilanguage:int, briefnr:int, link:str):
    msg_str = ""
    mess_result = "Failed to generate data"
    success_flag = False
    lvcarea:str = "fo-parxls-gs"
    ch:str = ""
    xls_dir:str = ""
        ...
        for ww1 in query(ww1_list, filters=(lambda ww1: ww1.grpflag == 0), sort_by=[("main_code",True)]):
            ...
            if ww1.main_code == 807:
                ...
                artikel = db_session.query(Artikel).filter(
                         (Artikel.artnr == ww1.artnr) & (Artikel.departement == ww1.dept)).first()

                if not artikel:
                    msg_str = msg_str + chr(2) + translateExtended ("Article for Cost-variable not found : ", lvcarea, "") + w1.varname
                    error_nr = -1
                ...
                if not artikel:
                    msg_str = msg_str + chr(2) + translateExtended ("No such article number : ", lvcarea, "") + to_string(ww1.artnr) + " " + translateExtended ("dept", lvcarea, "") + " " + to_string(ww1.dept)
    ...
    if msg_str != "":
        ...
        queasy = db_session.query(Queasy).filter(
                 (Queasy.key == 293) & (Queasy.deci1 == briefnr) & (Queasy.char2 == trim(link)) & (Queasy.date1 == get_current_date())).first()
