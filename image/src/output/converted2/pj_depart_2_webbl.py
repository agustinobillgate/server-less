...
from models import Htparam, Guest, Zimkateg, Reservation, Res_line, History, Guestseg, Nation, Bill, Mc_guest, Mc_types, Segment

setup_list_list, Setup_list = create_model("Setup_list", {"nr":int, "char":string})

def pj_depart_2_webbl(pvilanguage:int, case_type:int, disptype:int, curr_date:date, froom:string, troom:string, ota_only:bool, setup_list_list:[Setup_list]):

    prepare_cache ([Htparam, Guest, Zimkateg, Reservation, Res_line, Guestseg, Nation, Bill, Mc_guest, Mc_types, Segment])
    ...
    all_remark:string = ""
    htparam = guest = zimkateg = reservation = res_line = history = guestseg = nation = bill = mc_guest = mc_types = segment = None
    ...
    def create_departure():
        ...
        if disptype == 1:
            ...
            guest = Guest()
            gmember = Guest()
            for res_line.ankunft, res_line.abreise, res_line.resnr, res_line.reslinnr, res_line.name, res_line.zinr, res_line.erwachs, res_line.kind1, res_line.kind2, res_line.gratis, res_line.arrangement, res_line.flight_nr, res_line.changed_id, res_line.setup, res_line.zimmer_wunsch, res_line.abreisezeit, res_line.zimmeranz, res_line.resstatus, res_line.bemerk, res_line.active_flag, res_line._recid, zimkateg.kurzbez, zimkateg.bezeichnung, zimkateg._recid, reservation.gastnr, reservation.groupname, reservation._recid, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest.karteityp, guest._recid, guest.gastnr, guest.nation1, guest.email_adr, guest.adresse1, guest.adresse2, guest.adresse3, guest.telefon, guest.mobil_telefon, gmember.name, gmember.vorname1, gmember.anrede1, gmember.anredefirma, gmember.karteityp, gmember._recid, gmember.gastnr, gmember.nation1, gmember.email_adr, gmember.adresse1, gmember.adresse2, gmember.adresse3, gmember.telefon, gmember.mobil_telefon in db_session.query(Res_line.ankunft, Res_line.abreise, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.zinr, Res_line.erwachs, Res_line.kind1, Res_line.kind2, Res_line.gratis, Res_line.arrangement, Res_line.flight_nr, Res_line.changed_id, Res_line.setup, Res_line.zimmer_wunsch, Res_line.abreisezeit, Res_line.zimmeranz, Res_line.resstatus, Res_line.bemerk, Res_line.active_flag, Res_line._recid, Zimkateg.kurzbez, Zimkateg.bezeichnung, Zimkateg._recid, Reservation.gastnr, Reservation.groupname, Reservation._recid, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest.karteityp, Guest._recid, Guest.gastnr, Guest.nation1, Guest.email_adr, Guest.adresse1, Guest.adresse2, Guest.adresse3, Guest.telefon, Guest.mobil_telefon, Gmember.name, Gmember.vorname1, Gmember.anrede1, Gmember.anredefirma, Gmember.karteityp, Gmember._recid, Gmember.gastnr, Gmember.nation1, Gmember.email_adr, Gmember.adresse1, Gmember.adresse2, Gmember.adresse3, Gmember.telefon, Gmember.mobil_telefon).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                ...
                if do_it:
                    ...
                    cl_list.changed_id = res_line.changed_id
