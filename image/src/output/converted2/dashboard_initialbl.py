...
def dashboard_initialbl(exclude_article:str):
    his_res_list = []
    his_inv_list = []
    future_res_list = []
    future_inv_list = []
    ...
    while None != res_line:
        ...
        res_line = db_session.query(Res_line).filter(
                 ((Res_line.resstatus == 9) | (Res_line.resstatus == 10)) & (Res_line.active_flag == 2) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line.ankunft >= (ci_date - timedelta(days=storage_dur))) & (Res_line.ankunft <= bill_date)).filter(Res_line._recid > curr_recid).first()

    genstat = db_session.query(Genstat).filter(
             (Genstat.datum >= (ci_date - timedelta(days=storage_dur))) & (Genstat.datum <= bill_date) & (Genstat.resstatus != 0) & (Genstat.res_logi[inc_value(1)]) & (Genstat.zikatnr != 0)).first()
    while None != genstat:
        ...
        genstat = db_session.query(Genstat).filter(
                 (Genstat.datum >= (ci_date - timedelta(days=storage_dur))) & (Genstat.datum <= bill_date) & (Genstat.resstatus != 0) & (Genstat.res_logi[inc_value(1)]) & (Genstat.zikatnr != 0)).filter(Genstat._recid > curr_recid).first()

    zkstat = db_session.query(Zkstat).filter(
             (Zkstat.datum >= (ci_date - timedelta(days=storage_dur))) & (Zkstat.datum <= bill_date) & (Zkstat.zikatnr != 0)).first()
    while None != zkstat:
        ...
        zkstat = db_session.query(Zkstat).filter(
                 (Zkstat.datum >= (ci_date - timedelta(days=storage_dur))) & (Zkstat.datum <= bill_date) & (Zkstat.zikatnr != 0)).filter(Zkstat._recid > curr_recid).first()

    outorder = db_session.query(Outorder).filter(
             (Outorder.gespende >= (ci_date - timedelta(days=storage_dur))) & (Outorder.gespstart <= bill_date) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1)).first()
    while None != outorder:
        ...
        outorder = db_session.query(Outorder).filter(
                 (Outorder.gespende >= (ci_date - timedelta(days=storage_dur))) & (Outorder.gespstart <= bill_date) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1)).filter(Outorder._recid > curr_recid).first()
    ...
    while None != outorder:
        ...
        curr_recid = outorder._recid
        outorder = db_session.query(Outorder).filter(
                 (Outorder.gespende >= ci_date) & (Outorder.gespstart <= ci_date + timedelta(days=90)) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1)).filter(Outorder._recid > curr_recid).first()
    ...
    res_line = db_session.query(Res_line).filter(
             (Res_line.abreise >= ci_date) & (Res_line.ankunft <= ci_date + timedelta(days=90)) & (Res_line.resstatus != 4) & (Res_line.resstatus != 8) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.active_flag <= 1) & (Res_line.l_zuordnung[inc_value(2)] == 0)).first()
    while None != res_line:
        ...
        curr_recid = res_line._recid
        res_line = db_session.query(Res_line).filter(
                 (Res_line.abreise >= ci_date) & (Res_line.ankunft <= ci_date + timedelta(days=90)) & (Res_line.resstatus != 4) & (Res_line.resstatus != 8) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.active_flag <= 1) & (Res_line.l_zuordnung[inc_value(2)] == 0)).filter(Res_line._recid > curr_recid).first()
