...
def hk_change_room_status_ombl(pvilanguage:int, user_init:string, from_date:date, to_date:date, reason:string, room_list_list:[Room_list]):
    ...
    flag, t_zinr, t_zistatus = get_output(hk_statadmin_start_chgstatbl(5, bline_list_list))
    ...
    if flag == 6:
        ...
        bline_list_list, om_list_list, msg_str, z_list_list = get_output(hk_statadmin_chk_btn_exit1bl(bline_list_list, om_list_list, pvilanguage, TRUE, 0, t_zinr, bediener.nr, from_date, to_date, ci_date, reason))
    else:
