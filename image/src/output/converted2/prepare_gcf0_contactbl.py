...
def prepare_gcf0_contactbl(gastnr:int):
    ...
    aktkont_list_list, Aktkont_list = create_model_like(Akt_kont, {"nat":string, "email":string})
    akt_kont1_list, Akt_kont1 = create_model("Akt_kont1", {"gastnr":int, "name":string, "vorname":string, "anrede":string, "hauptkontakt":bool})
    t_akt_kont_list, T_akt_kont = create_model_like(Akt_kont, {"p_bezeich":string, "rec_id":int})
    t_queasy13_list, T_queasy13 = create_model("T_queasy13", {"number1":int, "char1":string})
