...
def ins_storerequest_btn_gobl(curr_pos:int, op_list_list:[Op_list], transdate:date, curr_lager:int, deptno:int, transfered:bool, to_stock:int, user_init:string, lscheinnr:string):
    ...
    for op_list in query(op_list_list, filters=(lambda op_list: op_list.anzahl != 0 and op_list.new_flag), sort_by=[("pos",False)]):
        zeit = zeit + 1
