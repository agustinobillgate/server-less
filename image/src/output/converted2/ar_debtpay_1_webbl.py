...
def ar_debtpay_1_webbl(artnr:int, userinit:string, bill_nr:int, temp_art2:int, bill_name:string, to_name:string, bill_saldo:Decimal, from_date:date, to_date:date, art_selected:int, foutstand:Decimal, outstand:Decimal):
    ...
    def generate_output():
        ...
        nonlocal artnr, userinit, bill_nr, temp_art2, bill_name, to_name, bill_saldo, from_date, to_date, art_selected, foutstand, outstand
    ...
    def create_age_list():
        ...
        if bill_nr != 0:
            ...
            for debitor in db_session.query(Debitor).filter(
                         (Debitor.artnr == temp_art2) & (Debitor.rechnr == bill_nr) & (Debitor.rgdatum >= from_date) & (Debitor.rgdatum <= to_date) & (Debitor.opart < 2)).order_by(Debitor.zahlkonto, Debitor.rgdatum).all():
            ...
            for debitor in db_session.query(Debitor).filter(
                         (Debitor.artnr == temp_art2) & (matches((Debitor.name,("*" + bill_name + "*"))) & (Debitor.name <= (to_name).lower())) & (Debitor.rgdatum >= from_date) & (Debitor.rgdatum <= to_date) & (Debitor.opart < 2)).order_by(Debitor.zahlkonto, Debitor.debref, Debitor.rgdatum).all():
            ...
            for debitor in db_session.query(Debitor).filter(
                         (Debitor.artnr == temp_art2) & (matches(Debitor.name,(bill_name))) & (Debitor.rgdatum >= from_date) & (Debitor.rgdatum <= to_date) & (Debitor.opart < 2)).order_by(Debitor.zahlkonto, Debitor.debref, Debitor.rgdatum).all():
            ...
            for debitor in db_session.query(Debitor).filter(
                         (Debitor.artnr == temp_art2) & (Debitor.rgdatum >= from_date) & (Debitor.rgdatum <= to_date) & (Debitor.opart < 2)).order_by(Debitor.zahlkonto, Debitor.rgdatum).all():
    ...
    for age_list in query(age_list_list):
        ...
        for t_pay in db_session.query(T_pay).filter(
                 (T_pay.artnr == temp_art2) & (T_pay.rechnr == age_list.rechnr) & (T_pay.counter == age_list.counter) & (T_pay.counter != 0) & (T_pay.opart < 2) & ((T_pay.rgdatum < from_date) | (T_pay.rgdatum > to_date))).order_by(T_pay.zahlkonto, T_pay.rgdatum).all():
