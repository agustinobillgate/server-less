...
def ordertaker_listbl(usr_nr:int, from_date:date, to_date:date):
    ...
    odtaker_list_list, Odtaker_list = create_model("Odtaker_list", {"datum":date, "tableno":string, "billno":int, "artno":int, "bezeich":string, "qty":int, "amount":Decimal, "departement":string, "zeit":string, "id":string, "tb":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal odtaker_list_list, long_digit, qty, sub_tot, tot, curr_date, count_data, htparam, h_artikel, h_bill, h_journal, hoteldpt
        nonlocal usr_nr, from_date, to_date


        nonlocal odtaker_list
        nonlocal odtaker_list_list
