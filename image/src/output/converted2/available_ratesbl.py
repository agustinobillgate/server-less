...
def available_ratesbl(frdate:date, todate:date, i_zikatnr:int, i_counter:int, adult_child_str:string, ind_gastno:int, created_list_list:[Created_list]):
    ...
    selected_ratelist_list, Selected_ratelist = create_model("Selected_ratelist", {"ratecode":string, "marknr":int, "argtno":int, "adult":int, "child":int, "minstay":int, "maxstay":int})
    ...
    def generate_output():
        ...
        nonlocal rate_list_list, dynarate_list_list, selected_ratelist_list

        return {"ind_gastno": ind_gastno, "created-list": created_list_list, "rate-list": rate_list_list}
    ...
    for rate_list in query(rate_list_list, filters=(lambda rate_list: rate_list.minadvance > 0), sort_by=[("minadvance",True)]):
