...
from models import Guest, Paramtext, Htparam, Res_line, Waehrung, Reservation, Zimkateg, Bresline, Guestseg, Master, Messages, Kontline, Gentable, Mc_guest, Reslin_queasy, Zimmer, Queasy, Fixleist

def arl_list_disp_arlist3_webbl(show_rate:bool, last_sort:int, lresnr:int, long_stay:int, ci_date:date, grpflag:bool, room:str, lname:str, sorttype:int, fdate1:date, fdate2:date, fdate:date, excl_rmshare:bool, voucher_no:str):
    rmlen = 0
    arl_list_list = []
    ...
    def generate_output():
        nonlocal rmlen, arl_list_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, stay, resbemerk, rescomment, blacklist_int, guest, paramtext, htparam, res_line, waehrung, reservation, zimkateg, bresline, guestseg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer, queasy, fixleist
        nonlocal show_rate, last_sort, lresnr, long_stay, ci_date, grpflag, room, lname, sorttype, fdate1, fdate2, fdate, excl_rmshare, voucher_no
        nonlocal gbuff, gmember


        nonlocal setup_list, gbuff, arl_list, b_arl_list, gmember
    ...
    def get_toname(lname:str):
        ...
        nonlocal rmlen, arl_list_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, stay, resbemerk, rescomment, blacklist_int, guest, paramtext, htparam, res_line, waehrung, reservation, zimkateg, bresline, guestseg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer, queasy, fixleist
        nonlocal show_rate, last_sort, lresnr, long_stay, ci_date, grpflag, room, sorttype, fdate1, fdate2, fdate, excl_rmshare, voucher_no
        nonlocal gbuff, gmember
        ...
        if sorttype == 1:
            ...
            if last_sort == 1:
                ...
                if room != "":
                    ...
                    if lname == "":
                        ...
                        res_line_obj_list = []
                        for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
            ...
            if last_sort == 1:
                ...
                if lname == "":
                    ...
                    res_line_obj_list = []
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft == ci_date) & (func.lower(Res_line.name).op("~")((("*" + lname + "*").lower().replace("*",".*")))) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Res_line.name, Res_line.zinr).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
            ...
            if last_sort == 1:
                ...
                if lname == "":
                    ...
                    if room == "":
                        ...
                        res_line_obj_list = []
                        for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                                 (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                            setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                            if not setup_list:
                                continue
                    ...
                    res_line_obj_list = []
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & (func.lower(Res_line.name).op("~")((("*" + lname + "*").lower().replace("*",".*")))) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Res_line.name, Res_line.zinr).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
    ...
    def disp_arrivea():
        ...
        if last_sort == 1:
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft >= htparam.fdate1) & (Res_line.ankunft <= htparam.fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by((Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft >= htparam.fdate1) & (Res_line.ankunft <= htparam.fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (func.lower(Res_line.name).op("~")((("*" + lname + "*".lower().replace("*",".*")))))).order_by(Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (func.lower(Res_line.name).op("~")(((lname).lower().replace("*",".*")))) & (Res_line.ankunft >= htparam.fdate1) & (Res_line.ankunft <= htparam.fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft >= htparam.fdate1) & (Res_line.ankunft <= htparam.fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Reservation.name, Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname) & (func.lower(Reservation.name).op("~")(((lname.lower().replace("*",".*")))))).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 0) & (Res_line.ankunft >= htparam.fdate1) & (Res_line.ankunft <= htparam.fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Reservation.name, Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            res_line_obj_list = []
            for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 0) & (Res_line.ankunft >= htparam.fdate1) & (Res_line.ankunft <= htparam.fdate2) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Res_line.ankunft, Reservation.name, Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                if not setup_list:
                    continue
    ...
    def sqry2():
        ...
        if sorttype == 2:
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (func.lower(Reservation.name) >= (lname).lower()) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (func.lower(Reservation.name).op("~")((("*" + lname + "*").lower().replace("*",".*")))) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
            ...
            if lname == "":
                ...
                if room == "":
                    ...
                    res_line_obj_list = []
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Reservation.name, Reservation.groupname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
                    ...
                    res_line_obj_list = []
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower()))).order_by(Res_line.zinr, Reservation.name, Reservation.groupname, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
    ...
    def sqry3():
        ...
        if sorttype == 2:
            ...
            if lresnr == 0:
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate)).order_by(Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & ((Res_line.resnr == lresnr) | (Res_line.l_zuordnung[inc_value(4)] == lresnr))).order_by(Res_line.l_zuordnung[inc_value(4)], Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
    ...
    def sqry4():
        ...
        if sorttype == 2:
            ...
            if fdate != None:
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.ankunft >= htparam.fdate) & (Res_line.abreise >= ci_date)).order_by(Res_line.ankunft, Reservation.name, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (func.lower(Res_line.name) >= (lname).lower()) & (func.lower(Res_line.name) <= (to_name).lower())).order_by(Res_line.name, Res_line.zinr).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & ((substring(Res_line.zinr, 0, to_int(rmlen))) >= ((room).lower())) & (func.lower(Res_line.name).op("~")(((lname.lower().replace("*",".*")))))).order_by(Res_line.name, Res_line.zinr).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
        ...
        if sorttype == 1:
            ...
            if fdate != None:
                ...
                if room != "":
                    ...
                    res_line_obj_list = []
                    for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (func.lower(Res_line.zinr) >= (room).lower()) & (Res_line.ankunft <= ci_date) & (Res_line.abreise >= ci_date)).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                        if not setup_list:
                            continue
            ...
            if room != "":
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 0) & (Res_line.ankunft == ci_date) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (func.lower(Res_line.resname) >= (lname).lower()) & (substring(Res_line.zinr, 0, to_int(rmlen)) == (room).lower())).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
            ...
            if room != "":
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & (func.lower(Res_line.resname) >= (lname).lower()) & (substring(Res_line.zinr, 0, to_int(rmlen)) == (room).lower())).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == htparam.fdate) & (func.lower(Res_line.resname) >= (lname).lower()) & (func.lower(Res_line.zinr) >= (room).lower())).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
    ...
    def sqry6():
        ...
        if sorttype == 1:
            ...
            if voucher_no != "" and voucher_no != None:
                ...
                res_line_obj_list = []
                for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (len(trim(Reservation.groupname)) >= igrpname)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                         (Res_line.active_flag == 0) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99) & (func.lower(Res_line.zimmer_wunsch).op("~")((("*Voucher*".lower().replace("*",".*")))))).order_by(Res_line.name).all():
                    setup_list = query(setup_list_list, (lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                    if not setup_list:
                        continue
    ...
    def create_it():
        ...
        guestseg = db_session.query(Guestseg).filter(
                 (Guestseg.gastnr == res_line.gastnrmember) & (Guestseg.segmentcode == blacklist_int)).first()
        ...
        if res_line.kontignr > 0:
            ...
            kontline = db_session.query(Kontline).filter(
                     (Kontline.kontignr == res_line.kontignr) & (Kontline.betriebsnr == 1) & (Kontline.kontstatus == 1)).first()
        ...
        fixleist = db_session.query(Fixleist).filter(
                 (Fixleist.resnr == res_line.resnr) & (Fixleist.reslinnr == res_line.reslinnr)).first()
    ...
    for arl_list in query(arl_list_list, sort_by=[("zinr",False),("sharer_no",False)]):
    ...
    if excl_rmshare:
        ...
        for arl_list in query(arl_list_list, filters=(lambda arl_list: arl_list.resstatus == 11 or arl_list.resstatus == 13)):
            arl_list_list.remove(arl_list)
