...
def gcjour_list_2bl(case_type:int, from_refno:string, sorttype:int, journaltype:int, jtype1:int, from_date:date, to_date:date):
    ...
    gl_jouhdr_list_list, Gl_jouhdr_list = create_model("Gl_jouhdr_list", {"datum":date, "refno":string, "bezeich":string, "debit":Decimal, "credit":Decimal, "remain":Decimal, "jnr":int, "activeflag":int, "jtype":string})
    b2_list_list, B2_list = create_model("B2_list", {"fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "bezeich":string, "map_acct":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal gl_jouhdr_list_list, b2_list_list, yy, jtype, gl_jouhdr, gl_journal, gl_acct
