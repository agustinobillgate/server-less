...
def ap_paymentlistbl(all_supp:bool, remark_flag:bool, from_supp:string, from_date:date, to_date:date, from_remark:string, price_decimal:int):
    ...
    ap_paymentlist_list, Ap_paymentlist = create_model("Ap_paymentlist", {"srecid":int, "remark":string, "billdate":date, "docu_nr":string, "ap_amount":Decimal, "pay_amount":Decimal, "pay_date":date, "id":string, "pay_art":string, "supplier":string, "deliv_note":string, "bank_name":string, "bank_an":string, "bank_acc":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal lief_nr1, ap_exist, err_code, ap_paymentlist_list, t_list_list, amount, l_lieferant, l_kredit, artikel, bediener
