...
def fa_podetailbl(docu_nr:string):
    ...
    tmp_tbl_data_list, Tmp_tbl_data = create_model("Tmp_tbl_data", {"fa_nr":int, "name":string, "asset":string, "order_qty":int, "order_price":Decimal, "order_amount":Decimal, "delivered_qty":int, "delivered_date":date, "last_user":string})
    ...
    s_order_list, S_order = create_model_like(Fa_order, {"last_user":string})
    ...
    mathis_obj_list = {}
    for mathis in db_session.query(Mathis)disclist = query(disclist_list, (lambda disclist: disclist.fa_recid == s_order.fa_pos), first=True)if not disclist:    continue
.filter(
             ((Mathis.nr.in_(list(set([s_order.fa_nr for s_order in s_order_list if s_order.order_nr == (docu_nr).lower() ])))))).order_by(s_order.fa_pos).all():
        if mathis_obj_list.get(mathis._recid):
            continue
        else:
            mathis_obj_list[mathis._recid] = True

        s_order = query(s_order_list, (lambda s_order: (mathis.nr == s_order.fa_nr)), first=True)
