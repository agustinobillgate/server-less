...
def ba_availability_create_listbl(ba_dept:int, z_zknr:int, curr_date:date):
    ...
    for bk_func in db_session.query(Bk_func).filter(
             ((Bk_func.veran_nr.in_(list(set([r_list.veran_nr for r_list in r_list_list])))) & (Bk_func.veran_seite == r_list.veran_seite))).order_by(Bk_func.uhrzeit, r_list.raum).all():
