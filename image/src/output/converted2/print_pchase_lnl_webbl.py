...
def print_pchase_lnl_webbl(pvilanguage:int, lnldelimeter:string, docunr:string, stattype:int, curr_status:string):
    ...
    str3_list, Str3 = create_model("Str3", {"bl_descript":string, "arr_date":string, "bl_qty":string, "d_unit":string, "bl_price":string, "bl_amount":string, "c_exrate":string, "bl_balance":string, "remark":string, "konto":string, "disc":Decimal, "disc2":Decimal, "vat":Decimal, "disc_value":Decimal, "disc2_value":Decimal, "epreis0":Decimal, "bl_vat":string, "artnr":int, "brutto":Decimal, "po_nr":string, "po_source":string, "vat1":Decimal, "vat2":Decimal, "add_vat":Decimal, "bl_amount_add_vat":string})
            ...
            for l_order in db_session.query(L_order).filter(
                ...
                if l_art:
                    ...
                    if l_art.jahrgang == 0 or length(l_order.stornogrund) <= 12:
                        ...
                        op_list = query(op_list_list, filters=(lambda op_list: op_list.artnr == l_order.artnr and op_list.epreis == l_order.einzelpreis and op_list.bezeich == l_art.bezeich and op_list.disc == disc and op_list.disc2 == disc2 and op_list.konto == l_order.stornogrund and op_list.remark == l_order.besteller), first=True)
                    else:
