...
def calls_listbl(case_type:int, last_sort:int, from_date:date, to_date:date, from_ext:string, to_ext:string, stattype:int, price_decimal:int, double_currency:bool, fr_number:string, to_number:string, dialed_nr:string):
    ...
    str_list_list, Str_list = create_model("Str_list", {"zero_rate":bool, "c_recid":int, "destination":string, "rechnr":int, "nebenstelle":string, "datum":date, "rufnummer":string, "username":string, "guest_rate":Decimal, "pabx_rate":Decimal, "zeit":string, "dauer":string, "zinr":string, "impulse":int, "leitung":int, "sequence":int, "print":string})
