...
def if_custom_pushall_availbl(currcode:string, from_date:date, to_date:date, becode:int, inp_str:string, pushrate:bool, temp_list_list:[Temp_list]):
    ...
    allotment_list, allotment = create_model("allotment", {"datum":date, "zikatnr":int, "res_allot":int, "allot":int, "ruecktage":int})
    ...
    def generate_output():
        ...
        nonlocal allotment_list, rmcat_list_list, r_list_list
    ...
    def count_rmcateg():
        ...
        for zimmer in db_session.query(Zimmer).filter(
            ...
            if zimkateg and zimkateg.verfuegbarkeit:
                ...
                temp_list = query(temp_list_list, filters=(lambda temp_list: temp_list.zikatnr == zikatnr), first=True)

                if (not all_room and temp_list) or all_room:

                    rmcat_list = query(rmcat_list_list, filters=(lambda rmcat_list: rmcat_list.zikatnr == zikatnr), first=True)
