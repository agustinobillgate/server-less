...
def arl_list_disp_arlist2bl(inp_gastnr:int, inp_grpname:str, ci_date:date, show_rate:bool, long_stay:int):
    ...
    def disp_grp_all_rline_all_rstatus():
        ...
        res_line_obj_list = []
        for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (func.lower(Reservation.groupname) == (inp_grpname).lower())).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
