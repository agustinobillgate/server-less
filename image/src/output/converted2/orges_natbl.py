...
def orges_natbl(fdate:date, to_date:date, show_ytd:bool):
    ...
    region_list_list, Region_list = create_model_like(Orges_list)
    ...
    def generate_output():
        ...
        nonlocal orges_list_list, region_list_list, out_list_list, out_list1_list
    ...
    def create_list():
        ...
        for queasy in db_session.query(Queasy).filter(
            ...
            region_list_list.append(region_list)
        ...
        for genstat, nation in db_session.query(Genstat, Nation).join(Nation,(Nation.nationnr == Genstat.nationnr)).filter(
            ...
            region_list = query(region_list_list, (lambda region_list: region_list.region_nr == nation.untergruppe), first=True)
            if not region_list:
                continue
        ...
        for orges_list in query(orges_list_list, sort_by=[("region_nr",False)]):
            region_list = query(region_list_list, (lambda region_list: region_list.region_nr == orges_list.region_nr), first=True)
            if not region_list:
                continue
            ...
            out_list = query(out_list_list, filters=(lambda out_list: out_list.num == 999 and out_list.region_nr == orges_list.region_nr), first=True)
    ...
    def create_list1():
        ...
        for orges_list in query(orges_list_list, sort_by=[("region_nr",False)]):
            ...
            out_list1 = query(out_list1_list, filters=(lambda out_list1: out_list1.num == 999 and out_list1.region_nr == orges_list.region_nr), first=True)
