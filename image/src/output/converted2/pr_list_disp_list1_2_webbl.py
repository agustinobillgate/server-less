...
def pr_list_disp_list1_2_webbl(char1:string, billdate:date, from_date:date, to_date:date, outstand_flag:bool, expired_flag:bool, approve_flag:bool, reject_flag:bool, artnumber:int, deptno:int, sorttype:int):
    ...
    P_list = S_list
    p_list_list = s_list_list

    T_list = S_list
    t_list_list = s_list_list
    ...
    for s_list in query(s_list_list, filters=(lambda s_list: s_list.pos == 0)):

        p_list = query(p_list_list, filters=(lambda p_list: p_list.pos > 0 and p_list.docu_nr == s_list.docu_nr), first=True)

        if not p_list:
            s_list_list.remove(s_list)

        t_list = query(t_list_list, filters=(lambda t_list: t_list.pos > 0 and t_list.docu_nr == s_list.docu_nr and t_list.loeschflag != sorttype), first=True)
    ...
    if deptno == 0:
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: not((s_list.artnr == 0 and s_list.loeschflag == sorttype) or (s_list.artnr > 0 and s_list.loeschflag == sorttype))), sort_by=[("docu_nr",False),("pos",False)]):
            s_list_list.remove(s_list)
    else:

        for s_list in query(s_list_list, filters=(lambda s_list: not(((s_list.artnr == 0 and s_list.loeschflag == sorttype) or (s_list.artnr > 0 and s_list.loeschflag == sorttype)) and s_list.deptnr == deptno)), sort_by=[("docu_nr",False),("pos",False)]):
            s_list_list.remove(s_list)
