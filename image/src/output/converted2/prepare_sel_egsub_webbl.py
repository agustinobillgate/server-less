...
def prepare_sel_egsub_webbl(main_nr:int, dept_nr:int):
    ...
    duration_list, Duration = create_model("Duration", {"dur_nr":int, "dur_str":string})
    q_list_list, Q_list = create_model("Q_list", {"sub_code":string, "bezeich":string, "dur_str":string, "dur_nr":int, "days":string, "hours":string, "minutes":string})
