...
def pj_depart_4bl(pvilanguage:int, case_type:int, disptype:int, curr_date:date, froom:string, troom:string, disp_accompany:bool, setup_list_list:[Setup_list], zikat_list_list:[Zikat_list]):
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, case_type, disptype, curr_date, froom, troom, disp_accompany
        ...
        if disptype == 1:
            ...
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                ...
                if not disp_accompany:
                    ...
                    cl_list = query(cl_list_list, filters=(lambda cl_list: cl_list.curr_zinr == res_line.zinr and cl_list.resnr == res_line.resnr and date_mdy(cl_list.arrive) == res_line.ankunft and cl_list.zipreis == 0 and (cl_list.a + cl_list.c) < 1 and cl_list.co < 1), first=True)
