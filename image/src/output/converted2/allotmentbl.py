...
def allotmentbl(case_type:int, gastnr:int, kon_gastnr:int, kontcode:str, ankunft:date, abreise:date, zimmeranz:int):
    ...
    def check_resline():
        ...
        res_line_obj_list = []
        for res_line, kontline in db_session.query(Res_line, Kontline).join(Kontline,(Kontline.kontignr == Res_line.kontignr) & (func.lower(Kontline.kontcode) == (kontcode).lower()) & (Kontline.kontstatus == 1)).filter(
    ...
    def create_slist():
        ...
        weekdays:List[str] = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
