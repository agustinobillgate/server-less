...
active_roomlist_list, Active_roomlist = create_model_like(Zimmer, {"room_selected":bool})

def res_gname2_auto_assignment_webbl(s_list_list:[S_list], active_roomlist_list:[Active_roomlist], v_mode:int, location:string, froom:string, troom:string):
    ...
    def auto_assignmen_with_selectedroom():
        ...
        for s1_list in query(s1_list_list, filters=(lambda s1_list: s1_list.zinr == "" and s1_list.resstatus != 11 and s1_list.zimmeranz == 1)):
            ...
            active_roomlist = query(active_roomlist_list, filters=(lambda active_roomlist: active_roomlist.zikatnr == rline.zikatnr and active_roomlist.room_selected and active_roomlist.setup == rline.setup and active_roomlist.zinr.lower()  > (last_zinr).lower()), first=True)
            while None != active_roomlist and not found:
                ...
                if do_it:
                    ...
                    active_roomlist = query(active_roomlist_list, filters=(lambda active_roomlist: active_roomlist.zikatnr == rline.zikatnr and active_roomlist.room_selected and active_roomlist.setup == rline.setup and active_roomlist.zinr.lower()  > (last_zinr).lower()), next=True)
        last_zinr = ""
        ...
        for s1_list in query(s1_list_list, filters=(lambda s1_list: s1_list.zinr == "" and s1_list.active_flag == 0 and s1_list.resstatus != 11)):
            ...
            while None != active_roomlist and not found:
                ...
                if do_it:
                    ...
                    s2_list = query(s2_list_list, filters=(lambda s2_list: s2_list.zinr == active_roomlist.zinr), first=True)
