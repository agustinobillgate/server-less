...
def prepare_setup_new_drr():
    ...
    resto_list_list, Resto_list = create_model("Resto_list", {"deptnr":int, "departement":string, "artikel":string, "resto_info":string})
    fbcover_list_list, Fbcover_list = create_model("Fbcover_list", {"deptnr":int, "departement":string, "food":string, "beverage":string, "material":string})
    paxcover_list_list, Paxcover_list = create_model("Paxcover_list", {"deptnr":int, "departement":string, "artikel":string})
    setup_list_list, Setup_list = create_model("Setup_list", {"payment":string, "statistic":string, "outlets":string})
    pay_list_list, Pay_list = create_model("Pay_list", {"payment_flag":string, "ledger_flag":string, "cash_flag":string, "foreign_flag":string, "deposit_flag":string, "other_pay_flag":string, "artnr_payment":string, "artnr_ledger":string, "artnr_cash":string, "artnr_foreign":string, "artnr_deposit":string, "artnr_other_pay":string})
    statistic_list_list, Statistic_list = create_model("Statistic_list", {"fo_rev_flag":string, "other_income_flag":string, "segment_rev_flag":string, "statistic_flag":string, "artnr_fo":string, "artnr_other":string, "fo_info":string, "other_info":string, "segmentcode":string, "segment_info":string, "statistic_zwkum":string, "statistic_artnr":string, "statistic_info":string})
    outlets_list_list, Outlets_list = create_model("Outlets_list", {"outlets_flag":string, "fbcover_flag":string, "fbsales_flag":string, "outlets_info":string, "fbcover_info":string, "fbsales_info":string})
    outlets_info_list, Outlets_info = create_model("Outlets_info", {"deptnr":int, "artnr":string, "otls_info":string})
    fbcover_info_list, Fbcover_info = create_model("Fbcover_info", {"deptnr":int, "food":string, "beverage":string, "material":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal resto_list_list, fbcover_list_list, paxcover_list_list, pay_list_list, statistic_list_list, outlets_list_list, gsheet_link, str1, str2, str3, st1, st2, st3, st4, st5, st6, st7, st8, st9, st10, st11, st12, st13, st14, st15, st16, st17, st18, st19, st20, st21, st22, n, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13, n14, n15, n16, n17, n18, n19, n20, hoteldpt, queasy


        nonlocal resto_list, fbcover_list, paxcover_list, setup_list, pay_list, statistic_list, outlets_list, outlets_info, fbcover_info
        nonlocal resto_list_list, fbcover_list_list, paxcover_list_list, setup_list_list, pay_list_list, statistic_list_list, outlets_list_list, outlets_info_list, fbcover_info_list

        return {"resto-list": resto_list_list, "fbcover-list": fbcover_list_list, "paxcover-list": paxcover_list_list, "pay-list": pay_list_list, "statistic-list": statistic_list_list, "outlets-list": outlets_list_list, "gsheet_link": gsheet_link}

    for hoteldpt in db_session.query(Hoteldpt).filter(
             (Hoteldpt.num != 0)).order_by(Hoteldpt._recid).all():
        resto_list = Resto_list()
        resto_list_list.append(resto_list)
        ...
        fbcover_list_list.append(fbcover_list)
        ...
        paxcover_list_list.append(paxcover_list)
    ...
    statistic_list = query(statistic_list_list, first=True)

    if not statistic_list:
        statistic_list = Statistic_list()
        statistic_list_list.append(statistic_list)
    ...
    outlets_list = query(outlets_list_list, first=True)

    if not outlets_list:
        outlets_list = Outlets_list()
        outlets_list_list.append(outlets_list)
        ...
        for n4 in range(1,num_entries(str3, "*")  + 1) :
                ...
                for n6 in range(1,num_entries(outlets_list.outlets_info, ";")  + 1) :
                    ...
                    for n7 in range(1,num_entries(st8, "|")  + 1) :
                        ...
                        if st9 != "" and n7 == 1:
                            ...
                            outlets_info_list.append(outlets_info)
            ...
            if substring(st7, 0, 9) == ("$FBcover$").lower()  and substring(st7, 9, 3) == ("YES").lower() :
                ...
                for n8 in range(1,num_entries(fbcover_info, ";")  + 1) :
                    ...
                    fbcover_info_list.append(fbcover_info)
    ...
    for resto_list in query(resto_list_list):

        outlets_info = query(outlets_info_list, filters=(lambda outlets_info: outlets_info.deptnr == resto_list.deptnr), first=True)

        if outlets_info:
            resto_list.artikel = outlets_info.artnr
            resto_list.resto_info = outlets_info.otls_info

    for fbcover_list in query(fbcover_list_list):

        fbcover_info = query(fbcover_info_list, filters=(lambda fbcover_info: fbcover_info.deptnr == fbcover_list.deptnr), first=True)
