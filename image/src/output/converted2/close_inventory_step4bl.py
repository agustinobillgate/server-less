...
def close_inventory_step4bl(inv_type:int, m_endkum:int, closedate:date, todate:date, user_init:str):
    ...
    def del_old_requests():
        ...
        for l_ophdr in db_session.query(L_ophdr).filter(
                 (func.lower(L_ophdr.op_typ) == ("REQ").lower()) & (L_ophdr.datum <= closedate)).order_by(L_ophdr._recid).all():
            del_it = False
    ...
    for l_lager in db_session.query(L_lager).order_by(L_lager._recid).all():
        ...
        while None != l_bestand:
            ...
            l_bestand = db_session.query(L_bestand).filter(
                     (L_bestand.lager_nr == l_lager.lager_nr) & ((L_bestand.anf_best_dat <= closedate) | (L_bestand.anf_best_dat == None))).filter(L_bestand._recid > curr_recid).first()
    ...
    while None != l_bestand:
        ...
        curr_recid = l_bestand._recid
        l_bestand = db_session.query(L_bestand).filter(
                     (L_bestand.lager_nr == 0) & ((L_bestand.anf_best_dat <= closedate) | (L_bestand.anf_best_dat == None))).filter(L_bestand._recid > curr_recid).first()
