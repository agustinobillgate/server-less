...
b_list_list, B_list = create_model("B_list", {"gastnr":int, "cust_name":string, "gname":string, "gesamtumsatz":Decimal, "logiernachte":Decimal, "argtumsatz":Decimal, "f_b_umsatz":Decimal, "sonst_umsatz":Decimal, "wohnort":string, "plz":string, "land":string, "sales_id":string, "ba_umsatz":Decimal, "ly_rev":Decimal, "region":string, "region1":string, "stayno":int, "resnr":string, "counter":int, "counterall":int, "resno":int, "reslinnr":int, "curr_pos":int, "count_room":string, "rm_sharer":string, "arrival":string, "depart":string})

def cust_turnover_sorting_detail_output_list_webbl(idflag:string, b_list_list:[B_list]):
    doneflag = False
    counter:int = 0
    queasy = None
    ...
    def generate_output():
        ...
        return {"doneflag": doneflag, "b-list": b_list_list}
