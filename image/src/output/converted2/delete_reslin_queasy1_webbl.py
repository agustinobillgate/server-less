...
del_list_list, Del_list = create_model("Del_list", {"recid_reslin":int})

def delete_reslin_queasy1_webbl(case_type:int, int1:int, char1:string, date1:date, del_list_list:[Del_list]):
    ...
    if case_type == 1:
        ...
        for del_list in query(del_list_list):

            reslin_queasy = get_cache (Reslin_queasy, {"_recid": [(eq, del_list.recid_reslin)]})
