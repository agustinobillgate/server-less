...
def hk_roomplan_create_browsebl(print_it:bool, from_room:string, curr_date:date, ci_date:date):
    ...
    room_list_list, Room_list = create_model("Room_list", {"zistatus":int, "ststr":string, "build":string, "build_flag":string, "recid1":[int,17], "etage":int, "zinr":string, "c_char":string, "i_char":string, "zikatnr":int, "rmcat":string, "connec":string, "avtoday":string, "gstatus":[int,17], "bcol":[int,17], "fcol":[int,17], "room":[string,17]})
