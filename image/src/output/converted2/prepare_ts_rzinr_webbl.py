...
def prepare_ts_rzinr_webbl(pvilanguage:int, dept:int, zinr:string, h_resnr:int, h_reslinnr:int, balance:Decimal):
    ...
    q1_list = guest2 = accomp_r = bbuff = None

    q1_list_list, Q1_list = create_model("Q1_list", {"resnr":int, "zinr":string, "code":string, "resstatus":int, "erwachs":int, "kind1":int, "gratis":int, "bemerk":string, "billnr":int, "g_name":string, "vorname1":string, "anrede1":string, "anredefirma":string, "bill_name":string, "ankunft":date, "abreise":date, "nation1":string, "parent_nr":int, "reslinnr":int, "resname":string, "name_bg_col":int, "name_fg_col":int, "bill_bg_col":int, "bill_fg_col":int, "accomp":string}, {"name_bg_col": 15, "bill_bg_col": 15})

    Guest2 = create_buffer("Guest2",Guest)
    Accomp_r = create_buffer("Accomp_r",Res_line)
    ...
    def generate_output():
        ...
        nonlocal guest2, accomp_r, bbuff


        nonlocal q1_list, guest2, accomp_r, bbuff
    ...
    for res_line.code, res_line.resnr, res_line.zinr, res_line.resstatus, res_line.erwachs, res_line.kind1, res_line.gratis, res_line.bemerk, res_line.ankunft, res_line.abreise, res_line.reslinnr, res_line.name, res_line._recid, guest.kreditlimit, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest.nation1, guest._recid, bbuff.gastnr, bbuff.saldo, bbuff._recid, bbuff.flag, bbuff.billnr, bbuff.name, bbuff.parent_nr in db_session.query(Res_line.code, Res_line.resnr, Res_line.zinr, Res_line.resstatus, Res_line.erwachs, Res_line.kind1, Res_line.gratis, Res_line.bemerk, Res_line.ankunft, Res_line.abreise, Res_line.reslinnr, Res_line.name, Res_line._recid, Guest.kreditlimit, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest.nation1, Guest._recid, Bbuff.gastnr, Bbuff.saldo, Bbuff._recid, Bbuff.flag, Bbuff.billnr, Bbuff.name, Bbuff.parent_nr).join(Guest,(Guest.gastnr == Res_line.gastnrmember)).join(Bbuff,(Bbuff.resnr == Res_line.resnr) & (Bbuff.reslinnr == Res_line.reslinnr)).filter(
        ...
        accomp_r = Res_line()
        guest2 = Guest()
        for accomp_r.code, accomp_r.resnr, accomp_r.zinr, accomp_r.resstatus, accomp_r.erwachs, accomp_r.kind1, accomp_r.gratis, accomp_r.bemerk, accomp_r.ankunft, accomp_r.abreise, accomp_r.reslinnr, accomp_r.name, accomp_r._recid, guest2.kreditlimit, guest2.name, guest2.vorname1, guest2.anrede1, guest2.anredefirma, guest2.nation1, guest2._recid in db_session.query(Accomp_r.code, Accomp_r.resnr, Accomp_r.zinr, Accomp_r.resstatus, Accomp_r.erwachs, Accomp_r.kind1, Accomp_r.gratis, Accomp_r.bemerk, Accomp_r.ankunft, Accomp_r.abreise, Accomp_r.reslinnr, Accomp_r.name, Accomp_r._recid, Guest2.kreditlimit, Guest2.name, Guest2.vorname1, Guest2.anrede1, Guest2.anredefirma, Guest2.nation1, Guest2._recid).join(Guest2,(Guest2.gastnr == Accomp_r.gastnrmember)).filter(
                 (Accomp_r.resnr == q1_list.resnr) & (Accomp_r.resstatus == 13) & (Accomp_r.l_zuordnung[inc_value(2)] == 1)).order_by(Accomp_r._recid).all():
            if accomp_r_obj_list.get(accomp_r._recid):
                continue
            else:
                accomp_r_obj_list[accomp_r._recid] = True


            q1_list.accomp = q1_list.accomp + guest2.name + " "
