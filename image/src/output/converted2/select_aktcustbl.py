...
def select_aktcustbl():
    ...
    aktcustlist_list, Aktcustlist = create_model("Aktcustlist", {"name":string, "gastnr":int, "anrede1":string, "anredefirma":string, "adresse1":string, "plz":string, "wohnort":string})
    ...
    for akt_cust, guest in db_session.query(Akt_cust, Guest).join(Guest,(Guest.gastnr == Akt_cust.gastnr)).filter(
             (Akt_cust.userinit == userinit)).order_by(Guest.name).all():
