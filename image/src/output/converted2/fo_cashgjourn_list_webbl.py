...
from functions.fo_cashgjourn_cldbl import fo_cashgjourn_cldbl

bline_list_list, Bline_list = create_model("Bline_list", {"flag":int, "userinit":string, "selected":bool, "name":string, "bl_recid":int})

def fo_cashgjourn_list_webbl(pvilanguage:int, case_type:int, curr_shift:int, summary_flag:bool, onlyjournal:bool, excljournal:bool, from_date:date, bline_list_list:[Bline_list]):
    ...
    fo_cashjour_list_list, Fo_cashjour_list = create_model("Fo_cashjour_list", {"datum":date, "zinr":string, "rechnr":int, "artnr":int, "bezeich":string, "dept":string, "l_amount":Decimal, "f_amount":Decimal, "gname":string, "zeit":string, "id":string})
    ...
    def generate_output():
        ...
        nonlocal output_list_list, fo_cashjour_list_list

        return {"from_date": from_date, "double_currency": double_currency, "foreign_curr": foreign_curr, "bline-list": bline_list_list, "fo-cashjour-list": fo_cashjour_list_list}


    from_date, double_currency, foreign_curr, bline_list_list, output_list_list = get_output(fo_cashgjourn_cldbl(pvilanguage, case_type, curr_shift, summary_flag, onlyjournal, excljournal, from_date, bline_list_list))
