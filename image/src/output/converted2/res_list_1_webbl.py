...
def res_list_1_webbl(case_type:int, from_date:date, to_date:date, resnr:int, gastnr:int, search_resno:int, search_voucher:string):
    ...
    res_listmain_list, Res_listmain = create_model("Res_listmain", {"resdat":date, "resnr":int, "name":string, "groupname":string, "depositgef":Decimal, "limitdate":date, "depositbez":Decimal, "zahldatum":date, "depositbez2":Decimal, "zahldatum2":date, "useridanlage":string, "mutdat":date, "useridmutat":string, "gastnr":int, "bemerk":string, "grpflag":bool, "activeflag":int, "resname":string, "address":string, "city":string, "sob":string, "segment":string, "tot_revenue_resv":Decimal})
    ...
    def count_total_reservation(resnr:int):
        ...
        nonlocal curr_select, res_listmain_list, res_listmember_list, vip_nr, res_bemerk, loopk, htparam, zimkateg, res_line, guest, reservation, segment, sourccod, reslin_queasy, guestseg
        nonlocal case_type, from_date, to_date, gastnr, search_resno, search_voucher
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            revenue_list_list = get_output(view_staycostbl(0, resnr, res_line.reslinnr, ""))
