...
def prepare_banquet_mealsbl(resnr:int, reslinno:int):
    ...
    venue_list_list, Venue_list = create_model("Venue_list", {"venuecode":string, "venue":string, "pax":int})
    meal_list_list, Meal_list = create_model("Meal_list", {"nr":int, "meals":string, "times":string, "venue":string, "pax":int, "setup":string})
    event_list_list, Event_list = create_model("Event_list", {"eventcode":string, "eventname":string})
    setup_list_list, Setup_list = create_model("Setup_list", {"setupcode":string, "setupname":string})
