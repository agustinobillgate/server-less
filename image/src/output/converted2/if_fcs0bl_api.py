...
def if_fcs0bl_api(p_type:int, p_status:string):
    ...
    tt_data_list, Tt_data = create_model("Tt_data", {"date_if":date, "resno":int, "roomno":string, "roomtype":string, "roomstatus":string, "guestname":string, "arrival":date, "departure":date, "status_send":string, "parameters":string, "flags":string, "irecid":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal v_nebenstelle, tt_data_list, interface, res_line, guest, zimmer
        nonlocal p_type, p_status


        nonlocal tt_data
        nonlocal tt_data_list

        return {"tt-data": tt_data_list}
