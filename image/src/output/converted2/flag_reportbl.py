...
def flag_reportbl(case_type:int, n:int, resnr:int, reslinnr:int, user_init:str, s_list_list:[S_list]):
    i:int = 0
    k:int = 0
    ...
    if case_type == 1:
        ...
        if not s_list:
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                     (func.lower(Reslin_queasy.key) == ("flag").lower()) & (Reslin_queasy.resnr == resnr) & (Reslin_queasy.reslinnr == reslinnr) & (Reslin_queasy.betriebsnr == n)).first()
