...
def glores_review_webbl(pvilanguage:int, delflag:bool, from_date:date, to_date:date, ci_date:date, resflag1:bool, gflag:bool, cflag:bool, from_name:string, to_name:string):
    ...
    sum_list_list, Sum_list = create_model("Sum_list", {"datum":date, "gastnr":int, "firma":string, "kontcode":string, "zikatnr":int, "kurzbez":string, "erwachs":int, "kind1":int, "gloanz":int, "gresanz":int, "resanz":int, "resnrstr":string})
    output_list_list, Output_list = create_model("Output_list", {"str":string})
    str_detail_list, Str_detail = create_model("Str_detail", {"company_name":string, "address":string, "city":string, "glorescode":string, "pricecode":string, "start_date":string, "pax":string, "ending_date":string, "rmcat":string, "argt":string, "id":string, "chgid":string, "create_date":string, "comments":string, "dates_period":string, "dates_number":[string,31], "dates_str":[string,31], "reserved_room":[string,31], "used_reservation":[string,31], "not_used":[string,31], "avail_room":[string,31], "overbooking":[string,31], "reservations":string, "residents":string, "cancel_res":string})
    str_summary_list, Str_summary = create_model("Str_summary", {"total_reserve":[string,31], "used_reserve":[string,31], "not_used":[string,31]})
    k_list_list, K_list = create_model("K_list", {"gastnr":int, "bediener_nr":int, "kontcode":string, "ankunft":date, "zikatnr":int, "argt":string, "zimmeranz":[int,31], "erwachs":int, "kind1":int, "ruecktage":int, "overbooking":int, "abreise":date, "useridanlage":string, "resdate":date, "bemerk":string})
    res_list_list, Res_list = create_model("Res_list", {"flag":string, "count":int, "s1":string})
    ...
    def create_alist():
        ...
        kontline = Kontline()
        guest = Guest()
        for kontline.gastnr, kontline.kontcode, kontline.bediener_nr, kontline.ankunft, kontline.zikatnr, kontline.arrangement, kontline.erwachs, kontline.kind1, kontline.ruecktage, kontline.overbooking, kontline.abreise, kontline.useridanlage, kontline.resdat, kontline.bemerk, kontline.zimmeranz, kontline._recid, guest.name, guest.anredefirma, guest.adresse1, guest.adresse2, guest.wohnort, guest.plz, guest.gastnr, guest._recid in db_session.query(Kontline.gastnr, Kontline.kontcode, Kontline.bediener_nr, Kontline.ankunft, Kontline.zikatnr, Kontline.arrangement, Kontline.erwachs, Kontline.kind1, Kontline.ruecktage, Kontline.overbooking, Kontline.abreise, Kontline.useridanlage, Kontline.resdat, Kontline.bemerk, Kontline.zimmeranz, Kontline._recid, Guest.name, Guest.anredefirma, Guest.adresse1, Guest.adresse2, Guest.wohnort, Guest.plz, Guest.gastnr, Guest._recid).join(Guest,(Guest.gastnr == Kontline.gastnr) & (Guest.name >= (from_name).lower()) & (Guest.name <= (to_name).lower())).filter(
