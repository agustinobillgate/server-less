...
def cr_fproduct_showdetail_webbl(v_key:str, v_value:str, cardtype:int, stattype:int, fr_date:date, to_date:date, excl_comp:bool, vhp_limited:bool, scin:bool):
    ...
    if v_key.lower()  == ("SOB").lower() :
        ...
        sourccod = db_session.query(Sourccod).filter(
                 (func.lower(Sourccod.bezeich) == (v_value).lower())).first()
