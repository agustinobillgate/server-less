...
def netsuite_setup_schedullerbl(case_type:int, month_val:int, week:int, send_date:date, from_date:date, to_date:date):
    ...
    if_list_list, If_list = create_model("If_list", {"month_val":int, "month_str":string, "week":int, "send_date":date, "fr_date":date, "to_date":date, "sendflag":bool, "resendflag":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal if_list_list, queasy
        nonlocal case_type, month_val, week, send_date, from_date, to_date


        nonlocal if_list
        nonlocal if_list_list
