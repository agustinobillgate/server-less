...
def mbar_report_journal_list_1bl(from_date:date, to_date:date, curr_dept:int, long_digit:bool):
    ...
    output_list_list, Output_list = create_model("Output_list", {"rechnr":int, "dept":int, "datum":date, "str":string})
    roomtransreportlist_list, Roomtransreportlist = create_model("Roomtransreportlist", {"rechnr":int, "dept":int, "datum":date, "rmno":string, "gname":string, "bezeich":string, "saldo":Decimal, "foreign":Decimal, "zeit":string, "id":string, "tb":string})
    ...
    roomtransreportlist_list.clear()
