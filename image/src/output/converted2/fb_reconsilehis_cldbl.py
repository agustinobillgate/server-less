...
from models import Htparam, H_artikel, L_besthis, Gl_acct, L_lager, L_ophis, L_ophhis, H_compli, Exrate, Artikel, Gl_main, H_cost, L_artikel, Hoteldpt, Umsatz
...
def fb_reconsilehis_cldbl(pvilanguage:int, from_grp:int, food:int, bev:int, from_date:date, to_date:date, ldry:int, dstore:int, double_currency:bool, foreign_nr:int, exchg_rate:Decimal, mi_opt_chk:bool, date1:date, date2:date):
    ...
    prepare_cache ([Htparam, H_artikel, L_besthis, Gl_acct, L_lager, L_ophis, H_compli, Exrate, Artikel, Gl_main, H_cost, Hoteldpt, Umsatz])
    ...
    htparam = h_artikel = l_besthis = gl_acct = l_lager = l_ophis = l_ophhis = h_compli = exrate = artikel = gl_main = h_cost = l_artikel = hoteldpt = umsatz = None

    output_list = s_list = None

    output_list_list, Output_list = create_model("Output_list", {"nr":int, "code":int, "bezeich":string, "s":string})
    ...
    def generate_output():
        nonlocal done, output_list_list, lvcarea, type_of_acct, counter, curr_nr, curr_reihe, coa_format, betrag, long_digit, htparam, h_artikel, l_besthis, gl_acct, l_lager, l_ophis, l_ophhis, h_compli, exrate, artikel, gl_main, h_cost, l_artikel, hoteldpt, umsatz
    ...
    def create_list():
        ...
        l_oh = L_besthis()
        l_lager = L_lager()
        for l_besthis.anz_anf_best, l_besthis.anz_eingang, l_besthis.anz_ausgang, l_besthis.val_anf_best, l_besthis.artnr, l_besthis._recid, l_oh.anz_anf_best, l_oh.anz_eingang, l_oh.anz_ausgang, l_oh.val_anf_best, l_oh.artnr, l_oh._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid in db_session.query(L_besthis.anz_anf_best, L_besthis.anz_eingang, L_besthis.anz_ausgang, L_besthis.val_anf_best, L_besthis.artnr, L_besthis._recid, L_oh.anz_anf_best, L_oh.anz_eingang, L_oh.anz_ausgang, L_oh.val_anf_best, L_oh.artnr, L_oh._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid).join(L_oh,(L_oh.anf_best_dat == from_date) & (L_oh.lager_nr == 0) & (L_oh.artnr == L_besthis.artnr)).join(L_lager,(L_lager.lager_nr == L_besthis.lager_nr)).filter(
                 (L_besthis.anf_best_dat == from_date) & (L_besthis.artnr <= 2999999)).order_by(L_besthis._recid).all():
        ...
        l_ophis = L_ophis()
        l_lager = L_lager()
        for l_ophis.lscheinnr, l_ophis.warenwert, l_ophis.fibukonto, l_ophis.artnr, l_ophis.anzahl, l_ophis._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid in db_session.query(L_ophis.lscheinnr, L_ophis.warenwert, L_ophis.fibukonto, L_ophis.artnr, L_ophis.anzahl, L_ophis._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid).join(L_lager,(L_lager.lager_nr == L_lager.lager_nr)).filter(
                 (L_ophis.datum >= from_date) & (L_ophis.datum <= to_date) & (L_ophis.artnr <= 2999999) & (L_ophis.op_art == 1) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.lscheinnr).all():
        ...
        l_ophis_obj_list = {}
        for l_ophis, l_lager, l_ophhis, gl_acct in db_session.query(L_ophis, L_lager, L_ophhis, Gl_acct).join(L_lager,(L_lager.lager_nr == L_ophis.lager_nr)).join(L_ophhis,(L_ophhis.lscheinnr == L_ophis.lscheinnr) & (L_ophhis.op_typ == ("STT").lower())).join(Gl_acct,(Gl_acct.fibukonto == L_ophis.fibukonto)).filter(
                 (L_ophis.datum >= from_date) & (L_ophis.datum <= to_date) & (L_ophis.artnr <= 2999999) & (L_ophis.op_art == 3) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.lscheinnr).all():
            if l_ophis_obj_list.get(l_ophis._recid):
                continue
            else:
                l_ophis_obj_list[l_ophis._recid] = True
        ...
        for h_compli.datum, h_compli.departement, h_compli.artnr, h_compli.anzahl, h_compli.epreis, h_compli._recid, h_art.artnrfront, h_art.departement, h_art.prozent, h_art._recid in db_session.query(H_compli.datum, H_compli.departement, H_compli.artnr, H_compli.anzahl, H_compli.epreis, H_compli._recid, H_art.artnrfront, H_art.departement, H_art.prozent, H_art._recid).join(H_art,(H_art.departement == H_compli.departement) & (H_art.artnr == H_compli.p_artnr) & (H_art.artart == 11)).filter(
                 (H_compli.datum >= from_date) & (H_compli.datum <= to_date) & (H_compli.departement != ldry) & (H_compli.departement != dstore) & (H_compli.betriebsnr == 0)).order_by(H_compli.rechnr).all():
                ...
                for l_ophis.lscheinnr, l_ophis.warenwert, l_ophis.fibukonto, l_ophis.artnr, l_ophis.anzahl, l_ophis._recid, gl_acct.fibukonto, gl_acct.bezeich, gl_acct.acc_type, gl_acct.main_nr, gl_acct._recid in db_session.query(L_ophis.lscheinnr, L_ophis.warenwert, L_ophis.fibukonto, L_ophis.artnr, L_ophis.anzahl, L_ophis._recid, Gl_acct.fibukonto, Gl_acct.bezeich, Gl_acct.acc_type, Gl_acct.main_nr, Gl_acct._recid).join(Gl_acct,(Gl_acct.fibukonto == L_ophis.fibukonto)).filter(
                    ...
                    if fibukonto.lower()  == (food_bev).lower() :
                        ...
                        if mi_opt_chk == False:
                            ...
                            s_list = query(s_list_list, filters=(lambda s_list: s_list.fibukonto.lower()  == (fibukonto).lower()  and s_list.reihenfolge == 1 and s_list.flag == 5), first=True)
