...
def s_stockout_read_request_recordsbl(rec_id:int, out_type:int, t_lschein:string, t_datum:date, user_init:string, t_amount:Decimal):
    ...
    t_l_ophdr_list, T_l_ophdr = create_model_like(L_ophdr, {"rec_id":int, "sr_remark":string})
    out_list_list, Out_list = create_model("Out_list", {"artnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal lscheinnr, cost_acct, lager_bezeich, lager_bez1, curr_lager, to_stock, to_stock_ro, t_l_ophdr_list, op_list_list, l_op, l_ophdr, bediener, l_artikel, gl_acct, l_lager, queasy
        ...
        return {"t_amount": t_amount, "lscheinnr": lscheinnr, "cost_acct": cost_acct, "lager_bezeich": lager_bezeich, "lager_bez1": lager_bez1, "curr_lager": curr_lager, "to_stock": to_stock, "to_stock_ro": to_stock_ro, "t-l-ophdr": t_l_ophdr_list, "op-list": op_list_list}
