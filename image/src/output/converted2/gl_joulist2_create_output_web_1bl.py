...
def gl_joulist2_create_output_web_1bl(idflag:string, out_list_list:[Out_list]):
    ...
    if pqueasy:
        ...
        tqueasy = db_session.query(Tqueasy).filter(
                 (Tqueasy.key == 285) & (Tqueasy.char1 == ("General Ledger").lower()) & (Tqueasy.number1 == 1) & (Tqueasy.char2 == idflag)).first()
