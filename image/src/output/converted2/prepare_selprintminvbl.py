...
def prepare_selprintminvbl(printer_nr:int, briefnr:int):
    ...
    s_list_list, S_list = create_model("S_list", {"ind":int, "nr":int, "bezeich":string}, {"ind": 1})
    b1_list_list, B1_list = create_model("B1_list", {"nr":int, "position":string, "path":string, "make":string})
    ...
    def generate_output():
        ...
        return {"printer_nr": printer_nr, "briefnr": briefnr, "n1": n1, "rate": rate, "usr_pr_defined": usr_pr_defined, "found": found, "briefnr2": briefnr2, "flag": flag, "b1-list": b1_list_list, "s-list": s_list_list}
    ...
    for printer in db_session.query(Printer).filter(
             (Printer.bondrucker == False) & (Printer.opsysname == ("DOS").lower())).order_by(Printer._recid).all():
        user_printers = query(user_printers_list, (lambda user_printers: user_printers.nr == printer.nr and user_printers.selected), first=True)
