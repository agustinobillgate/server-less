...
def check_esignature1_webbl(case_type:int, user_init:str, docu_nr:str, flag_type:str):
    found_flag = False
    esign_list_list = []
    esign_print_list = []
    ...
    if case_type == 1:
        ...
        if flag_type.lower()  == ("PR").lower() :
            ...
            guestbook = db_session.query(Guestbook).filter(
                     (Guestbook.gastnr >= -271150) & (Guestbook.gastnr <= -271080) & (func.lower(Guestbook.userinit) == (user_init).lower()) & (Guestbook.reserve_logic[inc_value(0))]).first()
            ...
            guestbook = db_session.query(Guestbook).filter(
                     (Guestbook.gastnr >= -271150) & (Guestbook.gastnr <= -271080) & (func.lower(Guestbook.userinit) == (user_init).lower()) & (Guestbook.reserve_logic[inc_value(1))]).first()
        ...
        if flag_type.lower()  == ("PR").lower() :
            ...
            for queasy in db_session.query(Queasy).filter(
                     (Queasy.key == 227) & (func.lower(Queasy.char1) == (docu_nr).lower())).order_by(Queasy._recid).all():
