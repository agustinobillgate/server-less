...
def check_esignature1_webbl(case_type:int, user_init:string, docu_nr:string, flag_type:string):
    ...
    reorder_esign_print_list, Reorder_esign_print = create_model_like(Esign_print)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal found_flag, esign_list_list, esign_print_list, i, guestbook, queasy
        nonlocal case_type, user_init, docu_nr, flag_type


        nonlocal esign_list, esign_print, reorder_esign_print
        nonlocal esign_list_list, esign_print_list, reorder_esign_print_list
    ...
    if case_type == 1:
        ...
        if flag_type.lower()  == ("PR").lower() :
            ...
            for i in range(1,4 + 1) :
                ...
                esign_print = query(esign_print_list, filters=(lambda esign_print: esign_print.sign_nr == i), first=True)

                if esign_print:
                    reorder_esign_print = Reorder_esign_print()
                    reorder_esign_print_list.append(reorder_esign_print)
            ...
            esign_print_list.clear()

            for reorder_esign_print in query(reorder_esign_print_list):
                esign_print = Esign_print()
                esign_print_list.append(esign_print)
