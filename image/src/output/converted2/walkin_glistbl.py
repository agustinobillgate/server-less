...
def walkin_glistbl(pvilanguage:int, arrival_flag:bool, walkin_flag:bool, sameday_flag:bool, from_date:date, to_date:date, ci_date:date, walk_in:int, wi_grp:int):
    ...
    walkin_glist_list, Walkin_glist = create_model("Walkin_glist", {"datum":date, "zinr":string, "name":string, "rsv_name":string, "zimmeranz":int, "pax":int, "ankunft":date, "abreise":date, "segm":string, "zipreis":string, "curr":string, "rstatus":string, "rec_id":int})
    ...
    def create_list():
        ...
        for walkin_glist in query(walkin_glist_list, filters=(lambda walkin_glist: matches(walkin_glist.name,r"Total") or matches(walkin_glist.name,r"Total Same day Rsv"))):
