...
def prepare_connect_groupbl(resno:int, reslinno:int):
    ...
    def generate_output():
        nonlocal res_list_list, mainres_list_list, guest_list_list, t_reservation_list, t_guest_list, res_line, reservation, guest
        nonlocal resno, reslinno


        nonlocal res_list, mainres_list, guest_list, t_reservation, t_res_line, t_guest
        nonlocal res_list_list, mainres_list_list, guest_list_list, t_reservation_list, t_res_line_list, t_guest_list

        return {"res-list": res_list_list, "mainres-list": mainres_list_list, "guest-list": guest_list_list, "t-reservation": t_reservation_list, "t-guest": t_guest_list}


    if resno != 0:
        res_list_list, mainres_list_list, guest_list_list = get_output(connect_groupbl(1, resno, reslinno, None, None, None, None))
        t_reservation_list = get_output(read_reservationbl(1, resno, None, ""))

        t_reservation = query(t_reservation_list, first=True)

        if t_reservation:
            t_res_line_list = get_output(read_res_linebl(1, resno, reslinno, 0, 0, "", None, None, 0, 0, ""))

            t_res_line = query(t_res_line_list, first=True)

            if t_res_line:
                t_guest_list = get_output(read_guestbl(1, t_reservation.gastnr, "", ""))
