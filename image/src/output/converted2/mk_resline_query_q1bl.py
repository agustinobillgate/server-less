...
def mk_resline_query_q1bl(pvilanguage:int, newflag:bool, chgnameflag:bool, inp_resno:int, rline_list_list:[Rline_list], reschanged_list_list:[Reschanged_list]):
    ...
    def generate_output():
        ...
        return {"rline-list": rline_list_list, "reschanged-list": reschanged_list_list}
    ...
    if newflag:
        ...
        for rline in db_session.query(Rline).filter(
            ...
            rline_list = query(rline_list_list, filters=(lambda rline_list: rline_list.resnr == rline.resnr and rline_list.reslinnr == rline.reslinnr), first=True)
            ...
            reschanged_list = query(reschanged_list_list, filters=(lambda reschanged_list: reschanged_list.reslinnr == rline_list.reslinnr), first=True)
    ...
    if chgnameflag:
        ...
        for rline_list in query(rline_list_list):
