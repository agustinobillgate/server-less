...
def pos_dashboard_postorder_webbl(od_cancel_list_list:[Od_cancel_list], post_order_list:[Post_order], menu_list_list:[Menu_list], case_type:int, user_init:string, cancel_str:string, post_curr_dept:int, post_order_no:int, post_tischnr:int, post_session_param:string, post_language_code:int, post_bill_recid:int, post_gname:string, post_pax:int, post_guestnr:int, post_curr_room:string, post_resnr:int, post_reslinnr:int):
    post_mess_str = ""
    post_bill_number = 0
    ...
    if case_type == 1:
        ...
        get_output(pos_dashboard_cancel_reasonbl(od_cancel_list_list, user_init, cancel_str))
        ...
        post_mess_str = get_output(pos_dashboard_cancel_orderbl(menu_list_list, post_curr_dept, post_order_no, post_tischnr, post_session_param))
    else:
        get_output(pos_dashboard_cancel_reasonbl(od_cancel_list_list, user_init, cancel_str))
        post_bill_number, post_mess_str = get_output(pos_dashboard_post_menubl(post_language_code, post_bill_recid, post_tischnr, post_curr_dept, user_init, post_gname, post_pax, post_guestnr, post_curr_room, post_resnr, post_reslinnr, post_session_param, post_order_no, menu_list_list))
