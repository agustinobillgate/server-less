...
kds_param_list, Kds_param = create_model("Kds_param", {"nr":int, "param_name":string, "param_value":string})

def kitchen_display_configbl(case_type:int, kds_param_list:[Kds_param]):
    ...
    def generate_output():
        ...
        return {"kds-param": kds_param_list}
    ...
    def create_queasy():
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 320)],"number1": [(eq, paramnr)]})
    ...
    if case_type == 1:
        ...
        for kds_param in query(kds_param_list):
    ...
    if case_type == 2:
        ...
        if queasy.char2 == "":
            kds_param = Kds_param()
            kds_param_list.append(kds_param)
