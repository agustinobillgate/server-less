...
def fb_detailreportbl(from_dept:int, to_dept:int, from_date:date, to_date:date):
    ...
    output_list_list, Output_list = create_model("Output_list", {"departement":int, "datum":date, "zeit":string, "artnr":int, "artbezeich":string, "bfastqty":int, "bfastamount":Decimal, "lunchqty":int, "lunchamount":Decimal, "dinnerqty":int, "dinneramount":Decimal, "supperqty":int, "supperamount":Decimal, "totqty":int, "totamount":Decimal})
    output_list2_list, Output_list2 = create_model("Output_list2", {"departement":string, "datum":string, "zeit":string, "artnr":string, "artbezeich":string, "bfastqty":string, "bfastamount":string, "lunchqty":string, "lunchamount":string, "dinnerqty":string, "dinneramount":string, "supperqty":string, "supperamount":string, "totqty":string, "totamount":string})
    ...
    for h_bill_line in db_session.query(H_bill_line).filter(
        ...
        for output_list in query(output_list_list, filters=(lambda output_list: output_list.departement == curr_dept)):
    ...
    for output_list in query(output_list_list, filters=(lambda output_list: output_list.artbezeich.lower()  == ("TOTAL").lower())):
