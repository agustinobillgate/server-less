...
def view_reservationbl(resnr:int):
    ...
    s_list_list, S_list = create_model("S_list", {"flag":int, "pos":int, "s_ankunft":string, "ankunft":date, "abreise":date, "rmcat":string, "zimmeranz":int, "erwachs":int, "gratis":int, "kind1":int, "kind2":int, "str":string})
    ...
    def cal_revenue():
        ...
        for s_list in query(s_list_list, filters=(lambda s_list: s_list.flag == 0), sort_by=[("pos",False)]):
