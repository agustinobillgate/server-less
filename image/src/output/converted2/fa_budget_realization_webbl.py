...
def fa_budget_realization_webbl(payload_list_list:[Payload_list]):

    prepare_cache ([Queasy, Fa_op, Fa_artikel, Mathis, Fa_ordheader, Fa_order])

    fa_budget_realization_list = []
    count_i:int = 0
    period_qty:int = 0
    start_jan:date = None
    ...
    def generate_output():
        nonlocal fa_budget_realization_list, count_i, period_qty, start_jan, period_amount, period_budget, period_variance, nr_budget, fa_artnr, grand_total_qty, grand_total_amount, grand_total_budget, grand_total_variance, tot_qty_item, tot_price_item, tot_amount_item, queasy, fa_op, fa_artikel, mathis, fa_ordheader, fa_order


        nonlocal fa_budget_realization, fix_asset_list, payload_list
        nonlocal fa_budget_realization_list, fix_asset_list_list

        return {"fa-budget-realization": fa_budget_realization_list}
        ...
        for fa_order, fa_op, fa_artikel, mathis, fa_ordheader in db_session.query(Fa_order, Fa_op, Fa_artikel, Mathis, Fa_ordheader).join(Fa_op,(Fa_op.loeschflag <= 1) & (Fa_op.opart == 1) & (Fa_op.anzahl > 0) & (Fa_op.docu_nr == Fa_order.Order_Nr) & (Fa_op.datum >= payload_list.from_date) & (Fa_op.datum <= payload_list.to_date)).join(Fa_artikel,(Fa_artikel.nr == Fa_op.nr)).join(Mathis,(Mathis.nr == Fa_op.nr)).join(Fa_ordheader,(Fa_ordheader.order_nr == Fa_op.docu_nr)).filter(
            ...
            if fa_order.ActiveReason.lower()  != (nr_budget).lower() :
                ...
                if nr_budget != "":
                    ...
                    fa_budget_realization_list.append(fa_budget_realization)
