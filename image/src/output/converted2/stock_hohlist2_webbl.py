...
from models import Htparam, L_artikel, L_besthis, L_lager, L_ophis, L_untergrup
...
def stock_hohlist2_webbl(from_lager:int, to_lager:int, from_art:int, to_art:int, to_date:date, move_art:bool, end_notzero:bool, global_oh:bool, from_date:date):
    ...
    prepare_cache ([Htparam, L_artikel, L_besthis, L_lager, L_ophis, L_untergrup])
    ...
    htparam = l_artikel = l_besthis = l_lager = l_ophis = l_untergrup = None

    stockoh_list = art_bestand = t_vkpreis = tot_t_vkpreis = None

    stockoh_list_list, Stockoh_list = create_model("Stockoh_list", {"artnr":string, "bezeich":string, "prevqty":string, "prevval":string, "incoming":string, "outgoing":string, "actqty":string, "actval":string, "avrg_pr":string, "incomingval":string, "outgoingval":string})
    art_bestand_list, Art_bestand = create_model("Art_bestand", {"lager":int, "artnr":int, "zwkum":int, "bezeich":string, "incoming":Decimal, "in_val":Decimal, "out_val":Decimal, "outgoing":Decimal, "prevqty":Decimal, "prevval":Decimal, "adjust":Decimal, "actqty":Decimal, "actval":Decimal, "avrg_pr":Decimal, "vk_preis":Decimal})
    t_vkpreis_list, T_vkpreis = create_model("T_vkpreis", {"lager":int, "artnr":int, "zwkum":int, "bezeich":string, "qty_end":Decimal, "val_end":Decimal, "vk_preis":Decimal})
    tot_t_vkpreis_list, Tot_t_vkpreis = create_model("Tot_t_vkpreis", {"lager":int, "artnr":int, "tot_val":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal stockoh_list_list, do_it, curr_art, curr_lager, out_bestand, zwkum, t_val, bezeich, j, tot_prev, tot_value, tot_in, tot_out, total_in, total_out, total_prev, total_value, start_from_date, loop, loop_date, diff_month, long_digit, htparam, l_artikel, l_besthis, l_lager, l_ophis, l_untergrup
        ...
        nonlocal stockoh_list_list, art_bestand_list, t_vkpreis_list, tot_t_vkpreis_list
    ...
    def create_stock1():
        ...
        for l_besthis.artnr, l_besthis.anz_anf_best, l_besthis.anz_eingang, l_besthis.anz_ausgang, l_besthis.val_anf_best, l_besthis.wert_eingang, l_besthis.wert_ausgang, l_besthis.lager_nr, l_besthis._recid, l_artikel.vk_preis, l_artikel.zwkum, l_artikel.bezeich, l_artikel._recid in db_session.query(L_besthis.artnr, L_besthis.anz_anf_best, L_besthis.anz_eingang, L_besthis.anz_ausgang, L_besthis.val_anf_best, L_besthis.wert_eingang, L_besthis.wert_ausgang, L_besthis.lager_nr, L_besthis._recid, L_artikel.vk_preis, L_artikel.zwkum, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_besthis.artnr)).filter(
            ...
            t_vkpreis = query(t_vkpreis_list, filters=(lambda t_vkpreis: t_vkpreis.artnr == l_besthis.artnr), first=True)
            ...
            if t_vkpreis:
                ...
                t_vkpreis = T_vkpreis()
                t_vkpreis_list.append(t_vkpreis)
        ...
        l_besthis = L_besthis()
        l_lager = L_lager()
        for l_besthis.artnr, l_besthis.anz_anf_best, l_besthis.anz_eingang, l_besthis.anz_ausgang, l_besthis.val_anf_best, l_besthis.wert_eingang, l_besthis.wert_ausgang, l_besthis.lager_nr, l_besthis._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid in db_session.query(L_besthis.artnr, L_besthis.anz_anf_best, L_besthis.anz_eingang, L_besthis.anz_ausgang, L_besthis.val_anf_best, L_besthis.wert_eingang, L_besthis.wert_ausgang, L_besthis.lager_nr, L_besthis._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid).join(L_lager,(L_lager.lager_nr == L_besthis.lager_nr)).filter(
                 (L_besthis.lager_nr >= from_lager) & (L_besthis.lager_nr <= to_lager) & (L_besthis.anf_best_dat == start_from_date) & (L_besthis.artnr >= from_art) & (L_besthis.artnr <= to_art)).order_by(L_besthis._recid).all():
            if l_besthis_obj_list.get(l_besthis._recid):
                continue
            else:
                l_besthis_obj_list[l_besthis._recid] = True
        ...
        for l_ophis in db_session.query(L_ophis).filter(
                 (L_ophis.lager_nr >= from_lager) & (L_ophis.lager_nr <= to_lager) & (L_ophis.artnr >= from_art) & (L_ophis.artnr <= to_art) & ((L_ophis.datum >= start_from_date) & (L_ophis.datum <= to_date)) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.lager_nr, L_ophis.artnr, L_ophis.datum).all():

            art_bestand = query(art_bestand_list, filters=(lambda art_bestand: art_bestand.artnr == l_ophis.artnr and art_bestand.lager == l_ophis.lager_nr), first=True)
        ...
        for l_ophis in db_session.query(L_ophis).filter(
                 (L_ophis.lager_nr >= from_lager) & (L_ophis.lager_nr <= to_lager) & (L_ophis.artnr >= from_art) & (L_ophis.artnr <= to_art) & ((L_ophis.datum >= start_from_date) & (L_ophis.datum <= from_date)) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.datum).all():
            ...
            if art_bestand:
                ...
                if l_ophis.op_art == 1 or l_ophis.op_art == 2:
                    art_bestand.incoming =  to_decimal(art_bestand.incoming) - to_decimal(l_ophis.anzahl)
                    art_bestand.in_val =  to_decimal(art_bestand.in_val) - to_decimal(l_ophis.anzahl) * to_decimal((art_bestand.vk_preis) / to_decimal((diff_month) + to_decimal(1)))
                    art_bestand.prevqty =  to_decimal(art_bestand.prevqty) + to_decimal(l_ophis.anzahl)
                ...
                if (move_art and end_notzero) and (art_bestand.incoming != 0 or art_bestand.outgoing != 0 or art_bestand.actqty != 0):
                    ...
                    if art_bestand.avrg_pr == 0:
                        ...
                        if art_bestand.actqty != 0:
                            ...
                            t_vkpreis = query(t_vkpreis_list, filters=(lambda t_vkpreis: t_vkpreis.artnr == art_bestand.artnr), first=True)
    ...
    def create_stock2():
        ...
        for art_bestand in query(art_bestand_list):
            ...
            if t_vkpreis:
                ...
                for l_ophis in db_session.query(L_ophis).filter(
                         (L_ophis.artnr == art_bestand.artnr) & ((L_ophis.datum >= start_from_date) & (L_ophis.datum <= from_date)) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.datum).all():
        ...
        for art_bestand in query(art_bestand_list, filters=(lambda art_bestand: art_bestand.artnr >= from_art and art_bestand.artnr <= to_art), sort_by=[("zwkum",False),("bezeich",False)]):
