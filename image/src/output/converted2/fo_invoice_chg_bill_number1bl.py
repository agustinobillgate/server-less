...
def fo_invoice_chg_bill_number1bl(bil_recid:int, curr_billnr:int):
    ...
    bill = db_session.query(Bill).filter(
             (Bill.resnr == bill1.resnr) & (Bill.parent_nr == bill1.parent_nr) & (Bill.flag == 0) & (Bill.zinr == bill1.zinr) & (Bill.billnr == curr_billnr)).first()

    if not bill:

        bill = db_session.query(Bill).filter(
                 (Bill.resnr == bill1.resnr) & (Bill.parent_nr == bill1.parent_nr) & (Bill.flag == 0) & (Bill.billnr == curr_billnr)).first()
