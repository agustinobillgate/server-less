...
def bfast_card_reportbl(room:str, fdate:date, mealtime:str, dummyinput:str):
            ...
            for argt_line in db_session.query(Argt_line).filter(
                ...
                if argt_line.argt_artnr == artgrp_abf and mealtime.lower()  == ("Breakfast").lower() :
                    ...
                    for j in range(1,num_entries(res_line.zimmer_wunsch, ";")  + 1) :
                        ...
                        if re.match(r".*ChAge.*",entry(j - 1, res_line.zimmer_wunsch, ";"), re.IGNORECASE):
    ...
    if fdate == None:
        ...
        for mealcoup in db_session.query(Mealcoup).filter(
                 (Mealcoup.name == mealtime) & (func.lower(Mealcoup.zinr).op("~")((("*" + room + "*".lower().replace("*",".*")))))).order_by(Mealcoup._recid).all():
        ...
        for mealcoup in db_session.query(Mealcoup).filter(
                 (Mealcoup.name == mealtime) & (fdate > Mealcoup.ankunft) & (fdate <= Mealcoup.abreise) & (func.lower(Mealcoup.zinr).op("~")((("*" + room + "*".lower().replace("*",".*")))))).order_by(Mealcoup._recid).all():
