...
def history_listbl(from_name:string, zinr:string, disptype:int, sorttype:int, all_flag:bool, f_date:date, t_date:date):
    ...
    def disp_itb():
        ...
        for h_list in query(h_list_list, sort_by=[("ind",False)]):
            i_list = query(i_list_list, (lambda i_list: i_list.s_recid == to_int(h_list._recid)), first=True)
            if not i_list:
                continue
