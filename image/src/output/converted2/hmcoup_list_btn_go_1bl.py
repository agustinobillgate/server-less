...
def hmcoup_list_btn_go_1bl(double_currency:bool, foreign_nr:int, exchg_rate:Decimal, billdate:date, from_dept:int, to_dept:int, from_date:date, to_date:date):
    ...
    def journal_list():
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            for h_list in query(h_list_list, filters=(lambda h_list: h_list.datum >= from_date and h_list.datum <= to_date and h_list.departement == hoteldpt.num), sort_by=[("rechnr",False)]):
