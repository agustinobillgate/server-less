...
t_param_list, T_param = create_model("T_param", {"number":int, "bezeich":string, "val":string})

def odx_setupbl(case_type:int, user_init:string, t_param_list:[T_param]):
    ...
    if case_type == 1:
        ...
        for t_param in query(t_param_list, sort_by=[("number",False)]):
