...
def mk_resline_leave_argbl(pvilanguage:int, res_mode:string, old_arg:string, contcode:string, curr_arg:string, fixed_rate:bool, ebdisc_flag:bool, kbdisc_flag:bool, rate_readonly:bool, bookdate:date, reslin_list_list:[Reslin_list]):
    ...
    def leave_argt():
        ...
        if curr_arg.lower()  != (old_arg).lower() :
            ...
            if (res_mode.lower()  == ("new").lower()  or res_mode.lower()  == ("insert").lower()):
                restricted_disc, new_rate, rate_tooltip = get_output(mk_resline_set_ratebl(False, fixed_rate, ebdisc_flag, kbdisc_flag, rate_readonly, reslin_list.gastnr, res_mode, curr_arg, contcode, bookdate, reslin_list_list))
        else:
