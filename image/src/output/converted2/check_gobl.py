...
def check_gobl(pvilanguage:int, user_init:str, gastno:int, res_mode:str, curr_segm:str, curr_source:str, currency:str, zikat_screen:str, memo_zinr:str, guestname:str, origcontcode:str, contcode:str, marknr:int, rm_bcol:int, inactive_flag:bool, reslin_list_list:[Reslin_list], prev_resline_list:[Prev_resline], zikatstr:str):
    error_number = 0
    still_error = True
    msg_str = ""
    pswd_str = ""
    ...
    max_adv:int = 0
    msg_str1:str = ""
    zinr_ecode:List[str] = create_empty_list(6,"")
    ...
    reslin_list = prev_resline = now_resline = None
    ...
    def generate_output():
        ...
        nonlocal reslin_list, prev_resline, now_resline
    ...
    def check_go():
        ...
        if max_rate != 0 and reslin_list.zipreis > 0:
            ...
            if reslin_list.betriebsnr > 0:
                ...
                waehrung = db_session.query(Waehrung).filter(
                         (Waehrung.waehrungsnr == reslin_list.betriebsnr)).first()
            else:
        ...
        if reslin_list.active_flag == 0:
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                     (func.lower(Reslin_queasy.key) == ("arrangement").lower()) & (Reslin_queasy.resnr == reslin_list.resnr) & (Reslin_queasy.reslinnr == reslin_list.reslinnr) & (Reslin_queasy.date1 <= reslin_list.ankunft) & (Reslin_queasy.date2 >= reslin_list.ankunft)).first()
        ...
        if check_allotment:
            ...
            if its_wrong:
                ...
                if re.match(r".*&Q.*",not msg_str, re.IGNORECASE):
                    error_number = 29
        ...
        if zinr != "" and not sharer:
            ...
            for curr_datum in date_range(beg_datum,(abreise - 1)) :
                ...
                zimplan1 = db_session.query(Zimplan1).filter(
                         (Zimplan1.datum == curr_datum) & (func.lower(Zimplan1.zinr) == (zinr).lower())).first()
            ...
            if room_blocked:
                for curr_datum in date_range(beg_datum,(abreise - 1)) :
                    ...
                    zimplan = db_session.query(Zimplan).filter(
                             (Zimplan.datum == curr_datum) & (func.lower(Zimplan.zinr) == (zinr).lower()) & (Zimplan.res_recid == resline_recid)).first()
            ...
            if rline.resstatus == 6 and (rline.zinr.lower()  != (new_zinr).lower()):
                ...
                if res_line1:
                    ...
                    for res_line3 in db_session.query(Res_line3).filter(
                             (Res_line3.resnr == resnr) & (Res_line3.zinr == rline.zinr) & (Res_line3.resstatus == 13)).order_by(Res_line3._recid).all():
    ...
    def check_min_maxstay():
        ...
        if kontline.arrangement:
            min_stay = kontline.arrangement.intervall
