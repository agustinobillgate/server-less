    ...
    def create_ghistory():
        ...
        if vkey == 1:
            ...
            for history in db_session.query(History).filter(
                     (History.gastnr == gastnr) & (History.abreise >= htparam.fdate) & (History.abreise <= tdate)).order_by(History.abreise.desc()).all():
                ghistory = Ghistory()
            ...
            for history in db_session.query(History).filter(
                     (History.gastnr == gastnr) & (History.ankunft >= htparam.fdate) & (History.ankunft <= tdate)).order_by(History.ankunft.desc()).all():
                ghistory = Ghistory()
                ghistory_list.append(ghistory)
