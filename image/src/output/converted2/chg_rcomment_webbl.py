...
def chg_rcomment_webbl(icase:int, resno:int, reslinno:int, user_init:str, res_com:str, resl_com:str, g_com:str, web_com:str):
    str1:str = ""
    str2:str = ""
    loopi:int = 0
    loopj:int = 0
                ...
                for loopj in range(1,num_entries(str2, ",")  + 1) :
                    ...
                    if queasy:
                        for loopk in range(1,num_entries(queasy.char1, ";")  + 1) :
                            ...
                            if re.match(r".*en.*",entry(loopk - 1, queasy.char1, ";"), re.IGNORECASE):
                                web_com = entry(1, entry(loopk - 1, queasy.char1, ";") , "=") + ", " + web_com


                                return

            elif re.match(r".*PRCODE.*",str1, re.IGNORECASE):
                web_com = web_com + "PromoCode: " + entry(2, str1, "$")

        buf_q = db_session.query(Buf_q).filter(
                 (Buf_q.key == 267) & (Buf_q.number1 == resno) & (Buf_q.number2 == reslinno)).first()
