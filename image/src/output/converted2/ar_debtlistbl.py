...
def ar_debtlistbl(from_name:string, to_name:string, from_date:date, to_date:date, from_art:int, to_art:int, tot_flag:bool, lesspay:bool, show_inv:bool, case_type:int):
    ...
    output_list_list, Output_list = create_model("Output_list", {"ar_recid":int, "info":string, "wabkurz":string, "maildate":date, "inv_no":string, "datum":date, "mflag":string, "bill_no":int, "rm_no":string, "receiver":string, "saldo":Decimal, "fsaldo":Decimal, "userinit":string, "vesrcod":string, "ref_no1":int, "ref_no2":string, "ci_date":date, "co_date":date, "nights":int, "verstat":int, "selected":bool, "ref_no3":int})
    edit_list_list, Edit_list = create_model("Edit_list", {"rechnr":int, "datum":date, "zinr":string, "billname":string, "lamt":Decimal, "famt":Decimal, "fcurr":string, "ar_recid":int, "amt_change":bool, "curr_change":bool, "curr_nr":int})
    ...
    def create_list():
        ...
        for debitor.gastnr, debitor.saldo, debitor.vesrdep, debitor.counter, debitor.debref, debitor.rechnr, debitor.rgdatum, debitor.zinr, debitor.bediener_nr, debitor.verstat, debitor.vesrcod, debitor._recid, debitor.versanddat, debitor.betrieb_gastmem, debitor.betriebsnr, artikel.artnr, artikel.bezeich, artikel._recid, guest.name, guest.vorname1, guest.anredefirma, guest.anrede1, guest.firmen_nr, guest.steuernr, guest._recid in db_session.query(Debitor.gastnr, Debitor.saldo, Debitor.vesrdep, Debitor.counter, Debitor.debref, Debitor.rechnr, Debitor.rgdatum, Debitor.zinr, Debitor.bediener_nr, Debitor.verstat, Debitor.vesrcod, Debitor._recid, Debitor.versanddat, Debitor.betrieb_gastmem, Debitor.betriebsnr, Artikel.artnr, Artikel.bezeich, Artikel._recid, Guest.name, Guest.vorname1, Guest.anredefirma, Guest.anrede1, Guest.firmen_nr, Guest.steuernr, Guest._recid).join(Artikel,(Artikel.artnr == Debitor.artnr) & (Artikel.departement == 0)).join(Guest,(Guest.gastnr == Debitor.gastnr) & (Guest.name >= (from_name).lower()) & (Guest.name <= (to_name).lower())).filter(
            ...
            if curr_gastnr != debitor.gastnr:
                ...
                if tot_saldo != 0 and tot_flag:
                    ...
                    output_list.saldo =  to_decimal(tot_saldo)
                    output_list.fsaldo =  to_decimal(tf_saldo)
            ...
            if saldo != 0:
                ...
                if artnr != artikel.artnr:
                    ...
                    if artnr != 0:
                        ...
                        if tot_flag:
                            ...
                            output_list.saldo =  to_decimal(t_debit)
                            output_list.fsaldo =  to_decimal(tf_debit)
                ...
                output_list.ref_no1 = guest.firmen_nr
                ...
                output_list.bill_no = debitor.rechnr
                output_list.rm_no = debitor.zinr
                output_list.receiver = receiver
                output_list.saldo =  to_decimal(saldo)
                ...
                output_list.fsaldo =  to_decimal(fsaldo)
                    ...
                    if bill:
                        ...
                        if bill.reslinnr == 0:
                            ...
                            if res_line:
                                ...
                                output_list.nights = to_int(res_line.abreise - res_line.ankunft)
        ...
        output_list.saldo =  to_decimal(tot_debit)
        output_list.fsaldo =  to_decimal(ttf_debit)
