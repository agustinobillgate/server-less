...
def ar_subledger_create_age_list1abl(incl:bool, t_artnr:int, t_dept:int, from_name:str, to_name:str, fr_date:date, to_date:date, bdate:date):
    ...
    for debitor in db_session.query(Debitor).filter(
             (Debitor.artnr == artikel.artnr) & (func.lower(Debitor.name) >= (from_name).lower()) & (Debitor.opart <= opart) & (Debitor.rgdatum >= fr_date) & (Debitor.rgdatum <= to_date) & (Debitor.zahlkonto == 0)).order_by(Debitor._recid).all():
        do_it = True
    ...
    for debitor in db_session.query(Debitor).filter(
             (Debitor.artnr == artikel.artnr) & (func.lower(Debitor.name) >= (from_name).lower()) & (Debitor.opart >= opart1) & (Debitor.opart <= opart2) & (Debitor.rgdatum >= fr_date) & (Debitor.rgdatum <= to_date) & (Debitor.zahlkonto > 0)).order_by(Debitor._recid).all():
        do_it = True
            ...
            if debitor.rechnr != 0:
                ...
                if bill and reservation:
                    ...
                    for res_line in db_session.query(Res_line).filter(
                             (Res_line.resnr == bill.resnr) & (Res_line.reslinnr == bill.parent_nr) & (Res_line.zipreis > 0)).order_by(Res_line.reslinnr).all():
                        gname = res_line.name
