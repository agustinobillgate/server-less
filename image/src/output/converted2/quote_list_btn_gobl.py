...
t_quote_list, T_quote = create_model("T_quote", {"artnr":int, "lief_nr":int, "supname":string, "artname":string, "devunit":string, "content":Decimal, "unitprice":Decimal, "curr":string, "from_date":date, "to_date":date, "remark":string, "filname":string, "activeflag":bool, "docu_nr":string, "minqty":Decimal, "delivday":int, "disc":Decimal, "avl":bool}, {"activeflag": True, "avl": True})
t_quote1_list, T_quote1 = create_model_like(T_quote)

def quote_list_btn_gobl(pvilanguage:int, curr_type:string, user_init:string, t_quote_list:[T_quote], t_quote1_list:[T_quote1]):
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, curr_type, user_init, t_quote1_list
        nonlocal b_lquote
