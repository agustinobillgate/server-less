...
def nation_productbl(pvilanguage:int, mm:int, yy:int, sorttype:int):
    ...
    room_list_list, Room_list = create_model("Room_list", {"flag":int, "num":int, "natnr":int, "nr":int, "bezeich":string, "room":[int,12], "ytd":int, "lytd":int})
    tt_nation_list, Tt_nation = create_model_like(Nation)
    tt_natbuff_list, Tt_natbuff = create_model_like(Nation)
    ...
    def generate_output():
        ...
        nonlocal room_list_list, tt_nation_list, tt_natbuff_list
    ...
    for nation in db_session.query(Nation).filter(
        ...
        tt_nation = Tt_nation()
        tt_nation_list.append(tt_nation)
    ...
    for natbuff in db_session.query(Natbuff).filter(
        ...
        tt_natbuff_list.append(tt_natbuff)
    ...
    for natstat1 in db_session.query(Natstat1).filter(
        ...
        tt_nation = query(tt_nation_list, filters=(lambda tt_nation: tt_nation.nationnr == natstat1.nationnr), first=True)

        tt_natbuff = query(tt_natbuff_list, filters=(lambda tt_natbuff: tt_natbuff.nationnr > natstat1.nationnr), first=True)
    ...
    for room_list in query(room_list_list, filters=(lambda room_list: room_list.ytd != 0 or room_list.lytd != 0), sort_by=[("ytd",True),("bezeich",False)]):
        i = i + 1
        room_list.nr = i
        room_list.num = i
