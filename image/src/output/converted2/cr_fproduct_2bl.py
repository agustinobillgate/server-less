...
from functions.create_forecast_history_2bl import create_forecast_history_2bl
import re
...
def cr_fproduct_2bl(pvilanguage:int, op_type:int, printer_nr:int, call_from:int, txt_file:string, fr_date:date, to_date:date, disptype:int, cardtype:int, stattype:int, rev_calc:int, exc_oral6pm:bool, excl_comp:bool, vhp_limited:bool, scin:bool, argt_code:string):
    ...
    prepare_cache ([Nation, Sourccod, Segment, Arrangement])
    ...
    argtcode:string = ""
    nation = sourccod = segment = arrangement = reslin_queasy = ratecode = kontplan = None
    ...
    output_list1_list, Output_list1 = create_model("Output_list1", {"flag":int, "firmen_nr":int, "refno":string, "avrg_amount":string, "name":string, "str2":string, "str4":string, "fcost":string, "name1":string, "zinr":string, "pax":string, "exp_rev":string, "curr":string, "loc_curr":string, "lodg":string, "bfast":string, "lunch":string, "dinner":string, "oth_rev":string})
    ...
    to_list_list, To_list = create_model("To_list", {"nationnr":int, "sob":int, "resnr":int, "gastnr":int, "firmen_nr":int, "refno":string, "name":string, "room":int, "pax":int, "zipreis":Decimal, "curr":string, "logis":Decimal, "rmonly":Decimal, "bfast":Decimal, "lunch":Decimal, "dinner":Decimal, "misc":Decimal, "proz":Decimal, "avrgrate":Decimal, "guest_nationnr":int, "segmentcode":int, "segmentbez":string, "fcost":Decimal, "argtcode":string})
    tot_list_list, Tot_list = create_model("Tot_list", {"curr":string, "zipreis":Decimal, "room":int, "pax":int, "logis":Decimal, "rmonly":Decimal, "bfast":Decimal, "lunch":Decimal, "dinner":Decimal, "misc":Decimal, "proz":Decimal, "fcost":Decimal})


    set_cache(Reslin_queasy, ((Reslin_queasy.date1 >= fr_date) & (Reslin_queasy.date1 <= to_date)) | (((Reslin_queasy.date2 >= fr_date) & (Reslin_queasy.date2 <= to_date))),[["key", "char1", "number1", "number2", "number3", "resnr", "reslinnr"], ["key", "resnr", "reslinnr"]], True, ["date1", "date2"],[])
    set_cache(Arrangement, None,[["arrangement"], ["argtnr"]], True, [],[])
    set_cache(Ratecode, ((Ratecode.startperiode >= fr_date) & (Ratecode.startperiode <= to_date) | (Ratecode.endperiode >= fr_date) & (Ratecode.endperiode <= to_date)),[["code", "marknr", "argtnr", "zikatnr", "wday", "erwachs", "kind1", "kind2"], ["code", "marknr", "zikatnr", "wday", "erwachs", "kind1", "kind2"], ["code", "marknr", "argtnr", "zikatnr", "wday", "erwachs"], ["code", "marknr", "zikatnr", "wday", "erwachs"]], True, ["startperiode", "endperiode"],[])
    set_cache(Kontplan, (Kontplan.datum >= fr_date) & (Kontplan.datum <= to_date),[["betriebsnr", "kontignr", "datum"]], True, [],[])

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list1_list, output_list2_list, lvcarea, report_title, tot_room, tot_pax, tot_logis, tot_bfast, tot_lunch, tot_dinner, tot_misc, tot_rmonly, tot_zipreis, tot_fcost, argtcode, nation, sourccod, segment, arrangement, reslin_queasy, ratecode, kontplan
        nonlocal pvilanguage, op_type, printer_nr, call_from, txt_file, fr_date, to_date, disptype, cardtype, stattype, rev_calc, exc_oral6pm, excl_comp, vhp_limited, scin, argt_code
    ...
    def query_to_list():
        ...
        if t_list:
            ...
            if disptype == 1 and cardtype != 3:
                ...
                if stattype == 0:
                    ...
                    for t_list in query(t_list_list, filters=(lambda t_list: t_list.flag_history and t_list.rsv_karteityp == cardtype), sort_by=[("rsv_name",False)]):
                        create_to_list1()


                elif stattype == 1:

                    for t_list in query(t_list_list, filters=(lambda t_list: t_list.flag_history and t_list.rsv_karteityp == cardtype and t_list.resstatus != 3), sort_by=[("rsv_name",False)]):
                        create_to_list1()
                ...
                if stattype == 0:
                    ...
                    for t_list in query(t_list_list, filters=(lambda t_list: t_list.flag_history), sort_by=[("rsv_name",False)]):
                        create_to_list2()


                elif stattype == 1:

                    for t_list in query(t_list_list, filters=(lambda t_list: t_list.flag_history and t_list.resstatus != 3), sort_by=[("rsv_name",False)]):
                        create_to_list2()
        ...
        if disptype == 1 and cardtype != 3:
            ...
            if stattype == 0:
                ...
                for t_list in query(t_list_list, filters=(lambda t_list: not t_list.flag_history and t_list.rsv_karteityp == cardtype), sort_by=[("rsv_name",False)]):
                    create_to_list1()


            elif stattype == 1:

                for t_list in query(t_list_list, filters=(lambda t_list: not t_list.flag_history and t_list.rsv_karteityp == cardtype and t_list.resstatus != 3), sort_by=[("rsv_name",False)]):
                    create_to_list1()
            ...
            if stattype == 0:
                ...
                for t_list in query(t_list_list, filters=(lambda t_list: not t_list.flag_history), sort_by=[("rsv_name",False)]):
                    create_to_list2()


            elif stattype == 1:

                for t_list in query(t_list_list, filters=(lambda t_list: not t_list.flag_history and t_list.resstatus != 3), sort_by=[("rsv_name",False)]):
                    create_to_list2()
    ...
    def create_to_list2():
        ...
        if disptype == 1:
            ...
            to_list = query(to_list_list, filters=(lambda to_list: matches(to_list.argtcode, t_list).argtcode and to_list.curr == t_list.currency), first=True)
        ...
        if not to_list:
            ...
            to_list.argtcode = t_list.argtcode
            ...
            if disptype == 2:
                ...
                if matches(t_list.argtcode,argtcode):

                    arrangement = db_session.query(Arrangement).filter(
                             (matches(Arrangement.arrangement,t_list.argtcode))).first()
    ...
    def create_browse():
        ...
        if rev_calc == 1:
            ...
            if disptype == 5:
                ...
                for to_list in query(to_list_list, filters=(lambda to_list: matches(to_list.argtcode,argtcode))):
            ...
            output_list1.exp_rev = fill("-", 19)
            output_list1.curr = fill("-", 4)
            output_list1.loc_curr = fill("-", 19)
            output_list1.lodg = fill("-", 18)
            output_list1.bfast = fill("-", 17)
            output_list1.lunch = fill("-", 17)
            output_list1.dinner = fill("-", 17)
            output_list1.oth_rev = fill("-", 17)

            for tot_list in query(tot_list_list, sort_by=[("curr",False)]):
                output_list1 = Output_list1()
                output_list1_list.append(output_list1)
            ...
            output_list2.exp_rev = fill("-", 19)
            output_list2.curr = fill("-", 4)
            output_list2.loc_curr = fill("-", 19)
            output_list2.lodg = fill("-", 18)
    ...
    t_list_list = get_output(create_forecast_history_2bl(fr_date, to_date, excl_comp, vhp_limited, scin))
