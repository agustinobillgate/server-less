...
def tada_load_orderlistbl(deptno:int):
    ...
    order_list_list, Order_list = create_model("Order_list", {"outlet_no":int, "order_id":int, "guest_name":string, "guest_email":string, "guest_phone":string, "bill_number":string, "order_date":string, "order_type":string, "status_order":string, "posted":bool, "billrecid":int, "table_no":string, "vhp_bill":int, "courier_mtd":string, "payment":string, "payment_amt":Decimal, "payment_art":int, "allow_completed":bool, "trans_date":date})
    order_detail_list, Order_detail = create_model("Order_detail", {"nr":int, "outlet_no":int, "order_id":int, "bezeich":string, "qty":int, "sp_req":string, "confirm":bool, "remarks":string, "order_date":string, "art_nr":int, "posted":bool, "table_no":string, "payment":string, "payment_amt":Decimal, "payment_art":int})
    ...
    def generate_output():
        nonlocal order_list_list, order_detail_list, tableno, tada_deptno, vhp_deptno, get_date_str, get_date, nr, queasy, h_bill
        nonlocal deptno
        nonlocal orderhdr, orderline


        nonlocal order_list, order_detail, crd_list, orderhdr, orderline
        nonlocal order_list_list, order_detail_list, crd_list_list

        return {"order-list": order_list_list, "order-detail": order_detail_list}
    ...
    crd_list = query(crd_list_list, filters=(lambda crd_list: crd_list.deptno == deptno), first=True)
    ...
    for order_list in query(order_list_list):
        ...
        if order_list.status_order.lower()  == ("EXPIRED").lower() :
            ...
            order_detail = query(order_detail_list, filters=(lambda order_detail: order_detail.order_id == order_list.order_id), first=True)
