...
payload_list_list, Payload_list = create_model("Payload_list", {"room":string, "fdate":date, "tdate":date, "mealtime":string, "lname":string}, {"room": "*"})

def bfast_card_report_webbl(payload_list_list:[Payload_list]):
    ...
    bfast_data_list, Bfast_data = create_model("Bfast_data", {"tdate":date, "roomnr":string, "resnr":int, "guestnr":int, "vip":bool, "nation":string, "guestname":string, "totaladult":int, "totalcompli":int, "totalchild":int, "totaluse":int, "dummychr":string, "arrivaldate":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal bfast_data_list, getdatanr, i, ii, sumadult, sumcompl, sumchild, sumtotuse, dateval, artgrp_abf, artgrp_lunch, artgrp_dinner, tempresnr, strdayuse, strday, counter, bill_date, htparam, mealcoup, res_line, guest, arrangement, argt_line
    ...
    bfast_data = query(bfast_data_list, first=True)
