...
def eg_chgreq_btn_go_webbl(treqdetail_list:[Treqdetail], request1_list:[Request1], stock_list:[Stock], attchment_list:[Attchment], svendor_list:[Svendor], pvilanguage:int, sub_str:string, solution:string, estfin_str:string, a:int, blout:int, reqno:int, user_init:string, prop_bezeich:string, sguestflag:bool):
    ...
    def generate_output():
        ...
        nonlocal request1_list, pvilanguage, sub_str, solution, estfin_str, a, blout, reqno, user_init, prop_bezeich, sguestflag
        nonlocal usr, buff_property


        nonlocal svendor, attchment, stock, request1, treqdetail, usr, buff_property, sbuff

        return {"request1": request1_list, "blrange": blrange, "avail_eg_staff": avail_eg_staff}
    ...
    def execute_it():
        ...
        for sbuff in query(sbuff_list, sort_by=[("stock_nr",False)]):
    ...
    def save_vendor():
        ...
        for svendor in query(svendor_list, filters=(lambda svendor: svendor.stat.lower()  == ("0").lower())):
        ...
        for svendor in query(svendor_list, filters=(lambda svendor: svendor.stat.lower()  == ("1").lower())):
