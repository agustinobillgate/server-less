...
from functions.pj_inhouse2_btn_go_3_cldbl import pj_inhouse2_btn_go_3_cldbl
from models import Queasy

def pj_inhouse2_btn_go_3_webbl(sorttype:int, datum:date, curr_date:date, curr_gastnr:int, froom:string, troom:string, exc_depart:bool, incl_gcomment:bool, incl_rsvcomment:bool, prog_name:string, disp_accompany:bool, disp_exclinact:bool, split_rsv_print:bool):
    output_list_list = []
    summary_list1_list = []
    summary_list2_list = []
    summary_list3_list = []
    ...
    s_list_list, S_list = create_model("S_list", {"rmcat":string, "bezeich":string, "nat":string, "anz":int, "adult":int, "proz":Decimal, "child":int, "rmqty":int})
    segm_list_list, Segm_list = create_model("Segm_list", {"segmcode":int, "segment":string, "anzahl":int})
    argt_list_list, Argt_list = create_model("Argt_list", {"argt":string, "tot_room":int, "tot_pax":int, "tot_breakfast":int})
    sum_list_list, Sum_list = create_model("Sum_list", {"curr":string, "zipreis":Decimal})
    ...
    lnl_sum_list, Lnl_sum = create_model("Lnl_sum", {"counter":int, "summ":string, "rm_type":string, "qty":string, "nation":string, "rm_qty":string, "adult":string, "percent":string, "child":string})
    summary_list1_list, Summary_list1 = create_model("Summary_list1", {"summ":string, "room_type":string, "qty":string, "nation":string, "rm_qty":string, "adult":string, "percent":string, "child":string})
    summary_list2_list, Summary_list2 = create_model("Summary_list2", {"summ":string, "curr":string, "room_rate":string})
    summary_list3_list, Summary_list3 = create_model("Summary_list3", {"summ":string, "segm_code":string, "rm_qty":string})
    summary_list4_list, Summary_list4 = create_model("Summary_list4", {"summ":string, "argt":string, "rm_qty":string, "pax":string, "bfast":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list_list, summary_list1_list, summary_list2_list, summary_list3_list, summary_list4_list, lnl_sum_list, t_buff_queasy_list, tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, tot_rmqty, inactive, curr_company, outnr, query_string, queasy
        nonlocal sorttype, datum, curr_date, curr_gastnr, froom, troom, exc_depart, incl_gcomment, incl_rsvcomment, prog_name, disp_accompany, disp_exclinact, split_rsv_print
    ...
    def create_inhouse_v2():
        ...
        nonlocal output_list_list, summary_list1_list, summary_list2_list, summary_list3_list, summary_list4_list, lnl_sum_list, t_buff_queasy_list, tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, tot_rmqty, inactive, curr_company, outnr, query_string, queasy
        nonlocal sorttype, datum, curr_date, curr_gastnr, froom, troom, exc_depart, incl_gcomment, incl_rsvcomment, disp_accompany, disp_exclinact, split_rsv_print
        ...
        tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, tot_rmqty, inactive, cl_list_list, s_list_list, t_buff_queasy_list = get_output(pj_inhouse2_btn_go_3_cldbl(sorttype, datum, curr_date, curr_gastnr, froom, troom, exc_depart, incl_gcomment, incl_rsvcomment, prog_name, disp_accompany, disp_exclinact, split_rsv_print))
        C_list = Cl_list
        ...
        if sorttype == 1 or sorttype == 3:
            ...
            if sorttype == 1:
                ...
                for cl_list in query(cl_list_list, sort_by=[("etage",False),("rmno",False)]):
                    create_outlist()

        else:

            for cl_list in query(cl_list_list, sort_by=[("company",False)]):
