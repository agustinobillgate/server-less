...
def chk_btn_go_res_rmratebl(pvilanguage:int, curr_select:str, max_rate:decimal, fact1:decimal, inp_wahrnr:int, inp_zikatnr:int, user_init:str, resnr:int, reslinnr:int, recid_reslin:int, contcode:str, p_list_list:[P_list]):
    msg_str = ""
    error_found1 = False
    error_code = 0
    ...
    def check_rate():
        ...
        waehrung = db_session.query(Waehrung).filter(
                 (Waehrung.waehrungsnr == inp_wahrnr)).first()
    ...
    def check_currency():
        ...
        reslin_queasy = db_session.query(Reslin_queasy).filter(
                 (func.lower(Reslin_queasy.key) == ("arrangement").lower()) & (Reslin_queasy.resnr == resnr) & (Reslin_queasy.reslinnr == reslinnr)).first()
    ...
    if p_list.argt != "":
        ...
        arrangement = db_session.query(Arrangement).filter(
                 (Arrangement.arrangement == p_list.argt)).first()
    ...
    if curr_select.lower()  == ("add").lower() :
        ...
        brq = db_session.query(Brq).filter(
                 (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (bRQ.date1 >= p_list.date1) & (bRQ.date1 <= p_list.date2)).first()

        if not bRQ:

            brq = db_session.query(Brq).filter(
                     (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (bRQ.date2 >= p_list.date1) & (bRQ.date2 <= p_list.date2)).first()

        if not bRQ:

            brq = db_session.query(Brq).filter(
                     (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (p_list.date1 >= bRQ.date1) & (p_list.date1 <= bRQ.date2)).first()

        if not bRQ:

            brq = db_session.query(Brq).filter(
                     (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (p_list.date2 >= bRQ.date1) & (p_list.date2 <= bRQ.date2)).first()
        ...
        if res_line.active_flag == 0:
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                     (func.lower(Reslin_queasy.key) == ("fixrate-trace-record").lower()) & (Reslin_queasy.resnr == resnr) & (Reslin_queasy.reslinnr == reslinnr)).first()
        ...
        brq = db_session.query(Brq).filter(
                 (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (brQ._recid != reslin_queasy._recid) & (bRQ.date1 >= p_list.date1) & (bRQ.date1 <= p_list.date2)).first()

        if not bRQ:

            brq = db_session.query(Brq).filter(
                     (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (brQ._recid != reslin_queasy._recid) & (bRQ.reslinnr == reslinnr) & (bRQ.date2 >= p_list.date1) & (bRQ.date2 <= p_list.date2)).first()

        if not bRQ:

            brq = db_session.query(Brq).filter(
                     (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (brQ._recid != reslin_queasy._recid) & (p_list.date1 >= bRQ.date1) & (p_list.date1 <= bRQ.date2)).first()

        if not bRQ:

            brq = db_session.query(Brq).filter(
                     (func.lower(Brq.key) == ("arrangement").lower()) & (bRQ.resnr == resnr) & (bRQ.reslinnr == reslinnr) & (brQ._recid != reslin_queasy._recid) & (p_list.date2 >= bRQ.date1) & (p_list.date2 <= bRQ.date2)).first()
        ...
        if res_line.active_flag == 0:
            ...
            breslin = db_session.query(Breslin).filter(
                     (func.lower(Breslin.key) == ("fixrate-trace-record").lower()) & (Breslin.resnr == resnr) & (Breslin.reslinnr == reslinnr)).first()
