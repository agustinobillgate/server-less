...
def rest_daysalesp2_btn_go_5_cldbl(bline_list_list:[Bline_list], buf_art_list:[Buf_art], disc_art1:int, disc_art2:int, disc_art3:int, curr_dept:int, all_user:bool, shift:int, from_date:date, to_date:date, art_str:string, voucher_art:int, zero_vat_compli:bool, exclude_compli:bool, show_fbodisc:bool, htl_dept_dptnr:int, incl_move_table:bool, wig:bool, inhouse:bool):
    ...
    set_cache(H_bill, ((H_bill.flag == 1) & (H_bill.departement == curr_dept)),[["rechnr", "departement"]], True, [],["resnr"])
    set_cache(Res_line, (Res_line.resnr.in_(get_cache_value_list(H_bill,"resnr"))),[["resnr", "reslinnr"]], True, [],[])
    set_cache(H_bill_line, (H_bill_line.bill_datum >= from_date) & (H_bill_line.bill_datum <= to_date),[["rechnr", "departement"], ["rechnr", "departement", "betriebsnr"]], True, ["bill_datum"],[])
    ...
    def generate_output():
        ...
        nonlocal other_art_list, temp_list, t_tot_betrag_list, t_nt_betrag_list, outstand_list_list, pay_list_list, turnover_list, summ_list_list, t_artnr_list
    ...
    if turnover:
        ...
        if pay_list:
            ...
            for tlist in query(tlist_list, filters=(lambda tlist: not tlist.compli and tlist.r_transfer != 0 and tlist.rechnr.lower()  != ("G-TOTAL").lower()  and tlist.rechnr.lower()  != ("R-TOTAL").lower())):
