...
payload_list_list, Payload_list = create_model("Payload_list", {"v_mode":int, "user_init":string, "guest_number":int, "guest_name":string})

def guest_deposit_list_webbl(payload_list_list:[Payload_list], pvilanguage:int):
    ...
    guest_deposit_list, Guest_deposit = create_model("Guest_deposit", {"guest_number":int, "guest_name":string, "guest_type":string, "deposit_amount":Decimal, "deposit_used":Decimal, "deposit_balance":Decimal})
    temp_guest_deposit_list, Temp_guest_deposit = create_model("Temp_guest_deposit", {"guest_number":int, "guest_name":string, "guest_type":string, "deposit_amount":Decimal, "deposit_used":Decimal, "deposit_balance":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal error_desc, guest_deposit_list, lvcarea, depoart_guest, depobez_guest, depoart_rsv, depoart_bqt, depoart_pos, total_deposit, total_used, total_balance, htparam, artikel, billjournal, guest, bill, bill_line
        nonlocal pvilanguage


        nonlocal guest_deposit, temp_guest_deposit, payload_list
        nonlocal guest_deposit_list, temp_guest_deposit_list

        return {"error_desc": error_desc, "guest-deposit": guest_deposit_list}
    ...
    if payload_list.v_mode == 1:
        ...
        if billjournal:
            ...
            while None != billjournal:
                ...
                temp_guest_deposit = query(temp_guest_deposit_list, filters=(lambda temp_guest_deposit: temp_guest_deposit.guest_number == billjournal.billjou_ref), first=True)

                if not temp_guest_deposit:
                    temp_guest_deposit = Temp_guest_deposit()
                    temp_guest_deposit_list.append(temp_guest_deposit)
            ...
            for temp_guest_deposit in query(temp_guest_deposit_list, sort_by=[("guest_name",False)]):
                guest_deposit = Guest_deposit()
                guest_deposit_list.append(guest_deposit)

                buffer_copy(temp_guest_deposit, guest_deposit)

            guest_deposit = query(guest_deposit_list, first=True)
            ...
            if guest_deposit:
                ...
                for bill, bill_line in db_session.query(Bill, Bill_line).join(Bill_line,(Bill_line.rechnr == Bill.rechnr) & (Bill_line.artnr == depoart_guest)).filter(
                         ((Bill.rechnr > 0) & (Bill.gastnr.in_(list(set([guest_deposit.guest_number for guest_deposit in guest_deposit_list])))))).order_by(Bill._recid).all():                    guest_deposit = query(guest_deposit_list, (lambda guest_deposit: (bill.rechnr > 0)), first=True)
