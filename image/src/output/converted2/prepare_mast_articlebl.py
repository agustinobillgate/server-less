...
def prepare_mast_articlebl(resnr:int, new_res:bool, curr_dept:int):
    ...
    hart_list_list, Hart_list = create_model("Hart_list", {"artnr":int, "departement":int, "bezeich":string, "artart":int})
    gart_list_list, Gart_list = create_model("Gart_list", {"artnr":int, "departement":int, "bezeich":string, "artart":int})
    ...
    def create_list():
        ...
        for artikel_list in query(artikel_list_list, filters=(lambda artikel_list:(artikel_list.artart == 0 or artikel_list.artart == 5))):
    ...
    for t_mast_art in query(t_mast_art_list):
    ...
    if new_res:
        ...
        for t_res_line in query(t_res_line_list):
            ...
            for t_fixleist in query(t_fixleist_list):
        ...
        get_output(write_mast_artbl(1, resnr, gart_list_list))
    create_list()
