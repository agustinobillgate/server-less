...
def dml_stockin_btn_go_webbl(pvilanguage:int, op_list_list:[Op_list], curr_dept:int, billdate:date, curr_lager:int, lscheinnr:string, lief_nr:int, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, user_init:string, t_amount:Decimal):
    ...
    t_op_list_list, T_op_list = create_model_like(L_op, {"bezeich":string, "username":string, "dml_code":string})
        ...
        if curr_dept == 0:
            ...
            if d_art1:
                ...
                reslin_queasy = db_session.query(Reslin_queasy).filter(
                             (Reslin_queasy.key == ("DML").lower()) & (Reslin_queasy.date1 == billdate) & (to_int(entry(0, Reslin_queasy.char1, ";")) == s_artnr) & (to_int(entry(1, Reslin_queasy.char1, ";")) == curr_dept)).first()
