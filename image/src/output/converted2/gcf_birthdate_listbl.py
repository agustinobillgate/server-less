...
def gcf_birthdate_listbl(from_mm:int, from_dd:int, to_mm:int, to_dd:int, from_age:int, to_age:int, sorttype:int):
    ...
    birth_list_list, Birth_list = create_model("Birth_list", {"name":str, "geburtdatum":date, "ankunft1":date, "abreise1":date, "zinr":str, "adresse":str, "wohnort":str})
    birth_list2_list, Birth_list2 = create_model("Birth_list2", {"name":str, "geburtdatum":date, "ankunft1":date, "abreise1":date, "zinr":str, "adresse":str, "wohnort":str, "telefon":str, "mobil_telefon":str, "email_addr":str})
    ...
    for birth_list in query(birth_list_list):
        ...
        birth_list2.adresse = entry(0, birth_list.adresse, chr(3))
        birth_list2.wohnort = birth_list.wohnort

        if num_entries(birth_list.adresse, chr(3)) > 1:
            birth_list2.email_addr = entry(1, birth_list.adresse, chr(3))

            if num_entries(birth_list.adresse, chr(3)) > 2:
                birth_list2.telefon = entry(2, birth_list.adresse, chr(3))
                birth_list2.mobil_telefon = entry(3, birth_list.adresse, chr(3))
