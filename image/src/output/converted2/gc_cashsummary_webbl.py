...
def gc_cashsummary_webbl(pvilanguage:int, from_date:date, to_date:date):
    ...
    cash_list_list, Cash_list = create_model("Cash_list", {"flag":int, "datum":date, "artnr":[int,20], "bezeich":[string,20], "amount":[Decimal,20], "str_amount":[string,20], "tot_str_amount":string})
    cash_art_list, Cash_art = create_model("Cash_art", {"pos_nr":int, "datum":date, "artnr":int, "bezeich":string, "amount":Decimal})
    t_cash_art_list, T_cash_art = create_model("T_cash_art", {"artnr":int, "bezeich":string})
    ...
    def cash_summ():
        ...
        for cash_list in query(cash_list_list, sort_by=[("datum",False)]):
