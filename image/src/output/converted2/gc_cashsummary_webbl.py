...
def gc_cashsummary_webbl(pvilanguage:int, from_date:date, to_date:date):
    ...
    def cash_summ():
        ...
        for billjournal, artikel in db_session.query(Billjournal, Artikel).join(Artikel,(Artikel.artnr == Billjournal.artnr) & (Artikel.departement == 0) & (Artikel.artart == 6)).filter(
