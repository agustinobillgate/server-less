...
def s_stockout_btn_go_webbl(op_list_list:[Op_list], pvilanguage:int, out_type:int, rec_id:int, curr_lager:int, jobnr:int, cost_acct:string, transdate:date, t_datum:date, transfered:bool, t_lschein:string, to_stock:int, lscheinnr:string, bediener_nr:int, qty:Decimal, t_amount:Decimal):
    ...
    op_list = stock_oh_tmp = out_list = None

    stock_oh_tmp_list, Stock_oh_tmp = create_model("Stock_oh_tmp", {"stock_oh":Decimal, "artnr":int})
    ...
    def generate_output():
        ...
        nonlocal op_list, stock_oh_tmp, out_list
        nonlocal stock_oh_tmp_list, out_list_list

        return {"op-list": op_list_list, "qty": qty, "t_amount": t_amount, "s_artnr": s_artnr, "err_flag": err_flag, "price": price, "amount": amount, "msg_str2": msg_str2, "msg_str3": msg_str3, "req_created": req_created}
    ...
    def create_l_op(zeit:int):
        ...
        if l_bestand:
            stock_oh_tmp = Stock_oh_tmp()
            stock_oh_tmp_list.append(stock_oh_tmp)

            stock_oh_tmp.stock_oh =  to_decimal(l_bestand.anz_anf_best) + to_decimal(l_bestand.anz_eingang) - to_decimal(l_bestand.anz_ausgang)
            stock_oh_tmp.artnr = l_bestand.artnr
        ...
        for op_list in query(op_list_list):
            ...
            if l_artikel.min_bestand != 0:
                ...
                for l_bestand in db_session.query(L_bestand).filter(
                         (L_bestand.artnr == l_artikel.artnr) & (L_bestand.lager_nr > 0)).order_by(L_bestand._recid).all():
                    ...
                    if curr_oh < l_artikel.min_bestand:
                        msg_str2 = msg_str2 + "&W" + translateExtended ("One (or more) stock item(s) under Minimum Onhand Level!", lvcarea, "") + chr_unicode(10) + to_string(l_artikel.artnr) + " " + l_artikel.bezeich + chr_unicode(10) + translateExtended ("Current Onhand:", lvcarea, "") + " " + to_string(curr_oh) + " <> " + translateExtended ("Minimum Onhand:", lvcarea, "") + " " + to_string(l_artikel.min_best)
                        break
    ...
    def update_request_records():
        ...
        while None != l_op:
            ...
            if op_list:
                ...
                stock_oh_tmp = query(stock_oh_tmp_list, filters=(lambda stock_oh_tmp: stock_oh_tmp.artnr == op_list.artnr), first=True)

                if stock_oh_tmp:

                    if stock_oh_tmp.stock_oh > 0:
    ...
    def get_to_stock(lscheinnr:string, artnr:int):

        nonlocal s_artnr, err_flag, price, amount, msg_str2, msg_str3, req_created, lvcarea, curr_pos, zeit, gl_notfound, its_ok, l_op, gl_acct, l_ophdr, l_bestand, l_verbrauch, l_artikel
        nonlocal pvilanguage, out_type, rec_id, curr_lager, jobnr, cost_acct, transdate, t_datum, transfered, t_lschein, to_stock, bediener_nr, qty, t_amount
    ...
    if not transfered:
        ...
        for op_list in query(op_list_list, filters=(lambda op_list: op_list.fibu.lower()  != "" and op_list.fibu.lower()  != ("00000000").lower()  and op_list.fibu.lower()  != None)):
