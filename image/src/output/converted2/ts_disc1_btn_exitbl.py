...
menu_list, Menu = create_model("Menu", {"artnr":int, "anzahl":int, "departement":int, "prtflag":int, "pos":int, "bcolor":int, "epreis":Decimal, "betrag":Decimal, "fremdwbetrag":Decimal, "bezeich":string, "bez0":string}, {"bcolor": 1})

def ts_disc1_btn_exitbl(rec_id:int, billart:int, dept:int, transdate:date, amount:Decimal, description:string, netto_betrag:Decimal, exchg_rate:Decimal, tischnr:int, curr_select:int, disc_value:Decimal, qty:int, cancel_str:string, curr_waiter:int, procent:Decimal, b_artnrfront:int, o_artnrfront:int, price_decimal:int, user_init:string, disc_list_list:[Disc_list], vat_list_list:[Vat_list], menu_list:[Menu]):
    ...
    def generate_output():
        ...
        nonlocal menu, disc_list, vat_list
    ...
    def update_bill(h_artart:int, h_artnrfront:int):
        ...
        for disc_list in query(disc_list_list, filters=(lambda disc_list:(disc_list.h_artnr == billart) or (disc_list.netto_amt != 0))):
            ...
            if disc_list.amount != 0:
                ...
                if procent > 0:
                    amount_list =  to_decimal(amount_list) + to_decimal(disc_list.amount)

                    if amount_list < 0:
                        disc_list.amount =  to_decimal(disc_list.amount) - to_decimal(amount_list)
    ...
    def update_rev_argtart(h_artnrfront:int):
        ...
        for h_artikel, artikel in db_session.query(H_artikel, Artikel).join(Artikel,(Artikel.artnr == H_artikel.artnrfront) & (Artikel.departement == H_artikel.departement) & (Artikel.artart == 9) & (Artikel.artgrp != 0)).filter(
                 ((H_artikel.artnr.in_(list(set([menu.artnr for menu in menu_list if menu.prtflag == 1])))) & (H_artikel.departement == h_bill.departement))).order_by(H_artikel._recid).all():
            if h_artikel_obj_list.get(h_artikel._recid):
                continue
            else:
                h_artikel_obj_list[h_artikel._recid] = True

            menu = query(menu_list, (lambda menu: (h_artikel.artnr == menu.artnr)), first=True)
    ...
    def cal_vat_amount():
        ...
        for menu in query(menu_list, filters=(lambda menu: menu.menu.prtflag == 1)):

            hart = get_cache (H_artikel, {"artnr": [(eq, menu.artnr)],"departement": [(eq, menu.departement)],"artart": [(eq, 0)]})

            foart = get_cache (Artikel, {"artnr": [(eq, hart.artnrfront)],"departement": [(eq, hart.departement)]})
            ...
            menu_amt =  to_decimal(menu.epreis) * to_decimal(qty)
            ...
            if incl_mwst:
                ...
                if qty != 0:
                    unit_price = ( to_decimal(menu.betrag) / to_decimal(qty)) / to_decimal(fact)
                else:
                    unit_price =  to_decimal(menu.epreis) / to_decimal(fact)
            ...
            if h_service == 0 and h_mwst == 0:
                ...
                if h_service == 0:
                    h_mwst =  to_decimal(menu.betrag) - to_decimal(menu_amt)
            ...
            if h_mwst != 0 and vathtp:
                ...
                vat_list.vatamt =  to_decimal(vat_list.vatamt) - to_decimal(h_mwst) * to_decimal(procent) / to_decimal("100")
                vat_list.betrag =  to_decimal(vat_list.betrag) - to_decimal(menu.betrag) * to_decimal(procent) / to_decimal("100")


        mwst =  to_decimal(mwst) * to_decimal(procent) / to_decimal("100")

        for vat_list in query(vat_list_list):
            vat_list.vatamt = to_decimal(round(vat_list.vatAmt , price_decimal))
            vat_list.netto =  to_decimal(vat_list.betrag) - to_decimal(vat_list.vatAmt)
