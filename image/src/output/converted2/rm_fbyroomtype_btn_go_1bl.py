...
def rm_fbyroomtype_btn_go_1bl(sum_month:bool, fr_date:date, to_date:date, to_year:int, ex_tent:bool, ex_comp:bool):
    ...
    output_list_list, Output_list = create_model("Output_list", {"counter":int, "datum":date, "str_datum":string, "room":int, "revenue":Decimal, "avrg_rev":Decimal, "str_room":string, "str_revenue":string, "str_avrg":string, "zikatnr":int, "roomtype":string})
    output_list1_list, Output_list1 = create_model("Output_list1", {"counter":int, "zikatnr":int, "roomtype":string, "room":[int,12], "revenue":[Decimal,12], "avrg_rev":[Decimal,12]})
    ...
    def generate_output():
        nonlocal output_list_list, output_list1_list, black_list, counter, ci_date, curr_date, tot_rm, trm, tot_rev, trev, trev1, trm1, loopi, htparam, zimkateg, genstat, res_line, reservation, arrangement, bill_line, zimmer, queasy
    ...
    def create_browse():
        ...
        for genstat.resstatus, genstat.zikatnr, genstat.datum, genstat.logis, genstat._recid, zimkateg.kurzbez, zimkateg._recid in db_session.query(Genstat.resstatus, Genstat.zikatnr, Genstat.datum, Genstat.logis, Genstat._recid, Zimkateg.kurzbez, Zimkateg._recid).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).filter(
            ...
            if genstat.resstatus != 11 and genstat.resstatus != 13:
                ...
                output_list = query(output_list_list, filters=(lambda output_list: output_list.zikatnr == genstat.zikatnr and output_list.datum == genstat.datum), first=True)
                    ...
                    for datum in date_range(datum1,datum2) :
                        ...
                        if datum == res_line.abreise:
                            ...
                            if res_line.resstatus != 11 and res_line.resstatus != 13 and not res_line.zimmerfix:
                                ...
                                output_list = query(output_list_list, filters=(lambda output_list: output_list.zikatnr == res_line.zikatnr and output_list.datum == datum), first=True)
    ...
    def 82():
        ...
        zimkateg = Zimkateg()
        for genstat.resstatus, genstat.zikatnr, genstat.datum, genstat.logis, genstat._recid, zimkateg.kurzbez, zimkateg._recid in db_session.query(Genstat.resstatus, Genstat.zikatnr, Genstat.datum, Genstat.logis, Genstat._recid, Zimkateg.kurzbez, Zimkateg._recid).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).filter(
                 (Genstat.datum >= frdate) & (Genstat.datum <= tdate) & (Genstat.segmentcode != 0) & (Genstat.nationnr != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)]) & (Genstat.resstatus != 13)).order_by(Genstat.zikatnr).all():
            if genstat_obj_list.get(genstat._recid):
                continue
            else:
                genstat_obj_list[genstat._recid] = True

            output_list1 = query(output_list1_list, filters=(lambda output_list1: output_list1.zikatnr == genstat.zikatnr), first=True)
                ...
                if segment:
                    ...
                    for datum in date_range(datum1,datum2) :
                        ...
                        if datum == res_line.abreise:
                            ...
                            output_list1 = query(output_list1_list, filters=(lambda output_list1: output_list1.zikatnr == res_line.zikatnr), first=True)
    ...
    if sum_month == False:
        ...
        for output_list in query(output_list_list, sort_by=[("datum",False),("zikatnr",False)]):
        ...
        for output_list1 in query(output_list1_list, sort_by=[("zikatnr",False)]):
