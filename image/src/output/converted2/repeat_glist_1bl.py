...
from models import Guest, Zimmer, Genstat, Waehrung, Res_line, Guestseg, Segment, Nation, Paramtext
...
def repeat_glist_1bl(pvilanguage:int, from_date:date, to_date:date, ci_date:date, create_inhouse:bool):
    ...
    guest = zimmer = genstat = waehrung = res_line = guestseg = segment = nation = paramtext = None
    ...
    def generate_output():
        nonlocal g_list_list, repeat_list_list, cur_date_list, tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, inactive, curr_date, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, integerflag, lvcarea, guest, zimmer, genstat, waehrung, res_line, guestseg, segment, nation, paramtext
    ...
    def create_inhouse():
        ...
        for guest in db_session.query(Guest).filter(
                 ((Guest.gastnr.in_(list(set([g_list.gastnr for g_list in g_list_list])))))).order_by(g_list.gastnr, g_list.resnr).all():
            if guest_obj_list.get(guest._recid):
                continue
            else:
                guest_obj_list[guest._recid] = True

            g_list = query(g_list_list, (lambda g_list: (guest.gastnr == g_list.gastnr)), first=True)
