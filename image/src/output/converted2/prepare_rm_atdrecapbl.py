...
def prepare_rm_atdrecapbl(pvilanguage:int):
    ...
    test_list, Test = create_model("Test", {"xx":string, "yy":string})
    cl_list_list, Cl_list = create_model("Cl_list", {"flag":int, "loc":string, "etage":string, "inact":int, "occ":int, "hu":int, "ooo":int, "com":int, "vac":int, "sum":int, "pax":int, "cpax":int, "rev":Decimal, "nrev":Decimal})
    ...
    def create_list():
        ...
        nonlocal msg_str, ci_date, price_decimal, long_digit, foreign_rate, curr_local, curr_foreign, exchg_rate, flag_t, t_occ, t_hu, t_ooo, t_com, t_vac, t_sum, t_inact, t_rev, t_nrev, t_pax, t_cpax, cl_list_list, frate, ex_rate, lvcarea, htparam, waehrung, zimmer, artikel, res_line, reservation, arrangement, argt_line
        nonlocal pvilanguage
        ...
        for cl_list in query(cl_list_list, filters=(lambda cl_list: not matches(cl_list.loc,r"Gr. TOTAL")), sort_by=[("loc",True)]):

            test = query(test_list, filters=(lambda test: test.xx == cl_list.loc), first=True)
        ...
        for cl_list in query(cl_list_list, filters=(lambda cl_list: not matches(cl_list.etage,r"G. TOTAL")), sort_by=[("etage",True)]):

            test = query(test_list, filters=(lambda test: test.yy == cl_list.etage), first=True)
        ...
        for test in query(test_list, filters=(lambda test: matches(test.xx,r"") and not matches(test.yy,r"G.TOTAL"))):
            t_occ = 0
            t_hu = 0
            t_ooo = 0
            t_com = 0
        ...
        for test in query(test_list, filters=(lambda test: not matches(test.xx,r""))):
            t_occ = 0
            t_hu = 0
            t_ooo = 0
            t_com = 0
            ...
            for cl_list in query(cl_list_list, filters=(lambda cl_list: matches(cl_list.loc,test.xx))):
    ...
    def gr_tot():
        ...
        for cl_list in query(cl_list_list, sort_by=[("etage",False)]):
