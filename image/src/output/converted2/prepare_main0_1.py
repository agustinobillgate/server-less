...
from models import Queasy, Bediener, Htparam, Paramtext, Zimmer, Hoteldpt

def prepare_main0_1(pvilanguage:int):
    ...
    lvcarea:string = "main0"
    queasy = bediener = htparam = paramtext = zimmer = hoteldpt = None
    ...
    def generate_output():
        nonlocal lstopped, lic_nr, htl_name, h_name, htl_city, h_city, htl_adr, htl_tel, vhp_multi, vhp_lite, rest_lic, eng_dept, i_param111, i_param297, p_decimal, d_currency, msg_str, coa_format, dateflag, lic_room, lic_pos, condo_flag, newdb, t_queasy_list, p_787, p_223, loyalty_flag, lvcarea, queasy, bediener, htparam, paramtext, zimmer, hoteldpt
        ...
        return {"lstopped": lstopped, "lic_nr": lic_nr, "htl_name": htl_name, "h_name": h_name, "htl_city": htl_city, "h_city": h_city, "htl_adr": htl_adr, "htl_tel": htl_tel, "vhp_multi": vhp_multi, "vhp_lite": vhp_lite, "rest_lic": rest_lic, "eng_dept": eng_dept, "i_param111": i_param111, "i_param297": i_param297, "p_decimal": p_decimal, "d_currency": d_currency, "msg_str": msg_str, "coa_format": coa_format, "dateflag": dateflag, "lic_room": lic_room, "lic_pos": lic_pos, "condo_flag": condo_flag, "newdb": newdb, "t-queasy": t_queasy_list}

    def check_htp():

        nonlocal lstopped, htl_name, h_name, htl_city, h_city, htl_adr, htl_tel, vhp_multi, vhp_lite, rest_lic, eng_dept, i_param111, i_param297, p_decimal, d_currency, msg_str, coa_format, dateflag, lic_room, lic_pos, condo_flag, newdb, t_queasy_list, p_787, p_223, loyalty_flag, lvcarea, queasy, bediener, htparam, paramtext, zimmer, hoteldpt
        nonlocal pvilanguage
    ...
    def check_room_pos_license():
        ...
        nonlocal lstopped, lic_nr, htl_name, h_name, htl_city, h_city, htl_adr, htl_tel, vhp_multi, vhp_lite, rest_lic, eng_dept, i_param111, i_param297, p_decimal, d_currency, msg_str, coa_format, dateflag, condo_flag, newdb, t_queasy_list, p_787, p_223, loyalty_flag, lvcarea, queasy, bediener, htparam, paramtext, zimmer, hoteldpt
        nonlocal pvilanguage
        ...
        if max_room > lic_room:
            msg_str = msg_str + translateExtended ("Maximum Room license", lvcarea, "") + " = " + to_string(lic_room) + " [" + to_string(max_room) + "!!]" + chr_unicode(10) + translateExtended ("Please contact our local Technical Support.", lvcarea, "") + chr_unicode(2)
            lstopped = True
        ...
        if max_pos > lic_pos:
            msg_str = msg_str + translateExtended ("Maximum Outlet license", lvcarea, "") + " = " + to_string(lic_pos) + " [" + to_string(max_pos) + "!!]" + chr_unicode(10) + translateExtended ("Please contact our local Technical Support.", lvcarea, "") + chr_unicode(2)
            lstopped = True
    ...
    newdb = None != bediener
    p_787 = get_output(htpchar(787))
    p_223 = get_output(htplogic(223))
    loyalty_flag = (p_787 != "") and (p_223 == True)
    msg_str = to_string(loyalty_flag) + chr_unicode(3)
    ...
    d_currency = get_output(htplogic(240))
    rest_lic = get_output(htplogic(990))
    vhp_multi = get_output(htplogic(996))
    vhp_lite = get_output(htplogic(1015))
    i_param111 = get_output(htpint(111))
    i_param297 = get_output(htpint(297))
    p_decimal = get_output(htpint(491))
    eng_dept = get_output(htpint(1060))
    dateflag = get_output(htpdate(976))
    lic_room = get_output(htpint(975))
    lic_pos = get_output(htpint(989))
    condo_flag = get_output(htplogic(981))

    queasy = get_cache (Queasy, {"key": [(eq, 1)]})
