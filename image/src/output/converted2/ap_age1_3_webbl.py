...
def ap_age1_3_webbl(pvilanguage:int, to_date:date, from_name:string, to_name:string, detailed:bool, mi_bill:bool, segm:int, curr_disp:int, round_zero:bool, by_supplier:bool):
    ...
    apage_list_list, Apage_list = create_model("Apage_list", {"docu_nr":string, "lschein":string, "rgdatum":string, "rechnr":string, "str":string, "bemerk":string})
    apage_list1_list, Apage_list1 = create_model("Apage_list1", {"docu_nr":string, "lschein":string, "rgdatum":string, "rechnr":string, "str":string, "nr":string, "firma":string, "prev_bal":string, "credit":string, "debit":string, "end_bal":string, "day":string, "day1":string, "day2":string, "day3":string, "bemerk":string})
    age_list_list, Age_list = create_model("Age_list", {"artnr":int, "rechnr":string, "lschein":string, "counter":int, "lief_nr":int, "rgdatum":date, "firma":string, "bemerk":string, "voucher_no":int, "p_bal":Decimal, "debit":Decimal, "credit":Decimal, "saldo":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal})
    ...
    def generate_output():
        ...
        nonlocal apage_list_list, apage_list1_list, age_list_list

        return {"apage-list1": apage_list1_list}
    ...
    def create_apage_list1():
        ...
        if by_supplier:
            ...
            for l_lieferant in db_session.query(L_lieferant).order_by(L_lieferant.firma).all():
                ...
                for apage_list in query(apage_list_list, filters=(lambda apage_list: not matches(apage_list.str,r"*---*") and trim(substring(apage_list.str, 7, 30)) == l_lieferant.firma)):
                    apage_list1 = Apage_list1()
                    apage_list1_list.append(apage_list1)
                    ...
                    sub_prev_bal =  to_decimal(sub_prev_bal) + to_decimal(to_decimal(trim(apage_list1.prev_bal)) )
                    sub_credit =  to_decimal(sub_credit) + to_decimal(to_decimal(trim(apage_list1.credit)) )
                    sub_debit =  to_decimal(sub_debit) + to_decimal(to_decimal(trim(apage_list1.debit)) )
                    sub_end_bal =  to_decimal(sub_end_bal) + to_decimal(to_decimal(trim(apage_list1.end_bal)) )
                    sub_day =  to_decimal(sub_day) + to_decimal(to_decimal(trim(apage_list1.day)) )
                    sub_day1 =  to_decimal(sub_day1) + to_decimal(to_decimal(trim(apage_list1.day1)) )
                    sub_day2 =  to_decimal(sub_day2) + to_decimal(to_decimal(trim(apage_list1.day2)) )
                    sub_day3 =  to_decimal(sub_day3) + to_decimal(to_decimal(trim(apage_list1.day3)) )
                    data_exist = 1
            ...
            apage_list = query(apage_list_list, filters=(lambda apage_list: not matches(apage_list.str,r"*---*") and substring(apage_list.str, 7, 9) == ("T O T A L").lower()), first=True)
            ...
            for apage_list in query(apage_list_list, filters=(lambda apage_list: not matches(apage_list.str,r"*---*"))):
                apage_list1 = Apage_list1()
                apage_list1_list.append(apage_list1)
