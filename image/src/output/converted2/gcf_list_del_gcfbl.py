...
def gcf_list_del_gcfbl(i_case:int, pvilanguage:int, gastno:int, user_init:string):

    prepare_cache ([Guest, Bediener, Akt_kont])
    ...
    if i_case == 1:
        ...
        if guest.anlage_datum == get_current_date():
            zugriff = substring(bediener.permissions, 1, 1) >= "2"
        else:
            zugriff = substring(bediener.permissions, 1, 1) >= "3"
    ...
    if error_code == 0:
        ...
        guest1 = get_cache (Guest, {"master_gastnr": [(eq, guest.gastnr)]})
        msg_str = translateExtended ("The file is currently used as Master-file of the guest file ", lvcarea, "") + guest1.name.upper() + ", " + guest1.vorname1.upper() + " " + guest1.anredefirma.upper() + guest1.anrede1.upper() + chr_unicode(10) + translateExtended ("Deletion not possible.", lvcarea, "")
        ...
        akt_kont = get_cache (Akt_kont, {"betrieb_gast": [(eq, guest.gastnr)]})

        gbuff = get_cache (Guest, {"gastnr": [(eq, akt_kont.gastnr)]})

        if gbuff:
            gname = gbuff.name
        msg_str = translateExtended ("Name contact using guest's GcfNO exists, deletion not possible.", lvcarea, "") + chr_unicode(10) + translateExtended ("Please check names contact list under:", lvcarea, "") + " " + gname
