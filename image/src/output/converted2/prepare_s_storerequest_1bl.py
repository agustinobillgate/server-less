...
def prepare_s_storerequest_1bl(user_init:string):
    ...
    temp_l_artikel_list, Temp_l_artikel = create_model("Temp_l_artikel", {"artnr":int, "betriebsnr":int, "endkum":int, "bezeich":string, "masseinheit":string, "vk_preis":Decimal, "lief_einheit":Decimal, "traubensort":string})
    t_parameters_list, T_parameters = create_model_like(Parameters)
    t_l_lager_list, T_l_lager = create_model_like(L_lager)
    art_list_list, Art_list = create_model("Art_list", {"artnr":int, "zwkum":int, "endkum":int, "zwkum_bezeich":string, "endkum_bezeich":string})
