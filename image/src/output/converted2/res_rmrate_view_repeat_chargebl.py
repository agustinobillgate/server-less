...
def res_rmrate_view_repeat_chargebl(pvilanguage:int, user_init:string, reslin_list_list:[Reslin_list]):
    ...
    dynarate_list_list, Dynarate_list = create_model("Dynarate_list", {"rcode":string, "fr_room":int, "to_room":int, "days1":int, "days2":int, "s_recid":int, "rmtype":string})
    ratecode_list_list, Ratecode_list = create_model("Ratecode_list", {"rcode_str":string})
    output_list_list, Output_list = create_model("Output_list", {"foreign_rate":bool, "double_currency":bool, "curr_foreign":string, "local_nr":int, "msg_str":string, "foreign_nr":int, "max_rate":Decimal, "long_digit":bool, "selected":bool, "contcode":string, "currency_add_first":string, "zimmer_wunsch":string, "btn_chgart":bool, "fact1":Decimal, "betriebsnr":int, "zipreis":Decimal, "adrflag":bool, "recid_resline":int})
    t_waehrung_list, T_waehrung = create_model("T_waehrung", {"wabkurz":string, "bezeich":string, "betriebsnr":int, "waehrungsnr":int, "exrate":Decimal})
    q2_reslin_queasy_list, Q2_reslin_queasy = create_model("Q2_reslin_queasy", {"date1":date, "date2":date, "deci1":Decimal, "char1":string, "number3":int, "char2":string, "char3":string, "recid_reslin":int})
    curr_add_last_list, Curr_add_last = create_model("Curr_add_last", {"bezeich":string})
    p_list_list, P_list = create_model("P_list", {"betrag":Decimal, "date1":date, "date2":date, "argt":string, "pax":int, "rcode":string})
    ...
    def generate_output():
        ...
        nonlocal dynarate_list_list, ratecode_list_list, output_list_list, t_waehrung_list, q2_reslin_queasy_list, curr_add_last_list, p_list_list, periode_list_list

        return {"rate_found": rate_found, "ci_date": ci_date, "output-list": output_list_list, "p-list": p_list_list, "curr-add-last": curr_add_last_list, "t-waehrung": t_waehrung_list, "q2-reslin-queasy": q2_reslin_queasy_list, "ratecode-list": ratecode_list_list, "rate_flag": rate_flag, "split_price": split_price}
    ...
    def calc_periode():
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 301)],"number1": [(eq, reslin_list.resnr)],"logi1": [(eq, True)]})
    ...
    def disp_query():
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
            ...
            periode_list = query(periode_list_list, filters=(lambda periode_list: periode_list.reslin_queasy.date1 >= periode_list.periode1 and reslin_queasy.date1 <= periode_list.periode2), first=True)

            if periode_list:

                q2_reslin_queasy = query(q2_reslin_queasy_list, filters=(lambda q2_reslin_queasy: q2_reslin_queasy.date1 == periode_list.periode1 and q2_reslin_queasy.date2 == periode_list.periode2), first=True)
                ...
                if not q2_reslin_queasy:
                    ...
                    q2_reslin_queasy.date2 = periode_list.periode2
                ...
                q2_reslin_queasy.deci1 =  to_decimal(q2_reslin_queasy.deci1) + to_decimal(reslin_queasy.deci1)
    ...
    def create_dynarate_list():
        ...
        for dynarate_list in query(dynarate_list_list, sort_by=[("rcode",False)]):
