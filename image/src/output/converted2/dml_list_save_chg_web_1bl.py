...
def dml_list_save_chg_web_1bl(c_list_list:[C_list], user_init:string, curr_dept:int, selected_date:date, curr_select:string):
    ...
    qlist_list, Qlist = create_model("Qlist", {"datum":date, "depart":int, "number1":int})
    ...
    def generate_output():
        ...
        nonlocal supply_list_list, qlist_list
    ...
    for c_list in query(c_list_list):
        ...
        if c_list.approved:
            ...
            qlist = query(qlist_list, filters=(lambda qlist: qlist.datum == selected_date and qlist.depart == curr_dept and qlist.number1 == counter), first=True)

            if not qlist:
                qlist = Qlist()
                qlist_list.append(qlist)
    ...
    if queasy:
        ...
        for qlist in query(qlist_list, sort_by=[("datum",False),("depart",False)]):
            pass
