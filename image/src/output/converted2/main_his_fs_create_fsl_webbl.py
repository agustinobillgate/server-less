...
def main_his_fs_create_fsl_webbl(inresnr:int, inresline:int):
    ...
    bkf_list, Bkf = create_model_like(B_history)
    fsl_list, Fsl = create_model_like(B_history, {"cutoff":date, "grund":[string,18], "in_sales":string, "in_conv":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal curr_gastnr, bkf_list, fsl_list, flag_his, b_history, bk_veran, guest, b_storno, bk_reser
        nonlocal inresnr, inresline


        nonlocal bkf, fsl
        nonlocal bkf_list, fsl_list

        return {"curr_gastnr": curr_gastnr, "bkf": bkf_list, "fsl": fsl_list, "flag_his": flag_his}
