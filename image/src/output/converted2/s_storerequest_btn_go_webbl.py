...
payload_list_list, Payload_list = create_model("Payload_list", {"recid_l_ophdr":int, "transdate":string, "curr_lager":int, "transfered":bool, "cost_acct":string, "deptno":int, "to_stock":int, "lscheinnr":string, "user_init":string, "sr_remark":string})
...
def s_storerequest_btn_go_webbl(payload_list_list:[Payload_list], op_list_list:[Op_list]):
    ...
    response_list_list, Response_list = create_model("Response_list", {"s_artnr":int, "qty":Decimal, "price":Decimal, "created":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal response_list_list, recid_l_ophdr, transdate, curr_lager, transfered, cost_acct, deptno, to_stock, lscheinnr, user_init, sr_remark, s_artnr, qty, price, created, curr_pos, zeit, amount, t_amount, l_op, bediener, l_ophdr, queasy
    ...
    def create_l_op(zeit:int):
        ...
        nonlocal response_list_list, recid_l_ophdr, transdate, curr_lager, transfered, cost_acct, deptno, to_stock, lscheinnr, user_init, sr_remark, s_artnr, qty, price, created, curr_pos, amount, t_amount, l_op, bediener, l_ophdr, queasy
    ...
    if payload_list:
        ...
        transdate = date_mdy(to_int(substring(payload_list.transdate, 0, 2)) , to_int(substring(payload_list.transdate, 3, 2)) , 2000 + timedelta(days=to_int(substring(payload_list.transdate, 6, 2))))
        ...
        response_list = query(response_list_list, first=True)
