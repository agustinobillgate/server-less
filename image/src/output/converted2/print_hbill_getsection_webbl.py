...
def print_hbill_getsection_webbl(pvilanguage:int, user_init:string, print_all:bool, printnr:int, hbrecid:int, use_h_queasy:bool):
    ...
    print_list_list, Print_list = create_model("Print_list", {"str":string, "str_pos":int, "pos":int, "flag_popup":bool, "npause":int, "sort_i":int})
    outlet_print_list_list, Outlet_print_list = create_model("Outlet_print_list", {"sort_i":int, "str_date":string, "str_time":string, "bill_no":string, "resto_name":string, "table_usr_id":string, "curr_time":string, "guest_name":string, "guest_member":string, "od_taker":string, "str_qty":string, "descrip":string, "str_price":string, "foot_note1":string, "foot_note2":string, "table_desc":string}, {"str_date": "", "str_time": "", "bill_no": "", "resto_name": "", "table_usr_id": "", "curr_time": "", "guest_name": "", "guest_member": "", "od_taker": "", "str_qty": "", "descrip": "", "str_price": "", "foot_note1": "", "foot_note2": "", "table_desc": ""})

    Paylist = Print_list
    paylist_list = print_list_list

    db_session = local_storage.db_session

    def generate_output():
        nonlocal msg_str, room_no, guest_name, outlet_print_list_list, lvcarea, winprinterflag, session_parameter, filename, table_no, usr_id, counter, gs, pay_str, depopay_str, pay_amount, curr_amount, curr_pay, rechnr_str, flag_change, flag_pay, flag_vat, foot_note, guest_addr, guest_addr1, count_i, gs_1, fo_depoart, fo_depobez, fb_depoart, fb_depobez, voucher_depo, gastno, ns_billno, curr_dept, str1, printer, h_bill, res_line, guest
        ...
        nonlocal t_printer_list, output_list_list, print_list_list, outlet_print_list_list

        return {"msg_str": msg_str, "room_no": room_no, "guest_name": guest_name, "outlet-print-list": outlet_print_list_list}
    ...
    print_all, filename, msg_str, winprinterflag, print_list_list, t_printer_list = get_output(print_hbill1_cldbl(pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all))

    if msg_str != "":

        return generate_output()

    for print_list in query(print_list_list, sort_by=[("sort_i",False)]):
        outlet_print_list = Outlet_print_list()
        outlet_print_list_list.append(outlet_print_list)
        ...
        if print_list.str_pos == 1:
            ...
            if trim(entry(5, print_list.str, "|")) != "":
                outlet_print_list.table_desc = trim(entry(5, print_list.str, "|"))
            outlet_print_list.table_usr_id = translateExtended (table_desc , lvcarea, "") + " / " + usr_id.upper()
            ...
            outlet_print_list.sort_i = -1
            outlet_print_list.descrip = fill("-", 42)
            outlet_print_list.str_price = fill("-", 19)
