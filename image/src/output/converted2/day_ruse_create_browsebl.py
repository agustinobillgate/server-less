...
def day_ruse_create_browsebl(from_room:str, curr_date:date):
    room_list_list = []
    zimmer = zimkateg = zinrstat = None

    room_list = None

    room_list_list, Room_list = create_model("Room_list", {"recid1":[int,18], "etage":int, "zinr":str, "c_char":str, "i_char":str, "zikatnr":int, "rmcat":str, "gstatus":[int,18], "room":[str,18]})
    ...
    def create_browse():
        ...
        for zimmer in db_session.query(Zimmer).filter(
                 (func.lower(Zimmer.zinr) >= (from_room).lower())).order_by(Zimmer._recid).all():
        ...
        for zinrstat in db_session.query(Zinrstat).filter(
                 ((Zinrstat.datum >= curr_date) & (Zinrstat.datum <= to_date)) & (func.lower(Zinrstat.zinr) >= (from_room).lower()) & (Zinrstat.zimmeranz > 0)).order_by(Zinrstat._recid).all():
            ...
            if room_list:
                datum1 = curr_date
                datum2 = to_date
                i = (zinrstat.datum - datum1).days + 1
