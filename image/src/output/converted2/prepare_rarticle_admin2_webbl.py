...
def prepare_rarticle_admin2_webbl(dept:int):
    ...
    wbuff_list, Wbuff = create_model("Wbuff", {"departement":int, "zknr":int, "bez":string})
    q1_list_list, Q1_list = create_model_like(H_artikel, {"bez":string, "zknr":int, "bezeich2":string, "zk_bezeich":string, "ek_bezeich":string, "fart_bezeich":string, "fo_dept":int, "barcode":string, "max_soldout_qty":int, "soldout_flag":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal long_digit, d_bezeich, only_corp_access, q1_list_list, h_artikel, htparam, wgrpdep, hoteldpt, queasy, wgrpgen, artikel
        nonlocal dept
        ...
        return {"long_digit": long_digit, "d_bezeich": d_bezeich, "only_corp_access": only_corp_access, "q1-list": q1_list_list}
    ...
    for h_artikel in db_session.query(H_artikel).filter(
        ...
        wbuff = query(wbuff_list, (lambda wbuff: wbuff.zknr == h_artikel.zwkum and wbuff.departement == h_artikel.departement), first=True)
        if not wbuff:
            continue
        ...
        artikel = get_cache (Artikel, {"artnr": [(eq, h_artikel.artnrfront)],"departement": [(eq, fo_dept)]})
        ...
        if queasy:
            q1_list.soldout_flag = queasy.logi2
            q1_list.max_soldout_qty = to_int(queasy.deci1)
