...
def hk_ooo_ombl(fdate:date, tdate:date, disptype:int, sorttype:int):
    ...
    om_list2_list, Om_list2 = create_model("Om_list2", {"zinr":string, "userinit":string, "ind":int, "reason":string, "gespstart":date, "gespende":date})
    om_list_list, Om_list = create_model("Om_list", {"zinr":string, "userinit":string, "ind":int, "reason":string, "gespstart":date, "gespende":date, "rec_id":int})
    ooo_list_list, Ooo_list = create_model("Ooo_list", {"zinr":string, "gespgrund":string, "gespstart":date, "gespende":date, "userinit":string, "etage":int, "ind":int, "bezeich":string, "betriebsnr":int, "selected_om":bool, "rec_id":int})
    t_ooo_list_list, T_ooo_list = create_model("T_ooo_list", {"zinr":string, "gespgrund":string, "gespstart":date, "gespende":date, "userinit":string, "etage":int, "ind":int, "bezeich":string, "betriebsnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal ci_date, om_list_list, ooo_list_list, user_init, outorder
