...
def printsplit_hbill_getsection_webbl(pvilanguage:int, user_init:string, print_all:bool, printnr:int, hbrecid:int, use_h_queasy:bool, billnr:int):
    ...
    def generate_output():
        ...
        nonlocal t_printer_list, output_list_list, print_list_list, outlet_split_bill_list

        return {"msg_str": msg_str, "room_no": room_no, "guest_name": guest_name, "outlet-split-bill": outlet_split_bill_list}
    ...
    print_all, winprinterflag, filename, msg_str, print_list_list, t_printer_list = get_output(pr_sphbill1_cldbl(pvilanguage, hbrecid, printnr, use_h_queasy, session:parameter, user_init, billnr, print_all))

    if msg_str != "":

        return generate_output()

    for print_list in query(print_list_list, sort_by=[("str_pos",False)]):
        outlet_split_bill = Outlet_split_bill()
        outlet_split_bill_list.append(outlet_split_bill)
