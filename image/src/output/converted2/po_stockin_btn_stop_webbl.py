...
t_l_order_list, T_l_order = create_model_like(L_order, {"rec_id":int, "art_bezeich":string, "jahrgang":int, "alkoholgrad":Decimal, "lief_einheit":Decimal, "curr_disc":int, "curr_disc2":int, "curr_vat":int})

def po_stockin_btn_stop_webbl(user_init:string, l_orderhdr_recid:int, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, lief_nr:int, billdate:date, docu_nr:string, lscheinnr:string, crterm:int, curr_lager:int, t_amount:Decimal, op_list_list:[Op_list], t_l_order_list:[T_l_order]):
