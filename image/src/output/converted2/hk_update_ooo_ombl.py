...
def hk_update_ooo_ombl(case_type:int, user_init:string, rec_id:int, from_date:date, to_date:date, service_flag:bool, reason:string, dept:int):
    ...
    if outorder:
        ...
        user_nr = bediener.nr
        msg_int, resno, resname, ankunft, abreise, ooo_list_ind = get_output(hk_ooo1bl(case_type, t_outorder_list, from_date, to_date, service_flag, t_outorder.zinr, user_nr, reason, dept, user_init))
