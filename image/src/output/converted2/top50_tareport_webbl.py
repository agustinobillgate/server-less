...
def top50_tareport_webbl(pvilanguage:int, curr_date:string, curr_month:int, sorttype:int, disptype:int):
    ...
    top50_list_list, Top50_list = create_model("Top50_list", {"num":int, "nr":int, "bezeich":string, "room":[int,12], "ytd":int, "lytd":int, "mtd":int, "gastnr":int})
    ...
    def generate_output():
        nonlocal top50_list_list, mm, yy, i, from_date, to_date, curr_date_loop, datum, jml, lvcarea, res_line, guest, genstat
    ...
    if sorttype == 0:
        ...
        for top50_list in query(top50_list_list, filters=(lambda top50_list: top50_list.ytd != 0 or top50_list.lytd != 0), sort_by=[("ytd",True),("bezeich",False)]):
            i = i + 1
            top50_list.nr = i
            top50_list.num = i
        ...
        for top50_list in query(top50_list_list, filters=(lambda top50_list: top50_list.mtd != 0 or top50_list.lytd != 0), sort_by=[("mtd",True),("bezeich",False)]):
            i = i + 1
            top50_list.nr = i
            top50_list.num = i
    ...
    for r_list in query(r_list_list, filters=(lambda r_list: r_list.num >= 1 and r_list.num <= 50)):
