...
if_list_list, If_list = create_model("If_list", {"perideno":int, "send_date":date, "fr_date":date, "to_date":date, "sendflag":bool, "resendflag":bool})

def netsuite_check_schedullerbl(casetype:int, month_val:int, if_list_list:[If_list]):
    ...
    period_list, Period = create_model("Period", {"perideno":int, "send_date":date, "from_date":date, "to_date":date})
    ...
    def generate_output():
        ...
        nonlocal period_list

        return {"if-list": if_list_list}
    ...
    if casetype == 1:
        ...
        for period in query(period_list, sort_by=[("perideno",True)]):
        ...
        for period in query(period_list):
        ...
        for if_list in query(if_list_list):
