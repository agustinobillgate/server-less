...
def hk_count_1_webbl(pvilanguage:int):
    ...
    depart_today_list, Depart_today = create_model("Depart_today", {"room":string, "rmtype":string, "gname":string})
    departed_list, Departed = create_model("Departed", {"room":string, "rmtype":string, "gname":string})
    arrival_today_list, Arrival_today = create_model("Arrival_today", {"room":string, "rmtype":string, "gname":string})
    arrived_list, Arrived = create_model("Arrived", {"room":string, "rmtype":string, "gname":string, "arrtime":string, "rsv_remark":string, "guest_pref":string, "room_status":string, "pci":bool})
    vacant_clean_checked_list, Vacant_clean_checked = create_model("Vacant_clean_checked", {"room":string, "rmtype":string, "gname":string})
    vacant_clean_unchecked_list, Vacant_clean_unchecked = create_model("Vacant_clean_unchecked", {"room":string, "rmtype":string, "gname":string})
    occ_clean_list, Occ_clean = create_model("Occ_clean", {"room":string, "rmtype":string, "gname":string})
    occ_dirty_list, Occ_dirty = create_model("Occ_dirty", {"room":string, "rmtype":string, "gname":string})
    vacant_dirty_list, Vacant_dirty = create_model("Vacant_dirty", {"room":string, "rmtype":string, "gname":string})
    ex_depart_list, Ex_depart = create_model("Ex_depart", {"room":string, "rmtype":string, "gname":string})
    off_market_list, Off_market = create_model("Off_market", {"room":string, "rmtype":string, "gname":string})
    ooo_room_list, Ooo_room = create_model("Ooo_room", {"room":string, "rmtype":string, "gname":string})
    paying_room_list, Paying_room = create_model("Paying_room", {"room":int, "pax":int, "percentage":Decimal})
    queue_room_list_list, Queue_room_list = create_model_like(Queasy)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal departed1, departed2, departing1, departing2, tot_depart1, tot_depart2, arrived1, arrived2, arriving1, arriving2, tot_arrive1, tot_arrive2, vclean, vuncheck, oclean, tot_clean, odirty, vdirty, atoday, tot_dirty, oroom1, oroom2, omroom1, omroom2, oooroom1, oooroom2, comproom1, comproom2, houseroom1, houseroom2, iroom1, iroom2, eocc1, eocc2, proz1, proz2, proz3, proz4, proz5, queue_room_list_list, depart_today_list, departed_list, arrival_today_list, arrived_list, vacant_clean_checked_list, vacant_clean_unchecked_list, occ_clean_list, occ_dirty_list, vacant_dirty_list, ex_depart_list, off_market_list, ooo_room_list, paying_room_list, lvcarea, ci_date, loop_i, str, str1, loop_j, loopj, resbemerk, queasy, htparam, zimmer, res_line, zimkateg, guest, reservation, segment, outorder
    ...
    def init_var():
        ...
        paying_room_list.append(paying_room)
