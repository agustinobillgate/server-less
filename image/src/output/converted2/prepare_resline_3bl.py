...
from models import Res_line, History, Zimkateg, Ratecode, Zimmer, Guest, Htparam, Queasy, Nation, Bediener, Master, Reslin_queasy, Reservation, Kontline, Gentable, Outorder, Arrangement, Guest_pr, Pricecod, Prmarket, Fixleist, Paramtext, Waehrung, Katpreis
...
def prepare_resline_3bl(pvilanguage:int, res_mode:string, session_date:string, user_init:string, inp_gastnr:int, inp_resnr:int, inp_reslinnr:int, rate_readonly:bool, qci_zinr:string, res_dynarate_list:[Res_dynarate]):

    prepare_cache ([Res_line, Zimkateg, Ratecode, Zimmer, Guest, Htparam, Queasy, Nation, Bediener, Reservation, Kontline, Arrangement, Guest_pr, Prmarket, Paramtext, Waehrung, Katpreis])
    ...
    new_reslinnr:int = 1
    curr_time:int = 0
    res_line = history = zimkateg = ratecode = zimmer = guest = htparam = queasy = nation = bediener = master = reslin_queasy = reservation = kontline = gentable = outorder = arrangement = guest_pr = pricecod = prmarket = fixleist = paramtext = waehrung = katpreis = None

    rline_list = reslin_list = curr_resline = t_history = currency_list = res_dynarate = reschanged_list = f_resline = nation_list = resline = resbuff = zimkateg1 = rbuff = qci_zimmer = bresline = bguest = qsy = None

    rline_list_list, Rline_list = create_model_like(Res_line, {"res_char":string, "rsvname":string, "kurzbez":string, "status_str":string})
    ...
    def generate_output():
        nonlocal msg_str, error_flag, record_use, init_time, init_date, avail_gdpr, avail_mark, avail_news, save_gdpr, curr_date, serv_date, f_resline_list, curr_resline_list, reslin_list_list, reschanged_list_list, t_history_list, rline_list_list, weekdays, i, str, loopi, loopj, str1, foreign_nr, tokcounter, iftask, mestoken, mesvalue, rcode, prevcode, do_it, do_it1, flag_ok, dayuse_flag, split_modify, logic_p1109, priscilla_active, loopk, resbemerk, lvcarea, new_reslinnr, curr_time, res_line, history, zimkateg, ratecode, zimmer, guest, htparam, queasy, nation, bediener, master, reslin_queasy, reservation, kontline, gentable, outorder, arrangement, guest_pr, pricecod, prmarket, fixleist, paramtext, waehrung, katpreis
        nonlocal pvilanguage, res_mode, session_date, user_init, inp_gastnr, inp_resnr, inp_reslinnr, rate_readonly, qci_zinr
        nonlocal resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy


        nonlocal rline_list, reslin_list, curr_resline, t_history, currency_list, res_dynarate, reschanged_list, f_resline, nation_list, resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy
        nonlocal rline_list_list, reslin_list_list, curr_resline_list, t_history_list, currency_list_list, reschanged_list_list, f_resline_list, nation_list_list
    ...
    def split_resline():
        ...
        nonlocal error_flag, record_use, init_time, init_date, avail_gdpr, avail_mark, avail_news, save_gdpr, curr_date, serv_date, f_resline_list, curr_resline_list, reslin_list_list, reschanged_list_list, t_history_list, rline_list_list, weekdays, str, loopi, loopj, str1, foreign_nr, tokcounter, iftask, mestoken, mesvalue, rcode, prevcode, do_it, do_it1, flag_ok, dayuse_flag, split_modify, logic_p1109, priscilla_active, loopk, resbemerk, lvcarea, new_reslinnr, curr_time, res_line, history, zimkateg, ratecode, zimmer, guest, htparam, queasy, nation, bediener, master, reslin_queasy, reservation, kontline, gentable, outorder, arrangement, guest_pr, pricecod, prmarket, fixleist, paramtext, waehrung, katpreis
        nonlocal pvilanguage, res_mode, session_date, user_init, inp_gastnr, inp_resnr, inp_reslinnr, rate_readonly, qci_zinr
        nonlocal resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy
    ...
    def get_currency():
        ...
        gbuff = get_cache (Guest, {"gastnr": [(eq, inp_gastnr)]})
    ...
    def set_roomrate(direct_change:bool):
        ...
        nonlocal msg_str, error_flag, record_use, init_time, init_date, avail_gdpr, avail_mark, avail_news, save_gdpr, curr_date, serv_date, f_resline_list, curr_resline_list, reslin_list_list, reschanged_list_list, t_history_list, rline_list_list, weekdays, str, loopi, loopj, str1, foreign_nr, tokcounter, iftask, mestoken, mesvalue, rcode, prevcode, do_it, do_it1, flag_ok, dayuse_flag, split_modify, logic_p1109, priscilla_active, loopk, resbemerk, lvcarea, new_reslinnr, curr_time, res_line, history, zimkateg, ratecode, zimmer, guest, htparam, queasy, nation, bediener, master, reslin_queasy, reservation, kontline, gentable, outorder, arrangement, guest_pr, pricecod, prmarket, fixleist, paramtext, waehrung, katpreis
        nonlocal pvilanguage, res_mode, session_date, user_init, inp_gastnr, inp_resnr, inp_reslinnr, rate_readonly, qci_zinr
        nonlocal resline, resbuff, zimkateg1, rbuff, qci_zimmer, bresline, bguest, qsy
        ...
        if reslin_list.erwachs == 0 and reslin_list.kind1 == 0 and reslin_list.kind2 == 0:
            ...
            if not rate_found:
                ...
                if res_mode.lower()  == ("inhouse").lower() :
                    ...
                    katpreis = get_cache (Katpreis, {"zikatnr": [(eq, curr_zikatnr)],"argtnr": [(eq, arrangement.argtnr)],"startperiode": [(le, datum)],"endperiode": [(ge, datum)],"betriebsnr": [(eq, tmpint)]})
    ...
    def check_dynarate():
        ...
        if not res_dynarate or res_dynarate.rmcat == "" or res_dynarate.rmcat == " " or res_dynarate.rmcat == None:
    ...
    if res_line.memozinr != None and res_line.memozinr != "":
    ...
    if f_resline.new_contrate:
        ...
        guest_pr = Guest_pr()
        queasy = Queasy()
        for guest_pr.code, guest_pr._recid, queasy.char3, queasy._recid in db_session.query(Guest_pr.code, Guest_pr._recid, Queasy.char3, Queasy._recid).join(Queasy,(Queasy.key == 2) & (Queasy.logi2) & (Queasy.char1 == Guest_pr.code)).filter(
                 (Guest_pr.gastnr == inp_gastnr)).order_by(Guest_pr._recid).all():
    ...
    if res_line.resstatus != 11 and res_line.resstatus != 13:
        ...
        if guest_pr:
            ...
            guest_pr = Guest_pr()
            queasy = Queasy()
            for guest_pr.code, guest_pr._recid, queasy.char3, queasy._recid in db_session.query(Guest_pr.code, Guest_pr._recid, Queasy.char3, Queasy._recid).join(Queasy,(Queasy.key == 2) & (Queasy.char1 == Guest_pr.code)).filter(
    ...
    if not split_modify:
        rline_list_list, reschanged_list_list = get_output(mk_resline_query_q1bl(pvilanguage, True, True, inp_resnr, rline_list_list, reschanged_list_list))
