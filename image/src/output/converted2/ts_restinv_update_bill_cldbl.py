...
def ts_restinv_update_bill_cldbl(pvilanguage:int, rec_id:int, rec_id_h_artikel:int, deptname:string, transdate:date, h_artart:int, cancel_order:bool, h_artikel_service_code:int, amount:Decimal, amount_foreign:Decimal, price:Decimal, double_currency:bool, qty:int, exchg_rate:Decimal, price_decimal:int, order_taker:int, tischnr:int, curr_dept:int, curr_waiter:int, gname:string, pax:int, kreditlimit:Decimal, add_zeit:int, billart:int, description:string, change_str:string, cc_comment:string, cancel_str:string, req_str:string, voucher_str:string, hoga_card:string, print_to_kitchen:bool, from_acct:bool, h_artnrfront:int, pay_type:int, guestnr:int, transfer_zinr:string, curedept_flag:bool, foreign_rate:bool, curr_room:string, user_init:string, hoga_resnr:int, hoga_reslinnr:int, incl_vat:bool, get_price:int, mc_str:string, segment_code:int, submenu_list_list:[Submenu_list]):

    prepare_cache ([H_bill, H_bill_line, H_mjourn, H_artikel, Hoteldpt, Artikel, Htparam, Queasy, Guest, Counters, H_umsatz, H_journal, Umsatz, Interface, Arrangement, Argt_line, Billjournal])
    ...
    t_kellner1 = t_h_bill = submenu_list = h_bline = kellner1 = bbill = buf_hbill = mjou = bill_guest = None
    ...
    Buf_hbill = create_buffer("Buf_hbill",H_bill)
    ...
    if rec_id == 0:
        ...
        buf_hbill = get_cache (H_bill, {"flag": [(eq, 0)],"tischnr": [(eq, tischnr)],"departement": [(eq, curr_dept)]})

        if buf_hbill:

            h_bill_line = get_cache (H_bill_line, {"rechnr": [(eq, buf_hbill.rechnr)],"departement": [(eq, buf_hbill.departement)]})
    ...
    if h_bill:
        ...
        bbill = get_cache (H_bill, {"tischnr": [(eq, tischnr)],"departement": [(eq, curr_dept)],"flag": [(eq, 0)]})
