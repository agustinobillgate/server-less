...
t_printer_list, T_printer = create_model("T_printer", {"nr":int, "path":string, "copies":int, "make":string, "emu":string, "position":string, "pglen":int, "spooled":bool, "bondrucker":bool, "opsysname":string})

def write_printer_web1bl(case_type:int, recid_emu:int, t_printer_list:[T_printer]):
    ...
    t_printer = query(t_printer_list, first=True)
