...
guest_list_list, Guest_list = create_model("Guest_list", {"guestnumber":int, "guestrefno":string, "guestname":string, "guesttype":string, "address":string, "selected":bool})

def industry_type_config_addbl(selected_category:int, user_init:string, guest_list_list:[Guest_list]):
    ...
    def generate_output():
        nonlocal str_msg, temp_list_list, nr, category, gname, assigned, queasy, bediener, res_history
        ...
        return {"str_msg": str_msg, "temp-list": temp_list_list}
    ...
    def add_guest():
        ...
        b_queasy = get_cache (Queasy, {"key": [(eq, 281)],"number1": [(eq, selected_category)]})
        ...
        for guest_list in query(guest_list_list):
            ...
            for queasy in db_session.query(Queasy).filter(
                     (Queasy.key == 282) & (Queasy.number1 == selected_category)).order_by(Queasy.number2.desc()).yield_per(100):
                nr = queasy.number2 + 1
                break

            queasy = get_cache (Queasy, {"key": [(eq, 282)],"number3": [(eq, guest_list.guestnumber)]})
