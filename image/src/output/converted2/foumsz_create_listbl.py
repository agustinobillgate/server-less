...
def foumsz_create_listbl(from_dept:int, to_dept:int, first_date:date, from_date:date, to_date:date, totvatflag:bool):
    ...
    not_avail_umstaz_list, Not_avail_umstaz = create_model("Not_avail_umstaz", {"artnr":int, "depart":int, "bezeich":string})
    ...
    def generate_output():
        nonlocal umsz_list_list, vhp_limited, vat_str, vat_artnr, serv_artnr, price_decimal, htparam, umsatz, artikel, budget, hoteldpt, kontplan
    ...
    def create_umsatz():
        ...
        umsatz = Umsatz()
        artikel = Artikel()
        for umsatz.artnr, umsatz.departement, umsatz.datum, umsatz.betrag, umsatz.anzahl, umsatz._recid, artikel.artnr, artikel.departement, artikel.bezeich, artikel._recid in db_session.query(Umsatz.artnr, Umsatz.departement, Umsatz.datum, Umsatz.betrag, Umsatz.anzahl, Umsatz._recid, Artikel.artnr, Artikel.departement, Artikel.bezeich, Artikel._recid).join(Artikel,(Artikel.artnr == Umsatz.artnr) & (Artikel.departement == Umsatz.departement) & ((Artikel.artart == 0) | (Artikel.artart == 8))).filter(
                ...
                if (curr_departement != umsatz.departement) or counter == 1:
                    ...
                    if counter != 1:
                        ...
                        if totvatflag:
                            ...
                            for vat_list in query(vat_list_list, filters=(lambda vat_list: vat_list.dptnr == hoteldpt.num), sort_by=[("vat",False)]):
                                cl_list = Cl_list()
                                cl_list_list.append(cl_list)
        ...
        if totvatflag:
            ...
            for vat_list in query(vat_list_list, sort_by=[("vat",False)]):
            ...
            for gvat_list in query(gvat_list_list, sort_by=[("vat",False)]):
                cl_list = Cl_list()
                cl_list_list.append(cl_list)
