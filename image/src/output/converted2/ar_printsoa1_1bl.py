...
from models import Guest, Bill, Debitor, Bill_line, H_bill_line, Res_line, Artikel, Queasy, Zimkateg, Reservation, H_bill, Hoteldpt, H_artikel

param_ar_list, Param_ar = create_model("Param_ar", {"param_nr":int, "param_name":string, "param_val":string, "param_type":string})

def ar_printsoa1_1bl(show_type:int, bof_month:date, eof_month:date, param_ar_list:[Param_ar], guestno:int, curr_day:date):

    prepare_cache ([Guest, Bill, Debitor, Bill_line, H_bill_line, Res_line, Artikel, Zimkateg, Reservation, H_bill, Hoteldpt, H_artikel])
    ...
    arrival:date = None
    departure:date = None
    debt_amount:Decimal = to_decimal("0.0")
    ...
    val_param:int = 0
    var_param:string = ""
    guest = bill = debitor = bill_line = h_bill_line = res_line = artikel = queasy = zimkateg = reservation = h_bill = hoteldpt = h_artikel = None

    soa_list = soabuff = param_ar = gast = bill1 = debt = debt1 = billine = hbilline = bresline = None

    soa_list_list, Soa_list = create_model("Soa_list", {"counter":int, "debref":int, "done_step":int, "artno":int, "vesrdep":Decimal, "to_sort":int, "outlet":bool, "datum":date, "ankunft":date, "abreise":date, "gastnr":int, "name":string, "inv_str":string, "rechnr":int, "refno":int, "voucherno":string, "voucherno1":string, "voucherno2":string, "saldo":Decimal, "fsaldo":Decimal, "printed":bool, "selected":bool, "printdate":date, "dptnr":int, "debt":Decimal, "credit":Decimal, "fdebt":Decimal, "fcredit":Decimal, "remarks":string, "arrecid":int, "newpayment":Decimal, "newfpayment":Decimal, "zinr":string, "erwachs":int, "child1":int, "child2":int, "roomrate":Decimal, "tot_amount":Decimal, "tot_balance":Decimal, "exrate":Decimal, "tot_exrate":Decimal, "gst_tot_non_taxable":Decimal, "gst_amount":Decimal, "gst_tot_sales":Decimal, "zimmeranz":int, "bill_datum":date, "resv_name":string}, {"printdate": get_current_date()})
    ...
    def generate_output():
        nonlocal zeit, zeit1, str, due_date, soa_list_list, rech_nr, bet_nr, msg_int, param1, param2, param3, param4, fnet_lodging, lnet_lodging, net_breakfast, net_lunch, net_dinner, net_others, tot_rmrev, nett_vat, nett_service, curr_gastnr, curr_refno, new_refno, cl_exist, saldo, s, i, arrival, departure, debt_amount, crdt, fdebt, fcrdt, new_crdt, new_fcrdt, isprintedb4, saldo1, saldo2, saldo3, saldo4, other_flag, count_zimm, nmonth, mnth1, year2, val_param, var_param, guest, bill, debitor, bill_line, h_bill_line, res_line, artikel, queasy, zimkateg, reservation, h_bill, hoteldpt, h_artikel
        ...
        return {"param-ar": param_ar_list, "due_date": due_date, "soa-list": soa_list_list, "rech_nr": rech_nr, "bet_nr": bet_nr, "msg_int": msg_int}

    def create_soalist():

        nonlocal zeit, zeit1, str, due_date, soa_list_list, rech_nr, bet_nr, msg_int, param1, param2, param3, param4, fnet_lodging, lnet_lodging, net_breakfast, net_lunch, net_dinner, net_others, tot_rmrev, nett_vat, nett_service, curr_gastnr, curr_refno, new_refno, cl_exist, saldo, s, i, arrival, departure, debt_amount, crdt, fdebt, fcrdt, new_crdt, new_fcrdt, isprintedb4, saldo1, saldo2, saldo3, saldo4, other_flag, count_zimm, guest, bill, debitor, bill_line, h_bill_line, res_line, artikel, queasy, zimkateg, reservation, h_bill, hoteldpt, h_artikel
        nonlocal show_type, bof_month, eof_month, guestno, curr_day
        nonlocal soabuff, gast, bill1, debt, debt1, billine, hbilline, bresline
        ...
        if show_type == 0:
            ...
            debitor = Debitor()
            artikel = Artikel()
            for debitor.gastnr, debitor.rechnr, debitor.betriebsnr, debitor._recid, debitor.rgdatum, debitor.vesrcod, debitor.artnr, debitor.counter, debitor.debref, debitor.saldo, debitor.vesrdep, debitor.gastnrmember, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debitor.gastnr, Debitor.rechnr, Debitor.betriebsnr, Debitor._recid, Debitor.rgdatum, Debitor.vesrcod, Debitor.artnr, Debitor.counter, Debitor.debref, Debitor.saldo, Debitor.vesrdep, Debitor.gastnrmember, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel,(Artikel.artnr == Debitor.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter(
            ...
            for soa_list in query(soa_list_list, filters=(lambda soa_list: soa_list.done_step == 0)):
                crdt =  to_decimal("0")
                debt_amount =  to_decimal("0")
                fcrdt =  to_decimal("0")
                fdebt =  to_decimal("0")
                ...
                for soabuff in query(soabuff_list, filters=(lambda soabuff: soabuff.rechnr == soa_list.rechnr and soabuff.dptnr == soa_list.dptnr and soabuff.artno == soa_list.artno and soabuff.done_step == 0)):
                    soabuff.done_step = 1
                    debt_amount =  to_decimal(soabuff.saldo)
                    ...
                    for debt.gastnr, debt.rechnr, debt.betriebsnr, debt._recid, debt.rgdatum, debt.vesrcod, debt.artnr, debt.counter, debt.debref, debt.saldo, debt.vesrdep, debt.gastnrmember, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debt.gastnr, Debt.rechnr, Debt.betriebsnr, Debt._recid, Debt.rgdatum, Debt.vesrcod, Debt.artnr, Debt.counter, Debt.debref, Debt.saldo, Debt.vesrdep, Debt.gastnrmember, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel,(Artikel.artnr == Debt.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter(
                        ...
                        debt_amount =  to_decimal(debt_amount) + to_decimal(debt.saldo)
                ...
                for soabuff in query(soabuff_list, filters=(lambda soabuff: soabuff.done_step == 1)):
                    soabuff.done_step = 2
                    soabuff.debt =  to_decimal(debt_amount)
                    soabuff.credit =  to_decimal(crdt)
                    soabuff.saldo =  to_decimal(debt_amount) - to_decimal(crdt)
            ...
            for soa_list in query(soa_list_list, filters=(lambda soa_list: soa_list.done_step == 0)):
                ...
                for soabuff in query(soabuff_list, filters=(lambda soabuff: soabuff.rechnr == soa_list.rechnr and soabuff.dptnr == soa_list.dptnr and soabuff.artno == soa_list.artno and soabuff.done_step == 0 and soabuff.arrecid == soa_list.arrecid)):
                    soabuff.done_step = 1
                    debt_amount =  to_decimal(soabuff.saldo)
                    fdebt =  to_decimal(fdebt) + to_decimal(soabuff.vesrdep)
            ...
            for debitor, guest in db_session.query(Debitor, Guest).join(Guest,(Guest.gastnr == Debitor.gastnr)).filter(
                     (Debitor._recid.in_(list(set([soa_list.arrecid for soa_list in soa_list_list]))))).order_by(Debitor._recid).all():
                if debitor_obj_list.get(debitor._recid):
                    continue
                else:
                    debitor_obj_list[debitor._recid] = True

                soa_list = query(soa_list_list, (lambda soa_list: (debitor._recid == soa_list.arrecid)), first=True)
                crdt =  to_decimal("0")
                debt_amount =  to_decimal("0")
                fcrdt =  to_decimal("0")
                fdebt =  to_decimal("0")
                ...
                if debitor.debref != 0:
                    isprintedb4 = True
                debt_amount =  to_decimal(debitor.saldo)
