...
def check_record_bedienerbl(pvilanguage:int, userinit:str, bed_userinit:str, nr:int):
    its_ok = True
    msg_str = ""
    lvcarea:str = "check-record-bediener"
    ...
    def check_record():
        ...
        bill_line = db_session.query(Bill_line).filter(
                 (func.lower(Bill_line.userinit) == (userinit).lower())).first()
        ...
        billjournal = db_session.query(Billjournal).filter(
                 (func.lower(Billjournal.userinit) == (userinit).lower())).first()
        ...
        if zimmer:
            its_ok = False
            msg_str = msg_str + chr(2) + translateExtended ("Last status changed of Room-# :", lvcarea, "") + zimmer.zinr + " " + translateExtended ("by this user-id, deleting not possible.", lvcarea, "")
