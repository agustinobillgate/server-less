...
def eg_maincalendardel_disp_webbl(from_date:date, to_date:date, user_init:string, all_room:bool):
    ...
    def fill_maintain():
        ...
        for maintain in query(maintain_list):
            tStatus = query(tstatus_list, (lambda tStatus: tStatus.stat_nr == maintain.type), first=True)
            if not tStatus:
                continue

            tmaintask = query(tmaintask_list, (lambda tmaintask: tmaintask.main_nr == maintain.maintask), first=True)
            if not tmaintask:
                continue

            tlocation = query(tlocation_list, (lambda tlocation: tlocation.loc_nr == maintain.location), first=True)
            if not tlocation:
                continue

            tproperty = query(tproperty_list, (lambda tproperty: tproperty.prop_nr == maintain.propertynr), first=True)
            if not tproperty:
                continue

            tpic = query(tpic_list, (lambda tpic: tpic.pic_nr == maintain.pic), first=True)
            if not tpic:
                continue

            tcategory = query(tcategory_list, (lambda tcategory: tcategory.categ_nr == maintain.categnr), first=True)
            if not tcategory:
                continue
