...
in_list_list, In_list = create_model("In_list", {"lscheinr":string})
s_list_list, S_list = create_model("S_list", {"datum":date, "lief_nr":int, "lager":int, "docu_nr":string, "lscheinnr":string, "userno":int, "zeit":int, "amount":Decimal, "storno":string, "loeschflag":int, "artnr":int, "inv_dept":bool, "user_init":string})

def check_incoming_ap_webbl(in_list_list:[In_list], s_list_list:[S_list]):
    ...
    rcv_list_list, Rcv_list = create_model("Rcv_list", {"datum":date, "lief_nr":int, "lager":int, "docu_nr":string, "lscheinnr":string, "userno":int, "zeit":int, "amount":Decimal, "storno":string, "loeschflag":int, "artnr":int, "inv_dept":bool, "user_init":string})
    out_list_list, Out_list = create_model("Out_list", {"msg_str":string})
    ...
    def generate_output():
        nonlocal out_list_list, amount, ct, l_kredit, ap_journal, queasy, bediener
        nonlocal b_kredit, buff_kredit, b_journal


        nonlocal in_list, s_list, rcv_list, out_list, b_kredit, buff_kredit, b_journal
        nonlocal rcv_list_list, out_list_list

        return {"out-list": out_list_list}


    in_list = query(in_list_list, first=True)

    if in_list:

        for s_list in query(s_list_list, filters=(lambda s_list: s_list.lscheinnr == in_list.lscheinr)):
            amount =  to_decimal("0")
            ...
            rcv_list = query(rcv_list_list, filters=(lambda rcv_list: rcv_list.docu_nr == s_list.docu_nr and rcv_list.lschein == s_list.lscheinnr), first=True)

            if not rcv_list:
                rcv_list = Rcv_list()
                rcv_list_list.append(rcv_list)
        ...
        rcv_list = query(rcv_list_list, first=True)
