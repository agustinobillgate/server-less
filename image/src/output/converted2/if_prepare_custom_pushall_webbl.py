...
def if_prepare_custom_pushall_webbl(becode:int):
    ...
    rmtype_list_list, Rmtype_list = create_model("Rmtype_list", {"bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal from_date, to_date, maxdate, t_list_list, t_push_list_list, rmtype_list_list, i, str, queasy, htparam
        nonlocal becode


        nonlocal t_list, t_push_list, rmtype_list
        nonlocal t_list_list, t_push_list_list, rmtype_list_list

        return {"from_date": from_date, "to_date": to_date, "maxdate": maxdate, "t-list": t_list_list, "t-push-list": t_push_list_list, "rmtype-list": rmtype_list_list}
    ...
    t_push_list = query(t_push_list_list, first=True)

    if t_push_list:
        rmtype_list = Rmtype_list()
        rmtype_list_list.append(rmtype_list)

        rmtype_list.bezeich = "*"

        for t_push_list in query(t_push_list_list):

            rmtype_list = query(rmtype_list_list, filters=(lambda rmtype_list: rmtype_list.bezeich == t_push_list.rmtypevhp), first=True)
