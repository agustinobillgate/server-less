...
def chk_btn_go_res_rmrate1bl(pvilanguage:int, curr_select:string, max_rate:Decimal, fact1:Decimal, inp_wahrnr:int, inp_zikatnr:int, user_init:string, resnr:int, reslinnr:int, recid_reslin:int, contcode:string, p_list_list:[P_list]):
    ...
    t_reslin_queasy_list, T_reslin_queasy = create_model("T_reslin_queasy", {"date1":date, "date2":date, "deci1":Decimal, "char1":string, "number3":int, "char2":string, "char3":string, "recid_reslin":int})
    ...
    def generate_output():
        nonlocal msg_str, error_found1, error_code, t_reslin_queasy_list, lvcarea, error_found, exrate2, wd_array, reslin_queasy, waehrung, bediener, arrangement, res_line, guest_pr, ratecode, pricecod, htparam, katpreis, queasy
