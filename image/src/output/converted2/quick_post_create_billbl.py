...
s_list_list, S_list = create_model("S_list", {"zeit":int, "dept":int, "artnr":int, "bezeich":string, "zinr":string, "anzahl":int, "preis":Decimal, "betrag":Decimal, "l_betrag":Decimal, "f_betrag":Decimal, "resnr":int, "reslinnr":int})

def quick_post_create_billbl(s_list_list:[S_list], pvilanguage:int, billart:int, curr_dept:int, amount:Decimal, double_currency:bool, foreign_rate:bool, user_init:string, voucher_nr:string):
    ...
    def generate_output():
        ...
        return {"s-list": s_list_list, "msg_str": msg_str, "msg_str2": msg_str2}
    ...
    def create_bill():
        ...
        for s_list in query(s_list_list):
            ...
            res_line = get_cache (Res_line, {"resnr": [(eq, s_list.resnr)],"reslinnr": [(eq, s_list.reslinnr)]})
            ...
            if res_line:
                ...
                if bill:
                    ...
                    if bill.flag == 1:
                        msg_str = msg_str + chr_unicode(2) + "&W" + translateExtended ("Bill Number", lvcarea, "") + " " + to_string(bill.rechnr) + " " + translateExtended ("RmNo", lvcarea, "") + " " + s_list.zinr + " : " + translateExtended ("Status closed, Posting not possible.", lvcarea, "")
                    else:
    ...
    def update_bill():
        ...
        artikel = get_cache (Artikel, {"artnr": [(eq, billart)],"departement": [(eq, curr_dept)]})
    ...
    def check_mbill(slist_dept:int, slist_artnr:int):
        ...
        artikel = get_cache (Artikel, {"departement": [(eq, slist_dept)],"artnr": [(eq, slist_artnr)]})
        ...
        if master_flag:
            ...
            bill = get_cache (Bill, {"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, 0)]})
        ...
        if res_line.memozinr != "" and res_line.memozinr != res_line.zinr:
            ...
            resline = get_cache (Res_line, {"zinr": [(eq, res_line.memozinr)],"resstatus": [(eq, 6)]})
