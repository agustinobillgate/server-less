...
def ba_plan_res_checkout2bl(mainres_recid:int, t_resnr:int, t_reslinnr:int, user_init:str):
    ci_date:date = None
    ...
    mainres = db_session.query(Mainres).filter(
             (Mainres._recid == mainres_recid)).first()
    ...
    while None != bk_resline:
        ...
        bk_resline = db_session.query(Bk_resline).filter(
                 (Bk_resline.veran_nr == resline.veran_nr) & (Bk_resline.resstatus == 1)).filter(Bk_resline._recid > curr_recid).first()

    bk_func = db_session.query(Bk_func).filter(
             (Bk_func.veran_nr == resline.veran_nr) & (Bk_func.resstatus == 1)).first()
    while None != bk_func:
        create_bahistory()
        bk_func.resstatus = 8
        ...
        bk_func = db_session.query(Bk_func).filter(
                 (Bk_func.veran_nr == resline.veran_nr) & (Bk_func.resstatus == 1)).filter(Bk_func._recid > curr_recid).first()
