...
def company_glist_1bl(sort_rmcat:bool, fr_date:date, to_date:date, curr_gastnr:int):
    ...
    output_list = temp_outlist = t_output = tbill = rguest = argtline = None

    output_list_list, Output_list = create_model("Output_list", {"ankunft":date, "abreise":date, "zinr":string, "resnr":int, "regno":int, "gname":string, "night":int, "zipreis":Decimal, "amount":Decimal, "str":string, "nr":int, "rmcat":string, "verstat":int, "bill_no":int, "lodging":Decimal, "bfast":Decimal, "lunch":Decimal, "dinner":Decimal, "ex_bed":Decimal, "add_room":Decimal})
    temp_outlist_list, Temp_outlist = create_model("Temp_outlist", {"ankunft":date, "abreise":date, "zinr":string, "resnr":int, "regno":int, "gname":string, "night":int, "zipreis":Decimal, "amount":Decimal, "str":string, "nr":int, "rmcat":string, "verstat":int, "bill_no":int, "lodging":Decimal, "bfast":Decimal, "lunch":Decimal, "dinner":Decimal, "ex_bed":Decimal, "add_room":Decimal})
    ...
    def generate_output():
        nonlocal output_list_list, curr_date, datum, todate, zikatnr, tot_nr, tot_zipreis, tot_amount, tot_night, tot_lodging, tot_bfast, tot_adroom, tot_exbed, gr_zipreis, gr_amount, gr_night, gr_lodging, gr_bfast, gr_adroom, gr_exbed, ct, take_it, contcode, f_betrag, argt_betrag, qty, bfast_art, lunch_art, dinner_art, lundin_art, fb_dept, exchg_rate, frate, argt_rate, add_it, delta, start_date, actflag1, actflag2, serv2, vat2, vat3, vat4, fact2, curr_resno, curr_reslinno, curr_rmno, do_it, a, bill, guest, htparam, artikel, res_line, waehrung, zimkateg, genstat, guest_pr, arrangement, argt_line, fixleist, reslin_queasy
        nonlocal sort_rmcat, fr_date, to_date, curr_gastnr
        nonlocal t_output, tbill, rguest, argtline


        nonlocal output_list, temp_outlist, t_output, tbill, rguest, argtline
        nonlocal output_list_list, temp_outlist_list

        return {"output-list": output_list_list}

    def get_argtline_rate(contcode:string, argt_recid:int):

        nonlocal output_list_list, curr_date, datum, todate, zikatnr, tot_nr, tot_zipreis, tot_amount, tot_night, tot_lodging, tot_bfast, tot_adroom, tot_exbed, gr_zipreis, gr_amount, gr_night, gr_lodging, gr_bfast, gr_adroom, gr_exbed, ct, take_it, f_betrag, argt_betrag, qty, bfast_art, lunch_art, dinner_art, lundin_art, fb_dept, exchg_rate, frate, argt_rate, add_it, delta, start_date, actflag1, actflag2, serv2, vat2, vat3, vat4, fact2, curr_resno, curr_reslinno, curr_rmno, do_it, a, bill, guest, htparam, artikel, res_line, waehrung, zimkateg, genstat, guest_pr, arrangement, argt_line, fixleist, reslin_queasy
        nonlocal sort_rmcat, fr_date, to_date, curr_gastnr
        nonlocal t_output, tbill, rguest, argtline
    ...
    if sort_rmcat :
        ...
        if fr_date >= curr_date:
            ...
            res_line = Res_line()
            guest = Guest()
            for genstat.erwachs, genstat.kind1, genstat.kind2, genstat.zipreis, genstat.resnr, genstat.zinr, genstat.res_date, genstat.zikatnr, genstat.gratis, genstat.res_int, genstat.argt, genstat.logis, genstat.res_deci, genstat._recid, res_line.l_zuordnung, res_line.zikatnr, res_line.erwachs, res_line.kind1, res_line.kind2, res_line.resnr, res_line.reslinnr, res_line.reserve_int, res_line.betriebsnr, res_line.ankunft, res_line.abreise, res_line.zinr, res_line.name, res_line.zipreis, res_line.zimmeranz, res_line.gratis, res_line.gastnr, res_line.zimmer_wunsch, res_line.arrangement, res_line.reserve_dec, res_line.active_flag, res_line._recid, guest.gastnr, guest._recid in db_session.query(Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.zipreis, Genstat.resnr, Genstat.zinr, Genstat.res_date, Genstat.zikatnr, Genstat.gratis, Genstat.res_int, Genstat.argt, Genstat.logis, Genstat.res_deci, Genstat._recid, Res_line.l_zuordnung, Res_line.zikatnr, Res_line.erwachs, Res_line.kind1, Res_line.kind2, Res_line.resnr, Res_line.reslinnr, Res_line.reserve_int, Res_line.betriebsnr, Res_line.ankunft, Res_line.abreise, Res_line.zinr, Res_line.name, Res_line.zipreis, Res_line.zimmeranz, Res_line.gratis, Res_line.gastnr, Res_line.zimmer_wunsch, Res_line.arrangement, Res_line.reserve_dec, Res_line.active_flag, Res_line._recid, Guest.gastnr, Guest._recid).join(Res_line,(Res_line.resnr == Genstat.resnr) & (Res_line.reslinnr == Genstat.res_int[inc_value(0)]) & (Res_line.l_zuordnung[inc_value(2)] == 0)).join(Guest,(Guest.gastnr == Genstat.gastnrmember)).filter(
                ...
                output_list = query(output_list_list, filters=(lambda output_list: output_list.resnr == genstat.resnr and output_list.zinr == genstat.zinr), first=True)
                        ...
                        if fixleist:
                            ...
                            if add_it:
                                ...
                                if artikel and artikel.artnr == 110 and argt_rate != 0:
                                    ...
                                    output_list.add_room =  to_decimal(argt_rate)
                    output_list.amount =  to_decimal(output_list.zipreis) * to_decimal(output_list.night)
