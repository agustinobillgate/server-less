...
def hk_discrepancybl(pvilanguage:int, case_type:string, zinno:string, housestat:int, feat:string, fo_stat1:string, hk_stat1:string, user_init:string, hk_str:string, hk_pax:int, hk_ch1:int):
    ...
    hkdiscrepancy_list_list, Hkdiscrepancy_list = create_model("Hkdiscrepancy_list", {"zinr":string, "features":string, "etage":int, "bezeich":string, "house_status":int, "zistatus":int, "userinit":string, "nr":int, "fo_adult":int, "fo_child":int})
    ...
    def generate_output():
        ...
        return {"msg_str": msg_str, "fo_stat": fo_stat, "hk_stat": hk_stat, "fo_pax": fo_pax, "fo_ch1": fo_ch1, "hkdiscrepancy-list": hkdiscrepancy_list_list, "rmplan": rmplan_list}
