...
def inv_adjustlist_btn_save_webbl(user_init:string, c_list_list:[C_list]):
    ...
    for c_list in query(c_list_list):
        ...
        l_op = db_session.query(L_op).filter(
                 (L_op.lscheinnr == c_list.lscheinnr) & (L_op.artnr == c_list.artnr) & (to_int(L_op.stornogrund) == 0)).first()
