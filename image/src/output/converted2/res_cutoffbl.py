...
def res_cutoffbl(guaranteed:bool, fr_date:date, to_date:date):
    ...
    if guaranteed == False:
        ...
        zimkateg = Zimkateg()
        for res_line.resnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.arrangement, res_line.zipreis, res_line.anztage, res_line.erwachs, res_line.gratis, res_line.kind1, res_line.resstatus, res_line._recid, reservation.grpflag, reservation.name, reservation.point_resnr, reservation.resdat, reservation.groupname, reservation._recid, zimkateg.kurzbez, zimkateg._recid in db_session.query(Res_line.resnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.arrangement, Res_line.zipreis, Res_line.anztage, Res_line.erwachs, Res_line.gratis, Res_line.kind1, Res_line.resstatus, Res_line._recid, Reservation.grpflag, Reservation.name, Reservation.point_resnr, Reservation.resdat, Reservation.groupname, Reservation._recid, Zimkateg.kurzbez, Zimkateg._recid).join(Reservation,(Reservation.resnr == Res_line.resnr) & (((Reservation.point_resnr > 0) & (Res_line.ankunft == fr_date)) | ((Reservation.point_resnr > 0) & ((Res_line.ankunft - Reservation.point_resnr) >= fr_date) & ((Res_line.ankunft - Reservation.point_resnr) <= to_date)))).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 (Res_line.active_flag == 0) & (Res_line.resstatus == 3)).order_by(Reservation.point_resnr, Res_line.ankunft, Reservation.name, Reservation.resnr).all():
        ...
        zimkateg = Zimkateg()
        for res_line.resnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.arrangement, res_line.zipreis, res_line.anztage, res_line.erwachs, res_line.gratis, res_line.kind1, res_line.resstatus, res_line._recid, reservation.grpflag, reservation.name, reservation.point_resnr, reservation.resdat, reservation.groupname, reservation._recid, zimkateg.kurzbez, zimkateg._recid in db_session.query(Res_line.resnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.arrangement, Res_line.zipreis, Res_line.anztage, Res_line.erwachs, Res_line.gratis, Res_line.kind1, Res_line.resstatus, Res_line._recid, Reservation.grpflag, Reservation.name, Reservation.point_resnr, Reservation.resdat, Reservation.groupname, Reservation._recid, Zimkateg.kurzbez, Zimkateg._recid).join(Reservation,(Reservation.resnr == Res_line.resnr) & (((Reservation.point_resnr > 0) & (Res_line.ankunft == fr_date)) | ((Reservation.point_resnr > 0) & ((Res_line.ankunft - Reservation.point_resnr) >= fr_date) & ((Res_line.ankunft - Reservation.point_resnr) <= to_date)))).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 (Res_line.active_flag == 0) & (Res_line.resstatus == 1)).order_by(Reservation.point_resnr, Res_line.ankunft, Reservation.name, Reservation.resnr).all():
            if res_line_obj_list.get(res_line._recid):
                continue
            else:
                res_line_obj_list[res_line._recid] = True
