...
def fa_movelistbl(c_procedure:string, bl_all:bool, user_init:string, typ_of:string, main_query:int, sub_query:int, int_query:int, fdate:date, tdate:date):
    ...
    output_list_list, Output_list = create_model("Output_list", {"asset_name":string, "asset_no":string, "move_from":string, "move_to":string, "datum":date, "usrid":string, "zeit":string, "qty":int, "price":Decimal, "amount":Decimal})
