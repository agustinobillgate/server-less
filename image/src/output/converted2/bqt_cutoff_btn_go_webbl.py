...
def bqt_cutoff_btn_go_webbl(inp_room:string, from_date:date, to_date:date, stat_screen_value:string, mi_created_chk:bool, mi_event_chk:bool, mi_engager_chk:bool, mi_room_chk:bool):
    ...
    output_list_list, Output_list = create_model("Output_list", {"bk_stat":string, "bk_datum":date, "engager":string, "contact":string, "bk_event":string, "venue":string, "bk_time":string, "pax":int, "room_rev":Decimal, "fb_rev":Decimal, "other_rev":Decimal, "total_rev":Decimal, "sales":string, "crdate":date, "cutoff":date, "resnr":int, "reslinnr":int, "resstatus":int, "datum":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal troomrev, tfbrev, tothrev, ttrev, output_list_list, total_estrev, total_paid, bk_veran, bk_func, bk_rart, bk_reser
