...
from models import Gc_pibline, Gc_pi, Gc_pitype, Htparam, Gl_acct, Bediener, Gc_piacct, Queasy

def prepare_mk_gcpibl(pvilanguage:int, pi_number:string, user_init:string):

    prepare_cache ([Htparam, Bediener, Gc_piacct, Queasy])
    ...
    gc_pibline = gc_pi = gc_pitype = htparam = gl_acct = bediener = gc_piacct = queasy = None
    ...
    s_list_list, S_list = create_model("S_list", {"reihe":int, "bezeich":string, "amount":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal pi_acctno, giro_tempacct, pi_mode, pi_status, pi_type1, fl_temp, fl_err, p_110, bemerk, rcvname, pay_acctno, department, pay_amount, pbuff_list, t_gc_pitype_list, t_gc_pibline_list, s_list_list, i, lvcarea, gc_pibline, gc_pi, gc_pitype, htparam, gl_acct, bediener, gc_piacct, queasy
