...
def ts_restinv_update_bill1bl(rec_id_h_bill:int, transdate:date, double_currency:bool, exchg_rate:Decimal, kellner1_kcredit_nr:int, bilrecid:int, foreign_rate:bool, user_init:string, gname:string, hoga_resnr:int, hoga_reslinnr:int, price_decimal:int, amount:Decimal, amount_foreign:Decimal):
    ...
    def generate_output():
        ...
        return {"amount": amount, "amount_foreign": amount_foreign, "bill_date": bill_date, "billart": billart, "qty": qty, "description": description, "cancel_str": cancel_str, "t-h-bill": t_h_bill_list}

    def cal_vat_amount():

        nonlocal bill_date, billart, description, cancel_str, t_h_bill_list, vat_amount, get_rechnr, get_amount, curr_dept, active_deposit, h_bill, bill, htparam, artikel, counters, bill_line, billjournal, h_bill_line, h_artikel, queasy, h_journal
    ...
    def cal_vatamt(vatproz:Decimal):
        ...
        nonlocal bill_date, billart, description, cancel_str, t_h_bill_list, vat_amount, multi_vat, get_rechnr, get_amount, curr_dept, active_deposit, h_bill, bill, htparam, artikel, counters, bill_line, billjournal, h_bill_line, h_artikel, queasy, h_journal
        nonlocal rec_id_h_bill, transdate, double_currency, exchg_rate, kellner1_kcredit_nr, bilrecid, foreign_rate, user_init, gname, hoga_resnr, hoga_reslinnr, price_decimal, amount_foreign
