...
from models import Res_line, Arrangement, Artikel, Argt_line, Reslin_queasy, Fixleist, Guest, Reservation, Zimkateg, Bill, Master

def abf_listbl(fdate:date, bfast_artnr:int, bfast_dept:int):
    abf_list_list = []
    res_line = arrangement = artikel = argt_line = reslin_queasy = fixleist = guest = reservation = zimkateg = bill = master = None

    abf_list = None

    abf_list_list, Abf_list = create_model("Abf_list", {"zinr":str, "name":str, "segmentcode":int, "ankunft":date, "anztage":int, "abreise":date, "kurzbez":str, "arrangement":str, "zimmeranz":int, "erwachs":int, "kind1":int, "gratis":int, "resnr":int, "bemerk":str, "gastnr":int, "resstatus":int, "resname":str, "address":str, "city":str, "comments":str, "datum":date})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal abf_list_list, res_line, arrangement, artikel, argt_line, reslin_queasy, fixleist, guest, reservation, zimkateg, bill, master
        nonlocal fdate, bfast_artnr, bfast_dept
    ...
    def disp_arlist():
        ...
        for res_line in db_session.query(Res_line).filter(
                 ((Res_line.resstatus != 3) & (Res_line.resstatus != 4) & (Res_line.resstatus != 8) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12)) & (Res_line.active_flag <= 1) & (Res_line.ankunft < fdate) & (Res_line.abreise >= htparam.fdate) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line.zinr).all():
            do_it = False
            roflag = True
            qty = 0
            ...
            if do_it and epreis == 0:
                ...
                if contcode != "":
                    ...
                    reslin_queasy = db_session.query(Reslin_queasy).filter(
                             (func.lower(Reslin_queasy.key) == ("argt-line").lower()) & (Reslin_queasy.char1 == guest_pr.code) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (Reslin_queasy.number3 == bfast_artnr) & (Reslin_queasy.resnr == bfast_dept) & (Reslin_queasy.reslinnr == res_line.zikatnr) & (Reslin_queasy.date1 <= htparam.fdate) & (Reslin_queasy.date2 >= htparam.fdate) & (Reslin_queasy.deci1 > 0)).first()

                if not reslin_queasy:

                    reslin_queasy = db_session.query(Reslin_queasy).filter(
                             (func.lower(Reslin_queasy.key) == ("fargt-line").lower()) & (Reslin_queasy.char1 == guest_pr.code) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (Reslin_queasy.number3 == bfast_artnr) & (Reslin_queasy.resnr == bfast_dept) & (Reslin_queasy.date1 <= htparam.fdate) & (Reslin_queasy.date2 >= htparam.fdate) & (Reslin_queasy.deci1 > 0)).first()
