...
def pickup_listbl(pvilanguage:int, disp_pickup:bool, check_ftd:bool, fdate:date, frdate:date, tdate:date):
    ...
    def disp_pickup():
        ...
        for res_line, zimkateg in db_session.query(Res_line, Zimkateg).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 ((Res_line.resstatus <= 5) | (Res_line.resstatus == 11)) & (Res_line.ankunft == fdate) & (matches(Res_line.zimmer_wunsch,("*pickup*")))).order_by(Res_line.resnr, Res_line.name).all():
    ...
    def disp_drop():
        ...
        res_line_obj_list = {}
        for res_line, zimkateg in db_session.query(Res_line, Zimkateg).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 (Res_line.active_flag <= 1) & (Res_line.abreise == fdate) & (matches(Res_line.zimmer_wunsch,("*drop-passanger*")))).order_by(Res_line.resnr, Res_line.name).all():
            if res_line_obj_list.get(res_line._recid):
                continue
            else:
                res_line_obj_list[res_line._recid] = True
    ...
    def disp_pickup1():
        ...
        res_line_obj_list = {}
        for res_line, zimkateg in db_session.query(Res_line, Zimkateg).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 ((Res_line.resstatus <= 5) | (Res_line.resstatus == 11)) & (Res_line.ankunft >= frdate) & (Res_line.ankunft <= tdate) & (matches(Res_line.zimmer_wunsch,("*pickup*")))).order_by(Res_line.resnr, Res_line.name).all():
            if res_line_obj_list.get(res_line._recid):
                continue
            else:
                res_line_obj_list[res_line._recid] = True
    ...
    def disp_drop1():
        ...
        res_line_obj_list = {}
        for res_line, zimkateg in db_session.query(Res_line, Zimkateg).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                 (Res_line.active_flag <= 1) & (Res_line.abreise >= frdate) & (Res_line.abreise <= tdate) & (matches(Res_line.zimmer_wunsch,("*drop-passanger*")))).order_by(Res_line.resnr, Res_line.name).all():
            if res_line_obj_list.get(res_line._recid):
                continue
            else:
                res_line_obj_list[res_line._recid] = True
