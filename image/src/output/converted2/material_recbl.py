...
def material_recbl(pvilanguage:int, from_date:date, to_date:date, from_grp:int, lager_no:int, from_main:int, to_main:int):
    ...
    mtreconsile_list_list, Mtreconsile_list = create_model("Mtreconsile_list", {"nr":int, "code":int, "bezeich":string, "col1":string, "col2":string, "col3":string, "col4":string, "col5":string})
    s_list_list, S_list = create_model("S_list", {"code":int, "reihenfolge":int, "lager_nr":int, "l_bezeich":string, "fibukonto":string, "bezeich":string, "flag":int, "anf_wert":Decimal, "end_wert":Decimal, "betrag":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal mtreconsile_list_list, curr_nr, curr_reihe, ldry, dstore, long_digit, foreign_nr, exchg_rate, double_currency, type_of_acct, counter, coa_format, f_date, lvcarea, htparam, waehrung, h_artikel, l_bestand, l_besthis, gl_acct, l_lager, l_artikel, l_untergrup, l_op, h_bill_line
        nonlocal pvilanguage, from_date, to_date, from_grp, lager_no, from_main, to_main


        nonlocal mtreconsile_list, s_list
        nonlocal mtreconsile_list_list, s_list_list

        return {"mtreconsile-list": mtreconsile_list_list}
    ...
    def create_list():
        ...
        if l_lager:
            ...
            l_oh = L_bestand()
            for l_bestand.anz_anf_best, l_bestand.anz_eingang, l_bestand.anz_ausgang, l_bestand.val_anf_best, l_bestand.artnr, l_bestand._recid, l_artikel.vk_preis, l_artikel._recid, l_untergrup.fibukonto, l_untergrup.bezeich, l_untergrup._recid, l_oh.anz_anf_best, l_oh.anz_eingang, l_oh.anz_ausgang, l_oh.val_anf_best, l_oh.artnr, l_oh._recid in db_session.query(L_bestand.anz_anf_best, L_bestand.anz_eingang, L_bestand.anz_ausgang, L_bestand.val_anf_best, L_bestand.artnr, L_bestand._recid, L_artikel.vk_preis, L_artikel._recid, L_untergrup.fibukonto, L_untergrup.bezeich, L_untergrup._recid, L_oh.anz_anf_best, L_oh.anz_eingang, L_oh.anz_ausgang, L_oh.val_anf_best, L_oh.artnr, L_oh._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr) & (L_artikel.endkum >= from_main) & (L_artikel.endkum <= to_main)).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).join(L_oh,(L_oh.lager_nr == 0) & (L_oh.artnr == L_bestand.artnr)).filter(
                ...
                s_list = query(s_list_list, filters=(lambda s_list: s_list.fibukonto == l_untergrup.fibukonto and s_list.reihenfolge == 1 and s_list.flag == 4), first=True)
            ...
            l_artikel = L_artikel()
            l_untergrup = L_untergrup()
            for l_op.warenwert, l_op.anzahl, l_op.op_art, l_op._recid, l_artikel.vk_preis, l_artikel._recid, l_untergrup.fibukonto, l_untergrup.bezeich, l_untergrup._recid in db_session.query(L_op.warenwert, L_op.anzahl, L_op.op_art, L_op._recid, L_artikel.vk_preis, L_artikel._recid, L_untergrup.fibukonto, L_untergrup.bezeich, L_untergrup._recid).join(L_artikel,(L_artikel.artnr == l_bestand.artnr) & (L_artikel.endkum >= from_main) & (L_artikel.endkum <= to_main)).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).filter(
                     (L_op.datum >= from_date) & (L_op.datum <= to_date) & ((L_op.op_art == 1) | (L_op.op_art == 3) | (L_op.op_art == 4)) & (L_op.lager_nr == lager_no) & (L_op.loeschflag <= 1)).order_by(L_op.artnr).all():
            ...
            l_oh = L_bestand()
            l_untergrup = L_untergrup()
            for l_bestand.anz_anf_best, l_bestand.anz_eingang, l_bestand.anz_ausgang, l_bestand.val_anf_best, l_bestand.artnr, l_bestand._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid, l_artikel.vk_preis, l_artikel._recid, l_oh.anz_anf_best, l_oh.anz_eingang, l_oh.anz_ausgang, l_oh.val_anf_best, l_oh.artnr, l_oh._recid, l_untergrup.fibukonto, l_untergrup.bezeich, l_untergrup._recid in db_session.query(L_bestand.anz_anf_best, L_bestand.anz_eingang, L_bestand.anz_ausgang, L_bestand.val_anf_best, L_bestand.artnr, L_bestand._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid, L_artikel.vk_preis, L_artikel._recid, L_oh.anz_anf_best, L_oh.anz_eingang, L_oh.anz_ausgang, L_oh.val_anf_best, L_oh.artnr, L_oh._recid, L_untergrup.fibukonto, L_untergrup.bezeich, L_untergrup._recid).join(L_lager,(L_lager.lager_nr == L_bestand.lager_nr)).join(L_artikel,(L_artikel.artnr == L_bestand.artnr) & (L_artikel.endkum >= from_main) & (L_artikel.endkum <= to_main)).join(L_oh,(L_oh.lager_nr == 0) & (L_oh.artnr == L_bestand.artnr)).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).order_by(L_bestand.lager_nr, L_untergrup.fibukonto, L_artikel.artnr).all():
                if l_bestand_obj_list.get(l_bestand._recid):
                    continue
                else:
                    l_bestand_obj_list[l_bestand._recid] = True
            ...
            l_artikel = L_artikel()
            l_untergrup = L_untergrup()
            for l_op.warenwert, l_op.anzahl, l_op.op_art, l_op._recid, l_lager.lager_nr, l_lager.bezeich, l_lager._recid, l_artikel.vk_preis, l_artikel._recid, l_untergrup.fibukonto, l_untergrup.bezeich, l_untergrup._recid in db_session.query(L_op.warenwert, L_op.anzahl, L_op.op_art, L_op._recid, L_lager.lager_nr, L_lager.bezeich, L_lager._recid, L_artikel.vk_preis, L_artikel._recid, L_untergrup.fibukonto, L_untergrup.bezeich, L_untergrup._recid).join(L_lager,(L_lager.lager_nr == L_op.lager_nr)).join(L_artikel,(L_artikel.artnr == L_op.artnr) & (L_artikel.endkum >= from_main) & (L_artikel.endkum <= to_main)).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).filter(
                     (L_op.datum >= from_date) & (L_op.datum <= to_date) & ((L_op.op_art == 1) | (L_op.op_art == 3) | (L_op.op_art == 4)) & (L_op.loeschflag <= 1)).order_by(L_op.lager_nr).all():
                if l_op_obj_list.get(l_op._recid):
                    continue
                else:
                    l_op_obj_list[l_op._recid] = True
        ...
        mtreconsile_list_list.append(mtreconsile_list)
