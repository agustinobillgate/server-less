...
def rm_hislistbl(t_date:date, f_date:date, zinr:string):
    ...
    t_hislist_list, T_hislist = create_model("T_hislist", {"gastinfo":string, "ankunft":date, "abreise":date, "abreisezeit":string, "zikateg":string, "zinr":string, "zipreis":Decimal, "zimmeranz":int, "arrangement":string, "resnr":int, "gesamtumsatz":Decimal, "zahlungsart":int, "segmentcode":int, "bemerk":string, "betriebsnr":int})
