...
def cancel_dpurchasebl(lief_nr:int, docu_nr:str, user_init:str):
    ...
    def check_onhand():
        ...
        for l_op in db_session.query(L_op).filter(
                 (L_op.lief_nr == lief_nr) & (func.lower(L_op.docu_nr) == (docu_nr).lower()) & (func.lower(L_op.lscheinnr) == (docu_nr).lower()) & (L_op.op_art == 1) & (L_op.loeschflag <= 1) & (L_op.pos > 0)).order_by(L_op._recid).all():
