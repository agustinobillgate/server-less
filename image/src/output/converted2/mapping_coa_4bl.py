...
def mapping_coa_4bl(coa_list_list:[Coa_list]):
        ...
        while None != coa_list:
            ...
            while None != acct_list:
                ...
                gl_acct = db_session.query(Gl_acct).filter(
                         (Gl_acct.fibukonto == coa_list.new_fibu)).first()
    ...
    def delete_gl():
        ...
        for coa_list in query(coa_list_list, filters=(lambda coa_list: coa_list.coaStat == -1)):
            ...
            gl_acct = db_session.query(Gl_acct).filter(
                     (Gl_acct.fibukonto == coa_list.old_fibu)).first()
            ...
            gl_accthis = db_session.query(Gl_accthis).filter(
                     (Gl_accthis.fibukonto == coa_list.old_fibu)).first()
            ...
            gl_jourhis = db_session.query(Gl_jourhis).filter(
                     (Gl_jourhis.fibukonto == coa_list.old_fibu)).first()
            ...
            gl_journal = db_session.query(Gl_journal).filter(
                     (Gl_journal.fibukonto == coa_list.old_fibu)).first()
