...
def pj_inhouse2_btn_go_3_cldbl(sorttype:int, datum:date, curr_date:date, curr_gastnr:int, froom:string, troom:string, exc_depart:bool, incl_gcomment:bool, incl_rsvcomment:bool, prog_name:string, disp_accompany:bool, disp_exclinact:bool, split_rsv_print:bool):
    ...
    str_list_list, Str_list = create_model("Str_list", {"flag":int, "rflag":bool, "line1":string, "line2":string, "line3":string, "company":string}, {"rflag": True})
    ...
    zinr_list_list, Zinr_list = create_model("Zinr_list", {"resnr":int, "reslinnr":int, "zinr":string})
    ...
    def create_inhouse1():
        ...
        nonlocal tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, tot_rmqty, inactive, cl_list_list, s_list_list, t_buff_queasy_list, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, tot_room, all_room, all_remark, queasy, htparam, zkstat, zinrstat, paramtext, guest, reservation, zimmer, zimkateg, res_line, guestseg, segment, mc_guest, mc_types, nation, waehrung, reslin_queasy, mealcoup, genstat
        nonlocal sorttype, datum, curr_date, froom, troom, exc_depart, incl_gcomment, incl_rsvcomment, prog_name, disp_accompany, disp_exclinact, split_rsv_print
    ...
    def create_genstat_inhouse2():
        ...
        gmember = Guest()
        zimmer = Zimmer()
        for genstat.resstatus, genstat.res_date, genstat.resnr, genstat.res_int, genstat.zinr, genstat.zipreis, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.kind3, genstat.gratis, genstat.argt, genstat.segmentcode, genstat.gastnr, genstat._recid, zimkateg.kurzbez, zimkateg.bezeichnung, zimkateg._recid, reservation.segmentcode, reservation.resdat, reservation.useridanlage, reservation.resnr, reservation._recid, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest.gastnr, guest.karteityp, guest._recid, gmember.name, gmember.vorname1, gmember.anrede1, gmember.anredefirma, gmember.gastnr, gmember.karteityp, gmember._recid, zimmer.etage, zimmer.bezeich, zimmer.sleeping, zimmer._recid in db_session.query(Genstat.resstatus, Genstat.res_date, Genstat.resnr, Genstat.res_int, Genstat.zinr, Genstat.zipreis, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.kind3, Genstat.gratis, Genstat.argt, Genstat.segmentcode, Genstat.gastnr, Genstat._recid, Zimkateg.kurzbez, Zimkateg.bezeichnung, Zimkateg._recid, Reservation.segmentcode, Reservation.resdat, Reservation.useridanlage, Reservation.resnr, Reservation._recid, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest.gastnr, Guest.karteityp, Guest._recid, Gmember.name, Gmember.vorname1, Gmember.anrede1, Gmember.anredefirma, Gmember.gastnr, Gmember.karteityp, Gmember._recid, Zimmer.etage, Zimmer.bezeich, Zimmer.sleeping, Zimmer._recid).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).join(Reservation,(Reservation.resnr == Genstat.resnr)).join(Guest,(Guest.gastnr == Genstat.gastnr)).join(Gmember,(Gmember.gastnr == Genstat.gastnrmember)).join(Zimmer,(Zimmer.zinr == Genstat.zinr) & (Zimmer.sleeping)).filter(
                 (Genstat.datum == datum) & (Genstat.zinr >= (froom).lower()) & (Genstat.zinr <= (troom).lower())).order_by(Genstat.zinr, Genstat.erwachs.desc(), Gmember.name).all():
            if genstat_obj_list.get(genstat._recid):
                continue
            else:
