...
table_list_list, Table_list = create_model("Table_list", {"rechnr":int, "tischnr":int, "saldo":Decimal, "belong":string}, {"belong": "L"})

def ts_restinv_waiter_transferbl(pvilanguage:int, table_list_list:[Table_list], transdate:date, curr_waiter:int, curr_dept:int, usr_nr:int):
    ...
    def transfer_now(k1:int, k2:int, bill_date:date):
        ...
        for table_list in query(table_list_list, filters=(lambda table_list: table_list.belong.lower()  == ("R").lower())):
