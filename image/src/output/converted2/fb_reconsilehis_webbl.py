...
def fb_reconsilehis_webbl(pvilanguage:int, from_grp:int, food:int, bev:int, from_date:date, to_date:date, ldry:int, dstore:int, double_currency:bool, foreign_nr:int, exchg_rate:Decimal, mi_opt_chk:bool, date1:date, date2:date):
    ...
    s_list_list, S_list = create_model("S_list", {"col1":string, "col2":string, "col3":string, "col4":string, "col5":string})
    ...
    done, output_list_list = get_output(fb_reconsilehis_cldbl(pvilanguage, from_grp, food, bev, from_date, to_date, ldry, dstore, double_currency, foreign_nr, exchg_rate, mi_opt_chk, date1, date2))
