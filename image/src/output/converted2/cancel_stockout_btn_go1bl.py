...
def cancel_stockout_btn_go1bl(from_grp:int, mi_alloc_chk:bool, mi_article_chk:bool, mi_docu_chk:bool, mi_date_chk:bool, from_lager:int, to_lager:int, from_date:date, to_date:date, from_art:int, to_art:int, show_price:bool, cost_acct:string, mattype:int):
    ...
    str_list_list, Str_list = create_model("Str_list", {"fibu":string, "other_fibu":bool, "op_recid":int, "lscheinnr":string, "s":string})
    cancel_stockout_list, Cancel_stockout = create_model("Cancel_stockout", {"datum":date, "lager":string, "lscheinnr":string, "artnr":int, "bezeich":string, "out_qty":Decimal, "avrg_price":Decimal, "amount":Decimal, "id":string, "reason":string})
    ...
    for str_list in query(str_list_list):
        ...
        cancel_stockout.datum = date_mdy(substring(str_list.s, 0, 8))
