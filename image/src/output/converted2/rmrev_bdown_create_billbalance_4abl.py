...
from models import Waehrung, Guest, Artikel, Htparam, Res_line, Zimmer, Arrangement, Reservation, Bill, Zimkateg, Reslin_queasy, Guest_pr, Pricecod, Argt_line, Fixleist

def rmrev_bdown_create_billbalance_4abl(exc_taxserv:bool, pvilanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, curr_date:date, sorttype:int):

    prepare_cache ([Waehrung, Guest, Artikel, Htparam, Res_line, Zimmer, Arrangement, Bill, Zimkateg, Reslin_queasy, Guest_pr, Pricecod, Argt_line, Fixleist])
    ...
    waehrung = guest = artikel = htparam = res_line = zimmer = arrangement = reservation = bill = zimkateg = reslin_queasy = guest_pr = pricecod = argt_line = fixleist = None

    sum_list = currency_list = cl_list = s_list = argt_list = waehrung1 = cc_list = None
    ...
    def generate_output():
        nonlocal msg_str, msg_warning, cl_list_list, currency_list_list, sum_list_list, s_list_list, argt_list_list, exchg_rate, frate, post_it, total_rev, rm_rate, lvcarea, waehrung, guest, artikel, htparam, res_line, zimmer, arrangement, reservation, bill, zimkateg, reslin_queasy, guest_pr, pricecod, argt_line, fixleist
        nonlocal exc_taxserv, pvilanguage, new_contrate, foreign_rate, price_decimal, curr_date, sorttype
        nonlocal waehrung1, cc_list


        nonlocal sum_list, currency_list, cl_list, s_list, argt_list, waehrung1, cc_list
        nonlocal sum_list_list, currency_list_list, cl_list_list, s_list_list, argt_list_list

        return {"msg_str": msg_str, "msg_warning": msg_warning, "cl-list": cl_list_list, "currency-list": currency_list_list, "sum-list": sum_list_list, "s-list": s_list_list, "argt-list": argt_list_list}
    ...
    def create_billbalance():
        ...
        if sorttype == 2:
            ...
            for res_line in db_session.query(Res_line).filter(
                     ((Res_line.active_flag <= 1)) & ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6)) & (not_ (Res_line.ankunft > curr_date)) & (not_ (Res_line.abreise < curr_date))).order_by(Res_line.resname).all():
                do_it = True
                ...
                if do_it:
                    ...
                    sum_list.pax = sum_list.pax +\
                            (res_line.erwachs + res_line.kind1 + res_line.kind2) * res_line.zimmeranz
                    ...
                    for curr_i in range(1,res_line.zimmeranz + 1) :
                        ...
                        if reslin_queasy:
                            ...
                            if res_line.zipreis != reslin_queasy.deci1:
