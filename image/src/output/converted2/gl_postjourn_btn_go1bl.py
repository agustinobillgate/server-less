...
def gl_postjourn_btn_go1bl(g_list_list:[G_list], pvilanguage:int, curr_step:int, bezeich:string, credits:[Decimal], debits:[Decimal], remains:[Decimal], refno:string, datum:date, adjust_flag:bool, journaltype:int):
    curr_jnr = 0
    msg_str = ""
    error_flag = False
    msg_str1 = ""
    ...
    if curr_step == 1:
        ...
        if datum == None or refno == "":
            ...
            curr_jnr, msg_str, error_flag = get_output(gl_postjourn_btn_gobl(g_list_list, pvilanguage, curr_step, bezeich, credits, debits, remains, refno, datum, adjust_flag, journaltype))
