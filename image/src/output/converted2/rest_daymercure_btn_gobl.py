...
bline_list_list, Bline_list = create_model("Bline_list", {"selected":bool, "name":string, "depart":string, "dept":int, "knr":int, "bl_recid":int})

def rest_daymercure_btn_gobl(bline_list_list:[Bline_list], shift:int, from_date:date, exchg_rate:Decimal):
    ...
    cash_art_list, Cash_art = create_model("Cash_art", {"sortnr":int, "usrnr":int, "usrnm":string, "artnr":int, "bezeich":string, "amount":Decimal})
    outstand_list_list, Outstand_list = create_model("Outstand_list", {"name":string, "rechnr":int, "foreign":Decimal, "saldo":Decimal})
    pay_list_list, Pay_list = create_model("Pay_list", {"compli":bool, "person":int, "flag":int, "bezeich":string, "artnr":int, "rechnr":int, "foreign":Decimal, "saldo":Decimal})
    turnover_list, Turnover = create_model("Turnover", {"departement":int, "deptname":string, "kellner_nr":int, "name":string, "tischnr":int, "rechnr":string, "belegung":int, "artnr":int, "info":string, "t_debit":Decimal, "t_credit":Decimal, "p_cash":Decimal, "p_cash1":Decimal, "r_transfer":Decimal, "c_ledger":Decimal, "compli":bool, "flag":int, "coupon":Decimal, "comp":Decimal, "gname":string})
    ...
    def generate_output():
        ...
        nonlocal cash_art_list, outstand_list_list, pay_list_list, turnover_list
    ...
    def daysale_list():
        ...
        for kellner in db_session.query(Kellner).filter(
                 (Kellner._recid.in_(list(set([bline_list.bl_recid for bline_list in bline_list_list if bline_list.selected]))))).order_by(Kellner.departement, Kellner.kellnername).all():
            if kellner_obj_list.get(kellner._recid):
                continue
            else:
                kellner_obj_list[kellner._recid] = True

            bline_list = query(bline_list_list, (lambda bline_list: (kellner._recid == bline_list.bl_recid)), first=True)
            t_cover = 0
            ...
            for h_bill in db_session.query(H_bill).filter(
                ...
                while None != h_bill_line:
                    ...
                    turnover = query(turnover_list, filters=(lambda turnover: turnover.departement == bline_list.dept and turnover.kellner_nr == kellner.kellner_nr and turnover.rechnr == to_string(h_bill.rechnr)), first=True)
                        ...
                        if h_artikel:
                            ...
                            if h_artikel.artart == 6:
                                ...
                                if artikel:
                                    ...
                                    cash_art = query(cash_art_list, filters=(lambda cash_art: cash_art.artnr == artikel.artnr and cash_art.usrnr == kellner.kellner_nr), first=True)
            ...
            tlist = query(tlist_list, filters=(lambda tlist: tlist.name == kellner.kellnername and tlist.departement == bline_list.dept), first=True)
