...
def stock_ohlist1_2bl(from_lager:int, to_lager:int, from_art:int, to_art:int, from_date:date, to_date:date, move_art:bool, end_notzero:bool, global_oh:bool):
    ...
    def generate_output():
        nonlocal stockoh_list_list, do_it, curr_art, curr_lager, out_bestand, zwkum, t_val, bezeich, j, tot_prev, tot_value, tot_in, tot_out, total_in, total_out, total_prev, total_value, tdate, qty, wert, long_digit, l_bestand, l_besthis, l_lager, htparam, l_artikel, l_op, l_untergrup
                ...
                for b_lager in db_session.query(B_lager).filter(
                    ...
                    for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter(
                        ...
                        if do_it:
                            ...
                            art_bestand = query(art_bestand_list, filters=(lambda art_bestand: art_bestand.artnr == l_bestand.artnr and art_bestand.lager == b_lager.lager_nr), first=True)
        ...
        if get_year(to_date) <= get_year(tdate) and get_year(from_date) <= get_year(tdate):
            ...
            if get_month(to_date) > get_month(tdate) and get_month(from_date) > get_month(tdate):
                ...
                for l_artikel in db_session.query(L_artikel).filter(
                         ((L_artikel.artnr.in_(list(set([art_bestand.artnr for art_bestand in art_bestand_list])))))).order_by(art_bestand.zwkum, art_bestand.artnr).all():
                    if l_artikel_obj_list.get(l_artikel._recid):
                        continue
                    else:
                        l_artikel_obj_list[l_artikel._recid] = True

                    art_bestand = query(art_bestand_list, (lambda art_bestand: (l_artikel.artnr == art_bestand.artnr)), first=True)
