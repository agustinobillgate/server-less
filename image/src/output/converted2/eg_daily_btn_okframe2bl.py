...
def eg_daily_btn_okframe2bl(scost_list:[Scost], blframe:int, a1:date, b1:date, intres:int):
    eg_cost = None
    ...
    if blframe == 0:
        ...
        for eg_cost in db_session.query(Eg_cost).filter(
                 (Eg_cost.datum >= a1) & (Eg_cost.datum <= b1) & (Eg_cost.resource_nr == intres)).order_by(Eg_cost._recid).all():
