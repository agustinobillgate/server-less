...
s_list_list, S_list = create_model("S_list", {"fibukonto":string, "debit":Decimal, "credit":Decimal, "remark":string}, {"fibukonto": "000000000000"})

def manual_ar_webbl(pvilanguage:int, s_list_list:[S_list], rgdatum:date, firma:string, art_fibukonto:string, user_init:string, invoice:int, saldo:Decimal, refno:string):
    ...
    for sbuff in query(sbuff_list, filters=(lambda sbuff: to_decimal(sbuff.fibukonto) != 0)):
