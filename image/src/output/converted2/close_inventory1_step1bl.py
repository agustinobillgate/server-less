...
def close_inventory1_step1bl(pvilanguage:int, inv_type:int, m_endkum:int, begindate:date, closedate:date):
    ...
    def close_onhand():
        ...
        while None != l_bestand:
            ...
            l_bestand = db_session.query(L_bestand).filter(
                     (L_bestand.lager_nr == 0) & ((L_bestand.anf_best_dat <= closedate) | (L_bestand.anf_best_dat == None) | (L_bestand.anf_best_dat >= closedate))).filter(L_bestand._recid > curr_recid).first()
