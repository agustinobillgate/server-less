...
def rm_statistic_webbl(pvilanguage:int, call_from:int, txt_file:string, to_date:date, grp_flag:bool, show_ytd:bool, show_other:bool):
    ...
    prepare_cache ([Htparam, Segment, Zinrstat, Genstat])
    ...
    rm_statistic_list_list, Rm_statistic_list = create_model("Rm_statistic_list", {"grpflag":bool, "flag":string, "segm":string, "bezeich":string, "dgroom":string, "proz1":string, "mgroom":string, "proz2":string, "ygroom":string, "proz3":string, "dgpax":string, "mgpax":string, "ygpax":string, "dgrate":string, "mgrate":string, "ygrate":string, "dgrev":string, "mgrev":string, "ygrev":string})
    rm_statistic_list1_list, Rm_statistic_list1 = create_model("Rm_statistic_list1", {"grpflag":bool, "flag":string, "segm":string, "bezeich":string, "droom":string, "proz1":string, "mroom":string, "proz2":string, "yroom":string, "proz3":string, "dpax":string, "mpax":string, "ypax":string, "drate":string, "mrate":string, "yrate":string, "drev":string, "mrev":string, "yrev":string})
    ...
    other_list_list, Other_list = create_model("Other_list", {"flag":string, "segm":int, "bezeich":string, "orev":Decimal, "morev":Decimal, "yorev":Decimal})
    s_list_list, S_list = create_model("S_list", {"nr":int, "bezeich":string, "droom":int, "mroom":int, "yroom":int, "dpax":int, "mpax":int, "ypax":int})
    ...
    def create_other():
        ...
        for zinrstat.logisumsatz, zinrstat.datum, zinrstat._recid, segment.segmentcode, segment.bezeich, segment.segmentgrup, segment._recid in db_session.query(Zinrstat.logisumsatz, Zinrstat.datum, Zinrstat._recid, Segment.segmentcode, Segment.bezeich, Segment.segmentgrup, Segment._recid).join(Segment,(Segment.segmentcode == Zinrstat.betriebsnr) & (Segment.segmentcode != black_list) & (Segment.segmentgrup <= 99)).filter(
            ...
            other_list = query(other_list_list, filters=(lambda other_list: other_list.segm == segment.segmentcode), first=True)
    ...
    def create_umsatz2():
        ...
        genstat = Genstat()
        segment = Segment()
        for genstat.resstatus, genstat.gratis, genstat.segmentcode, genstat.logis, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.kind3, genstat.zipreis, genstat.datum, genstat._recid, segment.segmentcode, segment.bezeich, segment.segmentgrup, segment._recid in db_session.query(Genstat.resstatus, Genstat.gratis, Genstat.segmentcode, Genstat.logis, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.kind3, Genstat.zipreis, Genstat.datum, Genstat._recid, Segment.segmentcode, Segment.bezeich, Segment.segmentgrup, Segment._recid).join(Segment,(Segment.segmentcode == Genstat.segmentcode) & (Segment.segmentcode != black_list)).filter(
                 (Genstat.datum >= from_date) & (Genstat.datum <= to_date) & (Genstat.segmentcode != 0) & (Genstat.nationnr != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)])).order_by(Genstat.segmentcode).all():
    ...
    def create_umsatz():
        ...
        nonlocal rm_statistic_list_list, rm_statistic_list1_list, lnldelimeter, long_digit, droom, proz1, mroom, proz2, yroom, proz3, dpax, mpax, ypax, drate, mrate, yrate, drev, mrev, yrev, lodg, olodg, gproz1, mgroom, gproz2, ygroom, gproz3, dgpax, mgpax, ygpax, dgrate, mgrate, ygrate, dgrev, mgrev, ygrev, dgroom1, mgroom1, ygroom1, dgpax1, mgpax1, ygpax1, dgrate1, mgrate1, ygrate1, dgrev1, mgrev1, ygrev1, ttrev, ttmrev, ttyrev, from_date, period_str, comp_room, curr_grup, tot_droom, tot_proz1, tot_mroom, tot_proz2, tot_yroom, tot_proz3, tot_dpax, tot_mpax, tot_ypax, tot_drate, tot_mrate, tot_yrate, tot_drev, tot_mrev, tot_yrev, lvcarea, htparam, segment, zinrstat, genstat
        nonlocal pvilanguage, call_from, txt_file, to_date, grp_flag, show_ytd, show_other
        nonlocal r_list, r_list1
        ...
        for cl_list in query(cl_list_list, filters=(lambda cl_list: not cl_list.flag_comp), sort_by=[("segm_grup",False)]):
        ...
        if cl_list:
            ...
            for cl_list in query(cl_list_list, filters=(lambda cl_list: cl_list.flag_comp), sort_by=[("segm",False)]):
                rm_statistic_list = Rm_statistic_list()
                rm_statistic_list_list.append(rm_statistic_list)
