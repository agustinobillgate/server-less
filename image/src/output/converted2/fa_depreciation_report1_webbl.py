...
payload_list_list, Payload_list = create_model("Payload_list", {"depn_month":string, "sorttype":int, "last_nr":int, "num_data":int, "mode":int})

def fa_depreciation_report1_webbl(payload_list_list:[Payload_list]):
    ...
    output_list_list, Output_list = create_model("Output_list", {"curr_nr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal depreciation_asset_list, output_list_list, tot_amount, tot_depn_value, tot_book_value, tot_acc_depn, datum, qty, price, amount, counter, counter_num_data, mm, yy, mathis, queasy, fa_artikel, fa_order, fa_ordheader, fa_op


        nonlocal depreciation_asset, payload_list, output_list
        nonlocal depreciation_asset_list, output_list_list

        return {"depreciation-asset": depreciation_asset_list, "output-list": output_list_list}
