...
s_list_list, S_list = create_model("S_list", {"s_flag":string, "selected":bool, "artnr":int, "bezeich":string, "qty":Decimal, "qty0":Decimal, "price":Decimal, "qty2":Decimal})
...
def dml_list_create_po_11bl(s_list_list:[S_list], c_list_list:[C_list], l_orderhdr_recid:int, l_lieferant_recid:int, lief_nr:int, currdate:date, selected_date:date, bediener_username:string, crterm:int, local_nr:int, curr_dept:int, dunit_price:bool):
    ...
    def generate_output():
        nonlocal t_l_orderhdr_list, docu_nr, t_qty, counter, l_orderhdr, l_lieferant, l_order, l_artikel, dml_art, reslin_queasy, dml_artdep
        nonlocal l_orderhdr_recid, l_lieferant_recid, lief_nr, currdate, selected_date, bediener_username, crterm, local_nr, curr_dept, dunit_price
    ...
    s_list = query(s_list_list, filters=(lambda s_list: s_list.selected), first=True)

    c_list = query(c_list_list, filters=(lambda c_list: c_list.artnr == s_list.artnr), first=True)
