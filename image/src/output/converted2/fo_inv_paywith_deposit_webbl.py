...
def fo_inv_paywith_deposit_webbl(pvilanguage:int, bill_recid:int, user_init:string, curr_rechnr:int, res_number:int, resllin_number:int, bill_flag:int, transdate:date, tbill_flag:int, change_date:bool, pay_depoamount:Decimal, amount_foreign:Decimal, curr_room:string, exchg_rate:Decimal, price_decimal:int, double_currency:bool, p_83:bool, kreditlimit:Decimal, foreign_rate:bool, bill_date:date, voucher_nr:string, cancel_str:string):
    ...
    def generate_output():
        nonlocal error_desc, balance, balance_foreign, void_approve, flag3, tot_balance, t_bill_list, t_bill_line_list, spbill_list_list, lvcarea, depoart, depobez, p_253, zugriff, billdatum, r_recid, na_running, gastnrmember, bill, bill_line, bediener, htparam, artikel, res_line, umsatz, billjournal
        ...
        return {"cancel_str": cancel_str, "error_desc": error_desc, "balance": balance, "balance_foreign": balance_foreign, "void_approve": void_approve, "flag3": flag3, "tot_balance": tot_balance, "t-bill": t_bill_list, "t-bill-line": t_bill_line_list, "spbill-list": spbill_list_list}
    ...
    def disp_bill_line():
        ...
        t_bill_line_list, spbill_list_list = get_output(fo_invoice_disp_bill_linebl(bill_recid, double_currency))
    ...
    if curr_rechnr != 0:
        t_bill_list = get_output(read_billbl(2, curr_rechnr, res_number, resllin_number, bill_flag))
