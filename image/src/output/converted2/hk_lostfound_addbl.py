...
def hk_lostfound_addbl(zinr:string, from_date:date, zeit:string, dept:int, reason:string, reportby:string, claim_date:date, phoneno:string, refno:string, foundby:string, location:string, submitted:string, user_init:string):
    ...
    prepare_cache ([Bediener, Counters, Queasy])
    ...
    s_list_list, S_list = create_model("S_list", {"nr":int, "betriebsnr":int, "s_recid":int, "date1":date, "zeit":string, "zinr":string, "userinit":string, "bezeich":string, "foundby":string, "submitted":string, "reportby":string, "claim_date":date, "location":string, "refno":string, "phoneno":string})
