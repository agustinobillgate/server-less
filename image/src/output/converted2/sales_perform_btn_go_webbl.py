...
def sales_perform_btn_go_webbl(from_date:string, to_date:string, usr_init:string, show_breakdown:bool):
    ...
    def check_budget():
        ...
        nonlocal its_ok, slist_list, usr_grp, user_total_lodg, user_total_lbudget, user_total_lproz, user_total_fbrev, user_total_fbbudget, user_total_fbproz, user_total_otrev, user_total_otbudget, user_total_otproz, user_total_rmnight, user_total_rbudget, user_total_rmproz, htparam, bediener, salesbud, salestat, genstat, guest, h_bill, h_artikel, artikel, h_bill_line
        nonlocal from_date, to_date, usr_init, show_breakdown
    ...
    def create_list():
        ...
        for loopi in date_range(htparam.fdate,tdate) :
            ...
            if show_breakdown:
                ...
                slist = query(slist_list, filters=(lambda slist: slist.mnth == get_month(loopi) and slist.yr == get_year(loopi) and slist.userinit == bediener.userinit), first=True)
            ...
            if do_it:
                ...
                if guest.phonetik3 == trim(bediener.userinit):
                    ...
                    if show_breakdown:
                        ...
                        slist = query(slist_list, filters=(lambda slist: slist.mnth == get_month(genstat.datum) and slist.yr == get_year(genstat.datum) and slist.userinit == bediener.userinit), first=True)
        ...
        for salestat.monat, salestat.jahr, salestat.sonst_umsatz, salestat.f_b_umsatz, salestat.bediener_nr, salestat._recid, salesbud.sonst_umsatz, salesbud.f_b_umsatz, salesbud.room_nights, salesbud.argtumsatz, salesbud.bediener_nr, salesbud.monat, salesbud.jahr, salesbud._recid in db_session.query(Salestat.monat, Salestat.jahr, Salestat.sonst_umsatz, Salestat.f_b_umsatz, Salestat.bediener_nr, Salestat._recid, Salesbud.sonst_umsatz, Salesbud.f_b_umsatz, Salesbud.room_nights, Salesbud.argtumsatz, Salesbud.bediener_nr, Salesbud.monat, Salesbud.jahr, Salesbud._recid).join(Salesbud,(Salesbud.bediener_nr == Salestat.bediener_nr) & (Salesbud.monat == Salestat.monat) & (Salesbud.jahr == Salestat.jahr)).filter(
            ...
            if show_breakdown:
                ...
                slist = query(slist_list, filters=(lambda slist:(slist.mnth == salestat.monat) and (slist.Yr == salestat.jahr) and slist.userinit == bediener.userinit), first=True)
    ...
    def count_ytd():
        ...
        for slist in query(slist_list, filters=(lambda slist: slist.Yr >= 0), sort_by=[("userinit",False),("yr",False),("mnth",False)]):

            if user_tmp != slist.userinit:

                sbuff = query(sbuff_list, filters=(lambda sbuff: sbuff.userinit.lower()  == (user_tmp).lower()  and sbuff.bezeich.lower()  == ("T O T A L").lower()), first=True)
    ...
    def count_total():
        ...
        for buff_slist in query(buff_slist_list, filters=(lambda buff_slist: buff_slist.Yr >= 0 and buff_slist.mnth >= 0)):
    ...
    def include_outlet_fnb():
        ...
        if show_breakdown:
            ...
            for slist in query(slist_list, filters=(lambda slist: slist.userinit == bediener.userinit and slist.Yr >= 0), sort_by=[("yr",False),("mnth",False)]):
            ...
            for slist in query(slist_list, filters=(lambda slist: slist.Yr >= 0), sort_by=[("yr",False),("mnth",False)]):
                from_date = date_mdy(slist.mnth, 1, slist.Yr)
