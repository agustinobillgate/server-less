...
def cr_availability1_webbl(pvilanguage:int, vhp_limited:bool, op_type:int, printer_nr:int, call_from:int, adult_child_str:string, statsort:int, dispsort:int, curr_date:date, incl_tentative:bool, mi_inactive:bool, show_rate:bool, indgastnr:int, qci_zinr:string):
    ...
    room_avail_list_list, Room_avail_list = create_model("Room_avail_list", {"avail_flag":bool, "allot_flag":bool, "zikatnr":int, "i_typ":int, "sleeping":bool, "allotment":[int,30], "bezeich":string, "room":[int,30], "coom":[string,30], "sort_prior":int}, {"sleeping": True})
    ...
    tmp_allotment_list, Tmp_allotment = create_model("Tmp_allotment", {"zikatnr":int, "res_allot":[int,30]})
    ...
    def generate_output():
        ...
        nonlocal sum_list_list, room_avail_list_list, date_list_list, room_list_list, rate_list_list, created_list_list, t_kontline_list, rmcat_list_list, tmp_resline_list, tmp_extra_list, temp_art_list, tmp_allotment_list
    ...
    def create_room_list():
        ...
        for room_avail_list in query(room_avail_list_list, sort_by=[("sort_prior",False)]):
            curr_i = curr_i + 1
            room_list = Room_list()
    ...
    def create_rate_list():
        ...
        for room_avail_list in query(room_avail_list_list, sort_by=[("sort_prior",False)]):
            ...
            indgastnr, created_list_list, rate_list_list = get_output(available_ratesbl(htparam.fdate, tdate, room_avail_list.zikatnr, curr_i, adult_child_str, indgastnr, created_list_list))
                ...
                for i in range(1,30 + 1) :
                    ...
                    if datum >= res_line.ankunft and datum < res_line.abreise:
                        ...
                        if kontline and datum >= (ci_date + timedelta(days=kontline.ruecktage)):
                            ...
                            tmp_allotment = query(tmp_allotment_list, filters=(lambda tmp_allotment: tmp_allotment.zikatnr == res_line.zikatnr), first=True)

                            if not tmp_allotment:
                                tmp_allotment = Tmp_allotment()
                                tmp_allotment_list.append(tmp_allotment)
        ...
        if not incl_tentative:
            ...
            if statsort == 1:
                ...
                for i in range(1,30 + 1) :
                    ...
                    for tmp_allotment in query(tmp_allotment_list):

                        room_avail_list = query(room_avail_list_list, filters=(lambda room_avail_list: room_avail_list.zikatnr == tmp_allotment.zikatnr and room_avail_list.sleeping), first=True)

                        if room_avail_list:
                            room_avail_list.allotment[i - 1] = room_avail_list.allotment[i - 1] - tmp_allotment.res_allot[i - 1]
                for i in range(1,30 + 1) :
    ...
    if op_type == 0:
        ...
        for room_avail_list in query(room_avail_list_list):
            ...
            queasy = get_cache (Queasy, {"key": [(eq, 325)],"number1": [(eq, room_avail_list.zikatnr)]})
