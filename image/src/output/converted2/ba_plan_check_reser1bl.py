...
def ba_plan_check_reser1bl(rml_raum:str, rsv_date:date, rsv_i:int):
    ...
    for bk_reser in db_session.query(Bk_reser).filter(
             (func.lower(Bk_reser.raum) == (rml_raum).lower()) & (Bk_reser.datum == rsv_date) & (rsv_i >= Bk_reser.von_i) & (rsv_i <= Bk_reser.bis_i) & (Bk_reser.resstatus <= 2)).order_by(Bk_reser._recid).all():
