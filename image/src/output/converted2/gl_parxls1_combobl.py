...
def gl_parxls1_combobl(foreign_flag:bool, from_date:date, to_date:date, t_parameters_list:[T_parameters]):
    ...
    def fill_revenue():
        ...
        for s_param in query(s_param_list, filters=(lambda s_param: s_param.num_entries(s_param.vstring, ":") == 1 and num_entries(s_param.varname, "-") == 3 and entry(2, s_param.varname, "-") == ("comboREV").lower()), sort_by=[("varname",False)]):
    ...
    def fill_segment():
        ...
        for s_param in query(s_param_list, filters=(lambda s_param: s_param.num_entries(s_param.vstring, ":") == 1 and num_entries(s_param.varname, "-") == 3 and entry(2, s_param.varname, "-") == ("comboFO").lower()  and substring(s_param.vstring, 0, 4) == ("segm").lower()), sort_by=[("varname",False)]):
