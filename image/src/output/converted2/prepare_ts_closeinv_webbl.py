...
def prepare_ts_closeinv_webbl(pvilanguage:int, curr_dept:int, inp_rechnr:int, user_init:string, user_name:string, curr_printer:int):
    ...
    for buff_hart in db_session.query(Buff_hart).filter(
             ((Buff_hart.artnr.in_(list(set([ordered_item.artnr for ordered_item in ordered_item_list])))) & (Buff_hart.departement == ordered_item.dept))).order_by(Buff_hart._recid).all():
