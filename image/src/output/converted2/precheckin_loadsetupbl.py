...
def precheckin_loadsetupbl(icase:int):
    ...
    pci_setup_list, Pci_setup = create_model("Pci_setup", {"number1":int, "number2":int, "descr":string, "setupvalue":string, "setupflag":bool, "price":Decimal, "remarks":string})
    ...
    def generate_output():
        nonlocal pci_setup_list, htl_name, country, f_char, def_nation, do_it, ci_date, tot_room, occ_room, occ_perc, occ1, occ2, p_223, queasy, nation, htparam, paramtext, res_line, zimmer
        nonlocal icase
        nonlocal bresline
    ...
    for t_nation in query(t_nation_list, sort_by=[("entry(",False))]:
        pci_setup = Pci_setup()
        pci_setup_list.append(pci_setup)
    ...
    for t_subnation in query(t_subnation_list, sort_by=[("bezeich",False)]):
        pci_setup = Pci_setup()
        pci_setup_list.append(pci_setup)
