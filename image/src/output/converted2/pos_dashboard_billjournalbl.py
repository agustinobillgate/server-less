...
def pos_dashboard_billjournalbl(from_date:date, to_date:date, from_dept:int, to_dept:int):
    ...
    journal_list_list, Journal_list = create_model("Journal_list", {"bill_date":date, "table_no":int, "bill_no":int, "order_no":int, "article_no":int, "article_name":string, "qty":int, "amount":Decimal, "payment":Decimal, "dept_name":string, "id":string, "bill_time":string, "guest_name":string, "room_no":string})
    t_list_list, T_list = create_model("T_list", {"bill_date":date, "dept_no":int, "rechnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal journal_list_list, disc_art1, disc_art2, disc_art3, htparam, queasy, hoteldpt, h_journal, h_artikel, h_bill, res_line, guest
        nonlocal from_date, to_date, from_dept, to_dept


        nonlocal journal_list, t_list
        nonlocal journal_list_list, t_list_list

        return {"journal-list": journal_list_list}
    ...
    def journal_list():
        ...
        for queasy in db_session.query(Queasy).filter(
            ...
            if validate_rechnr != 0:
                ...
                t_list = query(t_list_list, filters=(lambda t_list: t_list.rechnr == validate_rechnr and t_list.bill_date == queasy.date1 and t_list.dept_no == queasy.number1), first=True)
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            for h_journal in db_session.query(H_journal).filter(
                ...
                t_list = query(t_list_list, (lambda t_list: t_list.rechnr == h_journal.rechnr and t_list.dept_no == h_journal.departement), first=True)
                if not t_list:
                    continue
                ...
                if (h_journal.artnr == disc_art1 or h_journal.artnr == disc_art2 or h_journal.artnr == disc_art3) and h_journal.betrag == 0:
                    ...
                    journal_list_list.append(journal_list)
