...
def move_resmember1bl(pvilanguage:int, resno:int, sorttype:int):
    ...
    tlist_list, Tlist = create_model("Tlist", {"select_flag":bool, "name":string, "ankunft":date, "abreise":date, "zinr":string, "kurzbez":string, "zipreis":Decimal, "arrangement":string, "erwachs":int, "gratis":int, "kind1":int, "kind2":int, "resstatus":string, "zimmeranz":int, "anztage":int})
    t_zimkateg_list, T_zimkateg = create_model_like(Zimkateg)
    buf_r_list_list, Buf_r_list = create_model_like(R_list)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal r_list_list, tlist_list, lvcarea, stat_list, done, res_line, zimkateg
        nonlocal pvilanguage, resno, sorttype


        nonlocal r_list, tlist, t_zimkateg, buf_r_list
        nonlocal r_list_list, tlist_list, t_zimkateg_list, buf_r_list_list

        return {"r-list": r_list_list, "tlist": tlist_list}
    ...
    done, r_list_list = get_output(move_resmemberbl(1, resno, sorttype, None, buf_r_list_list))
    t_zimkateg_list = get_output(read_zimkategbl(4, None, ""))

    for r_list in query(r_list_list, sort_by=[("zinr",False),("kontakt_nr",False),("resstatus",False)]):
        t_zimkateg = query(t_zimkateg_list, (lambda t_zimkateg: t_zimkateg.zikatnr == r_list.zikatnr), first=True)
        if not t_zimkateg:
            continue
