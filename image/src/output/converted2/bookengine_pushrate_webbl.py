...
rate_list2_list, Rate_list2 = create_model("Rate_list2", {"origcode":string, "rcode":[Decimal,31], "berates":[Decimal,31], "datum":date})

def bookengine_pushrate_webbl(rate_list2_list:[Rate_list2]):
    echotoken = ""
    timestamp = ""
    rlist_list = []
    uuid:bytes = None
    ...
    rlist_list, Rlist = create_model("Rlist", {"rcode":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal echotoken, timestamp, rlist_list, uuid
        nonlocal rate_list2_list
