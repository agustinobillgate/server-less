...
sortir_list_list, Sortir_list = create_model("Sortir_list", {"from_date":date, "to_date":date, "location":int, "show_all":bool, "asset_name":string, "remark":string})

def prepare_fa_artlist1_webbl(sortir_list_list:[Sortir_list], idflag:string):
    ...
    def generate_output():
        nonlocal p_881, q1_list_list, fibu_list_list, sort_loc, mark, htl_no, counter, q1_list_datum, q1_list_first_depn, q1_list_next_depn, q1_list_last_depn, q1_list_created, q1_list_changed, q1_list_supplier, q1_list_price, q1_list_mark, q1_list_model, q1_list_spec, q1_list_remark, fa_grup, queasy
        nonlocal idflag
    ...
    pass
    p_881, q1_list_list, fibu_list_list = get_output(prepare_fa_artlist_cldbl(sortir_list_list))

    q1_list = query(q1_list_list, first=True)
    while None != q1_list:
        ...
        q1_list_list.remove(q1_list)

        q1_list = query(q1_list_list, next=True)

    fibu_list = query(fibu_list_list, first=True)
    while None != fibu_list:
        ...
        fibu_list_list.remove(fibu_list)

        fibu_list = query(fibu_list_list, next=True)
