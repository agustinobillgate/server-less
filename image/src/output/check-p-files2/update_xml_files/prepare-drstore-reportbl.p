...
IF AVAILABLE htparam THEN p-110 = htparam.fdate. 

FIND FIRST htparam WHERE paramnr = 1082 NO-LOCK NO-ERROR.  /* Drugstore Dept*/ 
IF AVAILABLE htparam THEN dstore-dept = htparam.finteger.               /* Rulita 211024 | Fixing for serverless */  
...
FOR EACH kellner WHERE kellner.departement = dstore-dept NO-LOCK, 
    FIRST hoteldpt WHERE hoteldpt.num = kellner.departement NO-LOCK BY kellner.kellner-nr. 
    CREATE user-list.
    ASSIGN
...
FOR EACH h-artikel WHERE h-artikel.departement = dstore-dept 
    ...
    DO: 
        ...
        FIND FIRST wgrpdep WHERE wgrpdep.departement = dstore-dept AND wgrpdep.zknr = h-artikel.zwkum NO-LOCK NO-ERROR. 
