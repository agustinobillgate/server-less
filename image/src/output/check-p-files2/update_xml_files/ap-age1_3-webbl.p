...
DEFINE TEMP-TABLE apage-list
    ...
    FIELD bemerk     AS CHARACTER. /* naufal afthar 18/09/24 - D59F3C */

DEFINE TEMP-TABLE apage-list1
    FIELD docu-nr   AS CHARACTER FORMAT "x(16)" 
    FIELD lschein   AS CHARACTER FORMAT "x(14)" 
    FIELD rgdatum   AS CHARACTER FORMAT "x(8)"
    FIELD rechnr    AS CHARACTER              /*FD April 30, 2021*/
    ...
    FIELD prev-bal  AS CHARACTER
    FIELD credit    AS CHARACTER
    FIELD debit     AS CHARACTER
    FIELD end-bal   AS CHARACTER
    FIELD day       AS CHARACTER
    FIELD day1      AS CHARACTER
    FIELD day2      AS CHARACTER
    FIELD day3      AS CHARACTER
    FIELD bemerk     AS CHARACTER. /* naufal afthar 18/09/24 - D59F3C */
...
DEFINE INPUT  PARAMETER by-supplier  AS LOGICAL.    /* Dzikri BE3CE3 & BB07DB - Sort by supplier */
DEFINE OUTPUT PARAMETER TABLE        FOR apage-list1.
...
DEFINE VARIABLE bemerk        AS CHARACTER NO-UNDO FORMAT "x(30)". /* naufal afthar 18/09/24 - D59F3C */
...
/* Dzikri BE3CE3 & BB07DB - Sort by supplier */
DEFINE VARIABLE sub-prev-bal AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-credit   AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-debit    AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-end-bal  AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-day      AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-day1     AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-day2     AS DECIMAL NO-UNDO.
DEFINE VARIABLE sub-day3     AS DECIMAL NO-UNDO.
DEFINE VARIABLE data-exist   AS INTEGER NO-UNDO INIT 0.
/* Dzikri BE3CE3 & BB07DB - END */
...
DEFINE WORKFILE age-list 
    ...
    FIELD bemerk           AS CHARACTER FORMAT "x(34)"
        ...
        FOR EACH l-kredit WHERE l-kredit.rgdatum LE to-date AND 
            ...
            DO:
                ...
                DO: 
                    ...
                    age-list.bemerk  = l-kredit.bemerk. /* naufal afthar 18/09/24 - D59F3C */
          ...
          DO:
              ...
              DO:
                  ASSIGN  
                      ...
                      bemerk        = age-list.bemerk
                  .
                  counter = counter + 1. 
              END.
              ELSE
              DO:
                  ASSIGN  curr-rgdatum = STRING(age-list.rgdatum, "99/99/99")
                          curr-po      = age-list.rechnr
                          curr-lschein = age-list.lschein
                          firma        = age-list.firma
...
PROCEDURE create-apage-list1:
    IF by-supplier THEN
    DO:
        FOR EACH l-lieferant NO-LOCK BY l-lieferant.firma:
            sub-prev-bal         = 0.
            sub-credit           = 0.
            sub-debit            = 0.
            sub-end-bal          = 0.
            ...
            sub-day3             = 0.
            data-exist           = 0.
            FOR EACH apage-list WHERE NOT apage-list.str MATCHES ("*---*") AND TRIM(SUBSTRING(apage-list.STR, 8, 30)) EQ l-lieferant.firma:
                CREATE apage-list1.
                BUFFER-COPY apage-list TO apage-list1.
                ASSIGN
                    apage-list1.nr       = SUBSTRING(apage-list.STR, 1, 7) 
                    apage-list1.firma    = SUBSTRING(apage-list.STR, 8, 30) 
                    apage-list1.prev-bal = SUBSTRING(apage-list.STR,56, 18) 
                    apage-list1.credit   = SUBSTRING(apage-list.STR,74, 18) 
                    apage-list1.debit    = SUBSTRING(apage-list.STR,92, 18) 
                    apage-list1.end-bal  = SUBSTRING(apage-list.STR,38, 18) 
                    apage-list1.day      = SUBSTRING(apage-list.STR,110, 18)
                    apage-list1.day1     = SUBSTRING(apage-list.STR,128, 18)
                    apage-list1.day2     = SUBSTRING(apage-list.STR,146, 18)
                    apage-list1.day3     = SUBSTRING(apage-list.STR,164, 18)
                    apage-list1.bemerk   = apage-list.bemerk
                    sub-prev-bal         = sub-prev-bal + DECIMAL(TRIM(apage-list1.prev-bal))
                    sub-credit           = sub-credit   + DECIMAL(TRIM(apage-list1.credit  ))
                    sub-debit            = sub-debit    + DECIMAL(TRIM(apage-list1.debit   ))
                    sub-end-bal          = sub-end-bal  + DECIMAL(TRIM(apage-list1.end-bal ))
                    sub-day              = sub-day      + DECIMAL(TRIM(apage-list1.day     ))
                    sub-day1             = sub-day1     + DECIMAL(TRIM(apage-list1.day1    ))
                    sub-day2             = sub-day2     + DECIMAL(TRIM(apage-list1.day2    ))
                    sub-day3             = sub-day3     + DECIMAL(TRIM(apage-list1.day3    ))
                    data-exist           = 1
                .
            END. 
            IF data-exist NE 0 THEN
            DO:
            CREATE apage-list1.
                ASSIGN
                    apage-list1.STR       = STRING(translateExtended ("T O T A L  A/P:",lvCAREA,""), "x(30)") 
                                            + STRING(ROUND(sub-prev-bal, 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-credit  , 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-debit   , 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-end-bal , 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-day     , 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-day1    , 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-day2    , 0), "->>,>>>,>>>,>>9.99")
                                            + STRING(ROUND(sub-day3    , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.nr        = "       " 
                    apage-list1.firma     = STRING(translateExtended ("T O T A L  A/P:",lvCAREA,""), "x(30)") 
                    apage-list1.prev-bal  = STRING(ROUND(sub-prev-bal, 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.credit    = STRING(ROUND(sub-credit  , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.debit     = STRING(ROUND(sub-debit   , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.end-bal   = STRING(ROUND(sub-end-bal , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.day       = STRING(ROUND(sub-day     , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.day1      = STRING(ROUND(sub-day1    , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.day2      = STRING(ROUND(sub-day2    , 0), "->>,>>>,>>>,>>9.99")
                    apage-list1.day3      = STRING(ROUND(sub-day3    , 0), "->>,>>>,>>>,>>9.99")
                .
            END.
        END.
        FIND FIRST apage-list WHERE NOT apage-list.str MATCHES ("*---*") AND SUBSTRING(apage-list.STR, 8, 9) EQ "T O T A L" NO-LOCK NO-ERROR.
        IF AVAILABLE apage-list THEN 
        DO:
            CREATE apage-list1.
            BUFFER-COPY apage-list TO apage-list1.
            ASSIGN
                apage-list1.nr       = SUBSTRING(apage-list.STR, 1, 7) 
                apage-list1.firma    = STRING(translateExtended ("Grand T O T A L  A/P:",lvCAREA,""), "x(30)") 
                apage-list1.prev-bal = SUBSTRING(apage-list.STR,56, 18) 
                apage-list1.credit   = SUBSTRING(apage-list.STR,74, 18) 
                apage-list1.debit    = SUBSTRING(apage-list.STR,92, 18) 
                apage-list1.end-bal  = SUBSTRING(apage-list.STR,38, 18) 
    ...
    ELSE
    DO:
        FOR EACH apage-list WHERE NOT apage-list.str MATCHES ("*---*"):
            CREATE apage-list1.
            BUFFER-COPY apage-list TO apage-list1.
            ASSIGN
                apage-list1.nr       = SUBSTRING(apage-list.STR, 1, 7) 
