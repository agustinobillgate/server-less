...
DEFINE TEMP-TABLE pay-list 
  FIELD compli  AS LOGICAL INITIAL NO /* Malik Serverless 634 change to capital letter no -> NO */ 
...
DEFINE VARIABLE d-found     AS LOGICAL INITIAL NO. /* Malik Serverless 634 change to logical not string : "no" -> NO */
DEFINE VARIABLE c-found     AS LOGICAL INITIAL NO. /* Malik Serverless 634 change to logical not string : "no" -> NO */
DEFINE VARIABLE vat         AS DECIMAL. 
DEFINE VARIABLE service     AS DECIMAL. 
DEFINE VARIABLE netto       AS DECIMAL. 
DEFINE VARIABLE i           AS INTEGER. 
DEFINE VARIABLE found       AS LOGICAL INITIAL NO. /* Malik Serverless 634 change to capital letter no -> NO */
...
DEF BUFFER h-bline FOR h-bill-line.
  ...
  FOR EACH bline-list WHERE bline-list.selected = YES, 
    FIRST kellner WHERE RECID(kellner) = bline-list.bl-recid /* Malik Serverless 634 change recid(kellner) -> RECID(kellner) */
    ...
    FOR EACH h-bill WHERE h-bill.flag EQ 0 and h-bill.saldo NE 0 
      ...
      NO-LOCK USE-INDEX dept1_ix by h-bill.rechnr: /* Malik Serverless 634 change to capital letter : use-index -> USE-INDEX */
