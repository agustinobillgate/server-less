...
DEFINE TEMP-TABLE payload-list 
  FIELD include-compliment   AS LOGICAL
  FIELD compliment-only      AS LOGICAL.
...
DEFINE VARIABLE t-anz-deb     AS INTEGER INITIAL 0. 
...
FIND FIRST payload-list NO-ERROR. /* Malik 0DA2D5 */
...
PROCEDURE create-h-umsatz1: 
    ...
    FOR EACH hoteldpt WHERE hoteldpt.num GE from-dept 
        ...
        dept = hoteldpt.num. 
        /* Malik 0DA2D5 */
        IF payload-list.compliment-only OR payload-list.include-compliment THEN
        DO:
          FOR EACH h-artikel WHERE h-artikel.artart = 0 
              ...
              DO: 
                  ...
                  */
                  ...
                  /* Malik 0DA2D5 */
                  IF NOT payload-list.compliment-only THEN
                  DO:
                    ...
                    /* Malik Include Compliment */
                    IF payload-list.include-compliment THEN     
                    DO:      
                      FOR EACH h-compli WHERE h-compli.datum GE from-date 
                        AND h-compli.datum LE to-date 
                        AND h-compli.departement EQ hoteldpt.num 
                        AND h-compli.betriebsnr EQ 0 
                        AND h-compli.artnr = h-artikel.artnr NO-LOCK:
                          h-list.anzahl = h-list.anzahl + h-compli.anzahl.
                          h-list.t-sales = h-list.t-sales + h-compli.epreis.
                          h-list.t-cost = h-list.t-cost + h-compli.anzahl * h-artikel.epreis1 * h-artikel.prozent / 100 * exchg-rate.
                          
                          t-anz = t-anz + h-compli.anzahl. 
                          t-sales = t-sales + h-compli.epreis.
                      END.
                    END.
                  END.
                  ELSE
                  DO:
                    /* Malik 0DA2D5 -> Compliment Only */
                    FOR EACH h-compli WHERE h-compli.datum GE from-date 
                      AND h-compli.datum LE to-date 
                      AND h-compli.departement EQ hoteldpt.num 
                      AND h-compli.betriebsnr EQ 0 
                      AND h-compli.artnr = h-artikel.artnr NO-LOCK:
                        ...
                        RUN calc-servtaxesbl.p(1, artikel.artnr, artikel.departement,h-compli.datum,  /*willi change datum to h-umsatz.datum 14/03/24*/
                                              OUTPUT serv, OUTPUT vat, OUTPUT vat2, OUTPUT fact).
                        ASSIGN vat = vat + vat2.
        ...
        DO:     
            ...
            */
            ...
            DO:
              ...
              /* Malik 0DA2D5 -> Include Compliment */
              IF payload-list.include-compliment THEN     
              DO:      
                FOR EACH h-compli WHERE h-compli.datum GE from-date 
                  AND h-compli.datum LE to-date 
                  ...
                  DO:
                    ...
                    DO:      
                      FOR EACH h-compli WHERE h-compli.datum GE from-date 
                        ...
                        AND h-compli.departement EQ hoteldpt.num 
                        AND h-compli.betriebsnr EQ 0 
                        AND h-compli.artnr EQ h-artikel.artnr NO-LOCK:
                          h-list.anzahl = h-list.anzahl + h-compli.anzahl.
                          h-list.t-sales = h-list.t-sales + h-compli.epreis.
                          h-list.t-cost = h-list.t-cost + h-compli.anzahl * h-artikel.epreis1 * h-artikel.prozent / 100 * exchg-rate.
