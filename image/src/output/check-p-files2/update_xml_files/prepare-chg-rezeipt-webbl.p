...
DEF OUTPUT PARAMETER cost-percent AS DECIMAL. /*bernatd*/
DEF OUTPUT PARAMETER poten-sell-price AS DECIMAL. /*bernatd*/
...
FIND FIRST queasy WHERE queasy.KEY EQ 252 AND queasy.number1 EQ h-artnr NO-LOCK NO-ERROR.
...
DEFINE buffer h-recipe FOR h-rezept.   
  ...
  DO:   
    ...
    ELSE
    DO:
      s-rezlin.cost = h-rezlin.menge / l-artikel.inhalt * l-artikel.ek-aktuell   
       / (1 - h-rezlin.lostfact / 100).
...
DEFINE VARIABLE artikel AS DECIMAL.
  ...
  FOR EACH h-rezln WHERE h-rezln.artnrrezept = p-artnr NO-LOCK: 
    ...
    DO:   
      ...
      MESSAGE cost "+" inh "/" l-artikel.inhalt "*" l-artikel.vk-preis
        VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
        IF price-type = 0 OR l-artikel.ek-aktuell = 0 THEN
        DO:
          artikel = inh / l-artikel.inhalt * l-artikel.vk-preis.
          cost = cost + inh / l-artikel.inhalt * l-artikel.vk-preis. 
           
        END. 
        ELSE
        DO:
          artikel = inh / l-artikel.inhalt * l-artikel.ek-aktuell. 
          cost = cost + inh / l-artikel.inhalt * l-artikel.ek-aktuell. 
