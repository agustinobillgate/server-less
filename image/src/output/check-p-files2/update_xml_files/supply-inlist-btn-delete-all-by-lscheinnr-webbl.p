DEFINE TEMP-TABLE l-op-not-eligible
...
DEF OUTPUT PARAMETER TABLE FOR l-op-not-eligible.
...
DO:
  ...
  FOR EACH gl-jouhdr WHERE gl-jouhdr.datum EQ str-list-billdate
...
PROCEDURE check-onhand-after-cancel-receiving: 
    ...
    FOR EACH l-op WHERE l-op.lief-nr GT 0 
        ...
        FIRST l-artikel WHERE l-artikel.artnr = l-op.artnr NO-LOCK:
            ...
            DO:
                ...
                CREATE l-op-not-eligible.
        
                ASSIGN
                    l-op-not-eligible.lscheinnr = l-op.lscheinnr
                    l-op-not-eligible.artnr     = l-op.artnr
                    l-op-not-eligible.bezeich   = l-artikel.bezeich
                    l-op-not-eligible.reason    = msg-str1
                .
    ...
    FIND FIRST l-op-not-eligible NO-LOCK NO-ERROR.
    IF NOT AVAILABLE l-op-not-eligible THEN
    DO:
        may-delete = YES.
    END.
END. 
