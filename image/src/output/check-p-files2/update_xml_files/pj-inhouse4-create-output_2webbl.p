...
DEFINE TEMP-TABLE tot-list
    FIELD tot-payrm    AS CHAR
    FIELD tot-rm       AS CHAR
    FIELD tot-a        AS CHAR
    FIELD tot-c        AS CHAR
    FIELD tot-co       AS CHAR
    FIELD tot-avail    AS CHAR
    FIELD inactive     AS CHAR
    FIELD tot-keycard  AS CHAR.
...
DEFINE OUTPUT PARAMETER TABLE FOR tot-list.
DEFINE OUTPUT PARAMETER TABLE FOR summary-list4. /*bernatd*/
...
IF doneFlag THEN DO:
    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum" 
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "roomtype" NO-LOCK BY queasy.number1:
        CREATE summary-roomtype.
        ASSIGN summary-roomtype.rmcat      = ENTRY(1, queasy.char2, "|")  
               summary-roomtype.bezeich    = ENTRY(2, queasy.char2, "|")  
               summary-roomtype.anz        = INTEGER(ENTRY(3, queasy.char2, "|"))  
               summary-roomtype.proz-qty   = DECIMAL(ENTRY(4, queasy.char2, "|"))           
               summary-roomtype.rev        = DECIMAL(ENTRY(5, queasy.char2, "|"))  
               summary-roomtype.proz-rev   = DECIMAL(ENTRY(6, queasy.char2, "|"))           
               summary-roomtype.arr        = DECIMAL(ENTRY(7, queasy.char2, "|")).
    END.

    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum" 
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "nation" NO-LOCK BY queasy.number1:
        CREATE summary-nation.
        ASSIGN summary-nation.nat     = ENTRY(1, queasy.char2, "|")  
               summary-nation.adult   = ENTRY(2, queasy.char2, "|")  
               summary-nation.proz    = ENTRY(3, queasy.char2, "|")  
               summary-nation.child   = ENTRY(4, queasy.char2, "|").
    END.

    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum" 
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "revenue" NO-LOCK BY queasy.number1:
        CREATE summary-revenue.
        ASSIGN summary-revenue.currency   = ENTRY(1, queasy.char2, "|")  
               summary-revenue.room-rate  = DECIMAL(ENTRY(2, queasy.char2, "|"))  
               summary-revenue.lodging    = DECIMAL(ENTRY(3, queasy.char2, "|"))  
               summary-revenue.b-amount   = DECIMAL(ENTRY(4, queasy.char2, "|"))           
               summary-revenue.l-amount   = DECIMAL(ENTRY(5, queasy.char2, "|"))  
               summary-revenue.d-amount   = DECIMAL(ENTRY(6, queasy.char2, "|"))           
               summary-revenue.o-amount   = DECIMAL(ENTRY(7, queasy.char2, "|")).
    END.
    
    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum" 
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "segment" NO-LOCK BY queasy.number1:
        CREATE summary-segment.                                                    
        ASSIGN summary-segment.segmcode  = INTEGER(ENTRY(1, queasy.char2, "|"))           
               summary-segment.segment   = ENTRY(2, queasy.char2, "|")
               summary-segment.anzahl    = INTEGER(ENTRY(3, queasy.char2, "|"))  
               summary-segment.proz-qty  = DECIMAL(ENTRY(4, queasy.char2, "|"))  
               summary-segment.rev       = DECIMAL(ENTRY(5, queasy.char2, "|"))  
               summary-segment.proz-rev  = DECIMAL(ENTRY(6, queasy.char2, "|"))  
               summary-segment.arr       = DECIMAL(ENTRY(7, queasy.char2, "|")).
    END.
    
    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum" 
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "summary" NO-LOCK BY queasy.number1:
        CREATE sum-list.                                                    
        ASSIGN sum-list.curr     = ENTRY(1, queasy.char2, "|")           
               sum-list.zipreis  = DECIMAL(ENTRY(2, queasy.char2, "|"))
               sum-list.lodging  = DECIMAL(ENTRY(3, queasy.char2, "|"))  
               sum-list.bfast    = DECIMAL(ENTRY(4, queasy.char2, "|"))  
               sum-list.lunch    = DECIMAL(ENTRY(5, queasy.char2, "|"))  
               sum-list.dinner   = DECIMAL(ENTRY(6, queasy.char2, "|"))  
               sum-list.other    = DECIMAL(ENTRY(7, queasy.char2, "|")).
    ...
    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum"
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "summary-list4" NO-LOCK BY queasy.number1:
        CREATE summary-list4.                                                    
        ASSIGN         
               summary-list4.argt    = ENTRY(1, queasy.char2,         "|")  
               summary-list4.rm-qty  = INTEGER(ENTRY(2, queasy.char2, "|"))  
               summary-list4.pax     = INTEGER(ENTRY(3, queasy.char2, "|")).  
               /*summary-list4.bfast   = DECIMAL(ENTRY(4, queasy.char2, "|")).*/
    ...
    FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Inhouse List Sum" 
        AND queasy.number2 = INT(idFlag)
        AND queasy.char3 = "total" NO-LOCK BY queasy.number1:
        CREATE tot-list.
        ASSIGN tot-list.tot-payrm     = ENTRY(1, queasy.char2, "|")          
               tot-list.tot-rm        = ENTRY(2, queasy.char2, "|") 
               tot-list.tot-a         = ENTRY(3, queasy.char2, "|") 
               tot-list.tot-c         = ENTRY(4, queasy.char2, "|") 
               tot-list.tot-co        = ENTRY(5, queasy.char2, "|") 
               tot-list.tot-avail     = ENTRY(6, queasy.char2, "|") 
               tot-list.inactive      = ENTRY(7, queasy.char2, "|")
               tot-list.tot-keycard   = ENTRY(8, queasy.char2, "|").

    END.
    */

    FIND FIRST pqueasy WHERE pqueasy.KEY = 280 
    AND pqueasy.char1 = "Inhouse List Sum"
    AND pqueasy.number2 = INT(idFlag) NO-LOCK NO-ERROR.
    IF AVAILABLE pqueasy THEN DO:
    END.
    ELSE DO:
        FIND FIRST tqueasy WHERE tqueasy.KEY = 285 
            AND tqueasy.char1 = "Inhouse List Sum" 
            AND tqueasy.number1 = 1
            AND tqueasy.number2 = INT(idFlag) NO-LOCK NO-ERROR.
        IF AVAILABLE tqueasy THEN DO:
        END.
