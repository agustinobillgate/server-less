...
DEFINE TEMP-TABLE room-list 
  ...
  FIELD room        AS DECIMAL EXTENT 32 FORMAT "->>>9" /* Malik Serverless : 21 -> 32 */ 
  FIELD coom        AS CHAR EXTENT 32 FORMAT "x(5)" /* Malik Serverless : 21 -> 32 */
  FIELD glores      AS LOGICAL. 

DEFINE TEMP-TABLE sum-list 
  FIELD bezeich     AS CHAR FORMAT "x(15)" 
  FIELD summe       AS CHAR EXTENT 32 FORMAT "x(5)". /* Malik Serverless : 21 -> 32 */
...
DEFINE INPUT  PARAMETER to-date          AS DATE.  /* Malik 6E62A6 */
/* DEFINE VAR to-date AS DATE INITIAL 10/23/24. */
...
DEFINE VARIABLE ttl-room                AS INTEGER EXTENT 32 NO-UNDO. /* Malik 6E62A6 21 -> 32 */
DEFINE VARIABLE occ-room                AS INTEGER EXTENT 32 NO-UNDO. /* Malik 6E62A6 21 -> 32 */
DEFINE VARIABLE ooo-room                AS INTEGER EXTENT 32 NO-UNDO. /* Malik 6E62A6 21 -> 32 */
...
DEFINE VAR tmp-date AS DATE. /* Malik 6E62A6 */
...
/***********MAIN Logic***********/
tmp-date = curr-date - 1. /* Malik 6E62A6 */
FIND FIRST htparam WHERE htparam.paramnr = 87 NO-LOCK. 
ci-date = htparam.fdate .

DO i = 1 TO (to-date - tmp-date): /* Malik 6E62A6 21 -> (to-date - tmp-date) */
  ASSIGN
    ttl-room[i] = 0
    occ-room[i] = 0
    ooo-room[i] = 0
...
PROCEDURE create-browse: 
  ...
  DEFINE VARIABLE tmp-list    AS INTEGER EXTENT 32. /* Malik 6E62A6 21 -> 32 */ 
  ...
  FOR EACH rmcat-list,
    ...
    create room-list. 
    m = 1. 
    DO WHILE m LE (to-date - tmp-date): /* Malik 6E62A6 21 -> (to-date - tmp-date) */
  ...
  FOR EACH outorder WHERE outorder.betriebsnr LE 1 NO-LOCK, 
    ...
    END. 
    ELSE 
    DO m = 1 TO (to-date - tmp-date): /* Malik 6E62A6 21 -> (to-date - tmp-date) */
  ...
  DO m = 1 TO (to-date - tmp-date): /* Malik 6E62A6 21 -> (to-date - tmp-date) */
    s = INTEGER(tmp-list[m]) / tot-room * 100. 
...
PROCEDURE create-rmcat-list: 
  ...
  ELSE n = 99.
  DO WHILE n LE (to-date - tmp-date): /* Malik 6E62A6 21 -> (to-date - tmp-date) */
