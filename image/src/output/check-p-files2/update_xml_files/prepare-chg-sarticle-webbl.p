...
.
DEFINE TEMP-TABLE tt-bezeich
    FIELD curr-i     AS INTEGER
    FIELD ss-bezeich AS CHAR
.
DEFINE TEMP-TABLE t-message-list        
    FIELD lock-flag1    AS LOGICAL
    FIELD lock-flag2    AS LOGICAL
.
...
DEFINE OUTPUT PARAMETER param89     AS LOGICAL.
DEFINE OUTPUT PARAMETER f-endkum    AS INTEGER.
DEFINE OUTPUT PARAMETER b-endkum    AS INTEGER.
DEFINE OUTPUT PARAMETER o-endkum    AS INTEGER.
...
DEFINE OUTPUT PARAMETER TABLE FOR t-message-list.
...
DEFINE VARIABLE count-lophis    AS INTEGER NO-UNDO.
DEFINE VARIABLE count-lop       AS INTEGER NO-UNDO.

/*FDL Nov 16, 2023 => Ticket 639F6F*/
FIND FIRST htparam WHERE htparam.paramnr EQ 89 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN param89 = htparam.flogic.
FIND FIRST htparam WHERE htparam.paramnr EQ 257 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN f-endkum = htparam.fint.
FIND FIRST htparam WHERE htparam.paramnr EQ 258 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN b-endkum = htparam.fint.
FIND FIRST htparam WHERE htparam.paramnr EQ 268 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN o-endkum = htparam.fint.
...
END. 
ELSE FIND FIRST l-artikel WHERE l-artikel.artnr = artnr NO-LOCK. 
...
DO: 
    FIND FIRST h-rezept WHERE h-rezept.artnrrezept = l-artikel.betriebsnr NO-LOCK NO-ERROR. 
    IF AVAILABLE h-rezept THEN recipe-bez = h-rezept.bezeich. 
END. 
 
IF l-art.lief-nr1 NE 0 THEN 
DO: 
    FIND FIRST l-lieferant WHERE l-lieferant.lief-nr = l-art.lief-nr1 NO-LOCK NO-ERROR. 
    IF AVAILABLE l-lieferant THEN firma1 = l-lieferant.firma. 
END. 
IF l-art.lief-nr2 NE 0 THEN 
DO: 
    FIND FIRST l-lieferant WHERE l-lieferant.lief-nr = l-art.lief-nr2 NO-LOCK NO-ERROR. 
    IF AVAILABLE l-lieferant THEN firma2 = l-lieferant.firma. 
END. 
IF l-art.lief-nr3 NE 0 THEN 
DO: 
    FIND FIRST l-lieferant WHERE l-lieferant.lief-nr = l-art.lief-nr3 NO-LOCK NO-ERROR. 
    IF AVAILABLE l-lieferant THEN firma3 = l-lieferant.firma. 
END. 
 
IF l-art.zwkum NE 0 THEN 
DO: 
    FIND FIRST l-untergrup WHERE l-untergrup.zwkum = l-art.zwkum NO-LOCK NO-ERROR. 
    IF AVAILABLE l-untergrup THEN zw-bezeich = l-untergrup.bezeich. 
END. 
 
IF l-art.endkum NE 0 THEN 
DO: 
    FIND FIRST l-hauptgrp WHERE l-hauptgrp.endkum = l-art.endkum NO-LOCK NO-ERROR. 
...
/*FDL Jan 06, 2025: 93E619*/
CREATE t-message-list.
FIND FIRST l-ophis WHERE l-ophis.artnr EQ artnr
    AND l-ophis.op-art EQ 1 NO-LOCK NO-ERROR.
IF AVAILABLE l-ophis THEN t-message-list.lock-flag1 = YES.
ELSE
DO:
    FOR EACH l-op WHERE l-op.artnr EQ artnr
        AND l-op.loeschflag NE 2
        AND l-op.op-art EQ 1 NO-LOCK:
        count-lop = count-lop + 1.
        IF count-lop GE 5 THEN LEAVE.
    END.
    IF count-lop NE 0 THEN t-message-list.lock-flag1 = YES.
END.

FIND FIRST h-rezlin WHERE h-rezlin.artnrlager EQ artnr NO-LOCK NO-ERROR.
IF AVAILABLE h-rezlin THEN t-message-list.lock-flag2 = YES.
/*************************** PROCEDURES ****************************************/ 
...
DEF BUFFER l-art1 FOR l-artikel. 
    ...
    FIND FIRST queasy WHERE queasy.KEY = 20 AND queasy.number1 = l-artikel.artnr NO-LOCK NO-ERROR. 
