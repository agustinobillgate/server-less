DEFINE TEMP-TABLE c-list 
  ...
  FIELD f-betrag AS DECIMAL FORMAT "->>>,>>>,>>9.99" 
    LABEL "    Food Amount" 
...
/** Beverage Betrag **/ 
      ...
      IF mi-detail1 THEN 
      DO: 
        FIND FIRST c2-list WHERE c2-list.datum = h-compli.datum 
          AND c2-list.dept = h-compli.departement 
          AND c2-list.rechnr = h-compli.rechnr 
          AND c2-list.artnr = h-compli.artnr NO-ERROR. 
        ...
        c2-list.betrag = c2-list.betrag 
          + h-compli.anzahl * h-compli.epreis * rate. 
        IF /*artikel.endkum = f-endkum*/ artikel.umsatzart = 3 
            OR artikel.umsatzart = 5 THEN c2-list.f-betrag = c2-list.f-betrag 
          + h-compli.anzahl * h-compli.epreis * rate. 
        ELSE IF /*artikel.endkum = b-endkum*/ artikel.umsatzart = 6 
          THEN c2-list.b-betrag 
          = c2-list.b-betrag + h-compli.anzahl * h-compli.epreis * rate. 
  ...
  IF it-exist THEN 
  DO: 
    FOR EACH s-list WHERE s-list.creditlimit NE 0 
      AND s-list.creditlimit LT s-list.betrag AND s-list.flag = 0: 
      create c-list. 
      nr = nr + 1. 
      c-list.nr = nr. 
      c-list.flag = 1. 
...
/** Beverage Betrag **/ 
  ...
  o-cost = 0. 
  FOR EACH c1-list WHERE c1-list.betrag NE 0 BY c1-list.name 
    BY c1-list.datum BY c1-list.dept: 
    IF curr-name = "" THEN curr-name = c1-list.name. 
    IF curr-name NE c1-list.name THEN 
    DO: 
      create c-list. 
