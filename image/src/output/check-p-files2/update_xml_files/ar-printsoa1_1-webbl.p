...
DEFINE TEMP-TABLE soa-list 
  ...
  /* Naufal Afthar - 96DED5 -> add room category*/
  FIELD rm-category     AS CHAR FORMAT "x(20)"

  FIELD gst-amount-str       AS CHAR FORMAT "->>,>>>,>>>,>>9.99" /* Naufal Afthar - 5EA9CC -> fix -1.0E-10 bug*/
...
/* Naufal Afthar - 5EA9CC -> fix -1.0E-10 bug*/
FOR EACH soa-list:
    soa-list.gst-amount-str = STRING(soa-list.gst-amount, "->>,>>>,>>>,>>9.9999999999999").
END.
...
PROCEDURE create-soalist: 
  ...
  */
  ...
  DO: 
      ...
      artikel.artart = 2 AND artikel.departement = 0 NO-LOCK:
        ...
        DO:              
          ...
          ASSIGN soa-list.to-sort   = 1 
            ...
            /* Naufal Afthar - 17A7F1*/
            FIND FIRST htparam WHERE htparam.paramnr EQ 143 NO-LOCK.
            ...
            DO:
                ...
                DO:
                    ASSIGN soa-list.exrate = waehrung.ankauf.
                    
                    FIND FIRST bill WHERE bill.rechnr EQ debitor.rechnr NO-LOCK NO-ERROR.
                    IF AVAILABLE bill THEN
                    DO:
                        FIND FIRST res-line WHERE res-line.resnr EQ bill.resnr 
                            AND res-line.resstatus EQ 8 NO-LOCK NO-ERROR.
                        IF AVAILABLE res-line THEN
                        DO:
                            FIND FIRST exrate WHERE exrate.artnr EQ waehrung.waehrungsnr
                                AND exrate.datum EQ res-line.abreise NO-LOCK NO-ERROR.
                            IF AVAILABLE exrate THEN ASSIGN soa-list.exrate = exrate.betrag.
                        END.
                    END.
                END.
            END.
    ...
    FIND FIRST soa-list WHERE soa-list.done-step = 0 USE-INDEX rechnr_ix NO-ERROR.
    DO WHILE AVAILABLE soa-list:
      ASSIGN 
        crdt = 0 debt = 0 fcrdt = 0 fdebt = 0
        new-crdt = 0 new-fcrdt = 0 
      ...
      FIND NEXT soa-list WHERE soa-list.done-step = 0 USE-INDEX rechnr_ix NO-ERROR.
    END.
    ...
    FOR EACH soa-list:
      ...
      DO:
        ...
        FIND FIRST bill-line WHERE bill-line.rechnr = debitor.rechnr 
          AND bill-line.artnr = debitor.artnr 
          AND bill-line.departement = 0 NO-LOCK NO-ERROR.
        ...
        DO:
          ...
          DO: 
            ...
            FOR EACH bresline WHERE bresline.resnr = res-line.resnr NO-LOCK:
              ASSIGN 
                count-zimm         = count-zimm + res-line.zimmeranz
            ...
            DO:
              ASSIGN
                soa-list.remarks = soa-list.remarks + CHR(3) + zimkateg.kurzbez + CHR(3) + zimkateg.bezeichnung /*Eko 21 Jan 2015*/
                soa-list.rm-category = zimkateg.bezeichnung. /* Naufal Afthar - 96DED5 -> add room category*/
            END.
            ...
            DO:
              /*bernatd 0ECB35 2024*/
              IF bill.billtyp EQ 2 AND bill.resnr GT 0 AND bill.reslinnr EQ 0 THEN
              DO:
                ASSIGN
      ...
      DO:
        ...
        ELSE
        DO:
          FIND FIRST hoteldpt WHERE hoteldpt.num = debitor.betriebsnr NO-LOCK.
          ...
          /*count gst for penang*/
          FOR EACH hbilline WHERE hbilline.rechnr = h-bill.rechnr 
              AND hbilline.departement EQ h-bill.departement NO-LOCK: /* Naufal Afthar - 91401B*/
  ...
  DO:
    ...
    DO WHILE AVAILABLE soa-list:
      ...
      FOR EACH soabuff WHERE soabuff.rechnr = soa-list.rechnr
        ...
        AND soabuff.artno = soa-list.artno
        AND soabuff.done-step = 0
        AND RECID(soabuff) = RECID(soa-list):
          ASSIGN
            soabuff.done-step = 1
            debt              = soabuff.saldo
            fdebt             = fdebt + soabuff.vesrdep
      ...
      /*ITA 020518*/
      FOR EACH soabuff WHERE soabuff.done-step EQ 1:   
        ASSIGN
          soabuff.done-step   = 2
          soabuff.debt        = debt
          soabuff.credit      = crdt
      ...
      FIND NEXT soa-list WHERE soa-list.done-step EQ 0
    ...
    FOR EACH soa-list:
      ...
      DO:
        ...
        FIND FIRST bill-line WHERE bill-line.rechnr = debitor.rechnr 
          AND bill-line.artnr = debitor.artnr AND bill-line.departement = 0
          NO-LOCK NO-ERROR.
        ...
        DO:
          ...
          DO: 
            ...
            IF AVAILABLE reservation THEN
            DO:
              /* Oscar (10/12/2024) - 5D6360 - handling guest name if from reservation using master bill*/
              IF bill.billtyp EQ 2 AND bill.resnr GT 0 AND bill.reslinnr EQ 0 THEN
              DO:
                ASSIGN
                  soa-list.voucherNo2  = reservation.vesrdepot
  ...
  DO: 
    ...
    FOR EACH soa-list,
      FIRST debitor WHERE INTEGER(RECID(debitor)) = soa-list.arRecid NO-LOCK,
      FIRST guest WHERE guest.gastnr = debitor.gastnr NO-LOCK:
        ASSIGN crdt = 0 debt = 0 fcrdt = 0 fdebt = 0
          new-crdt = 0 new-fcrdt = 0 isprintedb4 = FALSE.
        ...
        DO:
          ...
          FIND FIRST bill-line WHERE bill-line.rechnr = bill.rechnr 
            AND bill-line.artnr = artikel.artnr AND bill-line.departement = 0
            NO-LOCK NO-ERROR.
