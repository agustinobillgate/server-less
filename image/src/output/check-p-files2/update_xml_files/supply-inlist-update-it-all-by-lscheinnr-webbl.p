...
DEF VAR avrg-price      AS DECIMAL   NO-UNDO INIT 0.
...
DEF VAR time-cancel     AS CHAR      NO-UNDO.

time-cancel = STRING(TIME,"HH:MM:SS").

DEF BUFFER buff-l-op FOR l-op.
DEF BUFFER buff-l-artikel FOR l-artikel.
...
FOR EACH l-op WHERE l-op.lief-nr GT 0 
    AND l-op.lscheinnr EQ str-list-lscheinnr
    AND l-op.loeschflag LE 1
    AND l-op.op-art = 1
    NO-LOCK USE-INDEX lief_ix,
    FIRST l-artikel WHERE l-artikel.artnr EQ l-op.artnr:

    FIND FIRST buff-l-op WHERE RECID(buff-l-op) EQ RECID(l-op) NO-LOCK NO-ERROR.
    IF AVAILABLE buff-l-op THEN
    DO:
        FIND CURRENT buff-l-op EXCLUSIVE-LOCK.

        ASSIGN
        direct-issue = buff-l-op.flag
        buff-l-op.loeschflag = 2
        buff-l-op.stornogrund = bediener-username + ": " + STRING(TODAY) 
                          + "-" + time-cancel + ";Reason:" 
                          + cancel-reason
        .

        FIND CURRENT buff-l-op NO-LOCK.
        RELEASE buff-l-op. /* Oscar - fix_lock - fix record lock */
    END.
...
FIND FIRST l-op WHERE l-op.lscheinnr EQ str-list-lscheinnr NO-LOCK NO-ERROR.
...
PROCEDURE update-it: 
  ...
  DO: 
    ...
    DO: 
      ...
      FIND FIRST l-order WHERE l-order.lief-nr EQ l-op.lief-nr 
        ...
        AND l-order.artnr EQ l-op.artnr 
        AND l-order.einzelpreis EQ l-op.einzelpreis 
        AND l-order.geliefert GT l-op.anzahl /* Oscar (26/11/2024) - F0ADB1 - fix cancel receiving */
        EXCLUSIVE-LOCK NO-ERROR. 
    ...
    IF AVAILABLE l-order THEN 
    DO: 
      l-order.geliefert = l-order.geliefert - l-op.anzahl. /* Oscar (26/11/2024) - F0ADB1 - fix cancel receiving */
      ...
      RELEASE l-order1. /* Oscar - fix_lock - fix record lock */
      RELEASE l-order. /* Oscar - fix_lock - fix record lock */
    END. 
  END. 
  ...
  DO:
    ...
    DO:
      ...
      DO:
        ASSIGN reslin-queasy.deci3 = reslin-queasy.deci3 - l-op.anzahl.
        FIND CURRENT reslin-queasy NO-LOCK.
        RELEASE reslin-queasy. /* Oscar - fix_lock - fix record lock */
      END.
    END.
    ELSE
    DO:
      ...
      DO:
        ASSIGN dml-artdep.geliefert = dml-artdep.geliefert - l-op.anzahl.
        FIND CURRENT dml-artdep NO-LOCK.
        RELEASE dml-artdep. /* Oscar - fix_lock - fix record lock */
      END.
      ELSE
      DO:
        ...
        DO:
          ASSIGN dml-art.geliefert = dml-art.geliefert - l-op.anzahl.
          FIND CURRENT dml-art NO-LOCK.
          RELEASE dml-art. /* Oscar - fix_lock - fix record lock */
        END.
      END.
    END.
  END.
  ...
  DO: 
    ...
    DO: 
      ...
      DO: 
        FIND FIRST buff-l-artikel WHERE RECID(buff-l-artikel) EQ RECID(l-artikel) NO-LOCK NO-ERROR.
        IF AVAILABLE buff-l-artikel THEN
        DO:
          FIND CURRENT buff-l-artikel EXCLUSIVE-LOCK. 
          buff-l-artikel.vk-preis = avrg-price. 
          FIND CURRENT buff-l-artikel NO-LOCK. 
          RELEASE buff-l-artikel. /* Oscar - fix_lock - fix record lock */
        END.
      END. 
    END. 
    ...
    DO:
      ASSIGN
        l-bestand.anz-eingang  = l-bestand.anz-eingang - l-op.anzahl /* Oscar (26/11/2024) - F0ADB1 - fix cancel receiving */
        l-bestand.wert-eingang = l-bestand.wert-eingang - l-op.warenwert
      . 
      FIND CURRENT l-bestand NO-LOCK. 
      RELEASE l-bestand. /* Oscar - fix_lock - fix record lock */
    END. 
  ...
  DO: 
    ...
    DO:
      ...
      RELEASE l-op1. /* Oscar - fix_lock - fix record lock */
    END. 
  END. 
  ...
  DO:
    ...
    RELEASE l-liefumsatz. /* Oscar - fix_lock - fix record lock */
  END.
  ...
  */
  ...
  FOR EACH blop WHERE blop.lscheinnr EQ l-op.lscheinnr
      AND blop.lief-nr GT 0 AND blop.loeschflag LE 1 NO-LOCK:
...
PROCEDURE reorg-avrg-price:
  ...
  FOR EACH l-opbuff WHERE l-opbuff.artnr EQ curr-artnr
      ...
      DO:
        ...
        IF AVAILABLE l-bestand THEN
        IF l-opbuff.lager-nr EQ l-bestand.lager-nr THEN
        ASSIGN
