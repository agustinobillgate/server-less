DEFINE TEMP-TABLE lunchdinner-list
    ...
    FIELD lunch-revenue AS DECIMAL
    FIELD dinner-revenue AS DECIMAL
.
...
DEFINE INPUT  PARAMETER incl-accom      AS LOGICAL. 
DEFINE INPUT  PARAMETER incl-guarantee  AS LOGICAL. 
DEFINE INPUT  PARAMETER show-lunchdinner-rate AS LOGICAL.
DEFINE OUTPUT PARAMETER TABLE FOR lunchdinner-list.
...
DEFINE VARIABLE lunch-artnr     AS INTEGER NO-UNDO.
DEFINE VARIABLE dinner-artnr    AS INTEGER NO-UNDO.
DEFINE VARIABLE bfast-dept      AS INTEGER NO-UNDO.
...
FIND FIRST htparam WHERE htparam.paramnr = 227 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN lunch-artnr = htparam.finteger.
FIND FIRST htparam WHERE htparam.paramnr = 228 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN dinner-artnr = htparam.finteger.
FIND FIRST htparam WHERE htparam.paramnr = 126 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN bfast-dept = htparam.finteger.
...
IF fdate GE p-87 THEN   /*FORECAST*/
DO:
    IF v-key EQ 1 THEN /*LUNCH*/
    DO:
        IF NOT incl-guarantee THEN RUN create-lunch.
        ELSE RUN create-lunch-guarantee.    
    END.
    ELSE IF v-key EQ 2 THEN /*DINNER*/
    DO:
        IF NOT incl-guarantee THEN RUN create-dinner.
        ELSE RUN create-dinner-guarantee.
    END.
END.
ELSE    /*STATISTIC*/
DO:
    IF v-key EQ 1 THEN RUN create-statlunch. /*LUNCH*/
    ELSE IF v-key EQ 2 THEN RUN create-statdinner. /*DINNER*/
END.


/************************  PROCEDURE   **************************/ 
PROCEDURE create-lunch: 
...
DEFINE VARIABLE rsv-remark AS CHARACTER NO-UNDO.
    ...
    FOR EACH lunchdinner-list:
        DELETE lunchdinner-list.
    END.
    ...
    FOR EACH res-line WHERE (res-line.resstatus NE 3 AND res-line.resstatus NE 2 
        ...
        IF incl-accom AND res-line.l-zuordnung[3] EQ 1 THEN DO:
            ...
            IF AVAILABLE arrangement THEN DO:
                FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr NO-LOCK, 
                    ...
                    AND artikel.zwkum = lunch-artnr NO-LOCK BY argt-line.betrag DESCENDING :
                    ASSIGN
                        do-it  = YES
                        ROflag = NO
                    ...
                    DO: 
                        IF argt-line.betriebsnr = 0 THEN qty-argt = res-line.erwachs. 
                        ELSE qty-argt = argt-line.betriebsnr. 
                    END. 
                    ELSE IF argt-line.vt-percnt = 1 THEN qty-argt = res-line.kind1. 
                    ELSE IF argt-line.vt-percnt = 2 THEN qty-argt = res-line.kind2.
        ...
        DO:            
            ...
            DO:
                FIND FIRST reslin-queasy WHERE reslin-queasy.key = "argt-line" 
                    ...
                    AND reslin-queasy.number3  = lunch-artnr 
        ...
        DO:
            ...
            FOR EACH fixleist WHERE fixleist.resnr = res-line.resnr 
                ...
                AND fixleist.artnr = lunch-artnr
        ...
        DO: 
            ...
            CREATE lunchdinner-list.
            IF NOT ROflag THEN BUFFER-COPY res-line TO lunchdinner-list.
            ELSE 
            DO:    
                BUFFER-COPY res-line EXCEPT erwachs kind1 gratis TO lunchdinner-list.
                ASSIGN lunchdinner-list.erwachs = 0.
            END.
            ASSIGN         
                lunchdinner-list.segmentcode = reservation.segmentcode
                lunchdinner-list.kurzbez     = zikat-list.kurzbez
                lunchdinner-list.erwachs     = lunchdinner-list.erwachs + qty
                lunchdinner-list.gastnr      = res-line.gastnr
                lunchdinner-list.resname     = reservation.NAME          
                /*lunchdinner-list.comments    = reservation.bemerk*/                  
                lunchdinner-list.zipreis     = res-line.zipreis 
                lunchdinner-list.id          = reservation.useridanlage.                        
            
            /*FDL april 16, 2024 => Ticket #98B8C0*/
            IF AVAILABLE segment THEN lunchdinner-list.bezeich = segment.bezeich.
            ...
            lunchdinner-list.comments = rsv-remark.
            rsv-remark = "".
            ...
            lunchdinner-list.bemerk = resline-remark.
                        
            IF lunchdinner-list.comments NE "" THEN lunchdinner-list.comments = lunchdinner-list.comments + CHR(10).
            lunchdinner-list.comments = lunchdinner-list.comments + /*res-line.bemerk*/ resline-remark.
            resline-remark = "".
            /*END*/

            IF NOT ROflag THEN lunchdinner-list.kind1 = lunchdinner-list.kind1 + res-line.l-zuordnung[4].
            /*
            IF AVAILABLE zimkateg THEN
                lunchdinner-list.kurzbez     = zimkateg.kurzbez.
            */
            IF AVAILABLE ratecode THEN
                lunchdinner-list.CODE        = ratecode.CODE.

            IF AVAILABLE guest THEN
            DO:
                lunchdinner-list.address          = guest.adresse1.
                lunchdinner-list.city             = guest.wohnort + " " + guest.plz. 
                lunchdinner-list.nation1          = guest.nation1.
                lunchdinner-list.mobil-telefon    = guest.mobil-telefon. /*gerald 100221*/
                ...
                DO:
                    lunchdinner-list.mcard-number = mc-guest.cardnum.

                    FIND FIRST mc-type WHERE mc-type.nr EQ mc-guest.nr NO-LOCK NO-ERROR.
                    IF AVAILABLE mc-type THEN lunchdinner-list.mcard-type = mc-type.bezeich.
                END.
            END.
            /*ragung*/
            IF res-line.resstatus EQ 11 AND res-line.l-zuordnung[3] EQ 0 THEN ASSIGN lunchdinner-list.NAME =  lunchdinner-list.NAME + " *".
            IF res-line.resstatus EQ 11 AND res-line.l-zuordnung[3] EQ 1 THEN ASSIGN lunchdinner-list.NAME =  lunchdinner-list.NAME + " **".
            IF res-line.resstatus EQ 13 AND res-line.l-zuordnung[3] EQ 0 THEN ASSIGN lunchdinner-list.NAME =  lunchdinner-list.NAME + " *".
            IF res-line.resstatus EQ 13 AND res-line.l-zuordnung[3] EQ 1 THEN ASSIGN lunchdinner-list.NAME =  lunchdinner-list.NAME + " **".
            /*end*/                

            /*FDL Dec 05, 2024: Ticket 388744*/
            IF show-lunchdinner-rate THEN
            DO:
                FIND FIRST artikel WHERE artikel.artnr EQ lunch-artnr
                ...
                DO:
                    ...
                    DO:
                        ...
                        lunchdinner-list.lunch-revenue = epreis * qty-argt * exchg-rate.
                    END.
                END.
                ELSE lunchdinner-list.lunch-revenue = epreis * qty-argt.
            END.                
            ELSE lunchdinner-list.lunch-revenue = 0.
        END.
    END.
END.

PROCEDURE create-lunch-guarantee: 
DEFINE VARIABLE do-it       AS LOGICAL  NO-UNDO.
DEFINE VARIABLE ROflag      AS LOGICAL  NO-UNDO.
DEFINE VARIABLE epreis      AS DECIMAL  NO-UNDO.
DEFINE VARIABLE qty         AS INTEGER  NO-UNDO.
...
DEFINE VARIABLE rsv-remark AS CHARACTER NO-UNDO.
    ...
    FOR EACH res-line WHERE (res-line.resstatus NE 3 AND res-line.resstatus NE 2 
        ...
        IF incl-accom AND res-line.l-zuordnung[3] EQ 1 THEN DO:
            ...
            IF AVAILABLE arrangement THEN DO:
                FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr NO-LOCK, 
                    FIRST artikel WHERE artikel.artnr = argt-line.argt-artnr 
                    AND artikel.departement = bfast-dept 
                    AND artikel.zwkum = dinner-artnr NO-LOCK BY argt-line.betrag DESCENDING :
                    ASSIGN
                        do-it  = YES
                        ROflag = NO
                        epreis = argt-line.betrag.  
        ...
        DO:            
            ...
            DO:
                FIND FIRST reslin-queasy WHERE reslin-queasy.key = "argt-line" 
                    ...
                    AND reslin-queasy.number1  = res-line.reserve-int
                    AND reslin-queasy.number2  = arrangement.argtnr
                    AND reslin-queasy.number3  = dinner-artnr 
                    AND reslin-queasy.resnr    = bfast-dept 
                    AND reslin-queasy.reslinnr = res-line.zikatnr
                    AND reslin-queasy.date1 LE fdate
                    AND reslin-queasy.date2 GE fdate 
        ...
        DO:
            ...
            FOR EACH fixleist WHERE fixleist.resnr = res-line.resnr 
                AND fixleist.reslinnr = res-line.reslinnr 
                AND fixleist.artnr = dinner-artnr
                AND fixleist.departement = bfast-dept NO-LOCK:
        ...
        DO: 
            ...
            */
            ...
            IF show-lunchdinner-rate THEN 
            DO:
                FIND FIRST artikel WHERE artikel.artnr EQ dinner-artnr
                    AND artikel.departement EQ bfast-dept                    
                    AND artikel.betriebsnr NE 0 
                    AND artikel.pricetab NO-LOCK NO-ERROR.
                IF AVAILABLE artikel THEN
                DO:
                    ...
                    DO:
                        exchg-rate = waehrung.ankauf / waehrung.einheit.
                        lunchdinner-list.dinner-revenue = epreis * qty-argt * exchg-rate.
                    END.
                END.
                ELSE lunchdinner-list.dinner-revenue = epreis * qty-argt.
            END.
            ELSE lunchdinner-list.dinner-revenue = 0.
        END.
    END.
END.

PROCEDURE create-dinner-guarantee: 
DEFINE VARIABLE do-it       AS LOGICAL  NO-UNDO.
DEFINE VARIABLE ROflag      AS LOGICAL  NO-UNDO.
DEFINE VARIABLE epreis      AS DECIMAL  NO-UNDO.
DEFINE VARIABLE qty         AS INTEGER  NO-UNDO.
...
DEFINE BUFFER rline FOR res-line.
    ...
    FOR EACH genstat WHERE (genstat.resstatus NE 3 
        ...
        DO:
            ...
            CREATE lunchdinner-list.
            IF NOT ROflag THEN BUFFER-COPY genstat TO lunchdinner-list.
            ELSE 
            DO:    
                BUFFER-COPY genstat EXCEPT erwachs kind1 gratis datum TO lunchdinner-list.
                ASSIGN lunchdinner-list.erwachs = 0.
            END.
            ASSIGN
                lunchdinner-list.datum        = genstat.datum
                lunchdinner-list.segmentcode  = reservation.segmentcode 
                lunchdinner-list.kurzbez      = zikat-list.kurzbez
                lunchdinner-list.erwachs      = lunchdinner-list.erwachs + qty
                lunchdinner-list.gastnr       = genstat.gastnr
                lunchdinner-list.resname      = reservation.NAME
                lunchdinner-list.comments     = reservation.bemerk
                lunchdinner-list.bezeich      = segment.bezeich
                lunchdinner-list.zipreis      = genstat.zipreis
                lunchdinner-list.ankunft      = genstat.res-date[1]
                lunchdinner-list.abreise      = genstat.res-date[2]
                lunchdinner-list.arrangement  = genstat.argt   
                lunchdinner-list.id           = reservation.useridanlage
            . 
            ...
            IF AVAILABLE res-line THEN lunchdinner-list.bemerk = lunchdinner-list.comments + res-line.bemerk.*/
            FIND FIRST rline WHERE rline.resnr = genstat.resnr
                AND rline.reslinnr = genstat.res-int[1] NO-LOCK NO-ERROR.
            IF AVAILABLE rline THEN lunchdinner-list.bemerk = lunchdinner-list.comments + rline.bemerk.
            
            IF lunchdinner-list.comments NE "" THEN lunchdinner-list.comments = lunchdinner-list.comments + CHR(10).
            
            IF NOT ROflag THEN 
                lunchdinner-list.kind1 = lunchdinner-list.kind1 + genstat.kind3. /*FDL Feb 15, 2024 => Ticket CB58AC - Change kind1 ro kind3*/
            ...
            */
            ...
            DO:
                lunchdinner-list.address          = guest.adresse1.
                lunchdinner-list.city             = guest.wohnort + " " + guest.plz.
                lunchdinner-list.NAME             = guest.NAME. 
                lunchdinner-list.comments         = lunchdinner-list.comments + guest.bemerk.
                lunchdinner-list.nation1          = guest.nation1.
                lunchdinner-list.mobil-telefon    = guest.mobil-telefon. /*gerald 100221*/
