/*FDL March 05, 2024 => Ticket EAAD95 - Change LENGTH(reservation.groupname) To LENGTH(TRIM(reservation.groupname))*/
/*MCH Nov 08, 2024 => Ticket F126EA - Add Guest Mobile No for TADA Loyalty*/
...
DEFINE BUFFER gbuffmember FOR guest.  
...
DEFINE TEMP-TABLE arl-list  
    ...
    FIELD guest-blacklist   AS LOGICAL
    FIELD fixcost-flag      AS LOGICAL
    FIELD gastmobileno      AS CHAR
    FIELD special-request   AS CHAR
	.
...
DEFINE TEMP-TABLE t-payload-list
    FIELD argt-str          AS CHAR. /* Naufal Afthar - BEB554*/
...
DEFINE INPUT  PARAMETER voucher-no   AS CHARACTER.
DEFINE INPUT  PARAMETER nation-str   AS CHARACTER.
...
DEFINE VARIABLE vipnr10 AS INTEGER INITIAL 999999999. /* Naufal Afthar - 6B04C3 -> add missing genparam*/
...
DEF VAR tmpInt            AS INTEGER NO-UNDO.
...
DEFINE VARIABLE rsvbemerk  AS CHAR    NO-UNDO.  /* Oscar (24/12/2024) - 040E52 - handling special character bemerk */
DEFINE VARIABLE blacklist-int AS INT NO-UNDO.
...
RUN htpint.p (709, OUTPUT blacklist-int). 
...
/* FDL Comment => Follow existing procedure it's right #EAAD95
/* SY 01 JUL 2017 */
...
FOR EACH b-arl-list NO-LOCK:
    ...
    IF LENGTH(rescomment) LT 3 THEN rescomment = "".
    IF LENGTH(rescomment) EQ ? THEN rescomment = "".

    arl-list.comments = TRIM(rescomment).
    rescomment = "".

    /* Oscar (24/12/2024) - 040E52 - handling special character bemerk */
    rsvbemerk  = b-arl-list.rsv-bemerk.
    rsvbemerk  = REPLACE(rsvbemerk,CHR(10),"").
    rsvbemerk  = REPLACE(rsvbemerk,CHR(13),"").
    rsvbemerk  = REPLACE(rsvbemerk,"~n","").
    rsvbemerk  = REPLACE(rsvbemerk,"\n","").
    rsvbemerk  = REPLACE(rsvbemerk,"~r","").
    rsvbemerk  = REPLACE(rsvbemerk,"~r~n","").
    rsvbemerk  = REPLACE(rsvbemerk,CHR(10) + CHR(13),"").

    IF LENGTH(rsvbemerk) LT 3 THEN rsvbemerk = REPLACE(rsvbemerk,CHR(32),"").
    IF LENGTH(rsvbemerk) LT 3 THEN rsvbemerk = "".
    IF LENGTH(rsvbemerk) EQ ? THEN rsvbemerk = "".

    arl-list.rsv-bemerk = TRIM(rsvbemerk).
    rsvbemerk = "".
END.
/*End FDL*/
...
DEFINE VARIABLE iGrpname    AS INTEGER INITIAL 0    NO-UNDO.  
  ...
  DO:   
    ...
    DO:   
      ...
      DO:   
        ...
        FOR EACH res-line WHERE   
            ...
            NO-LOCK,   
            FIRST reservation WHERE reservation.resnr = res-line.resnr  
              AND LENGTH(TRIM(reservation.groupname)) GE iGrpname NO-LOCK,  
            FIRST zimkateg WHERE zimkateg.zikatnr = res-line.zikatnr NO-LOCK,   
            FIRST setup-list WHERE setup-list.nr = res-line.setup + 1   
            NO-LOCK BY res-line.zinr  
            BY (res-line.kontakt-nr * res-line.resnr) BY res-line.resstatus BY res-line.NAME:  
    ...
    ELSE IF last-sort = 6 THEN RUN sqry6.
    ELSE IF last-sort = 7 THEN RUN sqry7. /* Oscar (17/12/2024) - 040E52 - add new search reservation by nationality */
    ELSE IF last-sort = 8 THEN RUN sqry8. /* Naufal Afthar - BEB554 -> search by arrangement*/
  END.   
  ELSE IF sorttype = 3 THEN   /* Arrival TODAY */   
  DO:   
    IF last-sort = 1 THEN   
  ...
  DO:   
    ...
    DO:   
      ...
      FOR EACH res-line WHERE   
          ...
          NO-LOCK,   
          FIRST reservation WHERE reservation.resnr = res-line.resnr 
          AND LENGTH(TRIM(reservation.groupname)) GE iGrpname
    ...
    DO:   
      ...
      FOR EACH res-line WHERE active-flag = 0   
          ...
          /*AND res-line.ankunft LE (ci-date + 30)  FT serverless */
          AND res-line.ankunft LE tmpDate
          AND res-line.resstatus NE 9 AND res-line.resstatus NE 10 
          AND res-line.resstatus NE 99
          NO-LOCK,   
          FIRST gbuff WHERE gbuff.gastnr = res-line.gastnr NO-LOCK,  
    ...
    DO:
      ...
      DO:
        FOR EACH res-line WHERE active-flag = 1   
            AND res-line.resstatus NE 12
            AND res-line.resstatus NE 99 
            /*AND res-line.ankunft GE fdate AND res-line.abreise GE ci-date  
            AND res-line.zinr GE room*/
            AND res-line.ankunft LE ci-date AND res-line.abreise GE ci-date
            AND SUBSTR(res-line.zinr,1,INTEGER(rmlen)) EQ room NO-LOCK,   /*FDL May 05,2024 => Ticket 501A25*/
            FIRST gbuff WHERE gbuff.gastnr = res-line.gastnr NO-LOCK,  
            FIRST waehrung WHERE waehrung.waehrungsnr = res-line.betriebsnr   
            NO-LOCK,   
            FIRST reservation WHERE reservation.resnr = res-line.resnr  
...
PROCEDURE sqry6:   
    ...
    IF sorttype = 1 THEN   
    DO:         
        IF voucher-no NE "" AND voucher-no NE ? THEN
        DO:
            FOR EACH res-line WHERE active-flag EQ 0                 
                AND res-line.resstatus NE 9 
                AND res-line.resstatus NE 10  
                AND res-line.resstatus NE 99 
                AND res-line.zimmer-wunsch MATCHES ("*Voucher*") NO-LOCK,   
                FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK,  
                FIRST waehrung WHERE waehrung.waehrungsnr EQ res-line.betriebsnr NO-LOCK,   
                ...
                FIRST setup-list WHERE setup-list.nr EQ res-line.setup + 1 NO-LOCK  
                BY res-line.name:  
                
                IF reservation.vesrdepot EQ voucher-no THEN RUN create-it.
                ELSE
                DO:
                    DO i = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch,";") - 1:
                        str = ENTRY(i, res-line.zimmer-wunsch, ";").
                        IF SUBSTR(str,1,7) = "voucher" 
                            AND SUBSTR(str,8) EQ voucher-no THEN 
                        DO:
                            i = 999.
                            RUN create-it.
                        END.
    ...
    DO:          
        IF voucher-no NE "" AND voucher-no NE ? THEN
        DO:
            FOR EACH res-line WHERE active-flag EQ 1   
                AND res-line.resstatus NE 12
                AND res-line.resstatus NE 99 NO-LOCK,   
                FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK,  
                FIRST waehrung WHERE waehrung.waehrungsnr EQ res-line.betriebsnr NO-LOCK,   
                ...
                FIRST zimkateg WHERE zimkateg.zikatnr EQ res-line.zikatnr NO-LOCK,   
                FIRST setup-list WHERE setup-list.nr EQ res-line.setup + 1 NO-LOCK  
                BY res-line.zinr BY (res-line.kontakt-nr * res-line.resnr) BY res-line.resstatus BY res-line.NAME:  
...
/* Oscar (17/12/2024) - 040E52 - add new search reservation by nationality */
PROCEDURE sqry7:   
  DEFINE VARIABLE iGrpname AS INTEGER INITIAL 0    NO-UNDO. 
  DEFINE VARIABLE i     AS INTEGER INITIAL 0 NO-UNDO. 
  DEFINE VARIABLE str   AS CHARACTER NO-UNDO.  
  ...
  DO:
    ...
    DO:
      FOR EACH res-line WHERE active-flag EQ 0                 
        ...
        BY res-line.name:  
          /* Oscar (17/12/2024) - 040E52 - add new search reservation by nationality */
          FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember 
            AND guest.nation1 EQ nation-str NO-LOCK NO-ERROR.
  ...
  DO:          
    ...
    DO:
      FOR EACH res-line WHERE active-flag EQ 1   
        ...
        AND res-line.resstatus NE 99
        AND (SUBSTR(res-line.zinr,1,INTEGER(rmlen))) GE (room)   
        AND res-line.ankunft LE ci-date 
  ...
  DO:
    ...
    DO:
      FOR EACH res-line WHERE active-flag EQ 1 
        ...
        AND res-line.resstatus NE 99
        AND res-line.abreise = fdate   
        AND (SUBSTR(res-line.zinr,1,INTEGER(rmlen))) GE (room) NO-LOCK,   
        FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK,  
        FIRST waehrung WHERE waehrung.waehrungsnr EQ res-line.betriebsnr NO-LOCK,   
        FIRST reservation WHERE reservation.resnr EQ res-line.resnr  
        AND LENGTH(TRIM(reservation.groupname)) GE iGrpname NO-LOCK,   
...
/* Naufal Afthar - BEB554 -> search by arrangement*/
PROCEDURE sqry8:
    DEFINE VARIABLE iGrpname AS INTEGER INITIAL 0 NO-UNDO.
    ...
    DO:
        IF t-payload-list.argt-str EQ " " OR t-payload-list.argt-str EQ ? THEN
        DO:
        ...
        DO:
            FOR EACH res-line WHERE res-line.active-flag EQ 0
                ...
                AND res-line.resstatus NE 10
                AND res-line.resstatus NE 99 
                AND res-line.arrangement EQ t-payload-list.argt-str NO-LOCK,
                FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK,
                FIRST waehrung WHERE waehrung.waehrungsnr EQ res-line.betriebsnr NO-LOCK,
                FIRST reservation WHERE reservation.resnr EQ res-line.resnr 
                    AND LENGTH(TRIM(reservation.groupname)) GE iGrpname NO-LOCK,
    ...
    DO:
        ...
        DO:
            FOR EACH res-line WHERE res-line.active-flag EQ 0
                AND res-line.ankunft EQ ci-date
                AND (SUBSTR(res-line.zinr,1,INTEGER(rmlen))) GE (room)
                AND res-line.resstatus NE 9 AND res-line.resstatus NE 10
                AND res-line.resstatus NE 99 NO-LOCK,
                FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK,
    ...
    DO:
        ...
        DO:
            FOR EACH res-line WHERE res-line.active-flag EQ 1
                AND res-line.resstatus NE 12 AND res-line.resstatus NE 99
                AND res-line.resstatus NE 99 AND res-line.abreise EQ fdate
                AND (SUBSTR(res-line.zinr,1,INTEGER(rmlen))) GE (room) NO-LOCK,
                FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK,
                FIRST waehrung WHERE waehrung.waehrungsnr EQ res-line.betriebsnr NO-LOCK,
                FIRST reservation WHERE reservation.resnr EQ res-line.resnr 
...
PROCEDURE create-it:  
    ...
    arl-list.bestat-dat        = reservation.bestat-datum  /*FT serverless reservation.bestat-dat*/
    ...
    arl-list.gastmobileno      = gmember.mobil-telefon
	.
    ...
    /*FDL May 24, 2024 => Ticket 711780*/    
    FIND FIRST guestseg WHERE guestseg.gastnr EQ res-line.gastnrmember
        AND guestseg.segmentcode EQ blacklist-int NO-LOCK NO-ERROR.
    IF AVAILABLE guestseg THEN arl-list.guest-blacklist = YES.
    ...
    /* Naufal Afthar - 6B04C3 -> add missing genparam*/
    FIND FIRST htparam WHERE htparam.paramnr = 712 NO-LOCK NO-ERROR.
    IF AVAILABLE htparam AND htparam.finteger NE 0 THEN vipnr10 = htparam.finteger.
...
*/
    ...
    IF AVAILABLE res-line AND res-line.active-flag = 1 AND long-stay GT 0   
        /*AND (res-line.abreise - res-line.ankunft) GE long-stay   FT serverless*/ 
        AND tmpInt GE long-stay
...
*/
   ...
   /*Bernatd*/
    FIND FIRST fixleist WHERE fixleist.resnr EQ res-line.resnr AND fixleist.reslinnr EQ res-line.reslinnr NO-LOCK NO-ERROR.
    IF AVAILABLE fixleist THEN
    DO:
        arl-list.fixcost-flag = YES.  
    END. 

    /*FDL 26 Dec, 2024: 034BBD*/
    FIND FIRST reslin-queasy WHERE reslin-queasy.key EQ "specialRequest"
        AND reslin-queasy.resnr EQ res-line.resnr
        AND reslin-queasy.reslinnr EQ res-line.reslinnr NO-LOCK NO-ERROR.
    IF AVAILABLE reslin-queasy THEN arl-list.special-request = reslin-queasy.char3.
END.  
