...
DEFINE TEMP-TABLE gl-jouhdr-list
    ...
    FIELD jtype      AS CHAR. /*Ragung EA0818*/
...
DEF INPUT PARAMETER jnr AS INTEGER. /* Oscar (24/02/25) - E00833 - add new input parameter */
...
DEF VAR jtype AS CHAR EXTENT 6 NO-UNDO.
jtype[1]="From F/O".
jtype[2]="From A/R".
jtype[3]="From inventory".
jtype[4]="From A/P".
jtype[5]="From General Cashier".
jtype[6]="No Transfer".
...
PROCEDURE display-it: 
  ...
  DO: 
    ...
    FOR EACH gl-jouhdr 
       ...
       /*USE-INDEX datype_ix*/ /*FDL Comment Oct 02, 2023 => Ticket 8FF695*/
    ...
    /* Oscar (24/02/25) - E00833 - add jnr to find the gl-journal */
    IF jnr EQ 0 THEN
    DO:
    ...
    ELSE
    DO:
        FIND FIRST gl-jouhdr-list WHERE gl-jouhdr-list.jnr EQ jnr NO-ERROR.
        IF AVAILABLE gl-jouhdr-list THEN 
        RUN disp-it2.
    END.
...
PROCEDURE disp-it2:
  ...
  FOR EACH gl-journal WHERE gl-journal.jnr = gl-jouhdr-list.jnr 
    ...
    /*NA 271020 - add validation to check if the entry available*/
    IF NUM-ENTRIES(gl-acct.userinit, ";") GT 1 THEN
    DO:
        b2-list.map-acct  = ENTRY(2, gl-acct.userinit, ";"). /*NA 191020 - tambah field untuk mapping acct*/
    END.
    ELSE
    DO:
        b2-list.map-acct  = " ". /*NA 271020 - add an option if entry 2 n/a*/
    END.
  END.
END. 
...
PROCEDURE assign-it:
    ...
    IF gl-jouhdr.jtype = 0 THEN ASSIGN gl-jouhdr-list.jtype = jtype[6].
    ELSE gl-jouhdr-list.jtype = jtype[gl-jouhdr.jtype].
END.
