...
DEF INPUT PARAMETER from-date           AS DATE. /*bernatd 4ACE13*/
DEF INPUT PARAMETER to-date           AS DATE. /*bernatd 4ACE13*/
...
FOR EACH age-list:
    FOR EACH t-pay WHERE t-pay.artnr EQ temp-art2
        ...
        AND (t-pay.rgdatum LT from-date OR t-pay.rgdatum GT to-date)
...
PROCEDURE create-age-list: 
  ...
  DO: 
    ...
    DO:
      FOR EACH debitor WHERE debitor.artnr = /*artikel.artnr */ temp-art2
        AND (debitor.name /*GE*/ MATCHES("*" + bill-name + "*") AND debitor.name LE to-name)
        AND debitor.rgdatum GE from-date 
