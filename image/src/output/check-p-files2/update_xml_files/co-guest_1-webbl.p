...
DEFINE TEMP-TABLE cl-list 
  ...
  /* Naufal Afthar - 739AAE*/
  FIELD bill-flag        AS INTEGER 
  FIELD bill-type        AS CHAR
  FIELD guest-bl-recid   AS INTEGER
  FIELD master-bl-recid  AS INTEGER /* Naufal Afthar - 84481E*/
  FIELD fg-col           AS LOGICAL
  /* Naufal Afthar - A5A54B*/
  FIELD guest-nr   LIKE res-line.gastnrmember
  FIELD guest-typ  LIKE guest.karteityp
  FIELD gastnr     LIKE res-line.gastnr
  /* Naufal Afthar - 56D1E4*/
  FIELD rm-type    LIKE zimkateg.kurzbez
.
...
PROCEDURE disp-billbalance:
  ...
  /* Naufal Afthar - 739AAE*/
  DEFINE VARIABLE guest-bl-recid   AS INTEGER INITIAL 0. 
  DEFINE VARIABLE master-bl-recid  AS INTEGER INITIAL 0.
  DEFINE VARIABLE bill-flag  AS INTEGER.
  DEFINE VARIABLE bill-type  AS CHAR.
  DEFINE VARIABLE fg-col     AS LOGICAL INITIAL NO.
  ...
  FOR EACH res-line WHERE active-flag = 2 AND res-line.abreise GE fr-date 
    ...
    IF do-it = YES THEN DO:
        ...
        FIND FIRST bill WHERE bill.resnr = res-line.resnr 
          ...
          AND bill.reslinnr NE 0 NO-LOCK NO-ERROR.
        ...
        DO:
            ASSIGN
                ...
                b-bal  = bill.saldo
                /* Naufal Afthar - 739AAE*/
                guest-bl-recid = RECID(bill)
                bill-flag = bill.flag
                bill-type = "G". 

            FIND FIRST mbill WHERE mbill.resnr EQ bill.resnr /* check if a guest bill is also a master bill*/
            
            AND mbill.reslinnr EQ 0 NO-LOCK NO-ERROR.
            IF AVAILABLE mbill THEN 
            DO:
                fg-col = YES.
                master-bl-recid = RECID(mbill).
            END.
        END.
...
/*                 /* Naufal Afthar - 739AAE*/                            */
/*                 bl-recid = RECID(billhis)                              */
/*                 bill-flag = bill.flag                                  */
/*                 bill-type = "G".                                       */
/*           END.                                                         */
/*         END.                                                           */

        /* Naufal Afthar - 739AAE -> check if master bill or NS*/
        /*IF billno EQ 0 THEN */
/*         DO:                                                                */
/*             FIND FIRST bill WHERE bill.resnr EQ res-line.resnr             */
...
/*             ELSE                                                           */
/*             DO:                                                            */
/*                 FIND FIRST bill WHERE bill.gastnr EQ res-line.gastnrmember */
/*                     AND bill.resnr EQ 0                                    */
/*                     AND bill.reslinnr EQ 1 NO-LOCK NO-ERROR.               */
/*                 IF AVAILABLE bill THEN                                     */
/*                 DO:                                                        */
/*                     ASSIGN                                                 */
/*                         billno = bill.rechnr                               */
...
/*         END.                                                               */
        ...
        ASSIGN 
          ...
          /* Naufal Afthar - 739AAE*/
          cl-list.bill-flag         = bill-flag
          cl-list.bill-type         = bill-type
          cl-list.guest-bl-recid    = guest-bl-recid
          cl-list.master-bl-recid   = master-bl-recid
          cl-list.fg-col            = fg-col
          /* Naufal Afthar - A5A54B*/
          cl-list.guest-nr          = res-line.gastnrmember
          cl-list.gastnr            = res-line.gastnr.

        /* Naufal Afthar - A5A54B*/
        FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember NO-LOCK NO-ERROR.
        IF AVAILABLE guest THEN ASSIGN cl-list.guest-typ = guest.karteityp.

        /* Naufal Afthar - 56D1E4*/
        FIND FIRST zimkateg WHERE zimkateg.zikatnr EQ res-line.zikatnr NO-LOCK NO-ERROR.
        IF AVAILABLE zimkateg THEN ASSIGN cl-list.rm-type = zimkateg.kurzbez.
  ...
  FOR EACH res-line WHERE active-flag = 2 AND res-line.ankunft GE fr-date
    ...
    IF do-it = YES THEN DO:
        ...
        IF AVAILABLE bill THEN DO:
             ...
             FIND FIRST mbill WHERE mbill.resnr EQ bill.resnr
                AND mbill.reslinnr EQ 0 NO-LOCK NO-ERROR.
             IF AVAILABLE mbill THEN 
             DO: 
                 fg-col = YES.
...
/*         END.                                                           */
        ...
        /* Naufal Afthar - 739AAE -> check if master bill
        IF billno EQ 0 THEN 
        DO:
            FIND FIRST bill WHERE bill.resnr EQ res-line.resnr
                AND bill.reslinnr EQ 0 AND bill.flag EQ 1 NO-LOCK NO-ERROR.
            IF AVAILABLE bill THEN
            DO:
                ASSIGN
                    billno = bill.rechnr
