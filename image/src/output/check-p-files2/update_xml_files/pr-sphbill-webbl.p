...
DO:
    ...
    ASSIGN
        ...
        .
     t-print-head.table-usr-id  = translateExtended("Table",lvCAREA,"") + " " + table-no + "/" + CAPS(usr-id) + split-no.
END.

FIND FIRST print-list WHERE print-list.str-pos EQ 5 NO-LOCK NO-ERROR.
IF AVAILABLE print-list THEN
DO:
    ASSIGN
        guest-name = TRIM(ENTRY(2,print-list.str,"|")).
        GS = guest-name
        .
    IF NUM-ENTRIES(print-list.str, "|") GT 2 AND GS NE "" THEN
    DO:
        guest-name = guest-name + " | " + TRIM(ENTRY(3,print-list.str,"|")).
    END.
END.
