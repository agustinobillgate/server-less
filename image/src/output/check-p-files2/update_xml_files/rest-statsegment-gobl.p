...
DEFINE VARIABLE qh AS HANDLE.
    ...
    */
    ...
    FIND FIRST h-journal WHERE h-journal.bill-datum GE start-year 
        AND h-journal.bill-datum LE to-date USE-INDEX segment_ix NO-LOCK NO-ERROR.
    DO WHILE AVAILABLE h-journal:
                   
        FIND FIRST h-bill WHERE h-bill.rechnr EQ h-journal.rechnr AND h-bill.bilname NE "" 
            AND h-bill.departement EQ h-journal.departement NO-LOCK NO-ERROR.
            ...
            DO:
                ...
                DO:
                    FIND FIRST artikel WHERE artikel.artnr EQ h-artikel.artnrfront
                        ...
                        AND (artikel.endkum EQ f-endkum OR artikel.endkum EQ m-endkum 
                        OR artikel.umsatzart EQ 3 OR artikel.umsatzart EQ 5) NO-LOCK NO-ERROR.
                END.
                ELSE IF sorttype EQ 2 THEN /*Beverage*/
                DO:
                    FIND FIRST artikel WHERE artikel.artnr EQ h-artikel.artnrfront
                        AND artikel.departement EQ h-artikel.departement
                        AND (artikel.endkum EQ b-endkum OR artikel.umsatzart EQ 6) NO-LOCK NO-ERROR.
                END.
                ...
                DO:
                    FIND FIRST artikel WHERE artikel.artnr EQ h-artikel.artnrfront
                        AND artikel.departement EQ h-artikel.departement
                        AND (artikel.endkum EQ f-endkum OR artikel.endkum EQ m-endkum 
                        OR artikel.umsatzart EQ 3 OR artikel.umsatzart EQ 5) /*Food*/
                        OR (artikel.endkum EQ b-endkum OR artikel.umsatzart EQ 6) /*Beverage*/
                        OR (artikel.umsatzart EQ 4) /*Other*/ 
                        NO-LOCK NO-ERROR.
                END.

                IF AVAILABLE artikel THEN
                DO:                
                    FIND FIRST guest WHERE TRIM(guest.NAME + "," + guest.vorname1) EQ TRIM(h-bill.bilname) NO-LOCK NO-ERROR.
                    ...
                    DO:
                        ...
                        DO:
                            FIND FIRST t-list WHERE t-list.segm-no EQ guestseg.segmentcode NO-LOCK NO-ERROR.
                            IF NOT AVAILABLE t-list THEN
                            DO:
                                CREATE t-list.
                                t-list.segm-no = guestseg.segmentcode.
