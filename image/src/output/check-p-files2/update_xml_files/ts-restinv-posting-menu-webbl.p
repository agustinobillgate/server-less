...
DEF TEMP-TABLE t-h-bill-tmp LIKE h-bill
    FIELD rec-id AS INT.
...
DEFINE VARIABLE hbill-recid     AS INTEGER       INITIAL 0.
DEFINE VARIABLE long-time       AS INTEGER.
DEFINE VARIABLE first-flag      AS LOGICAL INITIAL NO.
DEFINE VARIABLE menulist-nr     AS INTEGER.
DEFINE VARIABLE menulist-artno  AS INTEGER.
DEFINE VARIABLE menulist-bez    AS CHARACTER.
...
IF NOT AVAILABLE t-h-bill THEN hbill-recid = 0.
ELSE hbill-recid = t-h-bill.rec-id.

/*Debug Double Bill*/
MESSAGE 
    "First Debug:"                  SKIP
    "Hbill-Recid="  hbill-recid     SKIP
    "Table-No="     tischnr         SKIP
    "Curr-Dept="    curr-dept       SKIP
    "Time="    STRING(TIME, "HH:MM:SS") SKIP
    "END 1ST"
    VIEW-AS ALERT-BOX INFO BUTTONS OK.

IF hbill-recid EQ 0 THEN
DO:
    /*FDL May 04,2025: 967A33*/
    DO TRANSACTION:
        FIND FIRST queasy WHERE queasy.key EQ 351 
            AND queasy.number1 EQ curr-dept
            AND queasy.number2 EQ tischnr EXCLUSIVE-LOCK NO-ERROR.
    ...
    DO:
        ...
        DO:
            /* Dzikri DD052F - same feature as merge validation for UI
            fl-code = 9999.
            RETURN.
            */
            /*Debug*/
            MESSAGE "IN Condition Occupied Table, Return."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
            ASSIGN 
                fl-code2 = 2
                avail-bill = YES
...
IF first-flag THEN
DO:
    /* Malik Serverless 392 */
    FOR EACH t-h-bill:
        CREATE t-h-bill-tmp.
        BUFFER-COPY t-h-bill TO t-h-bill-tmp.
    END.
    ...
    FOR EACH menu-list WHERE menu-list.nr GT 0 BY menu-list.nr: 
        ...
        /* ASSIGN bmenu.rec-menu = RECID(menu-list). Malik Serverless 392 comment */
    END.
    ...
    FOR EACH menu-list WHERE menu-list.nr GT 0 NO-LOCK BY menu-list.nr:
        ASSIGN
            menulist-nr     = menu-list.nr
            menulist-artno  = menu-list.artnr  
            menulist-bez    = menu-list.bezeich
            .

        curr-zeit = TIME.
        ASSIGN
            add-zeit    = add-zeit + 1
            /* menurecid   = RECID(menu-list) Malik Serverless 392 comment unused code */ 
        ...
        IF fl-code1 = 1 THEN DO:
            ...
            /*FDL Jan 22, 2025: 6D9460*/
            IF NOT zugriff AND amount LT 0 AND t-h-artikel.artart EQ 0 THEN     
            DO:
                ...
                DO:                
                    IF perm[79] LT 2 THEN zugriff = NO.
                    ELSE zugriff = YES.
                END.            
            END.
        ...
        END.
        add-zeit = 0.
        /* menurecid = 0. Malik Serverless 392 comment unused code */    
    END.
    
    /* Dzikri DD052F - Double post */
    FOR EACH t-h-bill:
        DELETE t-h-bill.
    END.
    
    FOR EACH t-h-bill-tmp NO-LOCK:
        /*Debug Double Bill*/
        MESSAGE 
            "Second Debug(After Post DB):"      SKIP
            "Bill-No="  t-h-bill-tmp.rechnr     SKIP
            "Hbill-Recid="  t-h-bill-tmp.rec-id SKIP  
            "END 2ND"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
    
        CREATE t-h-bill.
        BUFFER-COPY t-h-bill-tmp TO t-h-bill.
    END.
    /* Dzikri DD052F - END */

    /*FDL May 04,2025: 967A33*/
    FOR EACH queasy WHERE queasy.key EQ 351 
        AND queasy.number1 EQ curr-dept
        AND queasy.number2 EQ tischnr EXCLUSIVE-LOCK:
    ...
    /*Debug*/
    MESSAGE "Timming Duration API:" rechnr " " STRING(TIME - long-time, "HH:MM:SS")
        VIEW-AS ALERT-BOX INFO BUTTONS OK.
END.
...
PROCEDURE update-bill: 
    ...
    DEFINE VARIABLE tmp-recid   AS INT.
    ...
    DO:
        ...
        /* Malik Serverless 635 update query for t-h-bill */  
        /*    
        FIND FIRST t-h-bill NO-LOCK NO-ERROR.
        IF NOT AVAILABLE t-h-bill THEN rec-id = 0.
        ELSE rec-id = t-h-bill.rec-id.*/
        FIND FIRST t-h-bill-tmp NO-LOCK NO-ERROR.
        IF NOT AVAILABLE t-h-bill-tmp THEN rec-id = 0.
        ELSE rec-id = t-h-bill-tmp.rec-id.
                   
        /*Debug*/
        MESSAGE 
        "BEFORE ts-restinv-update-bill-cldbl:"  SKIP
        "menu-list-nr=" menulist-nr     "|"
        "t-h-bill-tmp-recid=" rec-id    "|"  
        "ArtNo=" menulist-artno         "|"
        "Desc="  menulist-bez                   SKIP
        "END BEFORE"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        ...
        RUN ts-restinv-update-bill-cldbl.p (pvILanguage, rec-id, rec-id-artikel, deptname, transdate,
            ...
            OUTPUT TABLE t-h-bill-tmp, OUTPUT TABLE kellner1).
        
        FIND FIRST t-h-bill-tmp NO-LOCK NO-ERROR.
        IF AVAILABLE t-h-bill-tmp THEN tmp-recid = t-h-bill-tmp.rec-id.
        ELSE tmp-recid = 0.

        /*Debug*/
        MESSAGE 
        "AFTER ts-restinv-update-bill-cldbl:"   SKIP
        "menu-list-nr=" menulist-nr     "|"
        "t-h-bill-tmp-recid=" tmp-recid "|"
        "BillNo=" rechnr                "|"
        "ArtNo=" menulist-artno         "|"
        "Desc="  menulist-bez           "|"
        "fl-code2=" fl-code2                    SKIP
        "END AFTER"
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        ...
        END.        
        /*FDL: 39BF03*/
        IF rec-id EQ 0 AND NOT closed THEN
        DO:
            FIND FIRST t-h-bill-tmp NO-LOCK NO-ERROR.
            IF AVAILABLE t-h-bill-tmp THEN
            DO:
                FIND FIRST queasy WHERE queasy.KEY EQ 31 
                    AND queasy.number1 EQ curr-dept
                    AND queasy.number2 EQ tischnr EXCLUSIVE-LOCK NO-ERROR.
                IF AVAILABLE queasy THEN
                DO:
