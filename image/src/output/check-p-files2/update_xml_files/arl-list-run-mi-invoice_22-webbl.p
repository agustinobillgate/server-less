...
DEFINE TEMP-TABLE s-list 
    ...
    FIELD zinr      AS CHAR
    FIELD wabkurz   AS CHAR INIT " " /* Dzikri 98C395 - Convert foreign currency to local currency */
.
...
/* Dzikri 98C395 - currency matching with reservation */
DEFINE VARIABLE currency            AS CHARACTER NO-UNDO INIT " ".
DEFINE VARIABLE local-currency      AS CHARACTER NO-UNDO INIT " ".
DEFINE VARIABLE frate               AS DECIMAL   NO-UNDO INIT 1.
DEFINE VARIABLE waehrungsnr         AS INTEGER   NO-UNDO INIT 1.
DEFINE VARIABLE price-decimal       AS INTEGER   NO-UNDO INIT 0.
/* Dzikri 98C395 - END */

DEFINE VARIABLE tot-deposit         AS DECIMAL   NO-UNDO INIT 0. /* Dzikri DB1155 - Clear split deposit payment refund */
...
DO:
    ...
    END.
    /* Dzikri 98C395 - find currency */
    IF currency NE local-currency THEN
    DO:
        FOR EACH t-list WHERE TRIM(t-list.wabkurz) EQ "" OR t-list.wabkurz EQ local-currency:
          t-list.wabkurz = currency.
          IF t-list.date1 LT billdate THEN
          DO:
              FIND FIRST exrate WHERE exrate.artnr = waehrungsnr
                      AND exrate.datum = t-list.date1 NO-LOCK NO-ERROR.
              IF AVAILABLE exrate THEN t-list.betrag = t-list.betrag / exrate.betrag.
          END.
          ELSE
          DO:
              t-list.betrag = t-list.betrag / frate.
          END.
          IF price-decimal NE 0 THEN t-list.betrag = ROUND(t-list.betrag,price-decimal).
        END.
    END.
    ELSE 
    DO:
      FOR EACH t-list WHERE TRIM(t-list.wabkurz) EQ "":
        t-list.wabkurz = local-currency.
      END.
    END.
END.
...
/*    IF resline.erwachs GT 0 THEN */ 
      ...
      /* Dzikri 98C395 - find currency */
      FIND FIRST waehrung WHERE waehrung.waehrungsnr = resline.betriebsnr NO-LOCK NO-ERROR.
      ...
      FIND FIRST s-list WHERE s-list.bezeich = vhp.arrangement.argt-rgbez 
        ...
        AND s-list.kind2 = resline.kind2 
        AND s-list.wabkurz = currency NO-ERROR. 
...
/**** additional fix cost ******/ 
  ...
  FOR EACH s-list BY s-list.ankunft BY s-list.datum BY s-list.bezeich 
    ...
    DO: 
      FIND FIRST t-list WHERE t-list.bezeich = s-list.bezeich 
        ...
        AND t-list.kind2 = s-list.kind2 
        AND t-list.wabkurz = s-list.wabkurz NO-ERROR. 
  ...
  ELSE DO:
    ...
    END.
    */
    /* Dzikri DB1155 - Clear split deposit payment refund
    FOR EACH t-list WHERE t-list.depo-billjour:
    ...
    END.
       Dzikri DB1155 - Clear split deposit payment refund */
    FIND FIRST billjournal WHERE billjournal.billjou-ref EQ t-resnr 
        AND billjournal.bezeich MATCHES "*Refund #*" NO-LOCK NO-ERROR.
    FOR EACH t-list WHERE t-list.depo-billjour EQ YES BY t-list.nr:
        tot-deposit = tot-deposit + t-list.betrag.
        IF AVAILABLE billjournal THEN
        DO:
            IF (t-list.betrag + billjournal.betrag) EQ 0 THEN 
            DO:
              DELETE t-list.
              tot-deposit = 0.
              FIND NEXT billjournal WHERE billjournal.billjou-ref EQ t-resnr
                  AND billjournal.bezeich MATCHES "*Refund #*" NO-LOCK NO-ERROR.
            END.
            ELSE IF (tot-deposit + billjournal.betrag) EQ 0 THEN 
            DO:
              FOR EACH buff-t WHERE buff-t.depo-billjour EQ YES AND buff-t.NR LE t-list.nr BY buff-t.nr:
                DELETE buff-t.
              END.
              tot-deposit = 0.
              FIND NEXT billjournal WHERE billjournal.billjou-ref EQ t-resnr
