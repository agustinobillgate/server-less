DEFINE TEMP-TABLE res-listmain
    ...
    FIELD city              AS CHAR
    FIELD sob               AS CHAR
    FIELD segment           AS CHAR /* Naufal Afthar - 44FA28*/
    FIELD tot-revenue-resv  AS DECIMAL
    FIELD prcode            AS CHAR /* MCH - 71E2E4 */
    .
...
DEFINE TEMP-TABLE res-listmember
    ...
    FIELD ratecode    AS CHARACTER FORMAT "x(18)"
...
DEFINE INPUT PARAMETER search-prcode    AS CHAR. /* MCH - 71E2E4 */
...
PROCEDURE update-browse-b2: 
    ...
    IF search-resno = 0 AND search-voucher = "" AND search-prcode = "" THEN
    DO:
        curr-select = "". 
        ...
        FOR EACH reservation WHERE reservation.resdat GE from-date 
          ...
          DO:
            ...
            /*FDL August 28, 2023 => Ticket 024948*/
            ASSIGN  
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,CHR(10),"").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,CHR(13),"").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,"~n","").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,"\n","").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,"~r","").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,"~r~n","").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,CHR(10) + CHR(13),"").
           
            res-bemerk = "".
            DO loopk = 1 TO LENGTH(res-listmain.bemerk):
                IF ASC(SUBSTR(res-listmain.bemerk, loopk, 1)) = 0 THEN.
                ELSE res-bemerk = res-bemerk + SUBSTR(res-listmain.bemerk, loopk, 1). 
            END.
            ASSIGN res-listmain.bemerk = res-bemerk.
           
            IF LENGTH(res-listmain.bemerk) LT 3 THEN res-listmain.bemerk = REPLACE(res-listmain.bemerk,CHR(32),"").
            IF LENGTH(res-listmain.bemerk) EQ ? THEN res-listmain.bemerk = "".
            /*End FDL*/

            /* Oscar (09/02/2025) - 57CA51 - add tot revenue per reservation on reservation by creation date */
            RUN count-total-reservation(reservation.resnr, OUTPUT res-listmain.tot-revenue-resv).
          END.
        END.
    ...
    DO:
        ...
        FOR EACH reservation WHERE reservation.vesrdepot EQ search-voucher 
          ...
          DO:
            ...
            /* Oscar (09/02/2025) - 57CA51 - copy solution from other filter to ensure correct output */
            /*FDL August 28, 2023 => Ticket 024948*/
            ASSIGN  
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,CHR(10),"").
              res-listmain.bemerk = REPLACE(res-listmain.bemerk,CHR(13),"").
    ...
    DO:
        ...
        FOR EACH reservation WHERE reservation.activeflag LE 1 
          ...
          FIND FIRST res-line WHERE res-line.resnr = reservation.resnr
            ...
            AND res-line.zimmer-wunsch MATCHES "*PromotionCode*" NO-LOCK NO-ERROR.
          IF AVAILABLE res-line THEN
          DO:
            CREATE res-listmain.
            ASSIGN
            ...
            DO i = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch, ";"):
              prcode = ENTRY(i,res-line.zimmer-wunsch, ";").
              IF prcode MATCHES "*$PRCODE$*" THEN
              DO:
                  ASSIGN res-listmain.prcode = trim(SUBSTRING(prcode, 9)).
              END.
            END. 
...
/* Oscar (09/02/2025) - 57CA51 - add tot revenue per reservation on reservation by creation date */
PROCEDURE count-total-reservation:
  DEFINE INPUT PARAMETER resnr AS INTEGER.
  DEFINE OUTPUT PARAMETER tot-revenue-resv AS DECIMAL.

  DEFINE VARIABLE excp-rev   AS CHARACTER.

  FOR EACH res-line WHERE res-line.resnr EQ resnr NO-LOCK:
    RUN view-staycostbl.p  
      (0, resnr, res-line.reslinnr, "", OUTPUT TABLE revenue-list).

    FIND LAST revenue-list NO-LOCK.
    IF AVAILABLE revenue-list THEN
    DO:
      excp-rev = STRING(TRIM(ENTRY(2, revenue-list.str, "="))).
      tot-revenue-resv = tot-revenue-resv + DECIMAL(excp-rev) * res-line.zimmeranz.
    END.
  END.
END.
