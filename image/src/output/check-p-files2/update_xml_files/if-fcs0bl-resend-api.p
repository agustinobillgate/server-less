...
DEFINE INPUT PARAMETER p-irecid         AS int64            NO-UNDO.
DEFINE OUTPUT PARAMETER p-status        AS CHARACTER        NO-UNDO.

/* Local 
DEFINE VARIABLE p-irecid    AS int64        NO-UNDO INIT 5036776.
...
DO :
    ...
    /*FIND FIRST INTERFACE WHERE RECID(INTERFACE) EQ p-irecid /*INTERFACE.resnr EQ p-resnr*/ AND
        (INTERFACE.KEY EQ 10 OR INTERFACE.KEY EQ 37) AND
    ...
    DO:
        IF (INTERFACE.KEY EQ 10 OR INTERFACE.KEY EQ 37) /*AND*/ OR
            INTERFACE.nebenstelle MATCHES "*$FCS0$*" AND
            (INTERFACE.parameters NE "modify" OR
