...
DEF INPUT PARAMETER selected-gastnr AS INT.
DEF INPUT PARAMETER depo-amount AS DECIMAL.
DEF INPUT PARAMETER ns-billno   AS INT.
DEF INPUT PARAMETER move-date   AS DATE.
...
ELSE
DO:
    IF move-date EQ curr-date THEN
    DO:
        FIND FIRST qsy WHERE qsy.KEY = 33 AND qsy.number1 = curr-dept
            AND qsy.number2 = moved-tisch AND qsy.date1 = curr-date
            AND qsy.logi3 = YES
    ...
    DO:
        FIND FIRST qsy WHERE qsy.KEY = 33 AND qsy.number1 = curr-dept
            AND qsy.number2 = moved-tisch AND qsy.date1 = move-date
            AND qsy.logi3 = YES
            AND von-zeit GE SUBSTR(qsy.char1,1,4)
            AND von-zeit LE SUBSTR(qsy.char1,5,4) 
            AND RECID(qsy) NE s-recid NO-LOCK NO-ERROR.
...
DO:
    ...
    ASSIGN
        ...
        queasy.char2   = gname + "&&" + STRING(selected-gastnr)
        queasy.char3   = user-init + ";" + REPLACE(comments, ";", ",") + ";"
        queasy.deci1   = depo-amount
        queasy.deci2   = ns-billno
        queasy.logi3   = YES
    .
...
DO:
    ...
    ASSIGN
        ...
        queasy.deci2   = ns-billno
    .
    IF move-date NE curr-date THEN queasy.date1 = move-date.
