...
DEFINE VARIABLE currnat AS INTEGER NO-UNDO INIT 0.
DEFINE VARIABLE do-it AS LOGICAL NO-UNDO INIT NO.
DEFINE VARIABLE currname AS CHAR NO-UNDO.
DEFINE VARIABLE currbez AS CHAR NO-UNDO.
 incl-comp = NOT mi-comp. 
  ...
  FOR EACH nationstat WHERE nationstat.datum GE from-date AND nationstat.datum LE to-date NO-LOCK BY nationstat.nationnr:

    do-it = NO.
    IF currnat NE nationstat.nationnr THEN
    DO:
      FIND FIRST nation WHERE nation.natcode = 0 AND nation.nationnr = nationstat.nationnr NO-LOCK NO-ERROR.
      IF AVAILABLE nation THEN
        ASSIGN
          currname = ENTRY(1,nation.bezeich, ";")
          currbez = nation.kurzbez
          currnat = nation.nationnr.
      ELSE currnat = 0.
    END.
      
    IF currnat NE 0 THEN do-it = YES.
      
    IF do-it THEN
    DO:
      FIND FIRST room-list WHERE room-list.NAME = currname AND room-list.bezeich = currbez NO-LOCK NO-ERROR.
