...
/*** SY 12/03/2025 ***/
DEFINE VARIABLE allotment-exist        AS LOGICAL NO-UNDO.
DEFINE VARIABLE global-allotment-exist AS LOGICAL NO-UNDO.
  FIND FIRST kontline WHERE (kontline.betriebsnr = 0 AND kontline.kontstat = 1) 
      AND ((kontline.ankunft LE curr-date AND kontline.abreise GE curr-date)
         OR (kontline.ankunft GE curr-date AND kontline.ankunft LE to-date))
      NO-LOCK NO-ERROR. 
  allotment-exist = AVAILABLE kontline.
...
DEFINE VARIABLE qsy14-exist AS LOGICAL NO-UNDO. 
  FIND FIRST queasy WHERE queasy.key = 14 AND 
    ((queasy.date1 LE curr-date AND queasy.date2 GE curr-date) OR
    (queasy.date1 GE curr-date AND queasy.date1 LE to-date)) 
     NO-LOCK NO-ERROR.      
  qsy14-exist = AVAILABLE queasy.
  
DEFINE TEMP-TABLE art-list   /* Used for lodging articles */
...
DEFINE VARIABLE ooo-exist  AS LOGICAL INITIAL NO NO-UNDO.
DEFINE VARIABLE ooo-frdate AS DATE NO-UNDO.
...
/*** SY 12/03/2025 Check if any future ooo exists within the period ***/ 
  IF to-date GE ci-date THEN
  DO:
      IF curr-date LT ci-date THEN ooo-frdate = ci-date.
      ELSE ooo-frdate = curr-date.
      FIND FIRST outorder WHERE 
      (NOT outorder.gespende  LT ooo-frdate AND 
       NOT outorder.gespstart GT to-date)
       AND outorder.betriebsnr LE 1 NO-LOCK NO-ERROR.
      ooo-exist = AVAILABLE outorder.
  END.
 ...
 /*** SY 12/03/2025 ***/
  ...
  DO i = 1 TO tmpInt: /* FT Serverless : (to-date - curr-date + 1)*/
    ...
    FOR EACH queasy WHERE queasy.key = 14 AND 
      ...
      BY queasy.number3: 
      FIND FIRST zimmer WHERE zimmer.zinr = queasy.char1 AND zimmer.sleeping 
        NO-LOCK.
  ...
  FOR EACH room-list WHERE room-list.wd NE 0 by room-list.datum: 
    ...
    DO: 
      ...
      END. 
    END. 
    ELSE IF ooo-exist THEN  /*** SY 12/03/2025 ***/
...
/*** SY 12/03/2025 replacing calc-othRev ***/
  IF do-it AND incl-oth THEN RUN calculate-other-revenue.

/**** replaced by calculate-other-revenue
  IF do-it THEN DO:
      ...
      IF incl-oth THEN DO:
          FOR EACH room-list:
              ...
              ASSIGN
                  ...
                  room-list.lodg[7] = room-list.lodg[7] + othRev / exchg-rate
                  .
...
/*** SY 12/03/2025 new design ***/
PROCEDURE calculate-other-revenue:
DEFINE VARIABLE curr-datum  AS DATE    NO-UNDO.
DEFINE VARIABLE othRev      AS DECIMAL NO-UNDO.
DEFINE VARIAble curr-exrate AS DECIMAL NO-UNDO.
...
DEFINE VARIABLE vat         AS DECIMAL.
    ...
    IF curr-date GT ci-date THEN RETURN.
    ...
    FOR EACH art-list:    
        ASSIGN curr-datum = 01/01/2000. /*** same result as curr-datum = ? ***/
        FOR EACH umsatz WHERE umsatz.artnr = art-list.artnr 
            ...
            DO:
                ...
                DO:
                  ...
                  ASSIGN 
                      t-lodg[5] = t-lodg[5] + othRev
                      t-lodg[7] = t-lodg[7] + othRev / curr-exrate
                  .
                END.                
                ...
                DO:
                   FIND FIRST exrate WHERE exrate.datum = room-list.datum 
                       AND exrate.artnr = waehrung.waehrungsnr NO-LOCK NO-ERROR. 
                  IF AVAILABLE exrate THEN curr-exrate = exrate.betrag.
                  ELSE curr-exrate = exchg-rate.
                END. 
                ELSE curr-exrate = exchg-rate.
                
                RUN calc-servvat.p(umsatz.departement, umsatz.artnr, 
                    umsatz.datum, art-list.service-code, art-list.mwst-code, 
        ...
        ASSIGN                  
            room-list.lodg[5] = room-list.lodg[5] + othRev
            room-list.lodg[7] = room-list.lodg[7] + othRev / curr-exrate
        .
        IF room-list.wd NE 0 THEN 
        ASSIGN 
            t-lodg[5] = t-lodg[5] + othRev.
