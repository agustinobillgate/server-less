...
/*** SY 12/03/2025 ***/
DEFINE TEMP-TABLE argtline-list LIKE argt-line.

DEFINE TEMP-TABLE art-list
    FIELD artnr        AS INTEGER
    FIELD service-code AS INTEGER 
    FIELD mwst-code    AS INTEGER.

DEFINE TEMP-TABLE art-vatlist
    FIELD artnr   AS INTEGER
...
/*** SY 12/03/2025 ***/
DEFINE VARIABLE global-allotment-exist AS LOGICAL NO-UNDO.
  FIND FIRST kontline WHERE kontline.betriebsnr = 1 AND kontline.kontstat = 1 
      NO-LOCK NO-ERROR. 
  global-allotment-exist = AVAILABLE kontline.
...
/*** SY 12/03/2025 ***/
  FIND FIRST kontline WHERE kontline.betriebsnr = 0 AND kontline.kontstat = 1 
      NO-LOCK NO-ERROR. /* Allotment, nowadays rarely used */
  ...
  */
  ...
  FOR EACH genstat WHERE genstat.res-date[2] GE datum1
    ...
    FIND FIRST waehrung WHERE waehrung.waehrungsnr = genstat.wahrungsnr NO-LOCK.
...
/* SY 12/03/2025 No longer needed, use .res-date[2] instead          
...
*/
            ...
            DO:
              ...
              IF AVAILABLE artikel THEN
              DO:
                  FIND FIRST art-vatlist WHERE art-vatlist.artnr = artikel.artnr
                      AND art-vatlist.dept = artikel.departement
                      AND art-vatlist.datum = genstat.datum NO-ERROR.
                  IF AVAILABLE art-vatlist THEN
                  ASSIGN
                      service = art-vatlist.service
                      vat     = art-vatlist.vat
                  .
                  ELSE
                  DO:
                     ...
                     CREATE art-vatlist.
                     ASSIGN
                         art-vatlist.artnr    = artikel.artnr
                         art-vatlist.dept     = artikel.departement
                         art-vatlist.datum    = genstat.datum
                         art-vatlist.service  = service
                         art-vatlist.vat      = vat
                     .
                  END.
              END.
          ...
          DO:
              ...
              DO:    
                  ...
                  DO:
                      FIND FIRST art-vatlist WHERE art-vatlist.artnr = artikel.artnr
                          AND art-vatlist.dept = artikel.departement
                          AND art-vatlist.datum = datum NO-ERROR.
                      IF AVAILABLE art-vatlist THEN
                      ASSIGN
                          service = art-vatlist.service
                          vat     = art-vatlist.vat
                  ...
                  ELSE IF show-rev EQ 2 THEN
                      ASSIGN room-list.fixleist  = room-list.fixleist + fcost                               
                           ...
                           /*tot-fixcost2        = tot-fixcost2 + room-list.fixleist2
                             ...
                             tot-fixcost         = tot-fixcost + fcost.
              END. 
          END. /* IF AVAILABLE room-list */
        END. /* DO datum... */
      END.  /* FOR EACH fixleist */
/*End of add */
        ...
        DO:
            ...
            ELSE DO:
             ...
             /* Dzikri 796D85 - arrangement fixcost */
              ...
              DO:
              /*** SY 12/03/2025 ***/
                FIND FIRST argtline-list WHERE argtline-list.argtnr = arrangement.argtnr NO-ERROR.
                IF NOT AVAILABLE argtline-list THEN
                DO:
                    FOR EACH argt-line WHERE argt-line.argtnr EQ arrangement.argtnr 
                        AND argt-line.kind2 NO-LOCK: 
                        CREATE argtline-list.
                        BUFFER-COPY argt-line to argtline-list.
                    END.
                END.                 
                /*$$$*/ 
                FOR EACH argtline-list WHERE argtline-list.argtnr EQ arrangement.argtnr:                       
                    RUN check-fixargt-posted(argtline-list.argt-artnr, argtline-list.departement, 
                            argtline-list.fakt-modus, argtline-list.intervall, res-line.ankunft, res-line.abreise, 
                            OUTPUT post-it).
                    ...
                    IF post-it THEN 
                    DO:
                        FIND FIRST artikel WHERE artikel.artnr = argtline-list.argt-artnr
                            AND artikel.departement = argtline-list.departement NO-LOCK NO-ERROR.
                        IF AVAILABLE artikel THEN
                        DO:
                          FIND FIRST art-vatlist WHERE art-vatlist.artnr = artikel.artnr
                              AND art-vatlist.dept = artikel.departement
                          ...
                          END.
                        END.
                        IF argtline-list.vt-percnt = 0 THEN 
                        DO: 
                          IF argtline-list.betriebsnr = 0 THEN pax = res-line.erwachs. 
                          ELSE pax = argtline-list.betriebsnr. 
                        END. 
                        ELSE IF argtline-list.vt-percnt = 1 THEN pax = res-line.kind1. 
                        ELSE IF argtline-list.vt-percnt = 2 THEN pax = res-line.kind2.
                        ELSE pax = 0.
                        price = 0.
                        FIND FIRST reslin-queasy WHERE reslin-queasy.key = "fargt-line" 
                            AND reslin-queasy.char1    = "" 
                            AND reslin-queasy.number1  = argtline-list.departement 
                            AND reslin-queasy.number2  = argtline-list.argtnr 
                            AND reslin-queasy.resnr    = res-line.resnr 
                            AND reslin-queasy.reslinnr = res-line.reslinnr 
                            AND reslin-queasy.number3  = argtline-list.argt-artnr 
                        ...
                        END.
                        IF price EQ 0 THEN
                        ASSIGN fcost = argtline-list.betrag * pax
                               fcost = fcost / (1 + service + vat).
                        IF show-rev EQ 1 THEN
                            ASSIGN room-list.fixleist = room-list.fixleist + fcost
                                   /*tot-fixcost        = tot-fixcost + room-list.fixleist*/
...
/*** SY 12/03/2025 ***/       
                IF global-allotment-exist THEN
                DO:
...
/*** SY 12/03/2025 ***/ 
  FOR EACH queasy WHERE queasy.key = 14 AND 
    (queasy.date1 LE curr-date AND queasy.date2 GE curr-date) OR
    (queasy.date1 GE curr-date AND queasy.date1 LE (curr-date + tmpInt)) 
     NO-LOCK: 
    datum = curr-date - 1. 
    DO i = 1 TO tmpInt: /* FT Serverless : (to-date - curr-date + 1)*/
      datum = datum + 1. 
      IF datum GE queasy.date1 AND datum LE queasy.date2 THEN
      DO:
        FIND FIRST room-list WHERE room-list.datum = datum. 
        FIND FIRST zimmer WHERE zimmer.zinr = queasy.char1 AND zimmer.sleeping NO-LOCK.
...
/*** SY 12/03/2025 ***/ 
  DEFINE VARIABLE from-ooo AS DATE NO-UNDO.
  DEFINE VARIABLE to-ooo   AS DATE NO-UNDO.
  ASSIGN
    from-ooo = 01/01/2000
    to-ooo   = 01/01/2000
  .  
  FOR EACH room-list WHERE room-list.wd NE 0 by room-list.datum: 
    ...
    ELSE 
    DO: 
      IF from-ooo EQ 01/01/2000 THEN from-ooo = room-list.datum.
      to-ooo = room-list.datum.
    END.
  ...
  IF from-ooo NE 01/01/2000 THEN
  FOR EACH outorder WHERE 
      ((outorder.gespstart LE from-ooo AND outorder.gespende GE from-ooo) OR
      (outorder.gespstart GE from-ooo AND outorder.gespstart LE to-ooo)) AND
       outorder.betriebsnr LE 1 NO-LOCK BY outorder.zinr: 
      FIND FIRST zimmer WHERE zimmer.zinr = outorder.zinr NO-LOCK.
      IF zimmer.sleeping THEN
      DO: 
        FIND FIRST room-list WHERE room-list.datum GE outorder.gespstart AND
          room-list.datum LE outorder.gespende NO-ERROR.
        IF AVAILABLE room-list THEN
        DO:
        /*MT 31/05/12 */
          IF NOT all-zikat THEN 
      ...
      END. 
  END. 
/*** SY 12/03/2025 Modification above ends here ***/
...
/*** SY 12/03/2025 ***/
  ...
  DO:
      ...
      /*t-lodg[5] = 0.*/
      IF incl-oth and curr-date LE ci-date THEN RUN calc-othRev. 
  END. /*end*/
/*** SY 12/03/25 End of modif */  
...
DEFINE VARIABLE curr-datum  AS DATE    NO-UNDO.
DEFINE VARIABLE othRev      AS DECIMAL NO-UNDO.
DEFINE VARIABLE calc-todate AS DATE    NO-UNDO.
...
DEFINE VARIABLE vat         AS DECIMAL NO-UNDO.
    ...
    IF to-date GT ci-date THEN calc-todate = ci-date.
    ELSE calc-todate = to-date.
    ...
    FOR EACH artikel WHERE artikel.departement = 0
        ...
        ASSIGN 
            ...
            art-list.service-code = artikel.service-code 
            art-list.mwst-code    = artikel.mwst-code        
        .
    END.

    FIND FIRST htparam WHERE htparam.paramnr = 144 NO-LOCK NO-ERROR. 
    FIND FIRST waehrung WHERE waehrung.wabkurz = htparam.fchar NO-LOCK NO-ERROR. 
    DO curr-datum = curr-date TO calc-todate:
       IF AVAILABLE waehrung THEN 
       DO:
         IF curr-datum LT ci-date THEN 
         DO:
            FIND FIRST exrate WHERE exrate.datum = curr-datum 
                AND exrate.artnr = waehrung.waehrungsnr NO-LOCK NO-ERROR. 
            IF AVAILABLE exrate THEN exchg-rate = exrate.betrag. 
            ELSE exchg-rate = waehrung.ankauf / waehrung.einheit. 
            CREATE exrate-list.
            ASSIGN
                exrate-list.datum = curr-datum
                exrate-list.exchg-rate = exchg-rate
            .
         END.
         ELSE
         DO:
            CREATE exrate-list.
            ASSIGN
                exrate-list.datum = ci-date
                exrate-list.exchg-rate = waehrung.ankauf / waehrung.einheit
            .
         END.
       END.
       ELSE
       DO:
           ...
           ASSIGN
               exrate-list.datum      = curr-datum
               exrate-list.exchg-rate = 1
           .
       
       END.
       IF curr-date = ci-date THEN LEAVE.
    END.
    
/* $$$$ */
    FOR EACH art-list:    
        ASSIGN 
            curr-datum = 01/01/2000
            othRev     = 0
        .
        FOR EACH umsatz WHERE umsatz.artnr = art-list.artnr 
            AND umsatz.departement = 0
            AND umsatz.datum GE curr-date 
            AND umsatz.datum LE calc-todate NO-LOCK by umsatz.datum:
            IF curr-datum NE umsatz.datum THEN
            DO:
                IF curr-datum NE 01/01/2000 THEN
                DO:
                  FIND FIRST exrate-list WHERE exrate-list.datum = curr-datum.
                  ASSIGN                  
                      room-list.lodg[5] = room-list.lodg[5] + othRev
                      room-list.lodg[7] = room-list.lodg[7] + othRev / exrate-list.exchg-rate
                  .
                  IF room-list.wd NE 0 THEN 
                  ASSIGN 
                      t-lodg[5] = t-lodg[5] + othRev
                      t-lodg[7] = t-lodg[7] + othRev / exrate-list.exchg-rate
                  .
                END.                
                curr-datum = umsatz.datum.
                FIND FIRST room-list WHERE room-list.datum = curr-datum. 

                FIND FIRST art-vatlist WHERE art-vatlist.artnr = umsatz.artnr
                    AND art-vatlist.dept  = umsatz.departement
                    AND art-vatlist.datum = umsatz.datum NO-ERROR.
                IF AVAILABLE art-vatlist THEN
                ASSIGN
                  service = art-vatlist.service
                  vat     = art-vatlist.vat
                ...
                DO:
                   ...
                   CREATE art-vatlist.
                   ASSIGN
                       art-vatlist.artnr    = umsatz.artnr
                       art-vatlist.dept     = umsatz.departement
                       art-vatlist.datum    = umsatz.datum
                       art-vatlist.service  = service
                       art-vatlist.vat      = vat
                   .
                END.
                ...
                ASSIGN
                    fact    = 1.00 + service + vat
                    othRev  = umsatz.betrag / fact
               .
               
            END.
            ELSE othRev = othRev + umsatz.betrag / fact.
        END.
        FIND FIRST exrate-list WHERE exrate-list.datum = curr-datum.
        ASSIGN                  
            room-list.lodg[5] = room-list.lodg[5] + othRev
...
/* ITA 07/03/25
  /*** SY *** 12/03/2025 cidate codate instead of res-line.ankunft and .abreise***/
  ...
  DO: 
    ...
    /*IF (codate - start-date) LT intervall FT serverless*/
