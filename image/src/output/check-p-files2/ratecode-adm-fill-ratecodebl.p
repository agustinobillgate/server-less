...
DEFINE BUFFER tb3Buff FOR ratecode.
...
to-date = DATE(01,01,1990).
  ...
  DO:
      ...
      ASSIGN 
          ...
          res-history.aenderung   = "Modify Contract Rate, Code: " + prcode + " RmType : " + zimkateg.kurzbez + 
                                    ", FROM : start:" + STRING(bef-start) + "|end:" + STRING(bef-end) + "|adult:" + STRING(bef-pax) + "|rate:" + STRING(bef-rate) + 
                                    ", TO : start:" + STRING(p-list.startperiode) + "|end:" + STRING(p-list.endperiode) + "|adult:" + STRING(p-list.erwach) + "|rate:" + STRING(p-list.zipreis) 
                                    .
  ...
  END. 
  IF to-date NE DATE(01,01,1990) THEN 
  DO: 
  ...
  FIND FIRST tb3Buff WHERE RECID(tb3Buff) = RECID(ratecode) NO-LOCK.
...
DEF BUFFER rbuff   FOR ratecode.
  ...
  FOR EACH child-list WHERE child-list.true-child = NO:      
    ...
    .
    FOR EACH ratecode WHERE ratecode.marknr = tb3Buff.marknr 
        AND ratecode.code    = child-list.child-code 
        AND ratecode.argtnr  = tb3Buff.argtnr 
...
DEF VAR rounded-rate AS DECIMAL NO-UNDO.
    IF in-percent THEN 
    DO:    
        child-ratecode.zipreis = tb3buff.zipreis * (1 + adjust-value * 0.01).
        ...
        END.
    END.
    ELSE child-ratecode.zipreis = tb3buff.zipreis + adjust-value.
...
DEF VAR rounded-rate AS DECIMAL NO-UNDO.
    IF in-percent THEN 
    DO:    
        ratecode.zipreis = tb3buff.zipreis * (1 + adjust-value * 0.01).
        ...
        END.
    END.
    ELSE ratecode.zipreis = tb3buff.zipreis + adjust-value.
END.
