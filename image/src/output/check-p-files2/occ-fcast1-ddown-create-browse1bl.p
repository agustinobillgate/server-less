...
IF AVAILABLE htparam THEN ASSIGN ci-date = htparam.fdate.
        ...
        FOR EACH res-line WHERE res-line.active-flag LE 2 
            ...
            DO: 
              FIND FIRST queasy WHERE queasy.key = 14 AND queasy.char1 = res-line.zinr 
                AND queasy.date1 LE (datum - 1) AND queasy.date2 GE (datum - 1) NO-LOCK NO-ERROR. 
            ...
            DO:                
                ...
                IF (datum - 1) = res-line.ankunft AND consider-it 
                ...
                IF res-line.resstatus NE 3 AND res-line.resstatus NE 4 
                  AND consider-it 
                  AND (res-line.abreise GT (datum - 1) AND res-line.ankunft LT (datum - 1)
                  AND res-line.ankunft NE (datum - 1)
                  AND res-line.abreise NE (datum - 1))
                  AND res-line.active-flag LE 1 THEN 
                DO: 
  ...
  DO:
     FIND FIRST zinrstat WHERE zinrstat.zinr = "ooo" 
        AND zinrstat.datum = room-list.datum NO-LOCK NO-ERROR. 
