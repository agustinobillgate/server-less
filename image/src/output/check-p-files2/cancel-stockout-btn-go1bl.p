...
FOR EACH str-list:
    ...
    ASSIGN             
       cancel-stockout.datum        = DATE(SUBSTRING(s,1,8))
       cancel-stockout.lager        = SUBSTRING(s,9,30)
       cancel-stockout.lscheinnr    = SUBSTRING(s,120,12)
       cancel-stockout.artnr        = INTEGER(SUBSTRING(s,39,7))
       cancel-stockout.bezeich      = SUBSTRING(s,46,32) 
       cancel-stockout.out-qty      = DECIMAL(SUBSTRING(s,78,14))         
       cancel-stockout.avrg-price   = DECIMAL(SUBSTRING(s,92,14)) 
       cancel-stockout.amount       = DECIMAL(SUBSTRING(s,106,14))        
       cancel-stockout.id           = SUBSTRING(s,132,2)
       cancel-stockout.reason       = SUBSTRING(s,142,24)
     .
END.
