...
DEFINE VARIABLE curr-note-out   AS CHAR.
...
DEFINE VARIABLE debt-count AS INTEGER.
  ...
  FOR EACH l-op WHERE l-op.pos GT 0 AND l-op.op-art = 3 AND l-op.loeschflag LT 2 
    ...
    DO: 
      ...
      IF curr-lschein NE l-ophdr.lscheinnr THEN 
      DO:
        IF debt-count GT 1 THEN 
            RUN add-list-out(curr-note-out).
        debt-count = 0.
      END.

      IF AVAILABLE l-lieferant THEN
          curr-note = STRING(l-ophdr.lager-nr,"99") + " - " + l-ophdr.lscheinnr 
                + " " + l-artikel.bezeich + "; " + l-lieferant.firma.
      ELSE 
          curr-note = STRING(l-ophdr.lager-nr,"99") + " - " + l-ophdr.lscheinnr 
                + " " + l-artikel.bezeich.

      debt-count = debt-count + 1.
    ...
    DO: 
      ...
      IF AVAILABLE l-lieferant THEN
      DO:
        IF debt-count LE 1 THEN
            curr-note = STRING(l-ophdr.lager-nr,"99") + " - " + l-ophdr.lscheinnr 
                + " " + l-artikel.bezeich + "; " + l-lieferant.firma.
        IF debt-count GT 1 THEN
            curr-note-out = STRING(l-ophdr.lager-nr,"99") + " - " 
                + l-ophdr.lscheinnr + " " + l-lieferant.firma. 
      END.
...
DEFINE VARIABLE curr-lschein AS CHAR. 
DEFINE VARIABLE curr-artikel-bezeich AS CHAR.
...
DEFINE VARIABLE credit-count AS INTEGER INITIAL 0.
...
DEFINE BUFFER gl-acc2 FOR gl-acct. 
  ...
  FOR EACH l-op WHERE l-op.pos GT 0 AND l-op.anzahl NE 0 
    ...
    DO:
        ...
        DO: 
          ...
          curr-artikel-bezeich = l-artikel.bezeich.
        ...
        DO: 
          IF AVAILABLE l-lieferant AND credit-count LE 1 THEN 
          DO:
              curr-note = curr-lschein + " - " + curr-artikel-bezeich 
              + "; " + l-lieferant.firma.
              credit-count = 0.
          END.
          ELSE IF AVAILABLE l-lieferant AND credit-count GE 1 THEN
          DO:
              curr-note = curr-lschein + " " + l-lieferant.firma.
              credit-count = 0.
          END.
          ELSE DO: 
            curr-note = curr-lschein + " " + l-artikel.bezeich + "; " + l-lieferant.firma.
          ...
          DO: 
              FIND FIRST l-kredit WHERE l-kredit.lscheinnr = l-op.lscheinnr NO-LOCK NO-ERROR.
          END.
  ...
  DO: 
    ...
    DO:
        ...
        credit-count = 0.
    END.
    ELSE curr-note = curr-lschein + " " + l-lieferant.firma. 
...
PROCEDURE add-list-out: 
DEFINE INPUT PARAMETER note AS CHAR FORMAT "x(30)". 
  curr-anz = curr-anz + 1. 
  g-list.bemerk = note. 
END. 
