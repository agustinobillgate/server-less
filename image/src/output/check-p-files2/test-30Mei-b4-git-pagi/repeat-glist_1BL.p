...
DEFINE VAR tmp-rmnite       AS INTEGER NO-UNDO.
...
DEFINE BUFFER gbuff  FOR g-list.
  ...
  DO:
     FOR EACH genstat WHERE genstat.datum GE from-date 
       ...
       FIRST guest WHERE guest.gastnr = genstat.gastnrmember NO-LOCK,
       FIRST gfirma WHERE gfirma.gastnr = genstat.gastnr NO-LOCK BY genstat.datum:
       ...
       DO:
           ...
           ASSIGN
               ...
               g-list.gNAME      = guest.NAME + ", " + guest.vorname1 + " " + guest.anrede1
               g-list.birthdate = guest.geburtdatum1
               g-list.email     = guest.email-adr 
               g-list.telefon   = guest.telefon
               g-list.city      = guest.wohnort
               g-list.remark    = guest.bemerk
               g-list.mobil-telefon = guest.mobil-telefon
               g-list.nation1     = guest.nation1
  ...
  DO:
    ...
    FOR EACH res-line WHERE res-line.active-flag LE 2 
      ...
      FIRST gfirma WHERE gfirma.gastnr = res-line.gastnr NO-LOCK BY res-line.ankunft:
  ...
  FOR EACH g-list NO-LOCK
    ...
    DO:
      ...
      ASSIGN 
        ...
        repeat-list.NAME      = g-list.gname
        repeat-list.birthdate = g-list.birthdate
        repeat-list.email     = g-list.email
        repeat-list.telefon   = g-list.telefon
        repeat-list.city      = g-list.city
        repeat-list.remark    = g-list.remark
        repeat-list.mobil-telefon = g-list.mobil-telefon
      .
      FIND FIRST nation WHERE nation.kurzbez = g-list.nation1 NO-LOCK NO-ERROR.
    ...
    DO:
      ...
      END.
    END.
    IF g-list.abreise = g-list.ankunft THEN tmp-rmnite = 1.
    ELSE tmp-rmnite = g-list.abreise - g-list.ankunft. 
    IF NOT g-list.arrFlag THEN
        ASSIGN 
            repeat-list.rmnite = repeat-list.rmnite + tmp-rmnite.
