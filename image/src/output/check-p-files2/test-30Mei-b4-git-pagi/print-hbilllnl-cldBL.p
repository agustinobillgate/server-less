...
DEFINE VARIABLE foot3           AS CHAR FORMAT "x(32)".
...
DEFINE VARIABLE guest-addr      AS CHARACTER NO-UNDO.
/*FD Dec 09, 2021*/
...
IF AVAILABLE htparam THEN serv-disc = htparam.flogical.

FIND FIRST htparam WHERE htparam.paramnr = 469 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN vat-disc = htparam.flogical.
...
DO:
    IF NOT htparam.flogical AND ENTRY(1,htparam.fchar,";") = "GST(MA)" THEN
        gst-logic = YES.
END.
...
DO:  
    FIND FIRST vhp.h-bill WHERE RECID(vhp.h-bill) = hbrecid NO-LOCK NO-ERROR.
    IF AVAILABLE vhp.h-bill THEN /*Alder - Serverless - Issue 629*/ /*IF AVAILABLE*/
    DO:
        FIND FIRST vhp.htparam WHERE vhp.htparam.paramnr = 132 NO-LOCK.   
        FIND FIRST vhp.artikel WHERE vhp.artikel.artnr = vhp.htparam.finteger AND vhp.artikel.departement = 0 NO-LOCK NO-ERROR.
        IF AVAILABLE vhp.h-bill THEN 
        DO:
            IF AVAILABLE vhp.artikel THEN   
            DO:   
                FIND FIRST artBuff WHERE artBuff.artnr = vhp.artikel.artnr AND artBuff.departement = vhp.h-bill.departement NO-LOCK NO-ERROR.  
        ...
        nbezeich = vhp.htparam.finteger.   /*FDL: 7DCA98*/
        /*nbezeich = 25.*/
        ...
        ELSE   
        DO:   
            FIND FIRST vhp.h-queasy WHERE vhp.h-queasy.number1 = (vhp.h-bill.departement + vhp.h-bill.rechnr * 100)   
        ...
        FIND FIRST vhp.hoteldpt WHERE vhp.hoteldpt.num = vhp.h-bill.departement NO-LOCK.   
        zeit = time.   
        ...
        DO:  
            FIND FIRST vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = vhp.h-bill.rechnr AND vhp.h-bill-line.departement = vhp.h-bill.departement NO-LOCK NO-ERROR.  
            IF AVAILABLE vhp.h-bill-line THEN bill-date = vhp.h-bill-line.bill-datum.  
        END.  
      
        FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = vhp.h-bill.rechnr AND vhp.h-bill-line.departement = vhp.h-bill.departement NO-LOCK:   
       
        IF vhp.h-bill-line.artnr NE 0 THEN   
        DO:   
            FIND FIRST vhp.h-artikel WHERE vhp.h-artikel.artnr = vhp.h-bill-line.artnr AND vhp.h-artikel.departement = vhp.h-bill-line.departement NO-LOCK.   
            IF (vhp.h-artikel.artart = 11 OR vhp.h-artikel.artart = 12) THEN comp-taxserv = NOT comp-taxserv.   /*FT*/
            ELSE IF vhp.h-artikel.artart = 0 THEN tot-sales = tot-sales + vhp.h-bill-line.betrag.  
        END.   
        ...
        IF (vhp.h-bill-line.artnr = 0 OR vhp.h-artikel.artart NE 0) AND NOT new-item THEN  
        ASSIGN  
            new-item = YES  
            printed  = YES.
        DO:   
        ...
        DO:   
            msg-str = msg-str + CHR(2) + translateExtended ("No new bill-lines was found.",lvCAREA,"").  
        ...
        DO:   
            filename = "billdir\" + STRING(vhp.h-bill.tischnr) + "_" + STRING(vhp.h-bill.departement,"99").   
        ...
        DO:   
            ...
            DO i = 1 TO npage:   
                ...
                DO WHILE AVAILABLE art-list AND (curr-j LE lpage): 
                    RUN print-billine.  
                    FIND NEXT art-list WHERE art-list.artart = 0 AND NOT art-list.printed NO-ERROR.   
                    IF NOT AVAILABLE art-list THEN i = npage.   
                END.   
                IF i = npage THEN  /* all bill-lines are printed  */   
                DO:   
...
DEF BUFFER aBuff FOR vhp.artikel.  
  ...
  DO:  
    ...
    FIND FIRST kontplan NO-LOCK WHERE kontplan.betriebsnr = depart   
      AND kontplan.kontignr = abuff.artnr AND kontplan.datum = /*datum*/ inpDate NO-ERROR.  
...
DEFINE VARIABLE new-sold-item  AS LOGICAL NO-UNDO.  
  ...
  DO:     
    ...
    FIND FIRST vhp.paramtext WHERE paramtext.txtnr = 713 NO-LOCK NO-ERROR.
    IF AVAILABLE paramtext THEN
    DO:
        IF foot3 = "" AND paramtext.ptexte NE "" THEN   
        DO:   
    ...
    FIND FIRST vhp.paramtext WHERE paramtext.txtnr = 712 NO-LOCK NO-ERROR.
    IF AVAILABLE paramtext THEN
    DO:
        IF foot2 = "" AND paramtext.ptexte NE "" THEN   
        DO:   
          foot2 = paramtext.ptexte.   
          anz-foot = 2.   
        END.
    END.       
    FIND FIRST vhp.paramtext WHERE paramtext.txtnr = 711 NO-LOCK NO-ERROR.
    IF AVAILABLE paramtext THEN
    DO:
        IF foot1 = "" AND paramtext.ptexte NE "" THEN          
        DO:   
...
DEFINE VARIABLE table-desc AS CHARACTER.
  ...
  FIND FIRST tisch WHERE tisch.tischnr EQ h-bill.tischnr
      AND tisch.departement EQ h-bill.departement NO-LOCK NO-ERROR.
  IF AVAILABLE tisch THEN table-desc = tisch.bezeich.

  IF printed-line = 0 OR print-all 
      /*OR NOT print-all*/ THEN /*FD => For can print partial*/
  DO:       
    DO i = 1 TO hmargin:  
      output-list.str = output-list.str + STRING(" ").  
      CREATE output-list.
    ...
    DO:        
      ...
      output-list.str = output-list.str  
                + "     " + translateExtended ("Table",lvCAREA,"") + "|"   
                         ...
                         + kname + "|"
                         + order-id + "|"
                         + table-desc           /*FDL - April 30, 2024 => Ticket 8FA2F9*/
                        .  
      CREATE output-list.
      output-list.sort-i = sort-i.
      sort-i = sort-i + 1.
      ...
      */  
      ...
      DO:
          ...
          ASSIGN
            ...
            sort-i = sort-i + 1
            output-list.str = output-list.str  
                              + STRING("       " + translateExtended ("GST ID : 00185060352",lvCAREA,"")).
          CREATE output-list.
          output-list.sort-i = sort-i.
          sort-i = sort-i + 1.    
      END.
...
DEFINE BUFFER h-art FOR vhp.h-artikel.  
  ...
  ELSE /*MTPUT STREAM s1 art-list.qty " "*/ 
      ASSIGN 
      output-list.str-pos = 10
      output-list.str = output-list.str + STRING("   ") + STRING(art-list.qty) + "|".  
...
/*  tot-amount = subtotal + service + mwst. */  
  ...
  ELSE  
  DO:  
      output-list.str = output-list.str + "|" + STRING(tot-amount, "->>,>>>,>>>,>>9.99").  
      CREATE output-list.
      output-list.sort-i = sort-i.
      sort-i = sort-i + 1.
  ...
  DO:
      ...
      IF AVAILABLE guest THEN guest-addr = guest.adresse1 + " " + guest.adresse2 + " " + guest.adresse3.
  END.
  ...
  DO:
    ...
    output-list.str = output-list.str +   
            /*STRING(translateExtended ("Guest :",lvCAREA,"")) +*/  
            STRING(" ") + STRING(vhp.h-bill.bilname, "x(64)") /*+ " " +
            STRING(guest-addr, "x(48)")*/.  
  END.
...
DEFINE VARIABLE compli-notax    AS LOGICAL INITIAL NO.  
  ...
  FOR EACH art-list WHERE art-list.artart NE 0:     /* payments */   
    ...
    DO:   
      ...
      DO:  
        ...
        ELSE  
        DO:  
            output-list.str = output-list.str + "|" + STRING(amt, "->>,>>>,>>>,>>9.99").  
            CREATE output-list.
            output-list.sort-i = sort-i.
            sort-i = sort-i + 1.
  ...
  DO:   
    ...
    DO:   
      ...
      ELSE  
      DO:  
          output-list.str = output-list.str + "|" + STRING(balance, "->>,>>>,>>>,>>9.99").  
          CREATE output-list.
          output-list.sort-i = sort-i.
          sort-i = sort-i + 1.
  ...
  DO:  
    ...
    output-list.str = output-list.str +   
        STRING(translateExtended ("   :",lvCAREA,"") + " "   
...
DEF VAR i AS INTEGER NO-UNDO.  
  ...
  DO:  
    ...
    printed-line = printed-line + 1.   
  END.  
  IF foot3 NE "" THEN   /*FDL: A534AA*/
  DO:  
    output-list.str-pos = 19.  
    output-list.str = output-list.str + STRING(foot3).  
    output-list.pos = 6.  
    CREATE output-list.
    output-list.sort-i = sort-i.
    sort-i = sort-i + 1.
...
DEF VAR i           AS INTEGER            NO-UNDO.  
/*DEF VAR f-discArt   AS INTEGER INITIAL -1 NO-UNDO.   */
