...
DO:
    ...
    END.
END.
ELSE IF count-i = 0 THEN DO: /*ITA Tiket 151CBF*/

    FIND FIRST mbill WHERE mbill.resnr EQ res-number 
        AND mbill.reslinnr EQ 0 AND mbill.saldo = 0
        AND mbill.zinr EQ "" AND mbill.flag EQ 0 NO-LOCK NO-ERROR. 
    IF AVAILABLE mbill THEN
    DO:
        FIND CURRENT mbill EXCLUSIVE-LOCK.
        ASSIGN 
            mbill.flag  = 1
            fl-error   = 3 /*Message to UI if Bill Moved To CLose BIll \ MASDOD181120*/
            billnumber = billnumber + ";" + STRING(mbill.rechnr)
            .
...
IF count-i = 0 THEN DO: /*ITA Tiket 151CBF*/
    FIND FIRST mbill WHERE mbill.resnr EQ res-number 
        AND mbill.reslinnr EQ 0 AND mbill.saldo = 0
        AND mbill.zinr EQ "" AND mbill.flag EQ 0 NO-LOCK NO-ERROR. 
    IF AVAILABLE mbill THEN
...
DO:
    RUN intevent-1.p(1, res-line.zinr, "CancelCI", res-line.resnr, res-line.reslinnr).
END.
