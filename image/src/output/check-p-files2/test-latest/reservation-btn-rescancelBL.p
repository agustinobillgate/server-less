...
DEF OUTPUT PARAMETER cancel-str  AS CHAR     NO-UNDO INIT "".
...
IF NOT AVAILABLE res-line THEN RETURN. /* Malik serverless 407 */

IF curr-select = "res-line" THEN RUN cancel-resline.
ELSE RUN cancel-mainres.

PROCEDURE cancel-resline:
    IF NOT AVAILABLE res-line THEN RETURN.
    IF res-line.l-zuordnung[3] = 1 THEN
    DO:
      msg-str = translateExtended ("Accompanying Guest can not be cancelled.",lvCAREA,"").
      RETURN.
    END.

    IF res-line.active-flag = 1 OR res-line.resstatus = 6 
      OR res-line.resstatus = 13 THEN   /* Inhouse Guest */ 
    DO: 
      msg-str = translateExtended( "Cancellation of Inhouse-Guests is not possible.", lvCAREA, "":U).
      RETURN. 
    END. 

    FIND FIRST htparam WHERE paramnr = 437 NO-LOCK.
    IF htparam.flogical = NO AND res-line.betrieb-gast > 0 THEN 
    DO: 
    ...
    FIND FIRST queasy WHERE queasy.KEY = 32 AND queasy.char3 NE ""
      NO-LOCK NO-ERROR.
    IF AVAILABLE reservation THEN cancel-str = reservation.vesrdepot2.
    ASSIGN
      error-flag  = NO
      queasy-flag = AVAILABLE queasy
      msg-str = "&Q"
            + translateExtended( "Do you really want to CANCEL the reservation:",lvCAREA, "":U) 
            + CHR(10) 
...
PROCEDURE cancel-mainres:
  ...
  DO: 
    msg-str = translateExtended( "Cancellation not possible, In-House guest exists.", lvCAREA, "":U). 
    RETURN. 
  END. 
  ...
  DO: 
    msg-str = translateExtended ("KeyCard has been generated for",lvCAREA,"") 
        ...
        + CHR(10)
        + translateExtended ("Cancellation for all members no longer possible.",lvCAREA,""). 
    RETURN. 
  END. 
  ...
  FIND FIRST guest WHERE guest.gastnr = reservation.gastnr NO-LOCK NO-ERROR. /* Malik Serverless : NO-LOCK : NO-LOCK NO-ERROR */
  IF AVAILABLE guest THEN 
  DO:
    ASSIGN
      msg-str = "&Q"
      + translateExtended( "Do you really want to CANCEL the main reservation of", lvCAREA, "":U) 
      + CHR(10)
  ...
  ASSIGN
    error-flag  = NO
    queasy-flag = AVAILABLE queasy
    cancel-str  = reservation.vesrdepot2.
    /*
    msg-str = "&Q"
            + translateExtended( "Do you really want to CANCEL the main reservation of", lvCAREA, "":U) 
            + CHR(10)
...
DEFINE BUFFER resline1     FOR res-line. 
  ...
  IF deposit AND reservation1.bestat-datum = ? /* guest mot checked-in yet */ THEN /* Malik Serverless : reservation1.bestat-dat -> reservation1.bestat-datum */
  DO: 
    ...
    IF AVAILABLE resline1 THEN RETURN. 
    /*
    /*ITA 06/03/23 Validasi room sharer*/
    FIND FIRST resline1 WHERE resline1.resnr = resno
          AND resline1.reslinnr = reslinno
          AND (resline1.resstatus = 11 OR resline1.resstatus = 13)
          AND resline1.l-zuordnung[3] = 0  NO-LOCK NO-ERROR. 
    IF AVAILABLE resline1 THEN RETURN. 
    */
    /*
    FOR EACH resline1 WHERE resline1.resnr = resno 
        AND resline1.active-flag = 0
        AND resline1.resstatus LE 6
        AND resline1.l-zuordnung[3] = 0 NO-LOCK: 
      anzahl = anzahl + 1. 
    END.
    */
    FOR EACH resline1 WHERE resline1.resnr = resno 
        AND resline1.active-flag = 0
        AND resline1.resstatus LE 6 NO-LOCK: 
      anzahl = anzahl + 1. 

      /*FDL March 29, 2023 => Ticket 375AB8*/
      IF (resline1.resstatus EQ 11 OR resline1.resstatus EQ 13 AND resline1.l-zuordnung[3] EQ 1)
        OR (resline1.resstatus EQ 11 OR resline1.resstatus EQ 13 AND resline1.l-zuordnung[3] EQ 0) THEN
      DO:
        anzahl = 1.
        LEAVE.
      END.
