...
DEFINE VARIABLE supplier-name       AS CHAR.
...
/*FDL Dec 11, 2024: Ticket 16AD8D*/
FIND FIRST l-kredit WHERE l-kredit.lscheinnr EQ invoice AND l-kredit.zahlkonto EQ 0 NO-LOCK NO-ERROR. 
IF AVAILABLE l-kredit THEN
DO:
    FIND FIRST l-lieferant WHERE l-lieferant.lief-nr EQ l-kredit.lief-nr NO-LOCK NO-ERROR.
    IF AVAILABLE l-lieferant THEN supplier-name = l-lieferant.firma.
    
    fl-code = 14.
    msg-str  = "A/P with same Delivery Number exists:"
        + " " + STRING(l-kredit.rgdatum)  + " " + supplier-name.
    err-code = "14 - " + msg-str.
    RETURN.
END.
