...
DEF OUTPUT PARAMETER vhp-licensedate AS DATE         NO-UNDO.

/* New VHP DB for new VHP setup */
DEF OUTPUT PARAMETER vhp-newDB   AS LOGICAL          NO-UNDO.

/* The hour to replace 6PM reservation status */
DEFINE VARIABLE i-param297       AS INTEGER          NO-UNDO.


/* All sales articles in forein Currency = double currency */ 
DEFINE VARIABLE foreign-currency AS LOGICAL          NO-UNDO.

DEFINE VARIABLE lstopped         AS LOGICAL          NO-UNDO.
DEFINE VARIABLE h-name           AS CHAR             NO-UNDO.
DEFINE VARIABLE h-city           AS CHAR             NO-UNDO.
...
RUN decode-strings (OUTPUT lstopped).  
/* paramtext FOR hotel name, city AND lic NO */ 
IF lstopped THEN 
DO:    
    error-code = 1002.
    RETURN.
...
DEFINE OUTPUT PARAMETER lstopped AS LOGICAL NO-UNDO INITIAL NO.
