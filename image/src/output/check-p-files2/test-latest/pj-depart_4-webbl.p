...
DEFINE INPUT  PARAMETER ota-only        AS LOGICAL.     /* Rulita 12/07/24 | Req vhpcloud filter ota-only fe andito */
...
DEFINE INPUT  PARAMETER TABLE FOR zikat-list.   /*FD Jan 06, 2022 => Req Prime Hotle Suite Plaza*/
...
PROCEDURE create-departure: 
    ...
    DO:
        ...
        BY reservation.groupname BY res-line.name BY res-line.zinr: 
            ...
            IF res-line.resstatus = 8 
            AND (res-line.ankunft = res-line.abreise) THEN 
            DO: 
            ...
            /* Rulita 12/07/24 | Req vhpcloud filter ota-only fe andito */
            IF ota-only THEN
            DO:
                FIND FIRST g-ota WHERE g-ota.gastnr EQ reservation.gastnr
                    AND g-ota.karteityp EQ 2 AND g-ota.steuernr NE "" NO-LOCK NO-ERROR.
                IF AVAILABLE g-ota THEN
                DO:
                    do-it = YES.
                    IF g-ota.NAME MATCHES "*e1-booking*" THEN do-it = NO.                
                END.
                ELSE
                DO:
                    do-it = NO.
            ...
            DO: 
                ...
                ASSIGN 
                    ...
                    cl-list.tot-night = res-line.anztage /* add field length of stay by Oscar (19 September 2024) - 7329DB */
                    .
                ...
                IF AVAILABLE setup-list THEN cl-list.rmcat = zikat-list.kurzbez + setup-list.char .
                ELSE cl-list.rmcat = zikat-list.kurzbez.
                ...
                IF gmember.telefon NE "" THEN
                    cl-list.company   = cl-list.company + ";" + gmember.telefon + ";" + gmember.mobil-telefon. /*Alder - Ticket 13A495*/
                */
                
                /* Rulita 18/07/24 | 1B8E60 Fixing coondition if phone is null */
                IF gmember.telefon NE "" AND gmember.mobil-telefon NE "" THEN
                    cl-list.company   = cl-list.company + ";" + gmember.telefon + ";" + gmember.mobil-telefon. 
                ELSE IF gmember.telefon NE "" AND gmember.mobil-telefon EQ "" THEN
                    cl-list.company   = cl-list.company + ";" + gmember.telefon. 
                ELSE IF gmember.telefon EQ "" AND gmember.mobil-telefon NE "" THEN
                    cl-list.company   = cl-list.company + "; ;" + gmember.mobil-telefon. 
                
                /**//* Rulita 18/07/24 | 1B8E60 Fixing condition if etd is 0000 */
                ...
                */
                ...
                /* move total counting to fixed cl-list by Oscar (10 Oktober 2024) - CB0F52 */
                /* tot-a = tot-a + res-line.erwachs + res-line.gratis. 
            ...
            DO: 
                /* Rulita 191224 | Fixing serverless issue git 257 */
                ...
                FIND FIRST cl-list WHERE  cl-list.rmno EQ res-line.zinr 
                    ...
                    AND cl-list.zipreis EQ 0 
                    AND (cl-list.a + cl-list.c) LT 1
    ...
    FOR EACH cl-list WHERE (cl-list.a + cl-list.co) GT 0 BY cl-list.nation BY cl-list.bezeich: 
        /* move total counting to fixed cl-list by Oscar (10 Oktober 2024) - CB0F52 */
        tot-a = tot-a + cl-list.a + cl-list.co. 
        tot-c = tot-c + cl-list.c. 
        tot-co = tot-co + cl-list.co.
    ...
    FOR EACH s-list WHERE s-list.nat NE "": 
        ...
        s-list.proz = s-list.adult / tot-a * 100. 
    END. 
END. 
...
PROCEDURE create-expected: /* OK */ 
    ...
    DO:
        FOR EACH res-line WHERE res-line.active-flag = 1 
            AND (res-line.resstatus = 6 OR resstatus = 13 OR res-line.resstatus = 11) 
            AND res-line.abreise = curr-date 
            AND ((res-line.zinr GE froom AND res-line.zinr LE troom)
                OR (res-line.zinr GE froom)) NO-LOCK, 
            /*FIRST zimkateg WHERE zimkateg.zikatnr = res-line.zikatnr NO-LOCK, */
