...
DO:
    /*MESSAGE "Masuk blok apabila from-date & to-date masih dibulan berjalan" VIEW-AS ALERT-BOX INFO.*/
            ...
            */
            ...
            DO: 
                ...
                FOR EACH l-op WHERE l-op.lager-nr = l-lager.lager-nr 
                   ...
                   AND (datum GE from-date AND datum LE to-date) 
            ...
            */
            ...
            DO: 
                ...
                FOR EACH l-ophis WHERE l-ophis.lager-nr = l-lager.lager-nr 
                   ...
                   AND (datum GE from-date AND datum LE to-date) NO-LOCK BY l-ophis.datum: 
...
FOR EACH l-lager WHERE l-lager.lager-nr GE from-lager 
    ...
    FOR EACH art-bestand WHERE art-bestand.lager = l-lager.lager-nr AND 
        art-bestand.artnr GE from-art AND art-bestand.artnr LE to-art NO-LOCK 
        BY art-bestand.zwkum BY art-bestand.bezeich: 
        IF (NOT move-art) OR art-bestand.incoming NE 0 OR 
            art-bestand.outgoing NE 0 THEN 
        DO: 
            IF zwkum = 0 THEN 
            DO: 
            ...
            DO: 
                ...
                */
                IF NOT long-digit THEN 
                    ASSIGN stockoh-list.bezeich = "Ttl " + STRING(SUBSTR(bezeich,1,9), "x(9)") 
            ...
            IF art-bestand.avrg-pr = 0 THEN 
                ...
                ELSE 
                    ASSIGN  art-bestand.avrg-pr = l-artikel.vk-preis
                            art-bestand.actval  = 0.
            
            CREATE stockoh-list. 
            IF NOT long-digit THEN 
                ASSIGN stockoh-list.artnr    = STRING(art-bestand.artnr, "9999999")
...
FIND FIRST printcod WHERE printcod.emu="Epson" AND code="rs" NO-ERROR. 
IF AVAILABLE printcod THEN PUT TRIM(Contcode). 
