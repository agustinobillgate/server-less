...
FOR EACH l-op WHERE /*MTl-op.docu-nr GE ponum 
    ...
    AND l-op.loeschflag = 0 AND l-op.flag 
    AND l-op.pos GE 1 NO-LOCK, 
    FIRST l-lieferant WHERE l-lieferant.lief-nr = l-op.lief-nr NO-LOCK, 
    FIRST l-artikel WHERE l-artikel.artnr = l-op.artnr 
    AND l-op.pos GT 0 AND l-op.op-art = 1 NO-LOCK BY l-op.docu-nr:
    CREATE q2-list.
    ASSIGN
    ...
    DO:
        IF queasy.deci1 NE 0 OR queasy.deci2 NE 0 THEN q2-list.einzelpreis = q2-list.einzelpreis + queasy.deci1 + queasy.deci2.
        IF queasy.deci3 NE 0 THEN q2-list.einzelpreis = q2-list.einzelpreis - queasy.deci3.
    END.
END.
