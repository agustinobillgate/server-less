...
DEFINE VARIABLE tmp-int   AS INTEGER NO-UNDO.                   /* Rulita 211124 | Fixing for serverless */
...
DO:
  ...
  .
  tmp-date = curr-date + 29.                                          /* Rulita 211124 | Fixing for serverless */
  IF tmp-date GT co-date THEN 
      /* num-day = co-date - curr-date. */
      IF MONTH(co-date) EQ MONTH(curr-date) 
        AND YEAR(co-date) EQ YEAR(curr-date) THEN
        DO:
          num-day = DAY(co-date) - DAY(curr-date).
        END.
        ELSE
        DO:
          num-day = co-date - curr-date.
...
i = 1.
tmp-int = num-day + 1.                                               /* Rulita 211124 | Fixing for serverless */
DO WHILE i LE tmp-int: 
    CREATE date-list.
    ASSIGN
        date-list.counter = i
...
CASE op-type:
    ...
    /*    Rulita 211124 | Fixing for serverless
    WHEN 1 THEN
        RUN design-lnl.
    WHEN 2 THEN
        RUN print-lnl.
...
DEFINE VARIABLE f-date    AS DATE. /* Malik Serverless 143 fdate -> f-date */
...
DEFINE BUFFER buff-ratelist FOR rate-list.
  ...
  f-date = curr-date. /* Malik Serverless 143 fdate -> f-date */
  tdate = curr-date + num-day.
  /* Malik Serverless 143 */
  IF tdate NE ? AND co-date NE ? THEN
  DO:
  ...
  FOR EACH room-avail-list:
      ...
      /* Malik Serverless 143 fdate -> f-date */
      RUN available-ratesbl.p(f-date, tdate, room-avail-list.zikatnr,
      ...
      FOR EACH rate-list BY rate-list.i-counter:
        ...
        DO:
          ...
          ASSIGN 
              ...
              room-list.rcode       = rate-list.ratecode /* Malik Serverless 143 : rate-list.rateCode -> rate-list.ratecode */
              ...
              room-list.bezeich     = rate-list.ratecode + "/" /* Malik Serverless 143 : rate-list.rateCode -> rate-list.ratecode */
          ...
          FIND FIRST queasy WHERE queasy.KEY = 2
              AND queasy.char1 = rate-list.ratecode NO-LOCK. /* Malik Serverless 143 : rate-list.rateCode -> rate-list.ratecode */
          ...
          loopRate = 0.
          DO datum = f-date TO tdate: /* Malik Serverless 143 fdate -> f-date */
              
              loopRate = loopRate + 1.
              ASSIGN 
                 room-list.datum[loopRate]  = datum
                 room-list.prcode[loopRate] = rate-list.statcode[loopRate] /* Malik Serverless 143 : rate-list.statCode -> rate-list.statcode */
...
DEFINE INPUT PARAMETER f-date AS DATE. /* Malik Serverless 143 fdate -> f-date */
    ...
    tdate = f-date + num-day. /* Malik Serverless 143 fdate -> f-date */
    /* Malik Serverless 143 */
    IF tdate NE ? AND co-date NE ? THEN
    DO:
      IF tdate GT co-date THEN tdate = co-date.
    ...
    FOR EACH res-line WHERE NOT (res-line.abreise LT f-date) AND NOT (res-line.ankunft GT tdate) /* Malik Serverless 143 fdate -> f-date */
        AND res-line.resstatus NE 9 AND res-line.resstatus NE 10 
                    ...
                    FOR EACH tmp-resline BY tmp-resline.resnr :
                        FOR EACH fixleist WHERE fixleist.resnr = tmp-resline.resnr 
                            ...
                            DO:
                                ...
                                DO:
                                    IF tmp-resline.ankunft < f-date THEN /* Malik Serverless 143 fdate -> f-date */
                                        bdate = f-date. /* Malik Serverless 143 fdate -> f-date */
                                    ELSE
                                ...
                                DO:
                                    ...
                                    IF fixleist.lfakt < f-date THEN /* Malik Serverless 143 fdate -> f-date */
                                        bdate = f-date. /* Malik Serverless 143 fdate -> f-date */
                                    ELSE
                        ...
                        FOR EACH reslin-queasy WHERE reslin-queasy.key = "fargt-line" AND reslin-queasy.resnr = tmp-resline.resnr 
                            ...
                            PUT reslin-queasy.number3 tmp-resline.zinr tmp-resline.ankunft tmp-resline.abreise reslin-queasy.date1 reslin-queasy.date2 f-date tdate SKIP. /* Malik Serverless 143 fdate -> f-date */
    
                            IF reslin-queasy.date1 < f-date THEN /* Malik Serverless 143 fdate -> f-date */
                                bdate = f-date. /* Malik Serverless 143 fdate -> f-date */
                            ELSE
    ...
    ndate = f-date. /* Malik Serverless 143 fdate -> f-date */
        ...
        FOR EACH temp-art :
            ...
            DO:
                ...
                DO i = 1 TO 30:                    
                    ...
                    ndate = f-date + i. /* Malik Serverless 143 fdate -> f-date */
                    tot-used = 0.
                END. 
            END.
        END.
...
DEFINE BUFFER kline FOR kontline.
  ...
  /* Malik Serverless 143 */
  IF to-date NE ? AND from-date NE ? THEN
  DO:
    /*DODY 291223 enhance stuck trace*/
    FOR EACH kontline WHERE kontline.betriebsnr = 0 
      AND kontline.ankunft LE to-date AND kontline.abreise GE from-date 
...
/*** %%% ***/    
  FOR EACH res-line WHERE resstatus LE 6 /*AND resstatus NE 3*/ 
    ...
    AND NOT res-line.ankunft GT tdate AND NOT res-line.abreise LE f-date /* Malik Serverless 143 fdate -> f-date */
    AND res-line.l-zuordnung[3] = 0 NO-LOCK:
  ...
  DO:
      ...
      DO: 
          ...
          DO: 
            ...
            ASSIGN
                ...
                room-avail-list.bezeich = zimkateg.kurzbez 
                  + " - " + STRING(zimkateg.overbooking,">>9") /* Malik Serverless 143 : zimkateg.overbook -> zimkateg.overbooking */
            . 
          END. 
        END. 
        ...
        DO WHILE i LE tmp-int: 
            FOR EACH res-line WHERE res-line.active-flag LE 1 
                  ...
                  IF AVAILABLE room-avail-list THEN DO:
                      ...
                      IF do-it THEN room-avail-list.room[i] = room-avail-list.room[i] - res-line.zimmeranz. 
                  END.                                     
            END. 
        ...
        /* Malik Serverless 143 */
        IF to-date NE ? AND co-date NE ? THEN
        DO:
...
/*  occupied */ 
      ...
      DO: 
        ...
        /* Malik Serverless 143 */
        IF datum1 NE ? AND co-date NE ? THEN
        DO:
...
/* will be vacant */ 
  ...
  FOR EACH room-avail-list WHERE room-avail-list.bezeich NE "": 
    DO i = 1 TO 30: 
      room-avail-list.coom[i] = STRING(room-avail-list.room[i], "->>>,>>>,>>9"). /* Malik Serverless 143 : coom[i] = STRING(room[i], "->>>,>>>,>>9") -> room-avail-list.coom[i] = STRING(room-avail-list.room[i], "->>>,>>>,>>9") */ 
    END. 
  END. 
