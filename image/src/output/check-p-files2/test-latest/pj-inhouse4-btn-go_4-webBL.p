...
DEFINE TEMP-TABLE company-list 
    FIELD flag          AS INTEGER 
    FIELD karteityp     AS INTEGER 
    FIELD nr            AS INTEGER   FORMAT ">>>9" 
    FIELD vip           AS CHAR      FORMAT "x(5)" 
...
DEFINE OUTPUT PARAMETER TABLE FOR inhouse-guest-list.
...
DEFINE VARIABLE prog-name       AS CHAR INIT "PJ-inhouse2".
...
FOR EACH output-list BY output-list.inhousedate:
...
FOR EACH tmplist WHERE tmplist.rmno GE froom AND tmplist.rmno LE troom BY tmplist.inhousedate:
