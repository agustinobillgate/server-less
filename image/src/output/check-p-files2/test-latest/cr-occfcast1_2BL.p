...
DEFINE VARIABLE tot-room1       AS INTEGER. 
...
bfast-art = finteger.
  ...
  FOR EACH genstat WHERE genstat.res-date[2] GE datum1
    ...
    FIRST segment WHERE segment.segmentcode = genstat.segmentcode 
      BY genstat.res-date[2] BY genstat.zinr BY genstat.datum:
        ...
        IF curr-zinr NE genstat.zinr THEN DO:
            ASSIGN do-it = YES.
            ...
            IF do-it THEN DO:  
                ...
                FIND FIRST room-list WHERE room-list.datum = genstat.res-date[2] EXCLUSIVE-LOCK NO-ERROR.
  ...
  FOR EACH genstat WHERE genstat.datum GE datum1 AND genstat.datum LE d2
      ...
      IF excl-compl AND do-it THEN DO:
          ...
          ELSE DO:
              IF genstat.zipreis = 0 AND genstat.gratis NE 0
                ...
                AND genstat.res-logic[2] THEN ASSIGN do-it = NO.
          END.
      END.
      ...
      IF do-it THEN
      DO: 
          FIND FIRST room-list WHERE room-list.datum = genstat.datum. 

          IF genstat.res-date[1] = genstat.datum 
             AND genstat.resstatus NE 13 THEN DO:
              ASSIGN
          ...
          IF show-rev EQ 1 THEN DO:

              ASSIGN room-list.lodg[4] = room-list.lodg[4] + ROUND(genstat.logis, price-decimal)
                     t-lodg[4]         = t-lodg[4]         + ROUND(genstat.logis, price-decimal)
                     room-list.lodg[5] = room-list.lodg[5] + ROUND(genstat.logis, price-decimal)  /*+ genstat.res-deci[2] + genstat.res-deci[3] + genstat.res-deci[4] + genstat.res-deci[5]*/
                     t-lodg[5]         = t-lodg[5]         + ROUND(genstat.logis, price-decimal)  /*+ genstat.res-deci[2] + genstat.res-deci[3] + genstat.res-deci[4] + genstat.res-deci[5]*/
                     room-list.rmrate  = room-list.rmrate + genstat.zipreis
                     .
          END.
          ELSE
          IF show-rev EQ 2 THEN
              ASSIGN room-list.lodg[4] = room-list.lodg[4] + ROUND(genstat.logis, price-decimal)
                     t-lodg[4]         = t-lodg[4]         + ROUND(genstat.logis, price-decimal)
                     room-list.lodg[5] = room-list.lodg[5] + ROUND(genstat.logis, price-decimal)  
                     t-lodg[5]         = t-lodg[5]         + ROUND(genstat.logis, price-decimal) 
                     room-list.lodg[6] = room-list.lodg[4] / exchg-rate
                     t-lodg[6]         = t-lodg[6] + (ROUND(genstat.logis, price-decimal) / exchg-rate)
                     room-list.lodg[7] = room-list.lodg[5] / exchg-rate
                     t-lodg[7]         = t-lodg[7] + (ROUND(genstat.logis, price-decimal) / exchg-rate)
                     room-list.rmrate  = room-list.rmrate + genstat.zipreis
          ...
          FIND FIRST arrangement WHERE arrangement.arrangement = genstat.argt NO-LOCK NO-ERROR. /*ft*/
          FIND FIRST artikel WHERE artikel.artnr = arrangement.argt-artikelnr 
              AND artikel.departement = 0 NO-LOCK NO-ERROR. 
    
          /*wen*/
          RUN calc-servtaxesbl.p (1, artikel.artnr, artikel.departement, curr-date, OUTPUT serv1, OUTPUT vat1, OUTPUT vat2,
                                  OUTPUT fact1).
          
          /*MT breakfast lunch dinner other */
          ASSIGN
              room-list.other[1] = room-list.other[1] + genstat.res-deci[2] 
              room-list.other[2] = room-list.other[2] + genstat.res-deci[3]
              room-list.other[3] = room-list.other[3] + genstat.res-deci[4]
              /*room-list.other[4] = room-list.other[4] + genstat.res-deci[5] + genstat.res-deci[6]*/
              room-list.other[5] = room-list.other[1] / exchg-rate
              room-list.other[6] = room-list.other[2] / exchg-rate
              room-list.other[7] = room-list.other[3] / exchg-rate
              room-list.other[8] = room-list.other[4] / exchg-rate
              .
          room-list.other[4] = room-list.other[4] + ROUND(((genstat.rateLocal / (1 + vat1 + vat2 + serv1)) - (genstat.logis + genstat.res-deci[2] + genstat.res-deci[3] + genstat.res-deci[4])),price-decimal).
          IF genstat.res-date[1] = genstat.res-date[2] /* day use */
            AND NOT rmSharer 
            AND genstat.res-logic[2] THEN /* of active-room */
          DO:
  ...
  FOR EACH res-line WHERE ((res-line.resstatus LE 13 
    ...
    DO: 
      ...
      DO datum = datum1 TO datum2: 
        ...
        FIND FIRST room-list WHERE room-list.datum = datum NO-LOCK NO-ERROR. 
        consider-it = YES. 
        ...
        ELSE DO:
            ...
            ASSIGN net-lodg = ROUND(net-lodg, price-decimal)
                   tot-breakfast = ROUND(tot-breakfast, price-decimal)
                   tot-lunch = ROUND(tot-lunch, price-decimal)
                   tot-dinner = ROUND(tot-dinner, price-decimal)
                   tot-other = ROUND(tot-other, price-decimal)
                 .
            
            /* END Breakfast lunch Dinner other From Room Rev*/
            ASSIGN room-list.other[1] = room-list.other[1] + tot-breakfast
                   room-list.other[2] = room-list.other[2] + tot-lunch
                   room-list.other[3] = room-list.other[3] + tot-dinner
                   room-list.other[4] = room-list.other[4] + tot-other
                   room-list.other[5] = room-list.other[1] / exchg-rate
                   room-list.other[6] = room-list.other[2] / exchg-rate    
                   room-list.other[7] = room-list.other[3] / exchg-rate   
                   room-list.other[8] = room-list.other[4] / exchg-rate . 
...
*/
  ...
  FOR EACH room-list WHERE room-list.wd NE 0: 
    ...
    /*ITA 18/06/24 : OCC dikurangi OOO*/
    IF exclOOO THEN DO:
        RUN get-active-room(room-list.datum, OUTPUT tot-room).
        ASSIGN
            tot-room1       = tot-room - room-list.room[15]
            accum-tot-room = accum-tot-room + tot-room1.

        IF tot-room1 NE 0 THEN
        DO:
          room-list.room[9] = room-list.room[7] / tot-room1 * 100. 
          prev-room = prev-room + room-list.room[7]. 
          room-list.room[10] = prev-room / accum-tot-room * 100. 
        END.
        
        IF (room-list.room[7] - anzahl-dayuse) LT tot-room1 THEN 
        DO: 
          room-list.room[11] = tot-room1 - room-list.room[7] + anzahl-dayuse. 
          rm-array[11] = rm-array[11] + tot-room1 - room-list.room[7] + anzahl-dayuse. 
        END. 
        ELSE 
        DO: 
          room-list.room[12] = room-list.room[7] - tot-room1 - anzahl-dayuse. 
          rm-array[12] = rm-array[12] + room-list.room[7] - tot-room1 - anzahl-dayuse. 
        END. 
  ...
  IF exclOOO THEN DO: 
     IF mtd-tot-room NE 0 THEN
        mtd-occ = rm-array[7] / (mtd-tot-room - rm-array[15]) * 100.
  END.
  ELSE DO:
      IF mtd-tot-room NE 0 THEN
  ...
  FOR EACH room-list WHERE room-list.wd NE 0:
      ...
      sum-breakfast = sum-breakfast + room-list.other[1].
      sum-lunch     = sum-lunch + room-list.other[2].
      sum-dinner    = sum-dinner + room-list.other[3].
      sum-other     = sum-other + room-list.other[4].

      sum-breakfast-usd = sum-breakfast-usd + room-list.other[5].
      sum-lunch-usd     = sum-lunch-usd + room-list.other[6].
      sum-dinner-usd    = sum-dinner-usd + room-list.other[7].
      sum-other-usd     = sum-other-usd + room-list.other[8].
  ...
  ASSIGN
      ...
      room-list.avrglodg2  = avrg-lodging2 / (to-date - curr-date + 1)
      room-list.avrgrmrev  = tavg-rmrev  /*(to-date - curr-date + 1)*/
      room-list.avrgrmrev2 = tavg-rmrev2
      room-list.other[1]   = sum-breakfast 
      room-list.other[2]   = sum-lunch   
      room-list.other[3]   = sum-dinner
      room-list.other[4]   = sum-other 
      room-list.other[5]   = sum-breakfast-usd 
      room-list.other[6]   = sum-lunch-usd   
      room-list.other[7]   = sum-dinner-usd
      room-list.other[8]   = sum-other-usd 
...
PROCEDURE argt-code-name:
    ...
    DO:              
        ...
        DO:
           CREATE print-list2.
           argm = SUBSTRING(argm-name,r + 1,a). 
           IF SUBSTRING(argm, 1, 1) = " " THEN
               argm = SUBSTRING(argm, 2, (LENGTH(argm) - 1)).
        END.
        ELSE
 ...
 PROCEDURE room-code-name:
   ...
   IF all-zikat /*:SCREEN-VALUE IN FRAME FRAME1= "Yes"*/ THEN
    ...
    DO:              
        ...
        DO:
           CREATE print-list3.
           room = SUBSTRING(room-name,r + 1,a). 
           IF SUBSTRING(room, 1, 1) = " " THEN
              room = SUBSTRING(room, 2, (LENGTH(room) - 1)).
        END.
        ELSE
...
/* NOTES: Tentative reservatio is needed here TO GET the sum, but does NOT 
  ...
  /*OUTPUT TO "C:\sources10\P-WORK\test.txt".*/
  FOR EACH res-line WHERE (res-line.active-flag LE 1 
    ...
    AND res-line.resstatus NE 12
    AND NOT (res-line.ankunft GT to-date) 
    AND NOT (res-line.abreise LT curr-date)) OR
    ((res-line.active-flag = 2 AND res-line.resstatus = 8
    AND res-line.ankunft = ci-date AND res-line.abreise = ci-date) OR 
    (res-line.active-flag = 2 AND res-line.resstatus = 8 
    AND res-line.abreise = ci-date))
    ...
    /*BY res-line.gastnr BY res-line.resnr BY res-line.zinr*/ BY res-line.resnr: /*MT 13/08/12 */
   ...
   .
    ...
    DO: 
      ...
      DO datum = datum1 TO datum2: 
        ...
        /*
        IF net-lodg NE 0 THEN
            PUT res-line.resnr ";" res-line.zinr ";" ROUND(net-lodg, price-decimal) FORMAT "->>>,>>>,>>>,>>9.99" 
                ";" ROUND(tot-breakfast, price-decimal) FORMAT "->>>,>>>,>>>,>>9.99" SKIP.*/
...
/*             DISP "1" res-line.resnr room-list.lodg[4] net-lodg FORMAT "->>>,>>>,>>>,>>9.99". */
...
/*               DISP "2" res-line.resnr room-list.lodg[4] net-lodg FORMAT "->>>,>>>,>>>,>>9.99". */
          END.
 ...
 /*OUTPUT STREAM s1 CLOSE.*/
  ...
  /*
  IF exclOOO THEN
      mtd-occ = rm-array[7] / ((tot-room - rm-array[15]) * (to-date - curr-date + 1)) * 100.
  ELSE mtd-occ = rm-array[7] / (tot-room * (to-date - curr-date + 1)) * 100.*/

  IF exclOOO THEN
      mtd-occ = rm-array[7] / (accum-tot-room) * 100.
  ELSE mtd-occ = rm-array[7] / (tot-room * (to-date - curr-date + 1)) * 100.
