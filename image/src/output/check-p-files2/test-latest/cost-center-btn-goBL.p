...
IF case-type = 1 THEN RUN add-costlist.
ELSE IF case-type = 2 THEN RUN update-costlist.
...
PROCEDURE update-costlist: 
  /*FIND FIRST parameters WHERE RECID(parameters) = num1 EXCLUSIVE-LOCK.*/
  /* Malik Serverless 773 change query for update */
  FIND FIRST parameters WHERE RECID(parameters) = num1 NO-LOCK NO-ERROR.
