...
DEFINE VARIABLE debt        AS DECIMAL FORMAT "->>,>>>,>>>,>>9.99".
DEFINE VARIABLE crdt        AS DECIMAL FORMAT "->>,>>>,>>>,>>9.99".
DEFINE VARIABLE fdebt       AS DECIMAL FORMAT "->>,>>>,>>>,>>9.99".
DEFINE VARIABLE fcrdt       AS DECIMAL FORMAT "->>,>>>,>>>,>>9.99".
...
DEFINE VARIABLE t-voucherno AS CHARACTER NO-UNDO. /*naufal afthar - D2A668*/
      ...
      FOR EACH debitor WHERE debitor.gastnr = guestno 
        ...
        DO:              
          ...
          ASSIGN soa-list.to-sort    = 1 
              ...
              soa-list.foreign-exchg = soa-list.saldo / dollar-rate.
        END.
      END.
      ...
      DO WHILE AVAILABLE soa-list:
        ...
        FOR EACH soabuff WHERE soabuff.done-step = 1:
          ASSIGN
            ...
            soabuff.foreign-exchg = soabuff.saldo / dollar-rate
          .
        END.
          ...
          DO:
            ...
            DO:
              ...
              IF debitor.debref NE 0 THEN DO:
                ...
                DO:
                  soa-list.printed = YES.                                 
                  soa-list.refNo   = debitor.debref.  /*geral get refno for reprint CD4450*/
                  soa-list.inv-str = "INV" + STRING(queasy.number2, "9999999"). /*geral get refno for reprint CD4450*/
                END.
            ...
            IF AVAILABLE bill-line THEN
                ASSIGN soa-list.voucherno  = ENTRY(2, bill-line.bezeich, "/") NO-ERROR.
            ...
            DO:
              ...
              DO: 
                ...
                /*naufal afthar - D2A668*/
                DO i = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch,";") - 1:
                    t-voucherno = ENTRY(i, res-line.zimmer-wunsch, ";").
                    IF SUBSTR(t-voucherno,1,7) EQ "voucher" THEN
                        ASSIGN soa-list.voucher-res-line = SUBSTR(t-voucherno,8).
                END.
                IF soa-list.voucher-res-line EQ "" THEN DO:
                    FIND FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK NO-ERROR.
                    IF AVAILABLE reservation THEN
                        ASSIGN soa-list.voucher-res-line = reservation.vesrdepot.
                END.
                ...
                IF AVAILABLE reservation THEN
                    ...
                    soa-list.resv-name   = reservation.NAME.
              END.
            END.
            ELSE DO:
                ASSIGN soa-list.remarks = soa-list.remarks + CHR(3) + CHR(3) + CHR(3) + CHR(3) + CHR(3). /*Eko Jika nonstay guest bill tidak terdapat renr dan roomcateg*/
                       soa-list.resv-name = bill.bilname.
            END.
...
/* &&& SLOW */
            ...
            DO:
              ...
              DO: 
                  ...
                  DO i = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch,";") - 1:
                      str = ENTRY(i, res-line.zimmer-wunsch, ";").
                      IF SUBSTR(str,1,7) = "voucher" THEN 
                          MESSAGE "voucher 1 from zimmer-wunsch" res-line.zimmer-wunsch.
                          soa-list.voucherNo1 = SUBSTR(str,8).
                  END.
  ...
  FOR EACH soa-list,
      ...
      DO:
        ...
        IF AVAILABLE bill-line THEN
            soa-list.voucherno  = ENTRY(2, bill-line.bezeich, "/") NO-ERROR.
            MESSAGE "voucher from bezeich: " bill-line.bezeich.
