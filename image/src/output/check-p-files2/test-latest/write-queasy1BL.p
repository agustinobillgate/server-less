...
CASE case-type:
    WHEN 1 THEN
    DO:
        FIND FIRST queasy WHERE RECID(queasy) = t-queasy.rec-id 
    ...
    WHEN 2 THEN /*For Web*/
    DO:   
        /*  Malik Serverless 321 Change query methode find first do while -> for each      
        ...
        DO WHILE AVAILABLE t-queasy:
            ...
            FIND FIRST queasy WHERE queasy.KEY EQ t-queasy.KEY
                ...
                AND queasy.char1 EQ t-queasy.char1 
                AND (queasy.deci1 NE t-queasy.deci1 
                OR queasy.deci2 NE t-queasy.deci2) NO-LOCK NO-ERROR.
            IF AVAILABLE queasy THEN
            DO:
