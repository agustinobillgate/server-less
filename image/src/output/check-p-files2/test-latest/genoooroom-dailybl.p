...
FOR EACH outorder WHERE outorder.zinr EQ roomnumber 
    AND outorder.gespstart LE sysdate /* Malik Serverless: AND sysdate GE outorder.gespstart AND sysdate LE outorder.gespende -> (Outorder.gespstart >= sysdate) &  (Outorder.gespende <= sysdate) */
    AND outorder.gespende GE sysdate 
