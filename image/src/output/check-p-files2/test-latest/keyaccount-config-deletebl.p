...
FIND FIRST queasy WHERE queasy.KEY = 212
...
DO:
    ...
    FIND FIRST b-queasy WHERE b-queasy.KEY EQ 211 AND b-queasy.number1 EQ num1 NO-LOCK NO-ERROR.
...
FOR EACH queasy WHERE queasy.KEY = 212 AND queasy.number1 = num1:
    ASSIGN
        nr = nr + 1
        queasy.number2 = nr.
END.
