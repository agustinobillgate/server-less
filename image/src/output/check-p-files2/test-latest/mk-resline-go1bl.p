...
FIND FIRST res-line WHERE res-line.resnr = curr-resline.resnr AND res-line.reslinnr = curr-resline.reslinnr NO-LOCK NO-ERROR. /* Malik Serverless : NO-LOCK -> NO-LOCK NO-ERROR */
...
DO WHILE AVAILABLE resline:
    ...
    /*IF do-it THEN
    DO:
       RUN check-timebl.p (1, resline.resnr, resline.reslinnr, 
           "res-line", ?, ?, OUTPUT do-it, OUTPUT init-time, 
           OUTPUT init-date).
       IF NOT do-it THEN msg-str = msg-str + "#" + STRING(resline.reslinnr,"99") + " "
           + resline.NAME + CHR(10)
