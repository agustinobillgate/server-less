...
DEF VAR curr-select    AS CHAR INIT "chg".
DEF VAR max-rate       AS INTEGER INIT 1000000000.
DEF VAR fact1          AS DECIMAL INIT 1.
DEF VAR inp-wahrnr     AS INTEGER INIT 1.
DEF VAR inp-zikatnr    AS INTEGER INIT 3.
DEF VAR user-init      AS CHAR INIT "01".
DEF VAR resnr          AS INTEGER INIT 21.
DEF VAR reslinnr       AS INTEGER INIT 1.
DEF VAR recid-reslin   AS INT INIT 2981934.
DEF VAR msg-str AS CHAR.
DEF VAR error-found1 AS LOGICAL.
CREATE p-list.
ASSIGN 
  p-list.betrag  = 0.00
  p-list.date1   = 01/01/08
  p-list.date2   = 01/30/08
  p-list.argt    = "".
  */
...
DEF BUFFER breslin   FOR reslin-queasy.
...
FIND FIRST p-list.
FIND FIRST res-line WHERE res-line.resnr = resnr AND res-line.reslinnr = reslinnr NO-LOCK. 
FIND FIRST bediener WHERE bediener.userinit = user-init NO-LOCK. 
FIND FIRST reslin-queasy WHERE RECID(reslin-queasy) = recid-reslin NO-LOCK NO-ERROR.

IF p-list.argt NE "" THEN
DO:
...
DO:
     FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
       AND bRQ.reslinnr = reslinnr AND bRQ.date1 GE p-list.date1 
       AND bRQ.date1 LE p-list.date2 NO-LOCK NO-ERROR. 
     IF NOT AVAILABLE bRQ THEN 
     FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
       AND bRQ.reslinnr = reslinnr AND bRQ.date2 GE p-list.date1 
       AND bRQ.date2 LE p-list.date2 NO-LOCK NO-ERROR. 
     IF NOT AVAILABLE bRQ THEN 
     FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
       AND bRQ.reslinnr = reslinnr AND p-list.date1 GE bRQ.date1 
       AND p-list.date1 LE bRQ.date2 NO-LOCK NO-ERROR. 
     IF NOT AVAILABLE bRQ THEN 
     FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
       AND bRQ.reslinnr = reslinnr AND p-list.date2 GE bRQ.date1 
       AND p-list.date2 LE bRQ.date2 NO-LOCK NO-ERROR. 
     IF AVAILABLE brQ THEN 
     DO: 
       msg-str = msg-str + CHR(2)
               + translateExtended ("Overlapping date found.",lvCAREA,"").
       /*MTAPPLY "entry" TO p-list.date1. */
       RETURN NO-APPLY. 
     END. 
...
/* SY 16 JUL 2017 used in mk-resline-go1bl, 
   prepare-reslineBL will update the date and time */
     IF res-line.active-flag = 0 THEN
     DO:
       FIND FIRST reslin-queasy WHERE 
         reslin-queasy.KEY      = "fixrate-trace-record" AND
         reslin-queasy.resnr    = resnr AND
         reslin-queasy.reslinnr = reslinnr EXCLUSIVE-LOCK NO-ERROR.
       IF NOT AVAILABLE reslin-queasy THEN
       DO:
         CREATE reslin-queasy.
         ASSIGN
             reslin-queasy.KEY      = "fixrate-trace-record" 
             reslin-queasy.resnr    = resnr 
             reslin-queasy.reslinnr = reslinnr
             reslin-queasy.date1    = TODAY
             reslin-queasy.number1  = TIME
             reslin-queasy.char3    = ""
         .
...
/* SY 09 Aug 2015 */    
    ...
    ASSIGN
        t-reslin-queasy.recid-reslin  = RECID(reslin-queasy).
/*
    t-reslin-queasy.date1   = reslin-queasy.date1
    t-reslin-queasy.date2   = reslin-queasy.date2
    t-reslin-queasy.deci1   = reslin-queasy.deci1
    t-reslin-queasy.char1   = reslin-queasy.char1
    t-reslin-queasy.number3 = reslin-queasy.number3
    t-reslin-queasy.char2   = reslin-queasy.char2
    t-reslin-queasy.char3   = reslin-queasy.char3
*/    
...
DO:
    FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
      AND bRQ.reslinnr = reslinnr 
      AND ROWID(brQ) NE ROWID(reslin-queasy) 
      AND bRQ.date1 GE p-list.date1 
      AND bRQ.date1 LE p-list.date2 NO-LOCK NO-ERROR. 
    IF NOT AVAILABLE bRQ THEN 
    FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
      AND ROWID(brQ) NE ROWID(reslin-queasy) 
    ...
    FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
      AND bRQ.reslinnr = reslinnr 
      AND ROWID(brQ) NE ROWID(reslin-queasy) 
      AND p-list.date1 GE bRQ.date1 
      AND p-list.date1 LE bRQ.date2 NO-LOCK NO-ERROR. 
    IF NOT AVAILABLE bRQ THEN 
    FIND FIRST bRQ WHERE bRQ.key = "arrangement" AND bRQ.resnr = resnr 
      AND bRQ.reslinnr = reslinnr 
      AND ROWID(brQ) NE ROWID(reslin-queasy) 
      AND p-list.date2 GE bRQ.date1 
      AND p-list.date2 LE bRQ.date2 NO-LOCK NO-ERROR. 
    IF AVAILABLE brQ THEN 
    DO: 
      msg-str = msg-str + CHR(2)
...
/* SY 16 JUL 2017 used in mk-resline-go1bl */
     IF res-line.active-flag = 0 THEN
     DO:
       FIND FIRST breslin WHERE 
         breslin.KEY      = "fixrate-trace-record" AND
         breslin.resnr    = resnr AND
         breslin.reslinnr = reslinnr EXCLUSIVE-LOCK NO-ERROR.
       IF NOT AVAILABLE breslin THEN
       DO:
         CREATE breslin.
         ASSIGN
             ...
             breslin.date1    = TODAY
             breslin.number1  = TIME
             breslin.char3    = ""
         .
         FIND CURRENT breslin NO-LOCK.
         RELEASE breslin.
       END.
     END.
...
DEF BUFFER rqy FOR reslin-queasy.
    ...
    IF NOT AVAILABLE res-line THEN RETURN.
    IF res-line.active-flag = 2 
        AND res-line.resstatus NE 12     /*FDL June 13, 2024 => Ticket 23DDF8*/ 
    THEN RETURN. /* res-mode = "NEW" */
