...
DO TRANSACTION: 
  ...
  /*ITA 24/04/25 - Leasing Feature*/
  FIND FIRST bqueasy WHERE bqueasy.KEY = 329
      AND bqueasy.number1 = res-line.resnr 
      AND bqueasy.number2 = res-line.reslinnr
      AND bqueasy.logi1 = NO NO-LOCK NO-ERROR.
  IF AVAILABLE bqueasy THEN DO:
       
        RUN leasing-cancel-rsvbl.p (RECID(bqueasy), user-init).

        FIND CURRENT bqueasy EXCLUSIVE-LOCK.
        ASSIGN bqueasy.logi1 = YES. /*indikator cancel*/
  ...
  /*end FT*/
   ...
   /*ITA 260525 Log Availability*/
    ...
    ASSIGN 
      ...
      res-history.aenderung = "Cancel ResLine: ResNo " + STRING(resnr) + " No " 
        + STRING(reslinnr) + " - " + res-line.NAME 
