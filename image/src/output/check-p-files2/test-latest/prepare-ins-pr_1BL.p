...
/*Alder - Serverless - Issue 529 - Start*/
...
DO:
    FIND FIRST parameters WHERE parameters.progname EQ "CostCenter"
        ...
        AND INTEGER(parameters.varname) EQ l-orderhdr.angebot-lief[1]
        NO-LOCK NO-ERROR.
    IF AVAILABLE parameters THEN
    DO:
        ASSIGN deptname = parameters.vstring.
    END.
    ASSIGN comments = l-orderhdr.lief-fax[3].
    CREATE t-l-orderhdr.
    BUFFER-COPY l-orderhdr TO t-l-orderhdr.
END.
/*Alder - Serverless - Issue 529 - End*/
...
FOR EACH l-order WHERE l-order.docu-nr = docu-nr 
    ...
    /*FIRST s-list WHERE s-list.artnr = l-order.artnr AND s-list.pos = l-order.pos BY s-list.pos DESCENDING. /*MNaufal 23112023 - add validation for remark doesn't displayed correctly*/*/
    CREATE ins-list.
    ...
    /*Alder - Serverless - Issue 529 - Start*/
    FIND FIRST s-list WHERE s-list.artnr EQ l-order.artnr
        AND s-list.pos EQ l-order.pos NO-LOCK NO-ERROR.
    ...
    /*545FBD gerald*/
    FIND FIRST l-bestand WHERE l-bestand.artnr = l-art.artnr
        AND l-bestand.lager-nr = 0 NO-LOCK NO-ERROR.
    IF AVAILABLE l-bestand THEN
    DO:
      ins-list.soh = l-bestand.anz-anf-best + l-bestand.anz-eingang
