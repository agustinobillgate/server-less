...
/* Dzikri E6E53C - repair shift column shift */
...
/* Dzikri E6E53C - repair shift column shift */
DEFINE VARIABLE shift      AS INTEGER INIT 0.
...
DEFINE BUFFER shift-buff FOR shift-list. /* Dzikri E6E53C - repair shift column shift */
...
/* Start - Oscar (21/01/25) - B6CF79 - handling concatenate string with null value */
FUNCTION handle-null-date RETURNS CHAR(inp-date AS DATE): 
    IF inp-date EQ ? THEN
        RETURN STRING("", "x(8)").
    ELSE RETURN STRING(inp-date, "99/99/99").
END FUNCTION.

FUNCTION handle-null-char RETURNS CHAR(inp-char AS CHAR):
    IF inp-char EQ ? THEN
        RETURN "".
    ELSE RETURN inp-char.
END FUNCTION.
/* End - Oscar (21/01/25) - B6CF79 - handling concatenate string with null value */
...
FOR EACH output-list:
    ...
    ASSIGN 
        ...
        fo-journal-list.segcode     = output-list.segcode
        ...
        fo-journal-list.datum       = output-list.datum
        fo-journal-list.billno      = output-list.billno
        fo-journal-list.artno       = output-list.artno
        fo-journal-list.depart      = output-list.depart
        fo-journal-list.outlet      = output-list.outlet
        fo-journal-list.qty         = output-list.qty
        fo-journal-list.amount      = output-list.amount
        fo-journal-list.zeit        = output-list.zeit
        fo-journal-list.id          = output-list.id
        fo-journal-list.sysdate     = output-list.sysdate.
...
*/
/*************** PROCEDURES ***************/
PROCEDURE custom-record-date: /*FT serverless*/
    ...
    /* Dzikri 0CD94D - Long loading
    FOR EACH output-list: */
    ASSIGN
        roomnumber = SUBSTRING(output-list.STR,9,6)
        /*zinrdate   = DATE(SUBSTRING(output-list.STR,1,8))FT serverless*/
        zinrdate   = billjournal.bill-datum
        billnumber = INT(SUBSTRING(output-list.STR,15,9))
        artikelnr  = INTEGER(SUBSTRING(output-list.STR,24,4)) /* Dzikri - FO Ticket 10/10/2024 */
        curr-str   = " "
        curr-resnr = 0
    .
    ...
    DO:
        ...
        DO:
            ...
            DO:
                ...
                FIND FIRST buffguest WHERE buffguest.gastnr EQ res-line.gastnrpay NO-LOCK NO-ERROR.
                FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember NO-LOCK NO-ERROR.
                FIND FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK NO-ERROR. /* Dzikri B6184F - Add new field */
                ...
                DO:
                    ...
                    output-list.nationality = guest.nation1. /* Dzikri B6184F - Add new field */
                    output-list.resnr       = res-line.resnr. /* Dzikri B6184F - Add new field */
                    output-list.resname     = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1. /* Dzikri 706CDB - Add reserve name */
                    /* Dzikri B6184F - Add new field */
                    ...
                    DO:  
                      ...
                      DO:  
                        output-list.book-source = sourccod.bezeich.
                      END.  
                    END.
                    /*output-list.book-source = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1. Dzikri B6184F - Add new field */
                END.
    ...
    END.
     */
    IF SUBSTRING(output-list.STR,78, 12) MATCHES "*T O T A L*" THEN 
        ASSIGN 
        output-list.guestname = ""
        output-list.segcode   = ""
        output-list.checkin   = ?  
    ...
    IF SUBSTRING(output-list.STR,78, 12) MATCHES "*Grand TOTAL*" THEN 
        ASSIGN 
        output-list.guestname = ""
        output-list.segcode   = ""
        output-list.checkin   = ?  
    ...
    DO:
        ...
        */
        IF temp-resnr NE 0 THEN DO:
            FIND FIRST res-line WHERE res-line.resnr EQ temp-resnr AND res-line.reslinnr EQ 1 NO-LOCK NO-ERROR.
            IF AVAILABLE res-line THEN
            DO:
                FIND FIRST reservation WHERE reservation.resnr EQ res-line.resnr NO-LOCK NO-ERROR.
                FIND FIRST buffguest WHERE buffguest.gastnr EQ res-line.gastnrpay NO-LOCK NO-ERROR.
    ...
    IF roomnumber NE "" AND billnumber EQ 0 AND zinrdate NE ? THEN
    DO:
        ...
        DO:
            ...
            IF AVAILABLE res-line THEN  /*FT serverless*/
            DO:
                FIND FIRST reservation WHERE reservation.resnr EQ res-line.resnr NO-LOCK NO-ERROR.
                FIND FIRST buffguest WHERE buffguest.gastnr EQ res-line.gastnrpay NO-LOCK NO-ERROR.
                FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember NO-LOCK NO-ERROR.
        ...
        DO:
            ...
            DO:
                ...
                FIND FIRST res-line WHERE reservation.resnr EQ res-line.resnr NO-LOCK NO-ERROR.
                IF AVAILABLE res-line THEN
                DO:
                    FIND FIRST buffguest WHERE buffguest.gastnr EQ res-line.gastnrpay NO-LOCK NO-ERROR.
                    FIND FIRST guest WHERE guest.gastnr EQ genstat.gastnrmember NO-LOCK NO-ERROR.
                    FIND FIRST gbuff WHERE gbuff.gastnr EQ genstat.gastnr NO-LOCK NO-ERROR.
                    IF AVAILABLE guest THEN
                    DO:
                        output-list.str        = output-list.str + guest.name + ", " + guest.vorname1 + " " + guest.anrede1.  
                        ...
                        IF AVAILABLE buffguest THEN output-list.gname = buffguest.name + ", " + buffguest.vorname1 + " " + buffguest.anrede1.
                        ELSE output-list.gname = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1.
                    
                        output-list.nationality = guest.nation1. /* Dzikri B6184F - Add new field */
                        output-list.resnr = genstat.resnr. /* Dzikri B6184F - Add new field */
                        output-list.resname     = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1. /* Dzikri 706CDB - Add reserve name */
                        /* Dzikri B6184F - Add new field */
                        IF reservation.resart NE 0 THEN   
                        DO:  
...
PROCEDURE journal-list: 
    ...
    FOR EACH artikel WHERE artikel.artnr GE from-art 
        ...
        DO curr-date = from-date TO to-date: 
            IF sorttype = 0 THEN 
                FOR EACH billjournal WHERE billjournal.artnr = artikel.artnr 
                    ...
                    AND billjournal.bill-datum = curr-date AND billjournal.anzahl NE 0 NO-LOCK BY billjournal.sysdate BY billjournal.zeit BY billjournal.zinr: 
                    it-exist = YES. 
                    do-it    = YES.
                    ...
                    DO:
                        ...
                        DO: 
                            ...
                            DO:
                                ...
                                DO:
                                    ...
                                    DO:
                                        FIND FIRST res-line WHERE res-line.resnr EQ bill.resnr AND res-line.zinr EQ bill.zinr /* Dzikri - FO Ticket 10/10/2024 : EQ roomnumber */ NO-LOCK NO-ERROR.
                                        IF AVAILABLE res-line THEN
                                        DO:
                                            FIND FIRST reservation WHERE reservation.resnr EQ res-line.resnr NO-LOCK NO-ERROR.
                                            FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember NO-LOCK NO-ERROR.
                                            FIND FIRST buffguest WHERE buffguest.gastnr EQ res-line.gastnrpay NO-LOCK NO-ERROR. /* Dzikri - FO Ticket 10/10/2024 */
                                            FIND FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK NO-ERROR. /* Dzikri B6184F - Add new field */
                                    ...
                                    DO:
                                        ...
                                        DO:
                                            ...
                                            DO:
                                                ...
                                                output-list.resname     = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1 + gbuff.anredefirma. /* Dzikri 706CDB - Add reserve name */
                                                output-list.nationality = gbuff.nation1. /* Dzikri B6184F - Add new field */
                                            END.
                                        END.                                 
                                    END.   /*available bill*/
                                END. /*bediener-nr = 0*/
                                    ...
                                    DO:
                                        ...
                                        */
                                        ...
                                        DO:
                                            ...
                                            IF AVAILABLE res-line THEN
                                            DO:
                                                FIND FIRST reservation WHERE reservation.resnr EQ res-line.resnr NO-LOCK NO-ERROR. /* Dzikri 29EBC7 - error reservation */
                                                FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember NO-LOCK NO-ERROR.
                                                FIND FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK NO-ERROR. /* Dzikri B6184F - Add new field */
                                                ASSIGN
                                                    output-list.checkin   = res-line.ankunft 
                                        ...
                                        DO:
                                            ...
                                            IF AVAILABLE bill THEN
                                            DO:
                                                FIND FIRST res-line WHERE res-line.resnr = bill.resnr AND res-line.reslinnr = bill.reslinnr NO-LOCK NO-ERROR.
                                                IF AVAILABLE res-line THEN
                                                DO:
                                                    FIND FIRST reservation WHERE reservation.resnr EQ res-line.resnr NO-LOCK NO-ERROR. /* Dzikri 29EBC7 - error reservation */
                                                    FIND FIRST guest WHERE guest.gastnr EQ res-line.gastnrmember NO-LOCK NO-ERROR.
                                ...
                                DO:
                                    ...
                                    DO:
                                        ...
                                        DO:
                                            ...
                                            FIND FIRST gbuff WHERE gbuff.gastnr EQ res-line.gastnr NO-LOCK NO-ERROR.
                                            IF AVAILABLE gbuff THEN output-list.resname = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1. /* Dzikri 706CDB - Add reserve name */
                                            ...
                                            DO:  
                                              ...
                                              END.  
                                            END.
                                            /*output-list.book-source = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1. */
                                            /* Dzikri B6184F - END */
                        ...
                        DO:
                            /* Dzikri E6E53C - repair shift column shift */
                            shift = 0.
                            /* Dzikri 0CD94D - repair shift column shift */
                            IF billjournal.betriebsnr NE 0 THEN
                            DO:
                                FIND FIRST h-journal WHERE h-journal.rechnr EQ billjournal.rechnr
                                    AND h-journal.departement EQ billjournal.betriebsnr NO-LOCK NO-ERROR.
                                IF AVAILABLE h-journal THEN
                                DO:
                                    FIND FIRST shift-list WHERE shift-list.ftime LE h-journal.zeit AND 
                                        shift-list.ttime GE h-journal.zeit NO-LOCK NO-ERROR.
                                    IF AVAILABLE shift-list THEN
                                        shift = shift-list.shift.
                                    ELSE 
                                    DO:
                                        FIND FIRST shift-buff WHERE shift-buff.ftime GE shift-buff.ttime NO-LOCK NO-ERROR.
                                        IF AVAILABLE shift-buff THEN
                                            shift = shift-buff.shift.
                                    END.                                            .
                                END.
                            END.
                            /* Dzikri 0CD94D - END */ /* Dzikri E6E53C - END */
                            ASSIGN 
                            output-list.c     = STRING(billjournal.betriebsnr,"99")
                            output-list.shift = STRING(shift,"99"). 
                        END.
                        ...
                        DO:
                            ...
                            /* Dzikri E6E53C - if not deposit dont show outlet (confuse user) */
                            IF billjournal.bezeich MATCHES("*Deposit*") AND billjournal.rechnr NE 0 THEN 
                            DO:
                                /* Oscar (21/01/25) - B6CF79 - handling concatenate string with null value */
                                IF NOT long-digit THEN output-list.STR = STRING(handle-null-date(billjournal.bill-datum), "x(8)")
                                    + STRING(handle-null-char(billjournal.zinr), "x(6)") /*MT 25/07/12 */
                                    + STRING(billjournal.rechnr, "999999999") 
                                    + STRING(billjournal.artnr, "9999") 
                                    + STRING(handle-null-char(descr1), "x(50)") 
                                    + STRING(handle-null-char(deptname), "x(12)") 
                                    ...
                                    + STRING(handle-null-char(billjournal.userinit),"x(4)") 
                                    + STRING(handle-null-date(billjournal.sysdate))
                                    + STRING(handle-null-char(voucher-no), "x(24)"). 
                                ELSE output-list.STR = STRING(handle-null-date(billjournal.bill-datum), "x(8)") 
                                    + STRING(handle-null-char(billjournal.zinr), "x(6)")  /*MT 25/07/12 */
                                    + STRING(billjournal.rechnr, "999999999") 
                                    + STRING(billjournal.artnr, "9999") 
                                    + STRING(handle-null-char(billjournal.bezeich), "x(50)") 
                                    + STRING(handle-null-char(deptname), "x(12)") 
                                    + STRING(billjournal.betriebsnr, ">>>>>>") /* Malik 2A7616 : "", "x(6)" -> billjournal.betriebsnr, ">>>>>>" */
                                    + STRING(billjournal.anzahl, "-9999") 
                                    ...
                                    + STRING(handle-null-char(voucher-no), "x(24)"). 
                                ASSIGN
                                    output-list.datum = billjournal.bill-datum
                                    output-list.billno = billjournal.rechnr
                                    output-list.artno = billjournal.artnr
                                    output-list.depart = deptname
                                    output-list.outlet = STRING(billjournal.betriebsnr)
                                    output-list.qty = billjournal.anzahl
                                    output-list.amount = amount
                                    output-list.zeit = STRING(billjournal.zeit,"HH:MM:SS")
                                    output-list.id = billjournal.userinit.
                                    output-list.sysdate = billjournal.sysdate.
                            END.
                            ELSE 
                            DO:
                                /* Oscar (21/01/25) - B6CF79 - handling concatenate string with null value */
                        ...
                        DO:
                            ...
                            temp-str = SUBSTRING(output-list.STR,101).
                            output-list.STR = SUBSTRING(output-list.STR,1,95).
                            /* Oscar (21/01/25) - B6CF79 - handling concatenate string with null value */
                            output-list.STR = output-list.STR + STRING(res-line.erwachs, "-9999") + temp-str.
                            temp-str = "".
                        END.
                        /* Dzikri B8BF2D 22/10/2024 - END*/
                        RUN custom-record-date. /* Dzikri 0CD94D - Long loading */
                    END. /*if do-it*/
                END. /*each billjournal*/
            ELSE IF sorttype = 1 THEN 
                FOR EACH billjournal WHERE billjournal.artnr = artikel.artnr 
                    AND billjournal.departement = artikel.departement 
                    AND billjournal.bill-datum = curr-date NO-LOCK BY billjournal.sysdate BY billjournal.zeit BY billjournal.zinr: 
                    it-exist = YES. 
                    do-it    = YES.
                    IF (mi-onlyjournal EQ YES AND billjournal.bediener-nr EQ 0) OR (mi-excljournal EQ YES AND billjournal.bediener-nr NE 0) THEN /* Dzikri - CA8E6D */
                    DO:
                    ...
                    DO:
                                ...
                                DO:
                                    ...
                                    DO:
                                        ...
                                        DO:
                                            ...
                                            /* Dzikri B6184F - Add new field */
                                            FIND FIRST gbuff WHERE guest.gastnr EQ res-line.gastnr NO-LOCK NO-ERROR.
                                            IF AVAILABLE gbuff AND gbuff.segment3 NE 0 THEN output-list.resname = gbuff.name + ", " + gbuff.vorname1 + " " + gbuff.anrede1. /* Dzikri 706CDB - Add reserve name */
                        ...
                        DO:
                            ...
                            IF billjournal.bezeich MATCHES("*Deposit*") AND billjournal.rechnr NE 0 THEN 
                            DO:
                                /* Oscar (21/01/24) - B6CF79 - handling concatenate string with null value */
                                IF NOT long-digit THEN output-list.STR = STRING(handle-null-date(billjournal.bill-datum), "x(8)")
                                    + STRING(handle-null-char(billjournal.zinr), "x(6)") /*MT 25/07/12 */
                                    + STRING(billjournal.rechnr, "999999999") 
                                    + STRING(billjournal.artnr, "9999") 
            ...
            DO: 
                IF mi-post = YES THEN 
                    FOR EACH billjournal WHERE billjournal.artnr = artikel.artnr 
                        AND billjournal.departement = artikel.departement 
                        AND billjournal.bill-datum = curr-date AND billjournal.anzahl EQ 0 NO-LOCK BY billjournal.sysdate BY billjournal.zeit BY billjournal.zinr: 
                        it-exist = YES.
                        do-it = YES.
                        IF (mi-onlyjournal EQ YES AND billjournal.bediener-nr EQ 0) OR (mi-excljournal EQ YES AND billjournal.bediener-nr NE 0) THEN /* Dzikri - CA8E6D */
                        DO:
        ...
        DO: 
            ...
            DO:
                output-list.STR = STRING("", "x(77)")     /*MT 28/11/12 */
            ...
            DO:
                ...
                t-vat     = 0.
            END.
            RUN custom-record. /* Dzikri 0CD94D - Long loading */
        END.
    END.

    CREATE output-list. 
    IF NOT long-digit THEN 
        ASSIGN output-list.STR = STRING("", "x(77)") /*MT 28/11/12 */
