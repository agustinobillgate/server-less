...
DEF INPUT  PARAMETER billnr AS INT.  
  
DEF INPUT-OUTPUT PARAMETER print-all AS LOGICAL.  
  
DEF OUTPUT PARAMETER winprinterFlag AS LOGICAL INIT NO.  
...
/*{preferencesUI.i}*/
{supertransBL.i}   
DEF VAR lvCAREA AS CHAR INITIAL "print-sphbill1".  
...
DEFINE VARIABLE mwst-str        AS CHAR FORMAT "x(16)".  
      mwst-str = translateExtended ("Government Tax",lvCAREA,""). 
DEFINE VARIABLE service-str     AS CHAR FORMAT "x(16)".  
    service-str =  translateExtended ("Service Charge",lvCAREA,"").  
...
DEFINE VARIABLE str451          AS CHARACTER            NO-UNDO.
DEFINE VARIABLE sc-art          AS INTEGER INITIAL 0    NO-UNDO.

/*gerald 13012021 persen tax and service*/
DEFINE VARIABLE mwst-perc       AS DECIMAL NO-UNDO.
DEFINE VARIABLE serv-perc       AS DECIMAL NO-UNDO. 
...
/* SCD rest article for food */
RUN htpchar.p(451, OUTPUT str451).
str451 = REPLACE(str451, "," , ";").
str451 = REPLACE(str451, "-" , ";").
ASSIGN sc-art  = INTEGER(ENTRY(1, str451, ";")) NO-ERROR.
...
DO:
    ...
    DO:
        print-all = YES.
        RUN print-hbill1-phbl.p (pvILanguage, session-parameter, user-init, 
               hbrecid, printnr,
               use-h-queasy, INPUT-OUTPUT print-all, OUTPUT filename,
               OUTPUT msg-str, OUTPUT winprinterFLag,
               OUTPUT TABLE output-list, OUTPUT TABLE t-printer).
        RETURN.
    END.
END.
...
DO:  
  FIND FIRST vhp.h-bill WHERE RECID(vhp.h-bill) = hbrecid NO-LOCK.  
...
/****** number OF printed lines ON bill ***/   
  ...
  FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = vhp.h-bill.rechnr   
    ...
    AND h-bill-line.waehrungsnr = billnr NO-LOCK:   
    ...
    DO:   
      ...
      IF (vhp.h-artikel.artart = 11 OR vhp.h-artikel.artart = 12) THEN comp-taxserv = NOT comp-taxserv.   /*FT*/
    ...
    DO:   
      ...
      DO:   
        ...
        IF AVAILABLE vhp.h-artikel AND vhp.h-artikel.artart = 0 THEN   
            art-list.condiment = /*vhp.h-bill-line.betriebsnr*/ h-artikel.betriebsnr. /*FDL: D9E3CE*/  
          
        art-list.disc-flag = prdisc-flag AND (art-list.zwkum = disc-zwkum).   
        tot-line = tot-line + 1.   

        /*FDL Sept 15, 2023 => Ticket F756DE*/
        FIND FIRST queasy WHERE queasy.KEY EQ 200
          AND queasy.number1 EQ h-bill-line.departement
          AND queasy.number2 EQ h-bill-line.artnr NO-LOCK NO-ERROR.
        IF AVAILABLE queasy THEN
        DO:
            art-list.barcode = queasy.char1.
        END.
      END.  
      ...
      DO:              
        ...
        /*gerald 130121 persen tax and service*/
        ASSIGN mwst-perc = mwst-percent * 100
               serv-perc = serv-percent * 100.
                                                    
        IF h-bill-line.artnr EQ f-discArt THEN /*FD Dec 13, 2021*/
        DO:
            IF h-bill-line.epreis NE h-bill-line.betrag THEN
            DO:
                IF NOT serv-disc AND h-bill-line.artnr = f-discArt THEN.
                ELSE
                DO:
                    ASSIGN
                        h-service = vhp.h-bill-line.betrag / fact * serv-percent.
                    /*FDL Jan 29, 2024 => Ticket #4EEB34 - Round Depending on Price Decimal*/
                    IF price-decimal EQ 0 THEN h-service = ROUND(h-service,0).
                ...
                DO:
                    ASSIGN
                        h-mwst = vhp.h-bill-line.betrag / fact * mwst-percent.
                    /*FDL Jan 29, 2024 => Ticket #4EEB34 - Round Depending on Price Decimal*/
                    IF price-decimal EQ 0 THEN h-mwst = ROUND(h-mwst,0).
                    ELSE h-mwst = ROUND(h-mwst,2).
                END.
        
                /*FDL 30 Sept, 2024: Ticket 2B4617*/
                IF NOT incl-service AND NOT incl-mwst THEN
                DO:
                    /*FDL Sept, 05 2024: Ticket 196BEE*/
                    amount = vhp.h-bill-line.betrag / fact.
                END.    
                ELSE IF NOT incl-mwst AND incl-service THEN /*FDL Oct, 04 2024: Ticket 5E47B2*/
                DO:               
                    amount = vhp.h-bill-line.betrag / fact.
                END.
                ELSE
                DO:
                    amount = vhp.h-bill-line.betrag.
                END.
                ...
                ASSIGN  
                    ...
                    */  
                    service = service + (amount * serv-percent).
                    IF price-decimal EQ 0 THEN service = ROUND(service,0).
                    ELSE service = ROUND(service,2). 
                .  
                ...
                ASSIGN  
                    ...
                    mwst1  = mwst1  + h-mwst 
                    */ 
                    mwst = mwst + (amount * mwst-percent).
                    mwst1 = mwst1 + (amount * mwst-percent).
                    IF price-decimal EQ 0 THEN mwst = ROUND(mwst,0).
                    ELSE mwst = ROUND(mwst,2). 
                    IF price-decimal EQ 0 THEN mwst1 = ROUND(mwst1,0).
                    ELSE mwst1 = ROUND(mwst1,2). 
                .
            END.
        END.
        ELSE /*ORI*/
        ...
        IF NOT art-list.disc-Flag THEN subtotal = subtotal + amount.         
      END.  
        
      ASSIGN art-list.amount = art-list.amount + amount.  
      IF vhp.h-bill-line.artnr NE 0 THEN   
        art-list.qty = art-list.qty + qty.   
  ...
  FOR EACH art-list:   
    ...
    DO:   
      ...
      bline-exist = YES.  
    END.   
    ELSE IF art-list.qty GT 999 OR art-list.qty LT -999   
        THEN qty1000 = YES.  
  END.   
  ...
  DO:   
    ...
    MESSAGE translateExtended ("No new bill-lines was found.",lvCAREA,"")   
  ...
  IF printnr GT 0 THEN   
  DO:      
    /*MTIF winprinterFlag THEN   
    DO:      
      SESSION:PRINTER-NAME = PRINTER.path.  
      OUTPUT STREAM s1 TO PRINTER PAGE-SIZE VALUE(printer.pglen)   
        UNBUFFERED.   
    END.  
    ELSE OUTPUT STREAM s1 TO VALUE(printer.path) PAGE-SIZE VALUE(printer.pglen)   
      UNBUFFERED.*/  
  END.  
  ...
  DO:   
    ...
    /*MTOUTPUT STREAM s1 TO value(filename) page-size 52 UNBUFFERED.*/  
  END.   
...
/*    page STREAM s1. */   
      ...
      ELSE IF curr-j GT lpage THEN DO:  
        ...
        MESSAGE translateExtended ("Please insert the new guestcheck, then press ENTER.",lvCAREA,"")   
          VIEW-AS ALERT-BOX INFORMATION.   
        PAUSE npause.  
        */  
        curr-j = 0.   
  ...
  ELSE   
  DO:   
    FIND FIRST art-list WHERE art-list.artart = 0 AND NOT art-list.printed  USE-INDEX DATE_ix NO-ERROR .   
    n = 0.   
    curr-j = printed-line.   
    DO i = 1 TO npage:   
      IF NOT AVAILABLE art-list THEN i = npage.   
      ...
      DO WHILE AVAILABLE art-list AND (curr-j LE lpage): 
        RUN print-billine.  
        FIND NEXT art-list WHERE art-list.artart = 0 AND NOT art-list.printed   
          NO-ERROR.   
        IF NOT AVAILABLE art-list THEN i = npage.   
      END.   
      IF i = npage THEN  /* all bill-lines are printed  */   
...
PROCEDURE readSession:
    RUN getLogicalParamValue("WINPRINTER", OUTPUT winprinterFlag).
    /*RUN getLogicalParamValue("Pr2Line", OUTPUT prTwoLine).
    RUN getLogicalParamValue("print-all", OUTPUT print-all).
    RUN getIntParamValue("top-margin", OUTPUT hmargin).
    RUN getIntParamValue("num-lines", OUTPUT lpage).
    RUN getIntParamValue("DesLen", OUTPUT nbezeich).*/
    RUN getIntParamValue("buttom-lines", OUTPUT buttom-lines).
    RUN getStrParamValue("header1", OUTPUT header1).
    RUN getStrParamValue("header2", OUTPUT header2).
    RUN getStrParamValue("foot1", OUTPUT foot1).
    IF foot1 NE "" THEN anz-foot = 1.
    RUN getStrParamValue("foot2", OUTPUT foot2).
    IF foot2 NE "" THEN anz-foot = 2.
END PROCEDURE.
*/  
...
DEF OUTPUT PARAMETER serv-percent          AS DECIMAL INITIAL 0.  
DEF OUTPUT PARAMETER mwst-percent          AS DECIMAL INITIAL 0.  
...
DEF VAR l-deci      AS INTEGER          NO-UNDO INIT 2.  
  
DEF BUFFER hbuff FOR vhp.h-artikel.  
DEF BUFFER aBuff FOR vhp.artikel.  
  
  IF bill-date LT bill-date110 AND (service-code NE 0 OR mwst-code NE 0)THEN  /*FT 080616*/ 
  DO:  
...
DEFINE VARIABLE kname AS CHAR.  
  ...
  DO:   
    ...
    DO:  
      ...
      output-list.sort-i = sort-i.
      sort-i = sort-i + 1.
      output-list.str-pos = 3.
      output-list.str = output-list.str  
                + "     " + translateExtended ("Table",lvCAREA,"") + " "   
                         + STRING(vhp.h-bill.tischnr, ">>>>>9") + "/"   
                         + STRING(vhp.h-bill.belegung,">>9 ") 
                         + "(" + STRING(billnr,">>>9") + ") " 
                         + STRING(kname, "x(32)") /*"x(32)"*/  /* Modified by Gerald 260220 */
                         + order-id.
      CREATE output-list.
      output-list.sort-i = sort-i.
      sort-i = sort-i + 1.
    ...
    DO:  
      ...
      ELSE
        ...
        output-list.str-pos = 1
        output-list.str = output-list.str  
            + "     " + STRING(bill-date) + " " + STRING(TIME, "HH:MM")   
                + " " + translateExtended ("BillNo",lvCAREA,"") + " "   
                + rechnr-str.  
...
DEFINE BUFFER h-art FOR vhp.h-artikel.  
  ...
  IF qty1000 THEN /*MTPUT STREAM s1 art-list.qty FORMAT "->>>> "*/  
    ASSIGN
    output-list.str-pos = 10
    output-list.str = output-list.str + STRING("   ") + STRING(art-list.qty, "->>>> ") + "|".
  ELSE DO: /*MTPUT STREAM s1 art-list.qty " "*/ 
      output-list.str-pos = 10.
      ...
      ELSE IF art-list.qty GT 9 OR art-list.qty LT -9 THEN
          output-list.str = output-list.str + STRING("  ") + STRING(art-list.qty) + STRING (" "). 
      ELSE*/ output-list.str = output-list.str + STRING("   ") + STRING(art-list.qty) + "|".
  END.
    
  DO i = 1 TO /*nbezeich*/ 30:   
  ...
  DO:  
    ...
    DO:   
       ...
       DO:  
           ...
           /*FDL March 07, 2023 => Ticket 305775*/
    ...
    /*FDL Sept 15, 2023 => Ticket F756DE*/
    output-list.str = output-list.str + art-list.barcode + "|".
  END.  
  CREATE output-list.
  output-list.sort-i = sort-i.
  sort-i = sort-i + 1.
...
/*  
  FIND FIRST vhp.queasy WHERE vhp.queasy.key = 38   
      AND vhp.queasy.number1 = vhp.h-bill.departement  
      AND vhp.queasy.number2 = art-list.artnr NO-LOCK NO-ERROR.  
  IF AVAILABLE vhp.queasy THEN  
  DO:  
    PUT STREAM s1 "" FORMAT "x(5)".  
    DO i = 1 TO nbezeich:   
      IF i GT LENGTH(vhp.queasy.char3) THEN PUT STREAM s1 " ".   
      ELSE PUT STREAM s1 SUBSTR(vhp.queasy.char3, i, 1) FORMAT "x(1)".   
    END.   
    PUT STREAM s1 "" SKIP.  
    ASSIGN  
      curr-j       = curr-j + 1   
...
DEFINE VARIABLE tot-str AS CHAR.   
  ...
  DO:   
    ...
    DO:   
        ...
        ELSE /*MTPUT STREAM s1 "" FORMAT "x(5)"*/  
            output-list.str = output-list.str + STRING("", "x(5)").  
        DO i = 1 TO (nbezeich + n11):   
          IF i LT (nbezeich + n11) THEN /*MTPUT STREAM s1 "-" FORMAT "x(1)"*/  
              output-list.str = output-list.str + STRING("-", "x(1)").  
          ELSE  
          DO:  
    ...
    DO:   
       ...
       DO:  
           ...
           output-list.sort-i = sort-i.
           sort-i = sort-i + 1.
           /*MTPUT STREAM s1 subtotal FORMAT "->>>,>>>,>>9" SKIP.*/  
       END.  
       ELSE  
       DO:  
           output-list.str = output-list.str + STRING(subtotal, "->,>>>,>>>,>>>,>>9").  
    ...
    DO:  
        ...
        output-list.sort-i = sort-i.
        sort-i = sort-i + 1.
        /*MTPUT STREAM s1 subtotal FORMAT "->>>,>>9.99" SKIP.*/  
    END.  
    printed-line = printed-line + 1.   
    ...
    DO:  
      DO i = 1 TO pos:  
          ...
          /*MTPUT STREAM s1 " ".*/  
      END.   
      DO i = 1 TO nbez1: /*DO i = 1 TO (nbez1 + 4):   *//*Naufal - format print tidak rata*/
        IF i GT LENGTH(service-str) THEN /*MTPUT STREAM s1 " "*/  
            ASSIGN
            output-list.str-pos = 12
            output-list.str = output-list.str + STRING(" ").  
        ELSE /*MTPUT STREAM s1 SUBSTR(service-str, i, 1) FORMAT "x(1)"*/  
            ASSIGN
            output-list.str-pos = 12
            output-list.str = output-list.str + STRING(SUBSTR(service-str, i, 1), "x(1)") .  
      END.   
    /*
      /*gerald 130121 persen tax and service*/ /*Naufal - format print tidak rata*/
      output-list.str = output-list.str + STRING(service-str) /*+ " " + STRING(serv-perc) + "%"*/.
    */
      IF price-decimal = 0 THEN   
      DO:   
         IF NOT long-digit THEN   
         DO:  
             output-list.str = output-list.str + STRING(service, "   ->>,>>>,>>>,>>9").  
             CREATE output-list.
             output-list.sort-i = sort-i.
             sort-i = sort-i + 1.
             /*MTPUT STREAM s1 service FORMAT "->>>,>>>,>>9" SKIP.*/  
         END.  
         ELSE  
         DO:  
             output-list.str = output-list.str + STRING(service, "->,>>>,>>>,>>>,>>9").  
      ...
      ELSE  
      DO:  
          output-list.str = output-list.str + STRING(service, "->>,>>>,>>>,>>9.99").  
          CREATE output-list.
          output-list.sort-i = sort-i.
          sort-i = sort-i + 1.
          /*MTPUT STREAM s1 service FORMAT "->>>,>>9.99" SKIP.*/  
      END.  
      printed-line = printed-line + 1.   
    END.  
    ...
    DO:  
      ...
      DO i = 1 TO nbez1: /*DO i = 1 TO (nbez1 + 4):   *//*Naufal - format print tidak rata*/   
        IF i GT LENGTH(mwst-str) THEN /*MTPUT STREAM s1 " "*/  
            ASSIGN
            output-list.str-pos = 12
            output-list.str = output-list.str + STRING(" ").  
        ELSE /*MTPUT STREAM s1 SUBSTR(mwst-str, i, 1) FORMAT "x(1)"*/  
            ASSIGN
            output-list.str-pos = 12
            output-list.str = output-list.str + STRING(SUBSTR(mwst-str, i, 1), "x(1)").  
      END.
    /* 
      /*gerald 130121 persen tax and service*//*Naufal - format print tidak rata*/ 
      output-list.str = output-list.str + STRING(mwst-str) /*+ " " + STRING(mwst-perc) + "%"*/.
    */
      IF price-decimal = 0 THEN   
      DO:   
         IF NOT long-digit THEN  
         DO: 
             ...
             output-list.sort-i = sort-i.
             sort-i = sort-i + 1.
             /*MTPUT STREAM s1 mwst FORMAT "->>>,>>>,>>9" SKIP.*/  
         END.  
         ELSE  
         DO: 
             output-list.str = output-list.str + STRING(mwst, "->,>>>,>>>,>>>,>>9").  
      ...
      DO:  
          ...
          output-list.sort-i = sort-i.
          sort-i = sort-i + 1.
          /*MTPUT STREAM s1 mwst FORMAT "->>>,>>9.99" SKIP.*/  
      END.  
      printed-line = printed-line + 1.   
    END.   
  END. /* IF service NE 0 OR mwst NE 0 */  
  ...
  FIND FIRST vhp.htparam WHERE paramnr = 846 NO-LOCK.   
  IF fchar = "" THEN tot-str = translateExtended ("TOTAL",lvCAREA,"").   
  ELSE tot-str = vhp.htparam.fchar.   
  DO i = 1 TO nbez1:   
      IF i GT LENGTH(tot-str) THEN /*MTPUT STREAM s1 " "*/  
          ASSIGN
          output-list.str-pos = 13
          output-list.str = output-list.str + STRING(" ").  
...
/*  tot-amount = subtotal + service + mwst. */  
  tot-amount = 0.  
  FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = vhp.h-bill.rechnr  
      AND vhp.h-bill-line.departement = vhp.h-bill.departement 
      AND h-bill-line.waehrungsnr = billnr NO-LOCK,
  ...
  DO:   
     IF NOT long-digit THEN  
     DO:  
         output-list.str = output-list.str + STRING(tot-amount, "   ->>,>>>,>>>,>>9").  
         CREATE output-list.
         output-list.sort-i = sort-i.
         sort-i = sort-i + 1.
  ...
  ELSE  
  DO:  
      output-list.str = output-list.str + STRING(tot-amount, "->>,>>>,>>>,>>9.99").  /*FT 051216*/
      CREATE output-list.
      output-list.sort-i = sort-i.
      sort-i = sort-i + 1.
  ...
  IF vhp.h-bill.bilname NE "" THEN
  DO:
    output-list.str-pos = 18.
    output-list.str = output-list.str +   
            STRING(" ") + STRING(vhp.h-bill.bilname, "x(30)").  
  END.
...
DEFINE VARIABLE compli-notax    AS LOGICAL INITIAL NO.  
  ...
  DO:  
    ...
    /*FDL Feb 13, 2024 => Ticket 9B58D7*/
    FIND FIRST queasy WHERE queasy.KEY EQ 286
        AND queasy.number1 EQ h-bill.rechnr
        AND queasy.number2 EQ h-bill.departement
        AND queasy.number3 EQ billnr NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN output-list.str = output-list.str + " | " + TRIM(STRING(queasy.char1, "x(32)")).
...
DEF VAR i AS INTEGER NO-UNDO.  
  ...
  DO:   
    ...
    output-list.str-pos = 19.   /*FD => Foot Note*/
