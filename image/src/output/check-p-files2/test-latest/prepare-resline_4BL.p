...
DEF TEMP-TABLE f-resline
    ...
    /*ITA leasing feature*/
    FIELD repeat-charge AS LOGICAL
    FIELD every-month   AS INTEGER
    FIELD repeat-amount AS DECIMAL
...
IF AVAILABLE htparam AND htparam.bezeich NE "not used" THEN DO:
...
END FUNCTION. 
  ...
  IF htparam.flogical = YES AND htparam.bezeich NE "not used" THEN 
      ASSIGN f-resline.ci-date  = TODAY
             f-resline.billdate = TODAY.
  ELSE ASSIGN f-resline.ci-date  = curr-date.
  ...
  DO:  
    ...
    DO: 
      ...
      FIND FIRST queasy WHERE queasy.key = 9 
        AND queasy.number1 = bill-instruct NO-LOCK NO-ERROR. 
    ...
    ASSIGN
      ...
      f-resline.sharer       = (res-line.resstatus = 11) 
                            OR (res-line.resstatus = 13)
    . 
...
/* check IF contract rate exists, fix rate defined */ 
  ...
  FIND FIRST bqueasy WHERE bqueasy.KEY = 301 
        AND bqueasy.number1 = res-line.resnr AND bqueasy.logi1 = YES NO-LOCK NO-ERROR.
  IF AVAILABLE bqueasy THEN
      ASSIGN f-resline.repeat-charge    = bqueasy.logi1
             /*f-resline.every-month      = bqueasy.number2
             f-resline.repeat-amount    = bqueasy.deci1
             f-resline.start-date       = bqueasy.date1
             f-resline.end-date         = bqueasy.date2*/
     .
...
PROCEDURE check-dynaRate:
  ...
  IF NOT AVAILABLE res-dynaRate THEN RETURN.
  ELSE IF AVAILABLE res-dynaRate AND res-dynaRate.date1 = ?
      AND res-dynaRate.date1 = ? THEN RETURN.
  ...
  ASSIGN
    reslin-list.ankunft         = res-dynaRate.date1
...
/*  IF res-dynaRate.rate GE 0 THEN   */
  ...
  /*FIND FIRST bqueasy WHERE bqueasy.KEY = 301 
        AND bqueasy.number1 = inp-resnr AND bqueasy.logi1 = YES NO-LOCK NO-ERROR.
  ...
  ELSE IF AVAILABLE bqueasy THEN DO:
      RUN calc-periode(inp-resnr, inp-reslinnr).
      ...
      DO datum = reslin-list.ankunft TO (reslin-list.abreise - 1):
          ...
          IF AVAILABLE periode-list THEN DO:
                ...
                FIND FIRST reslin-queasy WHERE reslin-queasy.key         = "arrangement"
                    ...
                    AND reslin-queasy.reslinnr    = f-resline.reslinnr /* new booking */
                ...
                IF NOT AVAILABLE reslin-queasy THEN DO:
                    ...
                    /*IF reslin-list.zipreis = ? OR reslin-list.zipreis = 0 THEN*/
                    ASSIGN reslin-list.zipreis = periode-list.amt-periode.
                END.               
          END.
      END.
  END.*/
...
PROCEDURE calc-periode:
    ...
    DEFINE BUFFER bbresline FOR res-line.


    FIND FIRST tqueasy WHERE tqueasy.KEY = 301
        AND tqueasy.number1 = resno AND tqueasy.logi1 = YES NO-LOCK NO-ERROR.
    IF AVAILABLE tqueasy THEN DO:

        FIND FIRST bbresline WHERE bbresline.resnr = resno
            AND bbresline.resnr = reslinnr NO-LOCK NO-ERROR.
        ASSIGN periode-rsv1 = bbresline.ankunft
               periode-rsv2 = bbresline.abreise
         .
        ...
        FOR EACH periode-list:
            ...
            IF periode-list.diff-day GE 30 THEN
                ASSIGN periode-list.amt-periode = tqueasy.deci1 / periode-list.diff-day
                       periode-list.tamount     = periode-list.amt-periode * periode-list.diff-day.
            ELSE DO:
                 ASSIGN periode-list.amt-periode = (tqueasy.deci1 * 12) / 365
                        periode-list.tamount     = periode-list.amt-periode * periode-list.diff-day.
            END.
        END.
    END.
