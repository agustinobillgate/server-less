...
DEFINE VARIABLE j AS INTEGER. /* Malik Serverless */
DEFINE VARIABLE counter-date AS INT. /* Malik Serverless */
  ...
  FOR EACH zinrstat WHERE (zinrstat.datum GE curr-date AND zinrstat.datum LE to-date) 
    ...
    DO:
      ...
      datum2 = to-date. 
      /* Malik serverless */
      IF MONTH(zinrstat.datum) EQ MONTH(datum1) THEN
      DO:
        j = DAY(zinrstat.datum) - DAY(datum1).
        counter-date = j.
      END.
      ELSE
      DO:
        j = counter-date + DAY(zinrstat.datum) - DAY(datum1).
      END.
      
      /* i = zinrstat.datum - datum1 + 1. uncomment for serverless MALIK */
      i = j + 1.
      /* END Malik */
