...
DEFINE VARIABLE bdate AS DATE. 
  ...
  FOR EACH res-line WHERE res-line.active-flag = 2 
    AND res-line.resstatus = 9 AND res-line.ankunft GE bdate 
    ...
    END.
    ASSIGN
      tmpInt = res-line.abreise - res-line.ankunft /*FT severless*/
...
DEFINE VARIABLE bdate AS DATE. 
  ...
  IF sorttype = 1 THEN 
  FOR EACH res-line WHERE res-line.active-flag = 2 
    AND res-line.resstatus = 9 AND res-line.cancelled GE bdate 
