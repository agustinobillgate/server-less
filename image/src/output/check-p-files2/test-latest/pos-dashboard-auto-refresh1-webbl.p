...
DEFINE OUTPUT PARAMETER sound-link      AS CHARACTER.
...
/*FDL Nov 09, 2023 => Ticket 3BCBFA - Expired Session Bill Closed When Session Involve*/
RUN pos-dashboard-expired-sessionbl.p(t-dept.nr, dynamic-qr).
...
FIND FIRST queasy WHERE queasy.KEY EQ 222 
    ...
    AND queasy.number2 EQ 29
    AND queasy.number3 EQ 5 NO-LOCK NO-ERROR.
IF AVAILABLE queasy THEN sound-link = queasy.char2.
