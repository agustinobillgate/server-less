...
PROCEDURE disp-it: 
  ...
  DO: 
    IF zinr = "" THEN DO:
        /*FDL Uncomment & Enhance April, 21 2025: 481BF0 - Slow Loading Time*/
        FOR EACH history WHERE history.betriebsnr LE 1 
            ...
            FIRST guest WHERE guest.gastnr = history.gastnr NO-LOCK BY history.ankunft BY history.gastinfo:
            /*FDL Comment: 481BF0
            AND guest.name GE from-name 
            AND guest.karteityp = disptype NO-LOCK BY history.gastinfo:*/
            IF guest.name GE from-name AND guest.karteityp = disptype THEN
            DO:
                FIND FIRST reservation WHERE reservation.resnr = history.resnr NO-LOCK NO-ERROR. /*bernatd 5727F5*/
                IF AVAILABLE reservation THEN RUN assign-it.
            END.            
        END.
        ...
        */
        ...
        DO WHILE AVAILABLE guest:
            ...
            FOR EACH history WHERE history.betriebsnr LE 1  
                ...
                AND history.ankunft GE f-date 
                AND history.ankunft LE t-date NO-LOCK BY history.ankunft:
                FIND FIRST reservation WHERE reservation.resnr = history.resnr NO-LOCK NO-ERROR. /*bernatd 5727F5*/
                IF AVAILABLE reservation THEN                                                    /* Rulita 060225 | Fixing Serverless AVAIlABLE l lowercase Issue git 524 */
                DO:
                  RUN assign-it.
                END.    
            END.
  ...
  DO:
    IF zinr = "" THEN DO:
        ...
        FOR EACH history WHERE history.betriebsnr LE 1 
            ...
            AND history.gastnr GT 0 NO-LOCK, 
            FIRST guest WHERE guest.gastnr = history.gastnr NO-LOCK BY history.abreise BY history.gastinfo:
            /*FDL Comment: 481BF0
            AND guest.name GE from-name 
            AND guest.karteityp = disptype BY history.gastinfo:*/
            IF guest.name GE from-name AND guest.karteityp = disptype THEN
  ...
  DO: 
    IF zinr = "" THEN DO:
        ...
        FOR EACH history WHERE history.betriebsnr LE 1 
            ...
            AND guest.karteityp = disptype BY history.gastinfo:*/
            IF guest.name MATCHES(from-name) AND guest.karteityp = disptype THEN
            DO:
                FIND FIRST reservation WHERE reservation.resnr = history.resnr NO-LOCK NO-ERROR. /*bernatd 5727F5*/
                IF AVAILABLE reservation THEN RUN assign-it.
            END.
...
PROCEDURE assign-it:    
    ...
    ASSIGN
        ...
        history-list.resv-name       = reservation.name.
