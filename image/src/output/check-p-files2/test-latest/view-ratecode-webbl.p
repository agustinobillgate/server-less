...
DEFINE INPUT PARAMETER market-combo    AS CHAR    NO-UNDO INIT "". /* Dzikri 2AD26E - repair matched with desktop */
...
PROCEDURE create-list0: 
  ...
  /* Dzikri 2AD26E - repair matched with desktop */   
  IF market-combo NE "" THEN 
  DO:
  ...
  DO:
    FOR EACH prtable WHERE prtable.prcode = pr-code NO-LOCK,
      ...
      IF NOT comments MATCHES ("*" + prmarket.bezeich + "*") THEN comments = comments + "|" + prmarket.bezeich .
...
PROCEDURE create-list: 
  ...
  IF guest.bemerk NE ? OR guest.bemerk NE "" THEN comments = REPLACE(guest.bemerk,"|","-").
  ELSE comments = "".
  /* Dzikri 2AD26E - repair matched with desktop */
