...
/**** additional fix cost ******/ 
  ...
  FIND FIRST queasy WHERE queasy.KEY = 301
      AND queasy.number1 = resnr AND queasy.logi1 = YES NO-LOCK NO-ERROR.
  IF AVAILABLE queasy THEN DO:
    ...
    FOR EACH s-list BY s-list.ankunft BY s-list.datum BY s-list.bezeich 
        ...
        FIND FIRST periode-list WHERE s-list.datum GE periode-list.periode1 
             AND s-list.datum LE periode-list.periode2 NO-LOCK NO-ERROR.
        IF AVAILABLE periode-list THEN DO:

            FIND FIRST t-list WHERE t-list.bezeich = s-list.bezeich 
                AND t-list.rmcat = s-list.rmcat 
                AND t-list.date1 = periode-list.periode1
                AND t-list.date2 = periode-list.periode2 NO-ERROR. 
            IF NOT AVAILABLE t-list THEN DO:
                CREATE t-list. 
                ASSIGN
                    t-list.nr       = s-list.nr 
                    t-list.bezeich  = s-list.bezeich
                    t-list.rmcat    = s-list.rmcat 
                    t-list.date1    = periode-list.periode1 
                    t-list.date2    = periode-list.periode2
            ...
            END.
             ...
             IF s-list.qty GE t-list.qty THEN t-list.tage = t-list.tage + 1.
             ASSIGN t-list.preis = t-list.preis + s-list.preis. 
        END.
    END.
...
PROCEDURE calc-periode:
    ...
    FIND FIRST queasy WHERE queasy.KEY = 301
        ...
        AND queasy.logi1 = YES NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN DO:
        ASSIGN periode-rsv1 = queasy.date1
               periode-rsv2 = queasy.date2
         .
        ...
        FOR EACH periode-list:
            ...
            IF periode-list.diff-day GE 30 THEN
                ASSIGN periode-list.amt-periode = queasy.deci1 / periode-list.diff-day
                       periode-list.tamount     = periode-list.amt-periode * periode-list.diff-day
                  .
            ELSE DO:
                 ASSIGN periode-list.amt-periode = (queasy.deci1 * 12) / 365
                        periode-list.tamount     = periode-list.amt-periode * periode-list.diff-day.
            END.
        END.
    END.
