...
PROCEDURE create-list: 
  ...
  END. 
  /*FT severless 141124*/
  FOR EACH reslin-queasy WHERE reslin-queasy.betriebsnr = 0 AND reslin-queasy.key = "ResChanges" 
    AND reslin-queasy.number1 = 0 AND reslin-queasy.date1 = ? AND reslin-queasy.char3 GT "" 
    AND reslin-queasy.deci1 = 0 AND reslin-queasy.logi1 = NO 
    AND reslin-queasy.date2 EQ from-date USE-INDEX k-int_ix NO-LOCK: 
    PROCESS EVENTS. 
    FIND FIRST zimkateg WHERE zimkateg.zikatnr 
      = INTEGER(TRIM(SUBSTR(reslin-queasy.char3, 51, 3))) NO-LOCK NO-ERROR. 
    FIND FIRST zimkateg1 WHERE zimkateg1.zikatnr 
      = INTEGER(TRIM(SUBSTR(reslin-queasy.char3, 54, 3))) NO-LOCK NO-ERROR. 
    ...
    DO: 
      ...
      DO:
        ...
        FIND FIRST zimkateg1 WHERE zimkateg1.zikatnr 
          = INTEGER(TRIM(SUBSTR(reslin-queasy.char3, 54, 3))) NO-LOCK NO-ERROR. 
        res-log1.ankunft1 = DATE(SUBSTR(reslin-queasy.char3, 1, 8)). 
        res-log1.ankunft2 = DATE(SUBSTR(reslin-queasy.char3, 9, 8)). 
        res-log1.abreise1 = DATE(SUBSTR(reslin-queasy.char3, 17, 8)). 
        res-log1.abreise2 = DATE(SUBSTR(reslin-queasy.char3, 25, 8)). 
        res-log1.qty1 = INTEGER(SUBSTR(reslin-queasy.char3, 33, 3)). 
        res-log1.qty2 = INTEGER(SUBSTR(reslin-queasy.char3, 36, 3)). 
        res-log1.Adult1 = INTEGER(SUBSTR(reslin-queasy.char3, 39, 2)). 
        res-log1.adult2 = INTEGER(SUBSTR(reslin-queasy.char3, 41, 2)). 
        res-log1.child1 = INTEGER(SUBSTR(reslin-queasy.char3, 43, 2)). 
        res-log1.child2 = INTEGER(SUBSTR(reslin-queasy.char3, 45, 2)). 
        res-log1.comp1 = INTEGER(SUBSTR(reslin-queasy.char3, 47, 2)). 
        res-log1.comp2 = INTEGER(SUBSTR(reslin-queasy.char3, 49, 2)). 
        IF AVAILABLE zimkateg THEN res-log1.rmcat1 
          = STRING(zimkateg.kurzbez,"x(6)"). 
        IF AVAILABLE zimkateg1 THEN res-log1.rmcat2 
          = STRING(zimkateg1.kurzbez,"x(6)"). 
        res-log1.zinr1 = SUBSTR(reslin-queasy.char3, 57, 4). 
        res-log1.zinr2 = SUBSTR(reslin-queasy.char3, 61, 4). 
        res-log1.argt1 = SUBSTR(reslin-queasy.char3, 65, 5). 
        res-log1.argt2 = SUBSTR(reslin-queasy.char3, 70, 5). 
        res-log1.rate1 = DECIMAL(SUBSTR(reslin-queasy.char3, 75, 12)). 
        res-log1.rate2 = DECIMAL(SUBSTR(reslin-queasy.char3, 87, 12)). 
        res-log1.id1 = SUBSTR(reslin-queasy.char3, 99, 2). 
        res-log1.id2 = SUBSTR(reslin-queasy.char3, 101, 2). 
        res-log1.date1 = DATE(SUBSTR(reslin-queasy.char3, 103, 8)). 
        IF SUBSTR(reslin-queasy.char3, 111, 8) = "        " THEN res-log1.date2 = ?. 
        ELSE res-log1.date2 = DATE(SUBSTR(reslin-queasy.char3, 111, 8)). 
        IF length(reslin-queasy.char3) GT 120 THEN 
        DO: 
          res-log1.name1 = SUBSTR(reslin-queasy.char3, 119, 16). 
          res-log1.name2 = SUBSTR(reslin-queasy.char3, 135, 16). 
        END. 
        IF length(reslin-queasy.char3) GT 151 THEN 
        DO: 
          res-log1.fixrate1 = SUBSTR(reslin-queasy.char3, 151, 3). 
          res-log1.fixrate2 = SUBSTR(reslin-queasy.char3, 154, 3). 
        END.
      END.
      res-log1.zeit = reslin-queasy.number2. 
    END. 
  END. 
END.
