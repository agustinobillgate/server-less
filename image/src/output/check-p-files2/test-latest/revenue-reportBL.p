...
DEFINE VARIABLE ekum-no             AS INTEGER              NO-UNDO.
...
DEFINE VARIABLE lm-mtd-datum        AS DATE                 NO-UNDO.
...
DO:   
    ...
    ly-jan1 = DATE(1, 1, year(jan1) - 1). 
    
    IF month(to-date) GE 2 THEN 
    DO: 
        IF day(from-date) = 1 THEN lm-fdate = DATE(month(from-date) - 1, 1, year(from-date)). 
        ELSE lm-fdate = from-date - 30. 

        IF day(to-date + 1) = 1 THEN lm-tdate = DATE(month(to-date), 1, year(to-date)) - 1. 
    ...
    IF day(from-date) = 29 AND month(from-date) = 2 THEN ly-fdate = DATE(2, 28, year(from-date) - 1). 
    ELSE ly-fdate = DATE(month(from-date), day(from-date), year(from-date) - 1). 

    IF day(to-date) = 29 AND month(to-date) = 2 THEN ly-tdate = DATE(2, 28, year(to-date) - 1). 
    ELSE ly-tdate = DATE(month(to-date), day(to-date), year(to-date) - 1). 
    ...
    FOR EACH hoteldpt NO-LOCK BY hoteldpt.num: 
        ...
        zwkum = 0. 
        FOR EACH artikel WHERE (artikel.artart = 0 OR artikel.artart = 8) /* lodging */ 
            AND artikel.departement = hoteldpt.num 
            /*AND artikel.activeflag*/ NO-LOCK BY artikel.zwkum: 
            ...
            IF do-it THEN 
            DO: 
                IF zwkum NE artikel.zwkum THEN 
                DO: 
                    zwkum = artikel.zwkum. 
                    FIND FIRST zwkum WHERE zwkum.zknr = artikel.zwkum 
                        AND zwkum.departement = artikel.departement NO-LOCK. 
                END. 
                ...
                DO datum = jan1 TO to-date: /* from jan 1 this year TO to-date */ 
                    ...
                    DO:                        
                        ...
                        DO: 
                            ...
                            /*budget*/
                            FOR EACH budget WHERE budget.departement = umsatz.departement
                                AND budget.artnr = umsatz.artnr AND budget.datum = datum NO-LOCK:
...
/*   LAST month MTD */ 
                        /* FDL Comment - Move to below, own calculation
...
/*   LAST year same DATE */ 
                    ...
                    ELSE 
                    DO: 
                        ly-datum = DATE(month(datum), day(datum), year(datum) - 1). 
                        FIND FIRST umsatz WHERE umsatz.artnr = artikel.artnr 
                            AND umsatz.departement = artikel.departement 
...
/*   LAST month MTD  */
                /*FDL Jan 11, 2023 => Ticket 1A61F0*/
                FOR EACH umsatz WHERE umsatz.artnr EQ artikel.artnr 
                    AND umsatz.departement EQ artikel.departement 
                    AND umsatz.datum GE lm-fdate
                    AND umsatz.datum LE lm-tdate NO-LOCK:
...
DO:
    ...
    FOR EACH artikel WHERE (artikel.artart = 0 OR artikel.artart = 8) /* lodging */  
        ...
        DO: 
            IF ekum-no = 0 THEN 
            DO: 
                ekum-no = artikel.endkum. 
                /* FDL Comment
                FIND FIRST ekum WHERE ekum.eknr = artikel.endkum NO-LOCK.
                */
                ...
                END.                
            END. 
            IF ekum-no NE artikel.endkum THEN 
            DO: 
                cl-list.dnet = dnet. 
                cl-list.mnet = mnet. 
                cl-list.mbudget   = mbudget.
            ...
            FIND FIRST cl-list WHERE cl-list.kum = artikel.endkum NO-LOCK NO-ERROR. 
            IF NOT AVAILABLE cl-list THEN 
            DO: 
                CREATE cl-list. 
                cl-list.kum = artikel.endkum. 
                ...
                FIND FIRST ekum WHERE ekum.eknr = artikel.endkum NO-LOCK NO-ERROR. 
                IF AVAILABLE ekum THEN cl-list.bezeich = ekum.bezeich. 
            END. 
            
            DO datum = jan1 TO to-date: /* from jan 1 this year TO to-date */ 
                FIND FIRST umsatz WHERE umsatz.artnr = artikel.artnr 
                    AND umsatz.departement = artikel.departement 
                    AND umsatz.datum EQ datum USE-INDEX umsatz_index NO-LOCK NO-ERROR. 
                IF AVAILABLE umsatz THEN 
                DO: 
                    /*ITA 0209*/
                    RUN calc-servvat.p(umsatz.departement, umsatz.artnr, umsatz.datum, 
...
/* LAST year same DATE */ 
                ...
                DO: 
                    ...
                    FIND FIRST umsatz WHERE umsatz.artnr = artikel.artnr 
                        AND umsatz.departement = artikel.departement 
                        AND umsatz.datum EQ ly-datum 
                        USE-INDEX umsatz_index NO-LOCK NO-ERROR. 
                    IF AVAILABLE umsatz THEN 
                    DO: 
                      /*ITA 020916 */
                      RUN calc-servvat.p(umsatz.departement, umsatz.artnr, umsatz.datum, 
