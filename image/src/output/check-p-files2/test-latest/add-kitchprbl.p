...
/*FDL: FC731B - Change h-artikel to temp-table t-h-artikel*/
FOR EACH h-artikel WHERE h-artikel.departement EQ dept NO-LOCK:
    CREATE t-h-artikel.
...
IF sort-subgrp = YES THEN DO:
    FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = rechnr 
      ...
      FIRST t-h-artikel WHERE t-h-artikel.artnr = vhp.h-bill-line.artnr 
      AND t-h-artikel.departement = dept AND t-h-artikel.artart = 0 NO-LOCK,
      FIRST vhp.wgrpdep WHERE vhp.wgrpdep.departement = dept 
      AND vhp.wgrpdep.zknr = t-h-artikel.zwkum NO-LOCK      
      BY t-h-artikel.bondruckernr[1]       
      BY t-h-artikel.zwkum
      BY vhp.h-bill-line.sysdate BY vhp.h-bill-line.zeit 
      BY RECID(vhp.h-bill-line)     /*FDL March 10, 2023 => Ticket BB727B*/
      /*BY t-h-artikel.endkum
      ...
      IF vhp.htparam.flogical /* i.e. kitchen vhp.printer is active */ 
        AND t-h-artikel.bondruckernr[1] GT 0 THEN 
      DO: 
        IF NOT print-single THEN
        DO:
          IF (k NE t-h-artikel.bondruckernr[1]) THEN 
          DO: 
            IF k GT 0 THEN RUN cut-it. 
            RUN create-bon-output. 
            k = t-h-artikel.bondruckernr[1]. 
          END. 
          RUN write-article. 
        END.
        ELSE
      ...
      DO TRANSACTION:
        FIND FIRST hbline WHERE RECID(hbline) = RECID(vhp.h-bill-line)
            EXCLUSIVE-LOCK.
        IF t-h-artikel.bondruckernr[1] NE 0 THEN 
        DO:
          IF hbline.steuercode = 0 THEN 
              hbline.steuercode = t-h-artikel.bondruckernr[1]. 
...
DO:
    FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = rechnr 
        ...
        DO TRANSACTION:
            ...
            IF t-h-artikel.bondruckernr[1] NE 0 THEN 
            DO:
                IF hbline.steuercode = 0 THEN hbline.steuercode = t-h-artikel.bondruckernr[1]. 
                ELSE hbline.steuercode = 9999. /* used by kitchen + order printer */
            END.
            ELSE 
            DO:    
...
ELSE DO:
    FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = rechnr 
      ...
      MESSAGE 
          ...
          ">>>" "ArtNo=" h-bill-line.artnr " | " "Desc=" h-bill-line.bezeich " | " "KP-No=" t-h-artikel.bondruckernr[1] SKIP
...
DEF BUFFER qsy FOR t-queasy.
    ...
    /*Debug*/
    MESSAGE "create-bon-output:=" t-h-artikel.bondruckernr[1]
        VIEW-AS ALERT-BOX INFO BUTTONS OK.

  FIND FIRST vhp.printer WHERE vhp.printer.nr = t-h-artikel.bondruckernr[1] NO-LOCK NO-ERROR. 
  IF NOT AVAILABLE vhp.printer THEN 
  DO: 
    error-str = error-str + translateExtended ("The kitchen vhp.printer number",lvCAREA,"")
      + " " + STRING(t-h-artikel.bondrucker[1]) + CHR(10) 
      + translateExtended ("in Article",lvCAREA,"") + " " 
      + STRING(t-h-artikel.artnr) + " - " + t-h-artikel.bezeich + CHR(10)
  ...
  ELSE DO: 
      ...
      FIND FIRST queasy WHERE queasy.KEY = 208 AND queasy.number1 = t-h-artikel.endkum
          NO-LOCK NO-ERROR.
      IF NOT AVAILABLE queasy THEN DO:
          CREATE queasy.
          ASSIGN 
              queasy.KEY     = 208 
              queasy.number1 = t-h-artikel.endkum
              queasy.number2 = 1.
      END.
      ELSE DO:
...
DEFINE BUFFER qbuff FOR t-queasy.
  ...
  IF vhp.h-bill-line.anzahl NE 0 /* AND t-h-artikel.aenderwunsch */ THEN 
  DO: 
  ...
  IF t-h-artikel.betriebsnr GT 0 THEN 
  DO: 
    FOR EACH submenu-list WHERE submenu-list.nr = t-h-artikel.betriebsnr 
    ...
    DO: 
      ...
      FOR EACH h-mjourn WHERE h-mjourn.departement = dept 
        ...
        AND NUM-ENTRIES(h-mjourn.request, "|") GT 1
        AND INT(ENTRY(1, h-mjourn.request, "|")) EQ recid-h-bill-line NO-LOCK: 
        ...
        DO: 
          ...
          /*IF NUM-ENTRIES(h-mjourn.request, "|") GT 1              
              AND ENTRY(2, h-mjourn.request, "|") NE "" THEN
          DO:
            t-queasy.char3 = t-queasy.char3 + ":::" + ENTRY(2, h-mjourn.request, "|") + CHR(10). 
          END.*/           
        END. 
      END. 
    END. 
...
DEF VAR i AS INTEGER.
  ...
  DO: 
      t-queasy.char3 = t-queasy.char3 + vhp.printcod.contcode + CHR(10). /* Malik Serverless 516 printcod.contcod -> printcod.contcode */
  END. 
  ELSE 
  DO:
    IF numcat2 = 0 THEN
