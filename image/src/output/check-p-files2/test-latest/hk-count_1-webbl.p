...
DEFINE TEMP-TABLE paying-room
    FIELD room       AS INT
    FIELD pax        AS INT
    FIELD percentage AS DECIMAL FORMAT ">>9.9".
...
DEFINE OUTPUT PARAMETER TABLE FOR paying-room.
...
PROCEDURE init-var: 
  ...
  DEFINE VARIABLE actflag1    AS INTEGER. /*bernatd*/
...
/*        ELSE  to be checked if this is correct */
    ...
    /*Start bernatd add paying room 8187DE - 2025*/
    FOR EACH zimmer WHERE zimmer.sleeping = YES NO-LOCK: 
        tot-avail = tot-avail + 1. 
    END. 

    FOR EACH res-line WHERE res-line.active-flag GE 1 
        AND res-line.active-flag LE 1 
        AND res-line.resstatus NE 9 AND res-line.resstatus NE 10 
        AND res-line.resstatus NE 12 AND res-line.ankunft LE ci-date
        AND res-line.abreise GE ci-date, 
        /*AND res-line.zinr GE " " AND res-line.zinr LE "zz" NO-LOCK,*/ 
        ...
        IF zimmer.sleeping AND (res-line.zipreis GT 0 OR res-line.zipreis = 0) AND res-line.resstatus NE 13
        AND res-line.erwachs GT 0 THEN 
        DO: 
            FIND FIRST queasy WHERE queasy.key = 14 AND queasy.char1 = res-line.zinr 
            AND queasy.date1 LE ci-date AND queasy.date2 GE ci-date
            NO-LOCK NO-ERROR.
            IF NOT AVAILABLE queasy THEN
            DO:
                tot-payrm = tot-payrm + res-line.zimmeranz.
                tot-pax   = tot-pax + res-line.erwachs. 
    ...
    CREATE paying-room.
    ASSIGN 
        paying-room.room = tot-payrm 
        paying-room.pax = tot-pax
        paying-room.percentage = tot-payrm / tot-avail * 100.
    /*end bernatd 8187DE - 2025*/


    /* Malik Serverless 699 handle division by zero and result undefined */
    IF tot-zimmer EQ 0 THEN
    DO:
        proz1 = ?.
        proz2 = ?.
        proz3 = ?.
