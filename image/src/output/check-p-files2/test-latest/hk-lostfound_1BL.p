...
PROCEDURE disp-it:
    ...
    FOR EACH s-list WHERE s-list.betriebsnr = sorttype
        ...
        /*FIRST queasy WHERE RECID(queasy) = s-list.s-recid NO-LOCK*/ /*FDL Comment #858 Serverless*/
        BY s-list.date1 BY s-list.zinr BY s-list.nr:

        FIND FIRST queasy WHERE RECID(queasy) = s-list.s-recid NO-LOCK NO-ERROR. /*FDL ticket #858 Serverless*/
        IF AVAILABLE queasy THEN RUN assign-it.
    END.
    ELSE
    FOR EACH s-list WHERE s-list.betriebsnr = sorttype
        AND s-list.date1 GE fr-date
