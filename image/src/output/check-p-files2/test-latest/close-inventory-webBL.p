...
DEFINE INPUT PARAMETER port         AS CHAR    NO-UNDO.
...
DEFINE VARIABLE msg-str2           AS CHAR.
...
IF doit THEN DO:
    ...
    /*IF OPSYS = "WIN32" THEN 
        ASSIGN 
                cfolder     = "c:\backupinv"
                curr-folder = "c:\backupinv\bkpinv-" + port + "-" + lic-nr + "-ALL-" 
                              + STRING(MONTH(period), "99") + STRING(YEAR(period), "9999")   .
    ELSE 
        ASSIGN
            cfolder     = "/usr1/vhp/backupinv"
            curr-folder = "/usr1/vhp/backupinv/bkpinv-" + port + "-" + lic-nr + "-ALL-" 
                         + STRING(MONTH(period), "99") + STRING(YEAR(period), "9999")   .*/
...
RUN close-inventory-assign-parambl.p(user-init, inv-type).
RUN cqueasy("Update Date of Inventory", "DONE").

RUN cqueasy("Creating closed Onhands", "PROCESS"). 
RUN close-inventory-step1bl.p(1, closedate, inv-type, m-endkum, OUTPUT its-ok,
                              OUTPUT msg-str, OUTPUT msg-str2).
RUN cqueasy("Creating closed Onhands", "DONE").
...
RUN cqueasy("Creating closed In/Out-records", "PROCESS"). 
RUN close-inventory-step2bl.p(inv-type, m-endkum, closedate).
RUN cqueasy("Creating closed In/Out-records", "DONE").
...
IF delete-oph THEN 
DO: 
  RUN cqueasy("Calculating closed Onhands", "PROCESS"). 
  RUN close-inventory-step3bl.p(inv-type, m-endkum, closedate).  
  RUN cqueasy("Calculating closed Onhands", "DONE").
END.

RUN cqueasy("Updating Onhands", "PROCESS"). 
RUN close-inventory-step4bl.p(inv-type, m-endkum, closedate, todate, user-init).
RUN cqueasy("Updating Onhands", "DONE").

RUN cqueasy("Updating Flag Closing", "PROCESS"). 
RUN close-inventory-step4bl.p(inv-type, m-endkum, closedate, todate, user-init).
RUN cqueasy("Updating Flag Closing", "DONE").
