/*FD Juny 16, 2021 => Add From Date - To Date*/
...
DEFINE TEMP-TABLE temp-res LIKE his-res
    /*Add Field For Daily History*/
    FIELD create-date   AS DATE
    FIELD hotel-code    AS CHARACTER
    FIELD adult         AS INTEGER
    FIELD argt          AS CHARACTER
    FIELD guest-id      AS INTEGER  
    FIELD gname         AS CHARACTER
    FIELD nat-code      AS CHARACTER
    FIELD nat-group     AS CHARACTER
    FIELD local-region  AS CHARACTER
.
...
DEFINE TEMP-TABLE daily-his-res
    FIELD number        AS INTEGER
    FIELD staydate      AS DATE
    FIELD create-date   AS DATE  
    FIELD ci-date       AS DATE
...
/*DEFINE INPUT PARAMETER pick-date    AS DATE.*/
...
DEFINE OUTPUT PARAMETER TABLE FOR daily-his-res.
/*
DEFINE VARIABLE pick-date AS DATE INIT 02/03/21.
...
DEFINE VARIABLE nationality1     AS CHARACTER NO-UNDO.
DEFINE VARIABLE nat-code1        AS CHARACTER NO-UNDO.
DEFINE VARIABLE nat-group1       AS CHARACTER NO-UNDO.
DEFINE VARIABLE local-region1    AS CHARACTER NO-UNDO.

FIND FIRST htparam WHERE htparam.paramnr = 87 NO-LOCK NO-ERROR.
IF from-date NE ? OR to-date NE ? THEN
DO:
  /*ci-date = pick-date.*/
  fdate = from-date.
  tdate = to-date.
...
/*1. yesterday's reservation data */

/* VAT code */
FIND FIRST htparam WHERE htparam.paramnr = 1 NO-LOCK NO-ERROR.
...
DO bill-date = from-date TO to-date:
  FOR EACH billjournal NO-LOCK WHERE (billjournal.bill-datum = bill-date 
      OR (billjournal.bill-datum LT bill-date AND billjournal.sysdate GT bill-date))
      ...
      DO:
          ...
          IF do-it THEN
          DO:
              IF artikel.artnr = 100 AND artikel.departement = 0
                  AND billjournal.bill-datum NE bill-date THEN
                      do-it = NO.
          END.
      END.
          ...
          DO:
              DO:
                  ...
                  DO:                                        
                      ...
                      /*File 6*/
                      /*FD March 15, 2021*/
                      FIND FIRST daily-his-res WHERE daily-his-res.staydate = billjournal.bill-datum
                          AND daily-his-res.resnr = bill-resnr
                          AND daily-his-res.reslinnr = 0 NO-LOCK NO-ERROR.
                      IF NOT AVAILABLE daily-his-res THEN
                      DO:
                          CREATE daily-his-res.
                          ASSIGN
                              daily-his-res.staydate = billjournal.bill-datum
                              daily-his-res.resnr = bill-resnr
                              daily-his-res.reserve-id = bill-gastnr
                              daily-his-res.reslinnr = 0
                              daily-his-res.nbrofroom = 0
                              /*daily-his-res.create-date = TODAY*/     /*FD Comment Nov 11, 2021*/
                              daily-his-res.resstatus = "CHECKEDOUT"
                          .
  
                          FIND FIRST guest WHERE guest.gastnr EQ bill-gastnr NO-LOCK NO-ERROR.
                          IF AVAILABLE guest THEN
                          DO:        
                              daily-his-res.guest-id = guest.gastnr.
                              daily-his-res.email = guest.email-adr.
                              daily-his-res.mobile = guest.mobil-telefon.

                              FIND FIRST nation WHERE nation.kurzbez EQ guest.nation1 NO-LOCK NO-ERROR.
                              IF AVAILABLE nation THEN
                              DO:
                                  ASSIGN
                                      daily-his-res.nationality   = nation.bezeich                                                                    
                                      daily-his-res.nat-code      = nation.kurzbez.                                
                                      
                                  FIND FIRST queasy WHERE queasy.KEY EQ 6 
                                      AND queasy.number1 EQ nation.untergruppe NO-LOCK NO-ERROR.
                                  IF AVAILABLE queasy THEN daily-his-res.nat-group =  queasy.char1.                               
                              END.
                                  
                              FIND FIRST buf-nation WHERE buf-nation.kurzbez EQ guest.nation2 NO-LOCK NO-ERROR.
                              IF AVAILABLE buf-nation THEN daily-his-res.local-region = buf-nation.bezeich.                            
                          END. 
  
                          FIND FIRST res-line WHERE res-line.resnr EQ bill-resnr
                              AND res-line.reslinnr EQ 0
                              /*AND res-line.resstatus NE 9 AND res-line.resstatus NE 10 
                              AND res-line.resstatus NE 12*/ NO-LOCK NO-ERROR.
                          IF AVAILABLE res-line THEN
                          DO:
                              ASSIGN
                                  daily-his-res.ci-date       = res-line.ankunft
                                  daily-his-res.co-date       = res-line.abreise
                                  daily-his-res.adult         = res-line.erwachs
                                  daily-his-res.argt          = res-line.arrangement
                                  daily-his-res.gname         = res-line.name.
  
                              IF res-line.CANCELLED NE ? THEN
                                  daily-his-res.cancel-date = STRING(MONTH(res-line.CANCELLED),"99") + "-" + 
                              ...
                              IF AVAILABLE zimkateg THEN daily-his-res.room-type = zimkateg.kurzbez.                            
                          END.
  
                          FIND FIRST reservation WHERE reservation.resnr EQ bill-resnr NO-LOCK NO-ERROR.
                          IF AVAILABLE reservation THEN
                          DO:          
                              daily-his-res.reserveName = reservation.NAME.
                              FIND FIRST segment WHERE segment.segmentcode EQ reservation.segmentcode NO-LOCK NO-ERROR.
                              IF AVAILABLE segment THEN
                                  daily-his-res.segmentcode = segment.bezeich.   

                              IF reservation.resdat NE ? THEN 
                                  daily-his-res.create-date = reservation.resdat.   /*FD Nov 11, 2021*/
                          END.
                      END.
                      
                      IF revCode EQ "ATS" THEN
                      DO:
                          his-res.rm-rev = his-res.rm-rev + netto.
                          daily-his-res.rm-rev = daily-his-res.rm-rev + netto.
                      END.                         
                      ELSE IF revCode EQ "ATM" THEN
                      DO:
                          his-res.fb-rev = his-res.fb-rev + netto.
                          daily-his-res.fb-rev = daily-his-res.fb-rev + netto.
                      END.                       
                      ELSE IF revCode EQ "ATZ" THEN
                      DO:
                          his-res.other-rev = his-res.other-rev + netto.
                          daily-his-res.other-rev = daily-his-res.other-rev + netto.
                      END.
                      /*End FD*/
                  END.
                  ...
                  DO:
                      ...
                      DO:                    
                          FIND FIRST his-res WHERE his-res.staydate EQ billjournal.bill-datum
                              AND bill-resnr EQ his-res.resnr
                              AND bill-reslinnr EQ his-res.reslinnr NO-LOCK NO-ERROR.
  
                          FIND FIRST daily-his-res WHERE daily-his-res.staydate EQ billjournal.bill-datum
                              AND bill-resnr EQ daily-his-res.resnr
                              AND bill-reslinnr EQ daily-his-res.reslinnr NO-LOCK NO-ERROR.
                      ...
                      DO:                    
                          FIND FIRST his-res WHERE his-res.staydate EQ billjournal.bill-datum
                              AND bill-resnr EQ his-res.resnr
                              AND INT(ENTRY(2,billjournal.comment,";")) EQ his-res.reslinnr 
                          NO-LOCK NO-ERROR.
  
                          FIND FIRST daily-his-res WHERE daily-his-res.staydate EQ billjournal.bill-datum
                              AND bill-resnr EQ daily-his-res.resnr 
                              AND INT(ENTRY(2,billjournal.comment,";")) EQ daily-his-res.reslinnr 
                          NO-LOCK NO-ERROR.
                      ...
                      DO:     
                          ...
                          ASSIGN                         
                              daily-his-res.staydate      = billjournal.bill-datum
                              daily-his-res.resnr         = bill-resnr
                              daily-his-res.reslinnr      = t-reslinnr
                              /*daily-his-res.create-date   = TODAY*/   /*FD Comment Nov 11, 2021*/
                              daily-his-res.reserve-id    = bill-gastnr.
  
                          FIND FIRST res-line WHERE res-line.resnr EQ bill-resnr
                              AND res-line.reslinnr EQ t-reslinnr /*AND res-line.resstatus NE 9
                          ...
                          DO:                                         
                              ...
                              IF billjournal.bill-datum LT bill-date AND billjournal.sysdate GT bill-date THEN
                                  daily-his-res.nbrofroom = 0. /*BACK DATED ROOM QTY MUST BE 0*/
                              ELSE
                                  daily-his-res.nbrofroom = res-line.zimmeranz.
  
                              IF daily-his-res.staydate = daily-his-res.co-date 
                                  AND daily-his-res.ci-date NE daily-his-res.co-date THEN
                              his-res.nbrofroom = 0. /*TRANSACTION ON GUEST'S CHECK OUT DAY, ROOM QTY MUST BE 0*/
                              ...
                              DO:                                    
                                  ...
                                  DO:    
                                      ...
                                      DO:                                        
                                          IF bufres-line.resstatus = 6 THEN
                                              daily-his-res.resstatus = "INHOUSE".
                                          ELSE IF bufres-line.resstatus = 13 THEN
                                              ASSIGN
                                                  daily-his-res.resstatus = "INHOUSE(ROOM SHARER)".
                                                  
                                          ELSE IF bufres-line.resstatus = 8 THEN
                                          DO:
                                              IF genstat.resstatus = 13 THEN
                                                  ASSIGN
                                                      daily-his-res.resstatus = "CHECKEDOUT(ROOM SHARER)".
                                              ELSE IF genstat.resstatus = 6 THEN
                                                  daily-his-res.resstatus = "CHECKEDOUT".
                                              ELSE IF genstat.resstatus = 8 THEN
                                                  daily-his-res.resstatus = "CHECKEDOUT". /*DAY-USE*/
                                          END.
                                      END.    
                                  END.
                              END.
...
DO bill-date = from-date TO to-date:
  FOR EACH h-bill-line NO-LOCK WHERE h-bill-line.rechnr GT 0 
      AND h-bill-line.bill-datum EQ bill-date
      ...
      FIND FIRST hbill-list WHERE hbill-list.dept = h-bill-line.departement
          AND hbill-list.rechnr = h-bill-line.rechnr NO-LOCK NO-ERROR.      
...
FOR EACH hbill-list WHERE hbill-list.do-it:
    FOR EACH h-bill-line WHERE h-bill-line.departement = hbill-list.dept
        ...
        FIND FIRST t-list WHERE t-list.rechnr = h-bill-line.rechnr
            ...
            AND t-list.bill-datum = h-bill-line.bill-datum NO-LOCK NO-ERROR.
        ...
        DO:                       
            IF artikel.artart = 9 THEN.
                /*MESSAGE h-artikel.artnr artikel.departement artikel.umsatzart artikel.artart artikel.artgrp " | " h-artikel.artart artikel.bezeich (h-bill-line.betrag / 1.21) 
...
/*BACK DATED
FOR EACH h-bill-line NO-LOCK WHERE h-bill-line.rechnr GT 0 
    AND h-bill-line.bill-datum LT bill-date 
    AND h-bill-line.sysdate GT bill-date
    AND h-bill-line.zeit GE 0
    AND h-bill-line.artnr GT 0
    AND h-bill-line.betrag NE 0 USE-INDEX bildat_index
    BY h-bill-line.departement BY h-bill-line.rechnr
...
FOR EACH hbill-list WHERE hbill-list.do-it AND hbill-list.i-fact NE 0:
    ...
    /* SY: 07/08/2013 */
            ...
            IF artikel.umsatzart = 3 OR artikel.umsatzart GE 5 THEN 
            ASSIGN
                t-list.fb         = t-list.fb + (netto * hbill-list.i-fact)
                t-list.fb-service = t-list.fb-service + (serv-betrag * hbill-list.i-fact)
            .
            ELSE 
            ASSIGN
                t-list.other         = t-list.other + (netto * hbill-list.i-fact)
                t-list.other-service = t-list.other-service + (serv-betrag * hbill-list.i-fact)
            .
        END.
    END.          
END.*/
/*
FOR EACH umsatz WHERE umsatz.departement NE 0
    AND umsatz.datum = bill-date NO-LOCK:
    netto = 0.
    ...
    DO:
       ...
       FIND FIRST t-list WHERE t-list.departement = umsatz.departement
           AND t-list.bill-datum = umsatz.datum NO-LOCK NO-ERROR.
       IF NOT AVAILABLE t-list THEN
       DO:
           CREATE t-list.
           t-list.departement = umsatz.departement.
           t-list.bill-datum  = umsatz.datum.
           IF artikel.umsatzart = 3 OR artikel.umsatzart GE 5 THEN 
           ASSIGN
               t-list.fb         = t-list.fb + netto                
           .
...
/*generating yesterday cancelled and no-show reservations*/
DO bill-date = from-date TO to-date:
  FIND FIRST res-line WHERE (res-line.resstatus EQ 9 OR res-line.resstatus EQ 10)
      AND res-line.active-flag EQ 2
      AND res-line.ankunft LE bill-date
      AND res-line.abreise GE bill-date
  ...
  DO WHILE AVAILABLE res-line:
      ...
      ASSIGN
          ...
          temp-res.create-date  = TODAY
          temp-res.adult        = res-line.erwachs
          temp-res.argt         = res-line.arrangement
          temp-res.gname        = res-line.name.
          .        
      ...
      DO:
          ...
          temp-res.guest-id     = gbuff.gastnr.
      END.
      ...
      DO datum = res-line.ankunft TO datum2:  
          ...
          DO:             
              ...
              CREATE daily-his-res.
              BUFFER-COPY temp-res TO daily-his-res.
              ASSIGN
                  daily-his-res.staydate = datum
                  daily-his-res.rm-rev = net-lodg
                  daily-his-res.fb-rev = tot-fb
                  daily-his-res.other-rev = tot-other
              .
          END.
      END.  
      ...
      IF AVAILABLE gbuff THEN
      DO:
          FIND FIRST nation WHERE nation.kurzbez EQ gbuff.nation1 NO-LOCK NO-ERROR.
          IF AVAILABLE nation THEN
          DO:
              ASSIGN
                  nationality1   = nation.bezeich                                                                    
                  nat-code1      = nation.kurzbez.                                    
                  
              FIND FIRST queasy WHERE queasy.KEY EQ 6 
                  AND queasy.number1 EQ nation.untergruppe NO-LOCK NO-ERROR.
              IF AVAILABLE queasy THEN nat-group1 = queasy.char1.                               
          END.
              
          FIND FIRST buf-nation WHERE buf-nation.kurzbez EQ gbuff.nation2 NO-LOCK NO-ERROR.
          IF AVAILABLE buf-nation THEN local-region1 = buf-nation.bezeich.
      END.

      ASSIGN
          daily-his-res.nationality     = nationality1 
          daily-his-res.nat-code        = nat-code1    
          daily-his-res.nat-group       = nat-group1
          daily-his-res.local-region    = local-region1
      .
  ...
  END.
END.
/* end of yesterday's reservation file */

/* 2. future's reservation file */
EMPTY TEMP-TABLE temp-res.
DO ci-date = fdate TO tdate:
...
/* end of future's reservation file */

/* 3. yesterday's inventory file */

/*generating yesterday inventory data for Compliment & Occupied status*/
DO bill-date = from-date TO to-date:
  FIND FIRST genstat WHERE genstat.datum EQ bill-date
  ...
  DO WHILE AVAILABLE genstat:
      ...
      FIND NEXT genstat WHERE genstat.datum EQ bill-date
      AND genstat.resstatus NE 0        
      AND genstat.res-logi[2] = YES
      AND genstat.zikatnr NE 0 NO-LOCK NO-ERROR.
  END. 
...
DO bill-date = from-date TO to-date:
  FIND FIRST zkstat WHERE zkstat.datum EQ bill-date
  ...
  DO WHILE AVAILABLE zkstat:
      ...
      FIND NEXT zkstat WHERE zkstat.datum EQ bill-date
          AND zkstat.zikatnr NE 0 NO-LOCK NO-ERROR.
  END.
END.
...
DO bill-date = from-date TO to-date:
  FIND FIRST outorder WHERE outorder.gespstart LE bill-date 
      AND outorder.gespende GE bill-date
  ...
  DO WHILE AVAILABLE outorder:
      ...
      FIND NEXT outorder WHERE outorder.gespstart LE bill-date 
          AND outorder.gespende GE bill-date
          AND outorder.zinr NE ""
          AND outorder.betriebsnr LE 1 NO-LOCK NO-ERROR.
  END.
...
/* end of yesterday's inventory file */

/* 4. future's inventory file */
...
DO ci-date = fdate TO tdate:
  FOR EACH res-line WHERE res-line.active-flag LE 1 
      ...
      AND res-line.ankunft LE ci-date + 365
      ...
      DO:
          ...
          DO datum2 = res-line.ankunft TO end-date:
              IF datum2 GE ci-date AND datum2 LE ci-date + 365 THEN
              DO:                
...
DO ci-date = fdate TO tdate:
  FOR EACH zimmer WHERE zimmer.sleeping = YES NO-LOCK BY zimmer.zikatnr: 
      ...
      DO: 
          DO datum = ci-date TO ci-date + 365:
...
DO ci-date = fdate TO tdate:
  FIND FIRST outorder WHERE outorder.gespende GE ci-date
      AND outorder.gespstart LE ci-date + 365
      AND outorder.zinr NE ""
      AND outorder.betriebsnr LE 1 NO-LOCK NO-ERROR.
      ...
      DO datum2 = outorder.gespstart TO outorder.gespende:
          ...
          DO:        
              FIND FIRST future-inv WHERE future-inv.datum = datum2
                  ...
                  AND future-inv.zikatnr = zimmer.zikatnr
                  AND future-inv.room-status = "OUTOFORDER" NO-LOCK NO-ERROR.
              IF NOT AVAILABLE future-inv THEN
              DO:
                  CREATE future-inv.
                  ASSIGN
                      future-inv.zikatnr = zimmer.zikatnr
                      future-inv.datum = datum2
                      future-inv.qty = 1
...
FOR EACH future-inv WHERE future-inv.room-status = "AVAILABLE":
    FIND FIRST buffuture-inv WHERE buffuture-inv.room-status = "OCCUPIED"       
        ...
        /*AND buffuture-inv.room-type = future-inv.room-type*/
        AND buffuture-inv.zikatnr = future-inv.zikatnr NO-LOCK NO-ERROR.
    ...
    IF AVAILABLE buffuture-inv THEN
        future-inv.qty = future-inv.qty - buffuture-inv.qty.
END.
/* end of future's inventory file */
