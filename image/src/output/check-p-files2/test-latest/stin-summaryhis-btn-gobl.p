...
/*
DEF variable sorttype AS INT   init 1.
DEF variable from-lager AS INT init 1.
DEF variable to-lager AS INT   init 8.
DEF variable from-date AS DATE init 09/01/23.
DEF variable to-date AS DATE   init 10/31/23.
DEF variable from-art AS INT   init 1.
DEF variable to-art AS INT     init 9999999.
DEF variable from-grp AS INT   init 0.
DEF variable to-grp AS INT     init 999.*/

MESSAGE "DOMISILI MANA" VIEW-AS ALERT-BOX.
...
PROCEDURE create-list1: 
  ...
  DEFINE buffer l-store FOR l-lager. 
    ...
    FOR EACH l-lager WHERE l-lager.lager-nr GE from-lager 
      ...
      t-qty = 0. 
      t-val = 0. 
      FOR EACH l-ophishis WHERE l-ophishis.lager-nr = l-lager.lager-nr 
        AND l-ophis.datum GE from-date AND l-ophis.datum LE to-date 
        AND l-ophis.artnr GE from-art AND l-ophis.artnr LE to-art 
        /*AND l-ophis.loeschflag LE 1*/   /* changedate: 10/29/99 */ 
        AND l-ophis.anzahl NE 0 
        AND l-ophis.op-art = 1 NO-LOCK, /*USE-INDEX artopart_ix*/ 
        ...
        END. 
        IF l-ophis.datum = to-date THEN 
        DO: 
          t-list.qty = t-list.qty + l-ophis.anzahl. 
          t-list.val = t-list.val + l-ophis.warenwert. 
          qty = qty + l-ophis.anzahl. 
          val = val + l-ophis.warenwert. 
          d-qty = d-qty + l-ophis.anzahl. 
          d-val = d-val + l-ophis.warenwert. 
        END. 
        t-list.t-qty = t-list.t-qty + l-ophis.anzahl. 
        t-list.t-val = t-list.t-val + l-ophis.warenwert. 
        t-qty = t-qty + l-ophis.anzahl. 
        t-val = t-val + l-ophis.warenwert. 
        m-qty = m-qty + l-ophis.anzahl. 
        m-val = m-val + l-ophis.warenwert. 
      END. 
      IF t-qty NE 0 THEN 
      DO: 
        create t-list. 
...
DEFINE buffer l-store FOR l-lager. 
  ...
  FOR EACH l-lager WHERE l-lager.lager-nr GE from-lager 
    ...
    t-qty = 0. 
    t-val = 0. 
    FOR EACH l-ophishis WHERE l-ophis.lager-nr = l-lager.lager-nr 
      AND l-ophis.datum GE from-date AND l-ophis.datum LE to-date 
      AND l-ophis.artnr GE from-art AND l-ophis.artnr LE to-art 
      /*AND l-ophis.loeschflag LE 1*/   /* changedate: 10/29/99 */ 
      AND l-ophis.anzahl NE 0 
      ...
      FIRST l-artikel WHERE l-artikel.artnr = l-ophis.artnr 
        AND l-artikel.zwkum GE from-grp AND l-artikel.zwkum LE to-grp NO-LOCK, FIRST l-untergrup WHERE l-untergrup.zwkum = l-artikel.zwkum 
      ...
      DO: 
        ...
        qty = qty + l-ophis.anzahl. 
        val = val + l-ophis.warenwert. 
        st-qty = st-qty + l-ophis.anzahl. 
        st-val = st-val + l-ophis.warenwert. 
        d-qty = d-qty + l-ophis.anzahl. 
        d-val = d-val + l-ophis.warenwert. 
      END. 
      t-list.t-qty = t-list.t-qty + l-ophis.anzahl. 
      ...
      m-qty = m-qty + l-ophis.anzahl. 
      m-val = m-val + l-ophis.warenwert. 
      sm-qty = sm-qty + l-ophis.anzahl. 
      sm-val = sm-val + l-ophis.warenwert. 
  ...
  DEFINE buffer l-store FOR l-lager. 
    ...
    FOR EACH l-lager WHERE l-lager.lager-nr GE from-lager 
      ...
      FOR EACH l-ophis WHERE l-ophis.lager-nr = l-lager.lager-nr 
        ...
        AND l-ophis.op-art = 1 NO-LOCK, 
        FIRST l-artikel WHERE l-artikel.artnr = l-ophis.artnr 
        AND l-artikel.zwkum GE from-grp AND l-artikel.zwkum LE to-grp NO-LOCK, 
        FIRST l-untergrup WHERE l-untergrup.zwkum = l-artikel.zwkum NO-LOCK BY l-artikel.zwkum BY l-artikel.artnr: 
        IF subgr NE l-untergrup.bezeich AND subgr NE "" AND sm-qty NE 0 THEN
        DO:
            CREATE t-list.
            ASSIGN
      ...
      FOR EACH l-ophis WHERE l-ophis.lager-nr = l-lager.lager-nr 
        ...
        DO: 
          ...
          DO: 
            ...
            f-lager = l-lager.lager-nr. 
          END. 
          tt-list.artnr = STRING(l-ophis.artnr, "9999999"). 
        ...
        IF l-ophis.datum = to-date THEN 
        DO: 
          tt-list.qty = tt-list.qty + l-ophis.anzahl. 
          tt-list.val = tt-list.val + l-ophis.warenwert. 
          qty = qty + l-ophis.anzahl. 
          val = val + l-ophis.warenwert. 
          d-qty = d-qty + l-ophis.anzahl. 
          d-val = d-val + l-ophis.warenwert. 
        END. 
        tt-list.t-qty = tt-list.t-qty + l-ophis.anzahl. 
        tt-list.t-val = tt-list.t-val + l-ophis.warenwert. 
        t-qty = t-qty + l-ophis.anzahl. 
        t-val = t-val + l-ophis.warenwert. 
        m-qty = m-qty + l-ophis.anzahl. 
        m-val = m-val + l-ophis.warenwert. 
