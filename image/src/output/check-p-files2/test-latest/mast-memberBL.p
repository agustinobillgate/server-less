...
DO:
  FIND FIRST mbill WHERE mbill.rechnr = billno NO-LOCK NO-ERROR. 
...
IF NOT AVAILABLE mbill THEN RETURN. /*FT serverless*/
...
FOR EACH res-line WHERE (res-line.resnr = resno 
  ...
  AND res-line.resnr NE resno) NO-LOCK BY res-line.zinr BY res-line.name:

  FIND FIRST b-list WHERE b-list.resnr = res-line.resnr 
    AND b-list.reslinnr = res-line.reslinnr NO-LOCK NO-ERROR.
    /*BY res-line.zinr BY b-list.parent-nr BY res-line.name: FT serverless*/
