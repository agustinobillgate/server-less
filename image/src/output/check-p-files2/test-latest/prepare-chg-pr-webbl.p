...
FIND FIRST htparam WHERE paramnr = 836 NO-LOCK NO-ERROR. /*wrong parameter number (386) it should be (836)*/
IF AVAILABLE htparam THEN hierarchie = htparam.flogical.
...
FIND FIRST htparam WHERE paramnr EQ 152 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN def-curr = htparam.fchar.
/*gerald path-lst A86829*/
FIND FIRST htparam WHERE htparam.paramnr = 417 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN path-lst = htparam.fchar.

 
FIND FIRST l-orderhdr WHERE l-orderhdr.docu-nr = docu-nr NO-LOCK NO-ERROR. 
IF NOT AVAILABLE l-orderhdr THEN RETURN. /*FT serverless*/
...
ASSIGN
  mm = MONTH(l-orderhdr.bestelldatum)
  yy = YEAR(l-orderhdr.bestelldatum). /*FT serverless*/
...
IF mm LT 12 THEN
DO:
    /*to-date     = DATE(mm + 1, 01, yy) - 1. FT serverless*/
    ASSIGN
        tmpInt = mm + 1
        tmpDate = DATE(tmpInt,1,yy)
        tmpDate = tmpDate - 1
        to-date = tmpDate.
END.
ELSE 
DO:
    /*to-date     = DATE(01, 01, yy + 1) - 1.FT serverless */
    ASSIGN
        tmpInt = yy + 1
        tmpDate = DATE(1,1,tmpInt)
        tmpDate = tmpDate - 1
        to-date = tmpDate. 
END.
...
DO:
    ASSIGN
        tmpInt = mm - 3
        from-date = DATE(tmpInt,1,yy).
    /*from-date   = DATE(mm - 3, 01, yy).FT serverless*/
END.
/*end naufal*/
...
FOR EACH l-artikel USE-INDEX artnr_ix NO-LOCK:
    ...
    t-l-artikel.traubensort   = l-artikel.traubensorte
...
PROCEDURE reload-s-list:
    FOR EACH l-order WHERE l-order.docu-nr = docu-nr AND l-order.pos GT 0 
        ...
        /* Dzikri 9374A9 - Last Supplier
        s-list.a-firma        = "".
        FOR EACH b-lorder WHERE b-lorder.artnr EQ l-artikel.artnr AND b-lorder.loeschflag NE 2 NO-LOCK,
            FIRST l-orderhdr WHERE l-orderhdr.docu-nr EQ b-lorder.docu-nr BY b-lorder.lieferdatum-eff DESC BY b-lorder.zeit DESC:
            FIND FIRST l-lieferant WHERE l-lieferant.lief-nr = l-orderhdr.lief-nr NO-LOCK NO-ERROR.
            IF AVAILABLE l-lieferant AND b-lorder.lieferdatum-eff NE ? THEN
            DO:
                ASSIGN
                ...
                LEAVE.
            END.
            ELSE IF b-lorder.lieferdatum-eff NE ? THEN LEAVE. /* IF supplier is not defined but item already received */
        END. 
        Dzikri 9374A9 - END */
        ...
        FOR EACH l-pprice WHERE l-pprice.artnr = l-artikel.artnr NO-LOCK,
          ...
          BY l-pprice.bestelldatum DESC:
            ASSIGN s-list.last-pdate  = l-pprice.bestelldatum
                   ...
                   s-list.a-firma     = l-lieferant.firma       /* Dzikri 8A3438 - longtime load for last purchase suplier */
            .
            LEAVE.
        END.  
