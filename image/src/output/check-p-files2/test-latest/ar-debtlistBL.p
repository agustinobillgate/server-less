...
DEFINE TEMP-TABLE output-list 
    ...
    FIELD bill-no   AS INTEGER  FORMAT ">>>,>>>,>>9"            LABEL "Bill-No"
    FIELD rm-no     AS CHAR     FORMAT "x(6)"                   LABEL "RmNo"
    FIELD receiver  AS CHAR     FORMAT "x(38)"                  LABEL "Bill Receiver"
    FIELD saldo     AS DECIMAL  FORMAT "->,>>>,>>>,>>>,>>9.99"  LABEL "Debt Amount"
    FIELD fsaldo    AS DECIMAL  FORMAT "->,>>>,>>>,>>>,>>9.99"  LABEL "Foreign Amt"
...
DEFINE VARIABLE fcurr  AS CHAR FORMAT "x(4)".
  ...
  FOR EACH debitor WHERE rgdatum GE from-date AND 
    ...
    DO: 
      ...
      DO: 
        ...
        ASSIGN
            ...
            output-list.saldo    = tot-saldo
            output-list.fsaldo   = tf-saldo.
        /*end*/
        create output-list. 
        tot-saldo = 0. 
        tf-saldo = 0. 
    ...
    FOR EACH debt WHERE debt.counter = debitor.counter 
      ...
      /*AND debt.rgdatum GE from-date AND debt.rgdatum LE to-date*/ NO-LOCK:            /*gerald yang sudah payment hilang ABI Project*/
    ...
    DO: 
        ...
        DO: 
          ...
          DO:
              ASSIGN
                  ...
                  output-list.saldo    = t-debit
                  output-list.fsaldo   = tf-debit.
          END.
          ELSE
          DO:
              ASSIGN
      ...
      create output-list. 
      ASSIGN 
          output-list.ref-no1 = guest.firmen-nr
              ...
              DO:
                  ...
                  DO:
                      ASSIGN  output-list.ci-date = res-line.ankunft
                              ...
                              output-list.nights  = INT(res-line.abreise - res-line.ankunft)
                          .
                  END.  
              END.
              ELSE
  ...
  ASSIGN
      ...
      output-list.saldo    = tot-debit
      output-list.fsaldo   = ttf-debit.
  /*end*/
END. 
