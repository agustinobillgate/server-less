...
DEF OUTPUT PARAMETER TABLE FOR q245.
...
IF AVAILABLE l-orderhdr THEN                                                  /* Rulita 110225 | Fixing serverless issue git 572 */
DO:
  ...
  ASSIGN t-l-order.rec-id = RECID(l-order)
        t-l-order.amount = l-order.warenwert .

  /*ITA request vietname*/
  ...
  FOR EACH queasy WHERE queasy.KEY EQ 245 AND queasy.char1 EQ docu-nr NO-LOCK BY queasy.number1:
      CREATE q245.
      ASSIGN
          q245.KEY       = queasy.KEY      
          q245.docu-nr   = queasy.char1  
          q245.user-init = queasy.char2
          q245.app-id    = queasy.char3   
          q245.app-no    = queasy.number1   
          q245.sign-id   = queasy.number2.
  END.
END.
...
PROCEDURE cal-tamount: 
  ...
  FOR EACH l-order WHERE l-order.docu-nr = docu-nr AND 
    ...
    FIND FIRST queasy WHERE queasy.KEY = 304 AND queasy.char1 = l-order.docu-nr
        AND queasy.number1 = l-order.artnr NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN DO:
        ASSIGN s-order.addvat-value = queasy.deci1
               s-order.addvat-no    = queasy.number2
               s-order.amount       = s-order.amount + (l-order.warenwert * (queasy.deci1 / 100))
        .
    END.
    ELSE s-order.amount = s-order.amount + l-order.warenwert.
    ...
    ASSIGN 
      ...
      disc-list.brutto = disc-list.price0 * s-order.anzahl*/
        ...
        /*
      disc-list.disc = DECIMAL(SUBSTR(s-order.quality,1,5)) 
      disc-list.vat = DECIMAL(SUBSTR(s-order.quality,6,5)) 
      disc-list.disc2 = DECIMAL(SUBSTR(s-order.quality,11,5)) */

      IF SUBSTR(SUBSTR(s-order.quality,1,2),2,2) = "." THEN DO:
           ASSIGN 
               disc-list.disc = DECIMAL(SUBSTR(s-order.quality,1,5)) 
               disc-list.vat = DECIMAL(SUBSTR(s-order.quality,6,5)) 
               disc-list.disc2 = DECIMAL(SUBSTR(s-order.quality,11,5))
      ...
      ELSE DO :
        ASSIGN
            disc-list.disc = DECIMAL(SUBSTR(s-order.quality,1,5)) 
            disc-list.vat = DECIMAL(SUBSTR(s-order.quality,7,5)) 
            disc-list.disc2 = DECIMAL(SUBSTR(s-order.quality,12,5)) 
        .
      END.     

      ASSIGN       
          disc-list.disc-val = DECIMAL(SUBSTR(s-order.quality,19,18))
          disc-list.disc2-val = DECIMAL(SUBSTR(s-order.quality,37,18))
          disc-list.vat-val = DECIMAL(SUBSTR(s-order.quality,55)). 
      ...
      IF disc-list.price0 = ? THEN ASSIGN disc-list.price0 = 0.      
  END. 
END. 
