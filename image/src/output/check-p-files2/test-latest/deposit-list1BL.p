...
/*DEF VAR fdate AS DATE INIT 08/01/14.
DEF VAR tdate AS DATE INIT 08/01/14.*/
...
PROCEDURE create-depolist1: 
  FOR EACH reservation WHERE reservation.depositgef NE 0
    ...
    FIRST res-line WHERE res-line.resnr = reservation.resnr AND res-line.resstatus LE 8 NO-LOCK:
    CREATE depo-list.
    ASSIGN
