DEFINE TEMP-TABLE s-list 
  ...
  FIELD str0                    AS CHARACTER FORMAT "x(10)"     LABEL "PR Number" 
  FIELD bestelldatum            AS CHARACTER FORMAT "x(8)"      LABEL "Issue Date" 
  FIELD lieferdatum             AS CHARACTER FORMAT "x(8)"      LABEL "Required" 
  FIELD pos                     AS INTEGER INITIAL 999999
  FIELD artnr                   AS INTEGER FORMAT ">>>>>>>"     LABEL "ArtNo" 
  FIELD bezeich                 AS CHARACTER FORMAT "x(60)"     LABEL "Description" 
  FIELD qty                     AS DECIMAL FORMAT ">>>,>>9.99" 
  FIELD str3                    AS CHARACTER FORMAT "x(10)"     LABEL "  Quantity" 
  FIELD dunit                   AS CHARACTER FORMAT "x(6)"      LABEL "D Unit" 
  FIELD lief-einheit            AS DECIMAL 
  FIELD str4                    AS CHARACTER FORMAT "x(6)"      LABEL " Cont."    
  FIELD userinit                AS CHARACTER FORMAT "x(3)"      LABEL "ID "                      
  FIELD pchase-nr               AS CHARACTER FORMAT "x(10)"     LABEL "PO Number" 
  FIELD pchase-date             AS DATE LABEL "Date" 
  FIELD app-rej                 AS CHARACTER FORMAT "x(60)"     LABEL "Approve/Reject"
  FIELD rej-reason              AS CHARACTER FORMAT "x(36)"     LABEL "Reject Reason"
  FIELD cid                     AS CHARACTER FORMAT "x(3)"      LABEL "CID"
  FIELD cdate                   AS DATE                         LABEL "CancDate"
  FIELD instruct                AS CHARACTER FORMAT "x(36)"     LABEL "Remark"
  FIELD konto                   LIKE gl-acct.fibukonto 
  /*M 04/06/12 -> show item based on selected supplier */
  FIELD supNo                   AS INTEGER 
  FIELD currNo                  AS INTEGER
  FIELD duprice                 AS DECIMAL
  FIELD du-price1               AS DECIMAL
  FIELD du-price2               AS DECIMAL
  FIELD du-price3               AS DECIMAL
  FIELD anzahl                  AS INTEGER FORMAT ">>>,>>9.99"
  FIELD txtnr                   LIKE l-order.txtnr
  FIELD suppn1                  AS CHARACTER FORMAT "x(30)"
  FIELD supp1                   AS INTEGER
  FIELD suppn2                  AS CHARACTER FORMAT "x(30)"
  FIELD supp2                   AS INTEGER
  FIELD suppn3                  AS CHARACTER FORMAT "x(30)"
  FIELD supp3                   AS INTEGER
  FIELD supps                   AS CHAR      FORMAT "x(35)"     LABEL "Supplier Selected"
  FIELD einzelpreis             AS DECIMAL FORMAT "->>>,>>>,>>>,>>9.99"    LABEL "DU Price"
...
/* Dzikri 9374A9 - Req last supplier */
DEFINE BUFFER b-lorder     FOR l-order.
DEFINE BUFFER b-lorderhdr  FOR l-orderhdr.
...
DEF INPUT  PARAMETER sort-app       AS CHARACTER.
DEF OUTPUT PARAMETER TABLE FOR s-list.

DEFINE VARIABLE estimated AS INT.
DEFINE VARIABLE testimated AS INT.
...
FOR EACH s-list:
    ...
    DO:
        ASSIGN s-list.lief-fax2 = l-orderhdr.lief-fax[2].
    END.
END.
...
DEFINE BUFFER tbuff FOR s-list.
  ...
  DO: 
    ...
    FIND FIRST l-orderhdr WHERE l-orderhdr.betriebsnr GE 9 
      ...
      AND l-orderhdr.lief-nr = 0 USE-INDEX l-orderhdr-dat NO-LOCK NO-ERROR. /*Alder - Serverless - Issue 821*/  /*NO-ERROR*/
    IF AVAILABLE l-orderhdr THEN                                            /*Alder - Serverless - Issue 821*/  /*IF AVAILABLE*/
    DO: 
        ...
        ASSIGN  s-list.rej-reason = TRIM(ENTRY(3, l-orderhdr.lief-fax[2], "|"))
                s-list.app-rej    = ENTRY(1,TRIM(ENTRY(4, ENTRY(1, l-orderhdr.lief-fax[2], "|"), ";"))," ") + " "
                                  + ENTRY(2,TRIM(ENTRY(4, ENTRY(1, l-orderhdr.lief-fax[2], "|"), ";"))," ")
                                  + " " + TRIM(ENTRY(2, l-orderhdr.lief-fax[2], "|"))
        .
        ...
        FIND FIRST l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
          ...
          AND l-order.loeschflag = sorttype USE-INDEX order_ix NO-LOCK NO-ERROR.
        DO WHILE AVAILABLE l-order:
            ...
            ASSIGN 
                ...
                FOR EACH l-pprice WHERE l-pprice.artnr = l-artikel.artnr NO-LOCK,
                    ...
                    BY l-pprice.bestelldatum DESC:
                      ASSIGN s-list.last-pdate  = l-pprice.bestelldatum
                             s-list.last-pbook  = l-pprice.einzelpreis
                             s-list.a-firma     = t-lieferant.firma       /* Dzikri 8A3438 - longtime load for last purchase suplier */
                      .
                      LEAVE.
                END.
                ...
                DO:
                    ASSIGN s-list.desc-coa       = gl-acct.bezeich.
                END. 
          ...
          FIND NEXT l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
             ...
             AND l-order.loeschflag = sorttype USE-INDEX order_ix NO-LOCK NO-ERROR.
        END.
    END. /*Alder - Serverless - Issue 821*/ /*END*/
    RETURN.  
  END.
  ...
  /*FD Jan 30, 2020 -> REQ Chanti Semarang sorting by approve ticket B3A90E*/
  DO:
    estimated = TIME.
    IF sort-app = "ALL" OR sort-app = "" THEN
    DO:
      FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
        ...
        USE-INDEX l-orderhdr-liefdat NO-LOCK 
        ...
        DO:
          ...
          END.
          IF INDEX(l-orderhdr.lief-fax[2],"|") > 0 THEN
          /* Dzikri A12609 - Reject Reason wrong data */
          ASSIGN  s-list.rej-reason = TRIM(ENTRY(3, l-orderhdr.lief-fax[2], "|"))
                  s-list.app-rej    = ENTRY(1,TRIM(ENTRY(4, ENTRY(1, l-orderhdr.lief-fax[2], "|"), ";"))," ") + " "
                                    + ENTRY(2,TRIM(ENTRY(4, ENTRY(1, l-orderhdr.lief-fax[2], "|"), ";"))," ")
                                    + " " + TRIM(ENTRY(2, l-orderhdr.lief-fax[2], "|"))
          ...
          /*FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
            AND l-order.pos GT 0 AND l-order.lief-nr = 0
            AND l-order.loeschflag = sorttype USE-INDEX order_ix NO-LOCK,
    ...
    ELSE IF sort-app = "No Approve" THEN
    DO:
      FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
        AND l-orderhdr.bestelldatum LE to-date AND l-orderhdr.betriebsnr GE 9
        AND ENTRY(1, l-orderhdr.lief-fax[2], ";") EQ " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") EQ " "
        AND ENTRY(3, l-orderhdr.lief-fax[2], ";") EQ " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") EQ " "
        USE-INDEX l-orderhdr-liefdat NO-LOCK BY l-orderhdr.bestelldatum BY l-orderhdr.docu-nr: 
        app-flag = (l-orderhdr.lief-fax[2] NE "" AND INDEX(l-orderhdr.lief-fax[2],"|") = 0)
            AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " " 
            AND ENTRY(3, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") NE " ". 
        rej-flag = (l-orderhdr.lief-fax[2] NE "" AND INDEX(l-orderhdr.lief-fax[2],"|") > 0). 
        ...
        DO:
          ...
          DO:
            /*FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
              AND l-order.pos GT 0 AND l-order.lief-nr = 0
              AND l-order.loeschflag = sorttype NO-LOCK,
              FIRST l-artikel WHERE l-artikel.artnr = l-order.artnr NO-LOCK
                 BY l-order.docu-nr BY l-artikel.bezeich: */
    ...
    ELSE IF sort-app = "Approve 1" THEN
    DO:
      FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
        AND l-orderhdr.bestelldatum LE to-date AND l-orderhdr.betriebsnr GE 9
        AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") EQ " "
        AND ENTRY(3, l-orderhdr.lief-fax[2], ";") EQ " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") EQ " "
        USE-INDEX l-orderhdr-liefdat NO-LOCK BY l-orderhdr.bestelldatum BY l-orderhdr.docu-nr: 
        app-flag = (l-orderhdr.lief-fax[2] NE "" AND INDEX(l-orderhdr.lief-fax[2],"|") = 0)
            AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " " 
        ...
        DO:
          ...
          IF ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") EQ " "
            AND ENTRY(3, l-orderhdr.lief-fax[2], ";") EQ " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") EQ " " THEN
          DO:
            /*FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
              AND l-order.pos GT 0 AND l-order.lief-nr = 0
    ...
    ELSE IF sort-app = "Approve 2" THEN
    DO:
      FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
        AND l-orderhdr.bestelldatum LE to-date AND l-orderhdr.betriebsnr GE 9
        AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " "
        ...
        DO:
          ...
          IF ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " "
            AND ENTRY(3, l-orderhdr.lief-fax[2], ";") EQ " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") EQ " " THEN
          DO:
            /*FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
              AND l-order.pos GT 0 AND l-order.lief-nr = 0
    ...
    ELSE IF sort-app = "Approve 3" THEN
    DO:
      FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
        AND l-orderhdr.bestelldatum LE to-date AND l-orderhdr.betriebsnr GE 9
        AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " "
        AND ENTRY(3, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") EQ " "
        USE-INDEX l-orderhdr-liefdat NO-LOCK BY l-orderhdr.bestelldatum BY l-orderhdr.docu-nr: 
        app-flag = (l-orderhdr.lief-fax[2] NE "" AND INDEX(l-orderhdr.lief-fax[2],"|") = 0)
            AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " " 
            AND ENTRY(3, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") NE " ". 
        ...
        DO:
          ...
          IF ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " "
            AND ENTRY(3, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") EQ " " THEN
          DO:
            /*FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
              AND l-order.pos GT 0 AND l-order.lief-nr = 0
              AND l-order.loeschflag = sorttype NO-LOCK,
    ...
    ELSE IF sort-app = "Approve 4" THEN
    DO:
      FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
        AND l-orderhdr.bestelldatum LE to-date AND l-orderhdr.betriebsnr GE 9
        AND ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " "
        ...
        DO:
          ...
          IF ENTRY(1, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(2, l-orderhdr.lief-fax[2], ";") NE " "
            AND ENTRY(3, l-orderhdr.lief-fax[2], ";") NE " " AND ENTRY(4, l-orderhdr.lief-fax[2], ";") NE " " THEN
          DO:
            /*FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
              AND l-order.pos GT 0 AND l-order.lief-nr = 0
              AND l-order.loeschflag = sorttype NO-LOCK,
  ...
  END.
  /*End FD*/
  /* Comment FD Move to Approve ALL
  DO: 
    FOR EACH l-orderhdr WHERE l-orderhdr.bestelldatum GE from-date 
      AND l-orderhdr.bestelldatum LE to-date AND l-orderhdr.betriebsnr GE 9 
...
DEFINE BUFFER tbuff FOR s-list.
  ...
  DO: 
    ...
    END.
    IF INDEX(l-orderhdr.lief-fax[2],"|") > 0 THEN
    ASSIGN  s-list.rej-reason = TRIM(ENTRY(2, l-orderhdr.lief-fax[2], "|"))
            s-list.app-rej          = TRIM(ENTRY (4, ENTRY(1, l-orderhdr.lief-fax[2], "|"), ";"))
            .
    IF AVAILABLE l-order THEN s-list.loeschflag = l-order.loeschflag. 
  ...
  DO: 
    ...
    FIND FIRST parameters WHERE parameters.progname = "CostCenter" 
      ...
      DO:
        ...
        FOR EACH l-order WHERE l-order.docu-nr = l-orderhdr.docu-nr 
          AND l-order.pos GT 0 AND l-order.lief-nr = 0 NO-LOCK, 
          FIRST l-artikel WHERE l-artikel.artnr = l-order.artnr NO-LOCK 
          BY l-order.docu-nr BY l-artikel.bezeich: 
          FIND FIRST usr WHERE usr.username = l-order.lief-fax[1] NO-LOCK. 
