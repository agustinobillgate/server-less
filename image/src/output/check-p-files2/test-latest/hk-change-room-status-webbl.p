...
DEFINE INPUT PARAMETER remark         AS CHAR.  
...
DO:
  ...
  DO:
    ...
    ELSE
    DO:
      RUN hk-statadmin-chg-zistatus-cldbl.p
        (INPUT-OUTPUT TABLE bline-list, INPUT-OUTPUT TABLE om-list,  
          0, ci-date, chgsort, "", user-init, bediener.nr, remark,
          OUTPUT curr-zinr, OUTPUT curr-stat, OUTPUT TABLE z-list).  
      msg-str = translateExtended ("Status Changed Success.",lvCAREA,"").
    END.
  END.
  ELSE
  DO:
