...
FOR EACH debitor WHERE debitor.artnr = artikel.artnr 
  ...
  AND debitor.opart LE opart AND debitor.rgdatum GE fr-date AND 
  debitor.rgdatum LE to-date AND debitor.zahlkonto = 0 NO-LOCK:
    do-it = YES.
    ...
    DO:
      CREATE ar-list.
      ASSIGN ar-list.arRecid = RECID(debitor).
    END.
END.

FOR EACH debitor WHERE debitor.artnr = artikel.artnr 
  AND debitor.name GE from-name
  AND debitor.opart GE opart1 AND debitor.opart LE opart2 
  AND debitor.rgdatum GE fr-date AND 
  debitor.rgdatum LE to-date AND debitor.zahlkonto GT 0 NO-LOCK:
  do-it = YES.
  IF debitor.opart = 2 AND debitor.rgdatum LT bDate THEN
  DO:
    FIND FIRST debt WHERE debt.rechnr = debitor.rechnr 
        AND debt.counter = debitor.counter 
        AND debt.artnr = debitor.artnr 
  ...
  DO:
    FIND FIRST debt WHERE debt.rechnr = debitor.rechnr 
      AND debt.counter = debitor.counter 
      AND debt.artnr = debitor.artnr 
      AND debt.zahlkonto = 0 NO-LOCK NO-ERROR.
    IF AVAILABLE debt THEN
    DO:
      FIND FIRST ar-list WHERE ar-list.arRecid = INTEGER(RECID(debt))
        NO-ERROR.
      IF NOT AVAILABLE ar-list THEN
      DO:
        CREATE ar-list.
        ASSIGN ar-list.arRecid = RECID(debt).
      END.
    END.
  END.
END.
...
FOR EACH ar-list,
  FIRST debitor WHERE INTEGER(RECID(debitor)) = ar-list.arRecid NO-LOCK, 
    ...
    DO:
        ...
        IF AVAILABLE bill THEN
          FIND FIRST reservation WHERE reservation.resnr = bill.resnr NO-LOCK 
            NO-ERROR.
        IF AVAILABLE bill AND AVAILABLE reservation THEN
          /*FOR EACH res-line WHERE res-line.resnr = reservation.resnr 
            AND res-line.zipreis GT 0 NO-LOCK BY res-line.reslinnr:
            gname = res-line.NAME.
            ankunft = res-line.ankunft.
            abreise = res-line.abreise.
            LEAVE.
          END.*/
          FOR EACH res-line WHERE res-line.resnr = bill.resnr 
                ...
                LEAVE.
           END.
        IF AVAILABLE reservation THEN voucherno = reservation.vesrdepot.
    END.
    ELSE
    DO:
        /*MT 130812 */
...
FOR EACH age-list:
    ...
    age-list.resname = guest.name + ", " + guest.vorname1 + guest.anredefirma 
            + " " + guest.anrede1 
            + chr(10) + guest.adresse1 
            + chr(10) + guest.wohnort + " " + guest.plz 
            + chr(10) + guest.land. 
    age-list.comp-name = guest.name + ", " + guest.vorname1 + guest.anredefirma 
            + " " + guest.anrede1 .
    age-list.comp-add = guest.adresse1 + " " + guest.wohnort + " " + guest.plz
        + " " + guest.land.
    age-list.comp-fax = guest.fax.
    age-list.comp-phone = guest.telefon.
END.
