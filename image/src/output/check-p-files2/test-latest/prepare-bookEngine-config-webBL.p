...
DO:
    ...
    DO i = 1 TO NUM-ENTRIES(queasy.char1,";"):
        ...
        /*ELSE IF SUBSTR(str,1,10) = "$progname$"  THEN t-list.progavail1      = SUBSTR(str,11). t-list.progavail1 -- > temp-str FT serverless*/
        ELSE IF SUBSTR(str,1,10) = "$progname$"  THEN temp-str              = SUBSTR(str,11).
...
PROCEDURE fill-list:
    ...
    IF NUM-ENTRIES(temp-str,"=") GT 1 THEN
    DO:
        ASSIGN
            t-list.progavail       = ENTRY(1,temp-str,"=")
            /*dyna-code       = ENTRY(2,t-list.progavail1,"=")*/
        .
        IF NUM-ENTRIES(temp-str,"=") GE 3 THEN
            t-list.pushratebypax   = LOGICAL(ENTRY(3,temp-str,"=")).
        ELSE t-list.pushratebypax  = NO.

        IF NUM-ENTRIES(temp-str,"=") GE 4 THEN
            t-list.upperCaseName   = LOGICAL(ENTRY(4,temp-str,"=")).
        ELSE t-list.upperCaseName  = NO.

        IF NUM-ENTRIES(temp-str,"=") GE 5 THEN
            ASSIGN
                t-list.delayRate  = INT(ENTRY(5,temp-str,"="))
                t-list.delayPull  = INT(ENTRY(6,temp-str,"="))
                t-list.delayAvail = INT(ENTRY(7,temp-str,"=")).
        ELSE
            ASSIGN
        ...
        IF NUM-ENTRIES(temp-str,"=") GE 8 THEN
            ASSIGN
                t-list.pushAll      = LOGICAL(ENTRY(8,temp-str,"="))
                t-list.re-calculate = LOGICAL(ENTRY(9,temp-str,"=")).
        ELSE 
            ASSIGN
                t-list.pushAll      = NO
                t-list.re-calculate = NO.

        IF NUM-ENTRIES(temp-str,"=") GE 10 THEN
            ASSIGN
                t-list.restriction   = LOGICAL(ENTRY(10,temp-str,"="))
                t-list.allotment     = LOGICAL(ENTRY(11,temp-str,"="))
                t-list.pax           = INT(ENTRY(12,temp-str,"="))
                t-list.bedsetup      = LOGICAL(ENTRY(13,temp-str,"=")).

        IF NUM-ENTRIES(temp-str,"=") GE 14 THEN
            ASSIGN
                t-list.pushbookflag  = LOGICAL(ENTRY(14,temp-str,"="))
                t-list.delaypushbook = INT(ENTRY(15,temp-str,"=")).

        IF NUM-ENTRIES(temp-str,"=") GE 16 THEN
            ASSIGN
                t-list.vcWSAgent     = ENTRY(16,temp-str,"=")
                t-list.vcWSAgent2    = ENTRY(17,temp-str,"=")
                t-list.vcWSAgent3    = ENTRY(18,temp-str,"=")
                t-list.vcWSAgent4    = ENTRY(19,temp-str,"=")               
                t-list.vcWebHost     = ENTRY(20,temp-str,"=")
                t-list.vcWebPort     = ENTRY(21,temp-str,"=")                
            .

        IF NUM-ENTRIES(temp-str,"=") GE 22 THEN
            ASSIGN
                t-list.email         = ENTRY(22,temp-str,"=").

        IF NUM-ENTRIES(temp-str,"=") GE 23 THEN
            t-list.vcWSAgent5    = ENTRY(23,temp-str,"=").

        IF NUM-ENTRIES(temp-str,"=") GE 24 THEN
        DO:
            t-list.inclTentative    = LOGICAL(ENTRY(24,temp-str,"=")).
        END.
    END. 
    ELSE 
        ASSIGN
            t-list.progavail       = temp-str
            t-list.dyna-code       = ""
