DEF TEMP-TABLE b1-list
    ...
    FIELD bill-datum    LIKE bill-line.bill-datum  /*naufal - add field created date*/
    FIELD vesrdepot2    LIKE bill.vesrdepot2 /*FDL - Cashless Feature*/
    .
...
PROCEDURE disp-bill-list0:
  ...
  DO: 
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.resnr = 0 AND bill.reslinnr = 1 AND bill.billtyp = dept,
      /*FIRST bill-line WHERE bill-line.rechnr EQ bill.rechnr USE-INDEX rechnr_index NO-LOCK, /*Naufal - add validation to get first transaction date*/*/
  ...
  DO: 
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.rechnr GE rechnr AND bill.resnr EQ 0 AND bill.reslinnr = 1 AND bill.billtyp = dept,
      /*FIRST bill-line WHERE bill-line.rechnr EQ bill.rechnr USE-INDEX rechnr_index NO-LOCK, /*Naufal - add validation to get first transaction date*/*/
  ...
  DO: 
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.rechnr EQ rechnr AND bill.resnr EQ 0 AND bill.reslinnr = 1 AND bill.billtyp = dept,
      /*FIRST bill-line WHERE bill-line.rechnr EQ bill.rechnr USE-INDEX rechnr_index NO-LOCK, /*Naufal - add validation to get first transaction date*/*/
      FIRST guest1 WHERE guest1.gastnr = bill.gastnr NO-LOCK BY bill.rechnr:
        RUN cr-table.
    END.
    ...
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      ...
      AND bill.resnr EQ 0 AND bill.reslinnr = 1 AND bill.billtyp = dept,
      /*FIRST bill-line WHERE bill-line.rechnr EQ bill.rechnr USE-INDEX rechnr_index NO-LOCK, /*Naufal - add validation to get first transaction date*/*/
      FIRST guest1 WHERE guest1.gastnr = bill.gastnr NO-LOCK BY bill.rechnr:
        RUN cr-table.
    END.
...
PROCEDURE disp-bill-list1:
  ...
  DO: 
    ...
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.rechnr GE rechnr AND bill.rechnr LE (rechnr + 100) 
      AND bill.resnr EQ 0 AND bill.reslinnr = 1 AND bill.billtyp = dept,
      /*FIRST bill-line WHERE bill-line.rechnr EQ bill.rechnr USE-INDEX rechnr_index NO-LOCK, /*Naufal - add validation to get first transaction date*/*/
      FIRST guest1 WHERE guest1.gastnr = bill.gastnr NO-LOCK BY bill.rechnr:
        RUN cr-table.
...
PROCEDURE cr-table:
    ...
    ASSIGN
      ...
      b1-list.bemerk        = guest1.bemerk + " " + bill.vesrdepot /* change by damen 02/05/2023 1FB354*/
      b1-list.plz           = guest1.plz
      b1-list.vesrdepot2    = bill.vesrdepot2  /*FDL - Cashless Feature*/
      .

      FIND FIRST bill-line WHERE bill-line.rechnr EQ bill.rechnr NO-LOCK NO-ERROR. /*Naufal - add validation to get first transaction date*/
      IF AVAILABLE bill-line THEN
      DO:
        b1-list.bill-datum    = bill-line.bill-datum. /*naufal - add assign field created date*/
      END.      
END.
