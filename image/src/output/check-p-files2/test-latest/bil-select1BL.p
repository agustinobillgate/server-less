...
PROCEDURE disp-bill-list0:
  ...
  DO: 
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.resnr = 0 AND bill.reslinnr = 1 AND bill.billtyp = dept NO-LOCK, 
  ...
  DO: 
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.rechnr GE rechnr AND bill.resnr EQ 0 AND bill.reslinnr = 1 
      AND bill.billtyp = dept NO-LOCK, 
  ...
  DO: 
    FOR EACH bill WHERE /* bill.zinr EQ "" AND */ 
      bill.flag = bil-flag 
      AND bill.rechnr EQ rechnr 
      AND bill.resnr EQ 0 AND bill.reslinnr = 1 AND bill.billtyp = dept NO-LOCK, 
      FIRST guest1 WHERE guest1.gastnr = bill.gastnr NO-LOCK BY bill.rechnr:
        RUN cr-table.
    END.
