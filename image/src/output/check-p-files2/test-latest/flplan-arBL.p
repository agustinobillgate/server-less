...
PROCEDURE fill-ooo:
    ...
    FIND FIRST outorder WHERE outorder.zinr = rmNo
        AND NOT outorder.gespstart GT ci-date /* Malik Serverless : res-line.abreise -> ci-date */
        AND NOT outorder.gespende  LE ci-date /* Malik Serverless : res-line.ankunft -> ci-date  */
        NO-LOCK NO-ERROR.
    IF AVAILABLE outorder THEN
