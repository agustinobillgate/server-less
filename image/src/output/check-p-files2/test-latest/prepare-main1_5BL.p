...
DEF OUTPUT PARAM p-473              AS LOGICAL.
DEF OUTPUT PARAM new-setup          AS LOGICAL.
DEF OUTPUT PARAM lic-nr             AS CHAR.
DEF OUTPUT PARAM p-282              AS LOGICAL.
DEF OUTPUT PARAM p-1371             AS LOGICAL.
...
IF AVAILABLE htparam THEN p-473 = htparam.flogical.
...
RUN htplogic.p (1371,  OUTPUT p-1371).

FIND FIRST htparam WHERE htparam.paramnr = 1371 AND htparam.bezeich NE "not used" NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN 
DO: 
    ASSIGN p-1371 = htparam.flogical.
END.
ELSE
DO:
    p-1371 = NO.
...
IF AVAILABLE htparam THEN DO: 
    ASSIGN p-282 = htparam.flogical.

    /*IF p-282 = NO THEN DO:*/
        FIND FIRST queasy WHERE queasy.KEY = 159
               AND queasy.char1 MATCHES "*E1-BOOKING*"
               AND queasy.number2 NE 0 NO-LOCK NO-ERROR.
        IF AVAILABLE queasy THEN ASSIGN p-282 = YES.
        ELSE DO:
            FIND FIRST bqueasy WHERE bqueasy.KEY = 159 NO-LOCK NO-ERROR.
            IF NOT AVAILABLE bqueasy THEN ASSIGN p-282 = YES.
            ELSE ASSIGN p-282 = NO.
        END.
    /*END.*/
END.
ELSE DO:
