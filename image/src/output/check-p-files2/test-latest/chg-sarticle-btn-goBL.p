...
DEFINE INPUT PARAMETER dml-art-inp  AS LOGICAL  NO-UNDO.
...
PROCEDURE update-l-artikel: 
    DO TRANSACTION: 
        /*Alder - Serverless - Issue 647 - Start*/
        FIND FIRST l-artikel WHERE RECID(l-artikel) EQ t-recid NO-LOCK NO-ERROR.
        IF AVAILABLE l-artikel THEN
        DO:
            ...
            ASSIGN l-artikel.artnr = artnr
                   ...
                   l-artikel.bestellt = dml-art-inp
            ...
            FIND FIRST queasy WHERE queasy.KEY = 20 AND queasy.number1 EQ artnr EXCLUSIVE-LOCK NO-ERROR. 
            IF ss-artnr[1] NE 0 OR ss-artnr[2] NE 0 OR ss-artnr[3] NE 0 OR picture-file NE "" THEN
            DO: 
            ...
            RELEASE l-artikel.
            /*Alder - Serverless - Issue 647 - End*/
        END.
    END. /* transaction */ 
END. 
...
DEFINE buffer l-art1 FOR l-artikel. 
    ...
    FIND FIRST dml-art WHERE dml-art.artnr = original-art NO-LOCK NO-ERROR.
    ...
    FIND FIRST dml-artdep WHERE dml-artdep.artnr = original-art NO-LOCK NO-ERROR.
