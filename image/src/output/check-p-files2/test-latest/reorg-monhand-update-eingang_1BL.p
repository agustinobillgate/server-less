...
/*ITA 23/07/18*/
/*direct-issue*/
FOR EACH l-op WHERE (op-art = 1 OR op-art = 2) AND l-op.loeschflag EQ 2 
    AND (l-op.datum GE from-date AND l-op.datum LE to-date) 
    AND l-op.pos GE 1 AND l-op.lager-nr GT 0 NO-LOCK BY l-op.artnr: 
    FIND FIRST l-artikel WHERE l-artikel.artnr = l-op.artnr NO-LOCK NO-ERROR. 
    IF AVAILABLE l-artikel AND l-artikel.endkum GE main-grp 
    AND l-artikel.endkum LE to-grp AND l-op.flag = YES THEN RUN checkin-outgoing.
END. 

PROCEDURE checkin-outgoing:
    ...
    DO: 
      ...
      DO TRANSACTION: 
        ...
        ASSIGN
          ...
          l-bestand.anz-eingang     = l-bestand.anz-eingang + /*(anzahl * l-artikel.lief-einheit) */ anzahl
      ...
      ELSE DO TRANSACTION:
          FIND FIRST bbest WHERE RECID(bbest) = RECID(l-bestand)
              EXCLUSIVE-LOCK.
          ASSIGN
            bbest.anz-eingang  = bbest.anz-eingang + /*(anzahl * l-artikel.lief-einheit) */ anzahl
