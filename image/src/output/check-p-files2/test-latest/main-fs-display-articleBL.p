...
FOR EACH ol: 
    DELETE ol. 
END. 
FIND FIRST bk-rart WHERE bk-rart.veran-nr = b1-resnr 
    AND bk-rart.veran-resnr = resline USE-INDEX nr-pg-ug-ix NO-LOCK NO-ERROR. 
...
END.
RUN create-bkrart.
/*FDL June 17, 2025: Ticket #588 - Chg from for each first to for each find first*/
FOR EACH bkrart NO-LOCK:
    FIND FIRST bk-func WHERE bk-func.veran-nr = bkrart.veran-nr 
        AND bk-func.veran-seite = bkrart.veran-seite NO-LOCK NO-ERROR.
