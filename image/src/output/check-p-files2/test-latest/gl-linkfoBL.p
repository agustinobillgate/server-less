...
*/
          ...
          DO: 
            ASSIGN
                ...
                service = wert * serv 
                tax     = wert * vat       /*william open gerald's comment - 6856F4 - 07/06/23*/
                tax2    = wert * vat2
                /*
                service = wert * serv
                tax     = wert * vat
            ...
            IF price-decimal EQ 0 THEN 
                ASSIGN service = ROUND(service,0)
                       tax     = ROUND(tax,0)
                       tax2    = ROUND(tax2,0).
            ELSE ASSIGN service = ROUND(service, price-decimal)
                       tax      = ROUND(tax, price-decimal)
                       tax2     = ROUND(tax2, price-decimal).

            ASSIGN sales   = umsatz.betrag - service - tax - tax2.  
      ...
      /*ITA 16/12/24 - Leasing Feature*/
      FOR EACH artikel WHERE artikel.departement = hoteldpt.num 
        AND artikel.artart = 13 
        NO-LOCK BY artart-list[artikel.artart + 1] BY artikel.artnr: 
        ...
        FOR EACH umsatz WHERE umsatz.datum EQ curr-date 
          ...
          DO: 
            ...
            DO: 
              ...
              /*ITA 16/12/24 - Create divvered Rental*/
              ...
              FIND FIRST g-list WHERE g-list.artnr = bartikel.artnr 
                AND g-list.dept = bartikel.departement 
                AND g-list.fibukonto = fibukonto 
                AND g-list.datum = curr-date /* AND g-list.credit NE 0 */ 
                NO-ERROR. 
 
              IF NOT AVAILABLE g-list THEN RUN add-list(0, YES, 
                (STRING(hoteldpt.num) + " - " + bartikel.bezeich 
                 + ";&&;" + STRING(bartikel.departement) + ";" 
                 + STRING(bartikel.artnr)), bartikel.artnr, bartikel.departement). 
 
              ELSE RUN add-list(0, NO, 
                (STRING(hoteldpt.num) + " - " + bartikel.bezeich), 
                 bartikel.artnr, bartikel.departement). 
  ...
  FOR EACH g-list NO-LOCK, 
    FIRST gl-acct1 WHERE gl-acct1.fibukonto = g-list.fibukonto NO-LOCK 
      ...
      ASSIGN
          ...
          /*buf-g-list.debit          = ROUND(g-list.debit, price-decimal)
          buf-g-list.credit         = ROUND(g-list.credit, price-decimal)
          credits                   = credits + buf-g-list.credit
          debits                    = debits + buf-g-list.debit*/ . 
  ...
  END.
  /*ASSIGN
      credits = ROUND(credits, price-decimal) 
      debits  = ROUND(debits, price-decimal).*/

  /*ITA -- revisi rounding
  IF debits GT credits THEN DO:
      IF (debits - credits) LE 3 THEN DO:
          FIND FIRST buf-g-list WHERE buf-g-list.credit NE 0 NO-ERROR.
          IF AVAILABLE buf-g-list THEN 
              ASSIGN buf-g-list.credit = buf-g-list.credit + (debits - credits)
                     credits           = credits + (debits - credits).
      END.
  END.
  ELSE IF credits GT debits THEN DO:
      IF (credits - debits) LE 3 THEN DO:
          FIND FIRST buf-g-list WHERE buf-g-list.debit NE 0 NO-ERROR.
          IF AVAILABLE buf-g-list THEN 
              ASSIGN buf-g-list.debit = buf-g-list.debit + (credits - debits)
                     debits           = debits + (credits - debits).
      END.
  END.
  ASSIGN remains = debits - credits. */
END. 
...
DEFINE INPUT PARAMETER dept AS INTEGER. 
  ...
  /*credits = credits + ROUND(debit-betrag, price-decimal). 
  debits = debits + ROUND(credit-betrag, price-decimal).*/
