...
PROCEDURE create-list: 
  ...
  DEFINE VARIABLE argt-betrag-out AS DECIMAL. /* Malik Serverless : argt-betrag -> argt-betrag-out */
  DEFINE VARIABLE lodging AS DECIMAL FORMAT "->,>>>>,>>>,>>9.99". 
  DEFINE VARIABLE i AS INTEGER. 
  DEFINE VARIABLE n AS INTEGER. 
  DEFINE VARIABLE tmp-value AS DECIMAL. /* Malik Serverless */
  DEFINE VARIABLE tmp-value2 AS DECIMAL. /* Malik Serverless */
  DEFINE VARIABLE tmp-value3 AS DECIMAL. /* Malik Serverless */
    ...
    DO: 
      ...
      FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr 
        ...
        RUN argt-betrag.p(RECID(res-line), RECID(argt-line), 
          OUTPUT argt-betrag-out, OUTPUT ex-rate). /* Malik Serverless : OUTPUT argt-betrag -> OUTPUT argt-betrag-out */
        lodging = lodging - argt-betrag-out. /* Malik Serverless : argt-betrag -> argt-betrag-out */
      END. 
      tmp-value = lodging * frate. /* Malik Serverless */
      lodging = round(tmp-value, price-decimal). /* Malik Serverless : round(lodging * frate, price-decimal) -> round(tmp-value, price-decimal) */
 ...
 FOR EACH test WHERE test.xx MATCHES ("") AND NOT test.yy MATCHES ("G.TOTAL") :
     ...
     FOR EACH cl-list WHERE cl-list.etage = test.yy:
       /* Malik Serverless */
       IF fact NE 0 AND fact NE ? THEN 
       DO:
        tmp-value2 = cl-list.rev / fact. /* Malik serverless */
       END.
       ELSE
       DO:
        tmp-value2 = 0.
       ...
       /* END Malik */        
         ...
         cl-list.nrev = round(tmp-value2, price-decimal). /* Malik Serverless : round(cl-list.rev / fact, price-decimal) -> round(tmp-value2, price-decimal) */
 ...
 END.
  ...
  FOR EACH test WHERE NOT test.xx MATCHES (""):
         ...
         FOR EACH cl-list WHERE cl-list.loc MATCHES test.xx:
            ...
            IF fact NE 0 AND fact NE ? THEN 
            DO:
              tmp-value3 = cl-list.rev / fact. /* Malik Serverless */
            END.
            ELSE
            DO:
              tmp-value3 = 0.
            ...
            /* END Malik */
             ...
             cl-list.sum = cl-list.occ + cl-list.hu + cl-list.ooo 
               + cl-list.com + cl-list.vac. 
             cl-list.nrev = round(tmp-value3, price-decimal). /* Malik Serverless : round(cl-list.rev / fact, price-decimal) -> round(tmp-value3, price-decimal) */
             t-occ = t-occ + cl-list.occ. 
             t-hu = t-hu + cl-list.hu. 
             t-ooo = t-ooo + cl-list.ooo. 
             t-com = t-com + cl-list.com. 
...
PROCEDURE gr-tot:
  DEFINE VARIABLE tmp-cl AS DECIMAL. /* Malik Serverless */
    t-occ = 0.
    t-hu = 0.
    t-ooo = 0.
    t-com = 0.
    ...
    FOR EACH cl-list BY cl-list.etage: 
      ...
      IF fact NE 0 AND fact NE ? THEN 
      DO:
        tmp-cl = cl-list.rev / fact. /* Malik Serverless */
      END.
      ELSE
      DO:
        tmp-cl = 0.
      ...
      cl-list.sum = cl-list.occ + cl-list.hu + cl-list.ooo 
        + cl-list.com + cl-list.vac. 
      cl-list.nrev = round(tmp-cl, price-decimal). /* Malik Serverless : round(cl-list.rev / fact, price-decimal) -> round(tmp-cl, price-decimal) */
      t-occ = t-occ + cl-list.occ. 
      t-hu = t-hu + cl-list.hu. 
      t-ooo = t-ooo + cl-list.ooo. 
      t-com = t-com + cl-list.com. 
      ...
      t-pax = t-pax + cl-list.pax. 
      t-cpax = t-cpax + cl-list.cpax. 
      /* STRING (t-etage) = cl-list.etage. */     /* Rulita 281024 | Fixing for serverless */
    END. 
