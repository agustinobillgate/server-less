...
FOR EACH artikel WHERE (artikel.artart = 2 OR 
    ...
    FOR EACH billjournal WHERE billjournal.userinit = user-init 
        ...
        NO-LOCK: 
        DO TRANSACTION: 
            FIND FIRST journal WHERE RECID(journal) = RECID(billjournal) NO-LOCK NO-ERROR. /*Alder - Serverless - Issue 741*/ /*EXCLUSIVE-LOCK -> NO-LOCK NO-ERROR*/
            IF AVAILABLE journal THEN
            DO:
                FIND CURRENT journal EXCLUSIVE-LOCK. /*Alder - Serverless - Issue 741*/ /*EXCLUSIVE-LOCK */
