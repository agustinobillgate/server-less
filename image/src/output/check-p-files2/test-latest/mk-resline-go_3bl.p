...
DEF BUFFER pqueasy FOR queasy.
DEF BUFFER tqueasy FOR queasy.
...
PROCEDURE update-repeat-charge:
    ...
    IF fixed-rate THEN RUN create-fixedRated.*/
    ...
    FIND FIRST pqueasy WHERE pqueasy.KEY = 329 
        AND pqueasy.number1 = res-line.resnr 
        AND pqueasy.number2 = res-line.reslinnr NO-LOCK NO-ERROR.
    IF NOT AVAILABLE pqueasy THEN DO:
        FOR EACH queasy WHERE queasy.KEY = 329 BY queasy.number3:
            ASSIGN counter = queasy.number3 + 1.
            LEAVE.
        END.
        IF counter = 0 THEN ASSIGN counter = 1.

        CREATE tqueasy.
        ASSIGN tqueasy.KEY       = 329
               tqueasy.date1     = ci-date
               tqueasy.date2     = reslin-list.ankunft
               tqueasy.date3     = reslin-list.abreise
               tqueasy.number1   = res-line.resnr
               tqueasy.number2   = res-line.reslinnr
               tqueasy.number3   = counter
               /*tqueasy.deci1     = amount*/
        .
       ...
       END.
        ...
        ASSIGN amount-periode = curr-amount / (MONTH(reslin-list.abreise) - MONTH(reslin-list.ankunft))
               tqueasy.deci1  = curr-amount.
    END.
    ELSE DO:
        FIND CURRENT pqueasy EXCLUSIVE-LOCK.
        ASSIGN pqueasy.date2     = reslin-list.ankunft
               pqueasy.date3     = reslin-list.abreise
         .
       ...
       END.
        ...
        ASSIGN amount-periode = curr-amount / (MONTH(reslin-list.abreise) - MONTH(reslin-list.ankunft))
               pqueasy.deci1  = curr-amount.

        FIND CURRENT pqueasy NO-LOCK.
        RELEASE pqueasy.
...
PROCEDURE create-fixedRated:
  ...
  FIND FIRST periode-list NO-ERROR.
  IF AVAILABLE periode-list THEN DO:
      DO datum = reslin-list.ankunft TO (reslin-list.abreise - 1):
          ASSIGN counter = counter + 1.

          FIND FIRST periode-list WHERE datum GE periode-list.periode1 
                AND datum LE periode-list.periode2 NO-ERROR.
          IF AVAILABLE periode-list THEN DO:
                
                FIND FIRST reslin-queasy WHERE reslin-queasy.key  = "arrangement"
                    AND reslin-queasy.resnr       = reslin-list.resnr
                    AND reslin-queasy.reslinnr    = reslin-list.reslinnr /* new booking */
                    AND reslin-queasy.date1       = datum
                    AND reslin-queasy.date2       = datum NO-LOCK NO-ERROR.
                IF NOT AVAILABLE reslin-queasy THEN DO:
                    CREATE reslin-queasy.
                    ASSIGN
                      reslin-queasy.key         = "arrangement"
                      reslin-queasy.resnr       = reslin-list.resnr
                      reslin-queasy.reslinnr    = reslin-list.reslinnr /* new booking */
                      reslin-queasy.date1       = datum
                      reslin-queasy.date2       = datum
                      reslin-queasy.deci1       = periode-list.amt-periode
                      reslin-queasy.char2       = ""
                      reslin-queasy.char3       = user-init
                    .
                    IF counter = 1 THEN reslin-list.zipreis = periode-list.amt-periode.
                END.
          END.
      END.
  END.      
...
/*    
    ...
    FIND FIRST queasy WHERE queasy.KEY = 301
        AND queasy.number1 = reslin-list.resnr AND queasy.logi1 = YES NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN DO:
        RUN calc-periode.

        DO bill-date = reslin-list.ankunft TO to-date:
            FIND FIRST periode-list WHERE bill-date GE periode-list.periode1 
                    AND bill-date LE periode-list.periode2 NO-ERROR.
            IF AVAILABLE periode-list THEN DO:
                CREATE reslin-queasy.
                ASSIGN
                  reslin-queasy.key      = "arrangement"
...
PROCEDURE calc-periode:
    ...
    IF AVAILABLE queasy THEN DO:
        ASSIGN periode-rsv1 = res-line.ankunft
               periode-rsv2 = res-line.abreise
         .
