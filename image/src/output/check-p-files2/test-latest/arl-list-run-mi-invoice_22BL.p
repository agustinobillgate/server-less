...
DEFINE TEMP-TABLE t-list 
    ...
    FIELD kind1     AS INTEGER 
    FIELD kind2     AS INTEGER
    /*NAUFAL 280521 - change to _1bl add field for VAT and SVC wink*/
    FIELD vat       AS DECIMAL
    FIELD svc       AS DECIMAL
    FIELD zinr      AS CHAR /*roomno Gerald 1D951E*/
    FIELD depo-billjour AS LOGICAL
    FIELD resno-billjour AS INT
    .

DEFINE TEMP-TABLE t-reservation LIKE reservation
  FIELD bill-receiver AS CHARACTER.
...
DEF INPUT PARAMETER split-bill          AS LOGICAL.
...
DEFINE VARIABLE bill-receiver       AS CHARACTER NO-UNDO.
DEFINE VARIABLE argt-options        AS CHARACTER. /* malik Serverless */



/*gerald include split-bill 1D951E*/
DEFINE VARIABLE t-char  AS CHAR.
DEFINE VARIABLE t-resnr AS INTEGER.
...
/* return bill receiver by Oscar (24 Oktober 2024) - 1BA03F */
...
DO:
    ...
    t-reservation.bill-receiver = bill-receiver.
END.
...
PROCEDURE read-proforma-inv: 
  ...
  DEFINE VARIABLE depo-flag       AS LOGICAL              NO-UNDO.

  DEFINE BUFFER w1                FOR vhp.waehrung. 
  DEFINE BUFFER resline           FOR vhp.res-line. 
  DEFINE BUFFER buff-bill-line    FOR bill-line. 
  DEFINE BUFFER buff-art          FOR artikel. 

  DEF VAR i AS INT.
  DEF VAR j AS INT.
  DEF VAR qty1 AS INT.
  DEF VAR argt-rgbez-temp AS CHAR. /* Malik Serverless 146 */
  DEF VAR argtnr-temp AS INT. /* Malik Serverless 146 */
  DEF VAR zimkateg-kurzbez-tmp AS CHAR. /* Malik Serverless 698 */
  ...
  FOR EACH resline WHERE resline.resnr = resnr 
    ...
    FIND FIRST zimkateg WHERE zimkateg.zikatnr = resline.zikatnr NO-LOCK NO-ERROR. 
    /* Malik Serverless 698 handle if available */
    IF AVAILABLE zimkateg THEN zimkateg-kurzbez-tmp = vhp.zimkateg.kurzbez.
    ELSE zimkateg-kurzbez-tmp = "".
   
    FIND FIRST arrangement WHERE arrangement.arrangement = resline.arrangement NO-LOCK NO-ERROR. /* Malik Serverless 146 : NO-LOCK -> NO-LOCK NO-ERROR */
    IF AVAILABLE arrangement THEN
    DO:
      ASSIGN
        argt-rgbez-temp = vhp.arrangement.argt-rgbez
        argtnr-temp = vhp.arrangement.argtnr
        argt-options = vhp.arrangement.options.
    END.
    ELSE
    DO:
      ASSIGN
    ...
    /*    IF resline.erwachs GT 0 THEN */ 
      ...
      DO: 
        FIND FIRST reslin-queasy WHERE reslin-queasy.key = "arrangement" 
          ...
          AND datum LE vhp.reslin-queasy.date2 FT serverless*/ NO-LOCK NO-ERROR. 
      ...
      DO: 
        ...
        /* Rudy 190325, error vhpFOR/arlListRunMiInvoice22  */
          ...
          DO: 
            ...
            DO:
              ...
              RUN ratecode-rate.p(ebdisc-flag, kbdisc-flag, resline.resnr, 
                ...
                resline.abreise, resline.reserve-int, argtnr-temp, /* Malik Serverless 146 vhp.arrangement.argtnr -> argtnr-temp */
            ...
            DO:
              RUN pricecod-rate.p(resline.resnr, resline.reslinnr,
                ...
                resline.reserve-int, argtnr-temp, curr-zikatnr, /* Malik Serverless 146 vhp.arrangement.argtnr -> argtnr-temp */
        ...
        END. /* available vhp.guest */
      END.     /* IF contract rate */ 

      FIND FIRST s-list WHERE s-list.bezeich = argt-rgbez-temp /* Malik Serverless vhp.arrangement.argt-rgbez -> argt-rgbez-temp */
        AND s-list.rmcat = zimkateg-kurzbez-tmp /* Malik Serverless 698 vhp.zimkateg.kurzbez -> zimkateg-kurzbez-tmp */
      ...
      DO: 
        CREATE s-list. 
        s-list.bezeich = argt-rgbez-temp. /* Malik Serverless vhp.arrangement.argt-rgbez -> argt-rgbez-temp */
        s-list.rmcat = zimkateg-kurzbez-tmp. /* Malik Serverless 698 vhp.zimkateg.kurzbez -> zimkateg-kurzbez-tmp */
        ...
        DO: 
          ...
          DEF VAR contcode AS CHAR.
            ...
            FIND FIRST vhp.reslin-queasy WHERE vhp.reslin-queasy.key = "argt-line" 
              ...
              AND vhp.reslin-queasy.number2 = argtnr-temp /* Malik Serverless 146 vhp.arrangement.argtnr -> argtnr-temp */
      ...
      FOR EACH bill-line WHERE bill-line.rechnr = rechnr NO-LOCK,
          ...
          IF do-it THEN DO:
            ...
            RUN calc-servtaxesbl.p (1, artikel.artnr, artikel.departement,
                ...
                OUTPUT fact1).
            ASSIGN
                netto       = bill-line.betrag / fact1
                t-list.vat  = netto * (vat1 + vat3).
          END.
      END.
  END.
  ELSE DO:
    FOR EACH billjournal WHERE billjournal.rechnr = 0 AND billjournal.anzahl NE 0 
        ...
        t-char = ENTRY(2, billjournal.bezeich, "#").
        t-resnr = integer(ENTRY(1,t-char, "]")).

        IF t-resnr = resnr THEN DO:
           ASSIGN 
             serv1   = 0         
             vat1    = 0         
             vat3    = 0         
           ...
           ASSIGN 
               curr-no        = curr-no + 1
               t-list.nr      = curr-no 
               t-list.betrag  = billjournal.betrag
               t-list.bezeich = billjournal.bezeich
               t-list.date1   = billjournal.bill-datum
               t-list.ankunft = res-line.ankunft
               t-list.abreise = res-line.abreise
               .
   
           RUN calc-servtaxesbl.p (1, artikel.artnr, artikel.departement,
              billjournal.bill-datum, OUTPUT serv1, OUTPUT vat1, OUTPUT vat3,
              OUTPUT fact1).
           ASSIGN
              netto       = billjournal.betrag / fact1
              t-list.vat  = netto * (vat1 + vat3).  

           /*FDL April 17, 2023 => Ticket 3855A5*/
           t-list.depo-billjour = YES.
           t-list.resno-billjour = t-resnr.
        END.
    END.

    /*dody 010424, rswitch query, stuck trace, ticket 99A6CF */
    /*FDL April 17, 2023 => Ticket 3855A5*/
    /*
    FOR EACH billjournal WHERE billjournal.rechnr = 0 AND billjournal.anzahl NE 0 
        AND billjournal.bezeich MATCHES "*Refund #*" NO-LOCK:

        t-char = ENTRY(2, billjournal.bezeich, "#").
        t-resnr = integer(ENTRY(1,t-char, "]")).

        FOR EACH t-list WHERE t-list.depo-billjour
            AND t-list.resno-billjour EQ t-resnr:
            
            IF (t-list.betrag + billjournal.betrag) EQ 0 THEN DELETE t-list.
        END.
    END.
    */
    FOR EACH t-list WHERE t-list.depo-billjour:
        FIND FIRST billjournal WHERE billjournal.billjou-ref EQ t-list.resno-billjour 
            AND billjournal.bezeich MATCHES "*Refund #*" NO-LOCK NO-ERROR.
...
PROCEDURE read-proforma-inv1: 
  ...
  DEF VAR argt-rgbez-temp1 AS CHAR. /* Malik Serverless 146 */
  DEF VAR argtnr-temp1 AS INT. /* Malik Serverless 146 */
  DEF VAR zimkateg-kurzbez-tmp AS CHAR. /* Malik Serverless 698 */
  ...
  FOR EACH resline WHERE resline.resnr = resnr 
    ...
    FIND FIRST vhp.zimkateg WHERE vhp.zimkateg.zikatnr = resline.zikatnr NO-LOCK NO-ERROR. 
    /* Malik Serverless 698 handle if available */
    IF AVAILABLE zimkateg THEN zimkateg-kurzbez-tmp = vhp.zimkateg.kurzbez.
    ELSE zimkateg-kurzbez-tmp = "".

    FIND FIRST vhp.arrangement WHERE vhp.arrangement.arrangement = resline.arrangement NO-LOCK. /* Malik Serverless 146 : NO-LOCK -> NO-LOCK NO-ERROR */ 
    IF AVAILABLE arrangement THEN
    DO:
      ASSIGN
        argt-rgbez-temp1 = vhp.arrangement.argt-rgbez
        argtnr-temp1 = vhp.arrangement.argtnr
        argt-options = vhp.arrangement.options.
    END.
    ELSE
    DO:
...
/*    IF resline.erwachs GT 0 THEN */ 
        ...
        DO:
          ...
          DO: 
            ...
            DO:
              ...
              RUN ratecode-rate.p(ebdisc-flag, kbdisc-flag, resline.resnr, 
                resline.reslinnr, vhp.guest-pr.CODE, ?, bill-date, resline.ankunft,
                resline.abreise, resline.reserve-int, argtnr-temp1, /* Malik Serverless 146 vhp.arrangement.argtnr -> argtnr-temp1 */
                curr-zikatnr, resline.erwachs, resline.kind1, resline.kind2,
                resline-exrate, resline.betriebsnr, OUTPUT rate-found,
                OUTPUT rm-rate, OUTPUT early-flag, OUTPUT kback-flag).
              ELSE
              RUN ratecode-rate.p(ebdisc-flag, kbdisc-flag, resline.resnr, 
                resline.reslinnr, vhp.guest-pr.CODE, ?, bill-date, resline.ankunft,
                resline.abreise, resline.reserve-int, argtnr-temp1,
                curr-zikatnr, resline.erwachs, resline.kind1, resline.kind2,
                resline.reserve-dec, resline.betriebsnr, OUTPUT rate-found,
                OUTPUT rm-rate, OUTPUT early-flag, OUTPUT kback-flag).
            END.
            ...
            DO:
              RUN pricecod-rate.p(resline.resnr, resline.reslinnr,
                vhp.guest-pr.CODE, bill-date, resline.ankunft, resline.abreise, 
                resline.reserve-int, argtnr-temp1, curr-zikatnr, 
                resline.erwachs, resline.kind1, resline.kind2,
                resline.reserve-dec, resline.betriebsnr, 
                OUTPUT rm-rate, OUTPUT rate-found).
              /*RUN usr-prog2(datum, INPUT-OUTPUT rm-rate, OUTPUT it-exist).*/
      ...
      FIND FIRST s-list WHERE s-list.bezeich = argt-rgbez-temp1 /* Malik Serverless 146 vhp.arrangement.argt-rgbez -> argt-rgbez-temp1 */
        AND s-list.rmcat = zimkateg-kurzbez-tmp /* Malik Serverless 698 vhp.zimkateg.kurzbez -> zimkateg-kurzbez-tmp */
        AND s-list.preis = rm-rate 
      ...
      DO: 
        CREATE s-list. 
        ASSIGN
          s-list.bezeich = argt-rgbez-temp1 /* Malik Serverless 146 vhp.arrangement.argt-rgbez -> argt-rgbez-temp1 */
          s-list.rmcat = zimkateg-kurzbez-tmp /* Malik Serverless 698 vhp.zimkateg.kurzbez -> zimkateg-kurzbez-tmp */
          s-list.preis = rm-rate
          s-list.lRate = lRate
          s-list.datum = datum 
...
/**** additional fix cost ******/ 
        ...
        DO: 
          ...
          DEF VAR contcode AS CHAR.
            ...
            FIND FIRST vhp.reslin-queasy WHERE vhp.reslin-queasy.key = "argt-line" 
              AND vhp.reslin-queasy.char1 = contcode 
              AND vhp.reslin-queasy.number1 = resline.reserve-int 
              AND vhp.reslin-queasy.number2 = argtnr-temp1 
              AND vhp.reslin-queasy.reslinnr = resline.zikatnr 
              AND vhp.reslin-queasy.number3 = vhp.fixleist.artnr 
              AND vhp.reslin-queasy.resnr = vhp.fixleist.departement 
              AND bill-date GE vhp.reslin-queasy.date1 
  ...
  DO:      
      ...
      ELSE
      DO:
         FOR EACH b-bill WHERE b-bill.resnr = resnr NO-LOCK:
            FOR EACH bill-line WHERE bill-line.rechnr = b-bill.rechnr NO-LOCK,
               FIRST artikel WHERE artikel.artnr = bill-line.artnr
               AND artikel.departement = bill-line.departement NO-LOCK
               BY bill-line.bill-datum BY bill-line.zeit:
               do-it = YES.
...
PROCEDURE create-bonus: 
  ...
  j = 1. 
  DO i = 1 TO 4: 
    stay = INTEGER(SUBSTR(argt-options, j, 2)). /* Malik Serverless 146 : vhp.arrangement.options -> argt-options */
    pay  = INTEGER(SUBSTR(argt-options, j + 2, 2)). 
