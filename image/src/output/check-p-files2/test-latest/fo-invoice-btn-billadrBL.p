...
DEFINE BUFFER bbuff FOR bill. /* Malik Serverless */
...
FIND FIRST bbuff WHERE RECID(bbuff) = bil-recid NO-LOCK NO-ERROR. /* Malik Serverless : bill EXCLUSIVE-LOCK -> bbuff NO-LOCK NO-ERROR */
FIND FIRST guest WHERE guest.gastnr = gastpay NO-LOCK. 

/* Malik Serverless */
IF AVAILABLE bbuff THEN
DO:
