...
DEFINE TEMP-TABLE to-list 
  ...
  FIELD ratecode      AS CHAR    FORMAT "x(15)" /* Room Type from zimkateg.kurzbez */
  FIELD ratecodename  AS CHAR    FORMAT "x(15)" /* Rate Code from guest-pr */
...
DEF VAR cardtype AS INT INIT 3.
...
DEFINE VARIABLE iftask  AS CHARACTER.
DEFINE VARIABLE rate-code   AS CHARACTER INITIAL "".
...
/* Test Data 
FOR EACH to-list WHERE to-list.flag = 1 BY to-list.counter:
    DISP to-list.ratecode 
        to-list.NAME
        to-list.avrgrate
        to-list.m-room
        to-list.y-room. 
...
DEF VAR curr-userID         AS CHAR.
  ...
  /* Add Validation For Sales ID */
  IF sales-ID = ? THEN sales-ID = "".
  ...
  FOR EACH genstat WHERE genstat.datum GE from-date 
      ...
      FIND FIRST zimkateg WHERE zimkateg.zikatnr = genstat.zikatnr
          NO-LOCK NO-ERROR.
      ASSIGN curr-code = zimkateg.kurzbez.

      /* BLY Update Rate Code */
      FIND FIRST res-line WHERE res-line.gastnr EQ guest.gastnr
          AND res-line.zimmer-wunsch MATCHES ("*$OrigCode$*") NO-LOCK NO-ERROR.
      ...
      DO:
          DO i = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch,";") - 1:
              ...
              DO:
                  rate-code = TRIM(ENTRY(3, iftask, "$")).
              END.
          END.
      END.

      IF rate-code EQ "" OR rate-code EQ ? THEN
          ASSIGN curr-ratecode = "UNKNOWN".
      ELSE
          ASSIGN curr-ratecode = rate-code.
      /* End Update */
      ...
      DO:
          ...
          */
          FIND FIRST to-list WHERE to-list.name = curr-ratecode
              AND to-list.ratecode = curr-code NO-LOCK NO-ERROR.
          ...
          DO:
            ...
            ASSIGN
                ...
                to-list.name      = curr-ratecode /* BLY */
            . 
          END.
          ...
          /* BLY 28/11/24 */
          FIND FIRST to-list WHERE to-list.NAME = "UNKNOWN"
              AND to-list.ratecode = curr-code NO-ERROR.
          IF NOT AVAILABLE to-list THEN
          DO:
              CREATE to-list.
              ASSIGN
                  to-list.ratecode  = curr-code
                  to-list.NAME      = "UNKNOWN"
                  .
          END.
...
PROCEDURE create-output:       
    ...
    DO:
      FOR EACH to-list NO-LOCK BY to-list.ratecode BY to-list.NAME: 
        FIND FIRST zimkateg WHERE zimkateg.kurzbez = to-list.ratecode NO-LOCK NO-ERROR.
        ...
        DO:
            ...
            DO:
                ...
                ELSE IF AVAILABLE zimkateg THEN 
                    buff-list.NAME = zimkateg.bezeichnung.
            END.
      ...
      /* BLY - Add Sub Total Last Row */
      IF curr-code NE "" THEN DO:
        i = i + 1.
        CREATE buff-list.
        ASSIGN 
            buff-list.counter = i.
