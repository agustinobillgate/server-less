...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 19 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    /*
    CREATE queasy.
    ASSIGN 
        ...
        queasy.number1  = 1
        queasy.number2  = 19
        queasy.char1    = "Article Number For Discount FO" /*"Department FO Transaction"*/
        queasy.number3  = 1
        queasy.char2    = "" 
        queasy.betriebsnr = dept-no
        . 
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 20  NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    /*
    CREATE queasy.
    ASSIGN 
        ...
        queasy.number1  = 1
        queasy.number2  = 20
        queasy.char1    = "Article Number For Discount FB"  /*"Department FB Transaction 1"*/
        queasy.number3  = 1
        queasy.char2    = "" 
        queasy.betriebsnr = dept-no
        .  */
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 21 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    /*
    CREATE queasy.
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 22 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    /*
    CREATE queasy.
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 23 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    /*
    CREATE queasy.
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 24 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    /*
    CREATE queasy.
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 27 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    CREATE queasy.
    ASSIGN 
        ...
        .  
END.
ELSE queasy.char1 = "Credential For FO (deptno;username;password;tadadept;vhpartdisc)".
/*num3 1=int, 2=decimal, 3=date, 4=logical, 5=char*/
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 28 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    CREATE queasy.
    ASSIGN 
        ...
        .  
END.
ELSE queasy.char1 = "Credential For Outlet (deptno;username;password;tadadept;vhpartdisc)".
/*num3 1=int, 2=decimal, 3=date, 4=logical, 5=char*/
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 29 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    CREATE queasy.
    ASSIGN 
...
FIND FIRST queasy WHERE queasy.KEY EQ 270 
    AND queasy.number1 EQ 1 
    AND queasy.betriebsnr EQ dept-no 
    AND queasy.number2 EQ 30 NO-ERROR.
IF NOT AVAILABLE queasy THEN
DO:
    CREATE queasy.
    ASSIGN 
