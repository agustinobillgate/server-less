...
DEFINE BUFFER debt FOR debitor. 
  ...
  FIND FIRST artikel WHERE artikel.artnr = from-art 
    AND artikel.departement = 0  NO-LOCK NO-ERROR. 
  IF NOT AVAILABLE artikel THEN RETURN. 
 
  FIND FIRST guest WHERE guest.gastnr = curr-gastnr NO-LOCK.
  gastname = guest.name + ", " + guest.vorname1 + " " 
    + guest.anrede1 + guest.anredefirma. 
 
  outlist = "". 
  RUN fill-in-list. 
  outlist = STRING(artikel.artnr,">>>>>9") + " - " 
         + STRING(artikel.bezeich, "x(30)"). 
  RUN fill-in-list.
  
  outlist = "". 
  RUN fill-in-list.
  outlist = "(" + TRIM(STRING(gastname, "x(30)")) + ")".  /*geral 54B832 051020*/
  RUN fill-in-list. 
...
/**** Full paid A/R records *****/ 
  ...
  FOR EACH age-list WHERE age-list.tot-debt NE 0: 
    ...
    IF price-decimal = 0 THEN 
      outlist = STRING(age-list.rechnr,">>>>>>>>9") 
         + STRING(age-list.gastname, "x(39)") 
         + STRING(age-list.tot-debt, "->>>>,>>>,>>>,>>9") 
         + STRING(to-date - age-list.rgdatum, ">>9") 
         + STRING(age-list.debt0, "->>>>,>>>,>>>,>>9") 
         + STRING(age-list.debt1, "->>>>,>>>,>>>,>>9") 
         + STRING(age-list.debt2, "->>>>,>>>,>>>,>>9") 
         + STRING(age-list.debt3, "->>>>,>>>,>>>,>>9"). 
    ELSE outlist = STRING(age-list.rechnr,">>>>>>>>9") 
         + STRING(age-list.gastname, "x(39)") 
         + STRING(age-list.tot-debt, "->,>>>,>>>,>>9.99") 
         + STRING(to-date - age-list.rgdatum, ">>9") 
         + STRING(age-list.debt0, "->,>>>,>>>,>>9.99") 
         + STRING(age-list.debt1, "->,>>>,>>>,>>9.99") 
         + STRING(age-list.debt2, "->,>>>,>>>,>>9.99") 
         + STRING(age-list.debt3, "->,>>>,>>>,>>9.99"). 
    ...
    output-list.inv-soa = "INV" + STRING(age-list.inv-soa, "999999999"). /*Gerald F616B1*/
  END. 
 
  fremdwbetrag = 0. 
  /*IF MENU-ITEM mi-disp:CHECKED IN MENU mbar = YES AND t-fdebt NE 0 THEN */
  fremdwbetrag = t-fdebt. 
  outlist = 
  "-----------------------------------------------------------------------------------------------------------------------------------------------------------". 
  ...
  ASSIGN
    ...
    output-list.inv-no = "---------".
  IF price-decimal = 0 THEN 
  outlist = STRING(translateExtended ("         T O T A L  A/R:",lvCAREA,""), "x(48)") 
    + STRING(t-saldo, "->>>>,>>>,>>>,>>9") 
    + "   " 
    + STRING(t-debt0, "->>>>,>>>,>>>,>>9") 
    + STRING(t-debt1, "->>>>,>>>,>>>,>>9") 
    + STRING(t-debt2, "->>>>,>>>,>>>,>>9") 
    + STRING(t-debt3, "->>>>,>>>,>>>,>>9"). 
  ELSE outlist = STRING(translateExtended ("         T O T A L  A/R:",lvCAREA,""), "x(48)") 
    + STRING(t-saldo, "->,>>>,>>>,>>9.99") 
    + "   " 
    + STRING(t-debt0, "->,>>>,>>>,>>9.99") 
    + STRING(t-debt1, "->,>>>,>>>,>>9.99") 
...
PROCEDURE fill-in-list: 
  ...
  IF fremdwbetrag NE 0 THEN 
    output-list.fbetrag = STRING(fremdwbetrag,"->>>,>>>,>>9.99"). 
END. 
