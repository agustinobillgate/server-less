...
FIND FIRST outorder WHERE outorder.zinr = zimmer.zinr 
  ...
  AND NOT outorder.gespstart GE to-date /* Malik Serverless 163 : AND NOT to-date   LE outorder.gespstart -> AND NOT outorder.gespstart GE to-date */
  AND NOT outorder.gespende LT from-date NO-LOCK NO-ERROR. /* Malik Serverless 163 : AND NOT from-date GT outorder.gespende -> AND NOT outorder.gespende LT from-date */
/*
  ...
  DO:
    ...
    DO:
      ...
      FIND FIRST resline WHERE resline.active-flag EQ 0
        AND resline.resstatus LE 5
        AND resline.zinr = zinr 
        AND resline.abreise GT from-date /* Malik Serverless 163 : AND from-date LT resline.abreise -> AND resline.abreise GT from-date */
        AND resline.ankunft LT to-date /* Malik Serverless 163 : AND to-date   GT resline.ankunft -> AND resline.ankunft LT to-date */
        AND RECID(resline) NE RECID(res-line)
        USE-INDEX res-zinr_ix NO-LOCK NO-ERROR.
      IF AVAILABLE resline THEN error-code = -3. 
    END.
