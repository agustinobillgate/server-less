...
FIND FIRST h-artikel WHERE RECID(h-artikel) = rec-h-artikel NO-LOCK NO-ERROR.
/* FIND FIRST h-bill WHERE RECID(h-bill) = rec-h-bill NO-LOCK NO-ERROR. */      /* Rulita 210225 | Fixing move in to if avail h-artikel serveless issue git 628 */
...
DO:
    ...
    FIND FIRST h-bill WHERE RECID(h-bill) = rec-h-bill NO-LOCK NO-ERROR.  /* Rulita 210225 | Fixing move from find first h-artikel serveless issue git 628 */
    IF AVAILABLE h-bill THEN                                              /* Rulita 210225 | Fixing if avail serveless issue git 628 */
    DO:
        DO transaction: 
...
procedure inv-ar:
  ...
  IF AVAILABLE vhp.htparam THEN foreign-rate = vhp.htparam.flogical.          /* Rulita 210225 | Fixing from fLOGICAL to flogical serveless issue git 628 */

  FIND FIRST vhp.htparam WHERE paramnr = 240 NO-LOCK.
  IF AVAILABLE vhp.htparam THEN double-currency = vhp.htparam.flogical.       /* Rulita 210225 | Fixing from fLOGICAL to flogical serveless issue git 628 */
