...
DEFINE VARIABLE v-log2 AS LOGICAL.
...
PROCEDURE fill-artikel:
    ...
    DO:        
        ...
        FIND FIRST queasy WHERE queasy.KEY EQ 222 
            ...
            AND queasy.number2 EQ h-list.artnr
            AND queasy.number3 EQ h-list.departement NO-LOCK NO-ERROR.
        IF AVAILABLE queasy THEN
        DO:
            IF (INT(queasy.deci1) NE h-list.max-soldout-qty)
                OR (queasy.logi2 NE h-list.soldout-flag) THEN
            DO:
                v-log = YES.
                v-log2 = YES.
            END.
        ...
        DO:
            ...
            IF v-log2 AND AVAILABLE queasy THEN
            DO:
                IF INT(queasy.deci1) NE h-list.max-soldout-qty THEN
                DO:
                    res-history.aenderung = res-history.aenderung + "MaxQtySoldOut " + STRING(INT(queasy.deci1))
                        + " to " + STRING(h-list.max-soldout-qty) + ";".
                END.
                ELSE IF queasy.logi2 NE h-list.soldout-flag THEN
                DO:
                    res-history.aenderung = res-history.aenderung + "SoldOut " + STRING(queasy.logi2)
                        + " to " + STRING(h-list.soldout-flag) + ";".
                END.
            END.
        END.
    END.
    ...
    FIND FIRST queasy WHERE queasy.KEY EQ 222 
         AND queasy.number1 EQ 2 
         AND queasy.number2 EQ h-list.artnr
         AND queasy.number3 EQ h-list.departement EXCLUSIVE-LOCK NO-ERROR.
    IF NOT AVAILABLE queasy THEN
    DO:
        CREATE queasy.
        ASSIGN 
            ...
            queasy.number1 = 2
            queasy.number2 = h-list.artnr
            queasy.number3 = h-list.departement
            queasy.logi2 = h-list.soldout-flag
            queasy.deci1 = h-list.max-soldout-qty
            .
    END.
    ELSE
    DO:
