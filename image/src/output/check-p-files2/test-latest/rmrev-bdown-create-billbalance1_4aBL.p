  /*testFT 230614 semua nilai ambil dr genstat*/
...
DEFINE TEMP-TABLE sum-list 
  ...
  FIELD ch2        AS INTEGER INITIAL 0 
    COLUMN-LABEL "Ch2"
...
DEFINE TEMP-TABLE cl-list 
  ...
  /* Dzikri - 921CDD */
  FIELD diffrate   AS LOGICAL INITIAL NO 
  . 
...
DEFINE TEMP-TABLE t-billjournal LIKE billjournal. /*william*/
...
DEF INPUT PARAMETER srttype      AS INTEGER.
...
/*
DEF variable exc-taxserv   AS LOGICAL NO-UNDO INIT NO.
DEF variable pvILanguage AS INTEGER  NO-UNDO INIT 0.
DEF variable new-contrate  AS LOGICAL INIT YES.
DEF variable foreign-rate  AS LOGICAL INIT NO.
DEF variable price-decimal AS INT INIT 0.
DEF variable curr-date     AS DATE INIT 01/03/24. 
DEF variable srttype      AS INTEGER INIT 1.*/
...
DEFINE VARIABLE temptotjur          AS DECIMAL FORMAT "->>>,>>>,>>9.9999".
...
PROCEDURE create-billbalance1: 
  ...
  DEFINE VARIABLE lcur AS CHAR. /*william */
  ...
  IF srttype = 2 THEN
  DO:
    FOR EACH genstat WHERE genstat.zinr NE "" AND genstat.datum = curr-date 
     ...
     BY res-line.resname: 
      ...
      FOR EACH billjournal WHERE billjournal.bill-datum EQ genstat.datum
          ...
          bill-flag1 = "".
          FIND FIRST bill WHERE bill.rechnr EQ billjournal.rechnr
              AND bill.resnr EQ genstat.resnr 
      ...
      END.
      ELSE
      DO: /*Naufal - Move from line 1106 for bugs fixing descripancy data tax & exc-tax*/
          ASSIGN
      ...
      ELSE DO:
          ...
          /*  Dzikri 92F66C - Pax disrepancy between list and summary
              ...
              argt-list.pax     = argt-list.pax + cl-list.pax.
      END.
          ...
          FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr 
              ...
              DO:
                  ...
                  ELSE DO: /*Naufal - Move from line 1106*/           
                      ASSIGN 
                          cl-list.bfast  = genstat.res-deci[2]
                          sum-list.bfast = sum-list.bfast + cl-list.bfast.   
                  END.
              END.
              ...
              ELSE 
              DO:
                  /*IF genstat.zinr EQ "209" THEN
                  MESSAGE f-betrag argt-betrag argt-line.betrag genstat.res-deci[5]
                      VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
                  IF argt-betrag NE 0 THEN
                  DO:
                      /*Naufal - move to line 555 for bugs fixing case aston madiun*/
                      /*ASSIGN 
                  ...
                  /*IF cl-list.misc LT 0 AND cl-list.misc GT -1 THEN DO: 
                      cl-list.misc = 0.00.
                  END. */ /*Naufal - move to line 555 for bugs fixing case aston madiun*/
              END.       
          END.
          /*Naufal - Move from line 539 for bugs fixing case aston madiun*/
          IF NOT exc-taxserv THEN
          DO:
              ASSIGN
    ...
    /*end*/
      ...
      DO:
        IF cl-list.zipreis NE ROUND(reslin-queasy.deci1,price-decimal) THEN cl-list.diffrate = YES. /* Dzikri Repair red background */
      ...
      DO:               
          /*FD Jan 17, 2021 => New Method get data s-list just from billjournal*/
          /*IF bill-flag1 = "Master Bill" THEN   william 809b0c comment masterbill
          DO:            
              FOR EACH billjournal WHERE billjournal.rechnr EQ bill-master
                  ...
                  DO: /*MNaufal - add validation for bug fixing deposit articles shouldn't be displayed*/
                      ...
                      IF lcur EQ waehrung1.wabkurz THEN   /*william 5fc6f6*/
                          ASSIGN
              ...
              bill-master = -0. /*william change -1 to 0*/
          END.*/
          
          IF bill-flag2 = "Guest Bill" THEN
          DO:            
              FOR EACH billjournal WHERE /*billjournal.rechnr EQ bill-rechnr  william-809B0C
                  AND*/ billjournal.bill-datum EQ genstat.datum
                  AND billjournal.zinr EQ genstat.zinr
                  AND billjournal.betrag NE 0 /*Naufal 22/09/2022 - change validation from GT 0 to NE 0 for bug fix negative value should be included*/ 
                  AND billjournal.anzahl NE 0
                  ...
                  DO: /*MNaufal - add validation for bug fixing deposit articles shouldn't be displayed*/
                      ...
                      FIND FIRST t-billjournal WHERE t-billjournal.rechnr EQ billjournal.rechnr /*15/01/24 william-AD6634 add condition to not count billjournal with same billno and artno again */
                          AND t-billjournal.artnr EQ billjournal.artnr AND t-billjournal.zinr EQ billjournal.zinr NO-LOCK NO-ERROR.
                      IF NOT AVAILABLE t-billjournal THEN
                      DO:
                          IF lcur EQ waehrung1.wabkurz THEN
                              ASSIGN
                                  s-list.f-betrag = s-list.f-betrag + billjournal.betrag.
                          ELSE
                              ASSIGN
              ...
              bill-rechnr = 0. /*william change -1 to 0*/
          END.
        ...
        FOR EACH t-argt-line WHERE t-argt-line.argtnr = arrangement.argtnr 
          ...
          /*FIRST*/ FOR EACH billjournal WHERE billjournal.artnr = t-argt-line.argt-artnr
            ...
            /*RUN calc-servtaxesbl.p (1, billjournal.artnr, billjournal.departement,
              billjournal.bill-datum, OUTPUT serv-1, OUTPUT vat-1, OUTPUT vat-2,
              OUTPUT fact-1).*/
      ...
      */
      ...
      DO:
        ...
        /*RUN calc-servtaxesbl.p (1, artikel1.artnr, artikel1.departement,
          curr-date, OUTPUT serv1, OUTPUT vat1, OUTPUT vat2,
          OUTPUT fact1).*/
        ...
        DO:                
          ASSIGN 
            ...
            sum-list.lodging = sum-list.lodging + lodge-betrag / (1 + vat1 + vat2 + serv1) 
            sum-list.t-rev = sum-list.t-rev + lodge-betrag / (1 + vat1 + vat2 + serv1)
          .           
        END.
        ELSE*/
        DO:
      ...
      FOR EACH fixleist WHERE fixleist.resnr = res-line.resnr 
        ...
        DO: 
          /*RUN calc-servtaxesbl.p (1, fixleist.artnr, fixleist.departement,
              curr-date, OUTPUT serv-2, OUTPUT vat-3, OUTPUT vat-4,
              OUTPUT fact-2).*/
  ...
  DO:
    ...
    /*end*/
      ...
      DO:               
          /*FD Jan 17, 2021 => New Method get data s-list just from billjournal*/
          /*IF bill-flag1 = "Master Bill" THEN   william 809b0c comment master bill
          DO: 
              FOR EACH billjournal WHERE billjournal.rechnr EQ bill-master
                  AND billjournal.bill-datum EQ genstat.datum
                  AND billjournal.zinr EQ genstat.zinr
  ...
  FOR EACH cl-list WHERE cl-list.flag NE "*":
    /* Naufal - Move to line 356 for bugs fixing descripancy data tax & exc-tax
