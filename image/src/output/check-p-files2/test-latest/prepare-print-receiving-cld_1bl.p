...
END. 
  ...
  DO:
    ...
    ELSE 
    DO: 
      FIND FIRST str-list WHERE str-list.artnr = STRING(l-op.artnr,">>>>>>>>") NO-ERROR. 
    ...
    DO: 
      ...
      IF show-price THEN 
        str-list.warenwert = STRING(DECIMAL(str-list.warenwert) + l-op.warenwert,"->>>,>>>,>>>,>>9.99"). 
    END. 
    ELSE 
    DO: 
      CREATE str-list.
      ASSIGN
        str-list.artnr = STRING(l-op.artnr,">>>>>>>>")
      ...
      IF show-price THEN str-list.warenwert = STRING(l-op.warenwert, "->>>,>>>,>>>,>>9.99"). 
      ASSIGN 
          str-list.bezeich      = l-artikel.bezeich
          str-list.lscheinnr    = l-op.lscheinnr
          str-list.unit-price   = STRING(unit-price, "->>>,>>>,>>>,>>9.99")
          .
  ...
  DO:
    ...
    FIND FIRST str-list WHERE INTEGER(str-list.artnr) = l-ophis.artnr NO-ERROR. 
    create-it = NOT AVAILABLE str-list. 
    IF NOT create-it THEN 
    DO: 
      str-list.artnr = STRING(l-ophis.artnr,">>>>>>>>").
      str-list.qty = str-list.qty + l-ophis.anzahl. 
      IF show-price THEN 
        str-list.warenwert = str-list.warenwert + STRING(l-ophis.warenwert, "->>>,>>>,>>>,>>9.99"). 
    END. 
    ELSE 
    DO: 
      CREATE str-list.
      ...
      FIND FIRST queasy WHERE queasy.KEY = 304 AND queasy.char1 = l-ophis.lscheinnr
      ...
      IF show-price THEN str-list.warenwert = STRING(l-ophis.warenwert, "->>>,>>>,>>>,>>9.99"). 
      ASSIGN 
          str-list.bezeich      = l-artikel.bezeich
  ...
  ASSIGN
      ...
      str-list.warenwert    = STRING(tot-amount,"->>>,>>>,>>>,>>9.99").
...
END. 
  ...
  DO:
    ...
    ELSE 
    DO: 
      FIND FIRST str-list WHERE INTEGER(str-list.artnr) = l-op.artnr NO-ERROR. 
      create-it = NOT AVAILABLE str-list. 
    END. 
    IF NOT create-it THEN 
    DO: 
