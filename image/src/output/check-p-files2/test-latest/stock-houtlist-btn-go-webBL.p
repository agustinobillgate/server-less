...
DEF TEMP-TABLE str-list 
  ...
  /*add by Bernatd 52C6EB*/
  FIELD datum           AS CHAR
  FIELD bezeich         AS CHAR FORMAT "x(36)"
  FIELD artnr           AS CHAR FORMAT "9999999"
  FIELD description     AS CHAR FORMAT "x(55)"
  FIELD anzahl          AS CHAR FORMAT "x(5)"
  FIELD einzelpreis     AS CHAR FORMAT " >>>,>>>,>>9"
  FIELD warenwert       AS CHAR FORMAT "->,>>>,>>>,>>9.99"
  
  /* Oscar (12/02/25) - DDB12D - show remark in outgoing report */
  FIELD remark-artikel  AS CHAR
  .
...
DEF INPUT  PARAMETER mi-sub-group   AS LOGICAL. /*ragung req web*/
...
END. 
    ...
    DO:
        FOR EACH vhp.l-ophis WHERE vhp.l-ophis.lager-nr = l-lager.lager-nr 
          ...
          DO:
            ...
            CREATE str-list.
            str-list.description = "Subtotal".
            str-list.anzahl = STRING(t-anz, "->,>>>,>>9.999").

            IF NOT long-digit THEN 
            str-list.warenwert = STRING(t-val, "->,>>>,>>>,>>9.99"). 
            ELSE 
            str-list.warenwert = STRING(t-val, "->>>,>>>,>>>,>>9"). 
            t-anz = 0. 
            t-val = 0. 
            CREATE str-list. 
          ...
          DO: 
            ...
            str-list.bezeich = STRING("", "x(8)") + STRING(l-lager.bezeich, "x(36)"). 
            CREATE str-list. 
            do-it = NO. 
          END. 
          ...
          DO: 
            ...
            str-list.op-recid = RECID(l-ophis).
            ...
            DO:
              ...
              DO:
                str-list.datum   = STRING(vhp.l-ophis.datum).
                str-list.bezeich = STRING(s-list.bezeich).
                str-list.artnr   = STRING(l-artikel.artnr).
                str-list.description = STRING(l-artikel.bezeich, "x(55)").
                str-list.anzahl  = STRING(vhp.l-ophis.anzahl, "->,>>>,>>9.999").
                str-list.einzelpreis = STRING(vhp.l-ophis.einzelpreis, " >>>,>>>,>>9").
                str-list.warenwert =  STRING(vhp.l-ophis.warenwert, "->,>>>,>>>,>>9.99").
              ...
              DO:
                ...
                str-list.bezeich = STRING(s-list.bezeich).
                str-list.artnr   = STRING(l-artikel.artnr).
                str-list.description = STRING(l-artikel.bezeich).
                str-list.anzahl  = STRING(vhp.l-ophis.anzahl, "->,>>>,>>9.999").
                str-list.einzelpreis = STRING(vhp.l-ophis.einzelpreis, ">,>>>,>>9.99").
                str-list.warenwert =  STRING(vhp.l-ophis.warenwert, "->,>>>,>>>,>>9.99").
            ...
            DO:
                ...
                str-list.anzahl  = STRING(vhp.l-ophis.anzahl, "->,>>>,>>9.999").
                str-list.einzelpreis = STRING(vhp.l-ophis.einzelpreis, " >>>,>>>,>>9").
                str-list.warenwert =  STRING(vhp.l-ophis.warenwert, "->>>,>>>,>>>,>>9").
  ...
  DO: 
    ...
    str-list.anzahl = STRING(t-anz, "->,>>>,>>9.999").  
    IF NOT long-digit THEN 
    str-list.warenwert = TRIM(STRING(t-val, "->,>>>,>>>,>>9.99")). 
    ELSE str-list.warenwert = TRIM(STRING(t-val, "->>>,>>>,>>>,>>9")). 
    CREATE str-list.
    /*
    CREATE str-list. 
    DO i = 1 TO 45: 
  ...
  str-list.anzahl = STRING(tot-anz, "->,>>>,>>9.999"). 
  IF NOT long-digit THEN 
  str-list.warenwert = TRIM(STRING(tot-amount, "->,>>>,>>>,>>9.99")). 
  ELSE str-list.warenwert = TRIM(STRING(tot-amount, "->>>>,>>>,>>>,>>9")).
  /*
  CREATE str-list. 
  DO i = 1 TO 45: 
    str-list.s = str-list.s + " ". 
  ...
  */
  ...
  FOR EACH s-list BY s-list.bezeich: 
    ...
    IF NOT long-digit THEN 
    DO:
        str-list.description = STRING(s-list.bezeich, "x(32)"). 
        str-list.warenwert   = TRIM(STRING(s-list.cost, "->,>>>,>>>,>>9.99")).
    END.
     
    ELSE 
    DO:
        str-list.description = STRING(s-list.bezeich, "x(32)").
        str-list.warenwert   = TRIM(STRING(s-list.cost, "->>>>,>>>,>>>,>>9")).
    END.
          ...
          DO: 
            ...
            DO:
              ...
              DO:
                ...
                str-list.description = STRING(l-artikel.bezeich, "x(55)").
                str-list.anzahl  = STRING(vhp.l-ophis.anzahl, "->,>>>,>>9.999").
                str-list.einzelpreis = STRING(vhp.l-ophis.einzelpreis, "->,>>>,>>9.999").
                str-list.warenwert =  STRING(vhp.l-ophis.warenwert, "->,>>>,>>>,>>9.99").
    ...
    DO:
        FOR EACH vhp.l-ophis WHERE vhp.l-ophis.lager-nr = l-lager.lager-nr 
          ...
          */ 
          ...
          IF curr-fibu = "" THEN curr-fibu = fibukonto. 
          IF curr-fibu NE fibukonto AND t-anz NE 0 AND create-it THEN 
    /*    IF (lschein NE vhp.l-ophis.lscheinnr) AND t-anz NE 0 THEN */ 
          DO:
            CREATE str-list.
            str-list.description = "Subtotal".
...
PROCEDURE create-listD:                                      /*ragung by sub group*/
DEFINE VARIABLE t-anz AS DECIMAL FORMAT "->,>>>,>>9.999". 
DEFINE VARIABLE t-val AS DECIMAL. 
DEFINE VARIABLE lschein AS CHAR INITIAL "". 
DEFINE VARIABLE cost-bezeich AS CHAR FORMAT "x(24)". 
...
DEFINE VARIABLE grp1 AS INTEGER INITIAL 0. 
DEFINE VARIABLE grp2 AS INTEGER INITIAL 1.
DEFINE VARIABLE curr-zwkum AS INT INIT 0.
...
/*  calculate the outgoing stocks within the given periods */ 
    ...
    DO:
        FOR EACH vhp.l-ophis WHERE vhp.l-ophis.lager-nr = l-lager.lager-nr 
          ...
          FIRST l-untergrup WHERE l-untergrup.zwkum = l-artikel.zwkum 
          AND (l-untergrup.betriebsnr GE grp1 AND l-untergrup.betriebsnr LE grp2)   
          BY l-untergrup.bezeich BY l-ophis.datum /*BY l-artikel.bezeich*/ BY l-ophis.artnr: 
          /* 
          BY SUBSTR(vhp.l-ophis.lscheinnr,4,12) BY vhp.l-ophis.zeit: 
          */ 
          it-exist = YES. 
    ...
    /*    IF (lschein NE vhp.l-ophis.lscheinnr) AND t-anz NE 0 THEN */ 
          ...
          DO:
            ...
            str-list.bezeich = STRING("", "x(8)") + STRING(l-untergrup.bezeich, "x(24)"). 
          END. 
