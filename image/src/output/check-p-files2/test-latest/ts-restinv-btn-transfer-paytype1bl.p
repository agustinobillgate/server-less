  ...
  DO:  
    ...
    DO:   
      ...
      .   
        ...
        /*Alder - Interface - 24/09/24*/
        RUN create-queasy-interface(h-bill.rechnr,curr-dept,paid,billart).
...
/*Alder - Interface - 24/09/24*/
PROCEDURE create-queasy-interface:
    DEFINE INPUT PARAMETER billno       AS INTEGER NO-UNDO.
    DEFINE INPUT PARAMETER department   AS INTEGER NO-UNDO.
    DEFINE INPUT PARAMETER paid         AS DECIMAL NO-UNDO.
    DEFINE INPUT PARAMETER artnr        AS INTEGER NO-UNDO.

    FIND FIRST queasy WHERE queasy.KEY EQ 313
        AND queasy.number1 EQ billno
        AND queasy.number2 EQ department
        AND queasy.number3 EQ artnr
        NO-LOCK NO-ERROR.
