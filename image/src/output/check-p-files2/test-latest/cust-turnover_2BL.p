/*ragung 692862 penyesuaian nilai tax and service*/
...
DEFINE VARIABLE exrate-tmp  AS DECIMAL NO-UNDO.                       /* Rulita 220425 | Fixing serverless var exrate sama dengan nama table db issue git 608 */ 

FIND FIRST htparam WHERE paramnr = 125 NO-LOCK NO-ERROR. 
IF AVAILABLE htparam THEN bfast-art = htparam.finteger.               /* Rulita 211024 | Fixing for serverless */ 
...
IF currency NE "" THEN DO:
    ...
    IF AVAILABLE waehrung THEN 
        ASSIGN 
            ...
            exrate-tmp  = waehrung.ankauf.                           /* Rulita 220425 | Fixing serverless var exrate sama dengan nama table db issue git 608 */ 
END.
...
IF cardtype = 3 THEN DO:
    ...
    DO: 
        ...
        FOR EACH genstat WHERE /*genstat.resstatus NE 13
            ...
            DO:
                ...
                ASSIGN
                  cust-list.cust-name = (guest.name + ", " + guest.vorname1 + " " + guest.anrede1 
                                       + guest.anredefirma)                                         /* Rulita 180225 | Fixing for serverless from guest.anredefirm to guest.anredefirma issue git 608 */ 
           ...
           /*ragung*/       IF currency NE "" THEN DO:
                            FIND FIRST exrate WHERE exrate.datum = genstat.datum AND exrate.artnr = exratenr NO-LOCK NO-ERROR.
                            IF AVAILABLE exrate THEN 
                                ASSIGN cust-list.sonst-umsatz = cust-list.sonst-umsatz + ((bill-line.betrag / (1 + service + vat)) / exrate.betrag)
                                       cust-list.gesamtumsatz = cust-list.gesamtumsatz + ((bill-line.betrag / (1 + service + vat)) / exrate.betrag).
                            END.
                            ELSE 
                                ASSIGN cust-list.sonst-umsatz = cust-list.sonst-umsatz + bill-line.betrag  / (1 + service + vat) 
                                       cust-list.gesamtumsatz = cust-list.gesamtumsatz + bill-line.betrag  / (1 + service + vat) . /*end*/
                        END.
                    END.
                END.
            END.
                ...
                IF AVAILABLE genstat THEN DO:
                    ...
                    IF AVAILABLE cust-list THEN DO:
                        ...
                        IF found-it = YES THEN DO:
                            FOR EACH h-bill-line WHERE h-bill-line.rechnr EQ INT(guest-queasy.char1)
                                AND h-bill-line.departement EQ guest-queasy.number1 NO-LOCK,
                                ...
                                AND artikel.departement EQ h-artikel.departement NO-LOCK:
                                
                                ASSIGN service = 0  
                                       vat     = 0.
                                
                                RUN calc-servvat.p(artikel.departement, artikel.artnr, guest-queasy.date1, 
                                    artikel.service-code, artikel.mwst-code, OUTPUT service, OUTPUT vat).    
                              ...
                              /*ragung*/  
                                IF currency NE " " THEN DO: 
                                    ...
                                    IF AVAILABLE exrate THEN 
                                        ASSIGN
                                            cust-list.f-b-umsatz   = cust-list.f-b-umsatz + (((guest-queasy.deci1 + guest-queasy.deci2) / (1 + service + vat)) / exrate.betrag)
                                            cust-list.sonst-umsatz = cust-list.sonst-umsatz + ((guest-queasy.deci3 / (1 + service + vat)) / exrate.betrag)
                                            cust-list.gesamtumsatz = cust-list.gesamtumsatz + (((guest-queasy.deci1 + guest-queasy.deci2 + guest-queasy.deci3) / (1 + service + vat)) / exrate.betrag).   
                                END.
                                ELSE 
                                    ASSIGN
                                        cust-list.f-b-umsatz   = cust-list.f-b-umsatz + ((guest-queasy.deci1 + guest-queasy.deci2) / (1 + service + vat))
                                        cust-list.sonst-umsatz = cust-list.sonst-umsatz + (guest-queasy.deci3 / (1 + service + vat))
                                        cust-list.gesamtumsatz = cust-list.gesamtumsatz + ((guest-queasy.deci1 + guest-queasy.deci2 + guest-queasy.deci3) / (1 + service + vat)) .      
        ...
        FOR EACH t-genstat WHERE t-genstat.datum GE fdate AND t-genstat.datum LE tdate
              ...
              ELSE DO:
                  ...
                  ASSIGN
                    cust-list.cust-name = (guest.name + ", " + guest.vorname1 + " " + guest.anrede1 
                                         + guest.anredefirma)                                         /* Rulita 160525 | Fixing for serverless from guest.anredefirm to guest.anredefirma issue git 608 */  
                    cust-list.gastnr    = guest.gastnr
                    cust-list.wohnort   = guest.wohnort 
                    cust-list.plz       = guest.plz 
                    cust-list.land      = guest.land
...
PROCEDURE create-forecast:
    ...
    /* Rulita 230425 | Fixing serverless split dd mm yyyy fdate and tdate git issue 608 */
    DEFINE VARIABLE fd-dd         AS INTEGER NO-UNDO.
    DEFINE VARIABLE fd-mm         AS INTEGER NO-UNDO.
    DEFINE VARIABLE fd-yyyy       AS INTEGER NO-UNDO.
    DEFINE VARIABLE td-dd         AS INTEGER NO-UNDO.
    DEFINE VARIABLE td-mm         AS INTEGER NO-UNDO.
    DEFINE VARIABLE td-yyyy       AS INTEGER NO-UNDO.
    /* End Rulita */
    ...
    FOR EACH res-line WHERE 
        ...
        DO:
            ...
            DO datum = datum1 TO datum2:
                ...
                IF currency NE " " THEN 
                    ASSIGN
                        cust-list.gesamtumsatz = cust-list.gesamtumsatz + ((net-lodg + tot-breakfast 
                                                 + tot-lunch + tot-dinner + tot-other) / exrate-tmp)                            /* Rulita 220425 | Fixing serverless var exrate sama dengan nama table db issue git 608 */ 
                        cust-list.argtumsatz = cust-list.argtumsatz + (net-lodg / exrate-tmp)                                   /* Rulita 220425 | Fixing serverless var exrate sama dengan nama table db issue git 608 */ 
                        cust-list.f-b-umsatz = cust-list.f-b-umsatz + ((tot-breakfast + tot-lunch + tot-dinner) / exrate-tmp)   /* Rulita 220425 | Fixing serverless var exrate sama dengan nama table db issue git 608 */ 
                        cust-list.sonst-umsatz = cust-list.sonst-umsatz + (tot-other / exrate-tmp).                             /* Rulita 220425 | Fixing serverless var exrate sama dengan nama table db issue git 608 */ 
                ELSE    
                    ASSIGN
                        cust-list.gesamtumsatz = cust-list.gesamtumsatz + net-lodg + tot-breakfast 
              ...
              /*ragung*/        IF currency NE "" THEN DO:
                                FIND FIRST exrate WHERE exrate.datum = datum AND exrate.artnr = exratenr NO-LOCK NO-ERROR.
                                IF AVAILABLE exrate THEN 
                                    ASSIGN cust-list.sonst-umsatz = cust-list.sonst-umsatz + ((bill-line.betrag / (1 + service + vat)) / exrate.betrag)
                                           cust-list.gesamtumsatz = cust-list.gesamtumsatz + ((bill-line.betrag / (1 + service + vat)) / exrate.betrag).
                                END.
                                ELSE 
                                    ASSIGN cust-list.sonst-umsatz = cust-list.sonst-umsatz + bill-line.betrag  / (1 + service + vat) 
                                           cust-list.gesamtumsatz = cust-list.gesamtumsatz + bill-line.betrag  / (1 + service + vat) .
                                END.   /*end*/ 
                            END.
                        END.
                END.
    ...
    IF excl-other = NO THEN DO:
        ...
        FOR EACH guest-queasy WHERE guest-queasy.KEY = "gast-info"
            ...
            IF AVAILABLE res-line THEN DO:
                ...
                /*ragung*/      IF currency NE " " THEN DO: 
                                FIND FIRST exrate WHERE exrate.datum = h-bill-line.bill-datum AND exrate.artnr = exratenr NO-LOCK NO-ERROR.
                                IF AVAILABLE exrate THEN 
                                    ASSIGN
                                        cust-list.f-b-umsatz   = cust-list.f-b-umsatz + (((guest-queasy.deci1 + guest-queasy.deci2) / (1 + service + vat)) / exrate.betrag)
                                        cust-list.sonst-umsatz = cust-list.sonst-umsatz + ((guest-queasy.deci3 / (1 + service + vat)) / exrate.betrag)
    ...
    IF fd-dd = 29 AND fd-mm = 2 THEN
        ly-fdate = DATE(fd-mm, 28, fd-yyyy - 1).
    ELSE ly-fdate = DATE(fd-mm, fd-dd, fd-yyyy - 1).

    IF td-dd = 29 AND td-mm = 2 THEN
        ly-tdate = DATE(td-mm, 28, td-yyyy - 1).
    ELSE ly-tdate = DATE(td-mm, td-dd, td-yyyy - 1).
...
PROCEDURE cr-ftd-all:
  ...
  FOR EACH genstat WHERE 
      ...
      IF cust-list.resnr NE "" THEN
      DO:
          IF NUM-ENTRIES(cust-list.resnr,";") GE 2 THEN
          DO:
              DO i = 1 TO NUM-ENTRIES(cust-list.resnr,";"):
                  IF ENTRY(i,cust-list.resnr,";") NE "" THEN
                  DO:
              ...
              END.
          END.
          IF NOT found THEN cust-list.resnr     = cust-list.resnr + STRING(genstat.resnr) + ";".
      END.    
      ELSE
          cust-list.resnr     = cust-list.resnr + STRING(genstat.resnr) + ";".
