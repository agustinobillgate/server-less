...
DEF INPUT PARAMETER excl-compl      AS LOGICAL    NO-UNDO.
...
DEF VAR num-year AS INT. /* Malik Serverless */ 
DEF VAR num-year-curr-from-date AS INT. /*  Malik Serverless */ 
DEF VAR num-year-curr-to-date AS INT. /*  Malik Serverless */ 
...
num-year = YEAR(curr-ci-date) - 1. /* Malik Serverless */
num-year-curr-from-date = YEAR(curr-from-date) - 1. /* Malik Serverless */
num-year-curr-to-date = YEAR(curr-to-date) - 1. /* Malik Serverless */

ASSIGN
    ci-date   = DATE(2, 28, num-year) /* Malik Serverless : YEAR(curr-ci-date) - 1 -> num-year */
    from-date = ci-date
    to-date   = ci-date
.

ASSIGN ci-date = DATE(MONTH(curr-ci-date), DAY(curr-ci-date),
num-year) NO-ERROR. /* Malik Serverless : YEAR(curr-ci-date) - 1) -> num-year */

ASSIGN from-date = DATE(MONTH(curr-from-date), DAY(curr-from-date),
num-year-curr-from-date) NO-ERROR. /* Malik Serverless : YEAR(curr-from-date) - 1) -> num-year-curr-from-date */

ASSIGN to-date = DATE(MONTH(curr-to-date), DAY(curr-to-date),
num-year-curr-to-date) NO-ERROR. /* Malik Serverless : YEAR(curr-to-date) - 1 -> num-year-curr-to-date */
...
ASSIGN room-list.ly-fcast   = STRING(tot-fcast,">>>>>>9")
 ...
 .
/* dody serverless comment 09042025 unused command
...
PROCEDURE calculate-2:
    ...
    DEF VAR rmsharer    AS LOGICAL  NO-UNDO.
    DEF VAR datum1      AS DATE     NO-UNDO.
    DEF VAR num-datum AS INT. /* Malik Serverless */
    ...
    FOR EACH genstat WHERE genstat.datum GE date-1 
        ...
        IF do-it THEN
        DO:
            IF genstat.res-date[1] LT genstat.datum AND genstat.res-date[2] = genstat.datum AND genstat.resstatus = 8 THEN do-it = NO.
        END.

        /* dody serverless comment 09042025 change command
        IF do-it AND excl-compl THEN DO:
          FIND FIRST segment WHERE segment.segmentcode = genstat.segmentcode
        ...
        END.
        */
        /*dody serverless 09042025 new command*/
        FIND FIRST segment WHERE segment.segmentcode = genstat.segmentcode NO-LOCK NO-ERROR.
        IF do-it AND excl-compl THEN 
        DO:
            IF AVAILABLE segment AND (segment.betriebsnr = 1 OR segment.betriebsnr = 2) THEN do-it = NO.
            ELSE
            DO:
                IF genstat.zipreis = 0 AND genstat.gratis NE 0 AND genstat.resstatus = 6 AND genstat.res-logic[2] EQ YES THEN ASSIGN do-it = NO.
            END.
        END.
      ...
      */
        ...
        IF do-it AND NOT all-segm THEN 
        DO: 
            FIND FIRST segm-list WHERE segm-list.segm = genstat.segmentcode AND segm-list.selected NO-LOCK NO-ERROR. 
            IF AVAILABLE segm-list THEN do-it = YES. 
        END. 
      
        IF do-it AND NOT all-argt THEN 
        DO: 
            FIND FIRST argt-list WHERE argt-list.argt = genstat.argt AND argt-list.selected NO-LOCK NO-ERROR. 
            IF AVAILABLE argt-list THEN do-it = YES. 
        END. 
      
        IF do-it AND NOT all-zikat THEN 
        DO: 
            FIND FIRST zikat-list WHERE zikat-list.zikatnr = genstat.zikatnr AND zikat-list.selected NO-LOCK NO-ERROR. 
            IF AVAILABLE zikat-list THEN do-it = YES. 
        END. 
    ...
    FOR EACH s-list:
        ASSIGN 
        datum1 = s-list.datum
        num-datum = YEAR(datum1) + 1
        datum1 = DATE(MONTH(datum1), DAY(datum1), num-datum) NO-ERROR /* Malik Serverless : YEAR(datum1) + 1 -> num-datum */
        .   
        FIND FIRST room-list WHERE room-list.datum = datum1 NO-ERROR.
        IF AVAILABLE room-list THEN
        ASSIGN
