DEFINE TEMP-TABLE tfa-order LIKE fa-order
    ...
    /* FIELD budget-assetnr          AS DECIMAL  */
    /* FIELD budget-date             AS DATE */
    /* Dzikri - 01B1DB END */

    /* add new field to contain nr-buget by Oscar (18 November 2024) */
...
FOR EACH tfa-order BY tfa-order.fa-pos  : 
    ...
    ASSIGN fa-order.Order-Nr        = order-nr
           ...
           fa-order.ActiveReason    = STRING(tfa-order.nr-budget)
        .

    /* not saving budget to field queasy, instead retrieve it directly from PnL Budget Setting by Oscar (18 Oktober 2024) - C11EAE */
    /* Dzikri - 01B1DB create new input */

    /* comment because different concept of budget asset applied */
    /* 
    CREATE queasy.
    ASSIGN queasy.key                = 315
           ...
           queasy.char2              = pr-nr
           /* queasy.char3              =  */
           queasy.number1            = tfa-order.Fa-Nr
           /* queasy.deci1              = tfa-order.budget-assetnr */
           queasy.number3            = repos
           queasy.date1              = tfa-order.budget-date
           .
    */      
    /* Dzikri - 01B1DB END */
