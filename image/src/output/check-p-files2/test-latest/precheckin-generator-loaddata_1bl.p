...
DEFINE TEMP-TABLE arl-list  
    ...
    FIELD phonenumber       AS CHAR FORMAT "x(15)" LABEL "Phone Number"
    FIELD email-address     AS CHAR FORMAT "x(35)" LABEL "Email Address"
    FIELD link-pci-eng      AS CHAR
    FIELD link-pci-oth      AS CHAR
...
DEFINE INPUT  PARAMETER TABLE FOR segm1-list.   /*FD Feb 22, 2022*/
DEFINE INPUT  PARAMETER TABLE FOR argt-list.    /*FD Feb 22, 2022*/
DEFINE INPUT  PARAMETER TABLE FOR zikat-list.   /*FD Feb 22, 2022*/

DEFINE OUTPUT PARAMETER rmlen      AS INTEGER.  
DEFINE OUTPUT PARAMETER mail-eng   AS CHAR.  
DEFINE OUTPUT PARAMETER mail-oth   AS CHAR.
DEFINE OUTPUT PARAMETER hotel-name AS CHAR.
DEFINE OUTPUT PARAMETER hotel-telp AS CHAR.
DEFINE OUTPUT PARAMETER hotel-mail AS CHAR.
...
/*
FIND FIRST queasy WHERE queasy.KEY EQ 216 AND queasy.number1 EQ 8 AND queasy.number2 EQ 31 NO-LOCK NO-ERROR.
IF AVAILABLE queasy THEN hotelcode = queasy.char3.
FIND FIRST queasy WHERE queasy.KEY EQ 216 AND queasy.number1 EQ 8 AND queasy.number2 EQ 32 NO-LOCK NO-ERROR.
IF AVAILABLE queasy THEN mail-eng = queasy.char3.
FIND FIRST queasy WHERE queasy.KEY EQ 216 AND queasy.number1 EQ 8 AND queasy.number2 EQ 33 NO-LOCK NO-ERROR.
IF AVAILABLE queasy THEN mail-oth = queasy.char3.
*/
...
FOR EACH queasy WHERE queasy.KEY EQ 216 AND queasy.number1 EQ 8 NO-LOCK:
    ...
    IF queasy.number2 EQ 32 THEN mail-eng   = queasy.char3.
    IF queasy.number2 EQ 33 THEN mail-oth   = queasy.char3.
    IF queasy.number2 EQ 19 THEN hotel-name = queasy.char3.
    IF queasy.number2 EQ 22 THEN hotel-telp = queasy.char3.
    IF queasy.number2 EQ 23 THEN hotel-mail = queasy.char3.
END.
...
DO:
    FOR EACH arl-list:
        arl-list.link-pci-eng = "HotelCode Not Configured Yet".
        arl-list.link-pci-oth = "HotelCode Belum Terkonfigurasi Dengan Benar".
    END.
END.
ELSE
DO:
    FOR EACH arl-list:
        EN-hotelencrip  = "ENG|" + hotelcode + "|" + STRING(arl-list.ankunft) + "|" + STRING(arl-list.resnr).
        OTH-hotelencrip = "IDN|" + hotelcode + "|" + STRING(arl-list.ankunft) + "|" + STRING(arl-list.resnr).
        ...
        ASSIGN 
            ...
            /*arl-list.link-pci-eng = precheckinurl + "?" + EN-hotelencrip + "&hc=" + hotelcode.*/
            arl-list.link-pci-eng = precheckinurl + "?" + "hc=" + hotelcode + "&ec=" + EN-hotelencrip.
            ...
            /*arl-list.link-pci-oth = precheckinurl + "?" + OTH-hotelencrip + "&hc=" + hotelcode.*/
            arl-list.link-pci-oth = precheckinurl + "?" + "hc=" + hotelcode + "&ec=" + OTH-hotelencrip.
    END.
END.
    ...
    DO:   
      ...
      FOR EACH res-line WHERE   
          ...
          FIRST reservation WHERE reservation.resnr = res-line.resnr  
            AND LENGTH(reservation.groupname) GE iGrpname NO-LOCK,   
          FIRST zimkateg WHERE zimkateg.zikatnr = res-line.zikatnr NO-LOCK,   
          FIRST setup-list WHERE setup-list.nr = res-line.setup + 1 NO-LOCK,
          FIRST segm1-list WHERE segm1-list.segm = reservation.segmentcode 
            AND segm1-list.SELECTED NO-LOCK,
          FIRST argt-list WHERE argt-list.argt = res-line.arrangement 
            AND argt-list.SELECTED  
          NO-LOCK BY (res-line.kontakt-nr * res-line.resnr) BY res-line.resstatus BY res-line.NAME:  
          RUN create-it.
      END.  
...
DEFINE VARIABLE iGrpname    AS INTEGER INITIAL 0    NO-UNDO.  
  ...
  DO:   
    ...
    FOR EACH res-line WHERE   
        ...
        AND res-line.resname LE to-name NO-LOCK,   
        FIRST gbuff WHERE gbuff.gastnr = res-line.gastnr NO-LOCK,  
        FIRST waehrung WHERE waehrung.waehrungsnr = res-line.betriebsnr   
        NO-LOCK,   
        FIRST reservation WHERE reservation.resnr = res-line.resnr   
...
PROCEDURE create-it:  
    ...
    arl-list.segmentcode       = segm1-list.segm
    ...
    arl-list.arrangement       = argt-list.argt  
    ...
    arl-list.phonenumber       = gmember.mobil-tel
    arl-list.email-address     = gmember.email-adr
        .  
