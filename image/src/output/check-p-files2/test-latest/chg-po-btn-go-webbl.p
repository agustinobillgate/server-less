...
/* Malik Serverless 571 add if available */
IF AVAILABLE t-l-orderhdr THEN
DO:
    FIND FIRST l-orderhdr WHERE RECID(l-orderhdr) = t-l-orderhdr.rec-id NO-LOCK NO-ERROR.
...
FOR EACH s-order NO-LOCK: 
    ...
    /* Malik Serverless 571 */
    FIND FIRST l-order WHERE RECID(l-order) = s-order.rec-id NO-LOCK NO-ERROR. 
