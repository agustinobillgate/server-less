...
DEFINE VARIABLE i-typ AS INT NO-UNDO.
DEFINE VARIABLE currcode AS CHAR NO-UNDO.
DEFINE VARIABLE user-init AS CHAR NO-UNDO.
DEFINE VARIABLE created AS LOGICAL NO-UNDO INIT NO.
DEFINE VARIABLE avail-qsy AS LOGICAL NO-UNDO INIT NO.
DEFINE VARIABLE betribnr AS INTEGER NO-UNDO.
...
FIND FIRST queasy WHERE queasy.KEY = 171 AND queasy.char1 EQ "" NO-LOCK NO-ERROR.
IF AVAILABLE queasy THEN betribnr = queasy.betriebsnr. /* NC - 05/02/2025 */
...
DO datum = from-date TO to-date:
    ...
    DO:
        ...
        ASSIGN 
            ...
            res-history.action = "UpdateAllotmentByRateCode"
        . 
    END.
    ...
    DO:
        ...
        DO:
            FIND FIRST queasy WHERE queasy.KEY = 171 AND queasy.char1 = ""
                AND queasy.date1 = datum AND queasy.number1 = i-typ AND queasy.betriebsnr EQ betribnr NO-LOCK NO-ERROR.
...
ASSIGN 
    ...
    res-history.datum  = TODAY 
    res-history.zeit   = TIME 
    res-history.action = "UpdateAllotment"
    res-history.aenderung = "Allotment Updated: RateCOde: " + currcode + " Room Type: " + STRING(rmtype) + "Allotment : " + STRING(allotment) + " " +  
