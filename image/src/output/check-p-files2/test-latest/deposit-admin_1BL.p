...
DEFINE INPUT PARAMETER ex-rate      AS DECIMAL.
...
DEFINE BUFFER b-exrate FOR exrate. /* Oscar - serverless #754 and #748 */

/* SY 05 Sept 2015 */
RUN create-itlist.


PROCEDURE create-itlist:
DEFINE VARIABLE to-name AS CHAR INITIAL "". 
  ...
  DO: 
    ...
    DO: 
      ...
      DO:
        FOR EACH reservation WHERE activeflag = 0 
            AND reservation.depositgef NE 0 
            AND reservation.resnr GE lresnr /* Malik Serverless : AND reservation.name MATCHES ("*" + lname + "*") -> AND reservation.resnr GE lresnr */
            AND reservation.name MATCHES ("*" + lname + "*") NO-LOCK, /* Malik Serverless : AND reservation.resnr GE lresnr -> AND reservation.name MATCHES ("*" + lname + "*") */
            FIRST res-line WHERE res-line.resnr = reservation.resnr 
            AND res-line.resstatus LE 8 AND res-line.ankunft GE fdate 
  ...
  DO:
    ...
    DO:
      ...
      END.
      ASSIGN
        total-saldo = total-saldo * ex-rate
        arriv-saldo = arriv-saldo * ex-rate
      .
    END.
  END.
...
PROCEDURE create-depo:
    ...
    DO:
        /* Oscar - serverless #754 and #748 */
        FIND FIRST b-exrate WHERE b-exrate.artnr = depo-curr 
          AND b-exrate.datum = depo-list.datum1 NO-LOCK NO-ERROR.
        IF AVAILABLE b-exrate THEN
            exchg-rate = b-exrate.betrag.
        ELSE found-it = NO.
    END.
   ...
   DO:
      /* Oscar - serverless #754 and #748 */
      FIND FIRST b-exrate WHERE b-exrate.artnr = depo-curr 
        AND b-exrate.datum = depo-list.datum2 NO-LOCK NO-ERROR.
      IF AVAILABLE b-exrate THEN
          exchg-rate = b-exrate.betrag.
      ELSE found-it = NO.
   END.
   ...
   ASSIGN             
        ...
        depo-list.remark        = " "
        depo-list.stafid        = " "
        depo-list.adult         = res-line.erwachs /* Malik Serverless : res-line.erwach -> res-line.erwachs */
...
PROCEDURE create-b1:
    ...
    DEFINE VARIABLE tmp-date AS DATE. /* Malik Serverless */
    ...
    ASSIGN
        ...
        b1-list.remark        = " "
        b1-list.stafid        = " "
        b1-list.adult         = res-line.erwachs /* Malik Serverless : res-line.erwach -> res-line.erwachs */
    ...
    DO datum = res-line.ankunft TO tmp-date: /* Malik Serverless : TO res-line.abreise - 1 -> TO tmp-date */
...
PROCEDURE create-b1-print:
    ...
    ASSIGN
        ...
        b1-print.remark        = " "
        b1-print.stafid        = " "
        b1-print.adult         = res-line.erwachs /* Malik Serverless : res-line.erwach -> res-line.erwachs */
    ...
    tmp-date = res-line.abreise - 1. /* Malik serverless */       
    DO datum = res-line.ankunft TO tmp-date: /* Malik Serverless : TO res-line.abreise - 1 -> TO tmp-date */
        FOR EACH fixleist WHERE fixleist.resnr = res-line.resnr
            AND fixleist.reslinnr = res-line.reslinnr NO-LOCK: 
