...
PROCEDURE create-list: 
    ...
    FOR EACH l-lager WHERE l-lager.lager-nr GE from-store3 AND l-lager.lager-nr LE to-store3 NO-LOCK:
        i = 0.
        FOR EACH l-artikel WHERE l-artikel.artnr GE n1 AND l-artikel.artnr LE n2 AND l-artikel.min-bestand GT 0 NO-LOCK BY l-artikel.artnr: /*Alder - Serverless - Issue 672*/
            curr-best = 0. 
            ...
            DO:
                ...
                IF curr-best LT l-artikel.min-bestand THEN /*Alder - Serverless - Issue 672*/
                DO: 
                    i = i + 1.
                    IF i = 1 THEN
                    DO:
                    ...
                    CREATE min-onhand-list.
                        ASSIGN 
                           ...
                           min-onhand-list.min-oh     = l-artikel.min-bestand /*Alder - Serverless - Issue 672*/
...
PROCEDURE create-list2: 
    ...
    FOR EACH l-lager WHERE l-lager.lager-nr GE from-store3 AND l-lager.lager-nr LE to-store3 NO-LOCK:
        i = 0.
        FOR EACH l-artikel WHERE l-artikel.artnr GE n1 AND l-artikel.artnr LE n2 AND l-artikel.min-bestand GT 0 NO-LOCK BY l-artikel.bezeich: /*Alder - Serverless - Issue 672*/
            curr-best = 0. 
            FIND FIRST l-bestand WHERE l-bestand.artnr = l-artikel.artnr AND l-bestand.lager-nr = l-lager.lager-nr NO-LOCK NO-ERROR. 
            IF AVAILABLE l-bestand THEN 
            DO:
