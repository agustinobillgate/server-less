  ...
  DO: 
    FOR EACH htparam WHERE htparam.paramgr = i NO-LOCK 
      ...
      DO: 
        ...
        . 
        IF htparam.feldtyp = 1 THEN htp.wert = STRING(htparam.finteger). 
        ELSE IF htparam.feldtyp = 2 THEN htp.wert = STRING(htparam.fdecimal). 
        ELSE IF htparam.feldtyp = 3 THEN htp.wert = STRING(htparam.fdate). 
        ELSE IF htparam.feldtyp = 4 THEN 
        DO: 
          htp.wert = STRING(htparam.flogical). 
          htp.logv = htparam.flogical. 
        END. 
        ELSE IF htparam.feldtyp = 5 THEN htp.wert = STRING(htparam.fchar). 
