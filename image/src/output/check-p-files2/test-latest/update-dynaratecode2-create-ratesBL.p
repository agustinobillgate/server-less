...
FOR EACH dynarate-list WHERE dynarate-list.rmtype = rmtype
    ...
    i = 0.
    DO curr-i = DAY(from-date) TO DAY(to-date):
      i = i + 1.
      ASSIGN 
      ...
      IF dynarate-list.w-day GT 0 AND dynarate-list.w-day NE w-day 
        THEN rate-list1.rcode[i] = ?.
      ELSE
      DO:
        RELEASE ratecode.
        IF AVAILABLE zimkateg THEN
        FIND FIRST ratecode WHERE ratecode.CODE = dynarate-list.rCode
          AND ratecode.startperiode LE to-date
          AND ratecode.endperiode GE from-date
        ...
        FIND FIRST ratecode WHERE ratecode.CODE = dynarate-list.rCode
          ...
          AND ratecode.marknr  = market-number NO-LOCK NO-ERROR.
        IF NOT AVAILABLE ratecode THEN rate-list1.rcode[i] = ?.
        ELSE
        DO: 
          rate-list1.rcode[i] = dynarate-list.rcode.
          ...
          ASSIGN rate-list1.rcode[i] = queasy.char3.
        END.
      END.
    END.
END.
