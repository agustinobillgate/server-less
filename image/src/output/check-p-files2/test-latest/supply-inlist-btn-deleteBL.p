...
/*DEF BUFFER l-op FOR l-op.FT serverless*/
...
FIND FIRST l-artikel WHERE l-artikel.artnr = str-list-artnr NO-LOCK NO-ERROR.
...
/* search if stock incoming GL journal exists */
  FOR EACH gl-jouhdr WHERE gl-jouhdr.datum EQ str-list-billdate
    AND gl-jouhdr.jtype = 6 AND gl-jouhdr.refno MATCHES "RCV*" NO-LOCK:
...
/* search if stock outgoing GL journal exists */
  FOR EACH gl-jouhdr WHERE gl-jouhdr.datum EQ str-list-billdate
    AND gl-jouhdr.jtype = 3 AND gl-jouhdr.refno MATCHES "OUT*" NO-LOCK:
    FIND FIRST gl-journal WHERE gl-journal.jnr = gl-jouhdr.jnr
        AND gl-journal.fibukonto = art-fibu NO-LOCK NO-ERROR.
    IF AVAILABLE gl-journal THEN
    DO:
...
DEFINE VARIABLE trf-gl          AS DATE     NO-UNDO. /* Add by Michael @ 03/07/2019 for using htparam transfer to G/L date - ticket no 653938 */
  ...
  FIND FIRST l-op WHERE RECID(l-op) = str-list-l-recid NO-LOCK NO-ERROR. 
  IF AVAILABLE l-op AND l-op.flag THEN  /* indicates direct issue */ 
  DO: 
      may-delete = YES.
      RETURN.
  END.
