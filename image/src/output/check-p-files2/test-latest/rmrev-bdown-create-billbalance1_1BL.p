...
DEFINE TEMP-TABLE cl-list 
  ...
  FIELD c-zipreis    AS CHAR FORMAT "x(17)" LABEL "        Room Rate" 
  FIELD c-localrate  AS CHAR FORMAT "x(17)" LABEL "   Local Currency" 
  FIELD c-lodging    AS CHAR FORMAT "x(17)" LABEL "          Lodging" 
  FIELD c-bfast      AS CHAR FORMAT "x(16)" LABEL "       Breakfast" 
  FIELD c-lunch      AS CHAR FORMAT "x(16)" LABEL "           Lunch" 
  FIELD c-dinner     AS CHAR FORMAT "x(16)" LABEL "          Dinner" 
  FIELD c-misc       AS CHAR FORMAT "x(16)" LABEL "       Other Rev" 
  FIELD c-fixcost    AS CHAR FORMAT "x(14)" LABEL "       FixCost" 
  FIELD ct-rev       AS CHAR FORMAT "x(17)" LABEL "       Total Rate" 
  ...
  FIELD age2       AS INTEGER INITIAL 0 COLUMN-LABEL "Age"
  /* End of add */
  . 
...
PROCEDURE create-billbalance1: 
  ...
  FOR EACH genstat WHERE genstat.zinr NE "" AND genstat.datum = curr-date 
	...
	*/
    ...
    DO: 
      /*MThide MESSAGE NO-PAUSE. 
      MESSAGE translateExtended ("Bill not found: RmNo ",lvCAREA,"") + res-line.zinr + " - " + res-line.name 
    ...
    IF cl-list.zipreis EQ 0 THEN ASSIGN cl-list.pax = cl-list.com + cl-list.comch.
    ELSE cl-list.pax = res-line.erwachs + res-line.kind1 + res-line.kind2 + cl-list.com + cl-list.comch.
	...
	FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr 
		...
		*/
        ...
        IF artikel.zwkum = bfast-art AND 
            ...
            ASSIGN cl-list.bfast  = genstat.res-deci[2] * (1 + vat3 + serv2).
        ELSE IF artikel.zwkum = lunch-art AND 
            (artikel.umsatzart = 3 OR artikel.umsatzart GE 5) THEN 
            ASSIGN cl-list.lunch  = genstat.res-deci[3] * (1 + vat3 + serv2). 
        ELSE IF artikel.zwkum = dinner-art AND 
            (artikel.umsatzart = 3 OR artikel.umsatzart GE 5) THEN 
            ASSIGN cl-list.dinner = genstat.res-deci[4] * (1 + vat3 + serv2).
        ELSE IF artikel.zwkum = lundin-art AND 
            (artikel.umsatzart = 3 OR artikel.umsatzart GE 5) THEN 
            ASSIGN cl-list.lunch = genstat.res-deci[3] * (1 + vat3 + serv2). 
        
        ASSIGN cl-list.misc = genstat.zipreis - (cl-list.lodging + cl-list.bfast + cl-list.lunch + cl-list.dinner).
        IF cl-list.misc LT 0 AND cl-list.misc GT -1 THEN DO: 
            cl-list.misc = 0.00.
        END.
        /*ASSIGN cl-list.misc = cl-list.misc + argt-betrag.
        ASSIGN cl-list.misc = genstat.res-deci[5] * (1 + vat3 + serv2).*/
        ...
        ELSE IF artikel.zwkum = lundin-art AND 
            (artikel.umsatzart = 3 OR artikel.umsatzart GE 5) THEN 
            ASSIGN cl-list.lunch = cl-list.lunch + argt-betrag. 
        ELSE ASSIGN cl-list.misc = cl-list.misc + argt-betrag.*/
    END.
    /*end*/
...
*/        
    ...
    IF genstat.zipreis NE 0 THEN            /*wenni 040117 */
      FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr 
        ...
        IF take-it THEN 
        DO: 
          RUN get-genstat-argt-betrag(INPUT frate, INPUT-OUTPUT argt-betrag).
          ...
          IF s-list.artnr = 12 AND   s-list.dept = 1  THEN
