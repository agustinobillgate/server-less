...
DEF INPUT  PARAMETER zeroprice-flag     AS LOGICAL.
...
FIND FIRST l-orderhdr WHERE RECID(l-orderhdr) = t-l-orderhdr.rec-id NO-LOCK NO-ERROR. /* Malik Serverless 574 add NO-LOCK NO-ERROR */
IF NOT AVAILABLE l-orderhdr THEN RETURN. /* Malik Serverless 574 */
...
DO:
    fl-code = 2.
    IF NOT zeroprice-flag THEN RETURN NO-APPLY.
    ELSE
    DO:
        FIND FIRST hdrbuff WHERE hdrbuff.docu-nr = l-orderhdr.docu-nr
        AND RECID(hdrbuff) NE RECID(l-orderhdr) NO-LOCK NO-ERROR.
...
FOR EACH t-l-order WHERE t-l-order.pos GT 0:
  ...
  DO:
    ...
    FIND FIRST queasy WHERE queasy.KEY = 304 AND queasy.char1 = t-l-order.docu-nr
          AND queasy.number1 = t-l-order.artnr NO-LOCK NO-ERROR.
    IF NOT AVAILABLE queasy THEN DO:
        CREATE queasy.
        ASSIGN queasy.KEY     = 304 
               queasy.char1   = t-l-order.docu-nr
               queasy.number1 = t-l-order.artnr
               queasy.number2 = t-l-order.addvat-no
               queasy.deci1   = t-l-order.addvat-value
         .   
        RELEASE queasy.
    END.
    ELSE DO:
        FIND CURRENT queasy EXCLUSIVE-LOCK.
        ASSIGN queasy.char1   = t-l-order.docu-nr
               queasy.number1 = t-l-order.artnr
               queasy.number2 = t-l-order.addvat-no
               queasy.deci1   = t-l-order.addvat-value
         .   
