...
FIND FIRST vhp.htparam WHERE vhp.htparam.paramnr = 271 NO-LOCK NO-ERROR. 
IF AVAILABLE vhp.htparam AND vhp.htparam.feldtyp = 4 THEN multi-vat = vhp.htparam.flogical.

FIND FIRST vhp.h-bill WHERE RECID(vhp.h-bill) = h-recid NO-LOCK NO-ERROR. 
IF AVAILABLE vhp.h-bill THEN /*FT serverless*/
DO:
