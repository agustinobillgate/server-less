...
DEFINE INPUT  PARAMETER repeat-charge AS LOGICAL.
...
DEFINE VARIABLE curr-diff    AS DECIMAL NO-UNDO.
...
IF repeat-charge = YES THEN DO:
    RUN calc-periode.
    RUN cal-revenue1.
END.
ELSE RUN cal-revenue.
        ...
        DO:         
            FOR EACH reslin-queasy WHERE reslin-queasy.key = "fargt-line" 
              ...
              AND bill-date LE reslin-queasy.date2 NO-LOCK:
                argt-defined = YES. 
                IF reslin-queasy.char2 NE "" AND reslin-queasy.char2 NE "0" THEN argt-rate = rm-rate * INT(reslin-queasy.char2) / 100. 
                ELSE
                DO:
...
PROCEDURE calc-periode:
    ...
    FIND FIRST queasy WHERE queasy.KEY = 301
        ...
        AND queasy.logi1 = repeat-charge NO-LOCK NO-ERROR.
        ...
        FOR EACH periode-list:
            ...
            IF periode-list.diff-day GE 30 THEN
                ASSIGN periode-list.amt-periode = queasy.deci1 / periode-list.diff-day
                       ...
                       curr-diff                = periode-list.diff-day.
            ELSE DO:
...
DEFINE BUFFER w1 FOR waehrung. 
  ...
  rm-rate = res-line.zipreis. 
    ...
    DO datum = res-line.ankunft TO co-date:
        IF datum LT ci-date THEN DO:
             FIND FIRST genstat WHERE genstat.datum = datum
                  AND genstat.resnr = res-line.resnr
                  AND genstat.res-int[1] = res-line.reslinnr NO-LOCK NO-ERROR.
             IF AVAILABLE genstat THEN DO:
                 FIND FIRST periode-list WHERE genstat.datum GE periode-list.periode1 
                        AND genstat.datum LE periode-list.periode2 NO-LOCK NO-ERROR.
                 IF AVAILABLE periode-list THEN DO:
                     FIND FIRST output-list1 WHERE output-list1.datum = periode-list.periode1 NO-ERROR.
                     IF NOT AVAILABLE output-list1 THEN DO:
                         CREATE output-list1.
                         output-list1.datum = periode-list.periode1.
                     END.
                     ASSIGN output-list1.rate = output-list1.rate + genstat.zipreis.
                 END.
             END.
        END.
        ...
        IF datum GE ci-date THEN DO:
              ...
              FIND FIRST periode-list WHERE datum GE periode-list.periode1 AND datum LE periode-list.periode2 NO-LOCK NO-ERROR.
              IF AVAILABLE periode-list THEN DO:
                 FIND FIRST output-list1 WHERE output-list1.datum = periode-list.periode1 NO-ERROR.
                 IF NOT AVAILABLE output-list1 THEN DO:
                     CREATE output-list1.
                     output-list1.datum = periode-list.periode1.
                 END.
                 ASSIGN output-list1.rate = output-list1.rate + rm-rate.
              END.
        END.
    END.
    
    ASSIGN tot-rate = 0.
    FOR EACH output-list1 BY output-list1.datum:
        CREATE output-list. 
        ASSIGN 
               counter          = counter + 1   
               output-list.flag = counter
               output-list.STR  = STRING(output-list1.datum) + "   " + translateExtended ("Roomrate",lvCAREA,"") 
                                   + "   = " + TRIM(STRING(output-list1.rate,"->>>,>>>,>>9.99"))
               tot-rate         = tot-rate + output-list1.rate. 
    END.
    CREATE output-list. 
    ASSIGN
         counter          = counter + 1   
         output-list.flag = counter
         output-list.STR = "  " + translateExtended ("Expected total revenue    =",lvCAREA,"") 
                              + " " + TRIM(STRING(tot-rate,"->,>>>,>>>,>>9.99")).
END. 
