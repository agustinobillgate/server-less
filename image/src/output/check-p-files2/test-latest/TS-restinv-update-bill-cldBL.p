...
DEF INPUT PARAMETER segment-code            AS INTEGER.
...
DEFINE BUFFER buf-hbill FOR vhp.h-bill.
...
IF rec-id NE 0 THEN
FIND FIRST vhp.h-bill WHERE RECID(vhp.h-bill) = rec-id NO-ERROR.
...
IF rec-id EQ 0 THEN
DO:
    FIND FIRST buf-hbill WHERE buf-hbill.flag EQ 0
        AND buf-hbill.tischnr EQ tischnr
        AND buf-hbill.departement EQ curr-dept NO-LOCK NO-ERROR.
    IF AVAILABLE buf-hbill THEN
    DO:
        FIND FIRST h-bill-line WHERE h-bill-line.rechnr EQ buf-hbill.rechnr
            AND h-bill-line.departement EQ buf-hbill.departement NO-LOCK NO-ERROR.
        IF AVAILABLE h-bill-line THEN
        DO:
            /*Debug*/
            MESSAGE " CLDBL.p IN Condition Occupied Table, Return."
                VIEW-AS ALERT-BOX INFO BUTTONS OK.
            RETURN.
        END.            
    END.
...
DO TRANSACTION: 
  ...
  ELSE DO: 
    ...
    ASSIGN 
      ...
      vhp.h-bill.segmentcode = segment-code  /*FDL April 15, 2024 => Ticket 65C56B | CD4BDA*/
      . 

    FIND FIRST vhp.queasy WHERE vhp.queasy.KEY = 31 
        AND vhp.queasy.number1 = curr-dept
        AND vhp.queasy.number2 = tischnr EXCLUSIVE-LOCK NO-ERROR.
