...
DEFINE VARIABLE vipnr10 AS INTEGER INITIAL 999999999. 
...
END.   
  ...
  /* Naufal Afthar - 6B04C3 -> add genparam 712*/
  FIND FIRST htparam WHERE htparam.paramnr EQ 712 NO-LOCK NO-ERROR.
  ...
  FIND FIRST gsegm-list WHERE   
   (gsegm-list.segmentcode = vipnr1 OR   
    ...
    gsegm-list.segmentcode = vipnr9 OR
    gsegm-list.segmentcode = vipnr10) NO-LOCK NO-ERROR.   
  IF AVAILABLE gsegm-list THEN   
  DO:   
    vip-flag2 = YES.   
