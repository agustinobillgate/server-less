...
/* TEST
DEF VAR selected-category AS INT INIT 1.
FIND FIRST guest WHERE guest.gastnr = 202.
CREATE guest-list.
ASSIGN 
    guest-list.guestnumber = guest.gastnr
    guest-list.guestrefno  = guest.steuernr
    guest-list.guestname   = guest.NAME + " " + guest.vorname1 + "," + guest.anrede1.
    IF guest.karteityp EQ 0 THEN
        guest-list.guesttype   = "Individual".
    ELSE IF guest.karteityp EQ 1 THEN
        guest-list.guesttype   = "Company".
    ELSE IF guest.karteityp EQ 2 THEN
        guest-list.guesttype   = "Travel Agent".
    guest-list.address     = guest.adresse1 + " " + guest.adresse2 + " " + guest.adresse3.
    guest-list.SELECTED    = NO.

FIND FIRST guest WHERE guest.gastnr = 101.
CREATE guest-list.
ASSIGN 
    guest-list.guestnumber = guest.gastnr
    guest-list.guestrefno  = guest.steuernr
...
PROCEDURE add-guest:
    FIND FIRST b-queasy WHERE b-queasy.KEY EQ 211 AND b-queasy.number1 EQ selected-category NO-LOCK NO-ERROR.
    ...
    FOR EACH guest-list:
        FOR EACH queasy WHERE queasy.KEY EQ 212 AND queasy.number1 EQ selected-category BY queasy.number2 DESC:
            nr = queasy.number2 + 1.
            LEAVE.
        END.
        
        FIND FIRST queasy WHERE queasy.KEY EQ 212 AND queasy.number3 EQ guest-list.guestnumber NO-LOCK NO-ERROR.
        ...
        DO:
            FIND FIRST b-queasy WHERE b-queasy.KEY EQ 211 AND b-queasy.number1 EQ queasy.number1 NO-LOCK.
            IF AVAILABLE b-queasy THEN
            /*DO:
                assigned = b-queasy.char1.
            END.   
            error-msg = error-msg + queasy.char1 + "                " + TRIM(b-queasy.char1) + CHR(10).*/
    ...
    FOR EACH queasy WHERE queasy.KEY EQ 212 NO-LOCK:
        FIND FIRST b-queasy WHERE b-queasy.KEY EQ 211 AND b-queasy.number1 EQ queasy.number1 NO-LOCK NO-ERROR.
        IF AVAILABLE b-queasy THEN
        DO:
            CREATE temp-list.
            ASSIGN
