...
IF AVAILABLE htparam THEN long-digit = htparam.flogical. 
...
PROCEDURE create-stock1:
    ...
    /*FOR EACH l-lager WHERE l-lager.lager-nr GE from-lager AND l-lager.lager-nr LE to-lager NO-LOCK: FT serverless*/
        zwkum = 0. 
        t-val = 0. 
        FOR EACH l-besthis WHERE /*l-besthis.lager-nr = l-lager.lager-nr 
            AND*/ l-besthis.lager-nr GE from-lager
            AND l-besthis.lager-nr LE to-lager
            AND l-besthis.anf-best-dat EQ start-from-date
            AND l-besthis.artnr GE from-art AND l-besthis.artnr LE to-art NO-LOCK,
            FIRST l-lager WHERE l-lager.lager-nr = l-besthis.lager-nr NO-LOCK:
            /* FDL Comment Serverless: #671
                ...
                */
                ...
                DO: 
                    ...
                    DO: 
                        ...
                        art-bestand.vk-preis = t-vkpreis.vk-preis.
        ...
        FOR EACH l-ophis WHERE /*l-ophis.lager-nr = l-lager.lager-nr 
            AND*/ l-ophis.lager-nr GE from-lager
            AND l-ophis.lager-nr LE to-lager  /*l-ophis.artnr = l-besthis.artnr */
            AND l-ophis.artnr GE from-art
            AND l-ophis.artnr LE to-art
            AND (l-ophis.datum GE start-from-date AND l-ophis.datum LE to-date) 
            AND NOT vhp.l-ophis.fibukonto MATCHES "*;CANCELLED*" NO-LOCK BY l-ophis.lager-nr BY l-ophis.artnr BY l-ophis.datum:
        
            FIND FIRST art-bestand WHERE art-bestand.artnr = l-ophis.artnr AND art-bestand.lager = l-ophis.lager-nr NO-LOCK.
            IF AVAILABLE art-bestand THEN
            DO:
                IF l-ophis.op-art = 1 OR l-ophis.op-art = 2 THEN 
                DO: 
                    art-bestand.incoming = art-bestand.incoming + l-ophis.anzahl. 
                    art-bestand.in-val = art-bestand.in-val + l-ophis.anzahl * (art-bestand.vk-preis / (diff-month + 1)). 
                ...
                DO: 
                    ...
                    art-bestand.out-val = art-bestand.out-val + l-ophis.anzahl * (art-bestand.vk-preis / (diff-month + 1)).
        ...
        FOR EACH art-bestand:
            art-bestand.prevval  = art-bestand.prevqty * (art-bestand.vk-preis / (diff-month + 1)).
            art-bestand.actval   = art-bestand.actqty * (art-bestand.vk-preis / (diff-month + 1)).
        END.
        ...
        FOR EACH art-bestand WHERE art-bestand.lager = l-lager.lager-nr  
            ...
            DO:                                  
                ...
                DO: 
                    ...
                    FIND FIRST l-untergrup WHERE l-untergrup.zwkum = zwkum NO-LOCK NO-ERROR. 
                    IF AVAILABLE l-untergrup THEN bezeich = l-untergrup.bezeich. 
                END. 
                ...
                DO:
                    ...
                    DO:
                        ...
                        FIND FIRST t-vkpreis WHERE t-vkpreis.artnr = art-bestand.artnr NO-LOCK NO-ERROR. 
                        IF AVAILABLE t-vkpreis THEN art-bestand.avrg-pr = (t-vkpreis.vk-preis / (diff-month + 1)).
                    END.                     
                END.
