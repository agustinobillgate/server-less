...
DEFINE TEMP-TABLE cl-list 
  ...
  /*Ragung Penambahan flag-rate for web , apa*/
  ...
  FIELD flag-rate       AS LOGICAL   
  FIELD flag-argtcode   AS LOGICAL
  . 
...
DEFINE VARIABLE price               AS DECIMAL  NO-UNDO. /* Dzikri 796D85 - selecting f-argt if exist*/
...
PROCEDURE create-billbalance: 
  ...
  DEFINE VARIABLE rmrate              AS DECIMAL. /*Agung tambah validasi untuk mengecek fixed rate*/
  DEFINE VARIABLE fix-rate            AS LOGICAL.
  DEFINE VARIABLE it-exist            AS LOGICAL.
  DEFINE VARIABLE ebdisc-flag         AS LOGICAL.
  DEFINE VARIABLE kbdisc-flag         AS LOGICAL.
  DEFINE VARIABLE bill-date           AS DATE.
  DEFINE VARIABLE argtnr              AS CHAR.
  DEFINE VARIABLE rate-found          AS LOGICAL.
  DEFINE VARIABLE restricted-disc     AS LOGICAL.
  DEFINE VARIABLE kback-flag          AS LOGICAL.
  DEFINE VARIABLE bonus-array         AS LOGICAL EXTENT 999.
  DEFINE VARIABLE wd-array AS INTEGER EXTENT 8 
  INITIAL [7, 1, 2, 3, 4, 5, 6, 7].
  DEFINE VARIABLE w-day                AS INTEGER.
  DEFINE VARIABLE rack-rate            AS LOGICAL. 
...
/* add sorttype by damen 07/03/23 485054 */
  ...
  DO:
     FOR EACH res-line WHERE (res-line.active-flag LE 1) AND 
      (res-line.resstatus LE 2 OR res-line.resstatus = 5 OR res-line.resstatus = 6) AND
      NOT (res-line.ankunft GT curr-date) AND
      NOT (res-line.abreise LT curr-date) NO-LOCK BY res-line.resname: 

            do-it = YES.
            IF res-line.abreise = curr-date THEN do-it = res-line.ankunft = curr-date. /* DAY USE */
        
            IF do-it THEN
            DO:
                RELEASE zimmer.
                IF res-line.zinr NE "" THEN FIND FIRST zimmer WHERE zimmer.zinr = res-line.zinr NO-LOCK.
                ...
                IF res-line.zipreis NE 0 THEN r-qty = r-qty + res-line.zimmeranz. 
                ...
                ASSIGN 
                  sum-list.pax = sum-list.pax + 
                    (res-line.erwachs + res-line.kind1 + res-line.kind2) * res-line.zimmeranz 
                  sum-list.adult = sum-list.adult + res-line.erwachs * res-line.zimmeranz   /*FD*/
                  sum-list.com = sum-list.com + res-line.gratis * res-line.zimmeranz
                . 
            /* 
              sum-list.lodging = sum-list.lodging + res-line.zipreis * frate 
              sum-list.t-rev = sum-list.t-rev + res-line.zipreis * frate. 
            */ 
                DO curr-i = 1 TO res-line.zimmeranz:  /* SY 16 Dec 2021 */
                    fix-rate = NO.
                    ...
                    ASSIGN
                      ...
                      cl-list.name = res-line.NAME + "-" /*NAUFAL 250221 - tambah delimeter untuk case billing address tidak muncul*/   
                	...
                	IF res-line.zimmeranz GT 1 THEN ASSIGN cl-list.zinr = "#" + STRING(curr-i,"99").
                    IF AVAILABLE zimmer THEN cl-list.sleeping = zimmer.sleeping.
                    ...
                    /*MNaufal 170322 - MNaufal 170322 - for Ramada Solo request DFDC33*/
                    ...
                    .
                    IF AVAILABLE bill THEN cl-list.rechnr   = bill.rechnr.
                    ...
                    IF NOT AVAILABLE argt-list THEN DO:
                        ...
                        ASSIGN 
                            ...
                            argt-list.bezeich   = arrangement.argt-bez                    /* Rulita 261124 | Fixing serverless Issue 161 */
                            argt-list.room      = 1
                            argt-list.pax       = res-line.erwachs + res-line.kind1 + res-line.kind2 + res-line.gratis + cl-list.comch.
                    END.
                    ELSE DO:
                        ASSIGN
                            argt-list.room    = argt-list.room + 1
                            argt-list.pax     = argt-list.pax + (res-line.erwachs + res-line.gratis).
                    END.
                    ...
                    IF AVAILABLE reslin-queasy THEN DO: 
                        ...
                        ASSIGN cl-list.fix-rate = "F".
                    END.
                    IF NOT fix-rate THEN
                    DO:
                    ...
                    IF res-line.zipreis NE rmrate AND fix-rate THEN DO:
                        cl-list.flag-rate = YES.
                    END.
                    ELSE cl-list.flag-rate = NO.
                    ...
                    IF AVAILABLE reslin-queasy AND res-line.arrangement NE reslin-queasy.char1 THEN cl-list.flag-argtcode = YES.
                      ...
                      FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr 
                        ...
                        DO: 
                          ...
                          DO:
                            ...
                            /*MESSAGE res-line.zinr argt-betrag
                    ...
                    FOR EACH fixleist WHERE fixleist.resnr = res-line.resnr 
                      ...
                      END. 
                    END.
                    /* Dzikri 796D85 - arrangment fixed cost appear in RRB */
                    FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr 
                      ...
                      /* Dzikri 3DC423 - Repair Arrangemnt type 6
                        RUN check-fixargt-posted(argt-line.argt-artnr, argt-line.departement, 
                            argt-line.fakt-modus, argt-line.intervall,
                            OUTPUT post-it). */
                        IF argt-line.fakt-modus EQ 6 THEN
                        DO:
                          FIND FIRST argt6-list WHERE argt6-list.argtnr EQ argt-line.argtnr 
                            AND argt6-list.departement EQ argt-line.departement 
                            AND argt6-list.argt-artnr  EQ argt-line.argt-artnr 
                            AND argt6-list.vt-percnt   EQ argt-line.vt-percnt
                            AND argt6-list.resnr       EQ res-line.resnr 
                            AND argt6-list.reslinnr    EQ res-line.reslinnr 
                            AND argt6-list.is-charged  EQ 1  NO-LOCK NO-ERROR.
                          IF NOT AVAILABLE argt6-list THEN
                          DO:
                            CREATE argt6-list.
                            ASSIGN
                              argt6-list.argtnr      = argt-line.argtnr 
                              argt6-list.departement = argt-line.departement 
                              argt6-list.argt-artnr  = argt-line.argt-artnr 
                              argt6-list.vt-percnt   = argt-line.vt-percnt
                              argt6-list.is-charged  = 1
                              argt6-list.resnr       = res-line.resnr 
                              argt6-list.reslinnr    = res-line.reslinnr 
                            .
                          END.
                          IF argt6-list.period LT argt-line.intervall THEN
                          DO:
                            ...
                            DO:
                              ...
                              DO:
                                RUN check-fixargt-posted(argt-line.argt-artnr, argt-line.departement, 
                                    argt-line.fakt-modus, argt-line.intervall, reslin-queasy.date1,
                                    OUTPUT post-it).
                                IF post-it EQ YES THEN argt6-list.period = argt6-list.period + 1.
                              END.
                            END.
                            ELSE
                            DO:
                              RUN check-fixargt-posted(argt-line.argt-artnr, argt-line.departement, 
                                argt-line.fakt-modus, argt-line.intervall, res-line.ankunft,
                                OUTPUT post-it). 
                              IF post-it EQ YES THEN argt6-list.period = argt6-list.period + 1.
                            END.
                          END.
                      ...
                      DO: 
                          ...
                          FOR EACH reslin-queasy WHERE reslin-queasy.key = "fargt-line" 
                            ...
                            AND curr-date LE reslin-queasy.date2 NO-LOCK:
                              ...
                              DO:
                                ...
                                IF AVAILABLE zwkum THEN price = rm-rate * INT(reslin-queasy.char2) / 100 * (-1).
                                ELSE price = rm-rate * INT(reslin-queasy.char2) / 100. 
                              END.
                              ELSE
                              DO:
                        ...
                        /* Dzikri C5ABD0 - IN% calculated */
                        DO:
                          IF argt-line.betrag GT 0 THEN fcost = argt-line.betrag * pax.
                          ELSE
                          DO:
                            FIND FIRST zwkum WHERE zwkum.zknr EQ artikel.zwkum 
                              AND zwkum.departement EQ artikel.departement
                              AND zwkum.bezeich MATCHES "*DISCOUNT*" NO-LOCK NO-ERROR.
                            IF AVAILABLE zwkum THEN fcost = (rm-rate * (argt-line.betrag / 100)) * pax.
                            ELSE fcost = (rm-rate * (- argt-line.betrag / 100)) * pax.
                          END.
                          ASSIGN
                          /*fcost = argt-line.betrag * pax*/
                            /**/cl-list.t-rev = cl-list.t-rev + fcost
                            sum-list.t-rev = sum-list.t-rev + fcost * frate
                          . 
                        END.
                        ...
                        DO:
                          ASSIGN 
                            s-list.f-betrag = s-list.f-betrag + fcost
                            s-list.l-betrag = s-list.l-betrag + fcost * frate 
                            s-list.anzahl   = s-list.anzahl + pax 
                            /**/cl-list.bfast   = cl-list.bfast + fcost
                            sum-list.bfast  = sum-list.bfast + fcost * frate
                          .
                            IF res-line.adrflag THEN Ltot-bfast = Ltot-bfast + fcost * frate. 
  ...
  DO:
      ...
      (res-line.resstatus LE 2 OR res-line.resstatus = 5 OR res-line.resstatus = 6) AND
      NOT (res-line.ankunft GT curr-date) AND
      NOT (res-line.abreise LT curr-date) NO-LOCK BY res-line.zinr BY res-line.resnr: 
        ...
        DO:
            ...
            FIND FIRST arrangement WHERE 
                arrangement.arrangement = res-line.arrangement NO-LOCK NO-ERROR. 
            IF NOT AVAILABLE arrangement THEN                                                     /* Rulita 221124 | Fixing serverless issue 161 */
            DO:
              RETURN.
            END.
            FIND FIRST artikel WHERE artikel.artnr = arrangement.argt-artikelnr 
        ...
        */ 
            DO curr-i = 1 TO res-line.zimmeranz:  /* SY 16 Dec 2021 */
                ...
                IF res-line.zimmer-wunsch MATCHES("*ChAge*") THEN
                DO:
                    DO loop2 = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch, ";") - 1:
                        s = ENTRY(loop2, res-line.zimmer-wunsch, ";").
                        IF SUBSTR(s,1,5) = "ChAge" THEN
                            /*ASSIGN cl-list.age2 = INT(SUBSTR(s,6)).*/
                            ASSIGN cl-list.age2 = SUBSTR(s,6).
                    END.
                ...
                DO: 
                  ...
                  DO: 
                    ...
                    DO j = 1 TO finteger: 
                      m = m * 10. 
                    END. 
                    lodge-betrag = ROUND(lodge-betrag / m, 0) * m. 
                  END. 
                END. 
...
DEFINE BUFFER argtline FOR argt-line. 
    ...
    DO: 
      /* Dzikri 3DC423 - Repair Arrangemnt type 6
          IF (res-line.ankunft + (argt-line.intervall - 1)) GE bill-date 
            THEN add-it = YES. */
        FIND FIRST argt6-list WHERE argt6-list.argtnr EQ argtline.argtnr 
          AND argt6-list.departement EQ argtline.departement 
          AND argt6-list.argt-artnr  EQ argtline.argt-artnr 
          AND argt6-list.vt-percnt   EQ argtline.vt-percnt
          AND argt6-list.resnr       EQ res-line.resnr 
          AND argt6-list.reslinnr    EQ res-line.reslinnr 
          AND argt6-list.is-charged  EQ 0 NO-LOCK NO-ERROR.
        IF NOT AVAILABLE argt6-list THEN
        DO:
          CREATE argt6-list.
          ASSIGN
            argt6-list.argtnr      = argtline.argtnr 
            argt6-list.departement = argtline.departement 
            argt6-list.argt-artnr  = argtline.argt-artnr 
            argt6-list.vt-percnt   = argtline.vt-percnt
            argt6-list.is-charged  = 0
            argt6-list.resnr       = res-line.resnr 
            argt6-list.reslinnr    = res-line.reslinnr 
          .
        END.
        IF argt6-list.period LT argtline.intervall THEN
        DO:
          FIND FIRST reslin-queasy WHERE reslin-queasy.key = "fargtline" 
            AND reslin-queasy.char1    = "" 
          ...
          DO:
            ...
            IF AVAILABLE reslin-queasy THEN 
            DO:
              IF (reslin-queasy.date1 + (argtline.intervall - 1 /**/)) GE curr-date THEN
              DO:
                add-it = YES.
                argt6-list.period = argt6-list.period + 1.
              END.
            END.
          END.
          ELSE
          DO:
            IF (res-line.ankunft + (argtline.intervall - 1 /**/)) GE curr-date THEN 
            DO:
              add-it = YES.
              argt6-list.period = argt6-list.period + 1.
            END.
  ...
  DO: 
    ...
    ELSE 
    DO:
      argt-betrag = (rm-rate * (argt-betrag / 100)) * qty.                        /* Rulita 261124 | Fixing serverless issue 161 */
      argt-betrag = -1 * argt-betrag.
    END.
