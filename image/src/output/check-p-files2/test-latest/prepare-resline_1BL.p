...
IF AVAILABLE htparam AND htparam.bezeich NE "not used" THEN avail-gdpr = htparam.flogical.
...
END FUNCTION. 
  ...
  RUN htpdate.p  (87,  OUTPUT f-resline.ci-date).
  ...
  DO:  
    ...
    DO: 
      ...
      FIND FIRST queasy WHERE queasy.key EQ 9 AND queasy.number1 EQ bill-instruct NO-LOCK NO-ERROR. 
...
*/
  ...
  FIND FIRST zimkateg WHERE zimkateg.kurzbez = res-dynaRate.rmcat NO-LOCK.
  ...
  FIND FIRST paramtext WHERE paramtext.txtnr = (9200 + res-dynaRate.setup) 
    NO-LOCK. 
  FIND FIRST queasy WHERE queasy.KEY = 18 AND queasy.number1 = res-dynaRate.markNo
    NO-LOCK.
  FIND FIRST waehrung WHERE waehrung.wabkurz = queasy.char3 NO-LOCK.
  
  ASSIGN
    reslin-list.ankunft         = res-dynaRate.date1
    reslin-list.arrangement     = res-dynaRate.argt
    reslin-list.zikatnr         = zimkateg.zikatnr
    reslin-list.reserve-int     = res-dynaRate.markNo
    reslin-list.betriebsnr      = waehrung.waehrungsnr
    reslin-list.l-zuordnung[1]  = zimkateg.zikatnr
    ...
    f-resline.currency          = queasy.char3
  .
