...
DEF INPUT PARAMETER TABLE FOR active-roomlist. /*FDL Nov 28, 2024: Ticket 9824CF*/
DEF INPUT PARAMETER v-mode AS INTEGER. /*FDL Nov 28, 2024: Ticket 9824CF*/
DEF INPUT PARAMETER location AS CHAR.
DEF INPUT PARAMETER froom AS CHAR.
DEF INPUT PARAMETER troom AS CHAR.

IF v-mode EQ 1 THEN RUN auto-assignment.
ELSE RUN auto-assignmen-with-selectedroom. /*FDL Nov 28, 2024: Ticket 9824CF*/
...
PROCEDURE auto-assignmen-with-selectedroom: 
DEFINE BUFFER s1-list FOR s-list. 
DEFINE BUFFER s2-list FOR s-list. 
DEFINE BUFFER rline   FOR res-line. 
DEFINE BUFFER resline FOR res-line. 

DEFINE VARIABLE last-zinr AS CHAR. 
DEFINE VARIABLE do-it AS LOGICAL. 
DEFINE VARIABLE found AS LOGICAL. 

    FOR EACH s1-list WHERE s1-list.zinr EQ "" AND s1-list.resstatus NE 11 AND s1-list.zimmeranz EQ 1: 
        FIND FIRST rline WHERE RECID(rline) EQ s1-list.res-recid NO-LOCK. 
        found = NO. 
        
        FIND FIRST active-roomlist WHERE active-roomlist.zikatnr EQ rline.zikatnr
            AND active-roomlist.room-selected
            AND active-roomlist.setup EQ rline.setup 
            AND active-roomlist.zinr GT last-zinr NO-LOCK NO-ERROR. 
        DO WHILE AVAILABLE active-roomlist AND NOT found: 
            do-it = YES.           
            IF do-it THEN 
            DO: 
                FIND FIRST outorder WHERE outorder.zinr EQ active-roomlist.zinr 
                    AND outorder.betriebsnr NE rline.resnr 
                    AND ((rline.ankunft GE gespstart AND rline.ankunft LE gespende) 
                    OR (rline.abreise GT gespstart AND rline.abreise LE gespende) 
                    OR (gespstart GE rline.ankunft AND gespstart LT rline.abreise) 
                    OR (gespende GE rline.ankunft AND gespende LE rline.abreise)) NO-LOCK NO-ERROR. 
                IF AVAILABLE outorder THEN do-it = NO. 
            END. 
            IF do-it THEN 
            DO: 
                FIND FIRST resline WHERE RECID(resline) NE RECID(rline) 
                    AND resline.resstatus LE 6 AND resline.active-flag LE 1 
                    AND resline.zinr EQ active-roomlist.zinr 
                    AND ((rline.ankunft GE resline.ankunft AND rline.ankunft LT resline.abreise) 
                    OR (rline.abreise GT resline.ankunft AND rline.abreise LE resline.abreise) 
                    OR (resline.ankunft GE rline.ankunft AND resline.ankunft LT rline.abreise) 
                    OR (resline.abreise GT rline.ankunft AND resline.abreise LE rline.abreise)) NO-LOCK NO-ERROR. 
                IF AVAILABLE resline THEN do-it = NO. 
            END. 
            IF do-it THEN 
            DO: 
                s1-list.zinr = active-roomlist.zinr. 
                last-zinr = active-roomlist.zinr. 
                found = YES. 
            END.     
            ELSE
            DO:
                FIND NEXT active-roomlist WHERE active-roomlist.zikatnr EQ rline.zikatnr 
                    AND active-roomlist.room-selected
                    AND active-roomlist.setup EQ rline.setup 
                    AND active-roomlist.zinr GT last-zinr NO-LOCK NO-ERROR. 
            END.          
    ...
    last-zinr = "". 
    FOR EACH s1-list WHERE s1-list.zinr EQ "" AND s1-list.active-flag EQ 0 AND s1-list.resstatus NE 11: 
        FIND FIRST rline WHERE RECID(rline) EQ s1-list.res-recid NO-LOCK. 
        found = NO. 
        
        FIND FIRST active-roomlist WHERE active-roomlist.zikatnr EQ rline.zikatnr 
            AND active-roomlist.room-selected
            AND active-roomlist.setup NE rline.setup 
            AND active-roomlist.zinr GT last-zinr NO-LOCK NO-ERROR. 
        DO WHILE AVAILABLE active-roomlist AND NOT found: 
            do-it = YES.       
            IF do-it THEN 
            ...
            IF do-it THEN 
            DO: 
                FIND FIRST s2-list WHERE s2-list.zinr EQ active-roomlist.zinr NO-LOCK NO-ERROR. 
                IF AVAILABLE s2-list THEN do-it = NO. 
            END. 
            IF do-it THEN 
            DO: 
