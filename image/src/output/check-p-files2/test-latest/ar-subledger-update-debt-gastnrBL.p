...
/*start bernatd 8F2A06 */
FIND FIRST h-artikel WHERE h-artikel.artnrfront = t-artnr 
    AND h-artikel.departement = t-dept NO-LOCK NO-ERROR.
IF NOT AVAILABLE h-artikel THEN
DO:
  FIND FIRST artikel WHERE artikel.artnr = t-artnr 
    AND artikel.departement = t-dept NO-LOCK NO-ERROR.
END.
IF AVAILABLE h-artikel OR AVAILABLE artikel THEN
DO:
