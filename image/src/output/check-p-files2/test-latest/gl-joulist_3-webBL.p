/*FDL Nov 10, 2023 => Ticket 7EDC2B - New Concept With Scrolling Then Hit API Like Guest Profile*/
...
DEFINE TEMP-TABLE out-list
    ...
    FIELD counter       AS INTEGER /*FD*/
    /*INDEX counter_idx counter /*FD Dec 23, 2021*/*/
    .
...
DEFINE INPUT PARAMETER vMode        AS INT.
...
DEFINE INPUT PARAMETER srt-no       AS LOGICAL.		/* add by damen 16/03/2023 40B108*/							
DEFINE INPUT PARAMETER curr-srecid  AS INTEGER.
DEFINE INPUT PARAMETER curr-of-coa  AS CHAR.
DEFINE INPUT PARAMETER curr-date    AS DATE.
DEFINE INPUT PARAMETER curr-tdebit  AS DECIMAL.
DEFINE INPUT PARAMETER curr-tcredit AS DECIMAL.
DEFINE INPUT PARAMETER curr-refno   AS CHAR.
DEFINE OUTPUT PARAMETER last-srecid AS INTEGER.  
DEFINE OUTPUT PARAMETER last-of-coa AS CHAR.
DEFINE OUTPUT PARAMETER last-date   AS DATE.
DEFINE OUTPUT PARAMETER last-tdebit AS DECIMAL.
DEFINE OUTPUT PARAMETER last-tcredit AS DECIMAL.
DEFINE OUTPUT PARAMETER last-refno  AS CHAR.
DEFINE OUTPUT PARAMETER TABLE FOR out-list.


DEFINE VARIABLE datum1 AS DATE NO-UNDO.
DEFINE VARIABLE datum2 AS DATE NO-UNDO.
DEFINE VARIABLE count-i AS INTEGER INITIAL 0 NO-UNDO.
DEFINE VARIABLE counter AS INTEGER INITIAL 0 NO-UNDO.
DEFINE VARIABLE out-flag AS LOGICAL NO-UNDO.
DEFINE VARIABLE eor-refno AS CHARACTER NO-UNDO.
...
DO: 
    ...
    IF vMode EQ 1 THEN
    DO:
        FOR EACH out-list WHERE out-list.bezeich MATCHES "*T O T A L*":
            last-tdebit     = last-tdebit + out-list.debit.
            last-tcredit    = last-tcredit + out-list.credit.
        END.
    END.
    ELSE
    DO:
        ...
        END.
        ASSIGN
            last-tdebit     = last-tdebit + curr-tdebit 
            last-tcredit    = last-tcredit + curr-tcredit
        .

        FOR EACH gl-jouhdr WHERE gl-jouhdr.datum GE from-date
            AND gl-jouhdr.datum LE to-date NO-LOCK BY gl-jouhdr.datum DESC BY gl-jouhdr.refno DESC:
    
            eor-refno = gl-jouhdr.refno.
            LEAVE.
        END.
        IF last-refno EQ eor-refno THEN
        DO:
            CREATE out-list.                        
            ASSIGN
                out-list.bezeich        = "GRAND T O T A L" 
                out-list.debit          = last-tdebit
                out-list.credit         = last-tcredit
                out-list.debit-str      = STRING(last-tdebit, "->,>>>,>>>,>>>,>>9.99")  
                out-list.credit-str     = STRING(last-tcredit, "->,>>>,>>>,>>>,>>9.99")
            .
        END.
    END.    
    */
          ...
          FOR EACH j-list NO-LOCK BY j-list.fibu BY j-list.datum:
            ...
            ELSE DO:
                  ...
                  IF AVAILABLE gl-journal THEN DO:
                     ...
                     FIND FIRST gl-account WHERE gl-account.fibukonto = gl-journal.fibukonto NO-LOCK.
      ...
      DO:
         FOR EACH j-list NO-LOCK BY j-list.fibu BY j-list.datum:
            ...
            IF AVAILABLE gl-jourhis THEN DO:
                FIND FIRST vhp.gl-jhdrhis WHERE vhp.gl-jhdrhis.jnr = vhp.gl-jourhis.jnr NO-LOCK NO-ERROR.
                FIND FIRST gl-acct WHERE gl-acct.fibukonto = vhp.gl-jourhis.fibukonto AND gl-acct.main-nr = gl-main.nr NO-LOCK NO-ERROR.
                DELETE j-list.
            ...
            ELSE DO:
                 ...
                 IF AVAILABLE gl-journal THEN DO:
                        FIND FIRST gl-jouhdr WHERE gl-jouhdr.jnr = gl-journal.jnr NO-LOCK NO-ERROR.
                        FIND FIRST gl-acct WHERE gl-acct.fibukonto = gl-journal.fibukonto AND gl-acct.main-nr = gl-main.nr NO-LOCK NO-ERROR.
                        DELETE j-list.
...
PROCEDURE create-hlist: 
  ...
  DO:
      IF sorttype = 2 THEN
      DO:
        /* add by damen 16/03/2023 40B108*/
          IF srt-no = NO THEN
          DO:
             FOR EACH g-list,
          ...
          DO:
             FOR EACH g-list,
                ...
                FIRST vhp.gl-jhdrhis WHERE vhp.gl-jhdrhis.jnr = vhp.gl-jourhis.jnr NO-LOCK,
                FIRST gl-acct WHERE gl-acct.fibukonto = vhp.gl-jourhis.fibukonto NO-LOCK 
                BY vhp.gl-jourhis.fibukonto BY vhp.gl-jourhis.datum 
                BY gl-jhdrhis.jnr : 
                DELETE g-list.
                /*
                      FOR EACH gl-jourhis WHERE  gl-jourhis.fibukonto GE from-fibu 
...
PROCEDURE create-glist:
    ...
    DO:
        ...
        DO:
            FOR EACH gl-jouhdr WHERE gl-jouhdr.datum GE from-date
                AND gl-jouhdr.datum LE to-date NO-LOCK BY gl-jouhdr.datum BY gl-jouhdr.refno:                   
                ...
                FOR EACH gl-journal WHERE gl-journal.jnr = gl-jouhdr.jnr
                    AND gl-journal.fibukonto GE from-fibu
                    AND gl-journal.fibukonto LE to-fibu 
                    NO-LOCK BY gl-journal.fibukonto /*BY RECID(gl-journal)*/:    
                    ...
                    last-of-coa = gl-journal.fibukonto.
                    last-srecid = RECID(gl-journal).
                END.
                last-date = gl-jouhdr.datum.
                last-refno = gl-jouhdr.refno.

                counter = counter + 1.
                IF counter GE 15 THEN LEAVE.
            END.
        END.
        ELSE
        DO:
            FOR EACH gl-jouhdr WHERE gl-jouhdr.datum GE curr-date
                AND gl-jouhdr.datum LE to-date
                /*AND gl-jouhdr.refno GT curr-refno*/
                NO-LOCK BY gl-jouhdr.datum BY gl-jouhdr.refno:                   
                

                IF gl-jouhdr.datum EQ curr-date AND gl-jouhdr.refno GT curr-refno THEN
                DO:
                    IF journaltype EQ 0 AND excl-other AND gl-jouhdr.jtype NE 0 THEN 
                        NEXT.
                    ELSE IF journaltype NE 0 AND other-dept 
                    ...
                    ELSE IF journaltype NE 0 AND NOT other-dept 
                      ...
                      AND gl-jouhdr.jtype NE journaltype1 THEN NEXT.
                    FOR EACH gl-journal WHERE gl-journal.jnr = gl-jouhdr.jnr
                        AND gl-journal.fibukonto GE curr-of-coa
                        AND gl-journal.fibukonto LE to-fibu 
                        /*AND RECID(gl-journal) GT curr-srecid*/
                        NO-LOCK BY gl-journal.fibukonto /*BY RECID(gl-journal)*/:    
                    ...
                    END.
                END.   
                ELSE IF gl-jouhdr.datum GT curr-date THEN
                DO:
                    IF journaltype EQ 0 AND excl-other AND gl-jouhdr.jtype NE 0 THEN 
                        NEXT.
                    ELSE IF journaltype NE 0 AND other-dept 
    ...
    DO: 
        ...
        DO:       
            FOR EACH g-list,
                ...
                IF gl-journal.jnr EQ 28638 THEN
                DO:
                    MESSAGE gl-jouhdr.refno SKIP gl-journal.fibukonto SKIP gl-jouhdr.datum
                ...
                */
                IF gl-account.acc-type = 1 OR gl-account.acc-type = 4 THEN
                    ASSIGN balance = balance - gl-journal.debit + gl-journal.credit.
                ELSE 
                    ASSIGN balance     = balance + gl-journal.debit - gl-journal.credit.
...
DEF BUFFER joubuff    FOR gl-journal.
    ...
    prev-bal = 0. 
    FIND FIRST gl-account WHERE gl-account.fibukonto = fibu NO-LOCK.
