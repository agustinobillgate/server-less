...
DEFINE TEMP-TABLE summary-list
    FIELD arrangement AS CHAR FORMAT "X(6)"
    FIELD rooms      AS INT INIT 0
    FIELD pax        AS INT INIT 0
    FIELD comp       AS INT INIT 0
    FIELD ch1        as int init 0
    .
...
/*DEFINE variable case-type       AS INTEGER init 1.
DEFINE variable fdate           AS DATE    init 01/12/2019.
DEFINE variable tdate           AS DATE    init 01/14/2019.
DEFINE variable walk-in         AS INTEGER init 1.
DEFINE variable wi-grp          AS INTEGER init 1.
DEFINE variable walkin-flag     AS LOGICAL init NO.*/


DEFINE VARIABLE wi-int                          AS INTEGER.
DEFINE VARIABLE tot-rm                          AS INTEGER format ">>>>9" INIT 0.
DEFINE VARIABLE tot-pax                         AS INTEGER format ">>>>9" INIT 0.
DEFINE VARIABLE tot-comp                         AS INTEGER format ">>>>9" INIT 0.
define variable tot-ch1                         as integer format ">>>>9" init 0.
...
CREATE summary-list.
ASSIGN
    summary-list.arrangement = "TOTAL"
    summary-list.rooms = tot-rm  
    summary-list.pax   = tot-pax 
    summary-list.comp  = tot-comp
    summary-list.ch1   = tot-ch1 
    .

PROCEDURE disp-arlist1:
    
  IF fdate = ? OR tdate = ? THEN 
  DO:
     IF NOT walkin-flag THEN
     DO: 
         FOR EACH res-line WHERE 
           (resstatus = 6 OR resstatus = 13) AND res-line.active-flag = 1 /* Malik Serverless : active-flag -> res-line.active-flag */
      ...
      DO:
         FOR EACH res-line WHERE 
           (resstatus = 6 OR resstatus = 13) AND res-line.active-flag = 1 NO-LOCK, /* Malik Serverless : active-flag -> res-line.active-flag */
           FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK, 
  ...
  DO:
     ...
     DO:
         FOR EACH res-line WHERE 
            res-line.ankunft GE fdate AND res-line.ankunft LE tdate AND res-line.gastnr EQ wi-int 
           AND (resstatus NE 9 AND resstatus NE 10 AND resstatus NE 12) 
           AND res-line.active-flag LE 2 NO-LOCK, /* Malik Serverless : active-flag -> res-line.active-flag */
           FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK, 
     ...
     DO:
         FOR EACH res-line WHERE 
            res-line.ankunft GE fdate AND res-line.ankunft LE tdate
           AND (resstatus NE 9 AND resstatus NE 10 AND resstatus NE 12) 
           AND res-line.active-flag LE 2 NO-LOCK, /* Malik Serverless : active-flag -> res-line.active-flag */
           FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK, 
           FIRST segment WHERE (segment.segmentcode = reservation.segmentcode) AND 
     ...
     DO:
        FOR EACH res-line WHERE 
           (resstatus = 6 OR resstatus = 13) 
            AND res-line.gastnr EQ wi-int AND res-line.active-flag = 1 NO-LOCK, /* Malik Serverless : active-flag -> res-line.active-flag */
           FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK, 
           FIRST segment WHERE (segment.segmentcode = reservation.segmentcode) 
            NO-LOCK,
           FIRST zimkateg WHERE zimkateg.zikatnr = res-line.zikatnr 
...
PROCEDURE create-walkin-list:
    ...
    .
    FIND FIRST summary-list WHERE summary-list.arrangement EQ t-walkin-list.arrangement NO-LOCK NO-ERROR.
    IF NOT AVAILABLE summary-list THEN
    DO:
        CREATE summary-list.
        ASSIGN
            summary-list.arrangement = t-walkin-list.arrangement
            summary-list.rooms = summary-list.rooms + t-walkin-list.zimmeranz
            summary-list.pax   = summary-list.pax   + t-walkin-list.erwachs * t-walkin-list.zimmeranz
            summary-list.comp  = summary-list.comp  + t-walkin-list.gratis * t-walkin-list.zimmeranz        
            summary-list.ch1   = summary-list.ch1   + t-walkin-list.kind1 * t-walkin-list.zimmeranz     
            tot-rm    = tot-rm    + t-walkin-list.zimmeranz
            tot-pax   = tot-pax   + t-walkin-list.erwachs * t-walkin-list.zimmeranz
            tot-comp  = tot-comp  + t-walkin-list.gratis * t-walkin-list.zimmeranz      
            tot-ch1   = tot-ch1   + t-walkin-list.kind1 * t-walkin-list.zimmeranz                               
            .
    END.
    ELSE
    DO:
