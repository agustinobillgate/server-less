...
DEFINE BUFFER b-reservation FOR reservation. 
...
IF NOT AVAILABLE res-line THEN                          /* RULITA 131124 | Fixing for serverless */
DO:
    RETURN.
END. 
...
FIND FIRST b-reservation WHERE b-reservation.resnr = res-line.resnr 
  AND b-reservation.gastnr = res-line.gastnr NO-LOCK NO-ERROR.
IF NOT AVAILABLE b-reservation THEN
DO:
  msg-str = translateExtended ("Guest number mismatch detected. Refresh the reservation records now?",lvCAREA,"").
  RETURN.
END.
