...
CASE casetype:
    ...
    */
    ...
    DO:
        flogical = ?.
        FIND FIRST htparam WHERE htparam.paramnr EQ inp-param NO-LOCK NO-ERROR.
        IF AVAILABLE htparam THEN flogical = htparam.flogical.
    END.
    WHEN 2 THEN 
    DO:
        fdate = ?.
        FIND FIRST htparam WHERE htparam.paramnr EQ inp-param NO-LOCK NO-ERROR.
        IF AVAILABLE htparam THEN fdate = htparam.fdate.        
    END.
    WHEN 3 THEN 
    DO:
        fchar = ?.
        FIND FIRST htparam WHERE htparam.paramnr EQ inp-param NO-LOCK NO-ERROR.
        IF AVAILABLE htparam THEN fchar = htparam.fchar.  
    END.
    WHEN 4 THEN 
    DO:
        fint = ?.
        FIND FIRST htparam WHERE htparam.paramnr EQ inp-param NO-LOCK NO-ERROR.
        IF AVAILABLE htparam THEN fint = htparam.fint.  
    END.
    WHEN 5 THEN 
    DO:
        fdec = ?.
        FIND FIRST htparam WHERE htparam.paramnr EQ inp-param NO-LOCK NO-ERROR.
        IF AVAILABLE htparam THEN fdec = htparam.fdecimal.  
    END.
END CASE.
