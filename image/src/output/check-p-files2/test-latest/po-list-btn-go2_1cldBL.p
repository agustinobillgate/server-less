...
IF app-sort EQ "Approve 4" THEN
    ASSIGN approval = 3.
IF app-sort EQ "Approved" THEN /*bernatd EA0180*/
    ASSIGN approval = 4.
      ...
      DO:
          FOR EACH l-orderhdr 
              ...
              BY l-lieferant.firma BY l-orderhdr.bestelldatum BY l-orderhdr.docu-nr:
                ...
                ELSE IF pr-only THEN
                DO:
                    IF SUBSTR(l-order1.lief-fax[1],1,1) EQ "R" THEN RUN create-temp-table-list.
                END.
                ELSE IF excl-dml-pr THEN
                DO:
                    IF SUBSTR(l-order1.lief-fax[1],1,1) NE "R" AND 
                        (SUBSTR(l-order1.lief-fax[1],1,1) NE "D" AND l-order1.lief-fax[3] NE "DML") THEN RUN create-temp-table-list.
                END.
...
PROCEDURE cr-temp-table:
    ...
    DO:
        ...
        DO:
            ...
            END.
        END.
        FIND FIRST l-order2 WHERE l-order2.docu-nr = l-orderhdr.docu-nr 
        AND l-order2.pos GT 0 AND l-order2.loeschflag = l-order1.loeschflag NO-LOCK NO-ERROR.
        DO WHILE AVAILABLE l-order2:
            ...
            AND l-order2.loeschflag = l-order1.loeschflag NO-LOCK NO-ERROR.
        END.
        ASSIGN
    ...
    DO:
        ...
        DO:
            ...
            DO:
                ...
                AND l-order2.loeschflag = l-order1.loeschflag NO-LOCK NO-ERROR. /* Oscar (29 Oktober 2024) - 6B33F6 */
                DO WHILE AVAILABLE l-order2:
                    FIND FIRST l-art WHERE l-art.artnr = l-order2.artnr NO-LOCK NO-ERROR.
                    IF AVAILABLE l-art THEN 
                    DO:
