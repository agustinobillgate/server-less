...
CASE op-type:
   ...
   DO:
       ...
       ELSE IF disptype = 8 THEN RUN create-umsatz8. /* Add By Bernatd 2025 -  display by semgment*/
   END.
   /*MT
   WHEN 1 THEN
   DO:
     ...
     DO:
       ...
       DO: 
           ...
           ASSIGN to-list.gastnr = genstat.gastnr
               ...
               END.
               /*Bernatd 48D8E1 2025*/
               IF MONTH(genstat.datum) EQ MONTH(to-date) /*genstat.datum GE fdate-mtd*/ AND ytd-flag = 1 OR ytd-flag = 2 THEN
               DO:
...
/*
PROCEDURE create-umsatz8:          /*Segement By Bernatd*/
   DEFINE VARIABLE mm AS INTEGER. 
   DEFINE VARIABLE yy AS INTEGER. 
   DEFINE VARIABLE from-date AS DATE. 
  ...
  tot-y-anztage = 0.
   ...
   DO:
       FOR EACH genstat WHERE
          ...
          AND genstat.res-logic[2] /*MU 27032012 sleeping = yes */
          USE-INDEX nat_ix NO-LOCK,
          FIRST segm1-list WHERE 
            segm1-list.segm = genstat.segmentcode AND 
            segm1-list.SELECTED = YES NO-LOCK,
          FIRST zikat-list WHERE 
            zikat-list.zikatnr = genstat.zikatnr AND 
            zikat-list.SELECTED = YES NO-LOCK,
          FIRST res-line WHERE res-line.resnr EQ genstat.resnr AND res-line.reslinnr EQ genstat.res-int[1] NO-LOCK  
          BY genstat.segmentcode 
          BY genstat.zikatnr:
