...
/*FIND FIRST vhp.gl-journal WHERE vhp.gl-journal.jnr = jnr AND INTEGER(RECID(vhp.gl-journal)) = srecid NO-LOCK NO-ERROR.FT serverless*/
FIND FIRST vhp.gl-journal WHERE vhp.gl-journal.jnr = jnr AND RECID(vhp.gl-journal) = srecid NO-LOCK NO-ERROR.
...
/*FOR EACH joulist WHERE joulist.jnr = jnr NO-LOCK, 
    ...
    BY joulist.selFlag DESCENDING BY gl-acct1.fibukonto:FT serverless*/
FOR EACH joulist WHERE joulist.jnr = jnr NO-LOCK 
    BY joulist.selFlag DESCENDING BY joulist.fibukonto:
    FIND FIRST gl-acct1 WHERE gl-acct1.fibukonto = joulist.fibukonto NO-LOCK NO-ERROR.
