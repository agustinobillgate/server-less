...
DEFINE VARIABLE zikatnr AS INTEGER INITIAL 0. 
  ...
  FOR EACH zimmer WHERE zimmer.sleeping = YES NO-LOCK,
      FIRST zkbuff WHERE zkbuff.zikatnr = zimmer.zikatnr
...
DEF BUFFER rlist            FOR room-list. 
  ...
  FOR EACH rmcat-list,
    ...
    i = 1. 
    DO WHILE i LE 21: 
      room-list.room[i] = rmcat-list.anzahl. 
      i = i + 1. 
    END. 
    ASSIGN 
    ...
    ELSE room-list.bezeich = STRING(rmcat-list.kurzbez1, "x(10)") + "GBAL". 
  END. 
  ...
  DO WHILE i LE 21: 
    FOR EACH kontline WHERE kontline.kontignr GT 0 AND kontline.betriebsnr = 1 
      ...
      occ-room[i]       = occ-room[i] + kontline.zimmeranz.
    END. 
    ...
    FOR EACH res-line WHERE res-line.active-flag LE 1 
      ...
      AND res-line.resstatus NE 4 AND res-line.ankunft LE datum 
      AND res-line.abreise GT datum AND res-line.kontignr LT 0 
  ...
  FOR EACH outorder WHERE outorder.betriebsnr LE 1 NO-LOCK, 
    ...
    FIND FIRST room-list WHERE room-list.zikatnr = zimmer.zikatnr 
      AND room-list.setup = zimmer.setup AND room-list.glores = NO NO-ERROR. 
    ...
    ELSE 
    DO i = 1 TO 21: 
      IF outorder.gespstart LE datum AND outorder.gespende GE datum THEN 
      ASSIGN 
        room-list.room[i] = room-list.room[i] - 1
        ooo-room[i]       = ooo-room[i] + 1
        .
      datum = datum + 1. 
    END. 
  END. 
  ...
  DO i = 1 TO 21:
    sum-list.summe[i] = STRING(ttl-room[i], "->>>9"). 
  END.

  CREATE sum-list.
  sum-list.bezeich = ("TOTAL OCC"). 
  DO i = 1 TO 21:
    sum-list.summe[i] = STRING(occ-room[i], "->>>9"). 
  END.

  CREATE sum-list.
  sum-list.bezeich = ("TOTAL OOO"). 
  DO i = 1 TO 21:
    sum-list.summe[i] = STRING(ooo-room[i], "->>>9"). 
  END.
  ...
  DO i = 1 TO 21: 
    s = 0. 
    FOR EACH room-list: 
      s = s + room-list.room[i]. 
    END. 
    sum-list.summe[i] = STRING(s, "->>>9"). 
    tmp-list[i] = s.
  END. 
  ...
  DO i = 1 TO 21: 
    s = INTEGER(tmp-list[i]) / tot-room * 100. 
    IF s LT 0 THEN sum-list.summe[i] = STRING(s, "->>9.9"). 
    ELSE sum-list.summe[i] = STRING(s, ">>9.9"). 
  END. 

  CREATE sum-list. /*FT 130614*/
  sum-list.bezeich = ("OCC in %"). 
  DO i = 1 TO 21:  
    s = occ-room[i] / ttl-room[i] * 100.
    IF s LT 0 THEN sum-list.summe[i] = STRING(s, "->>9.99"). 
    ELSE sum-list.summe[i] = STRING(s, ">>9.99").
  END. 
  ...
  DO i = 1 TO 21: 
    anz = 0. 
    IF tmp-list[i] LT 0 THEN anz = tmp-list[i]. 
    IF anz LT 0 THEN sum-list.summe[i] = STRING(- anz, "->>>9"). 
    ELSE sum-list.summe[i] = STRING(0, "->>>9"). 
  END. 
  FOR EACH room-list: 
    DO i = 1 TO 21: 
...
DEFINE VARIABLE datum AS DATE NO-UNDO. 
  ...
  DO: 
    FOR EACH zimkateg WHERE zimkateg.verfuegbarkeit = YES NO-LOCK: 
      haupt = YES. 
      DO i = 1 TO anz-setup: 
        /*FIND FIRST paramtext WHERE paramtext.txtnr = (i + 9200) NO-LOCK NO-ERROR. */
          FIND FIRST paramtext WHERE paramtext.txtnr = (isetup-array[i] + 9200) NO-LOCK NO-ERROR. 
