...
IF AVAILABLE t-gl-jouhdr THEN
DO:
  RUN disp-it2.
END. 
ELSE 
DO:
  IF sorttype = 1 THEN DO:
    RUN display-it1.
    FIND FIRST t-gl-jouhdr NO-ERROR.
    IF AVAILABLE t-gl-jouhdr THEN RUN disp-it3.
  END.
END.
...
/*add bernatd DA71D9 2025*/
PROCEDURE display-it1:
  FOR EACH gl-jhdrhis
      WHERE gl-jhdrhis.activeflag = sorttype        
      AND gl-jhdrhis.datum GE from-date
      AND gl-jhdrhis.datum LE to-date NO-LOCK
      BY gl-jhdrhis.datum BY gl-jhdrhis.refno BY gl-jhdrhis.bezeich:
      CREATE t-gl-jouhdr.
      BUFFER-COPY gl-jhdrhis TO t-gl-jouhdr.
  END.
END.
...
PROCEDURE disp-it3:
  FOR EACH note-list: 
      DELETE note-list. 
  END. 
  FOR EACH gl-jourhis WHERE gl-jourhis.jnr = t-gl-jouhdr.jnr NO-LOCK: 
      CREATE note-list. 
