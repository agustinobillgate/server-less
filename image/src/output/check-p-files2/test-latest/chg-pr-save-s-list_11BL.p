...
DEF INPUT PARAMETER TABLE FOR t-guestbook.
...
FOR EACH t-guestbook WHERE t-guestbook.reserve-char[1] EQ "$pr" 
    AND t-guestbook.reserve-char[2] EQ docu-nr:
    FIND FIRST guestbook WHERE guestbook.gastnr EQ t-guestbook.gastnr
        AND guestbook.reserve-char[1] EQ t-guestbook.reserve-char[1]
        AND guestbook.reserve-char[2] EQ t-guestbook.reserve-char[2] NO-ERROR.
    IF AVAILABLE guestbook THEN
    DO:
        BUFFER-COPY t-guestbook TO guestbook.
...
ASSIGN 
    ...
    res-history.aenderung = "Change Supplier name from: " + CHR(10) + CHR(10) +
        STRING(t-l-order.bestellart) + "*** Changed to:" + CHR(10) + CHR(10) +
        STRING(l-order.bestellart).
