...
DEF TEMP-TABLE t-l-order LIKE l-order
    ...
    FIELD masseinheit   AS CHARACTER  /* Dzikri 87227D */
    FIELD traubensorte  AS CHARACTER  /* Dzikri 87227D */
.
...
DO WHILE AVAILABLE l-order:
    ...
    ASSIGN 
        ...
        /* Dzikri 87227D : Add unit */
        t-l-order.masseinheit   = l-artikel.masseinheit
        t-l-order.traubensorte  = l-artikel.traubensorte
        /* Dzikri 87227D : Add unit - END */
        .
    ASSIGN
        t-l-order.curr-disc = INTEGER(SUBSTR(l-order.quality,1,2)) * 100 
                            + INTEGER(SUBSTR(l-order.quality,4,2))
        t-l-order.curr-vat = INTEGER(SUBSTR(l-order.quality,7,2)) * 100 
                            + INTEGER(SUBSTR(l-order.quality,10,2)).
    IF length(t-l-order.quality) GE 17 THEN 
        t-l-order.curr-disc2 = INTEGER(SUBSTR(l-order.quality,13,2)) * 100 
            + INTEGER(SUBSTR(l-order.quality,16,2)). 

    IF t-l-order.lief-einheit GT 1 AND t-l-order.geliefert GT 0 THEN
    DO:
        t-l-order.angebot-lief[1] = t-l-order.geliefert MOD t-l-order.lief-einheit.
        t-l-order.geliefert = (t-l-order.geliefert - t-l-order.angebot-lief [1]) / t-l-order.lief-einheit.
    END.
    ...
    IF AVAILABLE queasy THEN DO:
        ASSIGN t-l-order.add-vat = l-order.warenwert * (queasy.deci1 / 100)
