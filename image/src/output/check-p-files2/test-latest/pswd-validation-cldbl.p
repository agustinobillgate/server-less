...
DEF VARIABLE tot-lower-char AS INT.
DEF VARIABLE tot-upper-char AS INT.
...
END.
*/
/*FDL Feb 05, 2025: AC960B New Method*/
DO curr-i = 1 TO LENGTH(inp-pswd):
    IF SUBSTR(inp-pswd, curr-i, 1) GE "0" 
        AND SUBSTR(inp-pswd, curr-i, 1) LE "9" THEN
    DO:
        tot-digit = tot-digit + 1.
    END.
    ELSE IF ASC(SUBSTR(inp-pswd, curr-i, 1)) GE ASC("a") 
      AND ASC(SUBSTR(inp-pswd, curr-i, 1)) LE ASC("z") THEN
    DO:
        tot-lower-char = tot-lower-char + 1.
    END.
    ELSE IF ASC(SUBSTR(inp-pswd, curr-i, 1)) GE ASC("A") 
      AND ASC(SUBSTR(inp-pswd, curr-i, 1)) LE ASC("Z") THEN
    DO:
        tot-upper-char = tot-upper-char + 1.
    END.
    ELSE tot-special = tot-special + 1. 
END.
IF tot-digit LT 2 OR tot-special EQ 0 OR tot-upper-char EQ 0 OR tot-lower-char EQ 0 THEN
DO:
    pswd-level = -2.
    RETURN.
END.
...
DEFINE INPUT PARAMETER in-passwd       AS CHAR CASE-SENSITIVE NO-UNDO.        /* Rulita 230125 | Fixing serverless issue git 339 chg input param pass to in-passwd */
...
DEFINE VARIABLE intScore    AS INTEGER INIT 0 NO-UNDO.
  ...
  longer = LENGTH(in-passwd).                           /* Rulita 230125 | Fixing serverless issue git 339 chg input param pass to in-passwd */
...
/*total upper case  and total lower case */
  DO i = 1 TO longer:
     DO index1 = 1 TO 26:
        pointer = SUBSTRING(in-passwd, i, 1).           /* Rulita 230125 | Fixing serverless issue git 339 chg input param pass to in-passwd */
