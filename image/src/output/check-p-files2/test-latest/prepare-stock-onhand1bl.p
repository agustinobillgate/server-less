...
FOR EACH str-list:
    CREATE soh-list.
    ASSIGN             
       soh-list.store      = INTEGER(SUBSTR(str-list.STR, 1, 2))
       soh-list.bezeich    = str-list.l-bezeich
       soh-list.from-date  = SUBSTR(str-list.STR, 3, 8)
       soh-list.init-stock = DECIMAL(SUBSTR(str-list.STR, 11, 13))
       soh-list.init-value = DECIMAL(SUBSTR(str-list.STR, 24, 14)) 
       soh-list.in-qty     = DECIMAL(SUBSTR(str-list.STR, 38, 13)) 
       soh-list.in-amount  = DECIMAL(SUBSTR(str-list.STR, 51, 14))        
       soh-list.out-qty    = DECIMAL(SUBSTR(str-list.STR, 65, 13))         
       soh-list.out-amount = DECIMAL(SUBSTR(str-list.STR, 78, 14))        
       soh-list.adjustment = DECIMAL(SUBSTR(str-list.STR, 92, 13))
       soh-list.end-qty    = DECIMAL(SUBSTR(str-list.STR, 105, 13))
       soh-list.end-value  = DECIMAL(SUBSTR(str-list.STR, 118, 14))
       .
END.
