...
DEFINE BUFFER p-list FOR s-list.
DEFINE BUFFER t-list FOR s-list.
FOR EACH s-list WHERE s-list.pos EQ 0:
    FIND FIRST p-list WHERE p-list.pos GT 0 AND p-list.docu-nr EQ s-list.docu-nr NO-ERROR.
    IF NOT AVAILABLE p-list THEN
    DELETE s-list.   
    FIND FIRST t-list WHERE t-list.pos GT 0 AND t-list.docu-nr EQ s-list.docu-nr 
        AND t-list.loeschflag NE sorttype NO-ERROR.
    IF AVAILABLE t-list THEN DELETE s-list.
END.

IF deptno = 0 THEN
DO:
    FOR EACH s-list WHERE NOT ((s-list.artnr = 0 
        AND s-list.loeschflag EQ sorttype) OR (s-list.artnr GT 0 AND s-list.loeschflag EQ sorttype)) 
        BY s-list.docu-nr BY s-list.pos:
          DELETE s-list.
    END.
END.
ELSE
DO: 
    FOR EACH s-list WHERE NOT (((s-list.artnr = 0 
        AND s-list.loeschflag EQ sorttype) OR (s-list.artnr GT 0 AND s-list.loeschflag EQ sorttype)) 
        AND s-list.deptnr = deptno) BY s-list.docu-nr BY s-list.pos:
          DELETE s-list.
    END.
END.
