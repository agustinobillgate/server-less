...
/* Malik Serverless 550 */
DEFINE VARIABLE tmp-date AS DATE. 
tmp-date =  DATE(month(close-date), 1, year(close-date)).
tmp-date = tmp-date - 1.
/* END Malik */
  sales = 0. 
  cost = 0. 
  ...
  IF to-date LE tmp-date THEN /* Malik Serverless 550 DATE(month(close-date), 1, year(close-date)) - 1 ->  */
    act-flag = 1. 
  ...
  DO: 
    FOR EACH gl-department WHERE gl-department.nr = from-depart NO-LOCK: 
      ...
      tot-diff = 0. 
      CREATE summary-list.
      /* Rd, #756, 27Mar25, gl-depart -> gl-department */
      summary-list.bezeich = STRING(gl-department.nr, ">>9") + " - " 
        + SUBSTR(gl-department.bezeich, 1, 32). 
      FOR EACH gl-acct WHERE gl-acct.deptnr = gl-department.nr 
        NO-LOCK BY gl-acct.fibukonto: 
        t-debit = 0. 
        t-credit = 0. 
        p-bal = 0. 
        ...
        DO: 
          ...
          .
          /* Rd, #756, 27Mar25, gl-acct.acct-type -> gl-acct.acc-type  */
          IF gl-acct.acc-type = 1 OR gl-acct.acc-type = 4 THEN 
            diff = - t-debit + t-credit. 
