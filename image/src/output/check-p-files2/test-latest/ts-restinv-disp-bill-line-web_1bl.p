DEF TEMP-TABLE t-h-bill-line LIKE h-bill-line  
    ...
    FIELD kds-stat      AS INTEGER INIT -1
    FIELD kds-stat-str  AS CHAR
    FIELD posting-id    AS INT
    FIELD t-time          AS CHAR. /*add bernatd FA0A2F 2025 */
...
DEF INPUT PARAMETER hbill-recid     AS INT.
...
DEF OUTPUT PARAMETER amount-bill AS DECIMAL NO-UNDO.
DEF OUTPUT PARAMETER depo-flag AS LOGICAL.
DEF OUTPUT PARAMETER rsvtable-text AS CHARACTER.
/*
DEF VAR double-currency AS LOGICAL INIT NO.  
DEF VAR rechnr          AS INT     INIT 7199.  
DEF VAR curr-dept       AS INT     INIT 1.  
DEF VAR avail-t-h-bill  AS LOGICAL INIT YES. 
DEF VAR amount-bill     AS DECIMAL NO-UNDO.
*/
...
FOR EACH h-artikel WHERE h-artikel.departement = curr-dept
    ...
    OR h-artikel.artart = 12 OR h-artikel.artart = 5) NO-LOCK:
    CREATE t-h-artikel.
...
FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = rechnr  
    ...
    ASSIGN t-h-bill-line.rec-id = RECID(h-bill-line)
           t-h-bill-line.t-time = STRING(h-bill-line.zeit, "HH:MM:SS").  /*add bernatd FA0A2F 2025 */
    
    /*start bernatd 54A013*/
    FIND FIRST h-journal where h-journal.rechnr EQ vhp.h-bill-line.rechnr
        AND h-journal.zeit EQ vhp.h-bill-line.zeit 
        AND h-journal.schankbuch EQ INT(RECID(vhp.h-bill-line)) NO-LOCK NO-ERROR.
    IF AVAILABLE h-journal THEN 
    DO:
        ASSIGN
        t-h-bill-line.posting-id = h-journal.kellner-nr.  
    END.
    /*end bernatd 54A013*/
    RUN show-submenu.  
    RUN ts-restinv-disp-requestbl.p(RECID(h-bill-line), OUTPUT request-str).  
    ASSIGN t-h-bill-line.request-str = request-str
           amount-bill = amount-bill + vhp.h-bill-line.betrag.  
    ...
    /*FDL August 22, 2023 => Get Status KDS
    FIND FIRST queasy WHERE queasy.number3 EQ INT(RECID(h-bill-line)) NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN
    DO:
        t-h-bill-line.kds-stat = INT(queasy.char3).
        IF t-h-bill-line.kds-stat EQ 0 THEN t-h-bill-line.kds-stat-str = "NEW".
        ELSE IF t-h-bill-line.kds-stat EQ 1 THEN t-h-bill-line.kds-stat-str = "COOKING".
        ELSE IF t-h-bill-line.kds-stat EQ 2 THEN t-h-bill-line.kds-stat-str = "DONE".
    END.*/
END.  
ELSE   
FOR EACH vhp.h-bill-line WHERE vhp.h-bill-line.rechnr = rechnr  
    ...
    BUFFER-COPY h-bill-line TO t-h-bill-line.
    ASSIGN 
    t-h-bill-line.rec-id = RECID(h-bill-line)
    t-h-bill-line.t-time = STRING(h-bill-line.zeit, "HH:MM:SS"). /*add bernatd FA0A2F 2025 */
    ...
    CREATE ordered-item.
    ASSIGN ordered-item.dept    = t-h-bill-line.departement
           ordered-item.artnr   = t-h-bill-line.artnr    
           ordered-item.rec-id  = t-h-bill-line.rec-id   
           ordered-item.qty     = t-h-bill-line.anzahl   
           ordered-item.epreis  = t-h-bill-line.epreis   
...
/*FD Oct 25, 2022 => Feature Deposit Resto*/
...
DO:
    FOR EACH h-bill-line WHERE h-bill-line.rechnr EQ rechnr  
        ...
        BY vhp.h-bill-line.bill-datum DESC BY vhp.h-bill-line.zeit DESC:

        IF h-artikel.endkum EQ wgrpgen.eknr THEN
        DO:
            depo-flag = YES.
            LEAVE.
        END.
    END.
    /* Comment not used, refund from FO
    /*FD Nov 30, 2022 => Feature Deposit Resto*/
    FIND FIRST queasy WHERE queasy.KEY EQ 251 AND queasy.number1 EQ hbill-recid NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN rsvtable-text = queasy.char1.
    */
END.
...
FOR EACH ordered-item:
    sub-tot   = sub-tot + (ordered-item.epreis * ordered-item.qty).
...
ASSIGN 
    ...
    summary-bill.gt-not-round   = grand-tot
    .
