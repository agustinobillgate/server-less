DEFINE TEMP-TABLE age-list 
    ...
    FIELD bank-name       LIKE l-lieferant.bank
    FIELD bank-acct       LIKE l-lieferant.kontonr
    FIELD fibu            AS CHAR
    FIELD remark          AS CHAR
    FIELD amount          AS CHAR
    FIELD is-manual       AS INTEGER /* Dzikri E6B3EC - flag manual AP */
    .
...
DEFINE VARIABLE p-986       AS CHAR.
DEFINE VARIABLE p-395       AS CHAR.


/*ragung add check coa in param and acct-type = 4 di munculkan*/
FIND FIRST htparam WHERE htparam.paramnr EQ 986 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN ASSIGN p-986 = htparam.fchar.

FIND FIRST htparam WHERE htparam.paramnr = 395 NO-LOCK NO-ERROR.
IF AVAILABLE htparam THEN ASSIGN p-395 = htparam.fchar.
      ...
      FOR EACH l-kredit WHERE 
        ...
        DO: 
          ...
          DO i = 1 TO LENGTH(l-lieferant.bank):
              ...
              LEAVE.
          END.
          age-list.bank-name = SUBSTR(l-lieferant.bank,1, i - 1).
          age-list.bank-acct = l-lieferant.kontonr.
        ...
        DO:
          FOR EACH gl-journal WHERE gl-journal.jnr = gl-jouhdr.jnr NO-LOCK,
              ...
              AND (gl-acct.acc-type = 2  OR gl-acct.acc-type = 3
                   OR gl-acct.acc-type = 4 OR gl-acct.acc-type = 5):
              ...
              /*agung */
              IF gl-journal.fibukonto EQ p-986 OR gl-journal.fibukonto EQ p-395 THEN .
              ELSE
                ASSIGN 
                  age-list.fibu   = age-list.fibu + ";" + gl-journal.fibukonto 
                  age-list.remark = age-list.remark + ";" + gl-acct.bezeich 
                  age-list.amount = age-list.amount + ";" + STRING(age-list.debt2).
          END.
        END.
        age-list.is-manual = l-kredit.steuercode. /* Dzikri E6B3EC - flag manual AP */
      END.  
      art-selected = 1. 
    END. 
END.     
