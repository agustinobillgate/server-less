...
DEFINE BUFFER buf-q FOR queasy.

IF icase = 1 THEN RUN read-comment.
ELSE IF icase = 2 THEN RUN update-comment.

PROCEDURE read-comment:
  FIND FIRST res-line WHERE res-line.resnr = resno AND res-line.reslinnr = reslinno NO-LOCK NO-ERROR. /* Malik Serverless : NO-LOCK -> NO-LOCK NO-ERROR */ 
  ...
  /*ITA*/   
   DO loopi = 1 TO NUM-ENTRIES(res-line.zimmer-wunsch,";") - 1:
    ...
    /*FDL August 23, 2023 => Ticket F9ADAE*/
    FIND FIRST buf-q WHERE buf-q.KEY EQ 267
        AND buf-q.number1 EQ resno
        AND buf-q.number2 EQ reslinno NO-LOCK NO-ERROR.
    IF AVAILABLE buf-q THEN
    DO:
        web-com = web-com + buf-q.char1.
    END.    
END.

PROCEDURE update-comment:
  FIND FIRST res-line WHERE res-line.resnr = resno AND res-line.reslinnr = reslinno NO-LOCK NO-ERROR. /* Malik Serverless : EXCLUSIVE-LOCK -> NO-LOCK NO-ERROR */
  IF NOT AVAILABLE res-line THEN RETURN. /* Malik Serverless */
  FIND CURRENT res-line EXCLUSIVE-LOCK. /* Malik Serverless */
  ...
  FIND FIRST buf-q WHERE buf-q.KEY EQ 267
      AND buf-q.number1 EQ resno
      AND buf-q.number2 EQ reslinno NO-ERROR.
  IF NOT AVAILABLE buf-q THEN
  DO:
      MESSAGE 'masuk'
      ...
      ASSIGN
          buf-q.number1 = resno
          buf-q.number2 = reslinno
          buf-q.char1   = web-com
      .
  END.
  ELSE
  DO:
      FIND CURRENT buf-q EXCLUSIVE-LOCK.
      buf-q.char1 = web-com.
      FIND CURRENT buf-q NO-LOCK.
  END.
  ...
  RELEASE res-line. /* Malik Serverless */
  RELEASE reservation. /* Malik Serverless */
  RELEASE guest. /* Malik Serverless */
END.
