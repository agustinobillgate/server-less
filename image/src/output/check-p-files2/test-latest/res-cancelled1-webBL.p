...
DEFINE TEMP-TABLE res-cancelled-list
    ...
    FIELD firma             AS CHAR
    FIELD cancelled-time    LIKE res-line.cancelled-id  /* Dzikri 09BE32 - add time field */
.
...
/* Naufal Afthar - 7521F6 */
DEFINE VARIABLE ori-resline-remark AS CHARACTER NO-UNDO.
DEFINE VARIABLE ori-rsv-remark     AS CHARACTER NO-UNDO.
/* Naufal Afthar - 7521F6 */
...
FOR EACH res-cancelled-list:
    /* Naufal Afthar - 7521F6 */
    ori-resline-remark = res-cancelled-list.bemerk.

    IF ori-resline-remark EQ ? THEN ori-resline-remark = "".
    RUN rmv-ascii-null (ori-resline-remark, OUTPUT resline-remark).
    ...
    /* Naufal Afthar - 7521F6 */
    ori-rsv-remark = res-cancelled-list.rsv-bemerk.

    IF ori-rsv-remark EQ ? THEN ori-rsv-remark = "".
    RUN rmv-ascii-null (ori-rsv-remark, OUTPUT rsv-remark).
            ...
            DO:
                ...
                ELSE DO:
                    FOR EACH res-line WHERE resstatus = 9 
                       ...
                       FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK, 
                       FIRST zimkateg WHERE zimkateg.zikatnr = res-line.zikatnr AND zimkateg.kurzbez = curr-rmType NO-LOCK,
                       FIRST guest WHERE guest.gastnr = res-line.gastnr AND guest.karteityp = kart NO-LOCK BY res-line.ankunft BY res-line.name:
                       RUN assign-it.
                    END.
            ...
            DO:
                IF res-status = 1 THEN DO:
                    FOR EACH res-line WHERE resstatus = 9 
                       ...
                       FIRST reservation WHERE reservation.resnr = res-line.resnr NO-LOCK, 
                       FIRST zimkateg WHERE zimkateg.zikatnr = res-line.zikatnr AND zimkateg.kurzbez = curr-rmType NO-LOCK,
                       FIRST guest WHERE guest.gastnr = res-line.gastnr AND guest.karteityp = kart NO-LOCK BY res-line.ankunft BY res-line.name: /* Malik Serverless : BY res-line.ankunft BY res-line.name -> BY res-line.ankunft BY res-line.name */
                        RUN assign-it.
                    END.
...
PROCEDURE assign-it:
    ...
    /* Dzikri 09BE32 - separate cancelled id and datetime value */
    IF NUM-ENTRIES(res-line.cancelled-id,";") GE 2 THEN
    DO:
        ASSIGN
                res-cancelled-list.cancelled-id      = ENTRY(1,res-line.cancelled-id,";")   
                res-cancelled-list.cancelled-time    = ENTRY(2,ENTRY(2,res-line.cancelled-id,";"),"-") 
        .
    END.
    /* Dzikri 09BE32 - END */
...
/* Naufal Afthar - 7521F6 Filter null ascii character */
PROCEDURE rmv-ascii-null:
    DEFINE INPUT PARAMETER remark AS CHARACTER.
    DEFINE OUTPUT PARAMETER new-remark AS CHARACTER.
    DEFINE VARIABLE i AS INTEGER.

    DO i = 1 TO LENGTH(remark):
        IF ASC(SUBSTRING(remark,i,1)) NE 0 THEN
            new-remark = new-remark + SUBSTRING(remark,i,1).
    END.

    new-remark = TRIM(remark).
END.
