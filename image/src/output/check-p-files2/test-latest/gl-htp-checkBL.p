...
PROCEDURE check-closing-date:
  ...
  DO:
    ...
    ASSIGN 
        ...
        date1 = DATE(MONTH(close-date), 1, YEAR(close-date))
        date2 = date1 - 1.
    /*IF dateval LT (DATE(MONTH(close-date), 1, YEAR(close-date)) - 1) THEN FT serverless*/
    IF dateval LT date2 THEN 
    DO:
      do-it = NO.
  ...
  DO:
      ASSIGN
          m1 = MONTH(dateval) + 1
          date1 = DATE(m1, 1, YEAR(dateval))
          date2 = date1 - 1.
      IF MONTH(dateval) = 12 THEN d2 = DATE(12, 31, YEAR(dateval)).
      ELSE /*d2 = DATE(MONTH(dateval) + 1, 1, YEAR(dateval)) - 1 FT serverless*/
          d2 = date2.
  ...
  DO:
      ...
      IF mm = 12 THEN
          d2 = DATE(12, 31, yy).
      ELSE /*d2 = DATE(mm + 1, 1, yy)  - 1.FT serverless*/
          d2 = date2.
