...
DEFINE INPUT  PARAMETER search1   AS CHAR NO-UNDO.
...
IF search1 = "" THEN
DO:
    FOR EACH guest WHERE guest.name GE search1  
        AND guest.gastnr GT 0 AND phonetik3 = user-init 
...
ELSE
DO:
    FOR EACH guest WHERE guest.name MATCHES("*" + search1 + "*")  
        AND guest.gastnr GT 0 AND phonetik3 = user-init 
        AND guest.karteityp = sorttype NO-LOCK BY guest.NAME:
        CREATE gast.
        ASSIGN
