...
/* Rd, #757, 27Mar25, function lastday */
DEFINE VARIABLE tmp-lastday AS DATE NO-UNDO.

FUNCTION lastday RETURNS DATE ( INPUT d AS DATE ): /*Alder - Serverless - Issue 757*/ /*lastDay -> lastday*/
    /* Malik Serverless */
    DEF VAR tmp-date AS DATE.
    DEF VAR tot-date AS DATE. 
    tmp-date = ADD-INTERVAL( DATE( MONTH( d ), 1, YEAR( d )), 1, "month" ).
    tot-date = tmp-date - 1.
    RETURN tot-date.
    /* END Malik */
END FUNCTION.
...
/* Rd, #757, 27Mar25, function lastday  */
tmp-lastday = lastday(t-from-date). /*Alder - Serverless - Issue 757*/ /*lastDay -> lastday*/
FIND FIRST gl-jouhdr WHERE gl-jouhdr.datum LE tmp-lastday NO-LOCK NO-ERROR.
