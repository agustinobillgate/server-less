  ...
  FOR EACH l-bestand WHERE l-bestand.lager-nr = 0 NO-LOCK, 
    ...
    FOR EACH l-op WHERE l-op.datum GE from-date AND l-op.datum LE to-date 
      ...
      /* BLY - C38BE6 */
        ...
        END.  
      END. 
      ELSE IF l-op.op-art LE 4 THEN
      DO:
        FIND FIRST l-ophdr WHERE l-ophdr.lscheinnr = l-op.lscheinnr 
          AND l-ophdr.op-typ = "STT" AND l-ophdr.fibukonto NE "" NO-LOCK NO-ERROR. 
        FIND FIRST gl-acct WHERE gl-acct.fibukonto = l-op.stornogrund NO-LOCK NO-ERROR.
        IF AVAILABLE l-ophdr AND AVAILABLE gl-acct THEN
        DO:
