...
DEFINE TEMP-TABLE output-list2 
  ...
  /* FIELD refNo       AS CHAR    FORMAT "x(16)" LABEL "RefNo"  Rd, #745, 26Mar2025 , refNo -> refno */
  FIELD refno       AS CHAR    FORMAT "x(16)" LABEL "RefNo" /* Rd, #745, 26Mar2025 , refno -> refno */
...
DEFINE INPUT PARAMETER argt-code        AS CHARACTER.
...
DEFINE TEMP-TABLE to-list 
  ...
  /* FIELD refNo            AS CHAR Rd, #745, 26Mar2025 , refNo -> refno */
  FIELD refno            AS CHAR
...
DEFINE VARIABLE argtcode as character.
...
/****************** MAIN LOGIC ******************/
/*add argtcode in t-list */
RUN create-forecast-history_2bl.p(fr-date, to-date, excl-comp,vhp-limited, scin, OUTPUT TABLE t-list).
CASE op-type:
    WHEN 0 THEN
    DO:
        RUN query-to-list.
...
PROCEDURE create-to-list2:
    ...
    /* add by damen 15/05/23  DCB8A3*/
    ELSE IF disptype eq 5 then
        find first to-list where to-list.argtcode matches t-list.argtcode
            and to-list.curr = t-list.currency no-lock no-error.
    /* end of add */
    ...
    DO: 
        ...
        ASSIGN to-list.name       = t-list.rsv-name
               ...
               to-list.argtcode    = t-list.argtcode
            .
        ...
        ELSE IF disptype eq 5 then
        do:
            if t-list.argtcode matches argtcode then
            do:
                find first arrangement where arrangement.arrangement matches t-list.argtcode no-error.
                if available arrangement then assign to-list.name = arrangement.argt-bez.
            end.
...
PROCEDURE create-browse:
    ...
    DO:
        ...
        IF disptype eq 5 then
        do:
            FOR EACH to-list where to-list.argtcode matches argtcode no-lock:            
                CREATE output-list1.
                ASSIGN 
        ...
        output-list1.exp-rev     = FILL("-", 19).
        output-list1.curr        = FILL("-", 4).
        output-list1.loc-curr    = FILL("-", 19).
        output-list1.lodg        = FILL("-", 18).
        output-list1.bfast       = FILL("-", 17).
        output-list1.lunch       = FILL("-", 17).
        output-list1.dinner      = FILL("-", 17).
        output-list1.oth-rev     = FILL("-", 17).
    ...
    DO:
        ...
        output-list2.exp-rev        = FILL("-", 19).
        output-list2.curr           = FILL("-", 4).
        output-list2.loc-curr       = FILL("-", 19).
        output-list2.lodg           = FILL("-", 18).
