...
DO:
    ...
    RUN to-date1.                               /* Rulita 050225 | Fixing serverless issue change procedure name from to-date to to-date1 git 106 */
END.
ELSE
DO:
    /*f-date    = from-date.*/
...
/******************************* PROCEDURE *********************************/
/* Rulita 050225 | Fixing serverless issue change procedure name from to-date to to-date1 git 106 */
PROCEDURE to-date1:
    FOR EACH cl-list:
        DELETE cl-list.
    END.
    ...
    /* Rulita 050225 | Fixing serverless issue git 106 */
    /* ASSIGN
        hotela = (tocc-rm  / tsale) * 100
        hotelb = (tocc-rm1 / tsale1) * 100
        hotelc = (tocc-rm2 / tsale2) * 100. */

    IF tsale NE ? AND tsale NE 0 THEN ASSIGN hotela = (tocc-rm  / tsale) * 100.
    IF tsale1 NE ? AND tsale1 NE 0 THEN ASSIGN hotelb = (tocc-rm1 / tsale1) * 100.
    IF tsale2 NE ? AND tsale2 NE 0 THEN ASSIGN hotelc = (tocc-rm2 / tsale2) * 100.
    /* End Rulita */
