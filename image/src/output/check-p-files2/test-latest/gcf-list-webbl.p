...
DEFINE VARIABLE priority-flag AS INT.
/*
FOR EACH guest WHERE guest.karteityp EQ sorttype NO-LOCK:
    total-record = total-record + 1.
END.
*/
/*dody 24032025 serverless
total-record = 0.
FIND FIRST guest WHERE guest.karteityp EQ sorttype NO-LOCK NO-ERROR.
DO WHILE AVAILABLE guest:
    total-record = total-record + 1.
    FIND NEXT guest WHERE guest.karteityp EQ sorttype NO-LOCK NO-ERROR.
END.
*/
total-record = 0.
CASE case-type:
  ...
  DO:
    ...
    FOR EACH guest WHERE (guest.gastnr > 0 
      ...
      FIND FIRST akt-kont NO-LOCK NO-ERROR.
      counter = counter + 1.
      ...
      ASSIGN
        ...
        t-guest.namekontakt = guest.namekontakt
      .
      /*bernatd*/
      ...
      DO:
        t-guest.namekontakt = akt-kont.name + ", "   
...
FOR EACH t-guest :
  ...
  DO:
    ...
    END.
  END.
END. dody 24032025 comment for change query, serverless
*/

FOR EACH t-guest :
    FIND FIRST mc-guest WHERE mc-guest.gastnr = t-guest.gastnr AND mc-guest.activeflag = YES NO-LOCK NO-ERROR.
    ASSIGN t-guest.mc-flag = AVAILABLE mc-guest.

    priority-flag = 0.
    FOR EACH guestseg WHERE guestseg.gastnr = t-guest.gastnr,FIRST segment WHERE segment.segmentcode = guestseg.segmentcode:
        IF priority-flag LT 3 AND
            (guestseg.segmentcode = vipnr1 OR   
            guestseg.segmentcode = vipnr2 OR   
            guestseg.segmentcode = vipnr3 OR   
            guestseg.segmentcode = vipnr4 OR   
            guestseg.segmentcode = vipnr5 OR   
            ...
            guestseg.segmentcode = vipnr7 OR   
            guestseg.segmentcode = vipnr8 OR   
            guestseg.segmentcode = vipnr9) THEN
        DO:
            priority-flag = 3.
            t-guest.alertbox = YES.
            t-guest.bezeich = ENTRY(1, segment.bezeich, "$$0").      
        END.
        ELSE IF priority-flag LT 2 AND segment.betriebsnr = 4 THEN
        DO:
            priority-flag = 2.
            t-guest.alertbox = YES.
            t-guest.bezeich = ENTRY(1, segment.bezeich, "$$0").      
        END.
        ELSE IF priority-flag LT 1 THEN
        DO:
            priority-flag = 1.
            t-guest.bezeich = ENTRY(1, segment.bezeich, "$$0").      
        END.
...
PROCEDURE get-vipnr: 
  ...
  IF htparam.finteger NE 0 THEN vipnr10 = htparam.finteger.
END. 
