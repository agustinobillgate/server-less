...
DEFINE BUFFER buf-arrangement     FOR arrangement. /* malik Serverless : arr -> buf-arrangement */
...
PROCEDURE check-go: 
  ...
  DEFINE VARIABLE tmp-date        AS DATE. /* Malik Serverless 163 */
  ...
  IF NOT AVAILABLE res-line THEN RETURN. /* Malik Serverless 163 */

  /*FDL April 04, 2025: 6DCD86*/
  IF res-mode EQ "inhouse" AND AVAILABLE res-line THEN
  DO:
      IF res-line.resstatus EQ 8 THEN
      DO:
          msg-str = translateExtended ("Guest in this reservation already C/O. Update not possible.",lvCAREA,"").
          error-number = 51.
          RETURN.
      END.
  END.
  ...
  */
  ...
  /* Dzikri 1A505D - seperate validation
  IF reslin-list.active-flag LE 1 THEN /*FDL Ticket 34FAAE - CHG = to LE 1*/
  DO:
  ...
  */
  ...
  IF reslin-list.active-flag EQ 0 THEN /*FDL Ticket 34FAAE - CHG = to LE 1*/
  DO:
    FIND FIRST reslin-queasy WHERE reslin-queasy.key = "arrangement" 
      AND reslin-queasy.resnr = reslin-list.resnr 
      AND reslin-queasy.reslinnr = reslin-list.reslinnr 
  ...
  IF reslin-list.active-flag EQ 1 THEN /*FDL Ticket 34FAAE - CHG = to LE 1*/
  DO:
    FIND FIRST reslin-queasy WHERE reslin-queasy.key = "arrangement" 
      AND reslin-queasy.resnr = reslin-list.resnr 
      AND reslin-queasy.reslinnr = reslin-list.reslinnr 
      AND reslin-queasy.date1 LE ci-date 
      AND reslin-queasy.date2 GE ci-date NO-LOCK NO-ERROR.
    IF AVAILABLE reslin-queasy AND reslin-queasy.char1 NE "" 
       AND reslin-queasy.char1 NE reslin-list.arrangement THEN
    DO:
      ASSIGN
  ...
  /* check fixed-rate */
  tmp-date = reslin-list.abreise - 1. /* Malik Serverless 163 */
  ...
  DO:
    ...
    DO:
      /* DQ 9C7C08,72B16C - fixrate breakdown error validation */
      /* datum = reslin-list.abreise - 1. */
      DO datum = reslin-list.ankunft TO tmp-date:
        FIND FIRST reslin-queasy WHERE reslin-queasy.key = "arrangement" 
          AND reslin-queasy.resnr = res-line.resnr 
          AND reslin-queasy.reslinnr = res-line.reslinnr 
          AND reslin-queasy.date1 LE datum /* Malik Serverless 163 : AND datum GE reslin-queasy.date1 -> AND reslin-queasy.date1 LE datum */
          AND reslin-queasy.date2 GE datum NO-LOCK NO-ERROR. /* Malik Serverless 163 : AND datum LE reslin-queasy.date2 -> AND reslin-queasy.date2 GE datum */
        ...
        END.
      END.
      /* DQ 9C7C08,72B16C - END */
    END.
  END.
  /* DQ 9C7C08,72B16C - fixrate breakdown error validation after modifying to dayuse */
  IF reslin-list.ankunft EQ reslin-list.abreise THEN 
  DO:
    FIND FIRST reslin-queasy WHERE reslin-queasy.key = "arrangement" 
      AND reslin-queasy.resnr = res-line.resnr 
      AND reslin-queasy.reslinnr = res-line.reslinnr NO-LOCK NO-ERROR. 
...
PROCEDURE assign-zinr:
  ...
  DEFINE VARIABLE tmp-datum AS DATE. /* Malik Serverless 163 */
  ...
  DO:
    ...
    room-blocked = no.
    tmp-datum = abreise - 1. /* Malik Serverless 163 */
    do curr-datum = beg-datum TO tmp-datum: /* Malik Serverless 163 : (abreise - 1) -> tmp-datum */
...
PROCEDURE release-zinr:
  ...
  FIND FIRST rline WHERE rline.resnr = res-line.resnr /* Malik Serverless 163 : resnr -> res-line.resnr */
      AND rline.reslinnr = res-line.reslinnr NO-LOCK NO-ERROR. /* Malik Serverless 163 : reslinnr -> res-line.reslinnr */
  IF AVAILABLE rline AND rline.zinr NE "" THEN
  DO: 
    ...
    DO:
      ...
      DO TRANSACTION:
        ...
        IF AVAILABLE res-line1 THEN 
          ...
          */
        DO:       
          FOR EACH res-line3 WHERE res-line3.resnr = res-line.resnr /* Malik Serverless 163 : resnr -> res-line.resnr */
...
PROCEDURE check-min-maxstay:
  ...
  DEF VARIABLE num-day        AS INT     NO-UNDO. /* Malik Serverless 163 */

  FIND FIRST buf-arrangement WHERE buf-arrangement.arrangement = reslin-list.arrangement NO-LOCK 
   NO-ERROR. 
  IF AVAILABLE buf-arrangement THEN min-stay = buf-arrangement.intervall.
  IF contcode NE "" THEN
  DO:
  ...
  num-day = reslin-list.abreise - reslin-list.ankunft. /* Malik Serverless 163 */
  IF min-stay GT num-day THEN  
  DO:
