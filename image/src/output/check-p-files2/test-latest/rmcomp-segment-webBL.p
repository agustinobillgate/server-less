...
{ supertransbl.i }
DEF VAR lvCAREA AS CHAR INITIAL "rmcomp-segment-web". 

/*************** MAIN LOGIC ***************/
FIND FIRST htparam WHERE htparam.paramnr = 491. 
price-decimal = htparam.finteger.

IF sorttype = 0 OR sorttype = 1 THEN RUN create-umsatz.
ELSE RUN create-umsatz2.
...
PROCEDURE create-umsatz:
    ...
    ASSIGN
      ...
      gty-avrgrate = 0
      . 
    FOR EACH rmcomp-segm-list:
        DELETE rmcomp-segm-list.
    END.
    ...
    DO: 
     ASSIGN
          ...
          fdate = DATE(MONTH(t-date),1,YEAR(t-date)).
    END.           
    ...
    FOR EACH genstat WHERE 
        ...
        DO:
            ...
            IF mi-ftd AND genstat.datum = t-date THEN
            DO: 
                IF incl-comp THEN
                DO:
                    ASSIGN
            ...
            IF NOT mi-ftd AND genstat.datum = to-date THEN
            DO: 
                IF incl-comp THEN
                DO:
                    ASSIGN
            ...
            IF NOT mi-ftd AND /*MONTH(genstat.datum) = mm AND*/ YEAR(genstat.datum) = yy THEN 
            DO:
                IF incl-comp THEN
                DO:
                    IF MONTH(genstat.datum) = mm AND YEAR(genstat.datum) = yy THEN
            ...
            /*IF mi-ftd AND genstat.datum GE fdate AND genstat.datum LE t-date THEN*/
            IF mi-ftd AND MONTH(genstat.datum) = mm AND YEAR(genstat.datum) = yy THEN
            DO:
                IF incl-comp THEN
                    ASSIGN
                        s-list.m-room      = s-list.m-room  + 1
            ...
            IF mi-ftd AND genstat.datum GE f-date AND genstat.datum LE t-date THEN 
            DO:
                IF incl-comp THEN
                DO:
                    ASSIGN 
