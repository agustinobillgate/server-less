...
PROCEDURE read-request-records:
    ...
    DEF VAR sr-remark AS CHARACTER.
    ...
    FOR EACH l-op WHERE l-op.datum = t-datum
        ...
        IF AVAILABLE bediener THEN op-list.fuellflag = bediener.nr.
        IF out-type = 2 THEN
        DO:
    ...
    DO TRANSACTION: 
        ...
        ASSIGN 
            t-l-ophdr.rec-id = RECID(l-ophdr)
            t-l-ophdr.sr-remark = sr-remark. /* Oscar (14/03/25) - 3E510E - show field sr-remark */
    END.
END.
