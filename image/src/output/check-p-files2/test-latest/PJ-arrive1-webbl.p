...
DEFINE TEMP-TABLE cl-list 
  ...
  FIELD night       AS INTEGER /*FT serverless*/
  FIELD phonenum    AS CHAR
  FIELD member-typ  AS CHAR 
 . 
...
DO curr-date = from-date TO to-date:
  IF (curr-date GT ci-date) THEN
  DO:
    RUN create-arrival(curr-date). 
  END. 
  ELSE IF curr-date LT ci-date THEN 
  DO:
    RUN create-arrival1(curr-date). 
  END.
  ...
  FOR EACH res-line WHERE res-line.active-flag LE 1 
    ...
    DO:    
        FIND FIRST cl-list WHERE cl-list.rmno EQ res-line.zinr 
            ...
            AND DEC(cl-list.zipreis) EQ 0 
            AND cl-list.a = 0 AND (cl-list.res-stat = 11 OR cl-list.res-stat = 13) 
            AND cl-list.co LT 1 EXCLUSIVE-LOCK NO-ERROR. /*modified room sharer by Bernatd 2024*/
  ...
  FOR EACH res-line WHERE res-line.active-flag LE 1 
    ...
    DO:    
        FIND FIRST cl-list WHERE cl-list.rmno EQ res-line.zinr 
            ...
            AND DEC(cl-list.zipreis) EQ 0 
            AND cl-list.a = 0 
            AND (cl-list.res-stat = 11 OR cl-list.res-stat = 13)  
            AND cl-list.co LT 1 EXCLUSIVE-LOCK NO-ERROR. /*modified room sharer by Bernatd 2024*/
        IF AVAILABLE cl-list THEN
        DO:
            DELETE cl-list.
...
PROCEDURE add-cllist:
    ...
    DEFINE VARIABLE schar AS CHAR NO-UNDO.
    ...
    IF AVAILABLE setup-list THEN schar = setup-list.CHAR. /*FT serverless*/
    ...
    ASSIGN 
      ...
      /*cl-list.rmcat     = zimkateg.kurzbez + setup-list.char FT serverless*/
      cl-list.rmcat     = zimkateg.kurzbez + schar
    ...
    IF res-line.abreise GT res-line.ankunft THEN
      ASSIGN cl-list.night = res-line.abreise - res-line.ankunft.
    ELSE cl-list.night = 1. /*FT serverless*/
    ...
    IF gmember.telefon NE "" AND gmember.telefon NE ? THEN /*modified by bernatd C2C6F0*/
      /*cl-list.company   = cl-list.company + ";" + gmember.telefon.FT serverless*/
        cl-list.phonenum = gmember.telefon.
    ...
    DO:
        ...
        DO:
           /*cl-list.memberno = mc-guest.cardnum + ";" + mc-types.bezeich.FT serverless*/
            ASSIGN
                cl-list.memberno = mc-guest.cardnum
                cl-list.member-typ = mc-types.bezeich.
        END.
    END.
  ...
  FOR EACH res-line WHERE res-line.active-flag = 1 
    ...
    DO:    
        FIND FIRST cl-list WHERE cl-list.rmno EQ res-line.zinr 
            AND cl-list.resnr EQ res-line.resnr
            AND DATE(cl-list.arrival) EQ res-line.ankunft 
            AND DEC(cl-list.zipreis) EQ 0 AND cl-list.a = 0 
            AND (cl-list.res-stat = 11 OR cl-list.res-stat = 13) 
            AND cl-list.co LT 1 EXCLUSIVE-LOCK NO-ERROR. /*modified room sharer by Bernatd 2024*/
        IF AVAILABLE cl-list THEN
        DO:
  ...
  FOR EACH res-line WHERE res-line.active-flag EQ 0 
    ...
    DO:    
        FIND FIRST cl-list WHERE cl-list.rmno EQ res-line.zinr 
            ...
            AND DEC(cl-list.zipreis) EQ 0 
            AND cl-list.a = 0 
            AND (cl-list.res-stat = 8  OR cl-list.res-stat = 11 OR cl-list.res-stat = 13) 
            AND cl-list.co LT 1 EXCLUSIVE-LOCK NO-ERROR. /*modified room sharer by Bernatd 2024*/
        IF AVAILABLE cl-list THEN
        DO:
            DELETE cl-list.
...
PROCEDURE add-cllist1:
    ...
    FIND FIRST setup-list WHERE setup-list.nr = res-line.setup + 1 NO-ERROR.
    IF AVAILABLE setup-list THEN schar = setup-list.CHAR.
      nr = nr + 1. 
      vip-flag = "". 
      FIND FIRST guestseg WHERE guestseg.gastnr = gmember.gastnr 
        AND (guestseg.segmentcode = vipnr1 OR 
      ...
      IF guest.karteityp NE 0 THEN
        cl-list.company   = guest.name + ", " + guest.vorname1 
          + " " + guest.anrede1 + guest.anredefirma.
      IF gmember.telefon NE "" AND gmember.telefon NE ? THEN /*modifed by bernatd C2C6F0*/
        /*cl-list.company   = cl-list.company + ";" + gmember.telefon.FT serverless*/
        cl-list.phonenum = gmember.telefon.
...
DEFINE VARIABLE tot-tkind  AS INTEGER.
    ...
    FOR EACH cl-list:
        CREATE t-cl-list.
        ASSIGN
            t-cl-list.ci-id = cl-list.ci-id
            t-cl-list.stat-flag = cl-list.stat-flag
            t-cl-list.datum = cl-list.datum
            t-cl-list.flag = cl-list.flag
            t-cl-list.nr = cl-list.nr
            t-cl-list.vip = cl-list.vip
            t-cl-list.gastnr = cl-list.gastnr
            t-cl-list.resnr = cl-list.resnr
            t-cl-list.NAME = t-cl-list.NAME
            t-cl-list.groupname = cl-list.groupname
            t-cl-list.zimmeranz = cl-list.zimmeranz
            t-cl-list.rmno = cl-list.rmno
            t-cl-list.qty = cl-list.qty
            t-cl-list.zipreis = cl-list.zipreis
            t-cl-list.arrival = cl-list.arrival
            t-cl-list.depart = cl-list.depart
            t-cl-list.rmcat = cl-list.rmcat
            t-cl-list.kurzbez = cl-list.kurzbez
            t-cl-list.bezeich = cl-list.bezeich
            t-cl-list.a = cl-list.a
            t-cl-list.c = cl-list.c
            t-cl-list.co = cl-list.co
            t-cl-list.pax = cl-list.pax
            t-cl-list.nat = cl-list.nat
            t-cl-list.nation = cl-list.nation
            t-cl-list.argt = cl-list.argt
            t-cl-list.company = cl-list.company
            t-cl-list.flight = cl-list.flight
            t-cl-list.etd = cl-list.etd
            t-cl-list.stay = cl-list.stay
            t-cl-list.segment = cl-list.segment
            t-cl-list.rate-code = cl-list.rate-code
            t-cl-list.eta = cl-list.eta
            t-cl-list.email = cl-list.email
            t-cl-list.bemerk = cl-list.bemerk
            t-cl-list.bemerk01 = cl-list.bemerk01
            t-cl-list.bemerk02 = cl-list.bemerk02
            t-cl-list.bemerk03 = cl-list.bemerk03
            t-cl-list.bemerk04 = cl-list.bemerk04
            t-cl-list.bemerk05 = cl-list.bemerk05
            t-cl-list.bemerk06 = cl-list.bemerk06
            t-cl-list.bemerk07 = cl-list.bemerk07
            t-cl-list.bemerk08 = cl-list.bemerk08
            t-cl-list.spreq = cl-list.spreq
            t-cl-list.memberno = cl-list.memberno
            t-cl-list.resdate = cl-list.resdate
            t-cl-list.sob = cl-list.sob
            t-cl-list.created-by = cl-list.created-by
            t-cl-list.ci-time = cl-list.ci-time
            t-cl-list.city = cl-list.city
            t-cl-list.res-stat = cl-list.res-stat
            t-cl-list.res-stat-str = cl-list.res-stat-str
            t-cl-list.nation2 = cl-list.nation2
            t-cl-list.birthdate = cl-list.birthdate
            t-cl-list.rsv-comment = cl-list.rsv-comment
            t-cl-list.other-comment = cl-list.other-comment
            t-cl-list.g-comment = cl-list.g-comment
            t-cl-list.zinr-bez = cl-list.zinr-bez
            t-cl-list.flag-guest = cl-list.flag-guest
            t-cl-list.night = cl-list.night
            t-cl-list.phonenum = cl-list.phonenum
            t-cl-list.member-typ = cl-list.member-typ.
        ...
        ELSE
            t-cl-list.company = cl-list.company.FT serverless*/
        ...
        END.
        ELSE t-cl-list.memberno = cl-list.memberno. FT serverless*/
    
        IF t-cl-list.stay GT 1 THEN t-cl-list.repeat-guest = "*".
    
        /*ASSIGN t-cl-list.night = DATE(cl-list.depart) - DATE(cl-list.arrival). FT serverless*/
    END.
