DEFINE TEMP-TABLE str-list 
  ...
  /* Oscar (13/01/24) - DDB12D - add serial number and invoice-date in incoming report */
...
FOR EACH str-list WHERE str-list.description NE "T O T A L" AND str-list.description NE "GRAND TOTAL":
    IF str-list.disc-amount NE 0 THEN str-list.price = str-list.price + str-list.disc-amount.
    /*IF str-list.disc-amount2 NE 0 THEN str-list.price = str-list.price + str-list.disc-amount2.*/
    IF str-list.vat-amount NE 0 THEN str-list.price = str-list.price - str-list.vat-amount.
END.
...
PROCEDURE create-list11: 
  ...
  IF store = 0 THEN 
    ...
    AND l-op.op-art = 1 
    AND l-op.anzahl NE 0 /* Oscar (24/03/2025) - 89F2BD - excluding incoming with qty 0 */
  ...
  RUN create-hislist(INPUT-OUTPUT t-anz, INPUT-OUTPUT t-amt, INPUT-OUTPUT t-amountexcl, 
...
PROCEDURE assign-create-list11:
    ...
    FIND FIRST l-ophdr WHERE l-ophdr.op-typ = "STI" AND l-ophdr.lscheinnr = l-op.lscheinnr AND l-ophdr.datum = l-op.datum NO-LOCK NO-ERROR. 
    ...
    DO: 
      ...
      IF AVAILABLE queasy THEN
          ASSIGN 
            t-amt = t-amt + (l-op.warenwert + (l-op.warenwert * (queasy.deci1 / 100)))
  ...
  DO: 
    ...
    DO:
        ASSIGN 
          amt                 = l-op.warenwert * taxcode-list.taxamount
    ...
    FIND FIRST queasy WHERE queasy.KEY EQ 336 
        ...
        AND queasy.number2 EQ l-op.artnr
        AND DEC(queasy.char2) EQ l-op.einzelpreis
        AND queasy.number1 EQ INT(RECID(l-op)) NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN
    DO:
        ASSIGN
  ...
  DO: 
    ...
    /* Oscar (20/02/25) - A74530 - add AP Voucher Number to show */
    FIND FIRST buff-l-kredit WHERE buff-l-kredit.lscheinnr EQ l-op.lscheinnr NO-LOCK NO-ERROR.
    ...
    FIND FIRST queasy WHERE queasy.KEY EQ 335 AND queasy.char1 EQ l-op.lscheinnr
      AND queasy.number1 = l-op.artnr NO-LOCK NO-ERROR.
    IF AVAILABLE queasy THEN
    DO:
    ...
    ASSIGN
      ...
      str-list.d-unit = l-artikel.traubensorte /*Alder - Serverless - Issue 667*/ 
    ...
    DO:
        ASSIGN 
            str-list.tax-amount = l-op.warenwert * taxcode-list.taxamount
            t-tax               = t-tax   + (l-op.warenwert * taxcode-list.taxamount)
            tot-tax             = tot-tax + (l-op.warenwert * taxcode-list.taxamount) 
...
*/  
  ...
  FOR EACH l-ophis NO-LOCK WHERE 
      ...
      l-ophis.anzahl NE 0 AND /* Oscar (24/03/2025) - 89F2BD - excluding incoming with qty 0 */
      ...
      IF AVAILABLE l-kredit THEN
          str-list.ap-voucher = l-kredit.rechnr.
      ELSE
          str-list.ap-voucher = 0.
...
PROCEDURE convert-fibu: 
  ...
  FIND FIRST htparam WHERE paramnr = 977 NO-LOCK NO-ERROR.
