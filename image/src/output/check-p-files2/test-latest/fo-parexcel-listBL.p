...
DEFINE VARIABLE num-year   AS INTEGER. /* Malik Serverless */  
DEFINE VARIABLE tmp-month   AS DATE. /* Malik Serverless */  
...
DO:   
  ...
  num-year = year(to-date) - 1. /* Malik Serverless */ 
  Ljan1 = DATE(1, 1, num-year). /* Malik Serverless : year(to-date) - 1 -> num-year */   
  ...
  DO:   
    ...
    jan1      = start-date.   
    num-year = year(jan1) - 1. /* Malik Serverless */
    Ljan1     = DATE(month(jan1), day(jan1), num-year). /* Malik Serverless : year(jan1) - 1 -> num-year */
  END.   
   
  DO:   
    Pto-date = from-date - 1.   
    tmp-month = to-date + 1. /* Malik Serverless */
    IF (MONTH(to-date) = MONTH(tmp-month)) THEN  /* Malik Serverless :MONTH(to-date + 1) -> MONTH(tmp-month) */
    DO:  
  ...
  DO:
    num-year = year(to-date) - 1. /* Malik Serverless */
    Lto-date = DATE(month(to-date), day(to-date), num-year). /* Malik Serverless : year(to-date) - 1 -> num-year */
  END.   
  ELSE 
  DO:
    num-year = year(to-date) - 1. /* Malik serverless */
    Lto-date = DATE(month(to-date), 28, num-year). /* Malik Serverless : year(to-date) - 1 -> num-year */   
  END.  
