...
DEFINE INPUT PARAMETER url-pr              AS CHAR    NO-UNDO.
...
FIND FIRST l-artikel WHERE l-artikel.bestellt
    ...
    AND NOT l-artikel.bezeich MATCHES "(Don't Use )*"
    AND NOT l-artikel.bezeich MATCHES "*(Don't Use)"
    AND ((l-artikel.traubensort = " " OR l-artikel.masseinheit = " ") 
        OR (l-artikel.traubensort = ? OR l-artikel.masseinheit = ?)) NO-LOCK NO-ERROR.
