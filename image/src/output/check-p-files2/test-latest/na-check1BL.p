...
FOR EACH res-line WHERE res-line.active-flag = 1 
    ...
    FIND FIRST reslin-queasy WHERE reslin-queasy.key = "arrangement" 
        ...
        AND cidate GE reslin-queasy.date1 
        AND cidate LE reslin-queasy.date2
        AND reslin-queasy.char1 NE "" NO-LOCK NO-ERROR. 
    IF AVAILABLE reslin-queasy AND res-line.arrangement NE reslin-queasy.char1 THEN 
    DO:
        msg-str = msg-str + CHR(2)
                + translateExtended ("Different Arrangement Code between reservation and fixed rate.",lvCAREA,"")
                + CHR(10) 
                + translateExtended ("Night Audit process not possible. Please check it.",lvCAREA,"").
        w-flag = YES.
        LEAVE.
    END.
