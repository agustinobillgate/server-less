...
DEFINE VARIABLE amount AS DECIMAL. 
  ...
  /* sales */ 
  FIND FIRST htparam WHERE paramnr = 862 NO-LOCK NO-ERROR. 
  IF AVAILABLE htparam THEN f-eknr = htparam.finteger.               /* Rulita 211024 | Fixing for serverless */  
  FIND FIRST htparam WHERE paramnr = 892 NO-LOCK NO-ERROR. 
  IF AVAILABLE htparam THEN b-eknr = htparam.finteger.               /* Rulita 211024 | Fixing for serverless */  
 
/* cost, see paramgroup 21 */ 
  FIND FIRST htparam WHERE paramnr = 257 NO-LOCK NO-ERROR. 
  IF AVAILABLE htparam THEN fl-eknr = htparam.finteger.               /* Rulita 211024 | Fixing for serverless */  
  FIND FIRST htparam WHERE paramnr = 258 NO-LOCK NO-ERROR. 
  IF AVAILABLE htparam THEN bl-eknr = htparam.finteger.               /* Rulita 211024 | Fixing for serverless */  
  ...
  FOR EACH l-store WHERE l-store.lager-nr NE main-storage AND 
    ...
    RUN func-food-bev(fl-eknr, bl-eknr).                /* Rulita 210225 | Fixing change name procedure from food-bev serverless issue git 653 */
    ...
    FOR EACH s-list WHERE s-list.flag = 1 AND s-list.reihenfolge = 2 NO-LOCK 
      ...
      ASSIGN
        ...
        fb-flash.mtd-consumed = s-list.t-betrag.                                               /* Rulita 240225 | Fixing from t-betrag to s-list.t-betrag serveless issue git 653 */
    END. 
...
PROCEDURE beverage-to-food: 
  ...
  IF AVAILABLE htparam THEN                                                           /* Rulita 210225 | Fixing if avail serveless issue git 653 */
  DO:
      FIND FIRST gl-acct WHERE gl-acct.fibukonto = htparam.fchar NO-LOCK NO-ERROR.    /* Rulita 210225 | Fixing from fchar to htparam.fchar serveless issue git 653 */ 
      IF AVAILABLE gl-acct THEN                                                       /* Rulita 210225 | Fixing if avail serveless issue git 653 */
      DO:
          bev-food = fchar. 
...
END. 
*/ 
/* Rulita 210225 | Fixing change name procedure from food-bev serverless issue git 653 */
PROCEDURE func-food-bev:          
DEFINE INPUT PARAMETER fl-eknr AS INTEGER. 
DEFINE INPUT PARAMETER bl-eknr AS INTEGER. 
