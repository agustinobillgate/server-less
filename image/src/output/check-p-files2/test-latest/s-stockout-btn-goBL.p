DEFINE TEMP-TABLE op-list LIKE l-op
  FIELD fibu AS CHAR
  FIELD a-bezeich AS CHAR /*LIKE l-artikel.bezeich*/
  FIELD a-lief-einheit AS DECIMAL /*LIKE l-artikel.lief-einheit*/
  FIELD a-traubensort AS CHAR /*LIKE l-artikel.traubensort*/
  .
...
/* Malik Serverless 826 comment and move line 
...
/* Malik Serverless 826 */
FIND FIRST l-ophdr WHERE RECID(l-ophdr) = rec-id NO-LOCK NO-ERROR.
        ...
        AND l-bestand.lager-nr > 0 NO-LOCK:
          ...
          DO:
            msg-str2 = msg-str2 + "&W" + translateExtended ("One (or more) stock item(s) under Minimum Onhand Level!",lvCAREA,"")
                     ...
                     + " " + STRING(l-artikel.min-bestand). /* Malik Serverless 826 change l-artikel.min-best ->  l-artikel.min-bestand */
            LEAVE.
          END.
      END.
    END.
...
PROCEDURE check-qty: 
  ...
  FOR EACH op-list: 
    ...
    curr-oh = l-bestand.anz-anf-best + l-bestand.anz-eingang - l-bestand.anz-ausgang. /* Malik Serverless 826 add table name anz-anf-best + anz-eingang - anz-ausgang -> l-bestand.anz-anf-best + l-bestand.anz-eingang - l-bestand.anz-ausgang */
