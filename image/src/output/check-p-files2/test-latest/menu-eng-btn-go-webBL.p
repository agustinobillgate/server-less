...
DEFINE VARIABLE tmp-anzahl AS INTEGER.
              ...
              */
              ...
              DO: 
                  ...
                  IF AVAILABLE h-journal THEN 
                    ASSIGN
                      cost = anz * h-journal.epreis * h-artikel.prozent / 100
                      h-list.cost = h-journal.epreis * h-artikel.prozent / 100. /* Naufal Afthar - 5E405E*/
                  ELSE 
                    ASSIGN
                      cost = anz * h-artikel.epreis1 * h-artikel.prozent / 100 * exchg-rate 
                      h-list.cost = h-artikel.epreis1 * h-artikel.prozent / 100 * exchg-rate. 
              END. 
              cost = cost / fact1. 
              h-list.anzahl = h-list.anzahl + anz. 
              ...
              /* Rulita 090625 | Fixing serverless issue git 832 */

              IF h-list.anzahl NE 0 AND h-list.anzahl NE ? THEN
                  tmp-anzahl = 0.
              ELSE
                tmp-anzahl = h-list.anzahl.
              
              /* Dzikri D635B2 - CHANGE PRICE TO AVERAGE PRICE */
              IF vat-included AND tmp-anzahl NE 0 THEN 
                h-list.epreis = (h-list.t-sales / tmp-anzahl) * exchg-rate / fact. 
              ELSE IF tmp-anzahl NE 0 THEN 
                h-list.epreis = (h-list.t-sales / tmp-anzahl) * exchg-rate / fact1.
              ELSE 
                h-list.epreis = 0.
              /* Dzikri D635B2 - END */
