 ...
 /*ITA*/
  ...
  FOR EACH umsatz WHERE umsatz.departement GE from-dept 
       ...
       DO:
           ...
           IF AVAILABLE artikel AND (artikel.artart = 0 OR artikel.artart = 8) THEN*/
           DO:
            ...
            */
            ...
            END.
              ...
              ASSIGN                /*Wenni 12/7/14 */
                ...
                nett-serv = nett-amt * serv
                nett-tax  = nett-amt * vat
                nett-amt  = umsatz.betrag - nett-serv - nett-tax

                /*nett-amt  =  ROUND(nett-amt, price-decimal)
              ...
              /*DO:
                  IF price-decimal EQ 2 THEN*/
                      ...
                      ASSIGN
                        cl-list.dnet = (nett-amt / fact1)
                        cl-list.dgros = (umsatz.betrag / fact1) 
                        cl-list.dserv = (nett-serv / fact1)
                        cl-list.dtax = (nett-tax / fact1)
                        dnet = dnet + ROUND(cl-list.dnet, price-decimal)
                        dserv = dserv + ROUND(cl-list.dserv, price-decimal) 
                        dtax = dtax + ROUND(cl-list.dtax, price-decimal)
                        dgros = dgros + ROUND(cl-list.dgros, price-decimal) 
                        t-dnet = t-dnet + ROUND(cl-list.dnet, price-decimal) 
                        t-dserv = t-dserv + ROUND(cl-list.dserv, price-decimal) 
                        t-dtax = t-dtax + ROUND(cl-list.dtax, price-decimal)
                        t-dgros = t-dgros + ROUND(cl-list.dgros, price-decimal)
              ...
              DO:
                  ASSIGN
                      vat-list.dtax = vat-list.dtax + (nett-tax / fact1)
                      vat-list.dnet = vat-list.dnet + (nett-amt / fact1) 
                      vat-list.dgros = vat-list.dgros + (umsatz.betrag / fact1) 
                      vat-list.dserv = vat-list.dserv + (nett-serv / fact1) 
                   .
              END.
              ...
              IF umsatz.datum GE first-date AND umsatz.datum LE to-date
                  ...
                  ASSIGN
                  vat-list.mtax = vat-list.mtax + (nett-tax / fact1) 
                  vat-list.mnet = vat-list.mnet + (nett-amt / fact1) 
                  vat-list.mserv = vat-list.mserv + (nett-serv / fact1) 
                  vat-list.mgros = vat-list.mgros + (umsatz.betrag / fact1) .
              ...
              IF AVAILABLE vat-list THEN
                  ASSIGN
                      vat-list.ytax = vat-list.ytax + (nett-tax / fact1) 
                      vat-list.ynet = vat-list.ynet + (nett-amt / fact1) 
                      vat-list.yserv = vat-list.yserv + (nett-serv / fact1) 
                      vat-list.ygros = vat-list.ygros + (umsatz.betrag / fact1) 
                      .              
       END. /*end do-it*/
  END.   /*end umsatz*/
  /*end ita*/
    ...
    DO:
      ...
      DO:
        ...
        ASSIGN
          umsz-list.artnr       = STRING(cl-list.artnr, ">>>>")  
          umsz-list.bezeich     = cl-list.bezeich
          umsz-list.day-nett    = STRING(ROUND(cl-list.dnet, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.day-serv    = STRING(ROUND(cl-list.dserv, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.day-tax     = STRING(ROUND(cl-list.dtax, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.day-gros    = STRING(ROUND(cl-list.dgros, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.day-persen  = STRING(ROUND(cl-list.proz2, price-decimal), "->>>>9.99")
          umsz-list.mtd-nett    = STRING(ROUND(cl-list.mnet, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.mtd-serv    = STRING(ROUND(cl-list.mserv, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.mtd-tax     = STRING(ROUND(cl-list.mtax, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.mtd-gros    = STRING(ROUND(cl-list.mgros, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.mtd-persen  = STRING(ROUND(cl-list.proz4, price-decimal), "->>>>9.99")
          umsz-list.ytd-nett    = STRING(ROUND(cl-list.ynet, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.ytd-serv    = STRING(ROUND(cl-list.yserv, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.ytd-tax     = STRING(ROUND(cl-list.ytax, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.ytd-gros    = STRING(ROUND(cl-list.ygros, price-decimal), "->>>,>>>,>>>,>>9.99")
          umsz-list.ytd-persen  = STRING(ROUND(cl-list.proz6, price-decimal), "->>>>9.99")
          umsz-list.month-bud   = STRING(ROUND(cl-list.mbudget, price-decimal), "->,>>>,>>>,>>>,>>9.99")
        .
      END.
      ELSE
      DO:
        ...
        ASSIGN
          umsz-list.artnr       = STRING(cl-list.artnr, ">>>>")  
          umsz-list.bezeich     = cl-list.bezeich
          umsz-list.day-nett    = STRING(ROUND(cl-list.dnet, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.day-serv    = STRING(ROUND(cl-list.dserv, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.day-tax     = STRING(ROUND(cl-list.dtax, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.day-gros    = STRING(ROUND(cl-list.dgros, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.day-persen  = STRING(ROUND(cl-list.proz2, price-decimal), "->>>>9")
          umsz-list.mtd-nett    = STRING(ROUND(cl-list.mnet, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.mtd-serv    = STRING(ROUND(cl-list.mserv, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.mtd-tax     = STRING(ROUND(cl-list.mtax, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.mtd-gros    = STRING(ROUND(cl-list.mgros, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.mtd-persen  = STRING(ROUND(cl-list.proz4, price-decimal), "->>>>9")
          umsz-list.ytd-nett    = STRING(ROUND(cl-list.ynet, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.ytd-serv    = STRING(ROUND(cl-list.yserv, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.ytd-tax     = STRING(ROUND(cl-list.ytax, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.ytd-gros    = STRING(ROUND(cl-list.ygros, price-decimal), "->>>,>>>,>>>,>>9")
          umsz-list.ytd-persen  = STRING(ROUND(cl-list.proz6, price-decimal), "->>>>9")
          umsz-list.month-bud   = STRING(ROUND(cl-list.mbudget, price-decimal), "->,>>>,>>>,>>>,>>9")
        .
      END.  
...
PROCEDURE sum-budget :
    ...
    DEFINE VARIABLE tmp-from-date AS DATE NO-UNDO.
    DEFINE VARIABLE tmp-to-date   AS DATE NO-UNDO.

    RUN calculate-date-range(to-date, OUTPUT tmp-from-date, OUTPUT tmp-to-date).

    FOR EACH budget WHERE budget.datum GE tmp-from-date AND
        budget.datum LE tmp-to-date  AND
...
PROCEDURE calculate-date-range:
    DEFINE INPUT PARAMETER input-date AS DATE.
    
    DEFINE OUTPUT PARAMETER from-date AS DATE.
    DEFINE OUTPUT PARAMETER to-date AS DATE.
    
    
    from-date = DATE(MONTH(input-date), 1, YEAR(input-date)).
    to-date = from-date + 31.
    to-date = DATE(MONTH(to-date), 1, YEAR(to-date)).
    to-date = to-date - 1.
