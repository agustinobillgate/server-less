...
PROCEDURE update-gl:
   ...
   /*
    ...
    DO TRANSACTION:
        FIND FIRST gl-journal NO-LOCK NO-ERROR.
        DO WHILE AVAILABLE gl-journal:
          FIND FIRST coa-list WHERE coa-list.old-fibu = gl-journal.fibukonto
            AND coa-list.new-fibu NE ? NO-LOCK NO-ERROR.
          IF AVAILABLE coa-list THEN
          DO:
                FIND FIRST gljourBuff2 WHERE RECID(gljourBuff2) = RECID(gl-journal) EXCLUSIVE-LOCK.
                ASSIGN gljourBuff2.fibukonto = coa-list.new-fibu.
                FIND CURRENT gljourBuff2 NO-LOCK.
                RELEASE gljourBuff2.
          END.
          FIND NEXT gl-journal NO-LOCK NO-ERROR.
        END.
    ...
    DO TRANSACTION:
        ...
        DO WHILE AVAILABLE gl-jourhis:
          ...
          IF AVAILABLE coa-list THEN
          DO:
                FIND FIRST jouhisBuff2 WHERE RECID(jouhisBuff2) = RECID(gl-jourhis) EXCLUSIVE-LOCK.
                ASSIGN jouhisBuff2.fibukonto = coa-list.new-fibu.
                FIND CURRENT jouhisBuff2 NO-LOCK.
                RELEASE jouhisBuff2.
