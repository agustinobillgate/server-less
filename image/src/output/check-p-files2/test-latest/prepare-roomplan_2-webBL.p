...
DEFINE TEMP-TABLE room-list 
  ...
  FIELD bezeich     AS CHAR FORMAT "x(30)"   LABEL "RmDesc" /*Irfan 080118*/
...
DEFINE VARIABLE do-it1  AS LOGICAL.
  ...
  FOR EACH res-line WHERE 
    ...
    AND NOT res-line.ankunft GT to-date AND NOT res-line.abreise LE curr-date)
  ...
  FOR EACH t-res-line :
    ...
    DO: 
      ...
      DO datum = datum1 TO datum2: 
        IF t-res-line.resstatus EQ 13 AND room-list.recid1[i] NE 0 THEN 
        DO: 
          /* NO ENTRY AS the main guest exists */ 
        END. 
        ELSE room-list.room[i] = SUBSTR(t-res-line.name, 1, LENGTH(t-res-line.NAME)). 
        i = i + 1. 
      END. 
    END. 
    ASSIGN
