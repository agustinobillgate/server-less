...
DEF OUTPUT PARAMETER split-price AS LOGICAL NO-UNDO.
...
ASSIGN split-price = htparam.flogical.
...
PROCEDURE calc-periode:
    ...
    IF AVAILABLE queasy THEN DO:
        ASSIGN periode-rsv1 = reslin-list.ankunft
               periode-rsv2 = reslin-list.abreise
         .
...
PROCEDURE disp-query: 
  ...
  FOR EACH reslin-queasy WHERE key = "arrangement"
      ...
      IF AVAILABLE periode-list THEN DO:
          FIND FIRST q2-reslin-queasy WHERE q2-reslin-queasy.date1 = periode-list.periode1 
              AND q2-reslin-queasy.date2 = periode-list.periode2 NO-ERROR.
          IF NOT AVAILABLE q2-reslin-queasy THEN DO:
              CREATE q2-reslin-queasy.
              ASSIGN 
                  q2-reslin-queasy.date1   = periode-list.periode1 
                  q2-reslin-queasy.date2   = periode-list.periode2 
               ...
               .
          END.
          ASSIGN q2-reslin-queasy.deci1   = q2-reslin-queasy.deci1 + reslin-queasy.deci1.
      END.
  END.
END.
