...
PROCEDURE modify-dml:
    ...
    IF curr-dept = 0 THEN DO:
        FOR EACH dml-art WHERE dml-art.datum = selected-date NO-LOCK,
            ...
            ELSE
            DO:
                /* show old remark if user not save new remark after new update by Oscar (18 November 2024) - 0F5CB1 */
                FIND FIRST queasy WHERE queasy.KEY = 202 
                AND queasy.number1 = 0
                AND queasy.number2 = dml-art.artnr
                AND queasy.number3 = 0
                AND queasy.date1 = dml-art.datum NO-LOCK NO-ERROR.
                IF AVAILABLE queasy THEN DO:
                    ASSIGN c-list.remark = queasy.char1.
                END.
        ...
        FOR EACH l-artikel WHERE l-artikel.bestellt USE-INDEX artnr_ix NO-LOCK,
            ...
            FIND FIRST c-list WHERE c-list.artnr = l-artikel.artnr NO-LOCK NO-ERROR.
    ...
    ELSE DO:
        ...
        DO:
            ...
            AND ENTRY(2, reslin-queasy.char3, ";") EQ dml-no NO-LOCK,
        ...
        ELSE DO:
            ...
            FIRST l-untergrup WHERE l-untergrup.zwkum = l-artikel.zwkum NO-LOCK:
                ...
                fill-dml-nr = "D" + STRING(dml-artdep.departement, "99") + SUBSTR(STRING(YEAR(dml-artdep.datum)),3,2) 
                            + STRING(MONTH(dml-artdep.datum), "99") + STRING(DAY(dml-artdep.datum), "99").
                ...
                ASSIGN
                    ...
                    IF c-list.dml-nr EQ "" OR c-list.dml-nr EQ ? THEN
                        c-list.dml-nr  = fill-dml-nr + STRING(1, "999").
                ELSE
                DO:
