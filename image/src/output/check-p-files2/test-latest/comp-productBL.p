...
DEFINE BUFFER gstat FOR guestat1.
...
lfdate = DATE(1,1,jml).
IF mm = 12 THEN
    ltdate = DATE(1,1,jml + 1) - 1.
ELSE ltdate = DATE(mm + 1, 1, jml) - 1.

FOR EACH guestat1 WHERE guestat1.datum GE from-date
    AND guestat1.datum LE to-date NO-LOCK BY guestat1.gastnr BY guestat1.datum:

    i = MONTH(guestat1.datum).
    IF curr-guest = 0 OR curr-guest NE guestat1.gastnr THEN
    DO:
        FIND FIRST guest WHERE guest.karteityp = 1 AND guest.gastnr = guestat1.gastnr NO-LOCK NO-ERROR.
        IF AVAILABLE guest THEN
        DO:
            CREATE c-list.
            ASSIGN
                c-list.gastnr = guest.gastnr
                c-list.bezeich = guest.NAME
                c-list.room[i] = guestat1.zimmeranz. 
                c-list.ytd = c-list.ytd + guestat1.zimmeranz
            .
        END.
    END.
    ELSE
    DO:
        FIND FIRST c-list WHERE c-list.gastnr = guestat1.gastnr NO-ERROR.
        IF AVAILABLE c-list THEN
        DO:
            ASSIGN
                c-list.room[i] = c-list.room[i] + guestat1.zimmeranz
                c-list.ytd = c-list.ytd + guestat1.zimmeranz.
        END.

    END.
    curr-guest = guestat1.gastnr.
    
    /*DO:
        i = MONTH(guestat1.datum).
        FIND FIRST c-list WHERE c-list.gastnr = guest.gastnr NO-ERROR.
...
FOR EACH guestat1 WHERE guestat1.datum GE lfdate
    AND guestat1.datum LE ltdate NO-LOCK BY guestat1.gastnr:
    ...
    DO: 
        ...
        IF AVAILABLE guest THEN
        DO:
            FIND FIRST c-list WHERE c-list.gastnr = guest.gastnr NO-LOCK NO-ERROR.
            IF NOT AVAILABLE c-list THEN
            DO:
                CREATE c-list.
                ASSIGN
                    c-list.gastnr = guest.gastnr
                    c-list.bezeich = guest.NAME
                    c-list.lytd = c-list.lytd + guestat1.zimmeranz.
            END.
            ELSE c-list.lytd = c-list.lytd + guestat1.zimmeranz.
        END.
    END.
    ELSE
    DO:
        FIND FIRST c-list WHERE c-list.gastnr = guestat1.gastnr NO-LOCK NO-ERROR.
        IF AVAILABLE c-list THEN c-list.lytd = c-list.lytd + guestat1.zimmeranz.
    END.                         
END.
...
counter = 0.
FOR EACH c-list BY c-list.ytd DESCENDING BY c-list.bezeich:
    DO:
        counter = counter + 1.
        CREATE compproduct-list.  
        ASSIGN
            compproduct-list.nr = counter
            compproduct-list.num = counter
            compproduct-list.bezeich = c-list.bezeich
            compproduct-list.ytd = c-list.ytd
            compproduct-list.lytd = c-list.lytd
            tytd = tytd + c-list.ytd
            tlytd = tlytd + c-list.lytd. 
        DO i = 1 TO 12:
            ASSIGN
                compproduct-list.room[i] = c-list.room[i]
                troom[i] = troom[i] + c-list.room[i].
        END.
    END.
END.
...
CREATE compproduct-list. 
ASSIGN
    compproduct-list.num = 9999 
    compproduct-list.bezeich = translateExtended ("T O T A L",lvCAREA,"")
    compproduct-list.ytd = tytd
    compproduct-list.lytd = tlytd.
DO i = 1 TO 12:
    compproduct-list.room[i] = troom[i].
END.
