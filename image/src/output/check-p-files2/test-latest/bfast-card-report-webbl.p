...
DEFINE TEMP-TABLE payload-list
    FIELD room         AS CHARACTER INITIAL "*"
    FIELD fdate        AS DATE
    FIELD tdate        AS DATE
    FIELD mealTime     AS CHARACTER
...
IF payload-list.fdate EQ ? OR payload-list.tdate EQ ? THEN
DO:
    FOR EACH mealcoup WHERE mealcoup.NAME = payload-list.mealTime
        AND mealcoup.zinr MATCHES("*" + payload-list.room + "*") NO-LOCK:
        FIND FIRST res-line WHERE res-line.resnr = mealcoup.resnr 
        AND res-line.zinr = mealcoup.zinr
        AND res-line.name MATCHES("*" + payload-list.lname + "*") NO-LOCK NO-ERROR.
        ...
        DO:
            ...
            */
            ...
            strdayuse = "".
            DO ii = 1 TO 32:
                IF mealcoup.verbrauch[ii] NE 0 THEN DO:
                    getDataNr = getDataNr + 1.
                    strdayuse = strdayuse + STRING(ii) + ";".
                END.
            END.
...
DO:
    FOR EACH mealcoup WHERE mealcoup.NAME = payload-list.mealTime
    AND mealcoup.ankunft LE payload-list.tdate /* arrival */
    AND mealcoup.abreise GE payload-list.fdate /* departure */
    AND mealcoup.zinr MATCHES("*" + payload-list.room + "*") NO-LOCK BY mealcoup.ankunft DESC:
        FIND FIRST res-line WHERE res-line.resnr = mealcoup.resnr 
        AND res-line.zinr = mealcoup.zinr
        AND res-line.name MATCHES("*" + payload-list.lname + "*") NO-LOCK NO-ERROR.
        IF AVAILABLE res-line THEN 
        DO:
            ...
            */
            ...
            /* MESSAGE STRING("Arrival: " + STRING(mealcoup.ankunft) + " Departure: " + STRING(mealcoup.abreise) + " Room: " + STRING(mealcoup.zinr)) VIEW-AS ALERT-BOX. */
            ...
            DO ii = 1 TO 32:
                ...
                END.
            END.
            /* MESSAGE strdayuse VIEW-AS ALERT-BOX INFO. */
...
FIND FIRST bfast-data NO-ERROR.
IF AVAILABLE bfast-data THEN
DO:
    CREATE bfast-data.
    ASSIGN
...
PROCEDURE fill-data:
    ...
    tmpDate = mealcoup.ankunft + used. /* arrival */
    day-use = mealcoup.ankunft + daycount. /* arrival */

    /* MESSAGE STRING("T-Date: " + STRING(tmpDate) + " Day-Use: " + STRING(day-use)) VIEW-AS ALERT-BOX. */

    IF day-use GE payload-list.fdate AND day-use LE payload-list.tdate THEN DO:
        ...
        FOR EACH argt-line WHERE argt-line.argtnr = arrangement.argtnr:
            IF argt-line.argt-artnr = artGrp-abf AND payload-list.mealTime = "Breakfast" THEN 
            DO:
                IF argt-line.fakt-modus = 3 AND (tmpDate - res-line.ankunft EQ 1) THEN DO: /*Second day of stay*/
                    totAdult = res-line.erwachs. 
                    totCompli = res-line.gratis.
                END.
                ELSE IF argt-line.fakt-modus = 6 AND (tmpDate - res-line.ankunft LE argt-line.intervall) THEN DO: /*Special*/
                    totAdult = res-line.erwachs.
                    totCompli = res-line.gratis.
                END.
