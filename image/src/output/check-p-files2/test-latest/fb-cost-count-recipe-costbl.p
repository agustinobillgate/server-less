...
FOR EACH h-rezlin WHERE h-rezlin.artnrrezept = grid-list-artnrrezept NO-LOCK: 
    ...
    DO: 
      FIND FIRST l-artikel WHERE l-artikel.artnr = h-rezlin.artnrlager NO-LOCK NO-ERROR. /* Malik Serverless 725 add NO-ERROR and if available */ 
...
DEFINE buffer h-rezlin1 FOR h-rezlin. 
  ...
  FOR EACH h-rezlin1 WHERE h-rezlin1.artnrrezept = p-artnr NO-LOCK: 
    ...
    ELSE 
    DO: 
      FIND FIRST l-artikel WHERE l-artikel.artnr = h-rezlin1.artnrlager NO-LOCK NO-ERROR. /* Malik Serverless 725 add NO-ERROR and if available */
      IF AVAILABLE l-artikel THEN
      DO:  
        IF price-type = 0 OR l-artikel.ek-aktuell = 0 THEN 
          vk-preis = l-artikel.vk-preis. 
