/*FT severless validasi pembagian 0*/
...
DEFINE VARIABLE last-year   AS INTEGER.
...
PROCEDURE create-umsatz1:
   ...
   ASSIGN
       ...
       last-year = YEAR(to-date) - 1 /*FT serverless*/
       last-fdate = DATE(1,1, last-year)
    ...
    DO:       
        last-tdate = DATE(MONTH(to-date),28,last-year).
    END.    
    ELSE last-tdate = DATE(MONTH(to-date),DAY(to-date),last-year).
    ...
    IF tocc-rm NE 0 THEN tavr        = trmrev / tocc-rm.
    IF tocc-rm1 NE 0 THEN tavr1       = trmrev1 / tocc-rm1.
    IF tocc-rm2 NE 0 THEN tavr2       = trmrev2 / tocc-rm2.
    IF tocc-rm3 NE 0 THEN tavr3       = trmrev3 / tocc-rm3.
    IF tocc-rm4 NE 0 THEN tavr4       = trmrev4 / tocc-rm4.
    IF tsale NE 0 THEN 
        ASSIGN
    ...
    FOR EACH cl-list:
        IF cl-list.saleable NE 0 AND tocc-proz NE 0 THEN
            cl-list.mpi = cl-list.occ-proz / tocc-proz.
        IF cl-list.saleable NE 0 AND cl-list.rmrev NE 0 AND trevpar NE 0 THEN
            cl-list.rgi = cl-list.yield / trevpar.
        IF tavr NE 0 THEN
            cl-list.ari = cl-list.avrgrate / tavr.

        IF cl-list.saleable1 NE 0 AND tocc-proz1 NE 0 THEN
            cl-list.mpi1 = cl-list.occ-proz1 / tocc-proz1.
        IF cl-list.saleable1 NE 0 AND cl-list.rmrev1 NE 0 AND trevpar1 NE 0 THEN
            cl-list.rgi1 = cl-list.yield1 / trevpar1.
        IF tavr1 NE 0 THEN
            cl-list.ari1 = cl-list.avrgrate1 / tavr1.

        IF cl-list.saleable2 NE 0 AND tocc-proz2 NE 0 THEN
            cl-list.mpi2 = cl-list.occ-proz2 / tocc-proz2.
        IF cl-list.saleable2 NE 0 AND cl-list.rmrev2 NE 0  AND trevpar2 NE 0 THEN
            cl-list.rgi2 = cl-list.yield2 / trevpar2.
        IF tavr2 NE 0 THEN
            cl-list.ari2 = cl-list.avrgrate2 / tavr2.

        IF cl-list.saleable3 NE 0 AND tocc-proz3 NE 0 THEN
            cl-list.mpi3 =  cl-list.occ-proz3 / tocc-proz3.
        IF cl-list.saleable3 NE 0 AND cl-list.rmrev3 NE 0 AND trevpar3 NE 0 THEN
            cl-list.rgi3 = cl-list.yield3 / trevpar3.
        IF tavr3 NE 0 THEN
            cl-list.ari3 = cl-list.avrgrate3 / tavr3.
        
        IF cl-list.saleable4 NE 0 AND tocc-proz4 NE 0 THEN
            cl-list.mpi4 = cl-list.occ-proz4 / tocc-proz4.
        IF cl-list.saleable4 NE 0 AND cl-list.rmrev4 NE 0 AND trevpar4 NE 0 THEN
        ...
        IF trevpar NE 0 THEN
            trgi        = trevpar / trevpar.
        IF trevpar1 NE 0 THEN
            trgi1       = trevpar1 / trevpar1.
        IF trevpar2 NE 0 THEN
            trgi2       = trevpar2 / trevpar2.
        IF trevpar3 NE 0 THEN
            trgi3       = trevpar3 / trevpar3.
        IF trevpar4 NE 0 THEN
        ...
        IF tocc-proz3 NE 0 THEN
            tmpi3       = tocc-proz3 / tocc-proz3.
        IF tocc-proz4 NE 0 THEN
    ...
    CREATE cl-list.
    ASSIGN
        /*cl-list.htlname     = STRING("T O T A L", "x(24) ")  FT severless*/
        cl-list.htlname     = "T O T A L"
