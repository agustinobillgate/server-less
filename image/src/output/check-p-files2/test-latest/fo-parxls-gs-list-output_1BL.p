...
FOR EACH brief-list: 
  ...
  DO: 
    ...
    DO: 
      ...
      ELSE IF curr-cmd EQ 2060 THEN RUN create-var0(curr-texte, YES).
      /*MG BED81E : WALK IN GUEST BY ARRIVAL (ANKUNFT)*/
      ELSE IF curr-cmd EQ 2061 THEN RUN create-var0(curr-texte, YES).
...
IF msg-str NE "" THEN 
DO:
    FIND FIRST queasy WHERE queasy.KEY = 293
      AND queasy.deci1 = briefnr
      AND queasy.char2 = TRIM(link)
      AND queasy.date1 = TODAY
      AND queasy.char1 = TRIM(user-init) EXCLUSIVE-LOCK NO-ERROR.
...
/*--------------------------------------*/
  ...
  FOR EACH ww1 WHERE ww1.grpflag = 0 BY ww1.main-code DESC: 
    ...
    DO:
       ...
       FIND FIRST w11 WHERE w11.main-code = 810 NO-LOCK NO-ERROR. 
       IF NOT AVAILABLE w11 THEN 
       DO: 
         msg-str = msg-str + CHR(2)
       ...
       END. 
    END.
    ELSE IF ww1.main-code = 1008 THEN
    DO:
       FIND FIRST artikel WHERE artikel.artnr = ww1.artnr 
         AND artikel.departement = ww1.dept NO-LOCK NO-ERROR. 
    ...
    DO:
       ASSIGN
           sg-dept = INT(SUBSTR(ww1.s-artnr,1,2))
           subgr   = INT(SUBSTR(ww1.s-artnr,3)).
       ...
       DO: 
         msg-str = msg-str + CHR(2)
                 + translateExtended ("SubGroup not found : ",lvCAREA,"") + ww1.varname.
         error-nr = -1. 
         RETURN. 
       END. 
    END.
    ELSE IF ww1.main-code = 1084 THEN 
    DO:
       FIND FIRST w11 WHERE RECID(w11) = RECID(ww1).

       FIND FIRST artikel WHERE artikel.artnr = w11.artnr 
         AND artikel.departement = w11.dept NO-LOCK NO-ERROR. 
       
       IF NOT AVAILABLE artikel THEN 
       DO:
           msg-str = msg-str + CHR(2)
                 + translateExtended ("No such article number : ",lvCAREA,"") + STRING(w11.artnr)
                 + " " + translateExtended ("Dept",lvCAREA,"") + " " + STRING(w11.dept).
           RETURN.
       END.
    END.
    ELSE IF ww1.main-code = 809 THEN
    DO:
       FIND FIRST artikel WHERE artikel.artnr = ww1.artnr 
         AND artikel.departement = ww1.dept NO-LOCK NO-ERROR.
       IF NOT AVAILABLE artikel THEN 
       DO: 
         msg-str = msg-str + CHR(2)
                 + translateExtended ("No such article number : ",lvCAREA,"") + STRING(ww1.artnr)
                 + " " + translateExtended ("Dept",lvCAREA,"") + " " + STRING(ww1.dept).
         RETURN.
       END.
    END.
  END.
