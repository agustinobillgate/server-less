...
FIND FIRST str-list WHERE str-list.op-recid = op-recid NO-LOCK NO-ERROR.
      ...
      DO: 
        FOR EACH str-list WHERE str-list.lscheinnr = l-ophdr.lscheinnr: 
          ...
          DO: 
            FIND FIRST l-op WHERE RECID(l-op) = str-list.op-recid NO-LOCK NO-ERROR. 
            IF AVAILABLE l-op THEN 
            RUN create-lartjob.p(RECID(l-ophdr), l-op.artnr, - l-op.anzahl,- l-op.warenwert, l-op.datum, NO).
          END. */
        END.
