...
PROCEDURE update-allotment:
  ...
  DEFINE VARIABLE tmp-date AS DATE. /* Malik Serverless 318 */
  ...
  FOR EACH kontline WHERE kontline.kontcode = currcode NO-LOCK BY kontline.ankunft:
    tmp-date = kontline.abreise + 1. /* Malik Serverless 318 */
      FIND FIRST kline WHERE kline.kontcode = currcode
          AND kline.ankunft = tmp-date /* Malik Serverless 318 : kontline.abreise + 1 -> tmp-date */
          AND kline.zimmeranz = kontline.zimmeranz NO-LOCK NO-ERROR.
      IF AVAILABLE kline THEN
      DO:
        FIND FIRST kbuff WHERE RECID(kbuff) = RECID(kontline) NO-LOCK NO-ERROR. /* Malik Serverless 318 : -> NO-LOCK NO-ERROR */
        IF AVAILABLE kbuff THEN
        DO:
          FIND CURRENT kbuff EXCLUSIVE-LOCK.
