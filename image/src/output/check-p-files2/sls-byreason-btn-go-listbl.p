DEFINE TEMP-TABLE p-list 
  ...
  FIELD reason-name AS CHAR FORMAT "x(15)"
...
DEF OUTPUT PARAMETER TABLE FOR slreas-list.
/*
...
*/
{SupertransBL.i} 
DEF VAR lvCAREA AS CHAR INITIAL "sls-byreason".
...
DEF BUFFER buf-akthdr   FOR akthdr.
  ...
  DO:
    nr = 0.
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 5 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.grund = akt-code1.aktionscode 
        AND buf-akthdr.flag = 3 AND buf-akthdr.grund NE 0 NO-LOCK
      ...
      DO:
        ...
        CREATE p-list.
        ASSIGN
          p-list.reason-name = STRING(nr, ">9") + " - " + akt-code1.bezeich.

        hnr  = 0.
        amt  = 0.
        tamt = 0.

        FOR EACH akthdr1 WHERE akthdr1.grund = akt-code1.aktionscode 
          AND akthdr1.flag = 3 NO-LOCK,
        ...
        CREATE p-list.
        ASSIGN
          p-list.pcomp = "TOTAL " + STRING(akt-code1.bezeich, "x(12)") + ":" + STRING(hnr, ">>>>")
  ...
  DO:
    ...
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 5 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.grund = akt-code1.aktionscode 
        AND buf-akthdr.flag = 3 AND buf-akthdr.grund NE 0 
      ...
      DO:
        ...
        FOR EACH akthdr1 WHERE akthdr1.grund = akt-code1.aktionscode 
          AND akthdr1.flag = 3 AND akthdr1.userinit = usr-init NO-LOCK,
          FIRST guest1 WHERE guest1.gastnr = akthdr1.gastnr NO-LOCK,                                                    
          FIRST akt-kont1 WHERE akt-kont1.gastnr = guest1.gastnr 
            AND akt-kont1.kontakt-nr = akthdr1.kontakt-nr NO-LOCK BY guest1.NAME:
...
DEF BUFFER buf-akthdr   FOR akthdr.
  ...
  DO:
    ...
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 5 NO-LOCK,
      ...
      DO:
        ...
        FOR EACH akthdr1 WHERE akthdr1.grund = akt-code1.aktionscode AND akthdr1.flag = 3 
  ...
  DO:
    ...
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 5 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.grund = akt-code1.aktionscode 
        AND buf-akthdr.flag = 3 AND buf-akthdr.grund NE 0 AND buf-akthdr.userinit = usr-init 
        AND buf-akthdr.erl-datum GE next-date AND buf-akthdr.erl-datum LE to-date NO-LOCK 
      BY akt-code1.aktionscode:
      IF AVAILABLE akt-code1 THEN
      DO:
        ...
        FOR EACH akthdr1 WHERE akthdr1.grund = akt-code1.aktionscode 
          AND akthdr1.flag = 3 AND akthdr1.userinit = usr-init 
          AND akthdr1.erl-datum GE next-date AND akthdr1.erl-datum LE to-date NO-LOCK,
          FIRST guest1 WHERE guest1.gastnr = akthdr1.gastnr NO-LOCK,                                                    
          FIRST akt-kont1 WHERE akt-kont1.gastnr = guest1.gastnr 
            AND akt-kont1.kontakt-nr = akthdr1.kontakt-nr NO-LOCK BY guest1.NAME:
...
PROCEDURE fill-reason:
  FOR EACH p-list:
    CREATE slreas-list.
    BUFFER-COPY p-list TO slreas-list.
  END.
END PROCEDURE.
