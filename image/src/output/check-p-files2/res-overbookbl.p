...
IF res-argt NE "" THEN FIND FIRST arrangement WHERE
    arrangement.arrangement = res-argt NO-LOCK.
...
/* SY 19 AUG 2015 overbook-flag will be set as TRUE if the new guest stay 
   is within res-line stay (before modify) and on changes in the room type.
   if overvook-flag = TRUE and overbooking found then mk-reslineUI
   only display warning only
*/

/*
IF (res-mode = "modify"  OR res-mode = "inhouse")
    AND curr-date        GE res-line.ankunft
    AND inp-abreise      LE res-line.abreise
    AND zimkateg.zikatnr EQ res-line.zikatnr 
    AND qty              LE res-line.zimmeranz THEN RETURN.
/* SY 27 AUG 2015
    ASSIGN overbook-flag EQ YES.
*/
*/
...
/*
  ...
  DO: 
    i = 1. 
    DO WHILE i LE 13: 
      IF i = 1 OR i = 2 OR i = 6 THEN anz = anz + resplan.anzzim[i]. 
      i = i + 1. 
    END. 
  END. 
*/ 
...
/* SY 18 AUG 2015
  FOR EACH res-line WHERE res-line.active-flag LE 1 AND res-line.resstatus LE 6 
    ...
    IF (res-mode = "new" OR res-mode = "insert" OR res-mode = "qci") AND 
      (res-line.resnr EQ resnr AND res-line.reslinnr EQ reslinnr) THEN . 
    ELSE 
    DO: 
      anz = anz + res-line.zimmeranz. 
      IF res-line.kontignr = 0 THEN anz0 = anz0 + res-line.zimmeranz. 
    END. 
  END. 
  ...
  FOR EACH kontline WHERE kontline.gastnr > 0 AND kontline.ankunft LE curr-date 
    ...
    ELSE IF curr-date GE (ci-date + kontline.ruecktage) THEN
      anzalot = anzalot + kontline.zimmeranz. 
  END. 

  FOR EACH outorder WHERE curr-date GE outorder.gespstart 
    AND curr-date LE outorder.gespende AND outorder.betriebsnr LE 1 NO-LOCK, 
    FIRST zimmer WHERE zimmer.zinr = outorder.zinr 
    AND zimmer.sleeping = YES AND zimmer.zikatnr = zimkateg.zikatnr NO-LOCK: 
    anzooo = anzooo + 1. 
  END. 
  anz0 = anz0 + anzooo. 
  anz = anz + anzooo. 
 
  IF (anz + qty) > (maxzimmer + zimkateg.overbooking) 
    THEN overmax = YES. 
  IF NOT overbook AND (anz + qty) > (maxzimmer) THEN 
  DO: 
    overbook = YES. 
    overanz = maxzimmer - anz - qty. 
    overanz = - overanz. 
    overdate = curr-date. 
  END. 

  IF (NOT overmax) AND ask-it THEN 
  DO: 
    delta = (anz0 + anzalot + qty) - (maxzimmer + zimkateg.overbooking). 
    IF delta > 0 THEN 
    DO: 
      ASSIGN
        msg-str = "&Q" + 
          translateExtended ("Overbooking found due to Allotment / Global Reservation.",lvCAREA,"")
          + CHR(10)
          + STRING(curr-date) + " - " + translateExtended ("Actual Overbooking:",lvCAREA,"")
          + " " + STRING(delta) + CHR(10)
          + translateExtended ("Do you wish to continue?",lvCAREA,"")
        overanz    = delta 
        overdate   = curr-date 
        incl-allot = YES
      . 
...
*/
  FOR EACH res-line WHERE res-line.active-flag LE 1 AND res-line.resstatus LE 6 
    ...
    IF (res-mode = "new" OR res-mode = "insert" OR res-mode = "qci") 
      AND (res-line.resnr EQ inp-resnr 
       AND res-line.reslinnr EQ inp-reslinnr) THEN . 
    ELSE 
    /*IF (res-mode = "modify" OR res-mode = "inhouse" 
          ...
          DO:
            ...
            DO:
               ...
               IF NOT AVAILABLE queasy THEN
                 occ-list.anz-alot = occ-list.anz-alot - res-line.zimmeranz. 
            END.
          END.
      END.
    END. 
  ...
  FOR EACH kontline WHERE kontline.gastnr > 0 AND kontline.ankunft LE curr-date 
      ...
      /* global reservation */
      IF kontline.betriebsnr = 1 THEN
        occ-list.anz-avail = occ-list.anz-avail - kontline.zimmeranz. 
      ...
      /* global allotment for the OTA */    
        ...
        IF NOT AVAILABLE queasy AND
          curr-date GE (ci-date + kontline.ruecktage) THEN
          occ-list.anz-alot = occ-list.anz-alot + kontline.zimmeranz. 
      END.
  END.
...
FOR EACH occ-list BY occ-list.datum:
    ...
    DO:
      ...
      .
      /*
      IF overbook-flag THEN overbook = YES.
      ELSE overmax = YES.
      */
      overbook = YES.
...
IF NOT overmax THEN
FOR EACH occ-list BY occ-list.datum:
    IF (occ-list.anz-avail + zimkateg-overbook - occ-list.anz-ooo
        - occ-list.anz-alot) LT 0 THEN
    DO:
        ASSIGN
            overmax    = YES
            overanz    = - (occ-list.anz-avail + zimkateg-overbook 
                       - occ-list.anz-ooo - occ-list.anz-alot)
            overdate   = occ-list.datum
            incl-allot = YES
        .
        LEAVE.
...
DEF BUFFER rbuff FOR res-line.  
  ...
  DO:
    FIND FIRST queasy WHERE queasy.char1 = origcontcode
        AND queasy.number1 = zimkateg.zikatnr AND queasy.KEY = 171
        AND queasy.date1 = inp-ankunft AND queasy.number3 NE 0 NO-LOCK NO-ERROR.
    ASSIGN ratecode-found = AVAILABLE ratecode.
    IF ratecode-found THEN allotment = queasy.number3.
    
    /*FIND FIRST ratecode WHERE ratecode.CODE EQ statcode
      AND ratecode.zikatnr EQ zimkateg.zikatnr
      AND ratecode.argtnr  EQ arrangement.argtnr
      AND ratecode.startperiode LE inp-ankunft
      AND ratecode.endperiode GE inp-ankunft
  ...
  DO:
    FIND FIRST queasy WHERE queasy.char1 = origcontcode
        AND queasy.number1 = zimkateg.typ AND queasy.KEY = 171
        AND queasy.date1 = inp-ankunft AND queasy.number3 NE 0 NO-LOCK NO-ERROR.
    ASSIGN ratecode-found = AVAILABLE ratecode.
    IF ratecode-found THEN allotment = queasy.number3.
  ...
  DO curr-date = inp-ankunft TO inp-abreise - 1:
    ...
    FOR EACH rbuff WHERE rbuff.gastnr = res-line.gastnr
      ...
      doit-flag = (rbuff.resnr NE inp-resnr) 
                OR (rbuff.reslinnr NE inp-reslinnr).
      ...
      ELSE
      DO: 
        FIND FIRST zbuff WHERE zbuff.zikatnr = rbuff.zikatnr
          NO-LOCK.
      ...
      DO curr-i = 1 TO NUM-ENTRIES(rbuff.zimmer-wunsch,";") - 1:
          ...
          DO:
              ...
              IF rline-origcode = origcontcode THEN 
                occ-room = occ-room + rbuff.zimmeranz.
              LEAVE.
          END.
      END.
    END.
    ...
    DO: 
      ...
      ASSIGN
        ...
        + CHR(10)
        + STRING(curr-date) + " - " + translateExtended ("Actual Overbooking:",lvCAREA,"")
        + " " + STRING(occ-room + qty - allotment) + CHR(10)
      .
      ELSE
      ASSIGN
        msg-str = msg-str 
    ...
    END.
  END.
  IF msg-str NE "" THEN msg-str = msg-str
      + translateExtended ("Do you wish to continue?",lvCAREA,"").
END.
