...
DEFINE VARIABLE flight-info         AS CHAR    NO-UNDO.
  ...
  FOR EACH s-list: 
    FIND FIRST res-line WHERE RECID(res-line) = s-list.res-recid.
    ...
    DO:
      ...
      IF gastmember GT 0 THEN
        FIND FIRST guest WHERE guest.gastnr = gastmember.
      ...
      . 
    END.
    ELSE FIND FIRST guest WHERE guest.gastnr = res-line.gastnrmember.
...
*/
    ...
    FIND FIRST resline WHERE RECID(resline) = s-list.res-recid EXCLUSIVE-LOCK. 
    ...
    DO: 
      ...
      DO: 
        FIND FIRST bill WHERE bill.resnr = resline.resnr 
          AND bill.reslinnr = resline.reslinnr NO-ERROR. 
    ...
    DO: 
      ...
      DO: 
        ...
        DO: 
          FIND FIRST zimmer WHERE zimmer.zinr = s-list.zinr NO-LOCK. 
          ASSIGN 
        ...
        DO: 
          ...
          FOR EACH bill WHERE bill.resnr = resline.resnr 
            ...
            AND resline1.reslinnr = bill.reslinnr NO-LOCK. 
            IF resline1.resstatus = 12 /* i.e. res-line FOR additional bill */ 
            THEN DO: 
    ...
    IF t-resline.NAME NE resline.NAME 
        OR t-resline.zinr NE resline.zinr THEN RUN res-changes.
  END. 
END. 
