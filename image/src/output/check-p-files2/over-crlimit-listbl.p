...
DEFINE TEMP-TABLE out-list LIKE output-list.
...
PROCEDURE create-list:
  ...
  FOR EACH c-list :
      FIND FIRST queasy WHERE queasy.KEY = 105 AND queasy.char1 = c-list.NAME NO-LOCK NO-ERROR.
      IF AVAILABLE queasy THEN
      DO:
         FIND FIRST output-list WHERE output-list.guest-name = c-list.NAME NO-ERROR.
         IF NOT AVAILABLE output-list THEN
         DO:
           CREATE output-list.
           ASSIGN
             output-list.guest-name       = c-list.NAME
             output-list.credit-limit     = queasy.deci3
             output-list.artnr            = queasy.number3
             output-list.card-no          = queasy.char2.

           FIND FIRST h-artikel WHERE h-artikel.departement = c-list.dept AND h-artikel.artnr = queasy.number3 NO-LOCK NO-ERROR.
         ...
         output-list.balanced = output-list.amount - queasy.deci3.
      END.
  END.

  FOR EACH output-list WHERE output-list.artnr GE from-art AND output-list.artnr LE to-art  
      AND output-list.balanced GE 0 NO-LOCK BY output-list.guest-name :

      CREATE out-list.
      BUFFER-COPY output-list TO out-list.
  END.
