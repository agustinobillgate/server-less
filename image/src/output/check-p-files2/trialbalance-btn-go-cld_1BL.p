...
DEFINE TEMP-TABLE output-listhis LIKE output-list.

DEFINE TEMP-TABLE result-list LIKE output-list.
...
DEFINE TEMP-TABLE t-res-listhis LIKE t-res-list.
...
DEFINE TEMP-TABLE g-listpre LIKE g-list.
DEFINE TEMP-TABLE g-listhis LIKE g-list. /*Eko*/
...
DEFINE VARIABLE lastPrevMonthDate     AS DATE.
DEFINE VARIABLE firstMonthDate        AS DATE.
...
FUNCTION lastDay RETURNS DATE ( INPUT d AS DATE ):
    DEFINE VARIABLE lastMonthDate AS DATE.
    DEFINE VARIABLE nextMonth  AS INTEGER.
    DEFINE VARIABLE firstDate AS DATE.
    nextMonth = MONTH(d) + 1.
    firstDate = DATE(nextMonth,1,YEAR(d)).
    lastMonthDate = firstDate - 1.
...
ASSIGN
    ...
    from-date = ?
    t-date = ?
    firstMonthDate = DATE(month(close-date), 1, year(close-date))
    lastPrevMonthDate = firstMonthDate - 1.
...
DEFINE VARIABLE t-debit AS DECIMAL FORMAT "->>,>>>,>>>,>>9.99"  INITIAL 0 NO-UNDO. 
DEFINE VARIABLE t-credit LIKE t-debit INITIAL 0 NO-UNDO. 
...
DEFINE VARIABLE curr-ttbal      AS DECIMAL.
  ...
  /*IF to-date LE DATE(month(close-date), 1, year(close-date)) - 1 THEN 
    act-flag = 1. FT serverless*/
  IF to-date LE lastPrevMonthDate THEN act-flag = 1.
    ...
    IF acct-type = 0 THEN DO:
        FOR EACH gl-acct WHERE gl-acct.fibukonto GE from-fibu 
          ...
          DO: 
            ...
            output-list.ref-no = STRING(c, "x(32)").
            output-list.begin-bal = gl-acct.bezeich.
            output-list.tot-debit = "".
            output-list.is-show-depart = YES.

            /* Added field dept name and dept nr for trial balance by Oscar (11 Oktober 2024) - FBF73C */
            FIND FIRST gl-department WHERE gl-department.nr EQ gl-acct.deptnr NO-ERROR.
            IF AVAILABLE gl-department THEN
            DO:
                output-list.dept-name  = gl-department.bezeich.
                output-list.dept-nr    = gl-department.nr.
            END.
            ELSE
            DO:
                output-list.dept-name  = "".
                output-list.dept-nr    = 0.
            END.
            ...
            FOR EACH g-list WHERE g-list.fibu EQ gl-acct.fibukonto 
                ...
                IF g-list.grecid NE 0 THEN DO:
                    IF from-date NE ? AND to-date NE ? THEN DO:
                        IF t-date GE from-date AND t-date LE to-date THEN DO:
                            FIND FIRST gl-journal WHERE RECID(gl-journal) EQ g-list.grecid NO-LOCK.
                            FIND FIRST gl-jouhdr WHERE gl-jouhdr.jnr EQ gl-journal.jnr NO-LOCK NO-ERROR.
                            DELETE g-list.
                            ...
                            DO:
                                ...
                                /*/*FDL June 11, 2024 => Ticket 62094C*/
                                output-list.ref-no = STRING(gl-jouhdr.refno, "x(32)").
                                output-list.begin-bal = "".
                                output-list.net-change = "0.00".
                                output-list.ending-bal = "0.00".*/
    
                                /*Alder - Ticket 6EEA84*/
                                output-list.ref-no = STRING(gl-jouhdr.refno, "x(32)").
                                output-list.begin-bal = STRING(tot-bal - gl-journal.debit + gl-journal.credit, "->>,>>>,>>>,>>>,>>9.99").
                                output-list.net-change = STRING((gl-journal.debit - gl-journal.credit), "->>,>>>,>>>,>>>,>>9.99").
                                output-list.ending-bal = STRING((tot-bal), "->>,>>>,>>>,>>>,>>9.99").
                                output-list.is-show-depart = NO.
                                ...
                                */
                                ...
                                DO:
                                    STR = STR + STRING(gl-journal.debit, ">>>,>>>,>>>,>>>,>>9.99").
                                    output-list.tot-debit = STRING(gl-journal.debit, ">>>,>>>,>>>,>>>,>>9.99").
                                END.
                                ELSE
                                DO:
                                    STR = STR + STRING(gl-journal.debit, "->>,>>>,>>>,>>>,>>9.99").
                                ...
                                DO:
                                    STR = STR + STRING(gl-journal.credit, ">>>,>>>,>>>,>>>,>>9.99").
                                    output-list.tot-credit = STRING(gl-journal.credit, ">>>,>>>,>>>,>>>,>>9.99").
                                END.
                                ELSE 
                                DO:
                                    STR = STR + STRING(gl-journal.credit, "->>,>>>,>>>,>>>,>>9.99").
                    ...
                    IF from-datehis NE ? AND to-datehis NE ? THEN DO:
                        IF t-date GE from-datehis AND t-date LE to-datehis THEN DO:
                            ...
                            DO:
                                ...
                                /*/*FDL June 11, 2024 => Ticket 62094C*/
                                output-list.ref-no = STRING(gl-jhdrhis.refno, "x(32)").
                                output-list.begin-bal = "".
                                output-list.net-change = "0.00".
                                output-list.ending-bal = "0.00".*/
                                ...
                                /*Alder - Ticket 6EEA84*/
                                output-list.ref-no = STRING(gl-jhdrhis.refno, "x(32)").
                                output-list.begin-bal = STRING(tot-bal - gl-jourhis.debit + gl-jourhis.credit, "->>,>>>,>>>,>>>,>>9.99").
                                output-list.net-change = STRING((gl-jourhis.debit - gl-jourhis.credit), "->>,>>>,>>>,>>>,>>9.99").
                                output-list.ending-bal = STRING((tot-bal), "->>,>>>,>>>,>>>,>>9.99").
                                output-list.is-show-depart = NO.
                                ...
                                */
                                ...
                                DO:
                                    STR = STR + STRING(gl-jourhis.debit, ">>>,>>>,>>>,>>>,>>9.99").
                                    output-list.tot-debit = STRING(gl-jourhis.debit, ">>>,>>>,>>>,>>>,>>9.99").
                                END.
                                ELSE
                                DO:
                                    STR = STR + STRING(gl-jourhis.debit, "->>,>>>,>>>,>>>,>>9.99").
                                ...
                                DO:
                                    STR = STR + STRING(gl-jourhis.credit, ">>>,>>>,>>>,>>>,>>9.99").
                                    output-list.tot-credit = STRING(gl-jourhis.credit, ">>>,>>>,>>>,>>>,>>9.99").
                                END.
                                ELSE 
                                DO:
                                    STR = STR + STRING(gl-jourhis.credit, "->>,>>>,>>>,>>>,>>9.99").
            ...
            DO:
              ...
              DO: 
                ...
                /*FDL June 11, 2024 => Ticket 62094C*/
                output-list.ref-no = "T O T A L".
                output-list.begin-bal = c.
                output-list.is-show-depart = NO.
                 /* FD Comment
                ...
                */
                ...
                DO:
                    STR = STR + STRING(t-debit, ">>>,>>>,>>>,>>>,>>9.99").
                    output-list.tot-debit = STRING(t-debit, ">>>,>>>,>>>,>>>,>>9.99").
                END.
                ELSE
                DO:
                    STR = STR + STRING(t-debit, "->>,>>>,>>>,>>>,>>9.99").
                ...
                DO:
                    STR = STR + STRING(t-credit, ">>>,>>>,>>>,>>>,>>9.99").
                    output-list.tot-credit = STRING(t-credit, ">>>,>>>,>>>,>>>,>>9.99").
                END.
                ELSE
                DO:
                    STR = STR + STRING(t-credit, "->>,>>>,>>>,>>>,>>9.99").
                ...
                STR = STR + STRING(c, "x(22)"). 
                output-list.net-change = STRING(c, "x(22)").
                ASSIGN t-bal = p-bal + tot-diff. /*Alder - Ticket 6EEA84*/
                RUN convert-balance(t-bal, OUTPUT c). 
                STR = STR + STRING(c, "x(22)"). 
                output-list.ending-bal = STRING(c, "x(22)").
                CREATE output-list. 
                ASSIGN counter = counter + 1
                       output-list.nr = counter.
              END. 
                ...
                IF g-list.grecid NE 0 THEN DO:
                    IF t-date GE from-date AND t-date LE to-date THEN DO:
                        ...
                        DO:
                            ...
                            output-list.begin-bal = "".
                            output-list.net-change = "0.00".
                            output-list.ending-bal = "0.00". /*Alder Debug*/
                            output-list.is-show-depart = NO.
...
/*      CREATE output-list. */ 
        ...
        STR = STR + STRING(c, "x(22)"). 
        /*FDL June 11, 2024 => Ticket 62094C*/
        output-list.ref-no = "Grand TOTAL".
        output-list.begin-bal = c.    
        output-list.is-show-depart = NO.            
        ...
        */
        ...
        DO:
            STR = STR + STRING(tot-debit, ">>>,>>>,>>>,>>>,>>9.99").
            output-list.tot-debit = STRING(tot-debit, ">>>,>>>,>>>,>>>,>>9.99").
        END.
        ELSE
        DO:
            STR = STR + STRING(tot-debit, "->>,>>>,>>>,>>>,>>9.99").
        ...
        DO:
            STR = STR + STRING(tot-credit, ">>>,>>>,>>>,>>>,>>9.99").
            output-list.tot-credit = STRING(tot-credit, ">>>,>>>,>>>,>>>,>>9.99").
        END.
        ELSE
        DO:
            STR = STR + STRING(tot-credit, "->>,>>>,>>>,>>>,>>9.99").
        ...
        STR = STR + STRING(c, "x(22)"). 
        output-list.net-change = STRING(c, "x(22)").
        ASSIGN tot-bal = prev-bal + tot-diff. /*Alder - Ticket 6EEA84*/
            ...
            AND gl-acct.fibukonto LE to-fibu NO-LOCK BY gl-acct.fibukonto: 
                ...
                FOR EACH g-list WHERE g-list.fibu EQ gl-acct.fibukonto USE-INDEX fibu_ix:
                    ...
                    IF g-list.grecid NE 0 THEN DO:
                        ...
                        IF from-date NE ? AND t-date NE ? THEN DO:
                            IF t-date GE from-date AND t-date LE to-date THEN DO:
                                FIND FIRST gl-journal WHERE RECID(gl-journal) = g-list.grecid NO-LOCK.
                                FIND FIRST gl-jouhdr WHERE gl-jouhdr.jnr = gl-journal.jnr NO-LOCK NO-ERROR.
                                DELETE g-list.
                ...
                IF p-bal NE 0 OR t-debit NE 0 OR t-credit NE 0 
                    ...
                    IF year(close-year) = year(to-date) THEN DO: 
                        IF gl-acct.acc-type = 1 THEN 
                            output-list.budget = gl-acct.budget[month(to-date)] * -1. 
                        ELSE IF gl-acct.acc-type = 2 OR gl-acct.acc-type = 5 THEN 
                            output-list.budget = gl-acct.budget[month(to-date)]. 
                    END. 
                    ELSE IF year(close-year) = year(to-date) + 1 THEN DO: 
                        IF gl-acct.acc-type = 1 THEN 
                            output-list.budget = gl-acct.ly-budget[month(to-date)] * -1. 
                        ELSE IF gl-acct.acc-type = 2 OR gl-acct.acc-type = 5 THEN 
...
IF sorttype = 2 THEN DO: 
    ...
    BY gl-main.code:
        ...
        AND gl-acct.fibukonto GE from-fibu 
        AND gl-acct.fibukonto LE to-fibu 
        AND gl-acct.deptnr = from-dept NO-LOCK BY gl-acct.fibukonto: 
            ASSIGN
                konto    = gl-acct.fibukonto
                t-debit  = 0 
                t-credit = 0 
...
DEF VARIABLE lastYear AS INTEGER NO-UNDO.
DEF VARIABLE tmp-date AS INTEGER NO-UNDO.
DEF VARIABLE tmp-todate AS DATE  NO-UNDO.
DEF BUFFER gbuff FOR gl-acct.

    ASSIGN
      lastYear = YEAR(t-from-date) - 1.    
    ...
    DO:
      ...
      ELSE 
        FIND FIRST gl-accthis WHERE gl-accthis.fibukonto = fibu
        AND gl-accthis.YEAR = lastYear /*YEAR(t-from-date) - 1 FT serverless*/ NO-LOCK NO-ERROR.
    ...
    ASSIGN
        tmp-todate = t-to-date + 1
        tmp-date   = DAY(tmp-todate).
    IF tmp-date NE 1 THEN RETURN. /* to-date must be end of month */
...
PROCEDURE calcRevCost:
  ...
  DEF VAR lastMonth                AS INTEGER           NO-UNDO.

  ASSIGN
      lastMonth = MONTH(from-date) - 1. 
  ...
  DO:
    ...
    DO n = 1 TO lastMonth /*MONTH(from-date) - 1 FT serverless*/:
...
DEFINE BUFFER hbuff   FOR gl-accthis.
  ...
  DO:
    FIND FIRST gl-accthis WHERE gl-accthis.fibukonto = pnl-acct
      AND gl-accthis.YEAR = lastYear /*(YEAR(t-to-date) - 1) FT serverless*/ NO-LOCK NO-ERROR.
    IF year(close-year) = year(t-to-date) THEN 
    DO: 
      m = close-month - 1. 
      IF m GE 1 THEN p-bal = - gl-acct.actual[m]. 
