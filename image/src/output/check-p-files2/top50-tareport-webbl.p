...
FOR EACH genstat WHERE YEAR(genstat.datum) = yy AND MONTH(genstat.datum) LE mm
    ...
    AND genstat.res-logic[2] USE-INDEX DATE_ix NO-LOCK
    ...
    IF disptype EQ 0 THEN         /*william add display type all*/
        FIND FIRST guest WHERE guest.gastnr = genstat.gastnr USE-INDEX gastnr_index NO-LOCK.
    ELSE IF disptype EQ 1 THEN    /*william add display type company*/
        FIND FIRST guest WHERE guest.gastnr = genstat.gastnr AND guest.karteityp EQ 1 USE-INDEX gastnr_index NO-LOCK.
    ELSE IF disptype EQ 2 THEN    /*william add display type all travel agent*/
        FIND FIRST guest WHERE guest.gastnr = genstat.gastnr AND guest.karteityp EQ 2 USE-INDEX gastnr_index NO-LOCK.
    ELSE IF disptype EQ 3 THEN    /*william add display type offline travel agent*/
        FIND FIRST guest WHERE guest.gastnr = genstat.gastnr AND guest.karteityp EQ 2 AND guest.steuernr EQ "" USE-INDEX gastnr_index NO-LOCK.
    ELSE IF disptype EQ 4 THEN    /*william add display type online travel agent*/
        FIND FIRST guest WHERE guest.gastnr = genstat.gastnr AND guest.karteityp EQ 2 AND guest.steuernr NE "" USE-INDEX gastnr_index NO-LOCK.
