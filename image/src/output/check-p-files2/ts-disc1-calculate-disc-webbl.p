...
FIND FIRST htparam WHERE htparam.paramnr EQ 1203 NO-LOCK.
IF htparam.paramgr EQ 19 AND htparam.feldtyp EQ 4 THEN 
    disc-alert = htparam.flogical.

FIND FIRST htparam WHERE htparam.paramnr EQ 468 NO-LOCK. 
disc-service = htparam.flogical. /* disc reduce service? */ 

FIND FIRST htparam WHERE htparam.paramnr EQ 469 NO-LOCK. 
disc-tax = htparam.flogical. /* disc reduce vat? */ 

FIND FIRST htparam WHERE htparam.paramnr EQ 491 NO-LOCK. 
price-decimal = htparam.finteger. 

FIND FIRST htparam WHERE htparam.paramnr EQ 134 NO-LOCK. 
incl-mwst = htparam.flogical. 
FIND FIRST htparam WHERE htparam.paramnr EQ 135 NO-LOCK. 
incl-service = htparam.flogical. 
FIND FIRST htparam WHERE htparam.paramnr EQ 479 NO-LOCK. 
service-tax = htparam.flogical.

FIND FIRST htparam WHERE htparam.paramnr EQ 1001 NO-LOCK. 
voucher-art = htparam.finteger. 
FIND FIRST htparam WHERE htparam.paramnr EQ 557 NO-LOCK. 
disc-art1 = htparam.finteger. 
FIND FIRST htparam WHERE htparam.paramnr EQ 596 NO-LOCK. 
disc-art2 = htparam.finteger. 
FIND FIRST htparam WHERE htparam.paramnr EQ 556 NO-LOCK. 
disc-art3 = htparam.finteger.
...
FOR EACH menu-disc WHERE menu-disc.artnr NE disc-art1
    ...
    FIND FIRST disc-list WHERE disc-list.umsatzart EQ t-artikel.umsatzart  
        AND disc-list.mwst EQ t-h-artikel.mwst  
        AND disc-list.service EQ t-h-artikel.service NO-ERROR.  
    ...
    DO:          
        ...
        FIND FIRST disc-list WHERE disc-list.umsatzart EQ t-artikel.umsatzart  
            AND disc-list.mwst EQ t-h-artikel.mwst NO-ERROR.            
        IF NOT AVAILABLE disc-list THEN   
        DO:              
            IF disc-alert THEN  
            DO:
    ...
    DO:     
        ...
        DO:  
            ...
            DO:   
                h-mwst = f-dec.  
                IF service-tax THEN h-mwst = h-mwst * (menu-disc.epreis + h-service) / 100.   
