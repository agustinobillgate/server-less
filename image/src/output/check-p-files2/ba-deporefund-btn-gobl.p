...
PROCEDURE deposit-refund: 
DEFINE VARIABLE bill-date AS DATE. 
DEFINE VARIABLE i         AS INTEGER.
  FIND FIRST htparam WHERE paramnr = 110 USE-INDEX paramnr_ix NO-LOCK NO-ERROR. 
  ...
  IF AVAILABLE bk-veran THEN DO:
      ...
      ASSIGN
        bk-veran.deposit-pay[9]      = bk-veran.deposit-pay[9] - payment
        bk-veran.payment-date[9]     = bill-date
        bk-veran.payment-userinit[9] = user-init 
      ...
      ASSIGN balance = bk-veran.deposit - bk-veran.total-paid. 
...
PROCEDURE create-journal: 
  ...
  DO: 
    CREATE umsatz. 
    ASSIGN umsatz.artnr = artikel.artnr
  ...
  ASSIGN
    ...
    billjournal.bezeich         = artikel.bezeich  + " *BQT" + STRING(veran-nr) 
  ...
  ASSIGN
    ...
    billjournal.bezeich         =  depobezeich + " *BQT" + STRING(veran-nr)
    billjournal.epreis          = 0
    billjournal.zeit            = TIME 
    billjournal.userinit        = user-init
    billjournal.bill-datum      = bill-date
...
PROCEDURE inv-ar: 
  ...
  FIND FIRST htparam WHERE paramnr = 997 NO-LOCK NO-ERROR. 
