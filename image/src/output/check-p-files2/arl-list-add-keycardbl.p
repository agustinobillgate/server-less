...
/*DEFINE OUTPUT PARAMETER replace-keycard AS LOGICAL.*/
...
DO: 
    FIND FIRST resline WHERE resline.active-flag LE 1 
        AND ((resline.ankunft = res-line.ankunft) OR 
            (resline.abreise = res-line.ankunft)) 
    ...
    DO: 
      ...
      msg-str = msg-str + CHR(2)
              + translateExtended( "Keycard is being used for other guest:", lvCAREA, "":U)
              + " " + resline.NAME.
      RETURN. 
    END. 
    ...
    DO: 
      err-flag = 2.
      msg-str = msg-str + CHR(2)
              + translateExtended( "Number of given KeyCard =", lvCAREA, "":U)
              + " " + STRING(resline.betrieb-gast)
              + CHR(10)
              + translateExtended( "No more KeyCard can be generated for room",
                                   lvCAREA, "":U) + " " + resline.zinr.
      RETURN. 
    END. 
    ...
    IF resline.betrieb-gast = 0 AND resline.resstatus NE 11 
      AND resline.resstatus NE 13 THEN case1 = 1.
    ELSE case1 = 2.
    ASSIGN
...
DO: 
    ...
    DO: 
        FIND FIRST rline WHERE rline.resnr = resline.resnr 
          ...
          AND rline.zinr = resline.zinr AND rline.betrieb-gast GT 0 
          NO-LOCK NO-ERROR. 
        IF AVAILABLE rline THEN 
        DO: 
          msg-str = msg-str + CHR(2) + "&W"
                  + translateExtended( "Room Shrer found. Replace the KeyCard too.",
                                       lvCAREA, "":U).
        END. 
    END. 
    RETURN. 
...
DO: 
    ...
    FOR EACH resline WHERE resline.resnr = resno 
        ...
        AND resline.betrieb-gast = 0 NO-LOCK BY resline.zinr 
        BY resline.resstatus: 

        FIND FIRST rline WHERE rline.active-flag LE 1 
          AND ((rline.ankunft = resline.ankunft) OR 
               (rline.abreise = resline.ankunft)) 
          AND rline.zinr = resline.zinr AND rline.betrieb-gast > 0 
        ...
        DO: 
          ASSIGN
            t-keycard.msg-str = msg-str + CHR(2)
                  + translateExtended("RmNo",lvCAREA,"") + " " + rline.zinr
                  + ": "
                  + translateExtended( "Keycard is being used for other guest:", lvCAREA, "":U)
                  + " " + rline.NAME.
        END. 
