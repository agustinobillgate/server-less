...
DEFINE INPUT-OUTPUT PARAMETER TABLE FOR cust-list.
...
FOR EACH queasy WHERE queasy.KEY = 280 AND queasy.char1 = "Guest Turnover" 
    AND queasy.char3 = idFlag NO-LOCK BY queasy.number1:
     ...
     CREATE cust-list.
     ASSIGN  
        cust-list.gastnr        = INTEGER(ENTRY(1, queasy.char2, "|"))
        cust-list.cust-name     = ENTRY(2, queasy.char2, "|")
        cust-list.gesamtumsatz  = DECIMAL(ENTRY(3, queasy.char2, "|"))
        cust-list.logiernachte  = DECIMAL(ENTRY(4, queasy.char2, "|"))
        cust-list.argtumsatz    = DECIMAL(ENTRY(5, queasy.char2, "|"))
        cust-list.f-b-umsatz    = DECIMAL(ENTRY(6, queasy.char2, "|"))
        cust-list.sonst-umsatz  = DECIMAL(ENTRY(7, queasy.char2, "|"))
        cust-list.wohnort       = ENTRY(8, queasy.char2, "|")
        cust-list.plz           = ENTRY(9, queasy.char2, "|")
        cust-list.land          = ENTRY(10, queasy.char2, "|")
        cust-list.sales-id      = ENTRY(11, queasy.char2, "|")
        cust-list.ba-umsatz     = DECIMAL(ENTRY(12, queasy.char2, "|"))
        cust-list.ly-rev        = DECIMAL(ENTRY(13, queasy.char2, "|"))
        cust-list.region        = ENTRY(14, queasy.char2, "|")
        cust-list.region1       = ENTRY(15, queasy.char2, "|")
        cust-list.stayno        = INTEGER(ENTRY(16, queasy.char2, "|"))
        cust-list.resnr         = ENTRY(17, queasy.char2, "|")
        cust-list.counter       = INTEGER(ENTRY(18, queasy.char2, "|"))
        cust-list.counterall    = INTEGER(ENTRY(19, queasy.char2, "|"))
        cust-list.resno         = INTEGER(ENTRY(20, queasy.char2, "|"))
        cust-list.reslinnr      = INTEGER(ENTRY(21, queasy.char2, "|"))
        cust-list.curr-pos      = INTEGER(ENTRY(22, queasy.char2, "|"))
     .
...
FIND FIRST pqueasy WHERE pqueasy.KEY = 280 
    AND pqueasy.char1 = "Guest Turnover"
...
ELSE DO:
    FIND FIRST tqueasy WHERE tqueasy.KEY = 285 
        AND tqueasy.char1 = "Guest Turnover" 
