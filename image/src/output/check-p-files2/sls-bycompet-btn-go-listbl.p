...
DEFINE TEMP-TABLE p-list 
  ...
  FIELD hotel-name AS CHAR FORMAT "x(30)"
...
DEFINE TEMP-TABLE slcompet-list
  FIELD pnr AS INTEGER INITIAL 0 
  FIELD sflag AS INTEGER
  FIELD hotel-name AS CHAR FORMAT "x(30)"
  FIELD pcomp AS CHAR FORMAT "x(50)" 
...
DEF OUTPUT PARAMETER TABLE FOR slcompet-list.
/*
...
*/
{SupertransBL.i} 
DEF VAR lvCAREA AS CHAR INITIAL "sls-bycompet".
...
DEF BUFFER buf-akthdr   FOR akthdr.
  ...
  FOR EACH slcompet-list:
    DELETE slcompet-list.
  END.

  IF all-flag THEN
  DO:
    nr = 0.
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 4 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.mitbewerber[1] = akt-code1.aktionscode 
        AND buf-akthdr.flag = 3 AND buf-akthdr.mitbewerber[1] NE 0 NO-LOCK
      ...
      DO:
        ...
        CREATE p-list.
        ASSIGN
          p-list.hotel-name = STRING(nr, ">9") + " - " + akt-code1.bezeich.

        hnr  = 0.
        amt  = 0.
        tamt = 0.

        FOR EACH akthdr1 WHERE akthdr1.flag = 3 AND akthdr1.mitbewerber[1] = akt-code1.aktionscode NO-LOCK,
          ...
          ASSIGN
            ...
            p-list.pmain1 = akthdr1.mitbewerber[1]
  ...
  DO:
    ...
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 4 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.mitbewerber[1] = akt-code1.aktionscode 
        AND buf-akthdr.flag = 3 AND buf-akthdr.mitbewerber[1] NE 0 
        AND buf-akthdr.userinit = usr-init NO-LOCK BY akt-code1.aktionscode:
      IF AVAILABLE akt-code1 THEN
      DO:
        nr = nr + 1.
        CREATE p-list.
        ...
        FOR EACH akthdr1 WHERE akthdr1.flag = 3 AND akthdr1.userinit = usr-init
          AND akthdr1.mitbewerber[1] = akt-code1.aktionscode NO-LOCK,
          FIRST guest1 WHERE guest1.gastnr = akthdr1.gastnr NO-LOCK,
          FIRST akt-kont1 WHERE akt-kont1.gastnr = guest1.gastnr 
            AND akt-kont1.kontakt-nr = akthdr1.kontakt-nr NO-LOCK
          BY akthdr1.stufe BY guest1.NAME:
...
DEF BUFFER buf-akthdr   FOR akthdr.
  ...
  DO:
    ...
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 4 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.mitbewerber[1] = akt-code1.aktionscode
        AND buf-akthdr.flag = 3 AND buf-akthdr.mitbewerber[1] NE 0 
        AND buf-akthdr.erl-datum GE next-date AND buf-akthdr.erl-datum LE to-date NO-LOCK
      BY akt-code1.aktionscode:
      IF AVAILABLE akt-code1 THEN
      DO:
        nr = nr + 1.
        ...
        FOR EACH akthdr1 WHERE akthdr1.flag = 3 AND akthdr1.mitbewerber[1] = akt-code1.aktionscode 
          AND akthdr1.erl-datum GE next-date AND akthdr1.erl-datum LE to-date NO-LOCK,
          FIRST guest1 WHERE guest1.gastnr = akthdr1.gastnr NO-LOCK,
          FIRST akt-kont1 WHERE akt-kont1.gastnr = guest1.gastnr 
            AND akt-kont1.kontakt-nr = akthdr1.kontakt-nr NO-LOCK
          BY akthdr1.stufe BY guest1.NAME:
  ...
  DO:
    nr = 0.
    FOR EACH akt-code1 WHERE akt-code1.aktiongrup = 4 NO-LOCK,
      FIRST buf-akthdr WHERE buf-akthdr.mitbewerber[1] = akt-code1.aktionscode AND buf-akthdr.flag = 3 
        AND buf-akthdr.mitbewerber[1] NE 0 AND buf-akthdr.userinit = usr-init 
        AND buf-akthdr.erl-datum GE next-date AND buf-akthdr.erl-datum LE to-date NO-LOCK 
      BY akt-code1.aktionscode:
      IF AVAILABLE akt-code1 THEN
      DO:
        ...
        FOR EACH akthdr1 WHERE akthdr1.flag = 3 AND akthdr1.userinit = usr-init
          AND akthdr1.mitbewerber[1] = akt-code1.aktionscode 
          AND akthdr1.erl-datum GE next-date AND akthdr1.erl-datum LE to-date NO-LOCK,
          FIRST guest1 WHERE guest1.gastnr = akthdr1.gastnr NO-LOCK,
          FIRST akt-kont1 WHERE akt-kont1.gastnr = guest1.gastnr 
            AND akt-kont1.kontakt-nr = akthdr1.kontakt-nr NO-LOCK
...
PROCEDURE fill-compet:
  FOR EACH p-list:
    CREATE slcompet-list.
    BUFFER-COPY p-list TO slcompet-list.
  END.
END PROCEDURE.
