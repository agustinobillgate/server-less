...
FIND FIRST b-storno WHERE b-storno.bankettnr = resnr 
      AND b-storno.breslinnr = resline NO-ERROR.
...
DO:
  CASE curr-amd:
      ...
      DO:
          ...
          DO:
              b-storno.grund[7]  = fsl.grund[7].
              b-storno.usercode  = b-storno.usercode + "7:" + user-init + ":" + STRING(TODAY) + ":" + STRING(TIME) + ":" + "Change Amandment 7" + ";". ";".
          END.              
      END.
      WHEN "8" THEN
      DO:
...
FOR EACH b-storno WHERE b-storno.bankettnr = resnr 
  AND b-storno.breslinnr = resline AND b-storno.gastnr = curr-gastnr :
    CREATE bstorno.
