DEFINE TEMP-TABLE output-list 
  ...
  FIELD nr       AS INTEGER 
  FIELD STR      AS CHAR 
  FIELD budget   LIKE gl-acct.budget[1] LABEL "CurrMonth Budget" 
...
FIND FIRST gl-jouhdr WHERE gl-jouhdr.datum LE to-date NO-LOCK NO-ERROR.
            ...
            FOR EACH gl-main NO-LOCK, 
                ...
                FOR EACH gl-acct WHERE gl-acct.main-nr = gl-main.nr 
                    ...
                    IF p-bal NE 0 OR t-debit NE 0 OR t-credit NE 0 
                        ...
                        output-list.fibukonto = STRING(c, "x(16)").
                        ...
                        DO:
                           t-bal = y-bal.
                        END.
                        ELSE IF gl-acct.acc-type = 4 THEN
                        DO:
                            p-bal = -1 * p-bal.
                            t-bal = -1 * t-bal.
                        END.
                        ...
                        IF t-bal EQ 0 AND (t-debit EQ 0 AND t-credit EQ 0) THEN t-bal = p-bal. 
                ...
                FOR EACH gl-acct WHERE  gl-acct.acc-type = acct-type 
                    ...
                    IF p-bal NE 0 /*OR t-debit NE 0 OR t-credit NE 0*/ 
                        ...
                        IF output-list.budget NE 0 THEN 
                            output-list.proz = t-bal / output-list.budget * 100. 
                    END.
                    ELSE IF t-credit EQ 0 AND t-debit EQ 0 THEN
                    DO:
                        tot-diff = p-bal.
                    END.
                END. 
