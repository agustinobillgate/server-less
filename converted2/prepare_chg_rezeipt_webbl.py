...
def prepare_chg_rezeipt_webbl(pvilanguage:int, h_artnr:int, description:string):
    ...
    def generate_output():
        nonlocal h_bezeich, katbezeich, katnr, portion, price_type, amount, msg_str, msg_str2, msg_str3, record_use, init_time, init_date, cost_percent, poten_sell_price, r_list_data, s_rezlin_data, t_l_artikel_data, t_h_rezept_data, curr_i, flag_ok, lvcarea, h_rezept, l_artikel, htparam, h_rezlin, queasy
    ...
    def create_rezlin():
        ...
        if h_rezlin.recipe_flag == False:
            ...
            if h_recipe:    
                ...
                ccost =  to_decimal("0")
                ccost = cal_cost(h_recipe.artnrrezept, 1, ccost)
                s_rezlin.cost = ( to_decimal(ccost) / to_decimal(h_recipe.portion)) * to_decimal(h_rezlin.menge)
