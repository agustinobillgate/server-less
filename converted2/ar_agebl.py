...
def ar_agebl(pvilanguage:int, from_art:int, to_art:int, to_date:date, from_name:string, to_name:string, disptype:int):
    ...
    output_list_data, Output_list = create_model("Output_list", {"curr":string, "gastnr":int, "age1":string, "age2":string, "age3":string, "age4":string, "creditlimit":Decimal, "str":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal outlist, output_list_data, lvcarea, default_fcurr, day1, day2, day3, price_decimal, long_digit, htparam, debitor, artikel, guest, waehrung
        nonlocal pvilanguage, from_art, to_art, to_date, from_name, to_name, disptype


        nonlocal ledger, age_list, output_list
        nonlocal ledger_data, age_list_data, output_list_data

        return {"outlist": outlist, "output-list": output_list_data}
