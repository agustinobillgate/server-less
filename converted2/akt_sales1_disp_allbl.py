...
def akt_sales1_disp_allbl(aktnr:int):
    ...
    q1_list_data, Q1_list = create_model_like(Akthdr, {"akthdr_recid":int, "guest_name":string, "guest_anredefirma":string, "akt_kont_name":string, "akt_kont_vorname":string, "akt_kont_anrede":string, "akt_code_bezeich":string})
    q2_list_data, Q2_list = create_model_like(Akt_line, {"recid_akt_line":int, "akt_code_bezeich":string, "bediener_username":string})
    ...
    def generate_output():
        nonlocal q1_list_data, q2_list_data, akthdr, akt_code, akt_line, guest, akt_kont, bediener
        nonlocal aktnr
    ...
    akt_line_obj_list = {}
    for akt_line, akt_code1, bediener in db_session.query(Akt_line, Akt_code1, Bediener).join(Akt_code1,(Akt_code1.aktionscode == Akt_line.aktionscode)).join(Bediener,(Bediener.userinit == Akt_line.userinit)).filter(
