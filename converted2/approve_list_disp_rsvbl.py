...
def approve_list_disp_rsvbl(resnr:int, reslinnr:int, gastnr:int):
    ...
    b3_list_data, B3_list = create_model("B3_list", {"resnr":int, "ankunft":date, "abreise":date, "name":string, "vorname1":string, "zimmeranz":int, "kurzbez":string, "zinr":string, "resstatus":int, "erwachs":int, "kind1":int, "kind2":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal b3_list_data, guest, zimkateg, res_line
        nonlocal resnr, reslinnr, gastnr


        nonlocal b3_list
        nonlocal b3_list_data
