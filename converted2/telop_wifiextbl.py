    ...
    if case_type == 1:
        ...
        while None != interface:
            ...
            interface = db_session.query(Interface).filter(
                     (Interface.key == 9) & (Interface.zinr == res_line.zinr) & (Interface.decfield <= 3) & (Interface._recid > curr_recid)).first()
        ...
        while None != interface:
            ...
            curr_recid = interface._recid
            interface = db_session.query(Interface).filter(
                     (Interface.key == 10) & (Interface.zinr == res_line.zinr) & (Interface.decfield <= 10) & (Interface._recid > curr_recid)).first()
        ...
        if sameresno:
            ...
            for rbuff in db_session.query(Rbuff).filter(
                ...
                while None != interface:
                    ...
                    curr_recid = interface._recid
                    interface = db_session.query(Interface).filter(
                             (Interface.key == 10) & (Interface.zinr == rbuff.zinr) & (Interface.decfield <= 10) & (Interface._recid > curr_recid)).first()
