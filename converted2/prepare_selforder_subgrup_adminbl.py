...
def prepare_selforder_subgrup_adminbl():
    ...
    t_subgrp_data, T_subgrp = create_model("T_subgrp", {"key":int, "subgrp_no":int, "subgrp":string, "maingrp_no":int, "maingrp":string, "dept":int, "dept_str":string, "prior":int})
    t_maingrp_data, T_maingrp = create_model_like(Queasy)
    ...
    def generate_output():
        nonlocal t_subgrp_data, t_maingrp_data, t_dept_data, queasy, hoteldpt, wgrpdep
        nonlocal buf_queasy


        nonlocal t_subgrp, t_maingrp, t_dept, buf_queasy
        nonlocal t_subgrp_data, t_maingrp_data, t_dept_data

        return {"t-subgrp": t_subgrp_data, "t-maingrp": t_maingrp_data, "t-dept": t_dept_data}
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        t_maingrp = T_maingrp()
        t_maingrp_data.append(t_maingrp)
