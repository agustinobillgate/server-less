...
def allotmentbl(case_type:int, gastnr:int, kon_gastnr:int, kontcode:string, ankunft:date, abreise:date, zimmeranz:int):
    ...
    check_resline_list_data, Check_resline_list = create_model("Check_resline_list", {"resnr":int, "name":string, "abreise":date, "ankunft":date})
    ...
    def generate_output():
        nonlocal allotment_list_data, check_resline_list_data, allot_list_data, s_list_data, d1, d2, datum, d, kontline, bediener, zimkateg, res_line
        ...
        nonlocal allotment_list_data, check_resline_list_data, allot_list_data, s_list_data

        return {"allotment-list": allotment_list_data, "check-resline-list": check_resline_list_data, "allot-list": allot_list_data, "s-list": s_list_data}
    ...
    def assign_it():
        ...
        for kontline, bediener in db_session.query(Kontline, Bediener).join(Bediener,(Bediener.nr == Kontline.bediener_nr)).filter(
            ...
            allotment_list_data.append(allotment_list)
    ...
    def check_resline():
        ...
        for res_line, kontline in db_session.query(Res_line, Kontline).join(Kontline,(Kontline.kontignr == Res_line.kontignr) & (Kontline.kontcode == (kontcode).lower()) & (Kontline.kontstatus == 1)).filter(
            ...
            if res_line.abreise <= kontline.ankunft or res_line.ankunft > kontline.abreise:
                ...
                check_resline_list_data.append(check_resline_list)
