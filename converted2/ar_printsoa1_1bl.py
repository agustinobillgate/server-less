...
def ar_printsoa1_1bl(show_type:int, bof_month:date, eof_month:date, param_ar_data:[Param_ar], guestno:int, curr_day:date):
    ...
    def create_soalist():
        ...
        if show_type == 0:
            ...
            for soa_list in query(soa_list_data, filters=(lambda soa_list: soa_list.done_step == 0)):
                crdt =  to_decimal("0")
            ...
            for debitor, guest in db_session.query(Debitor, Guest).join(Guest,(Guest.gastnr == Debitor.gastnr)).filter(
                     (Debitor._recid.in_(list(set([soa_list.arrecid for soa_list in soa_list_data]))))).order_by(Debitor._recid).all():
                if debitor_obj_list.get(debitor._recid):
                    continue
                else:
                    debitor_obj_list[debitor._recid] = True

                soa_list = query(soa_list_data, (lambda soa_list: (debitor._recid == soa_list.arrecid)), first=True)
                crdt =  to_decimal("0")
                debt_amount =  to_decimal("0")
                fcrdt =  to_decimal("0")
                fdebt =  to_decimal("0")
