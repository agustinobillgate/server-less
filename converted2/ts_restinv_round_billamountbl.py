...
def ts_restinv_round_billamountbl(pvilanguage:int, price_decimal:int, curr_dept:int, rec_id:int, rest_amount:Decimal, double_currency:bool, exchg_rate:Decimal, transdate:date, cancel_flag:bool, foreign_rate:bool):
    ...
    def generate_output():
        nonlocal printed, billart, qty, description, price, fl_code, fl_code1, amount_foreign, amount, msg_str, t_h_artikel_data, lvcarea, gst_logic, h_artikel, htparam, h_bill, artikel, waehrung, exrate
        ...
        return {"printed": printed, "billart": billart, "qty": qty, "description": description, "price": price, "fl_code": fl_code, "fl_code1": fl_code1, "amount_foreign": amount_foreign, "amount": amount, "msg_str": msg_str, "t-h-artikel": t_h_artikel_data}
