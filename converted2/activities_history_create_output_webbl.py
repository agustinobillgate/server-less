...
b1_list_data, B1_list = create_model("B1_list", {"datum":string, "action":string, "aenderung":string, "username":string, "zeit":string})

def activities_history_create_output_webbl(idflag:string, b1_list_data:[B1_list]):
    doneflag = False
    counter:int = 0
    queasy = None
    ...
    def generate_output():
        ...
        nonlocal idflag, b1_list_data
        nonlocal bqueasy, pqueasy, tqueasy


        nonlocal b1_list, bqueasy, pqueasy, tqueasy

        return {"doneflag": doneflag, "b1-list": b1_list_data}
