...
def mk_hmenu_disp_itbl(hmenu_list_data:[Hmenu_list], menu_list_data:[Menu_list], sorttype:int, dept:int, from_artnr:int, from_bezeich:string):
    ...
    def generate_output():
        nonlocal q1_list_data, q2_list_data, h_menu, h_artikel
    ...
    if sorttype == 1:
        ...
        for h_art1 in db_session.query(H_art1).filter(
                 ((H_art1.departement.in_(list(set([hmenu_list.dept for hmenu_list in hmenu_list_data if hmenu_list.artnr >= from_artnr])))) & (H_art1.artnr == hmenu_list.artnr))).order_by(hmenu_list.artnr).all():
            if h_art1_obj_list.get(h_art1._recid):
                continue
            else:
                h_art1_obj_list[h_art1._recid] = True

            hmenu_list = query(hmenu_list_data, (lambda hmenu_list: (h_art1.departement == dept)), first=True)
            q1_list = Q1_list()
        ...
        h_art1_obj_list = {}
        for h_art1 in db_session.query(H_art1).filter(
                 ((H_art1.departement.in_(list(set([hmenu_list.dept for hmenu_list in hmenu_list_data])))) & (H_art1.artnr == hmenu_list.artnr) & (H_art1.bezeich >= (from_bezeich).lower()))).order_by(H_art1.bezeich).all():
            if h_art1_obj_list.get(h_art1._recid):
                continue
            else:
                h_art1_obj_list[h_art1._recid] = True
    ...
    for h_art2 in db_session.query(H_art2).filter(
             ((H_art2.departement.in_(list(set([menu_list.dept for menu_list in menu_list_data])))) & (H_art2.artnr == menu_list.artnr))).order_by(H_art2.zwkum, menu_list.artnr).all():
