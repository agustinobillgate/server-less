...
def stock_hohlist2_webbl(from_lager:int, to_lager:int, from_art:int, to_art:int, to_date:date, move_art:bool, end_notzero:bool, global_oh:bool, from_date:date):
    ...
    def generate_output():
        nonlocal stockoh_list_data, do_it, curr_art, curr_lager, out_bestand, zwkum, t_val, bezeich, j, tot_prev, tot_value, tot_in, tot_out, total_in, total_out, total_prev, total_value, start_from_date, loop, loop_date, diff_month, long_digit, htparam, l_artikel, l_besthis, l_lager, l_ophis, l_untergrup
    ...
    def create_stock1():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            for l_besthis in db_session.query(L_besthis).filter(
                     (L_besthis.lager_nr == l_lager.lager_nr) & (L_besthis.anf_best_dat == start_from_date) & (L_besthis.artnr >= from_art) & (L_besthis.artnr <= to_art)).order_by(L_besthis._recid).all():
                ...
                if t_vkpreis:
                    ...
                    if do_it:
                        ...
                        for l_ophis in db_session.query(L_ophis).filter(
                                 (L_ophis.lager_nr == l_lager.lager_nr) & (L_ophis.artnr == l_besthis.artnr) & (L_ophis.datum >= start_from_date) & (L_ophis.datum <= to_date) & (not_(matches(L_ophis.fibukonto,"*;CANCELLED*")))).order_by(L_ophis.datum).all():
