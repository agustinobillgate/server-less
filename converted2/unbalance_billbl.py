...
def unbalance_billbl():
    ...
    bill_list_data, Bill_list = create_model("Bill_list", {"rechnr":int, "resnr":int, "reslinnr":int, "resv_name":string, "gname":string, "ci":string, "co":string, "rmno":string, "balanced":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal bill_list_data, bill_date, res_line, guest, bill


        nonlocal bill_list
        nonlocal bill_list_data

        return {"bill-list": bill_list_data}
