...
def mk_po_btn_go1_web_1bl(t_l_orderhdr_data:[T_l_orderhdr], t_l_order_data:[T_l_order], docu_nr:string, lief_nr:int, billdate:date, create_new:bool, pr:string, globaldisc:Decimal, currency_screen_value:string, zeroprice_flag:bool):
    ...
    for t_l_order in query(t_l_order_data, filters=(lambda t_l_order: t_l_order.pos > 0)):
        ...
        if l_order:
            ...
            if t_l_order.disc_val == 0 and t_l_order.disc != 0:
                t_l_order.disc_val = to_decimal(round(t_l_order.brutto , 2) * (t_l_order.disc / 100))

            if t_l_order.disc2_val == 0 and t_l_order.disc2 != 0:
                t_l_order.disc2_val = to_decimal(round(t_l_order.brutto , 2) * (t_l_order.disc2 / 100))

            if t_l_order.vat_val == 0 and t_l_order.vat != 0:
                t_l_order.vat_val = (to_decimal(round(t_l_order.brutto , 2) - t_l_order.disc_val - t_l_order.disc2_val) * (t_l_order.vat / 100))
