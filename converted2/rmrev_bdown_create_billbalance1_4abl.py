...
def rmrev_bdown_create_billbalance1_4abl(exc_taxserv:bool, pvilanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, curr_date:date, srttype:int):
    ...
    def generate_output():
        nonlocal cl_list_data, currency_list_data, sum_list_data, s_list_data, argt_list_data, exchg_rate, frate, post_it, total_rev, temptotjur, lvcarea, argt_line, billjournal, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, bill, zimkateg, reslin_queasy
        ...
        nonlocal sum_list_data, currency_list_data, cl_list_data, s_list_data, argt_list_data, t_argt_line_data, t_billjournal_data
                    ...
                    if bill_flag2.lower()  == ("Guest Bill").lower() :
                        ...
                        for billjournal, artikel in db_session.query(Billjournal, Artikel).join(Artikel,(Artikel.artnr == Billjournal.artnr) & (Artikel.departement == Billjournal.departement) & (Artikel.artart != 9)).filter(
                            ...
                            if billjournal.artnr != deposit_art:
                                ...
                                t_billjournal = query(t_billjournal_data, filters=(lambda t_billjournal: t_billjournal.rechnr == billjournal.rechnr and t_billjournal.artnr == billjournal.artnr and t_billjournal.zinr == billjournal.zinr), first=True)
    ...
    def get_genstat_argt_betrag(frate:Decimal, argt_betrag:Decimal):
        ...
        nonlocal cl_list_data, currency_list_data, sum_list_data, s_list_data, argt_list_data, exchg_rate, post_it, total_rev, temptotjur, lvcarea, argt_line, billjournal, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, bill, zimkateg, reslin_queasy
        nonlocal exc_taxserv, pvilanguage, new_contrate, foreign_rate, price_decimal, curr_date, srttype
        nonlocal waehrung1, cc_list
