#using conversion tools version: 1.0.0.117

from functions.additional_functions import *
from decimal import Decimal
from datetime import date
from models import Mathis, Fa_grup, Htparam

def prepare_fa_movelistbl(bill_date:date):

    prepare_cache ([Htparam])

    fdate = None
    tdate = None
    t_fa_grup_data = []
    t_mathis_data = []
    mathis = fa_grup = htparam = None

    t_mathis = t_fa_grup = None

    t_mathis_data, T_mathis = create_model_like(Mathis)
    t_fa_grup_data, T_fa_grup = create_model_like(Fa_grup)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal fdate, tdate, t_fa_grup_data, t_mathis_data, mathis, fa_grup, htparam
        nonlocal bill_date


        nonlocal t_mathis, t_fa_grup
        nonlocal t_mathis_data, t_fa_grup_data

        return {"fdate": fdate, "tdate": tdate, "t-fa-grup": t_fa_grup_data, "t-mathis": t_mathis_data}


    htparam = get_cache (Htparam, {"paramnr": [(eq, 110)]})

    if htparam:
        bill_date = htparam.fdate
    fdate = date_mdy(get_month(bill_date) , 1, get_year(bill_date))
    tdate = fdate - timedelta(days=1)

    for fa_grup in db_session.query(Fa_grup).order_by(Fa_grup._recid).all():
        t_fa_grup = T_fa_grup()
        t_fa_grup_data.append(t_fa_grup)

        buffer_copy(fa_grup, t_fa_grup)

    for mathis in db_session.query(Mathis).order_by(Mathis._recid).yield_per(100):
        t_mathis = T_mathis()
        t_mathis_data.append(t_mathis)

        buffer_copy(mathis, t_mathis)
        break

    return generate_output()