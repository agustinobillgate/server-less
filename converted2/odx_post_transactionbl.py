...
def odx_post_transactionbl(from_date:date, to_date:date):
    ...
    summary_bill_data, Summary_bill = create_model("Summary_bill", {"datum":date, "times":string, "department":int, "rechnr":int, "total_food":Decimal, "total_bev":Decimal, "total_other":Decimal, "total_service":Decimal, "total_tax":Decimal, "total_disc":Decimal, "total_tips":Decimal, "total_amount":Decimal, "post_result":string, "body":string, "response":string, "guest_name":string, "res_no":int, "room_no":string})
    datalist_data, Datalist = create_model("Datalist", {"vkey":string, "vvalue":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal summary_bill_data, loop_i, messtaken, messkeyword, messvalue, response_char, response_code, currdata, time_second, queasy
        nonlocal from_date, to_date


        nonlocal summary_bill, datalist
        nonlocal summary_bill_data, datalist_data

        return {"summary-bill": summary_bill_data}
    ...
    def get_data(vkey:string):
        ...
        datalist = query(datalist_data, filters=(lambda datalist: datalist.dataList.vkey == vkey), first=True)
