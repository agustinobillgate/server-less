...
def selforder_getparamsbl(session_parameter:string):
    ...
    selforder_setup_data = []
    queasy = None

    selforder_setup = None

    selforder_setup_data, Selforder_setup = create_model("Selforder_setup", {"nr":int, "setup_keyword":string, "setup_value":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal department_number, table_no, guest_name, pax, room, checkin_date, checkout_date, package, mess_result, session_expired, selforder_setup_data, queasy
        nonlocal session_parameter


        nonlocal selforder_setup
        nonlocal selforder_setup_data

        return {"department_number": department_number, "table_no": table_no, "guest_name": guest_name, "pax": pax, "room": room, "checkin_date": checkin_date, "checkout_date": checkout_date, "package": package, "mess_result": mess_result, "session_expired": session_expired, "selforder-setup": selforder_setup_data}
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        selforder_setup_data.append(selforder_setup)
