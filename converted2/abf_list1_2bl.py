...
from models import Res_line, Genstat, Arrangement, Artikel, Argt_line, Reslin_queasy, Fixleist, Guest, Reservation, Segment, Ratecode, Bill, Master

zikat_list_data, Zikat_list = create_model("Zikat_list", {"selected":bool, "zikatnr":int, "kurzbez":string, "bezeich":string})

def abf_list1_2bl(fdate:date, bfast_artnr:int, bfast_dept:int, zikat_list_data:[Zikat_list]):

    prepare_cache ([Res_line, Arrangement, Argt_line, Fixleist, Guest, Reservation, Segment, Ratecode])

    abf_list_data = []
    res_line = genstat = arrangement = artikel = argt_line = reslin_queasy = fixleist = guest = reservation = segment = ratecode = bill = master = None
    ...
    def generate_output():
        nonlocal abf_list_data, res_line, genstat, arrangement, artikel, argt_line, reslin_queasy, fixleist, guest, reservation, segment, ratecode, bill, master
