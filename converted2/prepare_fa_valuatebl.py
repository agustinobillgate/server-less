...
def prepare_fa_valuatebl():
    ...
    t_fa_grup_data, T_fa_grup = create_model_like(Fa_grup)
    t_fa_lager_data, T_fa_lager = create_model_like(Fa_lager)
    lagerbuff_data, Lagerbuff = create_model_like(Fa_lager)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal price_decimal, last_acctdate, mm, yy, from_month, maxnr, p_977, p_224, lagerbuff_data, t_fa_grup_data, t_fa_lager_data, fa_grup, fa_lager, htparam


        nonlocal t_fa_grup, t_fa_lager, lagerbuff
        nonlocal t_fa_grup_data, t_fa_lager_data, lagerbuff_data

        return {"price_decimal": price_decimal, "last_acctdate": last_acctdate, "mm": mm, "yy": yy, "from_month": from_month, "maxnr": maxnr, "p_977": p_977, "p_224": p_224, "lagerBuff": lagerbuff_data, "t-fa-grup": t_fa_grup_data, "t-fa-lager": t_fa_lager_data}
    ...
    def load_fa_lager():
        ...
        for fa_lager in db_session.query(Fa_lager).order_by(Fa_lager.lager_nr).all():
            lagerbuff = Lagerbuff()
            lagerbuff_data.append(lagerbuff)
