...
def ar_subledger_prepare_disp_fobillbl(a_rechnr:int):
    ...
    t_bill_line_data, T_bill_line = create_model_like(Bill_line, {"rec_id":int, "run_disp_restbill":bool})
    t_blinehis_data, T_blinehis = create_model_like(Blinehis, {"rec_id":int, "run_disp_restbill":bool})
    ...
    def generate_output():
        nonlocal t_bill_line_data, t_blinehis_data, bill_line, blinehis, artikel
        nonlocal a_rechnr
        nonlocal foart


        nonlocal t_bill_line, t_blinehis, foart
        nonlocal t_bill_line_data, t_blinehis_data

        return {"t-bill-line": t_bill_line_data, "t-blinehis": t_blinehis_data}
    ...
    if bill_line:
        ...
        for blinehis in db_session.query(Blinehis).filter(
            ...
            t_blinehis_data.append(t_blinehis)
