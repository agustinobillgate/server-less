...
payload_list_data, Payload_list = create_model("Payload_list", {"t_lschein":string, "t_datum":string, "t_amount":Decimal, "lscheinnr":string})

def chg_storerequest_read_data_webbl(payload_list_data:[Payload_list]):
    ...
    response_list_data, Response_list = create_model("Response_list", {"t_amount":Decimal, "lscheinnr":string, "curr_lager":int, "deptno":int, "transfered":bool, "out_type":int, "to_stock":int, "deptname":string, "lager_bezeich":string, "lager_bez1":string, "curr_pos":int, "sr_remark":string})
    ...
    def generate_output():
        nonlocal t_lschein, tmp_datum, num_year, t_amount, lscheinnr, curr_lager, deptno, transfered, out_type, to_stock, deptname, lager_bezeich, lager_bez1, curr_pos, sr_remark, op_list_data, response_list_data, l_op, bediener, parameters, l_lager, l_artikel, l_bestand, gl_acct, queasy
        nonlocal sys_user


        nonlocal op_list, payload_list, response_list, sys_user
        nonlocal op_list_data, response_list_data
    ...
    def read_data():
        ...
        l_op = get_cache (L_op, {"datum": [(eq, tmp_datum)],"lscheinnr": [(eq, t_lschein)],"pos": [(gt, 0)]})
        ...
        if l_op:
            ...
            for l_op in db_session.query(L_op).filter(
                     (L_op.datum == tmp_datum) & (L_op.lscheinnr == (t_lschein).lower()) & (L_op.pos > 0) & (L_op.loeschflag == 0)).order_by(L_op.pos).all():
                op_list = Op_list()
    ...
    if payload_list:
        ...
        num_year = 2000 + to_int(substring(payload_list.t_datum, 6, 2))
        tmp_datum = date_mdy(to_int(substring(payload_list.t_datum, 0, 2)) , to_int(substring(payload_list.t_datum, 3, 2)) , num_year)
        read_data()
