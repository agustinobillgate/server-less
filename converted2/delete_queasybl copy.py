...
def delete_queasybl(case_type:int, t_queasy_list:[T_queasy]):
    ...
    if case_type == 1:
        ...
        if not queasy or not(queasy.key == t_queasy.key and queasy.char1 == t_queasy.char1):
        ...
        if not queasy or not(queasy.key == t_queasy.key and queasy.number1 == t_queasy.number1 and queasy.char1 == t_queasy.char1):
            queasy = db_session.query(Queasy).filter(
