...
def ts_splitbill_cal_balance_webbl(curr_dept: int, curr_select: int, t_rechnr: int):
    ...
    ordered_item_data, Ordered_item = create_model(
        "Ordered_item",
        {
            "dept": int,
            "artnr": int,
    ...
    summary_bill_data, Summary_bill = create_model(
        "Summary_bill",
        {
            "subtotal": Decimal,
    ...
    t_h_artikel_data, T_h_artikel = create_model_like(
        H_artikel,
        {
            "rec_id": int
        })
    ...
    def generate_output():
        ...
        return {
            ...
            "summary-bill": summary_bill_data,
    ...
    if htparam:
        if not htparam.flogical and entry(0, htparam.fchar, ";").lower == "gst(ma)":
            gst_logic = True
    ...
    for ordered_item in query(ordered_item_data):
        ...
        h_artikel = get_cache(
            H_artikel, {"departement": [(eq, ordered_item.dept)], "artnr": [(eq, ordered_item.artnr)], "artart": [(eq, 0)]})

        if h_artikel:
            netto_bet = to_decimal(
                netto_bet) + to_decimal((ordered_item.epreis) * to_decimal(ordered_item.qty))
        ...
        if h_artikel:
            if ordered_item.artnr != f_discart:
                ...
                if vat_code != 0 and not incl_mwst:
                    ...
                    if htparam and htparam.fdecimal != 0:
                        if num_entries(htparam.fchar, chr_unicode(2)) >= 2:
                            t_h_mwst = to_decimal(to_decimal(
                                entry(1, htparam.fchar, chr_unicode(2)))) / to_decimal("10000")
                        ...
                        if serv_vat and not tax_vat:
                            t_h_mwst = to_decimal(
                                t_h_mwst) + to_decimal(t_h_mwst) * to_decimal(t_h_service) / to_decimal("100")

                        elif serv_vat and tax_vat:
                            t_h_mwst = to_decimal(t_h_mwst) + to_decimal(t_h_mwst) * to_decimal(
                                (t_h_service) + to_decimal(t_h_mwst2)) / to_decimal("100")

                        elif not serv_vat and tax_vat:
                            t_h_mwst = to_decimal(
                                t_h_mwst) + to_decimal(t_h_mwst) * to_decimal(t_h_mwst2) / to_decimal("100")
                ...
                if t_h_service != 0 or t_h_mwst != 0:
                    ...
                    fact_scvat = to_decimal(
                        "1") + to_decimal(t_h_service) + to_decimal(t_h_mwst) + to_decimal(t_h_mwst2)
                    h_service = to_decimal(
                        ordered_item.betrag) / to_decimal(fact_scvat) * to_decimal(t_h_service)
                    h_service = to_decimal(round(h_service, 2))
                    h_mwst = to_decimal(ordered_item.betrag) / \
                        to_decimal(fact_scvat) * to_decimal(t_h_mwst)
                ...
                if ordered_item.epreis != ordered_item.betrag:
                    if t_h_service != 0 or t_h_mwst != 0:
                        t_h_service = to_decimal(
                            t_h_service) / to_decimal("100")
                        t_h_mwst = to_decimal(t_h_mwst) / to_decimal("100")
                        t_h_mwst2 = to_decimal(t_h_mwst2) / to_decimal("100")
                        ...
                        h_service = to_decimal(round(h_service, 2))
                        h_mwst = to_decimal(
                            ordered_item.betrag) / to_decimal(fact_scvat) * to_decimal(t_h_mwst)
                        h_mwst = to_decimal(round(h_mwst, 2))

                        if not incl_service:
                            service = to_decimal(
                                service) + to_decimal(h_service)
    ...
    grand_tot = to_decimal(sub_tot) + \
        to_decimal(tot_serv) + to_decimal(tot_tax)
