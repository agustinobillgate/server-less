...
def create_forecast_historybl(fr_date:date, to_date:date, excl_comp:bool, vhp_limited:bool):

    prepare_cache ([Htparam, Res_line, Kontline, Guest, Nation, Genstat, Waehrung, Reservation, Sourccod, Arrangement, Queasy, Reslin_queasy])
    ...
    t_list_data, T_list = create_model("T_list", {"resnr":int, "reslinnr":int, "logis_guaranteed":Decimal, "bfast_guaranteed":Decimal, "lunch_guaranteed":Decimal, "dinner_guaranteed":Decimal, "misc_guaranteed":Decimal, "pax_guaranteed":int, "room_guaranteed":int, "logis_tentative":Decimal, "bfast_tentative":Decimal, "lunch_tentative":Decimal, "dinner_tentative":Decimal, "misc_tentative":Decimal, "pax_tentative":int, "room_tentative":int, "rsv_karteityp":int, "rsv_name":string, "rsv_nationnr":int, "guest_karteityp":int, "guest_name":string, "guest_nationnr":int, "sob":int, "resstatus":int, "currency":string, "zipreis":Decimal, "flag_history":bool, "firmen_nr":int, "steuernr":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_list_data, ci_date, htparam, res_line, kontline, guest, nation, genstat, segment, waehrung, reservation, sourccod, arrangement, bill_line, zimmer, queasy, reslin_queasy
        nonlocal fr_date, to_date, excl_comp, vhp_limited
    ...
    def create_umsatz():
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            if do_it:
                ...
                t_list.misc_guaranteed =  to_decimal(t_list.misc_guaranteed) + to_decimal(genstat.res_deci[4] + genstat.res_deci[5])
