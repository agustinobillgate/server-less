...
def max_onhand_btn_go2bl(sorttype:int, main_grp:int, from_store3:int, to_store3:int, show_price:bool):
    ...
    max_onhand_list_data = []
    long_digit:bool = False
    htparam = l_lager = l_artikel = l_bestand = l_pprice = None

    max_onhand_list = None

    max_onhand_list_data, Max_onhand_list = create_model("Max_onhand_list", {"artnr":int, "name":string, "max_oh":Decimal, "curr_oh":Decimal, "avrgprice":Decimal, "ek_aktuell":Decimal, "datum":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal max_onhand_list_data, long_digit, htparam, l_lager, l_artikel, l_bestand, l_pprice
        nonlocal sorttype, main_grp, from_store3, to_store3, show_price


        nonlocal max_onhand_list
        nonlocal max_onhand_list_data

        return {"max-onhand-list": max_onhand_list_data}
                ...
                if l_bestand:
                    ...
                    if curr_best > l_artikel.anzverbrauch:
                        ...
                        if i == 1:
                            ...
                            max_onhand_list_data.append(max_onhand_list)
