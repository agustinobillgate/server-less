...
def check_gledger_show_lodgingbl(currdate:date):
    ...
    lod_list_data, Lod_list = create_model("Lod_list", {"resnr":int, "reslinnr":int, "zinr":string, "rmrate":Decimal, "lodge":Decimal, "bfast":Decimal, "tot":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal lod_list_data, bill, bill_line, billjournal, res_line, arrangement, argt_line
        nonlocal currdate


        nonlocal lod_list
        nonlocal lod_list_data

        return {"lod-list": lod_list_data}
