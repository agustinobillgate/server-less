...
def prepare_sales_activity_listbl(resnr:int):
    ...
    print_list_data, Print_list = create_model("Print_list", {"guest":string, "refno":string, "resstatus":string, "rechnr":string, "datum":string, "zeit":string})
    t_b_storno_data, T_b_storno = create_model_like(B_storno)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal counter_reason, output_list_data, print_list_data, b_storno, bk_reser, bk_veran, guest
        nonlocal resnr


        nonlocal output_list, print_list, t_b_storno
        nonlocal output_list_data, print_list_data, t_b_storno_data

        return {"counter_reason": counter_reason, "output-list": output_list_data, "print-list": print_list_data}
    ...
    def create_outlist():
        ...
        for i in range(1,18 + 1) :
            ...
            if b_storno.grund[i - 1] != "":
                ...
                print_list = query(print_list_data, first=True)
    ...
    t_b_storno = query(t_b_storno_data, first=True)
