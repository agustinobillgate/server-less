...
def tada_dashboard_getparam_1bl(dept:int):
    ...
    t_dept = t_status = crd_list = t_queasy270 = qsy230 = session_table = posted_item = None

    t_dept_data, T_dept = create_model("T_dept", {"nr":int, "dept":string})
    t_status_data, T_status = create_model("T_status", {"nr":int, "status_str":string})
    ...
    def generate_output():
        nonlocal t_dept_data, t_queasy270_data, licensenr, interval_time, cancel_exist, t_status_data, crd_list_data, pax, orderdatetime, gname, room, gastnr, resnr, reslinnr, mess_str, i_str, mess_token, mess_keyword, mess_value, table_no, dtime, queasy, hoteldpt, paramtext
        nonlocal dept
        nonlocal qsy230, session_table, posted_item


        nonlocal t_dept, t_status, crd_list, t_queasy270, qsy230, session_table, posted_item
        nonlocal t_dept_data, t_status_data, crd_list_data, t_queasy270_data

        return {"t-dept": t_dept_data, "t-queasy270": t_queasy270_data, "licensenr": licensenr, "interval_time": interval_time, "cancel_exist": cancel_exist, "t-status": t_status_data, "crd-list": crd_list_data}
