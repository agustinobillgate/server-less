...
def prepare_select_glacctbl(curr_dept:int):
    ...
    glacct_list_data, Glacct_list = create_model("Glacct_list", {"fibukonto":string, "bezeich":string, "acc_type":int, "deptnr":int, "subdept_nr":int, "subdept_bez":string, "main_nr":int, "activeflag":bool})
    gl_depart_list_data, Gl_depart_list = create_model_like(Gl_department)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal from_fibu, glacct_list_data, gl_depart_list_data, t_queasy_data, queasy, gl_department, htparam, gl_acct
        nonlocal curr_dept


        nonlocal t_queasy, glacct_list, gl_depart_list
        nonlocal t_queasy_data, glacct_list_data, gl_depart_list_data

        return {"from_fibu": from_fibu, "glacct-list": glacct_list_data, "gl-depart-list": gl_depart_list_data, "t-queasy": t_queasy_data}
