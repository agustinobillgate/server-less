...
from models import Waehrung, Genstat, Guest, Htparam, Nation, Queasy, Artikel, exrate, Bill, Bill_line, Guest_queasy, Res_line, Reservation, Sourccod, Segment, Arrangement, Zimmer

def export_topgcf_getturnoverbl(cardtype:int, sort_type:int, curr_sort1:int, fdate:date, tdate:date, check_ftd:bool, currency:string, excl_other:bool, curr_sort2:int):

    prepare_cache ([Waehrung, Genstat, Guest, Htparam, Nation, Queasy, Artikel, exrate, Guest_queasy, Res_line, Reservation, Arrangement])
    ...
    waehrung = genstat = guest = htparam = nation = queasy = artikel = exrate = bill = bill_line = guest_queasy = res_line = reservation = sourccod = segment = arrangement = zimmer = None
    ...
    def generate_output():
        nonlocal cust_list_data, bfast_art, lunch_art, dinner_art, lundin_art, service, vat, datum, end_date, net_lodg, fnet_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, curr_i, i, found, ly_fdate, ly_tdate, ci_date, pos, curr_gastnr, curr_resnr, curr_reslinnr, t_logiernachte, t_argtumsatz, t_fb_umsatz, t_sonst_umsatz, t_ba_umsatz, t_gesamtumsatz, tot_logiernachte, tot_argtumsatz, tot_fb_umsatz, tot_sonst_umsatz, tot_ba_umsatz, tot_gesamtumsatz, tot_stayno, loopj, found_it, exratenr, exrate, curr_resnr2, curr_reslinnr2, curr_resnr1, curr_reslinnr1, waehrung, genstat, guest, htparam, nation, queasy, artikel, exrate, bill, bill_line, guest_queasy, res_line, reservation, sourccod, segment, arrangement, zimmer
    ...
    def create_forecast():
        ...
        nonlocal cust_list_data, bfast_art, lunch_art, dinner_art, lundin_art, service, vat, end_date, net_lodg, fnet_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, curr_i, i, found, ly_fdate, ly_tdate, ci_date, pos, curr_gastnr, t_logiernachte, t_argtumsatz, t_fb_umsatz, t_sonst_umsatz, t_ba_umsatz, t_gesamtumsatz, tot_logiernachte, tot_argtumsatz, tot_fb_umsatz, tot_sonst_umsatz, tot_ba_umsatz, tot_gesamtumsatz, tot_stayno, loopj, found_it, exratenr, exrate, curr_resnr2, curr_reslinnr2, curr_resnr1, curr_reslinnr1, waehrung, genstat, guest, htparam, nation, queasy, artikel, exrate, bill, bill_line, guest_queasy, res_line, reservation, sourccod, segment, arrangement, zimmer
        nonlocal cardtype, sort_type, curr_sort1, fdate, tdate, check_ftd, currency, excl_other, curr_sort2
        nonlocal t_genstat, blist, glist, clist
                        ...
                        if curr_resnr != res_line.resnr or curr_reslinnr != res_line.reslinnr:
                            ...
                            for bill in db_session.query(Bill).filter(
                                ...
                                for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & (Artikel.artart == 0)).filter(
                                    ...
                                    if currency != " ":
                                        cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal((bill_line.betrag) / to_decimal(exrate) )
                                        cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal((bill_line.betrag) / to_decimal(exrate) )


                                    else:
                                        cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal(bill_line.betrag)
                                        cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal(bill_line.betrag)
                ...
                if res_line:
                    ...
                    if cust_list:
                        ...
                        if found_it :
                            ...
                            if currency != " ":
                                cust_list.f_b_umsatz =  to_decimal(cust_list.f_b_umsatz) + to_decimal(((guest_queasy.deci1) + to_decimal(guest_queasy.deci2)) / to_decimal(exrate) )
                                cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal((guest_queasy.deci3) / to_decimal(exrate) )
                                cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal(((guest_queasy.deci1) + to_decimal(guest_queasy.deci2) + to_decimal(guest_queasy.deci3)) / to_decimal(exrate) )


                        else:
                            cust_list.f_b_umsatz =  to_decimal(cust_list.f_b_umsatz) + to_decimal(guest_queasy.deci1) + to_decimal(guest_queasy.deci2)
                            cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal(guest_queasy.deci3)
                            cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal(guest_queasy.deci1) + to_decimal(guest_queasy.deci2) + to_decimal(guest_queasy.deci3)
    ...
    def cr_ftd():
        ...
        nonlocal cust_list_data, bfast_art, lunch_art, dinner_art, lundin_art, service, vat, datum, end_date, net_lodg, fnet_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, curr_i, i, found, ly_fdate, ly_tdate, ci_date, pos, curr_gastnr, t_logiernachte, t_fb_umsatz, t_sonst_umsatz, t_ba_umsatz, t_gesamtumsatz, tot_logiernachte, tot_argtumsatz, tot_fb_umsatz, tot_sonst_umsatz, tot_ba_umsatz, tot_gesamtumsatz, tot_stayno, loopj, found_it, exratenr, exrate, curr_resnr2, curr_reslinnr2, curr_resnr1, curr_reslinnr1, waehrung, genstat, guest, htparam, nation, queasy, artikel, exrate, bill, bill_line, guest_queasy, res_line, reservation, sourccod, segment, arrangement, zimmer
        nonlocal cardtype, sort_type, curr_sort1, fdate, tdate, check_ftd, currency, excl_other, curr_sort2
        nonlocal t_genstat, blist, glist, clist
                    ...
                    for bill in db_session.query(Bill).filter(
                        ...
                        for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & (Artikel.artart == 0)).filter(
                            ...
                            if currency != " ":
                                ...
                                if exrate:
                                    cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal((bill_line.betrag) / to_decimal(exrate.betrag) )
                                    cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal((bill_line.betrag) / to_decimal(exrate.betrag) )
                    ...
                    if cust_list:
                        ...
                        if found_it :
                            ...
                            if currency != " ":
                                ...
                                if exrate:
                                    cust_list.f_b_umsatz =  to_decimal(cust_list.f_b_umsatz) + to_decimal(((guest_queasy.deci1) + to_decimal(guest_queasy.deci2)) / to_decimal(exrate.betrag) )
                                    cust_list.sonst_umsatz =  to_decimal(cust_list.sonst_umsatz) + to_decimal((guest_queasy.deci3) / to_decimal(exrate.betrag) )
                                    cust_list.gesamtumsatz =  to_decimal(cust_list.gesamtumsatz) + to_decimal(((guest_queasy.deci1) + to_decimal(guest_queasy.deci2) + to_decimal(guest_queasy.deci3)) / to_decimal(exrate.betrag) )
