#using conversion tools version: 1.0.0.85
...
def ta_nationstat_go_listbl(from_date:str, last_period:str, sorttype:int, text2:str, text3:str):
    ...
    def create_list():
        ...
        genstat = Genstat()
        guest = Guest()
        for genstat.gastnr, genstat.argt, genstat.datum, genstat.logis, genstat.ratelocal, genstat._recid, guest.land, guest.name, guest.anredefirma, guest._recid in db_session.query(Genstat.gastnr, Genstat.argt, Genstat.datum, Genstat.logis, Genstat.ratelocal, Genstat._recid, Guest.land, Guest.name, Guest.anredefirma, Guest._recid).join(Guest,(Guest.gastnr == Genstat.gastnr)).filter(
