...
from models import Argt_line, Res_line, Htparam, Waehrung, Kontline, Zimmer, Genstat, Guest, Zimkateg, Segment, Exrate, Fixleist, Artikel, Reservation, Arrangement, Bill_line, Queasy, Reslin_queasy, Guestseg, Zinrstat, Outorder, Zkstat, Umsatz
...
def gcf_list_webbl_sy(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):

    prepare_cache ([Res_line, Htparam, Waehrung, Zimmer, Genstat, Exrate, Fixleist, Artikel, Reservation, Arrangement, Queasy, Reslin_queasy, Zinrstat, Outorder, Zkstat, Umsatz])
    ...
    curr_time:int = 0
    tmpint:int = 0
    argt_line = res_line = htparam = waehrung = kontline = zimmer = genstat = guest = zimkateg = segment = exrate = fixleist = artikel = reservation = arrangement = bill_line = queasy = reslin_queasy = guestseg = zinrstat = outorder = zkstat = umsatz = None

    room_list = segm_list = argt_list = zikat_list = outlook_list = print_list = print_list2 = print_list3 = argtline_list = art_list = art_vatlist = exrate_list = rline1 = active_rm_list = dayuse_list = s_list = a_list = z_list = o_list = bsegm = bargt = broom = s_list = a_list = z_list = o_list = s_list = z_list = None

    room_list_list, Room_list = create_model("Room_list", {"wd":int, "datum":date, "bezeich":string, "room":[Decimal,17], "coom":[string,17], "k_pax":int, "t_pax":int, "lodg":[Decimal,7], "avrglodg":Decimal, "avrglodg2":Decimal, "avrgrmrev":Decimal, "avrgrmrev2":Decimal, "others":[Decimal,8], "ly_fcast":string, "ly_actual":string, "ly_avlodge":string, "room_exccomp":int, "room_comp":int, "fixleist":Decimal, "fixleist2":Decimal, "rmrate":Decimal, "rmrate2":Decimal, "revpar":Decimal, "revpar2":Decimal, "avrglodg_inclcomp":Decimal, "avrglodg_exclcomp":Decimal, "rmocc_exclcomp":Decimal})
    print_list_list, Print_list = create_model("Print_list", {"code_name":string})
    print_list2_list, Print_list2 = create_model("Print_list2", {"argm":string})
    print_list3_list, Print_list3 = create_model("Print_list3", {"room":string})
    argtline_list_list, Argtline_list = create_model_like(Argt_line)
    art_list_list, Art_list = create_model("Art_list", {"artnr":int, "service_code":int, "mwst_code":int})
    art_vatlist_list, Art_vatlist = create_model("Art_vatlist", {"artnr":int, "dept":int, "datum":date, "service":Decimal, "vat":Decimal})
    exrate_list_list, Exrate_list = create_model("Exrate_list", {"datum":date, "exchg_rate":Decimal})
    ...
    def generate_output():
        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, datum, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, curr_time, tmpint, argt_line, res_line, htparam, waehrung, kontline, zimmer, genstat, guest, zimkateg, segment, exrate, fixleist, artikel, reservation, arrangement, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        ...
        nonlocal room_list, segm_list, argt_list, zikat_list, outlook_list, print_list, print_list2, print_list3, argtline_list, art_list, art_vatlist, exrate_list, rline1, active_rm_list, dayuse_list, s_list, a_list, z_list, o_list, bsegm, bargt, broom, s_list, a_list, z_list, o_list, s_list, z_list
        nonlocal room_list_list, print_list_list, print_list2_list, print_list3_list, argtline_list_list, art_list_list, art_vatlist_list, exrate_list_list, active_rm_list_list, dayuse_list_list

        return {"room-list": room_list_list}

    def create_browse():

        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, curr_time, tmpint, argt_line, res_line, htparam, waehrung, kontline, zimmer, genstat, guest, zimkateg, segment, exrate, fixleist, artikel, reservation, arrangement, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
        ...
        t_room_comp:int = 0
        tmp_date:date = None
        global_allotment_exist:bool = False
        from_ooo:date = None
        to_ooo:date = None
        tmax:int = 0
        tmin:int = 0
        ...
        kontline = get_cache (Kontline, {"betriebsnr": [(eq, 1)],"kontstatus": [(eq, 1)]})
        global_allotment_exist = None != kontline
        ...
        kontline = get_cache (Kontline, {"betriebsnr": [(eq, 0)],"kontstatus": [(eq, 1)]})
        ...
        genstat = Genstat()
        for genstat.gastnr, genstat.zikatnr, genstat.wahrungsnr, genstat.segmentcode, genstat.zinr, genstat.argt, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.res_date, genstat.datum, genstat.resstatus, genstat.res_logic, genstat.logis, genstat.ratelocal, genstat.res_deci, genstat.resnr, genstat.res_int, genstat._recid in db_session.query(Genstat.gastnr, Genstat.zikatnr, Genstat.wahrungsnr, Genstat.segmentcode, Genstat.zinr, Genstat.argt, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.res_date, Genstat.datum, Genstat.resstatus, Genstat.res_logic, Genstat.logis, Genstat.ratelocal, Genstat.res_deci, Genstat.resnr, Genstat.res_int, Genstat._recid).filter(
            ...
            if do_it:
                ...
                if room_list:
                    ...
                    for fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt, fixleist.betrag, fixleist.number, fixleist._recid in db_session.query(Fixleist.artnr, Fixleist.departement, Fixleist.sequenz, Fixleist.dekade, Fixleist.lfakt, Fixleist.betrag, Fixleist.number, Fixleist._recid).filter(
                        ...
                        post_it = check_fixleist_posted(genstat.datum, fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt, genstat.res_date[0], genstat.res_date[1])
                        ...
                        if post_it:
                            ...
                            if artikel:
                                ...
                                art_vatlist = query(art_vatlist_list, filters=(lambda art_vatlist: art_vatlist.artnr == artikel.artnr and art_vatlist.dept == artikel.departement and art_vatlist.datum == genstat.datum), first=True)

                                if art_vatlist:
                                    service =  to_decimal(art_vatlist.service)
                                    vat =  to_decimal(art_vatlist.vat)
                                    ...
                                    art_vatlist = Art_vatlist()
                                    art_vatlist_list.append(art_vatlist)

                                    art_vatlist.artnr = artikel.artnr
                                    art_vatlist.dept = artikel.departement
                                    art_vatlist.datum = genstat.datum
                                    art_vatlist.service =  to_decimal(service)
                                    art_vatlist.vat =  to_decimal(vat)
                ...
                if do_it:
                    ...
                    for fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt, fixleist.betrag, fixleist.number, fixleist._recid in db_session.query(Fixleist.artnr, Fixleist.departement, Fixleist.sequenz, Fixleist.dekade, Fixleist.lfakt, Fixleist.betrag, Fixleist.number, Fixleist._recid).filter(
                        ...
                        for datum in date_range(datum1,datum2) :
                            ...
                            if room_list:
                                post_it = check_fixleist_posted(datum, fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt, res_line.ankunft, res_line.abreise)
                                service =  to_decimal("0")
                                vat =  to_decimal("0")
                                fcost =  to_decimal("0")
                                ...
                                if post_it:
                                    ...
                                    if artikel:
                                        ...
                                        art_vatlist = query(art_vatlist_list, filters=(lambda art_vatlist: art_vatlist.artnr == artikel.artnr and art_vatlist.dept == artikel.departement and art_vatlist.datum == datum), first=True)
                        ...
                        if room_list:
                            ...
                            if datum == res_line.abreise:
                                ...
                                if arrangement:
                                    ...
                                    argtline_list = query(argtline_list_list, filters=(lambda argtline_list: argtline_list.argtnr == arrangement.argtnr), first=True)

                                    if not argtline_list:

                                        for argt_line in db_session.query(Argt_line).filter(
                                                 (Argt_line.argtnr == arrangement.argtnr) & (Argt_line.kind2)).order_by(Argt_line._recid).all():
                                            argtline_list = Argtline_list()
                                            argtline_list_list.append(argtline_list)

                                            buffer_copy(argt_line, argtline_list)

                                    for argtline_list in query(argtline_list_list, filters=(lambda argtline_list: argtline_list.argtnr == arrangement.argtnr)):
                                        post_it = check_fixargt_posted(argtline_list.argt_artnr, argtline_list.departement, argtline_list.fakt_modus, argtline_list.intervall, res_line.ankunft, res_line.abreise)
                                        service =  to_decimal("0")
                                        vat =  to_decimal("0")
                                        fcost =  to_decimal("0")
                                        ...
                                        if post_it:
                                            ...
                                            artikel = get_cache (Artikel, {"artnr": [(eq, argtline_list.argt_artnr)],"departement": [(eq, argtline_list.departement)]})
                                            ...
                                            if argtline_list.vt_percnt == 0:

                                                if argtline_list.betriebsnr == 0:
                                                    pax = res_line.erwachs
                                                else:
                                                    pax = argtline_list.betriebsnr

                                            elif argtline_list.vt_percnt == 1:
                                                pax = res_line.kind1

                                            elif argtline_list.vt_percnt == 2:
                                                pax = res_line.kind2
                                            else:
                                                pax = 0
                                            price = 0

                                            reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "fargt-line")],"char1": [(eq, "")],"number1": [(eq, argtline_list.departement)],"number2": [(eq, argtline_list.argtnr)],"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, res_line.reslinnr)],"number3": [(eq, argtline_list.argt_artnr)],"date1": [(le, curr_date)],"date2": [(ge, curr_date)]})
                                            ...
                                            if reslin_queasy:
                                                ...
                                                for reslin_queasy.number3, reslin_queasy.deci1, reslin_queasy.deci2, reslin_queasy.deci3, reslin_queasy._recid in db_session.query(Reslin_queasy.number3, Reslin_queasy.deci1, Reslin_queasy.deci2, Reslin_queasy.deci3, Reslin_queasy._recid).filter(
                                                         (func.lower(Reslin_queasy.key) == ("fargt-line").lower()) & (Reslin_queasy.char1 == "") & (Reslin_queasy.number1 == argtline_list.departement) & (Reslin_queasy.number2 == argtline_list.argtnr) & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argtline_list.argt_artnr) & (curr_date >= Reslin_queasy.date1) & (curr_date <= Reslin_queasy.date2)).order_by(Reslin_queasy._recid).all():
                                            ...
                                            if price == 0:
                                                fcost =  to_decimal(argtline_list.betrag) * to_decimal(pax)
                                                fcost =  to_decimal(fcost) / to_decimal((1) + to_decimal(service) + to_decimal(vat) )
        ...
        queasy = Queasy()
        for queasy.date1, queasy.date2, queasy.char1, queasy.number3, queasy.char2, queasy.number1, queasy.char3, queasy._recid in db_session.query(Queasy.date1, Queasy.date2, Queasy.char1, Queasy.number3, Queasy.char2, Queasy.number1, Queasy.char3, Queasy._recid).filter(
                 (Queasy.key == 14) & ((Queasy.date1 <= curr_date) & (Queasy.date2 >= curr_date)) | ((Queasy.date1 >= curr_date) & (Queasy.date1 <= (curr_date + timedelta(days=tmpint))))).order_by(Queasy._recid).all():
            datum = curr_date - timedelta(days=1)
            for i in range(1,tmpint + 1) :
                datum = datum + timedelta(days=1)

                if datum >= queasy.date1 and datum <= queasy.date2:

                    room_list = query(room_list_list, filters=(lambda room_list: room_list.datum == datum), first=True)

                    zimmer = db_session.query(Zimmer).filter(
                             (Zimmer.zinr == queasy.char1) & (Zimmer.sleeping)).first()
        ...
        from_ooo = date_mdy(1, 1, 2000)
        to_ooo = date_mdy(1, 1, 2000)
        ...
        for room_list in query(room_list_list, filters=(lambda room_list: room_list.wd != 0)):
            ...
            if room_list.datum < ci_date:
                ...
                if from_ooo == 01/01/2000:
                    from_ooo = room_list.datum
                to_ooo = room_list.datum
        ...
        if from_ooo != 01/01/2000:
            ...
            outorder = Outorder()
            for outorder.zinr, outorder.gespstart, outorder.gespende, outorder._recid in db_session.query(Outorder.zinr, Outorder.gespstart, Outorder.gespende, Outorder._recid).filter(
                     (((Outorder.gespstart <= from_ooo) & (Outorder.gespende >= from_ooo)) | ((Outorder.gespstart >= from_ooo) & (Outorder.gespstart <= to_ooo))) & (Outorder.betriebsnr <= 1)).order_by(Outorder.zinr).all():

                zimmer = get_cache (Zimmer, {"zinr": [(eq, outorder.zinr)]})

                if zimmer.sleeping:

                    room_list = query(room_list_list, filters=(lambda room_list: room_list.datum >= outorder.gespstart and room_list.datum <= outorder.gespende), first=True)
        ...
        if do_it:
            ...
            if incl_oth and curr_date <= ci_date:
                calc_othrev()
        room_list = Room_list()
        room_list_list.append(room_list)
    ...
    def room_code_name():
        ...
        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, datum, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, tmpint, argt_line, res_line, htparam, waehrung, kontline, zimmer, genstat, guest, zimkateg, segment, exrate, fixleist, artikel, reservation, arrangement, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
    ...
    def create_browse1():
        ...
        nonlocal room_list_list, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, tot_room, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, curr_time, tmpint, argt_line, res_line, htparam, waehrung, kontline, zimmer, genstat, guest, zimkateg, segment, exrate, fixleist, artikel, reservation, arrangement, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
        ...
        for i in range(1,tmpint + 1) :
            ...
            queasy = Queasy()
            zimmer = Zimmer()
            for queasy.date1, queasy.date2, queasy.char1, queasy.number3, queasy.char2, queasy.number1, queasy.char3, queasy._recid, zimmer.zikatnr, zimmer.typ, zimmer._recid in db_session.query(Queasy.date1, Queasy.date2, Queasy.char1, Queasy.number3, Queasy.char2, Queasy.number1, Queasy.char3, Queasy._recid, Zimmer.zikatnr, Zimmer.typ, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Queasy.char1) & (Zimmer.sleeping)).filter(
        ...
        for room_list in query(room_list_list, filters=(lambda room_list: room_list.wd != 0)):
            ...
            if room_list.datum < ci_date:
                ...
                outorder = Outorder()
                zimmer = Zimmer()
                for outorder.zinr, outorder.gespstart, outorder.gespende, outorder._recid, zimmer.zikatnr, zimmer.typ, zimmer._recid in db_session.query(Outorder.zinr, Outorder.gespstart, Outorder.gespende, Outorder._recid, Zimmer.zikatnr, Zimmer.typ, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Outorder.zinr) & (Zimmer.sleeping)).filter(
    ...
    def rsv_closeout(datum:date):
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 37)],"number1": [(eq, get_year(datum))]})
    ...
    def calc_othrev():
        ...
        if not art_list:
            ...
            artikel = Artikel()
            for artikel.artnr, artikel.departement, artikel.service_code, artikel.mwst_code, artikel._recid in db_session.query(Artikel.artnr, Artikel.departement, Artikel.service_code, Artikel.mwst_code, Artikel._recid).filter(
                ...
                art_list.service_code = artikel.service_code
                art_list.mwst_code = artikel.mwst_code


        htparam = get_cache (Htparam, {"paramnr": [(eq, 144)]})

        waehrung = get_cache (Waehrung, {"wabkurz": [(eq, htparam.fchar)]})
        for curr_datum in date_range(curr_date,calc_todate) :

            if waehrung:

                if curr_datum < ci_date:

                    exrate = get_cache (Exrate, {"datum": [(eq, curr_datum)],"artnr": [(eq, waehrung.waehrungsnr)]})

                    if exrate:
                        exchg_rate =  to_decimal(exrate.betrag)
                    else:
                        exchg_rate =  to_decimal(waehrung.ankauf) / to_decimal(waehrung.einheit)
                    exrate_list = Exrate_list()
                    exrate_list_list.append(exrate_list)

                    exrate_list.datum = curr_datum
                    exrate_list.exchg_rate =  to_decimal(exchg_rate)
                    ...
                    exrate_list.datum = ci_date
                    exrate_list.exchg_rate =  to_decimal(waehrung.ankauf) / to_decimal(waehrung.einheit)
                ...
                exrate_list.datum = curr_datum
                exrate_list.exchg_rate =  to_decimal("1")

            if curr_date == ci_date:
                return

        for art_list in query(art_list_list):
            curr_datum = date_mdy(1, 1, 2000)
            othrev =  to_decimal("0")

            umsatz = Umsatz()
            for umsatz.datum, umsatz.artnr, umsatz.departement, umsatz.betrag, umsatz._recid in db_session.query(Umsatz.datum, Umsatz.artnr, Umsatz.departement, Umsatz.betrag, Umsatz._recid).filter(
                     (Umsatz.artnr == art_list.artnr) & (Umsatz.departement == 0) & (Umsatz.datum >= curr_date) & (Umsatz.datum <= calc_todate)).order_by(Umsatz._recid).all():

                if curr_datum != umsatz.datum:

                    if curr_datum != 01/01/2000:

                        exrate_list = query(exrate_list_list, filters=(lambda exrate_list: exrate_list.datum == curr_datum), first=True)
                        room_list.lodg[4] = room_list.lodg[4] + othrev
                        room_list.lodg[6] = room_list.lodg[6] + othrev / exrate_list.exchg_rate

                        if room_list.wd != 0:
                            t_lodg[4] = t_lodg[4] + othrev
                            t_lodg[6] = t_lodg[6] + othrev / exrate_list.exchg_rate


                    curr_datum = umsatz.datum

                    room_list = query(room_list_list, filters=(lambda room_list: room_list.datum == curr_datum), first=True)

                    art_vatlist = query(art_vatlist_list, filters=(lambda art_vatlist: art_vatlist.artnr == umsatz.artnr and art_vatlist.dept == umsatz.departement and art_vatlist.datum == umsatz.datum), first=True)
                    ...
                    if art_vatlist:
                        ...
                        service, vat = get_output(calc_servvat(artikel.departement, artikel.artnr, umsatz.datum, artikel.service_code, artikel.mwst_code))
                        art_vatlist = Art_vatlist()
                        art_vatlist_list.append(art_vatlist)

                        art_vatlist.artnr = umsatz.artnr
                        art_vatlist.dept = umsatz.departement
                        art_vatlist.datum = umsatz.datum
                        art_vatlist.service =  to_decimal(service)
                        art_vatlist.vat =  to_decimal(vat)


                    fact =  to_decimal(1.00) + to_decimal(service) + to_decimal(vat)
                    othrev =  to_decimal(umsatz.betrag) / to_decimal(fact)
    ...
    def check_fixleist_posted(curr_date:date, artnr:int, dept:int, fakt_modus:int, intervall:int, lfakt:date, cidate:date, codate:date):
        ...
        if fakt_modus == 1:
            ...
            if lfakt == None:
                ...
                delta = (lfakt - cidate).days

                if delta < 0:
                    delta = 0
            start_date = cidate + timedelta(days=delta)
            tmpint1 = (codate - start_date).days

            if tmpint1 < intervall:
                start_date = cidate
            tmpint1 = intervall - 1
            tmpdate = start_date + timedelta(days=tmpint1)

            if curr_date <= tmpdate:
                post_it = True
    ...
    def check_fixargt_posted(artnr:int, dept:int, fakt_modus:int, intervall:int, cidate:date, codate:date):
        ...
        if fakt_modus == 1:
            ...
            if curr_date <= (cidate + timedelta(days=(intervall - 1))):
                post_it = True
