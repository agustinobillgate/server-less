...
def prepare_deposit_paybl(pvilanguage:int, inp_resnr:int, depositgef:Decimal):
    ...
    def generate_output():
        nonlocal price_decimal, exchg_rate, depoart, depobezeich, ask_voucher, deposit_exrate, f_tittle, msg_str, balance, paybez1, paybez2, artikel_list_data, t_reservation_data, lvcarea, unallocated_subgrp, sorttype, reservation, htparam, waehrung, artikel
        nonlocal pvilanguage, inp_resnr, depositgef


        nonlocal t_reservation, artikel_list
        nonlocal t_reservation_data, artikel_list_data

        return {"price_decimal": price_decimal, "exchg_rate": exchg_rate, "depoart": depoart, "depobezeich": depobezeich, "ask_voucher": ask_voucher, "deposit_exrate": deposit_exrate, "f_tittle": f_tittle, "msg_str": msg_str, "balance": balance, "paybez1": paybez1, "paybez2": paybez2, "artikel-list": artikel_list_data, "t-reservation": t_reservation_data}
