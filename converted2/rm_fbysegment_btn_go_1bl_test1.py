#using conversion tools version: 1.0.0.94
...
def rm_fbysegment_btn_go_1bl(sum_month:bool, fr_date:date, to_date:date, to_year:int, ex_tent:bool, ex_comp:bool):
    ...
    output_list_list, Output_list = create_model("Output_list", {"counter":int, "segmentcode":int, "datum":date, "str_datum":string, "segment":string, "room":int, "revenue":Decimal, "avrg_rev":Decimal, "str_room":string, "str_revenue":string, "str_avrg":string, "check_flag":bool})
    output_list1_list, Output_list1 = create_model("Output_list1", {"counter":int, "segmentcode":int, "segment":string, "room":[int,12], "revenue":[Decimal,12], "avrg_rev":[Decimal,12], "check_flag":bool})
    ...
    def create_browse():
        ...
        if to_date >= ci_date:
            ...
            res_line = Res_line()
            for res_line.resnr, res_line.gratis, res_line.zipreis, res_line.resstatus, res_line.zimmerfix, res_line.ankunft, res_line.abreise, res_line._recid, res_line.zimmeranz in db_session.query(Res_line.resnr, Res_line.gratis, Res_line.zipreis, Res_line.resstatus, Res_line.zimmerfix, Res_line.ankunft, Res_line.abreise, Res_line._recid, Res_line.zimmeranz).filter(
                ...
                segment = get_cache (Segment, {"segmentcode": reservation.segmentcode, "segmentcode": (ne, black_list)}, ['bezeich', '_recid'])
    ...
    def create_browse1():
        ...
        WHERE 
            (
                ...
                AND (Res_line.resstatus != 4)
                AND (Res_line.resstatus != 9)
                AND (Res_line.resstatus != 10)
                AND (Res_line.resstatus != 12)
                AND (Res_line.active_flag <= 1)
                AND NOT (Res_line.ankunft > '2024-09-24') 
                AND NOT (Res_line.abreise < '2024-09-24')
            )
            OR 
            (
                (Res_line.resstatus = 8)
                AND (Res_line.active_flag = 2)
                AND (Res_line.ankunft = '2024-09-24')
                AND (Res_line.abreise = '2024-09-24')
            )
            AND (Res_line.gastnr > 0)
        ORDER BY 
            Res_line.resnr, 
            Res_line.reslinnr DESC;
                   
        """)
        print("Prepare resline_query")
        db_session.execute(resline_query, {"start_date": "curr_date", "to_date": to_date, "ci_date": ci_date})
        print("resline_query")
        ...
        segment_query = text("""
                ...
                SELECT * FROM segment r
                WHERE
                    EXISTS (
                        SELECT 1 FROM reservation rl WHERE rl.segmentcode = r.segmentcode
                    )
                                """)
        db_session.execute(segment_query, {})
        ...
        for res_line.resnr, res_line.gratis, res_line.zipreis, res_line.resstatus, res_line.zimmerfix, res_line.ankunft, res_line.abreise, res_line._recid, res_line.zimmeranz in db_session.query(Res_line.resnr, Res_line.gratis, Res_line.zipreis, Res_line.resstatus, Res_line.zimmerfix, Res_line.ankunft, Res_line.abreise, Res_line._recid, Res_line.zimmeranz).filter(
            ...
            segment = db_session.query(Segment).filter((Segment.segmentcode == reservation.segmentcode) & (Segment.segmentcode != black_list)).first()
            # segment = get_cache (Segment, {"segmentcode": reservation.segmentcode, "segmentcode": (ne, black_list)}, ['bezeich', '_recid'])
            # print("segment:", segment.bezeich)
            if segment:
    ...
    htparam = get_cache (Htparam, {"paramnr": 110}, ['fdate', 'finteger', '_recid'])

    if htparam:
        ci_date = htparam.fdate

    htparam = get_cache (Htparam, {"paramnr": 709}, ['fdate', 'finteger', '_recid'])
    ...
    if sum_month == False:
        ...
        for output_list in query(output_list_list, filters=(lambda output_list: output_list.check_flag), sort_by=[("datum",False),("segmentcode",False)]):
        ...
        for output_list1 in query(output_list1_list, filters=(lambda output_list1: output_list1.check_flag), sort_by=[("segmentcode",False)]):
