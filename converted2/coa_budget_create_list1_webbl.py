...
def coa_budget_create_list1_webbl(disp_all:bool, case_type:int, fstype_nr:int):
    ...
    coa_list_data, Coa_list = create_model("Coa_list", {"fibukonto":string, "main_nr":int, "bezeich":string, "b_flag":bool, "acct_type":int, "actual":[Decimal,12], "bemerk":string, "fs_type":int, "last_yr":[Decimal,12], "budget":[Decimal,12], "deptnr":int, "userinit":string, "chginit":string, "c_date":date, "m_date":date, "modifiable":bool, "activeflag":bool, "ly_budget":[Decimal,12], "debit":[Decimal,12], "credit":[Decimal,12], "is_group":bool})
    t_gl_fstype_data, T_gl_fstype = create_model_like(Gl_fstype)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal max_row, month_closing, year_closing, curr_year, coa_list_data, t_gl_fstype_data, fs_number_group, is_first, gl_fstype, htparam, gl_acct
        nonlocal disp_all, case_type, fstype_nr


        nonlocal coa_list, t_gl_fstype
        nonlocal coa_list_data, t_gl_fstype_data

        return {"max_row": max_row, "month_closing": month_closing, "year_closing": year_closing, "curr_year": curr_year, "coa-list": coa_list_data, "t-gl-fstype": t_gl_fstype_data}
