...
def dashboard_initialbl(exclude_article:string):
    ...
    def generate_output():
        ...
        nonlocal his_res_data, his_inv_data, temp_res_data, future_res_data, future_inv_data, non_room_data, t_list_data, hbill_list_data

        return {"his-res": his_res_data, "his-inv": his_inv_data, "future-res": future_res_data, "future-inv": future_inv_data, "non-room": non_room_data}
        ...
        if zimmer and zimmer.sleeping :
            for datum2 in date_range(outorder.gespstart,outorder.gespende) :
                ...
                if datum2 >= ci_date and datum2 <= ci_date + timedelta(days=90):
                    ...
                    future_inv = query(future_inv_data, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.room_type == zimmer.kbezeich and future_inv.room_status.lower()  == ("OUTOFORDER").lower()), first=True)
    ...
    for future_inv in query(future_inv_data, filters=(lambda future_inv: future_inv.room_status.lower()  == ("AVAILABLE").lower())):
        ...
        buffuture_inv = query(buffuture_inv_data, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("OCCUPIED").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.room_type == future_inv.room_type), first=True)

        if buffuture_inv:
            future_inv.qty = future_inv.qty - buffuture_inv.qty

        buffuture_inv = query(buffuture_inv_data, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("OUTOFORDER").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.room_type == future_inv.room_type), first=True)

        if buffuture_inv:
            future_inv.qty = future_inv.qty - buffuture_inv.qty

        buffuture_inv = query(buffuture_inv_data, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("COMPLIMENT").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.room_type == future_inv.room_type), first=True)
