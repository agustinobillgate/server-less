...
def add_reprint_kitchprbl(pvilanguage:int, session_parameter:string, dept:int, rechnr:int, billdate:date, user_init:string):
    ...
    t_queasy_data, T_queasy = create_model_like(Queasy, {"flag":bool})
    t_list_data, T_list = create_model("T_list", {"billno":int, "depart":int, "artno":int, "flag":bool})
    ...
    def generate_output():
        ...
        nonlocal t_queasy_data, t_list_data, submenu_list_data
            ...
            if num_entries(printer.path, "$") > 1:
                for loopi in range(1,num_entries(printer.path, "$")  + 1) :
                    ...
                    if bqsy:
                        ...
                        t_queasy = query(t_queasy_data, filters=(lambda t_queasy: t_queasy.key == 233 and t_queasy.number1 == to_int(check_printnr) and t_queasy.number3 == to_int(check_printnr)), first=True)
                ...
                if bqsy:
                    ...
                    t_queasy = query(t_queasy_data, filters=(lambda t_queasy: t_queasy.key == 233 and t_queasy.number1 == printer.nr and t_queasy.number3 == printer.nr), first=True)
    ...
    def write_article():
        ...
        if h_artikel.betriebsnr > 0:
            ...
            for submenu_list in query(submenu_list_data, filters=(lambda submenu_list: submenu_list.nr == h_artikel.betriebsnr and submenu_list.zeit == h_bill_line.zeit)):
                created = True
