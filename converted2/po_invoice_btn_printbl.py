...
def po_invoice_btn_printbl(s_list_data:[S_list]):
    ...
    sub_list_data, Sub_list = create_model("Sub_list", {"zwkum":int, "bezeich":string, "amt":Decimal, "disc":Decimal, "disc2":Decimal, "vat":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal sub_list_data, l_artikel, l_untergrup


        nonlocal s_list, sub_list
        nonlocal sub_list_data

        return {"sub-list": sub_list_data}
    ...
    for s_list in query(s_list_data, sort_by=[("bezeich",False),("betriebsnr",False)]):
        ...
        sub_list = query(sub_list_data, filters=(lambda sub_list: sub_list.zwkum == l_untergrup.zwkum), first=True)

        if not sub_list:
            sub_list = Sub_list()
            sub_list_data.append(sub_list)
