...
def prepare_gl_adjustmentbl(sorttype:int):
    ...
    b2_list_data, B2_list = create_model("B2_list", {"jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal from_date, param_983, t_gl_jouhdr_data, b2_list_data, to_date, gl_jouhdr, htparam, gl_journal, gl_acct
        nonlocal sorttype


        nonlocal t_gl_jouhdr, b2_list
        nonlocal t_gl_jouhdr_data, b2_list_data

        return {"from_date": from_date, "param_983": param_983, "t-gl-jouhdr": t_gl_jouhdr_data, "b2-list": b2_list_data}
