...
def sls_bycompet_btn_go_listbl(pvilanguage:int, next_date:date, to_date:date, all_flag:bool, usr_init:string):
    slcompet_list_data = []
    lvcarea:string = "sls-bycompet"
    akt_code = guest = akthdr = akt_kont = None

    p_list = slcompet_list = None

    p_list_data, P_list = create_model("P_list", {"pnr":int, "sflag":int, "hotel_name":string, "pcomp":string, "pcont":string, "pname":string, "pntot":string, "pnam1":int, "pnam2":int, "pnam3":int, "pnam4":int, "pamt":Decimal, "pamt1":Decimal, "pamt2":Decimal, "pamt3":Decimal, "pamt4":Decimal, "patot":Decimal, "pamt_str":string, "stnr":int, "stage":string, "proz":string, "popen":date, "pfnsh":date, "pmain1":int, "pmain2":string, "pmain3":string, "reason":string, "refer":string, "pid":string, "pcid":string, "ctotal":int})
    slcompet_list_data, Slcompet_list = create_model("Slcompet_list", {"pnr":int, "sflag":int, "hotel_name":string, "pcomp":string, "pcont":string, "pname":string, "pntot":string, "pnam1":int, "pnam2":int, "pnam3":int, "pnam4":int, "pamt":Decimal, "pamt1":Decimal, "pamt2":Decimal, "pamt3":Decimal, "pamt4":Decimal, "patot":Decimal, "pamt_str":string, "stnr":int, "stage":string, "proz":string, "popen":date, "pfnsh":date, "pmain1":int, "pmain2":string, "pmain3":string, "reason":string, "refer":string, "pid":string, "pcid":string, "ctotal":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal slcompet_list_data, lvcarea, akt_code, guest, akthdr, akt_kont
        nonlocal pvilanguage, next_date, to_date, all_flag, usr_init


        nonlocal p_list, slcompet_list
        nonlocal p_list_data, slcompet_list_data

        return {"slcompet-list": slcompet_list_data}
    ...
    def browse_open1():
        ...
        p_list_data.clear()
        slcompet_list_data.clear()

        if all_flag:
            nr = 0

            akt_code1_obj_list = {}
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.mitbewerber[inc_value(0)] == Akt_code1.aktionscode) & (Buf_akthdr.flag == 3) & (Buf_akthdr.mitbewerber[inc_value(0)] != 0)).filter(
                     (Akt_code1.aktiongrup == 4)).order_by(Akt_code1.aktionscode).all():
                ...
                if akt_code1:
                    ...
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == Guest1.gastnr) & (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                             (Akthdr1.flag == 3) & (Akthdr1.mitbewerber[inc_value(0)] == akt_code1.aktionscode)).order_by(Akthdr1.stufe, Guest1.name).all():
            ...
            akt_code1_obj_list = {}
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.mitbewerber[inc_value(0)] == Akt_code1.aktionscode) & (Buf_akthdr.flag == 3) & (Buf_akthdr.mitbewerber[inc_value(0)] != 0) & (Buf_akthdr.userinit == (usr_init).lower())).filter(
                     (Akt_code1.aktiongrup == 4)).order_by(Akt_code1.aktionscode).all():
                if akt_code1_obj_list.get(akt_code1._recid):
                    continue
                else:
                ...
                if akt_code1:
                    ...
                    akthdr1_obj_list = {}
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == Guest1.gastnr) & (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                             (Akthdr1.flag == 3) & (Akthdr1.userinit == (usr_init).lower()) & (Akthdr1.mitbewerber[inc_value(0)] == akt_code1.aktionscode)).order_by(Akthdr1.stufe, Guest1.name).all():
                        if akthdr1_obj_list.get(akthdr1._recid):
                            continue
                        else:
                            akthdr1_obj_list[akthdr1._recid] = True
    ...
    def browse_open2():
        ...
        if all_flag:
            ...
            akt_code1_obj_list = {}
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.mitbewerber[inc_value(0)] == Akt_code1.aktionscode) & (Buf_akthdr.flag == 3) & (Buf_akthdr.mitbewerber[inc_value(0)] != 0) & (Buf_akthdr.erl_datum >= next_date) & (Buf_akthdr.erl_datum <= to_date)).filter(
                     (Akt_code1.aktiongrup == 4)).order_by(Akt_code1.aktionscode).all():
                if akt_code1_obj_list.get(akt_code1._recid):
                    continue
                else:
                ...
                if akt_code1:
                    ...
                    akthdr1_obj_list = {}
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == Guest1.gastnr) & (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                             (Akthdr1.flag == 3) & (Akthdr1.mitbewerber[inc_value(0)] == akt_code1.aktionscode) & (Akthdr1.erl_datum >= next_date) & (Akthdr1.erl_datum <= to_date)).order_by(Akthdr1.stufe, Guest1.name).all():
                        if akthdr1_obj_list.get(akthdr1._recid):
                            continue
                        else:
                            akthdr1_obj_list[akthdr1._recid] = True
            ...
            akt_code1_obj_list = {}
            for akt_code1, buf_akthdr in db_session.query(Akt_code1, Buf_akthdr).join(Buf_akthdr,(Buf_akthdr.mitbewerber[inc_value(0)] == Akt_code1.aktionscode) & (Buf_akthdr.flag == 3) & (Buf_akthdr.mitbewerber[inc_value(0)] != 0) & (Buf_akthdr.userinit == (usr_init).lower()) & (Buf_akthdr.erl_datum >= next_date) & (Buf_akthdr.erl_datum <= to_date)).filter(
                     (Akt_code1.aktiongrup == 4)).order_by(Akt_code1.aktionscode).all():
                if akt_code1_obj_list.get(akt_code1._recid):
                    continue
                else:
                ...
                if akt_code1:
                    ...
                    akthdr1_obj_list = {}
                    for akthdr1, guest1, akt_kont1 in db_session.query(Akthdr1, Guest1, Akt_kont1).join(Guest1,(Guest1.gastnr == Akthdr1.gastnr)).join(Akt_kont1,(Akt_kont1.gastnr == Guest1.gastnr) & (Akt_kont1.kontakt_nr == Akthdr1.kontakt_nr)).filter(
                             (Akthdr1.flag == 3) & (Akthdr1.userinit == (usr_init).lower()) & (Akthdr1.mitbewerber[inc_value(0)] == akt_code1.aktionscode) & (Akthdr1.erl_datum >= next_date) & (Akthdr1.erl_datum <= to_date)).order_by(Akthdr1.stufe, Guest1.name).all():
                        if akthdr1_obj_list.get(akthdr1._recid):
                            continue
                        else:
                            akthdr1_obj_list[akthdr1._recid] = True
    ...
    def fill_compet():
        ...
        for p_list in query(p_list_data):
            ...
            slcompet_list_data.append(slcompet_list)
