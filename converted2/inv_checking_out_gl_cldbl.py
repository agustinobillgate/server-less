...
def inv_checking_out_gl_cldbl(invtype:int, d1:date, d2:date):
    frnr = 0
    tonr = 0
    ...
    def out_gl():
        ...
        for coa_list in query(coa_list_list):
            ...
            for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
                    (Gl_jouhdr.datum == coa_list.datum) &  (Gl_jouhdr.jtyp == 3) &  (substring(Gl_jouhdr.refNo, 0, len(refNo)) == refNo)).order_by(Gl_jouhdr._recid).all():
