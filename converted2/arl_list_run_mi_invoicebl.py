...
def arl_list_run_mi_invoicebl(resnr:int, curr_resnr:int, arl_list_reslinnr:int, t_active_flag:int, printtype:int):
    ...
    prepare_cache ([Reservation, Res_line, Bill, Htparam, Master, Waehrung, Zimkateg, Arrangement, Reslin_queasy, Guest_pr, Fixleist, Artikel, Bill_line, Genstat, Exrate])
    ...
    def generate_output():
        nonlocal err_flag, avail_master, avail_bill, reslinnr, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_data, t_res_line_data, t_guest_data, t_list_data, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, reservation, res_line, guest, bill, htparam, master, waehrung, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, artikel, bill_line, genstat, exrate
