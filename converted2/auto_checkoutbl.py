...
def auto_checkoutbl(resnr:int, sorttype:int):
    ...
    t_res_line_data, T_res_line = create_model("T_res_line", {"resnr":int, "reslinnr":int, "name":string, "zinr":string, "resstatus":int, "active_flag":int})
    resline1_data, Resline1 = create_model("Resline1", {"resnr":int, "reslinnr":int, "resstatus":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_res_line_data, resline1_data, res_line, bill, bill_line
        nonlocal resnr, sorttype


        nonlocal t_res_line, resline1
        nonlocal t_res_line_data, resline1_data

        return {"t-res-line": t_res_line_data, "resline1": resline1_data}
