...
def ins_pr_btn_go_webbl(pos:int, user_init:string, ins_list_data:[Ins_list]):
    ...
    for l_artikel in db_session.query(L_artikel).filter(
             ((L_artikel.artnr.in_(list(set([ins_list.artnr for ins_list in ins_list_data if ins_list.new_created])))))).order_by(L_artikel._recid).all():
        if l_artikel_obj_list.get(l_artikel._recid):
            continue
        else:
            l_artikel_obj_list[l_artikel._recid] = True

        ins_list = query(ins_list_data, (lambda ins_list: (l_artikel.artnr == ins_list.artnr)), first=True)
        pos = pos + 1
        l_order = L_order()
