...
def hk_change_room_status_ooobl(pvilanguage:int, user_init:string, from_date:date, to_date:date, dept:int, reason:string, service_flag:bool, room_list_data:[Room_list]):
    ...
    flag, t_zinr, t_zistatus = get_output(hk_statadmin_start_chgstatbl(4, bline_list_data))
    ...
    if flag == 6:
        ...
        bline_list_data, om_list_data, flag, msg_str, z_list_data = get_output(hk_statadmin_activate_ooobl(bline_list_data, om_list_data, pvilanguage, from_date, to_date, ci_date, dept, reason, service_flag, bediener.nr))
