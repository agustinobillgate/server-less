...
def ts_biltransfer2_webbl(mc_str:string, curr_dept:int):
    ...
    def generate_output():
        nonlocal do_it, msg_str, t_queasy_data, excl_str, count_i, dept, queasy
        nonlocal mc_str, curr_dept


        nonlocal t_queasy, dept_list
        nonlocal t_queasy_data, dept_list_data

        return {"do_it": do_it, "msg_str": msg_str, "t-queasy": t_queasy_data}
    ...
    if t_queasy:
        ...
        if excl_str != "":
            ...
            dept_list = query(dept_list_data, filters=(lambda dept_list: dept_list.deptno == curr_dept), first=True)
