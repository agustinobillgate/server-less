...
def prepare_rest_daysalesp2_1_cldbl():
    ...
    htl_dept_data, Htl_dept = create_model("Htl_dept", {"dptnr":int, "bezeich":string})
    t_hoteldpt_data, T_hoteldpt = create_model_like(Hoteldpt)
    buf_art_data, Buf_art = create_model("Buf_art", {"artnr":int, "bezeich":string, "departement":int, "art_type":int})
    usr1_data, Usr1 = create_model_like(Kellner, {"rec_id":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal disc_art1, disc_art2, disc_art3, exchg_rate, str, curr_foreign, serv_taxable, dpt_str, art_str, oth_str, anzahl, curr_dept, dept_name, voucher_art, use_voucher, from_date, to_date, htl_dept_dptnr, err_flag, p_110, p_240, active_deposit, buf_art_data, htl_dept_data, usr1_data, f_cash, i, str_tmp, curr_local, price_decimal, artnr_list, show_option, hoteldpt, kellner, htparam, waehrung, artikel, h_artikel


        nonlocal htl_dept, t_hoteldpt, buf_art, usr1
        nonlocal htl_dept_data, t_hoteldpt_data, buf_art_data, usr1_data

        return {"disc_art1": disc_art1, "disc_art2": disc_art2, "disc_art3": disc_art3, "exchg_rate": exchg_rate, "str": str, "curr_foreign": curr_foreign, "serv_taxable": serv_taxable, "dpt_str": dpt_str, "art_str": art_str, "oth_str": oth_str, "anzahl": anzahl, "curr_dept": curr_dept, "dept_name": dept_name, "voucher_art": voucher_art, "use_voucher": use_voucher, "from_date": from_date, "to_date": to_date, "htl_dept_dptnr": htl_dept_dptnr, "err_flag": err_flag, "p_110": p_110, "p_240": p_240, "active_deposit": active_deposit, "buf-art": buf_art_data, "htl-dept": htl_dept_data, "usr1": usr1_data}
    ...
    str = curr_local + ";" + to_string(price_decimal)
    ...
    for i in range(1,num_entries(dpt_str, ",")  + 1) :
        ...
        if to_int(entry(i - 1, dpt_str, ",")) != 0:
            ...
            if hoteldpt:
                htl_dept = Htl_dept()
                htl_dept_data.append(htl_dept)

                htl_dept.dptnr = hoteldpt.num
                htl_dept.bezeich = hoteldpt.depart

    htl_dept = query(htl_dept_data, first=True)
    ...
    if htparam.finteger > 0:
        ...
        if h_artikel:
            ...
            h_artikel = H_artikel()
            artikel = Artikel()
            for h_artikel.artnr, h_artikel._recid, artikel.artnr, artikel.bezeich, artikel.departement, artikel.umsatzart, artikel._recid in db_session.query(H_artikel.artnr, H_artikel._recid, Artikel.artnr, Artikel.bezeich, Artikel.departement, Artikel.umsatzart, Artikel._recid).join(Artikel,(Artikel.artnr == H_artikel.artnrfront) & (Artikel.departement == 0) & (Artikel.pricetab)).filter(
                     (H_artikel.artart == 6)).order_by(H_artikel._recid).yield_per(100):
