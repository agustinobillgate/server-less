...
from models import Htparam, L_lager, L_op, L_artikel, L_ophdr, Parameters, L_bestand, Bediener, Gl_acct

def storereq_list_create_list_1_webbl(from_date:date, to_date:date, from_dept:int, to_dept:int, curr_lschein:string, show_price:bool):

    prepare_cache ([Htparam, L_lager, L_op, L_artikel, L_ophdr, Parameters, L_bestand, Bediener, Gl_acct])

    it_exist = False
    t_list_data = []
    appflag:bool = False
    long_digit:bool = False
    htparam = l_lager = l_op = l_artikel = l_ophdr = parameters = l_bestand = bediener = gl_acct = None
    ...
    def generate_output():
        nonlocal it_exist, t_list_data, appflag, long_digit, htparam, l_lager, l_op, l_artikel, l_ophdr, parameters, l_bestand, bediener, gl_acct
    ...
    def create_list():
        ...
        nonlocal it_exist, t_list_data, long_digit, htparam, l_lager, l_op, l_artikel, l_ophdr, parameters, l_bestand, bediener, gl_acct
        nonlocal from_date, to_date, from_dept, to_dept, curr_lschein, show_price
