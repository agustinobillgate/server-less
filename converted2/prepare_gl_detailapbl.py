...
def prepare_gl_detailapbl(pvilanguage:int, fibu:string, bemerk:string):
    ...
    s_list_data, S_list = create_model("S_list", {"rgdatum":date, "artnr":int, "bezeich":string, "saldo":Decimal, "name":string, "lscheinnr":string, "fibu":string, "lief_nr":int, "lflag":bool})
    t_gl_acct_data, T_gl_acct = create_model_like(Gl_acct)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal receive_date, t_gl_acct_data, s_list_data, lvcarea, gl_acct, l_kredit, artikel
        ...
        return {"receive_date": receive_date, "t-gl-acct": t_gl_acct_data, "s-list": s_list_data}
