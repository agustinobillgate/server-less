...
def prepare_update_dynaratecode_webbl(booken_selected:int):
    ...
    t_queasy159_data, T_queasy159 = create_model_like(Queasy)
    t_queasy160_data, T_queasy160 = create_model_like(Queasy)
    t_queasy201_data, T_queasy201 = create_model_like(Queasy)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal ci_date, i_param439, cusername, cpassword, vcwsagent, htl_code, pushrate, queasy2_data, t_zimkateg_data, t_guest_pr_data, t_queasy159_data, t_queasy160_data, t_queasy201_data, gastnr, guest_pr, queasy, zimkateg
        nonlocal booken_selected


        nonlocal t_zimkateg, queasy2, t_guest_pr, t_queasy159, t_queasy160, t_queasy201
        nonlocal t_zimkateg_data, queasy2_data, t_guest_pr_data, t_queasy159_data, t_queasy160_data, t_queasy201_data

        return {"ci_date": ci_date, "i_param439": i_param439, "cusername": cusername, "cpassword": cpassword, "vcwsagent": vcwsagent, "htl_code": htl_code, "pushrate": pushrate, "queasy2": queasy2_data, "t-zimkateg": t_zimkateg_data, "t-guest-pr": t_guest_pr_data, "t-queasy159": t_queasy159_data, "t-queasy160": t_queasy160_data, "t-queasy201": t_queasy201_data}
    ...
    for guest_pr in db_session.query(Guest_pr).filter(
        ...
        t_guest_pr_data.append(t_guest_pr)
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        t_queasy159_data.append(t_queasy159)
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        t_queasy160_data.append(t_queasy160)
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        t_queasy201_data.append(t_queasy201)

        buffer_copy(queasy, t_queasy201)

    t_queasy160 = query(t_queasy160_data, filters=(lambda t_queasy160: t_queasy160.number1 == 3), first=True)
