...
def gcf_print_1bl(pvilanguage:int, fl_list:bool, city:string, sorttype:int, from_name:string, to_name:string, segm_all:bool, paytype:int, prtype:int, fl_email:bool, segmentcode:int, fdate:date, tdate:date, fl_mcid:bool, prov:bool):

    prepare_cache ([Guest, Mc_guest, Mc_types, Segment])
    ...
    gcf_print2_data, Gcf_print2 = create_model_like(Guest, {"ratecode":string, "memberno":string, "membertype":string, "count_num":int, "segment":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal output_list_data, gcf_print2_data, anzahl, lvcarea, guest, guestseg, guest_pr, mc_guest, mc_types, segment
        nonlocal pvilanguage, fl_list, city, sorttype, from_name, to_name, segm_all, paytype, prtype, fl_email, segmentcode, fdate, tdate, fl_mcid, prov
