...
def slc_aktcustbl(gname:string, user_init:string, sorttype:int):
    ...
    gast_data, Gast = create_model("Gast", {"gastnr":int, "name":string, "vorname1":string, "anrede1":string, "anredefirma":string, "adresse":string, "plz":string, "wohnort":string, "phonetik3":string, "karteityp":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal gast_data, akt_kont, guest
