...
def prepare_segm_budgetbl():
    ...
    from_date = None
    segment_list_data = []
    segmentstat_list_data = []
    segment = segmentstat = htparam = None

    segment_list = segmentstat_list = None

    segment_list_data, Segment_list = create_model_like(Segment)
    segmentstat_list_data, Segmentstat_list = create_model_like(Segmentstat)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal price_decimal, bill_date, from_date, segment_list_data, segmentstat_list_data, segment, segmentstat, htparam


        nonlocal segment_list, segmentstat_list
        nonlocal segment_list_data, segmentstat_list_data

        return {"price_decimal": price_decimal, "bill_date": bill_date, "from_date": from_date, "segment-list": segment_list_data, "segmentstat-list": segmentstat_list_data}
    ...
    segment_list = query(segment_list_data, first=True)
    ...
    if segment_list:
        ...
        for segmentstat in db_session.query(Segmentstat).filter(
            ...
            segmentstat_list_data.append(segmentstat_list)
