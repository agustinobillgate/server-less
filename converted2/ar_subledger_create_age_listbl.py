...
def ar_subledger_create_age_listbl(detail:bool, incl:bool, t_artnr:int, t_artart:int, t_dept:int, from_name:string, to_name:string, fr_date:date, to_date:date, bdate:date):
    ...
    def generate_output():
        nonlocal tot_debt, tot_paid, tot_bal, age_list_data, curr_art, debt_selected
        ...
        return {"tot_debt": tot_debt, "tot_paid": tot_paid, "tot_bal": tot_bal, "age-list": age_list_data}
    ...
    def create_age_list1():
        ...
        curr_art = t_artnr
        tot_debt, tot_paid, age_list_data = get_output(ar_subledger_create_age_list1bl(incl, t_artnr, t_dept, from_name, to_name, fr_date, to_date, bdate))
        ...
        if not detail:
            ...
            for age_list in query(age_list_data, sort_by=[("nr",True)]):
        ...
        if not detail and t_artart == 2:
            ...
            for age_list in query(age_list_data, filters=(lambda age_list: age_list.billname != "")):

                for abuff in query(abuff_data, filters=(lambda abuff: abuff.rechnr == age_list.rechnr and abuff.dept == age_list.dept and abuff.gastnr == age_list.gastnr and abuff._recid != age_list._recid)):
    ...
    def create_age_list1a():
        ...
        abuff_data = age_list_data
        curr_art = t_artnr
        tot_debt, tot_paid, age_list_data = get_output(ar_subledger_create_age_list1abl(incl, t_artnr, t_dept, from_name, to_name, fr_date, to_date, bdate))
        tot_bal =  to_decimal(tot_debt) - to_decimal(tot_paid)
        debt_selected = True
        pay_amt =  to_decimal("0")
