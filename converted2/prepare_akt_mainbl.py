...
def prepare_akt_mainbl(user_init:string, lname:string):
    ...
    q1_list_data, Q1_list = create_model("Q1_list", {"name":string, "anredefirma":string, "anrede1":string, "firmen_nr":int, "adresse1":string, "plz":string, "wohnort":string, "telefon":string, "fax":string, "rabatt":Decimal, "endperiode":date, "namekontakt":string, "gastnr":int, "userinit":string, "datum":date, "c_init":string, "rec_id":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal q1_list_data, guest, akt_cust
