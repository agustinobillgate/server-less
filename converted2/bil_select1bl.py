...
def bil_select1bl(bil_flag:int, sorttype:int, gastname:string, dept:int, ba_dept:int, rechnr:int):
    ...
    b1_list_data, B1_list = create_model("B1_list", {"resnr":int, "rechnr":int, "name":string, "vorname1":string, "anrede1":string, "saldo":Decimal, "printnr":int, "datum":date, "b_recid":int, "adresse1":string, "wohnort":string, "bemerk":string, "plz":string})
    ...
    def generate_output():
        nonlocal b1_list_data, fr_name, to_name, guest, bill
    ...
    def disp_bill_list0():
        ...
        if sorttype == 1 and gastname == "":
            ...
            bill = Bill()
            guest1 = Guest()
            for bill.resnr, bill.rechnr, bill.saldo, bill.printnr, bill.datum, bill._recid, guest1.name, guest1.vorname1, guest1.anrede1, guest1.adresse1, guest1.wohnort, guest1.bemerkung, guest1.plz, guest1._recid in db_session.query(Bill.resnr, Bill.rechnr, Bill.saldo, Bill.printnr, Bill.datum, Bill._recid, Guest1.name, Guest1.vorname1, Guest1.anrede1, Guest1.adresse1, Guest1.wohnort, Guest1.bemerkung, Guest1.plz, Guest1._recid).join(Guest1,(Guest1.gastnr == Bill.gastnr)).filter(
