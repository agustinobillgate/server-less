...
from models import Guest, Segment, Res_line, Htparam, Nation, Arrangement, Bill_line, Genfcast, Reservation, Zimkateg, Akt_cust, Bediener, Kontline, Waehrung, Artikel, Reslin_queasy, Argt_line, Queasy
...
def nt_genfcast():
    ...
    mm:int = 0
    yy:int = 0
    guest = segment = res_line = htparam = nation = arrangement = bill_line = genfcast = reservation = zimkateg = akt_cust = bediener = kontline = waehrung = artikel = reslin_queasy = argt_line = queasy = None
    ...
    def generate_output():
        nonlocal bill_date, created, price_decimal, rate, ratelocal, lodging, service, vat, vat2, fact, argt_betrag, ex_rate, exchg_rate, frate, def_nation, ratecode, do_it, rm_serv, rm_vat, serv_taxable, foreign_rate, new_contrate, dayuse, dd, mm, yy, guest, segment, res_line, htparam, nation, arrangement, bill_line, genfcast, reservation, zimkateg, akt_cust, bediener, kontline, waehrung, artikel, reslin_queasy, argt_line, queasy
    ...
    def genfcast_spaetabreise():
        ...
        nonlocal bill_date, created, price_decimal, rate, ratelocal, lodging, service, vat, vat2, fact, argt_betrag, ex_rate, exchg_rate, frate, def_nation, ratecode, rm_serv, rm_vat, serv_taxable, foreign_rate, new_contrate, dayuse, dd, mm, yy, guest, segment, res_line, htparam, nation, arrangement, bill_line, genfcast, reservation, zimkateg, akt_cust, bediener, kontline, waehrung, artikel, reslin_queasy, argt_line, queasy
        nonlocal rguest, compliment, rline
        ...
        for genfcast in db_session.query(Genfcast).filter(
                 (Genfcast.abreise == bill_date)).order_by(Genfcast._recid).all():
            do_it = True

            if genfcast.cancelled != None:
                do_it = False

            elif genfcast.noshow:
                do_it = False

            if do_it:

                arrangement = db_session.query(Arrangement).filter(
                         (Arrangement.arrangement == genfcast.argt)).first()

                bill_line = db_session.query(Bill_line).filter(
                         (Bill_line.departement == 0) & (Bill_line.artnr == arrangement.argt_artikelnr) & (Bill_line.bill_datum == bill_date) & (Bill_line.massnr == genfcast.resnr) & (Bill_line.billin_nr == genfcast.reslinnr)).first()

                if bill_line:

                    gbuff = db_session.query(Gbuff).filter(
                             (Gbuff._recid == genfcast._recid)).first()
    ...
    def create_genfcast():
        ...
        genfcast = db_session.query(Genfcast).filter(
                 (Genfcast.datum == created) & (Genfcast.resnr == res_line.resnr) & (Genfcast.reslinnr == res_line.reslinnr)).first()
    ...
    def genfcast_average_rmrate():
        ...
        for currdate in date_range(frdate,todate) :
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                     (func.lower(Reslin_queasy.key) == ("arrangement").lower()) & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.currdate >= Reslin_queasy.date1) & (Reslin_queasy.currdate <= Reslin_queasy.date2)).first()
            fixed_rate = None != reslin_queasy

            if reslin_queasy:
                rm_rate =  to_decimal(reslin_queasy.deci1)
                rm_rate, it_exist = usr_prog1(currdate, rm_rate)
            ...
            if not fixed_rate:
                ...
                if new_contrate:
                    ...
                    rm_rate, rate_found = get_output(pricecod_rate(res_line.resnr, res_line.reslinnr, guest_pr.code, currdate, res_line.ankunft, res_line.abreise, res_line.reserve_int, arrangement.argtnr, curr_zikatnr, res_line.erwachs, res_line.kind1, res_line.kind2, res_line.reserve_dec, res_line.betriebsnr))
                    rm_rate, it_exist = usr_prog2(currdate, rm_rate)
    ...
    def usr_prog1(bill_date:date, roomrate:decimal):
        ...
        nonlocal created, price_decimal, rate, ratelocal, lodging, service, vat, vat2, fact, argt_betrag, ex_rate, exchg_rate, frate, def_nation, ratecode, do_it, rm_serv, rm_vat, serv_taxable, foreign_rate, new_contrate, dayuse, dd, mm, yy, guest, segment, res_line, htparam, nation, arrangement, bill_line, genfcast, reservation, zimkateg, akt_cust, bediener, kontline, waehrung, artikel, reslin_queasy, argt_line, queasy
        nonlocal rguest, compliment, rline
        ...
        reslin_queasy = db_session.query(Reslin_queasy).filter(
                 (func.lower(Reslin_queasy.key) == ("rate-prog").lower()) & (Reslin_queasy.number1 == res_line.resnr) & (Reslin_queasy.number2 == 0) & (Reslin_queasy.char1 == "") & (Reslin_queasy.reslinnr == 1)).first()
        ...
        if prog_str != "":
            ...
            if not compiler:ERROR:
                roomrate = get_output(run_program(".\\_rate",(0, res_line.resnr, res_line.reslinnr, bill_date, roomrate, False)))
                it_exist = True
    ...
    def usr_prog2(bill_date:date, roomrate:decimal):
        ...
        queasy = db_session.query(Queasy).filter(
                 (Queasy.key == 2) & (Queasy.char1 == ratecode)).first()
