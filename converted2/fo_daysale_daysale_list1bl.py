...
def fo_daysale_daysale_list1bl(bline_list_data:[Bline_list], pvilanguage:int, shift:int, from_date:date, to_date:date):
    ...
    summary1_data, Summary1 = create_model("Summary1", {"usrinit":string, "username":string, "artnr":int, "amount":Decimal})
    cash_art_data, Cash_art = create_model("Cash_art", {"pos_nr":int, "artnr":int, "bezeich":string, "amount":Decimal, "tamount":Decimal})
    turnover_data, Turnover = create_model("Turnover", {"departement":int, "usr_nr":int, "name":string, "rechnr":string, "zinr":string, "info":string, "curr":string, "p_cash":Decimal, "f_cash":Decimal, "c_ledger":Decimal, "creditcard":Decimal, "flag":int, "gname":string}, {"zinr": ""})

    Cbuff = Cash_art
    cbuff_data = cash_art_data

    db_session = local_storage.db_session

    def generate_output():
        nonlocal curr_shift, nt_cash, nt_fcash, nt_ledger, nt_credit, turnover_data, summary1_data, cash_art_data, tot_cash, tot_fcash, tot_ledger, tot_credit, tot_amount, nt_amount, lvcarea, bediener, artikel, billjournal, bill, res_line, waehrung
        nonlocal pvilanguage, shift, from_date, to_date
        nonlocal cbuff


        nonlocal bline_list, summary1, cash_art, turnover, cbuff, tlist
        nonlocal summary1_data, cash_art_data, turnover_data

        return {"curr_shift": curr_shift, "nt_cash": nt_cash, "nt_fcash": nt_fcash, "nt_ledger": nt_ledger, "nt_credit": nt_credit, "turnover": turnover_data, "summary1": summary1_data, "cash-art": cash_art_data}
            ...
            if bediener:
                ...
                for billjournal.betriebsnr, billjournal.rechnr, billjournal.bezeich, billjournal.betrag, billjournal.fremdwaehrng, billjournal.userinit, billjournal._recid, artikel.betriebsnr, artikel.artnr, artikel.bezeich, artikel.artart, artikel._recid in db_session.query(Billjournal.betriebsnr, Billjournal.rechnr, Billjournal.bezeich, Billjournal.betrag, Billjournal.fremdwaehrng, Billjournal.userinit, Billjournal._recid, Artikel.betriebsnr, Artikel.artnr, Artikel.bezeich, Artikel.artart, Artikel._recid).join(Artikel,(Artikel.artnr == Billjournal.artnr) & (Artikel.departement == 0) & ((Artikel.artart == 2) | (Artikel.artart == 6) | (Artikel.artart == 7))).filter(
                    ...
                    if do_it:
                        ...
                        turnover = query(turnover_data, filters=(lambda turnover: turnover.rechnr == to_string(billjournal.rechnr) and turnover.usr_nr == bediener.nr), first=True)
                        ...
                        if artikel.artart == 2:
                            ...
                            cash_art = query(cash_art_data, filters=(lambda cash_art: cash_art.artnr == artikel.artnr), first=True)
                            ...
                            if not cash_art:
                                ...
                                cbuff = query(cbuff_data, last=True)
                            ...
                            summary1 = query(summary1_data, filters=(lambda summary1: summary1.usrinit == billjournal.userinit and summary1.artnr == artikel.artnr), first=True)

                            if not summary1:
                                summary1 = Summary1()
                                summary1_data.append(summary1)
