...
def occ_fcast_getdatabl(language_code:int, from_date:date, to_date:date, flag_i:int, all_segm:bool, all_argt:bool, all_zikat:bool, mi_lessooo:bool, mi_incltent:bool, rev_typ:int, mi_exclcomp:bool, all_outlook:bool, mi_lastyr:bool, segm_list_data:[Segm_list], argt_list_data:[Argt_list], zikat_list_data:[Zikat_list], outlook_list_data:[Outlook_list]):
    ...
    room_list_data, Room_list = create_model("Room_list", {"wd":int, "datum":date, "bezeich":string, "room":[Decimal,17], "coom":[string,17], "k_pax":int, "t_pax":int, "lodg":[Decimal,7], "avrglodg":Decimal, "avrglodg2":Decimal, "avrgrmrev":Decimal, "avrgrmrev2":Decimal, "others":[Decimal,8], "ly_fcast":string, "ly_actual":string, "ly_avlodge":string, "room_exccomp":int, "room_comp":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal msg_str, room_list_data, vhp_limited
        ...
        return {"msg_str": msg_str, "room-list": room_list_data}

    if to_date < from_date:
        msg_str = "ToDate can not be earlier THEN FromDate."

        return generate_output()
    room_list_data = get_output(cr_occfcast1_2bl(segm_list_data, argt_list_data, zikat_list_data, outlook_list_data, language_code, 0, flag_i, from_date, to_date, all_segm, all_argt, all_zikat, mi_lessooo, mi_incltent, rev_typ, vhp_limited, mi_exclcomp, all_outlook))

    if mi_lastyr:
        room_list_data = get_output(cr_occfcast1_1lybl(from_date, to_date, all_segm, all_argt, all_zikat, room_list_data, segm_list_data, argt_list_data, zikat_list_data))
