...
def prepare_htp_adminbl():
    ...
    s_list_data, S_list = create_model("S_list", {"nr":int})
    htp_list_data, Htp_list = create_model_like(Htparam)
    htgrp_data, Htgrp = create_model("Htgrp", {"number":int, "bezeich":string})
    htp_data, Htp = create_model("Htp", {"reihenfolge":int, "number":int, "bezeich":string, "typ":int, "logv":bool, "lupdate":date, "note":string, "wert":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal rest_flag, s_list_data, htp_data, htgrp_data, htp_list_data, fl_assign, gnr, anz_htp, arr, htp_arr, i, htparam, paramtext


        nonlocal s_list, htp_list, htgrp, htp
        nonlocal s_list_data, htp_list_data, htgrp_data, htp_data

        return {"rest_flag": rest_flag, "s-list": s_list_data, "htp": htp_data, "htgrp": htgrp_data, "htp-list": htp_list_data}
    ...
    def create_htgrp():
        ...
        nonlocal rest_flag, s_list_data, htp_data, htgrp_data, htp_list_data, fl_assign, gnr, anz_htp, arr, htp_arr, htparam, paramtext
    ...
    def create_htp(i:int):
        ...
        for htparam in db_session.query(Htparam).filter(
            ...
            if rest_flag:
                ...
                s_list = query(s_list_data, filters=(lambda s_list: s_list.nr == htparam.paramnr), first=True)
    ...
    htgrp = query(htgrp_data, first=True)
