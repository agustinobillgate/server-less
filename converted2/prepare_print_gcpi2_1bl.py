...
def prepare_print_gcpi2_1bl(docu_nr:string, user_init:string, printer_nr:int):
    ...
    output_list_data, Output_list = create_model("Output_list", {"docu_nr":string, "u_username":string, "b_username":string, "bezeich":string, "bemerk":string, "betrag":Decimal, "path":string, "avail_gc_pitype":bool})
    ...
    def generate_output():
        nonlocal output_list_data, t_gc_pibline_data, bediener, gc_pibline, gc_pi, gc_pitype, printer
        nonlocal docu_nr, user_init, printer_nr
        nonlocal ubuff


        nonlocal ubuff, output_list, t_gc_pibline
        nonlocal output_list_data, t_gc_pibline_data

        return {"output-list": output_list_data, "t-gc-PIbline": t_gc_pibline_data}
