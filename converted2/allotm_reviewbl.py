...
def allotm_reviewbl(pvilanguage:int, from_name:string, to_name:string, from_date:date, to_date:date, resflag:bool, gflag:bool, cflag:bool, detailflag:bool, curr_rmtype:string):

    prepare_cache ([Kontline, Bediener, Guest, Zimkateg, Guest_pr, Res_line, Reservation, Queasy])

    output_list_data = []
    lvcarea:string = "allotm-review"
    ...
    k_list_data, K_list = create_model("K_list", {"gastnr":int, "bediener_nr":int, "kontcode":string, "global_flag":bool, "global_str":string, "ankunft":date, "zikatnr":int, "argt":string, "zimmeranz":[int,31], "erwachs":int, "kind1":int, "ruecktage":int, "overbooking":int, "abreise":date, "useridanlage":string, "resdate":date, "bemerk":string}, {"global_str": ""})
    res_list_data, Res_list = create_model("Res_list", {"resno":int, "reslinnr":int, "flag":string, "count":int, "s1":string})
    output_list_data, Output_list = create_model("Output_list", {"reihe":int, "resno":int, "reslinnr":int, "str":string})
    ...
    def generate_output():
        nonlocal output_list_data, lvcarea, kontline, bediener, guest, zimkateg, guest_pr, res_line, reservation, queasy
        ...
        nonlocal k_list_data, res_list_data, output_list_data, allot_list_data
    ...
    def create_list():
        ...
        if k_list:
            ...
            for guest, usr in db_session.query(Guest, Usr).join(Usr,(Usr.nr == k_list.bediener_nr)).filter(
                ...
                res_line = Res_line()
                kontline = Kontline()
                for res_line.resnr, res_line.reslinnr, res_line.bemerk, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.active_flag, res_line._recid, kontline.zikatnr, kontline.kontcode, kontline.ankunft, kontline.arrangement, kontline.erwachs, kontline.kind1, kontline.ruecktage, kontline.overbooking, kontline.abreise, kontline.useridanlage, kontline.resdat, kontline.bemerk, kontline.gastnr, kontline.zimmeranz, kontline._recid in db_session.query(Res_line.resnr, Res_line.reslinnr, Res_line.bemerk, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.active_flag, Res_line._recid, Kontline.zikatnr, Kontline.kontcode, Kontline.ankunft, Kontline.arrangement, Kontline.erwachs, Kontline.kind1, Kontline.ruecktage, Kontline.overbooking, Kontline.abreise, Kontline.useridanlage, Kontline.resdat, Kontline.bemerk, Kontline.gastnr, Kontline.zimmeranz, Kontline._recid).join(Kontline,(Kontline.kontignr == Res_line.kontignr) & (Kontline.kontcode == k_list.kontcode) & (Kontline.kontstatus == 1)).filter(
    ...
    def create_alllist():
        ...
        guest = Guest()
        usr = Bediener()
        for kontline.zikatnr, kontline.kontcode, kontline.ankunft, kontline.arrangement, kontline.erwachs, kontline.kind1, kontline.ruecktage, kontline.overbooking, kontline.abreise, kontline.useridanlage, kontline.resdat, kontline.bemerk, kontline.gastnr, kontline.zimmeranz, kontline._recid, guest.gastnr, guest.name, guest.anredefirma, guest.adresse1, guest.adresse2, guest.wohnort, guest.plz, guest._recid, usr.userinit, usr.nr, usr._recid in db_session.query(Kontline.zikatnr, Kontline.kontcode, Kontline.ankunft, Kontline.arrangement, Kontline.erwachs, Kontline.kind1, Kontline.ruecktage, Kontline.overbooking, Kontline.abreise, Kontline.useridanlage, Kontline.resdat, Kontline.bemerk, Kontline.gastnr, Kontline.zimmeranz, Kontline._recid, Guest.gastnr, Guest.name, Guest.anredefirma, Guest.adresse1, Guest.adresse2, Guest.wohnort, Guest.plz, Guest._recid, Usr.userinit, Usr.nr, Usr._recid).join(Guest,(Guest.gastnr == Kontline.gastnr)).join(Usr,(Usr.nr == Kontline.bediener_nr)).filter(
            ...
            if matches(curr_rmtype,r"*-ALL-*"):
                ...
                zimkateg = get_cache (Zimkateg, {"kurzbez": [(eq, curr_rmtype)]})
    ...
    def create_alist():
        ...
        guest = Guest()
        usr = Bediener()
        for kontline.zikatnr, kontline.kontcode, kontline.ankunft, kontline.arrangement, kontline.erwachs, kontline.kind1, kontline.ruecktage, kontline.overbooking, kontline.abreise, kontline.useridanlage, kontline.resdat, kontline.bemerk, kontline.gastnr, kontline.zimmeranz, kontline._recid, guest.gastnr, guest.name, guest.anredefirma, guest.adresse1, guest.adresse2, guest.wohnort, guest.plz, guest._recid, usr.userinit, usr.nr, usr._recid in db_session.query(Kontline.zikatnr, Kontline.kontcode, Kontline.ankunft, Kontline.arrangement, Kontline.erwachs, Kontline.kind1, Kontline.ruecktage, Kontline.overbooking, Kontline.abreise, Kontline.useridanlage, Kontline.resdat, Kontline.bemerk, Kontline.gastnr, Kontline.zimmeranz, Kontline._recid, Guest.gastnr, Guest.name, Guest.anredefirma, Guest.adresse1, Guest.adresse2, Guest.wohnort, Guest.plz, Guest._recid, Usr.userinit, Usr.nr, Usr._recid).join(Guest,(Guest.gastnr == Kontline.gastnr) & (Guest.name >= (from_name).lower()) & (Guest.name <= (to_name).lower())).join(Usr,(Usr.nr == Kontline.bediener_nr)).filter(
                 (Kontline.betriebsnr == 0) & (Kontline.kontstatus == 1) & (not_ (Kontline.ankunft > to_date)) & (not_ (Kontline.abreise < from_date))).order_by(Guest.name, Kontline.kontcode, Kontline.ankunft).all():
            if kontline_obj_list.get(kontline._recid):
                continue
            else:
