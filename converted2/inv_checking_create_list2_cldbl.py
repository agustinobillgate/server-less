...
def inv_checking_create_list2_cldbl(coa_list_data:[Coa_list], invtype:int, d1:date, d2:date):
    ...
    def generate_output():
        nonlocal frnr, tonr, saldo, coa_list2_data, art_list3_data, d, mon, art1, art2, fibu, l_op, l_artikel, l_untergrup, l_ophis, gl_jouhdr, gl_journal
        nonlocal invtype, d1, d2


        nonlocal coa_list2, coa_list, art_list3
        nonlocal coa_list2_data, art_list3_data

        return {"frnr": frnr, "tonr": tonr, "saldo": saldo, "coa-list2": coa_list2_data, "art-list3": art_list3_data}
    ...
    def create_list2():
        ...
        if l_op:
            for d in date_range(d1,d2) :
                ...
                for l_op in db_session.query(L_op).filter(
                    ...
                    coa_list = query(coa_list_data, filters=(lambda coa_list: coa_list.fibukonto.lower()  == (fibu).lower()  and coa_list.datum == d), first=True)
                    ...
                    art_list3 = Art_list3()
                    art_list3_data.append(art_list3)
        ...
        for coa_list in query(coa_list_data):
            ...
            for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
                ...
                art_list3 = query(art_list3_data, filters=(lambda art_list3: art_list3.datum == coa_list.datum), first=True)

                if art_list3:
                    art_list3.saldo2 =  to_decimal(coa_list.debit) - to_decimal(coa_list.credit)

        for coa_list in query(coa_list_data, sort_by=[("datum",False)]):
            coa_list2 = Coa_list2()
            coa_list2_data.append(coa_list2)
    ...
    for coa_list2 in query(coa_list2_data, sort_by=[("datum",False)]):

        if coa_list2.diff == 0:
            coa_list2_data.remove(coa_list2)
