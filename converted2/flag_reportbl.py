...
s_list_data, S_list = create_model("S_list", {"newflag":bool, "id":string, "frdate":date, "datum":date, "note":string, "urgent":bool, "done":bool, "dept":string, "ciflag":bool, "coflag":bool}, {"newflag": True})

def flag_reportbl(case_type:int, n:int, resnr:int, reslinnr:int, user_init:string, s_list_data:[S_list]):
            ...
            for s_list in query(s_list_data, filters=(lambda s_list: s_list.datum != None and i < (k + 3)), sort_by=[("datum",False)]):
                ...
                if i == k:
                    ...
                    reslin_queasy.char1 = s_list.note +\
                            chr_unicode(2) + entry(0, s_list.id, chr_unicode(2))
                    ...
                    if num_entries(s_list.id, chr_unicode(2)) > 1:
                        reslin_queasy.char1 = reslin_queasy.char1 +\
                            chr_unicode(2) + entry(1, s_list.id, chr_unicode(2))
