...
def mn_del_old_resbl():
    ...
    def del_old_res():
        ...
        res_line = db_session.query(Res_line).filter(
                 ((Res_line.resstatus == 8) | (Res_line.resstatus == 12)) & (Res_line.active_flag == 2) & (Res_line.abreise < (ci_date - timedelta(days=anz)))).first()
        while None != res_line:
            ...
            if delete_it:
                ...
                for res_history in db_session.query(Res_history).filter(
                             (Res_history.resnr == res_line.resnr) & (Res_history.reslinnr == res_line.reslinnr) & (Res_history.datum >= res_line.ankunft) & (Res_history.zeit >= 0)).order_by(Res_history._recid).all():
                    db_session.delete(res_history)
                ...
                for mast_art in db_session.query(Mast_art).filter(
                             (Mast_art.resnr == res_line.resnr) & (Mast_art.reslinnr == res_line.reslinnr)).order_by(Mast_art._recid).all():
                    db_session.delete(mast_art)

                for res_history in db_session.query(Res_history).filter(
                             (Res_history.resnr == res_line.resnr) & (Res_history.reslinnr == res_line.reslinnr) & (Res_history.datum >= res_line.ankunft) & (Res_history.datum <= res_line.abreise) & (Res_history.zeit == 0) & (Res_history.action == ("N/A").lower())).order_by(Res_history._recid).all():
                    db_session.delete(res_history)
                pass
