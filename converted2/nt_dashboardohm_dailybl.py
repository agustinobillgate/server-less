...
from models import Guest, Res_line, Htparam, Billjournal, Bill, Artikel, Hoteldpt, Reservation, Segment, Genstat, Zimkateg, H_bill_line, H_artikel, Zkstat, Outorder, Zimmer, Akt_code, Zinrstat

def nt_dashboardohm_dailybl(pick_date:date, exclude_article:str, others_fb:str, guest_transport:str):
    his_res_list = []
    his_inv_list = []
    future_res_list = []
    future_inv_list = []
    ...
    outoforder:int = 0
    departure:int = 0
    compli_segment:int = 0
    guest = res_line = htparam = billjournal = bill = artikel = hoteldpt = reservation = segment = genstat = zimkateg = h_bill_line = h_artikel = zkstat = outorder = zimmer = akt_code = zinrstat = None

    his_res = his_inv = temp_res = future_res = depart_res = future_inv = non_room = t_list = b1_list = detail_res = gbuff = bufhis_inv = buffuture_inv = bufres_line = hbill_buff = hbill_list = None

    his_res_list, His_res = create_model("His_res", {"number":int, "resnr":int, "reslinnr":int, "staydate":date, "ci_date":date, "co_date":date, "nbrofroom":int, "room_type":str, "rm_rev":decimal, "fb_rev":decimal, "food_rev":decimal, "beverage_rev":decimal, "other_rev":decimal, "resstatus":str, "reserveid":int, "reservename":str, "bookdate":date, "rate_code":str, "nationality":str, "segmentcode":str, "cancel_date":str, "zinr":str, "adult":int, "child":int, "compliment":int, "others_fb":decimal, "guest_transport":decimal})
    ...
    depart_res_list, Depart_res = create_model_like(His_res)
    future_inv_list, Future_inv = create_model_like(His_inv)
    non_room_list, Non_room = create_model("Non_room", {"postingdate":date, "departmentid":int, "departmentname":str, "rm_rev":decimal, "food_rev":decimal, "beverage_rev":decimal, "other_rev":decimal, "others_fb":decimal, "guest_transport":decimal})
    t_list_list, T_list = create_model("T_list", {"departement":int, "rechnr":int, "bill_datum":date, "sysdate":date, "zeit":int, "food":decimal, "beverage":decimal, "other":decimal, "food_service":decimal, "beverage_service":decimal, "other_service":decimal, "pay":decimal, "compli":decimal})
    ...
    detail_res_list, Detail_res = create_model("Detail_res", {"datum":date, "cancel":int, "early_co":int, "outoforder":int, "departure":int})
    ...
    def generate_output():
        nonlocal his_res_list, his_inv_list, future_res_list, future_inv_list, non_room_list, b1_list_list, depart_res_list, detail_res_list, datum, datum2, ci_date, i, j, iftask, last_rechnr, curr_i, curr_date, end_date, fnet_lodg, net_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, tot_fb, do_it1, vat_proz, do_it, serv_taxable, serv, vat, netto, serv_betrag, outstr, outstr1, revcode, rechnr_nottax, bill_date, pay_amount, bill_resnr, bill_reslinnr, bill_parentnr, bill_gastnr, ex_article, t_reslinnr, cancel, early_co, outoforder, departure, compli_segment, guest, res_line, htparam, billjournal, bill, artikel, hoteldpt, reservation, segment, genstat, zimkateg, h_bill_line, h_artikel, zkstat, outorder, zimmer, akt_code, zinrstat
        nonlocal pick_date, exclude_article, others_fb, guest_transport
        nonlocal gbuff, bufhis_inv, buffuture_inv, bufres_line, hbill_buff


        nonlocal his_res, his_inv, temp_res, future_res, depart_res, future_inv, non_room, t_list, b1_list, detail_res, gbuff, bufhis_inv, buffuture_inv, bufres_line, hbill_buff, hbill_list
        nonlocal his_res_list, his_inv_list, temp_res_list, future_res_list, depart_res_list, future_inv_list, non_room_list, t_list_list, b1_list_list, detail_res_list, hbill_list_list

        return {"his-res": his_res_list, "his-inv": his_inv_list, "future-res": future_res_list, "future-inv": future_inv_list, "non-room": non_room_list, "b1-list": b1_list_list, "depart-res": depart_res_list, "detail-res": detail_res_list}
    ...
    compli_segment = htparam.finteger
    ...
    if pick_date != None:
        bill_date = pick_date - timedelta(days=1)
    else:
                ...
                if (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.zinr == "") or (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.comment == ""):
                    ...
                    if not his_res:
                        ...
                        reservation = db_session.query(Reservation).filter(
                                     (Reservation.resnr == bill_resnr)).first()
                    ...
                    elif revcode.lower()  == ("ATF").lower() :
                        his_res.food_rev =  to_decimal(his_res.food_rev) + to_decimal(netto)

                    elif revcode.lower()  == ("ATB").lower() :
                        his_res.beverage_rev =  to_decimal(his_res.beverage_rev) + to_decimal(netto)
                    ...
                    if not his_res:
                        ...
                        res_line = db_session.query(Res_line).filter(
                                     (Res_line.resnr == bill_resnr) & (Res_line.reslinnr == t_reslinnr)).first()
                        ...
                        if res_line:
                            ...
                            his_res.compliment = res_line.gratis
                            his_res.staydate = billjournal.bill_datum
                            ...
                            if res_line.resstatus == 12:
                                ...
                                bufres_line = db_session.query(Bufres_line).filter(
                                             (Bufres_line.resnr == bill_resnr) & (Bufres_line.reslinnr == bill_parentnr)).first()

                                if bufres_line:
                                    his_res.nbrofroom = 0

                                    genstat = db_session.query(Genstat).filter(
                                                 (Genstat.resnr == bufres_line.resnr) & (Genstat.res_int[inc_value(0)] == bufres_line.reslinnr)).first()
                                ...
                                genstat = db_session.query(Genstat).filter(
                                             (Genstat.resnr == res_line.resnr) & (Genstat.res_int[inc_value(0)] == res_line.reslinnr)).first()
                    ...
                    if revcode.lower()  == ("ATF").lower() :
                        non_room.food_rev =  to_decimal(non_room.food_rev) + to_decimal(netto)

                    elif revcode.lower()  == ("ATB").lower() :
                        non_room.beverage_rev =  to_decimal(non_room.beverage_rev) + to_decimal(netto)
    ...
    for hbill_list in query(hbill_list_list, filters=(lambda hbill_list: hbill_list.do_it and hbill_list.i_fact != 0)):
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).filter(
            ...
            if artikel.artart == 9:
                ...
                if artikel.umsatzart == 3 or artikel.umsatzart == 5:
                    t_list.food =  to_decimal(t_list.food) + to_decimal((netto) * to_decimal(hbill_list.i_fact) )
                    t_list.food_service =  to_decimal(t_list.food_service) + to_decimal((serv_betrag) * to_decimal(hbill_list.i_fact) )

                elif artikel.umsatzart == 6:
                    t_list.beverage =  to_decimal(t_list.beverage) + to_decimal((netto) * to_decimal(hbill_list.i_fact) )
                    t_list.beverage_service =  to_decimal(t_list.beverage_service) + to_decimal((serv_betrag) * to_decimal(hbill_list.i_fact) )
    ...
    for t_list in query(t_list_list):
        ...
        if not non_room:
            ...
            if hoteldpt:
                non_room.departmentname = hoteldpt.depart
        non_room.food_rev =  to_decimal(non_room.food_rev) + to_decimal(t_list.food)
        non_room.beverage_rev =  to_decimal(non_room.beverage_rev) + to_decimal(t_list.beverage)
    ...
    while None != res_line:
        ...
        for datum in date_range(res_line.ankunft,datum2) :
            ...
            if datum == bill_date:
                ...
                his_res.rm_rev =  to_decimal(tot_rmrev)
                his_res.food_rev =  to_decimal(tot_fb)
    ...
    res_line = db_session.query(Res_line).filter(
             (Res_line.abreise >= ci_date) & (Res_line.resstatus != 4) & (Res_line.resstatus != 8) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.active_flag <= 1) & (Res_line.l_zuordnung[inc_value(2)] == 0)).first()
    while None != res_line:
        ...
        for datum in date_range(res_line.ankunft,datum2) :
            ...
            if datum >= ci_date:
                ...
                future_res.rm_rev =  to_decimal(tot_rmrev)
    ...
    genstat = db_session.query(Genstat).filter(
             (Genstat.datum == bill_date) & (Genstat.resstatus != 0) & (Genstat.res_logi[inc_value(1)]) & (Genstat.zikatnr != 0)).first()
    while None != genstat:
        ...
        if genstat.resstatus != 13 and genstat.zinr != "":
            ...
            if genstat.gratis != 0 or (genstat.zipreis == 0 and genstat.erwachs > 0):
                ...
                if segment and segment.betriebsnr == 2:

                    his_inv = query(his_inv_list, filters=(lambda his_inv: his_inv.datum == genstat.datum and his_inv.zikatnr == genstat.zikatnr and his_inv.room_status.lower()  == ("HOUSEUSE").lower()), first=True)
                    ...
                    if not his_inv:
                        ...
                        his_inv.qty = 1
                        his_inv.room_status = "HOUSEUSE"


                    else:
                        his_inv.qty = his_inv.qty + 1

                elif segment and segment.betriebsnr != 2:
    ...
    zkstat = db_session.query(Zkstat).filter(
             (Zkstat.datum == bill_date) & (Zkstat.zikatnr != 0)).first()
    while None != zkstat:
    ...
    outorder = db_session.query(Outorder).filter(
             (Outorder.gespstart <= bill_date) & (Outorder.gespende >= bill_date) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1)).first()
    while None != outorder:
        ...
        if zimmer and zimmer.sleeping :
            for datum2 in date_range(outorder.gespstart,outorder.gespende) :
                ...
                if datum2 == bill_date:
                    ...
                    his_inv = query(his_inv_list, filters=(lambda his_inv: his_inv.datum == datum2 and his_inv.room_type == zimmer.kbezeich and his_inv.room_status == "outoforder"), first=True)
    ...
    for his_inv in query(his_inv_list):
        ...
        zimkateg = db_session.query(Zimkateg).filter(
                 (Zimkateg.zikatnr == his_inv.zikatnr)).first()
    ...
    for his_inv in query(his_inv_list, filters=(lambda his_inv: his_inv.room_status.lower()  == ("AVAILABLE").lower())):
        ...
        bufhis_inv = query(bufhis_inv_list, filters=(lambda bufhis_inv: bufhis_inv.room_status == "outoforder" and bufhis_inv.datum == his_inv.datum and bufhis_inv.room_type == his_inv.room_type), first=True)
        ...
        bufhis_inv = query(bufhis_inv_list, filters=(lambda bufhis_inv: bufhis_inv.room_status.lower()  == ("HOUSEUSE").lower()  and bufhis_inv.datum == his_inv.datum and bufhis_inv.room_type == his_inv.room_type), first=True)
                ...
                if datum2 >= ci_date and datum2 <= ci_date + timedelta(days=90):
                    ...
                    if res_line.gratis != 0 or (res_line.zipreis == 0 and res_line.erwachs > 0):
                        ...
                        if segment and segment.betriebsnr == 2:
                            ...
                            future_inv = query(future_inv_list, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.zikatnr == res_line.zikatnr and future_inv.room_status.lower()  == ("HOUSEUSE").lower()), first=True)
                            ...
                            if not future_inv:
                                ...
                                future_inv.room_status = "HOUSEUSE"
    ...
    outorder = db_session.query(Outorder).filter(
             (Outorder.gespende >= ci_date) & (Outorder.gespstart <= ci_date + timedelta(days=90)) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1)).first()
    while None != outorder:
        ...
        if zimmer and zimmer.sleeping :
            for datum2 in date_range(outorder.gespstart,outorder.gespende) :
                ...
                if datum2 >= ci_date and datum2 <= ci_date + timedelta(days=90):
                    ...
                    future_inv = query(future_inv_list, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.room_type == zimmer.kbezeich and future_inv.room_status == "outoforder"), first=True)
    ...
    for future_inv in query(future_inv_list):
        ...
        zimkateg = db_session.query(Zimkateg).filter(
                 (Zimkateg.zikatnr == future_inv.zikatnr)).first()
    ...
    for future_inv in query(future_inv_list, filters=(lambda future_inv: future_inv.room_status.lower()  == ("AVAILABLE").lower())):
        ...
        buffuture_inv = query(buffuture_inv_list, filters=(lambda buffuture_inv: buffuture_inv.room_status == "outoforder" and buffuture_inv.datum == future_inv.datum and buffuture_inv.room_type == future_inv.room_type), first=True)
        ...
        buffuture_inv = query(buffuture_inv_list, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("HOUSEUSE").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.room_type == future_inv.room_type), first=True)
    ...
    if pick_date == None:
        ...
        for zinrstat, akt_code in db_session.query(Zinrstat, Akt_code).join(Akt_code,(Akt_code.aktionscode == Zinrstat.betriebsnr) & (Akt_code.aktiongrup == 4)).filter(
                 (func.lower(Zinrstat.zinr) == ("Competitor").lower()) & (Zinrstat.datum == ci_date - timedelta(days=2))).order_by(Zinrstat._recid).all():
        ...
        zinrstat_obj_list = []
        for zinrstat, akt_code in db_session.query(Zinrstat, Akt_code).join(Akt_code,(Akt_code.aktionscode == Zinrstat.betriebsnr) & (Akt_code.aktiongrup == 4)).filter(
                 (func.lower(Zinrstat.zinr) == ("Competitor").lower()) & ((Zinrstat.datum == ci_date) | (Zinrstat.datum == ci_date - timedelta(days=1)))).order_by(Zinrstat._recid).all():
            if zinrstat._recid in zinrstat_obj_list:
                continue
            else:
                zinrstat_obj_list.append(zinrstat._recid)
    ...
    for res_line in db_session.query(Res_line).filter(
             (Res_line.resstatus == 8) & (Res_line.abreise == ci_date - timedelta(days=1)) & (Res_line.active_flag == 2)).order_by(Res_line._recid).all():
        depart_res = Depart_res()
        depart_res_list.append(depart_res)

        depart_res.resnr = res_line.resnr
        depart_res.reslinnr = res_line.reslinnr
        depart_res.nbrofroom = res_line.zimmeranz
        depart_res.adult = res_line.erwachs
        depart_res.child = res_line.kind1
        depart_res.compliment = res_line.gratis
        depart_res.ci_date = res_line.ankunft
        depart_res.co_date = res_line.abreise

        reservation = db_session.query(Reservation).filter(
                 (Reservation.resnr == res_line.resnr)).first()

        if reservation and reservation.resdat != None:
            depart_res.bookdate = reservation.resdat
    cancel = 0
    early_co = 0
    outoforder = 0
    departure = 0
    ...
    for res_line in db_session.query(Res_line).filter(
             (Res_line.resstatus == 9) & (Res_line.cancelled == ci_date - timedelta(days=1)) & (Res_line.betrieb_gastpay != 11) & (Res_line.betrieb_gastpay != 13) & (Res_line.betrieb_gastpay != 3) & (Res_line.betrieb_gastpay != 4)).order_by(Res_line._recid).all():
        cancel = cancel + res_line.zimmeranz

    for res_line in db_session.query(Res_line).filter(
             (Res_line.active_flag == 2) & (Res_line.resstatus == 8) & (Res_line.abreise >= ci_date - timedelta(days=1)) & (Res_line.abreise <= ci_date - timedelta(days=1)) & ((Res_line.abreise - Res_line.ankunft) < Res_line.anztage)).order_by(Res_line._recid).all():
        early_co = early_co + res_line.zimmeranz

    outorder_obj_list = []
    for outorder, zimmer in db_session.query(Outorder, Zimmer).join(Zimmer,(Zimmer.zinr == Outorder.zinr) & (Zimmer.sleeping)).filter(
             (Outorder.gespstart <= ci_date - timedelta(days=1)) & (Outorder.gespende >= ci_date - timedelta(days=1)) & (Outorder.betriebsnr <= 1)).order_by(Outorder._recid).all():
    ...
    for res_line in db_session.query(Res_line).filter(
             (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.abreise == ci_date)).order_by(Res_line._recid).all():
        departure = departure + res_line.zimmeranz
    detail_res = Detail_res()
    detail_res_list.append(detail_res)

    detail_res.datum = ci_date - timedelta(days=1)
    detail_res.cancel = cancel
    detail_res.early_co = early_co
    detail_res.outoforder = outoforder
    detail_res.departure = departure
