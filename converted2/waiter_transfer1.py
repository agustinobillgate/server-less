...
def waiter_transfer1(k1:int, k2:int, tablenr:int, curr_dept:int, bill_date:date):
    ...
    def generate_output():
        ...
        nonlocal k1, k2, tablenr, curr_dept, bill_date
    ...
    hbill = db_session.query(Hbill).filter(
             (Hbill.flag == 0) & (Hbill.departement == curr_dept) & (Hbill.tischnr == tablenr)).first()
    hbill.kellner_nr = k2
