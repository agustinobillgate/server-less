...
def load_macro_webbl(briefnr:int, excel_list_data:[Excel_list]):
    ...
    def generate_output():
        nonlocal coa_list_data, error_list_data, error_flag, flag_combo, row_combo, str, str2, parameters, gl_acct, artikel
        nonlocal briefnr
        nonlocal parambuff


        nonlocal excel_list, coa_list, error_list, t_parameters, parambuff
        nonlocal coa_list_data, error_list_data, t_parameters_data

        return {"coa-list": coa_list_data, "error-list": error_list_data, "error_flag": error_flag}
        ...
        if substring(excel_list.curr_val, 0, 1) == ("$").lower()  or substring(excel_list.curr_val, 0, 1) == ("^").lower() :
            ...
            if substring(excel_list.curr_val, 0, 1) == ("$").lower() :
                ...
                if not gl_acct:
                    ...
                    coa_list = query(coa_list_data, filters=(lambda coa_list: coa_list.fibukonto == gl_acct.fibukonto), first=True)
