...
def ci_cancelbl(pvilanguage:int, sortby:int, date1:date):
    ...
    ci_list_data, Ci_list = create_model("Ci_list", {"resno":int, "resname":string, "gname":string, "arrive":date, "depart":date, "rmno":string, "roomcat":string, "qty":int, "resstate":string, "usrid":string, "cancel":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal ci_list_data, lvcarea, stat_list, res_line, reservation, guest, zimkateg, bediener, res_history
        nonlocal pvilanguage, sortby, date1


        nonlocal ci_list
        nonlocal ci_list_data

        return {"ci-list": ci_list_data}
