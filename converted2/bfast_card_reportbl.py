...
def bfast_card_reportbl(room:string, fdate:date, mealtime:string, dummyinput:string):
    ...
    bfast_data_data, Bfast_data = create_model("Bfast_data", {"tdate":date, "roomnr":string, "resnr":int, "guestnr":int, "vip":bool, "nation":string, "guestname":string, "totaladult":int, "totalcompli":int, "totalchild":int, "totaluse":int, "dummychr":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal bfast_data_data, getdatanr, i, ii, sumadult, sumcompl, sumchild, sumtotuse, dateval, artgrp_abf, artgrp_lunch, artgrp_dinner, tempresnr, strdayuse, strday, htparam, mealcoup, res_line, guest, arrangement, argt_line
