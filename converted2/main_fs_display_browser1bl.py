...
def main_fs_display_browser1bl(glist_data:[Glist], b1_resnr:int, b1_resline:int, search_str:string, curr_gastnr:int, guestsort:int, rsvsort:int, to_name:string):
    ...
    ol_data, Ol = create_model("Ol", {"str":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal op_flag, ol_data, bk_reser, bk_veran
        ...
        return {"op_flag": op_flag, "ol": ol_data}
    ...
    if search_str == "" and curr_gastnr == 0:
        ...
        if guestsort < 3:
            ...
            for glist in query(glist_data, filters=(lambda glist: glist.karteityp == guestsort), sort_by=[("name",False)]):
        ...
        for glist in query(glist_data, filters=(lambda glist: glist.karteityp == guestsort and glist.name.lower()  >= (search_str).lower()  and glist.name.lower()  <= (to_name).lower()), sort_by=[("name",False)]):
        ...
        for glist in query(glist_data, filters=(lambda glist: glist.karteityp == guestsort and glist.gastnr >= curr_gastnr), sort_by=[("name",False)]):
