...
from models import Paramtext, Htparam, Nation, Nightaudit, Nitestor, Guest, Zimmer, Zimkateg, Reservation, Res_line, Guestseg
...
def nt_pjinhouse():
    ...
    htl_adr:str = ""
    htl_tel:str = ""
    paramtext = htparam = nation = nightaudit = nitestor = guest = zimmer = zimkateg = reservation = res_line = guestseg = None
    ...
    def generate_output():
        nonlocal domestic, tot_avail, tot_rm, inactive, tot_a, tot_c, tot_co, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, n, progname, night_type, reihenfolge, line_nr, line, p_width, p_length, curr_date, htl_name, htl_adr, htl_tel, paramtext, htparam, nation, nightaudit, nitestor, guest, zimmer, zimkateg, reservation, res_line, guestseg
    ...
    def inhouse_list():
        ...
        add_line(line)
        add_line(" ")
        line = "IN-HOUSE GUEST LIST" + " - " + to_string(curr_date)
        add_line(line)
        add_line(str1)
        line = " " + "No RmNo RmCat Guest-Name VIP A/C Co Nat" + " "
        line = line + "Company/TA CheckOut Flight Time Remark"
        add_line(line)
        add_line(str1)
        add_line(" ")
        add_line("##end-header")
        ...
        if not it_exist:
            ...
            for i in range(1,13 + 1) :
                line = line + " "
            line = line + "ROOM AVAILABLE" + " " + to_string(tot_avail, ">>>9")
            add_line(line)
            line = ""
            for i in range(1,13 + 1) :
                line = line + " "
            ...
            if inactive != 0:
                line = line + "OCCUPIED/inactive" + " " + to_string(tot_rm, ">>>9")
                line = line + "/" + to_string(inactive)
                for i in range(1,(26 - len(trim(to_string(inactive, ">>9"))))  + 1) :
                    line = line + " "
            else:
                line = line + "T O T A L OCCUPIED" + " " + to_string(tot_rm, ">>>9")
            ...
            if tot_avail != 0:
                line = line + "ROOM OCCUPANCY" + " (%) " + to_string(tot_rm / tot_avail * 100, ">>9.99") + to_string("", "x(6)") + "AVERAGE GUEST/ROOM" + " " + to_string((tot_a + tot_co) / tot_rm, ">9.99")
            add_line(line)
    ...
    def create_inhouse():
        ...
        for res_line, zimkateg, reservation, guest, gmember in db_session.query(Res_line, Zimkateg, Reservation, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                 (Res_line.active_flag == 1) & (Res_line.resstatus != 12) & (Res_line.abreise > curr_date)).order_by(Res_line.zinr, Res_line.name).all():
