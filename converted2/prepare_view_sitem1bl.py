...
def prepare_view_sitem1bl(user_init:string, docu_nr:string):
    ...
    sitem_list_data, Sitem_list = create_model("Sitem_list", {"artnr":int, "bezeich":string, "lief_fax":[string,3], "txtnr":int, "anzahl":Decimal, "geliefert":Decimal, "einzelpreis":Decimal, "warenwert":Decimal, "lieferdatum_eff":date, "angebot_lief":[int,3], "masseinheit":string, "jahrgang":int, "quality":string, "pos":int, "remark":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal show_price, comments, sitem_list_data, bediener, htparam, l_artikel, l_order, l_orderhdr
        nonlocal user_init, docu_nr


        nonlocal sitem_list
        nonlocal sitem_list_data

        return {"show_price": show_price, "comments": comments, "sitem-list": sitem_list_data}
    ...
    def assign_it():
        ...
        sitem_list_data.append(sitem_list)
    ...
    if show_price:
        ...
        l_order = L_order()
        l_artikel = L_artikel()
        for l_order.artnr, l_order.lief_fax, l_order.txtnr, l_order.anzahl, l_order.geliefert, l_order.einzelpreis, l_order.warenwert, l_order.lieferdatum_eff, l_order.angebot_lief, l_order.quality, l_order.pos, l_order.besteller, l_order.lief_nr, l_order._recid, l_artikel.bezeich, l_artikel.masseinheit, l_artikel.jahrgang, l_artikel._recid in db_session.query(L_order.artnr, L_order.lief_fax, L_order.txtnr, L_order.anzahl, L_order.geliefert, L_order.einzelpreis, L_order.warenwert, L_order.lieferdatum_eff, L_order.angebot_lief, L_order.quality, L_order.pos, L_order.besteller, L_order.lief_nr, L_order._recid, L_artikel.bezeich, L_artikel.masseinheit, L_artikel.jahrgang, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
