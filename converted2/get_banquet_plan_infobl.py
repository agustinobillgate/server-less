...
def get_banquet_plan_infobl(curr_view:string, rml_bezeich:string, rml_raum:string, rml_nr:int, rml_blocked:int, rml_resnr:int, rml_reslinnr:int, curr_i:int, from_date:date):
    ...
    rsl_data, Rsl = create_model("Rsl", {"resnr":int, "reslinnr":int, "resstatus":int, "sdate":date, "ndate":date, "stime":string, "ntime":string, "created_date":date, "venue":string, "userinit":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal mess_str, info1, info2, info3, rsl_data, curr_room, curr_status, odate, b1_resnr, b1_reslinnr, out_i
        ...
        return {"mess_str": mess_str, "info1": info1, "info2": info2, "info3": info3, "rsl": rsl_data}
    ...
    def disp_resdata():
        ...
        i:int = 0
        init_info()
        info1, info2, info3, rsl_data = get_output(ba_plan_disp_resdatabl(rml_raum, rml_resnr, rml_reslinnr))
