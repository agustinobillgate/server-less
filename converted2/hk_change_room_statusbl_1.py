...
def hk_change_room_statusbl(pvilanguage:int, chgsort:int, user_init:str, room_list_list:[Room_list]):
    ...
    if chgsort != 4 and chgsort != 5:
        ...
        for room_list in query(room_list_list):
            ...
            # print("RMList:", room_list)
            local_storage.debugging = local_storage.debugging + ",Nr:" + str(room_list.nr)

        flag, t_zinr, t_zistatus = get_output(hk_statadmin_start_chgstatbl(chgsort, bline_list))
        local_storage.debugging = local_storage.debugging + ",Flag:" + str(flag)
        ...
        if flag == 6:
            ...
            if t_zistatus == 6:
                ...
                if chgsort == 4:
                    ...
                    bline_list_list, om_list_list, z_list_list = get_output(hk_statadmin_deactivate_ooobl(bline_list, om_list, ci_date, bediener.nr, chgsort))
            else:
                bline_list_list, om_list_list, curr_zinr, curr_stat, z_list_list = get_output(hk_statadmin_chg_zistatusbl(bline_list, om_list, 0, ci_date, chgsort, "", user_init, bediener.nr))
        else:
