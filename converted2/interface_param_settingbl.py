...
hotel_config_data, Hotel_config = create_model("Hotel_config", {"keynr":int, "number1":int, "number2":int, "number3":int, "date1":date, "date2":date, "date3":date, "char1":string, "char2":string, "char3":string, "deci1":Decimal, "deci2":Decimal, "deci3":Decimal, "logi1":bool, "logi2":bool, "logi3":bool, "betriebsnr":int})

def interface_param_settingbl(case_type:int, hotel_config_data:[Hotel_config]):
    mess_result = ""
    queasy = None
    ...
    def generate_output():
        ...
        return {"hotel-config": hotel_config_data, "mess_result": mess_result}

    if case_type == 1:

        hotel_config = query(hotel_config_data, first=True)
    ...
    if queasy:
        ...
        hotel_config_data.append(hotel_config)
