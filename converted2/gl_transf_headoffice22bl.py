...
def gl_transf_headoffice22bl(htl_code:string, close_month:date, close_year:date, t_gl_acct_data:[T_gl_acct], t_gl_accthis_data:[T_gl_accthis]):
    ...
    hotel_list_data, Hotel_list = create_model("Hotel_list", {"selectflag":bool, "htl_code":string, "i_hotel":int, "c_hotel":string, "i_brand":int, "i_country":int, "i_region":int, "i_area":int, "i_city":int, "dispflag":bool, "c_users":string}, {"dispflag": True})
    ...
    t_gl_acct = query(t_gl_acct_data, first=True)
    while None != t_gl_acct:
        ...
        t_gl_acct = query(t_gl_acct_data, next=True)

    if close_year != None:
        i_year = get_year(close_year)

    t_gl_accthis = query(t_gl_accthis_data, first=True)
    while None != t_gl_accthis:
        ...
        t_gl_accthis = query(t_gl_accthis_data, next=True)
