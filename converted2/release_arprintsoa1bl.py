...
def release_arprintsoa1bl(guestno:int):
    ...
    def generate_output():
        nonlocal exist, bill_list_data, guestname, guest, debitor, bill, queasy, h_bill
        nonlocal guestno


        nonlocal bill_list
        nonlocal bill_list_data

        return {"exist": exist, "bill-list": bill_list_data}
    ...
    def cr_bill_list():
        ...
        for bill_list in query(bill_list_data):
        ...
        for debitor in db_session.query(Debitor).filter(
            ...
            if debitor.betriebsnr == 0:
                ...
                if bill and bill.rechnr != 0:
                    ...
                    bill_list = query(bill_list_data, filters=(lambda bill_list: bill_list.rechnr == bill.rechnr and bill_list.billref == bill.billref), first=True)
                    ...
                    if queasy:
                        ...
                        bill_list = query(bill_list_data, filters=(lambda bill_list: bill_list.rechnr == queasy.number1 and bill_list.billref == queasy.number2), first=True)
                ...
                if h_bill and h_bill.rechnr != 0:
                    ...
                    bill_list = query(bill_list_data, filters=(lambda bill_list: bill_list.rechnr == bill.rechnr), first=True)
