...
def selforder_loadarticlebl(outlet_no:int):
    ...
    def generate_output():
        nonlocal mess_result, article_list_data, maingroup_list_data, subgroup_list_data, ct, l_deci, serv_vat, tax_vat, tax, serv, service, vat, vat2, fact_scvat, servtax_use_foart, service_foreign, serv_code, vat_code, hoteldpt, queasy, h_artikel, artikel, htparam
        nonlocal outlet_no
        nonlocal bqsy


        nonlocal article_list, maingroup_list, subgroup_list, bqsy
        nonlocal article_list_data, maingroup_list_data, subgroup_list_data

        return {"mess_result": mess_result, "article-list": article_list_data, "maingroup-list": maingroup_list_data, "subgroup-list": subgroup_list_data}
    ...
    for h_artikel in db_session.query(H_artikel).filter(
        ...
        if queasy:
            ...
            article_list_data.append(article_list)
    ...
    for article_list in query(article_list_data, filters=(lambda article_list: article_list.art_active_flag == False)):
        article_list_data.remove(article_list)
