...
def prepare_eg_repdurationbl(all_maintask:bool, all_room:bool, user_init:string):
    ...
    def generate_output():
        nonlocal engid, groupid, ci_date, tlocation_data, tcategory_data, troom_data, tmaintask_data, tproperty_data, tpic_data, tsubtask_data, tsource_data, queasy, htparam, bediener, eg_location, zimmer, eg_property, eg_staff, eg_subtask
        ...
        nonlocal tsource_data, tsubtask_data, tpic_data, tproperty_data, tmaintask_data, troom_data, tlocation_data, tcategory_data

        return {"engid": engid, "groupid": groupid, "ci_date": ci_date, "tLocation": tlocation_data, "tcategory": tcategory_data, "troom": troom_data, "tMaintask": tmaintask_data, "tproperty": tproperty_data, "tpic": tpic_data, "tsubtask": tsubtask_data, "tsource": tsource_data}
    ...
    def create_room():
        ...
        Tbuff = Tlocation
        tbuff_data = tlocation_data
        troom_data.clear()

        tbuff = query(tbuff_data, filters=(lambda tbuff: tbuff.loc_selected  and tbuff.loc_guest), first=True)
        ...
        for comlocat in query(comlocat_data, filters=(lambda comlocat: comlocat.loc_selected)):
            ...
            if comlocat.loc_guest :
                ...
                for pbuff in db_session.query(Pbuff).filter(
                    ...
                    commain = query(commain_data, (lambda commain: commain.main_nr == pbuff.maintask and commain.main_selected), first=True)
                    if not commain:
                        continue

                    comroom = query(comroom_data, (lambda comroom: comroom.room_nm == pbuff.zinr and comroom.room_selected), first=True)
                    if not comroom:
                        continue
