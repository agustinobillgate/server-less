...
def read_nitehistbl(case_type:int, int1:int, int2:int, date1:date, char1:string):
    ...
    t_nitehist_data, T_nitehist = create_model_like(Nitehist)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_nitehist_data, nitehist
        nonlocal case_type, int1, int2, date1, char1


        nonlocal t_nitehist
        nonlocal t_nitehist_data

        return {"t-nitehist": t_nitehist_data}
    ...
    if case_type == 1:
        ...
        if nitehist:
            ...
            t_nitehist_data.append(t_nitehist)
