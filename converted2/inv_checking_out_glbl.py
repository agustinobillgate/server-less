...
def inv_checking_out_glbl(invtype:int, d1:date, d2:date):
    ...
    def generate_output():
        nonlocal frnr, tonr, saldo, coa_list3_data, art_list4_data, d, mon, art1, art2, fibu, l_op, l_artikel, l_untergrup, l_ophis, gl_jouhdr, gl_journal
        nonlocal invtype, d1, d2


        nonlocal coa_list, coa_list3, art_list4
        nonlocal coa_list_data, coa_list3_data, art_list4_data

        return {"frnr": frnr, "tonr": tonr, "saldo": saldo, "coa-list3": coa_list3_data, "art-list4": art_list4_data}
    ...
    def out_gl():
        ...
        for d in date_range(d1,d2) :
            ...
            for l_op in db_session.query(L_op).filter(
                ...
                art_list4 = Art_list4()
                art_list4_data.append(art_list4)
        ...
        for coa_list in query(coa_list_data):
            ...
            for gl_jouhdr in db_session.query(Gl_jouhdr).filter(
                ...
                art_list4 = query(art_list4_data, filters=(lambda art_list4: art_list4.datum == coa_list.datum), first=True)
        ...
        for coa_list in query(coa_list_data, sort_by=[("datum",False)]):
            coa_list3 = Coa_list3()
            coa_list3_data.append(coa_list3)
    ...
    for coa_list3 in query(coa_list3_data, sort_by=[("datum",False)]):

        if coa_list3.diff == 0:
            coa_list3_data.remove(coa_list3)
