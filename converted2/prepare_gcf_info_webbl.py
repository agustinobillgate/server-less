...
def prepare_gcf_info_webbl(pvilanguage:int, gastnr:int):
    ...
    gcfinfo_list_data, Gcfinfo_list = create_model("Gcfinfo_list", {"nr":int, "dept":int, "rechnr":int, "datum":date, "str":string, "dept_str":string, "amount_food":string, "amount_bev":string, "amount_other":string, "room_number":string, "room_type":string, "arrival":string, "departure":string})
    t_guest_data, T_guest = create_model_like(Guest)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal payment, t_guest_data, gcfinfo_list_data, guest, artikel, res_line, hoteldpt, guest_queasy, zimmer, zimkateg, history
        nonlocal pvilanguage, gastnr


        nonlocal gcfinfo_list, t_guest
        nonlocal gcfinfo_list_data, t_guest_data

        return {"payment": payment, "t-guest": t_guest_data, "gcfinfo-list": gcfinfo_list_data}
    ...
    def create_list():
        ...
        for gcfinfo_list in query(gcfinfo_list_data, filters=(lambda gcfinfo_list: gcfinfo_list.dept_str.lower()  == ("SUBTOTAL").lower())):
        ...
        for gcfinfo_list in query(gcfinfo_list_data, sort_by=[("nr",True)]):
            i = gcfinfo_list.nr + 1
            break

        gcfinfo_list = query(gcfinfo_list_data, first=True)

        if gcfinfo_list:
            gcfinfo_list = Gcfinfo_list()
            gcfinfo_list_data.append(gcfinfo_list)
