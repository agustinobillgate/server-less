...
def print_fa_pchase_lnlbl(pvilanguage:int, docunr:string, lnldelimeter:string):
    ...
    tmp_tbl_data_data, Tmp_tbl_data = create_model("Tmp_tbl_data", {"str3":string})
    ...
    def generate_output():
        nonlocal str1, str2, tmp_tbl_data_data, lvcarea, long_digit, foreign_currency, price_decimal, bill_recv, address1, address2, cp_name, telp, fax_no, bill_no, bill_date, refer, fa_source, dep_date, arr_date, delivery_date, bl_descript, bl_qty, d_unit, bl_price, bl_amount, c_exrate, bl_balance, balance, remark, htparam, fa_ordheader, l_lieferant, fa_order, parameters, l_artikel, mathis
        nonlocal pvilanguage, docunr, lnldelimeter


        nonlocal tmp_tbl_data, op_list
        nonlocal tmp_tbl_data_data, op_list_data

        return {"str1": str1, "str2": str2, "tmp-tbl-data": tmp_tbl_data_data}
