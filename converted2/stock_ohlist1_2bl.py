        ...
        if get_year(to_date) < get_year(tdate) and get_year(from_date) < get_year(tdate):
            ...
            if get_month(to_date) > get_month(tdate) and get_month(from_date) > get_month(tdate):
                ...
                for l_lager in db_session.query(L_lager).filter(
                    ...
                    l_bestand = L_bestand()
                    l_artikel = L_artikel()
                    for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter(
            ...
            if get_month(to_date) <= get_month(tdate) and get_month(from_date) <= get_month(tdate):
                ...
                for b_lager in db_session.query(B_lager).filter(
                    ...
                    l_artikel = L_artikel()
                    for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter(
                             (L_bestand.lager_nr == b_lager.lager_nr) & (L_bestand.artnr >= from_art) & (L_bestand.artnr <= to_art)).order_by(L_artikel.zwkum, L_artikel.artnr).all():
                        if l_bestand_obj_list.get(l_bestand._recid):
                            continue
                        else:
                            l_bestand_obj_list[l_bestand._recid] = True
                        ...
                        if do_it:
                            ...
                            for l_op in db_session.query(L_op).filter(
                                     (L_op.lager_nr == b_lager.lager_nr) & (L_op.artnr == l_bestand.artnr) & ((L_op.datum >= from_date) & (L_op.datum <= to_date)) & (L_op.loeschflag <= 1)).order_by(L_op.datum).all():
    ...
    def create_stock2():
        ...
        if get_year(to_date) <= get_year(tdate) and get_year(from_date) <= get_year(tdate):
            ...
            if get_month(to_date) > get_month(tdate) and get_month(from_date) > get_month(tdate):
                ...
                l_artikel = L_artikel()
                for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter(
                         (L_bestand.lager_nr == 0) & (L_bestand.artnr >= from_art) & (L_bestand.artnr <= to_art)).order_by(L_artikel.zwkum, L_artikel.artnr).all():
                    if l_bestand_obj_list.get(l_bestand._recid):
                        continue
                    else:
                        l_bestand_obj_list[l_bestand._recid] = True
                ...
                for l_artikel in db_session.query(L_artikel).filter(
                         ((L_artikel.artnr.in_(list(set([art_bestand.artnr for art_bestand in art_bestand_data])))))).order_by(art_bestand.zwkum, art_bestand.artnr).all():
