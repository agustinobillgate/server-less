...
def ap_paylist_btn_go1_cldbl(all_supp:bool, remark_flag:bool, from_supp:string, from_date:date, to_date:date, from_remark:string, price_decimal:int, sort_type:int):
    ...
    Obuff = Output_list
    obuff_data = output_list_data

    db_session = local_storage.db_session

    def generate_output():
        nonlocal lief_nr1, ap_exist, err_code, output_list_data, t_list_data, l_lieferant, l_kredit, artikel, bediener
        ...
        return {"lief_nr1": lief_nr1, "ap_exist": ap_exist, "err_code": err_code, "output-list": output_list_data, "t-list": t_list_data}
    ...
    def create_list1():
        ...
        for l_kredit.bemerk, l_kredit.zahlkonto, l_kredit.saldo, l_kredit._recid, l_kredit.bediener_nr, l_kredit.rgdatum, l_kredit.netto, debt.bemerk, debt.zahlkonto, debt.saldo, debt._recid, debt.bediener_nr, debt.rgdatum, debt.netto, art.artnr, art.bezeich, art._recid, l_lieferant.firma, l_lieferant.anredefirma, l_lieferant.lief_nr, l_lieferant._recid in db_session.query(L_kredit.bemerk, L_kredit.zahlkonto, L_kredit.saldo, L_kredit._recid, L_kredit.bediener_nr, L_kredit.rgdatum, L_kredit.netto, Debt.bemerk, Debt.zahlkonto, Debt.saldo, Debt._recid, Debt.bediener_nr, Debt.rgdatum, Debt.netto, Art.artnr, Art.bezeich, Art._recid, L_lieferant.firma, L_lieferant.anredefirma, L_lieferant.lief_nr, L_lieferant._recid).join(Debt,(Debt.counter == L_kredit.counter) & (Debt.zahlkonto == 0)).join(Art,(Art.artnr == L_kredit.zahlkonto) & (Art.departement == 0)).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr)).filter(
    ...
    def create_list1b():
        ...
        debt = L_kredit()
        art = Artikel()
        for l_kredit.bemerk, l_kredit.zahlkonto, l_kredit.saldo, l_kredit._recid, l_kredit.bediener_nr, l_kredit.rgdatum, l_kredit.netto, debt.bemerk, debt.zahlkonto, debt.saldo, debt._recid, debt.bediener_nr, debt.rgdatum, debt.netto, art.artnr, art.bezeich, art._recid in db_session.query(L_kredit.bemerk, L_kredit.zahlkonto, L_kredit.saldo, L_kredit._recid, L_kredit.bediener_nr, L_kredit.rgdatum, L_kredit.netto, Debt.bemerk, Debt.zahlkonto, Debt.saldo, Debt._recid, Debt.bediener_nr, Debt.rgdatum, Debt.netto, Art.artnr, Art.bezeich, Art._recid).join(Debt,(Debt.counter == L_kredit.counter) & (Debt.zahlkonto == 0)).join(Art,(Art.artnr == L_kredit.zahlkonto) & (Art.departement == 0)).filter(
