...
def cr_occfcast1_2_webbl_test(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):
    ...
    genstat_query = text("""
        ...
        FROM Genstat
        WHERE 
            res_date[2] >= :curr_date 
            AND res_date[2] <= :d2
            AND resstatus != 13
            AND segmentcode != 0
            AND res_logic[2] = TRUE
            AND zinr != '';

        """)
    db_session.execute(genstat_query, {"curr_date": curr_date, "d2": d2})
    ...
    #             """)
    #         db_session.execute(kontplan_query, {"curr_date": curr_date - timedelta(days=1), "ci_date": ci_date})
    #     else:
    ...
    #         db_session.execute(kontplan_query, {"curr_date": curr_date - timedelta(days=1), "to_date": to_date})
    # else:
    #     print("Skip Kontplan")
    ...
    #                     ;
    #             """)
    #         db_session.execute(zinrstat_query, {"curr_date": curr_date - timedelta(days=1), "ci_date": ci_date})
    #     else:
    #         # FOR EACH genstat WHERE genstat.datum GE curr- date AND  genstat.datum LE to-date.
    ...
    #         db_session.execute(zinrstat_query, {"curr_date": curr_date - timedelta(days=1), "to_date": to_date})
    # else:
    #     print("Skip zinrstat")
    # prep_end = time.perf_counter()
    ...
    #         and kontstatus=1;
    #         """)
    # db_session.execute(kontline_query, {"curr_date": curr_date - timedelta(days=1), "to_date": to_date})
    # prep_end = time.perf_counter()
