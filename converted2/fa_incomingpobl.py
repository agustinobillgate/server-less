...
def fa_incomingpobl(fromdate:date, todate:date, searchby:int, devnote_no:string, po_no:string, supp_no:int):
    ...
    op_list_data, Op_list = create_model("Op_list", {"lscheinnr":string, "name":string, "location":string, "einzelpreis":Decimal, "anzahl":int, "warenwert":Decimal, "firma":string, "datum":date, "docu_nr":string, "lief_nr":int, "rec_id":int})
