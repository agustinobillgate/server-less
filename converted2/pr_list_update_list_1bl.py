...
def pr_list_update_list_1bl(s_list_data:[S_list], docu_nr:string):
    ...
    def update_list():
        ...
        s_list = query(s_list_data, filters=(lambda s_list: s_list.s_recid == nr), first=True)
        s_list.lieferdatum = to_string(l_orderhdr.lieferdatum)
        s_list.instruct = l_orderhdr.lief_fax[2]

        for s_list in query(s_list_data, filters=(lambda s_list: s_list.docu_nr.lower()  == (docu_nr).lower()  and s_list.artnr > 0)):
        ...
        if l_orderhdr.lief_fax[1] != "":
            ...
            s_list = query(s_list_data, filters=(lambda s_list: s_list.docu_nr.lower()  == (docu_nr).lower()  and s_list.artnr == 0), first=True)
