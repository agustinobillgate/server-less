...
def pos_dashboard_prepare_webbl(user_init:string, init_posdept:int, bedi_username:string):
    ...
    def generate_output():
        nonlocal result_msg, urlws, licensenr, dynamic_qr, interval_time, asroom_service, cancel_exist, t_dept_data, t_queasy222_data, t_list_data, pick_table_data, order_list_data, order_item_data, active_waiter, queasy
        ...
        return {"result_msg": result_msg, "urlws": urlws, "licensenr": licensenr, "dynamic_qr": dynamic_qr, "interval_time": interval_time, "asroom_service": asroom_service, "cancel_exist": cancel_exist, "t-dept": t_dept_data, "t-queasy222": t_queasy222_data, "t-list": t_list_data, "pick-table": pick_table_data, "order-list": order_list_data, "order-item": order_item_data}
    ...
    t_dept_data, t_queasy222_data, urlws, licensenr, dynamic_qr, interval_time, asroom_service, cancel_exist = get_output(pos_dashboard_getparambl(1))

    t_dept = query(t_dept_data, filters=(lambda t_dept: t_dept.nr == 1), first=True)
