...
from models import Zimkateg, Htparam, Res_line, Zimmer, Genstat, Zkstat

def calculate_occupied_rooms(datum:date, rmtype:str):
    occ_rooms = 0
    use_it:bool = False
    ci_date:date = None
    tot_occ_rooms:int = 0
    ...
    zimkateg = htparam = res_line = zimmer = genstat = zkstat = None


    db_session = local_storage.db_session

    def generate_output():
        nonlocal occ_rooms, use_it, ci_date, tot_occ_rooms, rmcat_rooms, total_rooms, occ_rooms_1, i_method, d_occupancy, zimkateg, htparam, res_line, zimmer, genstat, zkstat
        nonlocal datum, rmtype
    ...
    def cal_method1():
        ...
        if datum >= ci_date:
            ...
            for zkstat in db_session.query(Zkstat).filter(
                ...
                if zkstat.zikatnr == zimkateg.zikatnr:
