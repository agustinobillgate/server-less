...
from models import Guest, Artikel, Hoteldpt, Billjournal, Bill, H_bill, Bk_veran, Reservation, Res_line, Guestseg, Segment

def fo_journal_1bl(from_art:int, to_art:int, from_dept:int, to_dept:int, from_date:date, to_date:date, sorttype:int, exclude_artrans:bool, long_digit:bool, foreign_flag:bool, mi_onlyjournal:bool, mi_excljournal:bool, mi_post:bool, mi_showrelease:bool):
    gtot = 0
    output_list_list = []
    curr_date:date = None
    descr1:str = ""
    ...
    curr_resnr:int = 0
    lvcs:str = ""
    guest = artikel = hoteldpt = billjournal = bill = h_bill = bk_veran = reservation = res_line = guestseg = segment = None

    output_list = gbuff = None

    output_list_list, Output_list = create_model("Output_list", {"bezeich":str, "c":str, "ns":str, "mb":str, "shift":str, "dept":str, "str":str, "remark":str, "gname":str, "descr":str, "voucher":str, "checkin":date, "checkout":date, "guestname":str, "segcode":str})
    ...
    def generate_output():
        nonlocal gtot, output_list_list, curr_date, descr1, voucher_no, ind, gdelimiter, roomnumber, zinrdate, billnumber, curr_str, curr_resnr, lvcs, guest, artikel, hoteldpt, billjournal, bill, h_bill, bk_veran, reservation, res_line, guestseg, segment
        nonlocal gbuff
                    ...
                    for billjournal in db_session.query(Billjournal).filter(
                        ...
                        if do_it:
                                ...
                                if not artikel.bezaendern:
                                    ...
                                    if ind != 0:
                                        ...
                                        if ind > len(artikel.bezeich):
                                            ...
                                            if cnt >= num_entries(billjournal.bezeich, gdelimiter):

                                                if num_entries(billjournal.bezeich, gdelimiter) > 1:
                                                    cnt = num_entries(billjournal.bezeich, gdelimiter) - 1
                                                else:
                                                    cnt = num_entries(billjournal.bezeich, gdelimiter)
                                            for i in range(1,cnt + 1) :
                            ...
                            if billjournal.bediener_nr == 0 and mi_onlyjournal == False:
                                ...
                                if not long_digit:
                                    STR = to_string(bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "999999999") + to_string(billjournal.artnr, "9999") + to_string(descr1, "x(50)") + to_string(deptname, "x(12)") + to_string("", "x(6)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->>,>>>,>>>,>>>,>>9.99") + to_string(zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate) + to_string(voucher_no, "x(24)")
                                else:
                                    STR = to_string(bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "999999999") + to_string(billjournal.artnr, "9999") + to_string(billjournal.bezeich, "x(50)") + to_string(deptname, "x(12)") + to_string("", "x(6)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->,>>>,>>>,>>>,>>>,>>9") + to_string(zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate) + to_string(voucher_no, "x(24)")
                    ...
                    for billjournal in db_session.query(Billjournal).filter(
                        ...
                        if do_it:
                            ...
                            if billjournal.bediener_nr == 0 and mi_onlyjournal == False:
                                ...
                                if not long_digit:
                                    STR = to_string(bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "999999999") + to_string(billjournal.artnr, "9999") + to_string(descr1, "x(50)") + to_string(hoteldpt.depart, "x(12)") + to_string("", "x(6)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->>,>>>,>>>,>>>,>>9.99") + to_string(zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate) + to_string(voucher_no, "x(24)")
                                else:
                                    STR = to_string(bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "999999999") + to_string(billjournal.artnr, "9999") + to_string(descr1, "x(50)") + to_string(hoteldpt.depart, "x(12)") + to_string("", "x(6)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->,>>>,>>>,>>>,>>>,>>9") + to_string(zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate) + to_string(voucher_no, "x(24)")
                                qty = qty + billjournal.anzahl
                                gqty = gqty + billjournal.anzahl
                        ...
                        for billjournal in db_session.query(Billjournal).filter(
                            ...
                            if do_it:
                                ...
                                if (billjournal.bediener_nr != 0 and mi_excljournal == False) or (billjournal.bediener_nr == 0 and mi_onlyjournal == False):
                                    ...
                                    if not long_digit:
                                        STR = to_string(bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "999999999") + to_string(billjournal.artnr, "9999") + to_string(billjournal.bezeich, "x(50)") + to_string(hoteldpt.depart, "x(12)") + to_string("", "x(6)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->>,>>>,>>>,>>>,>>9.99") + to_string(zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate)
                                    else:
                                        STR = to_string(bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "999999999") + to_string(billjournal.artnr, "9999") + to_string(billjournal.bezeich, "x(50)") + to_string(hoteldpt.depart, "x(12)") + to_string("", "x(6)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->,>>>,>>>,>>>,>>>,>>9") + to_string(zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate)
                                    qty = qty + billjournal.anzahl
                                    gqty = gqty + billjournal.anzahl
    ...
    def add_field():
        ...
        if billjournal.rechnr > 0:
            ...
            if bill:
                ...
                if bill.resnr != 0:
                    ...
                    if res_line:
                        ...
                        h_bill = db_session.query(H_bill).filter(
                                (H_bill.rechnr == billjournal.rechnr) &  (H_bill.departement == billjournal.departement)).first()

                        if h_bill:
                            output_list.guestname = h_bill.bilname

                            res_line = db_session.query(Res_line).filter(
                                    (Res_line.gastnrmember == h_bill.resnr)).first()
            ...
            IF1 + get_index(output_list.descr, "[#") > 0 THEN
            curr_str = substring(output_list.descr,0 + get_index(output_list.descr, "[#") + 2)
            curr_resnr = to_int(entry(0, curr_str, " "))
            ELSE IF1 + get_index(output_list.descr, " #") > 0 THEN
            curr_str = substring(output_list.descr,0 + get_index(output_list.descr, " #") + 2)
            curr_resnr = to_int(entry(0, curr_str, "]"))

            res_line = db_session.query(Res_line).filter(
                    (Res_line.resnr == curr_resnr)).first()
    ...
    while None != output_list:
        ...
        if output_list.MB.lower()  == "*" and roomnumber == "":
