...
def calls_deptlistbl(cost_list_data:[Cost_list], sorttype:int, cost_center:int, to_cc:int, price_decimal:int, from_date:date, to_date:date, double_currency:bool):
    ...
    def generate_output():
        nonlocal stattype, str_list_data, amount1, amount2, t_local, t_ldist, t_ovsea, curr_bezeich, last_sort, prstr, nebenst, calls, bediener
        ...
        return {"cost-list": cost_list_data, "stattype": stattype, "str-list": str_list_data}
    ...
    def create_list():
        ...
        if last_sort == 2:
            ...
            for nebenst in db_session.query(Nebenst).filter(
                ...
                cost_list = query(cost_list_data, (lambda cost_list: cost_list.num == nebenst.departement and cost_list.num >= from_cost and cost_list.num <= to_cost), first=True)
                if not cost_list:
                    continue
