...
def cr_occfcast1_2_webbl_test(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):
    ...
    param_list = [87, 550, 144, 127, 128, 125, 479, 491, 227, 228, 229, 126]
    param_query = text(f"""
    ...
    resline_query = text(f"""
            CREATE TEMP TABLE res_line AS  SELECT * FROM res_line  WHERE ankunft BETWEEN '2024-10-01' AND '2024-10-31';
                         """)
    db_session.execute(resline_query, {})

    reslinq_query = text(f"""
            CREATE TEMP TABLE reslin_queasy AS  SELECT * FROM reslin_queasy  WHERE date1 >= '2024-10-01' AND date2<= '2024-10-31';
                         """)
    db_session.execute(reslinq_query, {})
    
    kontline_query = text(f"""
            CREATE TEMP TABLE kontline AS  SELECT * FROM kontline  WHERE ankunft BETWEEN '2024-10-01' AND '2024-10-31';
                         """)
    db_session.execute(kontline_query, {})
    db_session.commit()
