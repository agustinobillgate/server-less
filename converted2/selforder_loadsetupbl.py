...
def selforder_loadsetupbl(outlet_no:int):
    ...
    language_list_data, Language_list = create_model("Language_list", {"lang_num":int, "lang_id":string, "lang_default":string, "lang_other":string})
    article_list_data, Article_list = create_model("Article_list", {"art_department":int, "art_recid":int, "art_number":int, "art_name":string, "art_group":int, "art_subgrp":int, "art_group_str":string, "art_subgrp_str":string, "art_desc":string, "art_price":Decimal, "art_image":string, "art_active_flag":bool})
    carousel_list_data, Carousel_list = create_model("Carousel_list", {"carousel_recid":int, "carousel_dept":int, "carousel_num":int, "carousel_title":string, "carousel_desc":string, "carousel_image":string})
    maingroup_list_data, Maingroup_list = create_model("Maingroup_list", {"maingrp_no":int, "maingrp_description":string})
    subgroup_list_data, Subgroup_list = create_model("Subgroup_list", {"subgrp_no":int, "subgrp_description":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal user_init, font_color, bg_color, outlet_name, hotel_name, image_logo, image_food, image_bev, image_other, mess_result, language_list_data, article_list_data, carousel_list_data, subgroup_list_data, maingroup_list_data, wgrpdep, wgrpgen, bediener, hoteldpt, paramtext, queasy
        nonlocal outlet_no


        nonlocal language_list, article_list, carousel_list, maingroup_list, subgroup_list
        nonlocal language_list_data, article_list_data, carousel_list_data, maingroup_list_data, subgroup_list_data

        return {"user_init": user_init, "font_color": font_color, "bg_color": bg_color, "outlet_name": outlet_name, "hotel_name": hotel_name, "image_logo": image_logo, "image_food": image_food, "image_bev": image_bev, "image_other": image_other, "mess_result": mess_result, "language-list": language_list_data, "article-list": article_list_data, "carousel-list": carousel_list_data, "subgroup-list": subgroup_list_data, "maingroup-list": maingroup_list_data}


    article_list_data.clear()
    maingroup_list_data.clear()
    subgroup_list_data.clear()
    ...
    for wgrpdep in db_session.query(Wgrpdep).filter(
        ...
        subgroup_list_data.append(subgroup_list)
    ...
    for wgrpgen in db_session.query(Wgrpgen).order_by(Wgrpgen.eknr).all():
        ...
        maingroup_list_data.append(maingroup_list)

        maingroup_list.maingrp_no = wgrpgen.eknr
        maingroup_list.maingrp_description = wgrpgen.bezeich


    language_list_data.clear()
    carousel_list_data.clear()
    ...
    mess_result, article_list_data = get_output(selforder_loadarticlebl(outlet_no))
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        carousel_list_data.append(carousel_list)
