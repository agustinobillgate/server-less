...
def bi_avalon_dailybl():
    ...
    while None != res_line:
        ...
        if res_line.cancelled != None:
            ...
            temp_res.cancel_date = 01/01/1970
        ...
        if reservation:
            ...
            if reservation.resdat != None:
                temp_res.bookdate = reservation.resdat - timedelta(days=10)
        ...
        res_line = db_session.query(Res_line).filter(
                 ((Res_line.resstatus == 9) | (Res_line.resstatus == 10) & (Res_line.active_flag == 2)) & (Res_line.ankunft <= bill_date) & (Res_line.abreise >= bill_date - timedelta(days=1)) & (Res_line.l_zuordnung[inc_value(2)] == 0)).filter(Res_line._recid > curr_recid).first()
    ...
    while None != genstat:
        ...
        if not his_res:
            ...
            if his_res.cancel_date == None:
                his_res.cancel_date = 01/01/1970
            ...
            if reservation:
                ...
                if reservation.resdat != None:
                    his_res.bookdate = reservation.resdat - timedelta(days=5)

        curr_recid = genstat._recid
        genstat = db_session.query(Genstat).filter(
                 ((Genstat.datum <= bill_date) & (Genstat.datum >= bill_date - timedelta(days=1))) & (Genstat.resstatus != 0) & (Genstat.zikatnr != 0) & (Genstat.res_logic[inc_value(1))]).filter(Genstat._recid > curr_recid).first()
    temp_res_list.clear()
    ...
    for res_line in db_session.query(Res_line).filter(
        ...
        if do_it:
            ...
            if reservation:
                temp_res.source_code = reservation.resart
                temp_res.bookdate = reservation.resdat - timedelta(days=5)
                temp_res.segmentcode = reservation.segmentcode
                temp_res.voucher_nr = reservation.vesrdepot
                temp_res.grpname = reservation.groupname
                temp_res.letterno = reservation.briefnr
    ...
    for zinrstat, akt_code in db_session.query(Zinrstat, Akt_code).join(Akt_code,(Akt_code.aktionscode == Zinrstat.betriebsnr) & (Akt_code.aktiongrup == 4)).filter(
             (func.lower(Zinrstat.zinr) == ("Competitor").lower()) & (Zinrstat.datum <= bill_date) & (Zinrstat.datum >= bill_date - timedelta(days=1))).order_by(Zinrstat._recid).all():
