...
def flag_report_create_listbl(user_init:str, f_date:date, t_date:date, sorttype:int, alldept_flag:bool):
    s_list_list = []
    bediener = reslin_queasy = res_line = None

    s_list = None

    s_list_list, S_list = create_model("S_list", {"frdate":date, "date":date, "name":str, "zinr":str, "flag":bool, "ci":bool, "co":bool, "ankunft":date, "abreise":date, "urgent":bool, "done":bool, "id":str, "dept":str, "bemerk":str, "resnr":int, "reslinnr":int, "ind":int, "s_recid":int})
    ...
    def create_list():
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == ("flag").lower()) &  (Reslin_queasy.date1 >= f_date)).order_by(Reslin_queasy.date1).all():
            ...
            if do_it:
                ...
                if reslin_queasy.date1 <= f_date and reslin_queasy.date1 > t_date:
                    ...
                    if not res_line or not(res_line.resnr == reslin_queasy.resnr and res_line.reslinnr == reslin_queasy.reslinnr and res_line.active_flag <= 1):
                        res_line = db_session.query(Res_line).filter(
                            (Res_line.resnr == reslin_queasy.resnr) &  (Res_line.reslinnr == reslin_queasy.reslinnr) &  (Res_line.active_flag <= 1)).first()
                else:

                    if not res_line or not(res_line.resnr == reslin_queasy.resnr and res_line.reslinnr == reslin_queasy.reslinnr and res_line.active_flag <= 2 and res_line.resstatus != 9 and res_line.resstatus != 10):
                        res_line = db_session.query(Res_line).filter(
                            (Res_line.resnr == reslin_queasy.resnr) &  (Res_line.reslinnr == reslin_queasy.reslinnr) &  (Res_line.active_flag <= 2) &  (Res_line.resstatus != 9) &  (Res_line.resstatus != 10)).first()
                ...
                if res_line:
                    ...
                    s_list.id = entry(1, reslin_queasy.char1, chr(2))
                    s_list.bemerk = entry(0, reslin_queasy.char1, chr(2))
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == ("flag").lower()) &  (Reslin_queasy.date2 >= f_date)).order_by(Reslin_queasy.date2).all():
            ...
            if do_it:
                ...
                if res_line:
                    ...
                    s_list.ankunft = res_line.ankunft
                    s_list.abreise = res_line.abreise
                    s_list.id = entry(1, reslin_queasy.char2, chr(2))
                    s_list.bemerk = entry(0, reslin_queasy.char2, chr(2))
                    s_list.resnr = reslin_queasy.resnr
                    s_list.reslinnr = reslin_queasy.reslinnr
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == ("flag").lower()) &  (Reslin_queasy.date3 >= f_date)).order_by(Reslin_queasy.date3).all():
            ...
            if do_it:
                ...
                if res_line:
                    ...
                    s_list.ankunft = res_line.ankunft
                    s_list.abreise = res_line.abreise
                    s_list.id = entry(1, reslin_queasy.char3, chr(2))
                    s_list.bemerk = entry(0, reslin_queasy.char3, chr(2))
                    s_list.resnr = reslin_queasy.resnr
                    s_list.reslinnr = reslin_queasy.reslinnr
    ...
    def check_do_it(inp_char:str, todate:date, done_flag:bool):
        ...
        if num_entries(inp_char, chr(2)) <= 2:
            ...
            return generate_inner_output()
        for curr_i in range(3,num_entries(inp_char, chr(2))  + 1) :
            ...
            if curr_i == 3:
                ...
                dept_str = entry(3, inp_char, chr(2))
        ...
        if re.match(((".*," + usrdept_str + ".*")),dept_str, re.IGNORECASE):

            return generate_inner_output()

        if substring(dept_str, 0, len((usrdept_str).lower() )) == (usrdept_str).lower() :
