...
def nt_onlinetax_batam_billdate_manual(fr_date:date, to_date:date):
    avail_flag = False
    n:int = 1
    do_it:bool = False
    ...
    def step_1(bill_date:date):
        ...
        for fo_list in query(fo_list_list, filters=(lambda fo_list: fo_list.amount != 0)):
            ...
            outstr = to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") + to_string(get_day(bill_date) , "99") + to_string(i_counter, "999999") + "|" + to_string(fo_list.rechnr) + "|" + fo_list.cdtrx + "|" + datetime2char (fo_list.bill_datum, fo_list.zeit) + "|" + fo_list.bezeich + "|" + dec2char (fo_list.amount) + "|" + "1" + chr(2) + to_string(fo_list.departement)
            add_line(outstr, bill_date)

        for vat_list in query(vat_list_list, filters=(lambda vat_list: vat_list.service_amt != 0)):
            i_counter = i_counter + 1
            outstr = to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") + to_string(get_day(bill_date) , "99") + to_string(i_counter, "999999") + "|" + to_string(vat_list.rechnr) + "|" + "ATV" + "|" + datetime2char (vat_list.bill_datum, vat_list.zeit) + "|" + "Service Charge" + "|" + dec2char (vat_list.service_amt) + "|" + "1" + chr(2) + to_string(vat_list.departement)
            add_line(outstr, bill_date)
        ...
        for t_list in query(t_list_list, filters=(lambda t_list: t_list.fb != 0 or t_list.other != 0)):
            ...
            if t_list.fb != 0:
                i_counter = i_counter + 1
                outstr = to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") +\
                        to_string(get_day(bill_date) , "99") + to_string(i_counter, "999999") + "|" +\
                        to_string(t_list.rechnr) + "|" +\
                        "ATM" + "|" +\
                        datetime2char (t_list.bill_datum, t_list.zeit) + "|" +\
                        "Food and Beverage" + "|" +\
                        dec2char (t_list.fb) + "|" +\
                        "1" + chr(2) + to_string(t_list.departement)
            ...
            if t_list.fb_service != 0:
                i_counter = i_counter + 1
                outstr1 = to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") +\
                        to_string(get_day(bill_date) , "99") + to_string(i_counter, "999999") + "|" +\
                        to_string(t_list.rechnr) + "|" +\
                        "ATV" + "|" +\
                        datetime2char (t_list.bill_datum, t_list.zeit) + "|" +\
                        "Service Charge Food and Beverage" + "|" +\
                        dec2char (t_list.fb_service) + "|" +\
                        "1" + chr(2) + to_string(t_list.departement)
            ...
            if t_list.other_service != 0:
                ...
                outstr1 = to_string(get_year(bill_date) - 2000, "99") + to_string(get_month(bill_date) , "99") +\
                        ...
                        "ATV" + "|" +\
                        datetime2char (t_list.bill_datum, t_list.zeit) + "|" +\
                        "Service Charge Other Revenue" + "|" +\
                        dec2char (t_list.other_service) + "|" +\
                        "1" + chr(2) + to_string(t_list.departement)
