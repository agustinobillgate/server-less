...
def s_stockins_create_apbl(lief_nr:int, lscheinnr:str, billdate:date, t_amount:decimal, user_init:str):
    ...
    def generate_output():
        ...
        nonlocal lief_nr, lscheinnr, billdate, t_amount, user_init
    ...
    def create_ap():
        ...
        if gl_acct:
            ...
            if l_lieferant.z_code != "":
                ...
                if gl_acct and (l_lieferant.z_code.lower()  != (ap_acct).lower()):
                    do_it = False
        ...
        if ap_license and do_it:
            ...
            l_kredit = db_session.query(L_kredit).filter(
                     (L_kredit.lief_nr == lief_nr) & (func.lower(L_kredit.name) == (lscheinnr).lower()) & (func.lower(L_kredit.lscheinnr) == (lscheinnr).lower()) & (L_kredit.rgdatum == billdate) & (L_kredit.opart <= 2) & (L_kredit.zahlkonto == 0)).first()

            if l_kredit:
                l_kredit.netto =  to_decimal(l_kredit.netto) + to_decimal(t_amount)
                l_kredit.saldo =  to_decimal(l_kredit.saldo) + to_decimal(t_amount)
