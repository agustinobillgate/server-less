...
def cancel_stockinhis_btn_go_webbl(pvilanguage:int, all_supp:bool, sorttype:int, from_grp:int, store:int, from_date:date, to_date:date, show_price:bool, from_supp:string):
    cancel_stockinhis_list_data = []
    ...
    cancel_stockinhis_list_data, Cancel_stockinhis_list = create_model("Cancel_stockinhis_list", {"datum":date, "lager":string, "lief_nr":int, "lief":string, "art":string, "bezeich":string, "unit":string, "epreis":Decimal, "in_qty":Decimal, "amount":Decimal, "docunr":string, "dlvnote":string, "note":string, "reason":string, "invnr":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal cancel_stockinhis_list_data
        ...
        nonlocal str_list_data, cancel_stockinhis_list_data

        return {"cancel-stockinhis-list": cancel_stockinhis_list_data}

    str_list_data = get_output(cancel_stockinhis_btn_go_cldbl(pvilanguage, all_supp, sorttype, from_grp, store, from_date, to_date, show_price, from_supp))
    cancel_stockinhis_list_data.clear()

    for str_list in query(str_list_data):
        cancel_stockinhis_list = Cancel_stockinhis_list()
        cancel_stockinhis_list_data.append(cancel_stockinhis_list)
