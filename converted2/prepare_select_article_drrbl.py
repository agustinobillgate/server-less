...
def prepare_select_article_drrbl(case_type:int, departement:int):
    ...
    str_list_data, Str_list = create_model("Str_list", {"nr":int, "bezeich":string, "used":bool})
    pay_list_data, Pay_list = create_model("Pay_list", {"artnr":int, "flag":bool})
    outlet_list_data, Outlet_list = create_model("Outlet_list", {"artnr":int, "departement":int, "flag":string, "flag_used":bool})
    stat_list_data, Stat_list = create_model("Stat_list", {"artnr":int, "zwkum":int, "flag":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal c_862, c_892, str1, str2, str3, dept, dept1, food, bev, st1, st2, st3, st4, st5, st6, st7, st8, st9, st10, st11, st12, st13, st14, st15, st16, st17, st18, st19, st20, st21, st22, n, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13, n14, n15, n16, n17, n18, n19, n20, n21, str_list_data, zwkum, zwkum1, htparam, queasy, artikel, hoteldpt
        nonlocal case_type, departement


        nonlocal str_list, pay_list, outlet_list, stat_list
        nonlocal str_list_data, pay_list_data, outlet_list_data, stat_list_data
    ...
    if case_type == 1:
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            pay_list = query(pay_list_data, filters=(lambda pay_list: pay_list.artnr == artikel.artnr), first=True)
        ...
        for hoteldpt in db_session.query(Hoteldpt).order_by(Hoteldpt._recid).all():
            ...
            outlet_list = query(outlet_list_data, filters=(lambda outlet_list: outlet_list.flag.lower()  == ("FBsales").lower()  and outlet_list.departement == hoteldpt.num), first=True)
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            outlet_list = query(outlet_list_data, filters=(lambda outlet_list: outlet_list.flag.lower()  == ("Outlet").lower()  and outlet_list.artnr == artikel.artnr and outlet_list.departemen == departement), first=True)
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            outlet_list = query(outlet_list_data, filters=(lambda outlet_list: outlet_list.flag.lower()  == ("FBcover").lower()  and outlet_list.artnr == artikel.artnr and outlet_list.departemen == departement), first=True)
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.artnr == artikel.artnr), first=True)
