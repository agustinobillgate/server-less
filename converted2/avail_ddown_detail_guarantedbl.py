...
def avail_ddown_detail_guarantedbl(datum:date):
    ...
    def detail_guaranted():
        ...
        for zimkateg in db_session.query(Zimkateg).order_by(Zimkateg._recid).all():
            ...
            for res_line in db_session.query(Res_line).filter(
                     (Res_line.active_flag == 0) & (Res_line.resstatus == 1) & (Res_line.zikatnr == zimkateg.zikatnr) & (Res_line.ankunft <= datum) & (Res_line.abreise > datum) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
                do_it = True
