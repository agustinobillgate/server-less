...
def cancel_ba_rsvbl(case_type:int, curr_room:string, curr_status:int, curr_i:int, language_code:int, rml_resnr:int, rml_reslinnr:int, answer_msgstr2:bool, answer_msgstr3:bool, cancel_str:string, user_init:string):
    ...
    waiting_list_data, Waiting_list = create_model("Waiting_list", {"veran_nr":int, "name":string, "von_zeit":string, "bis_zeit":string, "raum":string, "rec_id":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal msg_str1, msg_str2, msg_str3, mess_result, waiting_list_data, datum, raum, von_zeit, bis_zeit, resstatus, b1_resnr, b1_reslinnr, curr_resnr, bk_reser
        ...
        nonlocal t_resline_data, waiting_list_data

        return {"msg_str1": msg_str1, "msg_str2": msg_str2, "msg_str3": msg_str3, "mess_result": mess_result, "waiting-list": waiting_list_data}

    def check_waitinglist(datum:date, raum:string, von_zeit:string, bis_zeit:string, resstatus:int):

        nonlocal msg_str1, msg_str2, msg_str3, mess_result, waiting_list_data, b1_resnr, b1_reslinnr, curr_resnr, bk_reser
        nonlocal case_type, curr_room, curr_status, curr_i, language_code, rml_resnr, rml_reslinnr, answer_msgstr2, answer_msgstr3, cancel_str, user_init
        ...
        avail_resline, waiting_list_data = get_output(ba_plan_check_waitinglistbl(datum, raum, von_zeit, bis_zeit, resstatus))
    ...
    if case_type == 1:
        ...
        if answer_msgstr2 :
            ...
            if cancel_str == "":
                ...
                if answer_msgstr3 :
                    t_resline_data = get_output(ba_plan_res_cancel2bl(rml_resnr, rml_reslinnr))
