#using conversion tools version: 1.0.0.87
...
def cr_occfcast1_2_webbl_test(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):
    ...
    # print("Curr/To:",curr_date, to_date)
    ...
    param_list = [87, 550, 144, 127, 128, 125, 479, 491, 227, 228, 229, 126, 100]
    param_query = text("""
        CREATE TEMP TABLE htparam AS  SELECT * FROM htparam  WHERE paramnr IN :params
    """)

    db_session.execute(param_query, {"params": tuple(param_list)})
    param87 = db_session.execute(text("SELECT * FROM htparam WHERE paramnr=87")).mappings().fetchone()
    if param87:
        ci_date = param87["fdate"]  # Replace 'fieldname' with the actual column name
        # print(ci_date)
    # print("Temp HTParam created")


    # kontline_query = text("""
    #         CREATE TEMP TABLE kontline AS  SELECT * FROM kontline  
    #                       WHERE ankunft >= :start_date AND abreise <= :end_date
    #                       and kontstatus=1;
    #                      """)
    ...
    # print(f"Temp Kontline Queasy created: {curr_date} - {to_date}")

    # resline_query = text(f"""
    #         CREATE TEMPORARY TABLE resline AS
    #             SELECT 
    #                 resnr, ankunft, abreise, arrangement, reslinnr, zikatnr, zinr, gastnr, gratis, erwachs, kind1, zipreis, _recid, kind2, resstatus, zimmerfix, zimmeranz, kontignr
    #             FROM res_line
    #             WHERE ankunft >= :start_date AND abreise <= :to_date
    #                 AND resstatus <= 13
    ...
    # recs = db_session.query(Res_line).all()
    ...
    # print("Temp Reslin Queasy created")
    ...
    # print("Temp Arrangement created")

    artikel_query = text(""" 
        CREATE TEMP TABLE artikel  AS 
                SELECT a.* 
                FROM artikel  AS a 
                WHERE EXISTS (
                    SELECT 1 FROM fixleist rl WHERE rl.artnr = a.artnr
                ); 
        """)
    # db_session.execute(artikel_query, {})
    # print("Temp Artikel created")
    ...
    def create_browse():
       # print(inspect.currentframe().f_code.co_name)
        ...
        for i in range(1,tmpint + 1) :
            ...
            kontline = Kontline()
            recs = (
                 db_session.query(Kontline.zikatnr, Kontline.zimmeranz, Kontline.arrangement, Kontline.gastnr, Kontline.erwachs, Kontline.kind1, Kontline._recid).filter(
                     (Kontline.ankunft <= datum) & (Kontline.abreise >= datum) & (Kontline.betriebsnr == 0) & (Kontline.kontstatus == 1)).order_by(Kontline._recid).all()
            )
            for kontline.zikatnr, kontline.zimmeranz, kontline.arrangement, kontline.gastnr, kontline.erwachs, kontline.kind1, kontline._recid in recs:
                allot_doit = True
        ...
        genstat_obj_list = {}
        recs = (
            db_session.query(Genstat, Guest, Zimkateg, Waehrung, Segment).join(Guest,(Guest.gastnr == Genstat.gastnr)).join(Zimkateg,(Zimkateg.zikatnr == Genstat.zikatnr)).join(Waehrung,(Waehrung.waehrungsnr == Genstat.wahrungsnr)).join(Segment,(Segment.segmentcode == Genstat.segmentcode)).filter(
                 (Genstat.res_date[inc_value(1)] >= datum1) & (Genstat.res_date[inc_value(1)] <= d2) & 
                 (Genstat.resstatus != 13) & 
                 (Genstat.segmentcode != 0) & 
                 (Genstat.res_logic[inc_value(1)]) & 
                 (Genstat.zinr != " "))
                .order_by(Genstat.res_date[inc_value(1)], Genstat.zinr, Genstat.datum).all()
        )
        for genstat, guest, zimkateg, waehrung, segment in recs:
        ...
        for genstat.zinr, genstat.segmentcode, genstat.argt, genstat.zikatnr, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.res_date, genstat.datum, genstat.resstatus, genstat.res_logic, genstat.logis, genstat.ratelocal, genstat.res_deci, genstat.resnr, genstat.res_int, genstat._recid in db_session.query(Genstat.zinr, Genstat.segmentcode, Genstat.argt, Genstat.zikatnr, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.res_date, Genstat.datum, Genstat.resstatus, Genstat.res_logic, Genstat.logis, Genstat.ratelocal, Genstat.res_deci, Genstat.resnr, Genstat.res_int, Genstat._recid).filter(
            ...
            if do_it:
                ...
                if room_list:
                    ...
                    fixleist = Fixleist()
                    recs = (
                        db_session.query(Fixleist.artnr, Fixleist.departement, Fixleist.sequenz, Fixleist.dekade, Fixleist.lfakt, Fixleist.betrag, Fixleist.number, Fixleist._recid).filter(
                             (Fixleist.resnr == genstat.resnr) & (Fixleist.reslinnr == genstat.res_int[0])).order_by(Fixleist._recid).all()
                    )
                    for fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt, fixleist.betrag, fixleist.number, fixleist._recid in recs:
                    ...
                    (
                        (   (Res_line.active_flag == 2) & 
                            ...
                            (Res_line.abreise == ci_date)
                                                         ...
                                                         (Res_line.resstatus == 8) & (Res_line.abreise == ci_date))
                    ) 
                & (Res_line.gastnr > 0) & (Res_line.l_zuordnung[inc_value(2)] == 0)
                )
    ...
    def create_browse1():
       # print(inspect.currentframe().f_code.co_name)
        ...
        for i in range(1,tmpint + 1) :
            ...
            kontline = Kontline()
            recs_kontline = (
                db_session.query(Kontline.zikatnr, Kontline.zimmeranz, Kontline.arrangement, Kontline.gastnr, Kontline.erwachs, Kontline.kind1, Kontline._recid)
                .filter(
                     (Kontline.ankunft <= datum) & 
                     (Kontline.abreise >= datum) & 
                     (Kontline.betriebsnr == 0) & 
                     (Kontline.kontstatus == 1))
                    .order_by(Kontline._recid).all()
            )
            for kontline.zikatnr, kontline.zimmeranz, kontline.arrangement, kontline.gastnr, kontline.erwachs, kontline.kind1, kontline._recid in recs_kontline:
                allot_doit = True
        ...
        recs = (
            ...
            .filter(
                 ((Res_line.active_flag <= 1) & 
                  ...
                  (Res_line.resstatus != 12) & 
                  (Res_line.ankunft <= to_date) & 
                  (Res_line.abreise >= curr_date)) 
                  | 
                (((Res_line.active_flag == 2) & (Res_line.resstatus == 8) & (Res_line.ankunft == ci_date) & (Res_line.abreise == ci_date)) 
                 | ((Res_line.active_flag == 2) & (Res_line.resstatus == 8) & (Res_line.abreise == ci_date))
                ) & 
                (Res_line.gastnr > 0) & 
                ...
                for datum in date_range(datum1,datum2) :
                    ...
                    if room_list:
                        ...
                        if datum == res_line.abreise:
                            ...
                            fixleist = Fixleist()
                            recs_fixleist = (
                                db_session.query(Fixleist.artnr, Fixleist.departement, Fixleist.sequenz, Fixleist.dekade, Fixleist.lfakt, Fixleist.betrag, Fixleist.number, Fixleist._recid)
                                .filter(
                                     (Fixleist.resnr == res_line.resnr) & 
                                     (Fixleist.reslinnr == res_line.reslinnr))
                                .order_by(Fixleist._recid).all()
                            )
                            for fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt, fixleist.betrag, fixleist.number, fixleist._recid in recs_fixleist:
                                post_it = check_fixleist_posted(datum, fixleist.artnr, fixleist.departement, fixleist.sequenz, fixleist.dekade, fixleist.lfakt)
                                service =  to_decimal("0")
                                vat =  to_decimal("0")
                                fcost =  to_decimal("0")
    ...
    def cal_lastday_occ():
       # print(inspect.currentframe().f_code.co_name)
        ...
        if curr_date <= ci_date:
            ...
            genstat = Genstat()
            recs_genstat = (
                db_session.query(Genstat.zinr, Genstat.segmentcode, Genstat.argt, Genstat.zikatnr, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.res_date, Genstat.datum, Genstat.resstatus, Genstat.res_logic, Genstat.logis, Genstat.ratelocal, Genstat.res_deci, Genstat.resnr, Genstat.res_int, Genstat._recid)
                .filter(
                     (Genstat.datum == curr_date - timedelta(days=1)) & 
                     (Genstat.res_logic[inc_value(1)]) & (Genstat.zinr != "") & 
                     (Genstat.resstatus != 13)).order_by(Genstat._recid).all()
            )
            for genstat.zinr, genstat.segmentcode, genstat.argt, genstat.zikatnr, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.res_date, genstat.datum, genstat.resstatus, genstat.res_logic, genstat.logis, genstat.ratelocal, genstat.res_deci, genstat.resnr, genstat.res_int, genstat._recid in recs_genstat:
                do_it = True
    ...
    def calc_othrev(datum:date):
       # print(inspect.currentframe().f_code.co_name)
        ...
        artikel = Artikel()
        recs_artikel = (
            db_session.query(Artikel.departement, Artikel.artnr, Artikel.service_code, Artikel.mwst_code, Artikel._recid)
            .filter(
                 (Artikel.departement == 0) & 
                 (Artikel.artart == 0) & 
                 (Artikel.umsatzart == 1))
                .order_by(Artikel.artnr).all()
        )
        for artikel.departement, artikel.artnr, artikel.service_code, artikel.mwst_code, artikel._recid in recs_artikel:
            max_i = max_i + 1
        ...
        for i in range(1,max_i + 1) :
            ...
            if artikel:
                ...
                umsatz = Umsatz()
                recs_umsatz = (
                    db_session.query(Umsatz.departement, Umsatz.artnr, Umsatz.datum, Umsatz.betrag, Umsatz._recid)
                    .filter(
                         (Umsatz.artnr == artikel.artnr) & 
                         (Umsatz.departement == artikel.departement) & 
                         (Umsatz.datum == datum))
                    .order_by(Umsatz._recid).all()
                )
                for umsatz.departement, umsatz.artnr, umsatz.datum, umsatz.betrag, umsatz._recid in recs_umsatz:
