...
def po_inlistbl(lief_nr:int, docu_nr:string, from_date:date, to_date:date):
    ...
    t_l_op_data, T_l_op = create_model_like(L_op, {"l_artikel_artnr":int, "l_artikel_bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal l_lieferant_firma, t_l_op_data, l_op, l_lieferant, l_artikel
        nonlocal lief_nr, docu_nr, from_date, to_date


        nonlocal t_l_op
        nonlocal t_l_op_data

        return {"l_lieferant_firma": l_lieferant_firma, "t-l-op": t_l_op_data}
