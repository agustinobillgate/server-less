...
def get_inhouse_guestlistbl():
    ...
    guest_list_data, Guest_list = create_model("Guest_list", {"resnr":int, "reslinnr":int, "roomnumber":string, "checkindate":date, "checkoutdate":date, "guest_nr":int, "guest_lastname":string, "guest_firstname":string, "guest_title":string, "guest_nation":string, "guest_country":string, "guest_region":string, "guest_mobile":string, "guest_email":string, "guest_bemerk":string, "guest_birthday":date, "recid_resline":int, "wifi_password":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal guest_list_data, i, i_counter, str, ci_date, active_flag, res_line, guest, nation


        nonlocal guest_list
        nonlocal guest_list_data
