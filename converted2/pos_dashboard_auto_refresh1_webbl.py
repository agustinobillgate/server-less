...
def pos_dashboard_auto_refresh1_webbl(user_init:string, dept_no:int):
    ...
    def generate_output():
        nonlocal urlws, licensenr, dynamic_qr, interval_time, asroom_service, cancel_exist, found_new_order, ask_bill_flag, sound_link, t_dept_data, t_queasy222_data, t_list_data, pick_table_data, order_list_data, order_item_data, check_new_ordered, queasy
        nonlocal user_init, dept_no


        nonlocal t_list, pick_table, order_list, order_item, t_dept, t_queasy222
        nonlocal t_list_data, pick_table_data, order_list_data, order_item_data, t_dept_data, t_queasy222_data

        return {"urlws": urlws, "licensenr": licensenr, "dynamic_qr": dynamic_qr, "interval_time": interval_time, "asroom_service": asroom_service, "cancel_exist": cancel_exist, "found_new_order": found_new_order, "ask_bill_flag": ask_bill_flag, "sound_link": sound_link, "t-dept": t_dept_data, "t-queasy222": t_queasy222_data, "t-list": t_list_data, "pick-table": pick_table_data, "order-list": order_list_data, "order-item": order_item_data}

    t_dept_data, t_queasy222_data, urlws, licensenr, dynamic_qr, interval_time, asroom_service, cancel_exist = get_output(pos_dashboard_getparambl(dept_no))

    t_dept = query(t_dept_data, filters=(lambda t_dept: t_dept.nr == dept_no), first=True)
    get_output(pos_dashboard_expired_sessionbl(t_dept.nr, dynamic_qr))
    t_list_data, pick_table_data = get_output(pos_dashboard_opened_tischbl(t_dept.nr))
    order_list_data, order_item_data = get_output(pos_dashboard_load_orderbl(t_dept.nr))

    t_list = query(t_list_data, filters=(lambda t_list: t_list.ask_bill  and t_list.bill_print == False), first=True)

    if t_list:
        ask_bill_flag = True

    for order_list in query(order_list_data, filters=(lambda order_list: not order_list.posted)):
