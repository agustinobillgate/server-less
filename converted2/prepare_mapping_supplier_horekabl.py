...
def prepare_mapping_supplier_horekabl():
    ...
    def generate_output():
        nonlocal supplier_data, queasy, l_lieferant
        nonlocal bqueasy


        nonlocal supplier, bqueasy
        nonlocal supplier_data
    ...
    if not queasy:
        ...
        for l_lieferant in db_session.query(L_lieferant).order_by(L_lieferant._recid).all():
            ...
            supplier = query(supplier_data, filters=(lambda supplier: supplier.supplier_name == l_lieferant.firma), first=True)
