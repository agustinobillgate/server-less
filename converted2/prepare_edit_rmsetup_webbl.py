...
def prepare_edit_rmsetup_webbl(raum:string, curr_select:string, setup_id:int, record_id:int):
    ...
    bk_list_data, Bk_list = create_model_like(Bk_rset, {"rec_id":int})
    t_bk_raum_data, T_bk_raum = create_model_like(Bk_raum)
    t_bk_setup_data, T_bk_setup = create_model_like(Bk_setup)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal raum_bez, setup_bez, bk_list_data, t_bk_raum_data, t_bk_setup_data, bk_rset, bk_raum, bk_setup
        nonlocal raum, curr_select, setup_id, record_id


        nonlocal bk_list, t_bk_raum, t_bk_setup
        nonlocal bk_list_data, t_bk_raum_data, t_bk_setup_data

        return {"raum_bez": raum_bez, "setup_bez": setup_bez, "bk-list": bk_list_data, "t-bk-raum": t_bk_raum_data, "t-bk-setup": t_bk_setup_data}
