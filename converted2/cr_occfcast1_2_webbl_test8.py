...
def cr_occfcast1_2_webbl_test8(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):
    ...
    if curr_date < ci_date:
        # create temp table genstat
        # IF to-date GE ci-date THEN (SWAP) 
        #     FOR EACH genstat WHERE genstat.datum GE curr-date AND  genstat.datum LT ci-date.
        # ELSE  /*IF to-date GE ci-date */
        #     (SWAP) 
        # FOR EACH genstat WHERE genstat.datum GE curr- date AND  genstat.datum LE to-date.
        if to_date >= ci_date:
            print("Apply Genstat.1)")
            # FOR EACH genstat WHERE genstat.datum GE curr-date AND  genstat.datum LT ci-date.
            genstat_query = text("""
                CREATE TEMPORARY TABLE genstat AS
                    ...
                    FROM genstat g
                    WHERE 
                        g.datum >= :curr_date AND g.datum < :ci_date;
                """)
            db_session.execute(genstat_query, {"curr_date": curr_date - timedelta(days=31), "ci_date": ci_date})
        else:
            print("Apply Genstat.2)")
            # FOR EACH genstat WHERE genstat.datum GE curr- date AND  genstat.datum LE to-date.
            genstat_query = text("""
                CREATE TEMPORARY TABLE genstat AS
                    SELECT 
                    ...
                    FROM genstat g
                    WHERE 
                        g.datum >= :curr_date AND g.datum <= :to_date ;
                """)
            db_session.execute(genstat_query, {"curr_date": curr_date - timedelta(days=31), "to_date": to_date})
        pass
    ...
    print(f"Query genstat time: {prep_end - prep_start:.4f} seconds")

    # resline
    # IF to-date GE ci-date THEN
    # DO:
    # (SWAP) FOR EACH res-line WHERE
    # ((res-line.ankunft LT ci-date AND res-line.abreise GE ci-date) OR
    # (ankunft GE ci-date AND ankunft LE to-date)) AND validasi2 lain spt pada resstatus etc yg ada dlm .p
    # END.
    # awalnya:
    # resline_query = text("""
    ...
    if to_date >= ci_date:
        ...
        resline_query = text("""
            ...
            """)
        db_session.execute(resline_query, {"ci_date": ci_date, "to_date": to_date})
        # clear_8 = text("""
    ...
    prep_end = time.perf_counter()
    print(f"Query res_line time: {prep_end - prep_start:.4f} seconds")  

    # Kontplan
    # kontplan, serupa cara genstat
    # kontplan_query = text("""
        ...
        if to_date >= ci_date:
            ...
            kontplan_query = text("""
                CREATE TEMPORARY TABLE kontplan AS
                    ...
                    g.datum >= :curr_date AND g.datum < :ci_date ;
                """)
            db_session.execute(kontplan_query, {"curr_date": curr_date - timedelta(days=31), "ci_date": ci_date})
        else:
            # FOR EACH genstat WHERE genstat.datum GE curr- date AND  genstat.datum LE to-date.
            print("Apply Kontplan.2")
            kontplan_query = text("""
                CREATE TEMPORARY TABLE kontplan AS
                    SELECT 
                        *
            ...
            db_session.execute(kontplan_query, {"curr_date": curr_date - timedelta(days=31), "to_date": to_date})
    else:
        print("Skip Kontplan")
    prep_end = time.perf_counter()
    print(f"Query kontplan time: {prep_end - prep_start:.4f} seconds")
    ...
    # SWAP dgn cara serupa spt genstat, filtering + kondisi zinrstat.zinr = "ooo".
    # IF to-date GE ci-date THEN
    # DO:
    # (SWAP) FOR EACH res-line WHERE
    # ((res-line.ankunft LT ci-date AND res-line.abreise GE ci-date) OR
    ...
    if curr_date < ci_date:
        if to_date >= ci_date:
            ...
            zinrstat_query = text("""
                CREATE TEMPORARY TABLE zinrstat AS
                    ...
                    WHERE 
                        ...
                        ;
                """)
            db_session.execute(zinrstat_query, {"curr_date": curr_date - timedelta(days=31), "ci_date": ci_date})
        else:
            # FOR EACH genstat WHERE genstat.datum GE curr- date AND  genstat.datum LE to-date.
            print("Apply zinrstat.2")
            zinrstat_query = text("""
                CREATE TEMPORARY TABLE zinrstat AS
                    SELECT 
                        *
            ...
            db_session.execute(zinrstat_query, {"curr_date": curr_date - timedelta(days=31), "to_date": to_date})
    else:
        print("Skip zinrstat")
    prep_end = time.perf_counter()
    print(f"Query zinrstat time: {prep_end - prep_start:.4f} seconds")

    print("Apply kontline")
    prep_start = time.perf_counter()
    kontline_query = text(f"""
        CREATE TEMP TABLE kontline AS  
        SELECT * FROM kontline  
            WHERE ankunft  >= :curr_date AND ankunft  <= :to_date
            and betriebsnr =0
            and kontstatus=1;
            """)
    db_session.execute(kontline_query, {"curr_date": curr_date - timedelta(days=31), "to_date": to_date})
    prep_end = time.perf_counter()
    print(f"Query kontline time: {prep_end - prep_start:.4f} seconds")

    print("Apply reservation")
    prep_start = time.perf_counter()
    reservation_query = text("""
            ...
            WHERE
                EXISTS (
                    SELECT 1 FROM res_line rl WHERE rl.resnr = r.resnr
                )
                             """)
    db_session.execute(reservation_query, {})
    prep_end = time.perf_counter()
    print(f"Query reservation time: {prep_end - prep_start:.4f} seconds")

    
    print("Apply Fixleist")
    prep_start = time.perf_counter()
    fixleist_query = text(""" 
        CREATE TEMP TABLE fixleist AS 
                ...
                FROM fixleist AS a 
                WHERE EXISTS (
                    SELECT 1 FROM genstat g WHERE g.resnr = a.resnr
                )
                AND  EXISTS (
    ...
    prep_end = time.perf_counter()
    print(f"Query fixleist time: {prep_end - prep_start:.4f} seconds")
    ...
    prep_end = time.perf_counter()
    print(f"Query guest time: {prep_end - prep_start:.4f} seconds")

    print("Apply resline_queasy")
    prep_start = time.perf_counter()
    reslinq_query = text("""
        CREATE TEMP TABLE reslin_queasy AS  
        SELECT * 
        FROM reslin_queasy a  
        WHERE a.key='arrangement'
             AND EXISTS (
                SELECT 1 FROM res_line rl WHERE rl.resnr = a.resnr
            );
        """)
    ...
    prep_end = time.perf_counter()
    print(f"Query resline-queasy time: {prep_end - prep_start:.4f} seconds")
