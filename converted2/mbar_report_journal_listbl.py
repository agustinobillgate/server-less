...
def mbar_report_journal_listbl(from_date:date, to_date:date, curr_dept:int, long_digit:bool):
    ...
    def journal_list():
            ...
            for h_journal in db_session.query(H_journal).filter(
                ...
                if h_journal.artnr == 0 and rm_flag:
                    ...
                    if deci_place == 0:
                        ...
                        output_list.str = to_string(bill_datum) + to_string(rmno, "x(6)") + to_string(gname, "x(24)") + to_string(h_journal.rechnr, "x(9)") + to_string(h_journal.bezeich, "x(24)") + to_string(betrag, "x(14)") + to_string(h_journal.fremdwaehrng, "x(14)") + to_string(zeit, "HH:MM") + to_string(h_journal.kellner_nr, "x(3)") + to_string(h_journal.tischnr, "x(6)")
                    else:
                        h_journal_rechnr = to_string(h_journal.rechnr, ">,>>>,>>9")
        ...
        if deci_place == 0:
            ...
            output_list.str = to_string("", "x(47)") + to_string("T O T A L ", "x(24)") + to_string(tot, "x(14)") + to_string(tot_foreign, "x(14)")
        else:
