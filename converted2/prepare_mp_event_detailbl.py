...
def prepare_mp_event_detailbl(blockid:string, nr:int):
    ...
    t_event_data, T_event = create_model("T_event", {"blockid":string, "counter":int, "nr":int, "eventtype":string, "eventstatus":string, "eventname":string, "startdate":date, "enddate":date, "starttime":string, "endtime":string, "atendees":int, "minguaranted":int, "actual":int, "venue":string, "setup":string, "amount":Decimal})
    t_bkraum_data, T_bkraum = create_model_like(Bk_raum)
    t_bkrset_data, T_bkrset = create_model_like(Bk_rset)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal resstatus, guestno, pax, t_event_data, t_bkraum_data, t_bkrset_data, bk_raum, bk_rset
        nonlocal blockid, nr


        nonlocal t_event, t_bkraum, t_bkrset
        nonlocal t_event_data, t_bkraum_data, t_bkrset_data

        return {"resstatus": resstatus, "guestno": guestno, "pax": pax, "t-event": t_event_data, "t-bkraum": t_bkraum_data, "t-bkrset": t_bkrset_data}

    t_event = query(t_event_data, first=True)
