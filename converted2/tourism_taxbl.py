...
def tourism_taxbl(fdate:date, tdate:date):
    ...
    tourism_report_data, Tourism_report = create_model("Tourism_report", {"gastnr":int, "gname":string, "nat":string, "passport_no":string, "ci_date":date, "co_date":date, "tourism_tax":Decimal, "rmno":string, "rechnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal tourism_report_data, tot_tax, bill_line, res_line, guest, nation, bill
        nonlocal fdate, tdate


        nonlocal tourism_report
        nonlocal tourism_report_data

        return {"tourism-report": tourism_report_data}
    ...
    while None != bill_line:
        ...
        if res_line:
            ...
            if guest:
                ...
                tourism_report = query(tourism_report_data, filters=(lambda tourism_report: tourism_report.gastnr == guest.gastnr), first=True)

                if not tourism_report:

                    nation = get_cache (Nation, {"kurzbez": [(eq, guest.nation1)],"natcode": [(eq, 0)]})
                    tourism_report = Tourism_report()
                    tourism_report_data.append(tourism_report)
