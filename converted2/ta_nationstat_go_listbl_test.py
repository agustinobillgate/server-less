...
from models import Guest, Genstat, Kontplan, Nation, Htparam, Arrangement, Artikel

def ta_nationstat_go_listbl_test(from_date:str, last_period:str, sorttype:int, text2:str, text3:str):
    ...
    guest = genstat = kontplan = nation = htparam = arrangement = artikel = None
    ...
    # genstat_query = text(f"""
    # CREATE TEMP TABLE genstat AS 
    # SELECT gastnr, argt, datum, logis, ratelocal, _recid, zinr, karteityp, res_logic 
    # FROM genstat
    # WHERE EXTRACT(YEAR FROM datum) = 2024 
    #       AND EXTRACT(MONTH FROM datum) <= 9
    #       AND zinr <> ''
    #       AND karteityp = 2;
    # """)
    # db_session.execute(genstat_query, {})


    # guest_query = text(f"""
    #     CREATE TEMP TABLE guest AS 
    #     SELECT gastnr, land, name, anredefirma, _recid 
    #     FROM guest
    #     WHERE EXISTS (
    #         SELECT 1 FROM genstat g 
    #         WHERE g.gastnr = guest.gastnr
    #     );
    # """)
    ...
    #     WHERE EXISTS (
    #         SELECT 1 FROM genstat g 
    #         WHERE g.argt = arrangement.arrangement
    #     );
    # """)
    # db_session.execute(argt_query, {})


    # artikel_query = text(f"""
    #     CREATE TEMP TABLE artikel AS 
    #     SELECT *
    #     FROM artikel
    #     WHERE EXISTS (
    #         SELECT 1 FROM arrangement g 
    #         WHERE g.artnr_logis = artikel.artnr
    #     );
    # """)
    # db_session.execute(artikel_query, {})

    kontplan_query = text(f"""
                            CREATE TEMP TABLE kontplan AS 
                            SELECT *
                            FROM kontplan
                            WHERE EXTRACT(YEAR FROM datum) = 2024 
                            AND EXTRACT(MONTH FROM datum) <= 9
                            ;
                        """)
    db_session.execute(kontplan_query, {})

    db_session.commit()


    set_cache(Kontplan,[],[["betriebsnr", "kontignr", "datum"]], True)

    

    def generate_output():
        nonlocal ta_nat_stat_list, sub_mrm, sub_mlod, sub_margt, sub_yrm, sub_ylod, sub_yargt, grand_mrm, grand_mlod, grand_margt, grand_yrm, grand_ylod, grand_yargt, guest, genstat, kontplan, nation, htparam, arrangement, artikel
    ...
    def create_list():
        ...
        guest = Guest()
        recs = (
            db_session.query(Genstat.gastnr, Genstat.argt, Genstat.datum, Genstat.logis, Genstat.ratelocal, Genstat._recid, Guest.land, Guest.name, Guest.anredefirma, Guest._recid)
                ...
                .filter(
                 ...
                 .order_by(Genstat._recid).all()
        )
        for genstat.gastnr, genstat.argt, genstat.datum, genstat.logis, genstat.ratelocal, genstat._recid, guest.land, guest.name, guest.anredefirma, guest._recid in recs:
            if genstat_obj_list.get(genstat._recid):
                continue
            else:
                genstat_obj_list[genstat._recid] = True

            nation = get_cache (Nation, {"kurzbez": guest.land}, ['kurzbez', '_recid'])
            ...
            htparam = get_cache (Htparam, {"paramnr": 127}, ['flogical', '_recid'])
            ...
            if arrangement:
                ...
                artikel = get_cache (Artikel, {"artnr": arrangement.artnr_logis, "departement": 0}, ['artnr', 'departement', '_recid'])
