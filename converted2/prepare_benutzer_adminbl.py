...
def prepare_benutzer_adminbl():
    ...
    usr_list_data, Usr_list = create_model_like(Bediener, {"email":string, "mphone":string, "pager":string, "grp_str":string, "totp_flag":bool, "totp_status":string})
    ...
    def generate_output():
        nonlocal usr_list_data, bediener, queasy
        nonlocal totpdata


        nonlocal usr_list, totpdata
        nonlocal usr_list_data

        return {"usr-list": usr_list_data}
