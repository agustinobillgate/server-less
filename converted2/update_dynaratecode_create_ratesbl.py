...
dynarate_list_data, Dynarate_list = create_model("Dynarate_list", {"counter":int, "rcode":string, "w_day":int, "fr_room":int, "to_room":int, "days1":int, "days2":int, "rmtype":string, "s_recid":int})

def update_dynaratecode_create_ratesbl(dynarate_list_data:[Dynarate_list], rmtype:string, currcode:string, from_date:date, to_date:date, market_number:int):
    ...
    def generate_output():
        nonlocal rate_list1_data, curr_date, curr_i, w_day, inp_zikatnr, wd_array, zimkateg, ratecode, queasy
        nonlocal rmtype, currcode, from_date, to_date, market_number


        nonlocal rate_list1, dynarate_list
        nonlocal rate_list1_data
    ...
    for dynarate_list in query(dynarate_list_data, filters=(lambda dynarate_list: dynarate_list.rmtype.lower()  == (rmtype).lower()), sort_by=[("w_day",False),("fr_room",False),("days1",False),("days2",False)]):
        ...
        for curr_i in range(1,get_day(to_date)  + 1) :
            ...
            if dynarate_list.w_day > 0 and dynarate_list.w_day != w_day:
                ...
                if zimkateg:
                    ...
                    ratecode = get_cache (Ratecode, {"code": [(eq, dynarate_list.rcode)],"startperiode": [(le, curr_date)],"endperiode": [(ge, curr_date)],"zikatnr": [(eq, inp_zikatnr)],"marknr": [(eq, market_number)]})
                else:
