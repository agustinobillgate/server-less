...
def prepare_bookengine_ratecodepushbl(bookengid:int):
    ...
    def generate_output():
        nonlocal bookeng_name, t_push_list_data, cat_flag, gastnrbe, tokcounter, iftask, mestoken, mesvalue, scode, ratecode, queasy, zimkateg, guest_pr, arrangement
        nonlocal bookengid
        nonlocal bratecode


        nonlocal t_push_list, q_list, rmcat_list, dynarate_list, bratecode
        nonlocal t_push_list_data, q_list_data, rmcat_list_data, dynarate_list_data

        return {"bookeng_name": bookeng_name, "t-push-list": t_push_list_data}
    ...
    for zimkateg in db_session.query(Zimkateg).filter(
        ...
        if cat_flag and zimkateg.typ != 0:
            ...
            rmcat_list = query(rmcat_list_data, filters=(lambda rmcat_list: rmcat_list.typ == zimkateg.typ), first=True)
        else:

            rmcat_list = query(rmcat_list_data, filters=(lambda rmcat_list: rmcat_list.zikatnr == zimkateg.zikatnr), first=True)
    ...
    for q_list in query(q_list_data):
