...
def discrepancy_inlist_1bl(sorttype:int, from_lager:int, to_lager:int, from_date:date, to_date:date, from_art:int, to_art:int, mi_rec_chk:bool, mi_ord_chk:bool, mi_all_chk:bool):
    ...
    output_list_data, Output_list = create_model("Output_list", {"datum":string, "lager":string, "docunr":string, "art":string, "bezeich":string, "in_qty":string, "amount":string, "epreis1":string, "epreis2":string, "lief":string, "dlvnote":string})
    discrepancy_inlist_data, Discrepancy_inlist = create_model("Discrepancy_inlist", {"datum":date, "lager":string, "docunr":string, "art":string, "bezeich":string, "in_qty":Decimal, "amount":Decimal, "epreis1":Decimal, "epreis2":Decimal, "lief":string, "dlvnote":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal discrepancy_inlist_data, lager_bezeich, i, tot_anz, tot_amount, price_decimal, long_digit, note_str, deliver_no, htparam, l_lager, l_artikel, l_op, l_order, l_lieferant
        ...
        nonlocal str_list_data, output_list_data, discrepancy_inlist_data

        return {"discrepancy-inlist": discrepancy_inlist_data}
    ...
    if sorttype == 1:
        ...
        create_list3()
    discrepancy_inlist_data.clear()

    for output_list in query(output_list_data):
        discrepancy_inlist = Discrepancy_inlist()
        discrepancy_inlist_data.append(discrepancy_inlist)
