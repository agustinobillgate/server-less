...
def prepare_recipe_list_webbl():
    ...
    R1_list = R_list
    r1_list_data = r_list_data

    db_session = local_storage.db_session

    def generate_output():
        nonlocal cost_list_data, t_h_rezept_data, price_type, curr_i, h_rezept, htparam, queasy, h_rezlin, l_artikel
        nonlocal r1_list


        nonlocal cost_list, t_h_rezept, r_list, r1_list
        nonlocal cost_list_data, t_h_rezept_data, r_list_data

        return {"cost-list": cost_list_data, "t-h-rezept": t_h_rezept_data}
    ...
    def calculate_cost():
        ...
        for h_rezept in db_session.query(H_rezept).order_by(H_rezept._recid).all():
            ...
            cost_list = query(cost_list_data, filters=(lambda cost_list: cost_list.artnrrezept == h_rezept.artnrrezept), first=True)
