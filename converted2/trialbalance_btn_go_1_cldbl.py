...
def trialbalance_btn_go_1_cldbl(acct_type:int, from_fibu:str, to_fibu:str, sorttype:int, from_dept:int, from_date:date, to_date:date, close_month:int, close_date:date, pnl_acct:str, close_year:date, prev_month:int, show_longbal:bool, pbal_flag:bool, asremoteflag:bool):
    output_list_list = []
    num_acctype:int = 0
    ...
    output_list_list, Output_list = create_model("Output_list", {"gop_flag":bool, "nr":int, "str":str, "budget":decimal, "proz":decimal, "mark":bool, "ch":str, "acct_type":int})
            ...
            if acct_type == 0:
                ...
                for gl_acct in db_session.query(Gl_acct).filter(
                    ...
                    if do_it:
                        ...
                        for g_list in query(g_list_list, filters=(lambda g_list: g_list.fibu == gl_acct.fibukonto), sort_by=[("fibu",False),("datum",False)]):
                            pass
                            t_date = g_list.datum
    ...
    def create_list2():
        ...
        nonlocal output_list_list, num_acctype, sales, cost, gop_credit, gop_debit, tot_diff, curr_i, in_procedure, numsend, last_acct_close_priod, t_from_date, t_to_date, t_strgrp, t_str, t_int, t_date, from_datehis, to_datehis, readflag, coa_format, counter, tt_pbal2, gl_acct, gl_jouhdr, gl_journal, gl_jhdrhis, gl_jourhis, gl_main, htparam, gl_accthis
        nonlocal from_fibu, to_fibu, sorttype, from_dept, from_date, to_date, close_month, close_date, pnl_acct, close_year, prev_month, show_longbal, pbal_flag, asremoteflag
            ...
            if acct_type == 0:
                ...
                for gl_main, gl_account in db_session.query(Gl_main, Gl_account).join(Gl_account,(Gl_account.main_nr == Gl_main.nr) & (func.lower(Gl_account.fibukonto) >= (from_fibu).lower()) & (func.lower(Gl_account.fibukonto) <= (to_fibu).lower())).order_by(Gl_main.code).all():
                    ...
                    for gl_acct in db_session.query(Gl_acct).filter(
                        ...
                        konto = gl_acct.fibukonto
                        output_list.acct_type = gl_acct.acc_type
                        t_debit =  to_decimal("0")
                        t_credit =  to_decimal("0")
                        p_bal =  to_decimal("0")
                        t_bal =  to_decimal("0")
                ...
                gl_main_obj_list = []
                for gl_main, gl_account in db_session.query(Gl_main, Gl_account).join(Gl_account,(Gl_account.gl_acct.acc_type == to_int(acct_type)) & (Gl_account.main_nr == Gl_main.nr) & (func.lower(Gl_account.fibukonto) >= (from_fibu).lower()) & (func.lower(Gl_account.fibukonto) <= (to_fibu).lower())).order_by(Gl_main.code).all():
                    if gl_main._recid in gl_main_obj_list:
                        continue
                    else:
                        gl_main_obj_list.append(gl_main._recid)
                    ...
                    for gl_acct in db_session.query(Gl_acct).filter(
                             (Gl_acct.acc_type == to_int(acct_type)) & (Gl_acct.main_nr == gl_main.nr) & (func.lower(Gl_acct.fibukonto) >= (from_fibu).lower()) & (func.lower(Gl_acct.fibukonto) <= (to_fibu).lower())).order_by(Gl_acct.fibukonto).all():
                        konto = gl_acct.fibukonto
                        t_debit =  to_decimal("0")
                        t_credit =  to_decimal("0")
                        p_bal =  to_decimal("0")
    ...
    def create_list2d():
        ...
        if sorttype == 2:
            ...
            for gl_main, gl_account in db_session.query(Gl_main, Gl_account).join(Gl_account,(Gl_account.main_nr == Gl_main.nr) & (func.lower(Gl_account.fibukonto) >= (from_fibu).lower()) & (func.lower(Gl_account.fibukonto) <= (to_fibu).lower()) & (Gl_account.deptnr == from_dept)).order_by(Gl_main.code).all():
                ...
                for gl_acct in db_session.query(Gl_acct).filter(
                         (Gl_acct.acc_type == to_int(acct_type)) & (Gl_acct.main_nr == gl_main.nr) & (func.lower(Gl_acct.fibukonto) >= (from_fibu).lower()) & (func.lower(Gl_acct.fibukonto) <= (to_fibu).lower()) & (Gl_acct.deptnr == from_dept)).order_by(Gl_acct.fibukonto).all():
                    konto = gl_acct.fibukonto
                    output_list.acct_type = gl_acct.acc_type
                    t_debit =  to_decimal("0")
                    t_credit =  to_decimal("0")
