...
def create_historybl(resnr:int, reslinnr:int, old_zinr:string, res_mode:string, user_init:string, ziwechsel_str:string):
    ...
    def generate_output():
        nonlocal parent_nr, tot_umsatz, found, usr_id, res_line, bill, history, guest, bediener, reservation, zimkateg, htparam, akt_kont, bill_line, artikel, res_history
        nonlocal resnr, reslinnr, old_zinr, res_mode, user_init, ziwechsel_str
    ...
    if res_mode.lower()  == ("checkout").lower() :
        ...
        history.bemerk = to_string(htparam.fdate) + ": Moved to" + " " + res_line.zinr + chr_unicode(10) + ziwechsel_str
        history.bemerk = history.bemerk + chr_unicode(2) + user_init
