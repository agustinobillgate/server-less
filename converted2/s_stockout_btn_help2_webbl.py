...
def s_stockout_btn_help2_webbl(s_artnr:int, curr_lager:int):
    ...
    temp_l_artikel_data, Temp_l_artikel = create_model("Temp_l_artikel", {"artnr":int, "bezeich":string, "betriebsnr":int, "endkum":int, "masseinheit":string, "vk_preis":Decimal, "inhalt":Decimal, "lief_einheit":Decimal, "traubensort":string, "fibukonto":string, "cost_alloc":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_stock_oh, avail_l_bestand, temp_l_artikel_data, l_artikel, gl_acct
        ...
        return {"t_stock_oh": t_stock_oh, "avail_l_bestand": avail_l_bestand, "temp-l-artikel": temp_l_artikel_data}
