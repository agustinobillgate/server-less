...
def rm_drecap3bl(language_code:int, opening_date:date, from_date:date, to_date:date, fdate:date, tdate:date, segmtype_exist:bool, checked_mi_mtd:bool, checked_mi_ftd:bool, checked_mi_exchu:bool, checked_mi_exccomp:bool, long_digit:bool):
    ...
    output_list_data, Output_list = create_model("Output_list", {"segno":int, "flag":string, "str":string, "yroom":string, "proz3":string, "ypax":string, "yrate":string, "yrev":string, "zero_flag":bool})
    dos_list_data, Dos_list = create_model("Dos_list", {"segno":int, "flag":string, "guestsegment":string, "room":int, "proz_rm":Decimal, "mtd":int, "proz_mtd":Decimal, "ytd":int, "proz_ytd":Decimal, "pax":int, "mtd_pax":int, "ytd_pax":int, "avrg_rate":Decimal, "mtd_avrgrate":Decimal, "ytd_avrgrate":Decimal, "room_rev":Decimal, "mtd_rev":Decimal, "ytd_rev":Decimal, "zero_flag":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal dos_list_data
        ...
        nonlocal output_list_data, dos_list_data

        return {"dos-list": dos_list_data}

    output_list_data = get_output(rm_drecap2bl(language_code, opening_date, from_date, to_date, fdate, tdate, segmtype_exist, checked_mi_mtd, checked_mi_ftd, checked_mi_exchu, checked_mi_exccomp, long_digit))

    for output_list in query(output_list_data):
        dos_list = Dos_list()
        dos_list_data.append(dos_list)
