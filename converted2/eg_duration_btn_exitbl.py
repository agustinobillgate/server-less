...
duration_data, Duration = create_model_like(Eg_duration)

def eg_duration_btn_exitbl(duration_data:[Duration], case_type:int, rec_id:int):
    ...
    def generate_output():
        nonlocal fl_code, sduration_data, eg_duration
        nonlocal case_type, rec_id
        nonlocal queasy1


        nonlocal sduration, duration, queasy1
        nonlocal sduration_data

        return {"fl_code": fl_code, "sduration": sduration_data}
    ...
    duration = query(duration_data, first=True)
