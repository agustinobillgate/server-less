...
def sorder_list_btn_go_1bl(user_init:str, sorttype:int, s_artnr:int, from_date:date, to_date:date, from_sup:str, to_sup:str, closepo:bool):
    str_list_list = []
    ...
    def create_list1():
        ...
        if closepo == False:
            ...
            for l_order, l_orderhdr, l_lieferant, l_artikel in db_session.query(L_order, L_orderhdr, L_lieferant, L_artikel).join(L_orderhdr,(L_orderhdr.lief_nr == L_order.lief_nr) & (L_orderhdr.docu_nr == L_order.docu_nr)).join(L_lieferant,(L_lieferant.lief_nr == L_order.lief_nr) & (func.lower(L_lieferant.firma) >= (from_sup).lower()) & (func.lower(L_lieferant.firma) <= (to_sup).lower())).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                ...
                if not long_digit:
                    ...
                    if show_price:
                        ...
                        str_list.s = str_list.s + to_string(l_order.rechnungspreis * l_order.geliefert, ">>>,>>>,>>9.99")
                    else:
            ...
            str_list.s = str_list.s + to_string("T O T A L", "x(13)")
    ...
    def create_list2():
        ...
        if closepo == False:
            ...
            l_orderhdr_obj_list = []
            for l_orderhdr, l_order, l_lieferant, l_artikel in db_session.query(L_orderhdr, L_order, L_lieferant, L_artikel).join(L_order,(L_order.docu_nr == L_orderhdr.docu_nr) & (L_order.pos > 0) & (L_order.loeschflag == 0) & (L_order.betriebsnr == 0)).join(L_lieferant,(L_lieferant.lief_nr == L_orderhdr.lief_nr) & (L_lieferant.func.lower(L_lieferant.firma) >= (from_sup).lower()) & (func.lower(L_lieferant.firma) <= (to_sup).lower())).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
            ...
            l_orderhdr_obj_list = []
            for l_orderhdr, l_order, l_lieferant, l_artikel in db_session.query(L_orderhdr, L_order, L_lieferant, L_artikel).join(L_order,(L_order.docu_nr == L_orderhdr.docu_nr) & (L_order.pos > 0) & (L_order.loeschflag == 1) & (L_order.betriebsnr == 0)).join(L_lieferant,(L_lieferant.lief_nr == L_orderhdr.lief_nr) & (L_lieferant.func.lower(L_lieferant.firma) >= (from_sup).lower()) & (func.lower(L_lieferant.firma) <= (to_sup).lower())).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
                     (L_orderhdr.lieferdatum >= from_date) & (L_orderhdr.lieferdatum <= to_date)).order_by(L_orderhdr.lieferdatum, L_lieferant.firma, L_order.artnr).all():
                cost_list = query(cost_list_list, (lambda cost_list: cost_list.nr == l_orderhdr.angebot_lief[0]), first=True)
                if not cost_list:
                    continue
    ...
    def create_list22():
        ...
        if closepo == False:
            ...
            l_orderhdr_obj_list = []
            for l_orderhdr, l_order, l_lieferant, l_artikel in db_session.query(L_orderhdr, L_order, L_lieferant, L_artikel).join(L_order,(L_order.docu_nr == L_orderhdr.docu_nr) & (L_order.pos > 0) & (L_order.loeschflag == 0) & (L_order.betriebsnr == 0) & (L_order.artnr == s_artnr)).join(L_lieferant,(L_lieferant.lief_nr == L_orderhdr.lief_nr) & (L_lieferant.func.lower(L_lieferant.firma) >= (from_sup).lower()) & (func.lower(L_lieferant.firma) <= (to_sup).lower())).join(L_artikel,(L_artikel.artnr == L_order.artnr)).filter(
    ...
    def create_costlist():
        ...
        for parameters in db_session.query(Parameters).filter(
                 (func.lower(Parameters.progname) == ("CostCenter").lower()) & (func.lower(Parameters.section) == ("Name").lower()) & (Parameters.varname > "")).order_by(Parameters._recid).all():
            cost_list = Cost_list()
