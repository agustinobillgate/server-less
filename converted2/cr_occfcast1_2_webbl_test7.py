...
def cr_occfcast1_2_webbl_test7(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):

    prepare_cache ([Res_line, Htparam, Waehrung, Kontline, Zimmer, Genstat, Exrate, Fixleist, Artikel, Reservation, Arrangement, Queasy, Reslin_queasy, Argt_line, Zinrstat, Zkstat, Umsatz])
    ...
    # DO:
    # (SWAP) FOR EACH res-line WHERE
    # ((res-line.ankunft LE ci-date AND res-line.abreise GE ci-date) OR
    # (ankunft GE ci-date AND ankunft LE to-date)) AND
    # (resstatus NOT in [4, 9, 10, 12]))
    # END.
    if to_date >= ci_date:
        resline_query = text("""
            ...
            SELECT 
                *
            FROM res_line
            WHERE  resstatus NOT IN (4, 9, 10, 12)
                AND (
                        (ankunft < :ci_date AND abreise >= :ci_date) 
                        OR 
                        (ankunft >= :ci_date AND ankunft <= :to_date))
                            
        """)

    db_session.execute(resline_query, {"ci_date": ci_date - timedelta(days=1), "to_date": to_date})
    ...
    set_cache(Segment, None,[["segmentcode"]], True)
    set_cache(Zimmer, None,[["zinr"]], True)
    set_cache(Htparam, None,[["paramnr"]], True)
    set_cache(Arrangement, None,[["arrangement"]], True)
    ...
    def create_browse():
        ...
        for genstat.zinr, genstat.segmentcode, genstat.argt, genstat.zikatnr, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.res_date, genstat.datum, genstat.resstatus, genstat.res_logic, genstat.logis, genstat.ratelocal, genstat.res_deci, genstat.resnr, genstat.res_int, genstat._recid in db_session.query(Genstat.zinr, Genstat.segmentcode, Genstat.argt, Genstat.zikatnr, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.res_date, Genstat.datum, Genstat.resstatus, Genstat.res_logic, Genstat.logis, Genstat.ratelocal, Genstat.res_deci, Genstat.resnr, Genstat.res_int, Genstat._recid).filter(
            ...
            if do_it:
                ...
                segment = get_cache (Segment, {"segmentcode": genstat.segmentcode}, [])
        ...
        if to_date >= ci_date:
            ...
            for res_line.resnr, res_line.ankunft, res_line.abreise, res_line.arrangement, res_line.reslinnr, res_line.zikatnr, res_line.zinr, res_line.gastnr, res_line.gratis, res_line.erwachs, res_line.kind1, res_line.zipreis, res_line._recid, res_line.kind2, res_line.resstatus, res_line.zimmerfix, res_line.zimmeranz, res_line.kontignr in db_session.query(Res_line.resnr, Res_line.ankunft, Res_line.abreise, Res_line.arrangement, Res_line.reslinnr, Res_line.zikatnr, Res_line.zinr, Res_line.gastnr, Res_line.gratis, Res_line.erwachs, Res_line.kind1, Res_line.zipreis, Res_line._recid, Res_line.kind2, Res_line.resstatus, Res_line.zimmerfix, Res_line.zimmeranz, Res_line.kontignr).filter(
                ...
                if not vhp_limited:
                    ...
                    segment = get_cache (Segment, {"segmentcode": reservation.segmentcode}, [])
                    do_it = None != segment and segment.vip_level == 0
                ...
                if do_it and res_line.resstatus == 8 and res_line.ankunft == ci_date and res_line.abreise == ci_date:
                    ...
                    bill_line = get_cache (Bill_line, {"departement": 0, "artnr": arrangement.argt_artikelnr, "bill_datum": ci_date, "massnr": res_line.resnr, "billin_nr": res_line.reslinnr}, [])
                ...
                if do_it and zimmer:
                    ...
                    queasy = get_cache (Queasy, {"key": 14, "char1": res_line.zinr, "date1": (le, datum), "date2": (ge, datum)}, ['number3', 'char2', 'number1', 'char3', '_recid'])
                ...
                if do_it:
                    ...
                    for datum in date_range(datum1,datum2) :
                        ...
                        if res_line.zipreis != 0:
                            ...
                            reslin_queasy = get_cache (Reslin_queasy, {"key": "arrangement", "resnr": res_line.resnr, "reslinnr": res_line.reslinnr, "date1": (le, datum), "date2": (ge, datum)}, ['number3', 'deci1', 'deci2', 'deci3', '_recid'])
                        ...
                        if room_list:
                            ...
                            if res_line.zimmerfix:
                                ...
                                rline1 = get_cache (Res_line, {"resnr": res_line.resnr, "reslinnr": (ne, res_line.reslinnr), "resstatus": 8, "abreise": (ge, datum)}, ['resnr', 'ankunft', 'abreise', 'arrangement', 'reslinnr', 'zikatnr', 'zinr', 'gastnr', 'gratis', 'erwachs', 'kind1', 'zipreis', '_recid', 'kind2', 'resstatus', 'zimmerfix', 'zimmeranz', 'kontignr'])
                                ...
                                if arrangement:
                                    ...
                                    for argt_line.argt_artnr, argt_line.departement, argt_line.fakt_modus, argt_line.intervall, argt_line.betriebsnr, argt_line.argtnr, argt_line.betrag, argt_line._recid in db_session.query(Argt_line.argt_artnr, Argt_line.departement, Argt_line.fakt_modus, Argt_line.intervall, Argt_line.betriebsnr, Argt_line.argtnr, Argt_line.betrag, Argt_line._recid).filter(
                                        ...
                                        if post_it:
                                            ...
                                            reslin_queasy = get_cache (Reslin_queasy, {"key": "fargt-line", "char1": "", "number1": argt_line.departement, "number2": argt_line.argtnr, "resnr": res_line.resnr, "reslinnr": res_line.reslinnr, "number3": argt_line.argt_artnr, "date1": (le, curr_date), "date2": (ge, curr_date)}, ['number3', 'deci1', 'deci2', 'deci3', '_recid'])
                            ...
                            if res_line.kontignr > 0 and res_line.active_flag <= 1 and res_line.resstatus <= 6 and res_line.resstatus != 3 and res_line.resstatus != 4 and res_line.abreise > datum:
                                ...
                                if kline:
                                    ...
                                    kontline = get_cache (Kontline, {"kontcode": kline.kontcode, "ankunft": (le, datum), "abreise": (ge, datum), "betriebsnr": 0, "kontstatus": 1}, ['zikatnr', 'zimmeranz', 'arrangement', 'gastnr', 'erwachs', 'kind1', '_recid'])
        ...
        for datum in date_range(d2,to_date) :
            ...
            for kontline.zikatnr, kontline.zimmeranz, kontline.arrangement, kontline.gastnr, kontline.erwachs, kontline.kind1, kontline._recid in db_session.query(Kontline.zikatnr, Kontline.zimmeranz, Kontline.arrangement, Kontline.gastnr, Kontline.erwachs, Kontline.kind1, Kontline._recid).filter(
                ...
                if do_it and not all_segm:
                    ...
                    guest = get_cache (Guest, {"gastnr": kontline.gastnr}, [])

                    guestseg = get_cache (Guestseg, {"gastnr": guest.gastnr, "reihenfolge": 1}, [])

                    if not guestseg:

                        guestseg = get_cache (Guestseg, {"gastnr": guest.gastnr}, [])
        ...
        for i in range(1,tmpint + 1) :
            ...
            for queasy.number3, queasy.char2, queasy.number1, queasy.char3, queasy._recid, zimmer.zikatnr, zimmer.typ, zimmer._recid in db_session.query(Queasy.number3, Queasy.char2, Queasy.number1, Queasy.char3, Queasy._recid, Zimmer.zikatnr, Zimmer.typ, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Queasy.char1) & (Zimmer.sleeping)).filter(
                ...
                guestseg = get_cache (Guestseg, {"gastnr": queasy.number3, "reihenfolge": 1}, [])

                if not guestseg:

                    guestseg = get_cache (Guestseg, {"gastnr": queasy.number3}, [])
                do_it = None != guestseg
        ...
        if show_rev == 1 or show_rev == 2:
            ...
            for s_list in query(s_list_list, filters=(lambda s_list: s_list.selected)):
                ...
                segment = get_cache (Segment, {"segmentcode": s_list.segm}, [])
    ...
    def create_browse1():
        ...
        for res_line.resnr, res_line.ankunft, res_line.abreise, res_line.arrangement, res_line.reslinnr, res_line.zikatnr, res_line.zinr, res_line.gastnr, res_line.gratis, res_line.erwachs, res_line.kind1, res_line.zipreis, res_line._recid, res_line.kind2, res_line.resstatus, res_line.zimmerfix, res_line.zimmeranz, res_line.kontignr in db_session.query(Res_line.resnr, Res_line.ankunft, Res_line.abreise, Res_line.arrangement, Res_line.reslinnr, Res_line.zikatnr, Res_line.zinr, Res_line.gastnr, Res_line.gratis, Res_line.erwachs, Res_line.kind1, Res_line.zipreis, Res_line._recid, Res_line.kind2, Res_line.resstatus, Res_line.zimmerfix, Res_line.zimmeranz, Res_line.kontignr).filter(
            ...
            if not vhp_limited:
                ...
                segment = get_cache (Segment, {"segmentcode": curr_segm}, [])
                do_it = None != segment and segment.vip_level == 0
                ...
                for datum in date_range(datum1,datum2) :
                    ...
                    if room_list:
                        ...
                        if res_line.zimmerfix:
                            ...
                            rline1 = get_cache (Res_line, {"resnr": res_line.resnr, "reslinnr": (ne, res_line.reslinnr), "resstatus": 8, "abreise": (gt, datum)}, ['resnr', 'ankunft', 'abreise', 'arrangement', 'reslinnr', 'zikatnr', 'zinr', 'gastnr', 'gratis', 'erwachs', 'kind1', 'zipreis', '_recid', 'kind2', 'resstatus', 'zimmerfix', 'zimmeranz', 'kontignr'])
