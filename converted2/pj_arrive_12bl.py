...
def pj_arrive_12bl(pvilanguage:int, from_date:date, to_date:date, ci_date:date, disptype:int, incl_tentative:bool, sorttype:int, comment_type:int, incl_accompany:bool, zikat_list_data:[Zikat_list]):

    prepare_cache ([Guest, Htparam, Reservation, Sourccod, Res_line, Guestseg, Segment, Nation, Mc_guest, Mc_types, Reslin_queasy, Paramtext])
    ...
    def generate_output():
        nonlocal tot_rm, tot_a, tot_c, tot_co, cl_list_data, s_list_data, lvcarea, curr_date, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, ol_gastnr, sms_gastnr, wg_gastnr, indi_gastnr, nr, vip_flag, i, dummy_flag, do_it, last_gcf, tentres, all_remark, stat_list, guest, htparam, reservation, sourccod, res_line, guestseg, segment, nation, mc_guest, mc_types, reslin_queasy, zimkateg, history, paramtext
        nonlocal pvilanguage, from_date, to_date, ci_date, disptype, incl_tentative, sorttype, comment_type, incl_accompany
        nonlocal gmember, gbuff


        nonlocal setup_list, cl_list, s_list, zikat_list, gmember, gbuff
        nonlocal setup_list_data, cl_list_data, s_list_data

        return {"tot_rm": tot_rm, "tot_a": tot_a, "tot_c": tot_c, "tot_co": tot_co, "cl-list": cl_list_data, "s-list": s_list_data}

    def create_arrival(curr_date:date):

        nonlocal tot_rm, tot_a, tot_c, tot_co, cl_list_data, s_list_data, lvcarea, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, ol_gastnr, sms_gastnr, wg_gastnr, indi_gastnr, nr, vip_flag, i, dummy_flag, do_it, last_gcf, tentres, all_remark, stat_list, guest, htparam, reservation, sourccod, res_line, guestseg, segment, nation, mc_guest, mc_types, reslin_queasy, zimkateg, history, paramtext
        nonlocal pvilanguage, from_date, to_date, ci_date, disptype, incl_tentative, sorttype, comment_type, incl_accompany
        nonlocal gmember, gbuff
        ...
        if sorttype == 1:
            ...
            for res_line, reservation, sourccod, guest, gmember in db_session.query(Res_line, Reservation, Sourccod, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                ...
                if not incl_accompany:
                    ...
                    cl_list = query(cl_list_data, filters=(lambda cl_list: cl_list.rmno == res_line.zinr and cl_list.resnr == res_line.resnr and date_mdy(cl_list.arrival) == res_line.ankunft and to_decimal(cl_list.zipreis) == 0 and cl_list.co < 1), first=True)
    ...
    def create_actual(curr_date:date):
        ...
        if sorttype == 1:
            ...
            res_line_obj_list = {}
            for res_line, zimkateg, sourccod, guest, gmember in db_session.query(Res_line, Zimkateg, Sourccod, Guest, Gmember).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Sourccod,(Sourccod.source_code == reservation.resart)).join(Guest,(Guest.gastnr == reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
