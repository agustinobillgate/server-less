...
def prepare_fa_stinbl(a_bezeich:string):
    ...
    fa_list_data, Fa_list = create_model_like(Mathis, {"bezeich":string})
    ...
    def generate_output():
        ...
        return {"billdate": billdate, "fa-list": fa_list_data, "lief-list": lief_list_data}
    ...
    mathis_obj_list = {}
    for mathis, fa_artikel, fa_grup in db_session.query(Mathis, Fa_artikel, Fa_grup).join(Fa_artikel,(Fa_artikel.nr == Mathis.nr) & not_ (Fa_artikel.posted) & (Fa_artikel.loeschflag == 0) & (Fa_artikel.next_depn == None)).join(Fa_grup,(Fa_grup.gnr == Fa_artikel.gnr) & (Fa_grup.flag == 0)).filter(
             (Mathis.name >= (a_bezeich).lower())).order_by(Mathis._recid).all():
