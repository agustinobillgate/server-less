...
def birthday_list_1bl(pvilanguage:int):
    ...
    def create_list():
        ...
        res_line_obj_list = []
        recs =  (
                db_session.query(Res_line, Guest)
                        .join(Guest,
                              (Guest.gastnr == Res_line.gastnrmember) &  
                          (((get_month(Guest.geburtdatum1) == get_month(get_current_date())) &  
                            (get_day(Guest.geburtdatum1) == get_day(get_current_date()))) |  
                            ((get_month(Guest.geburtdatum1) == get_month(get_current_date())) &  
                             (get_day(Guest.geburtdatum1) == (get_day(get_current_date())) + 1)))
                             )
                        .filter(
                                (Res_line.active_flag == 0) &  
                                ((Res_line.resstatus <= 5) |  (Res_line.resstatus == 11)) &  
                                (Res_line.ankunft == get_current_date()))
                        .order_by(Guest.name, Res_line.zinr).all()
                    )
        ...
        #             (Guest.gastnr == Res_line.gastnrmember) &
        #             (
        #                     # Condition for today's birthday
        #                     ((extract('month', Guest.geburtdatum1) == current_date.month) &
        #                     (extract('day', Guest.geburtdatum1) == current_date.day)) |
        #                     # Condition for tomorrow's birthday
        #                     ((extract('month', Guest.geburtdatum1) == tomorrow.month) &
        #                     (extract('day', Guest.geburtdatum1) == tomorrow.day))
        #                 )
        #             )
        #             .filter(
        #                 (Res_line.active_flag == 0) &
        #                 ((Res_line.resstatus <= 5) | (Res_line.resstatus == 11)) &
        #                 (Res_line.ankunft == current_date)  # Assuming this is a date comparison
        #             )
        #             .order_by(Guest.name, Res_line.zinr)
        #             .all()
        # )
        for res_line, guest in recs:
            ...
            if not sbuff or not(sbuff.gastnr == guest.gastnr):
                sbuff = db_session.query(Sbuff).filter(
                    (Sbuff.gastnr == guest.gastnr)).first()
        ...
        for res_line, guest in recs:
            ...
            sbuff = db_session.query(Sbuff).filter(
                (Sbuff.gastnr == guest.gastnr) &  (func.lower(Sbuff.resstat) == ("Inhouse").lower())).first()
