...
def selforder_vieworderitembl(inp_outlet_num:int, inp_table_num:int):
    ...
    selected_menu_data, Selected_menu = create_model("Selected_menu", {"rec_id":int, "article_name":string, "quantity":int, "price":Decimal, "post_date":date, "post_time":string, "tax":Decimal, "service":Decimal, "subtotal":Decimal, "special_req":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal record_id, user_init, language_code, outlet_number, order_number, table_number, post_datetime, guest_number, guest_name, pax, room_number, res_number, reslin_numnber, subtotal, mess_result, selected_menu_data, h_bill, bediener, h_bill_line, res_line, htparam, h_journal
        nonlocal inp_outlet_num, inp_table_num


        nonlocal selected_menu
        nonlocal selected_menu_data

        return {"record_id": record_id, "user_init": user_init, "language_code": language_code, "outlet_number": outlet_number, "order_number": order_number, "table_number": table_number, "post_datetime": post_datetime, "guest_number": guest_number, "guest_name": guest_name, "pax": pax, "room_number": room_number, "res_number": res_number, "reslin_numnber": reslin_numnber, "subtotal": subtotal, "mess_result": mess_result, "selected-menu": selected_menu_data}


    selected_menu_data.clear()
    ...
    if h_bill:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            selected_menu_data.append(selected_menu)
