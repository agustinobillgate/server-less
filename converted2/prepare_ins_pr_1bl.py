...
def prepare_ins_pr_1bl(docu_nr:string):
    ...
    s_list_data, S_list = create_model("S_list", {"pos":int, "artnr":int, "new_created":bool, "bemerk":string})
    t_l_orderhdr_data, T_l_orderhdr = create_model("T_l_orderhdr", {"docu_nr":string, "besteller":string, "angebot_lief":[int,3], "bestelldatum":date, "lieferdatum":date})
    ins_list_data, Ins_list = create_model("Ins_list", {"t_recid":int, "artnr":int, "bezeich":string, "anzahl":Decimal, "traubensort":string, "txtnr":int, "lieferdatum":date, "stornogrund":string, "bemerk":string, "quality":string, "jahrgang":int, "new_created":bool, "lief_nr":int, "op_art":int, "docu_nr":string, "bestelldatum":date, "soh":Decimal})
    ...
    def generate_output():
        nonlocal billdate, comments, deptname, pos, t_l_orderhdr_data, ins_list_data, l_artikel, htparam, l_orderhdr, parameters, l_order, l_bestand
        nonlocal docu_nr
        nonlocal l_art


        nonlocal s_list, t_l_orderhdr, ins_list, l_art
        nonlocal s_list_data, t_l_orderhdr_data, ins_list_data

        return {"billdate": billdate, "comments": comments, "deptname": deptname, "pos": pos, "t-l-orderhdr": t_l_orderhdr_data, "ins-list": ins_list_data}
    ...
    for l_order.artnr, l_order.pos, l_order.besteller, l_order._recid, l_order.anzahl, l_order.txtnr, l_order.lieferdatum, l_order.stornogrund, l_order.quality, l_order.lief_nr, l_order.op_art, l_order.docu_nr, l_order.bestelldatum, l_art.bezeich, l_art.traubensorte, l_art.jahrgang, l_art.artnr, l_art._recid in db_session.query(L_order.artnr, L_order.pos, L_order.besteller, L_order._recid, L_order.anzahl, L_order.txtnr, L_order.lieferdatum, L_order.stornogrund, L_order.quality, L_order.lief_nr, L_order.op_art, L_order.docu_nr, L_order.bestelldatum, L_art.bezeich, L_art.traubensorte, L_art.jahrgang, L_art.artnr, L_art._recid).join(L_art,(L_art.artnr == L_order.artnr)).filter(
        ...
        s_list = query(s_list_data, filters=(lambda s_list: s_list.artnr == l_order.artnr and s_list.pos == l_order.pos), first=True)
