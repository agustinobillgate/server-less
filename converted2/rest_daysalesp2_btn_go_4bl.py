...
def rest_daysalesp2_btn_go_4bl(bline_list_list:[Bline_list], buf_art_list:[Buf_art], disc_art1:int, disc_art2:int, disc_art3:int, curr_dept:int, all_user:bool, shift:int, from_date:date, to_date:date, art_str:str, voucher_art:int, zero_vat_compli:bool, exclude_compli:bool, show_fbodisc:bool, htl_dept_dptnr:int, incl_move_table:bool, wig:bool, inhouse:bool):
    ...
    other_art = temp = t_tot_betrag = t_nt_betrag = bline_list = outstand_list = pay_list = pay_listbuff = turnover = summ_list = buf_art = t_artnr = tlist = tlist = tlist = tlist = None
    ...
    turnover_list, Turnover = create_model("Turnover", {"betrag":[decimal,20], "other":decimal, "compli":bool, "flag":int, "gname":str, "int_rechnr":int, "st_comp":int, "p_curr":str, "t_vat":decimal, "qty_fpax":int, "qty_bpax":int, "qty_opax":int, "rest_deposit":decimal, "resnr":int, "cvr_food":int, "cvr_bev":int})
    summ_list_list, Summ_list = create_model("Summ_list", {"amount_food":decimal, "amount_bev":decimal, "amount_other":decimal, "disc_food":decimal, "disc_bev":decimal, "disc_other":decimal, "qty_disc_food":int, "qty_disc_bev":int, "qty_disc_other":int, "amountcvr_food":decimal, "amountcvr_bev":decimal})
    ...
    def generate_output():
        ...
        nonlocal other_art, temp, t_tot_betrag, t_nt_betrag, bline_list, outstand_list, pay_list, pay_listbuff, turnover, summ_list, buf_art, t_artnr, tlist, tlist, tlist, tlist
    ...
    def calculate_disc():
        ...
        nonlocal t_betrag, t_foreign, exchg_rate, tot_serv, tot_tax, tot_debit, tot_cash, tot_cash1, tot_trans, tot_ledger, tot_cover, nt_cover, tot_other, nt_other, nt_serv, nt_tax, nt_debit, nt_cash, nt_cash1, nt_trans, nt_ledger, tot_vat, nt_vat, avail_outstand_list, turnover_list, t_tot_betrag_list, t_nt_betrag_list, outstand_list_list, pay_list_list, summ_list_list, tot_betrag, nt_betrag, t_cash1, tt_other, anz_comp, val_comp, anz_coup, val_coup, total_fdisc, total_bdisc, total_odisc, t_serv, t_tax, t_debit, t_cash, t_trans, t_ledger, t_cover, fo_disc1, fo_disc2, fo_disc3, tt_betrag, multi_vat, f_endkum, b_endkum, tot_deposit, t_deposit, nt_deposit, qty, counter, artnr_data, zeit, artnr_list, ct, l_deci, fact_scvat, pax_cash, pax, pax2, htparam, waehrung, h_bill, h_bill_line, artikel, h_artikel, h_journal, res_line, kellner, bill
        nonlocal disc_art1, disc_art2, disc_art3, curr_dept, all_user, shift, from_date, to_date, art_str, voucher_art, zero_vat_compli, exclude_compli, show_fbodisc, htl_dept_dptnr, incl_move_table, wig, inhouse
        nonlocal pay_listbuff
        ...
        for curr_date in date_range(from_date,to_date) :
            ...
            for h_journal, h_artikel, artikel in db_session.query(H_journal, H_artikel, Artikel).join(H_artikel,(H_artikel.artnr == H_journal.artnr) & (H_artikel.departement == curr_dept)).join(Artikel,(Artikel.artnr == H_artikel.artnrfront) & (Artikel.departement == curr_dept)).filter(
                ...
                turnover = query(turnover_list, (lambda turnover: turnover.rechnr == to_string(h_journal.rechnr) and turnover.departement == h_journal.departement and turnover.flag == 0), first=True)
                if not turnover:
                    continue
                ...
                fact_scvat =  to_decimal("1") + to_decimal(vat) + to_decimal(vat2) + to_decimal(service)
                ...
                netto =  to_decimal(h_journal.betrag) / to_decimal(fact_scvat)
        ...
        while available h_bill_line:
            ...
            if h_bill_line.artnr == 0:
                ...
                if h_artikel.artart == 11 or h_artikel.artart == 12:
                    ...
                    b_hbline = db_session.query(B_hbline).filter(
                                 (B_hbline.bill_datum == h_bill_line.bill_datum) & (B_hbline.rechnr == h_bill_line.rechnr) & (B_hbline.departement == h_bill_line.departement) & (B_hbline.artnr == h_bill_line.artnr) & (B_hbline.bezeich != h_artikel.bezeich) & (h_artikel.activeflag)).first()
                    ...
                    if not cancel_pay:
                        ...
                        if h_artikel.artart == 11:
                            ...
                            if h_artikel.artart == 12:
                                turnover.info = substring(h_artikel.bezeich, 0, 6) + " (Coupon)"
                        tot_trans = tot_trans - h_bill_line.betrag
                ...
                if h_artikel.artart == 0:
                    ...
                    netto =  to_decimal(h_bill_line.betrag) / to_decimal(fact_scvat)
