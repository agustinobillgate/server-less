...
def ap_age2_webbl(pvilanguage:int, to_date:date, from_name:string, to_name:string, mi_bill:bool, segm:int, curr_disp:int, round_zero:bool):
    ...
    apage_list_data, Apage_list = create_model("Apage_list", {"docu_nr":string, "lschein":string, "rgdatum":string, "rechnr":string, "ag_datum":string, "overdue":string, "term_pay":string, "t_no":string, "t_supp":string, "t_saldo":string, "t_prev":string, "t_debit":string, "t_credit":string, "t_debt0":string, "t_debt1":string, "t_debt2":string, "t_debt3":string})
    age_list_data, Age_list = create_model("Age_list", {"artnr":int, "rechnr":string, "lschein":string, "counter":int, "lief_nr":int, "rgdatum":date, "firma":string, "voucher_no":int, "p_bal":Decimal, "debit":Decimal, "credit":Decimal, "saldo":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal, "ap_term":int})
    ...
    if not mi_bill:
        ...
        for age_list in query(age_list_data, filters=(lambda age_list: age_list.tot_debt != 0), sort_by=[("firma",False)]):
            add_detail_date()
