...
def prepare_gl_detailcomplibl(fibu:string, bemerk:string, from_date:date):
    ...
    s_list_data, S_list = create_model("S_list", {"bill_datum":date, "departement":int, "artart":int, "rechnr":int, "artnr":int, "bezeich":string, "anzahl":int, "betrag":Decimal, "cost":Decimal, "tischnr":int, "sysdate":date, "zeit":int, "gname":string, "fibu":string, "kellner_nr":int, "waiter":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_gl_acct_data, s_list_data, gl_acct, htparam, waehrung, exrate, h_artikel, artikel, h_journal, kellner, h_bill, h_cost
        nonlocal fibu, bemerk, from_date


        nonlocal t_gl_acct, s_list
        nonlocal t_gl_acct_data, s_list_data

        return {"t-gl-acct": t_gl_acct_data, "s-list": s_list_data}
    ...
    def disp_it():
        ...
        for s_list in query(s_list_data, filters=(lambda s_list: s_list.artart == 11)):
