...
pay_list_data, Pay_list = create_model("Pay_list", {"dummy":string, "artnr":int, "bezeich":string, "proz":Decimal, "betrag":Decimal, "remark":string})

def ap_web_settle_paymentbl(age_list_data:[Age_list], pay_list_data:[Pay_list], rundung:int, outstand:Decimal, outstand1:Decimal, pay_date:date, remark:string, user_init:string):
    msg_str = ""
    anzahl:int = 0
    okflag:bool = False
    ...
    if okflag:
        pay_list_data, age_list_data, t_l_lieferant_data = get_output(ap_debtpay_settle_paymentbl(pay_list_data, age_list_data, user_init, outstand, outstand1, rundung, pay_date, remark))
