...
def prepare_annual_fcastbl():
    ...
    t_nation_data, T_nation = create_model("T_nation", {"bezeich":string, "kurzbez":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal ci_date, from_month, rm_serv, foreign_rate, ena_rmrev, t_nation_data, tot_room, inactive, nation, htparam, zimkateg, zimmer


        nonlocal t_nation
        nonlocal t_nation_data

        return {"ci_date": ci_date, "from_month": from_month, "rm_serv": rm_serv, "foreign_rate": foreign_rate, "ena_rmrev": ena_rmrev, "t-nation": t_nation_data}
