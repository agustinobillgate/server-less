...
def main_fs_display_browser3bl(b1_resnr:int, b1_resline:int, show_all:bool, rsvsort:int, recid_bk_veran:int):
    ...
    q3_list_data, Q3_list = create_model("Q3_list", {"bk_func_recid":int, "bk_rsv_recid":int, "veran_seite":int, "veran_resnr":int, "raum":string, "datum":date, "bis_datum":date, "resstatus":int, "kartentext":[string,8], "sonstiges":[string,8], "veran_nr":int, "von_zeit":string, "bis_zeit":string})
    ...
    def generate_output():
        nonlocal q3_list_data, bk_reser, bk_veran, bk_func
        nonlocal b1_resnr, b1_resline, show_all, rsvsort, recid_bk_veran
        nonlocal bk_rsv


        nonlocal bk_rsv, q3_list
        nonlocal q3_list_data

        return {"q3-list": q3_list_data}
