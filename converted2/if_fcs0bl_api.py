...
def if_fcs0bl_api(p_type:int, p_status:string):
    ...
    tt_data_data, Tt_data = create_model("Tt_data", {"date_if":date, "resno":int, "roomno":string, "roomtype":string, "roomstatus":string, "guestname":string, "arrival":date, "departure":date, "status_send":string, "parameters":string, "flags":string, "irecid":})
    ...
    for interface in db_session.query(Interface).filter(
             (Interface.key == 38) & (matches(Interface.nebenstelle,v_nebenstelle)) & ((Interface.parameters != ("modify").lower()) | ((Interface.parameters == ("modify").lower()) & (Interface.zinr != "")))).order_by(Interface._recid).all():
        tt_data = Tt_data()
        tt_data_data.append(tt_data)
            ...
            tt_data.status_send = (IF matches(interface.nebenstelle, "*$FCS1$*") THEN
            "SUCCESS"
            ELSE
            "FAILED")
            tt_data.irecid = interface._recid
        ...
        if p_type == 1:
            ...
            if res_line:
                ...
                if guest:
                    tt_data.guestname = trim(guest.vorname1) + trim(guest.name)
