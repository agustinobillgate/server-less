...
def avail_ddown_detail_tentativebl(datum:date, curr_date:date):
    ...
    def generate_output():
        nonlocal rlist_data, zimkateg, res_line, zimmer, reservation, segment, guest
        nonlocal datum, curr_date
    ...
    def tentative():
        ...
        for zimkateg in db_session.query(Zimkateg).order_by(Zimkateg._recid).all():
            ...
            for res_line in db_session.query(Res_line).filter(
                     (Res_line.active_flag <= 1) & (Res_line.resstatus <= 13) & (Res_line.resstatus != 4) & (Res_line.resstatus != 12) & (Res_line.zikatnr == zimkateg.zikatnr) & (Res_line.ankunft >= datum) & (Res_line.abreise <= datum) & (Res_line.gastnr > 0) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
                do_it = True
