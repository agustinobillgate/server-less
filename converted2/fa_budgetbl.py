...
def fa_budgetbl(from_date:date, to_date:date, detailed:bool):
    ...
    fa_budget_data, Fa_budget = create_model("Fa_budget", {"name":string, "asset":string, "datum":date, "bezeich":string, "fibukonto":string, "price":Decimal, "price_str":string, "anzahl":int, "anzahl_str":string, "warenwert":Decimal, "warenwert_str":string, "mtd_budget":Decimal, "mtd_budget_str":string, "mtd_balance":Decimal, "mtd_balance_str":string, "ytd_budget":Decimal, "ytd_budget_str":string, "ytd_balance":Decimal, "ytd_balance_str":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal fa_budget_data, i, c, fibu, beg_date, jan, mm, mtd_budget, mtd_balance, ytd_budget, ytd_balance, t_anzahl, t_warenwert, t_mtd_balance, t_ytd_balance, tt_anzahl, tt_warenwert, tt_mtd_balance, tt_ytd_balance, mathis, fa_artikel, fa_grup, gl_acct, fa_op, htparam
        nonlocal from_date, to_date, detailed


        nonlocal fa_budget
        nonlocal fa_budget_data

        return {"fa-budget": fa_budget_data}

    def convert_fibu(konto:string):

        nonlocal fa_budget_data, c, fibu, beg_date, jan, mm, mtd_budget, mtd_balance, ytd_budget, ytd_balance, t_anzahl, t_warenwert, t_mtd_balance, t_ytd_balance, tt_anzahl, tt_warenwert, tt_mtd_balance, tt_ytd_balance, mathis, fa_artikel, fa_grup, gl_acct, fa_op, htparam
        nonlocal from_date, to_date, detailed
    ...
    if detailed:
        ...
        for fa_op.anzahl, fa_op.warenwert, fa_op.datum, fa_op.einzelpreis, fa_op._recid, mathis.name, mathis._recid, fa_artikel.fibukonto, fa_artikel._recid, fa_grup.bezeich, fa_grup._recid, gl_acct.budget, gl_acct._recid in db_session.query(Fa_op.anzahl, Fa_op.warenwert, Fa_op.datum, Fa_op.einzelpreis, Fa_op._recid, Mathis.name, Mathis._recid, Fa_artikel.fibukonto, Fa_artikel._recid, Fa_grup.bezeich, Fa_grup._recid, Gl_acct.budget, Gl_acct._recid).join(Mathis,(Mathis.nr == Fa_op.nr)).join(Fa_artikel,(Fa_artikel.nr == Fa_op.nr)).join(Fa_grup,(Fa_grup.gnr == Fa_artikel.subgrp) & (Fa_grup.flag > 0)).join(Gl_acct,(Gl_acct.fibukonto == Fa_artikel.fibukonto)).filter(
            ...
            if fibu == "":
                ...
                fa_budget = Fa_budget()
                fa_budget_data.append(fa_budget)
