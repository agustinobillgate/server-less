...
def mbill_assignbl(resnr:int):
    ...
    s_list_data, S_list = create_model("S_list", {"resnr":int, "zinr":string, "name":string, "ankunft":date, "abreise":date, "reslinnr":int, "assigned":bool, "ass":bool})
    buf_s_list_data, Buf_s_list = create_model_like(S_list)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal s_list_data, res_line
        nonlocal resnr
    ...
    res_line_obj_list = {}
    for res_line in db_session.query(Res_line).filter(
             ((Res_line.resnr.in_(list(set([buf_s_list.resnr for buf_s_list in buf_s_list_data])))) & (Res_line.reslinnr == buf_s_list.reslinnr))).order_by(Res_line.resnr, Res_line.zinr, Res_line.name).all():
        if res_line_obj_list.get(res_line._recid):
            continue
        else:
            res_line_obj_list[res_line._recid] = True

        buf_s_list = query(buf_s_list_data, (lambda buf_s_list: (res_line.resnr == buf_s_list.resnr)), first=True)
        s_list = S_list()
