...
def prepare_fo_transdetailbl(bill_no:int, billdate:date, artno:int, amount:Decimal, systdate:date, systtime:int):
    ...
    temp_bjournal_data, Temp_bjournal = create_model_like(Billjournal, {"item_name":string}, {"item_name": ""})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal created, found, err, temp_bjournal_data, currdate, ci_dt, co_dt, fr_date, to_date, billjournal, bill, res_line, artikel
        nonlocal bill_no, billdate, artno, amount, systdate, systtime


        nonlocal temp_bjournal
        nonlocal temp_bjournal_data

        return {"created": created, "found": found, "err": err, "temp-bjournal": temp_bjournal_data}
