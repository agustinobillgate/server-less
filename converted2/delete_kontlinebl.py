...
from models import Queasy, Kontline, Zimkateg, Res_history

def delete_kontlinebl(case_type:int, kontignr:int, kontcode:string, gastno:int):

    prepare_cache ([Queasy, Zimkateg, Res_history])

    success_flag = False
    curr_date:date = None
    zikatnr:int = 0
    queasy = kontline = zimkateg = res_history = None
    ...
    if case_type == 1:
        ...
        res_history.aenderung = "Delete | kontignr: " + to_string(kontignr) + " kontcode: " + to_string(kontcode) +\
                " gastno: " + to_string(gastno)
