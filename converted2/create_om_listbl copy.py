...
def create_om_listbl(fdate:date, tdate:date):
    om_list_list = []
    datum:date = None
    ...
    if fdate != None and tdate != None:
        for datum in date_range(fdate,tdate) :
            ...
            for outorder in db_session.query(Outorder).filter(
                ...
                if not zimmer or not(zimmer.zinr == outorder.zinr):
                ...
                if zimmer:
                    ...
                    if num_entries(outorder.gespgrund, "$") >= 1:
                        ...
                        if not bediener or not(bediener.nr == to_int(entry(1, outorder.gespgrund, "$"))):
                            bediener = db_session.query(Bediener).filter(
                                (Bediener.nr == to_int(entry(1, outorder.gespgrund, "$")))).first()
                    else:

                        if not bediener or not(bediener.nr == zimmer.bediener_nr_stat):
                            bediener = db_session.query(Bediener).filter(
