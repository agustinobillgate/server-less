...
fixleist_list_data, Fixleist_list = create_model_like(Fixleist)

def res_fixart_webbl(fixleist_list_data:[Fixleist_list], pvilanguage:int, rec_id:int, resnr:int, reslinnr:int, case_type:int, user_init:string):
    ...
    fixleist_list = query(fixleist_list_data, first=True)
