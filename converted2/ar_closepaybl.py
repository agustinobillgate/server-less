...
def ar_closepaybl(case_type:int, bill_name:string, bill_nr:int, bill_artnr:int, counter:int, balance:Decimal):
    ...
    b2_list_data, B2_list = create_model("B2_list", {"name":string, "vorname1":string, "anrede1":string, "anredefirma":string, "rechnr":int, "artnr":int, "bezeich":string, "rgdatum":date, "saldo":Decimal, "counter":int, "b_resname":string, "b_comments":string})
    b3_list_data, B3_list = create_model("B3_list", {"rgdatum":date, "zahlkonto":int, "bezeich":string, "saldo":Decimal, "vesrdep":Decimal, "userinit":string})
    artikel2_data, Artikel2 = create_model("Artikel2", {"artnr":int, "bezeich":string})
    ...
    def generate_output():
        nonlocal b2_list_data, b3_list_data, artikel2_data, artikel, debitor, bediener, guest
        ...
        nonlocal b2_list_data, b3_list_data, artikel2_data

        return {"balance": balance, "b2-list": b2_list_data, "b3-list": b3_list_data, "artikel2": artikel2_data}
