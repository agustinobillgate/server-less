...
def gl_parxls1_2bl(briefnr:int, gl_year:int, from_date:date, to_date:date, foreign_flag:bool):
    ...
    t_list_data, T_list = create_model("T_list", {"cf":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "debit_lsyear":Decimal, "credit_lsyear":Decimal, "debit_lsmonth":Decimal, "credit_lsmonth":Decimal, "balance":Decimal, "ly_balance":Decimal, "pm_balance":Decimal})
    ...
    def generate_output():
        ...
        nonlocal coa_list_data, t_list_data, t_parameters_data, t_gl_accthis_data, t_gl_acct_data, t_artikel_data, t_umsz_data, g_list_data, temp_list_data, w1_data

        return {"combo_pf_file1": combo_pf_file1, "combo_pf_file2": combo_pf_file2, "t-parameters": t_parameters_data, "t-gl-accthis": t_gl_accthis_data, "t-gl-acct": t_gl_acct_data, "t-artikel": t_artikel_data, "t-umsz": t_umsz_data, "temp-list": temp_list_data, "t-list": t_list_data, "w1": w1_data}

    def fill_exrate():

        nonlocal combo_pf_file1, combo_pf_file2, t_parameters_data, t_gl_accthis_data, t_gl_acct_data, t_artikel_data, t_umsz_data, temp_list_data, t_list_data, w1_data, curr_i, serv, vat, fact, n_betrag, credit, debit, frate, price_decimal, from_year, to_year, cash_flow, prev_str, done_segment, datum1, jan1, ljan1, lfrom_date, lto_date, do_it, actual_exrate, budget_exrate, lyear_exrate, blyear_exrate, bnyear_exrate, parameters, gl_accthis, gl_acct, artikel, exrate, htparam, gl_jouhdr, gl_journal, waehrung, genstat, segmentstat, umsatz, budget
        nonlocal briefnr, gl_year, from_date, to_date, foreign_flag
        nonlocal buff_exrate
    ...
    def calc_cf():
        ...
        t_list_data = get_output(glacct_cashflowbl(from_date, to_date, from_lsyr, to_lsyr, pfrom_date, pto_date, coa_list_data))
