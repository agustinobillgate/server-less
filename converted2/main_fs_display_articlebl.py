...
def main_fs_display_articlebl(b1_resnr:int, resnr:int, resline:int):
    ...
    bkrart_data, Bkrart = create_model_like(Bk_rart, {"tischform":string, "amount":Decimal})
    t_bkrart_data, T_bkrart = create_model_like(Bkrart)
    ...
    def generate_output():
        nonlocal ol_data, t_bkrart_data, status_chr, bk_rart, bk_func, bk_reser
        nonlocal b1_resnr, resnr, resline
        nonlocal func_buff


        nonlocal ol, bkrart, t_bkrart, func_buff
        nonlocal ol_data, bkrart_data, t_bkrart_data

        return {"ol": ol_data, "t-bkrart": t_bkrart_data}
