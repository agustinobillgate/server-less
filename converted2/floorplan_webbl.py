...
gstat_list_data, Gstat_list = create_model("Gstat_list", {"selected":bool, "resstatus":int, "bezeich":string})
zikat_list_data, Zikat_list = create_model("Zikat_list", {"selected":bool, "zikatnr":int, "kurzbez":string, "bezeich":string})
zistat_list_data, Zistat_list = create_model("Zistat_list", {"selected":bool, "zistatus":int, "bezeich":string})

def floorplan_webbl(gstat_list_data:[Gstat_list], zikat_list_data:[Zikat_list], zistat_list_data:[Zistat_list], location:int, floor:int, all_gstat:bool, all_zikat:bool, all_zistat:bool, ci_date:date, pvilanguage:int):
    ...
    florplan_list_data, Florplan_list = create_model("Florplan_list", {"char1":string, "deci1":Decimal, "deci2":Decimal, "curr_n":int, "bcol":int, "fcol":int, "zistatus":int, "g_info":string, "r_info":string})
    ...
    Gtbuff = Gstat_list
    gtbuff_data = gstat_list_data

    Zkbuff = Zikat_list
    zkbuff_data = zikat_list_data

    Stbuff = Zistat_list
    stbuff_data = zistat_list_data

    db_session = local_storage.db_session

    def generate_output():
        nonlocal florplan_list_data, lvcarea, do_it, curr_n, zistatus, bcol, fcol, g_info, r_info, n_edit, c_edit, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, item_fgcol, zimmer, res_line, reservation, waehrung, outorder, htparam, queasy
        ...
        nonlocal florplan_list_data, t_zimmer_data, t_res_line_data, t_reservation_data, t_waehrung_data, t_outorder_data

        return {"florplan-list": florplan_list_data}
    ...
    def fill_ooo(curr_room:string):
        ...
        t_outorder_data.clear()
        t_res_line_data = get_output(read_res_linebl(20, None, None, 6, 1, curr_room, None, None, None, None, ""))

        t_res_line = query(t_res_line_data, first=True)

        if not t_res_line:
            t_res_line_data = get_output(read_res_linebl(20, None, None, 13, 1, curr_room, None, None, None, None, ""))

        if not t_res_line:
            t_res_line_data = get_output(read_res_linebl(55, None, None, None, None, curr_room, ci_date, None, None, None, ""))

        t_res_line = query(t_res_line_data, first=True)

        if not t_res_line:
            t_res_line_data = get_output(read_res_linebl(56, None, None, None, None, curr_room, None, None, None, None, ""))

        if t_res_line:
            t_reservation_data = get_output(read_reservationbl(1, t_res_line.resnr, None, ""))
            ...
            if t_res_line.betriebsnr > 0:
                t_waehrung_data = get_output(read_waehrungbl(1, t_res_line.betriebsnr, ""))
            ...
            if zimmer:
                t_outorder_data = get_output(read_outorderbl(4, zimmer.zinr, None, ci_date, None))
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        if not zimmer:
            ...
            if not all_gstat:
                ...
                for res_line in db_session.query(Res_line).filter(
                    ...
                    if res_line.active_flag == 0 and res_line.ankunft == ci_date:
                        ...
                        gtbuff = query(gtbuff_data, filters=(lambda gtbuff: gtbuff.selected and gtbuff.resstatus == 0), first=True)
                        ...
                        gtbuff = query(gtbuff_data, filters=(lambda gtbuff: gtbuff.selected and gtbuff.resstatus == 1), first=True)
                        ...
                        gtbuff = query(gtbuff_data, filters=(lambda gtbuff: gtbuff.selected and gtbuff.resstatus == 2), first=True)
            ...
            if do_it and not all_zikat:
                ...
                zkbuff = query(zkbuff_data, filters=(lambda zkbuff: zkbuff.selected and zkbuff.zikatnr == zimmer.zikatnr), first=True)
            ...
            if do_it and not all_zistat:
                ...
                stbuff = query(stbuff_data, filters=(lambda stbuff: stbuff.selected and stbuff.zistatus == zimmer.zistatus), first=True)
            ...
            if do_it:
                ...
                florplan_list_data.append(florplan_list)
