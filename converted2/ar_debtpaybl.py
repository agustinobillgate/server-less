...
def ar_debtpaybl(artnr:int, userinit:string, bill_nr:int, temp_art2:int, bill_date:date, bill_name:string, to_name:string, bill_saldo:Decimal, art_selected:int, foutstand:Decimal, outstand:Decimal):

    prepare_cache ([Debitor, Guest, Bill_line, Bediener, Waehrung, Res_line])
    ...
    def generate_output():
        nonlocal curr_art, age_list_data, t_debt_data, t_resnr, t_name, debitor, artikel, guest, bill_line, bediener, waehrung, reservation, res_line
        nonlocal artnr, userinit, bill_nr, temp_art2, bill_date, bill_name, to_name, bill_saldo, art_selected, foutstand, outstand
        nonlocal abuff, debthis


        nonlocal t_debt, age_list, abuff, debthis, age_list1
        nonlocal t_debt_data, age_list_data

        return {"art_selected": art_selected, "foutstand": foutstand, "outstand": outstand, "curr_art": curr_art, "age-list": age_list_data, "t-debthis": t_debt_data}
        ...
        if bill_nr != 0:
            ...
            for debitor in db_session.query(Debitor).filter(
                ...
                if debitor.zahlkonto == 0:
                    ...
                    age_list.rgdatum = debitor.rgdatum
                    age_list.debthis =  to_decimal(age_list.debthis) + to_decimal(debitor.saldo)
    ...
    def disp_guest_debt():
        ...
        if debitor.counter != 0:
            ...
            for debthis in db_session.query(Debt).filter(
                ...
                if debthis.vesrcod != "":
                    age_list.b_resname = age_list.b_resname + chr_unicode(10) + to_string(debthis.rgdatum) + " " + trim(to_string(debthis.saldo, "->>>,>>>,>>9.99")) + ": " + debthis.vesrcod
    ...
    for debthis in db_session.query(Debt).filter(
        ...
        if debt_obj_list.get(debthis._recid):
            continue
        else:
            debt_obj_list[debthis._recid] = True
