...
def get_gcflistbl(case_type:int, sorttype:int, lname:string, fname:string):
    ...
    def generate_output():
        nonlocal first_gastnr, current_lname, current_fname, t_guest_data
        ...
        return {"first_gastnr": first_gastnr, "current_lname": current_lname, "current_fname": current_fname, "t-guest": t_guest_data}

    if case_type == 1:
        t_guest_data.clear()
        first_gastnr, current_lname, current_fname, t_guest_data = get_output(gcf_listbl(1, sorttype, "", "", None))
    else:
        first_gastnr, current_lname, current_fname, t_guest_data = get_output(gcf_listbl(2, sorttype, lname, "", 0))
