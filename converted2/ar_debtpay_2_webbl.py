...
input_payload_data, Input_payload = create_model("Input_payload", {"artnr":int, "userinit":string, "bill_nr":int, "temp_art2":int, "from_date":date, "to_date":date, "bill_name":string, "to_name":string, "bill_saldo":Decimal, "period_flag":bool, "remark_str":string, "from_dpt":int, "to_dpt":int})

def ar_debtpay_2_webbl(input_payload_data:[Input_payload], art_selected:int, foutstand:Decimal, outstand:Decimal):
    ...
    def create_age_list():
        ...
        if input_payload.bill_saldo != 0:
            ...
            for age_list in query(age_list_data, filters=(lambda age_list: age_list.tot_debt != input_payload.bill_salDO)):
