...
def prepare_argt_adminbl(pvilanguage:int):
    ...
    w_list_data, W_list = create_model("W_list", {"bez":string, "first_bez":string})
    ...
    def generate_output():
        nonlocal double_currency, msg_str, w_list_data, t_arrangement_data, foreign_rate, local_nr, foreign_nr, lvcarea, arrangement, waehrung, htparam
        nonlocal pvilanguage
        nonlocal waehrung1


        nonlocal t_arrangement, w_list, waehrung1
        nonlocal t_arrangement_data, w_list_data

        return {"double_currency": double_currency, "msg_str": msg_str, "w-list": w_list_data, "t-arrangement": t_arrangement_data}
