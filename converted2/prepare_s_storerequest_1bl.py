...
def prepare_s_storerequest_1bl(user_init:string):
    ...
    temp_l_artikel_data, Temp_l_artikel = create_model("Temp_l_artikel", {"artnr":int, "betriebsnr":int, "endkum":int, "bezeich":string, "masseinheit":string, "vk_preis":Decimal, "lief_einheit":Decimal, "traubensort":string})
    t_parameters_data, T_parameters = create_model_like(Parameters)
    t_l_lager_data, T_l_lager = create_model_like(L_lager)
    art_list_data, Art_list = create_model("Art_list", {"artnr":int, "zwkum":int, "endkum":int, "zwkum_bezeich":string, "endkum_bezeich":string})
    ...
    def generate_output():
        ...
        return {"show_price": show_price, "req_flag": req_flag, "transdate": transdate, "t-parameters": t_parameters_data, "t-l-lager": t_l_lager_data, "temp-l-artikel": temp_l_artikel_data, "art-list": art_list_data}
