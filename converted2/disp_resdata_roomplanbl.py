...
def disp_resdata_roomplanbl(pvilanguage:int, curr_date:date, i:int, zinr:string, gstatus:int, recid1:int):
    ...
    buf_res_line_data, Buf_res_line = create_model("Buf_res_line", {"gastnr":int, "resnr":int, "reslinnr":int, "active_flag":int, "zinr":string, "kurzbez":string, "resstatus":int, "ankunft":date, "abreise":date, "betrieb_gast":int, "zipreis":Decimal, "was_status":int, "name":string, "ziwech_zeit":int, "recid1":int})
    buf_reservation_data, Buf_reservation = create_model("Buf_reservation", {"gastnr":int, "resnr":int, "grpflag":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal n_edit, c_edit, fg_col, flag_res_line, flag_outorder, last_zinr, buf_res_line_data, buf_reservation_data, lvcarea, res_line, reservation, waehrung, outorder, zimkateg
        nonlocal pvilanguage, curr_date, i, zinr, gstatus, recid1


        nonlocal buf_res_line, buf_reservation
        nonlocal buf_res_line_data, buf_reservation_data

        return {"n_edit": n_edit, "c_edit": c_edit, "fg_col": fg_col, "flag_res_line": flag_res_line, "flag_outorder": flag_outorder, "last_zinr": last_zinr, "buf-res-line": buf_res_line_data, "buf-reservation": buf_reservation_data}
    ...
    def disp_res_data():
        ...
        if res_line:
            ...
            buf_res_line_data.append(buf_res_line)
            ...
            if reservation:
                ...
                buf_reservation_data.append(buf_reservation)
