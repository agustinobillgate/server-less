...
def mp_select_instructionbl(dept_nr:int):
    ...
    t_queasy_data, T_queasy = create_model("T_queasy", {"instruct_no":int, "dept_no":int, "department":string, "instruction":string})
    ...
    for bk_queasy in query(bk_queasy_data, filters=(lambda bk_queasy: bk_queasy.key == 13)):
