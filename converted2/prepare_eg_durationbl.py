...
def prepare_eg_durationbl(user_init:string):
    ...
    t_eg_duration_data, T_eg_duration = create_model_like(Eg_duration, {"rec_id":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal groupid, engid, sduration_data, t_eg_duration_data, eg_duration, bediener, htparam
        nonlocal user_init


        nonlocal sduration, t_eg_duration
        nonlocal sduration_data, t_eg_duration_data

        return {"groupid": groupid, "engid": engid, "sduration": sduration_data, "t-eg-Duration": t_eg_duration_data}
    ...
    for eg_duration in db_session.query(Eg_duration).order_by(Eg_duration._recid).all():
        ...
        t_eg_duration_data.append(t_eg_duration)
