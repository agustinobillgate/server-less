...
def prepare_select_szwkumbl(main_nr:int):
    ...
    szwkum_list_data, Szwkum_list = create_model("Szwkum_list", {"zwkum":int, "bezeich":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal szwkum_list_data, l_untergrup, queasy
        nonlocal main_nr


        nonlocal szwkum_list
        nonlocal szwkum_list_data

        return {"szwkum-list": szwkum_list_data}

    if main_nr == 0:

        for l_untergrup in db_session.query(L_untergrup).order_by(L_untergrup._recid).all():
            szwkum_list = Szwkum_list()
            szwkum_list_data.append(szwkum_list)
        ...
        for queasy in db_session.query(Queasy).filter(
            ...
            if l_untergrup:
                ...
                szwkum_list = query(szwkum_list_data, filters=(lambda szwkum_list: szwkum_list.zwkum == l_untergrup.zwkum), first=True)
