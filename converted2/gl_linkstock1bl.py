...
def gl_linkstock1bl(pvilanguage:int, link_out:bool, link_in:bool, from_grp:int, from_date:date, to_date:date, user_init:string):
    ...
    t_g_list_data, T_g_list = create_model_like(G_list)
    s_list_data, S_list = create_model("S_list", {"nr":int, "name":string, "debit":Decimal, "credit":Decimal})
    ...
    def generate_output():
        nonlocal curr_anz, credits, debits, remains, msg_str, msg_str2, msg_str3, msg_str4, msg_str5, t_g_list_data, s_list_data, curr_zeit, curr_lschein, curr_note, add_note, fibukonto, debit_betrag, credit_betrag, tot_credit, tot_debit, counter, lvcarea, gl_acct, htparam, artikel, queasy, l_ophdr, l_artikel, l_untergrup, l_op, l_lieferant, l_kredit, fa_artikel, fa_op, fa_grup
        nonlocal pvilanguage, link_out, link_in, from_grp, from_date, to_date, user_init
        nonlocal gl_acct1


        nonlocal g_list, t_g_list, s_list, gl_acct1
        nonlocal g_list_data, t_g_list_data, s_list_data

        return {"curr_anz": curr_anz, "credits": credits, "debits": debits, "remains": remains, "msg_str": msg_str, "msg_str2": msg_str2, "msg_str3": msg_str3, "msg_str4": msg_str4, "msg_str5": msg_str5, "t-g-list": t_g_list_data, "s-list": s_list_data}
        ...
        for l_op.stornogrund, l_op.warenwert, l_op.lscheinnr, l_op.lief_nr, l_op.lager_nr, l_op.docu_nr, l_op.artnr, l_op.op_art, l_op._recid, l_ophdr.fibukonto, l_ophdr.lscheinnr, l_ophdr.lager_nr, l_ophdr._recid, l_artikel.bezeich, l_artikel.fibukonto, l_artikel.artnr, l_artikel._recid, l_untergrup.fibukonto, l_untergrup._recid in db_session.query(L_op.stornogrund, L_op.warenwert, L_op.lscheinnr, L_op.lief_nr, L_op.lager_nr, L_op.docu_nr, L_op.artnr, L_op.op_art, L_op._recid, L_ophdr.fibukonto, L_ophdr.lscheinnr, L_ophdr.lager_nr, L_ophdr._recid, L_artikel.bezeich, L_artikel.fibukonto, L_artikel.artnr, L_artikel._recid, L_untergrup.fibukonto, L_untergrup._recid).join(L_ophdr,(L_ophdr.lscheinnr == L_op.lscheinnr) & (L_ophdr.op_typ == ("STT").lower()) & (L_ophdr.fibukonto != "")).join(L_artikel,(L_artikel.artnr == L_op.artnr)).join(L_untergrup,(L_untergrup.zwkum == L_artikel.zwkum)).filter(
            ...
            if do_it:
                ...
                if gl_acct:
                    ...
                    if l_op.warenwert > 0:
                        ...
                        g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acct.fibukonto and g_list.lscheinnr == l_op.lscheinnr and g_list.credit != 0), first=True)
                        ...
                        g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acct.fibukonto and g_list.lscheinnr == l_op.lscheinnr and g_list.debit != 0), first=True)
            ...
            if cost_value != 0:
                ...
                if cost_value > 0:
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto.lower()  == (cost_account).lower()  and g_list.lscheinnr == l_op.lscheinnr and g_list.debit != 0), first=True)
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto.lower()  == (cost_account).lower()  and g_list.lscheinnr == l_op.lscheinnr and g_list.credit != 0), first=True)
    ...
    def step_three():
        ...
        nonlocal curr_anz, credits, debits, remains, msg_str, msg_str2, msg_str3, msg_str4, msg_str5, t_g_list_data, s_list_data, curr_zeit, curr_note, add_note, fibukonto, debit_betrag, credit_betrag, tot_credit, tot_debit, lvcarea, gl_acct, htparam, artikel, queasy, l_ophdr, l_artikel, l_untergrup, l_op, l_lieferant, l_kredit, fa_artikel, fa_op, fa_grup
        nonlocal pvilanguage, link_out, link_in, from_grp, from_date, to_date, user_init
        nonlocal gl_acct1
            ...
            if gl_acct:
                ...
                if queasy:
                    ...
                    if (l_op.warenwert * (queasy.deci1 / 100)) >= 0:
                        ...
                        g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acc2.fibukonto and g_list.lscheinnr == l_op.lscheinnr and g_list.debit != 0), first=True)
                        ...
                        g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acc2.fibukonto and g_list.lscheinnr == l_op.lscheinnr and g_list.credit != 0), first=True)
    ...
    def step_three_fa():
        ...
        nonlocal curr_anz, credits, debits, remains, msg_str, msg_str2, msg_str3, msg_str4, msg_str5, t_g_list_data, s_list_data, curr_zeit, curr_note, add_note, fibukonto, debit_betrag, credit_betrag, tot_credit, tot_debit, counter, lvcarea, gl_acct, htparam, artikel, queasy, l_ophdr, l_artikel, l_untergrup, l_op, l_lieferant, l_kredit, fa_artikel, fa_op, fa_grup
        nonlocal pvilanguage, link_out, link_in, from_grp, from_date, to_date, user_init
        nonlocal gl_acct1
        ...
        for fa_op.nr, fa_op.docu_nr, fa_op.lscheinnr, fa_op.warenwert, fa_op._recid, l_lieferant.firma, l_lieferant.z_code, l_lieferant._recid, fa_artikel.subgrp, fa_artikel.fibukonto, fa_artikel._recid in db_session.query(Fa_op.nr, Fa_op.docu_nr, Fa_op.lscheinnr, Fa_op.warenwert, Fa_op._recid, L_lieferant.firma, L_lieferant.z_code, L_lieferant._recid, Fa_artikel.subgrp, Fa_artikel.fibukonto, Fa_artikel._recid).join(L_lieferant,(L_lieferant.lief_nr == Fa_op.lief_nr)).join(Fa_artikel,(Fa_artikel.nr == Fa_op.nr)).filter(
            ...
            if fa_op.warenwert >= 0:
                ...
                g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acct.fibukonto and g_list.lscheinnr == fa_op.lscheinnr and g_list.debit != 0), first=True)
                ...
                g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acct.fibukonto and g_list.lscheinnr == fa_op.lscheinnr and g_list.credit != 0), first=True)
            ...
            if queasy:
                ...
                if (fa_op.warenwert * (queasy.deci1 / 100)) >= 0:
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acc2.fibukonto and g_list.lscheinnr == fa_op.lscheinnr and g_list.debit != 0), first=True)
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acc2.fibukonto and g_list.lscheinnr == fa_op.lscheinnr and g_list.credit != 0), first=True)
    ...
    def add_list1(create_it:bool, nr:int):
        ...
        if nr > 0:
            ...
            s_list = query(s_list_data, filters=(lambda s_list: s_list.nr == nr), first=True)
    ...
    for g_list in query(g_list_data, sort_by=[("zeit",False),("substring(",False))]:

        gl_acct1 = db_session.query(Gl_acct1).filter(
                 (Gl_acct1.fibukonto == g_list.fibukonto)).first()

        if gl_acct1:
            t_g_list = T_g_list()
            t_g_list_data.append(t_g_list)
