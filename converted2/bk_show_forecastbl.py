...
def bk_show_forecastbl(blockid:string):
    t_bk_event_data = []
    t_bk_room_data = []
    ...
    t_bk_event_data, T_bk_event = create_model("T_bk_event", {"bezeich":string, "amount":Decimal})
    t_bk_room_data, T_bk_room = create_model("T_bk_room", {"bezeich":string, "amount":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_bk_event_data, t_bk_room_data, grandtotal, totamount, i, blockcode, revenue, roomrates
        nonlocal blockid


        nonlocal t_bk_event, t_bk_room
        nonlocal t_bk_event_data, t_bk_room_data

        return {"t-bk-event": t_bk_event_data, "t-bk-room": t_bk_room_data}

    grandtotal =  to_decimal("0")
    totamount =  to_decimal("0")

    for bk_event in query(bk_event_data, filters=(lambda bk_event: bk_event.block_id == blockid and bk_event.flag.lower()  == ("**").lower())):
    ...
    t_bk_event_data.append(t_bk_event)
