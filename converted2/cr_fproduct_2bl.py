...
def cr_fproduct_2bl(pvilanguage:int, op_type:int, printer_nr:int, call_from:int, txt_file:string, fr_date:date, to_date:date, disptype:int, cardtype:int, stattype:int, rev_calc:int, exc_oral6pm:bool, excl_comp:bool, vhp_limited:bool, scin:bool, argt_code:string):
    ...
    output_list1_data, Output_list1 = create_model("Output_list1", {"flag":int, "firmen_nr":int, "refno":string, "avrg_amount":string, "name":string, "str2":string, "str4":string, "fcost":string, "name1":string, "zinr":string, "pax":string, "exp_rev":string, "curr":string, "loc_curr":string, "lodg":string, "bfast":string, "lunch":string, "dinner":string, "oth_rev":string})
    ...
    def generate_output():
        nonlocal output_list1_data, output_list2_data, lvcarea, report_title, tot_room, tot_pax, tot_logis, tot_bfast, tot_lunch, tot_dinner, tot_misc, tot_rmonly, tot_zipreis, tot_fcost, argtcode, nation, sourccod, segment, arrangement, reslin_queasy, ratecode, kontplan
    ...
    def create_to_list2():
        ...
        if disptype == 1:
            ...
            to_list = query(to_list_data, filters=(lambda to_list: matches(to_list.argtcode, t_list).argtcode and to_list.curr == t_list.currency), first=True)
    ...
    def create_browse():
        ...
        if rev_calc == 1:
            ...
            if disptype == 5:
                ...
                for to_list in query(to_list_data, filters=(lambda to_list: matches(to_list.argtcode,argtcode))):
    ...
    t_list_data = get_output(create_forecast_history_2bl(fr_date, to_date, excl_comp, vhp_limited, scin))
