...
def generate_gstbl(fdate:date, tdate:date, sorttype:int):
    ...
    t_rcv_trx_data, T_rcv_trx = create_model("T_rcv_trx", {"rcvtrx_suppname":string, "rcvtrx_suppbrn":int, "rcvtrx_invoicedate":date, "rcvtrx_invoicenumber":string, "rcvtrx_importdecno":int, "rcvtrx_linenumber":int, "rcvtrx_productdesc":string, "rcvtrx_pchasevalmyr":Decimal, "rcvtrx_gstvalmyr":Decimal, "rcvtrx_taxcode":string, "rcvtrx_fcycode":string, "rcvtrx_pchasefcy":int, "rcvtrx_gstfcy":int, "rcvtrx_artnr":int})
    t_supplier_data, T_supplier = create_model("T_supplier", {"suppid":int, "suppname":string, "suppbrn":int, "suppdategst":date, "suppgstno":int, "suppaddr1":string, "suppaddr2":string, "supptlp":string, "suppfax":string, "suppemail":string, "suppwebsite":string})
    t_customer_data, T_customer = create_model("T_customer", {"custid":int, "custname":string, "custbrn":int, "custinvdate":date, "custinvno":int, "custlineno":int, "custdesc":string, "custsuppvalmyr":Decimal, "custgstvalmyr":Decimal, "custtaxcode":string, "custcountry":string, "custfcycode":string, "custsuppfcy":Decimal, "custgstfcy":Decimal})
    t_gl_trx_data, T_gl_trx = create_model("T_gl_trx", {"gltrx_trxdate":date, "gltrx_accountid":string, "gltrx_accountname":string, "gltrx_trxdesc":string, "gltrx_name":string, "gltrx_trxid":int, "gltrx_srcdocumentid":string, "gltrx_srctype":string, "gltrx_debit":Decimal, "gltrx_credit":Decimal, "gltrx_balance":Decimal})
    t_gl_data, T_gl = create_model("T_gl", {"glid":string, "glaccname":string, "glacctype":int, "gldebit":Decimal, "glcredit":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal done, t_supplier_data, t_customer_data, t_gl_data, t_rcv_trx_data, t_gl_trx_data, a, b, l_artikel, l_lieferant, l_op, gl_acct, gl_jouhdr, gl_fstype, gl_journal
        nonlocal fdate, tdate, sorttype


        nonlocal t_rcv_trx, t_supplier, t_customer, t_gl_trx, t_gl
        nonlocal t_rcv_trx_data, t_supplier_data, t_customer_data, t_gl_trx_data, t_gl_data

        return {"done": done, "t-supplier": t_supplier_data, "t-customer": t_customer_data, "t-gl": t_gl_data, "t-rcv-trx": t_rcv_trx_data, "t-gl-trx": t_gl_trx_data}
    ...
    for l_op.datum, l_op.docu_nr, l_op.pos, l_op.warenwert, l_op._recid, l_artikel.bezeich, l_artikel.lief_artnr, l_artikel.artnr, l_artikel._recid, l_lieferant.lief_nr, l_lieferant.firma, l_lieferant.adresse1, l_lieferant.adresse2, l_lieferant.notizen, l_lieferant._recid in db_session.query(L_op.datum, L_op.docu_nr, L_op.pos, L_op.warenwert, L_op._recid, L_artikel.bezeich, L_artikel.lief_artnr, L_artikel.artnr, L_artikel._recid, L_lieferant.lief_nr, L_lieferant.firma, L_lieferant.adresse1, L_lieferant.adresse2, L_lieferant.notizen, L_lieferant._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr)).join(L_lieferant,(L_lieferant.lief_nr == L_op.lief_nr)).filter(
        ...
        t_supplier_data.append(t_supplier)
        ...
        t_rcv_trx = T_rcv_trx()
        t_rcv_trx_data.append(t_rcv_trx)
    ...
    t_customer_data = get_output(generate_gst_fobillbl(fdate, tdate))

    t_customer = query(t_customer_data, first=True)

    for gl_acct in db_session.query(Gl_acct).order_by(Gl_acct._recid).all():
        t_gl = T_gl()
        t_gl_data.append(t_gl)
