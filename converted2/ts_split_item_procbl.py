...
def ts_split_item_procbl(rec_id:int, split_qty:int, rest:Decimal, foreign_rest:Decimal, curr_qty:int, price_decimal:int, amount:Decimal, foreign_amt:Decimal, qty_sign:int):
    ...
    def generate_output():
        ...
        return {"amount": amount, "foreign_amt": foreign_amt, "qty_sign": qty_sign}
    ...
    for i in range(1,split_qty + 1) :
        ...
        if i == split_qty:
            ...
            h_bill_line.epreis = to_decimal(round(h_bill_line.epreis , price_decimal))
            h_bill_line.nettobetrag =  to_decimal(h_bill_line.epreis) * to_decimal(h_bill_line.anzahl)
            ...
            h_bline.nettobetrag =  to_decimal(h_bline.epreis) * to_decimal(h_bline.anzahl)
            ...
            if h_journal:
                ...
                if price_decimal == 0:
                    buff_h_journal.epreis = to_decimal(round(h_bline.epreis , 0))
            pass
