...
def arl_list_add_keycardbl(pvilanguage:int, recid_resline:int, keyint:int):
    ...
    t_keycard_data, T_keycard = create_model("T_keycard", {"msg_str":string, "rline_betrieb_gast":int, "rline_resstatus":int, "resline_resnr":int, "resline_reslinnr":int, "rline_recid":int})
    ...
    def generate_output():
        nonlocal msg_str, anz0, do_it, card_type, case1, resline_reslinnr, resline_resnr, err_flag, resline_betrieb_gast, resline_resstatus, resline_recid, t_keycard_data, lvcarea, resno, i, maxkey, replaced, res_line, htparam
        nonlocal pvilanguage, recid_resline, keyint
        nonlocal resline, rline


        nonlocal resline, rline, t_keycard
        nonlocal t_keycard_data

        return {"msg_str": msg_str, "anz0": anz0, "do_it": do_it, "card_type": card_type, "case1": case1, "resline_reslinnr": resline_reslinnr, "resline_resnr": resline_resnr, "err_flag": err_flag, "resline_betrieb_gast": resline_betrieb_gast, "resline_resstatus": resline_resstatus, "resline_recid": resline_recid, "t-keycard": t_keycard_data}
    ...
    if res_line.resstatus == 11 or res_line.resstatus == 13:
        ...
        for resline in db_session.query(Resline).filter(
            ...
            t_keycard = T_keycard()
            t_keycard_data.append(t_keycard)
