...
input_payload_data, Input_payload = create_model("Input_payload", {"disp_flag":int, "code":int, "to_date":string})

def bk_stats_sob_segment_detail_webbl(input_payload_data:[Input_payload]):
    ...
    t_list_data, T_list = create_model("T_list", {"code":int, "veran_nr":int, "gastnr":int, "bestellt__durch":string, "dgroom":int, "mgroom":int, "ygroom":int, "drev":Decimal, "mrev":Decimal, "yrev":Decimal, "d_fbrev":Decimal, "m_fbrev":Decimal, "y_fbrev":Decimal, "dorev":Decimal, "morev":Decimal, "yorev":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_list_data, from_date, fb_rev, other_rev, tot_dgroom, tot_mgroom, tot_ygroom, tot_drev, tot_mrev, tot_yrev, tot_d_fbrev, tot_m_fbrev, tot_y_fbrev, tot_dorev, tot_morev, tot_yorev, bk_veran, bk_func, artikel, bk_rart
    ...
    def create_list():
        ...
        for bk_func.bestellt__durch, bk_func.veran_nr, bk_func.veran_seite, bk_func.rpreis, bk_func.rpersonen, bk_func.technik, bk_func.datum, bk_func._recid, bk_veran.veran_nr, bk_veran.segmentcode, bk_veran.gastnr, bk_veran._recid in db_session.query(Bk_func.bestellt__durch, Bk_func.veran_nr, Bk_func.veran_seite, Bk_func.rpreis, Bk_func.rpersonen, Bk_func.technik, Bk_func.datum, Bk_func._recid, Bk_veran.veran_nr, Bk_veran.segmentcode, Bk_veran.gastnr, Bk_veran._recid).join(Bk_veran,(Bk_veran.veran_nr == Bk_func.veran_nr) & (Bk_veran.segmentcode == input_payload.code)).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.veran_nr == bk_veran.veran_nr), first=True)
    ...
    def create_list1():
        ...
        for bk_func in db_session.query(Bk_func).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.veran_nr == bk_func.veran_nr), first=True)
    ...
    input_payload = query(input_payload_data, first=True)
