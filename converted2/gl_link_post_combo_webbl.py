...
def gl_link_post_combo_webbl(g_list_data:[G_list], ar_glist_data:[Ar_glist], trans_dept_data:[Trans_dept], language_code:int, vkey:string, vmodule:string, combo_gastnr:int, user_init:string, pf_file1:string, pf_file2:string, from_date:date, to_date:date, refno:string, bezeich:string, curr_anz1:int, debit1:Decimal, credit1:Decimal, merge_flag:bool, g2_list_data:[G2_list], ar_g2list_data:[Ar_g2list], debits:Decimal, credits:Decimal, remains:Decimal):
    ...
    combo_glist_data, Combo_glist = create_model("Combo_glist", {"flag":int, "datum":date, "artnr":int, "dept":int, "jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "duplicate":bool, "acct_fibukonto":string, "bezeich":string}, {"sysdate": get_current_date(), "chgdate": None, "duplicate": True})
    combo_ar_glist_data, Combo_ar_glist = create_model("Combo_ar_glist", {"rechnr":int, "dept":int, "jnr":int, "fibukonto":string, "debit":Decimal, "credit":Decimal, "bemerk":string, "userinit":string, "sysdate":date, "zeit":int, "chginit":string, "chgdate":date, "duplicate":bool, "add_info":string, "counter":int, "acct_fibukonto":string, "bezeich":string}, {"sysdate": get_current_date(), "chgdate": None, "duplicate": True})
    t_htparam_data, T_htparam = create_model_like(Htparam)
    s2_list_data, S2_list = create_model("S2_list", {"fibukonto":string, "bezeich":string, "credit":Decimal, "debit":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal frame_title2, last_acctdate, curr_anz, created, message_result, s2_list_data, connect_param, connect_paramssl, lreturn, htl_name, debit2, credit2, curr_anz2, remain2, acct_error, art_dpt, art_artnr, art_bezeich, success_flag, err_flag, htparam, gl_acct
        nonlocal language_code, vkey, vmodule, combo_gastnr, user_init, pf_file1, pf_file2, from_date, to_date, refno, bezeich, curr_anz1, debit1, credit1, merge_flag, g2_list_data, ar_g2list_data, debits, credits, remains


        nonlocal g_list, g2_list, combo_glist, ar_glist, ar_g2list, combo_ar_glist, trans_dept, t_htparam, s2_list
        nonlocal combo_glist_data, combo_ar_glist_data, t_htparam_data, s2_list_data

        return {"g2-list": g2_list_data, "ar-g2list": ar_g2list_data, "debits": debits, "credits": credits, "remains": remains, "frame_title2": frame_title2, "last_acctdate": last_acctdate, "curr_anz": curr_anz, "created": created, "message_result": message_result, "s2-list": s2_list_data}
    ...
    if vmodule.lower()  == ("vhpIA").lower() :
        ...
        if vkey.lower()  == ("createJournalListIA").lower() :
            ...
            local_storage.combo_flag = True
            curr_anz2, debit2, credit2, acct_error, remain2, art_dpt, art_artnr, art_bezeich, g2_list_data = get_output(gl_linkfobl(trans_dept_data, from_date, to_date, user_init, refno, curr_anz2))
            local_storage.combo_flag = False
            ...
            combo_glist_data.clear()

            for g_list in query(g_list_data):
                combo_glist = Combo_glist()
                combo_glist_data.append(combo_glist)

                buffer_copy(g_list, combo_glist)

            for g2_list in query(g2_list_data):
                combo_glist = Combo_glist()
                combo_glist_data.append(combo_glist)

                buffer_copy(g2_list, combo_glist)

            for combo_glist in query(combo_glist_data):
            ...
            if err_flag:
                ...
                return generate_output()
            get_output(gl_linkfo_updatebl(language_code, remains, credits, debits, to_date, refno, bezeich, combo_glist_data))
            connect_param = "-H " + entry(0, pf_file2, ":") + " -S " + entry(1, pf_file2, ":") + " -DirectConnect -sessionModel Session-free"
            connect_paramssl = connect_param + " -ssl -nohostverify"
            lreturn = set_combo_session(connect_paramssl, None, None, None)
            ...
            local_storage.combo_flag = True
            success_flag = get_output(write_htparambl(2, t_htparam_data))
            local_storage.combo_flag = False
        ...
        if vkey.lower()  == ("createJournalListAR").lower() :
            ...
            local_storage.combo_flag = True
            curr_anz2, acct_error, debit2, credit2, remain2, ar_g2list_data, s2_list_data, art_artnr, art_bezeich = get_output(gl_linkarbl(merge_flag, from_date, to_date, user_init, refno, curr_anz2))
            local_storage.combo_flag = False
            ...
            combo_ar_glist_data.clear()

            for ar_glist in query(ar_glist_data):
                combo_ar_glist = Combo_ar_glist()
                combo_ar_glist_data.append(combo_ar_glist)
            ...
            if err_flag:
                ...
                return generate_output()
            get_output(gl_linkar_updatebl(language_code, remains, credits, debits, to_date, refno, bezeich, to_date, combo_ar_glist_data))
            connect_param = "-H " + entry(0, pf_file2, ":") + " -S " + entry(1, pf_file2, ":") + " -DirectConnect -sessionModel Session-free"
            connect_paramssl = connect_param + " -ssl -nohostverify"
            lreturn = set_combo_session(connect_paramssl, None, None, None)
