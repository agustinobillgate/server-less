...
def bartikel_list_webbl(from_cr:string, to_cr:string, curr_date:date):
    ...
    output_list_data, Output_list = create_model("Output_list", {"from_time":string, "to_time":string, "room":string, "rsv_no":int, "bezeich":string, "qty":int, "price":Decimal, "ba_status":string, "id":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal from_i, to_i, output_list_data, bk_reser, bk_rart, bediener
        ...
        return {"from_i": from_i, "to_i": to_i, "output-list": output_list_data}
