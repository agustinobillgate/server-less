...
def prepare_vip_listbl(user_init:str):
    ...
    def fill_vipnr():
        ...
        if not htparam or not(htparam.paramnr == 712):
            htparam = db_session.query(Htparam).filter(
                (Htparam.paramnr == 712)).first()
        t_vipnr.vip_nr10 = htparam.finteger


    if not htparam or not(htparam.paramnr == 417):
        htparam = db_session.query(Htparam).filter(
    ...
    if htparam.fchar != "":
        ...
        if substring(lnl_filepath, len(lnl_filepath) - 1, 1) != ("\\").lower() :
            lnl_filepath = lnl_filepath + "\\"

    if not htparam or not(htparam.paramnr == 297):
        htparam = db_session.query(Htparam).filter(
