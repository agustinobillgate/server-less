...
def po_delivernotebl(docu_nr:string):
    delivernote_list_data = []
    l_ophdr = None

    delivernote_list = None

    delivernote_list_data, Delivernote_list = create_model("Delivernote_list", {"datum":date, "lager_nr":int, "docu_nr":string, "lscheinnr":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal delivernote_list_data, l_ophdr
        nonlocal docu_nr


        nonlocal delivernote_list
        nonlocal delivernote_list_data

        return {"delivernote-list": delivernote_list_data}
    ...
    for l_ophdr in db_session.query(L_ophdr).filter(
        ...
        delivernote_list_data.append(delivernote_list)
