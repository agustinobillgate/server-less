...
def prepare_eg_merge_item_webbl(item_nr:int):
    ...
    merge_list_data, Merge_list = create_model_like(Eg_property, {"object_nm":string, "loc_nm":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal merge_list_data, object_no, loc_no, room_no, eg_property, eg_location, queasy
        nonlocal item_nr


        nonlocal merge_list
        nonlocal merge_list_data

        return {"merge-list": merge_list_data}
    ...
    for eg_property in db_session.query(Eg_property).filter(
        ...
        merge_list_data.append(merge_list)
