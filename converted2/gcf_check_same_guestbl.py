...
def gcf_check_same_guestbl(casetype:int, gastno:int, email_adr:string, mobil_phone:string):
    ...
    def generate_output():
        nonlocal same_guest, tlist_data, gphone, guest
        ...
        return {"same_guest": same_guest, "tlist": tlist_data}
        ...
        if guest:
            ...
            if guest.email_adr != " ":
                ...
                for bguest in db_session.query(Bguest).filter(
                    ...
                    tlist = query(tlist_data, filters=(lambda tlist: tlist.gastno == bguest.gastnr), first=True)
