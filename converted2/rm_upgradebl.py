...
def rm_upgradebl(datum:date, to_date:date):
    ...
    str_list_data, Str_list = create_model_like(Res_line, {"gname":string, "company":string, "rmtype":string, "currency":string, "cat":string, "id":string})
    ...
    def generate_output():
        nonlocal str_list_data, new_contrate, curr_date, ci_date, from_date, ct, curr_i, origzikatnr, origrmtype, res_line, guest, htparam, genstat, zimkateg, waehrung, reservation
    ...
    def upgrade1():
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            str_list = query(str_list_data, filters=(lambda str_list: str_list.resnr == genstat.resnr and str_list.reslinnr == genstat.res_int[0] and str_list.zikatnr == genstat.zikatnr and str_list.rmtype == origrmtype), first=True)
    ...
    def upgrade2():
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            str_list = query(str_list_data, filters=(lambda str_list: str_list.resnr == res_line.resnr and str_list.reslinnr == res_line.reslinnr and str_list.zikatnr == res_line.zikatnr and str_list.rmtype == origrmtype), first=True)
