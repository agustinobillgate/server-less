...
def compli_glist_1bl(pvilanguage:int, from_date:date, to_date:date, sorttype:int, bonus_flag:int, segmentcode:int):
    ...
    complig_list_data, Complig_list = create_model("Complig_list", {"resnr":int, "reslinnr":int, "datum":date, "zinr":string, "reserve_nm":string, "name":string, "zimmeranz":int, "pax":int, "ankunft":date, "abreise":date, "segm":string, "rstatus":string, "remark":string, "rname":string, "fsort":int, "fdatum":date, "vip":string, "nat":string, "rmtype":string, "rm_rate":Decimal, "rate_code":string, "argt":string, "bill_detail":string, "usr_id":string, "gratis":int})
    cl_list_data, Cl_list = create_model("Cl_list", {"segm":int, "bezeich":string, "zimmeranz":int, "pax":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal complig_list_data, do_it, last_sort, int_sort, t_anz, t_pax, tot_anz, tot_pax, ci_date, datum, new_contrate, bonus_array, vip_nr, lvcarea, htparam, segment, guest, genstat, zimkateg, reservation, res_line, guestseg, guest_pr, arrangement, reslin_queasy, queasy
        nonlocal pvilanguage, from_date, to_date, sorttype, bonus_flag, segmentcode


        nonlocal complig_list, cl_list
        nonlocal complig_list_data, cl_list_data

        return {"complig-list": complig_list_data}
    ...
    def create_list2():
        ...
        cl_list = query(cl_list_data, filters=(lambda cl_list: cl_list.segm == genstat.segmentcode), first=True)
    ...
    def create_list1():
        ...
        nonlocal complig_list_data, last_sort, int_sort, t_anz, t_pax, tot_anz, tot_pax, ci_date, datum, new_contrate, bonus_array, vip_nr, lvcarea, htparam, segment, guest, genstat, zimkateg, reservation, res_line, guestseg, guest_pr, arrangement, reslin_queasy, queasy
        nonlocal pvilanguage, from_date, to_date, sorttype, bonus_flag, segmentcode
