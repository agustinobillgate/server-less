...
def rate_code_changes_webbl(from_date:date, to_date:date):
    ...
    tt_rate_changes_data, Tt_rate_changes = create_model("Tt_rate_changes", {"dated1":date, "dated2":date, "timed":int, "logi":bool, "rate_before":string, "rate_after":string, "amount_after":Decimal, "amount_before":Decimal, "room_category":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal tt_rate_changes_data, delimiterd, char_data, v_rate_before, v_rate_after, v_amount_before, v_amount_after, v_room_category, reslin_queasy
        nonlocal from_date, to_date


        nonlocal tt_rate_changes
        nonlocal tt_rate_changes_data

        return {"tt-rate-changes": tt_rate_changes_data}


    tt_rate_changes_data.clear()
    ...
    for reslin_queasy in db_session.query(Reslin_queasy).filter(
        ...
        if num_entries(char_data, delimiterd) >= 5:
            ...
            tt_rate_changes_data.append(tt_rate_changes)
