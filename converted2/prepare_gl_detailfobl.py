...
def prepare_gl_detailfobl(pvilanguage:int, fibu:string, bemerk:string, from_date:date):
    ...
    s_list_data, S_list = create_model("S_list", {"datum":date, "departement":int, "artnr":int, "artart":int, "bezeich":string, "betrag":Decimal, "service":Decimal, "vat":Decimal, "nett":Decimal})
    t_gl_acct_data, T_gl_acct = create_model_like(Gl_acct)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_gl_acct_data, s_list_data, artnr, dept, lvcarea, gl_acct, artikel, umsatz, billjournal, htparam, bill, billhis, reservation
