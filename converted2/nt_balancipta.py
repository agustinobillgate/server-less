...
from models import Htparam, Paramtext, Nightaudit, Nitestor, Bill, Bill_line, Guest, Res_line, Fixleist

def nt_balancipta():
    long_digit:bool = False
    n:int = 0
    progname:str = "nt-balanCIPTA.p"
    night_type:int = 0
    reihenfolge:int = 0
    line_nr:int = 0
    line:str = ""
    ...
    htl_tel:str = ""
    curr_date:date = None
    htparam = paramtext = nightaudit = nitestor = bill = bill_line = guest = res_line = fixleist = None

    output_list = cl_list = None

    output_list_list, Output_list = create_model("Output_list", {"str":str})
    cl_list_list, Cl_list = create_model("Cl_list", {"flag":str, "zinr":str, "zipreis":decimal, "rechnr":int, "receiver":str, "ankunft":str, "abreise":str, "saldo":decimal, "discount":decimal, "name":str})


    db_session = local_storage.db_session

    def generate_output():
        nonlocal long_digit, n, progname, night_type, reihenfolge, line_nr, line, p_width, p_length, htl_name, htl_adr, htl_tel, curr_date, htparam, paramtext, nightaudit, nitestor, bill, bill_line, guest, res_line, fixleist
    ...
    def balance_list():
        ...
        add_line(line)
        add_line(" ")
        line = "** " + "RmNo RoomRate BillNo GuestName Arrival Depart BillBalance Discount BillReceiver"
        add_line(line)
        line = ""
        for i in range(1,129 + 1) :
            line = line + "-"
        ...
        for cl_list in query(cl_list_list):
            ...
            if cl_list.flag.lower()  == ("**").lower() :
                ...
                if not long_digit:
                    ...
                    line = line + "T O T A L " + to_string(cl_list.saldo, " ->,>>>,>>>,>>>,>>9 ") + to_string(cl_list.discount, "->>,>>>,>>9.99")
                add_line(line)
    ...
    def create_billbalance():
        ...
        curr_disc:decimal = to_decimal("0.0")
        ...
        for bill in db_session.query(Bill).filter(
            ...
            for bill_line in db_session.query(Bill_line).filter(
                     (Bill_line.rechnr == bill.rechnr) & (Bill_line.bill_datum > curr_date)).order_by(Bill_line._recid).all():
                g_saldo =  to_decimal(g_saldo) - to_decimal(bill_line.betrag)
            curr_disc =  to_decimal("0")

            if res_line.resstatus != 12:

                fixleist = db_session.query(Fixleist).filter(
                         (Fixleist.resnr == res_line.resnr) & (Fixleist.reslinnr == res_line.reslinnr) & (Fixleist.artnr == 0) & (Fixleist.departement == 0) & (Fixleist.sequenz == 1)).first()

                if not fixleist:

                    fixleist = db_session.query(Fixleist).filter(
                             (Fixleist.resnr == res_line.resnr) & (Fixleist.reslinnr == res_line.reslinnr) & (Fixleist.artnr == 0) & (Fixleist.departement == 0) & (Fixleist.sequenz == 6) & (Fixleist.lfakt <= curr_date) & ((Fixleist.lfakt + Fixleist.dekade) >= curr_date)).first()

                if fixleist:
                    curr_disc =  - to_decimal(fixleist.betrag)

            if (g_saldo != 0) or (curr_disc != 0):
                cl_list = Cl_list()
                cl_list_list.append(cl_list)
                ...
                cl_list.rechnr = bill.rechnr
                cl_list.saldo =  to_decimal(g_saldo)
                cl_list.discount =  to_decimal(curr_disc)
                tot_val =  to_decimal(tot_val) + to_decimal(g_saldo)
                tot_disc =  to_decimal(tot_disc) + to_decimal(curr_disc)
        cl_list = Cl_list()
        cl_list_list.append(cl_list)
        ...
        cl_list.name = "T O T A L"
        cl_list.saldo =  to_decimal(tot_val)
        cl_list.discount =  to_decimal(tot_disc)
        total_val =  to_decimal(total_val) + to_decimal(tot_val)
        cl_list = Cl_list()
        cl_list_list.append(cl_list)
