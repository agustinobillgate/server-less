...
def fobill_vatlistbl(pvilanguage:int, billno:int):
    ...
    def generate_output():
        nonlocal bline_vatlist_data, exchg_rate, price_decimal, curr_billdate, lvcarea, htparam, bill_line, artikel, bill, res_line, arrangement, argt_line, billjournal, waehrung, exrate
        nonlocal pvilanguage, billno


        nonlocal bline_vatlist
        nonlocal bline_vatlist_data

        return {"bline-vatlist": bline_vatlist_data}
    ...
    def calc_servtaxes(artno:int, deptno:int, betrag:Decimal, billdate:date):
        ...
        if vat != 0:
            ...
            bline_vatlist = query(bline_vatlist_data, filters=(lambda bline_vatlist: bline_vatlist.vatnr == artbuff.mwst_code), first=True)

            if not bline_vatlist:
                bline_vatlist = Bline_vatlist()
                bline_vatlist_data.append(bline_vatlist)
        ...
        if tax != 0:
            ...
            bline_vatlist = query(bline_vatlist_data, filters=(lambda bline_vatlist: bline_vatlist.vatnr == artbuff.prov_code), first=True)
        ...
        if serv != 0:
            ...
            bline_vatlist = query(bline_vatlist_data, filters=(lambda bline_vatlist: bline_vatlist.vatnr == artbuff.service_code), first=True)
        ...
        bline_vatlist = query(bline_vatlist_data, filters=(lambda bline_vatlist: bline_vatlist.seqnr == 1), first=True)
        ...
        bline_vatlist = query(bline_vatlist_data, filters=(lambda bline_vatlist: bline_vatlist.seqnr == 2), first=True)
