...
def activities_history_webbl(usrid:string, action:string, from_date:date, to_date:date, idflag:string):
    ...
    b1_list_data, B1_list = create_model("B1_list", {"datum":date, "action":string, "aenderung":string, "username":string, "zeit":int})
    ...
    def generate_output():
        nonlocal b1_list_data, counter, bediener, queasy, res_history
    ...
    while None != b1_list:
        ...
        b1_list = query(b1_list_data, next=True)
