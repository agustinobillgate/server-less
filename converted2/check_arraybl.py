...
def check_arraybl(rec_id:int):
    ...
    t_pricecod_data, T_pricecod = create_model_like(Pricecod)
    t_ratecode_data, T_ratecode = create_model_like(Ratecode)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal margt_list_data, hargt_list_data, hrmcat_list_data, mrmcat_list_data, t_pricecod_data, t_ratecode_data, prmarket, pricecod, ratecode, prtable, zimkateg, arrangement
        nonlocal rec_id


        nonlocal margt_list, hargt_list, hrmcat_list, mrmcat_list, t_pricecod, t_ratecode
        nonlocal margt_list_data, hargt_list_data, hrmcat_list_data, mrmcat_list_data, t_pricecod_data, t_ratecode_data

        return {"margt-list": margt_list_data, "hargt-list": hargt_list_data, "hrmcat-list": hrmcat_list_data, "mrmcat-list": mrmcat_list_data, "t-pricecod": t_pricecod_data, "t-ratecode": t_ratecode_data}
    ...
    for pricecod in db_session.query(Pricecod).filter(
        ...
        t_pricecod_data.append(t_pricecod)
