...
def odx_trans_logbl(art_dept:int, from_date:date, to_date:date):
    ...
    revenue_list_data, Revenue_list = create_model("Revenue_list", {"payment_type":string, "amount":Decimal, "accounttype":string, "accountname":string, "comment":string, "bill_datum":date, "revenue_recid":int, "bill_number":int, "send_flag":bool})
    ...
    def generate_output():
        nonlocal revenue_list_data, article_list, loop_i, messtoken, getartname, getartno, bill_date, queasy, h_journal
        nonlocal art_dept, from_date, to_date
        nonlocal buffqueasy


        nonlocal revenue_list, art_list, buffqueasy
        nonlocal revenue_list_data, art_list_data
