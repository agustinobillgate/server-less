...
def res_vouchernobl(case_type:int, voucherno:string, fname:string, tname:string):
    ...
    def generate_output():
        nonlocal t_res_voucherno_data, t_part_resline_data, reservation, res_line
        nonlocal case_type, voucherno, fname, tname


        nonlocal t_res_voucherno, t_part_resline
        nonlocal t_res_voucherno_data, t_part_resline_data

        return {"t-res-voucherno": t_res_voucherno_data, "t-part-resline": t_part_resline_data}
    ...
    if case_type == 1:
        ...
        for reservation in db_session.query(Reservation).filter(
            ...
            t_res_voucherno_data.append(t_res_voucherno)
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            t_part_resline_data.append(t_part_resline)
