...
from functions.view_staycost_cldbl import view_staycost_cldbl
from models import Htparam, Res_line, Guest_pr, Zimkateg, Arrangement, Waehrung, Queasy

def prepare_view_staycost_1bl(pvilanguage:int, resnr:int, reslinnr:int):

    prepare_cache ([Htparam, Res_line, Guest_pr, Zimkateg, Arrangement, Waehrung, Queasy])
    ...
    htparam = res_line = guest_pr = zimkateg = arrangement = waehrung = queasy = None

    output_list = t_res_line = None

    output_list_data, Output_list = create_model("Output_list", {"flag":int, "str":string, "str1":string})
    t_res_line_data, T_res_line = create_model("T_res_line", {"name":string, "zinr":string, "ankunft":date, "abreise":date})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal ci_date, contcode, ct, curr_rmcat, t_str, str_arrangement, kurzbez, repeat_charge, t_res_line_data, output_list_data, new_contrate, lvcarea, bonus_array, wd_array, htparam, res_line, guest_pr, zimkateg, arrangement, waehrung, queasy
        nonlocal pvilanguage, resnr, reslinnr


        nonlocal output_list, t_res_line
        nonlocal output_list_data, t_res_line_data

        return {"ci_date": ci_date, "contcode": contcode, "ct": ct, "curr_rmcat": curr_rmcat, "t_str": t_str, "str_arrangement": str_arrangement, "kurzbez": kurzbez, "repeat_charge": repeat_charge, "t-res-line": t_res_line_data, "output-list": output_list_data}
    ...
    output_list_data = get_output(view_staycost_cldbl(pvilanguage, resnr, reslinnr, contcode, repeat_charge))
