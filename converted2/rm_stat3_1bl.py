...
def rm_stat3_1bl(segmno:int, gastno:int, fr_date:date, to_date:date):
    ...
    t_list_data, T_list = create_model("T_list", {"gastnr":int, "gname":string, "nat":string, "zinr":string, "erwachs":int, "kind1":int, "gratis":int, "zipreis":Decimal, "curr":string, "resnr":int, "reslinnr":int, "ankunft":date, "abreise":date, "dlodge":Decimal, "drate":Decimal}, {"ankunft": None, "abreise": None})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_list_data, ci_date, htparam, genstat, guest, waehrung, reservation, res_line
    ...
    def create_list(f_date:date, t_date:date):
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.resnr == genstat.resnr and t_list.reslinnr == genstat.res_int[0]), first=True)
    ...
    def create_list2(f_date:date, t_date:date):
        ...
        for res_line, reservation in db_session.query(Res_line, Reservation).join(Reservation,(Reservation.resnr == Res_line.resnr) & (Reservation.segmentcode == segmno)).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.resnr == res_line.resnr and t_list.reslinnr == res_line.reslinnr), first=True)
