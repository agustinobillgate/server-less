#using conversion tools version: 1.0.0.92
...
from models import Res_line, Kontline, Zimmer, Guest, Zimkateg, Waehrung, Segment, Genstat, Fixleist, Bill_line, Argt_line, Reslin_queasy, Queasy, Outorder, Zkstat, Artikel, Umsatz, Htparam, Arrangement, Exrate, Reservation, Guestseg, Zinrstat
...
def cr_occfcast1_2_webbl_test(segm_list_list:[Segm_list], argt_list_list:[Argt_list], zikat_list_list:[Zikat_list], outlook_list_list:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):
    ...
    curr_time:int = 0
    tmpint:int = 0
    res_line = kontline = zimmer = guest = zimkateg = waehrung = segment = genstat = fixleist = bill_line = argt_line = reslin_queasy = queasy = outorder = zkstat = artikel = umsatz = htparam = arrangement = exrate = reservation = guestseg = zinrstat = None
    ...
    resline_query = text("""
            ...
            FROM res_line
            WHERE resstatus <= 13
                AND resstatus NOT IN (4, 9, 10, 12)
                AND (
    ...
    db_session.execute(resline_query, {"ci_date": ci_date - timedelta(days=30), "to_date": to_date})
                                ...
                                if arrangement:
                                    ...
                                    for argt_line.argt_artnr, argt_line.departement, argt_line.fakt_modus, argt_line.intervall, argt_line.betriebsnr, argt_line.argtnr, argt_line.betrag, argt_line._recid in db_session.query(Argt_line.argt_artnr, Argt_line.departement, Argt_line.fakt_modus, Argt_line.intervall, Argt_line.betriebsnr, Argt_line.argtnr, Argt_line.betrag, Argt_line._recid).filter(
                                        ...
                                        if post_it:
                                            ...
                                            reslin_queasy = get_cache (Reslin_queasy, {"key": "fargt-line", "char1": "", "number1": argt_line.departement, "number2": argt_line.argtnr, "resnr": res_line.resnr, "reslinnr": res_line.reslinnr, "number3": argt_line.argt_artnr, "curr_date": (ge, reslin_queasy.date1), "curr_date": (le, reslin_queasy.date2)}, ['number3', 'deci1', 'deci2', 'deci3', '_recid'])
    ...
    def check_fixleist_posted(curr_date:date, artnr:int, dept:int, fakt_modus:int, intervall:int, lfakt:date):
        ...
        if fakt_modus == 1:
            ...
            tmpint1 = (res_line.abreise - start_date).days
