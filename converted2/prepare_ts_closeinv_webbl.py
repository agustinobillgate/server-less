...
def prepare_ts_closeinv_webbl(pvilanguage:int, curr_dept:int, inp_rechnr:int, user_init:string, user_name:string, curr_printer:int):

    prepare_cache ([H_bill_line, H_artikel, Guest, Htparam, Waehrung, Artikel, Hoteldpt, Kellner, Tisch, Exrate])
    ...
    b_list = t_b_list = t_h_bill = t_h_artikel = ordered_item = summary_bill = bill_guest = buff_hart = None

    b_list_data, B_list = create_model_like(H_bill_line)
    t_b_list_data, T_b_list = create_model_like(B_list)
