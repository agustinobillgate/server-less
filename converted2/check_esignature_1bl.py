...
def check_esignature_1bl(case_type:int, user_init:string, docu_nr:string, flag_type:string):
    ...
    esign_list_data, Esign_list = create_model("Esign_list", {"sign_nr":int, "sign_name":string, "sign_img":bytes, "sign_use_for":string, "sign_position":string, "sign_userinit":string, "sign_id":int, "sign_select":bool, "sign_pass":string, "sign_pr":bool, "sign_po":bool})
    esign_print_data, Esign_print = create_model("Esign_print", {"sign_nr":int, "sign_name":string, "sign_img":bytes, "sign_date":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal found_flag, esign_list_data, esign_print_data, guestbook, queasy
        nonlocal case_type, user_init, docu_nr, flag_type


        nonlocal esign_list, esign_print
        nonlocal esign_list_data, esign_print_data
