...
def prepare_supply_umsatzbl(lief_nr:int):
    ...
    umsatz_list_data, Umsatz_list = create_model("Umsatz_list", {"lief_nr":int, "datum":date, "gesamtumsatz":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal umsatz_list_data, l_liefumsatz
        nonlocal lief_nr


        nonlocal umsatz_list
        nonlocal umsatz_list_data

        return {"umsatz-list": umsatz_list_data}
