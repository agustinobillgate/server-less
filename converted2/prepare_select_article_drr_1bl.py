...
def prepare_select_article_drr_1bl(case_type:int, departement:int):
    ...
    str_list_data, Str_list = create_model("Str_list", {"nr":int, "bezeich":string, "used":bool, "grup":bool, "grp_name":string})
    stat_list_data, Stat_list = create_model("Stat_list", {"artnr":int, "used":bool, "grup":bool, "grp_name":string, "flag":string})
    out_list_data, Out_list = create_model_like(Stat_list, {"deptnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal c_862, c_892, str2, str3, st1, st2, st3, st4, st5, st6, st7, st8, st9, st10, st11, st12, n, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, dept, str_list_data, htparam, queasy, artikel, segment
        nonlocal case_type, departement


        nonlocal str_list, stat_list, out_list
        nonlocal str_list_data, stat_list_data, out_list_data
    ...
    if case_type == 1:
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.artnr == str_list.nr and stat_list.flag.lower()  == ("FoRev").lower()), first=True)
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.artnr == str_list.nr and stat_list.flag.lower()  == ("OtherRev").lower()), first=True)
        ...
        for segment in db_session.query(Segment).order_by(Segment._recid).all():
            ...
            stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.artnr == str_list.nr and stat_list.flag.lower()  == ("Segment").lower()), first=True)
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            out_list = query(out_list_data, filters=(lambda out_list: out_list.artnr == str_list.nr and out_list.deptnr == departement), first=True)
