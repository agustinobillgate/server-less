...
def storereq_list_create_list_1bl(from_date:date, to_date:date, from_dept:int, to_dept:int, curr_lschein:string, show_price:bool):

    prepare_cache ([Htparam, L_lager, L_artikel, L_op, L_ophdr, Parameters, Gl_acct, Bediener])
    ...
    def generate_output():
        nonlocal it_exist, t_list_data, appflag, long_digit, htparam, l_lager, l_artikel, l_op, l_ophdr, parameters, gl_acct, bediener
    ...
    def create_list():
        ...
        nonlocal it_exist, t_list_data, long_digit, htparam, l_lager, l_artikel, l_op, l_ophdr, parameters, gl_acct, bediener
        nonlocal from_date, to_date, from_dept, to_dept, curr_lschein, show_price
        ...
        l_op = L_op()
        l_artikel = L_artikel()
        for l_op.lscheinnr, l_op.lager_nr, l_op.pos, l_op.reorgflag, l_op._recid, l_op.herkunftflag, l_op.datum, l_op.anzahl, l_op.deci1, l_op.einzelpreis, l_op.warenwert, l_op.artnr, l_op.stornogrund, l_op.fuellflag, l_op.op_art, l_artikel.bezeich, l_artikel.masseinheit, l_artikel.inhalt, l_artikel.lief_einheit, l_artikel.traubensorte, l_artikel.zwkum, l_artikel.endkum, l_artikel._recid in db_session.query(L_op.lscheinnr, L_op.lager_nr, L_op.pos, L_op.reorgflag, L_op._recid, L_op.herkunftflag, L_op.datum, L_op.anzahl, L_op.deci1, L_op.einzelpreis, L_op.warenwert, L_op.artnr, L_op.stornogrund, L_op.fuellflag, L_op.op_art, L_artikel.bezeich, L_artikel.masseinheit, L_artikel.inhalt, L_artikel.lief_einheit, L_artikel.traubensorte, L_artikel.zwkum, L_artikel.endkum, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
