...
def telop_webbl(sorttype:int, room:str, fdate1:date, fdate2:date, ci_date:date, lname:str, last_sort:int, lnat:str, lresnr:int):
    ...
    def disp_arl1():
        ...
        if last_sort == 1:
            if lname == "":
                ...
                local_storage.debugging = local_storage.debugging + ", Recs:" + str(len(recs))
                print("recs1:", recs)
                ...
                print("lname, 1.2.1", lname, fdate1, fdate2)
                res_line_obj_list = []
                recs = (
                    db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember)
                    .join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr))
                ...
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        (Res_line.active_flag == 0) &  (Res_line.ankunft >= fdate1) &  (Res_line.ankunft <= fdate2) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*")))).order_by(Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
            ...
            if lname == "":
                ...
                recs = (
                    ...
                    .filter(
                        ...
                        (Res_line.ankunft >= fdate1) &  
                        (Res_line.ankunft <= fdate2))
                    .order_by(Reservation.name, Res_line.ankunft, Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name)
                    .all()
                )
                local_storage.debugging = local_storage.debugging + ", Recs:" + str(len(recs))
                print("recs2:", recs)
    ...
    def disp_arl2():
        ...
        if last_sort == 1:
            rmlen_int = int(rmlen) if rmlen is not None else 0
            local_storage.debugging = local_storage.debugging + ", disparl2, lsort1, rmlen:" + str(len(room))
            if room != "":
                local_storage.debugging = local_storage.debugging + ",312" + ", Fdate:" + str(fdate1)
                if lname == "":
                    local_storage.debugging = local_storage.debugging + ",314"
                    res_line_obj_list = []
                    recs = (
                    db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember)
                        .join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr))
                    ...
                    local_storage.debugging = local_storage.debugging + ",326 nRec:" + str(len(recs)) 
                    for res_line, waehrung, reservation, zimkateg, gmember in recs:
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                    ...
                    local_storage.debugging = local_storage.debugging + ",335"
                    res_line_obj_list = []
                    lname_regex = lname.lower().replace("*", ".*") if lname else ""
                    filter_conditions = [
                        (Res_line.active_flag == 1),
                        (Res_line.resstatus != 12),
                        (func.substring(Res_line.zinr, 0, rmlen_int) >= (room).lower()),
                        (func.lower(Res_line.name).op("~")(lname_regex)),
                        (func.lower(Res_line.name) <= (to_name).lower()),
                    ]
                    if ci_date is not None:
                        filter_conditions.append(
                            or_(
                                Res_line.ankunft == None,
                                Res_line.ankunft <= ci_date
                            )
                        )
                        filter_conditions.append(
                            or_(
                                Res_line.abreise == None,
                                Res_line.abreise >= ci_date
                            )
                        )
                    recs = (
                        db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).\
                        join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).\
                        join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).\
                        filter(*filter_conditions).\
                        order_by(Res_line.zinr, 
                                    (Res_line.kontakt_nr * Res_line.resnr), 
                                    Res_line.resstatus, 
                                    Res_line.name).all()
                    )
                    for res_line, waehrung, reservation, zimkateg, gmember in recs:
                    ...
                    local_storage.debugging = local_storage.debugging + ",374"
                    res_line_obj_list = []
                    rmlen_int = int(rmlen) if rmlen is not None else 0
                    lname_regex = lname.lower().replace("*", ".*") if lname else ""
                    # filter_conditions = [(Res_line.active_flag == 1)]
                    filter_conditions = [
                                            (Res_line.active_flag == 1),
                                            (Res_line.resstatus != 12),
                                            or_(
                    ...
                    for condition in filter_conditions:
                        print(str(condition))
                    print("Filter 401:", filter_conditions)
                    recs = db_session.query(
                                Res_line, Waehrung, Reservation, Zimkateg, Gmember
                            ).join(
                                Waehrung, Waehrung.waehrungsnr == Res_line.betriebsnr
                            ).join(
                                Reservation, Reservation.resnr == Res_line.resnr
                            ).join(
                                Zimkateg, Zimkateg.zikatnr == Res_line.zikatnr
                            ).join(
                                Gmember, Gmember.gastnr == Res_line.gastnrmember
                            ).filter(
                                *filter_conditions
                            ).order_by(
                                Res_line.zinr,
                ...
                local_storage.debugging = local_storage.debugging + ",425"
                if lname == "":
                    print("lewat 426")
                    local_storage.debugging = local_storage.debugging + ",246"
                    res_line_obj_list = []
                    recs = (
                        db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember)
                        .join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr))
                    ...
                    print("lewat 453")
                    local_storage.debugging = local_storage.debugging + ",453"
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            (Res_line.active_flag == 1) &  
                            (Res_line.resstatus != 12) &  
                            ...
                            (Res_line.ankunft <= ci_date) &  
                            (Res_line.abreise >= ci_date)).order_by(Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
                    ...
                    print("lewat 470")
                    local_storage.debugging = local_storage.debugging + ",473"
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            (Res_line.active_flag == 1) &  
                            (Res_line.resstatus != 12) &  
            ...
            if lname == "" and room == "":
                ...
                recs = (
                    ...
                    .filter(
                        ...
                        (Res_line.ankunft <= ci_date) &  
                        (Res_line.abreise >= ci_date))
                        .order_by(Res_line.resname, Res_line.resnr, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all()
                )
                for res_line, waehrung, reservation, zimkateg, gmember in recs:
                    if res_line._recid in res_line_obj_list:
                        continue
    ...
    def disp_arl5():
        ...
        if last_sort == 1 and room != "":
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        (Res_line.resstatus == 8) &  (Res_line.abreise == fdate2)).order_by((Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        (Res_line.resstatus == 8) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.abreise == fdate2)).order_by(Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        (Res_line.resstatus == 8) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.abreise == fdate2)).order_by(Res_line.resname, Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
        ...
        if last_sort == 1:
            ...
            if room != "":
                ...
                if lname == "":
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            ((Res_line.active_flag == 1) &  (Res_line.resstatus != 12) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (Res_line.abreise == ci_date))).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.abreise == ci_date))).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            ((Res_line.active_flag == 1) &  (Res_line.resstatus != 12) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.name) >= lname.lower()) &  (func.lower(Res_line.name) <= to_name.lower()) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.name) >= lname.lower()) &  (func.lower(Res_line.name) <= to_name.lower()) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.name) >= lname.lower()) &  (func.lower(Res_line.name) <= to_name.lower()) &  (Res_line.abreise == ci_date))).order_by(Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
                ...
                if lname == "":
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  (Res_line.abreise == ci_date))).order_by((Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            ((Res_line.active_flag == 1) &  (Res_line.resstatus != 12) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  (func.lower(Res_line.name).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.abreise == ci_date))).order_by(Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
                    ...
                    res_line_obj_list = []
                    for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                            ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  (func.lower(Res_line.name) >= lname.lower()) &  (func.lower(Res_line.name) <= to_name.lower()) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  (func.lower(Res_line.name) >= lname.lower()) &  (func.lower(Res_line.name) <= to_name.lower()) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  (func.lower(Res_line.name) >= lname.lower()) &  (func.lower(Res_line.name) <= to_name.lower()) &  (Res_line.abreise == ci_date))).order_by(Res_line.name).all():
                        if res_line._recid in res_line_obj_list:
                            continue
                        else:
                            res_line_obj_list.append(res_line._recid)
            ...
            if lname == "" and room == "":
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (Res_line.abreise == ci_date))).order_by(Res_line.resname, Res_line.resnr, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.abreise == ci_date))).order_by(Res_line.resname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.resname) >= lname.lower()) &  (func.lower(Res_line.resname) <= to_name.lower()) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.resname) >= lname.lower()) &  (func.lower(Res_line.resname) <= to_name.lower()) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  ((substring(Res_line.zinr, 0, to_int(rmlen))) >= (room).lower()) &  (func.lower(Res_line.resname) >= lname.lower()) &  (func.lower(Res_line.resname) <= to_name.lower()) &  (Res_line.abreise == ci_date))).order_by(Res_line.resname, Res_line.zinr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
            ...
            if lname == "":
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  (Res_line.abreise == ci_date))).order_by(Gmember.nation1, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  (func.lower(Res_line.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Res_line.abreise == ci_date))).order_by(Gmember.nation1, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12) &  (func.lower(Res_line.resname) >= lname.lower()) &  (func.lower(Res_line.resname) <= to_name.lower()) &  (Res_line.ankunft <= ci_date) &  (Res_line.abreise >= ci_date)) |  ((Res_line.active_flag == 0) &  (func.lower(Res_line.resname) >= lname.lower()) &  (func.lower(Res_line.resname) <= to_name.lower()) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.Res_line.resstatus == 8) &  (func.lower(Res_line.resname) >= lname.lower()) &  (func.lower(Res_line.resname) <= to_name.lower()) &  (Res_line.abreise == ci_date))).order_by(Gmember.nation1, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
            ...
            if lresnr == 0:
                ...
                res_line_obj_list = []
                for res_line, waehrung, reservation, zimkateg, gmember in db_session.query(Res_line, Waehrung, Reservation, Zimkateg, Gmember).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                        ((Res_line.active_flag == 1) &  (Res_line.Res_line.resstatus != 12)) |  ((Res_line.active_flag == 0) &  (Res_line.ankunft == ci_date)) |  ((Res_line.active_flag == 2) &  (Res_line.resstatus == 8) &  (Res_line.abreise == ci_date))).order_by(Res_line.resnr, (Res_line.kontakt_nr * Res_line.resnr), Res_line.resstatus, Res_line.name).all():
                    if res_line._recid in res_line_obj_list:
                        continue
                    else:
                        res_line_obj_list.append(res_line._recid)
            ...
            if room != "":
                if lname == "":
                    ...
                    filter_conditions = [
                        ...
                        or_(
                            (Rline.zinr != None),
                            (func.substring(Rline.zinr, 0, rmlen_int) >= (room).lower())
                        ),
                        (func.lower(Rline.name) >= (lname).lower()),
                        (func.lower(Rline.name) <= (to_name).lower())
                    ]
                    ...
                    rmlen_int = int(rmlen) if rmlen is not None else 0
                    lname_lower = lname.lower() if lname else ""
                    to_name_lower = to_name.lower() if to_name else ""
                    rline_obj_list = []
                    ...
                    filter_conditions = [
                        ...
                        func.lower(Rline.name).op('>=')(lname_lower),
                        func.lower(Rline.name).op('<=')(to_name_lower),
                    ]
                    if ci_date is not None:
                        filter_conditions.append(
                            or_(
                ...
                if lname == "":
                    ...
                    lname_regex = (lname).lower().replace("*", ".*")
                    rline_obj_list = []
                    filter_conditions = [
                        (Rline.active_flag == 1),
                        (Rline.resstatus != 12),
                        func.lower(Rline.name).op("~")(lname_regex),
                    ]
                    if ci_date is not None:
                        filter_conditions.append(
                            or_(
        ...
        if last_sort == 1:
            ...
            if room != "":
                ...
                if lname == "":
                    ...
                    rline_obj_list = []
                    for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                            ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12) &  (Rline.ankunft <= ci_date) &  (Rline.abreise >= ci_date)) |  ((Rline.active_flag == 0) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.Rline.resstatus == 8) &  (Rline.abreise == ci_date))).order_by((Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                        if rline._recid in rline_obj_list:
                            continue
                        else:
                            rline_obj_list.append(rline._recid)
                    ...
                    rline_obj_list = []
                    for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                            ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12) &  (func.lower(Rline.name) >= lname.lower()) &  (func.lower(Rline.name) <= to_name.lower()) &  (Rline.ankunft <= ci_date) &  (Rline.abreise >= ci_date)) |  ((Rline.active_flag == 0) &  (func.lower(Rline.name) >= lname.lower()) &  (func.lower(Rline.name) <= to_name.lower()) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.Rline.resstatus == 8) &  (func.lower(Rline.name) >= lname.lower()) &  (func.lower(Rline.name) <= to_name.lower()) &  (Rline.abreise == ci_date))).order_by(Rline.name).all():
                        if rline._recid in rline_obj_list:
                            continue
                        else:
                            rline_obj_list.append(rline._recid)
            ...
            if lname == "":
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                        ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12) &  (Rline.ankunft <= ci_date) &  (Rline.abreise >= ci_date)) |  ((Rline.active_flag == 0) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.Rline.resstatus == 8) &  (Rline.abreise == ci_date))).order_by(gmember.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                        ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12) &  (func.lower(Rline.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Rline.ankunft <= ci_date) &  (Rline.abreise >= ci_date)) |  ((Rline.active_flag == 0) &  (func.lower(Rline.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.Rline.resstatus == 8) &  (func.lower(Rline.resname).op("~")(((lname)).lower().replace("*",".*"))) &  (Rline.abreise == ci_date))).order_by(gmember.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                        ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12) &  (func.lower(Rline.resname) >= lname.lower()) &  (func.lower(Rline.resname) <= to_name.lower()) &  (Rline.ankunft <= ci_date) &  (Rline.abreise >= ci_date)) |  ((Rline.active_flag == 0) &  (func.lower(Rline.resname) >= lname.lower()) &  (func.lower(Rline.resname) <= to_name.lower()) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.Rline.resstatus == 8) &  (func.lower(Rline.resname) >= lname.lower()) &  (func.lower(Rline.resname) <= to_name.lower()) &  (Rline.abreise == ci_date))).order_by(gmember.nation1, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
            ...
            if lresnr == 0:
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                        ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12)) |  ((Rline.active_flag == 0) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.resstatus == 8) &  (Rline.abreise == ci_date))).order_by(Rline.resnr, (Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
                ...
                rline_obj_list = []
                for rline, whrg, reservation, zkstat, gmember in db_session.query(Rline, Whrg, Reservation, Zkstat, Gmember).join(Whrg,(Whrg.waehrungsnr == Rline.betriebsnr)).join(Reservation,(Reservation.resnr == Rline.resnr)).join(Zkstat,(Zkstat.zikatnr == Rline.zikatnr)).join(Gmember,(Gmember.gastnr == Rline.gastnrmember)).filter(
                        ((Rline.active_flag == 1) &  (Rline.Rline.resstatus != 12) &  (Rline.resnr == lresnr)) |  ((Rline.active_flag == 0) &  (Rline.resnr == lresnr) &  (Rline.ankunft == ci_date)) |  ((Rline.active_flag == 2) &  (Rline.resstatus == 8) &  (Rline.resnr == lresnr) &  (Rline.abreise == ci_date))).order_by((Rline.kontakt_nr * Rline.resnr), Rline.resstatus, Rline.name).all():
                    if rline._recid in rline_obj_list:
                        continue
                    else:
                        rline_obj_list.append(rline._recid)
    ...
    def assign_it():
        ...
        if not reslin_queasy or not(reslin_queasy.key.lower()  == "flag"  \
            and reslin_queasy.resnr == res_line.resnr \
            and reslin_queasy.reslinnr == res_line.reslinnr \
            and reslin_queasy.betriebsnr == 0):
            reslin_queasy = (
                db_session.query(Reslin_queasy).filter(
                (func.lower(Reslin_queasy.key) == "flag") &  
                (Reslin_queasy.resnr == res_line.resnr) &  
                (Reslin_queasy.reslinnr == res_line.reslinnr) &  
                (Reslin_queasy.betriebsnr == 0)).first()
            )
