...
def nt_onlinetax_billdate_manual_palembang2(fr_date:date, to_date:date, art_spec:str):
    avail_flag = False
    n:int = 1
    do_it:bool = False
    ...
    vat_list_list, Vat_list = create_model("Vat_list", {"rechnr":int, "zeit":int, "bill_datum":date, "service_amt":decimal, "departement":int, "cdtrx":str})
    ...
    def generate_output():
        ...
        nonlocal fr_date, to_date, art_spec
        nonlocal hbill_buff
        ...
        for billjournal in db_session.query(Billjournal).filter(
            ...
            if do_it:
                ...
                if art_spec != " ":
                    ...
                    if num_entries(art_spec, ",") > 1:
                        for loopi in range(1,num_entries(art_spec, ",")  + 1) :

                            if billjournal.artnr == to_int(entry(loopi - 1, art_spec, ",")):
                                revcode = "ATV"
                        ...
                        if billjournal.artnr == to_int(entry(0, art_spec, ",")):
                            revcode = "ATV"
                ...
                if serv_betrag != 0:
                    ...
                    vat_list = query(vat_list_list, filters=(lambda vat_list: vat_list.rechnr == billjournal.rechnr and vat_list.bill_datum == billjournal.bill_datum and vat_list.cdtrx == revcode), first=True)
