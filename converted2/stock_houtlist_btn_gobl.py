...
def stock_houtlist_btn_gobl(pvilanguage:int, trans_code:string, from_grp:int, mi_alloc_chk:bool, mi_article_chk:bool, mi_docu_chk:bool, mi_date_chk:bool, from_lager:int, to_lager:int, from_date:date, to_date:date, from_art:int, to_art:int, deptno:int, long_digit:bool, cost_acct:string, mattype:int):

    prepare_cache ([Gl_acct, L_lager, L_artikel, L_ophis])
    ...
    str_list_data, Str_list = create_model("Str_list", {"fibu":string, "other_fibu":bool, "op_recid":int, "lscheinnr":string, "s":string, "mark":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal it_exist, str_list_data, tot_anz, tot_amount, i, lvcarea, gl_acct, l_lager, l_ophhis, l_artikel, l_ophis, parameters, l_untergrup
        nonlocal pvilanguage, trans_code, from_grp, mi_alloc_chk, mi_article_chk, mi_docu_chk, mi_date_chk, from_lager, to_lager, from_date, to_date, from_art, to_art, deptno, long_digit, cost_acct, mattype
    ...
    def create_lista():
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            if CONNECTED ("vhparch"):
                ...
                l_ophis_obj_list = {}
                for l_ophis, l_ophhis, gl_acct, l_artikel in db_session.query(L_ophis, L_ophhis, Gl_acct, L_artikel).join(L_ophhis,(L_ophhis.op_typ == ("STT").lower()) & (L_ophhis.lscheinnr == L_ophis.lscheinnr) & (L_ophhis.fibukonto != "")).join(Gl_acct,(Gl_acct.fibukonto == L_ophis.fibukonto)).join(L_artikel,(L_artikel.artnr == L_ophis.artnr)).filter(
        ...
        for l_lager in db_session.query(L_lager).filter(
            ...
            if CONNECTED ("vhparch"):
                ...
                for l_ophis, l_ophhis, gl_acct, l_artikel in db_session.query(L_ophis, L_ophhis, Gl_acct, L_artikel).join(L_ophhis,(L_ophhis.op_typ == ("STT").lower()) & (L_ophhis.lscheinnr == L_ophis.lscheinnr) & (L_ophhis.fibukonto != "")).join(Gl_acct,(Gl_acct.fibukonto == L_ophis.fibukonto)).join(L_artikel,(L_artikel.artnr == L_ophis.artnr)).filter(
                    ...
                    if create_it and curr_fibu == "":
                        curr_fibu = fibukonto

                    if create_it and curr_fibu.lower()  != (fibukonto).lower()  and t_anz != 0:
                        str_list = Str_list()
                        str_list_data.append(str_list)
