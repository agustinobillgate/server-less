...
def ar_age_2_webbl(pvilanguage:int, from_art:int, to_art:int, to_date:date, from_name:string, to_name:string, disptype:int, cvt_flag:bool, dollar_rate:Decimal):
    ...
    age_list_data, Age_list = create_model("Age_list", {"artnr":int, "rechnr":int, "counter":int, "gastnr":int, "creditlimit":Decimal, "rgdatum":date, "gastname":string, "saldo":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal, "fcurr":string})
    arage_list_data, Arage_list = create_model("Arage_list", {"curr":string, "gastnr":int, "number_str":string, "customer_name":string, "outstanding":string, "age1":string, "age2":string, "age3":string, "age4":string, "creditlimit":Decimal, "balance":Decimal, "creditlimit_str":string, "balance_str":string})
    ...
    def generate_output():
        ...
        return {"outlist": outlist, "arage-list": arage_list_data}
    ...
    def age_list():
                ...
                if debitor.counter != 0:
                    ...
                    if disptype == 0:
                        ...
                        for debt in db_session.query(Debt).filter(
                            ...
                            if not cvt_flag:
                                ar_saldo =  to_decimal(ar_saldo) + to_decimal(debitor.vesrdep)
                            else:
                                ar_saldo =  to_decimal(ar_saldo) + to_decimal((debitor.saldo) / to_decimal(dollar_rate))
                ...
                if disptype == 0:
                    ...
                    for debt in db_session.query(Debt).filter(
                        ...
                        if not cvt_flag:
                            ...
                            ar_saldo =  to_decimal(ar_saldo) + to_decimal((debt.saldo) / to_decimal(dollar_rate))
        ...
        for ledger in query(ledger_data, sort_by=[("artnr",False)]):
                ...
                if curr_gastnr == 0:
                    ...
                    if price_decimal == 0:
                        ...
                        if not long_digit:
                            ...
                            arage_list.outstanding = to_string(tot_debt, "->>,>>>,>>>,>>9.99")
                    ...
                    if arage_list.creditlimit != 0:
                        arage_list.balance =  to_decimal(arage_list.creditlimit) - to_decimal(tot_debt)
                    else:
                        arage_list.balance =  to_decimal("0")
                    arage_list.creditlimit_str = to_string(arage_list.creditlimit, "->>,>>>,>>>,>>>,>>9.99")
                    arage_list.balance_str = to_string(arage_list.balance, "->>,>>>,>>>,>>>,>>9.99")
                else:
            ...
            if price_decimal == 0:
                ...
                if not long_digit:
                    ...
                    arage_list.outstanding = to_string(ledger.tot_debt, "->>,>>>,>>>,>>9.99")
            ...
            if not long_digit:
                ...
                arage_list.outstanding = to_string((ledger.tot_debt / tmp_saldo * 100) , "->>,>>9.99")
                arage_list.age1 = to_string((ledger.debt0 / tmp_saldo * 100) , "->>,>>9.99")
                arage_list.age2 = to_string((ledger.debt1 / tmp_saldo * 100) , "->>,>>9.99")
                arage_list.age3 = to_string((ledger.debt2 / tmp_saldo * 100) , "->>,>>9.99")
                arage_list.age4 = to_string((ledger.debt3 / tmp_saldo * 100) , "->>,>>9.99")
        ...
        if price_decimal == 0:
            ...
            if not long_digit:
                ...
                arage_list.outstanding = to_string(t_saldo, "->>,>>>,>>>,>>9.99")
        ...
        if not long_digit:
            ...
            arage_list.age1 = to_string((t_debt0 / t_saldo * 100) , "->>,>>9.99")
            arage_list.age2 = to_string((t_debt1 / t_saldo * 100) , "->>,>>9.99")
            arage_list.age3 = to_string((t_debt2 / t_saldo * 100) , "->>,>>9.99")
            arage_list.age4 = to_string((t_debt3 / t_saldo * 100) , "->>,>>9.99")
