...
def fo_loadmacro_step1bl(briefnr:int):
    ...
    def generate_output():
        nonlocal art_list_data, n, l, continued, c, ct, i, j, parameters, briefzei
        nonlocal briefnr
        nonlocal parambuff


        nonlocal brief_list, art_list, parambuff
        nonlocal brief_list_data, art_list_data
    ...
    art_list = query(art_list_data, first=True)
    ...
    if not art_list:
        ...
        for art_list in query(art_list_data, filters=(lambda art_list: art_list.anzahl != 0)):
            art_list.anzahl = 0
