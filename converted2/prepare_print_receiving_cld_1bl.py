...
def prepare_print_receiving_cld_1bl(pvilanguage:int, docu_nr:string, user_init:string, po_nr:string, lief_nr:int, store:int, to_date:date):
    ...
    str_list_data, Str_list = create_model("Str_list", {"artnr":string, "qty":Decimal, "warenwert":string, "munit":string, "fibu":string, "fibu_ze":string, "addvat_value":Decimal, "bezeich":string, "lscheinnr":string, "unit_price":string})
