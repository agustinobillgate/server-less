...
def pj_inhouse4_1webbl(sorttype:int, from_date:date, to_date:date, froom:string, troom:string, exc_depart:bool, incl_gcomment:bool, incl_rsvcomment:bool, disp_accompany:bool, idflag:string):
    ...
    summary_list4_data, Summary_list4 = create_model("Summary_list4", {"argt":string, "rm_qty":int, "pax":int})
    ...
    for tmplist in query(tmplist_data, filters=(lambda tmplist: tmplist.rmno.lower()  >= (froom).lower()  and tmplist.rmno.lower()  <= (troom).lower())):
        ...
        summary_list4 = query(summary_list4_data, filters=(lambda summary_list4: summary_list4.argt == tmplist.argt), first=True)
        ...
        summary_list4.pax = summary_list4.pax + tmplist.a
    ...
    for inhouse_guest_list in query(inhouse_guest_list_data):
    ...
    if not queasy:
        ...
        for summary_roomtype in query(summary_roomtype_data):
        ...
        for summary_nation in query(summary_nation_data):
        ...
        for summary_revenue in query(summary_revenue_data):
            ct = ct + 1
        ...
        for summary_list4 in query(summary_list4_data):
            ct = ct + 1
            queasy = Queasy()
            db_session.add(queasy)
