...
def stock_retourlist_create_list1bl(from_date:date, to_date:date, from_supp:string, to_supp:string, show_price:bool):
    stock_retour_list_data = []

    str_list = stock_retour_list = None

    str_list_data, Str_list = create_model("Str_list", {"s":string})
    stock_retour_list_data, Stock_retour_list = create_model("Stock_retour_list", {"datum":date, "lief":string, "art":string, "bezeich":string, "qty":Decimal, "epreis":Decimal, "amount":Decimal, "reason":string, "id":string, "dlvnote":string, "lager":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal stock_retour_list_data
        nonlocal from_date, to_date, from_supp, to_supp, show_price


        nonlocal str_list, stock_retour_list
        nonlocal str_list_data, stock_retour_list_data

        return {"stock-retour-list": stock_retour_list_data}

    str_list_data = get_output(stock_retourlist_create_listbl(from_date, to_date, from_supp, to_supp, show_price))
    stock_retour_list_data.clear()

    for str_list in query(str_list_data):
        stock_retour_list = Stock_retour_list()
        stock_retour_list_data.append(stock_retour_list)
