...
def fo_invoice_inquirybillbl(inq_bill:int):
    ...
    def generate_output():
        ...
        nonlocal t_bill_data, t_billhis_data

        return {"msg_str": msg_str}

    t_bill_data = get_output(read_bill1bl(1, inq_bill, None, None, None, None, None, None, None, None))

    t_bill = query(t_bill_data, first=True)

    if not t_bill and inq_bill != 0:
        bill_exist, t_billhis_data = get_output(read_billhisbl(1, inq_bill, None, None))

        t_billhis = query(t_billhis_data, first=True)
