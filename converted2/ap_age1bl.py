...
def ap_age1bl(pvilanguage:int, to_date:date, from_name:string, to_name:string, detailed:bool, mi_bill:bool, segm:int, curr_disp:int):
    ...
    apage_list_data, Apage_list = create_model("Apage_list", {"docu_nr":string, "lschein":string, "rgdatum":string, "str":string})
    age_list_data, Age_list = create_model("Age_list", {"artnr":int, "rechnr":string, "lschein":string, "counter":int, "lief_nr":int, "rgdatum":date, "firma":string, "p_bal":Decimal, "debit":Decimal, "credit":Decimal, "saldo":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal})
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, to_date, from_name, to_name, detailed, mi_bill, segm, curr_disp
    ...
    def age_list():
        ...
        for l_kredit.counter, l_kredit.rgdatum, l_kredit.lief_nr, l_kredit.name, l_kredit.lscheinnr, l_kredit.netto, l_kredit.saldo, l_kredit.zahlkonto, l_kredit._recid, l_lieferant.segment1, l_lieferant.firma, l_lieferant._recid in db_session.query(L_kredit.counter, L_kredit.rgdatum, L_kredit.lief_nr, L_kredit.name, L_kredit.lscheinnr, L_kredit.netto, L_kredit.saldo, L_kredit.zahlkonto, L_kredit._recid, L_lieferant.segment1, L_lieferant.firma, L_lieferant._recid).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr) & (L_lieferant.firma >= (from_name).lower()) & (L_lieferant.firma <= (to_name).lower())).filter(
            ...
            if do_it:
                ...
                if (l_kredit.counter == 0) or (not age_list):
                    ...
                    if length(l_kredit.lscheinnr) <= 10:
                        age_list.lschein = l_kredit.lscheinnr
                    else:
                        age_list.lschein = substring(l_kredit.lscheinnr, 10, length(l_kredit.lscheinnr) - 10)
    ...
    def fill_in_list(fill_billno:bool, curr_po:string, curr_lschein:string, curr_rgdatum:string):
    ...
    if price_decimal == 0:
        outlist = " " + to_string(translateExtended ("T O T A L A/P:", lvcarea, "") , "x(30)") + to_string(t_saldo, "->,>>>,>>>,>>>,>>9") + to_string(t_prev, "->,>>>,>>>,>>>,>>9") + to_string(t_debit, "->,>>>,>>>,>>>,>>9") + to_string(t_credit, "->,>>>,>>>,>>>,>>9") + to_string(t_debt0, "->,>>>,>>>,>>>,>>9") + to_string(t_debt1, "->,>>>,>>>,>>>,>>9") + to_string(t_debt2, "->,>>>,>>>,>>>,>>9") + to_string(t_debt3, "->,>>>,>>>,>>>,>>9")
    else:
