...
def prepare_chg_pr_enter_suppbl(s_list_data:[S_list], docu_nr:string, flag_oe:int, artno:int, lieferdatum:date):
    ...
    supp_list_data, Supp_list = create_model("Supp_list", {"sno":int, "sname":string, "sprice":Decimal, "scurr":string, "fdate":date, "tdate":date, "flag":int, "sr":int})
    qsupp_list_data, Qsupp_list = create_model_like(Supp_list)
    rsupp_list_data, Rsupp_list = create_model_like(Supp_list)
    t_buff_order_data, T_buff_order = create_model_like(L_order)
    ...
    def generate_output():
        nonlocal pr_no, mainno, mainsupp, mainprice, maincurr, qsupp_list_data, rsupp_list_data, supp_list_data, t_buff_order_data, l_order, l_quote, l_lieferant
        ...
        nonlocal supp_list_data, qsupp_list_data, rsupp_list_data, t_buff_order_data

        return {"pr_no": pr_no, "mainno": mainno, "mainsupp": mainsupp, "mainprice": mainprice, "maincurr": maincurr, "qsupp-list": qsupp_list_data, "rsupp-list": rsupp_list_data, "supp-list": supp_list_data, "t-buff-order": t_buff_order_data}


    supp_list_data.clear()
    qsupp_list_data.clear()
    rsupp_list_data.clear()
    ...
    if flag_oe == 1:
        ...
        for l_quote in db_session.query(L_quote).filter(
            ...
            if l_lieferant:
                ...
                qsupp_list_data.append(qsupp_list)
        ...
        if b_order:
            ...
            if to_int(entry(1, entry(0, b_order.bestellart, "-") , ";")) != 0:
                ...
                if l_lieferant:
                    ...
                    rsupp_list_data.append(rsupp_list)
    ...
    s_list = query(s_list_data, filters=(lambda s_list: s_list.artnr == artno), first=True)
    ...
    if s_list:
        ...
        if s_list.du_price1 != 0:
            ...
            if l_lieferant:
                supp_list = Supp_list()
                supp_list_data.append(supp_list)
    ...
    for rsupp_list in query(rsupp_list_data):

        qsupp_list = query(qsupp_list_data, filters=(lambda qsupp_list: qsupp_list.sno == rsupp_list.sno and qsupp_list.sprice == rsupp_list.sprice and qsupp_list.flag == rsupp_list.flag), first=True)

        if qsupp_list:
            qsupp_list_data.remove(qsupp_list)

    for supp_list in query(supp_list_data):

        qsupp_list = query(qsupp_list_data, filters=(lambda qsupp_list: qsupp_list.sno == supp_list.sno and qsupp_list.sprice == supp_list.sprice and qsupp_list.flag == supp_list.flag), first=True)
    ...
    for supp_list in query(supp_list_data):
        ...
        rsupp_list = query(rsupp_list_data, filters=(lambda rsupp_list: rsupp_list.sno == supp_list.sno and rsupp_list.sprice == supp_list.sprice and rsupp_list.flag == supp_list.flag), first=True)

        if rsupp_list:
            rsupp_list_data.remove(rsupp_list)
    ...
    if buff_order:
        ...
        t_buff_order_data.append(t_buff_order)
