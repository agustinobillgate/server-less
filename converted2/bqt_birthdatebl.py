...
def bqt_birthdatebl(from_mm:int, from_dd:int, to_mm:int, to_dd:int):
    ...
    birth_list_data, Birth_list = create_model("Birth_list", {"company":string, "name":string, "geburtdatum":date, "geburt_ort1":string, "telephone":string, "email":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal birth_list_data, from_date, to_date, count, stopped, bk_veran, akt_kont, guest
