...
def tent_rsvlistbl(case_type:int, sorttype:int, from_date:date, to_date:date):
    ...
    t_tent_rsvlist_data, T_tent_rsvlist = create_model("T_tent_rsvlist", {"ankunft":date, "abreise":date, "arrangement":string, "kurzbez":string, "rsvname":string, "rsname":string, "segmentcode":int, "nation1":string, "zimmeranz":int, "erwachs":int, "gratis":int, "resstatus":int, "bemerk":string, "l_zuordnung":[int,5], "resnr":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_tent_rsvlist_data, reservation, zimkateg, guest, res_line
        nonlocal case_type, sorttype, from_date, to_date


        nonlocal t_tent_rsvlist
        nonlocal t_tent_rsvlist_data

        return {"t-tent-rsvlist": t_tent_rsvlist_data}
    ...
    def create_tent_rsvlist():
        ...
        t_tent_rsvlist_data.append(t_tent_rsvlist)
    ...
    for t_tent_rsvlist in query(t_tent_rsvlist_data):
        t_tent_rsvlist_data.remove(t_tent_rsvlist)
        pass
