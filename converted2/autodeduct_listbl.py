...
def autodeduct_listbl(sorttype:int, curr_lager:int, from_grp:int, transdate:date, fdate:date, tdate:date):
    ...
    c_list_data, C_list = create_model("C_list", {"artnr":int, "bezeich":string, "munit":string, "inhalt":Decimal, "zwkum":int, "endkum":int, "qty":Decimal, "qty1":Decimal, "amount":Decimal, "avrg_amount":Decimal, "fibukonto":string, "cost_center":string, "store":int, "dept":int, "price":Decimal, "item_artno":int})
    t_list_data, T_list = create_model("T_list", {"dept":int, "rechnr":int, "pay":Decimal, "rmtrans":Decimal, "compli":Decimal, "coupon":Decimal, "fibukonto":string, "pax":int}, {"fibukonto": "0000000000"})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal tot_amount, tot_avrg_amount, c_list_data, deduct_compli, l_artikel, l_untergrup, l_op, gl_acct, h_bill, h_bill_line, h_artikel, h_rezept, h_rezlin
        nonlocal sorttype, curr_lager, from_grp, transdate, fdate, tdate


        nonlocal c_list, t_list
        nonlocal c_list_data, t_list_data
                    ...
                    if not h_artikel:
                        ...
                        if h_artikel.artnrlager != 0:
                            ...
                            if l_artikel and l_artikel.endkum <= 2:
                                ...
                                c_list = query(c_list_data, filters=(lambda c_list: c_list.dept == t_list.dept and c_list.artnr == l_artikel.artnr and c_list.fibukonto == c_list.fibukonto and c_list.item_artno == h_bill_line.artnr), first=True)
                            ...
                            c_list = query(c_list_data, filters=(lambda c_list: c_list.dept == t_list.dept and c_list.artnr == h_artikel.artnr and c_list.fibukonto == c_list.fibukonto and c_list.item_artno == h_bill_line.artnr), first=True)
        ...
        for h_rezlin in db_session.query(H_rezlin).filter(
            ...
            if h_rezlin.recipe_flag :
                ...
                if l_artikel and l_artikel.endkum <= 2:
                    ...
                    c_list = query(c_list_data, filters=(lambda c_list: c_list.dept == t_list.dept and c_list.artnr == l_artikel.artnr and c_list.item_artno == h_bill_line.artnr), first=True)
        ...
        for h_rezlin in db_session.query(H_rezlin).filter(
            ...
            if h_rezlin.recipe_flag :
                ...
                if l_artikel and l_artikel.endkum <= 2:
                    ...
                    c_list = query(c_list_data, filters=(lambda c_list: c_list.dept == t_list.dept and c_list.artnr == l_artikel.artnr and c_list.fibukonto == t_list.fibukonto and c_list.item_artno == h_bill_line.artnr), first=True)
