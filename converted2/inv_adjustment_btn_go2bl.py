...
def inv_adjustment_btn_go2bl(from_grp:int, sorttype:int, curr_lager:int):
    ...
    def generate_output():
        nonlocal c_list_data, zwkum, a_bez, l_artikel, l_bestand
    ...
    def journal_list():
        ...
        if sorttype <= 2:
            ...
            l_bestand = L_bestand()
            l_artikel = L_artikel()
            for l_bestand.anz_anf_best, l_bestand._recid, l_artikel.artnr, l_artikel.bezeich, l_artikel.masseinheit, l_artikel.inhalt, l_artikel.endkum, l_artikel.zwkum, l_artikel.vk_preis, l_artikel._recid in db_session.query(L_bestand.anz_anf_best, L_bestand._recid, L_artikel.artnr, L_artikel.bezeich, L_artikel.masseinheit, L_artikel.inhalt, L_artikel.endkum, L_artikel.zwkum, L_artikel.vk_preis, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter(
                ...
                c_list.qty =  to_decimal(l_bestand.anz_anf_best) + to_decimal(anz_eingang) - to_decimal(anz_ausgang)
                c_list.qty1 =  to_decimal(l_bestand.anz_anf_best) + to_decimal(anz_eingang) - to_decimal(anz_ausgang)
