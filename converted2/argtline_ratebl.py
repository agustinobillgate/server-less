...
def argtline_ratebl(prcode:string, marknr:int, argtnr:int, zikatnr:int):

    prepare_cache ([Argt_line, Artikel, Reslin_queasy])

    t_argtline_rate_data = []
    argt_line = zimkateg = artikel = reslin_queasy = None

    t_argtline_rate = None

    t_argtline_rate_data, T_argtline_rate = create_model("T_argtline_rate", {"argt_artnr":int, "bezeich":string, "deci1":Decimal, "deci2":Decimal, "deci3":Decimal, "date1":date, "date2":date, "departement":int, "fakt_modus":int, "key":string, "char1":string, "number1":int, "number2":int, "number3":int, "resnr":int, "reslinnr":int, "artnr":int, "s_recid":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal t_argtline_rate_data, argt_line, zimkateg, artikel, reslin_queasy
        nonlocal prcode, marknr, argtnr, zikatnr


        nonlocal t_argtline_rate
        nonlocal t_argtline_rate_data

        return {"t-argtline-rate": t_argtline_rate_data}
    ...
    for reslin_queasy, argt_line, zimkateg, artikel in db_session.query(Reslin_queasy, Argt_line, Zimkateg, Artikel).join(Argt_line,(Argt_line.argtnr == Reslin_queasy.number2) & (Argt_line.argt_artnr == Reslin_queasy.number3) & (Argt_line.departement == Reslin_queasy.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Reslin_queasy.reslinnr)).join(Artikel,(Artikel.artnr == Argt_line.argt_artnr) & (Artikel.departement == Argt_line.departement)).filter(
             (Reslin_queasy.key == ("argt-line").lower()) & (Reslin_queasy.char1 == (prcode).lower()) & (Reslin_queasy.number1 == marknr) & (Reslin_queasy.number2 == argtnr) & (Reslin_queasy.reslinnr == zikatnr)).order_by(Argt_line.argt_artnr, Reslin_queasy.date1).all():
        ...
        t_argtline_rate_data.append(t_argtline_rate)
