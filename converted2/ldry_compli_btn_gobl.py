...
def ldry_compli_btn_gobl(foreign_nr:int, sorttype:int, from_dept:int, to_dept:int, from_date:date, to_date:date, billdate:date, exchg_rate:Decimal, double_currency:bool):
    ...
    c_list_data, C_list = create_model("C_list", {"nr":int, "datum":date, "dept":int, "rechnr":int, "name":string, "p_artnr":int, "bezeich":string, "betrag":Decimal, "t_cost":Decimal, "deptname":string})
    c1_list_data, C1_list = create_model_like(C_list)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal c_list_data, it_exist, n, h_artikel, artikel, hoteldpt, h_compli, exrate, h_bill, h_cost
    ...
    def journal_list1():
        ...
        for c1_list in query(c1_list_data, filters=(lambda c1_list: c1_list.betrag != 0), sort_by=[("p_artnr",False),("dept",False)]):
    ...
    def journal_list2():
        ...
        for c1_list in query(c1_list_data, filters=(lambda c1_list: c1_list.betrag != 0), sort_by=[("name",False)]):
