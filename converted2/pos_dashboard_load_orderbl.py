...
def pos_dashboard_load_orderbl(dept:int):
    ...
    queasy = db_session.query(Queasy).filter(
             (Queasy.key == 225) & (Queasy.number1 == dept) & (Queasy.logi1) & (Queasy.char1 == ("orderbill").lower()) & not_ (Queasy.logi3)).first()
    while None != queasy:
        ...
        queasy = db_session.query(Queasy).filter(
                 (Queasy.key == 225) & (Queasy.number1 == dept) & (Queasy.logi1) & (Queasy.char1 == ("orderbill").lower()) & not_ (Queasy.logi3) & (Queasy._recid > curr_recid)).first()
