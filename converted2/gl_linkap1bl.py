...
def gl_linkap1bl(pvilanguage:int, from_date:date, to_date:date, fibukonto:string, user_init:string, refno:string):
    ...
    def generate_output():
        nonlocal acct_error, credits, debits, remains, curr_anz, msg_str, t_g_list_data, s_list_data, curr_docu, curr_lschein, note, note1, add_note, curr_nr, debit_betrag, credit_betrag, lvcarea, gl_jouhdr, artikel, gl_acct, l_kredit, htparam
        nonlocal pvilanguage, from_date, to_date, fibukonto, user_init, refno


        nonlocal s_list, g_list, t_g_list, g_list1
        nonlocal s_list_data, g_list_data, t_g_list_data

        return {"acct_error": acct_error, "credits": credits, "debits": debits, "remains": remains, "curr_anz": curr_anz, "msg_str": msg_str, "t-g-list": t_g_list_data, "s-list": s_list_data}
    ...
    def step_two():
        ...
        for l_kredit in db_session.query(L_kredit).filter(
            ...
            if l_kredit.saldo < 0:
                ...
                g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.docu_nr == l_kredit.name and g_list.lscheinnr == l_kredit.lscheinnr and g_list.debit != 0), first=True)
                ...
                g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.docu_nr == l_kredit.name and g_list.lscheinnr == l_kredit.lscheinnr and g_list.credit != 0), first=True)
            ...
            if gl_acct:
                ...
                if l_kredit.saldo <= 0:
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == artikel.fibukonto and entry(0, g_list.bemerk, ";&&") == (note).lower()  and g_list.credit != 0), first=True)
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == artikel.fibukonto and entry(0, g_list.bemerk, ";&&") == (note).lower()  and g_list.debit != 0), first=True)
        ...
        for g_list in query(g_list_data, filters=(lambda g_list: g_list.nr == 1 and g_list.credit > 0)):

            g_list1 = query(g_list1_data, filters=(lambda g_list1: g_list1.fibukonto == g_list.fibukonto and g_list.nr == 1 and g_list1.lscheinnr == g_list.lscheinnr and (g_list1.debit > g_list.credit)), first=True)
    ...
    def add_list(create_it:bool):
        ...
        s_list = query(s_list_data, filters=(lambda s_list: s_list.fibukonto == gl_acct1.fibukonto), first=True)
