...
def check_esignature1_webbl(case_type:int, user_init:string, docu_nr:string, flag_type:string):
    ...
    reorder_esign_print_data, Reorder_esign_print = create_model_like(Esign_print)
    ...
    def generate_output():
        ...
        return {"found_flag": found_flag, "esign-list": esign_list_data, "esign-print": esign_print_data}
    ...
    if case_type == 1:
        ...
        elif flag_type.lower()  == ("DML").lower() :

            guestbook = db_session.query(Guestbook).filter(
                     (Guestbook.gastnr >= -271150) & (Guestbook.gastnr <= -271080) & (Guestbook.userinit == (user_init).lower()) & (Guestbook.reserve_logic[inc_value(2)])).first()
        ...
        if flag_type.lower()  == ("PR").lower() :
            ...
            for i in range(1,4 + 1) :
                ...
                esign_print = query(esign_print_data, filters=(lambda esign_print: esign_print.sign_nr == i), first=True)
            ...
            for queasy in db_session.query(Queasy).filter(
                     (Queasy.key == 352) & (Queasy.char1 == (docu_nr).lower())).order_by(Queasy._recid).all():

                guestbook = db_session.query(Guestbook).filter(
                         (Guestbook.gastnr == queasy.number2) & (Guestbook.reserve_logic[inc_value(2)])).first()
