...
from models import Guest, Nation, Res_line, Htparam, Billjournal, Bill, Artikel, Hoteldpt, Reservation, Segment, Queasy, Zimkateg, Genstat, H_bill_line, H_artikel, Zkstat, Outorder, Zimmer

def nt_dashboard_daily_1bl(from_date:date, to_date:date, exclude_article:string):

    prepare_cache ([Guest, Nation, Res_line, Htparam, Billjournal, Bill, Hoteldpt, Reservation, Segment, Queasy, Zimkateg, H_artikel, Zimmer])
    ...
    nat_code1:string = ""
    nat_group1:string = ""
    local_region1:string = ""
    guest = nation = res_line = htparam = billjournal = bill = artikel = hoteldpt = reservation = segment = queasy = zimkateg = genstat = h_bill_line = h_artikel = zkstat = outorder = zimmer = None

    his_res = his_inv = temp_res = future_res = future_inv = non_room = t_list = daily_his_res = gbuff = buf_nation = bufhis_inv = buffuture_inv = bufres_line = hbill_buff = hbill_list = None

    his_res_data, His_res = create_model("His_res", {"number":int, "resnr":int, "reslinnr":int, "staydate":date, "ci_date":date, "co_date":date, "nbrofroom":int, "room_type":string, "rm_rev":Decimal, "fb_rev":Decimal, "other_rev":Decimal, "resstatus":string, "reserveid":int, "reservename":string, "bookdate":date, "rate_code":string, "nationality":string, "segmentcode":string, "cancel_date":string, "zinr":string})
    his_inv_data, His_inv = create_model("His_inv", {"datum":date, "room_type":string, "room_status":string, "qty":int, "zikatnr":int})
    temp_res_data, Temp_res = create_model_like(His_res, {"create_date":date, "hotel_code":string, "adult":int, "argt":string, "guest_id":int, "gname":string, "nat_code":string, "nat_group":string, "local_region":string})
    ...
    daily_his_res_data, Daily_his_res = create_model("Daily_his_res", {"number":int, "staydate":date, "create_date":date, "ci_date":date, "co_date":date, "hotel_code":string, "resnr":int, "reslinnr":int, "reserve_id":int, "reservename":string, "nbrofroom":int, "room_type":string, "adult":int, "argt":string, "rm_rev":Decimal, "fb_rev":Decimal, "other_rev":Decimal, "resstatus":string, "segmentcode":string, "guest_id":int, "gname":string, "nationality":string, "nat_code":string, "nat_group":string, "local_region":string, "cancel_date":string, "mobile":string, "email":string})
    ...
    def generate_output():
        nonlocal his_res_data, his_inv_data, future_res_data, future_inv_data, non_room_data, daily_his_res_data, datum, datum2, ci_date, i, j, iftask, last_rechnr, curr_i, curr_date, end_date, fnet_lodg, net_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service, tot_fb, do_it1, vat_proz, do_it, serv_taxable, serv, vat, netto, serv_betrag, outstr, outstr1, revcode, rechnr_nottax, bill_date, pay_amount, bill_resnr, bill_reslinnr, bill_parentnr, bill_gastnr, ex_article, t_reslinnr, fdate, tdate, nationality1, nat_code1, nat_group1, local_region1, guest, nation, res_line, htparam, billjournal, bill, artikel, hoteldpt, reservation, segment, queasy, zimkateg, genstat, h_bill_line, h_artikel, zkstat, outorder, zimmer
        nonlocal from_date, to_date, exclude_article
        nonlocal gbuff, buf_nation, bufhis_inv, buffuture_inv, bufres_line, hbill_buff


        nonlocal his_res, his_inv, temp_res, future_res, future_inv, non_room, t_list, daily_his_res, gbuff, buf_nation, bufhis_inv, buffuture_inv, bufres_line, hbill_buff, hbill_list
        nonlocal his_res_data, his_inv_data, temp_res_data, future_res_data, future_inv_data, non_room_data, t_list_data, daily_his_res_data, hbill_list_data

        return {"his-res": his_res_data, "his-inv": his_inv_data, "future-res": future_res_data, "future-inv": future_inv_data, "non-room": non_room_data, "daily-his-res": daily_his_res_data}
    ...
    for bill_date in date_range(from_date,to_date) :

        for billjournal in db_session.query(Billjournal).filter(
                 ((Billjournal.bill_datum == bill_date) | ((Billjournal.bill_datum < bill_date) & (Billjournal.sysdate > bill_date))) & (Billjournal.anzahl != 0) & (Billjournal.betrag != 0)).order_by(Billjournal._recid).all():
            do_it = True
            ...
            if do_it:
                ...
                if do_it:
                    ...
                    if artikel.artnr == 100 and artikel.departement == 0 and billjournal.bill_datum != bill_date:
                        do_it = False
            ...
            if do_it:
                ...
                if netto != 0:
                    ...
                    if (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.zinr == "") or (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.comment == ""):
                        ...
                        his_res = query(his_res_data, filters=(lambda his_res: his_res.staydate == billjournal.bill_datum and his_res.resnr == bill_resnr and his_res.reslinnr == 0), first=True)
                        ...
                        daily_his_res = query(daily_his_res_data, filters=(lambda daily_his_res: daily_his_res.staydate == billjournal.bill_datum and daily_his_res.resnr == bill_resnr and daily_his_res.reslinnr == 0), first=True)

                        if not daily_his_res:
                            daily_his_res = Daily_his_res()
                            daily_his_res_data.append(daily_his_res)

                            daily_his_res.staydate = billjournal.bill_datum
                            daily_his_res.resnr = bill_resnr
                            daily_his_res.reserve_id = bill_gastnr
                            daily_his_res.reslinnr = 0
                            daily_his_res.nbrofroom = 0
                            daily_his_res.resstatus = "CHECKEDOUT"

                            guest = get_cache (Guest, {"gastnr": [(eq, bill_gastnr)]})

                            if guest:
                                daily_his_res.guest_id = guest.gastnr
                                daily_his_res.email = guest.email_adr
                                daily_his_res.mobile = guest.mobil_telefon

                                nation = get_cache (Nation, {"kurzbez": [(eq, guest.nation1)]})

                                if nation:
                                    daily_his_res.nationality = nation.bezeich
                                    daily_his_res.nat_code = nation.kurzbez

                                    queasy = get_cache (Queasy, {"key": [(eq, 6)],"number1": [(eq, nation.untergruppe)]})

                                    if queasy:
                                        daily_his_res.nat_group = queasy.char1

                                buf_nation = get_cache (Nation, {"kurzbez": [(eq, guest.nation2)]})

                                if buf_nation:
                                    daily_his_res.local_region = buf_nation.bezeich

                            res_line = get_cache (Res_line, {"resnr": [(eq, bill_resnr)],"reslinnr": [(eq, 0)]})

                            if res_line:
                                daily_his_res.ci_date = res_line.ankunft
                                daily_his_res.co_date = res_line.abreise
                                daily_his_res.adult = res_line.erwachs
                                daily_his_res.argt = res_line.arrangement
                                daily_his_res.gname = res_line.name

                                if res_line.cancelled != None:
                                    daily_his_res.cancel_date = to_string(get_month(res_line.cancelled) , "99") + "-" + to_string(get_day(res_line.cancelled) , "99") + "-" + to_string(get_year(res_line.cancelled) , "9999")

                                zimkateg = get_cache (Zimkateg, {"zikatnr": [(eq, res_line.zikatnr)]})

                                if zimkateg:
                                    daily_his_res.room_type = zimkateg.kurzbez

                            reservation = get_cache (Reservation, {"resnr": [(eq, bill_resnr)]})

                            if reservation:
                                daily_his_res.reservename = reservation.name

                                segment = get_cache (Segment, {"segmentcode": [(eq, reservation.segmentcode)]})

                                if segment:
                                    daily_his_res.segmentcode = segment.bezeich

                                if reservation.resdat != None:
                                    daily_his_res.create_date = reservation.resdat

                        if revcode.lower()  == ("ATS").lower() :
                            his_res.rm_rev =  to_decimal(his_res.rm_rev) + to_decimal(netto)
                            daily_his_res.rm_rev =  to_decimal(daily_his_res.rm_rev) + to_decimal(netto)

                        elif revcode.lower()  == ("ATM").lower() :
                            his_res.fb_rev =  to_decimal(his_res.fb_rev) + to_decimal(netto)
                            daily_his_res.fb_rev =  to_decimal(daily_his_res.fb_rev) + to_decimal(netto)

                        elif revcode.lower()  == ("ATZ").lower() :
                            his_res.other_rev =  to_decimal(his_res.other_rev) + to_decimal(netto)
                            daily_his_res.other_rev =  to_decimal(daily_his_res.other_rev) + to_decimal(netto)

                    elif (bill_reslinnr != 0 and bill_resnr > 0) or (bill_reslinnr == 0 and bill_resnr > 0 and billjournal.zinr != ""):

                        if bill_reslinnr != 0:

                            his_res = query(his_res_data, filters=(lambda his_res: his_res.staydate == billjournal.bill_datum and bill_resnr == his_res.resnr and bill_reslinnr == his_res.reslinnr), first=True)

                            daily_his_res = query(daily_his_res_data, filters=(lambda daily_his_res: daily_his_res.staydate == billjournal.bill_datum and bill_resnr == daily_his_res.resnr and bill_reslinnr == daily_his_res.reslinnr), first=True)
                            t_reslinnr = bill_reslinnr
                        else:

                            his_res = query(his_res_data, filters=(lambda his_res: his_res.staydate == billjournal.bill_datum and bill_resnr == his_res.resnr and to_int(entry(1, billjournal.comment, ";")) == his_res.reslinnr), first=True)

                            daily_his_res = query(daily_his_res_data, filters=(lambda daily_his_res: daily_his_res.staydate == billjournal.bill_datum and bill_resnr == daily_his_res.resnr and to_int(entry(1, billjournal.comment, ";")) == daily_his_res.reslinnr), first=True)
                        ...
                        if not daily_his_res:
                            ...
                            daily_his_res.staydate = billjournal.bill_datum
                            daily_his_res.resnr = bill_resnr
                            daily_his_res.reslinnr = t_reslinnr
                            daily_his_res.reserve_id = bill_gastnr
                            ...
                            if res_line:
                                ...
                                if billjournal.bill_datum < bill_date and billjournal.sysdate > bill_date:
                                    ...
                                    daily_his_res.nbrofroom = res_line.zimmeranz

                                if daily_his_res.staydate == daily_his_res.co_date and daily_his_res.ci_date != daily_his_res.co_date:
                                    his_res.nbrofroom = 0
                                ...
                                if res_line.resstatus == 12:
                                    ...
                                    if bufres_line:
                                        ...
                                        if genstat:
                                            ...
                                            if bufres_line.resstatus == 6:
                                                daily_his_res.resstatus = "INHOUSE"

                                            elif bufres_line.resstatus == 13:
                                                daily_his_res.resstatus = "INHOUSE(ROOM SHARER)"

                                            elif bufres_line.resstatus == 8:

                                                if genstat.resstatus == 13:
                                                    daily_his_res.resstatus = "CHECKEDOUT(ROOM SHARER)"
                        ...
                        non_room = query(non_room_data, filters=(lambda non_room: non_room.departmentID == billjournal.departement and non_room.postingdate == billjournal.bill_datum), first=True)

                        if not non_room:
                            non_room = Non_room()
                            non_room_data.append(non_room)
    ...
    for hbill_list in query(hbill_list_data, filters=(lambda hbill_list: hbill_list.do_it)):
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).filter(
            ...
            t_list = query(t_list_data, filters=(lambda t_list: t_list.rechnr == h_bill_line.rechnr and t_list.departement == h_bill_line.departement and t_list.bill_datum == h_bill_line.bill_datum), first=True)
    ...
    for t_list in query(t_list_data):
        ...
        non_room = query(non_room_data, filters=(lambda non_room: non_room.departmentID == t_list.departement and non_room.postingdate == t_list.bill_datum), first=True)
    ...
    for bill_date in date_range(from_date,to_date) :
        ...
        res_line = db_session.query(Res_line).filter(
                 ((Res_line.resstatus == 9) | (Res_line.resstatus == 10)) & (Res_line.active_flag == 2) & (Res_line.ankunft <= bill_date) & (Res_line.abreise >= bill_date) & (Res_line.l_zuordnung[inc_value(2)] == 0)).first()
        while None != res_line:
            ...
            temp_res.create_date = get_current_date()
            temp_res.adult = res_line.erwachs
            temp_res.argt = res_line.arrangement
            temp_res.gname = res_line.name
            ...
            if gbuff:
                ...
                temp_res.guest_id = gbuff.gastnr
            ...
            for datum in date_range(res_line.ankunft,datum2) :
                ...
                if datum == bill_date:
                    ...
                    buffer_copy(temp_res, daily_his_res)
                    daily_his_res.staydate = datum
                    daily_his_res.rm_rev =  to_decimal(net_lodg)
                    daily_his_res.fb_rev =  to_decimal(tot_fb)
                    daily_his_res.other_rev =  to_decimal(tot_other)
            ...
            if gbuff:
                ...
                if nation:
                    nationality1 = nation.bezeich
                    nat_code1 = nation.kurzbez

                    queasy = get_cache (Queasy, {"key": [(eq, 6)],"number1": [(eq, nation.untergruppe)]})

                    if queasy:
                        nat_group1 = queasy.char1

                buf_nation = get_cache (Nation, {"kurzbez": [(eq, gbuff.nation2)]})

                if buf_nation:
                    local_region1 = buf_nation.bezeich
            daily_his_res.nationality = nationality1
            daily_his_res.nat_code = nat_code1
            daily_his_res.nat_group = nat_group1
            daily_his_res.local_region = local_region1
            ...
            res_line = db_session.query(Res_line).filter(
                     ((Res_line.resstatus == 9) | (Res_line.resstatus == 10)) & (Res_line.active_flag == 2) & (Res_line.ankunft <= bill_date) & (Res_line.abreise >= bill_date) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line._recid > curr_recid)).first()
    temp_res_data.clear()
    for ci_date in date_range(fdate,tdate) :

        res_line = get_cache (Res_line, {"abreise": [(ge, ci_date)],"resstatus": [(ne, 4),(ne, 8),(ne, 9),(ne, 10),(ne, 12),(ne, 99)],"active_flag": [(le, 1)],"l_zuordnung[2]": [(eq, 0)]})
        while None != res_line:
            ...
            curr_recid = res_line._recid
            res_line = db_session.query(Res_line).filter(
                     (Res_line.abreise >= ci_date) & (Res_line.resstatus != 4) & (Res_line.resstatus != 8) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12) & (Res_line.resstatus != 99) & (Res_line.active_flag <= 1) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line._recid > curr_recid)).first()
    for bill_date in date_range(from_date,to_date) :

        genstat = get_cache (Genstat, {"datum": [(eq, bill_date)],"resstatus": [(ne, 0)],"res_logi[1]": [(eq, True)],"zikatnr": [(ne, 0)]})
        while None != genstat:
            ...
            if genstat.resstatus != 13 and genstat.zinr != "":
                ...
                if genstat.gratis != 0 or (genstat.zipreis == 0 and genstat.erwachs > 0):
                    ...
                    his_inv = query(his_inv_data, filters=(lambda his_inv: his_inv.datum == genstat.datum and his_inv.zikatnr == genstat.zikatnr and his_inv.room_status.lower()  == ("COMPLIMENT").lower()), first=True)
                    ...
                    his_inv = query(his_inv_data, filters=(lambda his_inv: his_inv.datum == genstat.datum and his_inv.zikatnr == genstat.zikatnr and his_inv.room_status.lower()  == ("OCCUPIED").lower()), first=True)
            ...
            genstat = db_session.query(Genstat).filter(
                     (Genstat.datum == bill_date) & (Genstat.resstatus != 0) & (Genstat.res_logi[inc_value(1)]) & (Genstat.zikatnr != 0) & (Genstat._recid > curr_recid)).first()
    for bill_date in date_range(from_date,to_date) :

        zkstat = get_cache (Zkstat, {"datum": [(eq, bill_date)],"zikatnr": [(ne, 0)]})
        while None != zkstat:

            his_inv = query(his_inv_data, filters=(lambda his_inv: his_inv.datum == zkstat.datum and his_inv.zikatnr == zkstat.zikatnr and his_inv.room_status.lower()  == ("AVAILABLE").lower()), first=True)
            ...
            zkstat = db_session.query(Zkstat).filter(
                     (Zkstat.datum == bill_date) & (Zkstat.zikatnr != 0) & (Zkstat._recid > curr_recid)).first()
    for bill_date in date_range(from_date,to_date) :

        outorder = get_cache (Outorder, {"gespstart": [(le, bill_date)],"gespende": [(ge, bill_date)],"zinr": [(ne, "")],"betriebsnr": [(le, 1)]})
        while None != outorder:
            ...
            if zimmer and zimmer.sleeping :
                for datum2 in date_range(outorder.gespstart,outorder.gespende) :
                    ...
                    if datum2 == bill_date:
                        ...
                        his_inv = query(his_inv_data, filters=(lambda his_inv: his_inv.datum == datum2 and his_inv.room_type == zimmer.kbezeich and his_inv.room_status.lower()  == ("OUTOFORDER").lower()), first=True)
            ...
            outorder = db_session.query(Outorder).filter(
                     (Outorder.gespstart <= bill_date) & (Outorder.gespende >= bill_date) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1) & (Outorder._recid > curr_recid)).first()

    for his_inv in query(his_inv_data):
    ...
    for his_inv in query(his_inv_data, filters=(lambda his_inv: his_inv.room_status.lower()  == ("AVAILABLE").lower())):

        bufhis_inv = query(bufhis_inv_data, filters=(lambda bufhis_inv: bufhis_inv.room_status.lower()  == ("OCCUPIED").lower()  and bufhis_inv.datum == his_inv.datum and bufhis_inv.room_type == his_inv.room_type), first=True)

        if bufhis_inv:
            his_inv.qty = his_inv.qty - bufhis_inv.qty

        bufhis_inv = query(bufhis_inv_data, filters=(lambda bufhis_inv: bufhis_inv.room_status.lower()  == ("OUTOFORDER").lower()  and bufhis_inv.datum == his_inv.datum and bufhis_inv.room_type == his_inv.room_type), first=True)

        if bufhis_inv:
            his_inv.qty = his_inv.qty - bufhis_inv.qty

        bufhis_inv = query(bufhis_inv_data, filters=(lambda bufhis_inv: bufhis_inv.room_status.lower()  == ("COMPLIMENT").lower()  and bufhis_inv.datum == his_inv.datum and bufhis_inv.room_type == his_inv.room_type), first=True)
    ...
    for ci_date in date_range(fdate,tdate) :
        ...
        for res_line in db_session.query(Res_line).filter(
                 (Res_line.active_flag <= 1) & (Res_line.resstatus <= 6) & (Res_line.resstatus != 3) & (Res_line.resstatus != 4) & (Res_line.kontignr >= 0) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line.ankunft <= ci_date + timedelta(days=365)) & (Res_line.abreise >= ci_date)).order_by(Res_line._recid).all():
            do_it1 = True
            ...
            if do_it1:
                ...
                for datum2 in date_range(res_line.ankunft,end_date) :
                    ...
                    if datum2 >= ci_date and datum2 <= ci_date + timedelta(days=365):

                        if res_line.gratis != 0 or (res_line.zipreis == 0 and res_line.erwachs > 0):

                            future_inv = query(future_inv_data, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.zikatnr == res_line.zikatnr and future_inv.room_status.lower()  == ("COMPLIMENT").lower()), first=True)
                            ...
                            future_inv = query(future_inv_data, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.zikatnr == res_line.zikatnr and future_inv.room_status.lower()  == ("OCCUPIED").lower()), first=True)
    ...
    for ci_date in date_range(fdate,tdate) :
        ...
        for zimmer in db_session.query(Zimmer).filter(
            ...
            if zimkateg.verfuegbarkeit:
                for datum in date_range(ci_date,ci_date + 365) :

                    future_inv = query(future_inv_data, filters=(lambda future_inv: future_inv.zikatnr == zimkateg.zikatnr and future_inv.room_status.lower()  == ("AVAILABLE").lower()  and future_inv.datum == datum), first=True)
    ...
    for ci_date in date_range(fdate,tdate) :
        ...
        outorder = get_cache (Outorder, {"gespende": [(ge, ci_date)],"gespstart": [(le, ci_date + 365)],"zinr": [(ne, "")],"betriebsnr": [(le, 1)]})
        while None != outorder:
            ...
            if zimmer and zimmer.sleeping :
                for datum2 in date_range(outorder.gespstart,outorder.gespende) :
                    ...
                    if datum2 >= ci_date and datum2 <= ci_date + timedelta(days=365):
                        ...
                        future_inv = query(future_inv_data, filters=(lambda future_inv: future_inv.datum == datum2 and future_inv.zikatnr == zimmer.zikatnr and future_inv.room_status.lower()  == ("OUTOFORDER").lower()), first=True)

                        if not future_inv:
                            future_inv = Future_inv()
                            future_inv_data.append(future_inv)

                            future_inv.zikatnr = zimmer.zikatnr
                            future_inv.datum = datum2
                            future_inv.qty = 1
            ...
            curr_recid = outorder._recid
            outorder = db_session.query(Outorder).filter(
                     (Outorder.gespende >= ci_date) & (Outorder.gespstart <= ci_date + timedelta(days=365)) & (Outorder.zinr != "") & (Outorder.betriebsnr <= 1) & (Outorder._recid > curr_recid)).first()

    for future_inv in query(future_inv_data):
    ...
    for future_inv in query(future_inv_data, filters=(lambda future_inv: future_inv.room_status.lower()  == ("AVAILABLE").lower())):

        buffuture_inv = query(buffuture_inv_data, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("OCCUPIED").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.zikatnr == future_inv.zikatnr), first=True)

        if buffuture_inv:
            future_inv.qty = future_inv.qty - buffuture_inv.qty

        buffuture_inv = query(buffuture_inv_data, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("OUTOFORDER").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.zikatnr == future_inv.zikatnr), first=True)

        if buffuture_inv:
            future_inv.qty = future_inv.qty - buffuture_inv.qty

        buffuture_inv = query(buffuture_inv_data, filters=(lambda buffuture_inv: buffuture_inv.room_status.lower()  == ("COMPLIMENT").lower()  and buffuture_inv.datum == future_inv.datum and buffuture_inv.zikatnr == future_inv.zikatnr), first=True)
