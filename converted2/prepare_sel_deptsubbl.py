...
def prepare_sel_deptsubbl():
    ...
    deptsub_data, Deptsub = create_model("Deptsub", {"deptsub_nr":int, "deptsub_nm":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal engid, msg_flag, deptsub_data, htparam, queasy, eg_subtask


        nonlocal deptsub
        nonlocal deptsub_data

        return {"engid": engid, "msg_flag": msg_flag, "DeptSub": deptsub_data}
    ...
    def create_deptsub():
        ...
        for qbuff in db_session.query(Qbuff).order_by(Qbuff._recid).all():
            ...
            deptsub = query(deptsub_data, filters=(lambda deptsub: deptsub.deptsub_nr == qbuff.dept_nr), first=True)
