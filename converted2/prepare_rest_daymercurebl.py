...
def prepare_rest_daymercurebl():
    ...
    t_kellner_data, T_kellner = create_model("T_kellner", {"kellnername":string, "kellner_nr":int, "departement":int, "rec_id":int})
    t_hoteldpt_data, T_hoteldpt = create_model("T_hoteldpt", {"num":int, "depart":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal exchg_rate, curr_local, curr_foreign, from_date, h_art_coupon, t_hoteldpt_data, t_kellner_data, htparam, waehrung, hoteldpt, kellner


        nonlocal t_kellner, t_hoteldpt
        nonlocal t_kellner_data, t_hoteldpt_data

        return {"exchg_rate": exchg_rate, "curr_local": curr_local, "curr_foreign": curr_foreign, "from_date": from_date, "h_art_coupon": h_art_coupon, "t-hoteldpt": t_hoteldpt_data, "t-kellner": t_kellner_data}
