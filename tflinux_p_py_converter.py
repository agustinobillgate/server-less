...
update_cache_list = {}
curr_set_cache_list = {}
update_yield_per_list = {}
curr_yield_per_list = {}

break_list = {}

curr_table_usage_dict = {}
...
def reset_variables():
    ...
    global inner_functions, incomplete_var_names, curr_cache_list, curr_yield_per_list
    global curr_vars, curr_input_params, curr_output_params, curr_body, all_vars, all_buffer_name_list
    ...
    global num_vars_include_file, include_file_flag, global_var_list
    global main_func_param_names, curr_inner_func_param_names, import_file_list, if_level, if_inside_if, check_find_first, assign_str, debug_flag, preprocesor_pos, curr_set_cache_list
...
def replace_matches_condition(match, is_db_query=False):
    ...
    if is_db_query:
        ...
        return f'matches({left_operand}, {regex_str})'
    else:
...
def remove_abl_comments(code):
    ...
    # code = code.replace("/**/","")
    code = code.replace("/*cache*","prepare-cache(").replace("*cache*/",")\nprepare-cache-end\n")
...
def get_dataclass_field_type(class_name: str, field_name: str):
    ...
    if not class_name in dataclass_field_list:
        return
...
def get_data_type(field):
    ...
    if len(words) == 1:
        if field in all_vars:
            data_type = all_vars[field]
...
def convert_data_type(data_type):
    ...
    elif re.match(r"CHAR.*",data_type, re.IGNORECASE): return "string"
    elif re.match(r"LONGCHAR.*",data_type, re.IGNORECASE): return "string"
...
def append_body(line, keep_indentation=False):
    ...
    global curr_temp_table_name, delete_curr_temp_table_flag, curr_set_cache_list
    ...
    if line_append.strip(" ") == "break":
        if not curr_file_name in break_list:
            break_list[curr_file_name] = ""


    tab_str = '    ' * (len(line_case) - line_case.count("DO:")  - line_case.count("DO TRANSACTION:")+ \
                            add_indentation + if_level)

    line_append = tab_str + line_append    
    

    for table_name in curr_set_cache_list:
        if curr_set_cache_list[table_name]["insert_before_line"] == line_append.strip(" "):
            
            if curr_set_cache_list[table_name]["condition"].strip(" ") == "":
                curr_set_cache_list[table_name]["condition"] = "None"  

            curr_set_cache_str = tab_str + "set_cache(" + convert_to_class_name(table_name) + ", " + curr_set_cache_list[table_name]["condition"] + "," + \
                str(curr_set_cache_list[table_name]["condition_field_names"]).replace("'","\"") + ", " + \
                str(curr_set_cache_list[table_name]["skip_flag"]).replace("'","\"") + "," + \
                str(curr_set_cache_list[table_name]["variable_condition_field_names"]).replace("'","\"") + "," + \
                str(curr_set_cache_list[table_name]["field_value_list"]).replace("'","\"") + ")\n"
            
            curr_body.append(curr_set_cache_str)
            curr_body.append("\n\n")
...
def convert_to_class_name(class_name):
    if class_name == None:
        return ""
    return class_name[0].upper() + class_name[1:].lower()
...
def add_table_import (table_name):
    ...
    if not converted_table_name in table_import_list and table_name.lower() in db_tables:
...
def query_db(table_name, condition = "", first_last = "", sort_by=[], join_tables=[], join_table_names=[]):
    ...
    if table_name in curr_buffer_list.keys():
        db_table_name = curr_buffer_list[table_name]
    elif table_name in buffer_list.keys():
        db_table_name = buffer_list[table_name]

    add_table_import(db_table_name)
    ...
    if first_last == "":
        ...
        if first_last == "PREV":
            ...
            if re.match(r".*\.filter\(.*", query_str):
                query_str = query_str[:-1] + " & (" + convert_to_class_name(table_name) + "._recid > curr_recid))"
            else:
                query_str += ".filter(" + convert_to_class_name(table_name) + "._recid > curr_recid)"
...
def query_dataclass(table_name, condition = "", first_last = "", sort_by=""):
    ...
    query_str += "query(" + table_name + "_data"
    ...
    if condition != "":
        ...
        query_str = query_str.replace(" " + table_name + ".matches(", " matches(")
        query_str = query_str.replace(" " + table_name + ".( ", "(")

        query_str = query_str.replace(" " + table_name + ".trim(", " trim(")
...
def convert_def_temp_table(lines):
    ...
    dataclass_str = table_name + "_data, " + convert_to_class_name(table_name) + " = "
    ...
    for line in lines.split("|")[1:]:
        if re.match(r"^FIELD .*",line,re.IGNORECASE):
            words = line.split(" ")
            ...
            if words[2] == "AS" or words[2] == "as":
                ...
                line = line.replace(" extent "," EXTENT ")
            ...
            if len(words) > 5:
                ...
                if words[4] in ("INIT","INITIAL","init","initial"):
                    def_value = words[5]
                    if def_value != "FALSE" and def_value != "NO" and def_value != "0" and def_value != "?":

                        if data_type == "date" and re.match(r"../../....",def_value):
                            def_value = "date_mdy(" + def_value.split("/")[0].lstrip("0") + ", " + def_value.split("/")[1].lstrip("0") + ", " + def_value.split("/")[2] + ")"
    ...
    add_dataclass_list(table_name + "_data")
...
def convert_def_var(line):
    ...
    if len(words) > 6:
        ...
        #     extent = int(words[6]) + 1
            if not re.match(r".* (INIT|INITIAL) .*",line,re.IGNORECASE):
                def_array_value = ""
                if data_type == "string":
                    def_array_value = "\"\""
                elif data_type == "int":
                ...
                def_value_str = "create_empty_list(" + string(extent) + "," + def_array_value + ")"
        ...
        if re.match(r".* (INIT|INITIAL) .*",line,re.IGNORECASE):   
            line = line.replace(" initial "," INITIAL ").replace(" init "," INIT ")             
        ...
        def_value_str = def_value_str.split(" FORMAT ")[0].split(" LABEL ")[0].strip(" ")
    if extent > 0 and re.match(r".* (INIT|INITIAL) .*",line, re.IGNORECASE):   
            if def_value[0] != "[":
                def_value_str = "create_empty_list(" + string(extent) + ", " + convert_value(def_value) + ")"
    ...
    if def_value_str != "":
        if data_type == "bool" and def_value_str in ["false","no",'"no"']:
            def_value_str = "False"
...
def get_incomplete_table_name(table_name):
    global incomplete_var_names, curr_file_name
    ...
    if (not convert_to_class_name(table_name) in table_import_list and 
        ...
        not (curr_buffer_list.get(table_name) and 
        convert_to_class_name(curr_buffer_list.get(table_name)) in table_import_list) and
        ...
        if table_match == []:
            table_match = list(filter(r.match, curr_buffer_list.keys()))
...
def convert_find(line):
    global db_tables, incomplete_var_names,curr_buffer_list, add_indentation, check_find_first

    check_find_first = True

    line = line.strip(".")

    use_index = re.search(r" USE_INDEX .*",line,re.IGNORECASE)
    ...
    if not table_name in data_class_objects or table_name in db_tables or buffer_list.get(table_name) in db_tables or curr_buffer_list.get(table_name) in db_tables:
        # if first_last in ["FIRST","first"] and (table_name in db_tables or \
        #                               convert_to_class_name(table_name) in curr_cache_list or \
        #                               convert_to_class_name(curr_buffer_list.get(table_name)) in curr_cache_list or\
        #                               convert_to_class_name(buffer_list.get(table_name)) in curr_cache_list):
        ...
        if first_last in ["FIRST","first"] and (table_name in db_tables or \
            ...
            if convert_to_class_name(curr_buffer_list.get(table_name)) in curr_cache_list:
                db_table_name = curr_buffer_list.get(table_name)
            elif convert_to_class_name(buffer_list.get(table_name)) in curr_cache_list:
                db_table_name = buffer_list.get(table_name)
            else:
                db_table_name = table_name

            db_table_name = convert_to_class_name(db_table_name)

            add_table_import(db_table_name)
            
            condition = condition.replace(" EQ "," = ").replace(" eq "," = ")


            tmp_condition = condition
            tmp_condition = tmp_condition.replace(" LT "," = (lt, ")
            tmp_condition = tmp_condition.replace(" lt "," = (lt, ")
            
            tmp_condition = tmp_condition.replace(" LE "," = (le, ")
            tmp_condition = tmp_condition.replace(" le "," = (le, ")

            tmp_condition = tmp_condition.replace(" GT "," = (gt, ")
            tmp_condition = tmp_condition.replace(" gt "," = (gt, ")

            tmp_condition = tmp_condition.replace(" GE "," = (ge, ")
            tmp_condition = tmp_condition.replace(" ge "," = (ge, ")
            
            tmp_condition = tmp_condition.replace(" NE "," = (ne, ")
            tmp_condition = tmp_condition.replace(" ne "," = (ne, ")
            
            condition_list = []
            condition_obj = {}

            # if re.match(r".* (GT|LT|GE|LE|OR) .*", condition, re.IGNORECASE):
            #     condition_list = []
            if re.match(r".* OR .*", tmp_condition, re.IGNORECASE):
                condition_list = []
            else:
                tmp_condition = tmp_condition.replace(" and ", " AND ")
                for condition_part in tmp_condition.split(" AND "):
                    operator_str = ""
                    if re.match(r".* \(lt, .*",condition_part, re.IGNORECASE):
                        operator_str = "(lt, "
                        condition_part = condition_part.replace("(lt, ","")
                    elif re.match(r".* \(le, .*",condition_part, re.IGNORECASE):
                        operator_str = "(le, "
                        condition_part = condition_part.replace("(le, ","")
                    elif re.match(r".* \(gt, .*",condition_part, re.IGNORECASE):
                        operator_str = "(gt, "
                        condition_part = condition_part.replace("(gt, ","")
                    elif re.match(r".* \(ge, .*",condition_part, re.IGNORECASE):
                        operator_str = "(ge, "
                        condition_part = condition_part.replace("(ge, ","")
                    elif re.match(r".* \(ne, .*",condition_part, re.IGNORECASE):
                        operator_str = "(ne, "
                        condition_part = condition_part.replace("(ne, ","")


                    if len(condition_part.split(" = ")) == 2:
                        condition_part = condition_part.strip("(").strip(" ").lower()
                        # condition_part = convert_var_assignment(condition_part)

                        words = condition_part.split(" = ")
                        field_name = words[0]
                        value = words[1]
                        
                        if not re.match(r".*" + table_name + r"\..*", field_name, re.IGNORECASE) and re.match(r".*" + table_name + r"\..*", value, re.IGNORECASE) :
                            tmp_str = value
                            value = field_name
                            field_name = tmp_str
                            if operator_str == "(lt, ":
                                ...
                                operator_str = "(le, "
                        # elif re.match(r".*" + table_name + r"\..*", field_name, re.IGNORECASE) and re.match(r".*" + table_name + r"\..*", value, re.IGNORECASE):
                        #     value = value.replace(table_name + ".", convert_to_class_name(table_name) + ".")

                        field_name = field_name.strip("(").strip(")")


                        if "(" in field_name or field_name[-1] == " ":
                            condition_list = []
                            break
                        ...
                        tmp_value = value.strip("(").strip(")").strip(" ")

                        if re.match(r".* (\-|\+) 1$", tmp_value):
                            operator_sign = ""
                            if re.match(r".* \- 1$", tmp_value):
                                operator_sign = " - "
                                tmp_value = tmp_value.replace(" - "," operator_sign ")
                            elif re.match(r".* \+ 1$", tmp_value):
                                operator_sign = " + "
                                tmp_value = tmp_value.replace(" + "," operator_sign ")

                            var_name = tmp_value.split(" operator_sign ")[0]
                            var_days = tmp_value.split(" operator_sign ")[1]

                            if get_data_type(var_name) == "date":
                                value = var_name + operator_sign + "timedelta(days=" + var_days + ")"


                        field_name = field_name.split(".")[-1].lower()
                        ...
                        value = operator_str + value.strip(" ").strip(")").strip(" ")
                        ...
                        right_value = ""
                        if operator_sign != "":
                            right_value = field_name.split(operator_sign)[1]
                            field_name = field_name.split(operator_sign)[0]
                        ...
                        value += operator_sign + right_value
                        ...
                        if not field_name in table_field_list[db_table_name.lower()]:
                            for table_field_name in table_field_list[db_table_name.lower()]:
                                if field_name in table_field_name:
                                    field_name = table_field_name

                        condition_list.append('"' + field_name + '": ' + value)

                        if not field_name in condition_obj:
                            condition_obj[field_name] = []

                        if operator_str == "":
                            value = "(eq, " + value + ")"
                        condition_obj[field_name].append(value)
                ...
                if condition_list != []:
                    if curr_table_usage_dict != {}:
                        condition_list = []
                        for field_name in condition_obj:                            
                            condition_list.append('"' + field_name + '": [' + ",".join(condition_obj[field_name]) + "]")
                        
                        condition_str = "{" + ",".join(condition_list) + "}"
                        # print(condition_obj)
                        # query = table_name + " = get_cache (" + db_table_name + ", " + "{" + ", ".join(condition_list) + "}, " + str(curr_table_usage_dict[db_table_name]["read"]) + ")"
                        # query = table_name + " = get_cache (" + db_table_name + ", " + condition_str + ", " + str(curr_table_usage_dict[db_table_name]["read"]) + ")"
                        query = table_name + " = get_cache (" + db_table_name + ", " + condition_str + ")"
...
def get_condition(line):
    ...
    if re.match(r".* WHERE .*", line, re.IGNORECASE):
...
def convert_for(line):
    global db_tables, add_indentation, curr_buffer_list, curr_file_name
    global curr_temp_table_name, curr_temp_table_line, curr_line_case, delete_curr_temp_table_flag, update_cache_list
    ...
    use_index = re.search(r" USE_INDEX[\s+][a-z_0-9]+",main_table, re.IGNORECASE)
    ...
    if curr_table_usage_dict != {}:
        ...
        main_db_table_name = main_table_name

        if buffer_list.get(main_table_name) in db_tables:
            main_db_table_name = buffer_list.get(main_table_name)
        elif curr_buffer_list.get(main_table_name) in db_tables:
            main_db_table_name = curr_buffer_list.get(main_table_name)


        table_info_dict[convert_to_class_name(main_table_name)] = {
            "is_db_table": main_table_is_db,
            "db_table_name": main_db_table_name,
        }


        if main_table_is_db and convert_to_class_name(main_db_table_name) in curr_table_usage_dict:
            table_info_dict[convert_to_class_name(main_table_name)]["read_fields"] = curr_table_usage_dict[convert_to_class_name(main_db_table_name)]["read"]
            table_info_dict[convert_to_class_name(main_table_name)]["read_only"] = curr_table_usage_dict[convert_to_class_name(main_db_table_name)]["write"] == []
        else:
            table_info_dict[convert_to_class_name(main_table_name)]["read_fields"] = []
            table_info_dict[convert_to_class_name(main_table_name)]["read_only"] = False
    ...
    for inner_table_line in table_list_line[1:]:
        ...
        use_index = re.search(r" USE_INDEX .*",inner_table_line, re.IGNORECASE)
        ...
        if curr_table_usage_dict != {}:
            ...
            inner_db_table_name = inner_table_name

            if curr_buffer_list.get(inner_table_name) in db_tables:
                inner_db_table_name = curr_buffer_list.get(inner_table_name)
            elif buffer_list.get(inner_table_name) in db_tables:
                inner_db_table_name = buffer_list.get(inner_table_name)

            table_info_dict[convert_to_class_name(inner_table_name)] = {
                "is_db_table": inner_table_is_db,
                "db_table_name": inner_db_table_name,
            }

            if inner_table_is_db:
                table_info_dict[convert_to_class_name(inner_table_name)]["read_fields"] = curr_table_usage_dict[convert_to_class_name(inner_db_table_name)]["read"]
                table_info_dict[convert_to_class_name(inner_table_name)]["read_only"] = curr_table_usage_dict[convert_to_class_name(inner_db_table_name)]["write"] == []
            else:
                table_info_dict[convert_to_class_name(inner_table_name)]["read_fields"] = []
                table_info_dict[convert_to_class_name(inner_table_name)]["read_only"] = False
        ...
        if main_table_is_db:
            ...
            # DB table join temp table
                ...
                later_join_tables.append(inner_table_name + " = query(" + inner_table_name + "_data, (lambda " + inner_table_name + ": " + inner_table_condition + "), first=True)")
            ...
            if inner_table_is_db:
                if first_inner_db_table_name == "":
                    ...
                    replace_condition = ".in_(list(set([" + table_name + "." + field_name + " for " + table_name + " in " + table_name + "_data" + main_condition + "])))"
            ...
            # Temp table join temp table
                ...
                join_tables.append(inner_table_name + " = query(" + inner_table_name + "_data, (lambda " + inner_table_name + ": " + inner_table_condition + "), first=True)")
    ...
    for table_name in table_info_dict:
        if not table_info_dict[table_name]["is_db_table"] or not table_info_dict[table_name]["read_only"] or table_info_dict[table_name]["read_fields"] == []:
            db_fields = []
            simplified_tables = []
            data_obj_list = []
            db_list = []
        ...
        # simplified_tables.append("_, Simplified_" + table_name.lower() + " = create_model_like(" + table_name + ")")
        # simplified_tables.append("simplified_" + table_name.lower() + " =  Simplified_" + table_name.lower() + "()" )

        # data_obj_list.append(table_name.lower()  + " = get_simplified_model("+ table_name + ")")
        data_obj_list.append(table_name.lower()  + " = " + convert_to_class_name(table_info_dict[table_name]["db_table_name"]) + "()")

        db_list.append(table_name)

        for field_name in table_info_dict[table_name]["read_fields"]:
            db_fields.append(table_name + "." + field_name)
    ...
    if length(db_list) > 1:
        ...
        for data_obj_str in data_obj_list:
            append_body(data_obj_str)
            append_body("\n")

        # query = query.replace("db_session.query(" + db_list_str + ")", "db_session.query(" + db_fields_str + ")")
        query = query.replace("for " + db_list_str.lower() + " in db_session.query(" + db_list_str + ")", 
                              "for " + db_fields_str.lower() + " in db_session.query(" + db_fields_str + ")")
    ...
    if first_flag and main_table_is_db:
        append_body("\n")
        add_indentation = 1
        append_body("if " +  main_table_name + "_obj_list.get(" + main_table_name + "._recid):" )
        ...
        append_body(main_table_name + "_obj_list[" + main_table_name + "._recid] = True")            
    ...
    if main_table_name_ori != "":
        add_indentation = 1
        append_body(main_table_name_ori + " = query(" + main_table_name_ori + "_data, (lambda " + main_table_name_ori + ": " + first_inner_db_table_condition_ori.lower().split("&")[0].split("|")[0].strip(" ") + "), first=True)")
    ...
    if curr_file_name.lower() in update_cache_list:
        if main_table_name in update_cache_list[curr_file_name.lower()]:
            append_body("\n")
            add_indentation = 1
            append_body("update_cache(" + main_table_name + ", " + str(update_cache_list[curr_file_name.lower()][main_table_name]) + ")")
...
def convert_run(line):
    ...
    line = line.split(' ', 1)[1]
    # line = line.lstrip("Run ").lstrip("run ").lstrip("RUN ").strip(".")
    ...
    if run_value_flag:
        ...
        if re.match(r".*\.lower \( \).*", function_name):
    ...
    if is_other_module and not no_import_flag:
...
def convert_db_query_condition(condition_str, table_name=""):
    ...
    for i in range(len(condition_str) - 1, -1, -1):
        char = substring(condition_str,i,1)
        if char in [")","]"]:
            end_brackets = char + end_brackets
        else:
            break    
    ...
    parts = re.split(r' (AND|OR|and|or) ', condition_str)
    ...
    for condition in conditions:
        ...
        if condition.startswith("Not "):
            ...
            if not re.match(r".*\.\_recid \.in\_\(list\(set\(\[.*",condition):
                processed_conditions.append("(" + condition + ")")
            else:
                processed_conditions.append(condition)
    ...
    for i, processed_condition in enumerate(processed_conditions):
        condition_str += processed_condition + (' ' + operators[i] + ' ' if i < len(operators) else '')
    condition_str = convert_string_comparison(condition_str, True, table_name)
    
    condition_str += end_brackets
...
def check_missing_table_name(var):
    ...
    if len(var.split(".")) == 1:
        ...
        if var in all_vars.keys() or var in db_tables or var in data_class_objects or var in curr_buffer_list:
            return
        
        for data_class_name in data_class_objects:
            if var in dataclass_field_list[data_class_name]:
                return
...
def convert_string_comparison(condition, is_sqlalchemy_query=False,table_name=""):    
    left_value_list = []
    right_value_list = []
        ...
        for condition_str in and_condition.split(" OR "):
            ...
            if len(var_values) == 1:
                if not is_sqlalchemy_query:
                    if re.match(r".* MATCHES .*",condition_str):
                        ...
                        if re.match(f"not .*", clean_var_value):
                            clean_var_value = " ".join(clean_var_value.split(" ")[1:])
                            not_flag = True                        

                        if re.match(r"\(.*\)", clean_var_value):
                            clean_var_value = clean_var_value[1:-1].strip(" ")

                        left_value = clean_var_value.split(" MATCHES ")[0]
                        # right_value = clean_var_value.split(" MATCHES ")[1].replace("*",".*").replace("$","\$")
                        right_value = clean_var_value.split(" MATCHES ")[1]

                        if re.match(r"\(.*\)", right_value):
                            right_value = right_value[1:-1].strip(" ")
                        ...
                        match_str = "matches(" + left_value + "," + right_value + "){additional_close_bracket}"
            ...
            if len(var_values) == 2:  
                ...
                if tmp_right_value.isdigit():
                    ...
                    right_data_type = "string"
                else:
                    right_data_type = all_vars.get(tmp_right_value.split(" ")[0])

                to_left_value = left_value
                if is_sqlalchemy_query:
                    if len(left_value.split(".")) == 1:
                        var_name = left_value.strip("(").replace("substring","").strip("(").split(",")[0].strip(" ").strip("(")

                        if (not var_name in main_func_param_names and 
                            # not var_name in curr_inner_func_param_names and 
                            not var_name in [var.split(":")[0].split("=")[0].strip(" ") for var in curr_inner_func_vars] and
                            not var_name in [var.split(":")[0].split("=")[0].strip(" ") for var in main_func_vars]):
                            ...
                            condition = condition.replace(convert_to_class_name(table_name) + "." + convert_to_class_name(table_name) + ".", convert_to_class_name(table_name) + ".")                            
                        ...
                        if curr_table_name != table_name and re.match(curr_table_name + ".*", table_name):
                ...
                if right_data_type == "string" and tmp_right_value != '""' and tmp_right_value != '"{space}"':   
    ...
    for left_value in left_value_list:
        if is_sqlalchemy_query:
            pass
            # update_import("from sqlalchemy import func")
            # condition = replace_exact(condition, left_value,"func.lower(" + convert_to_class_name(left_value.lower()).replace(".lower()","") +")")
        else:
    ...
    if is_sqlalchemy_query:
        ...
        condition = condition.replace("matches(not_ (","not_(matches(")

        if re.match(r".*matchesnot\_ \(.*",condition, re.IGNORECASE):
            condition = condition.replace("matchesnot_ (","not_(matches(")
            condition = condition.replace("{additional_close_bracket}",")")
    else:
        condition = condition.replace("matches(not ","not matches(")
...
def convert_condition(condition, is_sqlalchemy_query=False, table_name = ""):
    condition = condition.replace("\n","")
    ...
    if is_sqlalchemy_query:
        ...
        condition = convert_string_comparison(condition, False)
        condition = condition.replace(" And "," and ").replace(" AND "," and ").replace(" OR ", " or ")
...
def convert_do_to(line):
    ...
    if re.match(".* BY -1.*",line) :
        return line.replace(" = "," in range(").replace(" TO ",",").replace(" to ",",").replace( "BY -1"," - 1, -1")  + ") :"
...
def convert_do_while(line):
    ...
    if re.match(r"DO WHILE None != .*:",line, re.IGNORECASE):
        ...
        if table_name != "" and re.match(r"FIND FIRST " + table_name + " .*", curr_body[-1]):
            ...
            # curr_body[-2] = curr_body[-2].replace(" " + table_name + " = ", " for " + table_name + " in ").replace(".first()",".yield_per(1000):").replace(", first=True)","):")

            return ""

    return "while " + convert_condition(line.replace(" transaction "," ").replace(" TRANSACTION "," ").replace("DO WHILE ","").replace("do while ","")).replace("available ","") + ":"
...
def convert_create(line):
    global converted, if_without_do, add_indentation, keep_indentation
    ...
    if buffer_list.get(table_name) in db_tables:
        create_table_name = buffer_list[table_name]
    elif curr_buffer_list.get(table_name) in db_tables:
        create_table_name = curr_buffer_list[table_name]

    num_create_line_case = len(line_case)
    ...
    if table_name in db_tables or buffer_list.get(table_name) in db_tables or curr_buffer_list.get(table_name) in db_tables:
        ...
        append_str = table_name + "_data.append(" + table_name + ")"

    num_append_line_case = len(line_case)
    
    add_indentation = num_create_line_case - num_append_line_case
...
def convert_function_header(line):
    ...
    curr_func_name = line.split(" ")[1].lower().replace("-","_")
...
def convert_input_param_table(line):
    ...
    input_parameter = table_name + "_data" + ":" + "[" + convert_to_class_name(table_data_type) + "]"
    # input_parameter = table_name + "_data" + ":" + "List[" + convert_to_class_name(table_data_type) + "]"

    for def_str in def_dataclass:
        if re.match(table_name + "_data, .*", def_str):

            if not "PROCEDURE" in line_case:
                py_vars.append(def_str)
                def_dataclass.remove(def_str)
                global_var_list.append(table_name + "_data")

                if table_name + "_data" in data_class_list:
                    data_class_list.remove(table_name + "_data")
            ...
            if re.match(r".*create_model_like.*",def_str):
                ...
                for like_def_str in def_dataclass:
                    ...
                    if re.match(r".* " + like_table_name + " = .*",like_def_str):
                        ...
                        main_func_vars.append(like_table_name.lower().strip(" ") + "_data = []")
...
def convert_input_param_var(line):
    ...
    if len(line_words) > 6:
        line = line.replace(" extent ", " EXTENT ")
        if re.match(r".* EXTENT .*",line, re.IGNORECASE):
            # line = line.replace(" extent "," EXTENT ")
            # line = line.replace(" - 1]","]")
            # extent = int(line.split(" EXTENT ")[1].split(" ")[0])
            var_type = "[" + var_type + "]"
    ...
    for word in line_words:
        ...
        if word in ["INIT","INITIAL","init","initial"]:
            def_type = "INITIAL"
        elif word == "FORMAT":
            def_type = "FORMAT"
...
def convert_output_param_table(line, set_default_value=True):
    ...
    def_var_str = ""
    if "PROCEDURE" in line_case:
        curr_output_params.append(table_name + "_data")
    else:
        def_var_str = '"' + field_name.replace("__","_").replace("_","-") + '": ' + table_name + "_data"

        curr_output_params.append(def_var_str)

        if not table_name + ":[" + convert_to_class_name(table_name) + "]" in curr_input_params:
            if set_default_value:
                def_var_str = "    " + table_name + "_data = []"
                curr_vars.append(def_var_str)
...
def convert_decimal_operation(value):
    ...
    if re.match(r".*round\(.*", value):
        return "to_decimal( " + value + " )"


    # print(value)
    # if re.match(r".* round \( .*",value,re.IGNORECASE):
    #     return "to_decimal( " + value + " )"
...
def convert_date_operation(value):
    ...
    for match in matches:
        ...
        if len(words) > 1:
            ...
            elif (get_data_type(left_value) == date or left_value == "get_current_date()") and get_data_type(right_value) == date:
...
def convert_var_assignment(line):
    ...
    if len(line.split("=")) == 2:
        ...
        if not re.match(r".*\(.*\).*",var_name):
            ...
            if data_type == "date":
                if re.match(r"../../....", value_ori):
                    modified_value = "date_mdy(" + value_ori.split("/")[0].lstrip("0") + ", " + value_ori.split("/")[1].lstrip("0") + ", " + value_ori.split("/")[2] + ")"
                else:
            ...
            elif data_type == "int" and re.match(r".* = .* - .*",line):
                assign_value = line.split(" = ")[1]
                first_value = assign_value.split(" - ")[0].strip(" ").split("[")[0].strip(" ").split(" ")[0]
                second_value = assign_value.split(" - ")[1].strip(" ").split("[")[0].strip(" ").split(" ")[0]

                if (get_data_type(first_value).strip("[").strip("]") == "date" or first_value == "get_current_date()") and get_data_type(second_value).strip("[").strip("]") == "date":
                    line = line.strip(".").replace(" = "," = (") + ").days"
...
def convert_buffer(line):
    global curr_buffer_list, data_class_objects, curr_file_name, all_buffer_name_list
    ...
    if not buffer_name in all_buffer_name_list:
        all_buffer_name_list.append(buffer_name)
    ...
    if "PROCEDURE" in line_case:
        if table_name in db_tables:
            ...
            append_body("\n")
            append_body(buffer_name + "_data" + " = " + table_name + "_data")
            append_body("\n")
    else:
...
def convert_put_string(line):
    left_operand = line.split("(")[1].split(",")[0].strip(" ")
    right_operand = line.split("=")[1].replace(" ","").strip(".")
    
    append_body(f"{left_operand} = {right_operand}.encode('utf-8')")
    append_body("\n")
...
def convert_include_file(line):
    ...
    converted_program_name = "i_" + program_name.replace("-","_").lower().replace(".i","")

    import_only_list = ["inv_ar.i", "update_masterbl.i","master_taxserv.i"]

    if program_name in import_only_list:
        update_import("from functions." + converted_program_name + " import *")
    else:
...
def convert_delete(line):
    ...
    if table_name in db_tables or buffer_list.get(table_name) in db_tables or curr_buffer_list.get(table_name) in db_tables:
        ...
        return table_name + "_data.remove(" + table_name + ")"
...
def convert_other(line):
    ...
    if not(re.match(r"^DEF.* STREAM .*",line,re.IGNORECASE) or
           ...
           re.match(r".*OPEN QUERY.*",line,re.IGNORECASE) or
           re.match(r".*set_size \(.*",line,re.IGNORECASE) or
        ...
        # if re.match(r"^(\+|\*|/|\-).*",line):
        #     add_prev_line(line)
        elif re.match(".*date_mdy.*",line) and not re.match(".*substring.*",line,re.IGNORECASE):
            
            line = line.replace("date_mdy( 0", "date_mdy( ").replace(", 0",", ")
            append_body(line)
            append_body("\n")
        elif re.match(r".*\( get_current_date\(\) - .*",line) and not re.match(r".*\)\.days.*",line):   
            matches = re.findall(r"\( get_current_date\(\) - .* \)",line)

            for match in matches:
                line = line.replace(match,"(" + match.split(" )")[0] + " )" + ".days)")
...
# def convert_cache(line):
#     global converted
#     table_name_list = line.strip(")").split("(")[1].split(",")

#     for table_name in table_name_list:
#         curr_cache_list.append(convert_to_class_name(table_name.strip(" ")))
    
#     table_names_str = ",".join(curr_cache_list)
#     append_body(f"prepare_cache ([{table_names_str}])")
#     append_body("\n")
...
def convert_line(line):  
    ...
    global inner_functions, curr_func_name, check_find_first
    ...
    curr_clean_line = curr_clean_line.strip(" ")
    ...
    if check_if_do_statement:
        ...
        converted = True
    elif check_find_first:
        check_find_first = False
        # TODO: Check record availability
        # if not re.match("FIND FIRST .*", curr_clean_line, re.IGNORECASE):
        #     print(prev_clean_curr_line)
    
    # if re.match(r".*prepare\_cache.*",curr_clean_line):
    #     convert_cache(curr_clean_line)
    ...
    #     append_body("\n")
    elif re.match(r"^FIND CURRENT.*\.$", curr_clean_line, re.IGNORECASE):
        append_body("pass")
        append_body("\n")
    ...
    # elif re.match(r"^ASSIGN.*\.$",assign_str.replace("\n","").replace(" ",""), re.IGNORECASE):
    ...
    elif re.match(r"^\{.*",curr_clean_line):
        convert_include_file(curr_clean_line)
        append_body("\n")
        converted = True
        ...
        if prev_clean_curr_line.upper() == "DO:" or prev_clean_curr_line.upper() == "DO :" or re.match(r"IF .*",prev_clean_curr_line,re.IGNORECASE):
            append_body("pass")
            append_body("\n")
        ...
        curr_case_var = curr_clean_line.replace("CASE ","").strip(":").strip(".").strip(" ")
    ...
    elif re.match(r".* GET_STRING .* GET_SIZE .*",curr_clean_line, re.IGNORECASE):
        left_var = curr_clean_line.split("=")[0].strip(" ")
        right_var = curr_clean_line.split("=")[1].split("(")[1].split(",")[0].strip(" ")

        append_body(f"{left_var} = {right_var}")
    ...
    elif re.match(r".*put_string .*",curr_clean_line, re.IGNORECASE):
        convert_put_string(curr_clean_line)
...
def prepare_cache_list():
    global curr_cache_list, curr_table_usage_dict
    curr_cache_list = []
    curr_table_usage_dict = {}

    table_usage_info_file = "table_usage_info/" + curr_file_name + ".json"

    if os.path.isfile(table_usage_info_file):
        with open(table_usage_info_file, 'r') as file:
            curr_table_usage_dict = json.loads(file.read())

        for table_name in curr_table_usage_dict:
            read_list = curr_table_usage_dict[table_name]["read"]
            write_list = curr_table_usage_dict[table_name]["write"]

            if table_name.lower() in db_tables:
                if write_list == [] and read_list != []:
                    curr_cache_list.append(convert_to_class_name( table_name))
    
def prepare_content(file_content):
    global num_vars_include_file, include_file_flag, curr_cache_list, cache_list, curr_file_name
    ...
    file_content = file_content.replace("END PROCEDURE","END")

    file_content = file_content.replace(" assign "," ASSIGN ")
    ...
    file_content = replace_char_in_strings(file_content,"available","{a_v_a_i_l_a_b_l_e}")
    ...
    file_content = replace_char_in_strings(file_content,"|","{bar}")
    ...
    for line in file_content.split("\n"):        
        ...
        if ((re.match(r".*\{.*\.i.*\}.*",line) and 
                not re.match(r".*\{.*intevent_1\.i.*\}.*", line) and
                not re.match(r".*\{.*inv\_ar\.i.*\}.*", line) and
                not re.match(r".*\{.*update\_masterbl\.i.*\}.*", line) and
                not re.match(r".*\{.*master\_taxserv\.i.*\}.*", line)
        )or
    ...
    file_content = file_content.replace("REPEAT WHILE","DO WHILE")
    ...
    file_content = file_content.replace("RIGHT_TRIM (","right_trim(")
    file_content = file_content.replace("LEFT_TRIM (","left_trim(")
    ...
    file_content = file_content.replace(" STRING ("," to_string(")
    file_content = file_content.replace(" string ("," to_string(")
    file_content = file_content.replace("CHR (","chr_unicode(")
    file_content = file_content.replace("chr (","chr_unicode(")
    file_content = file_content.replace("ASC (","asc(")
    file_content = file_content.replace(" asc ("," asc(")
    ...
    file_content = file_content.replace("EXTENT (","length(")
    ...
    file_content = file_content.replace(" len,"," len_,")
    file_content = file_content.replace(" LENGTH ("," length(")
    file_content = file_content.replace("\nLENGTH ("," length(")
    file_content = file_content.replace(" length ("," length(")
    ...
    file_content = file_content.replace(".SECTION",".section")
    file_content = file_content.replace(".PARENT",".parent")
    file_content = file_content.replace(".TERMINATE",".terminate")
    file_content = file_content.replace(".WARNING",".warning")
    file_content = file_content.replace(".YEAR",".year")
    file_content = file_content.replace(".MONTH",".month")
    file_content = file_content.replace(".SELECTED",".selected")
    file_content = file_content.replace("_SELECTED","_selected")
    file_content = file_content.replace(".FREQUENCY",".frequency")
    file_content = file_content.replace(".TYPE",".type")
    file_content = file_content.replace(".POSITION",".position")
    file_content = file_content.replace(".MODIFIED",".modified")
    file_content = file_content.replace(".REQUEST",".request")
    file_content = file_content.replace(".LINE",".line")
    file_content = file_content.replace(".FILENAME",".filename")
    file_content = file_content.replace(".CANCELLED",".cancelled")
    ...
    file_content = file_content.replace(" AVAILABLE "," None != ")
    file_content = file_content.replace(" AVAIl;ABLE "," None != ")
    ...
    file_content = file_content.replace(".prCode",".prcode")
    file_content = file_content.replace(".markNo",".markno")
    ...
    file_content = file_content.replace(" INDEX ("," get_index(")


    file_content = file_content.replace(".NAME ",".name ")
    file_content = file_content.replace(".NAME\n",".name\n")
    file_content = file_content.replace(".KEY ",".key ")

    file_content = file_content.replace("comCategory","comcategory")

    file_content = file_content.replace("CASE_SENSITIVE","")
    file_content = file_content.replace("MESSAGE_DIGEST","message_digest")


    file_content = file_content.replace(".voucherNo",".voucherno")
    file_content = file_content.replace(".arRecid",".arrecid")

    file_content = file_content.replace(".currencyBE",".currencybe")
    file_content = file_content.replace(".currencyVHP",".currencyvhp")

    file_content = file_content.replace(".rcodeVHP",".rcodevhp")
    file_content = file_content.replace(".rcodeBE",".rcodebe")
    
    file_content = file_content.replace(".rmtypeVHP",".rmtypevhp")
    file_content = file_content.replace(".rmtypeBE",".rmtypebe")

    file_content = file_content.replace(".argtVHP",".argtvhp")

    file_content = file_content.replace(".CHAR1",".char1")

    file_content = file_content.replace(".nationBE",".nationbe")
    ...
    file_content = file_content.replace(".FOREIGN_RATE",".foreign_rate")
    file_content = file_content.replace(".DOUBLE_CURRENCY",".double_currency")
    ...
    file_content = file_content.replace("do: ","DO:\n")
    file_content = file_content.replace("do:\n","DO:\n")
    file_content = file_content.replace("\ndo:\n","\nDO:\n")
    file_content = file_content.replace(" do:\n"," DO:\n")
    ...
    file_content = file_content.replace("DO TRANSACTION","DO")
    file_content = file_content.replace("DO transaction","DO")
    ...
    file_content = file_content.replace(" matches "," MATCHES ")
    ...
    file_content = file_content.replace(" FIND CURRENT ","\nFIND CURRENT ")
    ...
    file_content = file_content.replace(" kline.kontstat "," kline.kontstatus ")
    file_content = file_content.replace(" Kline.kontstat "," kline.kontstatus ")

    file_content = file_content.replace("eg_Location"   , "eg_location")
    file_content = file_content.replace("Eg_Location"   , "Eg_location")

    file_content = file_content.replace("rateLocal"   , "ratelocal")


    file_content = file_content.replace("_SELECTED","_selected")
    file_content = file_content.replace(".SELECTED",".selected")

    file_content = file_content.replace("bRQ"  , "brq")
    file_content = file_content.replace("brQ"  , "brq")

    file_content = file_content.replace("argtlineBuff"  , "argtlinebuff")
    
    file_content = file_content.replace("prepare_cache_end","\n")
...
def replace_content(file_content, input_string, replace_with):    
    main_var_names = [var.split(":")[0].split("=")[0].strip(" ") for var in main_func_vars]

    if (not input_string in main_var_names and
        not input_string in [var_name.split(":")[0] for var_name in main_func_params]):

        file_content = file_content.replace(f" {input_string}.\n", f" {replace_with}.\n")
        file_content = file_content.replace(f" {input_string} ", f" {replace_with} ")
        file_content = file_content.replace(f" {input_string}\n", f" {replace_with}\n")
        file_content = file_content.replace(f"({input_string})", f"({replace_with})")
        file_content = file_content.replace(f"({input_string} ", f"({replace_with} ")
        file_content = file_content.replace(f"({input_string},", f"({replace_with},")
...
def update_var_names(file_content):
    file_content = replace_content(file_content, "paramnr", "htparam.paramnr")
    file_content = replace_content(file_content, "feldtyp", "htparam.feldtyp")
    file_content = replace_content(file_content, "fchar", "htparam.fchar")
    file_content = replace_content(file_content, "fdate", "htparam.fdate")
    file_content = replace_content(file_content, "finteger", "htparam.finteger")
    file_content = replace_content(file_content, "flogical", "htparam.flogical")
    file_content = replace_content(file_content, "ptexte", "paramtext.ptexte")

    file_content = replace_content(file_content, "wert_verbrau", "L_verbrauch.wert_verbrau")
    file_content = replace_content(file_content, "wert_verbrau", "L_verbrauch.wert_verbrau")

    file_content = replace_content(file_content, "argt_bez", "L_verbrauch.wert_verbrau")

    file_content = replace_content(file_content, "waehrungsnr", "waehrung.waehrungsnr")

    file_content = replace_content(file_content, "kontline.kontstat", "kontline.kontstatus")
    file_content = replace_content(file_content, "Kontline.kontstat", "Kontline.kontstatus")

    file_content = replace_content(file_content, "htparam.paramgr", "htparam.paramgruppe")
    file_content = replace_content(file_content, "Htparam.paramgr", "Htparam.paramgruppe")

    file_content = replace_content(file_content, "htparam.bezeich", "htparam.bezeichnung")
    file_content = replace_content(file_content, "Htparam.bezeich", "Htparam.bezeichnung")

    file_content = replace_content(file_content, "l_artikel.traubensort", "l_artikel.traubensorte")
    file_content = replace_content(file_content, "L_artikel.traubensort", "L_artikel.traubensorte")

    file_content = replace_content(file_content, "eg_request.property", "eg_request.propertynr")
    file_content = replace_content(file_content, "Eg_request.property", "Eg_request.propertynr")

    file_content = replace_content(file_content, "Eg_duration.DAY", "Eg_duration.days")
    file_content = replace_content(file_content, "Eg_duration.DAY", "Eg_duration.days")

    file_content = replace_content(file_content, "t_quote.activeFlag", "t_quote.activeflag")


    file_content = file_content.replace(" htparam.bezeich.", " htparam.bezeichnung.")
    file_content = file_content.replace(" Htparam.bezeich.", " Htparam.bezeichnung.")


    file_content = file_content.replace("Res_line.Res_line.", "Res_line.")

    file_content = file_content.replace("zimmer.zistat ", "zimmer.zistatus ")

    file_content = file_content.replace("Waehrung.aehrungsnr ", "Waehrung.waehrungsnr")

    file_content = replace_content(file_content, "htparam.fint ", "htparam.finteger ")
    file_content = file_content.replace(" guest.bemerk,", " guest.bemerkung,")

    file_content = file_content.replace(" STRING ("," to_string(")
    
    file_content = file_content.replace(" sbuff.to_decimal("," to_decimal(")

    file_content = file_content.replace("htparam.paramgr ","htparam.paramgruppe ")

    file_content = file_content.replace("htparam.flogic\n","htparam.flogical\n")
    file_content = file_content.replace("htparam.flogic ","htparam.flogical ")
    file_content = file_content.replace("htparam.flogic.","htparam.flogical.")

    file_content = file_content.replace("htparam.fint\n","htparam.finteger\n")
    file_content = file_content.replace("htparam.fint ","htparam.finteger ")


    file_content = file_content.replace("paramtext.ptext\n","paramtext.ptexte\n")
    file_content = file_content.replace("paramtext.ptext ","paramtext.ptexte ")

    file_content = file_content.replace("paramtext.ptext\n","paramtext.ptexte\n")
    file_content = file_content.replace("paramtext.ptext ","paramtext.ptexte ")


    file_content = file_content.replace("bediener.permission\n","bediener.permissions\n")
    file_content = file_content.replace("bediener.permission ","bediener.permissions ")
    file_content = file_content.replace("bediener.permission,","bediener.permissions,")
...
def fix_parenthesis(file_content):
    parenthesis_list = []
    new_file_content = ""
...
def create_table_usage_info():
    global buffer_list, curr_buffer_list, table_import_list, file_content

    table_usage_info = {}

    for table_name in table_import_list + list(curr_buffer_list.keys()) + list(buffer_list.keys()):
        db_table_name = table_name
        read_list = []
        skip_flag = len(re.findall(r"buffer_copy\(" + table_name.lower() + ",", file_content)) > 0

        if not skip_flag:
            skip_flag = len(re.findall(r"db_session.delete\(" + table_name.lower() + "\)", file_content)) > 0

        if not skip_flag:
            skip_flag = len(re.findall(r"None != " + table_name.lower(), file_content)) > 0
        ...
        if not db_table_name.lower() in db_tables:
            continue

        if not db_table_name in table_usage_info:
            table_usage_info[db_table_name] = {}
            read_list = []
            write_list = []
        else:
            read_list = table_usage_info[db_table_name]["read"]
            write_list = table_usage_info[db_table_name]["write"]


        matches = re.findall(r"\=.*[ |\(|\{]" + table_name.lower() + r"\..*[\(|\}|\n| |\,]", file_content)
        matches2 = re.findall(r"[\n| |\(|\{]" + table_name.lower() + r"\..*[==|<|>|<=|>=|!=] ", file_content)

        for match in matches + matches2:
            match = match + " "
            line = match.replace("\n","")

            field_names = re.findall(table_name.lower() + r"\..*?[ |\)|\}|\n|\,]",line)
            
            for field_name in field_names:
                field_name = field_name.replace(".lower()","").replace(".upper()","")
                field_name = field_name.strip(" ").strip("\n").strip(")").strip("}").strip(",").split(".")[-1].split("[")[0].strip(":").lower()
                ...
                if not field_name in read_list:
                    read_list.append(field_name)


        if table_name in db_tables and (table_name in curr_buffer_list.keys() or table_name in buffer_list.keys()):
            table_usage_info.pop(convert_to_class_name(table_name),None)
            continue

        if read_list != [] and not "_recid" in read_list:
            read_list.append("_recid")

        if skip_flag:
            read_list = []

        table_usage_info[db_table_name]["read"] = read_list

        matches = re.findall(r"[\n| |\(|\{]" + table_name + "\..* = ]", file_content)
        # matches = re.findall(r"[\n| |\(|\{]" + table_name + r"\..* [=|<|>|<=|<=|!=]", file_content)

        for match in matches:
            field_name = match.replace("\n","").replace("(","").replace("(","").replace("{","").split(".")[1].split("[")[0].strip(":").lower()

            if not field_name in table_field_list[db_table_name.lower()]:
                for table_field_name in table_field_list[db_table_name.lower()]:
                    if field_name in table_field_name:
                        field_name = table_field_name.lower()

            if not field_name in write_list:
                write_list.append(field_name)

        table_usage_info[db_table_name]["write"] = write_list

    usage_info_folder_path = "table_usage_info/"
    usage_info_file_path = usage_info_folder_path + curr_file_name + ".json"

    if not os.path.exists(usage_info_folder_path):
        os.makedirs(usage_info_folder_path)

    with open(usage_info_file_path, 'w') as output_file:
        json.dump(table_usage_info, output_file, indent=4)
...
def create_py_file(file_path, file_name):
    ...
    global data_class_list, curr_buffer_list, temp_table_and_db_loop_line, curr_yield_per_list, update_yield_per_list
    global include_file_flag, curr_file_name, curr_set_cache_list
    ...
    import_line = ["from functions.additional_functions import *","from decimal import Decimal"]
    ...
    curr_set_cache_list = set_cache_list.get(curr_file_name.lower(),{})
    curr_yield_per_list = update_yield_per_list.get(curr_file_name.lower(),{})
    ...
    if curr_set_cache_list:
        for table_name in curr_set_cache_list:
                
            add_table_import(table_name)
            if curr_set_cache_list[table_name]["insert_before_line"] == "":

                if curr_set_cache_list[table_name]["condition"].strip(" ") == "":
                    curr_set_cache_list[table_name]["condition"] = "None"                                


                curr_set_cache_str += "    set_cache(" + convert_to_class_name(table_name) + ", " + curr_set_cache_list[table_name]["condition"] + "," + \
                    str(curr_set_cache_list[table_name]["condition_field_names"]).replace("'","\"") + ", " + \
                    str(curr_set_cache_list[table_name]["skip_flag"]).replace("'","\"") + "," + \
                    str(curr_set_cache_list[table_name]["variable_condition_field_names"]).replace("'","\"") + "," + \
                    str(curr_set_cache_list[table_name]["field_value_list"]).replace("'","\"") + ")\n"
    ...
    for table_name in curr_cache_list:
        if table_name not in table_import_list:
            table_import_list.append(table_name)
    ...
    if main_func_vars:
        nonlocal_fields += "||||||||nonlocal "
        for var in main_func_vars:
            # var = var.strip(" ").replace ("= None","")
            if len(var.split("=")) <= 2:
                nonlocal_fields += var.split("=")[0].split(":")[0].strip(" ") + ", "
            else:
                nonlocal_fields += var.replace(" = ",", ").replace(" None","").strip(" ")
        nonlocal_fields = nonlocal_fields.strip(", ") + "\n"
    ...
    if main_func_param_names:
        nonlocal_fields += "||||||||nonlocal " 
        nonlocal_fields += ", ".join(main_func_param_names) + "\n"
            
    
    if buffer_list:
        nonlocal_fields += "||||||||nonlocal " 
        nonlocal_fields += ", ".join(buffer_list.keys()) + "\n"
    ...
    if global_var_list != []:
        ...
        global_fields += ", ".join(global_var_list) + "\n\n"
    ...
    if data_class_objects != []:
        ...
        nonlocal_fields += "||||||||nonlocal " 
        nonlocal_fields += ", ".join(data_class_objects) + "\n"


    if data_class_list != []:       
        nonlocal_fields += "||||||||nonlocal " 
        nonlocal_fields += ", ".join(data_class_list) + "\n"
    ...
    for name, params, body_lines, vars, output_params, input_param_names in inner_functions:
        ...
        # if global_var_list != []:
        #     for field in global_var_list:
        #         if not field in [param.split(":")[0] for param in params] and\
        #             not field in define_var_list and\
        #             not field in input_param_names:   
    ...
    main_return_func += nonlocal_fields.replace("|"," ") + "\n"
    ...
    for buffer_name,table_name in buffer_list.items():
        ...
        if not table_name in db_tables:
            buf_str += "    " + buffer_name + "_data = "  + table_name + "_data\n\n"

    cache_list_str = ""

    if curr_cache_list != []:
        cache_list_str = "\n    prepare_cache ([" + ", ".join(curr_cache_list) + "])\n\n"

    file_content = (
        '#using conversion tools version: ' + version + "\n\n" +
        "\n".join(import_line) + "\n\n" + 
    ...
    table_field_names = incomplete_var_names.values()

    for name in table_field_names:
        if len(name.split(".")) == 2:
            var_name = name.split(".")[1]
            # print(name, var_name)
            file_content = file_content.replace("." + name, "." + var_name)


    for table_name in table_import_list + all_buffer_name_list:
        table_name = table_name.lower()
        ...
        file_content = replace(file_content,convert_to_class_name(table_name) + "." + convert_to_class_name(table_name) + ".", convert_to_class_name(table_name) + ".")
    ...
    file_content = file_content.replace("(func.lower(not_ (","(not_(func.lower(")
    ...
    file_content = file_content.replace("fLocation_data", "flocation_data")
    file_content = file_content.replace("tLocation_data", "tlocation_data")
    file_content = file_content.replace("dynaRate_list" , "dynarate_list")
    file_content = file_content.replace("dynaRate_data" , "dynarate_data")
    file_content = file_content.replace("res_dynaRate"  , "res_dynarate")
    file_content = file_content.replace("gc_PIacct"    , "gc_piacct")
    file_content = file_content.replace("Gc_PIacct"    , "Gc_piacct")

    file_content = file_content.replace("UserSkill_list", "userskill_list")
    file_content = file_content.replace("UserSkill_data", "userskill_data")
    ...
    file_content = file_content.replace("tStatus_data"  , "tstatus_data")
    file_content = file_content.replace("tMaintask_data", "tmaintask_data")
    file_content = file_content.replace("comCategory"   , "comcategory")
    file_content = file_content.replace("eg_propMeter"  , "eg_propmeter")

    file_content = file_content.replace("gc_PI"  , "gc_pi")
    file_content = file_content.replace("Gc_PI"  , "Gc_pi")

    file_content = file_content.replace(".CHAR)"  , ".char)")
    file_content = file_content.replace(".CHAR\n"  , ".char\n")
    file_content = file_content.replace(".CHAR "  , ".char ")

    file_content = file_content.replace(".deptNo)"  , ".deptno)")
    file_content = file_content.replace(".deptNo\n"  , ".deptno\n")
    file_content = file_content.replace(".deptNo "  , ".deptno ")


    file_content = file_content.replace("egReq"  , "egreq")
    file_content = file_content.replace("EgReq"  , "Egreq")

    file_content = file_content.replace("L_artikel.min_best "  , "L_artikel.min_bestand ")
    file_content = file_content.replace("l_artikel.min_best\n"  , "l_artikel.min_bestand\n")
    file_content = file_content.replace("l_artikel.min_best:"  , "l_artikel.min_bestand:")
    file_content = file_content.replace("l_artikel.min_best)"  , "l)artikel.min_bestand)")

    file_content = file_content.replace("L_artikel.min_best "  , "L_artikel.min_bestand ")

    file_content = file_content.replace("l_art.traubensort "  , "l_art.traubensorte ")
    file_content = file_content.replace("l_art.traubensort\n"  , "l_art.traubensorte\n")

    file_content = file_content.replace("L_artikel.traubensort "  , "L_artikel.traubensorte ")
    file_content = file_content.replace("L_artikel.traubensort\n"  , "L_artikel.traubensorte\n")

    file_content = file_content.replace("L_artikel.min_best "  , "L_artikel.min_bestand ")
    file_content = file_content.replace("L_artikel.min_best\n"  , "L_artikel.min_bestand\n")

    file_content = file_content.replace("l_op.lager_nr_nr"  , "l_op.lager_nr")

    file_content = file_content.replace("Gc_pi.postDate"  , "Gc_pi.postdate")
    file_content = file_content.replace("gc_pi.postDate"  , "gc_pi.postdate")

    file_content = file_content.replace("Gc_pi.rcvName"  , "Gc_pi.rcvname")
    file_content = file_content.replace("gc_pi.rcvName"  , "gc_pi.rcvname")

    file_content = file_content.replace("Gc_pi.chequeNo"  , "Gc_pi.chequeno")
    file_content = file_content.replace("gc_pi.chequeNo"  , "gc_pi.chequeno")

    file_content = file_content.replace("Gc_pi.returnAmt"  , "Gc_pi.returnamt")
    file_content = file_content.replace("gc_pi.returnAmt"  , "gc_pi.returnamt")

    file_content = file_content.replace("Gc_pi.rcvID"  , "Gc_pi.rcvid")
    file_content = file_content.replace("gc_pi.rcvID"  , "gc_pi.rcvid")

    file_content = file_content.replace(" counter.counter" , " counters.counter")
    file_content = file_content.replace("gc-PIacct"  , "gc-piacct")

    file_content = file_content.replace("htparam.flogi "  , "htparam.flogic ")
    file_content = file_content.replace("htparam.flogi\n"  , "htparam.flogic\n")

    file_content = file_content.replace("zimkateg.bezeich "  , "zimkateg.bezeichnung ")
    file_content = file_content.replace("zimkateg.bezeich("  , "zimkateg.bezeichnung(")
    file_content = file_content.replace("zimkateg.bezeich\n"  , "zimkateg.bezeichnung\n")

    file_content = file_content.replace("sbuff.to_decimal("  , "to_decimal(")

    file_content = file_content.replace("qbuff.entry("  , "entry(")


    file_content = file_content.replace("zl_op.lager "  , "l_op.lager_nr ")
    file_content = file_content.replace("l_op.lager\n"  , "l_op.lager_nr\n")

    file_content = file_content.replace(".STR, "  , ".str, ")
    file_content = file_content.replace(".STR , "  , ".str , ")

    file_content = file_content.replace("TRANS"  , "trans")

    file_content = file_content.replace(".ACTIVE"  , ".active")

    file_content = file_content.replace(" OUTPUT_"  , " output_")
    file_content = file_content.replace(".NAME "  , ".name ")
    file_content = file_content.replace(".NAME\n"  , ".name\n")
    file_content = file_content.replace("::",":")


    file_content = file_content.replace("reslin_queasy.reslin_queasy."  , "reslin_queasy.")

    file_content = file_content.replace(" guest.bemerk "  , " guest.bemerkung ")
    file_content = file_content.replace(" guest.bemerk\n"  , " guest.bemerkung\n")

    file_content = file_content.replace(" gbuff.bemerk "  , " gbuff.bemerkung ")
    file_content = file_content.replace(" gbuff.bemerk\n"  , " gbuff.bemerkung\n")

    file_content = file_content.replace(" Guest.bemerk "  , " Guest.bemerkung ")
    file_content = file_content.replace(" Guest.bemerk\n"  , " Guest.bemerkung\n")

    file_content = file_content.replace(" guest.NAME,", " guest.name,")
    file_content = file_content.replace(" guest.NAME ", " guest.name ")
    file_content = file_content.replace(" guest.NAME\n", " guest.name\n")
    ...
    file_content = file_content.replace("Sourccod.source_cod "  , "Sourccod.source_code ")
    file_content = file_content.replace("Sourccod.source_cod)"  , "Sourccod.source_code)")
    file_content = file_content.replace("Sourccod.source_cod\n"  , "Sourccod.source_code\n")

    file_content = file_content.replace("[inc_value(1))]"  , "[inc_value(1)])")

    file_content = file_content.replace(" &  (not "," & not_(")

    file_content = fix_parenthesis(file_content)

    file_content = file_content.replace("to_decimal(()","to_decimal(")

    file_content = file_content.replace(" lastDay ("," lastday (")
    ...
    file_content = file_content.replace(" ()","()")
    file_content = file_content.replace(" .",".")
    file_content = file_content.replace("transACTION","TRANSACTION")
    ...
    file_content = file_content.replace("{v_h_p}","vhp")
    file_content = file_content.replace("{v__h__p}","vhp")
    file_content = file_content.replace("{a_v_a_i_l_a_b_l_e}","available")
    file_content = file_content.replace("{a__v__a__i__l__a__b__l__e}","available")
    ...
    file_content = file_content.replace("{bar}","|")

    file_content = file_content.replace("{plus}","+")
    file_content = file_content.replace("{minus}","-")
    file_content = remove_multiple_chars(file_content, "\n\n")

    for var_name in incomplete_var_names.values():
        file_content = file_content.replace('"' + var_name + '"', '"' + var_name.split(".")[-1] + '"')

    for target_string in curr_yield_per_list:
            file_content = file_content.replace(target_string, target_string.replace(".all()",".yield_per(100)"))


    create_table_usage_info()
...
def convert_files(read_table_usage_info=False):
    global curr_file_name, set_cache_list, temp_table_and_db_loop_line, update_cache_list, update_yield_per_list
    ...
    skip_file_list = ["get-user-tokenBL.p",
                    "bk-amendmentbl.p", "bk-cancel-masterbl.p", 
                    "bk-create-eventbl.p","bk-deposit-btn-okbl.p",
                    "bk-deposit-checkinbl.p","create-bk-event-detailbl.p",
                    "create-bk-masterBL.p","delete-bk-queasyBL",
                    "delete-bk-queasyBL","insert-eventbl.p",
                    "read-bk-queasyBL.p","write-bk-queasyBL.p"]
    ...
    input_file = csv.DictReader(open("Update Log - Prepare Cache List.csv"))

    for row in input_file:
        program_name = row["program_name"].lower()
        table_name = row["table_name"]
        condition_field_names = row["condition_field_names"]
        condition = row["condition"]
        insert_before_line = row["insert_before_line"]
        cache_skip_flag =  convert_to_class_name(row["skip_flag"])
        variable_condition_field_names = row["variable_condition_field_names"]
        field_value_list = row["field_value_list"]

        if not set_cache_list.get(program_name):
            set_cache_list[program_name] = {}
        
        if not table_name in set_cache_list[program_name]:
            set_cache_list[program_name][table_name] = {}
            set_cache_list[program_name][table_name]["condition_field_names"] = []
            set_cache_list[program_name][table_name]["variable_condition_field_names"] = []
            set_cache_list[program_name][table_name]["field_value_list"] = []

            if not set_cache_list[program_name][table_name].get("skip_flag"):
                set_cache_list[program_name][table_name]["skip_flag"] = cache_skip_flag

            if not set_cache_list[program_name][table_name].get("condition"):
                set_cache_list[program_name][table_name]["condition"] = ""

            if set_cache_list[program_name][table_name]["condition"] == "":
                set_cache_list[program_name][table_name]["condition"] = condition


            if not set_cache_list[program_name][table_name].get("insert_before_line"):           
                set_cache_list[program_name][table_name]["insert_before_line"] = ""

            if set_cache_list[program_name][table_name]["insert_before_line"] == "":           
                set_cache_list[program_name][table_name]["insert_before_line"] = insert_before_line

        field_names = []
        for field_name in condition_field_names.split(","):
            if field_name == "":
                continue
            field_names.append(field_name.strip(" "))

            if not field_names in set_cache_list[program_name][table_name]["condition_field_names"]:
                set_cache_list[program_name][table_name]["condition_field_names"].append(field_names)

        variable_field_names = []
        if set_cache_list[program_name][table_name]["variable_condition_field_names"] == []:
            for field_name in variable_condition_field_names.split(","):
                field_name = field_name.strip(" ")
                if field_name != "":
                    variable_field_names.append(field_name.strip(" "))

                if variable_field_names != [] :
                    set_cache_list[program_name][table_name]["variable_condition_field_names"] = variable_field_names

        field_value_list_names = []
        if set_cache_list[program_name][table_name]["field_value_list"] == []:
            for field_name in field_value_list.split(","):
                field_name = field_name.strip(" ")
                if field_name != "":
                    field_value_list_names.append(field_name.strip(" "))

                if field_value_list_names != [] :
                    set_cache_list[program_name][table_name]["field_value_list"] = field_value_list_names

    
    input_file = csv.DictReader(open("Update Log - Update Cache.csv"))
    ...
    for row in input_file:
        ...
        if not program_name in update_cache_list:
            update_cache_list[program_name] = {}

        update_cache_list[program_name][table_name] = []

        for field in fields.split(","):
            update_cache_list[program_name][table_name].append(field.strip(" "))


    input_file = csv.DictReader(open("Update Log - Update with yield_per.csv"))

    for row in input_file:
        program_name = row["program_name"].lower()
        target_string = row["target_string"]

        if not program_name in update_yield_per_list:
            update_yield_per_list[program_name] = []
        
        if not target_string in update_yield_per_list[program_name]:
            update_yield_per_list[program_name].append(target_string)


table_usage_info_path = os.getcwd() + "/table_usage_info"

if os.path.exists(table_usage_info_path):
    shutil.rmtree(table_usage_info_path)

convert_files(False)
convert_files(True)

break_list_path = os.getcwd() + "/break_list.txt"

with open(break_list_path, 'w') as output_file_all:
    break_list_str = "\n".join(list(break_list.keys()))
    output_file_all.write(f"{break_list_str}")
