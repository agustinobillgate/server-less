...
def res_checkin2bl(pvilanguage: int, resnr: int, reslinnr: int, user_init: string, silenzio: bool):
    ...
    prepare_cache([Guest, Bill, Queasy, Htparam, Reservation, Waehrung, Master, Counters, Bediener, Bill_line, Artikel, Billjournal, Umsatz, Res_history, Reslin_queasy, Exrate])
    ...
    strc: string = ""
    bill_no: int = 0
    art_security: int = 0  # ITA: Program terkait feature service apartement
    loopi: int = 0
    ar_ledger: int = 0  # ITA: Program terkait feature service apartement
    lvcarea: string = "res-checkin"
    res_line = guest = bill = queasy = htparam = outorder = reservation = waehrung = master = counters = bediener = bill_line = artikel = billjournal = umsatz = res_history = reslin_queasy = exrate = messages = zimplan = zimmer = resplan = zimkateg = None

    res_member = receiver = res_sharer = res_line1 = rline = b_receiver = buff_bill = buf_resline = bbill = bqueasy = art1 = None
    ...
    def generate_output():
        ...
        nonlocal res_member, receiver, res_sharer, res_line1, rline, b_receiver, buff_bill, buf_resline, bbill, bqueasy, art1
        nonlocal res_member, receiver, res_sharer, res_line1, rline, b_receiver, buff_bill, buf_resline, bbill, bqueasy, art1

        return {
            "new_resstatus": new_resstatus,
            "checked_in": checked_in,
            "ask_deposit": ask_deposit,
            "ask_keycard": ask_keycard,
            "ask_mcard": ask_mcard,
            "msg_str": msg_str
        }
    ...
    def calculate_deposit_amount():
        ...
        if not artikel.pricetab:
            deposit = - to_decimal(reservation.depositbez) - \
                to_decimal(reservation.depositbez2)
            ...
            if reservation.zahldatum == bill_date:
                if waehrung:
                    deposit_exrate = to_decimal(
                        waehrung.ankauf) / to_decimal(waehrung.einheit)
            else:
                exrate = get_cache(
                    Exrate, {"artnr": [(eq, artikel.betriebsnr)], "datum": [(eq, reservation.zahldatum)]})
                if exrate:
            ...
            if reservation.depositbez2 != 0:
                ...
                if reservation.zahldatum == bill_date:
                    ...
                    exrate = get_cache(
                        Exrate, {"artnr": [(eq, artikel.betriebsnr)], "datum": [(eq, reservation.zahldatum2)]})
                    if exrate:
                        deposit_exrate = to_decimal(exrate.betrag)
                    elif waehrung:
                        deposit_exrate = to_decimal(
                            waehrung.ankauf) / to_decimal(waehrung.einheit)
            deposit = to_decimal(
                deposit) - to_decimal(reservation.depositbez2) * to_decimal(deposit_exrate)
    ...
    def check_messages():
        ...
        messages = get_cache(
            Messages, {"resnr": [(eq, resnr)], "reslinnr": [(eq, reslinnr)]})

        if messages:
            get_output(
                intevent_1(4, res_line.zinr, "Message Lamp on!",res_line.resnr, res_line.reslinnr))

            if not silenzio:
                msg_str = msg_str + chr_unicode(2) + translateExtended(
                    "Message(s) exist for this guest.", lvcarea, "") + chr_unicode(10)
    ...
    def check_midnite_checkin():
        ...
        if htparam.fdate == get_current_date() and not silenzio:
            msg_str = msg_str + chr_unicode(2) + translateExtended(
                "EARLY CHECKED-IN GUEST! POST DAY-USE FEE IF NEEDED.", lvcarea, "") + chr_unicode(10)
    ...
    def assign_zinr(resline_recid: int, ankunft: date, abreise: date, zinr: string, resstatus: int, gastnrmember: int, bemerk: string, name: string):
        ...
        if zinr != "" and not sharer:
            ...
            for curr_datum in date_range(beg_datum, (abreise - timedelta(days=1))):
                ...
                if (not zimplan1) and (not room_blocked):
                    ...
                    if zimplan1 and (zimplan1.res_recid != resline_recid):
                        resline = get_cache(
                            Res_line, {"_recid": [(eq, zimplan1.res_recid)]})
            ...
            if room_blocked:
                ...
                for curr_datum in date_range(beg_datum, (abreise - timedelta(days=1))):
                    zimplan = get_cache(
                        Zimplan, {"datum": [(eq, curr_datum)], "zinr": [(eq, zinr)], "res_recid": [(eq, resline_recid)]})
        ...
        if rline.zinr != "":
            ...
            if res_mode == ("inhouse"):
                ...
                if rline.resstatus == 6 and (rline.zinr != (new_zinr)):
                    res_line1 = get_cache(
                        Res_line, {"resnr": [(eq, resnr)], "zinr": [(eq, rline.zinr)], "resstatus": [(eq, 13)]})
    ...
    if buf_resline:
        msg_str = translateExtended(
            "Guest already checkin by another user.", lvcarea, "")
    ...
    if not res_line:
        ...
        if htparam.feldtyp == 4 and htparam.flogical and bill and bill.rechnr == 0:
            # ITA: Program terkait feature service apartement
            queasy = get_cache(
                Queasy, {"key": [(eq, 329)], "number1": [(eq, res_line.resnr)], "number2": [(eq, res_line.reslinnr)]})
            ...
            if queasy:
                ...
                for loopi in range(1, 3 + 1):
                    ...
                    if loopi == 3:
                        bqueasy = get_cache(
                            Queasy, {"key": [(eq, 349)], "number1": [(eq, res_line.resnr)], "number2": [(eq, res_line.reslinnr)]})

                        if bqueasy:
                            bill_line = Bill_line()
                            db_session.add(bill_line)

                            bill_line.rechnr = bbill.rechnr
                            bill_line.artnr = art_security
                            bill_line.anzahl = 1
                            bill_line.betrag = to_decimal(bqueasy.deci1)
                            ...
                            bill_line.rechnr = bbill.rechnr
                            bill_line.artnr = bqueasy.number3
                            bill_line.anzahl = 1
                            bill_line.betrag = - to_decimal(bqueasy.deci1)
                            bill_line.departement = 0
                            bill_line.zeit = get_current_time_in_seconds()
                            ...
                            artikel = get_cache(
                                Artikel, {"artnr": [(eq, bqueasy.number3)], "departement": [(eq, 0)]})
                            bill_line.bezeich = artikel.bezeich

                            billjournal = Billjournal()
                            db_session.add(billjournal)

                            billjournal.artnr = art_security
                            billjournal.anzahl = 1
                            billjournal.fremdwaehrng = - \
                                to_decimal(bqueasy.deci1)
                            billjournal.betrag = - to_decimal(bqueasy.deci1)
                            billjournal.bezeich = artikel.bezeich + \
                                "[" + "Security Leasing #" + \
                                to_string(bqueasy.number1) + "]"
                            billjournal.epreis = to_decimal("0")
                            billjournal.zeit = get_current_time_in_seconds()
                            billjournal.billjou_ref = bqueasy.number1
                            ...
                            umsatz.betrag = to_decimal(
                                umsatz.betrag) - to_decimal(bqueasy.deci1)
            ...
            if not artikel:
                if not silenzio:
                    msg_str = msg_str + chr_unicode(2) + \
                        translateExtended("deposit article not defined, deposit transfer", lvcarea, "") + chr_unicode(10) + \
                        translateExtended("to the guest bill not possible!", lvcarea, "") + chr_unicode(10)
            else:
                answer = True
                ...
                if htparam.paramgruppe == 6 and htparam.flogical and not silenzio:
                    res_member = get_cache(Res_line, {"resnr": [(eq, resnr)], "reslinnr": [(
                        ne, reslinnr)], "active_flag": [(eq, 0)], "l_zuordnung[2]": [(eq, 0)]})

                    if res_member and htparam.paramgruppe == 6 and htparam.flogical and not silenzio:
                        answer = False
                        ask_deposit = True
                        msg_str = msg_str + chr_unicode(2) + "&Q" + translateExtended(
                            "Transfer deposit amount to the bill NOW?", lvcarea, "") + chr_unicode(10)
                ...
                if answer:
                    ...
                    umsatz.anzahl = umsatz.anzahl + 1
                    umsatz.betrag = to_decimal(
                        umsatz.betrag) + to_decimal(deposit)
        ...
        if htparam.flogical:
            get_output(
                intevent_1(1, res_line.zinr, "My Checkin!",res_line.resnr, res_line.reslinnr))
        else:
            queasy = db_session.query(Queasy).filter(
        ...
        if priscilla_active:
            get_output(
                intevent_1(1, res_line.zinr, "Priscilla",res_line.resnr, res_line.reslinnr))
        check_messages()
        checked_in = True
        ...
        if not silenzio:
            res_sharer = get_cache(Res_line, {"resnr": [(eq, res_line.resnr)], "reslinnr": [(
                ne, res_line.reslinnr)], "resstatus": [(eq, 11)], "zinr": [(eq, res_line.zinr)]})

            if res_sharer:
                msg_str = msg_str + chr_unicode(2) + \
                    translateExtended("NOTE: Room sharer", lvcarea, "") + " " + res_sharer.name + " " + \
                    translateExtended("not yet checked-in.", lvcarea, "") + chr_unicode(10)
            generate_keycard()

            if mc_flag and not silenzio:
                ask_mcard = True
            msg_str = msg_str + chr_unicode(2) + "&M" + translateExtended(
                "Guest checked-in.", lvcarea, "") + chr_unicode(10)
