...
from models import Reslin_queasy, Waehrung, Htparam, Res_line, Bediener, Arrangement, Queasy, Guest_pr, Ratecode, Pricecod, Katpreis
...
def chk_btn_go_res_rmrate_webbl(pvilanguage: int, curr_select: str, max_rate: Decimal, fact1: Decimal, inp_wahrnr: int, inp_zikatnr: int, user_init: str, resnr: int, reslinnr: int, recid_reslin: int, contcode: str, repeat_charge: bool, p_list_data: P_list):
    ...
    bill_date: date = None
    co_date: date = None
    reslin_queasy = waehrung = htparam = res_line = bediener = arrangement = queasy = guest_pr = ratecode = pricecod = katpreis = None
    ...
    def generate_output():
        nonlocal msg_str, error_found1, error_code, t_reslin_queasy_data, lvcarea, error_found, amount_periode, month_str1, month_str2, exrate2, wd_array, bill_date, co_date, reslin_queasy, waehrung, htparam, res_line, bediener, arrangement, queasy, guest_pr, ratecode, pricecod, katpreis
    ...
    def proc_repeat():
        ...
        if curr_select.lower() == "add":
            if res_line.active_flag == 0:
                reslin_queasy = get_cache(Reslin_queasy, {"key": [(
                    eq, "fixrate-trace-record")], "resnr": [(eq, resnr)], "reslinnr": [(eq, reslinnr)]})
    ...
    if curr_select.lower() == "add":
        ...
        if brq:
            msg_str = msg_str + chr_unicode(
                    2) + translateExtended("Overlapping date found.", lvcarea, "")
