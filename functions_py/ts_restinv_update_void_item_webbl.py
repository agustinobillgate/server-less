...
def ts_restinv_update_void_item_webbl(pvilanguage: int, rec_id: int, rec_id_h_artikel: int, deptname: string,
    ...
    prepare_cache([H_bill_line, Kellner, H_mjourn, H_artikel, Hoteldpt, Artikel, Htparam, Queasy,
                  Guest, Counters, H_umsatz, H_journal, Umsatz, Interface, Arrangement, Argt_line, Billjournal])
    ...
    curr_time: int = 0
    h_bill = h_bill_line = kellner = h_mjourn = h_artikel = hoteldpt = artikel = htparam = queasy = guest = counters = h_umsatz = h_journal = umsatz = interface = arrangement = argt_line = billjournal = None

    t_kellner1 = t_h_bill = submenu_list = h_bline = kellner1 = bbill = mjou = bill_guest = None

    t_kellner1_data, T_kellner1 = create_model(
        "T_kellner1",
        {
            "departement": int,
            "kumsatz_nr": int,
    ...
    def rev_bdown():
        ...
        arrangement = get_cache(
            Arrangement, {"argtnr": [(eq, artikel.artgrp)]})
            ...
            if argt_line.betrag != 0:
                ...
                if double_currency or artikel.pricetab:
                    argt_betrag = to_decimal(
                        round(argt_betrag * exchg_rate, price_decimal))
            else:
                argt_betrag = to_decimal(
                    amount) * to_decimal(argt_line.vt_percnt) / to_decimal("100")
                argt_betrag = to_decimal(round(argt_betrag, price_decimal))
            rest_betrag = to_decimal(rest_betrag) - to_decimal(argt_betrag)

            artikel1 = get_cache(Artikel, {"artnr": [
                                 (eq, argt_line.argt_artnr)], "departement": [(eq, argt_line.departement)]})

            # umsatz = get_cache(Umsatz, {"artnr": [(eq, artikel1.artnr)], "departement": [
            #                    (eq, artikel1.departement)], "datum": [(eq, bill_date)]})
        ...
        artikel1 = get_cache(Artikel, {"artnr": [
                             (eq, arrangement.artnr_logis)], "departement": [(eq, arrangement.intervall)]})
        ...
        if double_currency:
            billjournal.fremdwaehrng = to_decimal(
                round(rest_betrag / exchg_rate, 2))
        pass
    ...
    def update_selforder():
        ...
        for searchbill in db_session.query(Searchbill).filter(
                (Searchbill.key == 225) & (Searchbill.number1 == curr_dept) & (Searchbill.char1 == ("orderbill"))).order_by(Searchbill._recid).all():
        ...
        if paramqsy:
            ...
            if dynamic_qr:
                ...
                pickup_table = db_session.query(Pickup_table).filter(
                    (Pickup_table.key == 225) & (Pickup_table.char1 == ("taken-table")) & (Pickup_table.number1 == curr_dept) & (Pickup_table.logi1) & (Pickup_table.logi2) & (Pickup_table.number2 == paramqsy.number2) & (entry(0, Pickup_table.char3, "|") == (session_parameter))).first()

                if pickup_table:
                    pass
                    pickup_table.char3 = entry(0, pickup_table.char3, "|", session_parameter + "T" + replace_str(to_string(
                        get_current_date()), "/", "") + replace_str(to_string(get_current_time_in_seconds(), "HH:MM"), ":", ""))

                    pass
                    pass

            # orderbill = get_cache(Queasy, {"key": [(eq, 225)], "char1": [(eq, "orderbill")], "char3": [
            #                       (eq, session_parameter)], "logi1": [(eq, True)], "logi3": [(eq, True)]})
            orderbill = db_session.query(Orderbill).filter(
                (Orderbill.key == 225) & (Orderbill.char1 == ("orderbill")) & (Orderbill.char3 == (session_parameter)) & (Orderbill.logi1) & (Orderbill.logi3)).first()
            if orderbill:
                pass
                orderbill.deci1 = to_decimal(get_amount)
                orderbill.logi2 = False
                orderbill.char3 = session_parameter + "T" + replace_str(to_string(get_current_date(
                )), "/", "") + replace_str(to_string(get_current_time_in_seconds(), "HH:MM"), ":", "")
                orderbill.logi1 = False
                pass

                # orderbill_close = get_cache(Queasy, {"key": [(eq, 225)], "char1": [(eq, "orderbill")], "char3": [
                #                             (eq, session_parameter)], "logi1": [(eq, True)], "logi3": [(eq, True)]})
                orderbill_close = db_session.query(Orderbill_close).filter(
                    (Orderbill_close.key == 225) & (Orderbill_close.char1 == ("orderbill")) & 
                    (Orderbill_close.char3 == (session_parameter)) & (Orderbill_close.logi1) & (Orderbill_close.logi3)).with_for_update().first()
                while None != orderbill_close:
                    pass
                    orderbill_close.char3 = session_parameter + "T" + replace_str(to_string(get_current_date(
                    )), "/", "") + replace_str(to_string(get_current_time_in_seconds(), "HH:MM"), ":", "")
                    ...
                    orderbill_close = db_session.query(Orderbill_close).filter(
                        (Orderbill_close.key == 225) & (Orderbill_close.char1 == ("orderbill")) & (Orderbill_close.char3 == (session_parameter)) & 
            ...
            if dynamic_qr:
                ...
                if room_serviceflag:
                    paramqsy.char1 = session_parameter + "T" + replace_str(to_string(get_current_date(
                    )), "/", "") + replace_str(to_string(get_current_time_in_seconds(), "HH:MM"), ":", "")
                    paramqsy.char3 = paramqsy.char3 + \
                        "|BL=" + to_string(get_rechnr)
                    paramqsy.logi1 = True
                    ...
                    queasy.char1 = session_parameter + "T" + replace_str(to_string(get_current_date(
                    )), "/", "") + replace_str(to_string(get_current_time_in_seconds(), "HH:MM"), ":", "")
                    queasy.betriebsnr = 1
                    queasy.logi1 = True

                orderbilline = db_session.query(Orderbilline).filter(
                    (Orderbilline.key == 225) & (Orderbilline.char1 == ("orderbill-line")) & (entry(3, Orderbilline.char2, "|") == (session_parameter))).with_for_update().first()
                while None != orderbilline:
                    pass

                    if orderbilline.logi2 and orderbilline.logi3:
                        orderbilline.char2 = entry(0, orderbilline.char2, "|") + "|" + entry(1, orderbilline.char2, "|") + "|" + entry(2, orderbilline.char2, "|") + "|" + \
                            session_parameter + "T" + replace_str(to_string(get_current_date()), "/", "") + replace_str(
                    ...
                    orderbilline = db_session.query(Orderbilline).filter(
                        (Orderbilline.key == 225) & (Orderbilline.char1 == ("orderbill-line")) & (entry(3, Orderbilline.char2, "|") == (session_parameter)) & 
                        (Orderbilline._recid > curr_recid)).with_for_update().first()

            # qpayment_gateway = get_cache(Queasy, {"key": [(eq, 223)], "char3": [
            #                              (eq, session_parameter)], "betriebsnr": [(eq, get_rechnr)]})
    ...
    def remove_rsv_table():
        ...
        queasy = get_cache(
            Queasy, {"key": [(eq, 251)], "number1": [(eq, recid_hbill)]})
    ...
    if h_artikel:
        ...
        if servtax_use_foart:
            ...
            artikel = get_cache(Artikel, {"artnr": [
                                (eq, h_artikel.artnrfront)], "departement": [(eq, h_artikel.departement)]})
    ...
    if h_bill and h_artart == 0:
        ...
        h_bill_line = get_cache(H_bill_line, {"rechnr": [(eq, h_bill.rechnr)], "departement": [
                                (eq, h_bill.departement)], "bill_datum": [(ne, bill_date)]})
    ...
    if cancel_order:
        ...
        h_bline = get_cache(
            H_bill_line, {"_recid": [(eq, h_bill_line._recid)]})
    ...
    if h_artart == 0:
        ...
        subtotal_foreign = to_decimal(
            subtotal_foreign) + to_decimal(nett_amount_foreign)
    ...
    if not htparam.flogical and h_artart == 0 and h_artikel and serv_code != 0:
        ...
        if htparam and htparam.fdecimal != 0:
            ...
            if num_entries(htparam.fchar, chr_unicode(2)) >= 2:
                service = to_decimal(to_decimal(
                    entry(1, htparam.fchar, chr_unicode(2)))) / to_decimal("10000")
        ...
        if htparam and htparam.fdecimal != 0:
            ...
            if serv_vat and not tax_vat:
                vat = to_decimal(vat) + to_decimal(vat) * \
                    to_decimal(service) / to_decimal("100")

            elif serv_vat and tax_vat:
                vat = to_decimal(vat) + to_decimal(vat) * \
                    to_decimal((service) + to_decimal(vat2)) / \
                    to_decimal("100")

            elif not serv_vat and tax_vat:
                vat = to_decimal(vat) + to_decimal(vat) * \
                    to_decimal(vat2) / to_decimal("100")
    ...
    if h_artart == 0:
        ...
        fact_scvat = to_decimal("1") + to_decimal(service) + \
            to_decimal(vat) + to_decimal(vat2)
        ...
        if double_currency:
            ...
            service_foreign = to_decimal(
                service_foreign) + to_decimal(h_service_foreign) * to_decimal(qty)
        ...
        if double_currency:
            ...
            h_mwst = to_decimal(tax) * to_decimal(unit_price) * \
                to_decimal(exchg_rate)
        ...
        mwst_foreign = to_decimal(mwst_foreign) + \
            to_decimal(h_mwst_foreign) * to_decimal(qty)
    ...
    if h_bill:
        ...
        # queasy = get_cache(Queasy, {"key": [(eq, 31)], "number1": [
        #                    (eq, curr_dept)], "number2": [(eq, tischnr)]})
        ...
        if hoga_reslinnr == 0 and gname != "" and gname != None:
            ...
            if not guest:
                ...
                guest = db_session.query(Guest).filter(
                    ((Guest.name + "," + Guest.vorname1 + " " + Guest.anrede1 == (gname))) | ((Guest.name + "," + Guest.vorname1 + " " + Guest.anrede1 == (gname))) | ((Guest.name + "," + Guest.vorname1 + " " + Guest.anrede1 == (gname))) | ((Guest.name + "," + " " + Guest.anrede1 == (gname))) | ((Guest.name + "," + " " + Guest.anrede1 == (gname))) | ((Guest.name + "," + " " + Guest.anrede1 == (gname))) | ((Guest.name + "," + Guest.vorname1 + " " + Guest.anredefirma == (gname))) | ((Guest.name + "," + Guest.vorname1 + " " + Guest.anredefirma == (gname))) | ((Guest.name + "," + Guest.vorname1 + " " + Guest.anredefirma == (gname))) | ((Guest.name + "," + " " + Guest.anredefirma == (gname))) | ((Guest.name + "," + " " + Guest.anredefirma == (gname))) | ((Guest.name + "," + " " + Guest.anredefirma == (gname)))).first()
    ...
    kellner1 = get_cache(Kellner, {"kellner_nr": [
                         (eq, h_bill.kellner_nr)], "departement": [(eq, curr_dept)]})

    if kellner1:
        t_kellner1 = T_kellner1()
        t_kellner1_data.append(t_kellner1)

        t_kellner1.departement = kellner1.departement
        t_kellner1.kumsatz_nr = kellner1.kumsatz_nr
        t_kellner1.kcredit_nr = kellner1.kcredit_nr
        t_kellner1.kzahl_nr = kellner1.kzahl_nr
        t_kellner1.kellnername = kellner1.kellnername
        t_kellner1.updatedatum = kellner1.updatedatum
        t_kellner1.saldo = to_decimal(kellner1.saldo)
        t_kellner1.kellner_nr = kellner1.kellner_nr
        t_kellner1.sprachcode = kellner1.sprachcode
        t_kellner1.masterkey = kellner1.masterkey
        t_kellner1.nullbon = kellner1.nullbon
        t_kellner1.ignore_pers = kellner1.ignore_pers
        t_kellner1.kel_unique = kellner1.kel_unique
        t_kellner1.storno_begruendung = kellner1.storno_begruendung
        t_kellner1.betriebsnr = kellner1.betriebsnr

    if h_artart == 0:
        h_bill.gesamtumsatz = to_decimal(
            h_bill.gesamtumsatz) + to_decimal(amount)
    ...
    if billart != 0:
        ...
        # h_umsatz = get_cache(H_umsatz, {"artnr": [(eq, billart)], "departement": [
        #                      (eq, curr_dept)], "datum": [(eq, bill_date)]})
    ...
    if h_artart == 11:
        ...
        if mc_str != " ":
            ...
            # queasy = get_cache(Queasy, {"key": [(eq, 197)], "char1": [
            #                    (eq, mc_str)], "date1": [(eq, bill_date)], "number1": [(eq, billart)]})
            queasy = db_session.query(Queasy).filter(
    ...
    if h_artikel and h_artikel.artart == 0 and h_artikel.betriebsnr != 0:
        ...
        if not cancel_order:
            ...
            for submenu_list in query(submenu_list_data, filters=(lambda submenu_list: submenu_list.nr == h_artikel.betriebsnr)):
                ...
                h_mjourn.zeit = zeit
                h_mjourn.request = to_string(
                    recid_h_bill_line) + "|" + submenu_list.request
            ...
            h_journal = get_cache(H_journal, {"artnr": [(eq, h_bline.artnr)], "departement": [(eq, h_bline.departement)], "rechnr": [(
                eq, h_bline.rechnr)], "bill_datum": [(eq, h_bline.bill_datum)], "zeit": [(eq, h_bline.zeit)], "sysdate": [(eq, h_bline.sysdate)]})
    ...
    if h_artart == 0:
        ...
        # umsatz = get_cache(Umsatz, {"artnr": [(eq, h_artikel.artnrfront)], "departement": [
        #                    (eq, curr_dept)], "datum": [(eq, bill_date)]})
        umsatz = db_session.query(Umsatz).filter(
        ...
        artikel = get_cache(
            Artikel, {"artnr": [(eq, h_artnrfront)], "departement": [(eq, curr_dept)]})
    ...
    if h_artart == 2 or h_artart == 7:
        ...
        artikel = get_cache(
            Artikel, {"artnr": [(eq, h_artikel.artnrfront)], "departement": [(eq, 0)]})

        if foreign_rate and amount_foreign == 0:
            amount_foreign = to_decimal(amount) / to_decimal(exchg_rate)
        get_output(ts_restinv_rinv_arbl(artikel.artnr, curr_dept, curr_room, bill_guest.gastnr, bill_guest.gastnr,
                   h_bill.rechnr, amount, amount_foreign, bill_date, bill_guest.name, user_init, cc_comment, deptname))
    ...
    if h_artart == 2 or h_artart == 7 or h_artart == 11 or h_artart == 12:
        ...
        if balance == 0:
            ...
            for h_bill_line in db_session.query(H_bill_line).filter(
                ...
                h_artikel = get_cache(H_artikel, {"departement": [(eq, h_bill_line.departement)], "artnr": [
                                      (eq, h_bill_line.artnr)], "artart": [(ne, 0)]})

                if h_artikel:
                    get_amount = to_decimal(
                        get_amount) + to_decimal(h_bill_line.betrag)
