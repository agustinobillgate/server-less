...
def _3month_fsegment_webbl(from_date:date):
    ...
    output_list_data, Output_list = create_model("Output_list", {"segmentcode":int, "segment":string, "monthyear":date, "room":int, "revenue":Decimal, "avrg_rev":Decimal, "pax":int, "occpr":Decimal, "doccpr":Decimal})
                ...
                if segment and do_it:
                    ...
                    for datum in date_range(datum1,datum2) :
                        ...
                        if datum == res_line.abreise:
                            ...
                            output_list = query(output_list_data, filters=(lambda output_list: output_list.segmentcode == reservation.segmentcode and output_list.monthyear == date_mdy(get_month(datum) , 1, get_year(datum))), first=True)
    ...
    if get_month(fr_date) >= 10:
        ...
        to_date = date(get_year(from_date) + 1, 3, 1) - timedelta(days=1)
    else:
        # Rd 31/7/2025
    ...
    for output_list in query(output_list_data, sort_by=[("segmentcode",False)]):
        ...
        if output_list.segmentcode != 99999:
            ...
            boutput = query(boutput_data, filters=(lambda boutput: boutput.segmentcode == 99999 and boutput.monthyear == output_list.monthyear), first=True)
