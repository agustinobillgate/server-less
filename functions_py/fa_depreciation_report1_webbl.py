...
payload_list_data, Payload_list = create_model("Payload_list", {"from_date":string, "to_date":string, "sorttype":int, "last_nr":int, "mode":int})
...
def fa_depreciation_report1_webbl(payload_list_data:[Payload_list]):
    ...
    output_list_data, Output_list = create_model("Output_list", {"curr_nr":int, "curr_loc":int})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal depreciation_asset_data, tot_amount, tot_depn_value, tot_book_value, tot_acc_depn, datum, qty, price, amount, counter, counter_num_data, mm, yy, mathis, queasy, fa_artikel, fa_order, fa_ordheader, fa_op


        nonlocal depreciation_asset, payload_list, output_list
        nonlocal depreciation_asset_data, output_list_data

        return {"depreciation-asset": depreciation_asset_data}
    ...
    mm = to_int(substring(payload_list.from_date, 0, 2))
    yy = to_int(substring(payload_list.from_date, 2, 4))
