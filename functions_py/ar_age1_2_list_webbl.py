...
def ar_age1_2_list_webbl(pvilanguage:int, case_type:int, mtd_flag:bool, to_date:date, fdate:date, tdate:date, from_art:int, to_art:int, from_name:string, to_name:string, disptype:int, mi_bill:bool, day1:int, day2:int, day3:int, detailed:bool, cvt_flag:bool, dollar_rate:Decimal):
    ...
    set_cache(Artikel, ((Artikel.artart == 2) | (Artikel.artart == 7)) & (Artikel.artnr >= from_art) & (Artikel.artnr <= to_art) & (Artikel.departement == 0),[["artnr", "departement"]], True,[],["artnr"])
    set_cache(Debitor, (Debitor.rechnr > 0) & (Debitor.rgdatum <= to_date) & (((Debitor.artnr >= from_art) & (Debitor.artnr <= to_art) & (Debitor.opart <= 1)) | (Debitor.artnr.in_(get_cache_value_list(Artikel, "artnr")) & ((Debitor.opart == 2) & (Debitor.zahlkonto == 0)))),[], True,[],["rechnr"])
    set_cache(H_bill, (H_bill.rechnr.in_(get_cache_value_list(Debitor,"rechnr"))),[["rechnr"]], True,[],[])
    set_cache(Bill, (Bill.rechnr.in_(get_cache_value_list(Debitor,"rechnr"))),[["rechnr"]], True,[],["resnr"])
    set_cache(Res_line, (Res_line.resnr.in_(get_cache_value_list(Bill, "resnr"))),[["resnr"]], True,[],[])
    ...
    def age_list1():
        ...
        for debitor.artnr, debitor.counter, debitor.gastnr, debitor.rechnr, debitor.rgdatum, debitor.opart, debitor.betrieb_gastmem, debitor.saldo, debitor.vesrdep, debitor.zahlkonto, debitor.gastnrmember, debitor.name, debitor._recid, artikel.artnr, artikel.bezeich, artikel.artart, artikel._recid in db_session.query(Debitor.artnr, Debitor.counter, Debitor.gastnr, Debitor.rechnr, Debitor.rgdatum, Debitor.opart, Debitor.betrieb_gastmem, Debitor.saldo, Debitor.vesrdep, Debitor.zahlkonto, Debitor.gastnrmember, Debitor.name, Debitor._recid, Artikel.artnr, Artikel.bezeich, Artikel.artart, Artikel._recid).join(Artikel,((Artikel.artart == 2) | (Artikel.artart == 7)) & (Artikel.artnr == Debitor.artnr) & (Artikel.departement == 0)).filter(
            ...
            if debitor.zahlkonto == 0:
                ...
                if debitor.name.lower()  >= (from_name).lower() :
                    ...
                    age_list = query(age_list_data, filters=(lambda age_list: age_list.rechnr == debitor.rechnr and age_list.counter == debitor.counter), first=True)
        ...
        if ar_ledger != 0:
            ...
            for age_list in query(age_list_data, filters=(lambda age_list: age_list.artnr == ar_ledger)):
                ...
                tage_list = query(tage_list_data, filters=(lambda tage_list: tage_list.rechnr == age_list.rechnr), first=True)
                ...
                if not tage_list:
                    ...
                    tage_list.p_bal =  to_decimal(tage_list.p_bal) + to_decimal(age_list.p_bal)
                    tage_list.debit =  to_decimal(tage_list.debit) + to_decimal(age_list.debit)
                    tage_list.credit =  to_decimal(tage_list.credit) + to_decimal(age_list.credit)
                    tage_list.saldo =  to_decimal(tage_list.saldo) + to_decimal(age_list.saldo)
                    tage_list.debt0 =  to_decimal(tage_list.debt0) + to_decimal(age_list.debt0)
                    tage_list.debt1 =  to_decimal(tage_list.debt1) + to_decimal(age_list.debt1)
                    tage_list.debt2 =  to_decimal(tage_list.debt2) + to_decimal(age_list.debt2)
                    tage_list.debt3 =  to_decimal(tage_list.debt3) + to_decimal(age_list.debt3)
                    tage_list.tot_debt =  to_decimal(tage_list.tot_debt) + to_decimal(age_list.tot_debt)
    ...
    for arage_list in query(arage_list_data):
        ...
        if substring(arage_list.str, 0, 7) != ("-------").lower() :
            ...
            arage_balance_list.bill_nm = substring(arage_list.str, 37, 30)
            arage_balance_list.p_bal = substring(arage_list.str, 85, 18)
            arage_balance_list.debit = substring(arage_list.str, 103, 18)
            arage_balance_list.credit = substring(arage_list.str, 121, 18)
            arage_balance_list.end_bal = substring(arage_list.str, 67, 18)
            arage_balance_list.creditlimit_str = substring (arage_list.str, 211, 18)
