...
"""_yusufwijasena_30/10/2025
    ...
    _yusufwijasena_12/12/2025
        _remark_:   - fix assignment output_list.str value 
                    - fix billjournal.rechnr format to >>9999999
                    - use handling_negative to quantity & amount
"""
...
from functions.more_additional_functions import handling_negative
...
def fo_usrjournal_cld_1bl(mi_incl: bool, mi_excl: bool, mi_tran: bool, from_date: date, to_date: date, from_dept: int, to_dept: int, from_art: int, to_art: int, usr_init: str, long_digit: bool, foreign_flag: bool, sort_type: int):
    ...
    def journal_list1():
        ...
        for curr_date in date_range(from_date, to_date):
            for billjournal in db_session.query(Billjournal).filter(
                    (Billjournal.userinit == (usr_init).lower()) &
                    (Billjournal.bill_datum == curr_date) &
                    (Billjournal.departement >= from_dept) &
                    (Billjournal.departement <= to_dept) &
                    (Billjournal.artnr >= from_art) &
                    (Billjournal.artnr <= to_art)).order_by(Billjournal.departement, Billjournal.artnr, Billjournal.zeit).all():
                ...
                if last_artnr != billjournal.artnr or last_dept != billjournal.departement:
                    ...
                    if not long_digit:
                        output_list.str = to_string("", "x(87)") + \
                            to_string("T O T A L  ", "x(22)") + \
                            handling_negative(a_qty, "-9999") + \
                            handling_negative(a_tot, "->,>>>,>>>,>>9.99")
                    else:
                        output_list.str = to_string("", "x(87)") + \
                            to_string("T O T A L  ", "x(22)") + \
                            handling_negative(a_qty, "-9999") + \
                ...
                output_list_data.append(output_list)
                if not long_digit:
                    output_list.str = to_string(billjournal.bill_datum) + \
                        to_string(billjournal.zinr, "x(6)") + \
                        to_string(billjournal.rechnr, ">>9999999") + \
                        to_string(billjournal.artnr, "9999") + \
                        to_string(substring(billjournal.bezeich, 0, 60), "x(60)") + \
                        to_string(hoteldpt.depart, "x(22)") + \
                        handling_negative(billjournal.anzahl, "-9999") + \
                        handling_negative(amount, "->,>>>,>>>,>>9.99") + \
    ...
    def journal_list2():
        ...
        output_list_data.append(output_list)
        if not long_digit:
            output_list.str = to_string("", "x(87)") + to_string("T O T A L  ", "x(22)") + \
                handling_negative(a_qty, "-9999") + \
                handling_negative(a_tot, "->,>>>,>>>,>>9.99")
        else:
            output_list.str = to_string("", "x(87)") + to_string("T O T A L  ", "x(22)") + \
        ...
        for curr_date in date_range(from_date, to_date):
            for billjournal in db_session.query(Billjournal).filter(
                ...
                if last_artnr != billjournal.artnr or last_dept != billjournal.departement:
                    ...
                    if not long_digit:
                        output_list.str = to_string("", "x(87)") + to_string("T O T A L  ", "x(22)") + to_string(
