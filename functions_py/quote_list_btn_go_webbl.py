...
def quote_list_btn_go_webbl(pvilanguage:int, curr_type:string, user_init:string, 
                            base64image:string, t_quote_data:[T_quote], t_quote1_data:[T_quote1]):
    ...
    if curr_type.lower()  == ("new").lower() :
        ...
        # l_quote = get_cache (L_quote, {"artnr": [(eq, t_quote1.artnr)],"lief_nr": [(eq, t_quote1.lief_nr)],
        # "docu_nr": [(eq, t_quote1.docu_nr)],"from_date": [(eq, t_quote1.from_date)],
        # "to_date": [(eq, t_quote1.to_date)],"unitprice": [(eq, t_quote1.unitprice)],
        # "reserve_char[0]": [(eq, t_quote1.curr)],"remark": [(eq, t_quote1.remark)],
        # "filname": [(eq, t_quote1.filname)],"activeflag": [(eq, t_quote1.activeflag)]})
        l_quote = db_session.query(L_quote).filter(
                 (L_quote.artnr == t_quote1.artnr) &
                 (L_quote.lief_nr == t_quote1.lief_nr) &
                 (L_quote.docu_nr == t_quote1.docu_nr) &
                 (L_quote.from_date == t_quote1.from_date) &
                 (L_quote.to_date == t_quote1.to_date) &
                 (L_quote.unitprice == t_quote1.unitprice) &
                 (L_quote.reserve_char[0] == t_quote1.curr) &
                 (L_quote.remark == t_quote1.remark) &
                 (L_quote.filname == t_quote1.filname) &
                 (L_quote.activeflag == t_quote1.activeflag)).with_for_update().first()
        if l_quote:
        ...
        # l_quote = get_cache (L_quote, {"artnr": [(eq, t_quote.artnr)],"lief_nr": [(eq, t_quote.lief_nr)],
        #                                "from_date": [(eq, t_quote.from_date)],"to_date": [(eq, t_quote.to_date)]})
        l_quote = db_session.query(L_quote).filter(
