...
def dml_list_save_it_1blold(curr_dept:int, cbuff_artnr:int, cbuff_qty:Decimal, selected_date:date, user_init:string, cbuff_price:Decimal, cbuff_lief_nr:int, cbuff_approved:bool, cbuff_remark:string):
    ...
    if curr_dept == 0:
        ...
        dml_art = db_session.query(Dml_art).filter(
                 (Dml_art.artnr == cbuff_artnr) & (Dml_art.datum == selected_date)).with_for_update().first()
        ...
        if cbuff_approved:
            ...
            queasy = get_cache (Queasy, {"key": [(eq, 254)],"number1": [(eq, 0)],"date1": [(eq, dml_art.datum)],"logi1": [(eq, True)],"logi2": [(eq, False)]})
        ...
        if cbuff_approved:
            ...
            queasy = get_cache (Queasy, {"key": [(eq, 254)],"number1": [(eq, dml_artdep.departement)],"date1": [(eq, dml_artdep.datum)],"logi1": [(eq, True)],"logi2": [(eq, False)]})
