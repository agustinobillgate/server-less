...
def leasing_print_proforma_invoice_1bl(qrecid: int, user_init: str):
    ...
    t_bill_line = periode_list = header_list = mbill_line = bbill_line = cbill_line = bguest = bqueasy = breslin_queasy = treslin_queasy = pqueasy = preslin = None
    ...
    Preslin = create_buffer("Preslin", Reslin_queasy)
    ...
    def generate_output():
        ...
        nonlocal mbill_line, bbill_line, cbill_line, bguest, bqueasy, breslin_queasy, treslin_queasy, pqueasy, preslin
        nonlocal t_bill_line, periode_list, header_list, mbill_line, bbill_line, cbill_line, bguest, bqueasy, breslin_queasy, treslin_queasy, pqueasy, preslin
    ...
    if queasy:
        ...
        if not header_list:
            ...
            preslin = get_cache(
                Reslin_queasy, {"key": [(eq, "actual-invoice")], "resnr": [(eq, curr_resnr)], "reslinnr": [(eq, curr_reslinnr)], "betriebsnr": [(eq, qrecid)], "char1": [(eq, "")]})

            if preslin:
                header_list.pinvoice_no = header_list.pinvoice_no + "-" + to_string(preslin.number1)
            ...
            for billjournal in db_session.query(Billjournal).filter(
                    (matches(Billjournal.bezeich, "*Payment Leasing*")) & (to_int(entry(0, entry(1, Billjournal.bezeich, "#"), "]")) == queasy.number1) & (Billjournal.kassabuch_nr == qrecid) & (Billjournal.billin_nr == 1)).order_by(Billjournal._recid).all():
                header_list.payment = to_decimal(
                    header_list.payment) + (- to_decimal(billjournal.betrag))
                                            ...
                                            if breslin_queasy:
                                                for breslin_queasy in db_session.query(Breslin_queasy).filter(
                                                    ...
                                                    if argt_rate > 0:
                                                        ...
                                                        argt_rate = (to_decimal(reslin_queasy.deci1 * (- to_decimal(
                                                            netto_argt) / to_decimal(100))) * qty)
                                        ...
                                        if add_it:
                                            ...
                                            if breslin_queasy:
                                                for breslin_queasy in db_session.query(Breslin_queasy).filter(
                                                    ...
                                                    if breslin_queasy.char2.lower() != "" and breslin_queasy.char2.lower() != "0":
                                                        argt_rate = to_decimal(
                                                            reslin_queasy.deci1) * to_decimal(to_int(breslin_queasy.char2)) / to_decimal("100")
                                                    else:
                                                        if breslin_queasy.deci1 != 0:
                                                            argt_rate = to_decimal(
                                                                breslin_queasy.deci1)
        ...
        if queasy.char2 != "":
            reslin_queasy = db_session.query(Reslin_queasy).filter(
                (Reslin_queasy.key == "actual-invoice") & (Reslin_queasy.resnr == curr_resnr) & (Reslin_queasy.reslinnr == curr_reslinnr) & (Reslin_queasy.betriebsnr == qrecid) & ((Reslin_queasy.logi2) | ((Reslin_queasy.logi1 == False) & (Reslin_queasy.logi2 == False))) & (Reslin_queasy.char1 == "")).first()
    ...
    if header_list:
        ...
        header_list.balance = to_decimal(
            header_list.balance) - to_decimal(header_list.payment)
