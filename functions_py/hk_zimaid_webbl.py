...
def hk_zimaid_webbl(pvilanguage:int):
    ...
    output_list_data = []
    lvcarea:string = "hk-zimaid"
    stat_list:List[string] = create_empty_list(10,"")
    stat_list1:List[string] = ["VC", "VU", "VD", "ED", "OD", "OC", "OO", "OM", "DD", "OS", "EA"]
    ...
    output_list_data, Output_list = create_model("Output_list", {"personal":bool, "reihenfolge":int, "selected":bool, "cleanflag":bool, "odd_even":int, "flag":int, "zinr":string, "rstat":string, "rstat1":string, "departed":string, "gstat":string, "floor":int, "code":string, "inactive":string, "ldry":string, "towel":string, "kbezeich":string, "arrival":bool, "ankunft":date, "abreise":date, "nation":string, "zistatus":int, "gname":string, "resname":string, "bemerk":string, "rsv_flag":bool, "co_time":string, "vip":string, "flight_nr":string, "segm_type":int}, {"flag": 1})
    ...
    def fill_list():
        ...
        for zimmer in db_session.query(Zimmer).filter(
            ...
            if substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("1").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("3").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("5").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("7").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("9").lower() :
                output_list.odd_even = 1

            elif substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("0").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("2").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("4").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("6").lower()  or substring(zimmer.zinr, length(zimmer.zinr) - 1, 1) == ("8").lower() :
            ...
            if (zimmer.zistatus >= 3 and zimmer.zistatus <= 5) or zimmer.zistatus == 8:
                ...
                if res_line:
                    ...
                    if length(output_list.code) > 0 and substring(output_list.code, length(output_list.code) - 1, 1) == (",").lower() :
