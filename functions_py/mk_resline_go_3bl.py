...
def mk_resline_go_3bl(pvilanguage:int, accompany_tmpnr1:int, accompany_tmpnr2:int, accompany_tmpnr3:int, 
    ...
    def create_fixedrated():
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
                 (Reslin_queasy.key == ("arrangement")) & (Reslin_queasy.resnr == reslin_list.resnr) & 
                 (Reslin_queasy.reslinnr == reslin_list.reslinnr) & (Reslin_queasy.date1 >= ci_date)).order_by(Reslin_queasy._recid).with_for_update().all():
        ...
        if periode_list:
            for datum in date_range(reslin_list.ankunft,(reslin_list.abreise - timedelta(days=1))) :
                counter = counter + 1
                ...
                if periode_list:
                    ...
                    reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "arrangement")],"resnr": [(eq, reslin_list.resnr)],"reslinnr": [(eq, reslin_list.reslinnr)],"date1": [(eq, datum)],"date2": [(eq, datum)]})
                    ...
                    if not reslin_queasy:
                        ...
                        if counter == 1:
                            reslin_list.zipreis =  to_decimal(periode_list.amt_periode)
    ...
    def calc_periode():
        ...
        if queasy:
            ...
            for loopi in date_range(periode_rsv1,periode_rsv2 - timedelta(days=1)) :
    ...
    def rmchg_ressharer(act_zinr:string, new_zinr:string, main_nr:int):
        ...
        res_line2 = db_session.query(Res_line2).filter(
                 (Res_line2.resnr == reslin_list.resnr) & (Res_line2.kontakt_nr == main_nr) & 
                 (Res_line2.resstatus == 11) & (((Res_line2.zinr != "") & (Res_line2.zinr == (act_zinr))) | (Res_line2.zinr == ""))).with_for_update().first()
        while None != res_line2:
            ...
            res_line2 = db_session.query(Res_line2).filter(
                     (Res_line2.resnr == reslin_list.resnr) & (Res_line2.kontakt_nr == main_nr) & (Res_line2.resstatus == 11) & 
                     (((Res_line2.zinr != "") & (Res_line2.zinr == (act_zinr))) | (Res_line2.zinr == "")) & (Res_line2._recid > curr_recid)).with_for_update().first()
    ...
    def rmchg_sharer(act_zinr:string, new_zinr:string):
        ...
        for res_line1 in db_session.query(Res_line1).filter(
                 (Res_line1.resnr == reslin_list.resnr) & (Res_line1.zinr == (act_zinr)) & (Res_line1.resstatus == 13)).order_by(Res_line1._recid).all():
        ...
        if res_line.zikatnr != reslin_list.zikatnr:
            ...
            if res_line.resstatus != reslin_list.resstatus:
                ...
                if res_line.ankunft > reslin_list.ankunft:
                    for datum in date_range(reslin_list.ankunft,res_line.ankunft - timedelta(days=1)) :
                    ...
                    for datum in date_range(res_line.ankunft,reslin_list.ankunft - timedelta(days=1)) :
