...
def ts_splitbill_btn_transfer_paytype6bl(rec_id_h_bill:int, curr_select:int, p_artnr:int, balance:Decimal, price_decimal:int, transdate:date, dept:int, change_str:string, price:Decimal, add_zeit:int, hoga_card:string, cancel_str:string, curr_waiter:int, amount_foreign:Decimal, curr_room:string, user_init:string, cc_comment:string, guestnr:int, tischnr:int):
    ...
    def generate_output():
        nonlocal billart, qty, description, amount, bill_date, fl_code, t_h_bill_line_data, h_bill_line, kellner, h_bill, h_artikel, kellne1, h_umsatz, umsatz, h_compli, queasy
        ...
        return {"billart": billart, "qty": qty, "description": description, "amount": amount, "bill_date": bill_date, "fl_code": fl_code, "t-h-bill-line": t_h_bill_line_data}

    def adjust_mealcoupon_umsatz(curr_select:int):

        nonlocal billart, qty, description, bill_date, fl_code, t_h_bill_line_data, h_bill_line, kellner, h_bill, h_artikel, kellne1, h_umsatz, umsatz, h_compli, queasy
