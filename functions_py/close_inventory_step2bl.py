...
from models import L_op, L_ophdr, Htparam, L_artikel, L_ophis, Queasy, L_ophhis

def close_inventory_step2bl(inv_type:int, m_endkum:int, closedate:date):

    prepare_cache ([Htparam, L_artikel, L_ophis, Queasy])

    firstdate:date = None
    delete_oph:bool = False
    m_datum:date = None
    fb_datum:date = None
    l_op = l_ophdr = htparam = l_artikel = l_ophis = queasy = l_ophhis = None
    ...
    def generate_output():
        nonlocal firstdate, delete_oph, m_datum, fb_datum, l_op, l_ophdr, htparam, l_artikel, l_ophis, queasy, l_ophhis
