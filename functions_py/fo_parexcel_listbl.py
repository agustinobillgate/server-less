...
def fo_parexcel_listbl(pvilanguage:int, briefnr:int, to_date:date, check_only:bool, view_only:bool, jobnr:string):
    msg_str = ""
    success_flag = False
    ...
    w11_data, W11 = create_model("W11", {"nr":int, "debit":Decimal, "credit":Decimal})

    Parent_w1 = W1
    parent_w1_data = w1_data

    Child_w1 = W1
    child_w1_data = w1_data

    W1_ratio = W1
    w1_ratio_data = w1_data
    ...
    def generate_output():
        ...
        nonlocal detail_list_data, brief_list_data, batch_list_data, htp_list_data, htv_list_data, w1_data, w2_data, w11_data
    ...
    serv_vat, foreign_nr, start_date, price_decimal, no_decimal, outfile_dir, keycmd, keyvar, batch_list_data, htv_list_data, htp_list_data, brief_list_data = get_output(prepare_fo_parexcelbl(briefnr))
    ...
    num_year = get_year(to_date) - 1
    ljan1 = date_mdy(1, 1, num_year)

    if (start_date != None) and (start_date > from_date):
        from_date = start_date
        jan1 = start_date
        num_year = get_year(jan1) - 1
        ljan1 = date_mdy(get_month(jan1) , get_day(jan1) , num_year)
    pto_date = from_date - timedelta(days=1)
    tmp_month = to_date + timedelta(days=1)

    if (get_month(to_date) == get_month(tmp_month)):
    ...
    if (get_month(to_date) != 2) or (get_day(to_date) != 29):
        num_year = get_year(to_date) - 1
        lto_date = date_mdy(get_month(to_date) , get_day(to_date) , num_year)
    else:
        num_year = get_year(to_date) - 1
        lto_date = date_mdy(get_month(to_date) , 28, num_year)
