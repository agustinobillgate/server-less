...
from models import L_op, Gl_acct, L_ophdr, L_bestand, L_verbrauch, Queasy, L_artikel

op_list_data, Op_list = create_model_like(L_op, {"fibu":string, "a_bezeich":string, "a_lief_einheit":Decimal, "a_traubensort":string, "remark":string})
...
def s_stockout_btn_gobl(op_list_data:[Op_list], pvilanguage:int, out_type:int, rec_id:int, curr_lager:int, jobnr:int, cost_acct:string, transdate:date, t_datum:date, transfered:bool, t_lschein:string, to_stock:int, lscheinnr:string, bediener_nr:int, qty:Decimal, t_amount:Decimal):
    ...
    prepare_cache ([L_op, L_ophdr, L_bestand, L_verbrauch, Queasy, L_artikel])
    ...
    its_ok:bool = False
    l_op = gl_acct = l_ophdr = l_bestand = l_verbrauch = queasy = l_artikel = None
    ...
    def generate_output():
        nonlocal s_artnr, err_flag, price, amount, msg_str2, msg_str3, req_created, lvcarea, curr_pos, zeit, remark, gl_notfound, its_ok, l_op, gl_acct, l_ophdr, l_bestand, l_verbrauch, queasy, l_artikel
    ...
    def create_l_op(zeit:int):
        ...
        nonlocal s_artnr, err_flag, price, amount, msg_str2, msg_str3, req_created, lvcarea, curr_pos, remark, gl_notfound, its_ok, l_op, gl_acct, l_ophdr, l_bestand, l_verbrauch, queasy, l_artikel
        nonlocal pvilanguage, out_type, rec_id, curr_lager, jobnr, cost_acct, transdate, t_datum, transfered, t_lschein, to_stock, lscheinnr, bediener_nr, qty, t_amount
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 340)],"char1": [(eq, lscheinnr)],"number1": [(eq, s_artnr)],"deci1": [(eq, price)]})
        ...
        if not queasy:
            ...
            queasy.number1 = s_artnr
            queasy.deci1 =  to_decimal(price)
            queasy.char2 = remark
