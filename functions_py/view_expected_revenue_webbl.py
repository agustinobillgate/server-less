...
from models import Waehrung, Htparam, Reservation, Res_line, Guest_pr, Zimkateg, Arrangement, Reslin_queasy, Queasy, Katpreis, Argt_line, Artikel, Zwkum, Fixleist

def view_expected_revenue_webbl(pvilanguage:int, resno:int):

    prepare_cache ([Htparam, Reservation, Res_line, Guest_pr, Zimkateg, Arrangement, Katpreis, Argt_line, Artikel, Fixleist])

    ci_date = None
    title_str = ""
    t_res_line_data = []
    expectedrev_list_data = []
    ...
    waehrung = htparam = reservation = res_line = guest_pr = zimkateg = arrangement = reslin_queasy = queasy = katpreis = argt_line = artikel = zwkum = fixleist = None
    ...
    expectedrev_list_data, Expectedrev_list = create_model("Expectedrev_list", {"flag":int, "datum":date, "rmtype_no":int, "article_no":int, "dept_no":int, "str_rmtype":string, "str_desc":string, "str_amount":string, "str1":string, "calc_rmrate":Decimal, "calc_argtrate":Decimal, "calc_argtrate2":Decimal, "calc_argtrate_all":Decimal, "calc_qty":Decimal, "calc_fixcost":Decimal, "calc_lodg":Decimal, "calc_totrev":Decimal, "count_break":Decimal}, {"flag": -1, "dept_no": -1})
    t_res_line_data, T_res_line = create_model("T_res_line", {"resnr":int, "reslinnr":int, "name":string, "zinr":string, "ankunft":date, "abreise":date, "contcode":string, "str_argt":string, "ct_code":string, "kurzbez":string, "curr_rmcat":string, "rmcat_bez":string, "bonus_array":[bool,9999], "curr_date":date, "rmcat_no":int})
    argt_list_data, Argt_list = create_model("Argt_list", {"argtnr":int, "argt_artnr":int, "resnr":int, "reslinnr":int, "departement":int, "is_charged":int, "period":int, "vt_percnt":int})

    W1 = create_buffer("W1",Waehrung)
    Buf_list = Output_list
    buf_list_data = output_list_data
    ...
    def generate_output():
        ...
        return {"ci_date": ci_date, "title_str": title_str, "t-res-line": t_res_line_data, "expectedrev-list": expectedrev_list_data}
    ...
    def check_bonus():
        ...
        nonlocal ci_date, title_str, t_res_line_data, expectedrev_list_data, new_contrate, bonus_array, wd_array, zim_wunsch, rsv_name, str_common, tot_rate, abreise_date, curr_datum, datum, co_date, argt_rate, argt_rate2, rm_rate, daily_rate, add_it, c, fixed_rate, argt_defined, delta, start_date, qty, it_exist, exrate1, ex2, pax, child1, created_date, bill_date, curr_zikatnr, curr_i, w_day, rack_rate, ebdisc_flag, kbdisc_flag, rate_found, early_flag, kback_flag, ratecode_qsy, count_break, fixcost_rate, waehrung, htparam, reservation, res_line, guest_pr, zimkateg, arrangement, reslin_queasy, queasy, katpreis, argt_line, artikel, zwkum, fixleist
        nonlocal pvilanguage, resno
        nonlocal w1, buf_list, buf_out
    ...
    def cal_revenue():
        ...
        output_list = query(output_list_data, filters=(lambda output_list: output_list.datum == t_res_line.curr_date), first=True)
        ...
        if not output_list:
            ...
            output_list = query(output_list_data, filters=(lambda output_list: output_list.rmtype_no == t_res_line.rmcat_no and output_list.flag == 0 and output_list.datum == t_res_line.curr_date), first=True)
                    ...
                    if add_it:
                        ...
                        if reslin_queasy:
                            ...
                            if argt_line.vt_percnt == 0:
                                ...
                                reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "fargt-line")],"char1": [(eq, "")],"number1": [(eq, argt_line.departement)],"number2": [(eq, argt_line.argtnr)],"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, res_line.reslinnr)],"number3": [(eq, argt_line.argt_artnr)],"date1": [(le, bill_date)],"date2": [(ge, bill_date)],"deci1": [(ne, 0)]})

                            elif argt_line.vt_percnt == 1:

                                reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "fargt-line")],"char1": [(eq, "")],"number1": [(eq, argt_line.departement)],"number2": [(eq, argt_line.argtnr)],"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, res_line.reslinnr)],"number3": [(eq, argt_line.argt_artnr)],"date1": [(le, bill_date)],"date2": [(ge, bill_date)],"deci2": [(ne, 0)]})

                            elif argt_line.vt_percnt == 2:

                                reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "fargt-line")],"char1": [(eq, "")],"number1": [(eq, argt_line.departement)],"number2": [(eq, argt_line.argtnr)],"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, res_line.reslinnr)],"number3": [(eq, argt_line.argt_artnr)],"date1": [(le, bill_date)],"date2": [(ge, bill_date)],"deci3": [(ne, 0)]})
            ...
            if rm_rate != 0:
                        ...
                        if reslin_queasy:
                            ...
                            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                ...
                                if argt_rate != 0:
                                    ...
                                    output_list = query(output_list_data, filters=(lambda output_list: output_list.flag == 1 and output_list.article_no == artikel.artnr and output_list.dept_no == artikel.departement and output_list.datum == t_res_line.curr_date and output_list.rmtype_no == t_res_line.rmcat_no), first=True)
                ...
                for fixleist in db_session.query(Fixleist).filter(
                    ...
                    if argt_rate != 0:
                        ...
                        output_list = query(output_list_data, filters=(lambda output_list: output_list.flag == 2 and output_list.article_no == artikel.artnr and output_list.dept_no == artikel.departement and output_list.datum == t_res_line.curr_date and output_list.rmtype_no == t_res_line.rmcat_no), first=True)
    ...
    def usr_prog1(bill_date:date, roomrate:Decimal):
        ...
        nonlocal ci_date, title_str, t_res_line_data, expectedrev_list_data, new_contrate, bonus_array, wd_array, zim_wunsch, rsv_name, str_common, tot_rate, abreise_date, curr_datum, datum, co_date, argt_rate, argt_rate2, rm_rate, daily_rate, add_it, c, fixed_rate, argt_defined, delta, start_date, qty, it_exist, exrate1, ex2, pax, child1, n, created_date, curr_zikatnr, curr_i, w_day, rack_rate, ebdisc_flag, kbdisc_flag, rate_found, early_flag, kback_flag, ratecode_qsy, count_break, fixcost_rate, waehrung, htparam, reservation, res_line, guest_pr, zimkateg, arrangement, reslin_queasy, queasy, katpreis, argt_line, artikel, zwkum, fixleist
        nonlocal pvilanguage, resno
        nonlocal w1, buf_list, buf_out
    ...
    for output_list in query(output_list_data, sort_by=[("datum",False),("rmtype_no",False),("flag",False)]):
        ...
        if output_list.flag == 0 or output_list.flag == 1:
            ...
            buf_list = query(buf_list_data, filters=(lambda buf_list: buf_list.flag == 2 and buf_list.datum == output_list.datum and buf_list.rmtype_no == output_list.rmtype_no), first=True)

            if not buf_list:
                buf_list = Buf_list()
                buf_list_data.append(buf_list)
    ...
    for output_list in query(output_list_data, sort_by=[("datum",False),("rmtype_no",False),("flag",False)]):
        ...
        if output_list.flag == 0 or output_list.flag == 3:
            ...
            buf_out = query(buf_out_data, filters=(lambda buf_out: buf_out.flag == 4 and buf_out.datum == output_list.datum and buf_out.rmtype_no == output_list.rmtype_no), first=True)

            if not buf_out:
                buf_out = Buf_out()
                buf_out_data.append(buf_out)
