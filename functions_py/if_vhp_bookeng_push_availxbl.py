...
def if_vhp_bookeng_push_availxbl(pushrate:bool, inp_str:string, fdate:date, tdate:date, becode:int, temp_list_data:[Temp_list]):
    ...
    def count_availability(curr_date:date, i_typ:int):
        ...
        if cat_flag:
            ...
            bresline = get_cache (Res_line, {"active_flag": [(le, 1)],"resstatus": [(le, 6),(ne, 4),(ne, 12),(ne, 11),(ne, 13)],"ankunft": [(le, curr_date)],"abreise": [(gt, curr_date)],"kontignr": [(ge, 0)],"l_zuordnung[2]": [(eq, 0)]})
            while None != bresline :
                ...
                bresline = db_session.query(Bresline).filter(
                         (Bresline.active_flag <= 1) & (Bresline.resstatus <= 6) & (Bresline.resstatus != 4) & (Bresline.resstatus != 12) & (Bresline.resstatus != 11) & (Bresline.resstatus != 13) & (Bresline.ankunft <= curr_date) & (Bresline.abreise > curr_date) & (Bresline.kontignr >= 0) & (Bresline.l_zuordnung[inc_value(2)] == 0) & (Bresline._recid > curr_recid)).first()
            else:

                bresline = get_cache (Res_line, {"active_flag": [(le, 1)],"resstatus": [(le, 6),(ne, 4),(ne, 12),(ne, 11),(ne, 13)],"zikatnr": [(eq, i_typ)],"ankunft": [(le, curr_date)],"abreise": [(gt, curr_date)],"kontignr": [(ge, 0)],"l_zuordnung[2]": [(eq, 0)]})
                while None != bresline:
                    do_it = True
                    ...
                    curr_recid = bresline._recid
                    bresline = db_session.query(Bresline).filter(
                             (Bresline.active_flag <= 1) & (Bresline.resstatus <= 6) & (Bresline.resstatus != 4) & (Bresline.resstatus != 12) & (Bresline.resstatus != 11) & (Bresline.resstatus != 13) & (Bresline.zikatnr == i_typ) & (Bresline.ankunft <= curr_date) & (Bresline.abreise > curr_date) & (Bresline.kontignr >= 0) & (Bresline.l_zuordnung[inc_value(2)] == 0) & (Bresline._recid > curr_recid)).first()
    ...
    if queasy:
        ...
        for qsy159 in db_session.query(Qsy159).filter(
            ...
            while None != q_curr:
                ...
                qsy = db_session.query(Queasy).filter(Queasy._recid == q_curr._recid).with_for_update().first()
    ...
    if pushall:
        ...
        for queasy in db_session.query(Queasy).filter(
                     (Queasy.key == 171) & (Queasy.char1 == "") & (Queasy.betriebsnr == becode)).order_by(Queasy._recid).with_for_update().all():
        ...
        for res_line.zinr, res_line.resnr, res_line.abreise, res_line.ankunft, res_line.kontignr, res_line.zimmeranz, res_line.zimmer_wunsch, res_line._recid, res_line.zikatnr, zimkateg.typ, zimkateg.zikatnr, zimkateg.kurzbez, zimkateg._recid in db_session.query(Res_line.zinr, Res_line.resnr, Res_line.abreise, Res_line.ankunft, Res_line.kontignr, Res_line.zimmeranz, Res_line.zimmer_wunsch, Res_line._recid, Res_line.zikatnr, Zimkateg.typ, Zimkateg.zikatnr, Zimkateg.kurzbez, Zimkateg._recid).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                     (Res_line.active_flag <= 1) & (Res_line.resstatus <= 6) & (Res_line.resstatus != 4) & (Res_line.resstatus != 12) & (Res_line.resstatus != 11) & (Res_line.resstatus != 13) & (Res_line.kontignr >= 0) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line.ankunft <= valid_date) & (Res_line.abreise >= fdate)).order_by(Res_line._recid).all():
            ...
            if not qsy_allot:
                ...
                if qsy2:
                    ...
                    if rqsy:
                        ...
                        if rqsy.char2.lower()  == ("CLOSE").lower()  and rqsy.number2 == 0:
                            statnr = 0

                        if rqsy.char2.lower()  == ("CTA").lower()  and rqsy.number2 == 0:
                            statnr = 5

                        if rqsy.char2.lower()  == ("CTD").lower()  and rqsy.number2 == 0:
                            statnr = 6

                        if rqsy.char2.lower()  == ("CLOSE").lower()  and rqsy.number2 == 1:
                            statnr = 1

                        if rqsy.char2.lower()  == ("CTA").lower()  and rqsy.number2 == 1:
                            statnr = 2

                        if rqsy.char2.lower()  == ("CTD").lower()  and rqsy.number2 == 1:
                            statnr = 3
    ...
    while None != queasy :
        ...
        if not push_allot_list:
            ...
            if queasy.number2 == 1:
                ...
                if queasy.char2.lower()  == ("Close").lower() :
                    push_allot_list.statnr = 1

                elif queasy.char2.lower()  == ("CTA").lower() :
                    push_allot_list.statnr = 2

                elif queasy.char2.lower()  == ("CTD").lower() :
