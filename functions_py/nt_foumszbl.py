...
def nt_foumszbl():
    ...
    def generate_output():
        nonlocal pvilanguage, lvcarea, long_digit, line, progname, vat_artnr, price_decimal, n, night_type, reihenfolge, line_nr, p_width, p_length, index_nr, from_date, to_date, htl_name, htl_adr, htl_tel, paramtext, htparam, nightaudit, nitestor, hoteldpt, artikel, umsatz
    ...
    def umsatz_list():
        ...
        add_line(line)
        add_line(" ")
        line = translateExtended ("Turnover Report by department", lvcarea, "")
        add_line(line)
        line = ""
        add_line(" ")
        add_line(" ")
        ...
        for cl_list in query(cl_list_data, sort_by=[("nr",False)]):
            ...
            if cl_list.flag.lower()  == ("*").lower() :
                ...
                # Rulita,
                # - Fix space in string
                line = " " + translateExtended ("Art Description                 DAY NET       %       DAY GROS", lvcarea, "") + " " + "      %        " + translateExtended ("MONTH NET       %       MONTH GROS       %", lvcarea, "")
                add_line(line)
                line = ""
                for i in range(1,p_width + 1) :
                    line = line + "-"
