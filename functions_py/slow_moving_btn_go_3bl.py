...
from models import L_ophis, L_op, L_verbrauch, Htparam, L_artikel, L_bestand, L_pprice


def slow_moving_btn_go_3bl(storeno: int, main_grp: int, tage: int, show_price: bool, disptype: str):

    prepare_cache([L_ophis, L_op, L_verbrauch, Htparam, L_artikel, L_bestand, L_pprice])

    str_flag = ""
    s_list_data = []
    l_ophis = l_op = l_verbrauch = htparam = l_artikel = l_bestand = l_pprice = None
    ...
    def generate_output():
        nonlocal str_flag, s_list_data, l_ophis, l_op, l_verbrauch, htparam, l_artikel, l_bestand, l_pprice
    ...
    def create_list():
        ...
        for l_artikel in db_session.query(L_artikel).filter(
            ...
            l_bestand = get_cache(
                L_bestand, {"artnr": [(eq, l_artikel.artnr)], "lager_nr": [(eq, storeno)]})

            if l_bestand:
                curr_best = to_decimal(l_bestand.anz_anf_best) + to_decimal(
            ...
            if curr_best > 0:
                for l_op in db_session.query(L_op).filter(
                    ...
                    if l_op.datum <= (transdate - timedelta(days=tage)):
                        blop = get_cache(
                            L_op, {"artnr": [(eq, l_op.artnr)], "op_art": [(eq, l_op.op_art)], "lager_nr": [(eq, l_op.lager_nr)], "datum": [(gt, l_op.datum)]})

                        if not blop:
                            s_list = query(s_list_data, filters=(
                                lambda s_list: s_list.artnr == l_artikel.artnr), first=True)

                            if not s_list:
                                # start - MCH: BA49ED
                                for l_pprice in db_session.query(L_pprice).filter(
                                        (L_pprice.artnr == l_op.artnr)).order_by(L_pprice.bestelldatum.desc()).all():
                                # end - MCH: BA49ED
                                    s_list = S_list()
                                    ...
                                    if show_price:
                                        s_list.avrgprice = to_decimal(
                                            l_artikel.vk_preis)
                                        s_list.ek_aktuell = to_decimal(
                                            l_artikel.ek_aktuell)
                                    break
                ...
                for l_ophis in db_session.query(L_ophis).filter(
                    ...
                    if l_ophis.datum <= (transdate - timedelta(days=tage)):
                        l_op = get_cache(
                            L_op, {"artnr": [(eq, l_ophis.artnr)], "op_art": [ (eq, l_ophis.op_art)], "lager_nr": [(eq, l_ophis.lager_nr)], "datum": [(gt, l_ophis.datum)]})
                        ...
                        if not l_op:
                            ...
                            if not blophis:
                                ...
                                if not s_list:
                                    for l_pprice in db_session.query(L_pprice).filter(
                                            (L_pprice.artnr == l_ophis.artnr)).order_by(L_pprice.bestelldatum.desc()).all():
                                        s_list = S_list()
                                        s_list_data.append(s_list)
                                        ...
                                        s_list.endkum = l_artikel.endkum
                                        s_list.unit = l_artikel.masseinheit
                                        s_list.datum = l_pprice.bestelldatum  # MCH: BA49ED
            ...
            if curr_best > 0:
                for l_op in db_session.query(L_op).filter(
                    ...
                    if l_op.datum <= (transdate - timedelta(days=tage)):
                        ...
                        if not blop:
                            ...
                            if not s_list:
                                for l_pprice in db_session.query(L_pprice).filter(
                                        (L_pprice.artnr == l_op.artnr)).order_by(L_pprice.bestelldatum.desc()).all():  # MCH: BA49ED
                                    ...
                                    if bl_verbrauch:
                                        if bl_verbrauch.datum == bl_op.datum:
                                            ...
                                            s_list.endkum = l_artikel.endkum
                                            s_list.unit = l_artikel.masseinheit
                                            s_list.datum2 = l_pprice.bestelldatum  # MCH: BA49ED
                ...
                for l_ophis in db_session.query(L_ophis).filter(
                    ...
                    if l_ophis.datum <= (transdate - timedelta(days=tage)):
                        ...
                        if not l_op:
                            blophis = get_cache(
                                L_ophis, {"artnr": [(eq, l_ophis.artnr)], "op_art": [(eq, l_ophis.op_art)], "lager_nr": [(eq, l_ophis.lager_nr)], "datum": [(gt, l_ophis.datum)]})
        ...
        for l_artikel in db_session.query(L_artikel).filter(
            ...
            if curr_best > 0:
                ...
                for l_ophis in db_session.query(L_ophis).filter(
                    ...
                    if l_ophis.datum <= (transdate - timedelta(days=tage)):
                        l_op = get_cache(
                            L_op, {"artnr": [(eq, l_ophis.artnr)], "lager_nr": [(eq, l_ophis.lager_nr)], "datum": [(gt, l_ophis.datum)]})

                        if not l_op:
                            blophis = get_cache(L_ophis, {"artnr": [(eq, l_ophis.artnr)], "lager_nr": [
                                                (eq, l_ophis.lager_nr)], "datum": [(gt, l_ophis.datum)]})
    ...
    if disptype.lower() == "old":
        create_list()
    elif disptype.lower() == "new":
        create_list2()
    else:
        create_list3()
