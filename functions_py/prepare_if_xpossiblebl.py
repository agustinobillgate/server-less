...
def prepare_if_xpossiblebl(i_resnr:int, i_reslinnr:int):
    ...
    t_resline = get_cache (Res_line, {
        "resnr": [(eq, i_resnr)],
        "reslinnr": [(eq, i_reslinnr)]})

    if t_resline:
        if t_resline.resstatus <= 6:
            res_line = get_cache (Res_line, {
                "resnr": [(eq, t_resline.resnr)],
                "reslinnr": [(eq, t_resline.reslinnr)]})
    ...
    if res_line:
        ...
        if depart_time == 0:
            ...
            co_time = int(str(substring(htparam.fchar, 0, 2))) + int(str(substring(htparam.fchar, 3, 2))) / 60
            depart_time = int(co_time) * 3600
