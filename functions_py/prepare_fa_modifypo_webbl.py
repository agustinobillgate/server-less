...
def prepare_fa_modifypo_webbl(docu_nr: string):

    prepare_cache([Fa_order, Htparam, Waehrung, Bediener,
                  Queasy, Fa_op, L_lieferant, Mathis, Parameters])
    ...
    t_fa_ordheader = t_add_last = tfa_order = t_waehrung = disclist = t_mathis = t_lief_list = t_dept_list = budget_fix_asset_list = q245 = b_fa_order = None

    t_fa_ordheader_data, T_fa_ordheader = create_model_like(Fa_ordheader)
    t_add_last_data, T_add_last = create_model(
        "T_add_last",
        {
            "wabkurz": string
        })
    tfa_order_data, Tfa_order = create_model_like(
        Fa_order,
        {
            "desc_budget": string,
            "date_budget": date,
            "amount_budget": Decimal,
            "remain_budget": Decimal
        })
    ...
    disclist_data, Disclist = create_model(
        "Disclist",
        {
            "fa_recid": int,
            "fa_pos": int,
    ...
    t_mathis_data, T_mathis = create_model(
        "T_mathis",
        {
            "artnr": int,
            "asset_name": string,
    ...
    t_lief_list_data, T_lief_list = create_model(
        "T_lief_list",
        {
            "firma": string,
            "lief_nr": int
        })
    t_dept_list_data, T_dept_list = create_model(
        "T_dept_list",
        {
            "name": string,
            "nr": int
        })
    budget_fix_asset_list_data, Budget_fix_asset_list = create_model(
        "Budget_fix_asset_list",
        {
            "nr_budget": int,
            "desc_budget": string,
            "date_budget": date,
    ...
    def generate_output():
        nonlocal err_no, local_nr, str_approved1, enforce_rflag, deptname, billdate, t_amount, add_first_waehrung_wabkurz, add_last_waehrung_wabkurz, p_464, p_1093, p_220, t_fa_ordheader_data, tfa_order_data, disclist_data, t_add_last_data, t_waehrung_data, t_mathis_data, t_lief_list_data, t_dept_list_data, q245_data, mtd_budget, mtd_balance, t_warenwert, remain_budget, order_date, fa_ordheader, fa_order, htparam, waehrung, bediener, queasy, fa_op, l_lieferant, mathis, parameters
        nonlocal docu_nr
        nonlocal b_fa_order
        nonlocal t_fa_ordheader, t_add_last, tfa_order, t_waehrung, disclist, t_mathis, t_lief_list, t_dept_list, budget_fix_asset_list, q245, b_fa_order
        nonlocal t_fa_ordheader_data, t_add_last_data, tfa_order_data, t_waehrung_data, disclist_data, t_mathis_data, t_lief_list_data, t_dept_list_data, budget_fix_asset_list_data, q245_data

        return {
            "err_no": err_no,
            "local_nr": local_nr,
            "str_approved1": str_approved1,
            "enforce_rflag": enforce_rflag,
            "deptname": deptname,
            "billdate": billdate,
            "t_amount": t_amount,
            "add_first_waehrung_wabkurz": add_first_waehrung_wabkurz,
            "add_last_waehrung_wabkurz": add_last_waehrung_wabkurz,
            "p_464": p_464,
            "p_1093": p_1093,
            "p_220": p_220,
            "t-fa-ordheader": t_fa_ordheader_data,
            "tfa-order": tfa_order_data,
            "disclist": disclist_data,
            "t-add-last": t_add_last_data,
            "t-waehrung": t_waehrung_data,
            "t-mathis": t_mathis_data,
            "t-lief-list": t_lief_list_data,
            "t-dept-list": t_dept_list_data,
            "q245": q245_data
        }
    ...
    def call_tamount():
        ...
        for fa_order in fa_order_data.yield_per(100):
            ...
            if budget_fix_asset_list:
                ...
                tfa_order.amount_budget = to_decimal(
                    budget_fix_asset_list.amount_budget)
                tfa_order.remain_budget = to_decimal(
                    budget_fix_asset_list.remain_budget)
            ...
            disclist.price0 = to_decimal(fa_order.order_price) / to_decimal((1) - to_decimal(fa_order.discount1) * to_decimal(0.01)) /\
                ...
                to_decimal((1) + to_decimal(fa_order.vat) * to_decimal(0.01))
            disclist.brutto = to_decimal(
                disclist.price0) * to_decimal(fa_order.order_qty)
    ...
    def currency_list():
        ...
        for waehrung in db_session.query(Waehrung).filter(
                (Waehrung.ankauf > 0) & (Waehrung.betriebsnr != 0)).order_by(Waehrung.wabkurz):
    ...
    fa_ordheader = (
        db_session.query(Fa_ordheader)
        .filter(
            (Fa_ordheader.order_nr == docu_nr)
        )
        .first()
    )
    ...
    if fa_ordheader:
        ...
        log.write_log('prepare_fa_modifypo_webbl', f"[LOG] update data fa_orderheader: {t_fa_ordheader.__dict__}")
        order_date = fa_ordheader.order_date

        if fa_ordheader.approved_1:

            bediener = get_cache(
                Bediener, {"userinit": [(eq, fa_ordheader.approved_1_by)]})
        ...
        for queasy in queasy_data:
            ...
            fa_order_data = (
                db_session.query(Fa_order)
                .filter(
                    (Fa_order.activereason == to_string(queasy.number1))
                )
                .order_by(Fa_order._recid)
            )
            for fa_order in fa_order_data.yield_per(100):

                # fa_op = get_cache(Fa_op, {"loeschflag": [(le, 1)], "opart": [
                #                   (eq, 1)], "anzahl": [(gt, 0)], "docu_nr": [(eq, fa_order.order_nr)]})

                fa_op = (
                    db_session.query(Fa_op)
                    .filter(
                        (Fa_op.loeschflag <= 1) &
                        (Fa_op.opart == 1) &
                        (Fa_op.anzahl > 0) &
                        (Fa_op.docu_nr == fa_order.order_nr)
                    )
                    .order_by(Fa_op._recid)
                    .first()
                )

                if fa_op:
                    t_warenwert = to_decimal(
                        t_warenwert) + to_decimal(fa_op.warenwert)
            budget_fix_asset_list.remain_budget = to_decimal(
                queasy.deci1) - to_decimal(t_warenwert)
        call_tamount()
        currency_list()

        for waehrung in db_session.query(Waehrung).order_by(Waehrung._recid).yield_per(100):
        ...
        for mathis in db_session.query(Mathis).order_by(Mathis._recid).yield_per(100):
            t_mathis = T_mathis()
