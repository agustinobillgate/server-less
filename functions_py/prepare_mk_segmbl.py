...
def prepare_mk_segmbl(gastnr: int):
    ...
    def generate_output():
        ...
        return {
            ...
            "mainseg": mainseg,
            "vip_flag1": vip_flag1,
            "hsegm-list": hsegm_list_data,
            "gsegm-list": gsegm_list_data
        }

    vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9 = get_output(
        get_vipnrbl())
    ...
    for guestseg in db_session.query(Guestseg).filter(
        ...
        hsegm_list = query(hsegm_list_data, filters=(
            lambda hsegm_list: hsegm_list.segmentcode == gsegm_list.segmentcode), first=True)
