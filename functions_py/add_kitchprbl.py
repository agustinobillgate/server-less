...
def add_kitchprbl(pvilanguage:int, session_parameter:string, dept:int, rechnr:int, billdate:date, user_init:string):
    ...
    if sort_subgrp :
        ...
        #     t_h_artikel = query(t_h_artikel_data, (lambda t_h_artikel: t_h_artikel.artnr == h_bill_line.artnr and t_h_artikel.departement == dept and t_h_artikel.artart == 0), first=True)
        # Step 1: Filter all h_bill_line records matching criteria
        filtered_lines = (
            db_session.query(H_bill_line)
            .filter(
                H_bill_line.rechnr == rechnr,
                H_bill_line.departement == dept,
                H_bill_line.steuercode <= 0
            )
            .all()
        )

        # Step 2: Perform lookups and prepare combined data
        results = []

        for h_line in filtered_lines:
            # FIRST buf-h-artikel match
            h_artikel = next(
                (
                    a for a in t_h_artikel_data
                    if a.artnr == h_line.artnr and a.departement == dept and a.artart == 0
                ),
                None
            )
            if not h_artikel:
            ...
            # FIRST wgrpdep match
            wgrpdep = (
                db_session.query(Wgrpdep)
                .filter(
                    Wgrpdep.departement == dept,
                    Wgrpdep.zknr == h_artikel.zwkum
                )
                .first()
            )
            if not wgrpdep:
            ...
            results.append((h_line, h_artikel, wgrpdep))

        # Step 3: Sort results as per BY clause
        results.sort(key=lambda x: (
            x[1].bondruckernr[0] if x[1].bondruckernr else '',  # bondruckernr[1] in OpenEdge is index 0 in Python
            x[0].sysdate,
            x[0].zeit,
            x[0]._recid
        ))
        ...
        for h_line, wgrpdep in results:

        # for h_bill_line, wgrpdep in results:
            t_h_artikel = query(
                t_h_artikel_data,
                lambda item: item.artnr == h_bill_line.artnr 
                and item.departement == dept 
                and item.artart == 0,
                first=True
            )
            if not t_h_artikel:
