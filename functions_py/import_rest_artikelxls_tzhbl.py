...
def import_rest_artikelxls_tzhbl(rest_article_list_data:[Rest_article_list], rest_maingroup_list_data:[Rest_maingroup_list], rest_subgroup_list_data:[Rest_subgroup_list]):
    ...
    def import_artikel():
        ...
        for rest_maingroup_list in query(rest_maingroup_list_data):
            ...
            wgrpgen = db_session.query(Wgrpgen).filter(Wgrpgen.eknr == rest_maingroup_list.maingroup_no).with_for_update().first()
        ...
        for rest_subgroup_list in query(rest_subgroup_list_data):
            ...
            wgrpdep = db_session.query(Wgrpdep).filter((Wgrpdep.zknr == rest_subgroup_list.subgroup_no) & (Wgrpdep.departement == rest_subgroup_list.dept_no)).with_for_update().first()
