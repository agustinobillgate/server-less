...
def select_sartnr2_1bl(curr_lager: int, recipe: bool, sorttype: int, s_artnr: int, s_bezeich: string):
    ...
    sartnr_list_data, Sartnr_list = create_model(
        "Sartnr_list",
        {
            "artnr": int,
            "bezeich": string,
            "anz_anf_best": Decimal,
            "anz_eingang": Decimal,
            "anz_ausgang": Decimal,
            "masseinheit": string,
            "is_receipt": bool,
    ...
    def generate_output():
        ...
        return {
            "sartnr-list": sartnr_list_data
        }

    if curr_lager == 0:

        if sorttype == 1:
            query_l_artikel_sorttype_1 = (
                ...
                # )
                .filter(
                    # (L_bestand.anz_anf_best +
                    #  L_bestand.anz_eingang -
                    #  L_bestand.anz_ausgang) > 0,
                    L_artikel.artnr >= s_artnr
                )
                .order_by(L_artikel.artnr)
            )
            for l_artikel in query_l_artikel_sorttype_1.yield_per(100):
            ...
            if substring(s_bezeich, 0, 1) != "*":
                ...
                if substring(s_bezeich, length(s_bezeich) - 1, 1) != "*":
                    s_bezeich = s_bezeich + "*"

                for l_artikel in db_session.query(L_artikel).filter(
                        (matches(L_artikel.bezeich, (s_bezeich)))).order_by(L_artikel.bezeich):
                    sartnr_list = Sartnr_list()
                    sartnr_list_data.append(sartnr_list)
    ...
    if not recipe:
        ...
        if sorttype == 1:
            ...
            query_l_artikel_l_bestand_sorttype_1 = (
                db_session
                .query(L_artikel, L_bestand)
                .join(
                    L_bestand,
                    L_bestand.artnr == L_artikel.artnr
                )
            ...
            for l_artikel, l_bestand in query_l_artikel_l_bestand_sorttype_1.yield_per(100):
            ...
            if substring(s_bezeich, 0, 1) != "*":
                ...
                query_l_artikel_l_bestand = (
                    db_session.query(L_artikel, L_bestand)
                    .join(L_bestand,
                          (L_bestand.artnr == L_artikel.artnr) &
                          (L_bestand.lager_nr == curr_lager)
                          )
                    .filter(
                        (L_artikel.bezeich >= (s_bezeich).lower())).order_by(L_artikel.bezeich)
                )
                for l_artikel, l_bestand in query_l_artikel_l_bestand:
                    if l_artikel_obj_list.get(l_artikel._recid):
                        continue
                    else:
                        l_artikel_obj_list[l_artikel._recid] = True
                ...
                query_l_artikel_l_bestand = (
                    ...
                    .filter(
                        (matches(L_artikel.bezeich, (s_bezeich)))
                    )
                    .order_by(L_artikel.bezeich)
                )
                for l_artikel, l_bestand in query_l_artikel_l_bestand:
                    if l_artikel_obj_list.get(l_artikel._recid):
                        continue
