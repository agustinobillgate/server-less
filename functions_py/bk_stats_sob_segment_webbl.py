...
def bk_stats_sob_segment_webbl(input_payload_data:[Input_payload]):
    ...
    bk_statistic_list_data, Bk_statistic_list = create_model("Bk_statistic_list", {"code":int, "bezeich":string, "dgroom":int, "proz1":Decimal, "mgroom":int, "proz2":Decimal, "ygroom":int, "proz3":Decimal, "dpax":int, "mpax":int, "ypax":int, "drev":Decimal, "mrev":Decimal, "yrev":Decimal, "d_fbrev":Decimal, "m_fbrev":Decimal, "y_fbrev":Decimal, "dorev":Decimal, "morev":Decimal, "yorev":Decimal})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal bk_statistic_list_data, from_date, fb_rev, other_rev, tot_dgroom, tot_mgroom, tot_ygroom, tot_dpax, tot_mpax, tot_ypax, tot_drev, tot_mrev, tot_yrev, tot_d_fbrev, tot_m_fbrev, tot_y_fbrev, tot_dorev, tot_morev, tot_yorev, bk_veran, bk_func, queasy, artikel, bk_rart


        nonlocal bk_statistic_list, input_payload
        nonlocal bk_statistic_list_data

        return {"bk-statistic-list": bk_statistic_list_data}
    ...
    def create_list():
        ...
        for bk_func.veran_nr, bk_func.veran_seite, bk_func.rpersonen, bk_func.rpreis, bk_func.technik, bk_func.datum, bk_func._recid, bk_veran.segmentcode, bk_veran._recid in db_session.query(Bk_func.veran_nr, Bk_func.veran_seite, Bk_func.rpersonen, Bk_func.rpreis, Bk_func.technik, Bk_func.datum, Bk_func._recid, Bk_veran.segmentcode, Bk_veran._recid).join(Bk_veran,(Bk_veran.veran_nr == Bk_func.veran_nr)).filter(
            ...
            bk_statistic_list = query(bk_statistic_list_data, filters=(lambda bk_statistic_list: bk_statistic_list.code == bk_veran.segmentcode), first=True)

            if not bk_statistic_list:
                bk_statistic_list = Bk_statistic_list()
                bk_statistic_list_data.append(bk_statistic_list)
    ...
    def create_list1():
        ...
        for bk_func in db_session.query(Bk_func).filter(
            ...
            bk_statistic_list = query(bk_statistic_list_data, filters=(lambda bk_statistic_list: bk_statistic_list.code == to_int(bk_func.technik[1])), first=True)
