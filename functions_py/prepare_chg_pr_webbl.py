...
def prepare_chg_pr_webbl(docu_nr:string):
    ...
    t_l_artikel_data, T_l_artikel = create_model("T_l_artikel", {"rec_id":int, "artnr":int, "traubensort":string, "lief_einheit":Decimal, "bezeich":string, "jahrgang":int, "soh":Decimal})
    ...
    def generate_output():
        nonlocal billdate, comments, deptnr, lieferdatum, deptname, hierarchie, tot, path_lst, rej_id, s_list_data, t_l_orderhdr_data, t_l_artikel_data, t_parameters_data, t_waehrung_data, approved_data, local_curr, amt, def_curr, mm, yy, from_date, to_date, tot_avg_cons, app_str, i, tmpint, tmpdate, tot_anz, anz_anf_best, anz_eingang, anz_ausgang, l_order, l_orderhdr, waehrung, bediener, htparam, parameters, l_artikel, l_ophis, l_lieferant, gl_acct, l_pprice, l_bestand
        nonlocal docu_nr
        ...
        nonlocal s_list_data, t_parameters_data, t_l_orderhdr_data, t_waehrung_data, t_l_artikel_data, approved_data

        return {"billdate": billdate, "comments": comments, "deptnr": deptnr, "lieferdatum": lieferdatum, "deptname": deptname, "hierarchie": hierarchie, "tot": tot, "path_lst": path_lst, "rej_id": rej_id, "s-list": s_list_data, "t-l-orderhdr": t_l_orderhdr_data, "t-l-artikel": t_l_artikel_data, "t-parameters": t_parameters_data, "t-waehrung": t_waehrung_data, "approved": approved_data}
