...
def view_staycost_1bl(pvilanguage: int, resnr: int, reslinnr: int, contcode: string, repeat_charge: bool):
    ...
    periode: date = None
    loopi: date = None
    curr_diff = to_decimal("0.0")
    month_str1: List[int] = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]  # ITA: Program terkait feature service apartement
    month_str2: List[int] = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]  # ITA: Program terkait feature service apartement
    ...
    # start - ITA: Program terkait feature service apartement
    def lastday(d: date):
        nonlocal output_list_data, ci_date, new_contrate, wd_array, bonus_array, periode, loopi, curr_diff, month_str1, month_str2, lvcarea, reservation, htparam, res_line, arrangement, guest_pr, waehrung, genstat, reslin_queasy, queasy, katpreis, argt_line, artikel, fixleist
        nonlocal pvilanguage, resnr, reslinnr, contcode, repeat_charge
        nonlocal output_list, periode_list, output_list1
    ...
    def cal_revenue():
        ...
        daily_rate = to_decimal("0.0")
        tot_rate = to_decimal("0.0")
        add_it: bool = False
        c = ""
        ...
        ratecode_qsy = ""
        count_break = to_decimal("0.0")
        fixcost_rate = to_decimal("0.0")
        w1 = None
            ...
            if (datum >= ci_date) or rm_rate is None:
                ...
                if not fixed_rate:
                    if not it_exist:
                        ...
                        if not rate_found:
                            if bill_date is not None:  # ITA: Program terkait feature service apartement
                                w_day = wd_array[get_weekday(bill_date) - 1]  # ITA: Program terkait feature service apartement
    ...
    def calc_periode():
        ...
        if pqueasy:
            ...
            for pqueasy in db_session.query(Pqueasy).filter(
                ...
                periode_rsv1 = pqueasy.date2  # ITA: Program terkait feature service apartement
                periode_rsv2 = pqueasy.date3  # ITA: Program terkait feature service apartement
    ...
    def cal_revenue1():
        ...
        rm_rate = to_decimal(res_line.zipreis)
        for datum in date_range(res_line.ankunft, co_date):
            curr_i = curr_i + 1  # ITA: Program terkait feature service apartement
            ...
            if datum >= ci_date:
                ...
                if reslin_queasy:
                    ...
                    if reslin_queasy.char1 != "":
                        ...
                        arrangement = get_cache(
                            Arrangement, {"arrangement": [(eq, reslin_queasy.char1)]})  # ITA: Program terkait feature service apartement
        ...
        for output_list1 in query(output_list1_data, sort_by=[("datum", False)]):
            ...
            tot_amount = to_decimal(output_list1.rate)  # ITA: Program terkait feature service apartement
            output_list.str = to_string(output_list1.datum) + "  " + translateExtended("Roomrate", lvcarea, "") +\
                "   = " + trim(to_string(round(tot_amount, 0), "->>>,>>>,>>9.99"))
            tot_rate = to_decimal(tot_rate) + to_decimal(tot_amount)  # ITA: Program terkait feature service apartement
