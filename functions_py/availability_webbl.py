...
#-----------------------------------------
# Rd, 17-July-25, check edate: edate is not None
# parameter fdate di calc-extra diganti ffdate (duplicate dgn htparam.fdate)
# variable fdate di create_browse
#-----------------------------------------
...
def availability_webbl(pvilanguage:int, printer_nr:int, call_from:int, txt_file:string, curr_date:date, to_date:date, incl_tentative:bool, zikat_list_data:[Zikat_list]):
    ...
    room_list_data, Room_list = create_model("Room_list", {"flag":string, "setup":int, "haupt":bool, "zikatnr":int, "bezeich":string, "room":[Decimal,32], "coom":[string,32], "glores":bool})
    sum_list_data, Sum_list = create_model("Sum_list", {"bezeich":string, "summe":[string,32]})
    ...
    def generate_output():
        nonlocal msg_str, room_list_data, sum_list_data, lnldelimeter, ttl_room, occ_room, ooo_room, anz_setup, tot_room, isetup_array, csetup_array, ci_date, datum, week_list, i, j, curr_day, tmp_date, num_date, lvcarea, res_line, zimkateg, htparam, artikel, fixleist, arrangement, reslin_queasy, paramtext, zimmer, kontline, outorder, guest
        nonlocal pvilanguage, printer_nr, call_from, txt_file, curr_date, to_date, incl_tentative
        nonlocal zkbuff


        nonlocal rmcat_list, room_list, sum_list, tmp_resline, tmp_extra, temp_art, zikat_list, zkbuff, rlist
    ...
    def calc_extra(ffdate:date):
                            ...
                            for fixleist in db_session.query(Fixleist).filter(
                                ...
                                if tmp_resline.ankunft == tmp_resline.abreise:
                                    ...
                                    if fixleist.sequenz == 1 or fixleist.sequenz == 2 or fixleist.sequenz == 4 or fixleist.sequenz == 5:
                                        ...
                                        elif tmp_resline.abreise <= tdate and edate is not None:
                                            edate = tmp_resline.abreise

                                    # Rd, 17-July-25, edate is not None
                                    if fixleist.sequenz == 1 and edate is not None:
                                        while bdate < edate :
                                    ...
                                    elif fixleist.sequenz == 4 and edate is not None:
                                        while bdate < edate :
                                    ...
                                    elif fixleist.sequenz == 5  and edate is not None:
                                        while bdate < edate :
                                        ...
                                        if eposdate > tdate and edate is not None:
                                            edate = tdate + timedelta(days=1)
                                            ...
                                            elif eposdate <= tmp_resline.abreise and edate is not None:
                                                edate = eposdate
                                        while bdate < edate :
                                            create_tmpextra(art_nr, "Fix-cost line", to_string(fixleist.sequenz), bdate, tmp_resline.zinr, fixleist.number)
                                            bdate = bdate + timedelta(days=1)
        ...
        for temp_art in query(temp_art_data):
            ...
            if artikel:
                ...
                ndate = ffdate
                num_date = (to_date - tmp_date).days
    ...
    def create_browse():
        ...
        tmp_list:List[int] = create_empty_list(32,0)
        gmember = None
        Rlist = Room_list
        ...
        for rmcat_list in query(rmcat_list_data, sort_by=[("nr",False)]):
