...
def nt_fina():
    ...
    def umsatz_list():
        ...
        for i in range(1, 42 + 1):
            line = line + " "
        line = line + "Date/Time :" + " " + to_string(get_current_date()) + "  " + \
    ...
    def create_fina():
        ...
        lfdate = date_mdy(get_month(from_date), get_day(
            from_date), get_year(from_date) - 1)
        ...
        uebertrag = get_cache(
            Uebertrag, {"datum": [(eq, to_date - timedelta(days=1))]})
        ...
        for cl_list1 in query(cl_list1_data, filters=(lambda cl_list1: cl_list1.flag >= 2 and cl_list1.flag <= 4)):
            cl_list.dgros = to_decimal(
                cl_list.dgros) + to_decimal(cl_list1.dgros)
        ...
        for segment in db_session.query(Segment).order_by(Segment._recid).all():
            for segmentstat in db_session.query(Segmentstat).filter(
                ...
                if segmentstat.datum == to_date:
                    info_list = query(info_list_data, filters=(
                        lambda info_list: info_list.flag == 1), first=True)
                    info_list.lodging = to_decimal(
                        info_list.lodging) + to_decimal(segmentstat.logis)

                if segmentstat.datum >= from_date:
                    info_list = query(info_list_data, filters=(
                        lambda info_list: info_list.flag == 2), first=True)
                    info_list.lodging = to_decimal(
                        info_list.lodging) + to_decimal(segmentstat.logis)

                if segmentstat.datum >= jan1:
                    info_list = query(info_list_data, filters=(
                        lambda info_list: info_list.flag == 3), first=True)
                    info_list.lodging = to_decimal(
                        info_list.lodging) + to_decimal(segmentstat.logis)
                ...
                if segmentstat.datum >= lfdate and segmentstat.datum <= ltdate:
                    info_list = query(info_list_data, filters=(
                        lambda info_list: info_list.flag == 4), first=True)
                    info_list.lodging = to_decimal(
                        info_list.lodging) + to_decimal(segmentstat.logis)
                ...
                if segmentstat.datum >= ljan1 and segmentstat.datum <= ltdate:
                    info_list = query(info_list_data, filters=(
                        lambda info_list: info_list.flag == 5), first=True)
                    info_list.lodging = to_decimal(
                        info_list.lodging) + to_decimal(segmentstat.logis)
        ...
        for zinrstat in db_session.query(Zinrstat).filter(
                (Zinrstat.zinr == "vacant") & (((Zinrstat.datum >= ljan1) & (Zinrstat.datum <= lfdate)) | ((Zinrstat.datum >= jan1) & (Zinrstat.datum <= to_date)))).order_by(Zinrstat.datum).all():
        ...
        for zinrstat in db_session.query(Zinrstat).filter(
                (Zinrstat.zinr == "ooo") & (((Zinrstat.datum >= ljan1) & (Zinrstat.datum <= lfdate)) | ((Zinrstat.datum >= jan1) & (Zinrstat.datum <= to_date)))).order_by(Zinrstat.datum).all():
            if zinrstat.datum == to_date:
                info_list = query(info_list_data, filters=(
                    lambda info_list: info_list.flag == 1), first=True)
        ...
        for zimkateg in db_session.query(Zimkateg).order_by(Zimkateg._recid).all():
            ...
            for zkstat in db_session.query(Zkstat).filter(
                ...
                zinrstat = get_cache(
                    Zinrstat, {"zinr": [(eq, "tot-rm")], "datum": [(eq, zkstat.datum)]})
            ...
            if d != 0:
                cl_list.dgros = to_decimal(
                    dgros) / to_decimal(d) * to_decimal("100")

            if m != 0:
                cl_list.mgros = to_decimal(
                    mgros) / to_decimal(m) * to_decimal("100")

            if y != 0:
                cl_list.ygros = to_decimal(
                    ygros) / to_decimal(y) * to_decimal("100")

            if lm != 0:
                cl_list.lmgros = to_decimal(
                    lmgros) / to_decimal(lm) * to_decimal("100")

            if ly != 0:
                cl_list.lygros = to_decimal(
                    lygros) / to_decimal(ly) * to_decimal("100")
            cl_list = Cl_list()
            cl_list_data.append(cl_list)
        ...
        info_list = query(info_list_data, filters=(
            lambda info_list: info_list.flag == 4), first=True)
        cl_list.lmgros = to_decimal(
            info_list.ocrm) - to_decimal(info_list.rcom)
        ...
        if info_list.avrm != 0:
            cl_list.dgros = to_decimal(
                info_list.ocrm) / to_decimal(info_list.avrm) * to_decimal("100")
        ...
        if info_list.pers != 0:
            cl_list.dgros = to_decimal(
                info_list.lodging) / to_decimal(info_list.pers)
        ...
        if (info_list.ocrm - info_list.rcom) != 0:
            cl_list.dgros = to_decimal(
                info_list.lodging) / to_decimal((info_list.ocrm) - to_decimal(info_list.rcom))
