...
def dml_issue_btn_gobl(op_list_data:[Op_list], billdate:date, closedate:date, curr_lager:int, curr_dept:int, cost_acct:string, lief_nr:int, lscheinnr:string, user_init:string, t_amount:Decimal):
    ...
    def generate_output():
        ...
        return {"op-list": op_list_data, "curr_pos": curr_pos, "s_artnr": s_artnr, "qty": qty, "price": price, "created": created}
    ...
    def create_l_op():
        ...
        if curr_dept == 0:
            ...
            d_art = db_session.query(D_art).filter(
                     (D_art.artnr == s_artnr) & (D_art.datum == billdate)).with_for_update().first()
            d_art.geliefert =  to_decimal(d_art.geliefert) + to_decimal(anzahl)

        elif curr_dept > 0:

            d_art1 = db_session.query(D_art1).filter(
                     (D_art1.artnr == s_artnr) & (D_art1.datum == billdate) & (D_art1.departement == curr_dept)).with_for_update().first()
    ...
    def create_purchase_book(s_artnr:int, price:Decimal, qty:Decimal, datum:date, lief_nr:int):
        ...
        nonlocal curr_pos, l_op, bediener, l_ophdr, dml_art, dml_artdep, l_artikel, l_lieferant, l_liefumsatz, htparam, gl_acct, l_kredit, ap_journal, l_pprice, l_bestand
