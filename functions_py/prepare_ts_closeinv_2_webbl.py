...
from models import H_bill_line, H_bill, H_artikel, Guest, Artikel, Htparam, Queasy, Waehrung, Hoteldpt, Kellner, Tisch, Exrate, H_mjourn
...
def prepare_ts_closeinv_2_webbl(pvilanguage:int, curr_dept:int, inp_rechnr:int, user_init:string, user_name:string, curr_printer:int):
    ...
    prepare_cache ([H_bill_line, H_artikel, Guest, Artikel, Htparam, Waehrung, Hoteldpt, Kellner, Tisch, Exrate, H_mjourn])
    ...
    h_bill_line = h_bill = h_artikel = guest = artikel = htparam = queasy = waehrung = hoteldpt = kellner = tisch = exrate = h_mjourn = None

    b_list = t_b_list = t_h_bill = t_h_artikel = t_h_artsales = ordered_item = summary_bill = bill_guest = h_artikel_buff = buff_hart = abuff = None

    b_list_data, B_list = create_model_like(H_bill_line, {"rec_id":int})
    t_b_list_data, T_b_list = create_model_like(H_bill_line, {"rec_id":int, "t_time":string, "menu_flag":int, "sub_menu_bezeich":string, "sub_menu_betriebsnr":int, "sub_menu_qty":int})
    ...
    def disp_bill_line():
        ...
        if double_currency:
            ...
            for b_list in query(b_list_data, filters=(lambda b_list: b_list.rechnr == h_bill.rechnr and b_list.departement == curr_dept), sort_by=[("sysdate",True),("zeit",True)]):
                ...
                t_b_list.menu_flag = 1

                h_artikel = get_cache (H_artikel, {"artnr": [(eq, b_list.artnr)],"departement": [(eq, b_list.departement)]})
                ...
                if h_artikel:
                    ...
                    for h_mjourn.nr, h_mjourn.anzahl, h_mjourn.request, h_mjourn._recid, h_artikel_buff.bezeich, h_artikel_buff.artnr, h_artikel_buff._recid, h_artikel_buff.artnrfront, h_artikel_buff.departement, h_artikel_buff.artart in db_session.query(H_mjourn.nr, H_mjourn.anzahl, H_mjourn.request, H_mjourn._recid, H_artikel_buff.bezeich, H_artikel_buff.artnr, H_artikel_buff._recid, H_artikel_buff.artnrfront, H_artikel_buff.departement, H_artikel_buff.artart).join(H_artikel_buff,(H_artikel_buff.artnr == H_mjourn.artnr) & (H_artikel_buff.departement == H_mjourn.departement)).filter(
                             (H_mjourn.nr == h_artikel.betriebsnr) & (H_mjourn.departement == b_list.departement) & (H_mjourn.rechnr == b_list.rechnr)).order_by(H_mjourn._recid).all():
                        ...
                        if num_entries(h_mjourn.request, "|") > 1:
                            ...
                            if entry(0, h_mjourn.request, "|") == to_string(b_list.rec_id):
                                t_b_list = T_b_list()
                                t_b_list_data.append(t_b_list)

                                t_b_list.menu_flag = 2
                                t_b_list.sub_menu_bezeich = h_artikel_buff.bezeich
                                t_b_list.sub_menu_betriebsnr = h_mjourn.nr
                                t_b_list.sub_menu_qty = h_mjourn.anzahl
                                t_b_list.rec_id = b_list.rec_id
                                t_b_list.artnr = h_artikel_buff.artnr
                                t_b_list.rechnr = None
                                t_b_list.bill_datum = None
                                t_b_list.anzahl = None
                                t_b_list.epreis =  to_decimal(None)
                                t_b_list.betrag =  to_decimal(None)
                                t_b_list.steuercode = None
                                t_b_list.bezeich = None
                                t_b_list.fremdwbetrag =  to_decimal(None)
                                t_b_list.zeit = None
                                t_b_list.waehrungsnr = None
                                t_b_list.sysdate = None
                                t_b_list.departement = None
                                t_b_list.prtflag = None
                                t_b_list.tischnr = None
                                t_b_list.kellner_nr = None
                                t_b_list.nettobetrag =  to_decimal(None)
                                t_b_list.paid_flag = None
                                t_b_list.betriebsnr = None
                                t_b_list.segmentcode = None
                                t_b_list.transferred = None
                ...
                for b_list in query(b_list_data, filters=(lambda b_list: b_list.rechnr == h_bill.rechnr and b_list.departement == curr_dept), sort_by=[("sysdate",True),("zeit",True)]):
                    ...
                    h_artikel = get_cache (H_artikel, {"artnr": [(eq, b_list.artnr)],"departement": [(eq, curr_dept)]})
                    ...
                    if h_artikel:
                        ...
                        h_mjourn = H_mjourn()
                        h_artikel_buff = H_artikel()
                        for h_mjourn.nr, h_mjourn.anzahl, h_mjourn.request, h_mjourn._recid, h_artikel_buff.bezeich, h_artikel_buff.artnr, h_artikel_buff._recid, h_artikel_buff.artnrfront, h_artikel_buff.departement, h_artikel_buff.artart in db_session.query(H_mjourn.nr, H_mjourn.anzahl, H_mjourn.request, H_mjourn._recid, H_artikel_buff.bezeich, H_artikel_buff.artnr, H_artikel_buff._recid, H_artikel_buff.artnrfront, H_artikel_buff.departement, H_artikel_buff.artart).join(H_artikel_buff,(H_artikel_buff.artnr == H_mjourn.artnr) & (H_artikel_buff.departement == b_list.departement)).filter(
                                 (H_mjourn.nr == h_artikel.betriebsnr) & (H_mjourn.departement == b_list.departement) & (H_mjourn.rechnr == b_list.rechnr)).order_by(H_mjourn._recid).all():
                            if h_mjourn_obj_list.get(h_mjourn._recid):
                                continue
                            else:
