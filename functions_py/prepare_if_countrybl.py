...
def prepare_if_countrybl(fl_run_proc:bool):
    ...
    t_parameters_data, T_parameters = create_model_like(Parameters, {"rec_id":int})
    cost_list_data, Cost_list = create_model("Cost_list", {"rec_id":int, "zone":string, "grace":int, "wday":int, "ftime":int, "ttime":int, "tdura":int, "dura":int, "cost":Decimal, "info":string})
    zone_list_data, Zone_list = create_model("Zone_list", {"rec_id":int, "zone":string, "city":string, "acode":string, "info":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal cost_list_data, zone_list_data, t_parameters_data, parameters
        nonlocal fl_run_proc


        nonlocal t_parameters, cost_list, zone_list
        nonlocal t_parameters_data, cost_list_data, zone_list_data

        return {"cost-list": cost_list_data, "zone-list": zone_list_data, "t-parameters": t_parameters_data}
    ...
    def cleanup_zonelist():
        ...
        if parameters:
            ...
            for parameters in db_session.query(Parameters).filter(
                     (Parameters.progname == ("if-internal").lower()) & (Parameters.section == ("Dcode").lower()) & (Parameters.varname > "")).with_for_update().order_by(Parameters._recid).all():
    ...
    def create_costlist():
        ...
        for parameters in db_session.query(Parameters).filter(
                 (Parameters.progname == ("Interface").lower()) & (Parameters.section == ("zone").lower()) & (Parameters.varname > "")).order_by(Parameters._recid).all():
            cost_list = Cost_list()
    ...
    def create_zonelist():
        ...
        for parameters in db_session.query(Parameters).filter(
                 (Parameters.progname == ("Interface").lower()) & (Parameters.section == ("Dcode").lower()) & (Parameters.varname > "")).order_by(Parameters._recid).all():
            zone_list = Zone_list()
            zone_list_data.append(zone_list)
    ...
    for parameters in db_session.query(Parameters).filter(
             (Parameters.progname == ("if-internal").lower()) & (Parameters.section == ("zone").lower())).order_by(Parameters._recid).all():
