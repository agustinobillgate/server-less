...
from models import Htparam, Guest, Reslin_queasy, Queasy, H_artikel, Zimkateg, Reservation, Sourccod, Segment, Res_line, Genstat, Nation, Arrangement, Artikel, Mc_guest, Waehrung, Bediener, Zimmer, Bill, Bill_line, H_bill, H_bill_line

def tauziarpt_ascottreservation2bl(datum:date, propid:str, last_recid:int):

    prepare_cache ([Guest, Queasy, H_artikel, Zimkateg, Reservation, Sourccod, Segment, Res_line, Genstat, Nation, Arrangement, Artikel, Mc_guest, Waehrung, Bediener, Zimmer, Bill, Bill_line, H_bill, H_bill_line])
    ...
    todate:date 
    curr_time:int = 0
    htparam = guest = reslin_queasy = queasy = h_artikel = zimkateg = reservation = sourccod = segment = res_line = genstat = nation = arrangement = artikel = mc_guest = waehrung = bediener = zimmer = bill = bill_line = h_bill = h_bill_line = None

    data_list = gmember = gcomp = rqueasy = qsy19 = qsy2 = buffart = datalist = qsy231 = qsy289 = None
    ...
    def generate_output():
        nonlocal data_count, data_list_data, birthdate, str_rsv, contcode, lastrmno, rstat, reslinnr_str, i, loop_i, curr_i, datacount, billno, cancel_loop, billflag, do_it, resv_date, rsv_date, to_date, datum1, afterdatum1, flodging, lodging, breakfast, lunch, dinner, others, rmrate, net_vat, net_service, vat, service, t_rmrev, t_fbrev, t_others, betragvalue, grossbetrag, newbetrag, totalfb, totgrossfb, totalother, totgrossother, longcharoutput, ci_date, co_date, arrtime, deptime, co_date_abreise, actualdatum, serv1, serv2, vat2, vat3, vat4, fact, fact1, fact2, vat1, service1, counter, todate, curr_time, htparam, guest, reslin_queasy, queasy, h_artikel, zimkateg, reservation, sourccod, segment, res_line, genstat, nation, arrangement, artikel, mc_guest, waehrung, bediener, zimmer, bill, bill_line, h_bill, h_bill_line
        nonlocal datum, propid, last_recid
        nonlocal gmember, gcomp, rqueasy, qsy19, qsy2, buffart, datalist, qsy231, qsy289
        nonlocal data_list, gmember, gcomp, rqueasy, qsy19, qsy2, buffart, datalist, qsy231, qsy289
    ...
    def find_last_roomnumber(resno:int, guestno:int):
        ...
        for resbuff in db_session.query(Resbuff).filter(
                (Resbuff.gastnrmember == guestno) & (Resbuff.resnr != resno) & (Resbuff.resstatus == 8) & (Resbuff.abreise > resdate)).order_by(Resbuff._recid).all():
    ...
    def generate_tabledata(casetyp:str):
        ...
        if do_it:
            ...
            if ((datum1 >= datum - timedelta(days=90) and datum1 <= datum + timedelta(days=730)) or casetyp.lower()  == ("CheckedOut").lower()) and res_line.resstatus != 99 and res_line.resstatus != 12 and res_line.l_zuordnung[2] == 0:
                do_it = True
        ...
        if do_it:
            data_list = query(data_list_data, filters=(lambda data_list: data_list.resnr == res_line.resnr and data_list.reslinnr.lower()  == (reslinnr_str).lower()  and data_list.acdate.lower()  == (actualdatum).lower()), first=True)
            ...
            if not data_list or casetyp.lower()  == "cancel" :
                ...
                data_list.reslinnr = reslinnr_str
                ...
                if lodging > 0:
                    if lodging > 0:
                        t_rmrev =  to_decimal(t_rmrev + lodging)

                    if breakfast > 0 or lunch > 0 or dinner > 0:
                        t_fbrev =  to_decimal(t_fbrev + breakfast + lunch + dinner)

                    if others > 0:
                        t_others =  to_decimal(t_others + others)
                ...
                data_list.totrev =  to_decimal(data_list.rmrev + data_list.fbrev + data_list.others)
                ...
                data_list.totgross =  to_decimal(data_list.grossrm + data_list.grossfb + data_list.grossothers)
                ...
                if res_line.abreisezeit != 0:
                    ...
                    deptime = "00:00:00"
                data_list.arrdate = ci_date + " " + arrtime
                data_list.depdate = co_date + " " + deptime

                data_list.comp = "NULL"
                data_list.company__crm = "NULL"
                data_list.travelagent__crm = "NULL"
                data_list.companyid = "NULL"
                data_list.travelagentid = "NULL"
    ...
    def get_resstatus(ttype:str, stat:int):
    ...
    todate = datum + timedelta(days=720)
    ...
    segment = Segment()
    queasy = Queasy()
    for res_line.reserve_char, res_line.resnr, res_line.reslinnr, res_line.abreise, res_line.ankunft, res_line.resstatus, res_line.l_zuordnung, res_line.zipreis, res_line.erwachs, res_line.kind1, res_line.kind2, res_line.bemerk, res_line.cancelled, res_line._recid, res_line.ankzeit, res_line.abreisezeit, res_line.gastnr, res_line.betriebsnr, res_line.changed_id, res_line.zinr, res_line.zimmer_wunsch, res_line.gastnrmember, gmember.gastnr, gmember.vorname1, gmember.name, gmember.anrede1, gmember.adresse1, gmember.adresse2, gmember.mobil_telefon, gmember.telefon, gmember.plz, gmember.fax, gmember.wohnort, gmember.geburt_ort2, gmember.nation1, gmember.geburtdatum1, gmember.email_adr, gmember.geburt_ort1, gmember.ausweis_nr1, gmember.geschlecht, gmember._recid, gmember.anredefirma, gmember.karteityp, zimkateg.kurzbez, zimkateg.bezeichnung, zimkateg._recid, reservation.resdat, reservation.groupname, reservation.useridanlage, reservation._recid, sourccod.bezeich, sourccod._recid, segment.bezeich, segment.bemerkung, segment._recid, queasy.char3, queasy._recid, queasy.char2, queasy.char1 in db_session.query(Res_line.reserve_char, Res_line.resnr, Res_line.reslinnr, Res_line.abreise, Res_line.ankunft, Res_line.resstatus, Res_line.l_zuordnung, Res_line.zipreis, Res_line.erwachs, Res_line.kind1, Res_line.kind2, Res_line.bemerk, Res_line.cancelled, Res_line._recid, Res_line.ankzeit, Res_line.abreisezeit, Res_line.gastnr, Res_line.betriebsnr, Res_line.changed_id, Res_line.zinr, Res_line.zimmer_wunsch, Res_line.gastnrmember, Gmember.gastnr, Gmember.vorname1, Gmember.name, Gmember.anrede1, Gmember.adresse1, Gmember.adresse2, Gmember.mobil_telefon, Gmember.telefon, Gmember.plz, Gmember.fax, Gmember.wohnort, Gmember.geburt_ort2, Gmember.nation1, Gmember.geburtdatum1, Gmember.email_adr, Gmember.geburt_ort1, Gmember.ausweis_nr1, Gmember.geschlecht, Gmember._recid, Gmember.anredefirma, Gmember.karteityp, Zimkateg.kurzbez, Zimkateg.bezeichnung, Zimkateg._recid, Reservation.resdat, Reservation.groupname, Reservation.useridanlage, Reservation._recid, Sourccod.bezeich, Sourccod._recid, Segment.bezeich, Segment.bemerkung, Segment._recid, Queasy.char3, Queasy._recid, Queasy.char2, Queasy.char1).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Segment,(Segment.segmentcode == Reservation.segmentcode)).join(Queasy,(Queasy.key == 152) & (Queasy.number1 == Zimkateg.typ)).filter(
            (Res_line._recid > last_recid) & (((Res_line.ankunft <= datum) & (Res_line.abreise >= datum)) | (((Res_line.resstatus == 1) | (Res_line.resstatus == 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 3) | (Res_line.resstatus == 4) | (Res_line.resstatus == 11)) & (Res_line.active_flag == 0)) | ((Res_line.resstatus == 9) & ((Res_line.ankunft <= datum) & (Res_line.abreise >= datum)))) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line.resstatus != 12) & (Res_line.ankunft < todate)).order_by(Res_line._recid).all():
