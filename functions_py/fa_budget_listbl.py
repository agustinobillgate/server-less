...
def fa_budget_listbl(ytd_flag:bool, from_date:date, to_date:date, ytd_date:date, detailed:bool):
    ...
    fa_budget_ytd_data, Fa_budget_ytd = create_model("Fa_budget_ytd", {"asset":string, "descrip_str":string, "account_no":string, "anzahl_str":string, "amount_str":string, "mtd_budget_str":string, "mtd_variance_str":string, "anzahlytd_str":string, "amountytd_str":string, "ytd_budget_str":string, "ytd_variance_str":string})
    fa_budget_period_data, Fa_budget_period = create_model("Fa_budget_period", {"asset":string, "asset_date":date, "descrip_str":string, "account_no":string, "price_str":string, "anzahl_str":string, "amount_str":string, "budget_str":string, "variance_str":string, "budget_date":date})
        ...
        for fa_order, fa_op, fa_artikel, mathis in db_session.query(Fa_order, Fa_op, Fa_artikel, Mathis).join(Fa_op,(Fa_op.loeschflag <= 1) & (Fa_op.opart == 1) & (Fa_op.anzahl > 0) & (Fa_op.docu_nr == Fa_order.order_nr) & (Fa_op.datum >= start_jan) & (Fa_op.datum <= ytd_date)).join(Fa_artikel,(Fa_artikel.nr == Fa_op.nr)).join(Mathis,(Mathis.nr == Fa_op.nr)).filter(
            ...
            if fa_order.activereason.lower()  != (nr_budget).lower() :
                ...
                if nr_budget != "":
                    ...
                    fa_budget_ytd_data.append(fa_budget_ytd)
