...
# - fix cannot save l_orderhdr.angebot_lief and l_orderhdr.lief_fax
# - added flag_modified for angebot_lief and lief_fax
# -------------------------------------------------------
...
def prepare_mk_po1_web_1bl(docu_nr: string, pvilanguage: int, lief_nr: int, pr_deptnr: int, po_type: int, potype: int, bediener_username: string, ordername_screen_value: string, crterm: int):
    ...
    def generate_output():
        ...
        # log.write_log('prepare_mk_po1_web_1bl',
                    #   f't_l_orderhdr: {t_l_orderhdr_data}', 'outputFile.txt')
    ...
    def new_po_number():
        ...
        get_l_orderhdr1 = (
            db_session.query(L_orderhdr1)
            .filter(
                (L_orderhdr1.bestelldatum == billdate) &
                (L_orderhdr1.betriebsnr <= 1) &
                (matches(L_orderhdr1.docu_nr, "P*"))
            )
            .order_by(L_orderhdr1.docu_nr.desc())
        )
        for l_orderhdr1 in get_l_orderhdr1:
    ...
    if po_type == 1:
        ...
        l_orderhdr = (
            db_session.query(L_orderhdr).filter(
                (L_orderhdr.lief_nr == lief_nr) &
                (L_orderhdr.docu_nr == (docunr).lower()))
            .with_for_update()
            .first()
        )
        ...
        # print(
        #     f'[DEBUG] angerbot_lief: {l_orderhdr.angebot_lief}, deptnr: {pr_deptnr}, crterm: {crterm}, local_nr: {local_nr}')

        # log.write_log('prepare_mk_po1_web_1bl',
        #               f'l_orderhdr: {l_orderhdr.__dict__}', 'outputFile.txt')
    ...
    l_orderhdr = get_cache(
        L_orderhdr, {"lief_nr": [(eq, lief_nr)], "docu_nr": [(eq, docunr)]})
    ...
    get_l_order = (
        ...
        .filter(
            (L_order.docu_nr == (docunr).lower()) &
            (L_order.pos > 0)
        )
        .order_by(L_order._recid)
    )
    for l_order in get_l_order:
        t_l_order = T_l_order()
        ...
        if l_order.lief_nr == lief_nr and l_order.loeschflag == 0 and docunr != "":
            ...
            if length(l_order.quality) >= 5:
                t_l_order.disc = to_int(substring(
                    l_order.quality, 0, 2)) + to_int(substring(l_order.quality, 3, 2)) / 100

            if length(l_order.quality) >= 11:
                t_l_order.vat = to_int(substring(
                    l_order.quality, 6, 2)) + to_int(substring(l_order.quality, 9, 2)) / 100

            if length(l_order.quality) >= 17:
                t_l_order.disc2 = to_int(substring(
                    l_order.quality, 12, 2)) + to_int(substring(l_order.quality, 15, 2)) / 100
    ...
    get_parameters = (
        ...
        .filter(
            (Parameters.progname == ("CostCenter").lower()) &
            (Parameters.section == ("Name").lower())
        )
