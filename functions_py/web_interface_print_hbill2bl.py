...
def web_interface_print_hbill2bl(pvilanguage:int, session_parameter:str, user_init:str, hbrecid:int, printnr:int, use_h_queasy:bool, hbill_stat_str:str, print_all:bool):
    ...
    art_list_submenu_data, Art_list_submenu = create_model_like(H_mjourn, {
        "hline_recid":int, 
        "is_deleted":bool})
    ...
    def print_overhead1():
        ...
        if printed_line == 0 or print_all:
            ...
            if h_bill.bilname != "":
                if gst_logic:
                    output_list.str = output_list.str + to_string(" " + to_string(bill_date) + " " + to_string(time_lastposted , "HH:MM") + " " + translateExtended ("Tax Invoice No", lvcarea, "") + " " + to_string(rechnr_str, "x(43)"))
                else:
                    output_list.str = output_list.str + to_string(" " + to_string(bill_date) + " " + to_string(time_lastposted , "HH:MM") + " " + translateExtended ("Bill", lvcarea, "") + " " + to_string(rechnr_str, "x(9)"))
                output_list = Output_list()
                output_list_data.append(output_list)
                ...
                output_list.sort_i = sort_i
                sort_i = sort_i + 1
                output_list.str = output_list.str + to_string(" " + translateExtended ("Table", lvcarea, "") + " " + to_string(h_bill.tischnr, ">>>>>9") + "/" + to_string(h_bill.belegung, "->>9 ") + kname, "x(50)")
                output_list = Output_list()
                output_list_data.append(output_list)

                output_list.sort_i = sort_i
                sort_i = sort_i + 1
                output_list.str = output_list.str + to_string(" ") + to_string(translateExtended ("Guest", lvcarea, "")) + " " + trim(to_string(h_bill.bilname, "x(64)"))
    ...
    def print_overhead3():
        ...
        for art_list in query(art_list_data, filters=(lambda art_list: art_list.artart != 0)):
            ...
            if (art_list.artart == 11 or art_list.artart == 12) and comp_taxserv and comp_flag:
                if art_list.amount * subtotal > 0:
                    ...
                    amt =  - to_decimal(subtotal)
                balance =  to_decimal(balance + amt)
                last_amount =  to_decimal(last_amount + amt)
                compli_notax = True
                ...
                if not art_list.printed or print_all:
                    balance =  to_decimal(balance + art_list.amount)
                last_amount =  to_decimal(last_amount + art_list.amount)
        ...
        if h_bill.bilname != "":
            ...
            output_list.str = output_list.str + to_string(translateExtended ("Guest :", lvcarea, "")) + to_string(" ") + trim(to_string(h_bill.bilname, "x(64)"))
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    def cal_totalfb():
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).filter(
            ...
            if h_artikel.artnr == disc_art1:
                total_fdisc =  to_decimal(total_fdisc + fact * h_bill_line.epreis)

            elif h_artikel.artnr == disc_art2:
                total_bdisc =  to_decimal(total_bdisc + fact * h_bill_line.epreis)

            elif h_artikel.artnr == disc_art3:
                total_odisc =  to_decimal(total_odisc + fact * h_bill_line.epreis)
            else:
                ...
                if artikel.umsatzart == 3 or artikel.umsatzart == 5:
                    total_food =  to_decimal(total_food + fact * h_bill_line.anzahl * h_bill_line.epreis)

                elif artikel.umsatzart == 6:
                    total_bev =  to_decimal(total_bev + fact * h_bill_line.anzahl * h_bill_line.epreis)

                elif artikel.umsatzart == 4:
                    total_other =  to_decimal(total_other + fact * h_bill_line.anzahl * h_bill_line.epreis)
    ...
    def print_net_vat():
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0) & (H_artikel.mwst_code != 0)).filter(
            ...
            if htparam.fdecimal != 0:
                ...
                vat_list.betrag_amt =  to_decimal(vat_list.betrag_amt + h_bill_line.betrag)
        ...
        if vat_list:
            ...
            for vat_list in query(vat_list_data):
                ...
                net_amt =  to_decimal(net_amt + vat_list.betrag_amt) / to_decimal((1 + vat_list.vat_amt) / to_decimal(100))
    ...
    if printnr != 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
                ...
                if h_bill_line.artnr == f_discart:
                    if h_bill_line.epreis != h_bill_line.betrag:
                        if not serv_disc and h_bill_line.artnr == f_discart:
                            ...
                            h_service =  to_decimal(h_bill_line.betrag / fact * serv_percent)
                        ...
                        if not vat_disc and h_bill_line.artnr == f_discart:
                            ...
                            h_mwst =  to_decimal(h_bill_line.betrag / fact * mwst_percent)
            ...
            if price_decimal == 0:
                art_list.amount = to_decimal(round(art_list.amount + amount , 0))
            else:
