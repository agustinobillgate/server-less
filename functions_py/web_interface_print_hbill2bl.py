...
"""_yusufwijasena_30/10/2025
    ...
    Ticket ID: F6D79E
        _remark_:   - last update from FDL: 5367B9
"""
...
from functions_py.prepare_print_hbill1bl import prepare_print_hbill1bl
from models import H_mjourn, Printer, Artikel, H_bill, H_bill_line, Htparam, Hoteldpt, Queasy, H_queasy, Kellner, H_artikel, Kontplan, Paramtext, Bediener, Tisch, Res_line, Mc_guest, Bill, Printcod, Waehrung


def web_interface_print_hbill2bl(pvilanguage: int, session_parameter: str, user_init: str, hbrecid: int, printnr: int, use_h_queasy: bool, hbill_stat_str: str, print_all: bool):

    prepare_cache([Artikel, H_bill, H_bill_line, Htparam, Hoteldpt, Queasy, H_queasy, Kellner, Kontplan, Paramtext, Bediener, Tisch, Res_line, Mc_guest, Bill, Printcod, Waehrung])
    ...
    tot_line: int = 0
    h_service = to_decimal("0.0")
    h_mwst = to_decimal("0.0")
    serv_percent = to_decimal("0.0")
    mwst_percent = to_decimal("0.0")
    fact: Decimal = 1
    mwst1 = to_decimal("0.0")
    ...
    overhead3: int = 0
    overhead4: int = 0
    total_food = to_decimal("0.0")
    total_bev = to_decimal("0.0")
    total_other = to_decimal("0.0")
    total_fdisc = to_decimal("0.0")
    total_bdisc = to_decimal("0.0")
    total_odisc = to_decimal("0.0")
    ...
    time_lastposted: int = 0
    recid_h_bill_line: int = 0
    mjourn_qty: int = 0
    h_mjourn = printer = artikel = h_bill = h_bill_line = htparam = hoteldpt = queasy = h_queasy = kellner = h_artikel = kontplan = paramtext = bediener = tisch = res_line = mc_guest = bill = printcod = waehrung = None

    art_list = art_list_submenu = t_printer = vat_list = output_list = artbuff = abuff = bart_list_submenu = None
    ...
    def cal_servat(depart: int, h_artnr: int, service_code: int, mwst_code: int, inpdate: date):
        ...
        if bill_date < bill_date110 and (service_code != 0 or mwst_code != 0):
            ...
            abuff = get_cache(
                Artikel, {"artnr": [(eq, hbuff.artnrfront)], "departement": [(eq, depart)]})

            kontplan = get_cache(
                Kontplan, {"betriebsnr": [(eq, depart)], "kontignr": [(eq, abuff.artnr)], "datum": [(eq, inpdate)]})

            if kontplan:
                serv_htp = to_decimal(kontplan.anzkont / 10000)
                vat_htp = to_decimal(kontplan.anzconf / 10000)

                serv_percent = to_decimal(serv_htp)
                mwst_percent = to_decimal(vat_htp)
                servat = to_decimal(1 + serv_percent + mwst_percent)
        ...
        if service_code != 0:
            ...
            serv_htp = to_decimal(htparam.fdecimal / 100)

        if mwst_code != 0:
            htparam = get_cache(Htparam, {"paramnr": [(eq, mwst_code)]})
            vat_htp = to_decimal(htparam.fdecimal / 100)

        if service_taxable:
            serv_percent = to_decimal(serv_htp)
            mwst_percent = to_decimal((1 + serv_htp) * vat_htp)
            servat = to_decimal(1 + serv_percent + mwst_percent)
    ...
    def add_unitprice_text():
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, serv_percent, mwst_percent, fact, mwst1, subtotal, bline_exist, qty1000, i, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, str451, sc_art, mwst_perc, serv_perc, serv_code, vat_code, servtax_use_foart, time_lastposted, recid_h_bill_line, mjourn_qty, h_mjourn, printer, artikel, h_bill, h_bill_line, htparam, hoteldpt, queasy, h_queasy, kellner, h_artikel, kontplan, paramtext, bediener, tisch, res_line, mc_guest, bill, printcod, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, hbill_stat_str, print_all
        nonlocal artbuff, abuff, bart_list_submenu
        nonlocal art_list, art_list_submenu, t_printer, vat_list, output_list, artbuff, abuff, bart_list_submenu
        nonlocal art_list_data, art_list_submenu_data, t_printer_data, vat_list_data, output_list_data
        ...
        for art_list in query(art_list_data, filters=(lambda art_list: (art_list.qty != 1) and (art_list.qty != -1) and art_list.qty != 0 and art_list.artnr != 0)):
            ...
            if length(art_list.bezeich + s) <= nbezeich:
                ...
                art_list.bezeich = substring(
                    art_list.bezeich, 0, nbezeich - length(s)) + s
    ...
    def check_pages():
        ...
        art_list = query(art_list_data, filters=(
            lambda art_list: art_list.artart == 0), first=True)
    ...
    def print_overhead1():
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, serv_percent, mwst_percent, fact, mwst1, subtotal, bline_exist, qty1000, n, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, str451, sc_art, mwst_perc, serv_perc, serv_code, vat_code, servtax_use_foart, time_lastposted, recid_h_bill_line, mjourn_qty, h_mjourn, printer, artikel, h_bill, h_bill_line, htparam, hoteldpt, queasy, h_queasy, kellner, h_artikel, kontplan, paramtext, bediener, tisch, res_line, mc_guest, bill, printcod, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, hbill_stat_str, print_all
        nonlocal artbuff, abuff, bart_list_submenu
        nonlocal art_list, art_list_submenu, t_printer, vat_list, output_list, artbuff, abuff, bart_list_submenu
        nonlocal art_list_data, art_list_submenu_data, t_printer_data, vat_list_data, output_list_data
        ...
        if printed_line == 0 or print_all:
            ...
            if header1 != "":
                for i in range(1, length(header1) + 1):
                    output_list.str = output_list.str + \
                        to_string(substring(header1, i - 1, 1), "x(1)")
                # output_list.str = output_list.str + to_string("")
                output_list.str = f"{output_list.str} "
                output_list = Output_list()
                output_list_data.append(output_list)
            ...
            if header2 != "":
                for i in range(1, length(header2) + 1):
                    output_list.str = output_list.str + \
                        to_string(substring(header2, i - 1, 1), "x(1)")
                # output_list.str = output_list.str + to_string("")
                output_list.str = f"{output_list.str} "
                output_list = Output_list()
                output_list_data.append(output_list)
            ...
            if h_bill.bilname != "":
                if gst_logic:
                    output_list.str = output_list.str + to_string(" " + to_string(bill_date) + " " + to_string(
                        time_lastposted, "HH:MM") + " " + translateExtended("Tax Invoice No", lvcarea, "") + " " + to_string(rechnr_str, "x(43)"))
                else:
                    output_list.str = output_list.str + to_string(" " + to_string(bill_date) + " " + to_string(
                        time_lastposted, "HH:MM") + " " + translateExtended("Bill", lvcarea, "") + " " + to_string(rechnr_str, "x(9)"))
                output_list = Output_list()
                output_list_data.append(output_list)
                ...
                sort_i = sort_i + 1
                output_list.str = output_list.str + \
                    to_string(" " + hoteldpt.depart, "x(32)")
                output_list = Output_list()
                output_list_data.append(output_list)

                output_list.sort_i = sort_i
                sort_i = sort_i + 1
                output_list.str = output_list.str + to_string(" " + translateExtended("Table", lvcarea, "") + " " + to_string(
                    h_bill.tischnr, ">>>>>9") + "/" + to_string(h_bill.belegung, "->>9 ") + kname, "x(50)")
                output_list = Output_list()
                output_list_data.append(output_list)

                output_list.sort_i = sort_i
                sort_i = sort_i + 1
                output_list.str = output_list.str + to_string(" ") + to_string(translateExtended(
                    "Guest", lvcarea, "")) + " " + trim(to_string(h_bill.bilname, "x(64)"))

                if prtabledesc:
                    tisch = get_cache(
                        Tisch, {"tischnr": [(eq, h_bill.tischnr)], "departement": [(eq, h_bill.departement)]})

                    if tisch and tisch.bezeich != "":
                        output_list.str = output_list.str + \
                            to_string(" " + tisch.bezeich, "x(32)")
                        output_list = Output_list()
                        output_list_data.append(output_list)
                ...
                if gst_logic:
                    ...
                    output_list.str = output_list.str +\
                        to_string(
                            " " + translateExtended("Tax Invoice", lvcarea, ""))
                    ...
                    output_list.str = output_list.str +\
                        to_string(
                            " " + translateExtended("GST ID : 001865060352", lvcarea, ""))
                ...
                if h_bill.resnr > 0 and h_bill.reslinnr > 0:
                    ...
                    if bill:
                        mc_guest = db_session.query(Mc_guest).filter(
                            (Mc_guest.gastnr == bill.gastnr) & (Mc_guest.activeflag)).first()
                    else:
                        mc_guest = db_session.query(Mc_guest).filter(
                            (Mc_guest.gastnr == h_bill.resnr) & (Mc_guest.activeflag)).first()

                if mc_guest:
                    output_list.str = output_list.str + to_string(" " + translateExtended(
                        "MemberCard", lvcarea, "") + " " + mc_guest.cardnum, "x(32)")
                    output_list = Output_list()
                    output_list_data.append(output_list)
    ...
    def print_billine():
        ...
        if qty1000:
            output_list.str = output_list.str + \
                to_string(" ") + to_string(art_list.qty, "->>>>") + to_string(" ")
        else:
            if art_list.qty > 99 or art_list.qty < -99:
                output_list.str = output_list.str + \
                    to_string(" ") + to_string(art_list.qty) + to_string(" ")
            ...
            elif art_list.qty < 0 and art_list.qty >= -8:
                output_list.str = output_list.str + \
                    to_string(" ") + to_string(art_list.qty) + to_string(" ")
            else:
                output_list.str = output_list.str + \
        ...
        for i in range(1, nbezeich + 1):
            if i > length(bezeich):
                ...
                output_list.str = output_list.str + \
                    to_string(substring(bezeich, i - 1, 1), "x(1)")
        ...
        if prtwoline:
            ...
            if price_decimal == 0:
                if not long_digit:
                    output_list.str = output_list.str + \
                        to_string(art_list.amount, "->>>,>>>,>>9")
                else:
                    output_list.str = output_list.str + \
                        to_string(art_list.amount, "->>>,>>>,>>9")
            else:
                output_list.str = output_list.str + \
                    to_string(art_list.amount, "->>>,>>>,>>9.99")
            curr_j = curr_j + 1
            printed_line = printed_line + 1
        ...
        if prtwoline:
            ...
            if price_decimal == 0:
                ct = "@" + to_string(art_list.price, "->,>>>,>>9") + leerch + \
                    to_string(art_list.amount, "->>>,>>>,>>9") + \
                    chr_unicode(10)
            else:
                ct = "@" + to_string(art_list.price, "->>,>>9.99") + leerch + \
                    to_string(art_list.amount, "->>>,>>9.99") + chr_unicode(10)
        ...
        for h_mjourn in db_session.query(H_mjourn).filter(
                (H_mjourn.departement == art_list.dept) & (H_mjourn.h_artnr == art_list.artnr) & (H_mjourn.rechnr == h_bill.rechnr) & (H_mjourn.bill_datum == art_list.datum) & (H_mjourn.sysdate == art_list.sysdate) & (H_mjourn.zeit == art_list.zeit) & (H_mjourn.nr == art_list.condiment) & (num_entries(H_mjourn.request, "|") > 1) & (to_int(entry(0, H_mjourn.request, "|")) == art_list.hline_recid)).order_by(H_mjourn._recid).all():

            h_art = db_session.query(H_art).filter(
                # (H_art.artnr == h_mjourn.artnr) & (H_art.departement == dept)).first()
                (H_art.artnr == h_mjourn.artnr) & (H_art.departement == art_list.dept)).first()

            if h_art:
                bezeich = h_art.bezeich
                mjourn_qty = h_mjourn.anzahl

                if mjourn_qty > 0:
                    # output_list.str = output_list.str + to_string(" ") + to_string(mjourn_qty) + to_string(" ")
                    output_list.str = f"{output_list.str} {mjourn_qty} "
    ...
    def print_overhead2(prall_flag: int):
        ...
        if service != 0 or mwst != 0:
            ...
            for i in range(1, nbez1 + 1):
                if i > length(s):
                    ...
                    output_list.str = output_list.str + \
                        to_string(substring(s, i - 1, 1), "x(1)")

            if price_decimal == 0:
                if not long_digit:
                    output_list.str = output_list.str + \
                        to_string(subtotal, "->>>,>>>,>>9")
                    output_list = Output_list()
                    output_list_data.append(output_list)
            ...
            if service != 0:
                ...
                for i in range(1, nbez1 + 1):
                    if i > length(service_str):
                        ...
                        output_list.str = output_list.str + \
                            to_string(substring(service_str, i - 1, 1), "x(1)")
            ...
            if mwst != 0:
                ...
                for i in range(1, nbez1 + 1):
                    if i > length(mwst_str):
                        ...
                        output_list.str = output_list.str + \
                            to_string(substring(mwst_str, i - 1, 1), "x(1)")
        ...
        for i in range(1, nbez1 + 1):
            if i > length(tot_str):
                ...
                output_list.str = output_list.str + \
                    to_string(substring(tot_str, i - 1, 1), "x(1)")
    ...
    def print_overhead3():
        ...
        for art_list in query(art_list_data, filters=(lambda art_list: art_list.artart != 0)):
            if new_sold_item or not art_list.printed:
                ...
                for i in range(1, nbez1 + 1):
                    if i > length(art_list.bezeich):
                        ...
                        output_list.str = output_list.str + \
                            to_string(
                                substring(art_list.bezeich, i - 1, 1), "x(1)")
            ...
            if (art_list.artart == 11 or art_list.artart == 12) and comp_taxserv and comp_flag:
                ...
                last_amount = to_decimal(
                    last_amount) + to_decimal(art_list.amount)
        ...
        if not compli_notax:
            ...
            for h_bill_line in db_session.query(H_bill_line).filter(
                    (H_bill_line.rechnr == h_bill.rechnr) & (H_bill_line.departement == h_bill.departement)).order_by(H_bill_line._recid).all():
                balance = to_decimal(balance + h_bill_line.betrag)
        ...
        if (balance >= 0):
            if not rm_transfer or print_balance or balance != 0:
                ...
                for i in range(1, nbez1 + 1):
                    if i > length(bal_str):
                        ...
                        output_list.str = output_list.str + \
                            to_string(substring(bal_str, i - 1, 1), "x(1)")
            ...
            for i in range(1, nbez1 + 1):
                if i > length(chg_str):
                    ...
                    output_list.str = output_list.str + \
                        to_string(substring(chg_str, i - 1, 1), "x(1)")
        ...
        if h_bill.flag == 1 and hbill_stat_str.lower() == "closed":
            if qty1000:
                output_list.str = output_list.str + to_string("", "x(6)")
            else:
                output_list.str = output_list.str + to_string("", "x(5)")
            ...
            for i in range(1, pos + 1):
                output_list.str = f"{output_list.str} "
            # output_list.str = output_list.str + to_string("REPRINT - CLOSED BILL")
            output_list.str = f"{output_list.str} REPRINT - CLOSED BILL"
            output_list = Output_list()
            output_list_data.append(output_list)
        ...
        if h_bill.bilname != "":
            if h_bill.resnr > 0 and h_bill.reslinnr > 0:
                ...
                if res_line:
                    ...
                    output_list.str = output_list.str + \
                        to_string(translateExtended(
                            "Room  :", lvcarea, "") + " " + res_line.zinr)
                    output_list = Output_list()
                    output_list_data.append(output_list)
            ...
            output_list.str = output_list.str + to_string(translateExtended(
                "Guest :", lvcarea, "")) + to_string(" ") + trim(to_string(h_bill.bilname, "x(64)"))
            output_list = Output_list()
            output_list_data.append(output_list)
            ...
            output_list.str = output_list.str + to_string(translateExtended(
                "Time :", lvcarea, "") + " " + to_string(get_current_time_in_seconds(), "HH:MM:SS"), "x(20)")
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    def print_overhead4():
        ...
        if anz_foot > 0:
            ...
            output_list.str = f"{output_list.str} {foot1}"
            output_list.pos = 6
            output_list = Output_list()
            output_list_data.append(output_list)
        ...
        if foot2 != "":
            output_list.str = f"{output_list.str} {foot2}"
            output_list.pos = 6
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    def cut_it():
        ...
        if printcod:
            ...
            output_list.str = f"{output_list.str} {printcod.contcod}"
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    def cal_totalfb():
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, serv_percent, mwst_percent, mwst1, subtotal, bline_exist, qty1000, i, n, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, str451, sc_art, mwst_perc, serv_perc, serv_code, vat_code, servtax_use_foart, time_lastposted, recid_h_bill_line, mjourn_qty, h_mjourn, printer, artikel, h_bill, h_bill_line, htparam, hoteldpt, queasy, h_queasy, kellner, h_artikel, kontplan, paramtext, bediener, tisch, res_line, mc_guest, bill, printcod, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, hbill_stat_str, print_all
        nonlocal artbuff, abuff, bart_list_submenu
        nonlocal art_list, art_list_submenu, t_printer, vat_list, output_list, artbuff, abuff, bart_list_submenu
        nonlocal art_list_data, art_list_submenu_data, t_printer_data, vat_list_data, output_list_data
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel, (H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).filter(
                (H_bill_line.bill_datum == billdate) & (H_bill_line.departement == h_bill.departement) & (H_bill_line.rechnr == h_bill.rechnr)).order_by(H_bill_line._recid).all():
            if h_bill_line_obj_list.get(h_bill_line._recid):
                continue
            else:
                h_bill_line_obj_list[h_bill_line._recid] = True
            ...
            if h_artikel.artnr == disc_art1:
                total_fdisc = to_decimal(
                    total_fdisc + fact * h_bill_line.epreis)

            elif h_artikel.artnr == disc_art2:
                total_bdisc = to_decimal(
                    total_bdisc + fact * h_bill_line.epreis)

            elif h_artikel.artnr == disc_art3:
                total_odisc = to_decimal(
                    total_odisc + fact * h_bill_line.epreis)
            else:
                ...
                if artikel.umsatzart == 3 or artikel.umsatzart == 5:
                    total_food = to_decimal(
                        total_food + fact * h_bill_line.anzahl * h_bill_line.epreis)

                elif artikel.umsatzart == 6:
                    total_bev = to_decimal(
                        total_bev + fact * h_bill_line.anzahl * h_bill_line.epreis)

                elif artikel.umsatzart == 4:
                    total_other = to_decimal(
                        total_other + fact * h_bill_line.anzahl * h_bill_line.epreis)
    ...
    def multi_currency():
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, serv_percent, mwst_percent, fact, mwst1, subtotal, bline_exist, qty1000, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, str451, sc_art, mwst_perc, serv_perc, serv_code, vat_code, servtax_use_foart, time_lastposted, recid_h_bill_line, mjourn_qty, h_mjourn, printer, artikel, h_bill, h_bill_line, htparam, hoteldpt, queasy, h_queasy, kellner, h_artikel, kontplan, paramtext, bediener, tisch, res_line, mc_guest, bill, printcod, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, hbill_stat_str, print_all
        nonlocal artbuff, abuff, bart_list_submenu
        nonlocal art_list, art_list_submenu, t_printer, vat_list, output_list, artbuff, abuff, bart_list_submenu
        nonlocal art_list_data, art_list_submenu_data, t_printer_data, vat_list_data, output_list_data
        ...
        for i in range(1, num_entries(htparam.fchar, ";") + 1):
            ...
            if waehrung:
                exchg_rate = to_decimal(waehrung.ankauf / waehrung.einheit)
                foreign_amt = to_decimal(round(tot_amount / exchg_rate, 2))
                s = waehrung.wabkurz + " " + \
                    translateExtended("TOTAL", lvcarea, "")
                ...
                for n in range(1, (nbezeich - 2) + 1):
                    if n > length(s):
                        ...
                        output_list.str = output_list.str + \
                            to_string(substring(s, n - 1, 1), "x(1)")
                output_list.str = output_list.str + \
                    to_string(foreign_amt, "->>>,>>>,>>9.99")
                output_list = Output_list()
                output_list_data.append(output_list)
    ...
    def print_net_vat():
        ...
        lpage1: int = 0
        net_amt = to_decimal("0.0")
        vat_str = ""
        vat_proz = to_decimal("0.0")
        vat_num: int = 0
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel, (H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0) & (H_artikel.mwst_code != 0)).filter(
            ...
            htparam = get_cache(
                Htparam, {"paramnr": [(eq, h_artikel.mwst_code)]})

            if htparam.fdecimal != 0:
                vat_list = query(vat_list_data, filters=(
                    lambda vat_list: vat_list.vat_amt == htparam.fdecimal), first=True)
                ...
                vat_list.betrag_amt = to_decimal(
                    vat_list.betrag_amt + h_bill_line.betrag)
        ...
        if vat_list:
            ...
            for vat_list in query(vat_list_data):
                ...
                net_amt = to_decimal(net_amt + vat_list.betrag_amt) / \
                    to_decimal((1 + vat_list.vat_amt) / to_decimal(100))
                vat_list_data.remove(vat_list)
            net_amt = to_decimal(round(net_amt, price_decimal))
            mwst1 = to_decimal(tot_sales - net_amt)

        else:
            net_amt = to_decimal(tot_amount - mwst1)
        ...
        if qty1000:
            ...
            sort_i = sort_i + 1
            output_list.str = output_list.str + \
                to_string(translateExtended("NET", lvcarea, ""))
            output_list.pos = 7
            output_list.str = f"{output_list.str} "
        else:
            output_list.str = f"{output_list.str} "
        ...
        if qty1000:
            ...
            output_list.str = f"{output_list.str} {vat_str}"
            output_list.pos = 7
            output_list.str = f"{output_list.str} "
        else:
            output_list.str = f"{output_list.str} {vat_str}"
    ...
    def print_in_word():
        ...
        for i in range(1, length(str1) + 1):
            output_list.str = output_list.str + \
                to_string(substring(str1, i - 1, 1), "x(1)")
        output_list.str = output_list.str + to_string("")
        output_list = Output_list()
        output_list_data.append(output_list)
        ...
        if trim(str2) != "":
            ...
            for i in range(1, length(str2) + 1):
                output_list.str = output_list.str + \
                    to_string(substring(str2, i - 1, 1), "x(1)")
            output_list.str = output_list.str + to_string("")
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    if sc_art != 0:
        ...
        if h_bill_line:
            print_all = True
            print_all, filename, msg_str, winprinterflag, output_list_data, t_printer_data = get_output(
                print_hbill1_phbl(pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all))
    ...
    if gst_logic == False:
        htparam = get_cache(Htparam, {
            "paramnr": [(eq, 1027)],
            "bezeichnung": [(ne, "not used")]})

        if htparam:
            gst_logic = htparam.flogical
    ...
    if printnr > 0:
        ...
        if not printer:
            msg_str = translateExtended(
                "No such Printer Number:", lvcarea, "") + " " + to_string(printnr)
    ...
    order_id, prdisc_flag, disc_art1, disc_art2, disc_art3, disc_zwkum, print_balance, incl_service, incl_mwst, service_taxable, print_fbtotal = get_output(
        prepare_print_hbill1bl(hbrecid))
    ...
    if printnr != 0:
        ...
        artikel = get_cache(Artikel, {
            "artnr": [(eq, htparam.finteger)],
            "departement": [(eq, 0)]})

        if artikel:
            artbuff = get_cache(Artikel, {
                "artnr": [(eq, artikel.artnr)],
                "departement": [(eq, h_bill.departement)]})
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
                (H_bill_line.rechnr == h_bill.rechnr) & (H_bill_line.departement == h_bill.departement)).order_by(H_bill_line.bill_datum, H_bill_line.zeit.desc()).all():
            time_lastposted = h_bill_line.zeit
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if h_bill_line.artnr != 0:
                ...
                if (h_artikel.artart == 11 or h_artikel.artart == 12):
                    ...
                    tot_sales = to_decimal(tot_sales + h_bill_line.betrag)
            new_item = False
            printed = True
            ...
            if not use_h_queasy:
                queasy = get_cache(Queasy, {
                    "key": [(eq, 4)],
                    "number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],
                    "number2": [(eq, to_int(h_bill_line._recid))],
                    "deci2": [(eq, h_bill.billnr)]})
                ...
                if not queasy:
                    ...
                    if printnr >= 0:
                        ...
                        queasy.key = 4
                        queasy.number1 = (
                            h_bill.departement + h_bill.rechnr * 100)
            ...
            # do_it =  h_artikel and h_artikel.artart == 0 and h_artikel.zwkum != disc_zwkum
            ...
            if h_artikel and h_artikel.artart == 0 and h_artikel.zwkum != disc_zwkum:
                art_list = query(art_list_data, filters=(lambda art_list: art_list.artnr == h_bill_line.artnr and art_list.dept == h_bill_line.departement and art_list.bezeich == h_bill_line.bezeich and art_list.price == h_bill_line.epreis and art_list.condiment == 0 and not art_list.printed), first=True)
            ...
            if not art_list:
                ...
                art_list.hline_recid = recid_h_bill_line
                ...
                art_list.disc_flag = prdisc_flag and (
                    art_list.zwkum == disc_zwkum)
                tot_line = tot_line + 1
            art_list.betrag = to_decimal(art_list.betrag + h_bill_line.betrag)
            ...
            if art_list.artart == 0:
                serv_percent, mwst_percent, fact = cal_servat(
                    h_artikel.departement, h_artikel.artnr, h_artikel.service_code, h_artikel.mwst_code, h_bill_line.bill_datum)
                mwst_perc = to_decimal(mwst_percent * 100)
                serv_perc = to_decimal(serv_percent * 100)
                ...
                if h_bill_line.artnr == f_discart:
                    if h_bill_line.epreis != h_bill_line.betrag:
                        if not serv_disc and h_bill_line.artnr == f_discart:
                            ...
                            h_service = to_decimal(
                                h_bill_line.betrag / fact * serv_percent)

                            if price_decimal == 0:
                                h_service = to_decimal(round(h_service, 0))
                            else:
                        ...
                        if not vat_disc and h_bill_line.artnr == f_discart:
                            ...
                            h_mwst = to_decimal(
                                h_bill_line.betrag / fact * mwst_percent)

                            if price_decimal == 0:
                                h_mwst = to_decimal(round(h_mwst, 0))
                            else:
                                h_mwst = to_decimal(round(h_mwst, 2))

                        if not incl_service and not incl_mwst:
                            amount = to_decimal(h_bill_line.betrag / fact)

                        elif not incl_mwst and incl_service:
                            amount = to_decimal(h_bill_line.betrag / fact)
                        else:
                            amount = to_decimal(h_bill_line.betrag)
                        ...
                        if not incl_service:
                            service = to_decimal(
                                service + (amount * serv_percent))

                        if price_decimal == 0:
                            service = to_decimal(round(service, 0))
                        else:
                            service = to_decimal(round(service, 2))

                        if not incl_mwst:
                            mwst = to_decimal(mwst + (amount * mwst_percent))

                        mwst1 = to_decimal(mwst1 + (amount * mwst_percent))

                        if price_decimal == 0:
                            mwst = to_decimal(round(mwst, 0))
                        else:
                            mwst = to_decimal(round(mwst, 2))

                        if price_decimal == 0:
                            mwst1 = to_decimal(round(mwst1, 0))
                        else:
                            mwst1 = to_decimal(round(mwst1, 2))
                else:
                    if not serv_disc and h_bill_line.artnr == f_discart:
                        pass
                    h_service = to_decimal(
                    ...
                    if not incl_service:
                        service = to_decimal(service + (amount * serv_percent))
                ...
                if not art_list.disc_Flag:
                    if price_decimal == 0:
                        subtotal = to_decimal(round(subtotal + amount, 0))
                    else:
                        subtotal = to_decimal(subtotal + amount)

            if price_decimal == 0:
                art_list.amount = to_decimal(
                    round(art_list.amount + amount, 0))
            else:
                art_list.amount = to_decimal(art_list.amount + amount)
        ...
        if h_mjourn:
            for h_mjourn in db_session.query(H_mjourn).filter(
                ...
                if num_entries(h_mjourn.request, "|") > 1:
                    art_list_submenu.hline_recid = to_int(
                        entry(0, h_mjourn.request, "|"))
            ...
            for art_list in query(art_list_data, filters=(lambda art_list: art_list.condiment != 0)):
                art_list_submenu = query(art_list_submenu_data, filters=(
                    lambda art_list_submenu: art_list_submenu.hline_recid == art_list.hline_recid), first=True)
        ...
        for abuff in query(abuff_data, filters=(lambda abuff: (abuff.zwkum == disc_zwkum) and matches(abuff.bezeich, r"*-*"))):
            ...
            art_list = query(art_list_data, filters=(lambda art_list: art_list.artnr == abuff.artnr and art_list.bezeich.lower(
            ) == (disc_bezeich).lower() and art_list.amount == - abuff.amount), first=True)
        ...
        art_list = query(art_list_data, filters=(
            lambda art_list: art_list.printed == False), first=True)

        if not art_list and printnr >= 0 and not bline_exist:
            msg_str = msg_str + \
                chr_unicode(
                    2) + translateExtended("No new bill-lines was found.", lvcarea, "")
        ...
        if prdisc_flag and tot_ndisc_line >= 1 and tot_disc_line >= 1:
            ...
            for i in range(1, npage + 1):
                ...
                while art_list and (curr_j <= lpage):
                    print_billine()

                    art_list = query(art_list_data, filters=(
                        lambda art_list: art_list.artart == 0 and not art_list.disc_flag and not art_list.printed), next=True)
                ...
                if i == npage:
                    ...
                    for art_list in query(art_list_data, filters=(lambda art_list: art_list.artart == 0 and art_list.disc_flag)):
                        subtotal = to_decimal(subtotal + art_list.amount)
                        print_billine()
            ...
            art_list = query(art_list_data, filters=(
                lambda art_list: art_list.artart == 0 and not art_list.printed), first=True)
            n = 0
            curr_j = printed_line
            for i in range(1, npage + 1):
                if not art_list:
                ...
                while None != art_list and (curr_j <= lpage):
                    ...
                    art_list = query(art_list_data, filters=(
                        lambda art_list: art_list.artart == 0 and not art_list.printed), next=True)
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
                (H_bill_line.rechnr == h_bill.rechnr) & (H_bill_line.departement == h_bill.departement)).order_by(H_bill_line._recid).all():
            tot_amount = to_decimal(tot_amount + h_bill_line.betrag)
