...
def nt_revinatesurvey():
    ...
    prepare_cache ([Guest, Queasy, Nightaudit, Htparam, Paramtext, Res_line, Nation, Zimkateg, Guest_pr, Nitehist])
    ...
    data_list_data, Data_list = create_model("Data_list", {"guest_title":string, "first_name":string, "last_name":string, "email":string, "nation":string, "ci_date":string, "co_date":string, "rmno":string, "city":string, "propid":string, "concode":string, "rmrate":Decimal, "ci_id":string, "co_id":string, "rsv_name":string, "rcode":string, "mobile":string, "country":string, "rsvstatus":string, "rmtype":string})
    push_list_data, Push_list = create_model("Push_list", {"becode":int, "rmtypevhp":string, "rmtypebe":string, "bename":string})
    ...
    def generate_output():
        ...
        nonlocal data_list_data, push_list_data
    ...
    def create_reviewpro_data(out_str:string):
        ...
        push_list = query(push_list_data, filters=(lambda push_list: push_list.rmTypeVHP == data_list.rmType), first=True)
    ...
    for res_line, waehrung, reservation, gmember, guest in db_session.query(Res_line, Waehrung, Reservation, Gmember, Guest).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).join(Guest,(Guest.gastnr == Res_line.gastnr)).filter(
        ...
        nation = get_cache (Nation, {"kurzbez": [(eq, gmember.land)]})
        ...
        data_list.guest_title = replace_str(data_list.guest_title, chr_unicode(44) , chr_unicode(46))
        data_list.first_name = replace_str(data_list.first_name, chr_unicode(44) , chr_unicode(46))
        data_list.last_name = replace_str(data_list.last_name, chr_unicode(44) , chr_unicode(46))
        data_list.email = replace_str(data_list.email, chr_unicode(44) , chr_unicode(46))
        data_list.city = replace_str(data_list.city, chr_unicode(44) , chr_unicode(46))
        data_list.rsv_name = replace_str(data_list.rsv_name, chr_unicode(44) , chr_unicode(46))

    for queasy in db_session.query(Queasy).filter(
             (Queasy.key == 159) & (matches(Queasy.char1,"*reviewpro*"))).order_by(Queasy._recid).all():

        for tqueasy in db_session.query(Tqueasy).filter(
                 (Tqueasy.key == 161) & (Tqueasy.number1 == queasy.number1)).order_by(Tqueasy._recid).all():
            push_list = Push_list()
            push_list_data.append(push_list)
