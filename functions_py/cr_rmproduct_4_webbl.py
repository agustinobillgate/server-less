...
from models import Htparam, Waehrung, Nation, Res_line, Guest, Genstat, Guestseg, Exrate, Bediener, Queasy, Zimkateg, Sourccod, Arrangement
...
def cr_rmproduct_4_webbl(pvilanguage:int, op_type:int, printer_nr:int, call_from:int, txt_file:string, disptype_1:int, cardtype_1:int, currency_type:int, ytd_flag:int, excl_comp:bool, last_sort:int, f_date:date, t_date:date, to_date:date, argt_code:string, dispopt:bool, segm1_list_data:[Segm1_list], zikat_list_data:[Zikat_list]):
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, op_type, printer_nr, call_from, txt_file, disptype_1, cardtype_1, currency_type, ytd_flag, excl_comp, last_sort, f_date, t_date, to_date, argt_code, dispopt, segm1_list_data
    ...
    def create_umsatz1():
        ...
        for genstat, res_line, guest in db_session.query(Genstat, Res_line, Guest).join(Res_line,(Res_line.resnr == Genstat.resnr) & (Res_line.reslinnr == Genstat.res_int[inc_value(0)])).join(Guest,(Guest.gastnr == Genstat.gastnr) & (Guest.karteityp == cardtype)).filter(
            ...
            segm1_list = query(segm1_list_data, (lambda segm1_list: segm1_list.segm == genstat.segmentcode and segm1_list.selected), first=True)
            if not segm1_list:
                continue
            ...
            if do_it:
                ...
                if not to_list:
                    ...
                    if genstat.datum == to_date:
                        ...
                        tmp_room = query(tmp_room_data, filters=(lambda tmp_room: tmp_room.gastnr == genstat.gastnrmember and tmp_room.zinr == genstat.zinr and tmp_room.flag == 1), first=True)
                    ...
                    if get_month(genstat.datum) == get_month(to_date) and ytd_flag == 1 or ytd_flag == 2:
            ...
            if do_it:
                ...
                if not to_list:
                    ...
                    if genstat.datum == to_date:
                        ...
                        tmp_room1 = query(tmp_room1_data, filters=(lambda tmp_room1: tmp_room1.gastnr == genstat.gastnrmember and tmp_room1.zinr == genstat.zinr and tmp_room1.flag == 1), first=True)
    ...
    def create_umsatz21():
        ...
        for to_list1 in query(to_list1_data):
            ...
            for to_list in query(to_list_data, filters=(lambda to_list: to_list.comp_y_pax != 0 and to_list.subgroup == to_list1.gastnr), sort_by=[("subgroup",False),("gastnr",False)]):
                output_list = Output_list()
                output_list_data.append(output_list)
    ...
    if disptype_1 >= 20:
        ...
        disptype = disptype_1       #Rulita | Fixing inden else 
    ...
    if op_type == 0:
        ...
        # Rulita | Comment dont use
        # elif disptype == 8:
        #     create_umsatz8()


    for output_list in query(output_list_data, filters=(lambda output_list: output_list.name.lower()  != ("T o t a l").lower())):
        tot_pax_d = tot_pax_d + output_list.pax
        tot_pax_m = tot_pax_m + output_list.m_pax
        tot_pax_y = tot_pax_y + output_list.y_pax

    output_list = query(output_list_data, filters=(lambda output_list: output_list.name.lower()  == ("T o t a l").lower()), first=True)
