...
def gl_linkarbl(merge_flag:bool, from_date:date, to_date:date, user_init:string, refno:string, curr_anz:int):
    ...
    pay_list_data, Pay_list = create_model("Pay_list", {"counter":int, "ar_recid":int, "name":string, "artnr":int, "zahlkonto":int, "amount":Decimal, "add_info":string})
    ...
    def generate_output():
        nonlocal acct_error, debits, credits, remains, buf_g_list_data, s_list_data, art_artnr, art_bezeich, add_info, fibukonto, credit_betrag, debit_betrag, gl_acct, gl_jouhdr, artikel, debitor, guest
        nonlocal merge_flag, from_date, to_date, user_init, refno, curr_anz
        nonlocal gl_acct1


        nonlocal g_list, buf_g_list, s_list, pay_list, gl_acct1
        nonlocal g_list_data, buf_g_list_data, s_list_data, pay_list_data

        return {"curr_anz": curr_anz, "acct_error": acct_error, "debits": debits, "credits": credits, "remains": remains, "buf-g-list": buf_g_list_data, "s-list": s_list_data, "art_artnr": art_artnr, "art_bezeich": art_bezeich}
    ...
    def step_two():
        ...
        for debitor in db_session.query(Debitor).filter(
            ...
            if gl_acc1:
                ...
                if debitor.saldo < 0:
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acc1.fibukonto and g_list.rechnr == debitor.rechnr and g_list.dept == debitor.betriebsnr), first=True)
                    ...
                    g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto == gl_acc1.fibukonto and g_list.rechnr == debitor.rechnr), first=True)
        ...
        for g_list in query(g_list_data, sort_by=[("sysdate",True),("zeit",True)]):
    ...
    def step_twoa():
        ...
        for debitor in db_session.query(Debitor).filter(
            ...
            if debitor.betriebsnr == 0:
                ...
                pay_list = query(pay_list_data, filters=(lambda pay_list: pay_list.counter == debitor.betriebsnr and pay_list.artnr == debitor.artnr), first=True)
                ...
                pay_list = query(pay_list_data, filters=(lambda pay_list: pay_list.counter == debitor.betriebsnr and pay_list.zahlkonto == debitor.zahlkonto), first=True)
    ...
    def add_list(create_it:bool):
        ...
        s_list = query(s_list_data, filters=(lambda s_list: s_list.fibukonto == gl_acc1.fibukonto), first=True)
    ...
    def add_lista():
        ...
        g_list = query(g_list_data, filters=(lambda g_list: g_list.fibukonto.lower()  == (fibukonto).lower()  and g_list.counter == pay_list.counter), first=True)
