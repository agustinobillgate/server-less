...
def fo_parxls_gs_list_1bl(pvilanguage:int, briefnr:int, from_date:date, to_date:date, user_init:string, link:string):
    ...
    def create_var0(texte:string, int_flag:bool):
        ...
        nonlocal lvcarea, ch, xls_dir, budget_flag, ytd_flag, lytd_flag, lmtd_flag, pmtd_flag, budget_all, serv_vat, zeit, no_decimal, prog_error, error_nr, curr_row, curr_column, curr_cmd, curr_texte, curr_pos, actual_cmd, grp_nr, curr_nr, varname, bezeich, grpflag, lfrom_date, lto_date, jan1, ljan1, pfrom_date, pto_date, curr_month, n, keycmd, keyvar, foreign_flag, foreign_nr, start_date, lytoday, lytoday_flag, price_decimal, msg_ans, outfile_dir, anz0, msg_str, datum, brief, parameters
        nonlocal pvilanguage, briefnr, from_date, to_date, user_init, link, recid_w1_w2
    ...
    msg_str, error_nr = get_output(fo_parxls_gs_1bl(pvilanguage, ytd_flag, jan1, ljan1, lfrom_date, lto_date, pfrom_date, pto_date, from_date, to_date, start_date, lytd_flag, lmtd_flag, pmtd_flag, lytoday_flag, lytoday, foreign_flag, budget_flag, foreign_nr, price_decimal, briefnr, link, budget_all, user_init, w1_data, w2_data))
