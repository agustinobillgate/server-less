...
def prepare_sel_lookmaintain_webbl(pvilanguage:int, prop_nr:int):
    ...
    t_head_data, T_head = create_model("T_head", {"reqno":string, "datum":string, "task":string, "str_stat":string})
    t_line_cost_data, T_line_cost = create_model("T_line_cost", {"reqno":string, "artno":string, "bezeich2":string, "qty":string, "price":string, "tot_dtl":string})
    t_line_vendor_data, T_line_vendor = create_model("T_line_vendor", {"reqno":string, "outsource":string, "vendor_nm":string, "startdate":string, "finishdate":string, "price":string, "tot_dtl":string})
    mainaction_data, Mainaction = create_model("Mainaction", {"maintainnr":int, "action_nr":int, "action_nm":string, "create_date":date, "create_time":int, "time_str":string, "create_by":string})
    tmaintain_data, Tmaintain = create_model("Tmaintain", {"maintainnr":int, "workdate":date, "donedate":date, "estworkdate":date, "type":int, "type_nm":string, "typework":int, "typework_nm":string, "location":int, "location_nm":string, "zinr":string, "propertynr":int, "property_nm":string, "maintask":int, "maintask_nm":string, "category":int, "category_nm":string, "pic":int, "pic_nm":string, "comments":string, "create_by":string, "create_date":date, "memo":string})
    flocation_data, Flocation = create_model("Flocation", {"loc_nr":int, "loc_nm":string})
    tlocation_data, Tlocation = create_model("Tlocation", {"loc_nr":int, "loc_nm":string})
    q1_list_data, Q1_list = create_model("Q1_list", {"datum":date, "fr_room":string, "f_loc_nm":string, "t_loc_nm":string, "to_room":string})
    ...
    def generate_output():
        nonlocal e_price, e_datum, avail_eg_property, rec_meter, rec_hour, rec_date, rec_time, tot_cost, tot_vend, t_head_data, t_line_cost_data, t_line_vendor_data, q1_list_data, tmaintain_data, mainaction_data, atotal, tot, btotal, int_str, lvcarea, typestr, typeworkstr, eg_location, l_artikel, eg_moveproperty, eg_property, eg_request, eg_subtask, eg_queasy, eg_vperform, eg_maintain, eg_mdetail, eg_action, eg_staff, eg_propmeter
    ...
    def create_maintain():
        ...
        for mainbuff in db_session.query(Mainbuff).filter(
            ...
            if eg_staff:
                ...
                pic_nm = ""
            tmaintain = Tmaintain()
            tmaintain_data.append(tmaintain)
            ...
            for mdetailbuff in db_session.query(Mdetailbuff).filter(
                ...
                mainaction_data.append(mainaction)
    ...
    for qbuff in db_session.query(Qbuff).order_by(Qbuff._recid).all():
        ...
        flocation = Flocation()
        flocation_data.append(flocation)
    ...
    for eg_moveproperty in db_session.query(Eg_moveproperty).filter(
        ...
        flocation = query(flocation_data, (lambda flocation: flocation.loc_nr == eg_moveproperty.fr_location), first=True)
        if not flocation:
            continue

        tlocation = query(tlocation_data, (lambda tlocation: tlocation.loc_nr == eg_moveproperty.to_location), first=True)
        if not tlocation:
            continue
