...
def menu_eng_v2_list_webbl(subgr_list_data:[Subgr_list], sorttype:int, from_dept:int, to_dept:int, dstore:int, ldry_dept:int, all_sub:bool, from_date:date, to_date:date, fact1:int, exchg_rate:Decimal, vat_included:bool, mi_subgrp:bool, detailed:bool, curr_sort:int, short_flag:bool):
    ...
    def generate_output():
        nonlocal output_list2_data, t_anz, t_sales, t_cost, t_margin, st_sales, st_cost, st_margin, st_proz2, s_anzahl, s_proz1, gtotal_sold, gtotal_sold_perc, gtotal_cost, gtotal_revenue, gtotal_profit, avrg_item_profit, food_cost, menu_pop_factor, count_foodcost, counter, price_type, htparam, h_artikel, hoteldpt, artikel, h_umsatz, h_journal, wgrpdep
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            for h_artikel.zwkum, h_artikel.artnrrezept, h_artikel.departement, h_artikel.artnr, h_artikel.bezeich, h_artikel.epreis1, h_artikel.prozent, h_artikel._recid, artikel.artnr, artikel.departement, artikel._recid in db_session.query(H_artikel.zwkum, H_artikel.artnrrezept, H_artikel.departement, H_artikel.artnr, H_artikel.bezeich, H_artikel.epreis1, H_artikel.prozent, H_artikel._recid, Artikel.artnr, Artikel.departement, Artikel._recid).join(Artikel,(Artikel.artnr == H_artikel.artnrfront) & (Artikel.departement == H_artikel.departement) & ((Artikel.umsatzart == 3) | (Artikel.umsatzart == 5)) & (Artikel.endkum != disc_nr)).filter(
                ...
                if do_it:
                    ...
                    if vat_included:
                        h_list.epreis =  safe_divide(to_decimal(h_artikel.epreis1) * to_decimal(exchg_rate) , to_decimal(fact))
                    else:
                        h_list.epreis =  safe_divide(to_decimal(h_artikel.epreis1) * to_decimal(exchg_rate) , to_decimal(fact1))
                    ...
                    while None != h_umsatz:
                        ...
                        if vat_included:
                            h_list.epreis = safe_divide( to_decimal(h_list.t_sales) , to_decimal(h_list.anzahl)) * safe_divide(to_decimal(exchg_rate) , to_decimal(fact))
                        else:
                            h_list.epreis = safe_divide( to_decimal(h_list.t_sales) , to_decimal(h_list.anzahl)) * safe_divide(to_decimal(exchg_rate) , to_decimal(fact1))
                    ...
                    if h_list.epreis != 0:
                        h_list.margin =  safe_divide(to_decimal(h_list.cost) , to_decimal(h_list.epreis)) * to_decimal("100")
            create_list(pos)
            t_anz = 0
