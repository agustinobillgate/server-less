...
def mc_gcf0bl(g_list_data:[G_list], curr_mode:string, user_init:string, gastno:int):
    ...
    if curr_mode.lower()  == ("new").lower() :
        ...
        mc_guest = db_session.query(Mc_guest).filter(Mc_guest.gastnr == gastno).with_for_update().first()
        ...
        if (g_list.fdate != mc_guest.fdate) or (g_list.tdate != mc_guest.tdate):
            ...
            mc_fee = db_session.query(Mc_fee).filter(
                         ...
                         (Mc_fee.bis_datum == mc_guest.tdate)).with_for_update().first
        ...
        if g_list.activeflag != mc_guest.activeflag and tbuff.bezeich.lower()  == ("THE ONE").lower() :
            ...
            mc_aclub = db_session.query(Mc_aclub).filter(
                         (Mc_aclub.key == mc_types.nr) & (Mc_aclub.cardnum == mc_guest.cardnum)).with_for_update().first()
        ...
        if g_list.cardnum != mc_guest.cardnum and tbuff.bezeich.lower()  == ("THE ONE").lower() :
            ...
            for mc_aclub in db_session.query(Mc_aclub).filter(
                ...
                mcbuff = db_session.query(Mc_aclub).filter(Mc_aclub._recid == mc_aclub._recid).with_for_update().first()
                mcbuff.cardnum = g_list.cardnum
                pass

        db_session.refresh(mc_guest,with_for_update=True)
