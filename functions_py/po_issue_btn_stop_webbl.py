...
def po_issue_btn_stop_webbl(user_init:string, l_orderhdr_recid:int, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, lief_nr:int, billdate:date, docu_nr:string, lscheinnr:string, curr_lager:int, t_amount:Decimal, jobnr:int, curr_disc:int, crterm:int, op_list_data:[Op_list], t_l_order_data:[T_l_order]):
    ...
    def close_po():
        ...
        for l_od in db_session.query(L_od).filter(
                 (L_od.docu_nr == (docu_nr).lower()) & 
                 (L_od.pos > 0) & 
                 (L_od.loeschflag == 0)).with_for_update().order_by(L_od._recid).all():
    ...
    for op_list in query(op_list_data):
        ...
        if t_l_order:
            ...
            l_order = db_session.query(L_order).filter(
                 (L_order.docu_nr == (l_orderhdr.docu_nr).lower()) & (L_order.pos == op_list.pos)).first()

            l_artikel = db_session.query(L_artikel).filter(
                 (L_artikel.artnr == t_l_order.artnr)).with_for_update().first()
