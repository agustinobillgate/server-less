...
def pr_sphbill1_cldbl(pvilanguage:int, hbrecid:int, printnr:int, use_h_queasy:bool, session_parameter:str, user_init:str, billnr:int, print_all:bool):
    winprinterflag = False
    filename = ""
    msg_str = ""
    output_list_list = []
    t_printer_list = []
    lvcarea:str = "print-sphbill1"
    disc_bezeich:str = ""
    ...
    art_list = t_printer = vat_list = output_list = bline_vatlist = artbuff = abuff = art_list_submenu = None # Malik : C68F4A

    art_list_list, Art_list = create_model(
        "Art_list", {
            "printed":bool, 
            "disc_flag":bool, 
            "dept":int, 
    ...
    vat_list_list, Vat_list = create_model(
        "Vat_list", {
            "vat_amt":decimal, 
            "betrag_amt":decimal
            }
        )
    output_list_list, Output_list = create_model(
        "Output_list", {
            "str":str, 
            "str_pos":int, 
            "pos":int, 
    ...
    bline_vatlist_list, Bline_vatlist = create_model(
        "Bline_vatlist", {
            "seqnr":int, 
            "vatnr":int, 
            "bezeich":str, 
    ...
    def generate_output():
        ...
        nonlocal art_list, t_printer, vat_list, output_list, bline_vatlist, art_list_submenu, artbuff, abuff, bart_list_submenu
        nonlocal art_list_list, t_printer_list, vat_list_list, output_list_list, bline_vatlist_list, art_list_submenu_data
        return {
            "print_all": print_all, 
            "winprinterflag": winprinterflag, 
            "filename": filename, 
            "msg_str": msg_str, 
            "output-list": output_list_list, 
            "t-printer": t_printer_list
        }
    ...
    def add_unitprice_text():
        ...
        nonlocal pvilanguage, hbrecid, printnr, use_h_queasy, session_parameter, user_init, billnr, print_all
        nonlocal artbuff, abuff, bart_list_submenu
        nonlocal art_list, t_printer, vat_list, output_list, bline_vatlist, artbuff, abuff, art_list_submenu, bart_list_submenu
        nonlocal art_list_list, t_printer_list, vat_list_list, output_list_list, bline_vatlist_list, art_list_submenu_data
    ...
    def check_pages():
        ...
        if h_bill.saldo == 0 or print_all:
            paramtext = db_session.query(Paramtext).filter(
                    (Paramtext.txtnr == 713)).first() # FDL : DECE04
        ...
        if printed_line == 0 or print_all:
            ...
            if h_bill.bilname != "":
                if gst_logic:
                    ...
                    output_list.str = output_list.str + "     " + to_string(bill_date) + "|" + to_string(time, "HH:MM") + "|" + translateExtended ("Tax Invoice No", lvcarea, "") + "|" + rechnr_str

                else:
                    output_list.str_pos = 1

                output_list.str = output_list.str + "     " + to_string(bill_date) + "|" + to_string(time, "HH:MM") + "|" + translateExtended ("Bill No", lvcarea, "") + "|" + rechnr_str
                output_list = Output_list()
                output_list_list.append(output_list)
                ...
                output_list.str = output_list.str + "     " + translateExtended ("Table", lvcarea, "") + "|" + trim(to_string(h_bill.tischnr, ">>>>>9")) + "|" + trim(to_string(h_bill.belegung, ">>9")) + "|" + "(" + trim(to_string(billnr, ">>>9")) + ")" + "|" + kname
                output_list = Output_list()
                output_list_list.append(output_list)
                ...
                output_list.str = output_list.str + to_string("     ") + to_string(translateExtended ("Guest", lvcarea, "")) + "|" + trim(to_string(h_bill.bilname, "x(32)"))

                queasy = db_session.query(Queasy).filter(
                        (Queasy.key == 286) & (Queasy.number1 == h_bill.rechnr) & (Queasy.number2 == h_bill.departement) & (Queasy.number3 == billnr)).first()

                if queasy:
                    # output_list.str_pos = 9
                    # output_list.str = trim(to_string(queasy.char1, "x(32)"))
                    if output_list.str != "":
                ...
                if gst_logic:
                    ...
                    output_list.sort_i = sort_i
                    sort_i = sort_i + 1
                    output_list.str = output_list.str + to_string("       " + translateExtended ("GST ID : 001865060352", lvcarea, ""))
                ...
                if gst_logic:
                    ...
                    output_list.str_pos = 1
                    output_list.str = output_list.str + "     " + to_string(bill_date) + "|" + to_string(get_current_time_in_seconds(), "HH:MM") + "|" + " " + translateExtended ("Bill No", lvcarea, "") + "|" + rechnr_str
    ...
    def print_billine():
        ...
        nonlocal pvilanguage, hbrecid, printnr, use_h_queasy, session_parameter, user_init, billnr, print_all
        nonlocal artbuff, abuff, bart_list_submenu
        nonlocal art_list, t_printer, vat_list, output_list, bline_vatlist, art_list_submenu_data, artbuff, abuff, bart_list_submenu
        nonlocal art_list_list, t_printer_list, vat_list_list, output_list_list, bline_vatlist_list, art_list_submenu_data
        ...
        output_list.str = output_list.str + "1" # Malik : C68F4A
    ...
    def print_overhead3():
        ...
        if h_bill.bilname != "":
            ...
            if qty1000:
                ...
                output_list.str = output_list.str + to_string("", "x(5)")
            output_list.str = output_list.str + to_string(translateExtended ("Guest :", lvcarea, "")) + to_string(" ") + trim(to_string(h_bill.bilname, "x(32)"))
            ...
            if queasy:
                # output_list.str_pos = 9
                # output_list.str = trim(to_string(queasy.char1))
                if output_list.str != "":
                    output_list.str = output_list.str + " | " + trim(queasy.char1) # FDL : 6DCD86
    ...
    def print_overhead4():
        ...
        if foot3 != "":
            ...
            output_list.sort_i = sort_i
            sort_i = sort_i + 1
            printed_line = printed_line + 1  # FDL : 6DCD86
    ...
    if sc_art != 0:
        ...
        if h_bill_line:
            print_all = True
            print_all, filename, msg_str, winprinterflag, output_list_list, t_printer_list = get_output(print_hbill1_phbl(pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all))
    ...
    if printnr != 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not art_list:  # Malik : C68F4A
                ...
                art_list.hline_recid = h_bill_line._recid # Malik : C68F4A
                ...
                if h_artikel and h_artikel.artart == 0:
                    ...
                    art_list.condiment = h_artikel.betriebsnr # Malik : C68F4A
        ...
        if h_mjourn:
            for h_mjourn in db_session.query(H_mjourn).filter(
                (H_mjourn.rechnr == h_bill.rechnr) and (H_mjourn.departement == h_bill.departement)).order_by(H_mjourn.h_artnr, H_mjourn.artnr).all():
            ...
            for art_list in query(art_list_list, filters=(lambda art_list: art_list.condiment != 0)):
        ...
        for abuff in query(abuff_list, filters=(lambda abuff:(abuff.zwkum == disc_zwkum) and re.match(r".*-.*",abuff.bezeich, re.IGNORECASE))):
