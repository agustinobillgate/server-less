...
from models import L_bestand, L_besthis, L_lager, Htparam, L_artikel, L_op, L_untergrup

def stock_ohlist1_2bl(from_lager:int, to_lager:int, from_art:int, to_art:int, from_date:date, to_date:date, move_art:bool, end_notzero:bool, global_oh:bool):

    prepare_cache ([L_bestand, L_lager, Htparam, L_artikel, L_op, L_untergrup])
    ...
    l_bestand = l_besthis = l_lager = htparam = l_artikel = l_op = l_untergrup = None

    stockoh_list = art_bestand = l_oh = l_ohis = b_lager = None

    stockoh_list_data, Stockoh_list = create_model("Stockoh_list", {"artnr":string, "bezeich":string, "prevqty":string, "prevval":string, "incoming":string, "incomingval":string, "outgoing":string, "outgoingval":string, "actqty":string, "actval":string, "avrg_pr":string})
    art_bestand_data, Art_bestand = create_model("Art_bestand", {"lager":int, "artnr":int, "zwkum":int, "bezeich":string, "incoming":Decimal, "in_val":Decimal, "out_val":Decimal, "outgoing":Decimal, "prevqty":Decimal, "prevval":Decimal, "adjust":Decimal, "actqty":Decimal, "actval":Decimal, "avrg_pr":Decimal})

    L_oh = create_buffer("L_oh",L_bestand)
    L_ohis = create_buffer("L_ohis",L_besthis)
    B_lager = create_buffer("B_lager",L_lager)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal stockoh_list_data, do_it, curr_art, curr_lager, out_bestand, zwkum, t_val, bezeich, j, tot_prev, tot_value, tot_in, tot_out, total_in, total_out, total_prev, total_value, tdate, qty, wert, long_digit, l_bestand, l_besthis, l_lager, htparam, l_artikel, l_op, l_untergrup
        nonlocal from_lager, to_lager, from_art, to_art, from_date, to_date, move_art, end_notzero, global_oh
        nonlocal l_oh, l_ohis, b_lager


        nonlocal stockoh_list, art_bestand, l_oh, l_ohis, b_lager
        nonlocal stockoh_list_data, art_bestand_data
    ...
    def create_stock1():
        ...
        if get_year(to_date) < get_year(tdate) and get_year(from_date) < get_year(tdate):
            ...
            if get_month(to_date) > get_month(tdate) and get_month(from_date) > get_month(tdate):
                ...
                for l_lager in db_session.query(L_lager).filter(
                    ...
                    for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter((L_bestand.lager_nr == l_lager.lager_nr) & (L_bestand.artnr >= from_art) & (L_bestand.artnr <= to_art)).order_by(L_artikel.zwkum, L_artikel.artnr).all():
                        ...
                        if do_it:
                            ...
                            art_bestand = query(art_bestand_data, filters=(lambda art_bestand: art_bestand.artnr == l_bestand.artnr and art_bestand.lager == l_lager.lager_nr), first=True)
                            ...
                            for l_op in db_session.query(L_op).filter((L_op.lager_nr == l_lager.lager_nr) & (L_op.artnr == l_bestand.artnr) & ((L_op.datum >= from_date) & (L_op.datum <= to_date)) & (L_op.loeschflag <= 1)).order_by(L_op.datum).all():
            ...
            if get_month(to_date) <= get_month(tdate) and get_month(from_date) <= get_month(tdate):
                ...
                for b_lager in db_session.query(B_lager).filter(
                         (B_lager.lager_nr >= from_lager) & (B_lager.lager_nr <= to_lager)).order_by(B_lager._recid).all():
                    zwkum = 0
                    t_val =  to_decimal("0")
                    ...
                    for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter((L_bestand.lager_nr == b_lager.lager_nr) & (L_bestand.artnr >= from_art) & (L_bestand.artnr <= to_art)).order_by(L_artikel.zwkum, L_artikel.artnr).all():
                        ...
                        if do_it:
                            ...
                            art_bestand = query(art_bestand_data, filters=(lambda art_bestand: art_bestand.artnr == l_bestand.artnr and art_bestand.lager == b_lager.lager_nr), first=True)
                            ...
                            if not art_bestand:
                                ...
                                art_bestand.zwkum = l_artikel.zwkum
                                art_bestand.artnr = l_bestand.artnr
                                art_bestand.lager = b_lager.lager_nr
                                art_bestand.prevqty =  to_decimal(l_bestand.anz_anf_best)
                                art_bestand.prevval =  to_decimal(l_bestand.val_anf_best)
                                art_bestand.actqty =  to_decimal(l_bestand.anz_anf_best)
                                art_bestand.actval =  to_decimal(l_bestand.val_anf_best)
                                curr_art = l_bestand.artnr
                                curr_lager = b_lager.lager_nr

                            out_bestand =  to_decimal("0")

                            for l_op in db_session.query(L_op).filter((L_op.lager_nr == b_lager.lager_nr) & (L_op.artnr == l_bestand.artnr) & ((L_op.datum >= from_date) & (L_op.datum <= to_date)) & (L_op.loeschflag <= 1)).order_by(L_op.datum).all():
        ...
        for l_lager in db_session.query(L_lager).filter((L_lager.lager_nr >= from_lager) & (L_lager.lager_nr <= to_lager)).order_by(L_lager._recid).all():
            
            bezeich = ""
            stockoh_list = Stockoh_list()
            stockoh_list_data.append(stockoh_list)

            stockoh_list.bezeich = to_string(l_lager.lager_nr, "99") + " - " + format_fixed_length(l_lager.bezeich, 25)
            ...
            for art_bestand in query(art_bestand_data, filters=(lambda art_bestand: art_bestand.lager == l_lager.lager_nr and art_bestand.artnr >= from_art and art_bestand.artnr <= to_art), sort_by=[("zwkum",False),("bezeich",False)]):
                ...
                if (move_art and end_notzero) and (art_bestand.incoming != 0 or art_bestand.outgoing != 0 or art_bestand.actqty != 0):
                    ...
                    if zwkum != art_bestand.zwkum:
                        ...
                        if not long_digit:
                            stockoh_list.bezeich = "Ttl " + format_fixed_length(substring(bezeich, 0, 9) , 9) +\
                                    format_fixed_length(substring(bezeich, 9, 13) , 13)
                    ...
                    if not long_digit:
                        ...
                        stockoh_list.bezeich = format_fixed_length(art_bestand.bezeich, 30)
            ...
            if bezeich != "":
                ...
                if not long_digit:
                    stockoh_list.bezeich = "Ttl " + format_fixed_length(l_lager.bezeich, 25)
    ...
    def create_stock2():
        ...
        if get_year(to_date) <= get_year(tdate) and get_year(from_date) <= get_year(tdate):
            ...
            if get_month(to_date) > get_month(tdate) and get_month(from_date) > get_month(tdate):
                ...
                l_bestand = L_bestand()
                l_artikel = L_artikel()
                for l_bestand.artnr, l_bestand.anz_anf_best, l_bestand.val_anf_best, l_bestand.lager_nr, l_bestand._recid, l_artikel.bezeich, l_artikel.zwkum, l_artikel.vk_preis, l_artikel.artnr, l_artikel._recid in db_session.query(L_bestand.artnr, L_bestand.anz_anf_best, L_bestand.val_anf_best, L_bestand.lager_nr, L_bestand._recid, L_artikel.bezeich, L_artikel.zwkum, L_artikel.vk_preis, L_artikel.artnr, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_bestand.artnr)).filter((L_bestand.lager_nr == 0) & (L_bestand.artnr >= from_art) & (L_bestand.artnr <= to_art)).order_by(L_artikel.zwkum, L_artikel.artnr).all():
                    ...
                    if do_it:
                        ...
                        art_bestand.zwkum = l_artikel.zwkum
                        art_bestand.artnr = l_bestand.artnr
                        art_bestand.lager = l_bestand.lager_nr
                        art_bestand.prevqty =  to_decimal(l_bestand.anz_anf_best)
                        art_bestand.prevval =  to_decimal(l_bestand.val_anf_best)
                        art_bestand.actqty =  to_decimal(l_bestand.anz_anf_best)
                        art_bestand.actval =  to_decimal(l_bestand.val_anf_best)
                ...
                l_artikel_obj_list = {}
                for l_artikel in db_session.query(L_artikel).filter(((L_artikel.artnr.in_(list(set([art_bestand.artnr for art_bestand in art_bestand_data])))))).order_by(art_bestand.zwkum, art_bestand.artnr).all():
                    ...
                    art_bestand = query(art_bestand_data, (lambda art_bestand: (l_artikel.artnr == art_bestand.artnr)), first=True)

                    for l_op in db_session.query(L_op).filter(
                             (L_op.artnr == art_bestand.artnr) & ((L_op.datum >= from_date) & (L_op.datum <= to_date)) & (L_op.loeschflag <= 1)).order_by(L_op.datum).all():
        ...
        for art_bestand in query(art_bestand_data, filters=(lambda art_bestand: art_bestand.artnr >= from_art and art_bestand.artnr <= to_art), sort_by=[("zwkum",False),("bezeich",False)]):
            ...
            if (not move_art and end_notzero) and (art_bestand.actqty != 0):
            ...
            if (not move_art and not end_notzero):
