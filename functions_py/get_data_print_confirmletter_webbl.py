...
def get_data_print_confirmletter_webbl(resnumber:int, user_init:string):
    ...
    def generate_output():
        nonlocal print_list_data, room, rmtype, pricerm, tot_rm, allgst, all_total, night, co, guest, zimkateg, res_line, bediener, paramtext, reservation, waehrung, fixleist, queasy, akt_kont, reslin_queasy
        nonlocal resnumber, user_init
        nonlocal t_guest, t_zim, t_res


        nonlocal print_list, setup_list, t_guest, t_zim, t_res
        nonlocal print_list_data, setup_list_data
