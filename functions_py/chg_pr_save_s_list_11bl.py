...
def chg_pr_save_s_list_11bl(s_list_data:[S_list], tt_app_id_data:[Tt_app_id], t_guestbook_data:[T_guestbook], 
                            rec_id:int, deptnr:int, comments_screen_value:string, rej_id:string, lieferdatum:date, rej_flag:bool, user_init:string, docu_nr:string):
    ...
    for t_guestbook in query(t_guestbook_data, filters=(lambda t_guestbook: t_guestbook.reserve_char[0] == ("$pr").lower()  and t_guestbook.reserve_char[1] == (docu_nr).lower())):
    ...
    for sbuff in query(sbuff_data):
        ...
        l_order = db_session.query(L_order).filter(
                     (L_order._recid == sbuff.s_recid)).with_for_update().first()
