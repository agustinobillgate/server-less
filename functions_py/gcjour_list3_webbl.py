...
"""_yusufwijasena_
    ...
    ISSUE:  - fix definition variabel
            - update var jtype: List[string] = create_empty_list()
            - fix python indentation
            - add type ignore to avoid warning 
            - update ' if from_refno == "": '
"""
...
def gcjour_list3_webbl(t_payload_list_data:T_payload_list, case_type:int, from_refno:string, sorttype:int, journaltype:int, jtype1:int, from_date:date, to_date:date, jnr:int):
    ...
    def get_bemerk(bemerk:string):
        ...
        n = int(str(get_index(bemerk, ";&&"))  )

        if n > 0:
            s1 = str(substring(bemerk, 0, n - 1))  
        else:
            s1 = bemerk
        return s1
    ...
    def disp_it2():
        ...
        if t_payload_list.v_mode == 0:  # type: ignore t_payload_list belum terisi

            gl_journal = get_cache (Gl_journal, {
                "jnr": [(eq, gl_jouhdr_list.jnr)]})  # type: ignore gl_jouhdr_list masih bernilai none
            while gl_journal:
                count_data = count_data + 1

                curr_recid = gl_journal._recid
                gl_journal = db_session.query(Gl_journal).filter(
                (Gl_journal.jnr == gl_jouhdr_list.jnr) & (Gl_journal._recid > curr_recid)).first()  # type: ignore gl_jouhdr_list bernilai none
            t_output_list = T_output_list()
            t_output_list_data.append(t_output_list)

            t_output_list.total_data = count_data  # type: ignore model t_output_list belum tercreate

            if count_data >= 3000:

                gl_journal_obj_list = {}
                for gl_journal, gl_acct in db_session.query(Gl_journal, Gl_acct).join(Gl_acct,(Gl_acct.fibukonto == Gl_journal.fibukonto)).filter(
                (Gl_journal.jnr == gl_jouhdr_list.jnr)).order_by(Gl_acct.fibukonto, Gl_journal.sysdate, Gl_journal.zeit).all(): # type: ignore gl_jouhdr_list bernilai none
                    ...
                    if int(str(num_entries(gl_acct.userinit, ";"))) > 1:  
                ...
                gl_journal_obj_list = {}
                for gl_journal, gl_acct in db_session.query(Gl_journal, Gl_acct).join(Gl_acct,(Gl_acct.fibukonto == Gl_journal.fibukonto)).filter(
                (Gl_journal.jnr == gl_jouhdr_list.jnr)).order_by(Gl_acct.fibukonto, Gl_journal.sysdate, Gl_journal.zeit).all():  # type: ignore gl_jouhdr_list belum tercreate
                    if gl_journal_obj_list.get(gl_journal._recid):
                        continue
                    else:
                        gl_journal_obj_list[gl_journal._recid] = True
            ...
            gl_journal_obj_list = {}
            for gl_journal, gl_acct in db_session.query(Gl_journal, Gl_acct).join(Gl_acct,(Gl_acct.fibukonto == Gl_journal.fibukonto)).filter(
            (Gl_journal.jnr == gl_jouhdr_list.jnr) & (Gl_journal.fibukonto >= t_payload_list.fibukonto) & (Gl_journal.sysdate >= t_payload_list.sysdate) & (Gl_journal.zeit >= t_payload_list.zeit) & (Gl_journal._recid != to_int(b2_list.rec_id))).order_by(Gl_acct.fibukonto, Gl_journal.sysdate, Gl_journal.zeit).all():  # type: ignore model t_payload_list & b2_list belum tercreate
                if gl_journal_obj_list.get(gl_journal._recid):
                    continue
                else:
                    gl_journal_obj_list[gl_journal._recid] = True
    ...
    def disp_it_2_1():
        ...
        (Gl_journal.jnr == sorttype)).order_by(Gl_acct.fibukonto, Gl_journal.sysdate, Gl_journal.zeit).all():
            ...
            b2_list.fibukonto = gl_acct.fibukonto # type: ignore model b2_list belum tercreate
            b2_list.debit =  to_decimal(gl_journal.debit) # type: ignore model b2_list belum tercreate
            b2_list.credit =  to_decimal(gl_journal.credit) # type: ignore model b2_list belum tercreate
            b2_list.bemerk = get_bemerk (gl_journal.bemerk) # type: ignore model b2_list belum tercreate
            b2_list.userinit = gl_journal.userinit # type: ignore model b2_list belum tercreate
            b2_list.sysdate = gl_journal.sysdate # type: ignore model b2_list belum tercreate
            b2_list.zeit = gl_journal.zeit # type: ignore model b2_list belum tercreate
            b2_list.chginit = gl_journal.chginit # type: ignore model b2_list belum tercreate
            b2_list.chgdate = gl_journal.chgdate # type: ignore model b2_list belum tercreate
            b2_list.bezeich = gl_acct.bezeich # type: ignore model b2_list belum tercreate

            if num_entries(gl_acct.userinit, ";") > 1: # type: ignore return model sqlalchemy
                b2_list.map_acct = entry(1, gl_acct.userinit, ";") # type: ignore model b2_list belum tercreate
            else:
                b2_list.map_acct = " " # type: ignore model b2_list belum tercreate
    ...
    def assign_it():
        ...
        gl_jouhdr_list.datum = gl_jouhdr.datum  # type: ignore object bernilai none
        gl_jouhdr_list.refno = gl_jouhdr.refno  # type: ignore object bernilai none
        gl_jouhdr_list.bezeich = gl_jouhdr.bezeich  # type: ignore object bernilai none
        gl_jouhdr_list.debit =  to_decimal(gl_jouhdr.debit)  # type: ignore object bernilai none
        gl_jouhdr_list.credit =  to_decimal(gl_jouhdr.credit)  # type: ignore object bernilai none
        gl_jouhdr_list.remain =  to_decimal(gl_jouhdr.remain)  # type: ignore object bernilai none
        gl_jouhdr_list.jnr = gl_jouhdr.jnr  # type: ignore object bernilai none
        gl_jouhdr_list.activeflag = gl_jouhdr.activeflag  # type: ignore object bernilai none
        if gl_jouhdr.jtype == 0:  # type: ignore object bernilai none
            gl_jouhdr_list.jtype = jtype[5]  # type: ignore model gl_jouhdr_list belum tecreate
            
        else:
            gl_jouhdr_list.jtype = jtype[gl_jouhdr.jtype - 1]  # type: ignore gl_jouhdr_list belum tercreate


    t_payload_list = query(t_payload_list_data, first=True)  # type: ignore t_payload_list_data masih bernilai none
