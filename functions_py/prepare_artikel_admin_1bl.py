...
def prepare_artikel_admin_1bl(pvilanguage:int, from_bez:string, dept:int):
    ...
    t_artikel_data, T_artikel = create_model_like(Artikel, {"zk_bezeich":string, "ek_bezeich":string, "argt_bez":string, "rec_id":int, "minibar":bool})
    ...
    def assign_it():
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 266)],"number1": [(eq, artikel.departement)],"number2": [(eq, artikel.artnr)]})
