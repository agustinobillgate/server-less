"""_yusufwijasena_26/12/2025
        ...
        _issue_:    - changed filter query by artikel.artnr
"""
...
from models import Artikel, Htparam, Waehrung, Hoteldpt, Zwkum, Ekum, Arrangement, Queasy


def prepare_artikel_admin_1bl(pvilanguage: int, from_bez: string, dept: int):

    prepare_cache([Htparam, Waehrung, Hoteldpt, Zwkum, Ekum, Arrangement, Queasy])
    ...
    p121: int = 0
    artikel = htparam = waehrung = hoteldpt = zwkum = ekum = arrangement = queasy = None

    t_artikel = None

    t_artikel_data, T_artikel = create_model_like(
        Artikel,
        {
            "zk_bezeich": string,
            "ek_bezeich": string,
    ...
    def generate_output():
        nonlocal msg_str, msg_str2, local_nr, foreign_nr, d_bezeich, t_artikel_data, lvcarea, p121, artikel, htparam, waehrung, hoteldpt, zwkum, ekum, arrangement, queasy
    ...
    def assign_it():
        ...
        zwkum = get_cache(
            Zwkum, {"zknr": [(eq, artikel.zwkum)], "departement": [(eq, dept)]})
        ...
        queasy = get_cache(Queasy, {"key": [(eq, 266)], "number1": [
                           (eq, artikel.departement)], "number2": [(eq, artikel.artnr)]})

        if queasy:
            t_artikel.minibar = queasy.logi1
    ...
    # yusufwijasena, changed fillter query by artikel.artnr
    for artikel in db_session.query(Artikel).filter(
            (Artikel.departement == dept)).order_by(Artikel.artnr).all():
        # print(f'[LOG] artikel.artnr: {artikel.artnr}, artikel.bezeich: {artikel.bezeich}, dept: {dept}')
        assign_it()
