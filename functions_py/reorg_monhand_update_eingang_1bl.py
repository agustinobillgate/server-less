...
def reorg_monhand_update_eingang_1bl(art_type:int, user_init:string):
    ...
    def checkin_outgoing():
        ...
        l_op1 = db_session.query(L_op).filter(
             (L_op.artnr == l_op.artnr) & (L_op.op_art == 3) & (L_op.lief_nr == l_op.lief_nr) & (L_op.lscheinnr == l_op.lscheinnr) & (L_op.herkunftflag == 2) & (L_op.lager_nr == l_op.lager_nr)).with_for_update().first()
    ...
    def update_eingang():
        ...
        if l_op.op_art == 1 or (l_op.op_art == 2):
            ...
            if not l_bestand:
                ...
                bbest = db_session.query(Bbest).filter(Bbest._recid == l_bestand._recid).with_for_update().first()
