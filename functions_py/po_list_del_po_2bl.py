...
def po_list_del_po_2bl(q2_list_data:[Q2_list], user_init:string, billdate:date, l_orderhdr_docu_nr:string, bediener_username:string, curr_mode:string):
    ...
    def generate_output():
        ...
        nonlocal q2_list_data, user_init, billdate, l_orderhdr_docu_nr, bediener_username, curr_mode


        nonlocal cost_list, w_list, q2_list
        nonlocal cost_list_data, w_list_data
    ...
    def del_reason():
        ...
        q2_list = query(q2_list_data, filters=(lambda q2_list: q2_list.docu_nr.lower()  == (l_orderhdr_docu_nr).lower()), first=True)

        l_orderhdr = db_session.query(L_orderhdr).filter(
                 (L_orderhdr.docu_nr == (q2_list.docu_nr).lower())).first()
    ...
    def del_po():
        ...
        l_od = db_session.query(L_od).filter(
                 (L_od.docu_nr == (l_orderhdr_docu_nr).lower()) & (L_od.pos == 0)).first()
        ...
        for l_od in db_session.query(L_od).filter(
                 (L_od.docu_nr == (l_orderhdr_docu_nr).lower()) & (L_od.pos > 0) & (L_od.loeschflag == 0)).with_for_update().order_by(L_od._recid).all():
