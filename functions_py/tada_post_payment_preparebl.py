...
def tada_post_payment_preparebl(pvilanguage:int, curr_dept:int, curr_printer:int, user_init_str:string, transdate:date):
    ...
    def generate_output():
        nonlocal mealcoupon_cntrl, must_print, zero_flag, multi_cash, cancel_exist, msg_str, disc_art1, disc_art2, disc_art3, mi_ordertaker, price_decimal, curr_local, curr_foreign, double_currency, foreign_rate, exchg_rate, b_title, deptname, p_223, curr_waiter, fl_code, pos1, pos2, cashless_flag, c_param870, activate_deposit, hbill_data, t_kellner_data, lvcarea, user_init, from_acct, guest, printer, htparam, queasy, waehrung, hoteldpt, kellner, h_bill, bediener
    ...
    kellner = db_session.query(Kellner).filter(
        (Kellner.kellner_nr == curr_waiter) & (Kellner.departement == curr_dept)).with_for_update().first()
    ...
    for h_bill in db_session.query(H_bill).filter(
             (H_bill.departement == curr_dept) & (H_bill.flag == 0)).order_by(H_bill._recid).with_for_update().all():
        hbill = Hbill()
        hbill_data.append(hbill)
