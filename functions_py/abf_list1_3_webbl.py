...
# yusufwijasena, 07/11/2025 (F6D79E)
# - update from FDL: DFCF68
# - fix range(1, num_entries(genstat.res_char[1], ";") - 1 + 1)
# - fix ("string").lower()
# - fix func check_fixleist_posted to check_fixleist_posted1
# - fix var adult_qty, child_qty, and infant_qty
# ------------------------------------------
...
def abf_list1_3_webbl(fdate: date, bfast_artnr: int, bfast_dept: int, show_bfast_rate: bool, zikat_list_data: list[Zikat_list]):
    ...
    dont_post: bool = False  # FDL: DFCF68
    ...
    abf_list_data, Abf_list = create_model(
        "Abf_list",
        {
            ...
            "mcard_number": string,
            "mcard_type": string,
            "bfast_revenue": Decimal,
            "c_bfast_revenue": Decimal,  # FDL: DFCF68
            "i_bfast_revenue": Decimal  # FDL: DFCF68
        })
# start FDL: DFCF68
    t_argt_ratelist_data, T_argt_ratelist = create_model(
        "T_argt_ratelist",
        {
            "bfast_artnr": int,
            "bfast_dept": int,
    ...
    def disp_arlist1():
        ...
        for genstat, res_line in db_session.query(Genstat, Res_line).join(Res_line, (Res_line.resnr == Genstat.resnr) & (Res_line.reslinnr == Genstat.res_int[inc_value(0)])).filter(
                ((Genstat.resstatus != 3) & 
                 ...
                 (Genstat.resstatus != 8) & 
                 (Genstat.resstatus != 9) & 
                 (Genstat.resstatus != 10) & 
                 (Genstat.resstatus != 12)) & 
                (Genstat.datum == fdate)).order_by(Genstat.zinr).all():
...
# start FDL: DFCF68
            ...
            contcode = ""
            for i in range(1, num_entries(genstat.res_char[1], ";")):
                ...
                if t_argt_ratelist.do_it:
                    if t_argt_ratelist.based_on_adult:
                        reslin_queasy = get_cache(
                            Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "number1": [(eq, bfast_dept)], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, bfast_artnr)], "resnr": [(eq, genstat.resnr)], "reslinnr": [(eq, genstat.res_int[0])], "date1": [(le, fdate)], "date2": [(ge, fdate)], "deci1": [(gt, 0)]})

                        if reslin_queasy:
                            t_argt_ratelist.rate_adult = to_decimal("0")

                            for reslin_queasy in db_session.query(
                                Reslin_queasy).filter((Reslin_queasy.key == "fargt-line") & (Reslin_queasy.char1 == "") & (Reslin_queasy.number1 == bfast_dept) & (Reslin_queasy.number2 == arrangement.argtnr) & (Reslin_queasy.number3 == bfast_artnr) & (Reslin_queasy.resnr == genstat.resnr) & (Reslin_queasy.reslinnr == genstat.res_int[0]) & (Reslin_queasy.date1 <= fdate) & (Reslin_queasy.date2 >= fdate) & (Reslin_queasy.deci1 > 0)).order_by(Reslin_queasy._recid).all():
                                t_argt_ratelist.rate_adult = to_decimal(
                                    t_argt_ratelist.rate_adult) + to_decimal(reslin_queasy.deci1)
                        else:
                            reslin_queasy = get_cache(
                                Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, genstat.res_int[1])], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, bfast_artnr)], "resnr": [(eq, bfast_dept)], "reslinnr": [(eq, genstat.zikatnr)], "date1": [(le, fdate)], "date2": [(ge, fdate)], "deci1": [(gt, 0)]})
                        ...
                        reslin_queasy = get_cache(
                            Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "number1": [(eq, bfast_dept)], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, bfast_artnr)], "resnr": [(eq, genstat.resnr)], "reslinnr": [(eq, genstat.res_int[0])], "date1": [(le, fdate)], "date2": [(ge, fdate)], "deci2": [(gt, 0)]})
                        ...
                        if reslin_queasy:
                            ...
                            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                    ...
                                    (Reslin_queasy.char1 == "") & 
                                    (Reslin_queasy.number1 == bfast_dept) & 
                                    (Reslin_queasy.number2 == arrangement.argtnr) & 
                                    (Reslin_queasy.number3 == bfast_artnr) & 
                                    (Reslin_queasy.resnr == genstat.resnr) & 
                                    (Reslin_queasy.reslinnr == genstat.res_int[0]) & 
                                    (Reslin_queasy.date1 <= fdate) & 
                                    (Reslin_queasy.date2 >= fdate) & 
                                    (Reslin_queasy.deci2 > 0)).order_by(Reslin_queasy._recid).all():
                                t_argt_ratelist.rate_child = to_decimal(
                                    t_argt_ratelist.rate_child) + to_decimal(reslin_queasy.deci2)
                        else:
                            reslin_queasy = get_cache(
                                Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, genstat.res_int[1])], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, bfast_artnr)], "resnr": [(eq, bfast_dept)], "reslinnr": [(eq, genstat.zikatnr)], "date1": [(le, fdate)], "date2": [(ge, fdate)], "deci2": [(gt, 0)]})
                            ...
                            if reslin_queasy:
                                ...
                                for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                        ...
                                        (Reslin_queasy.char1 == (contcode).lower()) & 
                                        (Reslin_queasy.number1 == genstat.res_int[1]) & 
                                        (Reslin_queasy.number2 == arrangement.argtnr) & 
                                        (Reslin_queasy.number3 == bfast_artnr) & 
                                        (Reslin_queasy.resnr == bfast_dept) & 
                                        (Reslin_queasy.reslinnr == genstat.zikatnr) & 
                                        (Reslin_queasy.date1 <= fdate) & 
                                        (Reslin_queasy.date2 >= fdate) & 
                                        (Reslin_queasy.deci2 > 0)).order_by(Reslin_queasy._recid).all():
                                    t_argt_ratelist.rate_child = to_decimal(
                        ...
                        reslin_queasy = get_cache(
                            Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "number1": [(eq, bfast_dept)], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, bfast_artnr)], "resnr": [(eq, genstat.resnr)], "reslinnr": [(eq, genstat.res_int[0])], "date1": [(le, fdate)], "date2": [(ge, fdate)], "deci3": [(gt, 0)]})
                        ...
                        if reslin_queasy:
                            ...
                            for reslin_queasy in db_session.query(Reslin_queasy).filter(
                                    ...
                                    (Reslin_queasy.date1 <= fdate) & 
                                    (Reslin_queasy.date2 >= fdate) & 
                                    (Reslin_queasy.deci3 > 0)).order_by(Reslin_queasy._recid).all():
                                t_argt_ratelist.rate_infant = to_decimal(
                                    t_argt_ratelist.rate_infant) + to_decimal(reslin_queasy.deci3)
                        else:
                            reslin_queasy = get_cache(
                                Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, genstat.res_int[1])], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, bfast_artnr)], "resnr": [(eq, bfast_dept)], "reslinnr": [(eq, genstat.zikatnr)], "date1": [(le, fdate)], "date2": [(ge, fdate)], "deci3": [(gt, 0)]})
                    ...
                    for fixleist in db_session.query(Fixleist).filter(
                            (Fixleist.resnr == genstat.resnr) & 
                            (Fixleist.reslinnr == genstat.res_int[0]) & 
                            (Fixleist.artnr == t_argt_ratelist.bfast_artnr) & 
                            (Fixleist.departement == t_argt_ratelist.bfast_dept)).order_by(Fixleist._recid).all():
            ...
            t_argt_ratelist = query(t_argt_ratelist_data, filters=(
                lambda t_argt_ratelist: t_argt_ratelist.do_it), first=True)

            if t_argt_ratelist:
                guest = get_cache(
                    Guest, {"gastnr": [(eq, genstat.gastnrmember)]})

                reservation = get_cache(
                    Reservation, {"resnr": [(eq, genstat.resnr)]})
...
# end FDL: DFCF68
                ...
                if guest:
                    ...
                    mc_guest = get_cache(
                        Mc_guest, {"gastnr": [(eq, guest.gastnr)]})

                    if mc_guest:
                        abf_list.mcard_number = mc_guest.cardnum

                        mc_types = get_cache(
                            Mc_types, {"nr": [(eq, mc_guest.nr)]})
                ...
                mealcoup = get_cache(
                    Mealcoup, {"resnr": [(eq, genstat.resnr)], "zinr": [(eq, genstat.zinr)], "name": [(eq, "breakfast")]})
                ...
                if show_bfast_rate:
                    for t_argt_ratelist in query(t_argt_ratelist_data, filters=(lambda t_argt_ratelist: t_argt_ratelist.do_it)):
                        artikel = db_session.query(Artikel).filter(
                            (Artikel.artnr == t_argt_ratelist.bfast_artnr) & 
                            (Artikel.departement == t_argt_ratelist.bfast_dept) & 
                            (Artikel.betriebsnr != 0) & 
                            (Artikel.pricetab)).first()

                        if artikel:
                            waehrung = get_cache(
                                Waehrung, {"waehrungsnr": [(eq, artikel.betriebsnr)]})
                            ...
                            if waehrung:
                                ...
                                if t_argt_ratelist.based_on_adult:
                                    abf_list.bfast_revenue = to_decimal(abf_list.bfast_revenue) + to_decimal(
                                        t_argt_ratelist.rate_adult) * to_decimal(t_argt_ratelist.adult_qty) * to_decimal(exchg_rate)

                                elif t_argt_ratelist.based_on_child:
                                    abf_list.c_bfast_revenue = to_decimal(abf_list.c_bfast_revenue) + to_decimal(
                                        t_argt_ratelist.rate_child) * to_decimal(t_argt_ratelist.child_qty) * to_decimal(exchg_rate)

                                elif t_argt_ratelist.based_on_infant:
                                    abf_list.i_bfast_revenue = to_decimal(abf_list.i_bfast_revenue) + to_decimal(
                                        t_argt_ratelist.rate_infant) * to_decimal(t_argt_ratelist.infant_qty) * to_decimal(exchg_rate)
                        else:
                            if t_argt_ratelist.based_on_adult:
                                abf_list.bfast_revenue = to_decimal(
                                    abf_list.bfast_revenue) + to_decimal(t_argt_ratelist.rate_adult) * to_decimal(t_argt_ratelist.adult_qty)

                            elif t_argt_ratelist.based_on_child:
                                abf_list.c_bfast_revenue = to_decimal(
                                    abf_list.c_bfast_revenue) + to_decimal(t_argt_ratelist.rate_child) * to_decimal(t_argt_ratelist.child_qty)

                            elif t_argt_ratelist.based_on_infant:
                                abf_list.i_bfast_revenue = to_decimal(
                                    abf_list.i_bfast_revenue) + to_decimal(t_argt_ratelist.rate_infant) * to_decimal(t_argt_ratelist.infant_qty)
                else:
...
# end FDL: DFCF68
    ...
    def check_fixleist_posted1(artnr: int, dept: int, fakt_modus: int, intervall: int, lfakt: date):
        ...
        master = get_cache(
            Master, {"resnr": [(eq, res_line.resnr)], "active": [(eq, True)], "flag": [(eq, 0)]})
        ...
        if master_flag:
            invoice = db_session.query(Invoice).filter(
                (Invoice.resnr == genstat.resnr) & 
                (Invoice.reslinnr == 0)).first()
        else:
            invoice = db_session.query(Invoice).filter(
                (Invoice.zinr == genstat.zinr) & 
                (Invoice.resnr == genstat.resnr) & 
                (Invoice.reslinnr == genstat.res_int[0]) & 
                (Invoice.billtyp == 0) & 
                (Invoice.billnr == 1) & 
                (Invoice.flag == 0)).first()
