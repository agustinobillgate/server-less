...
from models import H_mjourn, Printer, Artikel, Htparam, H_bill, Hoteldpt, Queasy, H_queasy, Kellner, H_bill_line, H_artikel, Kontplan, Paramtext, Bediener, Tisch, Res_line, Mc_guest, Bill, Guest, Printcod, Waehrung

def print_hbilllnl_1bl(pvilanguage:int, session_parameter:string, user_init:string, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):

    prepare_cache ([Artikel, Htparam, H_bill, Hoteldpt, Queasy, H_queasy, Kellner, H_bill_line, Kontplan, Paramtext, Bediener, Tisch, Res_line, Mc_guest, Bill, Guest, Printcod, Waehrung])
    ...
    mjourn_qty:int = 0
    h_mjourn = printer = artikel = htparam = h_bill = hoteldpt = queasy = h_queasy = kellner = h_bill_line = h_artikel = kontplan = paramtext = bediener = tisch = res_line = mc_guest = bill = guest = printcod = waehrung = None

    art_list = art_list_submenu = t_printer = vat_list = output_list = artbuff = abuff = bart_list_submenu = None

    art_list_data, Art_list = create_model("Art_list", {"printed":bool, "disc_flag":bool, "dept":int, "artnr":int, "bezeich":string, "artart":int, "zwkum":int, "qty":int, "price":Decimal, "amount":Decimal, "betrag":Decimal, "happyhr":string, "datum":date, "sysdate":date, "zeit":int, "condiment":int, "barcode":string, "hline_recid":int, "is_deleted":bool})
    ...
    output_list_data, Output_list = create_model("Output_list", {"str":string, "str_pos":int, "pos":int, "flag_popup":bool, "npause":int, "sort_i":int})
            ...
            if h_bill.bilname != "":
                ...
                if gst_logic:
                    ...
                    output_list.str = output_list.str +\
                        " " + to_string(bill_date) + " " +\
                        to_string(time_lastposted , "HH:MM") + " " +\
                ...
                output_list.str = output_list.str + to_string(" ") + to_string(translateExtended ("Guest", lvcarea, "")) + " " + to_string(h_bill.bilname, "x(64)")
                output_list = Output_list()
                output_list_data.append(output_list)
                ...
                if gst_logic:
                    ...
                    output_list.str_pos = 1
                    output_list.str = output_list.str +\
                        " " + to_string(bill_date) + " " + to_string(time_lastposted , "HH:MM") +\
                        " " + translateExtended ("BillNo", lvcarea, "") + " " +\
                        rechnr_str
    ...
    def print_billine():
        ...
        if prtwoline:
            ...
            output_list.str = output_list.str + art_list.barcode + "|"
        output_list = Output_list()
        output_list_data.append(output_list)
    ...
    def print_overhead3():
        ...
        if h_bill.bilname != "":
            ...
            if qty1000:
                ...
                output_list.str = output_list.str + to_string("", "x(5)")
            output_list.str = output_list.str + to_string(translateExtended (" :", lvcarea, "") + " " + to_string(get_current_time_in_seconds(), "HH:MM:SS") , "x(20)")
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    if printnr != 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not art_list:
                ...
                queasy = get_cache (Queasy, {"key": [(eq, 200)],"number1": [(eq, h_bill_line.departement)],"number2": [(eq, h_bill_line.artnr)]})

                if queasy:
                    art_list.barcode = queasy.char1
