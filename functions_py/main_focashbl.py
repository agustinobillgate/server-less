    ...
    for artikel in db_session.query(Artikel).filter(
        ...
        if not all_user:
            for billjournal in db_session.query(Billjournal).filter(
                (Billjournal.userinit == (user_init).lower()) &
                (Billjournal.artnr == artikel.artnr) &
                (Billjournal.anzahl != 0) &
                (Billjournal.departement == artikel.departement) &
                (Billjournal.bill_datum == ci_date) &
                (Billjournal.betriebsnr == 0)
            ).order_by(Billjournal._recid).all():

                journal = db_session.query(Billjournal).filter(Billjournal._recid == billjournal._recid).first()

                if journal:
                    db_session.refresh(journal, with_for_update=True) 
