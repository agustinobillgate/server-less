...
def print_hbill1bl(pvilanguage:int, session_parameter:string, user_init:string, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):
    ...
    art_list_data, Art_list = create_model("Art_list", {"printed":bool, "disc_flag":bool, "dept":int, "artnr":int, "bezeich":string, "artart":int, "zwkum":int, "qty":int, "price":Decimal, "amount":Decimal, "betrag":Decimal, "happyhr":string, "datum":date, "sysdate":date, "zeit":int, "condiment":int, "hline_recid":int, "is_deleted":bool})
    art_list_submenu_data, Art_list_submenu = create_model_like(H_mjourn, {"hline_recid":int, "is_deleted":bool})
        ...
        if printed_line == 0 or print_all:
            ...
            if h_bill.bilname != "":
                ...
                if h_bill.resnr > 0 and h_bill.reslinnr > 0:
                    ...
                    bill = get_cache (Bill, {"resnr": [(eq, h_bill.resnr)],"reslinnr": [(eq, h_bill.reslinnr)]})
    ...
    def print_billine():
        ...
        for h_mjourn in db_session.query(H_mjourn).filter(
            ...
            h_art = db_session.query(H_art).filter(
                     (H_art.artnr == h_mjourn.artnr) & (H_art.departement == art_list.dept)).first() # Rulita from H_art.departement == dept to H_art.departement == art_list.dept
    ...
    def cal_totalfb():
        ...
        for h_bill_line, h_artikel in db_session.query(H_bill_line, H_artikel).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).filter(
            ...
            if h_artikel.artnr == disc_art1:
                total_fdisc =  to_decimal(total_fdisc) + to_decimal(fact) * to_decimal(h_bill_line.epreis)

            elif h_artikel.artnr == disc_art2:
                total_bdisc =  to_decimal(total_bdisc) + to_decimal(fact) * to_decimal(h_bill_line.epreis)

            elif h_artikel.artnr == disc_art3:
                total_odisc =  to_decimal(total_odisc) + to_decimal(fact) * to_decimal(h_bill_line.epreis)
            else:
                ...
                if artikel.umsatzart == 3 or artikel.umsatzart == 5:
                    total_food =  to_decimal(total_food) + to_decimal(fact) * to_decimal(h_bill_line.anzahl) * to_decimal(h_bill_line.epreis)

                elif artikel.umsatzart == 6:
                    total_bev =  to_decimal(total_bev) + to_decimal(fact) * to_decimal(h_bill_line.anzahl) * to_decimal(h_bill_line.epreis)

                elif artikel.umsatzart == 4:
                    total_other =  to_decimal(total_other) + to_decimal(fact) * to_decimal(h_bill_line.anzahl) * to_decimal(h_bill_line.epreis)
    ...
    if gst_logic == False:
        ...
        htparam = get_cache (Htparam, {"paramnr": [(eq, 1027)],"bezeichnung": [(ne, "not used")]})
    ...
    if printnr != 0:
        ...
        if not use_h_queasy:
            ...
            queasy = get_cache (Queasy, {"key": [(eq, 4)],"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],"number2": [(eq, 0)],"deci2": [(eq, h_bill.billnr)]}) # Rulita from billnr to h_bill.billnr
            ...
            if not queasy:
                ...
                queasy.number2 = 0
                queasy.deci2 =  to_decimal(h_bill.billnr) # Rulita from billnr to h_bill.billnr


        else:

            h_queasy = get_cache (H_queasy, {"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],"number2": [(eq, 0)],"billno": [(eq, h_bill.billnr)]}) # Rulita from billnr to h_bill.billnr
            ...
            if not h_queasy:
                ...
                h_queasy.number2 = 0
                h_queasy.billno = h_bill.billnr # Rulita from billnr to h_bill.billnr
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not use_h_queasy:
                ...
                queasy = get_cache (Queasy, {"key": [(eq, 4)],"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],"number2": [(eq, to_int(h_bill_line._recid))],"deci2": [(eq, h_bill.billnr)]}) # Rulita from billnr to h_bill.billnr 
                ...
                h_queasy = get_cache (H_queasy, {"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],"number2": [(eq, to_int(h_bill_line._recid))],"billno": [(eq, h_bill.billnr)]})  # Rulita from billnr to h_bill.billnr
            ...
            if art_list.artart == 0:
                ...
                mwst_perc =  to_decimal(mwst_percent) * to_decimal("100")
                serv_perc =  to_decimal(serv_percent) * to_decimal("100")
                ...
                if h_bill_line.artnr == f_discart:
                    ...
                    if h_bill_line.epreis != h_bill_line.betrag:
                        ...
                        if not serv_disc and h_bill_line.artnr == f_discart:
                            ...
                            h_service =  to_decimal(h_bill_line.betrag) / to_decimal(fact) * to_decimal(serv_percent)
                        ...
                        if not vat_disc and h_bill_line.artnr == f_discart:
                            ...
                            h_mwst =  to_decimal(h_bill_line.betrag) / to_decimal(fact) * to_decimal(mwst_percent)
                        ...
                        if not incl_service and not incl_mwst:
                            amount =  to_decimal(h_bill_line.betrag) / to_decimal(fact)
                        ...
                        if not incl_service:
                            service =  to_decimal(service) + to_decimal((amount) * to_decimal(serv_percent) )
                        ...
                        if not incl_mwst:
                            mwst =  to_decimal(mwst) + to_decimal((amount) * to_decimal(mwst_percent) )


                        mwst1 =  to_decimal(mwst1) + to_decimal((amount) * to_decimal(mwst_percent))
        ...
        h_mjourn = get_cache (H_mjourn, {"rechnr": [(eq, h_bill.rechnr)],"departement": [(eq, h_bill.departement)],"anzahl": [(lt, 0)]})
