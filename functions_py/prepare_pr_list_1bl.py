...
# yusufwijasena, 15/12/2025
# - fixed db_session.query(lbuff).with_for_update().first()
# -------------------------------------------------------
...
def prepare_pr_list_1bl():
    ...
    t_l_artikel_data, T_l_artikel = create_model(
        "T_l_artikel",
        {
            "rec_id": int,
            "artnr": int,
    ...
    def create_costlist():
        ...
        for parameters in db_session.query(Parameters).filter(
                ...
                (Parameters.varname > "")
        ).order_by(Parameters._recid).all():
            cost_list = Cost_list()
