...
from models import Htparam, Guest, Reservation, Res_line, History, Guestseg, Segment, Reslin_queasy, Nation, Bill, Mc_guest, Mc_types
...
def pj_depart_4_webbl(pvilanguage:int, case_type:int, disptype:int, curr_date:date, froom:string, troom:string, disp_accompany:bool, ota_only:bool, setup_list_data:[Setup_list], zikat_list_data:[Zikat_list]):

    prepare_cache ([Htparam, Guest, Reservation, Res_line, Guestseg, Segment, Reslin_queasy, Nation, Bill, Mc_guest, Mc_types])
    ...
    all_remark:string = ""
    curr_zinr:string = ""
    htparam = guest = reservation = res_line = history = guestseg = segment = reslin_queasy = nation = bill = mc_guest = mc_types = None
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, case_type, disptype, curr_date, froom, troom, disp_accompany, ota_only
    ...
    def create_departure():
        ...
        G_ota =  create_buffer("G_ota",Guest)
        tot_rm = 0
        tot_a = 0
        tot_c = 0
        tot_co = 0
        ...
        if disptype == 1:
            ...
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 8) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Reservation.groupname, Res_line.name, Res_line.zinr).all():
                ...
                if ota_only:
                    ...
                    g_ota = get_cache (Guest, {"gastnr": [(eq, reservation.gastnr)],"karteityp": [(eq, 2)],"steuernr": [(ne, "")]})

                    if g_ota:
                        do_it = True

                        if matches(g_ota.name,r"*e1-booking*"):
                            do_it = False
                    else:
                        do_it = False
                ...
                if do_it:
                    ...
                    if setup_list:
                        ...
                        cl_list.rmcat = zikat_list.kurzbez
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 8) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.zinr, Res_line.name).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 8) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Reservation.name, Reservation.groupname, Res_line.name, Res_line.zinr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 8) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.abreise, Res_line.name, Res_line.zinr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 8) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.resnr, Res_line.reslinnr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
        ...
        for cl_list in query(cl_list_data, filters=(lambda cl_list:(cl_list.a + cl_list.co) > 0), sort_by=[("nation",False),("bezeich",False)]):
            tot_a = tot_a + cl_list.a + cl_list.co
            tot_c = tot_c + cl_list.c
            tot_co = tot_co + cl_list.co
        ...
        if (tot_a) != 0:
            ...
            for s_list in query(s_list_data, filters=(lambda s_list: s_list.nat != "")):
                ...
                s_list.proz =  to_decimal(s_list.adult) / to_decimal(tot_a) * to_decimal("100")
    ...
    def create_departure1():
        ...
        if disptype == 1:
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.resstatus == 8) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.abreise, Res_line.name, Res_line.zinr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.resstatus == 8) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.resnr, Res_line.reslinnr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
    ...
    def create_actual():
        ...
        if disptype == 1:
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.resstatus == 8) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Reservation.resnr, Res_line.resnr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
    ...
    def create_expected():
        ...
        if disptype == 1:
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.active_flag == 1) & ((Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Reservation.groupname, Res_line.name, Res_line.zinr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.active_flag == 1) & ((Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.zinr, Res_line.name).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.active_flag == 1) & ((Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Reservation.name, Reservation.groupname, Res_line.name, Res_line.zinr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.active_flag == 1) & ((Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.abreise, Res_line.name, Res_line.zinr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     (Res_line.active_flag == 1) & ((Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 11)) & (Res_line.abreise == curr_date) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.resnr, Res_line.reslinnr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
