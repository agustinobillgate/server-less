...
def print_hbilllnlbl(pvilanguage:int, session_parameter:string, user_init:string, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):
    ...
    if printnr != 0:
        ...
        if not use_h_queasy:
            ...
            queasy = db_session.query(Queasy).filter(
                     (Queasy.key == 4) & (Queasy.number1 == (h_bill.departement + h_bill.rechnr * 100)) & (Queasy.number2 == 0) & (Queasy.deci2 == to_decimal(h_bill.billnr))).with_for_update().first()
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not use_h_queasy:
                ...
                h_queasy = db_session.query(H_queasy).filter(
                         (H_queasy.number1 == (h_bill.departement + h_bill.rechnr * 100)) & (H_queasy.number2 == to_int(h_bill_line._recid)) & (H_queasy.billno == h_bill.billnr)).with_for_update().first()
                if not h_queasy:
                    new_item = True
        ...
        if printnr >= 0:
            ...
            if not use_h_queasy:
                ...
                # h_queasy = get_cache (H_queasy, {"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],"number2": [(eq, 0)],"billno": [(eq, billnr)]})
                h_queasy = db_session.query(H_queasy).filter(
                         (H_queasy.number1 == (h_bill.departement + h_bill.rechnr * 100)) & (H_queasy.number2 == 0) & 
                         (H_queasy.billno == billnr)).with_for_update().first()
                if h_queasy:
