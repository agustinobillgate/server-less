...
def glacct_cashflow_1bl(from_date:date, to_date:date, from_lsyr:date, to_lsyr:date, pfrom_date:date, pto_date:date, from_month:date, from_year:date, coa_list_data:[Coa_list]):
    ...
    def generate_output():
        nonlocal t_list_data, close_month, close_year, last_close_yr, prev_month, gl_acct, gl_accthis, gl_journal, gl_jouhdr
    ...
    def calc_balance(i_case:int, fibu:string, from_date:date, to_date:date):
        ...
        if get_day(from_date) >= 2 and (gbuff.acc_type == 3 or gbuff.acc_type == 4):
            ...
            date2 = date_mdy(get_month(from_date) , get_day(from_date) - 1, get_year(from_date))    # Rulita, 744E3B | Fixing get day - 1
        ...
        for gl_jouhdr, gl_journal in db_session.query(Gl_jouhdr, Gl_journal).join(Gl_journal,(Gl_journal.jnr == Gl_jouhdr.jnr) & (Gl_journal.fibukonto == (fibu).lower())).filter(
            ...
            if i_case > 0:
                ...
                t_list = query(t_list_data, filters=(lambda t_list: t_list.fibukonto.lower()  == (fibu).lower()  and t_list.cf == i_cf), first=True)
