...
def prepare_fa_newpobl(order_nr: string):
    ...
    prepare_cache([L_lieferant, Mathis, Parameters,
                  Htparam, Waehrung, Fa_counter])
    ...
    def currency_list():
        ...
        waehrung_data = (
            db_session.query(Waehrung).filter(
                (Waehrung.waehrungsnr != local_nr) &
                (Waehrung.ankauf > 0) &
                (Waehrung.betriebsnr != 0)
            ).order_by(Waehrung.wabkurz)
        )

        for waehrung in waehrung_data:
