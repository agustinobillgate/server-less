...
def s_transform_btn_gobl(op_list_data:[Op_list], rec_id:int, curr_lager:int, curr_pos:int, transdate:date, wip_acct:string, to_stock:int, s_artnr1:int, qty1:Decimal, bediener_nr:int):
    ...
    def l_op_pos():
        ...
        for l_op1 in db_session.query(L_op1).filter(
                 (L_op1.lscheinnr == op_list.lscheinnr) & (L_op1.loeschflag >= 0) & (L_op1.pos > 0)).order_by(L_op1._recid).all():
    ...
    def create_l_op(zeit:int):
        ...
        l_verbrauch = db_session.query(L_verbrauch).filter(
            (L_verbrauch.artnr == s_artnr) & (L_verbrauch.datum == transdate)
        ).with_for_update().first()
    ...
    def create_transin():
        ...
        l_bestand = db_session.query(L_bestand).filter(
            (L_bestand.lager_nr == 0) & (L_bestand.artnr == s_artnr1)
        ).with_for_update().first()
        ...
        l_bestand = db_session.query(L_bestand).filter(
            (L_bestand.lager_nr == to_stock) & (L_bestand.artnr == s_artnr1)
        ).with_for_update().first()
