...
def fo_wordadmin_btn_exitbl(t_brief_data:[T_brief], case_type:int, last_column:string, kateg:int, user_init:string):
    ...
    if case_type == 1:
        ...
        brief = db_session.query(Brief).filter(Brief.briefnr == t_brief.briefnr).with_for_update().first()
