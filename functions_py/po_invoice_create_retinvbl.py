...
def po_invoice_create_retinvbl(pvilanguage:int, tot_amt:Decimal, tot_disc:Decimal, tot_disc2:Decimal, tot_vat:Decimal, tot_val:Decimal, qty:Decimal, brutto:Decimal, s_list_s_recid:int, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, bediener_nr:int):
    ...
    def generate_output():
        nonlocal msg_str, s_list_data, lvcarea, l_op, l_artikel, l_bestand, l_kredit, l_liefumsatz
        ...
        return {"tot_amt": tot_amt, "tot_disc": tot_disc, "tot_disc2": tot_disc2, "tot_vat": tot_vat, "tot_val": tot_val, "msg_str": msg_str, "s-list": s_list_data}
    ...
    def reorg_oh1():
        ...
        l_art = db_session.query(L_art).filter(L_art.artnr == l_op.artnr).first()
        ...
        l_kredit = db_session.query(L_kredit).filter(
                 (L_kredit.lief_nr == l_op.lief_nr) & (L_kredit.name == l_op.docu_nr) & (L_kredit.lscheinnr == l_op.lscheinnr) & (L_kredit.opart == 0) & (L_kredit.zahlkonto == 0)).with_for_update().first()
        ...
        l_liefumsatz = db_session.query(L_liefumsatz).filter(
                 (L_liefumsatz.lief_nr == l_op.lief_nr) & (L_liefumsatz.datum == l_op.datum)).with_for_update().first()
