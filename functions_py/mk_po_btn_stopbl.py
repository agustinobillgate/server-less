...
def mk_po_btn_stopbl(po_type:int, rec_id:int, lief_nr:int, docu_nr:string):
    ...
    for l_order in db_session.query(L_order).filter(
             (L_order.lief_nr == lief_nr) & (L_order.loeschflag == 0) & (L_order.pos >= 0) & 
             (L_order.docu_nr == (docu_nr).lower()) & (L_order.betriebsnr == 2)).order_by(L_order._recid).with_for_update().all():
