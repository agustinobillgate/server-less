...
def rest_cjourn_journal_list1bl(from_dept:int, to_dept:int, from_date:date, to_date:date):
    ...
    cancel_journal_data, Cancel_journal = create_model("Cancel_journal", {"dept":int, "rechnr":int, "billdate":date, "srecid":int, "depart":string, "tbno":string, "artno":string, "bezeich":string, "cancel":string, "qty":string, "amount":Decimal, "zeit":string, "cname":string})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal cancel_journal_data, hoteldpt, h_journal, kellner
        nonlocal from_dept, to_dept, from_date, to_date


        nonlocal cancel_journal
        nonlocal cancel_journal_data

        return {"cancel-journal": cancel_journal_data}
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            for curr_date in date_range(from_date,to_date) :
                ...
                for h_journal in db_session.query(H_journal).filter(
                    ...
                    cancel_journal_data.append(cancel_journal)
