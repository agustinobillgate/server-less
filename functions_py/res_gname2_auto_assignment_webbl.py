...
# # =================================================================
# Rulita, 05-12-2025
# Fixing issue While loop procedure auto_assignment
# Fixing issue While loop procedure auto_assignment_with_selectedroom
# ===================================================================
...
def res_gname2_auto_assignment_webbl(s_list_data:[S_list], active_roomlist_data:[Active_roomlist], v_mode:int, location:string, froom:string, troom:string):
        ...
        for s1_list in query(s1_list_data, filters=(lambda s1_list: s1_list.zinr == "" and s1_list.resstatus != 11 and s1_list.zimmeranz == 1)):
            ...
            if location != "":
                ...
                zimmer = db_session.query(Zimmer).filter(
                         (Zimmer.code == (location).lower()) & (Zimmer.zinr >= (froom).lower()) & (Zimmer.zinr <= (troom).lower()) & (Zimmer.zikatnr == rline.zikatnr) & (Zimmer.setup == rline.setup) & (Zimmer.zinr > (last_zinr).lower())).first()
            else:
                # zimmer = get_cache (Zimmer, {"zinr": [(ge, froom),(le, troom),(gt, last_zinr)],"zikatnr": [(eq, rline.zikatnr)],"setup": [(eq, rline.setup)]})
                zimmer = db_session.query(Zimmer).filter(
                         (Zimmer.zinr >= (froom).lower()) & (Zimmer.zinr <= (troom).lower()) & (Zimmer.zikatnr == rline.zikatnr) & (Zimmer.setup == rline.setup) & (Zimmer.zinr > (last_zinr).lower())).first()
                ...
                if do_it:
                    ...
                    if s1_list.resstatus <= 5:
                        ...
                        queasy = db_session.query(Queasy).filter(
                                 (Queasy.key == 359) & (Queasy.char1 == s1_list.zinr) & (Queasy.number1 == rline.resnr) & (Queasy.number2 == rline.reslinnr) & (Queasy.number3 == 1)).first()
    ...
    def auto_assignmen_with_selectedroom():
        ...
        for s1_list in query(s1_list_data, filters=(lambda s1_list: s1_list.zinr == "" and s1_list.resstatus != 11 and s1_list.zimmeranz == 1)):
            ...
            for active_roomlist in query(active_roomlist_data, filters=(lambda active_roomlist: active_roomlist.zikatnr == rline.zikatnr and active_roomlist.room_selected and active_roomlist.setup == rline.setup and active_roomlist.zinr.lower()  > (last_zinr).lower()), sort_by = [("zinr",False)]):
                if not found:
                    do_it = True
        ...
        for s1_list in query(s1_list_data, filters=(lambda s1_list: s1_list.zinr == "" and s1_list.active_flag == 0 and s1_list.resstatus != 11)):
            ...
            # print(len(active_roomlist))
            # while None != active_roomlist and not found:

            tmp_active_roomlist_data = active_roomlist_data.copy()
            for active_roomlist in query(tmp_active_roomlist_data, filters=(lambda active_roomlist: active_roomlist.zikatnr == rline.zikatnr and active_roomlist.room_selected and active_roomlist.setup != rline.setup and active_roomlist.zinr.lower()  > (last_zinr).lower()), sort_by = [("zinr",False)]):
                print("test loop")
                if not found:
                    do_it = True
