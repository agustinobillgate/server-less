...
from models import Queasy, Htparam, H_journal, H_bill, H_artikel, Artikel, Res_line, Guestseg, Segment
...
def rest_statsegment_go2_webbl(input_list_data:[Input_list]):

    prepare_cache ([Queasy, Htparam, H_journal, H_bill, H_artikel, Artikel, Res_line, Guestseg, Segment])
    ...
    queasy = htparam = h_journal = h_bill = h_artikel = artikel = res_line = guestseg = segment = None

    output_list = input_list = t_list = segm_list = bqueasy = tqueasy = None
    ...
    def generate_output():
        nonlocal output_list_data, pvilanguage, price_decimal, black_list, start_year, long_digit, f_endkum, m_endkum, b_endkum, bill_date, vat, vat2, service, fact, lvcarea, queasy, htparam, h_journal, h_bill, h_artikel, artikel, res_line, guestseg, segment
        nonlocal bqueasy, tqueasy


        nonlocal output_list, input_list, t_list, segm_list, bqueasy, tqueasy
    ...
    def create_list():
        ...
        rev_amount:Decimal = to_decimal("0.0")
        m_rev_amount:Decimal = to_decimal("0.0")
        y_rev_amount:Decimal = to_decimal("0.0")
        curr_segm:int = 0
        gname:string = ""
        curr_billno:string = ""
        curr_dept:int = 0
            ...
            if h_bill:
                ...
                if h_artikel:
                    ...
                    if artikel:
                        segm_no = 0
                        guest_no = 0
                        rev_amount =  to_decimal("0")
                        m_rev_amount =  to_decimal("0")
                        y_rev_amount =  to_decimal("0")

                        if input_list.excl_tax  and h_journal.betrag != 0:
                            service =  to_decimal("0")
                            vat =  to_decimal("0")
                            vat2 =  to_decimal("0")
                            fact =  to_decimal("1")
                            service, vat, vat2, fact = get_output(calc_servtaxesbl(1, artikel.artnr, artikel.departement, h_journal.bill_datum))
                            rev_amount =  to_decimal(h_journal.betrag) / to_decimal(fact)
                        else:
                            rev_amount =  to_decimal(h_journal.betrag)
                        ...
                        if segm_no != 0:
                            ...
                            if h_journal.bill_datum >= input_list.from_date and h_journal.bill_datum <= input_list.to_date:
                                ...
                                t_list.m_rev =  to_decimal(t_list.m_rev) + to_decimal(rev_amount)
                                tot_mrev =  to_decimal(tot_mrev) + to_decimal(rev_amount)
                            ...
                            t_list.y_rev =  to_decimal(t_list.y_rev) + to_decimal(rev_amount)
                            tot_yrev =  to_decimal(tot_yrev) + to_decimal(rev_amount)
        ...
        for t_list in query(t_list_data, sort_by=[("segm_no",False)]):
            ...
            queasy.key = 280
            queasy.char1 = "Rest Stat Guest Segment"
            queasy.char3 = input_list.id_flag
            queasy.char2 = trim(output_list.segm_no) +\
                    "|" + trim(output_list.g_segm) +\
                    "|" + trim(output_list.segm_gr) +\
                    "|" + trim(output_list.pax) +\
                    "|" + trim(output_list.t_rev) +\
                    "|" + trim(output_list.m_pax) +\
                    "|" + trim(output_list.m_rev) +\
                    "|" + trim(output_list.y_pax) +\
                    "|" + trim(output_list.y_rev) +\
                    "|" + trim(output_list.proz_pax) +\
                    "|" + trim(output_list.proz_trev) +\
                    "|" + trim(output_list.proz_mpax) +\
                    "|" + trim(output_list.proz_mrev) +\
                    "|" + trim(output_list.proz_ypax) +\
                    "|" + trim(output_list.proz_yrev)
        ...
        if output_list:
            ...
            queasy.char1 = "Rest Stat Guest Segment"
            queasy.char3 = input_list.id_flag
            queasy.char2 = "No Data Available" +\
                    "|" + "" +\
                    "|" + "" +\
                    "|" + "" +\
                    "|" + "" +\
