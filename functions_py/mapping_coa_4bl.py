...
def mapping_coa_4bl(coa_list_data:[Coa_list]):
    i:int = 0
    ...
    def update_gl3():
        ...
        while None != gl_acct:
            ...
            if coa_list:
                acct_list = Acct_list()
                acct_list_data.append(acct_list)
        ...
        coa_list = query(coa_list_data, filters=(lambda coa_list: coa_list.old_fibu != None), first=True)
        while None != coa_list:

            acct_list = query(acct_list_data, filters=(lambda acct_list: acct_list.fibukonto == coa_list.old_fibu), first=True)
            while None != acct_list:
                ...
                acct_list = query(acct_list_data, filters=(lambda acct_list: acct_list.fibukonto == coa_list.old_fibu), next=True)
