...
"""_yusufwijasena_20/11/2025
        _remark_:   - fix issue wrong name variable in guest.bemerk to guest.bemerkung
"""
...
def ns_web_open_bill_1bl(bil_recid: int, foreign_rate: bool, double_currency: bool, ba_dept: int):
    ...
    t_bill_data, T_bill = create_model(
        "T_bill",
        {
            "zinr": str,
            "flag": int,
    ...
    def generate_output():
        ...
        return {
            ...
            "enbtn_bareserve": enbtn_bareserve,
    ...
    def disp_bill_line(read_flag: bool):
        ...
        if read_flag:
            t_bill_line_data = get_output(read_bill_line_cldbl(
                3, 1, t_bill.rechnr, None, None, None, None, None))

        for t_bill_line in query(t_bill_line_data):

            spbill_list = query(spbill_list_data, filters=(
                lambda spbill_list: spbill_list.bl_recid == t_bill_line.bl_recid), first=True)
    ...
    rescomment = t_guest.bemerkung  # yusufwijasena: fix wrong variable name
    art_no = t_guest.zahlungsart
    guest_taxcode = to_string(t_guest.firmen_nr)

    if t_bill.bilname != "" and t_bill.name != t_bill.bilname:
        rescomment = "Guest Name: " + t_bill.bilname + \
            chr_unicode(10) + rescomment

    if t_bill.vesrdepot != "":
        rescomment = rescomment + \
            chr_unicode(10) + t_bill.vesrdepot + chr_unicode(10)
    ...
    if t_bill.flag == 0 and t_bill.rechnr > 0 and t_bill.billtyp == ba_dept and (t_bill.rechnr != int(invno)):
        t_bk_veran_data = get_output(
            read_bk_veranbl(3, None, None, t_bill.rechnr, 0))

        t_bk_veran = query(t_bk_veran_data, first=True)

        if t_bk_veran:
            t_bk_reser_data = get_output(read_bk_reserbl(
                4, t_bk_veran.veran_nr, None, 1, None))
