...
def ba_plan_create_wlistbl(rml_data:[Rml], from_date:date):
    ...
    def generate_output():
        nonlocal loopi, maxpar, bk_raum, guest, bk_veran, bk_reser
        nonlocal from_date
    ...
    for bk_reser, bk_veran, gast in db_session.query(Bk_reser, Bk_veran, Gast).join(Bk_veran,(Bk_veran.veran_nr == Bk_reser.veran_nr)).join(Gast,(Gast.gastnr == Bk_veran.gastnr)).filter(
        ...
        if childRM:
            ...
            if get_index(childRM.lu_raum, ";") > 0:
                maxpar = num_entries(childRM.lu_raum, ";")
                for loopi in range(1,maxpar + 1) :

                    rmbuff = db_session.query(Rmbuff).filter(
                             (rmBuff.raum == entry(loopi - 1, childRM.lu_raum, ";"))).first()
                    create_wlist2(bk_reser.raum, rmBuff.raum)
            else:
