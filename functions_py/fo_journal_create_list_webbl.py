...
def fo_journal_create_list_webbl(id_flag:string, fo_journal_list_data:[Fo_journal_list]):
    ...
    for queasy in db_session.query(Queasy).filter(
        ...
        fo_journal_list.artno = to_int(substring(queasy_str1, 23, 9))
        fo_journal_list.bezeich = trim(substring(queasy_str2, 12, 50))
        fo_journal_list.voucher = trim(substring(queasy_str2, 62, 40))
        fo_journal_list.depart = substring(queasy_str1, 82, 12)
        fo_journal_list.outlet = substring(queasy_str1, 94, 6)
        fo_journal_list.qty = to_int(substring(queasy_str1, 100, 5))
        fo_journal_list.amount = to_decimal(substring(queasy_str1, 105, 22))
