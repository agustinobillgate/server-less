...
# Rulita 17/12/2025
# - Fixing function buffer_copy to avoid copying _recid
# - Added function local buffer_copy to avoid copying _recid
# - Fixing command code progress not correct
...
def mk_resline_go1_1bl(pvilanguage: int, user_init: string, specrequest: bool, c_apply: string, incl_gname: bool, incl_ginhouse: bool, curr_resline_data: [Curr_resline]):
    ...
    curr_resline = t_resline = member_list = zwunsch_rline = zwunsch_rmember = update_fixrate = rline = rmember = rbuff = raccomp = spreqbuff = resline = None

    t_resline_data, T_resline = create_model_like(Res_line)
    member_list_data, Member_list = create_model(
        "Member_list",
        {
            "reslinnr": int
        })
    zwunsch_rline_data, Zwunsch_rline = create_model(
        "Zwunsch_rline",
        {
            "s_label": string,
            "s_value1": string,
    ...
    zwunsch_rmember_data, Zwunsch_rmember = create_model(
        "Zwunsch_rmember",
        {
            "s_label": string,
            "s_value2": string
        })
    update_fixrate_data, Update_fixrate = create_model(
        "Update_fixrate",
        {
            "date1": date,
            "date2": date,
    ...
    def check_fixrates_changes():
        ...
        reslin_queasy = db_session.query(Reslin_queasy).filter(Reslin_queasy.key == ("fixrate-trace-record"),
                        Reslin_queasy.resnr == curr_resline.resnr,
                        Reslin_queasy.reslinnr == curr_resline.reslinnr).first()
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
            ...
            (Reslin_queasy.resnr == curr_resline.resnr) &
            (Reslin_queasy.reslinnr == curr_resline.reslinnr)
        ).order_by(Reslin_queasy.date1).all():
            ...
            if reslin_queasy.date1 < reslin_queasy.date2:
                ...
                for curr_date in date_range(reslin_queasy.date1 + 1, reslin_queasy.date2):
                    ...
                    buffer_copy_local(reslin_queasy, rqsy,except_fields=["date1","date2"])
                    
                    # log_program.write_log('[DEBUG_2]', f'reslin_queasy._recid: {reslin_queasy._recid}')
                    # curr_queasy_recid = reslin_queasy._recid
                    # log_program.write_log('[DEBUG_2]', f'curr_queasy_recid: {curr_queasy_recid}')
                    
                    # buffer_copy(reslin_queasy, rqsy)
                    # rqsy.date1 = curr_date
                    # rqsy.date2 = curr_date
                    # rqsy.date3 = reslin_queasy.date3
                    # rqsy.logi1 = reslin_queasy.logi1
                    # rqsy.logi2 = reslin_queasy.logi2
                    # rqsy.logi3 = reslin_queasy.logi3
                    # rqsy.number1 = reslin_queasy.number1
                    # rqsy.number2 = reslin_queasy.number2
                    # rqsy.number3 = reslin_queasy.number3
                    # rqsy.deci1 = reslin_queasy.deci1
                    # rqsy.deci2 = reslin_queasy.deci2
                    # rqsy.deci3 = reslin_queasy.deci3
                    # rqsy.char1 = reslin_queasy.char1
                    # rqsy.char2 = reslin_queasy.char2
                    # rqsy.char3 = reslin_queasy.char3
                    # rqsy.reslinnr = reslin_queasy.reslinnr
                    # rqsy.key = reslin_queasy.key
                    # rqsy.resnr = reslin_queasy.resnr
                    # rqsy._recid = curr_queasy_recid
                    ...
                    # # log_program.write_log('LOG', f'rqsy: {rqsy.__dict__}')

                rqsy = db_session.query(Rqsy).filter(
                    (Rqsy._recid == reslin_queasy._recid)).with_for_update().first()
                rqsy.date2 = rqsy.date1

                update_fixrate = query(update_fixrate_data, filters=(
                    lambda update_fixrate: update_fixrate.date1 == rqsy.date1 and update_fixrate.date2 == rqsy.date2), first=True)

                if not update_fixrate:
                    update_fixrate = Update_fixrate()
                    update_fixrate_data.append(update_fixrate)

                    update_fixrate.date1 = rqsy.date1
                    update_fixrate.date2 = rqsy.date2
                    update_fixrate.argt = rqsy.char1

        for reslin_queasy in db_session.query(Reslin_queasy).filter(
            (Reslin_queasy.key == ("arrangement")) &
            (Reslin_queasy.resnr == rmember.resnr) &
            (Reslin_queasy.reslinnr == rmember.reslinnr)
        ).order_by(Reslin_queasy.date1).all():
            ...
            if reslin_queasy.date1 < reslin_queasy.date2:
                for curr_date in date_range(reslin_queasy.date1 + 1, reslin_queasy.date2):
                    ...
                    # log_program.write_log('LOG', f'reslin_queasy._recid: {reslin_queasy._recid}')
                    
                    # curr_queasy_recid = reslin_queasy._recid
                    # log_program.write_log('LOG', f'curr_queasy_recid: {curr_queasy_recid}')
                    # buffer_copy(reslin_queasy, rqsy)
                    # rqsy.date1 = curr_date
                    # rqsy.date2 = curr_date
                    # rqsy.date3 = reslin_queasy.date3
                    ...
                    if update_fixrate:
                        rqsy.char1 = update_fixrate.argt
        ...
        for reslin_queasy in db_session.query(Reslin_queasy).filter(
            ...
            do_it = (start_date < reslin_queasy.date3) or (
                (start_date == reslin_queasy.date3) and (start_time <= reslin_queasy.number2))
            ...
            if do_it:
                ...
                chgflag = not None != rqsy or (
                    None != rqsy and reslin_queasy.deci1 != rqsy.deci1)
                ...
                if chgflag:
                    ...
                    fixrate_changes(chg_mode, reslin_queasy.date1,
                                    rqsy.deci1, reslin_queasy.deci1)

                    # Rulita, 17/12/2025
                    # Fixing function buffer_copy to avoid copying _recid
                    # buffer_copy(reslin_queasy, rqsy, except_fields=[ "_recid", "resnr", "reslinnr"])
                    buffer_copy_local(
                        reslin_queasy,
                        rqsy,
                        except_fields=["resnr", "reslinnr"]
                    )
    ...
    def fixrate_changes(chg_mode: string, curr_date: date, old_rate: Decimal, new_rate: Decimal):
        ...
        if chg_mode == ("CHG"):
            ...
            rqy.char3 = to_string(rmember.ankunft) + ";" + \
                ...
                to_string(rmember.abreise) + ";" + \
                to_string(rmember.abreise) + ";" + \
                to_string(rmember.zimmeranz) + ";" + \
                to_string(rmember.zimmeranz) + ";" + \
                to_string(rmember.erwachs) + ";" + \
                to_string(rmember.erwachs) + ";" + \
                to_string(rmember.kind1) + ";" + \
                to_string(rmember.kind1) + ";" + \
                to_string(rmember.gratis) + ";" + \
                to_string(rmember.gratis) + ";" + \
                to_string(rmember.zikatnr) + ";" + \
                to_string(rmember.zikatnr) + ";" + \
                to_string(rmember.zinr) + ";" + \
                to_string(rmember.zinr) + ";" + \
                to_string(rmember.arrangement) + ";" + \
                to_string(rmember.arrangement) + ";" + \
                to_string(rmember.zipreis) + ";" + \
                to_string(rmember.zipreis) + ";" + \
                to_string(cid) + ";" + \
                to_string(user_init) + ";" + \
                to_string(cdate, "x(8)") + ";" + \
                to_string(get_current_date()) + ";" + \
                to_string("CHG Fixrate FR:") + ";" + \
                to_string(curr_date) + \
                "-" + to_string(old_rate) + ";" + \
                to_string("YES", "x(3)") + ";" + \
                to_string("YES", "x(3)") + ";"
    ...
    def res_changes():
        ...
        reslin_queasy.char3 = to_string(t_resline.ankunft) + ";" + \
            to_string(rbuff.ankunft) + ";" + \
            to_string(t_resline.abreise) + ";" + \
            to_string(rbuff.abreise) + ";" + \
            to_string(t_resline.zimmeranz) + ";" + \
            to_string(rbuff.zimmeranz) + ";" + \
            to_string(t_resline.erwachs) + ";" + \
            to_string(rbuff.erwachs) + ";" + \
            to_string(t_resline.kind1) + ";" + \
            to_string(rbuff.kind1) + ";" + \
            to_string(t_resline.gratis) + ";" + \
            to_string(rbuff.gratis) + ";" + \
            to_string(t_resline.zikatnr) + ";" + \
            to_string(rbuff.zikatnr) + ";" + \
            to_string(t_resline.zinr) + ";" + \
            to_string(rbuff.zinr) + ";"

        if rbuff.reserve_int == t_resline.reserve_int:
            reslin_queasy.char3 = reslin_queasy.char3 + \
                to_string(t_resline.arrangement) + ";" + \
                to_string(rbuff.arrangement) + ";"
        else:
            reslin_queasy.char3 = reslin_queasy.char3 + \
                to_string(t_resline.arrangement) + ";" + \
                to_string(t_resline.reserve_int) + ";"
        reslin_queasy.char3 = reslin_queasy.char3 + \
            to_string(t_resline.zipreis) + ";" + \
            to_string(rbuff.zipreis) + ";" + \
            to_string(cid) + ";" + \
            to_string(user_init) + ";" + \
            to_string(cdate, "x(8)") + ";" + \
            to_string(heute) + ";" + \
            to_string(t_resline.name) + ";" + \
            to_string(t_resline.bemerk) + ";"
        ...
        if t_resline.bemerk != rbuff.bemerk:
            ...
            res_history.aenderung = t_resline.bemerk + chr_unicode(10) + chr_unicode(10) \
    ...
    # Rulita, 17/12/2025
    # Added function local buffer_copy to avoid copying _recid
    def buffer_copy_local(src, dst, except_fields=None, debug=False):
        if except_fields is None:
            except_fields = []

        exclude = set(except_fields) | {"_recid"}
        mapper = inspect(src).mapper
        ...
        for column in mapper.columns:
            ...
            value = getattr(src, col_name)
            setattr(dst, col_name, value)
    ...
    res_line = db_session.query(Res_line).filter(
        (Res_line.resnr == curr_resline.resnr) &
        (Res_line.reslinnr == curr_resline.reslinnr)
    ).first()

    if specrequest:

        spreqbuff = db_session.query(spreqbuff).filter(
            (spreqbuff.key == ("specialRequest")) &
            (spreqbuff.resnr == curr_resline.resnr) &
            (spreqbuff.reslinnr == curr_resline.reslinnr)
        ).first()
    ...
    for i in range(1, num_entries(res_line.zimmer_wunsch, ";") - 1 + 1):
        ...
        if substring(str, 0, 7) == ("voucher"):
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("voucher")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("ChAge")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("&CODE$")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("DATE,")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("SEGM_PUR")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("ebdisc")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("kbdisx")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("restricted")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("pickup")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("drop-passanger")), first=True)
            ...
            zwunsch_rline = query(zwunsch_rline_data, filters=(
                lambda zwunsch_rline: zwunsch_rline.s_label == ("WCI-req")), first=True)
    ...
    resline = db_session.query(Resline).filter(
        (Resline.resnr == curr_resline.resnr) &
        (Resline.active_flag == 1) &
        (Resline.reslinnr != curr_resline.reslinnr) &
        (Resline.l_zuordnung[inc_value(2)] == 0)
    ).first()
    ...
    while None != resline:
        ...
        if allflag:
            ...
            member_list = query(member_list_data, filters=(
                lambda member_list: member_list.reslinnr == resline.reslinnr), first=True)
        ...
        if do_it:
            ...
            if incl_ginhouse:
                ...
                if incl_gname:
                    rbuff.gastnrmember = res_line.gastnrmember

                if resline.resstatus != 11 and resline.resstatus != 13:
    ...
    for rmember in db_session.query(Rmember).filter(
        (Rmember.resnr == curr_resline.resnr) &
        (Rmember.active_flag == 0) &
        (Rmember.reslinnr != curr_resline.reslinnr) & 
        (Rmember.l_zuordnung[inc_value(2)] == 0)).all():
        
        if allflag:
            do_it = True
        else:
            member_list = query(member_list_data, filters=(
                lambda member_list: member_list.reslinnr == rmember.reslinnr), first=True)
            do_it = None != member_list

        if do_it:
            do_it, init_time, init_date = get_output(check_timebl(
                1, rmember.resnr, rmember.reslinnr, "res-line", None, None))

            if not do_it:

                msg_str = msg_str + "#" + to_string(rmember.reslinnr, "99") + " " + \
                    rmember.name + chr_unicode(10) + \
                    translateExtended("Reservation being modified by other user.",
                                      lvcarea, "") + chr_unicode(10) + chr_unicode(10)
        ...
        if do_it:
            ...
            if (curr_resline.ankunft != res_line.ankunft) or (curr_resline.abreise != res_line.abreise) or (curr_resline.zikatnr != res_line.zikatnr):
                ...
                if not do_it:
                    msg_str = msg_str + "#" + to_string(rmember.reslinnr) + " " + \
                        rmember.name + chr_unicode(10) + \
                        translateExtended("Wrong check-in/check-out date", lvcarea, "") + ": " + to_string(ankunft1) + "/" + to_string(abreise1) + \
                        chr_unicode(10) + chr_unicode(10)

                if do_it and rmember.zinr != "":

                    outorder = get_cache(Outorder, {"zinr": [(eq, rmember.zinr)], "gespstart": [
                                         (ge, abreise1)], "gespende": [(lt, ankunft1)]})
                    do_it = not None != outorder

                    if not do_it:
                        msg_str = msg_str + "#" + to_string(rmember.reslinnr) + " " + \
                            rmember.name + chr_unicode(10) + translateExtended("O-O-O found RmNo", lvcarea, "") + \
                            " " + rmember.zinr + \
                            ": " + to_string(outorder.gespstart) + \
                            "/" + to_string(outorder.gespende) + \
                            chr_unicode(10) + chr_unicode(10)
                    else:
                        ...
                        if not do_it:
                            msg_str = msg_str + "#" + to_string(rmember.reslinnr) + " " + \
                                rmember.name + chr_unicode(10) + translateExtended("Overlapping with other reservation - RmNo", lvcarea, "") + \
                                " " + rmember.zinr + \
                                ": " + to_string(rline.ankunft) + \
                                "/" + to_string(rline.abreise) + \
                                chr_unicode(10) + \
                                chr_unicode(10)
                ...
                if do_it:
                    ...
                    if curr_resline.zikatnr != res_line.zikatnr and curr_resline.zikatnr == rmember.zikatnr:
                        ...
                        zimkateg = get_cache(
                            Zimkateg, {"zikatnr": [(eq, rmember.zikatnr)]})
                    zikatstr = zimkateg.kurzbez
                    overbook, overmax, overanz, overdate, incl_allot, curr_msg, rmcat_ovb = get_output(res_overbookbl(
                        pvilanguage, "modify", rmember.resnr, rmember.reslinnr, res_line.ankunft, res_line.abreise, 1, zikatstr, None, True))
                    do_it = not overbook
            ...
            # Rulita, 17/12/2025
            # Fixing looping replace old rate to new rate
            if do_it and res_line.zipreis != rmember.zipreis and rmember.resstatus != 11 and rmember.resstatus != 13:
            ...
            for zwunsch_rmember in query(zwunsch_rmember_data):
                ...
                zwunsch_rline = query(zwunsch_rline_data, filters=(
                    lambda zwunsch_rline: zwunsch_rline.s_label == zwunsch_rmember.s_label), first=True)
            ...
            res_changes()
            do_it, init_time, init_date = get_output(check_timebl(
                2, rmember.resnr, rmember.reslinnr, "res-line", init_time, init_date))
