...
t_data_data, T_data = create_model("T_data", {"gastnr":int, "infostr":string, "orig_infostr":string})

def guestbook_opr_listbl(instr:string, base64file:string, t_data_data:[T_data]):
    outstr = ""
    mess_result = ""
    tcase:int = 0
    tgastnr:int = 0
    ...
    def generate_output():
        ...
        return {"base64file": base64file, "t-data": t_data_data, "outstr": outstr, "mess_result": mess_result}
    ...
    if tcase == 1:
        ...
        t_data = query(t_data_data, first=True)
        ...
        # guestbook = get_cache (Guestbook, {"gastnr": [(eq, tgastnr)]})
        guestbook = db_session.query(Guestbook).filter(Guestbook.gastnr == tgastnr).first()
