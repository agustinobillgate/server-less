...
def pj_depart1_4_webbl(pvilanguage:int, case_type:int, disptype:int, fdate:date, tdate:date, froom:string, troom:string, disp_accompany:bool, ota_only:bool, setup_list_data:[Setup_list], zikat_list_data:[Zikat_list]):
    ...
    def create_departure0():
        ...
        if disptype == 1:
            ...
            res_line_obj_list = {}
            for res_line, reservation, guest, gmember in db_session.query(Res_line, Reservation, Guest, Gmember).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Guest,(Guest.gastnr == Reservation.gastnr)).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).filter(
                     ((Res_line.resstatus <= 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 6) | (Res_line.resstatus == 13) | (Res_line.resstatus == 8) | (Res_line.resstatus == 11)) & (Res_line.abreise >= fdate) & (Res_line.abreise <= tdate) & (((Res_line.zinr >= (froom).lower()) & (Res_line.zinr <= (troom).lower())) | ((Res_line.zinr >= (froom).lower() )))).order_by(Res_line.resnr, Res_line.reslinnr).all():
                zikat_list = query(zikat_list_data, (lambda zikat_list: zikat_list.zikatnr == res_line.zikatnr and zikat_list.selected), first=True)
                if not zikat_list:
                    continue
