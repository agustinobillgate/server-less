...
from models import Res_line, Htparam, Waehrung, Kontline, Zimmer, Guest, Zimkateg, Segment, Genstat, History, Exrate, Arrangement, Artikel, Fixleist, Reservation, Bill_line, Queasy, Reslin_queasy, Guestseg, Zinrstat, Outorder, Zkstat, Umsatz
...
def cr_occfcast1_2bl(segm_list_data:[Segm_list], argt_list_data:[Argt_list], zikat_list_data:[Zikat_list], outlook_list_data:[Outlook_list], pvilanguage:int, op_type:int, flag_i:int, curr_date:date, to_date:date, all_segm:bool, all_argt:bool, all_zikat:bool, exclooo:bool, incl_tent:bool, show_rev:int, vhp_limited:bool, excl_compl:bool, all_outlook:bool, incl_oth:bool):

    prepare_cache ([Res_line, Htparam, Waehrung, Kontline, Zimmer, Genstat, Exrate, Arrangement, Artikel, Fixleist, Reservation, Queasy, Reslin_queasy, Zinrstat, Zkstat, Umsatz])
    ...
    curr_time:int = 0
    res_line = htparam = waehrung = kontline = zimmer = guest = zimkateg = segment = genstat = history = exrate = arrangement = artikel = fixleist = reservation = bill_line = queasy = reslin_queasy = guestseg = zinrstat = outorder = zkstat = umsatz = None
    ...
    dayuse_list_data, Dayuse_list = create_model("Dayuse_list", {"datum":date, "zimmeranz":int, "pax":int})
    ...
    def generate_output():
        nonlocal room_list_data, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, datum, tot_room, tot_room1, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, curr_time, res_line, htparam, waehrung, kontline, zimmer, guest, zimkateg, segment, genstat, history, exrate, arrangement, artikel, fixleist, reservation, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        ...
        nonlocal room_list_data, print_list_data, print_list2_data, print_list3_data, active_rm_list_data, dayuse_list_data
    ...
    def create_browse():
        ...
        nonlocal room_list_data, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, tot_room, tot_room1, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, curr_time, res_line, htparam, waehrung, kontline, zimmer, guest, zimkateg, segment, genstat, history, exrate, arrangement, artikel, fixleist, reservation, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
        ...
        for genstat in db_session.query(Genstat).filter(
            ...
            if do_it and not all_outlook:
                ...
                if zimmer:
                    ...
                    o_list = query(o_list_data, filters=(lambda o_list: o_list.selected  and o_list.outlook_nr == zimmer.typ), first=True)
                    do_it = None != o_list
            ...
            if do_it:
                ...
                if show_rev == 1:
                    room_list.lodg[3] = room_list.lodg[3] + round(genstat.logis, price_decimal)
                    t_lodg[3] = t_lodg[3] + round(genstat.logis, price_decimal)
                    room_list.lodg[4] = room_list.lodg[4] + round(genstat.logis, price_decimal)
                    t_lodg[4] = t_lodg[4] + round(genstat.logis, price_decimal)
                    ...
                    t_lodg[5] = t_lodg[5] + (round(genstat.logis, price_decimal) / exchg_rate)
                    room_list.lodg[6] = room_list.lodg[4] / exchg_rate
                    t_lodg[6] = t_lodg[6] + (round(genstat.logis, price_decimal) / exchg_rate)
                    room_list.rmrate =  to_decimal(room_list.rmrate) + to_decimal(genstat.zipreis)
                ...
                room_list.other[3] = room_list.other[3] + round(((genstat.ratelocal / (1 + vat1 + vat2 + serv1)) - (genstat.logis + genstat.res_deci[1] + genstat.res_deci[2] + genstat.res_deci[3])) , price_decimal)

                if genstat.res_date[0] == genstat.res_date[1] and not rmsharer and genstat.res_logic[1]:

                    dayuse_list = query(dayuse_list_data, filters=(lambda dayuse_list: dayuse_list.datum == genstat.datum), first=True)

                    if not dayuse_list:
                        dayuse_list = Dayuse_list()
                        dayuse_list_data.append(dayuse_list)
                        ...
                        if datum == res_line.abreise:
                            ...
                            if (show_rev == 1 or show_rev == 2) and res_line.zipreis > 0:
                                ...
                                if incl_tent == False:
                                    ...
                                    fnet_lodg, net_lodg, tot_breakfast, tot_lunch, tot_dinner, tot_other, tot_rmrev, tot_vat, tot_service = get_output(get_room_breakdown(res_line._recid, datum, curr_i, curr_date))
                            net_lodg = to_decimal(round(net_lodg , price_decimal))
                            tot_breakfast = to_decimal(round(tot_breakfast , price_decimal))
                            tot_lunch = to_decimal(round(tot_lunch , price_decimal))
                            tot_dinner = to_decimal(round(tot_dinner , price_decimal))
                            tot_other = to_decimal(round(tot_other , price_decimal))
        ...
        for datum in date_range(d2,to_date) :
            ...
            for kontline in db_session.query(Kontline).filter(
                ...
                if do_it and not all_argt:
                    ...
                    a_list = query(a_list_data, filters=(lambda a_list: a_list.argt == kontline.arrangement and a_list.selected), first=True)
                    do_it = None != a_list
        ...
        for i in range(1,(to_date - curr_date + 1)  + 1) :
            ...
            for queasy.number3, queasy.char2, queasy.number1, queasy.char3, queasy._recid, zimmer.zikatnr, zimmer.typ, zimmer._recid in db_session.query(Queasy.number3, Queasy.char2, Queasy.number1, Queasy.char3, Queasy._recid, Zimmer.zikatnr, Zimmer.typ, Zimmer._recid).join(Zimmer,(Zimmer.zinr == Queasy.char1) & (Zimmer.sleeping)).filter(
                ...
                if do_it and not all_argt:
                    ...
                    a_list = query(a_list_data, filters=(lambda a_list: a_list.argt == queasy.char2 and a_list.selected), first=True)
                    do_it = None != a_list
        ...
        for room_list in query(room_list_data, filters=(lambda room_list: room_list.wd != 0)):
            ...
            if exclooo:
                tot_room = get_active_room(room_list.datum)
                tot_room1 = tot_room - room_list.room[14]
                accum_tot_room = accum_tot_room + tot_room1

                if tot_room1 != 0:
                    room_list.room[8] = room_list.room[6] / tot_room1 * 100
                    prev_room = prev_room + room_list.room[6]
                    room_list.room[9] = prev_room / accum_tot_room * 100

                if (room_list.room[6] - anzahl_dayuse) < tot_room1:
                    room_list.room[10] = tot_room1 - room_list.room[6] + anzahl_dayuse
                    rm_array[10] = rm_array[10] + tot_room1 - room_list.room[6] + anzahl_dayuse
                else:
                    room_list.room[11] = room_list.room[6] - tot_room1 - anzahl_dayuse
                    rm_array[11] = rm_array[11] + room_list.room[6] - tot_room1 - anzahl_dayuse
        ...
        for room_list in query(room_list_data, filters=(lambda room_list: room_list.wd != 0)):
            ...
            for i in range(16,17 + 1) :
                room_list.coom[i - 1] = to_string(room_list.room[i - 1], ">>>>>>9")
        jml_date = (to_date - curr_date + 1).days
        do_it = True
        ...
        if exclooo:
            ...
            if mtd_tot_room != 0:
                mtd_occ =  to_decimal(rm_array[6]) / to_decimal((mtd_tot_room - rm_array[14])) * to_decimal("100")
        else:
    ...
    def segm_code_name():
        ...
        counter:int = 0
        Bsegm = Segm_list
        bsegm_data = segm_list_data
        segm_name = ""
        print_list_data.clear()
        ...
        if all_segm:
            ...
            for bsegm in query(bsegm_data, filters=(lambda bsegm: bsegm.selected)):
        ...
        if a > 80:
            ...
            if substring(code_name, 0, 1) == " ":
                print_list.code_name = substring(code_name, 1, (length(code_name) - 1))
    ...
    def argt_code_name():
        ...
        if all_argt:
            ...
            for bargt in query(bargt_data, filters=(lambda bargt: bargt.selected)):
        ...
        if a > 80:
            ...
            print_list2_data.append(print_list2)
    ...
    def room_code_name():
        ...
        nonlocal room_list_data, lvcarea, tot_rmrev, bonus_array, week_list, tent_pers, datum, tot_room, tot_room1, mtd_tot_room, accum_tot_room, actual_tot_room, segm_name, argm_name, room_name, ci_date, pax, t_lodg, jml_date, tot_avrg, t_rmrate, t_rmrate2, t_revpar, t_revpar2, price_decimal, new_contrate, rm_vat, rm_serv, rm_array, exchg_rate, sum_comp, post_it, fcost, res_line, htparam, waehrung, kontline, zimmer, guest, zimkateg, segment, genstat, history, exrate, arrangement, artikel, fixleist, reservation, bill_line, queasy, reslin_queasy, guestseg, zinrstat, outorder, zkstat, umsatz
        nonlocal pvilanguage, op_type, flag_i, curr_date, to_date, all_segm, all_argt, all_zikat, exclooo, incl_tent, show_rev, vhp_limited, excl_compl, all_outlook, incl_oth
        nonlocal rline1
        ...
        curr_time:int = 0
        Broom = Zikat_list
        broom_data = zikat_list_data
        room_name = ""
        print_list3_data.clear()
        ...
        if all_zikat:
            ...
            for broom in query(broom_data, filters=(lambda broom: broom.selected)):
        ...
        if a > 80:
            ...
            print_list3_data.append(print_list3)
    ...
    def create_browse1():
        ...
        res_line = Res_line()
        waehrung = Waehrung()
        for res_line.resnr, res_line.ankunft, res_line.abreise, res_line.arrangement, res_line.reslinnr, res_line.zikatnr, res_line.zinr, res_line.gastnr, res_line.gratis, res_line.erwachs, res_line.kind1, res_line.zipreis, res_line._recid, res_line.resstatus, res_line.zimmerfix, res_line.zimmeranz, res_line.kontignr, res_line.active_flag, waehrung.waehrungsnr, waehrung.ankauf, waehrung.einheit, waehrung._recid in db_session.query(Res_line.resnr, Res_line.ankunft, Res_line.abreise, Res_line.arrangement, Res_line.reslinnr, Res_line.zikatnr, Res_line.zinr, Res_line.gastnr, Res_line.gratis, Res_line.erwachs, Res_line.kind1, Res_line.zipreis, Res_line._recid, Res_line.resstatus, Res_line.zimmerfix, Res_line.zimmeranz, Res_line.kontignr, Res_line.active_flag, Waehrung.waehrungsnr, Waehrung.ankauf, Waehrung.einheit, Waehrung._recid).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).filter(
            ...
            if do_it:
                ...
                if dayuse_flag:
                    ...
                    dayuse_list = query(dayuse_list_data, filters=(lambda dayuse_list: dayuse_list.datum == ci_date), first=True)
        ...
        if exclooo:
            mtd_occ =  to_decimal(rm_array[6]) / to_decimal((accum_tot_room)) * to_decimal("100")
        else:
    ...
    def create_active_room_list():
        ...
        if all_zikat:
            ...
            for zkstat in db_session.query(Zkstat).filter(
                ...
                z_list = query(z_list_data, filters=(lambda z_list: z_list.zikatnr == zkstat.zikatnr and z_list.selected), first=True)
