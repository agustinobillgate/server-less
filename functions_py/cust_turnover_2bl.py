...
def cust_turnover_2bl(cardtype:int, sort_type:int, curr_sort1:int, fdate:date, tdate:date, check_ftd:bool, currency:string, excl_other:bool, curr_sort2:int):
    ...
    def create_forecast():
        ...
        def get_day(d): return d.day
        def get_month(d): return d.month
        def get_year(d): return d.year
        def date_mdy(month, day, year): return date(year, month, day)

        # Calculate last year's from-date
        if get_day(fdate) == 29 and get_month(fdate) == 2:
            ly_fdate = date_mdy(get_month(fdate), 28, get_year(fdate) - 1)
        else:
            ly_fdate = date_mdy(get_month(fdate), get_day(fdate), get_year(fdate) - 1)

        # Calculate last year's to-date
        if get_day(tdate) == 29 and get_month(tdate) == 2:
