...
op_list_data, Op_list = create_model_like(L_op, {"rec_id":int})
...
def po_stockin_btn_stop_webbl(user_init:string, l_orderhdr_recid:int, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, lief_nr:int, billdate:date, docu_nr:string, lscheinnr:string, crterm:int, curr_lager:int, t_amount:Decimal, op_list_data:[Op_list], t_l_order_data:[T_l_order]):
    ...
    def create_l_op():
        ...
        if l_art.ek_aktuell != op_list.einzelpreis:
            ...
            l_art.ek_aktuell =  to_decimal(op_list.einzelpreis)
        ...
        if ((l_art.endkum == f_endkum or l_art.endkum == b_endkum) and billdate <= fb_closedate) or (l_art.endkum >= m_endkum and billdate <= m_closedate):
            ...
            l_bestand = db_session.query(L_bestand).filter(
                     (L_bestand.lager_nr == op_list.lager_nr) & (L_bestand.artnr == l_art.artnr)).with_for_update().first()
        ...
        l_liefumsatz = db_session.query(L_liefumsatz).filter(
                 (L_liefumsatz.lief_nr == op_list.lief_nr) & (L_liefumsatz.datum == billdate)).with_for_update().first()
