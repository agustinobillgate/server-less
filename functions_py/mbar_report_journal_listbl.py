...
#------------------------------------------
# Rd, 16/9/2025
# remove .lower(), kurang nama table
# adjust string output disesuaikan dgn output e1
# decimal digit disesuaikan.
#------------------------------------------
...
def mbar_report_journal_listbl(from_date:date, to_date:date, curr_dept:int, long_digit:bool):
    ...
    def journal_list():
            ...
            for h_journal in db_session.query(H_journal).filter(
                ...
                if h_journal.artnr == 0 and rm_flag:
                    ...
                    if deci_place == 0:
                        ...
                        tmp_betrag = to_string(-1 * h_journal.betrag, "->,>>>,>>>,>>9")
                        
                        h_journal_fremdwaehrng = to_string(-1 * h_journal.fremdwaehrng, "->,>>>,>>>,>>9")

                        h_journal_kellner_nr = to_string(h_journal.kellner_nr, "999")
                        h_journal_tischnr = to_string(h_journal.tischnr, ">>>>>9")
                        output_list.str = to_string(h_journal.bill_datum) + to_string(rmno, "x(6)") + \
                                            format_fixed_length(to_string(gname, "x(30)"), 30) + \
                                            format_fixed_length(to_string(h_journal_rechnr, "x(9)"), 9) + \
                                            format_fixed_length(to_string(h_journal.bezeich, "x(24)"), 24) + \
                                            format_fixed_length(tmp_betrag, 14) + \
                                            format_fixed_length(h_journal_fremdwaehrng, 14) + \
                                            to_string(h_journal.zeit, "HH:MM") + \
                                            to_string(h_journal_kellner_nr, "x(3)") + \
                                            to_string(h_journal_tischnr, "x(6)")
                    else:
                        h_journal_rechnr = to_string(h_journal.rechnr, ">,>>>,>>9")
                        tmp_betrag = to_string(-1 * h_journal.betrag, "->>,>>>,>>9.99")
                        tmp_betrag = format_fixed_length(tmp_betrag, 14)
                        h_journal_fremdwaehrng = to_string(-1 * h_journal.fremdwaehrng, "->>,>>>,>>9.99")
                        h_journal_fremdwaehrng = format_fixed_length(h_journal_fremdwaehrng, 14)
                        h_journal_kellner_nr = to_string(h_journal.kellner_nr, "999")
                        h_journal_tischnr = to_string(h_journal.tischnr, ">>>>>9")
        ...
        if deci_place == 0:
            ...
            tmp_tot_foreign = to_string(tot_foreign, "->,>>>,>>>,>>9")
            output_list.str = to_string("", "x(53)") + to_string("T O T A L   ", "x(24)") + to_string(tmp_tot, "x(14)") + to_string(tmp_tot_foreign, "x(14)")
        else:
