...
# ============================================================
# Warning !!! 
# do not re-compile this program !!!
# ============================================================
# Rulita, 05-11-2025 | 744E3B
# Issue:
# - Fixing error indent
# - Fixing rev-list.ct = ct to rev-list.ct = ct1
# - Fix missing table umsat to umsatz
# - Fix add generate file txt to function

# Rulita, 05-11-2025 | 744E3B
# - Fix run program write-sheet.php replace to write_sheet.py
# - Fix delete file .txt 
# ============================================================
...
from functions.write_sheet import write_sheet   # Rulita, new program for replace program write-sheet.php
...
def gl_parxls3_listbl(briefnr:int, from_date:date, to_date:date, user_init:string, gl_month:int, link:string):
    ...
    def generate_output():
        ...
        nonlocal t_parameters_data, t_gl_accthis_data, glacct_list_data, t_gl_acct_data, t_artikel_data, t_umsz_data, g_list_data, temp_list_data, coa_list_data, t_list_data, stat_list_data, rev_list_data
    ...
    # Rulita, Fix put Stream progress chages to function in py
    ...
    def fill_segment():
        ...
        nonlocal mess_result, output_sheet, month_str, chcol, curr_date, curr_i, curr_row, curr_col, serv, vat, fact, n_betrag, credit, debit, frate, price_decimal, gl_year, from_year, to_year, foreign_flag, prev_str, cell_val, exrate_betrag, hist_flag, curr_close_year, found_flag, ytd_bal, val_sign, j, i, diff, lmdiff, mtd_betrag, ytd_betrag, start_row, start_col, end_row, end_col, htl_no, datum1, jan1, ljan1, lfrom_date, lto_date, cash_flow, foreign_curr, ct1, mon_saldo, vat2, d_flag, n_serv, n_tax, actual_exrate, budget_exrate, lyear_exrate, blyear_exrate, bnyear_exrate, parameters, gl_accthis, gl_acct, artikel, exrate, waehrung, htparam, paramtext, gl_jouhdr, gl_journal, umsatz, genstat, segmentstat, budget, zinrstat, zkstat, segment
        nonlocal briefnr, from_date, to_date, user_init, gl_month, link
        nonlocal b_stat_list, b_stat, buff_exrate, b_param
        ...
        for s_param in db_session.query(S_param).filter(
            ...
            if s_param.vtype == 83:
                # Rulita, Fixing for put stream jadi manggil fungsi generate file
                if tday_rev != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(tday-rev) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(tday_rev)
                    generate_files(file_message)

                elif tday_pers != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(tday-pers) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(tday_pers)
                    generate_files(file_message)

                elif tday_room != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(tday-room) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(tday_room)
                    generate_files(file_message)
                else:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                    generate_files(file_message)

            elif s_param.vtype == 23:
                # Rulita, Fixing for put stream jadi manggil fungsi generate file
                if mtd_rev != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(mtd-rev) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(mtd_rev)
                    generate_files(file_message)

                elif mtd_pers != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(mtd-pers) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(mtd_pers)
                    generate_files(file_message)

                elif mtd_room != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(mtd-room) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(mtd_room)
                    generate_files(file_message)
                else:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                ...
                # Rulita, Fixing for put stream jadi manggil fungsi generate file
                if ytd_rev != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(ytd-rev) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(ytd_rev)
                    generate_files(file_message)

                elif ytd_pers != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(ytd-pers) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(ytd_pers)
                    generate_files(file_message)

                elif ytd_room != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(ytd-room) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(ytd_room)
                    generate_files(file_message)
                else:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
            ...
            if lmtd_flag or lytd_flag:
                ...
                if s_param.vtype == 86:
                    # Rulita, Fixing for put stream jadi manggil fungsi generate file
                    if lmtd_rev != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lmtd-rev) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lmtd_rev)
                        generate_files(file_message)

                    elif lmtd_pers != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lmtd-pers) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lmtd_pers)
                        generate_files(file_message)

                    elif lmtd_room != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lmtd-room) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lmtd_room)
                        generate_files(file_message)
                    else:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                    ...
                    # Rulita, Fixing for put stream jadi manggil fungsi generate file
                    if lytd_rev != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lytd-rev) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lytd_rev)
                        generate_files(file_message)

                    elif lytd_pers != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lytd-pers) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lytd_pers)
                        generate_files(file_message)

                    elif lytd_room != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lytd-room) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lytd_room)
                        generate_files(file_message)
                    else:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
            ...
            if mtd_budget_flag or ytd_budget_flag:
                ...
                if s_param.vtype == 84:
                    # Rulita, Fixing for put stream jadi manggil fungsi generate file
                    if bmtd_rev != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(bmtd-rev) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(bmtd_rev)
                        generate_files(file_message)

                    elif bmtd_pers != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(bmtd-pers) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(bmtd_pers)
                        generate_files(file_message)

                    elif bmtd_room != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(bmtd-room) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(bmtd_room)
                        generate_files(file_message)
                    else:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                    ...
                    # Rulita, Fixing for put stream jadi manggil fungsi generate file
                    if bytd_rev != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(bytd-rev) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(bytd_rev)
                        generate_files(file_message)

                    elif bytd_pers != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(bytd-pers) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(bytd_pers)
                        generate_files(file_message)

                    elif bytd_room != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(bytd-room) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(bytd_room)
                        generate_files(file_message)
                    else:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
        ...
        for s_param in db_session.query(S_param).filter(
            ...
            if artikel:
                ...
                if s_param.vtype == 23 and do_it == False:
                    # Rulita, Fixing for put stream jadi manggil fungsi generate file
                    if saldo_betrag != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(saldo-betrag) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(saldo_betrag)
                        generate_files(file_message)
                    else:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                    ...
                    # Rulita, Fixing for put stream jadi manggil fungsi generate file
                    if ytd_betrag != 0:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(ytd-betrag) SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(ytd_betrag)
                        generate_files(file_message)
                    else:
                        # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                        file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                ...
                if lmtd_flag or lytd_flag:
                    ...
                    if s_param.vtype == 86 and do_it == False:
                        # Rulita, Fixing for put stream jadi manggil fungsi generate file
                        if lastyr_betrag != 0:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lastyr-betrag) SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lastyr_betrag)
                            generate_files(file_message)
                        else:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP. 
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                        ...
                        # Rulita, Fixing for put stream jadi manggil fungsi generate file
                        if lytd_saldo != 0:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(lytd-saldo) SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(lytd_saldo)
                            generate_files(file_message)
                        else:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                ...
                if mtd_budget_flag or ytd_budget_flag:
                    ...
                    if s_param.vtype == 84 and do_it == False:
                        # Rulita, Fixing for put stream jadi manggil fungsi generate file
                        if mtd_budget != 0:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(mtd-budget) SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(mtd_budget)
                            generate_files(file_message)
                        else:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
                        ...
                        # Rulita, Fixing for put stream jadi manggil fungsi generate file
                        if ytd_budget != 0:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(ytd-budget) SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(ytd_budget)
                            generate_files(file_message)
                        else:
                            # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP.
                            file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
        ...
        for b_param in db_session.query(B_param).filter(
            ...
            if stat_list:
                ...
                if b_param.vtype == 83 and stat_list.t_day != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(stat-list.t-day) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(stat_list.t_day)
                    generate_files(file_message)

                elif b_param.vtype == 23 and stat_list.mtd != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(stat-list.mtd) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(stat_list.mtd)
                    generate_files(file_message)

                elif b_param.vtype == 24 and stat_list.ytd != 0:
                    # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" STRING(stat-list.ytd) SKIP.
                    file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(stat_list.ytd)
                    generate_files(file_message)
    ...
    def create_rev_list():
        ...
        nonlocal mess_result, output_sheet, month_str, chcol, curr_date, curr_i, curr_row, curr_col, serv, vat, fact, n_betrag, credit, debit, frate, price_decimal, gl_year, from_year, to_year, foreign_flag, prev_str, cell_val, exrate_betrag, hist_flag, curr_close_year, found_flag, ytd_bal, val_sign, j, i, diff, lmdiff, mtd_betrag, ytd_betrag, start_row, start_col, end_row, end_col, htl_no, datum1, jan1, ljan1, lfrom_date, lto_date, cash_flow, foreign_curr, mon_saldo, vat2, ytd_flag, d_flag, n_serv, n_tax, actual_exrate, budget_exrate, lyear_exrate, blyear_exrate, bnyear_exrate, parameters, gl_accthis, gl_acct, artikel, exrate, waehrung, htparam, paramtext, gl_jouhdr, gl_journal, umsatz, genstat, segmentstat, budget, zinrstat, zkstat, segment
        nonlocal briefnr, from_date, to_date, user_init, gl_month, link
        nonlocal b_stat_list, b_stat, buff_exrate, b_param
    ...
    def fill_tot_room():
        ...
        stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.flag.lower()  == ("stat-tot-rm").lower()), first=True)
    ...
    def fill_tot_avail():
        ...
        stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.flag.lower()  == ("stat-act-rm").lower()), first=True)
    ...
    def fill_tot_rev():
        ...
        if not stat_list:
            ...
            rev_list = query(rev_list_data, filters=(lambda rev_list: rev_list.flag.lower()  == ("Room").lower()  and matches(rev_list.descr,r"*Room Revenue*")), first=True)
    ...
    def fill_avg_rmrate_rp():
        ...
        if not stat_list:
            ...
            b_stat = query(b_stat_data, filters=(lambda b_stat: b_stat.flag.lower()  == ("stat-tot-rev").lower()), first=True)
    ...
    def fill_gs_inhouse():
        ...
        stat_list = query(stat_list_data, filters=(lambda stat_list: stat_list.flag.lower()  == ("stat-nguest").lower()), first=True)
    ...
    def calc_cf():
        ...
        # Rulita, Fixing days - 1 kena convert timedelta 
        ...
        t_list_data = get_output(glacct_cashflow_1bl(from_date, to_date, from_lsyr, to_lsyr, pfrom_date, pto_date, from_month, from_year, coa_list_data))
    ...
    # Rulita, fix add file dir for py temporarily file additional_files
    file_dir = "/usr1/serverless/src/additional_files"
    os.makedirs(file_dir, exist_ok=True)
    file_path = os.path.join(file_dir, "output_" + htl_no + ".txt")
        ...
        if prev_str != parameters.varname:
            ...
            if substring(parameters.vstring, 0, 1) != ("$").lower() :
                ...
                if gl_accthis:
                    ...
                    t_gl_accthis = query(t_gl_accthis_data, filters=(lambda t_gl_accthis: t_gl_accthis.fibukonto == gl_accthis.fibukonto and t_gl_accthis.year == gl_accthis.year), first=True)
                ...
                if gl_acct:
                    ...
                    t_gl_acct = query(t_gl_acct_data, filters=(lambda t_gl_acct: t_gl_acct.fibukonto == gl_acct.fibukonto), first=True)
                ...
                if parameters.vtype == 25 or parameters.vtype == 26:
                    ...
                    for g_list in query(g_list_data, filters=(lambda g_list: g_list.fibu == parameters.vstring), sort_by=[("fibu",False),("datum",False)]):
                        pass
                ...
                if parameters.vtype == 23 and num_entries(parameters.varname, "-") == 2:
                    ...
                    if artikel:
                        ...
                        t_artikel = query(t_artikel_data, filters=(lambda t_artikel: t_artikel.artnr == artikel.artnr and t_artikel.departement == artikel.departement), first=True)
    ...
    glacct_list_data.append(glacct_list)
    ...
    t_parameters = query(t_parameters_data, filters=(lambda t_parameters: t_parameters.progname.lower()  == ("GL-Macro").lower()  and num_entries(t_parameters.varname, "-") == 3 and entry(2, t_parameters.varname, "-") == ("CF").lower()), first=True)

    if t_parameters:

        for t_parameters in query(t_parameters_data, filters=(lambda t_parameters: t_parameters.num_entries(t_parameters.varname, "-") == 3 and entry(2, t_parameters.varname, "-") == ("CF").lower()  and num_entries(t_parameters.vstring, ":") > 1)):
            ...
            if num_entries(t_parameters.vstring, ":") > 1:
                ...
                t_list = query(t_list_data, filters=(lambda t_list: t_list.fibukonto == entry(0, t_parameters.vstring, ":") and t_list.cf == to_int(entry(1, t_parameters.vstring, ":"))), first=True)
            ...
            # Rulita, Fixing for put stream jadi manggil fungsi generate file
            if cell_val != "":
                # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" cell-val SKIP."
                file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + to_string(cell_val)
                generate_files(file_message)
            else:
                # PUT STREAM s1 UNFORMATTED STRING(curr-row) ";" STRING(curr-col) ";" "''" SKIP."
                file_message = to_string(curr_row) + ";" + to_string(curr_col) + ";" + "''"
    ...
    for t_parameters in query(t_parameters_data, filters=(lambda t_parameters: t_parameters.num_entries(t_parameters.varname, "-") <= 2), sort_by=[("varname",False)]):
        curr_row = to_int(entry(0, t_parameters.varname, "-"))
        curr_col = to_int(entry(1, t_parameters.varname, "-"))
        end_row = curr_row
        end_col = curr_col
        ...
        if t_parameters.vtype == 0:
            ...
            if hist_flag:
                ...
                t_gl_accthis = query(t_gl_accthis_data, filters=(lambda t_gl_accthis: t_gl_accthis.fibukonto == t_parameters.vstring and t_gl_accthis.year == gl_year), first=True)
                ...
                t_gl_acct = query(t_gl_acct_data, filters=(lambda t_gl_acct: t_gl_acct.fibukonto == t_parameters.vstring), first=True)
        ...
        if t_parameters.vtype > 0:
            ...
            if found_flag and (glacct_list.acc_type == 1 or glacct_list.acc_type == 4):
                ...
                for curr_i in range(1,gl_month + 1) :
                    ...
                    if glacct_list.acc_type == 4:
                        ytd_bal =  - to_decimal(glacct_list.actual[curr_i - 1])
                    else:
            ...
            elif found_flag and glacct_list.acc_type == 3:
            ...
            if found_flag and (t_parameters.vtype == 25 or t_parameters.vtype == 26):
                ...
                temp_list = query(temp_list_data, filters=(lambda temp_list: temp_list.vstring == t_parameters.vstring), first=True)
            ...
            if t_parameters.vtype == 23:
                ...
                t_artikel = query(t_artikel_data, filters=(lambda t_artikel: t_artikel.artnr == to_int(substring(t_parameters.vstring, 2)) and t_artikel.departement == to_int(substring(t_parameters.vstring, 0, 2))), first=True)
                ...
                if t_artikel:
                    for curr_date in date_range(date_mdy(01, 01, get_year(to_date)),to_date) :
                        ...
                        t_umsz = query(t_umsz_data, filters=(lambda t_umsz: t_umsz.curr_date == curr_date and t_umsz.artnr == to_int(substring(t_parameters.vstring, 2)) and t_umsz.dept == to_int(substring(t_parameters.vstring, 0, 2))), first=True)
            ...
            # Rulita, merubah write_sheet.php menjadi program py
            mess_result = "Successfully generate data"

            output_sheet = get_output(write_sheet(file_path, link))
