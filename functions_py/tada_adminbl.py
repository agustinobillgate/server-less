...
def tada_adminbl(deptno:int, grup:int, paramnr:int, intval:int, decval:Decimal, dateval:date, logval:bool, charval:string):
    ...
    def update_queasy():
        ...
        # queasy = get_cache (Queasy, {"key": [(eq, 270)],"betriebsnr": [(eq, deptno)],"number1": [(eq, grup)],"number2": [(eq, paramnr)]})
        queasy = db_session.query(Queasy).filter(Queasy.key == 270, Queasy.betriebsnr == deptno, Queasy.number1 == grup, Queasy.number2 == paramnr).with_for_update().first()
