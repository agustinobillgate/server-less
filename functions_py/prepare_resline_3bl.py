...
# ------------------------------------------
# Rdm 25/11/2025,
# with_for_update added where needed

# yusufwijasena, 05/12/2025
# added validation on check_dynarate() to return when res_dynarate_data == []
# ------------------------------------------
...
def prepare_resline_3bl(pvilanguage: int, res_mode: string, session_date: string, user_init: string, inp_gastnr: int, inp_resnr: int, inp_reslinnr: int, rate_readonly: bool, qci_zinr: string, res_dynarate_data: [Res_dynarate]):
    ...
    rline_list = reslin_list = curr_resline = t_history = currency_list = res_dynarate = reschanged_list = f_resline = nation_list = resline = resbuff = zimkateg1 = rbuff = qci_zimmer = bresline = bguest = q359 = qsy = None

    rline_list_data, Rline_list = create_model_like(
        Res_line,
        {
            "res_char": string,
            "rsvname": string,
    ...
    reschanged_list_data, Reschanged_list = create_model(
        "Reschanged_list",
        {
            "reslinnr": int
        })
    f_resline_data, F_resline = create_model(
        "F_resline",
        {
            "guestname": string,
            "curr_segm": string,
            ...
            "l_night": int,
            "six_pm": int,
            "accompany_gastnr": int,
            "accompany_gastnr2": int,
            "accompany_gastnr3": int,
            "ci_date": date,
            "billdate": date,
            "bookdate": date,
            "l_ankunft": date,
    ...
    def generate_output():
        ...
        return {
            ...
            "init_time": init_time,
            "init_date": init_date,
            "avail_gdpr": avail_gdpr,
            "avail_mark": avail_mark,
            "avail_news": avail_news,
            "save_gdpr": save_gdpr,
            "curr_date": curr_date,
            "serv_date": serv_date,
            "f-resline": f_resline_data,
            "curr-resline": curr_resline_data,
            "reslin-list": reslin_list_data,
            "reschanged-list": reschanged_list_data,
            "t-history": t_history_data,
            "rline-list": rline_list_data
        }
    ...
    def split_resline():
        ...
        bline = get_cache(
            Res_line, {"resnr": [(eq, inp_resnr)], "reslinnr": [(eq, inp_reslinnr)]})

        if bline:
            its_wrong, com_rm, max_comp, pswd_str, msg_str = get_output(check_complimentbl(
                pvilanguage, bline.resnr, bline.reslinnr, bline.gastnr, bline.ankunft, f_resline.marknr, bline.zikatnr, bline.arrangement, bline.zimmeranz, bline.zipreis))
        ...
        for rline in db_session.query(Rline).filter(
            ...
            if priscilla_active:
                ...
                if res_line.zimmeranz > 1:
                    get_output(intevent_1(9, res_line.zinr,
                               "Priscilla", res_line.resnr, res_line.reslinnr))

            gentable = get_cache(Gentable, {"key": [(eq, "reservation")], "number1": [
                                 (eq, res_line.resnr)], "number2": [(eq, res_line.reslinnr)]})
            ...
            while i <= res_line.zimmeranz:
                ...
                buffer_copy(res_line, resmember, except_fields=[
                            "zimmeranz", "reslinnr", "resname"])
                ...
                if priscilla_active:
                    get_output(intevent_1(13, "", "Priscilla",
                               resmember.resnr, resmember.reslinnr))
                ...
                for reslin_queasy in db_session.query(Reslin_queasy).filter(
                    ...
                    buffer_copy(reslin_queasy, m_queasy,
                                except_fields=["reslinnr"])
                    pass
        ...
        res_line = get_cache(
            Res_line, {"resnr": [(eq, inp_resnr)], "reslinnr": [(eq, reihe)]})
    ...
    def check_bedsetup():
        ...
        if reslin_list.setup != 0:
            ...
            if zimkateg:
                ...
                zimmer = get_cache(Zimmer, {"zikatnr": [(eq, zimkateg.zikatnr)], "setup": [
                                   (eq, reslin_list.setup)]})
            ...
            if not zimmer:
                ...
                paramtext = get_cache(
                    Paramtext, {"txtnr": [(eq, (9200 + reslin_list.setup))]})
    ...
    def disp_allotment():
        ...
        if reslin_list.kontignr > 0:
            ...
            kontline = get_cache(
                Kontline, {"kontignr": [(eq, reslin_list.kontignr)], "kontstatus": [(eq, 1)]})
        else:

            kontline = get_cache(Kontline, {"kontignr": [(
                eq, - reslin_list.kontignr)], "betriebsnr": [(eq, 1)], "kontstatus": [(eq, 1)]})
    ...
    def get_currency():
        ...
        if not waehrung1:
            msg_str = translateExtended(
                "Local Currency Code incorrect! (Param 152 / Grp 7)", lvcarea, "") + chr_unicode(10)
        ...
        if (not waehrung1) and f_resline.foreign_rate:
            msg_str = translateExtended(
                "Foreign Currency Code incorrect! (Param 144 / Grp 7)", lvcarea, "") + chr_unicode(10)
        ...
        if gbuff.notizen[2] != "":
            ...
            waehrung1 = get_cache(
                Waehrung, {"wabkurz": [(eq, gbuff.notizen[2])]})
        ...
        if reslin_list.betriebsnr == 0 or f_resline.marknr != 0:
            ...
            if (res_mode.lower() == ("new").lower() or res_mode.lower() == ("insert").lower() or res_mode.lower() == ("qci").lower() or f_resline.marknr != 0):
                ...
                if f_resline.contcode != "":
                    ...
                    guest_pr = get_cache(
                        Guest_pr, {"code": [(eq, f_resline.contcode)]})

                if not guest_pr:

                    guest_pr = get_cache(
                        Guest_pr, {"gastnr": [(eq, inp_gastnr)]})
                ...
                if guest_pr:
                    ...
                    if f_resline.marknr != 0:
                        ...
                        queasy = get_cache(
                            Queasy, {"key": [(eq, 18)], "number1": [(eq, f_resline.marknr)]})
            ...
            if not found:
                ...
                if reslin_list.adrflag or not f_resline.foreign_rate:
                    ...
                    waehrung1 = get_cache(
                        Waehrung, {"waehrungsnr": [(eq, f_resline.local_nr)]})
                else:
        ...
        if waehrung1:
            ...
            if f_resline.currency != None:
                ...
                f_resline.currency = waehrung1.wabkurz
    ...
    def set_roomrate(direct_change: bool):
        ...
        if reslin_list.erwachs == 0 and reslin_list.kind1 == 0 and reslin_list.kind2 == 0:
            ...
            if guest_pr and not rate_found:
                ...
                queasy = get_cache(Queasy, {"key": [(eq, 18)], "number1": [
                                   (eq, reslin_list.reserve_int)]})
                ...
                if f_resline.new_contrate:
                    ...
                    reslin_list.zipreis, rate_found = get_output(pricecod_rate(reslin_list.resnr, reslin_list.reslinnr, ("!" + f_resline.contcode), datum, reslin_list.ankunft, reslin_list.abreise,
                                                                 reslin_list.reserve_int, arrangement.argtnr, curr_zikatnr, reslin_list.erwachs, reslin_list.kind1, reslin_list.kind2, reslin_list.reserve_dec, reslin_list.betriebsnr))
                ...
                if queasy and queasy.logi1:
                    ...
                    f_resline.rate_tooltip = translateExtended(
                        "Contract rate in LOCAL CURRENCY.", lvcarea, "")

            if not rate_found:
                tmpint = wd_array[get_weekday(datum) - 1]

                if res_mode.lower() == ("inhouse").lower():

                    katpreis = get_cache(Katpreis, {"zikatnr": [(eq, curr_zikatnr)], "argtnr": [(eq, arrangement.argtnr)], "startperiode": [
                                         (le, datum)], "endperiode": [(ge, datum)], "betriebsnr": [(eq, tmpint)]})

                    if not katpreis:

                        katpreis = get_cache(Katpreis, {"zikatnr": [(eq, curr_zikatnr)], "argtnr": [(
                ...
                if katpreis:
                    reslin_list.zipreis = to_decimal(get_rackrate(
                        reslin_list.erwachs, reslin_list.kind1, reslin_list.kind2))
                else:
                    reslin_list.zipreis = to_decimal("0")
    ...
    def check_dynarate():
        ...
        if not res_dynarate_data:  # yusufwijasena, add validation when res_dyanerate_data == []
            return
        ...
        paramtext = get_cache(
            Paramtext, {"txtnr": [(eq, (9200 + res_dynarate.setup))]})
        ...
        if queasy:
            ...
            if htparam:
                f_resline.currency = htparam.fchar
        ...
        queasy = get_cache(Queasy, {"key": [(eq, 2)], "char1": [
                           (eq, f_resline.origcontcode)]})
    ...
    def check_bedsetup1():
        ...
        if reslin_list.setup == 0 and zimkateg:
            ...
            if anz_setup == 1:
                ...
                paramtext = get_cache(
                    Paramtext, {"txtnr": [(eq, (9200 + curr_setup))]})
                f_resline.c_setup = substring(paramtext.notes, 0, 1)
    ...
    if avail_gdpr:
        ...
        nation = Nation()
        queasy = Queasy()
        for nation.nationnr, nation.kurzbez, nation.bezeich, nation._recid, queasy.char3, queasy._recid in db_session.query(Nation.nationnr, Nation.kurzbez, Nation.bezeich, Nation._recid, Queasy.char3, Queasy._recid).join(Queasy, (Queasy.key == 6) & (Queasy.number1 == Nation.untergruppe) & (matches(Queasy.char1, "*europe*"))).filter(
    ...
    flag_ok, init_time, init_date = get_output(check_timebl(
        1, inp_resnr, inp_reslinnr, "res-line", None, None))
    ...
    if res_mode.lower() == ("modify").lower() or res_mode.lower() == ("inhouse").lower():
        ...
        if not resbuff:
            msg_str = translateExtended(
                "Reservation is being modified by other user.", lvcarea, "")
            error_flag = True
        ...
        if res_mode.lower() == ("modify").lower() and resbuff.active_flag == 1:
            msg_str = translateExtended(
                "Guest already checked-in.", lvcarea, "")
            error_flag = True
    ...
    reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "rate-prog")], "char1": [(
        eq, "")], "reslinnr": [(eq, 1)], "number1": [(eq, inp_resnr)], "number2": [(eq, 0)]})
    ...
    if res_mode.lower() == ("new").lower() or res_mode.lower() == ("qci").lower():
        ...
        if res_line.kontignr > 0:
            ...
            kontline = get_cache(
                Kontline, {"kontignr": [(eq, res_line.kontignr)], "kontstatus": [(eq, 1)]})

            if kontline:
                f_resline.allot_tooltip = translateExtended(
                    "Allotment Code", lvcarea, "")
            ...
            if kontline:
                f_resline.allot_tooltip = translateExtended(
                    "Global Reservation Code", lvcarea, "")
        ...
        gentable = get_cache(Gentable, {"key": [(eq, "reservation")], "number1": [
                             (eq, inp_resnr)], "number2": [(eq, inp_reslinnr)]})
        f_resline.gentable = None != gentable

        resline = get_cache(Res_line, {"resnr": [(eq, res_line.resnr)], "active_flag": [
                            (le, 1)], "kontakt_nr": [(eq, res_line.reslinnr)], "l_zuordnung[2]": [(eq, 1)]})
        ...
        if res_line.code != "" and res_line.code.lower() != ("0").lower():
            ...
            queasy = get_cache(Queasy, {"key": [(eq, 9)], "number1": [
                               (eq, f_resline.bill_instruct)]})
        ...
        f_resline.sharer = (res_line.resstatus == 11) or (
    ...
    if f_resline.res_bemerk != None and f_resline.res_bemerk != "":
        f_resline.res_bemerk = f_resline.res_bemerk +\
            chr_unicode(2) + to_string(to_int(logic_p1109))

    else:
        f_resline.res_bemerk = to_string(to_int(logic_p1109))
    ...
    if f_resline.res_bemerk != None and f_resline.res_bemerk != "":
        for loopk in range(1, length(f_resline.res_bemerk) + 1):
            ...
            if asc(substring(f_resline.res_bemerk, loopk - 1, 1)) == 0 or asc(substring(f_resline.res_bemerk, loopk, 1)) > 255:
                ...
                resbemerk = resbemerk + \
                    substring(f_resline.res_bemerk, loopk - 1, 1)
        f_resline.res_bemerk = resbemerk

        if length(f_resline.res_bemerk) < 3:
            f_resline.res_bemerk = replace_str(
                f_resline.res_bemerk, chr_unicode(32), "")

        if length(f_resline.res_bemerk) == None:
            f_resline.res_bemerk = ""
    resbemerk = ""

    if f_resline.rline_bemerk != None and f_resline.rline_bemerk != "":
        for loopk in range(1, length(f_resline.rline_bemerk) + 1):
            ...
            if asc(substring(f_resline.rline_bemerk, loopk - 1, 1)) == 0 or asc(substring(f_resline.rline_bemerk, loopk, 1)) > 255:
                ...
                resbemerk = resbemerk + \
                    substring(f_resline.rline_bemerk, loopk - 1, 1)
        f_resline.rline_bemerk = resbemerk

        if length(f_resline.rline_bemerk) < 3:
            f_resline.rline_bemerk = replace_str(
                f_resline.rline_bemerk, chr_unicode(32), "")
    ...
    if res_mode.lower() == ("new").lower() or res_mode.lower() == ("insert").lower() or res_mode.lower() == ("qci").lower():
        ...
        if res_mode.lower() == ("insert").lower():
            ...
            resline = get_cache(Res_line, {"resnr": [(eq, inp_resnr)], "active_flag": [
                                (le, 1)], "resstatus": [(ne, 12)], "l_zuordnung[2]": [(eq, 0)]})
            ...
            if resline:
                ...
                if resline.active_flag == 0:
                    ...
                    res_line.anztage = (
                        res_line.abreise - res_line.ankunft).days
        ...
        arrangement = get_cache(
            Arrangement, {"arrangement": [(eq, htparam.fchar)]})
    ...
    if res_mode.lower() != ("split").lower():
        ...
        if qci_zimmer:
            ...
            zimkateg = get_cache(
                Zimkateg, {"zikatnr": [(eq, qci_zimmer.zikatnr)]})
        else:
        ...
        if reslin_list.bemerk != None and reslin_list.bemerk != "":
            reslin_list.bemerk = replace_str(
                reslin_list.bemerk, chr_unicode(10), "")

            reslin_list.bemerk = replace_str(
                reslin_list.bemerk, chr_unicode(13), "")
            ...
            reslin_list.bemerk = replace_str(
                reslin_list.bemerk, chr_unicode(10) + chr_unicode(13), "")
            resbemerk = " "
            ...
            for loopk in range(1, length(reslin_list.bemerk) + 1):
                ...
                if asc(substring(reslin_list.bemerk, loopk - 1, 1)) == 0 or asc(substring(reslin_list.bemerk, loopk, 1)) > 255:
                    ...
                    resbemerk = resbemerk + \
                        substring(reslin_list.bemerk, loopk - 1, 1)
            reslin_list.bemerk = resbemerk

            if length(reslin_list.bemerk) < 3:
                reslin_list.bemerk = replace_str(
                    reslin_list.bemerk, chr_unicode(32), "")
        ...
        f_resline.pickup_flag = matches(
            reslin_list.zimmer_wunsch, ("*pickup*"))
        f_resline.drop_flag = matches(
            reslin_list.zimmer_wunsch, ("*drop-passanger*"))
        f_resline.marknr = reslin_list.reserve_int

        if reslin_list.l_zuordnung[0] != 0:

            zimkateg1 = get_cache(
                Zimkateg, {"zikatnr": [(eq, reslin_list.l_zuordnung[0])]})
    ...
    if f_resline.new_contrate:
        f_resline.ebdisc_flag = matches(
            reslin_list.zimmer_wunsch, ("*ebdisc*"))
        f_resline.kbdisc_flag = matches(
            reslin_list.zimmer_wunsch, ("*kbdisc*"))
        f_resline.restricted = matches(
            reslin_list.zimmer_wunsch, ("*restricted*"))
    ...
    if guest_pr:
        ...
        if queasy:
            ...
            if f_resline.new_contrate:
                while None != guest_pr:
                    ...
                    if queasy:
                        ...
                        if do_it:
                            ...
                            ratecode = get_cache(
                                Ratecode, {"code": [(eq, guest_pr.code)], "char1[0]": [(ne, "")]})

                            if ratecode:
                                f_resline.enable_ebdisc = True

                            ratecode = get_cache(
                                Ratecode, {"code": [(eq, guest_pr.code)], "char1[1]": [(ne, "")]})
                ...
                pricecod = get_cache(Pricecod, {"code": [(eq, guest_pr.code)], "endperiode": [
                                     (ge, f_resline.ci_date)]})
    ...
    for i in range(1, num_entries(reslin_list.zimmer_wunsch, ";") - 1 + 1):
        ...
        if substring(str, 0, 7) == ("voucher"):
            ...
            f_resline.bookdate = date_mdy(to_int(substring(str, 9, 2)), to_int(
                substring(str, 11, 2)), to_int(substring(str, 5, 4)))
            ...
            for loopi in range(1, num_entries(str1, ",") + 1):
                ...
                queasy = get_cache(Queasy, {"key": [(eq, 160)], "number1": [
                                   (eq, to_int(entry(loopi - 1, str1, ",")))]})

                if queasy:
                    for loopj in range(1, num_entries(queasy.char1, ";") + 1):

                        if matches(entry(loopj - 1, queasy.char1, ";"), r"*en*"):
                            f_resline.wci_flag = entry(
                                1, entry(loopj - 1, queasy.char1, ";"), "=") + ", " + f_resline.wci_flag
    ...
    reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "specialrequest")], "resnr": [
                              (eq, reslin_list.resnr)], "reslinnr": [(eq, reslin_list.reslinnr)]})
                ...
                if do_it:
                    ...
                    if f_resline.contcode == "":
                        ...
                        if queasy.logi2:
                            ...
                            rbuff = get_cache(
                                Ratecode, {"code": [(eq, f_resline.contcode)]})
                            ...
                            if rcode != "":
                                ...
                                rbuff = get_cache(
                                    Ratecode, {"code": [(eq, rcode)]})

                                if rbuff:

                                    qsy = get_cache(
                                        Queasy, {"key": [(eq, 18)], "number1": [(eq, rbuff.marknr)]})
                        else:
                            ...
                            if prmarket:
                                ...
                                qsy = get_cache(
                                    Queasy, {"key": [(eq, 18)], "number1": [(eq, prmarket.nr)]})

                        if qsy:

                            currency_list = query(currency_list_data, filters=(
                                lambda currency_list: currency_list.wabkurz == qsy.char3), first=True)
    ...
    if (res_line.resstatus == 11 or res_line.resstatus == 13) and f_resline.contcode == "":
        ...
        bresline = get_cache(Res_line, {"resnr": [(eq, res_line.resnr)], "reslinnr": [
                             (ne, res_line.reslinnr)], "kontakt_nr": [(eq, res_line.kontakt_nr)]})
    ...
    if reslin_list.active_flag == 0 and reslin_list.zinr != "":
        ...
        outorder = get_cache(Outorder, {"zinr": [(eq, reslin_list.zinr)], "betriebsnr": [
                             (eq, reslin_list.resnr)]})
        f_resline.offmarket = None != outorder
    disp_history()

    # print(f"[LOG] res_dyanarate: {res_dynarate_data}")
    ...
    if not split_modify:
        rline_list_data, reschanged_list_data = get_output(mk_resline_query_q1bl(
            pvilanguage, True, True, inp_resnr, rline_list_data, reschanged_list_data))

    if f_resline.restricted:
        msg_str = msg_str + chr_unicode(2) + "&W" + translateExtended("This is Reservation with Restricted Discounted rate", lvcarea, "") + chr_unicode(10) + translateExtended(
            "Any reservation data changes such as C/i- or C/O-date", lvcarea, "") + chr_unicode(10) + translateExtended("might have impact to the given room rate.", lvcarea, "") + chr_unicode(10)

    if res_mode.lower() == ("split").lower():
        flag_ok, init_time, init_date = get_output(check_timebl(
            2, inp_resnr, inp_reslinnr, "res-line", init_time, init_date))

    fixleist = get_cache(
        Fixleist, {"resnr": [(eq, inp_resnr)], "reslinnr": [(eq, inp_reslinnr)]})
