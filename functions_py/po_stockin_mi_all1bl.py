...
def po_stockin_mi_all1bl(user_init:string, l_order_recid:int, l_orderhdr_recid:int, docu_nr:string, exchg_rate:Decimal, price_decimal:int, billdate:date, f_endkum:int, b_endkum:int, m_endkum:int, fb_closedate:date, m_closedate:date, curr_lager:int, lief_nr:int, lscheinnr:string, t_amount:Decimal):
    ...
    t_l_order_data, T_l_order = create_model_like(L_order, {"rec_id":int, "art_bezeich":string, "jahrgang":int, "alkoholgrad":Decimal})
    ...
    def create_l_op(wert:Decimal, disp_flag:bool):
        ...
        if queasy:
            s_list.ss_artnr[0] = s_list.ss_artnr[0]
            s_list.ss_artnr[1] = s_list.ss_artnr[1]
            s_list.ss_artnr[2] = s_list.ss_artnr[2]
            s_list.ss_in[0] = s_list.ss_in[0]
            s_list.ss_in[1] = s_list.ss_in[1]
            s_list.ss_in[2] = s_list.ss_in[2]
            s_list.ss_out[0] = s_list.ss_out[0]
            s_list.ss_out[1] = s_list.ss_out[1]
            s_list.ss_out[2] = s_list.ss_out[2]
    ...
    def create_purchase_book():
        ...
        if curr_anz >= max_anz:
            ...
            l_price1 = db_session.query(L_pprice).filter(
                 (L_pprice.artnr == l_op.artnr) & (L_pprice.counter == 1)).with_for_update().first()
