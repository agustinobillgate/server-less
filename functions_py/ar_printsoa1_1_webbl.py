...
from models import Guest, Bill, Debitor, Bill_line, H_bill_line, Res_line, Artikel, Htparam, Waehrung, Exrate, Queasy, Zimkateg, Reservation, H_bill, Hoteldpt, H_artikel

param_ar_data, Param_ar = create_model("Param_ar", {"param_nr":int, "param_name":string, "param_val":string, "param_type":string})

def ar_printsoa1_1_webbl(show_type:int, bof_month:date, eof_month:date, param_ar_data:[Param_ar], guestno:int, curr_day:date):

    prepare_cache ([Guest, Bill, Debitor, Bill_line, H_bill_line, Res_line, Artikel, Htparam, Waehrung, Exrate, Zimkateg, Reservation, H_bill, Hoteldpt, H_artikel])
    ...
    arrival:date = None
    departure:date = None
    debt_tmp:Decimal = to_decimal("0.0")
    ...
    val_param:int = 0
    var_param:string = ""
    soa_list_recid:int = 1

    if type(bof_month) == string:
        bof_month = date(1,1,1)

    if type(eof_month) == string:
        eof_month = date(1,1,1)

    bof_month:date =  bof_month if bof_month else date(1,1,1)
    eof_month:date =  eof_month if eof_month else date(1,1,1)
    curr_day:date =  curr_day if curr_day else date(1,1,1)

    guest = bill = debitor = bill_line = h_bill_line = res_line = artikel = htparam = waehrung = exrate = queasy = zimkateg = reservation = h_bill = hoteldpt = h_artikel = None

    soa_list = soabuff = param_ar = gast = bill1 = debt = debt1 = billine = hbilline = bresline = None

    soa_list_data, Soa_list = create_model("Soa_list", {"counter":int, "debref":int, "done_step":int, "artno":int, "vesrdep":Decimal, "to_sort":int, "outlet":bool, "datum":date, "ankunft":date, "abreise":date, "gastnr":int, "name":string, "inv_str":string, "rechnr":int, "refno":int, "voucherno":string, "voucherno1":string, "voucherno2":string, "saldo":Decimal, "fsaldo":Decimal, "printed":bool, "selected":bool, "printdate":date, "dptnr":int, "debt":Decimal, "credit":Decimal, "fdebt":Decimal, "fcredit":Decimal, "remarks":string, "arrecid":int, "newpayment":Decimal, "newfpayment":Decimal, "zinr":string, "erwachs":int, "child1":int, "child2":int, "roomrate":Decimal, "tot_amount":Decimal, "tot_balance":Decimal, "exrate":Decimal, "tot_exrate":Decimal, "gst_tot_non_taxable":Decimal, "gst_amount":Decimal, "gst_tot_sales":Decimal, "zimmeranz":int, "bill_datum":date, "resv_name":string, "rm_category":string, "gst_amount_str":string, "_recid":int}, {"printdate": get_current_date()})
    ...
    def generate_output():
        nonlocal zeit, zeit1, str, due_date, soa_list_data, rech_nr, bet_nr, msg_int, param1, param2, param3, param4, fnet_lodging, lnet_lodging, net_breakfast, net_lunch, net_dinner, net_others, tot_rmrev, nett_vat, nett_service, curr_gastnr, curr_refno, new_refno, cl_exist, saldo, s, i, arrival, departure, debt_tmp, crdt, fdebt, fcrdt, new_crdt, new_fcrdt, isprintedb4, saldo1, saldo2, saldo3, saldo4, other_flag, count_zimm, nmonth, mnth1, year2, val_param, var_param, guest, bill, debitor, bill_line, h_bill_line, res_line, artikel, htparam, waehrung, exrate, queasy, zimkateg, reservation, h_bill, hoteldpt, h_artikel, soa_list_recid
        nonlocal show_type, bof_month, eof_month, guestno, curr_day
    ...
    def create_soalist():
        ...
        nonlocal zeit, zeit1, str, due_date, soa_list_data, rech_nr, bet_nr, msg_int, param1, param2, param3, param4, fnet_lodging, lnet_lodging, net_breakfast, net_lunch, net_dinner, net_others, tot_rmrev, nett_vat, nett_service, curr_gastnr, curr_refno, new_refno, cl_exist, saldo, s, i, arrival, departure, debt_tmp, crdt, fdebt, fcrdt, new_crdt, new_fcrdt, isprintedb4, saldo1, saldo2, saldo3, saldo4, other_flag, count_zimm, guest, bill, debitor, bill_line, h_bill_line, res_line, artikel, htparam, waehrung, exrate, queasy, zimkateg, reservation, h_bill, hoteldpt, h_artikel, soa_list_recid
        nonlocal show_type, bof_month, eof_month, guestno, curr_day
        nonlocal soabuff, gast, bill1, debt, debt1, billine, hbilline, bresline
        ...
        if show_type == 0:
            ...
            for debitor.gastnr, debitor.rechnr, debitor.betriebsnr, debitor._recid, debitor.rgdatum, debitor.vesrcod, debitor.artnr, debitor.counter, debitor.debref, debitor.saldo, debitor.vesrdep, debitor.gastnrmember, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debitor.gastnr, Debitor.rechnr, Debitor.betriebsnr, Debitor._recid, Debitor.rgdatum, Debitor.vesrcod, Debitor.artnr, Debitor.counter, Debitor.debref, Debitor.saldo, Debitor.vesrdep, Debitor.gastnrmember, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel,(Artikel.artnr == Debitor.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter((Debitor.gastnr == guestno) & (Debitor.opart <= 1) & (Debitor.saldo != 0) & (Debitor.zahlkonto == 0)).order_by(Debitor._recid).all():
                ...
                if not soa_list:
                    ...
                    soa_list._recid = soa_list_recid
                    ...
                    if htparam.flogical:
                        ...
                        if waehrung:
                            soa_list.exrate =  to_decimal(waehrung.ankauf)
                            ...
                            if bill:
                                ...
                                if res_line:
                                    ...
                                    exrate = get_cache (Exrate, {"artnr": [(eq, waehrung.waehrungsnr)],"datum": [(eq, res_line.abreise)]})

                                    if exrate:
                                        soa_list.exrate =  to_decimal(exrate.betrag)
            ...
            for soa_list in query(soa_list_data, filters=(lambda soa_list: soa_list.done_step == 0), sort_by=[("_recid",False)]):

                crdt =  to_decimal("0")
                debt_tmp =  to_decimal("0")
                fcrdt =  to_decimal("0")
                fdebt =  to_decimal("0")
                new_crdt =  to_decimal("0")
                ...
                for soabuff in query(soabuff_data, filters=(lambda soabuff: soabuff.rechnr == soa_list.rechnr and soabuff.dptnr == soa_list.dptnr and soabuff.artno == soa_list.artno and soabuff.done_step == 0)):
                    ...
                    soabuff.done_step = 1
                    debt_tmp =  to_decimal(soabuff.saldo)
                    ...
                    for debt.gastnr, debt.rechnr, debt.betriebsnr, debt._recid, debt.rgdatum, debt.vesrcod, debt.artnr, debt.counter, debt.debref, debt.saldo, debt.vesrdep, debt.gastnrmember, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debt.gastnr, Debt.rechnr, Debt.betriebsnr, Debt._recid, Debt.rgdatum, Debt.vesrcod, Debt.artnr, Debt.counter, Debt.debref, Debt.saldo, Debt.vesrdep, Debt.gastnrmember, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel,(Artikel.artnr == Debt.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter((Debt.rechnr == soabuff.rechnr) & (Debt.gastnr == soabuff.gastnr) & (Debt.betriebsnr == soabuff.dptnr) & (Debt.opart <= 1) & (soabuff.saldo != 0) & (Debt.zahlkonto == 0) & (Debt._recid != soa_list.arrecid)).order_by(Debt._recid).all():
                        ...
                        debt_tmp =  to_decimal(debt_tmp) + to_decimal(debt.saldo)
                        fdebt =  to_decimal(fdebt) + to_decimal(debt.vesrdep)

                        if debt.counter > 0:

                            for debt1 in db_session.query(Debt1).filter((Debt1.rechnr == debt.rechnr) & (Debt1.counter == debt.counter) & (Debt1.artnr == debt.artnr) & (Debt1.gastnr == debitor.gastnr) & (Debt1.zahlkonto > 0)).order_by(debt.rgdatum).all():
                ...
                for soabuff in query(soabuff_data, filters=(lambda soabuff: soabuff.done_step == 1)):
                    soabuff.done_step = 2
                    soabuff.debt =  to_decimal(debt_tmp)
                    soabuff.credit =  to_decimal(crdt)
                    soabuff.saldo =  to_decimal(debt_tmp) - to_decimal(crdt)
                ...
                if debitor.betriebsnr == 0:
                    ...
                    if bill.resnr > 0:
                        ...
                        if res_line:
                            ...
                            if zimkateg:
                                ...
                                soa_list.rm_category = zimkateg.bezeichnung
                            ...
                            if reservation:
                                ...
                                if bill.billtyp == 2 and bill.resnr > 0 and bill.reslinnr == 0:
                                    soa_list.voucherno2 = reservation.vesrdepot
                                    soa_list.name = res_line.name
                    ...
                    if not h_bill:
                        ...
                        for hbilline in db_session.query(Hbilline).filter(
                                 (Hbilline.rechnr == h_bill.rechnr) & (Hbilline.departement == h_bill.departement)).order_by(Hbilline._recid).all():
                            ...
                            soa_list.bill_datum = hbilline.bill_datum
            ...
            for soa_list in query(soa_list_data, filters=(lambda soa_list: soa_list.done_step == 0), sort_by=[("_recid",False)]):
                ...
                for soabuff in query(soabuff_data, filters=(lambda soabuff: soabuff.rechnr == soa_list.rechnr and soabuff.dptnr == soa_list.dptnr and soabuff.artno == soa_list.artno and soabuff.done_step == 0 and soabuff._recid == soa_list._recid)):
                    ...
                    if soabuff.counter > 0:
                        ...
                        for debt in db_session.query(Debt).filter((Debt.rechnr == soabuff.rechnr) & (Debt.counter == soabuff.counter) & (Debt.gastnr == guestno) & (Debt.zahlkonto > 0)).order_by(Debt._recid).all():
            ...
            for soa_list in query(soa_list_data):
                ...
                debitor = Debitor()
                artikel = Artikel()
                for debitor.gastnr, debitor.rechnr, debitor.betriebsnr, debitor._recid, debitor.rgdatum, debitor.vesrcod, debitor.artnr, debitor.counter, debitor.debref, debitor.saldo, debitor.vesrdep, debitor.gastnrmember, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debitor.gastnr, Debitor.rechnr, Debitor.betriebsnr, Debitor._recid, Debitor.rgdatum, Debitor.vesrcod, Debitor.artnr, Debitor.counter, Debitor.debref, Debitor.saldo, Debitor.vesrdep, Debitor.gastnrmember, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel,(Artikel.artnr == Debitor.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter((Debitor.gastnr == guestno) & (Debitor.saldo != 0) & (Debitor.zahlkonto == 0) & (Debitor.rgdatum >= bof_month) & (Debitor.rgdatum <= eof_month)).order_by(Debitor.rgdatum, Debitor.rechnr).all():
                ...
                for debitor, guest in db_session.query(Debitor, Guest).join(Guest,(Guest.gastnr == Debitor.gastnr)).filter(((cast(Debitor._recid, Integer).in_(list(set([soa_list.arrecid for soa_list in soa_list_data])))))).order_by(Debitor._recid).all():
                    ...
                    if debitor.debref != 0:
                        isprintedb4 = True
                    debt_tmp =  to_decimal(debitor.saldo)
                    ...
                    for debt.gastnr, debt.rechnr, debt.betriebsnr, debt._recid, debt.rgdatum, debt.vesrcod, debt.artnr, debt.counter, debt.debref, debt.saldo, debt.vesrdep, debt.gastnrmember, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debt.gastnr, Debt.rechnr, Debt.betriebsnr, Debt._recid, Debt.rgdatum, Debt.vesrcod, Debt.artnr, Debt.counter, Debt.debref, Debt.saldo, Debt.vesrdep, Debt.gastnrmember, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel,(Artikel.artnr == Debt.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter((Debt.rechnr == debitor.rechnr) & (Debt.gastnr == debitor.gastnr) & (Debt.betriebsnr == debitor.betriebsnr) & (Debt.opart <= 1) & (debitor.saldo != 0) & (Debt.zahlkonto == 0) & (Debt._recid != debitor._recid)).order_by(Debt._recid).all():
    ...
    for soa_list in query(soa_list_data):
        soa_list.gst_amount_str = to_string(soa_list.gst_amount, "->>,>>>,>>>,>>9.9999999999999")
