...
param_ar_data, Param_ar = create_model(
    "Param_ar", {"param_nr": int, "param_name": string, "param_val": string, "param_type": string})


def ar_printsoa1_1_webbl(show_type: int, bof_month: date, eof_month: date, param_ar_data: [Param_ar], guestno: int, curr_day: date):

    prepare_cache([Guest, Bill, Debitor, Bill_line, H_bill_line, Res_line, Artikel,
                  Htparam, Waehrung, Exrate, Zimkateg, Reservation, H_bill, Hoteldpt, H_artikel])
    ...
    soa_list_data, Soa_list = create_model(
        "Soa_list",
        {
            "counter": int,
            "debref": int,
            ...
            "exrate": Decimal,
            "tot_exrate": Decimal,
            "gst_tot_non_taxable": Decimal,
            "gst_amount": Decimal,
            "gst_tot_sales": Decimal,
            "zimmeranz": int,
            "bill_datum": date,
            "resv_name": string,
            "rm_category": string,
            "gst_amount_str": string
        },
        {
            "printdate": get_current_date()
        })
    ...
    def generate_output():
        ...
        return {
            "param-ar": param_ar_data,
            "due_date": due_date,
            "soa-list": soa_list_data,
            "rech_nr": rech_nr,
            "bet_nr": bet_nr,
            "msg_int": msg_int
        }
    ...
    def create_soalist():
        ...
        nonlocal zeit, zeit1, str, due_date, soa_list_data, rech_nr, bet_nr, msg_int, param1, param2, param3, param4, fnet_lodging, lnet_lodging, net_breakfast, net_lunch, net_dinner, net_others, tot_rmrev, nett_vat, nett_service, curr_gastnr, curr_refno, new_refno, cl_exist, saldo, s, i, arrival, departure, debt, crdt, fdebt, fcrdt, new_crdt, new_fcrdt, isprintedb4, saldo1, saldo2, saldo3, saldo4, other_flag, count_zimm, guest, bill, debitor, bill_line, h_bill_line, res_line, artikel, htparam, waehrung, exrate, queasy, zimkateg, reservation, h_bill, hoteldpt, h_artikel
        nonlocal show_type, bof_month, eof_month, guestno, curr_day
        nonlocal soabuff, gast, bill1, debt, debt1, billine, hbilline, bresline
        ...
        if show_type == 0:
            ...
            for debitor.gastnr, debitor.rechnr, debitor.betriebsnr, debitor._recid, debitor.rgdatum, debitor.vesrcod, debitor.artnr, debitor.counter, debitor.debref, debitor.saldo, debitor.vesrdep, debitor.gastnrmember, debitor.zahlkonto, debitor.opart, artikel.artart, artikel.artnr, artikel.mwst_code, artikel._recid in db_session.query(Debitor.gastnr, Debitor.rechnr, Debitor.betriebsnr, Debitor._recid, Debitor.rgdatum, Debitor.vesrcod, Debitor.artnr, Debitor.counter, Debitor.debref, Debitor.saldo, Debitor.vesrdep, Debitor.gastnrmember, Debitor.zahlkonto, Debitor.opart, Artikel.artart, Artikel.artnr, Artikel.mwst_code, Artikel._recid).join(Artikel, (Artikel.artnr == Debitor.artnr) & (Artikel.artart == 2) & (Artikel.departement == 0)).filter(
                ...
                if not soa_list:
                    ...
                    soa_list.saldo = to_decimal(
                        soa_list.saldo) + to_decimal(debitor.saldo)
                    ...
                    if htparam.flogical:
                        ...
                        if waehrung:
                            ...
                            bill = get_cache(
                                Bill, {"rechnr": [(eq, debitor.rechnr)]})

                            if bill:

                                res_line = get_cache(
                                    Res_line, {"resnr": [(eq, bill.resnr)], "resstatus": [(eq, 8)]})

                                if res_line:

                                    exrate = get_cache(Exrate, {"artnr": [(eq, waehrung.waehrungsnr)], "datum": [
                                                       (eq, res_line.abreise)]})

                                    if exrate:
                                        soa_list.exrate = to_decimal(
                                            exrate.betrag)

            Debbt = create_buffer("Debbt", Debitor)

            soa_list = query(soa_list_data, filters=(
                lambda soa_list: soa_list.done_step == 0), first=True)
            while None != soa_list:
                crdt = to_decimal("0")
                debt = to_decimal("0")
                fcrdt = to_decimal("0")
                    ...
                    if soabuff.counter > 0:
                        ...
                        for debt in db_session.query(debt).filter(
                            ...
                            fcrdt = to_decimal(fcrdt) + \
                                to_decimal(debt.vesrdep)
                            crdt = to_decimal(crdt) + to_decimal(debt.saldo)

                            if isprintedb4 and debt.debref == 0:
                                new_crdt = to_decimal(
                                    new_crdt) + to_decimal(crdt)
                                new_fcrdt = to_decimal(
                                    new_fcrdt) + to_decimal(fcrdt)
                        ...
                        if debt.counter > 0:
                            ...
                            for debt1 in db_session.query(Debt1).filter(
                                ...
                                crdt = to_decimal(crdt) + \
                                    to_decimal(debt.saldo)
                                fcrdt = to_decimal(
                                    fcrdt) + to_decimal(debt.vesrdep)
                ...
                print(f"[DEBUG] debt value: {debt}", flush=True)

                for soabuff in query(soabuff_data, filters=(lambda soabuff: soabuff.done_step == 1)):
                    soabuff.done_step = 2
                    soabuff.debt = to_decimal(debt.saldo)
                    soabuff.credit = to_decimal(crdt)
                    soabuff.saldo = to_decimal(debt.saldo) - to_decimal(crdt)
            ...
            for soa_list in query(soa_list_data):
                ...
                debitor = get_cache(
                    Debitor, {"_recid": [(eq, soa_list.arrecid)]})
                ...
                if debitor.betriebsnr == 0:
                    ...
                    if not bill:
                        ...
                        if debitor.debref != 0:
                            ...
                            queasy = get_cache(
                                Queasy, {"key": [(eq, 192)], "number1": [(eq, debitor.rechnr)]})
                        ...
                        if gast:
                            soa_list.name = gast.name + ", " + gast.vorname1 + \
                                " " + gast.anrede1 + gast.anredefirma
                        else:
                            soa_list.name = guest.name + ", " + guest.vorname1 + \
                                " " + guest.anrede1 + guest.anredefirma
                        continue
                    zeit1 = get_current_time_in_seconds()
                    other_flag = False

                    bill_line = get_cache(Bill_line, {"rechnr": [(eq, debitor.rechnr)], "artnr": [
                                          (eq, debitor.artnr)], "departement": [(eq, 0)]})
                    ...
                    if bill.billref != 0:
                        ...
                        bdebt = get_cache(Debitor, {"rechnr": [(eq, soa_list.rechnr)], "debref": [
                                          (ne, 0)], "counter": [(eq, soa_list.counter)]})

                        if bdebt:
                            soa_list.saldo = to_decimal(
                                soa_list.saldo) - to_decimal(bdebt.saldo)
                            soa_list.debt = to_decimal(
                                soa_list.debt) - to_decimal(bdebt.saldo)

                            if soa_list.saldo != 0:

                                bdebt1 = get_cache(
                                    Debitor, {"debref": [(eq, 0)], "rechnr": [(eq, soa_list.rechnr)]})
                    ...
                    if bill.resnr > 0:
                        ...
                        if res_line:
                            ...
                            soa_list.remarks = soa_list.remarks + \
                                chr_unicode(3) + to_string(res_line.resnr)
                            ...
                            if zimkateg:
                                soa_list.remarks = soa_list.remarks + \
                                    chr_unicode(3) + zimkateg.kurzbez + \
                                    chr_unicode(3) + zimkateg.bezeichnung
                                soa_list.rm_category = zimkateg.bezeichnung

                            else:
                                soa_list.remarks = soa_list.remarks + \
                                    chr_unicode(3) + chr_unicode(3)
                            fnet_lodging, lnet_lodging, net_breakfast, net_lunch, net_dinner, net_others, tot_rmrev, nett_vat, nett_service = get_output(
                                get_room_breakdown(res_line._recid, res_line.ankunft, 0, date_mdy(get_current_date())))
                            soa_list.remarks = soa_list.remarks + chr_unicode(3) + to_string(
                                nett_vat, ">>>,>>>,>>9.99") + chr_unicode(3) + to_string(nett_service, ">>>,>>>,>>9.99")
                        ...
                        soa_list.remarks = soa_list.remarks + \
                            chr_unicode(
                                3) + chr_unicode(3) + chr_unicode(3) + chr_unicode(3) + chr_unicode(3)

                    for billine in db_session.query(Billine).filter(
                            (Billine.rechnr == debitor.rechnr) & (Billine.anzahl != 0)).order_by(Billine._recid).all():

                        artikel = get_cache(
                            Artikel, {"artnr": [(eq, billine.artnr)]})

                        if artikel:

                            if artikel.artart == 0 or artikel.artart == 1 or artikel.artart == 8 or artikel.artart == 9:
                                soa_list.gst_tot_sales = to_decimal(
                                    soa_list.gst_tot_sales) + to_decimal(billine.betrag)

                                if artikel.artart == 1:
                                    soa_list.gst_amount = to_decimal(
                                        soa_list.gst_amount) + to_decimal((billine.betrag) / to_decimal(1.06))
                                    ...
                                    if artikel.mwst_code == 0:
                                        soa_list.gst_tot_non_taxable = to_decimal(
                                            soa_list.gst_tot_non_taxable) + to_decimal(billine.betrag)

                else:

                    h_bill = get_cache(H_bill, {"rechnr": [(eq, debitor.rechnr)], "departement": [
                                       (eq, debitor.betriebsnr)]})
                    ...
                    if not h_bill:
                        ...
                        hoteldpt = get_cache(
                            Hoteldpt, {"num": [(eq, debitor.betriebsnr)]})
                        ...
                        if h_bill.bilname != "":
                            ...
                            if gast:
                                soa_list.name = hoteldpt.depart + " - " + gast.name + ", " + \
                                    gast.vorname1 + " " + gast.anrede1 + gast.anredefirma
                            else:
                                soa_list.name = hoteldpt.depart + " - " + guest.name + ", " + \
                                    guest.vorname1 + " " + guest.anrede1 + guest.anredefirma

                        if h_bill.service[6] != 0:

                            if length(to_string(h_bill.service[6])) < 8:
                                soa_list.printdate = date_mdy(to_int(substring(to_string(h_bill.service[6]), 0, 1)), to_int(
                                    substring(to_string(h_bill.service[6]), 1, 2)), to_int(substring(to_string(h_bill.service[6]), 3, 4)))
                            else:
                                soa_list.printdate = date_mdy(to_int(substring(to_string(h_bill.service[6]), 0, 2)), to_int(
                                    substring(to_string(h_bill.service[6]), 2, 2)), to_int(substring(to_string(h_bill.service[6]), 4, 4)))
                        else:
                            soa_list.printdate = get_current_date()

                        if to_int(h_bill.service[5]) != 0:
                            soa_list.inv_str = "INV" + \
                                to_string(h_bill.service[5], "9999999")
                        ...
                        for hbilline in db_session.query(Hbilline).filter(
                            ...
                            h_artikel = get_cache(
                                H_artikel, {"artnr": [(eq, hbilline.artnr)]})

                            if h_artikel:

                                artikel = get_cache(
                                    Artikel, {"artnr": [(eq, h_artikel.artnrfront)]})

                                if artikel:

                                    if artikel.artart == 0 or artikel.artart == 1 or artikel.artart == 8 or artikel.artart == 9:
                                        soa_list.gst_tot_sales = to_decimal(
                                            soa_list.gst_tot_sales) + to_decimal(hbilline.betrag)

                                        if artikel.artart == 1:
                                            soa_list.gst_amount = to_decimal(
                                                soa_list.gst_amount) + to_decimal((hbilline.betrag) / to_decimal(1.06))
                                            ...
                                            if artikel.mwst_code == 0:
                                                soa_list.gst_tot_non_taxable = to_decimal(
                                                    soa_list.gst_tot_non_taxable) + to_decimal(hbilline.betrag)
            ...
            while None != soa_list:
                ...
                soa_list = query(soa_list_data, filters=(
                    lambda soa_list: soa_list.done_step == 0), next=True)
            ...
            if param1.lower() == ("yes").lower():
                counter_saldo = to_decimal(
                    counter_saldo) + to_decimal(saldo1) + to_decimal(saldo2)
            else:
            ...
            debitor_obj_list = {}
            for debitor, guest in db_session.query(Debitor, Guest).join(Guest, (Guest.gastnr == Debitor.gastnr)).filter(
                ...
                soa_list = query(soa_list_data, (lambda soa_list: (
                    to_int(debitor._recid) == soa_list.arrecid)), first=True)
                crdt = to_decimal("0")
                debt = to_decimal("0")
                fcrdt = to_decimal("0")
                fdebt = to_decimal("0")
                ...
                if debitor.betriebsnr == 0:
                    ...
                    bill_line = get_cache(Bill_line, {"rechnr": [(eq, bill.rechnr)], "artnr": [
                                          (eq, artikel.artnr)], "departement": [(eq, 0)]})
    ...
    def call_paramar(nr: int):
        ...
        param_ar = query(param_ar_data, filters=(
            lambda param_ar: param_ar.param_nr == nr), first=True)
    ...
    for soa_list in query(soa_list_data):
        soa_list.gst_amount_str = to_string(
            soa_list.gst_amount, "->>,>>>,>>>,>>9.9999999999999")
