...
def bil_selectbl(pvilanguage:int, sorttype:int, zinr:string, bil_int:int, curr_gastnr:int, ci_date:date, gastname:string, to_name:string, rechnr:int):
    ...
    def disp_bill_list0():
        ...
        if sorttype == 1:
            ...
            bill = Bill()
            rline = Res_line()
            for bill.zinr, bill.saldo, bill._recid, rline.resnr, rline.reslinnr, rline.zinr, rline._recid, rline.gastnrmember, rline.abreise, rline.bemerk in db_session.query(Bill.zinr, Bill.saldo, Bill._recid, Rline.resnr, Rline.reslinnr, Rline.zinr, Rline._recid, Rline.gastnrmember, Rline.abreise, Rline.bemerk).join(Rline,(Rline.resnr == Bill.resnr) & (Rline.reslinnr == Bill.parent_nr) & (Rline.active_flag == actflag)).filter(
                     (Bill.zinr >= (zinr).lower()) & (Bill.zinr != "") & (Bill.flag == bil_int)).order_by(Bill.zinr, Bill.billnr).all():
