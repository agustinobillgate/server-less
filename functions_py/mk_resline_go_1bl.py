...
def mk_resline_go_1bl(pvilanguage:int, accompany_tmpnr1:int, accompany_tmpnr2:int, accompany_tmpnr3:int, 
                      accompany_gastnr:int, accompany_gastnr2:int, accompany_gastnr3:int, comchild:int, rm_bcol:int, 
                      marknr:int, bill_instruct:int, restype:int, restype0:int, restype1:int, contact_nr:int, cutoff_days:int, 
                      segm__purcode:int, deposit:Decimal, limitdate:date, wechsel_str:string, origcontcode:string, 
                      groupname:string, guestname:string, main_voucher:string, resline_comment:string, mainres_comment:string, 
                      purpose_svalue:string, letter_svalue:string, segm_svalue:string, source_svalue:string, res_mode:string, 
                      prev_zinr:string, memo_zinr:string, voucherno:string, contcode:string, child_age:string, flight1:string, 
                      flight2:string, eta:string, etd:string, user_init:string, currency_changed:bool, fixed_rate:bool, grpflag:bool, 
                      memozinr_readonly:bool, group_enable:bool, init_fixrate:bool, oral_flag:bool, pickup_flag:bool, drop_flag:bool, 
                      ebdisc_flag:bool, kbdisc_flag:bool, restricted:bool, sharer:bool, coder_exist:bool, gname_chged:bool, 
                      earlyci:bool, gdpr_flag:bool, mark_flag:bool, news_flag:bool, reslin_list_data:[Reslin_list], 
                      res_dynarate_data:[Res_dynarate], tot_qty:int):
    ...
    resline_comment = resline_comment.strip()
    mainres_comment = mainres_comment.strip()
    purpose_svalue = purpose_svalue.strip()
    letter_svalue = letter_svalue.strip()
    segm_svalue = segm_svalue.strip()
        ...
        if reslin_list.zipreis > 0 and res_line.l_zuordnung[2] == 1:
            ...
            if not bill:
                ...
                resline = db_session.query(Resline).filter(
                         (Resline._recid == res_line._recid)).with_for_update().first()
    ...
    def update_resline():
        ...
        if accompany_gastnr > 0 or accompany_tmpnr[0] > 0:
            ...
            resline = db_session.query(Resline).filter(
                     (Resline.resnr == res_line.resnr) & (Resline.active_flag <= 1) & 
                     (Resline.kontakt_nr == res_line.reslinnr) & 
                     (Resline.gastnrmember == accompany_gastnr) & (Resline.l_zuordnung[inc_value(2)] == 1)).with_for_update().first()
        ...
        if accompany_gastnr2 > 0 or accompany_tmpnr[1] > 0:
            ...
            resline = db_session.query(Resline).filter(
                     (Resline.resnr == res_line.resnr) & (Resline.active_flag <= 1) & 
                     (Resline.kontakt_nr == res_line.reslinnr) & (Resline.gastnrmember == accompany_gastnr2) & (Resline.l_zuordnung[inc_value(2)] == 1)).with_for_update().first()
        ...
        if accompany_gastnr3 > 0 or accompany_tmpnr[2] > 0:
            ...
            resline = db_session.query(Resline).filter(
                     (Resline.resnr == res_line.resnr) & (Resline.active_flag <= 1) & 
                     (Resline.kontakt_nr == res_line.reslinnr) & 
                     (Resline.gastnrmember == accompany_gastnr3) & (Resline.l_zuordnung[inc_value(2)] == 1)).with_for_update().first()
    ...
    def resline_reserve_dec():
        ...
        if not reservation.insurance:
            ...
            for rline in db_session.query(Rline).filter(
                     (Rline.resnr == reservation.resnr) & ((Rline.resstatus == 6) | (Rline.resstatus == 13)) & (Rline.reserve_dec != 0)).order_by(Rline._recid).with_for_update().all():
    ...
    if rm_bcol != 15 and reslin_list.ankunft != res_line.ankunft and reslin_list.resstatus == 1:
        ...
        outorder = db_session.query(Outorder).filter(
                 (Outorder.zinr == reslin_list.zinr) & (Outorder.betriebsnr == reslin_list.betriebsnr)).with_for_update().first()
