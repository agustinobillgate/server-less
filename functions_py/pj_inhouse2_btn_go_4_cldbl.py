...
# yusufwijasena, 26/12/2025
# - fix query for res_line 
# ------------------------------------------
...
def pj_inhouse2_btn_go_4_cldbl(sorttype: int, datum: date, curr_date: date, curr_gastnr: int, froom: string, troom: string, exc_depart: bool, incl_gcomment: bool, incl_rsvcomment: bool, prog_name: string, disp_accompany: bool, disp_exclinact: bool, split_rsv_print: bool, exc_compli: bool):
    ...
    prepare_cache([Htparam, Zkstat, Zinrstat, Paramtext, Guest, Reservation, Zimmer, Zimkateg, Res_line,
    ...
    def create_inhouse():
        ...
        # yusufwijasena, fix new query for res_line
        ...
        create_inhouse_query = (
            ...
            .filter(
                # Res_line.active_flag.between(actflag1, actflag2),
                (Res_line.active_flag >= actflag1) &
                (Res_line.active_flag <= actflag2) &
                (Res_line.resstatus != 9) &
                (Res_line.resstatus != 10) &
            ...
            .order_by(
                Res_line.zinr,
                Res_line.erwachs.desc(),
                Res_line.name,
            )
        )

        for res_line, zimkateg, reservation, guest, gmember in create_inhouse_query.all():
            # log.write_log('log', f"Processing Res_line _recid: {res_line._recid}, zinr: {res_line.zinr}, name: {res_line.name}")
            ...
            if exc_depart and res_line.abreise == datum:
                ...
                # if zimmer:
                #     log.write_log('log',
                #         f"[LOG] zimmer: {zimmer.zinr}, etage: {zimmer.etage}, sleeping: {zimmer.sleeping}")
                # else:
                #     log.write_log('error', f"[ERROR] Zimmer not found for zinr={res_line.zinr}")

                nr = nr + 1
                vip_flag = ""

                guestseg = db_session.query(Guestseg).filter(
                    (Guestseg.gastnr == gmember.gastnr) &
                    ((Guestseg.segmentcode == vipnr1) | (Guestseg.segmentcode == vipnr2) | (Guestseg.segmentcode == vipnr3) | (Guestseg.segmentcode == vipnr4) | (Guestseg.segmentcode == vipnr5) | (Guestseg.segmentcode == vipnr6) | (Guestseg.segmentcode == vipnr7) | (Guestseg.segmentcode == vipnr8) | (Guestseg.segmentcode == vipnr9))).first()
        ...
        # yusufwijasena, end - fix new query for res_line
                ...
                nation = get_cache(
                    Nation, {"kurzbez": [(eq, gmember.nation2)]})
                ...
                mealcoup = get_cache(Mealcoup, {"name": [(eq, "breakfast")], "resnr": [
                                     (eq, res_line.resnr)], "zinr": [(eq, res_line.zinr)]})
                ...
                if not split_rsv_print:
                    ...
                    for i in range(1, length(res_line.bemerk) + 1):
                        ...
                        if substring(res_line.bemerk, i - 1, 1) == chr_unicode(10):
                            ...
                            cl_list.bemerk = cl_list.bemerk + \
                                substring(trim(res_line.bemerk), i - 1, 1)
                    ...
                    cl_list.bemerk02 = to_string(
                        substring(all_remark, 225, 225))
                    cl_list.bemerk03 = to_string(
                        substring(all_remark, 450, 225))
                    cl_list.bemerk04 = to_string(
                        substring(all_remark, 675, 225))
                else:
                    for i in range(1, length(res_line.bemerk) + 1):
                        cl_list.bemerk = cl_list.bemerk + \
                            substring(trim(res_line.bemerk), i - 1, 1)
                    ...
                    queasy = get_cache(Queasy, {"key": [(eq, 267)], "number1": [
                ...
                zinr_list = query(zinr_list_data, filters=(lambda zinr_list: zinr_list.zinr ==
                                  res_line.zinr and zinr_list.resnr == res_line.resnr and zinr_list.reslinnr == res_line.reslinnr), first=True)
                ...
                if not zinr_list:
                    ...
                    queasy = get_cache(Queasy, {"key": [(eq, 14)], "char1": [
                                       (eq, res_line.zinr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})
    ...
    def create_genstat_inhouse():
        ...
        for genstat.resstatus, genstat.res_date, genstat.resnr, genstat.res_int, genstat.zinr, genstat.zipreis, genstat.erwachs, genstat.kind1, \
            ...
            if genstat.res_date[0] < datum and genstat.res_date[1] == datum and genstat.resstatus == 8:
                ...
                res_line = get_cache(Res_line, {"resnr": [(eq, genstat.resnr)], "reslinnr": [
                ...
                zinr_list = query(zinr_list_data, filters=(lambda zinr_list: zinr_list.zinr ==
                                  genstat.zinr and zinr_list.resnr == res_line.resnr and zinr_list.reslinnr == res_line.reslinnr), first=True)
                ...
                if not zinr_list:
                    ...
                    queasy = get_cache(Queasy, {"key": [(eq, 14)], "char1": [
                                       (eq, genstat.zinr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})
        ...
        for cl_list in query(cl_list_data, sort_by=[("nation", False), ("bezeich", False)]):
            ...
            mealcoup = get_cache(Mealcoup, {"name": [(eq, "breakfast")], "resnr": [
                                 (eq, cl_list.resnr)], "zinr": [(eq, cl_list.rmno)]})
    ...
    def create_genstat_inhouse1():
        ...
        for cl_list in query(cl_list_data, sort_by=[("nation", False), ("bezeich", False)]):
            ...
            s_list = query(s_list_data, filters=(
                lambda s_list: s_list.bezeich == cl_list.bezeich), first=True)
    ...
    if datum < curr_date:
        ...
        zinrstat = get_cache(
            Zinrstat, {"zinr": [(eq, "tot-rm")], "datum": [(eq, datum)]})
