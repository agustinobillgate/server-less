...
def leasing_cancel_rsvbl(qrecid:int, user_init:string):
    ...
    tot_nettamount:Decimal = to_decimal("0.0")
    ...
    def create_bill_line(billno:int):
        ...
        if bartikel:
            ...
            bill_line.bezeich = bill_line.bezeich + "[" + "Cancel service Apartment #" + to_string(queasy.number1) + "]"


            pass

        bartikel = get_cache (Artikel, {"artnr": [(eq, ar_ledger)],"departement": [(eq, 0)]})
    ...
    def create_ar():
        ...
        debitor = get_cache (Debitor, {"vesrcod": [(eq, queasy.char2)],"zahlkonto": [(eq, 0)]})
        ...
        artikel = get_cache (Artikel, {"artnr": [(eq, ar_ledger)],"departement": [(eq, 0)]})
        ...
        billjournal.bezeich = artikel.bezeich + "[" + "Cancel service Apartment#" + to_string(queasy.number1) + "]"
        ...
        umsatz = get_cache (Umsatz, {"artnr": [(eq, ar_ledger)],"departement": [(eq, 0)],"datum": [(eq, bill_date)]})
        ...
        artikel = get_cache (Artikel, {"artnr": [(eq, divered_rental)],"departement": [(eq, 0)]})
        ...
        umsatz = get_cache (Umsatz, {"artnr": [(eq, divered_rental)],"departement": [(eq, 0)],"datum": [(eq, bill_date)]})
        ...
        umsatz.anzahl = umsatz.anzahl + 1
        umsatz.betrag =  to_decimal(umsatz.betrag) + to_decimal(- to_decimal(tot_amount) )
    ...
    def create_journal():
        ...
        gl_jouhdr.refno = "CANCEL-" + to_string(queasy.number1) + "-" + to_string(bill_date)
        ...
        if guest:
            gl_jouhdr.bezeich = "CANCEL-service RESIDENT-" + to_string(queasy.number1) + "-" + to_string(bill_date)
        ...
        if get_month(periode_rsv1) + 1 > 12:
            ...
            if get_day(periode_rsv1) >= 29:
                ...
                if get_year(periode_rsv1) % 4 != 0:
                    ...
                    periode = date_mdy(get_month(periode_rsv1) + 1, month_str1[get_month(periode_rsv1) + 1, get_year(periode_rsv1) ])
                    ...
                    periode = date_mdy(get_month(periode_rsv1) + 1, month_str2[get_month(periode_rsv1), get_year(periode_rsv1) ]) - timedelta(days=1)
            ...
            # periode = date_mdy(get_month(periode_rsv1) + timedelta(days=1, get_day(periode_rsv1) , get_year(periode_rsv1)) - 1)
            periode = date_mdy(get_month(periode_rsv1), get_day(periode_rsv1) , get_year(periode_rsv1)) - timedelta(days=1)
                ...
                if get_month(periode_rsv1) + 1 > 12:
                    ...
                    if get_day(periode_rsv1) >= 29:
                        ...
                        if get_year(periode_rsv1) % 4 != 0:
                            ...
                            # periode = date_mdy(get_month(periode_rsv1) + 1, month_str2[get_month(periode_rsv1) + 1 - timedelta(days=1), get_year(periode_rsv1) ])
                            periode = date_mdy(get_month(periode_rsv1) + 1, month_str2[get_month(periode_rsv1) + 1], get_year(periode_rsv1) )
    ...
    queasy = get_cache (Queasy, {"key": [(eq, 329)],"_recid": [(eq, qrecid)],"char2": [(ne, "")],"logi1": [(eq, False)]})
