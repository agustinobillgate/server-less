...
def bill_parserbl(pvilanguage:int, case_type:int, briefnr:int, reslinnr:int, resnr:int, rechnr:int, gastnr:int, f_page:bool, spbill_flag:bool, inv_type:int, user_init:string, printnr:int, t_spbill_list_data:[T_spbill_list]):
    ...
    bill_list_data, Bill_list = create_model("Bill_list", {"rechnr":int, "name":string, "rechnr2":int})
    output_list_data, Output_list = create_model("Output_list", {"str":string, "pos":int})
    s_list_data, S_list = create_model("S_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "datum":date, "qty":int, "erwachs":int, "kind1":int, "kind2":int})
    t_list_data, T_list = create_model("T_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "tage":int, "date1":date, "date2":date, "qty":int, "betrag":Decimal, "erwachs":int, "kind1":int, "kind2":int})
    ...
    def generate_output():
        ...
        return {"succes_flag": succes_flag, "outfile": outfile, "run_ask": run_ask, "bill-list": bill_list_data, "output-list": output_list_data}
    ...
    def do_billlinea():
        ...
        for bill_line in db_session.query(Bill_line).filter(
            ...
            bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.artnr == bill_line.artnr and bline_list.dept == bill_line.departement and bline_list.bezeich == bill_line.bezeich and bline_list.datum == bill_line.bill_datum and bline_list.saldo == - bill_line.betrag), first=True)
        ...
        for bill_line in db_session.query(Bill_line).filter(
                 (Bill_line.rechnr == rechnr)).order_by(Bill_line.sysdate, Bill_line.zeit).all():
            bline_list = query(bline_list_data, (lambda bline_list: bline_list.bl_recid == to_int(bill_line._recid)), first=True)
            if not bline_list:
                continue
    ...
    def do_billlineb():
        ...
        for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & ((Artikel.artart == 0) | (Artikel.artart == 1) | (Artikel.artart == 8) | (Artikel.artart == 9) | ((Artikel.artart == 6) & (Artikel.zwkum == paidout)))).filter(
            ...
            if bill_line.origin_id != "":
                ...
                bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.vatProz == vat_perc), first=True)
                ...
                bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.mwst_code == artikel.mwst_code), first=True)
    ...
    def do_billlinec():
        ...
        for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & ((Artikel.artart == 0) | (Artikel.artart == 1) | (Artikel.artart == 8) | (Artikel.artart == 9))).filter(
            ...
            if inv_type == 3:
                ...
                if artikel.bezaendern or artikel.artart == 1:
                    ...
                    bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.artnr == bill_line.artnr and bline_list.dept == bill_line.departement and bline_list.bezeich == bill_line.bezeich), first=True)
                else:

                    bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.artnr == bill_line.artnr and bline_list.dept == bill_line.departement), first=True)

            elif inv_type == 4:

                if artikel.bezaendern or artikel.artart == 1:

                    bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.artnr == bill_line.artnr and bline_list.dept == bill_line.departement and bline_list.bezeich == bill_line.bezeich and bline_list.zinr == bill_line.zinr), first=True)
                else:

                    bline_list = query(bline_list_data, filters=(lambda bline_list: bline_list.artnr == bill_line.artnr and bline_list.dept == bill_line.departement and bline_list.zinr == bill_line.zinr), first=True)
        ...
        for bline_list in query(bline_list_data, filters=(lambda bline_list: bline_list.saldo == 0)):
            bline_list_data.remove(bline_list)
        bline_nr = 0
    ...
    def do_spbillline0b():
        ...
        bill_line_obj_list = {}
        for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & ((Artikel.artart == 0) | (Artikel.artart == 1) | (Artikel.artart == 8) | (Artikel.artart == 9) | ((Artikel.artart == 6) & (Artikel.zwkum == paidout)))).filter(
                 (Bill_line._recid.in_(list(set([t_spbill_list.bl_recid for t_spbill_list in t_spbill_list_data if t_spbill_list.selected]))))).order_by(Bill_line._recid).all():
            if bill_line_obj_list.get(bill_line._recid):
                continue
            else:
                bill_line_obj_list[bill_line._recid] = True
    ...
    def do_spbillline0c():
        ...
        bill_line_obj_list = {}
        for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement) & ((Artikel.artart == 0) | (Artikel.artart == 1) | (Artikel.artart == 8) | (Artikel.artart == 9) | ((Artikel.artart == 6) & (Artikel.zwkum == paidout)))).filter(
                 (Bill_line._recid.in_(list(set([t_spbill_list.bl_recid for t_spbill_list in t_spbill_list_data if t_spbill_list.selected]))))).order_by(Bill_line.zinr, Bill_line.bezeich, Bill_line.bill_datum).all():
            if bill_line_obj_list.get(bill_line._recid):
                continue
            else:
                bill_line_obj_list[bill_line._recid] = True
    ...
    def do_pbill_line():
        ...
        for t_list in query(t_list_data, sort_by=[("nr",False),("date1",False)]):
