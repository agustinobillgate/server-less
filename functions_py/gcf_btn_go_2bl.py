...
def gcf_btn_go_2bl(icase:int, pvilanguage:int, user_init:string, refno4:string, t_guest_data:[T_guest]):

    prepare_cache ([Guest, Htparam, Bediener, Nation, Res_history, Queasy])
    ...
    def update_record():
        ...
        guest = db_session.query(Guest).filter(Guest.gastnr == gastno).with_for_update().first()
        ...
        if guest:
            ...
            if refno4 != "":
                ...
                # queasy = get_cache (Queasy, {"key": [(eq, 231)],"number1": [(eq, gastno)]})
    ...
    if icase == 1:
        ...
        guestseg = db_session.query(Guestseg).filter(Guestseg.gastnr == gastno).with_for_update().first()
    ...
    if t_guest.phonetik3 != "":
        ...
        if htparam.flogical:
            ...
            akt_cust = db_session.query(Akt_cust).filter(Akt_cust.gastnr == t_guest.gastnr).with_for_update().first()
