...
def ts_restinv_pay_cash7bl(rec_id: int, transdate: date, curr_dept: int, disc_art1: int, disc_art2: int, disc_art3: int, kellner_kellner_nr: int):
    ...
    prepare_cache([Htparam, H_bill, Interface,
                  H_bill_line, Queasy, H_umsatz, Artikel])
    ...
    def fill_cover():
        ...
        tbuff = db_session.query(Tbuff).filter(
            (Tbuff.tischnr == h_bill.tischnr) &
            (Tbuff.departement == h_bill.departement)
        ...
        if h_bill.resnr > 0:
            get_output(rest_addgastinfo(h_bill.departement, h_bill.rechnr,
                       h_bill.resnr, h_bill.reslinnr, 0, transdate))
        pass
        ...
        if h_bill.belegung != 0:
            ...
            h_bill_line_query = (
                db_session.query(H_bill_line, H_art1, Artikel).join(H_art1, (H_art1.artnr == H_bill_line.artnr) & (H_art1.departement == H_bill_line.departement) & (H_art1.artart == 0)).join(Artikel, (Artikel.artnr == H_art1.artnrfront) & (Artikel.departement == H_art1.departement)).filter(
                    (H_bill_line.rechnr == h_bill.rechnr) &
                    (H_bill_line.departement == h_bill.departement) &
                    (H_bill_line.artnr != disc_art1) &
                    (H_bill_line.artnr != disc_art2) &
                    (H_bill_line.artnr != disc_art3)
                )
                .order_by(
                    H_bill_line._recid
                )
            ...
            for h_bill_line, h_art1, artikel in h_bill_line_query.all():
        ...
        h_umsatz.nettobetrag = to_decimal(
            h_umsatz.nettobetrag) + to_decimal(b_pax)
    ...
    def release_tbplan():
        ...
        queasy = db_session.query(Queasy).filter(
            (Queasy.key == 31) &
            (Queasy.number1 == h_bill.departement) &
            (Queasy.number2 == h_bill.tischnr)
        ).with_for_update().first()
    ...
    def update_selforder():
        ...
        genparamso_query = (
            db_session.query(Genparamso).filter(
                (Genparamso.key == 222) &
                (Genparamso.number1 == 1) &
                (Genparamso.betriebsnr == curr_dept)
            )
            .order_by(Genparamso._recid)
        )
        for genparamso in genparamso_query.all():
        ...
        searchbill_query = (
            db_session.query(Searchbill).filter(
                (Searchbill.key == 225) &
                (Searchbill.number1 == curr_dept) &
                (Searchbill.char1 == "orderbill")
            )
            .order_by(Searchbill._recid)
        )

        for searchbill in searchbill_query.yield_per(100):
        ...
        paramqsy = db_session.query(Paramqsy).filter(
            (Paramqsy.key == 230) &
            (Paramqsy.char1 == session_parameter)
        ).with_for_update().first()
        ...
        if paramqsy:
            ...
            if dynamic_qr:
                ...
                pickup_table = db_session.query(Pickup_table).filter(
                    (Pickup_table.key == 225) &
                    (Pickup_table.char1 == "taken-table") &
                    (Pickup_table.number1 == curr_dept) &
                    (Pickup_table.logi1) & (Pickup_table.logi2) &
                    (Pickup_table.number2 == paramqsy.number2) &
                    (entry(0, Pickup_table.char3, "|")
                     == (session_parameter).lower())
                ).with_for_update().first()
            ...
            orderbill = db_session.query(Orderbill).filter(
                (Orderbill.key == 225) &
                (Orderbill.char1 == "orderbill") &
                (Orderbill.char3 == (session_parameter).lower()) &
                (Orderbill.logi1) &
                (Orderbill.logi3)
            ).with_for_update().first()
            ...
            if orderbill:
                ...
                while orderbill_close:
                    ...
                    orderbill_close = db_session.query(Orderbill_close).filter(
                        (Orderbill_close.key == 225) &
                        (Orderbill_close.char1 == ("orderbill")) &
                        (Orderbill_close.char3 == (session_parameter).lower()) &
                        (Orderbill_close.logi1) &
                        (Orderbill_close.logi3) &
                        (Orderbill_close._recid > curr_recid)
                    ).first()
            ...
            if dynamic_qr:
                ...
                while orderbilline:
                    ...
                    orderbilline = db_session.query(Orderbilline).filter(
                        (Orderbilline.key == 225) &
                        (Orderbilline.char1 == ("orderbill-line")) &
                        (entry(3, Orderbilline.char2, "|") == (session_parameter).lower()) &
                        (Orderbilline._recid > curr_recid)
                    ).with_for_update().first()
            ...
            qpayment_gateway = db_session.query(Qpayment_gateway).filter(
                (Qpayment_gateway.key == 223) &
                (Qpayment_gateway.char3 == session_parameter) &
                (Qpayment_gateway.betriebsnr == get_rechnr)
            ).with_for_update().first()
    ...
    h_bill_line_h_artikel_query = (
        db_session.query(H_bill_line).filter(
            # yusufwijasena, fix H_bill_line.department
            (H_bill_line.departement == h_bill.departement) &
            (H_bill_line.rechnr == h_bill.rechnr) &
            (H_bill_line.betrag < 0)
        )
        .order_by(H_bill_line._recid)
    )

    for h_bill_line in h_bill_line_h_artikel_query.all():
        h_artikel = get_cache(H_artikel, {"departement": [(eq, h_bill_line.departement)], "artnr": [
                              (eq, h_bill_line.artnr)], "artart": [(ne, 0)]})
        if h_artikel:
            get_amount = to_decimal(get_amount) + \
