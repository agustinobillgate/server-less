    ...
    def cal_revenue():
        ...
        for datum in date_range(res_line.ankunft, co_date):
            ...
            if datum >= ci_date or not arrangement:
                ...
                arrangement = get_cache(
                    Arrangement, {"arrangement": [(eq, res_line.arrangement.strip())]})
                ...
                if not fixed_rate:
                    ...
                    if not it_exist:
                        ...
                        if guest_pr:
                            ...
                            if new_contrate:
                                rate_found, rm_rate, early_flag, kback_flag = get_output(ratecode_rate(ebdisc_flag, kbdisc_flag, res_line.resnr, res_line.reslinnr, contcode, None, bill_date, res_line.ankunft,
                                                                                         res_line.abreise, res_line.reserve_int, arrangement.argtnr, curr_zikatnr, res_line.erwachs, res_line.kind1, res_line.kind2, res_line.reserve_dec, res_line.betriebsnr))
                            else:
                                rm_rate, rate_found = get_output(pricecod_rate(res_line.resnr, res_line.reslinnr, guest_pr.code, bill_date, res_line.ankunft, res_line.abreise,
                                                                 res_line.reserve_int, arrangement.argtnr, curr_zikatnr, res_line.erwachs, res_line.kind1, res_line.kind2, res_line.reserve_dec, res_line.betriebsnr))
                        ...
                        if not rate_found:
                            ...
                            if (bill_date == ci_date) or (bill_date == res_line.ankunft):
                                ...
                                katpreis = get_cache(Katpreis, {"zikatnr": [(eq, curr_zikatnr)], "argtnr": [(eq, arrangement.argtnr)], "startperiode": [
                                                     (le, bill_date)], "endperiode": [(ge, bill_date)], "betriebsnr": [(eq, w_day)]})

                                if not katpreis:

                                    katpreis = get_cache(Katpreis, {"zikatnr": [(eq, curr_zikatnr)], "argtnr": [(eq, arrangement.argtnr)], "startperiode": [
                                                         (le, bill_date)], "endperiode": [(ge, bill_date)], "betriebsnr": [(eq, 0)]})
            ...
            output_list.str = to_string(datum) + " " + translateExtended("Roomrate", lvcarea, "") + " = " + trim(
                to_string(rm_rate, "->>>,>>>,>>9.99")) + " - " + trim(ratecode_qsy)
                ...
                for argt_line in db_session.query(Argt_line).filter(
                    ...
                    if qty > 0:
                        ...
                        if argt_line.fakt_modus == 1:
                            ...
                            argt_list = query(argt_list_data, filters=(lambda argt_list: argt_list.argtnr == argt_line.argtnr and argt_list.departement ==
                                              argt_line.departement and argt_list.argt_artnr == argt_line.argt_artnr and argt_list.vt_percnt == argt_line.vt_percnt and argt_list.is_charged == 0), first=True)
                            ...
                            if argt_list.period < argt_line.intervall:
                                ...
                                reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "number1": [(eq, argt_line.departement)], "number2": [(eq, argt_line.argtnr)], "resnr": [
                                                          (eq, res_line.resnr)], "reslinnr": [(eq, res_line.reslinnr)], "number3": [(eq, argt_line.argt_artnr)], "date1": [(le, res_line.abreise)], "date2": [(ge, res_line.ankunft)]})
                    ...
                    if add_it:
                        ...
                        reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "number1": [(eq, argt_line.departement)], "number2": [(eq, argt_line.argtnr)], "resnr": [
                                                  (eq, res_line.resnr)], "reslinnr": [(eq, res_line.reslinnr)], "number3": [(eq, argt_line.argt_artnr)], "date1": [(le, bill_date)], "date2": [(ge, bill_date)]})

                        if reslin_queasy:

                            if argt_line.vt_percnt == 0:
                                query = db_session.query(Reslin_queasy).filter(
                                    (Reslin_queasy.key == 'fargt-line') &
                                    (Reslin_queasy.char1 == '') &
                                    (Reslin_queasy.number1 == argt_line.departement) &
                                    (Reslin_queasy.number2 == argt_line.argtnr) &
                            ...
                            for reslin_queasy in query.all():
                                ...
                                if argt_rate != 0:
                                    ...
                                    output_list.str = to_string(translateExtended(" Incl.", lvcarea, ""), "x(10)") + " " + to_string(
                                        c, "x(16)") + " = " + trim(to_string(argt_rate, "->>>,>>>,>>9.99"))
                                    count_break = to_decimal(
                                        count_break) + to_decimal(argt_rate)

                        if guest_pr and not argt_defined:

                            reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, res_line.reserve_int)], "number2": [(eq, arrangement.argtnr)], "reslinnr": [
                            ...
                            if reslin_queasy:
                                ...
                                if argt_line.vt_percnt == 0:
                                    ...
                                    query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'argt-line') & (Reslin_queasy.char1 == contcode) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (
                                        Reslin_queasy.reslinnr == curr_zikatnr) & (Reslin_queasy.number3 == argt_line.argt_artnr) & (Reslin_queasy.resnr == argt_line.departement) & (Reslin_queasy.date1 <= bill_date) & (Reslin_queasy.date2 >= bill_date) & (Reslin_queasy.deci1 != 0))

                                elif argt_line.vt_percnt == 1:
                                    query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'argt-line') & (Reslin_queasy.char1 == contcode) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (
                                        Reslin_queasy.reslinnr == curr_zikatnr) & (Reslin_queasy.number3 == argt_line.argt_artnr) & (Reslin_queasy.resnr == argt_line.departement) & (Reslin_queasy.date1 <= bill_date) & (Reslin_queasy.date2 >= bill_date) & (Reslin_queasy.deci2 != 0))

                                elif argt_line.vt_percnt == 2:
                                    query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'argt-line') & (Reslin_queasy.char1 == contcode) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (
                                        Reslin_queasy.reslinnr == curr_zikatnr) & (Reslin_queasy.number3 == argt_line.argt_artnr) & (Reslin_queasy.resnr == argt_line.departement) & (Reslin_queasy.date1 <= bill_date) & (Reslin_queasy.date2 >= bill_date) & (Reslin_queasy.deci3 != 0))
                        ...
                        if argt_rate2 > 0:
                            ...
                            if zwkum:
                                argt_rate2 = (to_decimal(
                                    rm_rate) * to_decimal((argt_rate2) / to_decimal(100))) * to_decimal(qty)
                            else:
                                argt_rate2 = (to_decimal(
                        ...
                        if argt_rate == 0:
                            ...
                            c = to_string(qty) + " " + artikel.bezeich
                            output_list.str = to_string(translateExtended(" Incl.", lvcarea, ""), "x(10)") + " " + to_string(
                                c, "x(16)") + " = " + trim(to_string(argt_rate2, "->>>,>>>,>>9.99"))
                            count_break = to_decimal(
            ...
            for argt_line in db_session.query(Argt_line).filter(
                ...
                if qty > 0:
                    ...
                    if argt_line.fakt_modus == 1:
                        ...
                        argt_list = query(argt_list_data, filters=(lambda argt_list: argt_list.argtnr == argt_line.argtnr and argt_list.departement ==
                                          argt_line.departement and argt_list.argt_artnr == argt_line.argt_artnr and argt_list.vt_percnt == argt_line.vt_percnt and argt_list.is_charged == 1), first=True)
                ...
                if add_it:
                    ...
                    if reslin_queasy:
                        ...
                        if argt_line.vt_percnt == 0:
                            query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'fargt-line') & (Reslin_queasy.char1 == '') & (Reslin_queasy.number1 == argt_line.departement) & (Reslin_queasy.number2 == argt_line.argtnr) & (
                                Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argt_line.argt_artnr) & (Reslin_queasy.date1 <= bill_date) & (Reslin_queasy.date2 >= bill_date) & (Reslin_queasy.deci1 != 0))

                        elif argt_line.vt_percnt == 1:
                            query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'fargt-line') & (Reslin_queasy.char1 == '') & (Reslin_queasy.number1 == argt_line.departement) & (Reslin_queasy.number2 == argt_line.argtnr) & (
                                Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argt_line.argt_artnr) & (Reslin_queasy.date1 <= bill_date) & (Reslin_queasy.date2 >= bill_date) & (Reslin_queasy.deci2 != 0))

                        elif argt_line.vt_percnt == 2:
                            query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'fargt-line') & (Reslin_queasy.char1 == '') & (Reslin_queasy.number1 == argt_line.departement) & (Reslin_queasy.number2 == argt_line.argtnr) & (
                                Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argt_line.argt_artnr) & (Reslin_queasy.date1 <= bill_date) & (Reslin_queasy.date2 >= bill_date) & (Reslin_queasy.deci3 != 0))
                        ...
                        for reslin_queasy in query.all():
                            ...
                            if argt_rate != 0:
                                ...
                                output_list.str = to_string("", "x(5)") + "Excl. " + to_string(
                                    artikel.bezeich, "x(16)") + " = " + trim(to_string(argt_rate, "->>>,>>>,>>9.99"))
                                tot_rate = to_decimal(
                                    tot_rate) + to_decimal(argt_rate)
                    ...
                    if argt_rate == 0:
                        ...
                        output_list.str = to_string("", "x(5)") + "Excl. " + to_string(
                            artikel.bezeich, "x(16)") + " = " + trim(to_string(argt_rate2, "->>>,>>>,>>9.99"))
                        tot_rate = to_decimal(
                            tot_rate) + to_decimal(argt_rate2)
                        fixcost_rate = to_decimal(
                            fixcost_rate) + to_decimal(argt_rate2)
            ...
            for fixleist in db_session.query(Fixleist).filter(
                ...
                if argt_rate != 0:
                    ...
                    output_list.str = to_string("", "x(8)") + " " + to_string(
                        artikel.bezeich, "x(16)") + " = " + trim(to_string(argt_rate, "->>>,>>>,>>9.99"))
            ...
            if count_break != 0:
                ...
                output_list.flag = 2
                output_list.str = to_string(translateExtended(
                    " Lodging", lvcarea, ""), "x(11)") + " = " + trim(to_string(daily_rate - count_break, "->>>,>>>,>>9.99"))
            output_list.str = output_list.str + " Total = " + \
                trim(to_string((daily_rate + fixcost_rate), "->>>,>>>,>>9.99"))
        output_list = Output_list()
        output_list_data.append(output_list)
