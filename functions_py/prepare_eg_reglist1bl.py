...
def prepare_eg_reglist1bl(user_init:string):
    ...
    q_133_data, Q_133 = create_model("Q_133", {"number1":int, "char1":string})
    ...
    def generate_output():
        nonlocal ci_date, store_number, groupid, engid, p_992, q_133_data, tpic_data, dept_link_data, queasy, htparam, l_lager, bediener, eg_staff, zimmer, eg_location, eg_property
        nonlocal user_init
        nonlocal qbuff
        ...
        nonlocal q_133_data, tproperty_data, tmaintask_data, tcategory_data, tsource_data, troom_data, tlocation_data, tstatus_data, tpic_data, dept_link_data
    ...
    def create_property():
        ...
        for eg_property in db_session.query(Eg_property).filter(
                 ((Eg_property.location.in_(list(set([comlocat.loc_nr for comlocat in comlocat_data if comlocat.loc_selected])))))).order_by(Eg_property.location).all():
            if eg_property_obj_list.get(eg_property._recid):
                continue
            else:
                eg_property_obj_list[eg_property._recid] = True

            comlocat = query(comlocat_data, (lambda comlocat: (eg_property.location == comlocat.loc_nr)), first=True)

            if comlocat.loc_guest :

                commain = query(commain_data, filters=(lambda commain: commain.main_nr == eg_property.maintask and commain.main_selected), first=True)

                if commain:

                    comroom = query(comroom_data, filters=(lambda comroom: comroom.room_nm == eg_property.zinr and comroom.room_selected), first=True)
