...
from models import Res_line, Exrate, Reservation, Waehrung, Arrangement, Zimkateg, Fixleist

def deposit_admin_1bl(case_type:int, depo_foreign:bool, lname:string, deposittype:int, sorttype:int, lresnr:int, fdate:date, ex_rate:Decimal, bill_date:date, depo_curr:int):

    prepare_cache ([Res_line, Exrate, Reservation, Waehrung, Arrangement, Zimkateg, Fixleist])
    ...
    res_line = exrate = reservation = waehrung = arrangement = zimkateg = fixleist = None

    b1_list = b1_print = depo_list = bresline = b_exrate = None
    ...
    def generate_output():
        nonlocal total_saldo, arriv_saldo, depo_list_data, b1_list_data, b1_print_data, grpstr, res_line, exrate, reservation, waehrung, arrangement, zimkateg, fixleist
        nonlocal case_type, depo_foreign, lname, deposittype, sorttype, lresnr, fdate, ex_rate, bill_date, depo_curr
        nonlocal bresline, b_exrate


        nonlocal b1_list, b1_print, depo_list, bresline, b_exrate
    ...
    def create_itlist():
        ...
        if lname == "":
            ...
            if deposittype == 1:
                ...
                if sorttype == 1:
                    ...
                    reservation = Reservation()
                    res_line = Res_line()
                    for reservation.resnr, reservation.grpflag, reservation.name, reservation.groupname, reservation.depositgef, reservation.depositbez, reservation.depositbez2, reservation.limitdate, reservation.zahldatum, reservation.zahldatum2, reservation.zahlkonto, reservation.zahlkonto2, reservation._recid, res_line.resnr, res_line.reslinnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.zipreis, res_line.erwachs, res_line.resstatus, res_line.arrangement, res_line.zimmer_wunsch, res_line.zikatnr, res_line._recid in db_session.query(Reservation.resnr, Reservation.grpflag, Reservation.name, Reservation.groupname, Reservation.depositgef, Reservation.depositbez, Reservation.depositbez2, Reservation.limitdate, Reservation.zahldatum, Reservation.zahldatum2, Reservation.zahlkonto, Reservation.zahlkonto2, Reservation._recid, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.zipreis, Res_line.erwachs, Res_line.resstatus, Res_line.arrangement, Res_line.zimmer_wunsch, Res_line.zikatnr, Res_line._recid).join(Res_line,(Res_line.resnr == Reservation.resnr) & (Res_line.resstatus <= 8)).filter(
                ...
                if sorttype == 1:
                    ...
                    reservation = Reservation()
                    res_line = Res_line()
                    for reservation.resnr, reservation.grpflag, reservation.name, reservation.groupname, reservation.depositgef, reservation.depositbez, reservation.depositbez2, reservation.limitdate, reservation.zahldatum, reservation.zahldatum2, reservation.zahlkonto, reservation.zahlkonto2, reservation._recid, res_line.resnr, res_line.reslinnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.zipreis, res_line.erwachs, res_line.resstatus, res_line.arrangement, res_line.zimmer_wunsch, res_line.zikatnr, res_line._recid in db_session.query(Reservation.resnr, Reservation.grpflag, Reservation.name, Reservation.groupname, Reservation.depositgef, Reservation.depositbez, Reservation.depositbez2, Reservation.limitdate, Reservation.zahldatum, Reservation.zahldatum2, Reservation.zahlkonto, Reservation.zahlkonto2, Reservation._recid, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.zipreis, Res_line.erwachs, Res_line.resstatus, Res_line.arrangement, Res_line.zimmer_wunsch, Res_line.zikatnr, Res_line._recid).join(Res_line,(Res_line.resnr == Reservation.resnr) & ((Res_line.resstatus <= 5) | (Res_line.resstatus == 9) | (Res_line.resstatus == 10))).filter(
                ...
                if sorttype == 2:
                    ...
                    reservation = Reservation()
                    res_line = Res_line()
                    for reservation.resnr, reservation.grpflag, reservation.name, reservation.groupname, reservation.depositgef, reservation.depositbez, reservation.depositbez2, reservation.limitdate, reservation.zahldatum, reservation.zahldatum2, reservation.zahlkonto, reservation.zahlkonto2, reservation._recid, res_line.resnr, res_line.reslinnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.zipreis, res_line.erwachs, res_line.resstatus, res_line.arrangement, res_line.zimmer_wunsch, res_line.zikatnr, res_line._recid in db_session.query(Reservation.resnr, Reservation.grpflag, Reservation.name, Reservation.groupname, Reservation.depositgef, Reservation.depositbez, Reservation.depositbez2, Reservation.limitdate, Reservation.zahldatum, Reservation.zahldatum2, Reservation.zahlkonto, Reservation.zahlkonto2, Reservation._recid, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.zipreis, Res_line.erwachs, Res_line.resstatus, Res_line.arrangement, Res_line.zimmer_wunsch, Res_line.zikatnr, Res_line._recid).join(Res_line,(Res_line.resnr == Reservation.resnr) & ((Res_line.resstatus <= 5) | (Res_line.resstatus == 9) | (Res_line.resstatus == 10)) & (Res_line.ankunft >= fdate)).filter(
            ...
            if deposittype == 1:
                ...
                if sorttype <= 2:
                    ...
                    reservation = Reservation()
                    res_line = Res_line()
                    for reservation.resnr, reservation.grpflag, reservation.name, reservation.groupname, reservation.depositgef, reservation.depositbez, reservation.depositbez2, reservation.limitdate, reservation.zahldatum, reservation.zahldatum2, reservation.zahlkonto, reservation.zahlkonto2, reservation._recid, res_line.resnr, res_line.reslinnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.zipreis, res_line.erwachs, res_line.resstatus, res_line.arrangement, res_line.zimmer_wunsch, res_line.zikatnr, res_line._recid in db_session.query(Reservation.resnr, Reservation.grpflag, Reservation.name, Reservation.groupname, Reservation.depositgef, Reservation.depositbez, Reservation.depositbez2, Reservation.limitdate, Reservation.zahldatum, Reservation.zahldatum2, Reservation.zahlkonto, Reservation.zahlkonto2, Reservation._recid, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.zipreis, Res_line.erwachs, Res_line.resstatus, Res_line.arrangement, Res_line.zimmer_wunsch, Res_line.zikatnr, Res_line._recid).join(Res_line,(Res_line.resnr == Reservation.resnr) & (Res_line.resstatus <= 8) & (Res_line.ankunft >= fdate) & (matches(Res_line.name,("*" + lname + "*")))).filter(
                    ...
                    reservation = Reservation()
                    res_line = Res_line()
                    for reservation.resnr, reservation.grpflag, reservation.name, reservation.groupname, reservation.depositgef, reservation.depositbez, reservation.depositbez2, reservation.limitdate, reservation.zahldatum, reservation.zahldatum2, reservation.zahlkonto, reservation.zahlkonto2, reservation._recid, res_line.resnr, res_line.reslinnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.zipreis, res_line.erwachs, res_line.resstatus, res_line.arrangement, res_line.zimmer_wunsch, res_line.zikatnr, res_line._recid in db_session.query(Reservation.resnr, Reservation.grpflag, Reservation.name, Reservation.groupname, Reservation.depositgef, Reservation.depositbez, Reservation.depositbez2, Reservation.limitdate, Reservation.zahldatum, Reservation.zahldatum2, Reservation.zahlkonto, Reservation.zahlkonto2, Reservation._recid, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.zipreis, Res_line.erwachs, Res_line.resstatus, Res_line.arrangement, Res_line.zimmer_wunsch, Res_line.zikatnr, Res_line._recid).join(Res_line,(Res_line.resnr == Reservation.resnr) & (Res_line.resstatus <= 8) & (matches(Res_line.name,("*" + lname + "*")))).filter(
                             (Reservation.activeflag == 0) & (Reservation.depositgef != 0) & (Reservation.resnr >= lresnr) & (Reservation.limitdate >= fdate)).order_by(Reservation.limitdate, Reservation.name).all():
                        if reservation_obj_list.get(reservation._recid):
                            continue
                        else:
                ...
                if sorttype <= 2:
                    ...
                    reservation = Reservation()
                    res_line = Res_line()
                    for reservation.resnr, reservation.grpflag, reservation.name, reservation.groupname, reservation.depositgef, reservation.depositbez, reservation.depositbez2, reservation.limitdate, reservation.zahldatum, reservation.zahldatum2, reservation.zahlkonto, reservation.zahlkonto2, reservation._recid, res_line.resnr, res_line.reslinnr, res_line.name, res_line.ankunft, res_line.abreise, res_line.zimmeranz, res_line.zipreis, res_line.erwachs, res_line.resstatus, res_line.arrangement, res_line.zimmer_wunsch, res_line.zikatnr, res_line._recid in db_session.query(Reservation.resnr, Reservation.grpflag, Reservation.name, Reservation.groupname, Reservation.depositgef, Reservation.depositbez, Reservation.depositbez2, Reservation.limitdate, Reservation.zahldatum, Reservation.zahldatum2, Reservation.zahlkonto, Reservation.zahlkonto2, Reservation._recid, Res_line.resnr, Res_line.reslinnr, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.zimmeranz, Res_line.zipreis, Res_line.erwachs, Res_line.resstatus, Res_line.arrangement, Res_line.zimmer_wunsch, Res_line.zikatnr, Res_line._recid).join(Res_line,(Res_line.resnr == Reservation.resnr) & ((Res_line.resstatus <= 5) | (Res_line.resstatus == 9) | (Res_line.resstatus == 10)) & (matches(Res_line.name,("*" + lname + "*")))).filter(
        ...
        if case_type == 1:
            ...
            if depo_foreign:
                ...
                total_saldo =  to_decimal(total_saldo) * to_decimal(ex_rate)
                arriv_saldo =  to_decimal(arriv_saldo) * to_decimal(ex_rate)
    ...
    def create_depo():
        ...
        if depo_list.datum1 < bill_date and depo_list.datum1 != None:
            ...
            b_exrate = get_cache (Exrate, {"artnr": [(eq, depo_curr)],"datum": [(eq, depo_list.datum1)]})

            if b_exrate:
                exchg_rate =  to_decimal(b_exrate.betrag)
            else:
                found_it = False
        ...
        if depo_list.datum2 < bill_date and depo_list.datum2 != None:
            ...
            b_exrate = get_cache (Exrate, {"artnr": [(eq, depo_curr)],"datum": [(eq, depo_list.datum2)]})
    ...
    def create_b1():
        ...
        for datum in date_range(res_line.ankunft,tmp_date) :
...
def deposit_admin_1bl(case_type:int, depo_foreign:bool, lname:string, deposittype:int, sorttype:int, lresnr:int, fdate:date, exrate:Decimal, bill_date:date, depo_curr:int):
    ...
    def generate_output():
        ...
        nonlocal case_type, depo_foreign, lname, deposittype, sorttype, lresnr, fdate, exrate, bill_date, depo_curr
        nonlocal bresline
        ...
        if case_type == 1:
            ...
            if depo_foreign:
                ...
                for b1_list in query(b1_list_data):
                    ...
                    if not res_line:
                        arriv_saldo =  to_decimal(arriv_saldo) + to_decimal(b1_list.depositbez) + to_decimal(b1_list.depositbez2)
                total_saldo =  to_decimal(total_saldo) * to_decimal(exrate)
                arriv_saldo =  to_decimal(arriv_saldo) * to_decimal(exrate)
