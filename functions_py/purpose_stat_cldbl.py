...
from models import Queasy, Nation, Guest, Genstat, Zimmer, Zimkateg, Segment


def purpose_stat_cldbl(pvilanguage: int, mi_comp1: bool, mi_ftd1: bool, f_date: date, t_date: date, to_date: date, cardtype: int, price_decimal: int, display_opt: int, idflag: str):

    prepare_cache([Nation, Guest, Genstat, Zimkateg])
    ...
    nat_name = ""
    counter: int = 0
    lvcarea: str = "purpuse-stat-web"
    queasy = nation = guest = genstat = zimmer = zimkateg = segment = None

    pstay_list = s_list = t_queasy = nat_buff = bqueasy = pstay_buff = sbuff = None
    ...
    Nat_buff = create_buffer("Nat_buff", Nation)
    Bqueasy = create_buffer("Bqueasy", Queasy)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal pstay_list_data, t_queasy_data, mm, yy, from_date, datum, do_it, i, pur_nr, num, s, curr_code, incl_comp, room, c_room, pax, logis, avrgrate, proz, m_room, mc_room, m_pax, m_logis, m_avrgrate, m_proz, y_room, yc_room, y_pax, y_logis, y_avrgrate, y_proz, st_room, stc_room, st_pax, st_logis, st_avrgrate, st_proz, stm_room, stmc_room, stm_pax, stm_logis, stm_avrgrate, stm_proz, sty_room, styc_room, sty_pax, sty_logis, sty_avrgrate, sty_proz, gt_room, gtc_room, gt_pax, gt_logis, gt_avrgrate, gtm_room, gtmc_room, gtm_pax, gtm_logis, gtm_avrgrate, gtm_proz, gty_room, gtyc_room, gty_pax, gty_logis, gty_avrgrate, gty_proz, nat_code, nat_name, counter, lvcarea, queasy, nation, guest, genstat, zimmer, zimkateg, segment
        nonlocal pvilanguage, mi_comp1, mi_ftd1, f_date, t_date, to_date, cardtype, price_decimal, display_opt, idflag
        nonlocal nat_buff, bqueasy
        nonlocal pstay_list, s_list, t_queasy, nat_buff, bqueasy, pstay_buff, sbuff
    ...
    def create_output():
        ...
        if display_opt >= 0:
            for s_list in query(s_list_data, sort_by=[("purnr", True)]):
                if i != s_list.purnr and sub_section != None:
                    create_sub()

                if sub_section != s_list.purnr:
                    pstay_list = Pstay_list()
                    pstay_list_data.append(pstay_list)

                    pstay_list.purstr = s_list.purstr
                    sub_section = s_list.purnr
                    i = s_list.purnr
                ...
                if price_decimal == 0:
                    ...
                    count_sub1_by_nat_domestic()
                else:
                    pstay_list = Pstay_list()
                    pstay_list_data.append(pstay_list)

                    count_sub2_by_nat_domestic()
    ...
    def count_sub1():
        ...
        pstay_list.m_avrgrate = to_string(
            s_list.m_avrgrate, "->>,>>>,>>>,>>>,>>9")
        ...
        pstay_list.y_avrgrate = to_string(
            s_list.y_avrgrate, "->>,>>>,>>>,>>>,>>9")
    ...
    def create_sub():
        ...
        if (st_room - stc_room) != 0:
            st_avrgrate = to_decimal(
                st_logis) / to_decimal((st_room) - to_decimal(stc_room))
        stm_avrgrate = to_decimal("0")

        if (stm_room - stmc_room) != 0:
            stm_avrgrate = to_decimal(
                stm_logis) / to_decimal((stm_room) - to_decimal(stmc_room))
        sty_avrgrate = to_decimal("0")

        if (sty_room - styc_room) != 0:
            sty_avrgrate = to_decimal(
                sty_logis) / to_decimal((sty_room) - to_decimal(styc_room))

        if price_decimal == 0:
            pstay_list.purstr = translateExtended(
                "s u b T o t a l", lvcarea, "")
            ...
            pstay_list.proz = to_string(st_proz, ">>9.99")
            pstay_list.avrgrate = to_string(
                st_avrgrate, "->>>,>>>,>>>,>>9.99")
            pstay_list.m_room = to_string(stm_room, "->>>,>>9")
            pstay_list.m_pax = to_string(stm_pax, "->>>,>>9")
            pstay_list.m_logis = to_string(
                stm_logis, "->>>,>>>,>>>,>>9.99")
            pstay_list.m_proz = to_string(stm_proz, ">>9.99")
            pstay_list.m_avrgrate = to_string(
                stm_avrgrate, "->>>,>>>,>>>,>>9.99")
    ...
    def count_sub1_by_nat_domestic():
        nonlocal pstay_list_data, t_queasy_data, mm, yy, from_date, datum, do_it, i, pur_nr, num, s, curr_code, incl_comp, room, c_room, pax, logis, avrgrate, proz, m_room, mc_room, m_pax, m_logis, m_avrgrate, m_proz, y_room, yc_room, y_pax, y_logis, y_avrgrate, y_proz, st_room, stc_room, st_pax, st_logis, st_avrgrate, st_proz, stm_room, stmc_room, stm_pax, stm_logis, stm_avrgrate, stm_proz, sty_room, styc_room, sty_pax, sty_logis, sty_avrgrate, sty_proz, gt_room, gtc_room, gt_pax, gt_logis, gt_avrgrate, gtm_room, gtmc_room, gtm_pax, gtm_logis, gtm_avrgrate, gtm_proz, gty_room, gtyc_room, gty_pax, gty_logis, gty_avrgrate, gty_proz, nat_code, nat_name, counter, lvcarea, queasy, nation, guest, genstat, zimmer, zimkateg, segment
        nonlocal pvilanguage, mi_comp1, mi_ftd1, f_date, t_date, to_date, cardtype, price_decimal, display_opt, idflag
        nonlocal nat_buff, bqueasy
        nonlocal pstay_list, s_list, t_queasy, nat_buff, bqueasy, pstay_buff, sbuff
        ...
        pstay_list.purstr = s_list.nat_bezeich
        pstay_list.room = to_string(s_list.room, "->>>,>>9")
        pstay_list.pax = to_string(s_list.pax, "->>>,>>9")
        pstay_list.logis = to_string(s_list.logis, "->>,>>>,>>>,>>>,>>9")
        pstay_list.proz = to_string(s_list.proz, ">>9.99")
    ...
    def count_sub2_by_nat_domestic():
        nonlocal pstay_list_data, t_queasy_data, mm, yy, from_date, datum, do_it, i, pur_nr, num, s, curr_code, incl_comp, room, c_room, pax, logis, avrgrate, proz, m_room, mc_room, m_pax, m_logis, m_avrgrate, m_proz, y_room, yc_room, y_pax, y_logis, y_avrgrate, y_proz, st_room, stc_room, st_pax, st_logis, st_avrgrate, st_proz, stm_room, stmc_room, stm_pax, stm_logis, stm_avrgrate, stm_proz, sty_room, styc_room, sty_pax, sty_logis, sty_avrgrate, sty_proz, gt_room, gtc_room, gt_pax, gt_logis, gt_avrgrate, gtm_room, gtmc_room, gtm_pax, gtm_logis, gtm_avrgrate, gtm_proz, gty_room, gtyc_room, gty_pax, gty_logis, gty_avrgrate, gty_proz, nat_code, nat_name, counter, lvcarea, queasy, nation, guest, genstat, zimmer, zimkateg, segment
        nonlocal pvilanguage, mi_comp1, mi_ftd1, f_date, t_date, to_date, cardtype, price_decimal, display_opt, idflag
        nonlocal nat_buff, bqueasy
        nonlocal pstay_list, s_list, t_queasy, nat_buff, bqueasy, pstay_buff, sbuff
    ...
    if display_opt == 1:
        ...
        genstat = Genstat()
        guest = Guest()
        for genstat.zinr, genstat.zikatnr, genstat.segmentcode, genstat.nationnr, genstat.resstatus, genstat.datum, genstat.res_char, genstat.erwachs, genstat.gratis, genstat.kind1, genstat.kind2, genstat.kind3, genstat.logis, genstat.ratelocal, genstat.domestic, genstat.zipreis, genstat._recid, guest.karteityp, guest._recid in db_session.query(Genstat.zinr, Genstat.zikatnr, Genstat.segmentcode, Genstat.nationnr, Genstat.resstatus, Genstat.datum, Genstat.res_char, Genstat.erwachs, Genstat.gratis, Genstat.kind1, Genstat.kind2, Genstat.kind3, Genstat.logis, Genstat.ratelocal, Genstat.domestic, Genstat.zipreis, Genstat._recid, Guest.karteityp, Guest._recid).join(Guest, (Guest.gastnr == Genstat.gastnr)).filter(
            ...
            if nation:
                nat_code = nation.kurzbez
                nat_name = nation.bezeich
            ...
            if do_it:
                ...
                for num in range(1, num_entries(genstat.res_char[1], ";") + 1):
                    ...
                    if matches(s, r"SEGM_PUR*"):
                        pur_nr = to_int(
                            substring(s, get_index(s, "SEGM_PUR") + 8 - 1))

                s_list = query(s_list_data, filters=(lambda s_list: s_list.purnr == pur_nr and s_list.nat_kurbez.lower() == (nat_code).lower()), first=True)
                ...
                if not s_list:
                    ...
                    s_list.nat_bezeich = nat_name
                    s_list.nat_kurbez = nat_code

                    queasy = get_cache(
                        Queasy, {"key": [(eq, 143)], "number1": [(eq, pur_nr)]})
                    ...
                    if queasy:
                        ...
                        s_list.purstr = translateExtended(
                            "UNKNOWN", lvcarea, "")
                ...
                if genstat.datum == to_date:
                    ...
                    s_list.pax = s_list.pax + genstat.erwachs + genstat.gratis + \
                        genstat.kind1 + genstat.kind2 + genstat.kind3
                    s_list.logis = to_decimal(
                        s_list.logis) + to_decimal(genstat.logis)
                    pax = pax + genstat.erwachs + genstat.gratis + \
                        genstat.kind1 + genstat.kind2 + genstat.kind3
                    logis = to_decimal(logis) + to_decimal(genstat.logis)
                    avrgrate = to_decimal(avrgrate) + \
                        to_decimal(genstat.ratelocal)
                ...
                if get_month(genstat.datum) == mm and get_year(genstat.datum) == yy:
                    ...
                    s_list.m_pax = s_list.m_pax + genstat.erwachs + \
                        genstat.gratis + genstat.kind1 + genstat.kind2 + genstat.kind3
                    s_list.m_logis = to_decimal(
                        s_list.m_logis) + to_decimal(genstat.logis)
                    m_pax = m_pax + genstat.erwachs + genstat.gratis + \
                        genstat.kind1 + genstat.kind2 + genstat.kind3
                    m_logis = to_decimal(m_logis) + to_decimal(genstat.logis)
                    m_avrgrate = to_decimal(
                        m_avrgrate) + to_decimal(genstat.ratelocal)
                ...
                s_list.y_pax = s_list.y_pax + genstat.erwachs + \
                    genstat.gratis + genstat.kind1 + genstat.kind2 + genstat.kind3
                s_list.y_logis = to_decimal(
                    s_list.y_logis) + to_decimal(genstat.logis)
                y_pax = y_pax + genstat.erwachs + genstat.gratis + \
                    genstat.kind1 + genstat.kind2 + genstat.kind3
                y_logis = to_decimal(y_logis) + to_decimal(genstat.logis)
                y_avrgrate = to_decimal(y_avrgrate) + \
                    to_decimal(genstat.ratelocal)

                for s_list in query(s_list_data):
                    if (s_list.room - s_list.c_room) != 0:
                        s_list.avrgrate = to_decimal(
                            s_list.logis) / to_decimal((s_list.room) - to_decimal(s_list.c_room))

                    if (s_list.m_room - s_list.mc_room) != 0:
                        s_list.m_avrgrate = to_decimal(
                            s_list.m_logis) / to_decimal((s_list.m_room) - to_decimal(s_list.mc_room))

                    if (s_list.y_room - s_list.yc_room) != 0:
                        s_list.y_avrgrate = to_decimal(
                            s_list.y_logis) / to_decimal((s_list.y_room) - to_decimal(s_list.yc_room))

                    if logis != 0:
                        s_list.proz = to_decimal(
                            s_list.logis) / to_decimal(logis) * to_decimal("100")

                    if m_logis != 0:
                        s_list.m_proz = to_decimal(
                            s_list.m_logis) / to_decimal(m_logis) * to_decimal("100")

                    if y_logis != 0:
                        s_list.y_proz = to_decimal(
                            s_list.y_logis) / to_decimal(y_logis) * to_decimal("100")
                gt_room = 0
                gtc_room = 0
                gt_pax = 0
                gt_logis = to_decimal("0")
                ...
                for s_list in query(s_list_data):
                    ...
                    gt_avrgrate = to_decimal(
                        gt_avrgrate) + to_decimal(s_list.avrgrate)
                    gtm_room = gtm_room + s_list.m_room
                    gtmc_room = gtmc_room + s_list.mc_room
                    gtm_pax = gtm_pax + s_list.m_pax
                    gtm_logis = to_decimal(gtm_logis) + \
                        to_decimal(s_list.m_logis)
                    gtm_avrgrate = to_decimal(
                        gtm_avrgrate) + to_decimal(s_list.m_avrgrate)
                    gty_room = gty_room + s_list.y_room
                    gtyc_room = gtyc_room + s_list.yc_room
                    gty_pax = gty_pax + s_list.y_pax
                    gty_logis = to_decimal(gty_logis) + \
                        to_decimal(s_list.y_logis)
                    gty_avrgrate = to_decimal(
                        gty_avrgrate) + to_decimal(s_list.y_avrgrate)
        ...
        for genstat.zinr, genstat.zikatnr, genstat.segmentcode, genstat.nationnr, genstat.resstatus, genstat.datum, genstat.res_char, genstat.erwachs, genstat.gratis, genstat.kind1, genstat.kind2, genstat.kind3, genstat.logis, genstat.ratelocal, genstat.domestic, genstat.zipreis, genstat._recid, guest.karteityp, guest._recid in db_session.query(Genstat.zinr, Genstat.zikatnr, Genstat.segmentcode, Genstat.nationnr, Genstat.resstatus, Genstat.datum, Genstat.res_char, Genstat.erwachs, Genstat.gratis, Genstat.kind1, Genstat.kind2, Genstat.kind3, Genstat.logis, Genstat.ratelocal, Genstat.domestic, Genstat.zipreis, Genstat._recid, Guest.karteityp, Guest._recid).join(Guest, (Guest.gastnr == Genstat.gastnr)).filter(
            ...
            nation = get_cache(Nation, {"nationnr": [(eq, genstat.domestic)]})
