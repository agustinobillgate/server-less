...
def mkres_ratesbl(from_date:date, user_init:string, chg_zikat:bool, chg_flag:bool, reslin_list_data:[Reslin_list], room_list_data:[Room_list], res_dynarate_data:[Res_dynarate]):
    ...
    t_rqy_data, T_rqy = create_model_like(Reslin_queasy, {"count_i":int})
    ...
    def generate_output():
        ...
        return {"ratecode_bez": ratecode_bez, "new_segm": new_segm, "fixed_rate": fixed_rate, "room_rate": room_rate, "Res-Dynarate": res_dynarate_data}
    ...
    room_list = query(room_list_data, first=True)
    ...
    for t_rqy in query(t_rqy_data, sort_by=[("count_i",False)]):
        reslin_queasy = Reslin_queasy()
        db_session.add(reslin_queasy)
    ...
    if room_list.dynarate:
        ...
        for curr_i in range(1,(abreise - from_date)  + 1) :
            ...
            res_dynarate_data.append(res_dynarate)
    ...
    if reslin_list.active_flag == 0:
        res_dynarate_data = get_output(check_bonus_nightbl(reslin_list.ankunft, reslin_list.abreise, res_dynarate_data))
