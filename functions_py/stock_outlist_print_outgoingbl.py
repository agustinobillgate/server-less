...
from models import L_op, L_ophdr, L_artikel, Gl_acct, Queasy

def stock_outlist_print_outgoingbl(s_op_recid:int, from_grp:int, show_price:bool):

    prepare_cache ([L_op, L_ophdr, L_artikel, Gl_acct, Queasy])
    ...
    l_op = l_ophdr = l_artikel = gl_acct = queasy = None

    print_list = l_op1 = None

    print_list_data, Print_list = create_model("Print_list", {"datum":date, "lager_nr":int, "artnr":int, "bezeich":string, "anzahl":Decimal, "gl_bezeich":string, "preis":Decimal, "wert":Decimal, "remark":string})
    ...
    def generate_output():
        nonlocal tot_amount, l_op1_lscheinnr, print_list_data, preis, wert, l_op, l_ophdr, l_artikel, gl_acct, queasy
    ...
    if l_op1:
        ...
        if from_grp == 0:
            ...
            l_op = L_op()
            l_artikel = L_artikel()
            for l_op.einzelpreis, l_op.warenwert, l_op.stornogrund, l_op.datum, l_op.lager_nr, l_op.artnr, l_op.anzahl, l_op.lscheinnr, l_op._recid, l_artikel.bezeich, l_artikel._recid in db_session.query(L_op.einzelpreis, L_op.warenwert, L_op.stornogrund, L_op.datum, L_op.lager_nr, L_op.artnr, L_op.anzahl, L_op.lscheinnr, L_op._recid, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter(
                ...
                if queasy:
                    print_list.remark = queasy.char2
                else:
            ...
            l_op = L_op()
            l_artikel = L_artikel()
            for l_op.einzelpreis, l_op.warenwert, l_op.stornogrund, l_op.datum, l_op.lager_nr, l_op.artnr, l_op.anzahl, l_op.lscheinnr, l_op._recid, l_artikel.bezeich, l_artikel._recid in db_session.query(L_op.einzelpreis, L_op.warenwert, L_op.stornogrund, L_op.datum, L_op.lager_nr, L_op.artnr, L_op.anzahl, L_op.lscheinnr, L_op._recid, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr) & (L_artikel.endkum == from_grp)).filter(
                     (L_op.datum == l_op1.datum) & (L_op.op_art == 3) & (L_op.lscheinnr == l_op1.lscheinnr) & (L_op.loeschflag <= 1)).order_by(l_artikel.bezeich).all():
                if l_op_obj_list.get(l_op._recid):
                    continue
                else:
