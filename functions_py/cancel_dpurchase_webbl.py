...
from models import Bediener, Htparam, L_op, L_artikel, Queasy, L_bestand, L_liefumsatz, Gl_acct, L_kredit, Ap_journal

def cancel_dpurchase_webbl(lief_nr:int, docu_nr:string, user_init:string):

    prepare_cache ([Bediener, Htparam, L_op, L_artikel, Queasy, L_bestand, L_liefumsatz])
    ...
    doc_nr:string = ""
    bediener = htparam = l_op = l_artikel = queasy = l_bestand = l_liefumsatz = gl_acct = l_kredit = ap_journal = None

    db_session = local_storage.db_session

    def generate_output():
        nonlocal f_endkum, b_endkum, m_endkum, billdate, fb_closedate, m_closedate, negative_oh, lscheinnr, curr_lager, anzahl, wert, t_amount, tot_anz, tot_wert, doc_nr, bediener, htparam, l_op, l_artikel, queasy, l_bestand, l_liefumsatz, gl_acct, l_kredit, ap_journal
