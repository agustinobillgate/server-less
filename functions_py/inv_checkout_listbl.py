...
# - update from Alder: Centralized Guest
...
from models import Res_line, Bill, Guest, Bediener, Queasy, Reservation, Htparam, Outorder, Reslin_queasy, Bill_line, Zimmer, Guestat, Akt_cust, Salestat, Res_history, Zimplan, Resplan, Mealcoup, Interface, Master, Zimkateg


def inv_checkout_listbl(pvilanguage: int, case_type: int, resnr: int, reslinnr: int, user_init: str, reason_str: str, silenzio: bool):

    prepare_cache([Res_line, Bill, Guest, Bediener, Queasy, Reservation, Htparam, Outorder, Reslin_queasy, Bill_line, Zimmer, Guestat, Akt_cust, Salestat, Res_history, Resplan, Mealcoup, Interface, Master])
    ...
    centralized_flag: bool = False  # Alder: Centralized Guest
    lvcarea: str = "inv-checkout-listbl"
    bl_saldo: Decimal = to_decimal("0.0")
    res_line = bill = guest = bediener = queasy = reservation = htparam = outorder = reslin_queasy = bill_line = zimmer = guestat = akt_cust = salestat = res_history = zimplan = resplan = mealcoup = interface = master = zimkateg = None

    resline = mbill = rguest = rline = bline = buf_rline = bqueasy = tbuff = None
    ...
    Bqueasy = create_buffer("Bqueasy", Queasy)  # Alder: Centralized Guest
    ...
    def check_billstatus():
        ...
        if unbalanced_bill == False:
            bill1 = get_cache(
                Bill, {"resnr": [(eq, resnr)], "reslinnr": [(eq, reslinnr)], "flag": [(eq, 0)], "zinr": [(eq, zinr)]})
            ...
            if bill1:
                ...
                for bill1 in db_session.query(Bill1).filter(
                    ...
                    bill_line = get_cache(
                        Bill_line, {"rechnr": [(eq, bill1.rechnr)]})
        ...
        if htparam.fchar != "":
            resline = get_cache(
                Res_line, {"resnr": [(eq, resnr)], "active_flag": [(eq, 1)], "resstatus": [(ne, 12)], "reslinnr": [(ne, reslinnr)]})

            if not resline:
                mbill = get_cache(
                    Bill, {"resnr": [(eq, resnr)], "zinr": [(eq, "")], "flag": [(eq, 0)]})
    ...
    def check_co_time():
        ...
        if htparam.feldtyp == 4 and htparam.flogical:
            ...
            if (co_datum == bill_date) and (zeit > co_zeit):
                msg_int = 4
                msg_str = translateExtended("C/O Time", lvcarea, "") + " " + to_string(co_zeit, "HH:MM:SS") + \
                    " " + translateExtended("exceeded. Current time is", lvcarea, "") + \
                    " " + to_string(zeit, "HH:MM:SS") + chr_unicode(10) + \
                    translateExtended("Exceeded time =", lvcarea, "") + \
                    " " + to_string((zeit - co_zeit), "HH:MM:SS") + chr_unicode(10)
    ...
    def guest_checkout():
        ...
        reslin_queasy = get_cache(
            Reslin_queasy, {"key": [(eq, "flag")], "resnr": [(eq, resnr)], "reslinnr": [(eq, reslinnr)], "betriebsnr": [(eq, 0)]})
        ...
        if res_line.betrieb_gast > 0:
            msg_int = 5
            msg_str = translateExtended("Number of created KeyCard(s) =", lvcarea, "") + \
                " " + to_string(res_line.betrieb_gast) + chr_unicode(10)
        ...
        for sharer in db_session.query(Sharer).filter(
            ...
            if real_guest:
                get_output(create_historybl(
                    sharer.resnr, sharer.reslinnr, sharer.zinr, "checkout", user_init, ""))
        ...
        for bill1 in db_session.query(Bill1).filter(
            ...
            if tot_umsatz != 0:
                ...
                guest.argtumsatz = to_decimal(
                    guest.argtumsatz) + to_decimal(bill1.argtumsatz)
                guest.f_b_umsatz = to_decimal(
                    guest.f_b_umsatz) + to_decimal(bill1.f_b_umsatz)
                guest.sonst_umsatz = to_decimal(
                    guest.sonst_umsatz) + to_decimal(bill1.sonst_umsatz)
                ...
                guestat.logisumsatz = to_decimal(
                    guestat.logisumsatz) + to_decimal(bill1.logisumsatz)
                guestat.argtumsatz = to_decimal(
                    guestat.argtumsatz) + to_decimal(bill1.argtumsatz)
                guestat.f_b_umsatz = to_decimal(
                    guestat.f_b_umsatz) + to_decimal(bill1.f_b_umsatz)
                guestat.sonst_umsatz = to_decimal(
                    guestat.sonst_umsatz) + to_decimal(bill1.sonst_umsatz)
                guestat.gesamtumsatz = to_decimal(
                    guestat.gesamtumsatz) + to_decimal(bill1.gesamtumsatz)

                akt_cust = get_cache(
                    Akt_cust, {"gastnr": [(eq, res_line.gastnr)]})

                if akt_cust:
                    bediener = get_cache(
                        Bediener, {"userinit": [(eq, akt_cust.userinit)]})
                ...
                if bediener:
                    ...
                    salestat.logisumsatz = to_decimal(
                        salestat.logisumsatz) + to_decimal(bill1.logisumsatz)
                    salestat.argtumsatz = to_decimal(
                        salestat.argtumsatz) + to_decimal(bill1.argtumsatz)
                    salestat.f_b_umsatz = to_decimal(
                        salestat.f_b_umsatz) + to_decimal(bill1.f_b_umsatz)
                    salestat.sonst_umsatz = to_decimal(
                        salestat.sonst_umsatz) + to_decimal(bill1.sonst_umsatz)
                    salestat.gesamtumsatz = to_decimal(
                        salestat.gesamtumsatz) + to_decimal(bill1.gesamtumsatz)
        ...
        msg_str = translateExtended(
            "Guest checked-out.", lvcarea, "") + chr_unicode(10)
        ...
        if not silenzio:
            ...
            if resline:
                if resline.resstatus == 6:
                    statusstr = translateExtended(
                        "Status: Main Guest; RmRate =", lvcarea, "")
                else:
                    statusstr = translateExtended(
                        "Status: Room Sharer; RmRate =", lvcarea, "")
                    msg_int = 7
                    msg_str = translateExtended("Other Expected Departure Guest FOUND in the same room:", lvcarea, "") + \
                        chr_unicode(10) + \
                        resline.name + " " + to_string(resline.ankunft) + \
                        " - " + to_string(resline.abreise) + \
                        chr_unicode(10) + \
                        statusstr + " " + to_string(resline.zipreis)

        mbill = get_cache(
            Bill, {"resnr": [(eq, resnr)], "zinr": [(eq, "")]})

        if not mbill and res_line.l_zuordnung[4] != 0 and res_line.l_zuordnung[1] == 0:
            mbill = get_cache(
                Bill, {"resnr": [(eq, res_line.l_zuordnung[4])], "zinr": [(eq, "")], "flag": [(eq, 0)]})

        if mbill:
            resline = get_cache(
                Res_line, {"resnr": [(eq, mbill.resnr)], "active_flag": [(le, 1)]})
        ...
        if goto_master:
            msg_int = 8
            msg_str = translateExtended("All guests are checked-out, close the master bill NOW?", lvcarea, "") + chr_unicode(10) + \
                translateExtended("BillNo :", lvcarea, "") + \
                " " + to_string(mbill.rechnr) + " " + \
                translateExtended("- ResNo :", lvcarea, "") + \
                " " + to_string(mbill.resnr) + chr_unicode(10)
# start - Alder: Centralized Guest
        if centralized_flag:
            centralized_guest_checkout_realtime(resnr, reslinnr)
# end Alder: Centralized Guest
    ...
    def centralized_guest_checkout_realtime(resnr: int, reslinnr: int):
        ...
        if bqueasy:
            ...
            bqueasy.key = 373
            bqueasy.number1 = resnr
            bqueasy.number2 = reslinnr
            bqueasy.logi1 = True
            bqueasy.logi2 = False
...
# end Alder: Centralized Guest
    ...
    if case_type == 1:
        ...
        if rline:
            ...
            outorder = get_cache(Outorder, {"zinr": [(eq, res_line.zinr)], "gespstart": [
                                 (le, co_date)], "gespende": [(ge, co_date)], "betriebsnr": [(le, 1)]})

            if outorder:
                msg_int = 1
                msg_str = translateExtended("Out-Of-Order Record found for this room:", lvcarea, "") + \
                    " " + to_string(outorder.gespstart) + "-" + to_string(outorder.gespende) + chr_unicode(10) + \
            ...
            if early_co:
                if substring(bediener.permissions, 69, 1) >= ("2"):
                    msg_int = 2
                    msg_str = translateExtended("Early Check-out", lvcarea, "") + \
                        " " + res_line.name + chr_unicode(10) + \
                        translateExtended("ROOM", lvcarea, "") + " " + \
                        res_line.zinr + " ?" + chr_unicode(10)
                else:
                    msg_str = translateExtended(
                        "No Access Right for Early Checkout [70,2].", lvcarea, "") + chr_unicode(10)
                    error_code = 4
            else:
                msg_int = 3
                msg_str = translateExtended("Check-out", lvcarea, "") + \
                    " " + res_line.name + chr_unicode(10) + \
                    translateExtended("ROOM", lvcarea, "") + " " + \
                    res_line.zinr + " ?" + chr_unicode(10)
