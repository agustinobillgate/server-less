...
def chg_sarticle_btn_gobl(pvilanguage:int, tt_artnr_data:[Tt_artnr], tt_content_data:[Tt_content], s_unit:string, artnr:int, t_recid:int, fibukonto:string, dml_art_inp:bool, bez_aend:bool, picture_file:string, original_art:int, user_init:string, l_art_data:[L_art]):

    prepare_cache ([L_artikel, L_untergrup, Bediener, Res_history, L_op, L_ophis, L_order, L_bestand, L_lager, L_verbrauch, L_pprice, H_rezlin])
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, s_unit, artnr, t_recid, fibukonto, dml_art_inp, bez_aend, picture_file, original_art, user_init
    ...
    def update_l_artikel():
        ...
        if l_artikel:
            ...
            l_artikel.bestellt = dml_art_inp
    ...
    def update_artnr():
        ...
        while None != l_besthis:
            ...
            l_besthis = db_session.query(L_besthis).filter(
                     (L_besthis.artnr == original_art) & (L_besthis._recid > curr_recid)).first()
        ...
        while None != dml_art:
            ...
            dml_art = db_session.query(Dml_art).filter(
                     (Dml_art.artnr == original_art) & (Dml_art._recid > curr_recid)).first()

        dml_artdep = get_cache (Dml_artdep, {"artnr": [(eq, original_art)]})
        while None != dml_artdep:
            ...
            dml_artdep = db_session.query(Dml_artdep).filter(
                     (Dml_artdep.artnr == original_art) & (Dml_artdep._recid > curr_recid)).first()


    l_art = query(l_art_data, first=True)

    for tt_artnr in query(tt_artnr_data):
        ss_artnr[tt_artnr.curr_i - 1] = tt_artnr.ss_artnr

    for tt_content in query(tt_content_data):
    ...
    if artnr == 0:
        ...
        l_artikel = get_cache (L_artikel, {"artnr": [(eq, artnr)],"_recid": [(ne, t_recid)]})

        if l_artikel:
            str_msg = translateExtended ("Article Number ", lvcarea, "") + to_string(artnr) + " - " + l_artikel.bezeich + chr_unicode(2) + "already exists"
        else:
