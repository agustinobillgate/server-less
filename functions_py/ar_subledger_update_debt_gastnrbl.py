...
def ar_subledger_update_debt_gastnrbl(age_list_data:[Age_list], t_artnr:int, t_dept:int, gastpay:int, a_gastnr:int, a_rechnr:int, user_init:string):
    ...
    def update_debt_gastnr():
        ...
        for age_list in query(age_list_data, filters=(lambda age_list: age_list.rechnr == a_rechnr and age_list.gastnr == a_gastnr)):
        ...
        for debitor in db_session.query(Debitor).filter(
            ...
            if debitor.betriebsnr == 0:
                ...
                debt = db_session.query(Debt).filter(Debt._recid == debitor._recid).with_for_update().first()
                ...
                if debt:
                    ...
                    if debt.zahlkonto == 0:
                        ...
                        bill = db_session.query(Bill).filter(Bill.rechnr == a_rechnr).with_for_update().first()
