...
def prepare_fa_polistbl():
    ...
    prepare_cache([Paramtext, Htparam, Parameters, Waehrung,
                  Fa_ordheader, Fa_order, Bediener])
    ...
    l_supp_data, L_supp = create_model_like(L_lieferant)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal long_digit, billdate, briefnr, p_464, htl_name, htl_adr, htl_tel, cost_list_data, w_list_data, username_data, l_supp_data, l_lieferant, paramtext, htparam, parameters, waehrung, fa_ordheader, fa_order, bediener
        nonlocal cost_list, w_list, username, l_supp
        nonlocal cost_list_data, w_list_data, username_data, l_supp_data

        return {
            "long_digit": long_digit,
            "billdate": billdate,
            "briefnr": briefnr,
            "p_464": p_464,
            "htl_name": htl_name,
    ...
    def create_costlist():
        ...
        parameters_data = (
            db_session.query(Parameters).filter(
                ...
                (Parameters.section == "name") & (Parameters.varname > "")
            )
    ...
    def currency_list():
        ...
        for waehrung in db_session.query(Waehrung).order_by(Waehrung.wabkurz).yield_per(100):
            w_list = W_list()
            w_list_data.append(w_list)
    ...
    def create_bediener():
        ...
        #     fa_order_data = db_session.query(Fa_order).filter(
        #             (Fa_order.order_nr == fa_ordheader.order_nr)).order_by(Fa_order.delivered_date)
        #     for fa_order in fa_order_data.yield_per(100):

        fa_ordheader_data = (
            db_session.query(Fa_ordheader, Fa_order)
            .join(Fa_order, Fa_order.order_nr == Fa_ordheader.order_nr)
            .order_by(Fa_ordheader._recid, Fa_order.delivered_date)
        )

        for fa_ordheader, fa_order in fa_ordheader_data.yield_per(100):
            ...
            if fa_ordheader.activeflag == 2:
                ...
                bediener = get_cache(
                    Bediener, {"userinit": [(eq, fa_ordheader.close_by)]})
            ...
            # fa_order_data = db_session.query(Fa_order).filter(
            #     (Fa_order.order_nr == fa_ordheader.order_nr) & (Fa_order.activeflag == 0)).order_by(Fa_order._recid)
            # for fa_order in fa_order_data.yield_per(100):
