...
# - added handling for rcode_element parameter
# -------------------------------------------
...
def ratecode_adm_prcode_2bl(curr_select: string, prcode: string, bezeich: string,
    ...
    tb1_data, Tb1 = create_model_like(
        Queasy,
        {
            "waehrungsnr": int,
            "wabkurz": string,
    ...
    if curr_select == ("add"):
        ...
        bqueasy = get_cache(
            Queasy, {"key": [(eq, 289)], "char1": [(eq, prcode)]})
        ...
        if gastnr == 0:
            ...
            guest = db_session.query(Guest).order_by(
                Guest._recid.desc()).first()
        ...
        if bediener:
            ...
            res_history.aenderung = "Add Contract Rate, Code: " + prcode + \
                " segmentcode: " + segmentcode + " Description: " + bezeich
        ...
        if queasy:
            ...
            if not matches(queasy.char3, r"*;*"):
                ...
                queasy.char3 = segmentcode + ";" + \
                    substring(queasy.char3, length(ct) + 1 - 1)
        ...
        if bediener:
            ...
            res_history.datum = get_current_date()
            res_history.zeit = get_current_time_in_seconds()
            res_history.aenderung = "Modify Contract Rate, Code: " + prcode + \
                " segmentcode: " + segmentcode + " Description: " + bezeich
