...
def hk_lostfound_1bl(casetype:int, comments:string, sorttype:int, fr_date:date, to_date:date, user_init:string, rec_id:int):
    ...
    buf_s_list_data, Buf_s_list = create_model_like(S_list)

    db_session = local_storage.db_session

    def generate_output():
        nonlocal buf_s_list_data, bediener, queasy, res_history
        nonlocal casetype, comments, sorttype, fr_date, to_date, user_init, rec_id


        nonlocal s_list, buf_s_list
        nonlocal s_list_data, buf_s_list_data

        return {"buf-s-list": buf_s_list_data}
    ...
    def disp_it():
        ...
        if comments == "":
            ...
            for s_list in query(s_list_data, filters=(lambda s_list: s_list.betriebsnr == sorttype and s_list.date1 >= fr_date and s_list.date1 <= to_date), sort_by=[("date1",False),("zinr",False),("nr",False)]):
            ...
            for s_list in query(s_list_data, filters=(lambda s_list: s_list.betriebsnr == sorttype and s_list.date1 >= fr_date and s_list.date1 <= to_date and matches(s_list.bezeich,s)), sort_by=[("date1",False),("zinr",False),("nr",False)]):
    ...
    def assign_it():
        ...
        buf_s_list_data.append(buf_s_list)
