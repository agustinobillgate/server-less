...
def leasing_create_journal_print_proformabl(qrecid: int, pinvoice_no: str, user_init: str):
    ...
    prepare_cache([Artikel, Htparam, Queasy, Res_line, Arrangement, Reslin_queasy, Bill, Counters,
                  Reservation, Guest, Bediener, Bill_line, Debitor, Billjournal, Umsatz, Gl_jouhdr, Gl_journal])
    ...
    artikel = htparam = queasy = res_line = arrangement = reslin_queasy = bill = counters = reservation = guest = bediener = bill_line = debitor = billjournal = umsatz = gl_jouhdr = gl_journal = None
    ...
    def generate_output():
        nonlocal success_flag, log_artnr, ar_ledger, divered_rental, bill_date, tot_amount, tot_nettamount, tot_serv, tot_tax, datum, netto, service, tax, tax2, serv, vat, vat2, fact, loopi, serv_acctno, vat_acctno, vat_fibu, vat2_fibu, serv_fibu, div_fibu, rechnr, prev_tax, prev_serv, prev_amount_debit, prev_amount_credit, month_str1, month_str2, artikel, htparam, queasy, res_line, arrangement, reslin_queasy, bill, counters, reservation, guest, bediener, bill_line, debitor, billjournal, umsatz, gl_jouhdr, gl_journal
    ...
    def create_journal():
        ...
        if bjouhdr:
            tjournal = get_cache(Gl_journal, {"jnr": [(eq, gl_jouhdr.jnr)], "debit": [(ne, 0), (ne, tot_amount)]})
    ...
    def calc_periode():
        nonlocal success_flag, log_artnr, ar_ledger, divered_rental, bill_date, tot_amount, tot_nettamount, tot_serv, tot_tax, datum, netto, service, tax, tax2, serv, vat, vat2, fact, serv_acctno, vat_acctno, vat_fibu, vat2_fibu, serv_fibu, div_fibu, rechnr, prev_tax, prev_serv, prev_amount_debit, prev_amount_credit, month_str1, month_str2, artikel, htparam, queasy, res_line, arrangement, reslin_queasy, bill, counters, reservation, guest, bediener, bill_line, debitor, billjournal, umsatz, gl_jouhdr, gl_journal
        nonlocal qrecid, pinvoice_no, user_init
        nonlocal bartikel
        nonlocal periode_list, bartikel
        nonlocal periode_list_data
