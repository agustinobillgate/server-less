...
def leasing_pay_depositbl(qrecid: int, pinvoice_no: str, artikel_no: int, pay_amount: Decimal, user_init: str):
    ...
    def generate_output():
        ...
        nonlocal qrecid, pinvoice_no, artikel_no, pay_amount, user_init
            ...
            if reslin_queasy:
                ...
                for periode_list in query(periode_list_data, filters=(lambda periode_list: periode_list.counter >= v_start and periode_list.counter <= v_end)):
                    for reslin_queasy in db_session.query(Reslin_queasy).filter(
                            (Reslin_queasy.key == "arrangement") & (Reslin_queasy.resnr == queasy.number1) & (Reslin_queasy.reslinnr == queasy.number2) & (Reslin_queasy.date1 >= periode_list.periode1) & (Reslin_queasy.date1 <= periode_list.periode2)).order_by(Reslin_queasy.date1).all():
                        curr_amount = to_decimal(curr_amount + reslin_queasy.deci1)
    ...
    def create_turnover():
        ...
        #     pinvoice_no + "[Payment Leasing #" + \
        #     to_string(queasy.number1) + "]"
        billjournal.bezeich = f"{artikel.bezeich}- Invoice No : {pinvoice_no}[Payment Leasing #{to_string(queasy.number1)}]"
    ...
    def create_ar():
        ...
        if debitor:
            ...
            if bart:
                ...
                # bdebt.vesrcod = pinvoice_no + "|PAYMENT DEPOSIT"
                bdebt.vesrcod = f"{pinvoice_no}|PAYMENT DEPOSIT"
            ...
            bdebt.vesrdat = get_current_date()
            bdebt.transzeit = get_current_time_in_seconds()
            # bdebt.vesrcod = pinvoice_no + "|VOID PAYMENT DEPOSIT"
            bdebt.vesrcod = f"{pinvoice_no}|VOID PAYMENT DEPOSIT"
    ...
    def create_journal():
        ...
        if guest:
            ...
            gl_jouhdr.bezeich = f"{pinvoice_no}-Leasing-{guest.name}"
    ...
    if queasy:
        ...
        create_queasy()
        queasy.logi2 = True
        queasy.deci2 = to_decimal(queasy.deci2 + pay_amount)

        reslin_queasy = db_session.query(Reslin_queasy).filter(
            (Reslin_queasy.key == "actual-invoice") & (Reslin_queasy.resnr == queasy.number1) & (Reslin_queasy.reslinnr == queasy.number2) & (Reslin_queasy.deci1 == extendflag) & (Reslin_queasy.betriebsnr == qrecid) & ((Reslin_queasy.logi2) | ((Reslin_queasy.logi1 == False) & (Reslin_queasy.logi2 == False))) & (Reslin_queasy.logi3 == False)).first()
