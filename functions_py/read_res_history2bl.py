...
def read_res_history2bl(case_type:int, int1:int, int2:int, int3:int, datum1:date, char1:string, char2:string):
    ...
    t_res_history_data, T_res_history = create_model_like(Res_history, {"gname":string, "zinr":string, "ankunft":date, "abreise":date, "grpname":string, "wa_time":string, "ack":bool, "res":string})
    ...
    def generate_output():
        nonlocal t_res_history_data, ct, st, curr_i, yesterday, rmno, wa_time, res_history, res_line, reservation, nebenst, zimmer
        nonlocal case_type, int1, int2, int3, datum1, char1, char2


        nonlocal t_res_history
        nonlocal t_res_history_data

        return {"t-res-history": t_res_history_data}
    ...
    if case_type == 1:
        ...
        if res_history:
            ...
            t_res_history_data.append(t_res_history)
        ...
        res_history = db_session.query(Res_history).filter(
                 (Res_history.resnr == int1) & (Res_history.reslinnr == int2) & 
                 (Res_history.zeit == int3) & (Res_history.datum == datum1) & 
                 (Res_history.action == ("Wake Up Call"))).with_for_update().first()
        ...
        for res_history in db_session.query(Res_history).filter(
                 (Res_history.resnr == int1) & (Res_history.zeit == int3) & (Res_history.datum == datum1) & (Res_history.action == ("Wake Up Call"))).order_by(Res_history._recid).all():
            t_res_history = T_res_history()
            t_res_history_data.append(t_res_history)
        ...
        for res_history in db_session.query(Res_history).filter(
                 (Res_history.zeit >= 0) & (Res_history.datum >= yesterday) & (Res_history.action == ("Wake Up Call"))).order_by(Res_history._recid).all():
            t_res_history = T_res_history()
            t_res_history_data.append(t_res_history)
        ...
        if rmno != "":
            ...
            if res_line:
                ...
                res_history = db_session.query(Res_history).filter(
                         (Res_history.action == ("Wake Up Call")) & (Res_history.resnr == res_line.resnr) & 
                         (Res_history.reslinnr == res_line.reslinnr) & (Res_history.datum == datum1 - timedelta(days=1)) & 
                         (matches(Res_history.aenderung,("* " + res_line.zinr + ";*"))) & (matches(Res_history.aenderung,("*" + wa_time + ";*")))).with_for_update().first()

                if not res_history:

                    res_history = db_session.query(Res_history).filter(
                             (Res_history.action == ("Wake Up Call")) & (Res_history.resnr == res_line.resnr) & 
                             (Res_history.reslinnr == res_line.reslinnr) & (Res_history.datum == datum1) & 
                             (matches(Res_history.aenderung,("* " + res_line.zinr + ";*"))) & 
                             (matches(Res_history.aenderung,("*" + wa_time + ";*")))).with_for_update().first()
