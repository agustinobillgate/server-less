...
def pchase_parserbl(briefnr:int, printnr:int, docu_nr:string):
    ...
    def generate_output():
        ...
        nonlocal output_list_data, op_list_data, brief_list_data, htp_list_data, loop_list_data, loop1_list_data, header_list_data

        return {"outfile": outfile, "printer_pglen": printer_pglen, "err_code": err_code, "output-list": output_list_data}
    ...
    def do_billline():
        ...
        for l_artikel in db_session.query(L_artikel).filter(
                 ((L_artikel.artnr.in_(list(set([op_list.artnr for op_list in op_list_data])))))).order_by(L_artikel._recid).all():
