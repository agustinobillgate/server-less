...
wgrpdep_list_data, Wgrpdep_list = create_model("Wgrpdep_list", {"betriebsnr":int, "bezeich":string, "departement":int, "fibukonto":string, "zknr":int, "bg_color":int, "kiosk_flag":bool})
...
def rsubgrp_admin_btn_exitbl(wgrpdep_list_data:[Wgrpdep_list], case_type:int, dept:int):
    ...
    if type(wgrpdep_list.betriebsnr) == str:
        wgrpdep_list.betriebsnr = to_int(wgrpdep_list.betriebsnr.strip())
    if type(wgrpdep_list.zknr) == str:
        wgrpdep_list.zknr = to_int(wgrpdep_list.zknr.strip())
