...
"""_yusufwijasena_11/11/2025
    ...
    Ticket ID:
        _remark_:   - fix cl_list.c_zipreis = "S U M M A R Y:"
"""
...
def rmrev_bdown_create_billbalance1_4a_webbl(exc_taxserv: bool, pvilanguage: int, new_contrate: bool, foreign_rate: bool, price_decimal: int, curr_date: date, srttype: int):
    ...
    currency_list_data, Currency_list = create_model(
        "Currency_list",
        {
            "code": string
        })
    ...
    argt_list_data, Argt_list = create_model(
        "Argt_list",
        {
            "argtnr": int,
            "argtcode": string,
    ...
    def generate_output():
        ...
        return {
            "cl-list": cl_list_data,
            "currency-list": currency_list_data,
            "sum-list": sum_list_data,
            "s-list": s_list_data,
            "argt-list": argt_list_data
        }
    ...
    def create_billbalance1():
        ...
        artikel = get_cache(
            Artikel, {"zwkum": [(eq, bfast_art)], "departement": [(eq, fb_dept)]})
        ...
        artikel = get_cache(
            Artikel, {"zwkum": [(eq, lunch_art)], "departement": [(eq, fb_dept)]})
        ...
        artikel = get_cache(
            Artikel, {"zwkum": [(eq, dinner_art)], "departement": [(eq, fb_dept)]})
        ...
        artikel = get_cache(
            Artikel, {"zwkum": [(eq, lundin_art)], "departement": [(eq, fb_dept)]})
        ...
        if srttype == 2:
            ...
            for genstat.argt, genstat.datum, genstat.zinr, genstat.resnr, genstat.res_int, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.kind3, genstat.resstatus, genstat.zipreis, genstat.ratelocal, genstat.logis, genstat.res_deci, genstat.zikatnr, genstat.res_char, genstat._recid, res_line.betriebsnr, res_line.reserve_dec, res_line.gastnrpay, res_line.gastnrmember, res_line.resnr, res_line.l_zuordnung, res_line.zikatnr, res_line._recid, res_line.name, res_line.ankunft, res_line.abreise, res_line.resname, res_line.zimmer_wunsch, res_line.reslinnr, res_line.adrflag, res_line.zinr, res_line.zipreis, res_line.reserve_int, zimmer.sleeping, zimmer._recid in db_session.query(Genstat.argt, Genstat.datum, Genstat.zinr, Genstat.resnr, Genstat.res_int, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.kind3, Genstat.resstatus, Genstat.zipreis, Genstat.ratelocal, Genstat.logis, Genstat.res_deci, Genstat.zikatnr, Genstat.res_char, Genstat._recid, Res_line.betriebsnr, Res_line.reserve_dec, Res_line.gastnrpay, Res_line.gastnrmember, Res_line.resnr, Res_line.l_zuordnung, Res_line.zikatnr, Res_line._recid, Res_line.name, Res_line.ankunft, Res_line.abreise, Res_line.resname, Res_line.zimmer_wunsch, Res_line.reslinnr, Res_line.adrflag, Res_line.zinr, Res_line.zipreis, Res_line.reserve_int, Zimmer.sleeping, Zimmer._recid).join(Res_line, (Res_line.resnr == Genstat.resnr) & (Res_line.reslinnr == Genstat.res_int[inc_value(0)]) & (Res_line.l_zuordnung[inc_value(2)] == 0)).join(Zimmer, (Zimmer.zinr == Genstat.zinr)).filter(
                ...
                artikel = get_cache(
                    Artikel, {"artnr": [(eq, arrangement.argt_artikelnr)], "departement": [(eq, 0)]})
                serv1, vat1, vat2, fact1 = get_output(calc_servtaxesbl(
                    1, artikel.artnr, artikel.departement, curr_date))

                waehrung1 = get_cache(
                    Waehrung, {"waehrungsnr": [(eq, res_line.betriebsnr)]})

                exrate = get_cache(Exrate, {"datum": [(eq, curr_date)], "artnr": [
                                   (eq, waehrung1.waehrungsnr)]})
                ...
                for billjournal in db_session.query(Billjournal).filter(
                    ...
                    bill = get_cache(Bill, {"rechnr": [(eq, billjournal.rechnr)], "resnr": [
                                     (eq, genstat.resnr)], "reslinnr": [(eq, 0)]})
                ...
                bill = get_cache(Bill, {"resnr": [(eq, genstat.resnr)], "reslinnr": [
                                 (eq, genstat.res_int[0])]})
                ...
                if not exc_taxserv:
                    ...
                    cl_list.lodging = to_decimal(
                        genstat.logis) * to_decimal((1) + to_decimal(vat1) + to_decimal(vat2) + to_decimal(serv1))
                    cl_list.fixcost = to_decimal(genstat.res_deci[5]) * to_decimal(
                        (1) + to_decimal(vat1) + to_decimal(vat2) + to_decimal(serv1))

                else:
                    cl_list.zipreis = to_decimal(
                        round((genstat.zipreis / (1 + vat1 + vat2 + serv1)), price_decimal))
                    cl_list.localrate = to_decimal(
                        round((genstat.ratelocal / (1 + vat1 + vat2 + serv1)), price_decimal))
                    cl_list.t_rev = to_decimal(
                        round((genstat.zipreis / (1 + vat1 + vat2 + serv1)), price_decimal))
                    cl_list.lodging = to_decimal(
                        round(genstat.logis, price_decimal))
                    cl_list.fixcost = to_decimal(
                        round(genstat.res_deci[5], price_decimal))

                sum_list.lodging = to_decimal(
                    sum_list.lodging) + to_decimal(cl_list.lodging)
                sum_list.t_rev = to_decimal(
                    sum_list.t_rev) + to_decimal(genstat.zipreis)
                sum_list.fixcost = to_decimal(
                    sum_list.fixcost) + to_decimal(cl_list.fixcost)

                if bill_flag1.lower() == ("Master Bill").lower():

                    billjournal = get_cache(Billjournal, {"rechnr": [(
                        eq, bill_master)], "bill_datum": [(eq, genstat.datum)]})
                ...
                if bill_flag2.lower() == ("Guest Bill").lower():
                    ...
                    billjournal = get_cache(Billjournal, {"rechnr": [(
                        eq, bill_rechnr)], "bill_datum": [(eq, genstat.datum)]})
                ...
                zimkateg = get_cache(
                    Zimkateg, {"zikatnr": [(eq, genstat.zikatnr)]})

                if zimkateg:
                    cl_list.rmtype = zimkateg.kurzbez

                segment = get_cache(
                    Segment, {"segmentcode": [(eq, reservation.segmentcode)]})
                ...
                if cl_list.zipreis == 0 and cl_list.adult == 0:
                    ...
                    cl_list.pax = genstat.erwachs + genstat.kind1 + \
                        genstat.kind2 + cl_list.com + cl_list.comch

                if guest:
                    cl_list.name = cl_list.name + guest.name + \
                        ", " + guest.vorname1 + "-" + guest.adresse1
                    cl_list.currency = waehrung1.wabkurz

                argt_list = query(argt_list_data, filters=(
                    lambda argt_list: argt_list.argtnr == arrangement.argtnr), first=True)
                ...
                if not argt_list:
                    ...
                    if cl_list.zipreis == 0 and cl_list.adult == 0:
                        ...
                        argt_list.pax = genstat.erwachs + genstat.kind1 + \
                            genstat.kind2 + cl_list.com + cl_list.comch
                else:
                    ...
                    if cl_list.zipreis == 0 and cl_list.adult == 0:
                        ...
                        argt_list.pax = argt_list.pax + genstat.erwachs + \
                            genstat.kind1 + genstat.kind2 + cl_list.com + cl_list.comch

                if guest.geburtdatum1 != None and guest.geburtdatum2 != None:

                    if guest.geburtdatum1 < guest.geburtdatum2:
                        cl_list.age1 = get_year(
                            guest.geburtdatum2) - get_year(guest.geburtdatum1)
                ...
                if genstat.zipreis != 0:
                    ...
                    for argt_line, artikel in db_session.query(Argt_line, Artikel).join(Artikel, (Artikel.artnr == Argt_line.argt_artnr) & (Artikel.departement == Argt_line.departement)).filter(
                        ...
                        take_it, f_betrag, argt_betrag, qty = get_argtline_rate(
                            contcode, argt_line._recid)
                        serv2, vat3, vat4, fact2 = get_output(calc_servtaxesbl(
                            1, artikel.artnr, artikel.departement, curr_date))
                        ...
                        if artikel.zwkum == bfast_art and (artikel.umsatzart == 3 or artikel.umsatzart >= 5):
                            ...
                            if not exc_taxserv:
                                cl_list.bfast = to_decimal(
                                    genstat.res_deci[1]) * to_decimal((1) + to_decimal(vat3) + to_decimal(serv2))
                                sum_list.bfast = to_decimal(
                                    sum_list.bfast) + to_decimal(cl_list.bfast)
                            ...
                            if not exc_taxserv:
                                cl_list.lunch = to_decimal(
                                    genstat.res_deci[2]) * to_decimal((1) + to_decimal(vat3) + to_decimal(serv2))
                                sum_list.lunch = to_decimal(
                                    sum_list.lunch) + to_decimal(cl_list.lunch)
                            ...
                            if not exc_taxserv:
                                cl_list.dinner = to_decimal(
                                    genstat.res_deci[3]) * to_decimal((1) + to_decimal(vat3) + to_decimal(serv2))
                                sum_list.dinner = to_decimal(
                                    sum_list.dinner) + to_decimal(cl_list.dinner)
                    ...
                    if not exc_taxserv:
                        cl_list.misc = to_decimal(cl_list.localrate) - to_decimal((cl_list.lodging) + to_decimal(
                            cl_list.bfast) + to_decimal(cl_list.lunch) + to_decimal(cl_list.dinner))
                        sum_list.misc = to_decimal(
                            sum_list.misc) + to_decimal(cl_list.misc)
                ...
                if htparam.flogical and not exc_taxserv:
                    cl_list.zipreis = to_decimal(
                        round(cl_list.zipreis, price_decimal))
                    cl_list.lodging = to_decimal(
                        round(cl_list.lodging, price_decimal))
                    cl_list.bfast = to_decimal(
                        round(cl_list.bfast, price_decimal))
                    cl_list.lunch = to_decimal(
                        round(cl_list.lunch, price_decimal))
                    cl_list.dinner = to_decimal(
                        round(cl_list.dinner, price_decimal))
                    cl_list.misc = to_decimal(
                        round(cl_list.misc, price_decimal))
                    cl_list.fixcost = to_decimal(
                        round(cl_list.fixcost, price_decimal))
                    cl_list.localrate = to_decimal(
                        round(cl_list.localrate, price_decimal))
                    cl_list.t_rev = to_decimal(
                        round(cl_list.t_rev, price_decimal))

                if matches(res_line.zimmer_wunsch, r"*$CODE$*"):
                    s = substring(res_line.zimmer_wunsch, (get_index(
                        res_line.zimmer_wunsch, "$CODE$") + 6) - 1)
                ...
                reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "arrangement")], "resnr": [(eq, res_line.resnr)], "reslinnr": [
                                          (eq, res_line.reslinnr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})

                if reslin_queasy:
                    cl_list.fix_rate = "F"

                tot_rate = to_decimal(tot_rate) + to_decimal(cl_list.zipreis)
                tot_lrate = to_decimal(tot_lrate) + \
                    to_decimal(cl_list.localrate)
                ...
                for argt_line in db_session.query(Argt_line).filter(
                    ...
                    t_argt_line = query(t_argt_line_data, filters=(lambda t_argt_line: t_argt_line.argt_artnr ==
                                        argt_line.argt_artnr and t_argt_line.argtnr == arrangement.argtnr and t_argt_line.departement == argt_line.departement), first=True)
                    ...
                    if bill_flag1.lower() == ("Master Bill").lower():
                        ...
                        for billjournal.rechnr, billjournal.artnr, billjournal.departement, billjournal.bezeich, billjournal.fremdwaehrng, billjournal.betrag, billjournal._recid, artikel.artnr, artikel.departement, artikel.umsatzart, artikel.zwkum, artikel._recid in db_session.query(Billjournal.rechnr, Billjournal.artnr, Billjournal.departement, Billjournal.bezeich, Billjournal.fremdwaehrng, Billjournal.betrag, Billjournal._recid, Artikel.artnr, Artikel.departement, Artikel.umsatzart, Artikel.zwkum, Artikel._recid).join(Artikel, (Artikel.artnr == Billjournal.artnr) & (Artikel.departement == Billjournal.departement) & (Artikel.artart != 9)).filter(
                            ...
                            if billjournal.artnr != deposit_art:
                                ...
                                s_list = query(s_list_data, filters=(lambda s_list: s_list.artnr == billjournal.artnr and s_list.dept ==
                                               billjournal.departement and s_list.curr == waehrung1.wabkurz), first=True)
                                ...
                                s_list.f_betrag = to_decimal(
                                    s_list.f_betrag) + to_decimal(billjournal.fremdwaehrng)
                                s_list.l_betrag = to_decimal(
                                    s_list.l_betrag) + to_decimal(billjournal.betrag)
                ...
                if res_line.adrflag:
                    ltot_lodging = to_decimal(
                        ltot_lodging) + to_decimal(cl_list.lodging)
                else:
                    tot_lodging = to_decimal(
                        tot_lodging) + to_decimal(cl_list.lodging)
                ...
                if foreign_rate and price_decimal == 0 and not res_line.adrflag:
                    ...
                    if htparam.finteger != 0:
                        ...
                        for i in range(1, htparam.finteger + 1):
                            n = n * 10
                        lodge_betrag = to_decimal(
                            round(lodge_betrag / n, 0) * n)
        ...
        for cc_list in query(cc_list_data, filters=(lambda cc_list: cc_list.flag == ""), sort_by=[("currency", False)]):
            ...
            if curr_code != cc_list.currency:
                ...
                waehrung = get_cache(
                    Waehrung, {"wabkurz": [(eq, cc_list.currency)]})
        ...
        for cl_list in query(cl_list_data, filters=(lambda cl_list: cl_list.flag.lower() != ("*").lower())):
            ...
            if cl_list.lodging < 0:
                cl_list.c_lodging = to_string(
                    cl_list.lodging, "->>,>>>,>>>,>>9.99")
            else:
                cl_list.c_lodging = to_string(
                    cl_list.lodging, ">>>,>>>,>>>,>>9.99")
            cl_list.c_zipreis = to_string(
                cl_list.zipreis, ">>>,>>>,>>>,>>9.99")
            cl_list.c_localrate = to_string(
                cl_list.localrate, ">>>,>>>,>>>,>>9.99")
            ...
            argt_list = query(argt_list_data, filters=(
                lambda argt_list: argt_list.argtcode == cl_list.argt), first=True)

            if argt_list:
                argt_list.bfast = to_decimal(
                    argt_list.bfast) + to_decimal(cl_list.bfast)

        if exc_taxserv:

            for s_list in query(s_list_data):
                s_list.f_betrag = to_decimal(
                    round((s_list.f_betrag / (1 + vat1 + vat2 + serv1)), price_decimal))
                s_list.l_betrag = to_decimal(
                    round((s_list.l_betrag / (1 + vat1 + vat2 + serv1)), price_decimal))

            for sum_list in query(sum_list_data):
                sum_list.lodging = to_decimal(
                    round((sum_list.lodging / (1 + vat1 + vat2 + serv1)), price_decimal))
                sum_list.bfast = to_decimal(
                    round((sum_list.bfast / (1 + vat1 + vat2 + serv1)), price_decimal))
                sum_list.lunch = to_decimal(
                    round((sum_list.lunch / (1 + vat1 + vat2 + serv1)), price_decimal))
                sum_list.dinner = to_decimal(
                    round((sum_list.dinner / (1 + vat1 + vat2 + serv1)), price_decimal))
                sum_list.misc = to_decimal(
                    round((sum_list.misc / (1 + vat1 + vat2 + serv1)), price_decimal))
                sum_list.fixcost = to_decimal(
                    round((sum_list.fixcost / (1 + vat1 + vat2 + serv1)), price_decimal))
                sum_list.t_rev = to_decimal(
                    round((sum_list.t_rev / (1 + vat1 + vat2 + serv1)), price_decimal))
    ...
    def get_argtline_rate(contcode: string, argt_recid: int):
        ...
        if add_it:
            ...
            reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "resnr": [(eq, res_line.resnr)], "reslinnr": [(eq, res_line.reslinnr)], "number1": [
                                      (eq, argtline.departement)], "number2": [(eq, argtline.argtnr)], "number3": [(eq, argtline.argt_artnr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})

            if reslin_queasy:

                if argtline.vt_percnt == 0:
                    query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'fargt-line') & (Reslin_queasy.char1 == '') & (Reslin_queasy.number1 == argtline.departement) & (Reslin_queasy.number2 == argtline.argtnr) & (
                        Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argtline.argt_artnr) & (Reslin_queasy.date1 <= curr_date) & (Reslin_queasy.date2 >= curr_date) & (Reslin_queasy.deci1 != 0))

                elif argtline.vt_percnt == 1:
                    query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'fargt-line') & (Reslin_queasy.char1 == '') & (Reslin_queasy.number1 == argtline.departement) & (Reslin_queasy.number2 == argtline.argtnr) & (
                        Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argtline.argt_artnr) & (Reslin_queasy.date1 <= curr_date) & (Reslin_queasy.date2 >= curr_date) & (Reslin_queasy.deci2 != 0))

                elif argtline.vt_percnt == 2:
                    query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'fargt-line') & (Reslin_queasy.char1 == '') & (Reslin_queasy.number1 == argtline.departement) & (Reslin_queasy.number2 == argtline.argtnr) & (
                        Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.number3 == argtline.argt_artnr) & (Reslin_queasy.date1 <= curr_date) & (Reslin_queasy.date2 >= curr_date) & (Reslin_queasy.deci3 != 0))

                for reslin_queasy in query.all():
                    if reslin_queasy.char2.lower() != "" and reslin_queasy.char2.lower() != ("0").lower():
                        argt_betrag = (to_decimal(res_line.zipreis) * to_decimal(
                            to_int(reslin_queasy.char2)) / to_decimal(100)) * to_decimal(qty)
                    else:
                        if argt_line.vt_percnt == 0:
                            argt_betrag = to_decimal(
                                reslin_queasy.deci1) * to_decimal(qty)

                        elif argt_line.vt_percnt == 1:
                            argt_betrag = to_decimal(
                                reslin_queasy.deci2) * to_decimal(qty)

                        elif argt_line.vt_percnt == 2:
                            argt_betrag = to_decimal(
                                reslin_queasy.deci3) * to_decimal(qty)

                    f_betrag = to_decimal(argt_betrag)

                    waehrung = get_cache(
                        Waehrung, {"_recid": [(eq, waehrung1._recid)]})
            ...
            if contcode != "":
                ...
                reslin_queasy = get_cache(Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, res_line.reserve_int)], "number2": [(eq, arrangement.argtnr)], "number3": [
                                          (eq, argtline.argt_artnr)], "resnr": [(eq, argtline.departement)], "reslinnr": [(eq, curr_zikatnr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})

                if reslin_queasy:
                    if argt_line.vt_percnt == 0:
                        query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'argt-line') & (Reslin_queasy.char1 == contcode) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (
                            Reslin_queasy.number3 == argtline.argt_artnr) & (Reslin_queasy.resnr == argtline.departement) & (Reslin_queasy.reslinnr == curr_zikatnr) & (Reslin_queasy.date1 <= curr_date) & (Reslin_queasy.date2 >= curr_date) & (Reslin_queasy.deci1 != 0))

                    elif argt_line.vt_percnt == 1:
                        query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'argt-line') & (Reslin_queasy.char1 == contcode) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (
                            Reslin_queasy.number3 == argtline.argt_artnr) & (Reslin_queasy.resnr == argtline.departement) & (Reslin_queasy.reslinnr == curr_zikatnr) & (Reslin_queasy.date1 <= curr_date) & (Reslin_queasy.date2 >= curr_date) & (Reslin_queasy.deci2 != 0))

                    elif argt_line.vt_percnt == 2:
                        query = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == 'argt-line') & (Reslin_queasy.char1 == contcode) & (Reslin_queasy.number1 == res_line.reserve_int) & (Reslin_queasy.number2 == arrangement.argtnr) & (
                            Reslin_queasy.number3 == argtline.argt_artnr) & (Reslin_queasy.resnr == argtline.departement) & (Reslin_queasy.reslinnr == curr_zikatnr) & (Reslin_queasy.date1 <= curr_date) & (Reslin_queasy.date2 >= curr_date) & (Reslin_queasy.deci3 != 0))
            ...
            arrangement = get_cache(
                Arrangement, {"argtnr": [(eq, argt_line.argtnr)]})

            waehrung = get_cache(
                Waehrung, {"waehrungsnr": [(eq, arrangement.betriebsnr)]})
            f_betrag = to_decimal(argt_betrag) * to_decimal(qty)

            if res_line.betriebsnr != arrangement.betriebsnr:
                argt_betrag = to_decimal(argt_betrag) * to_decimal(
                    (waehrung.ankauf) / to_decimal(waehrung.einheit)) / to_decimal(frate)

            if argt_betrag > 0:
                argt_betrag = to_decimal(argt_betrag) * to_decimal(qty)
            else:
                argt_betrag = (to_decimal(
                    rm_rate) * to_decimal((argt_betrag) / to_decimal(100))) * to_decimal(qty)
    ...
    def get_genstat_argt_betrag(f_rate: Decimal, argt_betrag: Decimal):
        ...
        for tokcounter in range(1, num_entries(genstat.res_char[3], ";") + 1):
            ...
            if mestoken != "":
                ...
                a_betrag = to_decimal(to_decimal(
                    entry(2, mestoken, ","))) * to_decimal(0.01)
                x_betrag = to_decimal(to_decimal(
                    entry(3, mestoken, ","))) * to_decimal(0.01)

                if curr_artnr == argt_line.argt_artnr and curr_dept == argt_line.departement:
                    argt_betrag = to_decimal(
                        a_betrag) * to_decimal(x_betrag) / to_decimal(f_rate)
