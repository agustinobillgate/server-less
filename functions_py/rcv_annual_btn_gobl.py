...
def rcv_annual_btn_gobl(pvilanguage:int, from_grp:int, mm:int, yy:int, sorttype:int):
    ...
    str_list_data, Str_list = create_model("Str_list", {"num":int, "artnr":int, "bezeich":string, "qty":[Decimal,12], "avrg":[Decimal,12], "amt":[Decimal,12], "tot_qty":Decimal, "tot_amt":Decimal, "del_flag":bool})

    db_session = local_storage.db_session

    def generate_output():
        nonlocal str_list_data, lvcarea, htparam, l_artikel, l_ophis, l_op
    ...
    def create_list(mm:int, yy:int):
        ...
        for i in range(mm,mm + 1) :
            ...
            if i == 12:
                tmp_year = yy + 1
                to_date = date_mdy(1, 1, tmp_year)
                to_date = to_date - timedelta(days=1)
            else:
                tmp_month = i + 1
                to_date = date_mdy(tmp_month, 1, yy)
                to_date = to_date - timedelta(days=1)
            ...
            for datum in date_range(from_date, to_date) :
                ...
                if datum <= closed_date:
                    ...
                    for l_ophis.artnr, l_ophis.anzahl, l_ophis.warenwert, l_ophis._recid, l_artikel.artnr, l_artikel.bezeich, l_artikel._recid in db_session.query(L_ophis.artnr, L_ophis.anzahl, L_ophis.warenwert, L_ophis._recid, L_artikel.artnr, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_ophis.artnr)).filter((L_ophis.datum == datum) & (L_ophis.op_art == 1)).order_by(L_ophis.artnr).all():
                        ...
                        str_list = query(str_list_data, filters=(lambda str_list: str_list.artnr == l_ophis.artnr), first=True)
                    ...
                    for l_op.artnr, l_op.anzahl, l_op.warenwert, l_op._recid, l_artikel.artnr, l_artikel.bezeich, l_artikel._recid in db_session.query(L_op.artnr, L_op.anzahl, L_op.warenwert, L_op._recid, L_artikel.artnr, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr)).filter((L_op.datum == datum) & (L_op.op_art == 1) & (L_op.loeschflag <= 1)).order_by(L_op.artnr).all():
                        ...
                        str_list = query(str_list_data, filters=(lambda str_list: str_list.artnr == l_op.artnr), first=True)
        ...
        if sorttype == 0:
            for str_list in query(str_list_data, filters=(lambda str_list: str_list.tot_qty == 0)):
                str_list_data.remove(str_list)
            ...
            for str_list in query(str_list_data, filters=(lambda str_list: str_list.tot_amt == 0)):
                str_list_data.remove(str_list)
        ...
        for i in range(mm,mm + 1) :
            ...
            for datum in date_range(from_date,to_date) :
                ...
                if datum <= closed_date:
                    ...
                    for l_ophis.artnr, l_ophis.anzahl, l_ophis.warenwert, l_ophis._recid, l_artikel.artnr, l_artikel.bezeich, l_artikel._recid in db_session.query(L_ophis.artnr, L_ophis.anzahl, L_ophis.warenwert, L_ophis._recid, L_artikel.artnr, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_ophis.artnr) & (L_artikel.endkum == from_grp)).filter((L_ophis.datum == datum) & (L_ophis.op_art == 1)).order_by(L_ophis.artnr).all():
                    ...
                    for l_op.artnr, l_op.anzahl, l_op.warenwert, l_op._recid, l_artikel.artnr, l_artikel.bezeich, l_artikel._recid in db_session.query(L_op.artnr, L_op.anzahl, L_op.warenwert, L_op._recid, L_artikel.artnr, L_artikel.bezeich, L_artikel._recid).join(L_artikel,(L_artikel.artnr == L_op.artnr) & (L_artikel.endkum == from_grp)).filter((L_op.datum == datum) & (L_op.op_art == 1) & (L_op.loeschflag < 2)).order_by(L_op.artnr).all():
