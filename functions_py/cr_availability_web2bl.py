...
import random
# from functions.available_rates_cld1bl import available_rates_cld1bl
from functions_py.available_rates_cld1bl import available_rates_cld1bl
...
def cr_availability_web2bl(pvilanguage: int, vhp_limited: bool, op_type: int, printer_nr: int, call_from: int, adult_child_str: str, statsort: int, dispsort: int, curr_date: date, incl_tentative: bool, mi_inactive: bool, show_rate: bool, indgastnr: int, qci_zinr: str):
    ...
    def create_rate_list():
        ...
        rate_list_data = get_output(available_rates_cld1bl(
            htparam.fdate, tdate, adult_child_str, indgastnr, room_avail_list_data))
        ...
        for room_avail_list in query(room_avail_list_data):
            ...
            for rate_list in query(rate_list_data, filters=(lambda rate_list: rate_list.room_type == room_avail_list.zikatnr), sort_by=[("i_counter", False)]):
                ...
                if int_art != "":
                    ...
                    if artikel:
                        ...
                        for tmp_resline in query(tmp_resline_data, sort_by=[("resnr", False)]):
                            ...
                            bargt = get_cache(
                                Arrangement, {"arrangement": [(eq, tmp_resline.arrangement)]})
            ...
            if do_it:
                for i in range(1, num_day + 1):
                    ...
                    if datum >= res_line.ankunft and datum < res_line.abreise:
                        ...
                        kontline = get_cache(
                            Kontline, {"kontcode": [(eq, kline.kontcode)], "betriebsnr": [(eq, 0)], "kontstatus": [(eq, 1)]})
            ...
            if statsort == 1:
                ...
                for zimkateg in db_session.query(Zimkateg).filter(
                    ...
                    if rmcat_list:
                        ...
                        room_avail_list.bezeich = zimkateg.kurzbez + " - " + to_string(zimkateg.overbook, ">>9")
                ...
                for zimkateg in db_session.query(Zimkateg).filter(
                    ...
                    rmcat_list = query(rmcat_list_data, (lambda rmcat_list: (
                        zimkateg.zikatnr == rmcat_list.zikatnr)), first=True)
                    room_avail_list = Room_avail_list()
                    room_avail_list_data.append(room_avail_list)
    ...
    if qci_zinr != "":
        qci_zimmer = get_cache(
            Zimmer, {"zinr": [(eq, qci_zinr)]})
    i = 1
    while i <= (num_day + 1):
        date_list = Date_list()
    ...
    if op_type == 0:
        ...
        if not show_rate:
            ...
            create_rate_list()
    # procedure design_lnl, print_lnl, print_txt tidak ada
    # elif op_type == 1:
    #     design_lnl()
    # elif op_type == 2:
    #     print_lnl()
