...
def bonds_held_refundbl(rechnr: int, user_init: str):
    ...
    bill = get_cache(
        Bill, {"rechnr": [(eq, rechnr)], "flag": [(eq, 0)], "billnr": [(eq, 4)]})
    ...
    if bill:
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            if tlist.amount == 0:
                ...
                umsatz = get_cache(
                    Umsatz, {"artnr": [(eq, artikel.artnr)], "departement": [(eq, artikel.departement)], "datum": [(eq, bill_date)]})
                ...
                if not artikel.autosaldo:
                    ...
                    bill.rgdruck = 0
                bill.saldo = to_decimal(bill.saldo + curr_amount)
