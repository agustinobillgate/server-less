...
def fo_parxls_gs_1bl(pvilanguage:int, ytd_flag:bool, jan1:date, ljan1:date, lfrom_date:date, lto_date:date, pfrom_date:date, pto_date:date, from_date:date, to_date:date, start_date:date, lytd_flag:bool, lmtd_flag:bool, pmtd_flag:bool, lytoday_flag:bool, lytoday:date, foreign_flag:bool, budget_flag:bool, foreign_nr:int, price_decimal:int, briefnr:int, link:string, budget_all:bool, user_init:string, w1_data:[W1], w2_data:[W2]):
    ...
    prepare_cache ([Segmentstat, Arrangement, Res_line, Zkstat, H_umsatz, Segment, Genstat, Uebertrag, Artikel, H_artikel, Htparam, Exrate, H_cost, H_journal, Reservation, Zimkateg, H_bill_line, H_bill, Bill, Queasy, Waehrung, L_op, H_compli, Fbstat, Sources, Paramtext, Parameters])

    msg_str = ""
    error_nr = 0
    lfr_date:date = date(1,1,1)
    exrate_betrag:Decimal = to_decimal("0.0")
    frate:Decimal = 1
    prog_error:bool = False
    ch:string = ""
    month_str:List[string] = ["JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"]
    ...
    t_list_data, T_list = create_model("T_list", {"dept":int, "datum":date, "shift":int, "pax_food":int, "pax_bev":int})
    t_rechnr_data, T_rechnr = create_model("T_rechnr", {"datum":date, "dept":int, "rechnr":int, "shift":int, "found_food":bool, "found_bev":bool})
    temp_rechnr_data, Temp_rechnr = create_model("Temp_rechnr", {"datum":date, "dept":int, "rechnr":int, "shift":int, "belegung":int, "artnrfront":int, "compli_flag":bool, "tischnr":int, "betrag":Decimal, "artnr":int, "artart":int, "resnr":int, "reslinnr":int, "f_pax":int, "b_pax":int, "f_qty":int, "b_qty":int})
    res_cancelled_list_data, Res_cancelled_list = create_model("Res_cancelled_list", {"resnr":int, "reslinnr":int, "gastnr":int, "ankunft":date, "abreise":date, "counter":int})
    s_list_data, S_list = create_model("S_list", {"datum":date, "reihenfolge":int, "lager_nr":int, "fibukonto":string, "bezeich":string, "flag":int, "betrag":Decimal, "t_betrag":Decimal, "betrag1":Decimal, "t_betrag1":Decimal}, {"reihenfolge": 1, "flag": 2})
    fbstat_dept_data, Fbstat_dept = create_model("Fbstat_dept", {"done":bool, "dept":int})
    ...
    def generate_output():
        ...
        nonlocal pvilanguage, ytd_flag, jan1, ljan1, lfrom_date, lto_date, pfrom_date, pto_date, from_date, to_date, start_date, lytd_flag, lmtd_flag, pmtd_flag, lytoday_flag, lytoday, foreign_flag, budget_flag, foreign_nr, price_decimal, briefnr, link, budget_all, user_init, w1_data, w2_data
        nonlocal segmbuff


        nonlocal w1, shift_list, w2, tmp_room, t_list, t_rechnr, temp_rechnr, res_cancelled_list, s_list, fbstat_dept, stream_list, segmbuff, curr_child, ww1, ww2, wdu, w11, w12, w13, w11, w12, w13, w11, tbuff, w1a, w11, w12, w11, w12, w11, w12, w11, w753, w754, w755, w11, w11, w11, w11, w12, w13, wlos, wspc, btemp_rechnr, wf1, wf2, wf3, wf4, wb1, wb2, wb3, wb4, wo1, wo2, wo3, wo4, w11, w11, w12, w13, parent, child, curr_child, curr_w2
        nonlocal shift_list_data, tmp_room_data, t_list_data, t_rechnr_data, temp_rechnr_data, res_cancelled_list_data, s_list_data, fbstat_dept_data, stream_list_data
    ...
    def fill_value():
        ...
        Curr_child = W1
        curr_child_data = w1_data
        Ww1 = W1
        ww1_data = w1_data
        Ww2 = W1
        ww2_data = w1_data
        fbstat_dept_data.clear()

        for ww1 in query(ww1_data, filters=(lambda ww1: ww1.grpflag == 0)):
            num__row = num__row + 1

        for ww1 in query(ww1_data, filters=(lambda ww1: ww1.grpflag == 0)):

            ww2 = query(ww2_data, filters=(lambda ww2: ww2.varname == ww1.varname and ww1._recid != ww2._recid), first=True)
        ...
        for ww1 in query(ww1_data, filters=(lambda ww1: ww1.grpflag == 0), sort_by=[("main_code",True)]):
            z = z + 1
            ...
            elif ww1.main_code == 2060:
                fill_new_cancel(ww1._recid)

            elif ww1.main_code == 9000:
                done_los = fill_los()

            elif ww1.main_code == 9106 or ww1.main_code == 2061:
            ...
            elif ww1.main_code == 7196:
                fill_canc_cidate_nite(ww1._recid)
        ...
        for w1 in query(w1_data, filters=(lambda w1: w1.grpflag >= 1 and w1.grpflag != 9), sort_by=[("nr",False)]):

            for w2 in query(w2_data, filters=(lambda w2: w2.nr1 == w1.nr)):

                curr_child = query(curr_child_data, filters=(lambda curr_child: curr_child.nr == w2.nr2), first=True)
    ...
    def fill_adddayuse():
        ...
        for res_line.ankunft, res_line.resnr, res_line.reslinnr, res_line.zimmeranz, res_line.gastnr, res_line.abreise, res_line.active_flag, res_line._recid, arrangement.argt_artikelnr, arrangement._recid in db_session.query(Res_line.ankunft, Res_line.resnr, Res_line.reslinnr, Res_line.zimmeranz, Res_line.gastnr, Res_line.abreise, Res_line.active_flag, Res_line._recid, Arrangement.argt_artikelnr, Arrangement._recid).join(Arrangement,(Arrangement.arrangement == Res_line.arrangement)).filter((Res_line.active_flag == 2) & (Res_line.ankunft >= datum1) & (Res_line.ankunft <= to_date) & (Res_line.abreise == Res_line.ankunft) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Res_line.zinr, Res_line.resnr, Res_line.reslinnr).all():
    ...
    def fill_totroom(rec_w1:int):
        ...
        w1 = query(w1_data, filters=(lambda w1: w1._recid == rec_w1), first=True)
        ...
        if (get_day(to_date) == 31 and get_month(to_date) != 8 and get_month(to_date) != 1) or (get_day(to_date) == 30 and get_month(to_date) == 3) or (get_day(date_mdy(3, 1, get_year(to_date)) - timedelta(days=1)) == 28 and get_month(to_date) == 3 and get_day(to_date) == 29):
    ...
    def fill_rmavail(rec_w1:int):
        ...
        if (get_day(to_date) == 31 and get_month(to_date) != 8 and get_month(to_date) != 1) or (get_day(to_date) == 30 and get_month(to_date) == 3) or (get_day(date_mdy(3, 1, get_year(to_date)) - timedelta(days=1)) == 28 and get_month(to_date) == 3 and get_day(to_date) == 29):
            ...
            if zimkateg:
                ...
                zkstat = get_cache (Zkstat, {"datum": [(eq, curr_date)],"zikatnr": [(eq, None)]})

            if zkstat:
                w1.lm_today =  to_decimal(w1.lm_today) + to_decimal(zkstat.anz100)

        for zkstat in db_session.query(Zkstat).filter((Zkstat.datum >= datum1) & (Zkstat.datum <= to_date)).order_by(Zkstat._recid).all():
        ...
        if lytd_flag or lmtd_flag:
            ...
            for zkstat in db_session.query(Zkstat).filter((Zkstat.datum >= datum2) & (Zkstat.datum <= lto_date)).order_by(Zkstat._recid).all():
        ...
        if lytoday_flag:
            ...
            for zkstat in db_session.query(Zkstat).filter((Zkstat.datum == lytoday)).order_by(Zkstat._recid).all():
                w1.lytoday =  to_decimal(w1.lytoday) + to_decimal(zkstat.anz100)

        if pmtd_flag:

            for zkstat in db_session.query(Zkstat).filter((Zkstat.datum >= pfrom_date) & (Zkstat.datum <= pto_date)).order_by(Zkstat._recid).all():
    ...
    def fill_cover(rec_w1:int):
        ...
        w12 = query(w12_data, filters=(lambda w12: w12.main_code == 192 and w12.dept == w1.dept), first=True)

        w13 = query(w13_data, filters=(lambda w13: w13.main_code == 197 and w13.dept == w1.dept), first=True)
    ...
    def fill_canc_cidate_nite(rec_w1:int):
        ...
        if w1:
            ...
            for resbuff in db_session.query(Resbuff).filter(
                ...
                if resbuff.ankunft == to_date:
                    w1.tday =  to_decimal(w1.tday) + to_decimal((resbuff.abreise) - to_decimal(resbuff.ankunft))

                if resbuff.cancelled < from_date:
                    w1.ytd_saldo =  to_decimal(w1.ytd_saldo) + to_decimal((resbuff.abreise) - to_decimal(resbuff.ankunft))
                else:
                    w1.saldo =  to_decimal(w1.saldo) + to_decimal((resbuff.abreise) - to_decimal(resbuff.ankunft))
            ...
            if lytd_flag or lmtd_flag:
                ...
                for resbuff in db_session.query(Resbuff).filter(
                    ...
                    if resbuff.ankunft < lfrom_date:
                        w1.lytd_saldo =  to_decimal(w1.ytd_saldo) + to_decimal((resbuff.abreise) - to_decimal(resbuff.ankunft))
                    else:
                        w1.lastyr =  to_decimal(w1.lastyr) + to_decimal((resbuff.abreise) - to_decimal(resbuff.ankunft))

                        if lytd_flag:
                            w1.lytd_saldo =  to_decimal(w1.lytd_saldo) + to_decimal((resbuff.abreise) - to_decimal(resbuff.ankunft))
    ...
    def fill_arrdep(rec_w1:int, key_word:string, number1:int, number2:int, number3:int):
        ...
        if number1 != 0:
            ...
            w11 = query(w11_data, filters=(lambda w11: w11.main_code == number1 and not w11.done), first=True)

        if number2 != 0:

            w12 = query(w12_data, filters=(lambda w12: w12.main_code == number2 and not w12.done), first=True)

        if number3 != 0:

            w13 = query(w13_data, filters=(lambda w13: w13.main_code == number3 and not w13.done), first=True)
    ...
    def fill_rmocc(rec_w1:int):
        ...
        w1a = query(w1a_data, filters=(lambda w1a: w1a.main_code == 810), first=True)
    ...
    def fill_new_rmocc(rec_w1:int):
        ...
        for genstat in db_session.query(Genstat).filter((Genstat.datum >= datum1) & (Genstat.datum <= to_date) & (Genstat.resstatus != 13) & (Genstat.nationnr != 0) & (Genstat.segmentcode != 0) & (Genstat.nationnr != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)])).order_by(Genstat.segmentcode).all():
        ...
        for segment in db_session.query(Segment).order_by(Segment._recid).all():
            ...
            for segmentstat in db_session.query(Segmentstat).filter((Segmentstat.datum >= datum1) & (Segmentstat.datum <= to_date) & (Segmentstat.segmentcode == segment.segmentcode)).order_by(Segmentstat._recid).all():
        ...
        if lytd_flag or lmtd_flag:
            ...
            for genstat in db_session.query(Genstat).filter((Genstat.datum >= datum2) & (Genstat.datum <= lto_date) & (Genstat.resstatus != 13) & (Genstat.nationnr != 0) & (Genstat.segmentcode != 0) & (Genstat.nationnr != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)])).order_by(Genstat.segmentcode).all():
            ...
            for segmentstat in db_session.query(Segmentstat).filter((Segmentstat.datum >= datum2) & (Segmentstat.datum <= lto_date) & (Segmentstat.segmentcode == segment.segmentcode)).order_by(Segmentstat._recid).all():
    ...
    def fill_gledger(rec_w1:int):
        ...
        for uebertrag in db_session.query(Uebertrag).filter((Uebertrag.datum >= (from_date - timedelta(days=1))) & (Uebertrag.datum <= (to_date - timedelta(days=1)))).order_by(Uebertrag.datum).all():
            w1.mon_saldo[get_day(uebertrag.datum + timedelta(days=1)) - 1] = uebertrag.betrag
    ...
    def fill_rmocc_perc(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 805), first=True)
        ...
        w12 = query(w12_data, filters=(lambda w12: w12.main_code == 806), first=True)
    ...
    def fill_docc_perc(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 806), first=True)
        ...
        w12 = query(w12_data, filters=(lambda w12: w12.main_code == 810), first=True)
    ...
    def fill_persocc(rec_w1:int):
        ...
        w753 = query(w753_data, filters=(lambda w753: w753.main_code == 753 and not w753.done), first=True)

        w754 = query(w754_data, filters=(lambda w754: w754.main_code == 754 and not w754.done), first=True)

        w755 = query(w755_data, filters=(lambda w755: w755.main_code == 755 and not w755.done), first=True)
    ...
    def fill_avrgrate(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 842), first=True)
    ...
    def fill_avrglrate(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 46), first=True)
    ...
    def fill_avrglodge(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 811), first=True)
    ...
    def fill_zinrstat(rec_w1:int, main_nr:int):
        ...
        if main_nr == 800:
            ...
            w12 = query(w12_data, filters=(lambda w12: w12.main_code == 180 and w12.artnr == rmno and w12.s_artnr.lower()  == (s_rmno).lower()), first=True)

            w13 = query(w13_data, filters=(lambda w13: w13.main_code == 181 and w13.artnr == rmno and w13.s_artnr.lower()  == (s_rmno).lower()), first=True)
            ...
            w11 = query(w11_data, filters=(lambda w11: w11.main_code == 800 and w11.artnr == rmno and w11.s_artnr.lower()  == (s_rmno).lower()), first=True)
                ...
                if w1.main_code == 9106:
                    ...
                    for res_line in db_session.query(Res_line).filter(
                        ...
                        if segment.segmentcode == walk_in and segment.segmentgrup != 0:
                            ...
                            if d_flag:
                                w1.mon_saldo[get_day(datum) - 1] = w1.mon_saldo[get_day(datum) - 1] + 1
    ...
    def fill_new_cancel(rec_w1:int):
        ...
        for res_line, reservation, zimkateg, guest in db_session.query(Res_line, Reservation, Zimkateg, Guest).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Guest,(Guest.gastnr == Res_line.gastnr)).filter((Res_line.resstatus == 9) & ((Res_line.betrieb_gastpay != 3) & (Res_line.betrieb_gastpay != 99)) & (Res_line.ankunft >= datum1) & (Res_line.ankunft <= to_date) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(ankunft, Res_line.name).all():
            ...
            res_cancelled_list.counter = 1

        for res_cancelled_list in query(res_cancelled_list_data):

            if res_cancelled_list.ankunft == to_date:
                w1.tday =  to_decimal(w1.tday) + to_decimal("1")

            if get_month(res_cancelled_list.ankunft) == get_month(to_date) and get_year(res_cancelled_list.ankunft) == get_year(to_date):
                w1.saldo =  to_decimal(w1.saldo) + to_decimal("1")

            if ytd_flag:
                w1.ytd_saldo =  to_decimal(w1.ytd_saldo) + to_decimal("1")
            d_flag = (get_month(res_cancelled_list.ankunft) == get_month(to_date)) and (get_year(res_cancelled_list.ankunft) == get_year(to_date))

            if d_flag:
                w1.mon_saldo[get_day(res_cancelled_list.ankunft) - 1] = w1.mon_saldo[get_day(res_cancelled_list.ankunft) - 1] + 1
    ...
    def fill_los():
        ...
        nonlocal msg_str, error_nr, lfr_date, exrate_betrag, frate, prog_error, ch, month_str, lvcarea, prev_param, k, j, curr_row, curr_col, htl_no, cell_value, chcol, dayname, segmentstat, arrangement, res_line, bill_line, zimmer, zinrstat, zkstat, h_umsatz, segment, genstat, uebertrag, artikel, h_artikel, wgrpdep, umsatz, budget, htparam, exrate, h_cost, h_journal, reservation, zimkateg, guest, h_bill_line, h_bill, bill, queasy, gl_acct, waehrung, l_lager, l_artikel, l_op, h_compli, fbstat, sources, paramtext, parameters
        nonlocal pvilanguage, ytd_flag, jan1, ljan1, lfrom_date, lto_date, pfrom_date, pto_date, from_date, to_date, lytd_flag, lmtd_flag, pmtd_flag, lytoday_flag, lytoday, foreign_flag, budget_flag, foreign_nr, price_decimal, briefnr, link, budget_all, user_init, w1_data, w2_data
        nonlocal segmbuff
        ...
        wlos = query(wlos_data, filters=(lambda wlos: wlos.main_code == 9000), first=True)

        for res_line in db_session.query(Res_line).filter(
                 (Res_line.abreise >= datum1) & (Res_line.ankunf <= to_date) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line.zinr != "") & (Res_line.l_zuordnung[0] != 0) & (Res_line.resstatus != 9) & (Res_line.resstatus != 99) & (Res_line.resstatus != 10) & (Res_line.resstatus != 12)).order_by(Res_line._recid).all():
    ...
    def fill_pax_cover_shift():
        ...
        for h_bill_line.artnr, h_bill_line.departement, h_bill_line.rechnr, h_bill_line.bezeich, h_bill_line.zeit, h_bill_line.bill_datum, h_bill_line.betriebsnr, h_bill_line.tischnr, h_bill_line.betrag, h_bill_line.anzahl, h_bill_line._recid, h_artikel.departement, h_artikel.artnr, h_artikel.service_code, h_artikel.mwst_code, h_artikel.prozent, h_artikel.artnrfront, h_artikel.artart, h_artikel._recid, artikel.departement, artikel.artnrfront, artikel.artnr, artikel.umsatzart, artikel.endkum, artikel._recid, h_bill.belegung, h_bill.resnr, h_bill.reslinnr, h_bill._recid in db_session.query(H_bill_line.artnr, H_bill_line.departement, H_bill_line.rechnr, H_bill_line.bezeich, H_bill_line.zeit, H_bill_line.bill_datum, H_bill_line.betriebsnr, H_bill_line.tischnr, H_bill_line.betrag, H_bill_line.anzahl, H_bill_line._recid, H_artikel.departement, H_artikel.artnr, H_artikel.service_code, H_artikel.mwst_code, H_artikel.prozent, H_artikel.artnrfront, H_artikel.artart, H_artikel._recid, Artikel.departement, Artikel.artnrfront, Artikel.artnr, Artikel.umsatzart, Artikel.endkum, Artikel._recid, H_bill.belegung, H_bill.resnr, H_bill.reslinnr, H_bill._recid).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0) & (H_artikel.artart != 11) & (H_artikel.artart != 12)).join(Artikel,(Artikel.artnr == H_artikel.artnrfront) & (Artikel.departement == H_artikel.departement)).join(H_bill,(H_bill.rechnr == H_bill_line.rechnr) & (H_bill.departement == H_bill_line.departement)).filter(
            ...
            for buff.artnr, buff.departement, buff.rechnr, buff.bezeich, buff.zeit, buff.bill_datum, buff.betriebsnr, buff.tischnr, buff.betrag, buff.anzahl, buff._recid, art_buff.departement, art_buff.artnr, art_buff.service_code, art_buff.mwst_code, art_buff.prozent, art_buff.artnrfront, art_buff.artart, art_buff._recid in db_session.query(Buff.artnr, Buff.departement, Buff.rechnr, Buff.bezeich, Buff.zeit, Buff.bill_datum, Buff.betriebsnr, Buff.tischnr, Buff.betrag, Buff.anzahl, Buff._recid, Art_buff.departement, Art_buff.artnr, Art_buff.service_code, Art_buff.mwst_code, Art_buff.prozent, Art_buff.artnrfront, Art_buff.artart, Art_buff._recid).join(Art_buff,(Art_buff.artnr == Buff.artnr) & (Art_buff.departement == Buff.departement)).filter(
                     (Buff.rechnr == h_bill_line.rechnr) & (Buff.departement == h_bill_line.departement)).order_by(Art_buff.artart).all():
            ...
            if do_it:
                ...
                shift_list = query(shift_list_data, filters=(lambda shift_list: shift_list.ftime <= h_bill_line.zeit and shift_list.ttime >= h_bill_line.zeit), first=True)

                if shift_list:
                    shift = shift_list.shift

                t_list = query(t_list_data, filters=(lambda t_list: t_list.dept == h_bill_line.departement and t_list.datum == h_bill_line.bill_datum and t_list.shift == h_bill_line.betriebsnr), first=True)
                ...
                t_rechnr = query(t_rechnr_data, filters=(lambda t_rechnr: t_rechnr.rechnr == h_bill_line.rechnr and t_rechnr.dept == h_bill_line.departement and t_rechnr.datum == h_bill_line.bill_datum and t_rechnr.shift == shift), first=True)

                if not t_rechnr:
                    t_rechnr = T_rechnr()
                    t_rechnr_data.append(t_rechnr)
        ...
        for t_list in query(t_list_data, sort_by=[("dept",False),("shift",False),("datum",False)]):

            wspc = query(wspc_data, filters=(lambda wspc: wspc.main_code == 1995 and to_int(substring(wspc.s_artnr, 0, 2)) == t_list.dept and to_int(substring(wspc.s_artnr, 2)) == t_list.shift), first=True)
            ...
            wspc = query(wspc_data, filters=(lambda wspc: wspc.main_code == 1996 and to_int(substring(wspc.s_artnr, 0, 2)) == t_list.dept and to_int(substring(wspc.s_artnr, 2)) == t_list.shift), first=True)
    ...
    def fill_pax_cover_shift1(rec_w1:int):
        ...
        btemp_rechnr_data = temp_rechnr_data
        ...
        temp_rechnr = query(temp_rechnr_data, first=True)
        ...
        if temp_rechnr:
            ...
            for h_bill_line.artnr, h_bill_line.departement, h_bill_line.rechnr, h_bill_line.bezeich, h_bill_line.zeit, h_bill_line.bill_datum, h_bill_line.betriebsnr, h_bill_line.tischnr, h_bill_line.betrag, h_bill_line.anzahl, h_bill_line._recid, h_artikel.departement, h_artikel.artnr, h_artikel.service_code, h_artikel.mwst_code, h_artikel.prozent, h_artikel.artnrfront, h_artikel.artart, h_artikel._recid, h_bill.belegung, h_bill.resnr, h_bill.reslinnr, h_bill._recid in db_session.query(H_bill_line.artnr, H_bill_line.departement, H_bill_line.rechnr, H_bill_line.bezeich, H_bill_line.zeit, H_bill_line.bill_datum, H_bill_line.betriebsnr, H_bill_line.tischnr, H_bill_line.betrag, H_bill_line.anzahl, H_bill_line._recid, H_artikel.departement, H_artikel.artnr, H_artikel.service_code, H_artikel.mwst_code, H_artikel.prozent, H_artikel.artnrfront, H_artikel.artart, H_artikel._recid, H_bill.belegung, H_bill.resnr, H_bill.reslinnr, H_bill._recid).join(H_artikel,(H_artikel.artnr == H_bill_line.artnr) & (H_artikel.departement == H_bill_line.departement) & (H_artikel.artart == 0)).join(H_bill,(H_bill.rechnr == H_bill_line.rechnr) & (H_bill.departement == H_bill_line.departement)).filter((H_bill_line.bill_datum >= datum1) & (H_bill_line.bill_datum <= to_date) & (H_bill_line.artnr > 0)).order_by(H_bill_line.bill_datum, H_bill_line.departement, H_bill_line.rechnr, H_bill_line.betriebsnr).all():
                ...
                if curr_rechnr != h_bill_line.rechnr:
                    ...
                    for bh_bill_line.artnr, bh_bill_line.departement, bh_bill_line.rechnr, bh_bill_line.bezeich, bh_bill_line.zeit, bh_bill_line.bill_datum, bh_bill_line.betriebsnr, bh_bill_line.tischnr, bh_bill_line.betrag, bh_bill_line.anzahl, bh_bill_line._recid, bh_artikel.departement, bh_artikel.artnr, bh_artikel.service_code, bh_artikel.mwst_code, bh_artikel.prozent, bh_artikel.artnrfront, bh_artikel.artart, bh_artikel._recid in db_session.query(Bh_bill_line.artnr, Bh_bill_line.departement, Bh_bill_line.rechnr, Bh_bill_line.bezeich, Bh_bill_line.zeit, Bh_bill_line.bill_datum, Bh_bill_line.betriebsnr, Bh_bill_line.tischnr, Bh_bill_line.betrag, Bh_bill_line.anzahl, Bh_bill_line._recid, Bh_artikel.departement, Bh_artikel.artnr, Bh_artikel.service_code, Bh_artikel.mwst_code, Bh_artikel.prozent, Bh_artikel.artnrfront, Bh_artikel.artart, Bh_artikel._recid).join(Bh_artikel,(Bh_artikel.artnr == Bh_bill_line.artnr) & (Bh_artikel.departement == Bh_bill_line.departement) & (Bh_artikel.artart >= 11) & (Bh_artikel.artart <= 12)).filter((Bh_bill_line.rechnr == h_bill_line.rechnr) & (Bh_bill_line.departement == h_bill_line.departement)).order_by(Bh_artikel.artart.desc()).all():
                        ...
                        bline = db_session.query(Bline).filter((Bline.rechnr == bh_bill_line.rechnr) & (Bline.artnr == bh_bill_line.artnr) & (Bline.anzahl == - bh_bill_line.anzahl) & (Bline.recid(bline) != recid(bh_bill_line))).first()
        ...
        for temp_rechnr in query(temp_rechnr_data, filters=(lambda temp_rechnr: temp_rechnr.dept == w1.dept)):
    ...
    def fill_fb_flash(rec_w1:int):
        ...
        nonlocal msg_str, error_nr, lfr_date, exrate_betrag, frate, prog_error, ch, month_str, lvcarea, prev_param, k, j, curr_row, curr_col, htl_no, cell_value, chcol, dayname, segmentstat, arrangement, res_line, bill_line, zimmer, zinrstat, zkstat, h_umsatz, segment, genstat, uebertrag, artikel, h_artikel, wgrpdep, umsatz, budget, htparam, exrate, h_cost, h_journal, reservation, zimkateg, guest, h_bill_line, h_bill, bill, queasy, gl_acct, waehrung, l_lager, l_artikel, l_op, h_compli, fbstat, sources, paramtext, parameters
        nonlocal pvilanguage, ytd_flag, jan1, ljan1, lfrom_date, lto_date, pfrom_date, pto_date, from_date, to_date, start_date, lytd_flag, lmtd_flag, pmtd_flag, lytoday_flag, lytoday, foreign_flag, budget_flag, price_decimal, briefnr, link, budget_all, user_init, w1_data, w2_data
        nonlocal segmbuff
        ...
        if w1.main_code == 2056:
            ...
            for l_op, l_lager, l_artikel in db_session.query(L_op, L_lager, L_artikel).join(L_lager,(L_lager.lager_nr == L_op.lager_nr)).join(L_artikel,(L_artikel.artnr == L_op.artnr) & (L_artikel.endkum == w1.dept)).filter(
                ...
                s_list = query(s_list_data, filters=(lambda s_list: s_list.flag == w1.dept), first=True)
            ...
            for s_list in query(s_list_data, filters=(lambda s_list: s_list.flag == w1.dept)):
    ...
    def fill_fbstat(rec_w1:int):
        ...
        fbstat_dept = query(fbstat_dept_data, filters=(lambda fbstat_dept: fbstat_dept.dept == fbdept), first=True)
        ...
        if do_it:
            ...
            fbstat_dept_data.append(fbstat_dept)

            fbstat_dept.dept = fbdept

            wf1 = query(wf1_data, filters=(lambda wf1: wf1.main_code == 1997 and to_int(substring(wf1.s_artnr, 0, 2)) == fbdept and to_int(substring(wf1.s_artnr, 2)) == 1), first=True)

            wf2 = query(wf2_data, filters=(lambda wf2: wf2.main_code == 1997 and to_int(substring(wf2.s_artnr, 0, 2)) == fbdept and to_int(substring(wf2.s_artnr, 2)) == 2), first=True)

            wf3 = query(wf3_data, filters=(lambda wf3: wf3.main_code == 1997 and to_int(substring(wf3.s_artnr, 0, 2)) == fbdept and to_int(substring(wf3.s_artnr, 2)) == 3), first=True)

            wf4 = query(wf4_data, filters=(lambda wf4: wf4.main_code == 1997 and to_int(substring(wf4.s_artnr, 0, 2)) == fbdept and to_int(substring(wf4.s_artnr, 2)) == 4), first=True)

            wb1 = query(wb1_data, filters=(lambda wb1: wb1.main_code == 1998 and to_int(substring(wb1.s_artnr, 0, 2)) == fbdept and to_int(substring(wb1.s_artnr, 2)) == 1), first=True)

            wb2 = query(wb2_data, filters=(lambda wb2: wb2.main_code == 1998 and to_int(substring(wb2.s_artnr, 0, 2)) == fbdept and to_int(substring(wb2.s_artnr, 2)) == 2), first=True)

            wb3 = query(wb3_data, filters=(lambda wb3: wb3.main_code == 1998 and to_int(substring(wb3.s_artnr, 0, 2)) == fbdept and to_int(substring(wb3.s_artnr, 2)) == 3), first=True)

            wb4 = query(wb4_data, filters=(lambda wb4: wb4.main_code == 1998 and to_int(substring(wb4.s_artnr, 0, 2)) == fbdept and to_int(substring(wb4.s_artnr, 2)) == 4), first=True)

            wo1 = query(wo1_data, filters=(lambda wo1: wo1.wb1.main_code == 1999 and to_int(substring(wo1.s_artnr, 0, 2)) == fbdept and to_int(substring(wo1.s_artnr, 2)) == 1), first=True)

            wo2 = query(wo2_data, filters=(lambda wo2: wo2.main_code == 1999 and to_int(substring(wo2.s_artnr, 0, 2)) == fbdept and to_int(substring(wo2.s_artnr, 2)) == 2), first=True)

            wo3 = query(wo3_data, filters=(lambda wo3: wo3.main_code == 1999 and to_int(substring(wo3.s_artnr, 0, 2)) == fbdept and to_int(substring(wo3.s_artnr, 2)) == 3), first=True)

            wo4 = query(wo4_data, filters=(lambda wo4: wo4.main_code == 1999 and to_int(substring(wo4.s_artnr, 0, 2)) == fbdept and to_int(substring(wo4.s_artnr, 2)) == 4), first=True)
    ...
    def fill_avrgllodge(rec_w1:int):
        ...
        w11 = query(w11_data, filters=(lambda w11: w11.main_code == 812), first=True)
    ...
    def fill_value1(recid1_w1:int, recid2_w1:int, val_sign:int):
        ...
        curr_child_data = w1_data
        Curr_w2 = W2
        curr_w2_data = w2_data

        parent = query(parent_data, filters=(lambda parent: parent._recid == recid1_w1), first=True)

        child = query(child_data, filters=(lambda child: child._recid == recid2_w1), first=True)
    ...
    open(file_path, "w").close()
        ...
        for parameters in db_session.query(Parameters).filter((Parameters.progname == ("FO-macro").lower()) & (Parameters.section == to_string(briefnr))).order_by(Parameters.varname).all():
            ...
            if prev_param != parameters.varname:
                ...
                if parameters.vtype != 0:
                    w1 = query(w1_data, filters=(lambda w1: w1.varname == parameters.vstring), first=True)
        ...
        for stream_list in query(stream_list_data, sort_by=[("ccol", False), ("crow", False)]):
            if stream_list.cval != "":
                with open(f"/usr1/vhp/tmp/outputFO_{htl_no}.txt", "a") as f:
                    f.write(f"{stream_list.crow};{stream_list.ccol};{stream_list.cval}\n")
            else:
                with open(f"/usr1/vhp/tmp/outputFO_{htl_no}.txt", "a") as f:
    ...
    if not queasy:
        ...
        queasy.key = 293
        queasy.deci1 = to_decimal(briefnr)
        queasy.deci2 = to_decimal(get_current_time_in_seconds())
        queasy.char1 = trim(user_init)
        queasy.char2 = trim(link)
        queasy.char3 = "From-date : " + to_string(from_date, "99/99/99") + " - " + "To-date : " + to_string(to_date, "99/99/99")
