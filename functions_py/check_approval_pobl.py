...
def check_approval_pobl(user_init:string):
    ...
    po_list_data, Po_list = create_model("Po_list", {"lief_nr":int, "docu_nr":string, "bestelldatum":date, "lieferdatum":date, "wabkurz":string, "bestellart":string, "gedruckt":date, "besteller":string, "lief_fax_3":string, "lief_fax_2":string, "rechnungswert":Decimal, "rec_id":int, "approval_lvl":[bool,4], "approval_id":[string,4], "need_approval":string})
    q245_data, Q245 = create_model("Q245", {"key":int, "docu_nr":string, "user_init":string, "app_id":string, "app_no":int, "sign_id":int})
    mess_list_data, Mess_list = create_model("Mess_list", {"nr":int, "reslinnr":int, "betriebsnr":int, "mess_recid":int, "datum":date, "mess_str":string})
    qsy_list_data, Qsy_list = create_model("Qsy_list", {"docu_nr":string})
    ...
    def generate_output():
        ...
        nonlocal po_list_data, q245_data, mess_list_data, qsy_list_data
    ...
    while None != l_orderhdr:
        ...
        while None != queasy:
            ...
            qsy_list = query(qsy_list_data, filters=(lambda qsy_list: trim(qsy_list.docu_nr) == trim(queasy.char1)), first=True)

            if not qsy_list:
                qsy_list = Qsy_list()
                qsy_list_data.append(qsy_list)
    ...
    if use_po_esignature:
        ...
        qsy_list = query(qsy_list_data, first=True)
        while None != qsy_list:
            ...
            qsy_list = query(qsy_list_data, next=True)
