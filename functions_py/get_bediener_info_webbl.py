...
def get_bediener_info_webbl(user_name:string, user_init:string):
    ...
    signature_list_data = []
    dept_name:string = ""
    queasy = bbediener = paramtext = None
    ...
    bbediener = get_cache (Bediener, {"userinit": [(eq, user_init)],"username": [(eq, user_name)]})

    if bbediener:

        buff_user = get_cache (Queasy, {"key": [(eq, 134)],"number1": [(eq, bbediener.nr)],"betriebsnr": [(eq, 0)],"deci1": [(eq, 0)],"logi1": [(eq, False)]})

        bdept = get_cache (Queasy, {"key": [(eq, 19)],"number1": [(eq, bbediener.user_group)]})
        ...
        bediener_info.user_number = bbediener.nr
        bediener_info.user_init = bbediener.userinit
        bediener_info.user_name = bbediener.username
        bediener_info.dept_number = bbediener.user_group
        ...
        totpdata = get_cache (Queasy, {"key": [(eq, 341)],"char1": [(eq, bbediener.username)]})
        ...
        epoch_signature, signature_list_data = create_signature(bbediener.username, value_list_data)
