...
def co_guest_1_webbl(case_type:int, pvilanguage:int, fr_date:date, to_date:date, price_decimal:int):
    ...
    def generate_output():
        nonlocal cl_list_data, tot_rm, tot_deposit, tot_cash, tot_cc, tot_cl, tot_amt, tot_bb, lvcarea, res_line, bill, reservation, guest, zimkateg, bill_line, artikel, arrangement
    ...
    def disp_dubalance():
        ...
        for res_line.resnr, res_line.resstatus, res_line.reslinnr, res_line.zinr, res_line.zipreis, res_line.ankunft, res_line.abreise, res_line.abreisezeit, res_line.gastnrmember, res_line.gastnr, res_line.zikatnr, res_line.name, res_line.arrangement, res_line._recid, reservation.resnr, reservation.name, reservation._recid in db_session.query(Res_line.resnr, Res_line.resstatus, Res_line.reslinnr, Res_line.zinr, Res_line.zipreis, Res_line.ankunft, Res_line.abreise, Res_line.abreisezeit, Res_line.gastnrmember, Res_line.gastnr, Res_line.zikatnr, Res_line.name, Res_line.arrangement, Res_line._recid, Reservation.resnr, Reservation.name, Reservation._recid).join(Reservation,(Reservation.resnr == Res_line.resnr)).filter((Res_line.active_flag == 2) & (Res_line.ankunft >= fr_date) & (Res_line.ankunft <= to_date) & (Res_line.abreise == Res_line.ankunft) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 99)).order_by(Res_line.zinr, Res_line.resnr, Res_line.reslinnr).all():
            ...
            if do_it :
                ...
                if res_line.arrangement != None:
                    tmp_argt = res_line.arrangement.strip()
                else:
                    tmp_argt = res_line.arrangement
