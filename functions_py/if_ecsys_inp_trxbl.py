...
from models import Billjournal, H_bill_line, Artikel, Htparam, H_artikel

def if_ecsys_inp_trxbl(outlet:string, inp_excl_art:string):

    prepare_cache ([Billjournal, H_bill_line, Artikel, Htparam, H_artikel])
    ...
    service_code:int = 0
    tot_price_amount:Decimal = to_decimal("0.0")
    tot_vat:Decimal = to_decimal("0.0")
    tot_service:Decimal = to_decimal("0.0")
    tot_amount:Decimal = to_decimal("0.0")
    curr_rechnr:int = 0
    billjournal = h_bill_line = artikel = htparam = h_artikel = None

    trx_list = journal_list = h_bill_list = t_invoice = t_excl = b_billjournal = b_h_bill_line = b_artikel = buffjournal = h_bill_buff = None
    ...
    def generate_output():
        nonlocal trx_list_data, datechar, bill_date, i, excl_pair, excl_art, excl_dept, serv, vat, vat2, fact, netto, serv_charge, serv_betrag, vat_proz, service_proz, service_code, tot_price_amount, tot_vat, tot_service, tot_amount, curr_rechnr, billjournal, h_bill_line, artikel, htparam, h_artikel
        nonlocal outlet, inp_excl_art
        nonlocal b_billjournal, b_h_bill_line, b_artikel, buffjournal, h_bill_buff


        nonlocal trx_list, journal_list, h_bill_list, t_invoice, t_excl, b_billjournal, b_h_bill_line, b_artikel, buffjournal, h_bill_buff
    ...
    if outlet != "":
        ...
        if to_int(outlet) == 0:
            ...
            for h_bill_line in db_session.query(H_bill_line).filter(
                     (H_bill_line.rechnr > 0) & (H_bill_line.bill_datum == bill_date) & (H_bill_line.departement == to_int(outlet)) & (H_bill_line.zeit >= 0) & (H_bill_line.artnr > 0) & (H_bill_line.betrag != 0)).order_by(H_bill_line.departement, H_bill_line.rechnr, H_bill_line.sysdate.desc(), H_bill_line.zeit.desc()).all():
