...
def web_interface_arl_list_run_mi_invoice_1bl(resnr:int, curr_resnr:int, arl_list_reslinnr:int, t_active_flag:int, printtype:int, split_bill:bool):
    ...
    s_list = t_list = t_reservation = t_res_line = t_guest = g_list = rline = mbill = mainres = b_bill = b_guest = None
    ...
    t_list_data, T_list = create_model("T_list", {"nr":int, "ankunft":date, "abreise":date, "bezeich":string, "rmcat":string, "preis":Decimal, "lrate":Decimal, "tage":int, "date1":date, "date2":date, "qty":int, "betrag":Decimal, "erwachs":int, "kind1":int, "kind2":int, "vat":Decimal, "svc":Decimal, "zinr":string, "depo_billjour":bool, "resno_billjour":int, "period_stay":string})
    ...
    g_list_data, G_list = create_model_like(Guest)
    ...
    def generate_output():
        nonlocal err_flag, avail_master, avail_bill, reslinnr, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_data, t_res_line_data, t_guest_data, t_list_data, g_list_data, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, serv1, vat1, vat3, fact1, netto, t_char, t_resnr, reservation, res_line, guest, bill, htparam, master, waehrung, bill_line, artikel, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, billjournal, genstat, exrate
        nonlocal resnr, curr_resnr, arl_list_reslinnr, t_active_flag, printtype, split_bill
        nonlocal rline, mbill, mainres, b_bill, b_guest


        nonlocal s_list, t_list, t_reservation, t_res_line, t_guest, g_list, rline, mbill, mainres, b_bill, b_guest
        nonlocal s_list_data, t_list_data, t_reservation_data, t_res_line_data, t_guest_data, g_list_data

        return {"err_flag": err_flag, "avail_master": avail_master, "avail_bill": avail_bill, "reslinnr": reslinnr, "master_rechnr": master_rechnr, "bill_rechnr": bill_rechnr, "mainres_gastnr": mainres_gastnr, "t-reservation": t_reservation_data, "t-res-line": t_res_line_data, "t-guest": t_guest_data, "t-list": t_list_data, "g-list": g_list_data}
    ...
    def read_proforma_inv(resnr:int, rechnr:int):
        ...
        for s_list in query(s_list_data, sort_by=[("ankunft",False),("bezeich",False),("erwachs",False)]):
        ...
        for t_list in query(t_list_data):
            ...
            t_list.period_stay = to_string(t_list.date1, "99/99/9999") + " - " + to_string(t_list.date2, "99/99/9999")
        ...
        if rechnr > 0:
            ...
            for bill_line.bezeich, bill_line.betrag, bill_line.bill_datum, bill_line._recid, artikel.bezeich, artikel.artnr, artikel.departement, artikel.epreis, artikel.artart, artikel._recid in db_session.query(Bill_line.bezeich, Bill_line.betrag, Bill_line.bill_datum, Bill_line._recid, Artikel.bezeich, Artikel.artnr, Artikel.departement, Artikel.epreis, Artikel.artart, Artikel._recid).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement)).filter(
                ...
                if do_it:
                    ...
                    t_list.svc =  to_decimal(netto) * to_decimal(serv1)
            ...
            for billjournal in db_session.query(Billjournal).filter(
                     (Billjournal.rechnr == 0) & (Billjournal.anzahl != 0) & (matches(Billjournal.bezeich,"*Refund #*"))).order_by(Billjournal._recid).all():
                t_char = entry(1, billjournal.bezeich, "#")
                t_resnr = to_int(entry(0, t_char, "]"))

                for t_list in query(t_list_data, filters=(lambda t_list: t_list.depo_billjour and t_list.resno_billjour == t_resnr)):
    ...
    if res_line:
        ...
        b_guest = db_session.query(B_guest).filter(
                 (B_guest.gastnr == res_line.gastnrmember)).first()

        if b_guest:
            g_list = G_list()
            g_list_data.append(g_list)

            buffer_copy(b_guest, g_list)
