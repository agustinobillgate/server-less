...
def ts_restinv_posting_menu_webbl(case_type:int, pvilanguage:int, tischnr:int, curr_dept:int, cancel_reason:string, 
    ...
    def update_bill(h_artart:int, h_artnrfront:int, menu_number:int):
        ...
        if zugriff:
            t_submenu_list_data.clear()

            for submenu_list in query(submenu_list_data, filters=(lambda submenu_list: submenu_list.zknr == menu_number)):
    ...
    if first_flag:
        ...
        for menu_list in query(menu_list_data, filters=(lambda menu_list: menu_list.nr > 0), sort_by=[("nr",False)]):
            ...
            if round(amount, price_decimal) != amount:
            ...
            if price != 0 and amount == 0:
                ...
                update_bill(t_h_artikel.artart, t_h_artikel.artnrfront, menulist_nr)
