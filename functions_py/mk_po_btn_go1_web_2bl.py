...
def mk_po_btn_go1_web_2bl(t_l_orderhdr_data: [T_l_orderhdr], t_l_order_data: [T_l_order], docu_nr: string,
    ...
    avail_hdrbuff = False
    new_docu_nr = ""
    l_orderhdr = l_order = l_artikel = waehrung = queasy = bediener = res_history = htparam = None
    ...
    def generate_output():
        ...
        return {"fl_code": fl_code, 
                "avail_hdrbuff": avail_hdrbuff, 
                "new_docu_nr": new_docu_nr
                }
    ...
    def new_po_number():
        ...
        htparam = db_session.query(Htparam).filter(Htparam.paramnr == 973).first()
        ...
        if htparam.paramgruppe == 21 and htparam.flogical:
            ...
            for l_orderhdr1 in db_session.query(L_orderhdr1).filter((get_month(L_orderhdr1.bestelldatum) == mm) & 
                                                                    (get_year(L_orderhdr1.bestelldatum) == yy) & 
                                                                    (L_orderhdr1.betriebsnr <= 1) & 
                                                                    (matches(L_orderhdr1.docu_nr, "P*"))).order_by(L_orderhdr1.docu_nr.desc()).all():
    ...
    waehrung = db_session.query(Waehrung).filter(Waehrung.wabkurz == currency_screen_value).first()
    if waehrung:
    ...
    if not t_l_order:
        ...
        hdrbuff = db_session.query(Hdrbuff).filter(
            (Hdrbuff.docu_nr == l_orderhdr.docu_nr) & (Hdrbuff._recid != l_orderhdr._recid)).with_for_update().first()
    ...
    for t_l_order in query(t_l_order_data, filters=(lambda t_l_order: t_l_order.pos > 0)):
        ...
        l_art1 = db_session.query(L_art1).filter(L_art1.artnr == t_l_order.artnr).first()
        ...
        if l_order:
            l_order.quality = f"{to_string(t_l_order.disc, "99.99")}" +\
                f" {to_string(t_l_order.vat, "99.99")}" +\
                f" {to_string(t_l_order.disc2, "99.99")}" +\
                f" {to_string(t_l_order.disc_val, ">,>>>,>>>,>>9.999")}" +\
                f" {to_string(t_l_order.disc2_val, ">,>>>,>>>,>>9.999")}" +\
                f" {to_string(t_l_order.vat_val, ">,>>>,>>>,>>9.999")}" +\
                f" {to_string(t_l_order.price0, ">>,>>>,>>>,>>9.99")}" +\
                f" {to_string(t_l_order.brutto, ">>,>>>,>>>,>>9.99")}"
            ...
            queasy = db_session.query(Queasy).filter(
                (Queasy.key == 304) & 
                (Queasy.char1 == t_l_order.docu_nr) & 
                (Queasy.number1 == t_l_order.artnr)).with_for_update().first()
    ...
    l_od = db_session.query(L_od).filter(
        (L_od.docu_nr == docu_nr) &
        (L_od.pos == 0) &
        (L_od.op_art == 2)).with_for_update().first()
