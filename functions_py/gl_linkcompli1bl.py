...
from models import H_artikel, Gl_acct, Htparam, Hoteldpt, H_compli, Exrate, Artikel, H_cost, L_artikel, H_rezept
...
def gl_linkcompli1bl(pvilanguage:int, from_date:date, to_date:date, double_currency:bool, foreign_nr:int, exchg_rate:Decimal, user_init:string):
    ...
    prepare_cache ([H_artikel, Gl_acct, Htparam, Hoteldpt, H_compli, Exrate, Artikel, H_cost, L_artikel, H_rezept])
    ...
    h_artikel = gl_acct = htparam = hoteldpt = h_compli = exrate = artikel = h_cost = l_artikel = h_rezept = None

    c_list = s_list = g_list = t_g_list = None

    c_list_data, C_list = create_model("C_list", {"datum":date, "dept":int, "bemerk":string, "add_note":string, "fibukonto":string, "f_acct":string, "b_acct":string, "o_acct":string, "rechnr":int, "p_artnr":int, "f_cost":Decimal, "b_cost":Decimal, "o_cost":Decimal, "t_cost":Decimal})
    s_list_data, S_list = create_model("S_list", {"fibukonto":string, "bezeich":string, "credit":Decimal, "debit":Decimal})
    ...
    def generate_output():
        nonlocal curr_anz, debits, credits, remains, msg_str, t_g_list_data, s_list_data, lvcarea, curr_i, billdate, price_type, price, rate, incl_service, incl_mwst, h_artikel, gl_acct, htparam, hoteldpt, h_compli, exrate, artikel, h_cost, l_artikel, h_rezept
        ...
        nonlocal c_list_data, s_list_data, g_list_data, t_g_list_data

        return {"curr_anz": curr_anz, "debits": debits, "credits": credits, "remains": remains, "msg_str": msg_str, "t-g-list": t_g_list_data, "s-list": s_list_data}
    ...
    def step_two():
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            if transfer_ldry or (not transfer_ldry and hoteldpt.num != ldry):
                ...
                h_compli = H_compli()
                h_art = H_artikel()
                for h_compli.datum, h_compli.departement, h_compli.rechnr, h_compli.p_artnr, h_compli.artnr, h_compli.anzahl, h_compli.epreis, h_compli._recid, h_art.artnrfront, h_art.departement, h_art.artnr, h_art.bezeich, h_art.artnrlager, h_art.artnrrezept, h_art.prozent, h_art._recid in db_session.query(H_compli.datum, H_compli.departement, H_compli.rechnr, H_compli.p_artnr, H_compli.artnr, H_compli.anzahl, H_compli.epreis, H_compli._recid, H_art.artnrfront, H_art.departement, H_art.artnr, H_art.bezeich, H_art.artnrlager, H_art.artnrrezept, H_art.prozent, H_art._recid).join(H_art,(H_art.departement == H_compli.departement) & (H_art.artnr == H_compli.p_artnr) & (H_art.artart == 11)).filter(
        ...
        for c_list in query(c_list_data, filters=(lambda c_list: c_list.t_cost != 0), sort_by=[("dept",False),("rechnr",False)]):
            add_list()
    ...
    def disp_it():
        ...
        for gl_acct in db_session.query(Gl_acct).filter(
                 ((Gl_acct.fibukonto.in_(list(set([g_list.fibukonto for g_list in g_list_data])))))).order_by(g_list.zeit).all():
    ...
    def add_list():
        ...
        s_list = query(s_list_data, filters=(lambda s_list: s_list.fibukonto == c_list.fibukonto), first=True)
        ...
        if c_list.f_acct != "":
            ...
            s_list = query(s_list_data, filters=(lambda s_list: s_list.fibukonto == c_list.f_acct), first=True)
        ...
        if c_list.b_acct != "":
            ...
            s_list = query(s_list_data, filters=(lambda s_list: s_list.fibukonto == c_list.b_acct), first=True)
        ...
        if c_list.o_acct != "":
            ...
            s_list = query(s_list_data, filters=(lambda s_list: s_list.fibukonto == c_list.o_acct), first=True)
