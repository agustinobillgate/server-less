...
def web_interface_pr_list_disp_list1_2lnlbl(char1: str, billdate: date, from_date: date, to_date: date, outstand_flag: bool, expired_flag: bool, approve_flag: bool, reject_flag: bool, artnumber: int, deptno: int, sorttype: int):
    ...
    s_list = usrbuff = t_lieferant = p_list = t_list = sbuff = tbuff = None
    ...
    P_list = S_list
    p_list_data = s_list_data

    T_list = S_list
    t_list_data = s_list_data

    db_session = local_storage.db_session

    def generate_output():
        nonlocal s_list_data, bediener, l_lieferant, l_orderhdr, parameters, l_order, l_artikel, l_pprice, gl_acct
        nonlocal char1, billdate, from_date, to_date, outstand_flag, expired_flag, approve_flag, reject_flag, artnumber, deptno, sorttype
        nonlocal usrbuff, t_lieferant, p_list, t_list
        nonlocal s_list, usrbuff, t_lieferant, p_list, t_list, sbuff, tbuff
    ...
    def disp_list(pr_nr: str):
        ...
        if pr_nr != "":
            l_orderhdr = get_cache(
                L_orderhdr, {"betriebsnr": [(ge, 9)], "docu_nr": [(eq, pr_nr)], "bestelldatum": [(eq, billdate)], "lief_nr": [(eq, 0)]})
            ...
            l_order = get_cache(
                L_order, {"docu_nr": [(eq, l_orderhdr.docu_nr)], "pos": [(eq, 0)], "lief_nr": [(eq, 0)]})
            s_list = S_list()
            ...
            if get_index(l_orderhdr.lief_fax[1], "|") > 0:
                ...
                s_list.app_rej = trim(
                    entry(3, entry(0, l_orderhdr.lief_fax[1], "|"), ";"))

            if l_order:
                s_list.loeschflag = l_order.loeschflag

            l_order = get_cache(
                L_order, {"docu_nr": [(eq, l_orderhdr.docu_nr)], "loeschflag": [(le, 1)], "pos": [(gt, 0)], "lief_nr": [(eq, 0)]})
            while l_order is not None:
                l_artikel = get_cache(
                ...
                if l_artikel:
                    ...
                    for l_pprice.bestelldatum, l_pprice.einzelpreis, l_pprice._recid, t_lieferant.firma, t_lieferant._recid in db_session.query(L_pprice.bestelldatum, L_pprice.einzelpreis, L_pprice._recid, T_lieferant.firma, T_lieferant._recid).join(T_lieferant, (T_lieferant.lief_nr == L_pprice.lief_nr)).filter(
                            (L_pprice.artnr == l_artikel.artnr)).order_by(L_pprice.bestelldatum.desc()).all():
                ...
                l_order = db_session.query(L_order).filter(
                    (L_order.docu_nr == l_orderhdr.docu_nr) & (L_order.loeschflag <= 1) & (L_order.pos > 0) & (L_order.lief_nr == 0) & (L_order._recid > curr_recid)).first()
        ...
        for l_orderhdr in db_session.query(L_orderhdr).filter(
            ...
            l_order = get_cache(
                L_order, {"docu_nr": [(eq, l_orderhdr.docu_nr)], "pos": [(eq, 0)], "lief_nr": [(eq, 0)], "artnr": [(eq, artnumber)]})
            do_it = True
            ...
            if do_it:
                ...
                sbuff = query(sbuff_data, filters=(
                    lambda sbuff: sbuff._recid == s_list._recid), first=True)

                l_order = get_cache(
                    L_order, {"docu_nr": [(eq, l_orderhdr.docu_nr)], "pos": [(gt, 0)], "lief_nr": [(eq, 0)], "artnr": [(eq, artnumber)]})
                while l_order is not None:
                    l_artikel = get_cache(
                        L_artikel, {"artnr": [(eq, l_order.artnr)]})
                    ...
                    curr_recid = l_order._recid
                    l_order = db_session.query(L_order).filter(
                        (L_order.docu_nr == l_orderhdr.docu_nr) & (L_order.pos > 0) & (L_order.lief_nr == 0) & (L_order.artnr == artnumber) & (L_order._recid > curr_recid)).first()
    ...
    for s_list in query(s_list_data):
        ...
        l_orderhdr = get_cache(
            L_orderhdr, {"docu_nr": [(eq, s_list.docu_nr)]})

        if l_orderhdr:
            s_list.lief_fax2 = l_orderhdr.lief_fax[1]
            s_list.lief_fax3 = l_orderhdr.lief_fax[2]

    for s_list in query(s_list_data, filters=(lambda s_list: s_list.pos == 0)):

        p_list = query(p_list_data, filters=(
            lambda p_list: p_list.pos > 0 and p_list.docu_nr == s_list.docu_nr), first=True)

        if not p_list:
            s_list_data.remove(s_list)

        t_list = query(t_list_data, filters=(lambda t_list: t_list.pos > 0 and t_list.docu_nr == s_list.docu_nr and t_list.loeschflag != sorttype), first=True)

        if t_list:
            s_list_data.remove(s_list)

    if deptno == 0:
        for s_list in query(s_list_data, filters=(lambda s_list: not ((s_list.artnr == 0 and s_list.loeschflag == sorttype) or (s_list.artnr > 0 and s_list.loeschflag == sorttype))), sort_by=[("docu_nr", False), ("pos", False)]):
            s_list_data.remove(s_list)
    else:
        for s_list in query(s_list_data, filters=(lambda s_list: not (((s_list.artnr == 0 and s_list.loeschflag == sorttype) or (s_list.artnr > 0 and s_list.loeschflag == sorttype)) and s_list.deptnr == deptno)), sort_by=[("docu_nr", False), ("pos", False)]):
            s_list_data.remove(s_list)
