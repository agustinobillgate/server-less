...
from models import Res_line, Outorder, Zimmer, Zimkateg, History

def roomplan2_1bl(from_room:string, curr_date:date, ci_date:date):
    ...
    a:int = 0
    res_line = outorder = zimmer = zimkateg = history = None
    ...
    t_res_line3_data, T_res_line3 = create_model_like(Res_line, {"rec_id":int})
    ...
    def generate_output():
        nonlocal do_it, to_date, t_res_line_data, t_res_line2_data, t_outorder_data, t_outorder1_data, room_list_data, item_fgcol, stat_list, a, res_line, outorder, zimmer, zimkateg, history
        nonlocal from_room, curr_date, ci_date


        nonlocal t_res_line, t_outorder, t_outorder1, t_res_line2, t_res_line3, room_list
        nonlocal t_res_line_data, t_outorder_data, t_outorder1_data, t_res_line2_data, t_res_line3_data, room_list_data

        return {"do_it": do_it, "to_date": to_date, "t-res-line": t_res_line_data, "t-res-line2": t_res_line2_data, "t-outorder": t_outorder_data, "t-outorder1": t_outorder1_data, "room-list": room_list_data}
    ...
    def create_browse():
        ...
        for zimmer in db_session.query(Zimmer).filter(
                 (Zimmer.zinr >= (from_room).lower())).order_by(Zimmer.zinr, Zimmer._recid).all():
        ...
        if curr_date >= ci_date:
            for res_line in db_session.query(Res_line).filter((Res_line.active_flag <= 1) & (Res_line.abreise == curr_date) & (Res_line.resstatus != 12) & (Res_line.zinr != "") & (Res_line.zinr >= (from_room).lower())).order_by(Res_line._recid).all():
            ...
            for res_line in db_session.query(Res_line).filter((Res_line.active_flag == 2) & (Res_line.abreise == curr_date) & (Res_line.resstatus == 8) & (Res_line.zimmerfix == False) & (Res_line.zinr >= (from_room).lower())).order_by(Res_line.ankunft.desc()).all():
        ...
        for res_line in db_session.query(Res_line).filter((((Res_line.resstatus <= 2) | (Res_line.resstatus == 5)) & (Res_line.zinr != "") & (Res_line.zinr >= (from_room).lower()) & (Res_line.active_flag == 0) & not_(to_date < Res_line.ankunft) & not_ (curr_date >= Res_line.abreise)) | (((Res_line.resstatus == 6) | (Res_line.resstatus == 13)) & (Res_line.active_flag == 1) & (Res_line.abreise > curr_date) & (Res_line.zinr >= (from_room).lower()))).order_by(Res_line._recid).all():
            ...
            if history:
                ...
                t_res_line3_data.append(t_res_line3)
        ...
        if curr_date < ci_date:
            for res_line in db_session.query(Res_line).filter((Res_line.resstatus == 8) & (Res_line.active_flag == 2) & (Res_line.zinr >= (from_room).lower()) & (Res_line.abreise >= Res_line.ankunft) & not_(Res_line.zimmerfix) & not_(to_date < Res_line.ankunft) & not_(curr_date >= Res_line.abreise)).order_by(Res_line.ankunft.desc()).all():
                ...
                if do_it:
                    ...
                    t_res_line2.ankunft = res_line.ankunft
                    t_res_line2.abreise = res_line.abreise
    ...
    def assign_room_list():
        ...
        for t_res_line in query(t_res_line_data):
            ...
            i = (datum1 - curr_date + timedelta(days=1)).days

            t_res_line3 = query(t_res_line3_data, filters=(lambda t_res_line3: t_res_line3.resnr == t_res_line.resnr and t_res_line3.reslinnr == t_res_line.reslinnr and t_res_line3.zinr == t_res_line.zinr), first=True)

            if t_res_line3:

                room_list = query(room_list_data, filters=(lambda room_list: room_list.zinr == t_res_line3.zinr), first=True)

                if t_res_line3.resstatus == 13 and room_list.recid1[i - 1] != 0:
                    pass
                else:
                    room_list.room[i - 1] = substring(t_res_line3.name, 0, length(t_res_line3.name))

                    if (t_res_line3.resstatus <= 2 or t_res_line3.resstatus == 5):
                        room_list.gstatus[i - 1] = 1

                        if t_res_line3.ankunft == curr_date:

                            t_outorder1 = query(t_outorder1_data, filters=(lambda t_outorder1: t_outorder1.zinr == t_res_line3.zinr and t_outorder1.betriebsnr == t_res_line3.resnr), first=True)
                    ...
                    elif t_res_line3.resstatus == 6:
                        room_list.gstatus[i - 1] = 2

                    elif t_res_line3.resstatus == 13:
                        room_list.gstatus[i - 1] = 3
                        
                    room_list.recid1[i - 1] = t_res_line3.rec_id
                    room_list.resnr[i - 1] = t_res_line3.resnr

                    if t_res_line3.ziwech_zeit != 0:
                        room_list.bcol[i - 1] = t_res_line3.ziwech_zeit
                        room_list.fcol[i - 1] = item_fgcol[room_list.bcol[i - 1]]
                
                i = i + 1
                datum1 = datum1 + timedelta(days=1)
