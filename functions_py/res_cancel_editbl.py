...
def res_cancel_editbl(res_mode:string, done:bool, inp_resno:int, tot_qty:int, reslinno:int, grpflag:bool):
    successflag = False
        ...
        if (res_mode.lower()  == ("new").lower()  or res_mode.lower()  == ("insert").lower()  or res_mode.lower()  == ("qci").lower()) and not done:
            ...
            if not res_line:
                ...
                rline = db_session.query(Rline).filter(
                         (Rline.resnr == inp_resno) & (Rline.reslinnr == reslinno)).with_for_update().first()
                gastno = rline.gastnr
            ...
            if res_mode.lower()  == ("new").lower()  or res_mode.lower()  == ("insert").lower() :
                ...
                # res_line = get_cache (Res_line, {"resnr": [(eq, inp_resno)],"reslinnr": [(eq, reslinno)]})
                res_line = db_session.query(Rline).filter(
                         (Rline.resnr == inp_resno) & (Rline.reslinnr == reslinno)).with_for_update().first()
