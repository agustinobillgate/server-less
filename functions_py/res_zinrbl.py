...
def res_zinrbl(case_type:int, resno:int, reslinno:int, zikatno:int, ankunft1:date, abreise1:date):
    ...
    def generate_output():
        nonlocal room_list_data, htl_feature_data, vhp_limited, res_rowid, ci_date, res_line, zimmer, outorder, paramtext, guest
        ...
        nonlocal om_list_data, htl_feature_data, room_list_data

        return {"room-list": room_list_data, "htl-feature": htl_feature_data}
    ...
    def create_list():
        ...
        for resline in db_session.query(Resline).filter(
            ...
            if to_int(resline._recid) == res_rowid:
                ...
                room_list = query(room_list_data, filters=(lambda room_list: room_list.zinr == resline.zinr), first=True)
    ...
    def complete_list():
        ...
        for outorder in db_session.query(Outorder).filter(
            ...
            om_list = query(om_list_data, filters=(lambda om_list: om_list.zinr == outorder.zinr), first=True)
        ...
        for room_list in query(room_list_data):
            ...
            om_list = query(om_list_data, filters=(lambda om_list: om_list.zinr == room_list.zinr), first=True)
        ...
        for i in range(1,length(zimmer.himmelsr)  + 1) :
            ...
            if substring(zimmer.himmelsr, i - 1, 1) == (";").lower()  or substring(zimmer.himmelsr, i - 1, 1) == (",").lower() :
                ...
                if (length(ch) != 0) and (substring(ch, 0, 1) != ("$").lower()):
                    ...
                    htl_feature = query(htl_feature_data, filters=(lambda htl_feature: htl_feature.s.lower()  == (ch).lower()), first=True)

                    if not htl_feature:
                        htl_feature = Htl_feature()
                        htl_feature_data.append(htl_feature)
