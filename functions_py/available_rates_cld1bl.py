...
from functions_py.calculate_occupied_roomsbl import calculate_occupied_roomsbl
from sqlalchemy import func
from models import Ratecode, Queasy, Htparam, Guest_pr, Zimkateg, Waehrung, Segment, Arrangement, Res_line
...
def available_rates_cld1bl(frdate: date, todate: date, adult_child_str: str, ind_gastno: int, room_avail_list_data: Room_avail_list):

    prepare_cache([Ratecode, Queasy, Htparam, Guest_pr, Zimkateg, Waehrung, Segment, Arrangement, Res_line])
    ...
    doit: bool = False
    i_counter: int = 0
    ratecode = queasy = htparam = guest_pr = zimkateg = waehrung = segment = arrangement = res_line = None

    created_list = rate_list = dynarate_list = selected_ratelist = room_avail_list = buff_rlist = buff_dynarate = bratecode = bqueasy = qsy18 = pratecode = ratebuff = dynabuff = pqueasy = None
    ...
    Pratecode = create_buffer("Pratecode", Ratecode)
    ...
    def generate_output():
        nonlocal rate_list_data, adult, child, rooms, inp_resnr, inp_reslinnr, rmtype, argtno, markno, wahrno, tokcounter, iftask, mestoken, mesvalue, currency, mapcode, datum, ankunft, dynacode, rmtype_str, curr_i, curr_date, rm_rate, rate_found, restricted, kback_flag, global_occ, dd, mm, yyyy, ci_date, co_date, map_code, use_it, w_day, occ_rooms, wd_array, occ_room_array, not_found_flag, do_it, rate_created, allotment_ok, calc_rm, doit, i_counter, ratecode, queasy, htparam, guest_pr, zimkateg, waehrung, segment, arrangement, res_line
        nonlocal frdate, todate, adult_child_str, ind_gastno
        nonlocal buff_rlist, buff_dynarate, bratecode, bqueasy, qsy18, pratecode, ratebuff, dynabuff, pqueasy
        nonlocal created_list, rate_list, dynarate_list, selected_ratelist, room_avail_list, buff_rlist, buff_dynarate, bratecode, bqueasy, qsy18, pratecode, ratebuff, dynabuff, pqueasy
        nonlocal created_list_data, rate_list_data, dynarate_list_data, selected_ratelist_data
    ...
    def check_allotment(origcode: str, statcode: str, curr_date: date):
        nonlocal rate_list_data, adult, child, rooms, inp_resnr, inp_reslinnr, rmtype, argtno, markno, wahrno, tokcounter, iftask, mestoken, mesvalue, currency, mapcode, datum, ankunft, dynacode, rmtype_str, rm_rate, rate_found, restricted, kback_flag, global_occ, dd, mm, yyyy, ci_date, co_date, map_code, use_it, w_day, occ_rooms, wd_array, occ_room_array, not_found_flag, do_it, rate_created, allotment_ok, calc_rm, doit, i_counter, ratecode, queasy, htparam, guest_pr, zimkateg, waehrung, segment, arrangement, res_line
        nonlocal frdate, todate, adult_child_str, ind_gastno
        nonlocal buff_rlist, buff_dynarate, bratecode, bqueasy, qsy18, pratecode, ratebuff, dynabuff, pqueasy
        nonlocal created_list, rate_list, dynarate_list, selected_ratelist, room_avail_list, buff_rlist, buff_dynarate, bratecode, bqueasy, qsy18, pratecode, ratebuff, dynabuff, pqueasy
        nonlocal created_list_data, rate_list_data, dynarate_list_data, selected_ratelist_data
        ...
        if zimkateg.typ == 0:
            ratecode = get_cache(Ratecode, {
                "code": [(eq, statcode)],
                "zikatnr": [(eq, rate_list.room_type)],
                "argtnr": [(eq, rate_list.argtno)],
                "startperiode": [(le, curr_date)],
                "endperiode": [(ge, curr_date)],
                "num1[0]": [(gt, 0)]})
            ...
            for ratecode.code, ratecode.zikatnr, ratecode.argtnr, ratecode.erwachs, ratecode.kind1, ratecode.marknr, ratecode.char1, ratecode._recid, zbuff.typ, zbuff.zikatnr, zbuff.kurzbez, zbuff._recid in db_session.query(Ratecode.code, Ratecode.zikatnr, Ratecode.argtnr, Ratecode.erwachs, Ratecode.kind1, Ratecode.marknr, Ratecode.char1, Ratecode._recid, Zbuff.typ, Zbuff.zikatnr, Zbuff.kurzbez, Zbuff._recid).join(Zbuff, (Zbuff.zikatnr == Ratecode.zikatnr) & (Zbuff.typ == zimkateg.typ)).filter(
                    (Ratecode.code == (statcode).lower()) & (Ratecode.zikatnr == rate_list.room_type) & (Ratecode.argtnr == rate_list.argtno) & (Ratecode.startperiode <= curr_date) & (Ratecode.endperiode >= curr_date) & (Ratecode.num1[inc_value(0)] > 0)).order_by(Ratecode._recid).all():
        ...
        for res_line in db_session.query(Res_line).filter(
            ...
            if doit_flag:
                for curr_i in range(1, num_entries(res_line.zimmer_wunsch, ";")):
    ...
    for room_avail_list in query(room_avail_list_data):
        # MESSAGE room-avail-list.zikatnr VIEW-AS ALERT-BOX INFO.
        print(room_avail_list.zikatnr)

        pratecode_obj_list = {}
        pratecode = Ratecode()
        guest_pr = Guest_pr()
        bqueasy = Queasy()
        for pratecode.code, pratecode.zikatnr, pratecode.argtnr, pratecode.erwachs, pratecode.kind1, pratecode.marknr, pratecode.char1, pratecode._recid, guest_pr.code, guest_pr._recid, bqueasy.char3, bqueasy._recid, bqueasy.char1, bqueasy.number1, bqueasy.number2, bqueasy.deci2, bqueasy.number3, bqueasy.deci3, bqueasy.date1, bqueasy.date2, bqueasy.logi1 in db_session.query(Pratecode.code, Pratecode.zikatnr, Pratecode.argtnr, Pratecode.erwachs, Pratecode.kind1, Pratecode.marknr, Pratecode.char1, Pratecode._recid, Guest_pr.code, Guest_pr._recid, Bqueasy.char3, Bqueasy._recid, Bqueasy.char1, Bqueasy.number1, Bqueasy.number2, Bqueasy.deci2, Bqueasy.number3, Bqueasy.deci3, Bqueasy.date1, Bqueasy.date2, Bqueasy.logi1).join(Guest_pr, (Guest_pr.gastnr == ind_gastno) & (Guest_pr.code == Pratecode.code)).join(Bqueasy, (Bqueasy.key == 2) & (Bqueasy.char1 == Guest_pr.code)).filter(
                (Pratecode.zikatnr == room_avail_list.zikatnr) & (Pratecode.erwachs == adult) & (Pratecode.kind1 == child) & (not_(Pratecode.startperiode > todate)) & (not_(Pratecode.endperiode < frdate))).order_by(Bqueasy.logi2, Bqueasy.number3.desc(), Bqueasy.deci3.desc()).all():
            if pratecode_obj_list.get(pratecode._recid):
                continue
            else:
                pratecode_obj_list[pratecode._recid] = True
            ...
            if bqueasy.number3 > ankunft - timedelta(days=ci_date):
                ...
                pqueasy = get_cache(Queasy, {
                    "key": [(eq, 264)],
                    "char1": [(eq, bqueasy.char1)]})
                ...
                if doit:
                    for bratecode in db_session.query(Bratecode).filter(
                            (Bratecode.code == trim(guest_pr.code)) & (Bratecode.zikatnr == room_avail_list.zikatnr) & (Bratecode.erwachs == adult) & (Bratecode.kind1 == child) & (not_(Bratecode.startperiode > todate)) & (not_(Bratecode.endperiode < frdate))).order_by(Bratecode.wday.desc(), Bratecode.erwachs, Bratecode.startperiode).all():
                        do_it = True
                        ...
                        if do_it:
                            ...
                            if not rate_list:
                                ...
                                if bqueasy.date1 and (ci_date < bqueasy.date1):
                                    rate_list.expired = True

                                if bqueasy.date2 and (ci_date > bqueasy.date2):
                                    rate_list.expired = True
                            curr_i = 0
                                ...
                                for curr_date in date_range(frdate, todate):
                                    ...
                                    if allotment_ok:
                                        rate_found, rm_rate, restricted, kback_flag = get_output(ratecode_rate(False, False, 0, 1, (
                                            "!" + bqueasy.char1), ci_date, curr_date, curr_date, curr_date, rate_list.marknr, rate_list.argtno, room_avail_list.zikatnr, rate_list.adult, rate_list.child, 0, 0, rate_list.currency))
                ...
                dynarate_list = query(dynarate_list_data, filters=(
                    lambda dynarate_list: dynarate_list.rmtype.lower() == "*"), first=True)
                # global_occ = None != dynarate_list and htparam.finteger == 1
                if dynarate_list and htparam.finteger == 1:
                    global_occ = True
                ...
                for dynarate_list in query(dynarate_list_data):
                    ...
                    for bratecode.code, bratecode.zikatnr, bratecode.argtnr, bratecode.erwachs, bratecode.kind1, bratecode.marknr, bratecode.char1, bratecode._recid, arrangement.argtnr, arrangement._recid in db_session.query(Bratecode.code, Bratecode.zikatnr, Bratecode.argtnr, Bratecode.erwachs, Bratecode.kind1, Bratecode.marknr, Bratecode.char1, Bratecode._recid, Arrangement.argtnr, Arrangement._recid).join(Arrangement, (Arrangement.argtnr == Bratecode.argtnr)).filter(
                            (Bratecode.code == dynarate_list.rcode) & (Bratecode.zikatnr == room_avail_list.zikatnr) & (Bratecode.erwachs == adult) & (Bratecode.kind1 == child) & (not_(Bratecode.startperiode > todate)) & (not_(Bratecode.endperiode < frdate))).order_by(Bratecode.wday, Arrangement.arrangement, Bratecode.erwachs, Bratecode.startperiode).all():
                        ...
                        if buff_rlist:
                            ...
                            if do_it:
                                rate_list = query(rate_list_data, filters=(lambda rate_list: rate_list.rateCode == guest_pr.code and rate_list.room_type == room_avail_list.zikatnr and rate_list.argtno == bratecode.argtnr and rate_list.adult == bratecode.erwachs and rate_list.child == bratecode.kind1), first=True)
                                ...
                                if not rate_list:
                                    ...
                                    rate_list.room_type = room_avail_list.zikatnr
                                    rate_list.argtno = bratecode.argtnr
                                    rate_list.adult = bratecode.erwachs
                                    rate_list.child = bratecode.kind1
                                    rate_list.marknr = bratecode.marknr
                                    ...
                                    if bqueasy.date1 and bqueasy.date2 and ((ci_date < bqueasy.date1) or (ci_date > bqueasy.date2)):
                                        rate_list.expired = True
                        ...
                        if dynarate_list:
                            ...
                            if not global_occ:
                                queasy = get_cache(Queasy, {
                                    "key": [(eq, 145)],
                                    "char1": [(eq, guest_pr.code)],
                                    "char2": [(eq, mapcode)],
                                    "number1": [(eq, room_avail_list.zikatnr)],
                                    "deci1": [(eq, dynarate_list.w_day)],
                                    "deci2": [(eq, dynarate_list.counter)],
                                    "date1": [(eq, curr_date)]})
                            else:
                                queasy = get_cache(Queasy, {
                                    "key": [(eq, 145)],
                            ...
                            for rate_list in query(rate_list_data, filters=(lambda rate_list: rate_list.dynaflag and rate_list.rateCode == guest_pr.code)):
                                ...
                                if rate_found:
                                    rate_found, rm_rate, restricted, kback_flag = get_output(ratecode_rate(False, False, 0, 1, (
                                        "!" + mapcode), ci_date, curr_date, curr_date, curr_date, rate_list.marknr, rate_list.argtno, room_avail_list.zikatnr, rate_list.adult, rate_list.child, 0, 0, rate_list.currency))
