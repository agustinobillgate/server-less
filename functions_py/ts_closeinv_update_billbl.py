...
def ts_closeinv_update_billbl(amount:Decimal, amount_foreign:Decimal, balance:Decimal, rec_bill_guest:int, foreign_rate:bool, 
                              ...
                              double_currency:bool, exchg_rate:Decimal, price_decimal:int, qty:int, kreditlimit:Decimal, billart:int, 
                              description:string, change_str:string, nett_amount:Decimal, tischnr:int, price:Decimal, bill_date:date, 
                              b_list_departement:int, avail_b_list:bool, cc_comment:string, b_list_waehrungsnr:int, hoga_card:string, 
                              cancel_str:string, req_str:string, curr_waiter:int, pay_type:int, transfer_zinr:string, curr_room:string, 
                              user_init:string, deptname:string):

    prepare_cache ([H_artikel, Htparam, H_bill_line, H_umsatz, H_journal, Umsatz, Guest, Artikel, Waehrung, Bediener, Billjournal])
    ...
    def generate_output():
        nonlocal service_foreign, mwst_foreign, service, mwst, bcol, balance_foreign, closed, t_h_bill_data, h_service, h_service_foreign, h_mwst, h_mwst_foreign, sysdate, zeit, h_bill, h_artikel, htparam, kellne1, h_bill_line, h_umsatz, h_journal, umsatz, guest, artikel, debitor, waehrung, bediener, billjournal
