...
def mk_gcpi_go1abl(pbuff_data, pi_type:string, pay_type:string, pay_acctno:string, rcvname:string, bemerk:string, pi_number:string):
    ...
    gc_pi = db_session.query(Gc_pi).filter(
             (Gc_pi.docu_nr == pi_number)).with_for_update().first()
