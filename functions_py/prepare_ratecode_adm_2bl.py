...
# # =============================================================
# Rulita, 05-12-2025
# Fixing issue contract rate looping after update with_for_update
# ===============================================================
...
def prepare_ratecode_adm_2bl(pvilanguage:int):
    ...
    tb1_data, Tb1 = create_model_like(Queasy, {"waehrungsnr":int, "wabkurz":string, "active_flag":bool, "rcode_element":string})

    Bqueasy = create_buffer("Bqueasy",Queasy)

    set_cache(Queasy, ((Queasy.key == 2) & (Queasy.number1 == 0)) | Queasy.key.in_([18,264, 289]),[["key"], ["key", "char1"], ["key", "number1"]], True,[],[])
    ...
    set_cache(Ratecode, Ratecode.endperiode >= cidate,[["code"]], False,["endperiode"],[])
    ...
    for queasy, waehrung in db_session.query(Queasy, Waehrung).join(Waehrung,(Waehrung.waehrungsnr == Queasy.number1)).filter(
        ...
        bqueasy = get_cache (Queasy, {"key": [(eq, 289)],"char1": [(eq, queasy.char1)]})

        if bqueasy:
            tb1.rcode_element = bqueasy.char2
