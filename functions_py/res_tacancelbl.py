...
def res_tacancelbl(pvilanguage:int, stattype:int, sorttype:int, fdate:date, fname:string, tname:string):
    ...
    def generate_output():
        nonlocal t_mnite, t_ynite, t_mtu, t_ytu, res_tacancel_data, lvcarea, tmpint, guest, zimkateg, res_line
        nonlocal pvilanguage, stattype, sorttype, fdate, fname, tname


        nonlocal res_tacancel
        nonlocal res_tacancel_data

        return {"t_mnite": t_mnite, "t_ynite": t_ynite, "t_mtu": t_mtu, "t_ytu": t_ytu, "res-tacancel": res_tacancel_data}
    ...
    def disp_noshow():
        ...
        if sorttype == 1:
            ...
            for res_line, guest, zimkateg in db_session.query(Res_line, Guest, Zimkateg).join(Guest,(Guest.gastnr == Res_line.gastnr) & (Guest.karteityp == 2)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                ...
                res_tacancel = query(res_tacancel_data, filters=(lambda res_tacancel: res_tacancel.resnr == res_line.resnr), first=True)

                if not res_tacancel:
                    res_tacancel = Res_tacancel()
                    res_tacancel_data.append(res_tacancel)
