...
def nt_fcjnal():
    ...
    def journal_list():
        ...
        add_line(line)
        add_line(" ")
        line = "Transaction Cancel Journal"
        add_line(line)
        line = ""
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            billjournal = get_cache (Billjournal, {"departement": [(eq, hoteldpt.num)],"bill_datum": [(eq, curr_date)],"stornogrund": [(ne, "")]})
            ...
            if billjournal:
                ...
                # Rulita,
                # - Fix space in string
                line = "RmNo      BillNo ArtNo  Qty Description                    Balance Cancel Reason             Time ID"
                add_line(line)
                line = ""
                ...
                for billjournal in db_session.query(Billjournal).filter(
                    ...
                    # - Fix missing table "billjournal"
                    if not long_digit:
                        line = to_string(billjournal.zinr) + " " + to_string(billjournal.rechnr, ">,>>>,>>9") + " " + to_string(billjournal.artnr, ">>>>9") + " " + to_string(billjournal.anzahl, "->>9") + " " + to_string(billjournal.bezeich, "x(23)") + " " + to_string(billjournal.betrag, "->>,>>>,>>9.99") + " " + to_string(billjournal.stornogrund, "x(24)") + " " + to_string(billjournal.zeit, "HH:MM") + " " + to_string(billjournal.userinit, "x(2)")
                    else:
                        line = to_string(billjournal.zinr) + " " + to_string(billjournal.rechnr, ">,>>>,>>9") + " " + to_string(billjournal.artnr, ">>>>9") + " " + to_string(billjournal.anzahl, "->>9") + " " + to_string(billjournal.bezeich, "x(23)") + " " + to_string(billjournal.betrag, "->,>>>,>>>,>>9") + " " + to_string(billjournal.stornogrund, "x(24)") + " " + to_string(billjournal.zeit, "HH:MM") + " " + to_string(billjournal.userinit, "x(2)")
                    add_line(line)
