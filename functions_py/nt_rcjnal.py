...
def nt_rcjnal():
    ...
    def journal_list():
        ...
        for hoteldpt in db_session.query(Hoteldpt).filter(
            ...
            h_journal = get_cache (H_journal, {"departement": [(eq, hoteldpt.num)],"bill_datum": [(eq, curr_date)],"stornogrund": [(ne, "")]})
            ...
            if h_journal:
                ...
                for h_journal in db_session.query(H_journal).filter(
                    ...
                    # - zeit
                    if not long_digit:
                        line = to_string(h_journal.zinr, "x(6)") + " " + to_string(h_journal.tischnr, ">>>9 ") + " " + to_string(h_journal.rechnr, ">,>>>,>>9") + " " + to_string(h_journal.artnr, ">>>>9") + " " + to_string(h_journal.anzahl, "->>9") + " " + to_string(h_journal.bezeich, "x(23)") + " " + to_string(h_journal.betrag, "->>,>>>,>>9.99") + " " + to_string(h_journal.stornogrund, "x(24)") + " " + to_string(h_journal.zeit, "HH:MM") + " " + to_string(h_journal.kellner_nr, ">>9")
                    else:
                        line = to_string(h_journal.zinr, "x(6)") + " " + to_string(h_journal.tischnr, ">>>9 ") + " " + to_string(h_journal.rechnr, ">,>>>,>>9") + " " + to_string(h_journal.artnr, ">>>>9") + " " + to_string(h_journal.anzahl, "->>9") + " " + to_string(h_journal.bezeich, "x(23)") + " " + to_string(h_journal.betrag, "->,>>>,>>>,>>9") + " " + to_string(h_journal.stornogrund, "x(24)") + " " + to_string(h_journal.zeit, "HH:MM") + " " + to_string(h_journal.kellner_nr, ">>9")
                    add_line(line)
