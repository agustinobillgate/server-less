...
# yusufwijasena, 17/11/2025 (D53C3D)
# - last update from Dzikri F70E68
...
segm_list_data, Segm_list = create_model(
    "Segm_list",
    {
        "selected": bool,
        "segm": int,
...
def occ_fcast1_ddown_create_browsebl(case_type: int, datum: date, curr_date: date, to_date: date, all_segm: bool, all_argt: bool, all_zikat: bool, argt_list_data: list[Argt_list], segm_list_data: list[Segm_list], zikat_list_data: list[Zikat_list]):
    ...
    def create_browse():
        ...
        rmsharer: bool = False
        create_it: bool = False  # Dzikri F70E68
        rsvstat: string = ""
        ...
        for zimmer in db_session.query(Zimmer).filter(
            ...
            if all_zikat:
                ...
                zikat_list = query(zikat_list_data, filters=(
                    lambda zikat_list: zikat_list.zikatnr == zimmer.zikatnr and zikat_list.selected), first=True)
        ...
        if case_type == 1:
            ...
            segment = Segment()
            # Dzikri F70E68
            for genstat.datum, genstat.resstatus, genstat.segmentcode, genstat.argt, genstat.zikatnr, genstat.resnr, genstat.zinr, genstat.res_date, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.kind3, genstat.res_int, genstat.logis, genstat._recid, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest.karteityp, guest._recid, zimkateg.kurzbez, zimkateg._recid, waehrung.wabkurz, waehrung._recid, res_line.anztage, res_line.resnr, res_line.zinr, res_line.abreise, res_line._recid, segment.bezeich, segment._recid in db_session.query(Genstat.datum, Genstat.resstatus, Genstat.segmentcode, Genstat.argt, Genstat.zikatnr, Genstat.resnr, Genstat.zinr, Genstat.res_date, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.kind3, Genstat.res_int, Genstat.logis, Genstat._recid, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest.karteityp, Guest._recid, Zimkateg.kurzbez, Zimkateg._recid, Waehrung.wabkurz, Waehrung._recid, Res_line.anztage, Res_line.resnr, Res_line.zinr, Res_line.abreise, Res_line._recid, Segment.bezeich, Segment._recid).join(Guest, (Guest.gastnr == Genstat.gastnr)).join(Zimkateg, (Zimkateg.zikatnr == Genstat.zikatnr)).join(Waehrung, (Waehrung.waehrungsnr == Genstat.wahrungsnr)).join(Res_line, (Res_line.resnr == Genstat.resnr) & (Res_line.reslinnr == Genstat.res_int[inc_value(0)])).join(Segment, (Segment.segmentcode == Genstat.segmentcode)).filter(
                ...
                if genstat.datum == (datum - timedelta(days=1)):
                    ...
                    if do_it and not all_segm:
                        s_list = query(s_list_data, filters=(
                            lambda s_list: s_list.segm == genstat.segmentcode and s_list.selected), first=True)
                        do_it = None != s_list

                    if do_it and not all_argt:
                        a_list = query(a_list_data, filters=(
                            lambda a_list: a_list.argt == genstat.argt and a_list.selected), first=True)
                        do_it = None != a_list

                    if do_it and not all_zikat:
                        z_list = query(z_list_data, filters=(
                            lambda z_list: z_list.zikatnr == genstat.zikatnr and z_list.selected), first=True)
                        do_it = None != z_list
                    ...
                    if do_it:
                        ...
                        room_list.pocc = (to_decimal(
                            room_list.qty) / to_decimal(tot_room)) * to_decimal("100")
                        ...
                        tot_pocc = to_decimal(
                            tot_pocc) + to_decimal((room_list.qty) / to_decimal(tot_room)) * to_decimal("100")
                    ...
                    if do_it:
                        ...
                        if guest.karteityp != 0:
                            room_list.company = guest.name + ", " + guest.vorname1 + \
                                " " + guest.anrede1 + guest.anredefirma
                        tot_pax = tot_pax + \
                            (genstat.erwachs + genstat.kind1 + genstat.kind2 + genstat.gratis)
                        tot_adult = tot_adult + genstat.erwachs
                        tot_ch1 = tot_ch1 + genstat.kind1
                        tot_compli = tot_compli + genstat.gratis
            ...
            for genstat.datum, genstat.resstatus, genstat.segmentcode, genstat.argt, genstat.zikatnr, genstat.resnr, genstat.zinr, genstat.res_date, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.kind3, genstat.res_int, genstat.logis, genstat._recid, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest.karteityp, guest._recid, zimkateg.kurzbez, zimkateg._recid, waehrung.wabkurz, waehrung._recid, segment.bezeich, segment._recid in db_session.query(Genstat.datum, Genstat.resstatus, Genstat.segmentcode, Genstat.argt, Genstat.zikatnr, Genstat.resnr, Genstat.zinr, Genstat.res_date, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.kind3, Genstat.res_int, Genstat.logis, Genstat._recid, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest.karteityp, Guest._recid, Zimkateg.kurzbez, Zimkateg._recid, Waehrung.wabkurz, Waehrung._recid, Segment.bezeich, Segment._recid).join(Guest, (Guest.gastnr == Genstat.gastnr)).join(Zimkateg, (Zimkateg.zikatnr == Genstat.zikatnr)).join(Waehrung, (Waehrung.waehrungsnr == Genstat.wahrungsnr)).join(Segment, (Segment.segmentcode == Genstat.segmentcode)).filter(
                ...
                if genstat.res_date[1] == datum:
                    history = get_cache(History, {"zinr": [(eq, genstat.zinr)], "resnr": [(
                        eq, genstat.resnr)], "reslinnr": [(eq, genstat.res_int[0])], "zi_wechsel": [(eq, True)]})
                    ...
                    if do_it:
                        ...
                        res_line = db_session.query(Res_line).filter(
                            (Res_line.resnr == genstat.resnr) & (Res_line.reslinnr == genstat.res_int[0]) & ((Res_line.resstatus == 6) | (Res_line.resstatus == 13))).first()

                        if res_line and res_line.abreise != genstat.res_date[1]:
                            room_list = query(room_list_data, filters=(lambda room_list: room_list.resnr == to_string(
                                res_line.resnr) and room_list.rmno == res_line.zinr), first=True)
                            ...
                            room_list = query(room_list_data, filters=(lambda room_list: room_list.resnr == to_string(
                                genstat.resnr) and room_list.rmno == genstat.zinr), first=True)
                        ...
                        if not room_list and create_it:
                            room_list = Room_list()
                            room_list_data.append(room_list)
            ...
            for genstat.datum, genstat.resstatus, genstat.segmentcode, genstat.argt, genstat.zikatnr, genstat.resnr, genstat.zinr, genstat.res_date, genstat.erwachs, genstat.kind1, genstat.kind2, genstat.gratis, genstat.kind3, genstat.res_int, genstat.logis, genstat._recid, guest.name, guest.vorname1, guest.anrede1, guest.anredefirma, guest.karteityp, guest._recid, zimkateg.kurzbez, zimkateg._recid, waehrung.wabkurz, waehrung._recid, segment.bezeich, segment._recid in db_session.query(Genstat.datum, Genstat.resstatus, Genstat.segmentcode, Genstat.argt, Genstat.zikatnr, Genstat.resnr, Genstat.zinr, Genstat.res_date, Genstat.erwachs, Genstat.kind1, Genstat.kind2, Genstat.gratis, Genstat.kind3, Genstat.res_int, Genstat.logis, Genstat._recid, Guest.name, Guest.vorname1, Guest.anrede1, Guest.anredefirma, Guest.karteityp, Guest._recid, Zimkateg.kurzbez, Zimkateg._recid, Waehrung.wabkurz, Waehrung._recid, Segment.bezeich, Segment._recid).join(Guest, (Guest.gastnr == Genstat.gastnr)).join(Zimkateg, (Zimkateg.zikatnr == Genstat.zikatnr)).join(Waehrung, (Waehrung.waehrungsnr == Genstat.wahrungsnr)).join(Segment, (Segment.segmentcode == Genstat.segmentcode)).filter(
                ...
                if genstat.res_date[0] < genstat.datum and genstat.res_date[1] == genstat.datum and genstat.resstatus == 8:
                    ...
                    if do_it:
                        ...
                        tot_qty = tot_qty + 1
                        tot_lodg = to_decimal(
                            tot_lodg) + to_decimal(genstat.logis)

        elif case_type == 15:
            zinrstat = get_cache(
                Zinrstat, {"zinr": [(eq, "ooo")], "datum": [(eq, datum)]})
