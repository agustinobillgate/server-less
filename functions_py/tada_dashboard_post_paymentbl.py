...
def tada_dashboard_post_paymentbl(user_init:string, deptno:int, orderid:int, payment_artnr:int, payment_amount:Decimal):
    ...
    def generate_output():
        ...
        nonlocal t_h_bill_data, t_submenu_list_data, t_kellner1_data
    ...
    bill_date, cancel_flag, fl_code, mwst, mwst_foreign, rechnr, balance, bcol, balance_foreign, fl_code1, fl_code2, fl_code3, p_88, closed, t_h_bill_data, t_kellner1_data = get_output(tada_post_paymentbl(language_code, rec_id, h_artikel._recid, "", None, h_artikel.artart, False, h_artikel.service_code, - to_decimal(payment_amount), - to_decimal(payment_amount), 0, False, 1, 1, 0, user_init, tischnr, curr_dept, user_init, "", pax, 0, 0, h_artikel.artnr, h_artikel.bezeich, "", "", "", "", "", "", True, False, h_artikel.artnrfront, 0, 0, "", False, False, "", user_init, resnr, reslinnr, t_submenu_list_data))

    if closed:

        # tablecolor = get_cache (Queasy, {"key": [(eq, 31)],"number1": [(eq, curr_dept)],"betriebsnr": [(eq, 0)],"number2": [(eq, tischnr)]})
        tablecolor = db_session.query(Queasy).filter(
            (Queasy.key == 31) & (Queasy.number1 == curr_dept) & (Queasy.betriebsnr == 0) & (Queasy.number2 == tischnr)).with_for_update().first()
