...
g_list_data, G_list = create_model_like(Gl_main)

def glmain_admin_btn_exitbl(pvilanguage:int, case_type:int, g_list_data:[G_list]):
    code_0 = False
    msg_str = ""
    success_flag = False
    ...
    if case_type == 1:
        ...
        gl_main = db_session.query(Gl_main).filter(Gl_main.nr == g_list.nr).with_for_update().first()

        if gl_main:
            msg_str = translateExtended ("Number already exists.", lvcarea, "")
            pass
