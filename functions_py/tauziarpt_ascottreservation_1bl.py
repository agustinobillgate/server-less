...
from models import Htparam, Guest, Reslin_queasy, Queasy, H_artikel, Res_line, Zimkateg, Reservation, Sourccod, Segment, Genstat, Zimmer, Nation, Arrangement, Artikel, Mc_guest, Waehrung, Bediener, Bill, Bill_line, H_bill, H_bill_line

def tauziarpt_ascottreservation_1bl(datum:date, propid:str):

    prepare_cache ([Htparam, Guest, Queasy, H_artikel, Zimkateg, Reservation, Sourccod, Segment, Genstat, Zimmer, Nation, Arrangement, Artikel, Mc_guest, Waehrung, Bediener, Bill, Bill_line, H_bill, H_bill_line])
    ...
    curr_time:int = 0
    counter:int = 0
    htparam = guest = reslin_queasy = queasy = h_artikel = res_line = zimkateg = reservation = sourccod = segment = genstat = zimmer = nation = arrangement = artikel = mc_guest = waehrung = bediener = bill = bill_line = h_bill = h_bill_line = None
    ...
    def generate_tabledata(casetyp:str):
        ...
        if ((datum1 >= datum - timedelta(days=90) and datum1 <= datum + timedelta(days=730)) or casetyp.lower()  == "checkedout") and res_line.resstatus != 99 and res_line.resstatus != 12 and res_line.l_zuordnung[2] == 0 and do_it:
                    ...
                    for bill_line in db_session.query(Bill_line).filter(
                        ...
                        if artikel:
                            if artikel.umsatzart == 4:
                                ...
                                data_list.billotherrev =  to_decimal(data_list.billotherrev + betragvalue)
                                data_list.grossbillother =  to_decimal(data_list.grossbillother + bill_line.betrag)
                                    ...
                                    if billflag :
                                        ...
                                        for h_bill_line in db_session.query(H_bill_line).filter(
                                            ...
                                            if h_bill_line.epreis > 0:
                                                ...
                                                betragvalue =  to_decimal(betragvalue + newbetrag)
                                                grossbetrag =  to_decimal(grossbetrag + h_bill_line.betrag)
                                
                                data_list.billfbrev =  to_decimal(data_list.billfbrev + betragvalue)
                                data_list.grossbillfb =  to_decimal(data_list.grossbillfb + grossbetrag)
    ...
    while None != res_line:
        ...
        res_line = db_session.query(Res_line).filter(
                ((Res_line.ankunft <= datum) & (Res_line.abreise >= datum)) | (((Res_line.resstatus == 1) | (Res_line.resstatus == 2) | (Res_line.resstatus == 5) | (Res_line.resstatus == 3) | (Res_line.resstatus == 4) | (Res_line.resstatus == 11)) & (Res_line.active_flag == 0)) | ((Res_line.resstatus == 9) & ((Res_line.ankunft <= datum) & (Res_line.abreise >= datum))) & (Res_line.l_zuordnung[inc_value(2)] == 0) & (Res_line.resstatus != 12) & (Res_line.ankunft < todate) & (Res_line._recid > curr_recid)).first()

    res_line_obj_list = {}
    for res_line, gmember, zimkateg, reservation, sourccod, segment in db_session.query(Res_line, Gmember, Zimkateg, Reservation, Sourccod, Segment).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Reservation,(Reservation.resnr == Res_line.resnr)).join(Sourccod,(Sourccod.source_code == Reservation.resart)).join(Segment,(Segment.segmentcode == Reservation.segmentcode)).filter(
    ...
    for reservation in db_session.query(Reservation).filter(
        ...
        res_line_obj_list = {}
        for res_line, gmember, zimkateg, sourccod, segment in db_session.query(Res_line, Gmember, Zimkateg, Sourccod, Segment).join(Gmember,(Gmember.gastnr == Res_line.gastnrmember)).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).join(Sourccod,(Sourccod.source_code == reservation.resart)).join(Segment,(Segment.segmentcode == reservation.segmentcode)).filter(
    ...
    for data_list in query(data_list_data, filters=(lambda data_list: data_list.entry(0, data_list.arrdate, " ") == entry(0, data_list.depdate, " ") and data_list.rmno.lower()  != ("NULL").lower())):
        ...
        if t_datalist:
            ...
            genstat = get_cache (Genstat, {"datum": [(eq, date_mdy(to_int(entry(1, entry(0, t_datalist.arrdate, " ") , "-")) , to_int(entry(2, entry(0, t_datalist.arrdate, " ") , "-")) , to_int(entry(0, entry(0, t_datalist.arrdate, " ") , "-"))))],"zinr": [(eq, t_datalist.rmno)],"gastnrmember": [(ne, t_datalist.guestid)]})
    ...
    for data_list in query(data_list_data, filters=(lambda data_list: data_list.entry(0, data_list.arrdate, " ") == entry(0, data_list.depdate, " ") and data_list.rmno.lower()  != ("NULL").lower())):
        ...
        genstat = get_cache (Genstat, {"datum": [(eq, date_mdy(to_int(entry(1, entry(0, data_list.arrdate, " ") , "-")) , to_int(entry(2, entry(0, data_list.arrdate, " ") , "-")) , to_int(entry(0, entry(0, data_list.arrdate, " ") , "-"))))],"zinr": [(eq, data_list.rmno)]})
    ...
    for zimmer in db_session.query(Zimmer).filter(
        ...
        data_list = query(data_list_data, filters=(lambda data_list: data_list.rmno == zimmer.zinr and data_list.rsvstatus.lower()  == ("InHouse").lower()  and entry(0, data_list.actualdate, " ") == to_string(get_year(datum) , "9999") + "-" + to_string(get_month(datum) , "99") + "-" + to_string(get_day(datum) , "99") and data_list.rmrev == 0), first=True)
        while None != data_list:
            data_list.occflag = "0"

            data_list = query(data_list_data, filters=(lambda data_list: data_list.rmno == zimmer.zinr and data_list.rsvstatus.lower()  == ("InHouse").lower()  and entry(0, data_list.actualdate, " ") == to_string(get_year(datum) , "9999") + "-" + to_string(get_month(datum) , "99") + "-" + to_string(get_day(datum) , "99") and data_list.rmrev == 0), next=True)
