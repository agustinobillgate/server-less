...
def fo_usrjournal_cldbl(mi_incl:bool, mi_excl:bool, mi_tran:bool, from_date:date, to_date:date, from_dept:int, to_dept:int, from_art:int, to_art:int, usr_init:string, long_digit:bool, foreign_flag:bool):
        ...
        for curr_date in date_range(from_date,to_date) :
            ...
            for billjournal in db_session.query(Billjournal).filter(
                ...
                if last_artnr != billjournal.artnr or last_dept != billjournal.departement:
                    ...
                    if not long_digit:
                        output_list.str = to_string("", "x(87)") + to_string("T O T A L ", "x(22)") + to_string(a_qty, "-9999") + to_string(a_tot, "->,>>>,>>>,>>9.99")
                    else:
                        output_list.str = to_string("", "x(87)") + to_string("T O T A L ", "x(22)") + to_string(a_qty, "-9999") + to_string(a_tot, "->,>>>,>>>,>>9.99")
                    a_qty = 0
                    a_tot =  to_decimal("0")
    ...
    def journal_list1():
        ...
        nonlocal output_list_data, def_rate, x_rate, output_list_data, def_rate, x_rate, waehrung, guest, billjournal, artikel, hoteldpt, bill, h_bill, bk_veran, reservation
        nonlocal mi_incl, mi_excl, mi_tran, from_date, to_date, from_dept, to_dept, from_art, to_art, usr_init, long_digit, foreign_flag, mi_incl, mi_excl, mi_tran, from_date, to_date, from_dept, to_dept, from_art, to_art, usr_init, long_digit, foreign_flag
        ...
        for curr_date in date_range(from_date,to_date) :
            ...
            for billjournal in db_session.query(Billjournal).filter(
                ...
                if not long_digit:
                    output_list.str = to_string(billjournal.bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "9,999,999") + to_string(billjournal.artnr, "9999") + to_string(billjournal.bezeich, "x(60)") + to_string(hoteldpt.depart, "x(22)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, "->,>>>,>>>,>>9.99") + to_string(billjournal.zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate) + to_string(billjournal._recid)
                else:
                    output_list.str = to_string(billjournal.bill_datum) + to_string(billjournal.zinr, "x(6)") + to_string(billjournal.rechnr, "9,999,999") + to_string(billjournal.artnr, "9999") + to_string(billjournal.bezeich, "x(60)") + to_string(hoteldpt.depart, "x(22)") + to_string(billjournal.anzahl, "-9999") + to_string(amount, " ->>>,>>>,>>>,>>9") + to_string(billjournal.zeit, "HH:MM:SS") + to_string(billjournal.userinit, "x(4)") + to_string(billjournal.sysdate) + to_string(billjournal._recid)
