...
# update penambahan field dalam for loop,
# update string replace \n di bemerk
# update arrival, depart, birtdate, localreg, created field to string

# yusufwijasena, 10/11/2025
# - update from Micheal Yohannes: 3B6A88
# - update from FT: 199267
# ------------------------------------------
from functions.additional_functions import *
from decimal import Decimal
from datetime import date
from models import Queasy, Htparam, Waehrung, Artikel, Nation, Zkstat, Zinrstat, Paramtext, Guest, Reservation, Zimmer, Arrangement, Bill, Res_line, Guest_pr, Guestseg, Segment, Sourccod, Bill_line, Argt_line, Mc_guest, Mc_types, Reslin_queasy, Genstat

zikat_list_data, Zikat_list = create_model(
    "Zikat_list",
    {
        "selected": bool,
        "zikatnr": int,
...
def pj_inhouse4_btn_go_5_cldbl(sorttype: int, from_date: date, to_date: date, curr_date: date, curr_gastnr: int, froom: string, troom: string, exc_depart: bool, incl_gcomment: bool, incl_rsvcomment: bool, prog_name: string, disp_accompany: bool, exc_compli: bool, zikat_list_data: List[Zikat_list]):

    prepare_cache([Htparam, Waehrung, Artikel, Nation, Zkstat, Zinrstat, Paramtext, Guest, Reservation, Zimmer, Arrangement, Bill, Res_line, Guest_pr, Guestseg, Segment, Sourccod, Bill_line, Argt_line, Mc_guest, Mc_types, Reslin_queasy, Genstat])
    ...
    queasy = htparam = waehrung = artikel = nation = zkstat = zinrstat = paramtext = guest = reservation = zimmer = arrangement = bill = res_line = guest_pr = guestseg = segment = sourccod = bill_line = argt_line = mc_guest = mc_types = reslin_queasy = genstat = None
    ...
    str_list_data, Str_list = create_model(
        "Str_list",
        {
            "flag": int,
            "rflag": bool,
    ...
    zinr_list_data, Zinr_list = create_model(
        "Zinr_list",
        {
            "resnr": int,
            "reslinnr": int,
    ...
    def generate_output():
        nonlocal tot_payrm, tot_rm, tot_a, tot_c, tot_co, tot_avail, inactive, tot_keycard, cl_list_data, s_list_data, t_buff_queasy_data, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, tot_room, all_room, frate, exchg_rate, ct, contcode, bfast_art, lunch_art, dinner_art, lundin_art, fb_dept, argt_betrag, take_it, prcode, qty, r_qty, lodge_betrag, f_betrag, s, tot_qty, tot_rev, queasy, htparam, waehrung, artikel, nation, zkstat, zinrstat, paramtext, guest, reservation, zimmer, arrangement, bill, res_line, guest_pr, guestseg, segment, sourccod, bill_line, argt_line, mc_guest, mc_types, reslin_queasy, genstat
        nonlocal sorttype, from_date, to_date, curr_date, curr_gastnr, froom, troom, exc_depart, incl_gcomment, incl_rsvcomment, prog_name, disp_accompany, exc_compli
        ...
        return {
            ...
            "tot_avail": tot_avail,
            "inactive": inactive,
            "tot_keycard": tot_keycard,
            "cl-list": cl_list_data,
            "s-list": s_list_data,
            "t-buff-queasy": t_buff_queasy_data
        }
    ...
    def create_inhouse():
        ...
        for res_line, reservation, guest, gmember, arrangement, artikel, bill in db_session.query(Res_line, Reservation, Guest, Gmember, Arrangement, Artikel, Bill).join(Reservation, (Reservation.resnr == Res_line.resnr)).join(Guest, (Guest.gastnr == Reservation.gastnr)).join(Gmember, (Gmember.gastnr == Res_line.gastnrmember)).join(Arrangement, (Arrangement.arrangement == Res_line.arrangement)).join(Artikel, (Artikel.artnr == Arrangement.argt_artikelnr) & (Artikel.departement == 0)).join(Bill, (Bill.resnr == Res_line.resnr) & (Bill.reslinnr == Res_line.reslinnr) & (Bill.zinr == Res_line.zinr)).filter(  # FT: 199267
                (Res_line.active_flag >= actflag1) &
                (Res_line.active_flag <= actflag2) &
                (Res_line.resstatus != 9) &
                (Res_line.resstatus != 10) &
                (Res_line.resstatus != 12) &
                (Res_line.ankunft <= from_date) &
                (Res_line.abreise >= to_date) &
                (Res_line.zinr >= (froom).lower()) &
                (Res_line.zinr <= (troom).lower())).order_by(Res_line.zinr, Res_line.ankunft, Res_line.erwachs.desc(), Res_line.name).all():
            ...
            if exc_depart and res_line.abreise == to_date:
                ...
                if res_line.reserve_int != 0:
                    guest_pr = get_cache(
                        Guest_pr, {"gastnr": [(eq, guest.gastnr)]})
                ...
                setup_list = query(setup_list_data, filters=(
                    lambda setup_list: setup_list.nr == res_line.setup + 1), first=True)
                ...
                cl_list.firstname = right_trim(
                    gmember.vorname1, chr_unicode(10))
                cl_list.firstname = right_trim(
                    cl_list.firstname, chr_unicode(13))
                cl_list.firstname = replace_str(
                    cl_list.firstname, chr_unicode(10), " ")
                cl_list.lastname = right_trim(gmember.name, chr_unicode(10))
                cl_list.lastname = right_trim(
                    cl_list.lastname, chr_unicode(13))
                cl_list.lastname = replace_str(
                    cl_list.lastname, chr_unicode(10), " ")

                if gmember.geburtdatum1 == None:
                    cl_list.birthdate = ""
                else:
                    cl_list.birthdate = to_string(
                        gmember.geburtdatum1, "99/99/9999")
                ...
                cl_list.night = to_string(
                    res_line.abreise - res_line.ankunft, ">>>>9")
...
# end - FT: 199267
                ...
                bill_line = Bill_line()
                buff_art = Artikel()
                for bill_line.artnr, bill_line._recid, buff_art.umsatzart, buff_art.zwkum, buff_art._recid, buff_art.artart \
                    in db_session.query(Bill_line.artnr, Bill_line._recid, Buff_art.umsatzart, Buff_art.zwkum, Buff_art._recid, Buff_art.artart).join(Buff_art, (Buff_art.artnr == Bill_line.artnr) & (Buff_art.departement == Bill_line.departement)).filter(
                    ...
                    if (buff_art.artart == 2 or buff_art.artart == 5 or buff_art.artart == 6 or buff_art.artart == 7):
                        cl_list.pay_art = cl_list.pay_art + \
                            to_string(bill_line.artnr) + ","
                cl_list.pay_art = substring(
                    cl_list.pay_art, 0, length(cl_list.pay_art) - 1)

                nation1 = get_cache(
                    Nation, {"kurzbez": [(eq, gmember.nation2.strip())]})
                ...
                for argt_line in db_session.query(Argt_line).filter(
                    ...
                    if take_it:
                        if artikel1.zwkum == bfast_art and (artikel1.umsatzart == 3 or artikel1.umsatzart >= 5):
                            cl_list.breakfast = to_decimal(
                                cl_list.breakfast) + to_decimal(argt_betrag)
                            cl_list.lodging = to_decimal(
                                cl_list.lodging) - to_decimal(argt_betrag)

                        elif artikel1.zwkum == lunch_art and (artikel1.umsatzart == 3 or artikel1.umsatzart >= 5):
                            cl_list.lunch = to_decimal(
                                cl_list.lunch) + to_decimal(argt_betrag)
                            cl_list.lodging = to_decimal(
                                cl_list.lodging) - to_decimal(argt_betrag)

                        elif artikel1.zwkum == dinner_art and (artikel1.umsatzart == 3 or artikel1.umsatzart >= 5):
                            cl_list.dinner = to_decimal(
                                cl_list.dinner) + to_decimal(argt_betrag)
                            cl_list.lodging = to_decimal(
                                cl_list.lodging) - to_decimal(argt_betrag)
                        else:
                            cl_list.otherev = to_decimal(
                                cl_list.otherev) + to_decimal(argt_betrag)
                            cl_list.lodging = to_decimal(
                                cl_list.lodging) - to_decimal(argt_betrag)
                ...
                cl_list.c_breakfast = to_string(
                    cl_list.breakfast, "->>,>>>,>>>,>>9.99")
                cl_list.c_lunch = to_string(
                    cl_list.lunch, "->>,>>>,>>>,>>9.99")
                cl_list.c_dinner = to_string(
                    cl_list.dinner, "->>,>>>,>>>,>>9.99")
                cl_list.c_otherev = to_string(
                    cl_list.otherev, "->>,>>>,>>>,>>9.99")
                # start Micheal Yohannes: 3B6A88
                cl_list.c_a = to_string(cl_list.a, ">9")
                cl_list.c_c = to_string(cl_list.c, ">9")
                # end Micheal Yohannes: 3B6A88
                cl_list.c_co = to_string(cl_list.co, ">9")
                cl_list.c_rechnr = to_string(cl_list.rechnr, ">>>>>9")
                cl_list.c_resnr = to_string(cl_list.resnr, ">>>>>9")

                mc_guest = get_cache(
                    Mc_guest, {"gastnr": [(eq, gmember.gastnr)]})
                    ...
                    if gbuff:
                        for i in range(1, length(gbuff.bemerkung) + 1):
                            if substring(gbuff.bemerkung, i - 1, 1) == chr_unicode(10):
                                ...
                                cl_list.bemerk = cl_list.bemerk + \
                                    substring(trim(gbuff.bemerkung), i - 1, 1)
                    cl_list.bemerk = cl_list.bemerk + " || "

                if incl_rsvcomment:
                    rbuff = get_cache(
                        Reservation, {"resnr": [(eq, reservation.resnr)]})
                    ...
                    if rbuff:
                        for j in range(1, length(rbuff.bemerk) + 1):
                            ...
                            if substring(rbuff.bemerk, j - 1, 1) == chr_unicode(10):
                                ...
                                cl_list.bemerk1 = cl_list.bemerk1 + \
                                    substring(trim(rbuff.bemerk), j - 1, 1)
                    cl_list.bemerk = cl_list.bemerk + cl_list.bemerk1

                if not incl_gcomment and not incl_rsvcomment:
                    cl_list.bemerk = trim(replace_str(
                        res_line.bemerk, chr_unicode(10), " "))

                reslin_queasy = get_cache(
                    Reslin_queasy, {"key": [(eq, "arrangement")], "resnr": [(eq, res_line.resnr)], "reslinnr": [(eq, res_line.reslinnr)]})
                    ...
                    if reslin_queasy.char2 != "":
                        ...
                        for i in range(1, num_entries(res_line.zimmer_wunsch, ";")):
                            ...
                            if substring(str, 0, 6) == ("$CODE$").lower():
                                ...
                                break
                cl_list.pax = to_string(
                    cl_list.a, ">9") + "/" + to_string(cl_list.c, "9") + " " + to_string(cl_list.co, "9")

                if cl_list.nat == "":
                    cl_list.nat = "?"
                else:
                    nation = get_cache(
                        Nation, {"kurzbez": [(eq, cl_list.nat)]})
                ...
                if not zinr_list:
                    ...
                    queasy = get_cache(
                        Queasy, {"key": [(eq, 14)], "char1": [(eq, res_line.zinr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})
            ...
            if exc_compli:
                cl_list = query(cl_list_data, filters=(lambda cl_list: cl_list.rmno == res_line.zinr and cl_list.resnr ==
                                res_line.resnr and cl_list.arrive == res_line.ankunft and cl_list.co > 0), first=True)
        ...
        for cl_list in query(cl_list_data, sort_by=[("nation", False), ("bezeich", False)]):
            s_list = query(s_list_data, filters=(
                lambda s_list: s_list.rmcat == cl_list.kurzbez.strip()), first=True)

            if not s_list:
                s_list = query(s_list_data, filters=(
                    lambda s_list: s_list.rmcat == ""), first=True)
            ...
            s_list = query(s_list_data, filters=(
                lambda s_list: s_list.nat == cl_list.nat.strip()), first=True)

            if not s_list:
                s_list = query(s_list_data, filters=(
                    lambda s_list: s_list.nat == ""), first=True)
        ...
        if (tot_a + tot_co) != 0:
            for s_list in query(s_list_data, filters=(lambda s_list: s_list.nat != "")):
                ...
                s_list.proz = to_decimal(
                    s_list.adult) / to_decimal((tot_a) + to_decimal(tot_co)) * to_decimal("100")

        for s_list in query(s_list_data):
            s_list.proz_qty = (to_decimal(s_list.anz) /
                               to_decimal(tot_qty)) * to_decimal("100")
            s_list.proz_rev = (to_decimal(s_list.rev) /
                               to_decimal(tot_rev)) * to_decimal("100")
    ...
    def create_genstat_inhouse():
        ...
        for genstat, reservation, guest, gmember, sourccod in db_session.query(Genstat, Reservation, Guest, Gmember, Sourccod).join(Reservation, (Reservation.resnr == Genstat.resnr)).join(Guest, (Guest.gastnr == Genstat.gastnr)).join(Gmember, (Gmember.gastnr == Genstat.gastnrmember)).join(Sourccod, (Sourccod.source_code == Reservation.resart)).filter(
            ...
            (Genstat.datum <= to_date) &
                (Genstat.zinr >= (froom).lower()) & (Genstat.zinr <= (troom).lower())).order_by(Genstat.zinr, Genstat.datum, Genstat.erwachs.desc(), Gmember.name).all():
            ...
            if exc_depart and genstat.res_date[0] <= genstat.datum and genstat.res_date[1] == genstat.datum and genstat.resstatus == 8:
                ...
                res_line = get_cache(
                    Res_line, {"resnr": [(eq, genstat.resnr)], "reslinnr": [(eq, genstat.res_int[0])]})
                ...
                cl_list.night = to_string(
                    genstat.res_date[1] - genstat.res_date[0], ">>>>9")
                cl_list.city = gmember.wohnort
                cl_list.keycard = to_string(res_line.betrieb_gast, ">>>>>9")
                cl_list.etage = zimmer.etage
                cl_list.zinr_bez = zimmer.bezeich
...
# end - FT: 199267
                ...
                segment = get_cache(
                    Segment, {"segmentcode": [(eq, genstat.segmentcode)]})
                ...
                if incl_gcomment:
                    ...
                    if gbuff:
                        if gbuff.bemerkung is not None:
                            for i in range(1, length(gbuff.bemerkung) + 1):
                ...
                reslin_queasy = get_cache(
                    Reslin_queasy, {"key": [(eq, "arrangement")], "resnr": [(eq, res_line.resnr)], "reslinnr": [(eq, res_line.reslinnr)], "date1": [(le, genstat.datum)], "date2": [(ge, genstat.datum)]})
                ...
                if not zinr_list:
                    ...
                    queasy = get_cache(
                        Queasy, {"key": [(eq, 14)], "char1": [(eq, genstat.zinr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})
            ...
            if not disp_accompany:
                cl_list = query(cl_list_data, filters=(lambda cl_list: cl_list.rmno == res_line.zinr and cl_list.resnr == res_line.resnr and cl_list.arrive ==
                                res_line.ankunft and cl_list.zipreis == 0 and (cl_list.a + cl_list.c) < 1 and cl_list.co < 1), first=True)
        ...
        for cl_list in query(cl_list_data, sort_by=[("nation", False), ("bezeich", False)]):
            s_list = query(s_list_data, filters=(
                lambda s_list: s_list.rmcat == cl_list.kurzbez), first=True)
            ...
            s_list = query(s_list_data, filters=(
                lambda s_list: s_list.nat == cl_list.nat), first=True)
            ...
            if exc_compli:
                s_list.adult = s_list.adult + cl_list.a
            else:
                s_list.adult = s_list.adult + cl_list.a + cl_list.co
            s_list.child = s_list.child + cl_list.c
    ...
    def get_argtline_rate(contcode: string, argt_recid: int):
        ...
        if add_it:
            reslin_queasy = get_cache(
                Reslin_queasy, {"key": [(eq, "fargt-line")], "char1": [(eq, "")], "resnr": [(eq, res_line.resnr)], "reslinnr": [(eq, res_line.reslinnr)], "number1": [(eq, argtline.departement)], "number2": [(eq, argtline.argtnr)], "number3": [(eq, argtline.argt_artnr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})
            ...
            if contcode != "":
                reslin_queasy = get_cache(
                    Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, res_line.reserve_int)], "number2": [(eq, arrangement.argtnr)], "number3": [(eq, argtline.argt_artnr)], "resnr": [(eq, argtline.departement)], "reslinnr": [(eq, curr_zikatnr)], "date1": [(le, curr_date)], "date2": [(ge, curr_date)]})
    ...
    if to_date < curr_date:
        ...
        zinrstat = get_cache(
            Zinrstat, {"zinr": [(eq, "tot-rm")], "datum": [(eq, to_date)]})
