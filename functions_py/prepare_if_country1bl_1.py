from functions.additional_functions import *
import decimal
from sqlalchemy import func
from models import Parameters

t_parameters_list, T_parameters = create_model_like(Parameters, {"rec_id":int})
cost_list_list, Cost_list = create_model("Cost_list", {"rec_id":int, "zone":str, "grace":int, "wday":int, "ftime":int, "ttime":int, "tdura":int, "dura":int, "cost":decimal, "info":str})
zone_list_list, Zone_list = create_model("Zone_list", {"rec_id":int, "zone":str, "city":str, "acode":str, "info":str})


def prepare_if_country1bl(fl_run_proc:bool):
    global cost_list_list, zone_list_list, t_parameters_list
    cost_list_list = []
    zone_list_list = []
    t_parameters_list = []
    parameters = None
    ifname:str = "if_internal"

    t_parameters = cost_list = zone_list = None
    db_session = local_storage.db_session

    def generate_output():
        nonlocal parameters
        nonlocal t_parameters, cost_list, zone_list
        global t_parameters_list, cost_list_list, zone_list_list
        return {"cost-list": cost_list_list, "zone-list": zone_list_list, "t-parameters": t_parameters_list}

    def cleanup_zonelist():

        nonlocal  parameters
        nonlocal t_parameters, cost_list, zone_list
        # nonlocal t_parameters_list, cost_list_list, zone_list_list
        global t_parameters_list, cost_list_list, zone_list_list

        parameters = db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == "if_internal") &  
                (func.lower(Parameters.section) == "dcode") &  
                (Parameters.varname > "")).first()

        if parameters:
            for parameters in db_session.query(Parameters).filter(
                    (func.lower(Parameters.progname) == "if_internal") &  
                    (func.lower(Parameters.section) == "dcode") &  
                    (Parameters.varname > "")).with_for_update().all():
                
                db_session.delete(parameters)

    def create_costlist():
        nonlocal  parameters
        nonlocal t_parameters, cost_list, zone_list
        global t_parameters_list, cost_list_list, zone_list_list
        print("CreateCost.")
        i:int = 0
        m:int = 1
        n:int = 0
        s:str = ""
        k:int = 0

        for parameters in db_session.query(Parameters).filter(
                (func.lower(Parameters.progname) == "if_internal") & 
                (func.lower(Parameters.section) == "zone") &  
                (Parameters.varname > "")).all():
            cost_list = Cost_list()
            cost_list_list.append(cost_list)

            cost_list.rec_id = parameters._recid
            cost_list.zone = parameters.varname
            i = 1
            n = 0
            m = 1
            while i <= 8 and n < len(parameters.vstring) :
                n = n + 1

                if substring(parameters.vstring, n - 1, 1) == ";":

                    if i == 1:
                        cost_list.grace = to_int(substring(parameters.vstring, m - 1, n - m))

                    elif i == 2:
                        cost_list.wday = to_int(substring(parameters.vstring, m - 1, n - m))

                    elif i == 3:
                        cost_list.ftime = to_int(substring(parameters.vstring, m - 1, n - m))

                    elif i == 4:
                        cost_list.ttime = to_int(substring(parameters.vstring, m - 1, n - m))

                    elif i == 5:
                        cost_list.tdura = to_int(substring(parameters.vstring, m - 1, n - m))

                    elif i == 6:
                        cost_list.dura = to_int(substring(parameters.vstring, m - 1, n - m))

                    elif i == 7:

                        if substring(parameters.vstring, n - 3 - 1, 1) == "." or substring(parameters.vstring, n - 3 - 1, 1) == ",":
                            cost_list.cost = decimal.Decimal(substring(parameters.vstring, m - 1, n - m - 3))
                            cost_list.cost = cost_list.cost + decimal.Decimal(substring(parameters.vstring, n - 2 - 1, 2)) / 100
                        else:
                            cost_list.cost = decimal.Decimal(substring(parameters.vstring, m - 1, n - m - 2))
                            cost_list.cost = cost_list.cost + decimal.Decimal(substring(parameters.vstring, n - 2 - 1, 2)) / 100

                    elif i == 8:
                        cost_list.info = substring(parameters.vstring, m - 1, n - m)
                    m = n + 1
                    i = i + 1

    def create_zonelist():
        nonlocal parameters, ifname
        nonlocal t_parameters, cost_list, zone_list
        global t_parameters_list, cost_list_list, zone_list_list
        print("CreateZone.")
        i:int = 0
        m:int = 1
        n:int = 0
        
        # IF substring(PROVERSION, 0, 1) >= "9" or substring(PROVERSION, 0, 1) = "1" &THEN ifname = "interface"
        ifname = "interface"

    if fl_run_proc:
        cleanup_zonelist()
    create_costlist()
    create_zonelist()

    for parameters in db_session.query(Parameters).filter(
            (func.lower(Parameters.progname) == "interface") &  (func.lower(Parameters.section) == "zone")).all():
        t_parameters = T_parameters()
        t_parameters_list.append(t_parameters)

        buffer_copy(parameters, t_parameters)
        t_parameters.rec_id = parameters._recid

    for parameters in db_session.query(Parameters).filter(
            (func.lower(Parameters.progname) == (ifname).lower()) &  (func.lower(Parameters.section) == "dcode") &  (Parameters.varname > "")).all():
        zone_list = Zone_list()
        zone_list_list.append(zone_list)

        zone_list.rec_id = parameters._recid
        zone_list.zone = parameters.varname
        i = 1
        n = 0
        m = 1
        while i <= 3 and n < len(parameters.vstring) :
            n = n + 1

            if substring(parameters.vstring, n - 1, 1) == ";":

                if i == 1:
                    zone_list.city = substring(parameters.vstring, m - 1, n - m)

                elif i == 2:
                    zone_list.acode = substring(parameters.vstring, m - 1, n - m)

                elif i == 3:
                    zone_list.info = substring(parameters.vstring, m - 1, n - m)
                m = n + 1
                i = i + 1

    return generate_output()