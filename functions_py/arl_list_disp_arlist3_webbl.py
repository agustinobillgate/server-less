...
def arl_list_disp_arlist3_webbl(show_rate:bool, last_sort:int, lresnr:int, long_stay:int, ci_date:date, grpflag:bool, room:string, lname:string, sorttype:int, fdate1:date, fdate2:date, fdate:date, excl_rmshare:bool, voucher_no:string):
    ...
    def generate_output():
        nonlocal rmlen, arl_list_data, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, tmpdate, tmpint, stay, resbemerk, rescomment, blacklist_int, guest, paramtext, htparam, res_line, waehrung, reservation, zimkateg, guestseg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer, queasy, fixleist
