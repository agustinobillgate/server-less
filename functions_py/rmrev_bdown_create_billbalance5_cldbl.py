...
def rmrev_bdown_create_billbalance5_cldbl(exc_taxserv:bool, pvILanguage:int, new_contrate:bool, foreign_rate:bool, price_decimal:int, fdate:date, tdate:date, srttype:int, id_flag:string):
    ...
    argt_line = waehrung = guest = artikel = htparam = res_line = zimmer = genstat = arrangement = exrate = reservation = billjournal = bill = segment = zimkateg = reslin_queasy = queasy = None
    ...
    def generate_output():
        nonlocal cl_list_data, currency_list_data, sum_list_data, s_list_data, argt_list_data, exchg_rate, frate, post_it, total_rev, counter, lvcarea, argt_line, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, billjournal, bill, segment, zimkateg, reslin_queasy, queasy
        nonlocal exc_taxserv, pvILanguage, new_contrate, foreign_rate, price_decimal, fdate, tdate, srttype, id_flag
    ...
    def handle_null_date(inp_date:date):
        ...
        if inp_date == None:
            ...
            return to_string(inp_date, "99/99/9999")
    ...
    def get_argtline_rate(contcode:string, argt_recid:int):
        ...
        if add_it:
            ...
            reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "fargt-line")],"char1": [(eq, "")],"resnr": [(eq, res_line.resnr)],"reslinnr": [(eq, res_line.reslinnr)],"number1": [(eq, argtline.departement)],"number2": [(eq, argtline.argtnr)],"number3": [(eq, argtline.argt_artnr)],"date1": [(le, tdate)],"date2": [(ge, fdate)]})
            ...
            if contcode != "":
                ...
                reslin_queasy = get_cache (Reslin_queasy, {"key": [(eq, "argt-line")],"char1": [(eq, contcode)],"number1": [(eq, res_line.reserve_int)],"number2": [(eq, arrangement.argtnr)],"number3": [(eq, argtline.argt_artnr)],"resnr": [(eq, argtline.departement)],"reslinnr": [(eq, curr_zikatnr)],"date1": [(le, tdate)],"date2": [(ge, fdate)]})
    ...
    def get_genstat_argt_betrag(frate:Decimal, argt_betrag:Decimal):
        ...
        nonlocal cl_list_data, currency_list_data, sum_list_data, s_list_data, argt_list_data, exchg_rate, post_it, total_rev, counter, lvcarea, argt_line, waehrung, guest, artikel, htparam, res_line, zimmer, genstat, arrangement, exrate, reservation, billjournal, bill, segment, zimkateg, reslin_queasy, queasy
        nonlocal exc_taxserv, pvILanguage, new_contrate, foreign_rate, price_decimal, fdate, tdate, srttype, id_flag
        nonlocal waehrung1, cc_list
    ...
    def check_fixleist_posted(artnr:int, dept:int, fakt_modus:int, intervall:int, lfakt:date):
        ...
        if fakt_modus == 1:
            ...
            if tdate <= (start_date + timedelta(days=(intervall - 1))):
                post_it = True
    ...
    def create_partial_cl_list():
        ...
        queasy_str = to_string(cl_list.zipreis) + "|" +\
                ...
                to_string(cl_list.lodging) + "|" +\
                to_string(cl_list.bfast) + "|" +\
                to_string(cl_list.lunch) + "|" +\
                to_string(cl_list.dinner) + "|" +\
                to_string(cl_list.misc) + "|" +\
                to_string(cl_list.fixcost) + "|" +\
                to_string(cl_list.t_rev) + "|" +\
                to_string(handle_null_char (cl_list.c_zipreis)) + "|" +\
                to_string(handle_null_char (cl_list.c_localrate)) + "|" +\
                to_string(handle_null_char (cl_list.c_lodging)) + "|" +\
                to_string(handle_null_char (cl_list.c_bfast)) + "|" +\
                to_string(handle_null_char (cl_list.c_lunch)) + "|" +\
                to_string(handle_null_char (cl_list.c_dinner)) + "|" +\
                to_string(handle_null_char (cl_list.c_misc)) + "|" +\
                to_string(handle_null_char (cl_list.c_fixcost)) + "|" +\
                to_string(handle_null_char (cl_list.ct_rev)) + "|" +\
                to_string(cl_list.res_recid) + "|" +\
                to_string(cl_list.sleeping) + "|" +\
                to_string(cl_list.row_disp) + "|" +\
                to_string(handle_null_char (cl_list.flag)) + "|" +\
                to_string(handle_null_char (cl_list.zinr)) + "|" +\
                to_string(cl_list.rstatus) + "|" +\
                to_string(handle_null_char (cl_list.argt)) + "|" +\
                to_string(handle_null_char (cl_list.currency)) + "|" +\
                to_string(handle_null_char (cl_list.ratecode)) + "|" +\
                to_string(cl_list.pax) + "|" +\
                to_string(cl_list.com) + "|" +\
                to_string(handle_null_date (cl_list.ankunft)) + "|" +\
                to_string(handle_null_date (cl_list.abreise)) + "|" +\
                to_string(cl_list.rechnr) + "|" +\
                to_string(handle_null_char (cl_list.name)) + "|" +\
                to_string(handle_null_char (cl_list.ex_rate)) + "|" +\
                to_string(handle_null_char (cl_list.fix_rate)) + "|" +\
                to_string(cl_list.adult) + "|" +\
                to_string(cl_list.ch1) + "|" +\
                to_string(cl_list.ch2) + "|" +\
                to_string(cl_list.comch) + "|" +\
                to_string(cl_list.age1) + "|" +\
                to_string(handle_null_char (cl_list.age2)) + "|" +\
                to_string(handle_null_char (cl_list.rmtype)) + "|" +\
                to_string(cl_list.resnr) + "|" +\
    ...
    def create_partial_sum_list():
        ...
        queasy_str = to_string(handle_null_char (sum_list.bezeich)) + "|" +\
                to_string(sum_list.pax) + "|" +\
                to_string(sum_list.adult) + "|" +\
                to_string(sum_list.ch1) + "|" +\
                to_string(sum_list.ch2) + "|" +\
                to_string(sum_list.comch) + "|" +\
                to_string(sum_list.com) + "|" +\
    ...
    def create_partial_s_list():
        ...
        queasy_str = to_string(s_list.artnr) + "|" +\
                to_string(s_list.dept) + "|" +\
                to_string(handle_null_char (s_list.bezeich)) + "|" +\
                to_string(s_list.curr) + "|" +\
                to_string(s_list.anzahl) + "|" +\
    ...
    def create_partial_argt_list():
        ...
        queasy_str = to_string(argt_list.argtnr) + "|" +\
                to_string(argt_list.argtcode) + "|" +\
                to_string(handle_null_char (argt_list.bezeich)) + "|" +\
                to_string(argt_list.room) + "|" +\
