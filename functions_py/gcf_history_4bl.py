...
from models import History, Paramtext, Guest, Res_line, Queasy

def gcf_history_4bl(gastnr:int, guest_phone:string, guest_name:string, guest_email:string, fdate:date, tdate:date):

    prepare_cache ([History, Paramtext, Guest, Res_line, Queasy])
    ...
    gastnr1:int = 0
    gastnr2:int = 0
    history = paramtext = guest = res_line = queasy = None
    ...
    def generate_output():
        nonlocal ghistory_data, summ_list_data, htl_name, str, i, gastnr1, gastnr2, history, paramtext, guest, res_line, queasy
        nonlocal gastnr, guest_phone, guest_name, guest_email, fdate, tdate
        nonlocal hist1
    ...
    def create_ghistory():
        ...
        for history in db_session.query(History).filter(
                 (History.gastnr == gastnr2) & (History.abreise >= fdate) & (History.abreise <= tdate)).order_by(History.abreise.desc()).all():
            ghistory = Ghistory()
            ...
            if guest:
                ...
                if queasy:
                    ...
                    ghistory.bemerk = guest.bemerkung
    ...
    def create_ghistory_all():
        ...
        for history in db_session.query(History).filter(
                 (History.gastnr == gastnr2)).order_by(History.abreise.desc()).all():
            ghistory = Ghistory()
            ghistory_data.append(ghistory)
    ...
    guest = get_cache (Guest, {"mobil_telefon": [(eq, guest_phone),(ne, None),(ne, " ")]})

    if guest:
        gastnr2 = guest.gastnr


    else:

        guest = get_cache (Guest, {"email_adr": [(eq, guest_email),(ne, None),(ne, " ")]})
