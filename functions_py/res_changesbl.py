...
"""_yusufwijasena_13/11/2025
    ...
    Ticket ID: F6D79E
        _remark_:   - last update from FDL: B57A07
...
def res_changesbl(pvilanguage:int, res_mode:str, guestname:str, mr_comment:str, rl_comment:str, user_init:str, earlyci:bool, fixed_rate:bool, reslin_list_data:list[Reslin_list]):
    ...
    def res_changes():
        ...
        old_bill_adr = ""
        new_bill_adr = ""
        old_rmtype = ""  # FDL: B57A07
        new_rmtype = ""  # FDL: B57A07
        rmtype_charge = ""  # FDL: B57A07
        ...
        if do_it:
            ...
            if res_line.resstatus != reslin_list.resstatus:
                reslin_queasy.char3 = reslin_queasy.char3 + \
                    ...
                    to_string(cdate, "x(8)") + ";" + \
                    to_string(heute) + ";" + \
                    to_string("ResStatus Changed:") + ";" + \
                    to_string(rstat_list[res_line.resstatus - 1]) + " -> " + to_string(rstat_list[reslin_list.resstatus - 1]) + ";"
            else:
                reslin_queasy.char3 = reslin_queasy.char3 + \
                    to_string(res_line.zipreis) + ";" + \
                    to_string(reslin_list.zipreis) + ";" + \
            ...
            if res_line.resstatus != reslin_list.resstatus:
                ...
                res_history.aenderung = "Resstatus " + chr_unicode(10) + \
                    rstat_list[res_line.resstatus - 1] + chr_unicode(10) + chr_unicode(10) + \
                    "*** Changed to:" + chr_unicode(10) + chr_unicode(10) + \
                    rstat_list[reslin_list.resstatus - 1] + \
                    " ResNo " + to_string(res_line.resnr) + chr_unicode(10) + \
                    " RmNo " + to_string(res_line.zinr)
            ...
            if (reservation.bemerk.lower()  != (mr_comment).lower()) or (res_line.bemerk.lower()  != (rl_comment).lower()):
                ...
                res_history.aenderung = to_string(res_line.resnr) + "-" + reservation.bemerk + chr_unicode(10) + \
                    res_line.bemerk + chr_unicode(10) + chr_unicode(10) + \
                    "*** Changed to:" + chr_unicode(10) + chr_unicode(10) + \
                    mr_comment + chr_unicode(10) + rl_comment

                if bediener:
                    res_history.betriebsnr = bediener.nr

            if (res_line.gastnrpay != reslin_list.gastnrpay) and (res_mode.lower()  == "modify"  or res_mode.lower()  == "inhouse"  or res_mode.lower()  == "split"):
        ...
        if res_line.l_zuordnung[0] != reslin_list.l_zuordnung[0]:
            ...
            reslin_queasy.char3 = to_string(reslin_list.ankunft) + ";" + \
                ...
                to_string("RTC changed:") + ";" + \
                to_string(rtc1) + " -> " + to_string(rtc2) + ";" + \
                to_string("YES", "x(3)") + ";" + \
                to_string("YES", "x(3)") + ";"
        # FDL: B57A07
        if res_line.zikatnr != reslin_list.zikatnr and res_line.l_zuordnung[0] == reslin_list.l_zuordnung[0]:
            zimkateg = get_cache (Zimkateg, {"zikatnr": [(eq, res_line.zikatnr)]})

            if zimkateg:
                old_rmtype = zimkateg.kurzbez

            zimkateg = get_cache (Zimkateg, {"zikatnr": [(eq, reslin_list.zikatnr)]})

            if zimkateg:
                new_rmtype = zimkateg.kurzbez

            zimkateg = get_cache (Zimkateg, {"zikatnr": [(eq, res_line.l_zuordnung[0])]})

            if zimkateg:
                rmtype_charge = zimkateg.kurzbez
            res_history = Res_history()
            db_session.add(res_history)
            ...
            res_history.zeit = zeit
            res_history.action = "Upgrade Room"
            res_history.aenderung = "Upgrade Room: Type From " + old_rmtype + " To " + new_rmtype + " " +\
                    "With RTC Is " + rmtype_charge + " By ResNo: " + to_string(res_line.resnr) + "/" + to_string(res_line.reslinnr, "999")
        # end FDL: B57A07
    def res_changes0():
        nonlocal rtc1, rtc2, lvcarea, res_line, zimkateg, bediener, guest, reservation, reslin_queasy, res_history
        nonlocal pvilanguage, res_mode, guestname, mr_comment, rl_comment, user_init, earlyci, fixed_rate
