...
def check_receiving_webbl(from_date:date, to_date:date, ref_no:string):
    ...
    l_kredit = l_op = l_artikel = l_lieferant = queasy = fa_artikel = fa_op = None
    l_kredit = L_kredit() 
    fa_op = Fa_op()
    queasy = Queasy()
    ...
    if l_op:
        for d in date_range(from_date,to_date) :
            ...
            l_op_obj_list = {}
            for l_op, l_artikel, l_lieferant in db_session.query(L_op, L_artikel, L_lieferant).join(L_artikel,(L_artikel.artnr == L_op.artnr)).join(L_lieferant,(L_lieferant.lief_nr == L_op.lief_nr)).filter((L_op.artnr >= 1000000) & (L_op.artnr <= 9999999) & (L_op.lief_nr > 0) & (L_op.datum == d) & (L_op.op_art <= 2) & (L_op.loeschflag <= 1)).order_by(L_lieferant.firma, L_op.datum, L_artikel.bezeich).all():
    ...
    fa_op_obj_list = {}
    for fa_op, l_lieferant, fa_artikel in db_session.query(Fa_op, L_lieferant, Fa_artikel).join(L_lieferant,(L_lieferant.lief_nr == Fa_op.lief_nr)).join(Fa_artikel,(Fa_artikel.nr == Fa_op.nr)).filter((Fa_op.anzahl != 0) & (Fa_op.loeschflag < 2) & (Fa_op.opart == 1) & (Fa_op.datum >= from_date) & (Fa_op.datum <= to_date)).order_by(Fa_op.lscheinnr).all():
    ...
    l_kredit_obj_list = {}
    for l_kredit, l_lieferant in db_session.query(L_kredit, L_lieferant).join(L_lieferant,(L_lieferant.lief_nr == L_kredit.lief_nr)).filter((L_kredit.lscheinnr != "") & (L_kredit.lief_nr > 0) & (L_kredit.zahlkonto == 0) & (L_kredit.opart <= 2) & (L_kredit.rgdatum >= from_date) & (L_kredit.rgdatum <= to_date) & (L_kredit.counter >= 0) & (L_kredit.netto != 0)).order_by(L_lieferant.firma, L_kredit.rgdatum).all():
        ...
        if l_op:
            ...
            if output_list:
                output_list.ap_amount =  to_decimal(output_list.ap_amount) + to_decimal(l_kredit.netto)
