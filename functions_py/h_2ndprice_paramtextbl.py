...
tt_prices_data, Tt_prices = create_model("Tt_prices", {"i_counter":int, "prices":int})
tt_zeiten_data, Tt_zeiten = create_model("Tt_zeiten", {"i_counter":int, "zeiten":int})

def h_2ndprice_paramtextbl(tt_prices_data:[Tt_prices], tt_zeiten_data:[Tt_zeiten], case_type:int, dept:int, tolerance:int):
    ...
    tt_prices = query(tt_prices_data, first=True)

    tt_zeiten = query(tt_zeiten_data, first=True)

    for tt_prices in query(tt_prices_data):
        prices[tt_prices.i_counter - 1] = tt_prices.prices

    for tt_zeiten in query(tt_zeiten_data):
        zeiten[tt_zeiten.i_counter - 1] = tt_zeiten.zeiten

    # paramtext = get_cache (Paramtext, {"txtnr": [(eq, (10000 + dept))],"number": [(eq, dept)]})
    paramtext = db_session.query(Paramtext).filter(Paramtext.txtnr == (10000 + dept), Paramtext.number == dept).with_for_update().first()
