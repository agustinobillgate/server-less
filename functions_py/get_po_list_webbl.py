...
def get_po_list_webbl(usrname: string, po_number: string, last_doc_nr: string, app_sort: string, dml_only: bool, t_liefno: int, deptnr: int, all_supp: bool, stattype: int, sorttype: int, from_date: date, to_date: date, billdate: date, pr_only: bool, excl_dml_pr: bool):
    ...
    if po_number != "":
        ...
        param267, t_list_data = get_output(po_list_btn_go_1cldbl(
            usrname, po_number, dml_only, pr_only, excl_dml_pr))
    else:
        
        first_docu_nr, curr_docu_nr, param267, last_docu_nr, t_list_data = get_output(po_list_btn_go2_1cldbl(
            t_liefno, last_doc_nr, sorttype, deptnr, all_supp, stattype, usrname, from_date, to_date, billdate, dml_only, app_sort, pr_only, excl_dml_pr))
    ...
    for t_list in query(t_list_data, sort_by=[("docu_nr", False)]):
        # log_program.write_log('DEBUG', f't_list_data: {t_list_data}', 'outputFile.txt')
        q2_list = Q2_list()
        q2_list_data.append(q2_list)

        buffer_copy(t_list, q2_list)

        l_order = get_cache(L_order, {"docu_nr": [(eq, t_list.docu_nr)], "loeschflag": [
                            (eq, loeschflag)], "pos": [(gt, 0)]})
        while l_order:
            q2_list = Q2_list()
            q2_list_data.append(q2_list)
            ...
            l_order = db_session.query(L_order).filter(
                (L_order.docu_nr == t_list.docu_nr) &
                (L_order.loeschflag == loeschflag) &
                (L_order.pos > 0) &
                (L_order._recid > curr_recid)
            ).first()
