...
from models import Guest, Reslin_queasy, Queasy, H_artikel, Htparam, Genstat, Zimkateg, Reservation, Sourccod, Segment, Res_line, Zimmer, Nation, Arrangement, Artikel, Mc_guest, Waehrung, Bediener, Bill, Bill_line, H_bill, H_bill_line
...
def ascottrpt_reservation2bl(datum:date, propid:str, data_list_data:Data_list):

    prepare_cache ([Guest, Queasy, H_artikel, Htparam, Genstat, Zimkateg, Reservation, Sourccod, Segment, Res_line, Zimmer, Nation, Arrangement, Artikel, Mc_guest, Waehrung, Bediener, Bill, Bill_line, H_bill, H_bill_line])
    ...
    guest = reslin_queasy = queasy = h_artikel = htparam = genstat = zimkateg = reservation = sourccod = segment = res_line = zimmer = nation = arrangement = artikel = mc_guest = waehrung = bediener = bill = bill_line = h_bill = h_bill_line = None
    ...
    def generate_tabledata(casetyp:str):
        ...
        if do_it:
            ...
            if not data_list or casetyp.lower()  == "cancel" :
                ...
                # if casetyp.lower()  == ("Cancel").lower() :
                if casetyp.lower()  == "cancel" :
                    data_list.rsvstatus = "Cancelled"

                # elif casetyp.lower()  == ("Guaranteed").lower() :
    ...
    def generate_tabledata2():
        ...
        if do_it:
            ...
            # if data_list.memberno.lower()  == ("NULL").lower() :
            if data_list.memberno.lower()  == "null" :
                data_list.guest_crm = "-"
            ...
            if bediener:
                ...
                qsy19 = get_cache (Queasy, {"key": [(eq, 19)],"number1": [(eq, bediener.user_group)]})
            ...
            # if data_list.idcardtype.lower()  == ("NULL").lower() :
            if data_list.idcardtype.lower()  == "null" :
                data_list.legacy_rsvid = "-"
            ...
            # if data_list.idcardnum.lower()  == ("NULL").lower() :
            if data_list.idcardnum.lower()  == "null" :
                data_list.legacy_guestid = "-"
    ...
    def get_resstatus(ttype:string, stat:int):
        ...
        elif ttype.lower()  == ("genstat").lower() :
            if stat == 6 or stat == 8:
                out_stat = "InHouse"
    ...
    startdate = datum - timedelta(days=90)

    for genstat in db_session.query(Genstat).filter(
            (Genstat.datum >= startdate) & (Genstat.datum < datum) & (Genstat.res_logic[inc_value(1)]) & (Genstat.zinr != "")).order_by(Genstat._recid).all():
        ...
        datum1 = genstat.datum

        generate_tabledata2()
    curr_time = get_current_time_in_seconds()

    for res_line in db_session.query(Res_line).filter(
            (Res_line.resstatus == 9) & (Res_line.cancelled >= startdate) & (Res_line.cancelled < datum) & (Res_line.l_zuordnung[inc_value(2)] == 0)).order_by(Res_line._recid).all():
