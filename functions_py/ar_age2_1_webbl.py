...
def ar_age2_1_webbl(pvilanguage:int, curr_gastnr:int, from_art:int, disptype:int, to_date:date, show_inv:bool):
    ...
    age_list_data, Age_list = create_model("Age_list", {"artnr":int, "rechnr":int, "inv_no":int, "counter":int, "gastnr":int, "creditlimit":Decimal, "rgdatum":date, "gastname":string, "fbetrag":Decimal, "saldo":Decimal, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal, "curr":string, "inv_soa":int})
    ledger_data, Ledger = create_model("Ledger", {"artnr":int, "bezeich":string, "debt0":Decimal, "debt1":Decimal, "debt2":Decimal, "debt3":Decimal, "tot_debt":Decimal})
    output_list_data, Output_list = create_model("Output_list", {"inv_no":string, "fbetrag":string, "creditlimit":string, "curr":string, "str":string, "inv_soa":string, "bill_no":string, "g_name":string, "check_in":string, "check_out":string, "outstand":string, "age":string, "day1":string, "day2":string, "day3":string, "day4":string})
    ...
    def generate_output():
        nonlocal msg_str, output_list_data, day1, day2, day3, price_decimal, default_fcurr, outlist, fremdwbetrag, lvcarea, debitor, artikel, guest, waehrung, bill
        nonlocal pvilanguage, curr_gastnr, from_art, disptype, to_date, show_inv
        nonlocal debt


        nonlocal age_list, ledger, output_list, debt
        nonlocal age_list_data, ledger_data, output_list_data
    ...
    def age_list():
        ...
        output_list.bill_no = to_string(artikel.artnr, ">>>>>>>>9") + " - "
        output_list.g_name = to_string(artikel.bezeich)
        ...
        output_list.g_name = "(" + trim(to_string(gastname)) + ")"
        curr_art = 0
        ...
        for age_list in query(age_list_data, filters=(lambda age_list: age_list.tot_debt != 0)):
            ...
            if price_decimal == 0:
                count_i = num_entries(age_list.gastname, ";")
                output_list.bill_no = to_string(age_list.rechnr, ">>>>>>>>9")
                output_list.inv_no = to_string(age_list.inv_no, ">>>>>>>>>")
                output_list.inv_soa = to_string(age_list.inv_soa, ">>>>>>>>>")
                output_list.curr = age_list.curr
                output_list.outstand = to_string(age_list.tot_debt, "->>>,>>>,>>>,>>>,>>9")
                output_list.age = to_string(to_date - age_list.rgdatum, ">>9")
                output_list.day1 = to_string(age_list.debt0, "->>>,>>>,>>>,>>>,>>9")
                output_list.day2 = to_string(age_list.debt1, "->>>,>>>,>>>,>>>,>>9")
                output_list.day3 = to_string(age_list.debt2, "->>>,>>>,>>>,>>>,>>9")
                output_list.day4 = to_string(age_list.debt3, "->>>,>>>,>>>,>>>,>>9")

                if count_i > 1:
                    output_list.g_name = entry(0, age_list.gastname, ";")

                    if matches(output_list.g_name,r"*././.-././.*"):
                        output_list.g_name = substring(output_list.g_name, 0, length(output_list.g_name) - 17)
                else:
                    output_list.g_name = age_list.gastname

                if fremdwbetrag != 0:
                    output_list.fbetrag = to_string(fremdwbetrag, "->>>,>>>,>>>,>>>,>>9")
            else:
                count_i = num_entries(age_list.gastname, ";")
                output_list.bill_no = to_string(age_list.rechnr, ">>>>>>>>9")
                output_list.inv_no = to_string(age_list.inv_no, ">>>>>>>>>")
                output_list.inv_soa = to_string(age_list.inv_soa, ">>>>>>>>>")
                output_list.curr = age_list.curr
                output_list.outstand = to_string(age_list.tot_debt, "->,>>>,>>>,>>>,>>9.99")
                output_list.age = to_string(to_date - age_list.rgdatum, ">>9")
                output_list.day1 = to_string(age_list.debt0, "->,>>>,>>>,>>>,>>9.99")
                output_list.day2 = to_string(age_list.debt1, "->,>>>,>>>,>>>,>>9.99")
                output_list.day3 = to_string(age_list.debt2, "->,>>>,>>>,>>>,>>9.99")
                output_list.day4 = to_string(age_list.debt3, "->,>>>,>>>,>>>,>>9.99")
        ...
        if price_decimal == 0:
            output_list.g_name = translateExtended ("T O T A L A/R:", lvcarea, "")
            output_list.outstand = to_string(t_saldo, "->>>,>>>,>>>,>>>,>>9")
            output_list.day1 = to_string(t_debt0, "->>>,>>>,>>>,>>>,>>9")
            output_list.day2 = to_string(t_debt1, "->>>,>>>,>>>,>>>,>>9")
            output_list.day3 = to_string(t_debt2, "->>>,>>>,>>>,>>>,>>9")
            output_list.day4 = to_string(t_debt3, "->>>,>>>,>>>,>>>,>>9")


        else:
            output_list.g_name = translateExtended ("T O T A L A/R:", lvcarea, "")
            output_list.outstand = to_string(t_saldo, "->>>,>>>,>>>,>>>,>>9.99")
            output_list.day1 = to_string(t_debt0, "->>>,>>>,>>>,>>>,>>9.99")
            output_list.day2 = to_string(t_debt1, "->>>,>>>,>>>,>>>,>>9.99")
            output_list.day3 = to_string(t_debt2, "->>>,>>>,>>>,>>>,>>9.99")
            output_list.day4 = to_string(t_debt3, "->>>,>>>,>>>,>>>,>>9.99")
