...
from models import Reservation, Res_line, Guest, Bill, Htparam, Master, Exrate, Waehrung, Bill_line, Artikel, Zimkateg, Arrangement, Reslin_queasy, Guest_pr, Queasy, Fixleist, Billjournal, Genstat


def arl_list_run_mi_invoice_23bl(resnr: int, curr_resnr: int, arl_list_reslinnr: int, t_active_flag: int, printtype: int, split_bill: bool):

    prepare_cache([Reservation, Res_line, Bill, Htparam, Master, Exrate, Waehrung, Bill_line, Artikel, Zimkateg, Arrangement, Reslin_queasy, Guest_pr, Queasy, Fixleist, Billjournal, Genstat])
    ...
    t_char = ""
    t_resnr: int = 0
    reservation = res_line = guest = bill = htparam = master = exrate = waehrung = bill_line = artikel = zimkateg = arrangement = reslin_queasy = guest_pr = queasy = fixleist = billjournal = genstat = None

    s_list = t_list = periode_list = t_reservation = t_res_line = t_guest = rline = mbill = mainres = b_bill = None
    ...
    t_reservation_data, T_reservation = create_model_like(
        Reservation,
        {
            "bill_receiver": str
        })
    ...
    def generate_output():
        nonlocal err_flag, avail_master, avail_bill, reslinnr, master_rechnr, bill_rechnr, mainres_gastnr, t_reservation_data, t_res_line_data, t_guest_data, t_list_data, new_contrate, resline_exrate, billdate, bonus_array, tot_amt, serv1, vat1, vat3, fact1, netto, bill_receiver, month_str1, month_str2, currency, local_currency, frate, waehrungsnr, price_decimal, t_char, t_resnr, reservation, res_line, guest, bill, htparam, master, exrate, waehrung, bill_line, artikel, zimkateg, arrangement, reslin_queasy, guest_pr, queasy, fixleist, billjournal, genstat
        nonlocal resnr, curr_resnr, arl_list_reslinnr, t_active_flag, printtype, split_bill
        nonlocal rline, mbill, mainres, b_bill
        nonlocal s_list, t_list, periode_list, t_reservation, t_res_line, t_guest, rline, mbill, mainres, b_bill
        nonlocal s_list_data, t_list_data, periode_list_data, t_reservation_data, t_res_line_data, t_guest_data

        return {
            "err_flag": err_flag,
            "avail_master": avail_master,
            "avail_bill": avail_bill,
            "reslinnr": reslinnr,
            "master_rechnr": master_rechnr,
            "bill_rechnr": bill_rechnr,
            "mainres_gastnr": mainres_gastnr,
            "t-reservation": t_reservation_data,
    ...
    def read_proforma_inv(resnr: int, rechnr: int):
        ...
        for resline in db_session.query(Resline).filter(
                ...
                (Resline.resstatus != 12) & 
                (Resline.resstatus != 9) & 
                (Resline.resstatus != 10) & 
                (Resline.resstatus != 99)).order_by(Resline._recid).all():
            ...
            zimkateg = get_cache(
                Zimkateg, {"zikatnr": [(eq, resline.zikatnr)]})

            arrangement = get_cache(
                Arrangement, {"arrangement": [(eq, resline.arrangement)]})
            ...
            for datum in date_range(resline.ankunft, co_date):
                ...
                if fixed_rate:
                    reslin_queasy = get_cache(
                        Reslin_queasy, {"key": [(eq, "arrangement")], "resnr": [(eq, resline.resnr)], "reslinnr": [(eq, resline.reslinnr)], "date1": [(le, datum)], "date2": [(ge, datum)]})
                    ...
                    guest = get_cache(
                        Guest, {"gastnr": [(eq, resline.gastnr)]})
                    ...
                    if guest_pr:
                        queasy = get_cache(
                            Queasy, {"key": [(eq, 18)], "number1": [(eq, resline.reserve_int)]})
                        ...
                        if new_contrate:
                            if resline_exrate != 0:
                                rate_found, rm_rate, early_flag, kback_flag = get_output(
                                    ratecode_rate(ebdisc_flag, kbdisc_flag, resline.resnr, resline.reslinnr, guest_pr.code, None, bill_date, resline.ankunft, resline.abreise, resline.reserve_int, arrangement.argtnr, curr_zikatnr, resline.erwachs, resline.kind1, resline.kind2, resline_exrate, resline.betriebsnr))
                            else:
                                rate_found, rm_rate, early_flag, kback_flag = get_output(
                                    ratecode_rate(ebdisc_flag, kbdisc_flag, resline.resnr, resline.reslinnr, guest_pr.code, None, bill_date, resline.ankunft, resline.abreise, resline.reserve_int, arrangement.argtnr, curr_zikatnr, resline.erwachs, resline.kind1, resline.kind2, resline.reserve_dec, resline.betriebsnr))
                        else:
                            rm_rate, rate_found = get_output(
                                pricecod_rate(resline.resnr, resline.reslinnr, guest_pr.code, bill_date, resline.ankunft, resline.abreise, resline.reserve_int, arrangement.argtnr, curr_zikatnr, resline.erwachs, resline.kind1, resline.kind2, resline.reserve_dec, resline.betriebsnr))

                            if it_exist:
                                rate_found = True

                            if not it_exist and bonus_array[datum - resline.ankunft]:
                ...
                for fixleist in db_session.query(Fixleist).filter(
                        (Fixleist.resnr == resline.resnr) & 
                        (Fixleist.reslinnr == resline.reslinnr)).order_by(Fixleist._recid).all():
                    add_it = False
                    argt_rate = to_decimal("0")
                    ...
                    if add_it:
                        ...
                        if not fixed_rate and guest_pr:
                            ...
                            reslin_queasy = get_cache(
                                Reslin_queasy, {"key": [(eq, "argt-line")], "char1": [(eq, contcode)], "number1": [(eq, resline.reserve_int)], "number2": [(eq, arrangement.argtnr)], "reslinnr": [(eq, resline.zikatnr)], "number3": [(eq, fixleist.artnr)], "resnr": [(eq, fixleist.departement)], "date1": [(le, bill_date)], "date2": [(ge, bill_date)]})

                            if reslin_queasy:
                                argt_rate = to_decimal(
                                    reslin_queasy.deci1) * to_decimal(fixleist.number)
                    ...
                    if argt_rate != 0:
                        ...
                        if not s_list:
                            ...
                            s_list.preis = to_decimal(
                                argt_rate) / to_decimal(fixleist.number)
                            s_list.datum = datum
                            s_list.ankunft = resline.ankunft
                            s_list.abreise = resline.abreise
                            s_list.erwachs = pax
                            s_list.kind1 = resline.kind1
                            s_list.kind2 = resline.kind2
                        s_list.qty = s_list.qty + \
                            (fixleist.number * resline.zimmeranz)

        queasy = get_cache(
            Queasy, {"key": [(eq, 301)], "number1": [(eq, resnr)], "logi1": [(eq, True)]})

        if queasy:
            calc_periode()

            for s_list in query(s_list_data, sort_by=[("ankunft", False), ("datum", False), ("bezeich", False), ("erwachs", False)]):
                periode_list = query(periode_list_data, filters=(lambda periode_list: periode_list.s_list.datum >= periode_list.periode1 and s_list.datum <= periode_list.periode2), first=True)

                if periode_list:
                    t_list = query(t_list_data, filters=(lambda t_list: t_list.bezeich == s_list.bezeich and t_list.rmcat == s_list.rmcat and t_list.date1 == periode_list.periode1 and t_list.date2 == periode_list.periode2), first=True)
                    ...
                    if not t_list:
                        ...
                        t_list.date1 = periode_list.periode1
                        t_list.date2 = periode_list.periode2
                        t_list.ankunft = periode_list.periode1
                        t_list.abreise = periode_list.periode2
                    ...
                    t_list.preis = to_decimal(
                        t_list.preis + round(s_list.preis, 0))
                    t_list.lrate = to_decimal(
                        t_list.lrate + round(s_list.lrate, 0))
                ...
                for s_list in query(s_list_data, sort_by=[("ankunft", False), ("datum", False), ("bezeich", False), ("erwachs", False)]):
                    ...
                    if s_list.nr == 0:
                        ...
                        t_list = query(t_list_data, filters=(lambda t_list: t_list.bezeich == s_list.bezeich and t_list.preis == s_list.preis and t_list.ankunft == s_list.ankunft and t_list.abreise == s_list.abreise and t_list.erwachs == s_list.erwachs and t_list.kind1 == s_list.kind1 and t_list.kind2 == s_list.kind2), first=True)
        ...
        for t_list in query(t_list_data):
            t_list.betrag = to_decimal(
                t_list.qty) * to_decimal(t_list.tage) * to_decimal(t_list.preis)
        ...
        if rechnr > 0:
            ...
            for bill_line.bezeich, bill_line.betrag, bill_line.bill_datum, bill_line._recid, artikel.bezeich, artikel.artnr, artikel.departement, artikel.epreis, artikel.artart, artikel._recid in db_session.query(Bill_line.bezeich, Bill_line.betrag, Bill_line.bill_datum, Bill_line._recid, Artikel.bezeich, Artikel.artnr, Artikel.departement, Artikel.epreis, Artikel.artart, Artikel._recid).join(Artikel, (Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement)).filter(
                ...
                if artikel.artart == 9:
                    arrangement = get_cache(
                        Arrangement, {"argt_artikelnr": [(eq, artikel.artnr)]})
                ...
                if do_it:
                    ...
                    if matches(artikel.bezeich, r"Heritage Fee*"):
                        t_list = query(t_list_data, filters=(
                            lambda t_list: t_list.bezeich == bill_line.bezeich), first=True)
                    ...
                    serv1, vat1, vat3, fact1 = get_output(calc_servtaxesbl(
                        1, artikel.artnr, artikel.departement, bill_line.bill_datum))
                    netto = to_decimal(bill_line.betrag) / to_decimal(fact1)
                    t_list.vat = to_decimal(
                        netto) * to_decimal((vat1) + to_decimal(vat3))
                ...
                for billjournal.bezeich, billjournal.betrag, billjournal.bill_datum, billjournal._recid, artikel.bezeich, artikel.artnr, artikel.departement, artikel.epreis, artikel.artart, artikel._recid in db_session.query(Billjournal.bezeich, Billjournal.betrag, Billjournal.bill_datum, Billjournal._recid, Artikel.bezeich, Artikel.artnr, Artikel.departement, Artikel.epreis, Artikel.artart, Artikel._recid).join(Artikel, (Artikel.artnr == Billjournal.artnr) & (Artikel.departement == Billjournal.departement)).filter(
                    ...
                    if t_resnr == resnr:
                        ...
                        serv1, vat1, vat3, fact1 = get_output(calc_servtaxesbl(
                            1, artikel.artnr, artikel.departement, billjournal.bill_datum))
                        netto = to_decimal(
                            billjournal.betrag) / to_decimal(fact1)
                        t_list.vat = to_decimal(
                            netto) * to_decimal((vat1) + to_decimal(vat3))
        ...
        if count_heritage < count_night:
            for loopi in range((count_heritage + 1), count_night + 1):
                ...
                if artikel:
                    bill_line = get_cache(
                        Bill_line, {"rechnr": [(eq, bill.rechnr)], "artnr": [(eq, artikel.artnr)]})
                    ...
                    if bill_line:
                        ...
                        t_list = query(t_list_data, filters=(
                            lambda t_list: t_list.bezeich == artikel.bezeich), first=True)
            ...
            for t_list in query(t_list_data, filters=(lambda t_list: matches(t_list.bezeich, r"Heritage Fee*"))):
                ...
                t_list.betrag = to_decimal(
                    t_list.betrag) * to_decimal(t_list.qty)
    ...
    def read_proforma_inv1(resnr: int, reslinnr: int, rechnr: int):
        ...
        for resline in db_session.query(Resline).filter(
            ...
            for datum in date_range(resline.ankunft, co_date):
                ...
                if datum < billdate:
                    genstat = get_cache(
                        Genstat, {"resnr": [(eq, resnr)], "res_int[0]": [(eq, reslinnr)], "datum": [(eq, datum)]})

                    if genstat:
                        rm_rate = to_decimal(genstat.ratelocal)
                    else:
                        exrate = get_cache(
                            Exrate, {"artnr": [(eq, resline.betriebsnr)], "datum": [(eq, datum)]})

                        if exrate:
                            lrate = to_decimal(rm_rate) * to_decimal(exrate.betrag)
                else:
                    waehrung = get_cache(
                        Waehrung, {"waehrungsnr": [(eq, resline.betriebsnr)]})

                    if waehrung:
                        lrate = to_decimal(
                            rm_rate) * to_decimal(waehrung.ankauf) / to_decimal(waehrung.einheit)
        ...
        for t_list in query(t_list_data):
            t_list.betrag = to_decimal(
                t_list.qty) * to_decimal(t_list.tage) * to_decimal(t_list.lrate)
                    ...
                    for bill_line.bezeich, bill_line.betrag, bill_line.bill_datum, bill_line._recid, artikel.bezeich, artikel.artnr, artikel.departement, artikel.epreis, artikel.artart, artikel._recid in db_session.query(Bill_line.bezeich, Bill_line.betrag, Bill_line.bill_datum, Bill_line._recid, Artikel.bezeich, Artikel.artnr, Artikel.departement, Artikel.epreis, Artikel.artart, Artikel._recid).join(Artikel, (Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement)).filter(
                        ...
                        if do_it:
                            ...
                            serv1, vat1, vat3, fact1 = get_output(calc_servtaxesbl(
                                1, artikel.artnr, artikel.departement, bill_line.bill_datum))
                            netto = to_decimal(
                                bill_line.betrag) / to_decimal(fact1)
                            t_list.vat = to_decimal(
                                netto) * to_decimal((vat1) + to_decimal(vat3))
    ...
    def calc_periode():
        ...
        if pqueasy:
            ...
            for pqueasy in db_session.query(Pqueasy).filter(
                    (Pqueasy.key == 329) & 
                    (Pqueasy.number1 == res_line.resnr) & 
                    (Pqueasy.number2 == res_line.reslinnr) & 
                    (Pqueasy.logi3)).order_by(Pqueasy._recid).all():
                ccount = ccount + 1
            ...
            for pqueasy in db_session.query(Pqueasy).filter(
                    ...
                    (Pqueasy.number1 == res_line.resnr) & 
                    (Pqueasy.number2 == res_line.reslinnr) & 
                    (Pqueasy.logi3)).order_by(Pqueasy.date2).all():
    ...
    rline = get_cache(
        Res_line, {"resnr": [(eq, resnr)], "active_flag": [(le, 1)]})
    ...
    bill = get_cache(
        Bill, {"resnr": [(eq, resnr)], "reslinnr": [(eq, reslinnr)]})
    ...
    if res_line:
        ...
        if guest:
            bill_receiver = guest.name + ", " + guest.vorname1 + \
        ...
        if currency.lower() != (local_currency).lower():
            for t_list in query(t_list_data, filters=(lambda t_list: trim(t_list.wabkurz.lower()) == "" or t_list.wabkurz.lower() == (local_currency).lower())):
                ...
                if t_list.date1 < billdate:
                    exrate = get_cache(
                        Exrate, {"artnr": [(eq, waehrungsnr)], "datum": [(eq, t_list.date1)]})

                    if exrate:
                        t_list.betrag = to_decimal(
                            t_list.betrag) / to_decimal(exrate.betrag)
                else:
                    t_list.betrag = to_decimal(
                        t_list.betrag) / to_decimal(frate)

                if price_decimal != 0:
                    t_list.betrag = to_decimal(
                        round(t_list.betrag, price_decimal))
        else:
