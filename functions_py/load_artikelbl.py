...
"""_yusufwijasena_13/11/2025
    ...
    Ticket ID: F6D79E
        _remark_:   - last update ITA: Program terkait feature service apartement
...
def load_artikelbl(case_type: int, deptno: int):
    ...
    artikel_list_data, Artikel_list = create_model(
        "Artikel_list",
        {
            "artnr": int,
            "departement": int,
    ...
    def generate_output():
        nonlocal artikel_list_data, t_artikel_data, artikel, hoteldpt, zwkum, ekum
        nonlocal case_type, deptno
        nonlocal t_artikel, artikel_list
        nonlocal t_artikel_data, artikel_list_data

        return {
            "artikel-list": artikel_list_data,
    ...
    def create_abuff(inp_dept: int):
        ...
        for artikel in db_session.query(Artikel).filter(
            ...
            buffer_copy(artikel, abuff, except_fields=[
                        "departement", "eigentuemer"])
            abuff.eigentuemer = False
            abuff.departement = deptno
            abuff.endkum = 100 + deptno

            if substring(abuff.bezeich, length(abuff.bezeich) - 1 - 1) == "01":
                abuff.bezeich = substring(abuff.bezeich, 0, length(
                    abuff.bezeich) - 2) + to_string(deptno, "99")
    ...
    if case_type == 1:
        ...
        for artikel in db_session.query(Artikel).filter(
                (Artikel.departement == 0) & 
                ((Artikel.artart == 2) | (Artikel.artart == 7) | (Artikel.artart == 14)) &   # ITA: Program terkait feature service apartement
                (Artikel.activeflag)).order_by(Artikel._recid).all():
            artikel_list = Artikel_list()
            artikel_list_data.append(artikel_list)
