...
"""_yusufwijasena_21/10/2025
    ...
    TicketID: 645147
        _issue_:    - fix python indentation
                    - fix var initialization
                    
    _yusufwijasena_24/11/2025
        _issue_:    - cannot post discount by percent
...
def ts_disc1_btn_exit_webbl(rec_id: int, billart: int, dept: int, transdate: date, amount: Decimal, description: str, netto_betrag: Decimal, 
    ...
    def update_bill(h_artart: int, h_artnrfront: int):
        ...
        if not separate_disc_flag:
            ...
            if payload_list is not None and trim(payload_list.voucher_number) != "":  # yusufwijasnea: check if payload_list is not None
                desc_bill = trim(description) + " " + "/" + \
                    " " + payload_list.voucher_number
            else:
            ...
            h_bill_line.fremdwbetrag = to_decimal(
                round(amount / exchg_rate, 2))
        ...
        # if trim(payload_list.voucher_number) != "":
        if payload_list is not None and trim(payload_list.voucher_number) != "":  # yusufwijasena: check if payload_list is not None
        ...
        for disc_list in query(disc_list_data, filters=(lambda disc_list: (disc_list.h_artnr == billart) or (disc_list.netto_amt != 0))):
            if disc_list.amount != 0:
                ...
                if separate_disc_flag:
                    ...
                    h_bill_line.fremdwbetrag = to_decimal(
                        round(disc_list.amount / exchg_rate, price_decimal))
                    h_bill_line.tischnr = tischnr
                    h_bill_line.departement = h_bill.departement
                    h_bill_line.zeit = curr_time
                    h_bill_line.bill_datum = bill_date
                ...
                umsatz.betrag = to_decimal(
                    umsatz.betrag + round(disc_list.amount, price_decimal))
    ...
    def update_rev_argtart(h_artnrfront: int):
        ...
        for h_artikel, artikel in result:
            ...
            menu = query(menu_data, (lambda menu: (
                h_artikel.artnr == menu.artnr)), first=True)
            amount = - (to_decimal(procent) / to_decimal("100")
                        * to_decimal(menu.betrag))
    ...
    def rev_bdown(h_artnrfront: int, qty: int, amount: Decimal):
        ...
        for argt_line in db_session.query(Argt_line).filter(
            ...
            if argt_line.betrag != 0:
                argt_betrag = - (to_decimal(procent) / to_decimal("100")
                                 * to_decimal(argt_line.betrag) * to_decimal(qty))
    ...
    def cal_vat_amount():
        ...
        if vat_list_data:  # yusufwijasena: check if vat_list_data is not None then clear vat_list_data
            vat_list_data.clear()
        else:
            pass
        ...
        for menu in query(menu_data, filters=(lambda menu: menu.menu.prtflag == 1)):
            ...
            if incl_mwst:
                h_service, h_mwst, vat2, fact = get_output(
                    calc_servtaxesbl(1, foart.artnr, foart.departement, None))
            ...
            h_service = to_decimal(
                round(h_service * unit_price, price_decimal))
            ...
            if h_mwst != 0 and vathtp:
                vat_list = query(vat_list_data, filters=(
                    lambda vat_list: vat_list.artNo == foart.artnr), first=True)
                ...
                vat_list.vatamt = to_decimal(
                    vat_list.vatamt - h_mwst * procent) / to_decimal("100")
                vat_list.betrag = to_decimal(
                    vat_list.betrag - menu.betrag * procent) / to_decimal("100")
        ...
        for vat_list in query(vat_list_data):
            ...
            vat_list.fbetrag = to_decimal(
                round(vat_list.betrag / exchg_rate, 2))
