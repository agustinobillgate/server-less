...
def storereq_list_create_list_3_webbl(from_date: date, to_date: date, from_dept: int, to_dept: int, curr_lschein: str, show_price: bool, filter: str):
    ...
    sr_remark_list_data, Sr_remark_list = create_model(
        "Sr_remark_list",
        {
            "lscheinnr": str,
            "sr_remark": str
    ...
    def generate_output():
        ...
        return {
            "it_exist": it_exist,
            "t-list": t_list_data,
            "sr-remark-list": sr_remark_list_data
        }
        ...
        if filter.lower() == "all":
            ...
            for l_op in create_all_query:
                ...
                lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername = create_list_data(
                    lscheinnr, qty, qty1, val, amount, t_qty, t_qty1, t_val, t_amount, appflag, deptno, curr_centername)

        elif filter.lower() == "no-approve":
            l_op_obj_list = {}
            l_op = L_op()
            l_artikel = L_artikel()
    ...
    def create_list_data(lscheinnr: str, qty: Decimal, qty1: Decimal, val: Decimal, amount: Decimal, t_qty: Decimal, t_qty1: Decimal, t_val: Decimal, t_amount: Decimal, appflag: bool, deptno: int, curr_centername: str):
        ...
        if do_it:
            ...
            if l_bestand:
                t_list.stock_oh = to_decimal(l_bestand.anz_anf_best) + to_decimal(
                ...
                if l_ophdr:
                    ...
                    if bediener:
                        t_list.approved_by = to_string(
                            bediener.userinit) + ", " + bediener.username
            ...
            if l_op.op_art == 13 and trim(l_op.stornogrund) != "":
                gl_acct = get_cache(
                    Gl_acct, {"fibukonto": [(eq, l_op.stornogrund)]})

                if not gl_acct:
                    gl_acct = get_cache(
                        Gl_acct, {"bezeich": [(eq, l_op.stornogrund)]})
            ...
            if l_op.anzahl != 0 and show_price:
                ...
                if not long_digit:
                    t_list.price = to_string(
                        (l_op.warenwert / l_op.anzahl), ">>,>>>,>>9.99")
                else:
                    t_list.price = to_string(
                        (l_op.warenwert / l_op.anzahl), ">,>>>,>>>,>>9")
            ...
            t_list.total = to_decimal(t_list.price) * to_decimal(t_list.qty)
            ...
            sr_remark_list = query(sr_remark_list_data, filters=(
                lambda sr_remark_list: sr_remark_list.lscheinnr == l_op.lscheinnr), first=True)
