...
def view_ratecode_webbl(pvilanguage:int, gastnr:int, pr_code:string, market_combo:string):
    ...
    t_viewrates_data, T_viewrates = create_model("T_viewrates", {"prcode":string, "desc_prcode":string, "currency":string, "market":string, "argt":string, "rmtype":string})
    t_viewrates_line_data, T_viewrates_line = create_model("T_viewrates_line", {"prcode":string, "desc_prcode":string, "currency":string, "market":string, "argt":string, "rmtype":string, "datum":string, "str_aci":string, "aci":string, "str_rate_aci":string, "adult_rate":string, "child_rate":string, "infant_rate":string, "deci4":string})
    ...
    def generate_output():
        ...
        nonlocal t_viewrates_data, t_viewrates_line_data, select_list_data

        return {"comments": comments, "t-viewrates": t_viewrates_data, "t-viewrates-line": t_viewrates_line_data}
    ...
    def create_list0():
        ...
        if market_combo != "":
            ...
            for prtable.prcode, prtable.marknr, prtable.product, prtable.zikatnr, prtable.argtnr, prtable._recid, prmarket.nr, prmarket.bezeich, prmarket._recid in db_session.query(Prtable.prcode, Prtable.marknr, Prtable.product, Prtable.zikatnr, Prtable.argtnr, Prtable._recid, Prmarket.nr, Prmarket.bezeich, Prmarket._recid).join(Prmarket,(Prmarket.nr == Prtable.marknr) & (Prmarket.bezeich == (market_combo).lower())).filter(
                ...
                for select_list in query(select_list_data, sort_by=[("argtnr",False),("zikatnr",False)]):
    ...
    def create_selectlist():
        ...
        help_str:string = ""
        product_str:string = ""
        select_list_data.clear()
        for i in range(1,99 + 1) :

            if prtable.product[i - 1] != 0:
                select_list = Select_list()
                select_list_data.append(select_list)
                ...
                if length(product_str) == 3:
                    ...
                    if select_list.zikatnr == 0 or select_list.argtnr == 0:
                        select_list_data.remove(select_list)
