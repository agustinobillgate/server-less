...
def prepare_sarticle_listbl():
    ...
    def generate_output():
        nonlocal first_artnr, curr_art, t_l_artikel_data, counter, l_artikel


        nonlocal t_l_artikel
        nonlocal t_l_artikel_data

        return {"first_artnr": first_artnr, "curr_art": curr_art, "t-l-artikel": t_l_artikel_data}
    ...
    def add_sunits():
        ...
        while None != l_artikel:
            if not matches(l_artikel.herkunft,r"*;*"):
                sbuff = db_session.query(Sbuff).filter(Sbuff._recid == l_artikel._recid).with_for_update().first()
