...
t_spbill_list_data, T_spbill_list = create_model("T_spbill_list", {"selected":bool, "bl_recid":int, "bj_recid":int}, {"selected": True})

def web_interface_print_bill_lnl_selected_1bl(t_spbill_list_data:[T_spbill_list], pvilanguage:int, curr_status:string, briefnr:int, resnr:int, reslinnr:int, inv_type:int, rechnr:int, curr_program:string, gastnr:int, spbill_flag:bool, user_init:string, lnldelimeter:string, for_curr:Decimal, conv_wabkurz:string, curr_bezeich:string, disp_transfer:bool):
    ...
    bline_list = temp_bjournal = bufftemp_bjournal = sum_tbl = t_str3 = t_spbill_list = bl_guest = bline_vatlist = guest1 = resline = None
    ...
    bufftemp_bjournal_data, Bufftemp_bjournal = create_model_like(Temp_bjournal)
    ...
    def generate_output():
        ...
        nonlocal bline_list, temp_bjournal, bufftemp_bjournal, sum_tbl, t_str3, t_spbill_list, bl_guest, bline_vatlist, guest1, resline
        nonlocal bline_list_data, temp_bjournal_data, bufftemp_bjournal_data, sum_tbl_data, t_str3_data, bl_guest_data, bline_vatlist_data
    ...
    if curr_program.lower()  == ("ns-invoice").lower() :
        ...
        if not spbill_flag:
            ...
            bill_line_obj_list = {}
            for bill_line in db_session.query(Bill_line).filter(
                     (Bill_line._recid.in_(list(set([t_spbill_list.bl_recid for t_spbill_list in t_spbill_list_data if t_spbill_list.selected & (t_spbill_list.bl_recid != 0)]))) & (Bill_line.rechnr == rechnr))).order_by(Bill_line._recid).all():
                if bill_line_obj_list.get(bill_line._recid):
                    continue
                else:
                    bill_line_obj_list[bill_line._recid] = True
        ...
        if disp_transfer:
            created, bufftemp_bjournal_data = get_output(prepare_fo_transdetail_lnlbl(rechnr, billdate, artno, amount, systdate, systtime))

            if spbill_flag:

                for t_spbill_list in query(t_spbill_list_data, filters=(lambda t_spbill_list: t_spbill_list.selected  and t_spbill_list.bj_recid != 0)):
                    bufftemp_bjournal = query(bufftemp_bjournal_data, (lambda bufftemp_bjournal: bufftemp_bjournal.bj_recid == t_spbill_list.bj_recid), first=True)
                    if not bufftemp_bjournal:
                        continue

                    temp_bjournal = Temp_bjournal()
                    temp_bjournal_data.append(temp_bjournal)

                    buffer_copy(bufftemp_bjournal, temp_bjournal)
            else:

                for bufftemp_bjournal in query(bufftemp_bjournal_data):
                    temp_bjournal = Temp_bjournal()
                    temp_bjournal_data.append(temp_bjournal)
        ...
        if curr_status.lower()  == ("design").lower() :
            ...
            for bline_list in query(bline_list_data, sort_by=[("datum",False)]):
                bl_voucher = ""
                bl_descript = bline_list.bezeich
                bl_descript0 = entry(0, bl_descript, "/")
            ...
            if created:
                ...
                for temp_bjournal in query(temp_bjournal_data):
                    ...
                    amount_bef_tax =  to_decimal(amount_bef_tax) / to_decimal(fact)
                    bl_balance =  to_decimal(bl_balance) + to_decimal(temp_bjournal.betrag)
                    str3 = to_string(temp_bjournal.bill_datum) + lnldelimeter + bl_descript + lnldelimeter + to_string(temp_bjournal.anzahl, "->>>") + lnldelimeter + to_string(temp_bjournal.zinr) + lnldelimeter + to_string(temp_bjournal.betrag, "->>,>>>,>>>,>>9.99") + lnldelimeter + to_string(temp_bjournal.userinit) + lnldelimeter + bl_guest + lnldelimeter + bl_descript0 + lnldelimeter + bl_voucher + lnldelimeter + to_string(amount_bef_tax, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(temp_bjournal.epreis, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(bl_balance, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + in_word + lnldelimeter + to_string(ma_gst_amount, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(for_curr, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + conv_wabkurz + lnldelimeter + curr_bezeich + lnldelimeter + to_string(" ") + lnldelimeter + to_string(" ")
                    t_str3 = T_str3()
                    t_str3_data.append(t_str3)
