...
def ap_debtpay_remove_voucherbl(age_list_data:[Age_list]):
    ...
    for age_list in query(age_list_data, filters=(lambda age_list: age_list.selected)):
        ...
        queasy = db_session.query(Queasy).filter(Queasy.key == 173, Queasy.number1 == age_list.lief_nr, Queasy.number2 == age_list.rechnr).first()
        ...
        l_kredit = db_session.query(L_kredit).filter(L_kredit._recid == age_list.ap_recid).with_for_update().first()
        l_kredit.rechnr = 0
