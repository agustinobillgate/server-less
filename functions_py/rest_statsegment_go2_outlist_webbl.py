...
def rest_statsegment_go2_outlist_webbl(input_list_data:[Input_list]):
    ...
    while None != queasy:
        ...
        queasy = db_session.query(Queasy).filter(
                 (Queasy.key == 280) & (Queasy.char1 == ("Rest Stat Guest Segment").lower()) & (Queasy._recid > curr_recid)).first()
    out_param_list = Out_param_list()
    out_param_list_data.append(out_param_list)


    pqueasy = db_session.query(Pqueasy).filter(
             (Pqueasy.key == 280) & (Pqueasy.char1 == ("Rest Stat Guest Segment").lower()) & (Pqueasy.char3 == input_list.id_flag)).first()
    ...
    if pqueasy:
        ...
        tqueasy = db_session.query(Tqueasy).filter(
                 (Tqueasy.key == 285) & (Tqueasy.char1 == ("Rest Stat Guest Segment").lower()) & (Tqueasy.number1 == 1) & (Tqueasy.char2 == input_list.id_flag)).first()
    ...
    if not output_list:
        ...
        for tqueasy in db_session.query(Tqueasy).filter(
                 (Tqueasy.key == 285) & (Tqueasy.char1 == ("Rest Stat Guest Segment").lower()) & (Tqueasy.char2 == input_list.id_flag)).order_by(Tqueasy._recid).all():
            db_session.delete(tqueasy)
            pass

        for queasy in db_session.query(Queasy).filter(
                 (Queasy.key == 280) & (Queasy.char1 == ("Rest Stat Guest Segment").lower())).order_by(Queasy._recid).all():
