...
def prepare_eg_mainscheduleed_webbl(pvilanguage:int, mainno:int, user_init:string):
    ...
    his_action_data, His_action = create_model("His_action", {"action_mainnr":int, "action_nr":int, "action_nm":string, "create_date":date, "create_time":int, "time_str":string, "create_by":string})
    mobile_data, Mobile = create_model("Mobile", {"nr":int, "name":string, "position":string, "mobilenr":string, "mobile_selected":bool})
    action_data, Action = create_model_like(Eg_action, {"selected":bool})
    maintain_data, Maintain = create_model_like(Eg_maintain)
    t_property_data, T_property = create_model("T_property", {"prop_nr":int, "prop_nm":string, "prop_selected":bool, "pcateg_nr":int, "pcateg":string, "pmain_nr":int, "pmain":string, "ploc_nr":int, "ploc":string, "pzinr":string})
    tpic_data, Tpic = create_model("Tpic", {"pic_nr":int, "pic_nm":string, "pic_dept":int})
    ...
    def generate_output():
        ...
        nonlocal t_eg_location_data, t_zimmer_data, t_queasy_data, send_alert_data, his_action_data, mobile_data, action_data, maintain_data, t_property_data, tpic_data

        return {"sguestflag": sguestflag, "groupid": groupid, "engid": engid, "sms_flag": sms_flag, "avail_eg_location": avail_eg_location, "h_category": h_category, "h_maintask": h_maintask, "h_location": h_location, "h_zinr": h_zinr, "str_property": str_property, "str_maintask": str_maintask, "str_categ": str_categ, "ci_date": ci_date, "flag1": flag1, "flag2": flag2, "flag3": flag3, "flag4": flag4, "flag5": flag5, "flag6": flag6, "flag7": flag7, "flag8": flag8, "send-alert": send_alert_data, "his-action": his_action_data, "mobile": mobile_data, "action": action_data, "maintain": maintain_data, "t-eg-location": t_eg_location_data, "t-zimmer": t_zimmer_data, "t-queasy": t_queasy_data, "t-property": t_property_data, "tpic": tpic_data}
    ...
    def getmaintain():
        ...
        mbuff = db_session.query(Mbuff).filter(
                 (Mbuff.maintainnr == mainno)).with_for_update().first()
        ...
        if not mbuff:
            ...
            if bprop:
                ...
                if quesbuff:
                    ...
                    action = query(action_data, filters=(lambda action: action.maintask == h_maintask), first=True)
    ...
    def create_mobile():
        ...
        for mobile in query(mobile_data):
            mobile_data.remove(mobile)
    ...
    def getprevaction():
        ...
        for dbuff, buff_action in db_session.query(Dbuff, Buff_action).join(Buff_action,(Buff_action.actionnr == Dbuff.nr)).filter(
            ...
            his_action_data.append(his_action)
    ...
    def create_property():
        ...
        for eg_property in db_session.query(Eg_property).order_by(Eg_property._recid).all():
            ...
            t_property_data.append(t_property)
