...
def bk_salestat_btn_go_webbl(sorttype:int, fdate:date, tdate:date):
    ...
    output_list_data, Output_list = create_model("Output_list", {"flag":string, "bezeich":string, "room":string, "id":string, "ba_event":string, "datum":date, "pax":int, "rmrev":Decimal, "fbrev":Decimal, "othrev":Decimal, "totrev":Decimal, "ev_time":string, "main_contact":string})
    f_list_data, F_list = create_model("F_list", {"bname":string, "room":string, "id":string, "event":string, "cdate":date, "pax":int, "rmrev":Decimal, "fbrev":Decimal, "otrev":Decimal, "totrev":Decimal, "payment_id":string, "ev_time":string, "main_contact":string})
    ...
    def generate_output():
        nonlocal output_list_data, str1, totrmrev, totfbrev, totother, totrev, totpax, subrmrev, subfbrev, subother, subrev, subpax, guest, bediener, bk_raum, ba_typ, bk_veran, bk_stat, bk_func, akt_kont
        nonlocal sorttype, fdate, tdate
        nonlocal gast, usr, room, event, id


        nonlocal output_list, f_list, gast, usr, room, event, id
        nonlocal output_list_data, f_list_data
    ...
    def create_browse():
        ...
        if sorttype == 0:
            ...
            for f_list in query(f_list_data, sort_by=[("payment_id",False)]):
                line1 = line1 + 1
