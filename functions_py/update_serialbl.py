...
def update_serialbl(inp_serial:string):
    ...
    def activate_license(lic_string:string):
        ...
        for curr_i in range(1,num_entries(lic_string, ",")  + 1) :
            ...
            if ct != "":
                ...
                htparam = db_session.query(Htparam).filter(Htparam.paramnr == to_int(ct)).with_for_update().first()
                htparam.flogical = True
                htpchar = to_string(lic_nr, "x(4)") + to_string(htparam.paramnr, "9999") +\
    ...
    def vhp_rooms_pos(i_param:int):
        ...
        htparam = db_session.query(Htparam).filter(Htparam.paramnr == i_param).with_for_update().first()

        htparam.finteger = i_number
        htpchar = to_string(lic_nr, "x(4)") + to_string(htparam.paramnr, "9999") + to_string(htparam.finteger)
        htpchar1 = encode_string(htpchar)
        htparam.fchar = htpchar1
        pass
    ...
    def extent_lic_date():
        ...
        htparam = db_session.query(Htparam).filter(Htparam.paramnr == 976).with_for_update().first()
        pass
        ...
        htparam = db_session.query(Htparam).filter(Htparam.paramnr == 1072).with_for_update().first()
        htparam.finteger = 0
    ...
    def number_char(in_int:int):
        ...
        if in_int > 0 and in_int < 27:
            ...
            out_char = chr_unicode(in_int + 21)

        elif in_int > 36 and in_int < 63:
            out_char = chr_unicode(in_int + 60)
