...
def stock_outlist_btn_chg_webbl(str_list_data, cost_acct:string, op_recid:int):
        ...
        if gl_acct and gl_acct.fibukonto != str_list.fibu:
            ...
            if str_list.other_fibu:
                ...
                l_op = db_session.query(L_op).filter(
                         (L_op._recid == str_list.op_recid)).with_for_update().first()
                ...
                l_ophdr = db_session.query(L_ophdr).filter(
                         (L_ophdr.lscheinnr == str_list.lscheinnr) & (L_ophdr.op_typ == "stt")).with_for_update().first()
