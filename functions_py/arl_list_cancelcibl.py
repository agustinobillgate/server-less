...
def arl_list_cancelcibl(recid_resline:int, user_init:string, t_ankunft:date):
    ...
    if count_i == 1:
        ...
        mbill = get_cache (Bill, {"resnr": [(eq, res_number)],"reslinnr": [(eq, 0)],"saldo": [(ne, 0)],"zinr": [(eq, "")],"flag": [(eq, 0)]})
    ...
    if htparam.flogical:
        get_output(intevent_1(1, room_nr, "CancelCI", res_line.resnr, res_line.reslinnr))   # Rulita 150825 | Modify cancelCI roomnr
