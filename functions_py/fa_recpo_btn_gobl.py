...
# - cannot assign data to fa_order.delivered_qty
# - added with_for_update & optimize query 
# =============================================
...
def fa_recpo_btn_gobl(op_list_data: [Op_list], docu_nr: string, billdate: date, user_init: string, supplier_nr: int, supp_name: string, del_note: string, order_nr: string):

    prepare_cache([Fa_op, Fa_order, Mathis, Fa_artikel, Queasy,
                  Htparam, Fa_ordheader, Bediener, L_kredit, Ap_journal])
            ...
            if fa_artikel:
                ...
                if queasy and queasy.date1 != None:
                    ...
                    if next_mon == 13:
                        ...
                        next_yr = next_yr + 1
                    next_date = date_mdy(
                        next_mon, 1, next_yr) - timedelta(days=1)
                    ...
                    if queasy.date1 < get_current_date():
                        ...
                        next_date = date_mdy(
                            next_mon, 1, next_yr) - timedelta(days=1)
                        fa_artikel.next_depn = next_date
                        queasy.date1 = date_mdy(get_month(get_current_date()), get_day(
                            queasy.date1), get_year(get_current_date()))
                else:
    ...
    def close_po():
        ...
        # print(f"[DEBUG] Closing PO: {order_nr}")

        for fa_order in db_session.query(Fa_order).filter(
                (Fa_order.order_nr == order_nr)).order_by(Fa_order._recid).yield_per(100):
            
            # print(f"[LOG] fa_order: {fa_order.order_nr}")
            
            if allowclose:
                # print(f"[LOG] Order Qty: {fa_order.order_qty}, Delivered Qty: {fa_order.delivered_qty}")
    ...
    for op_list in query(op_list_data):
        ...
        # print(f"[LOG] incoming fa_order: {fa_order.order_nr} | {fa_order.order_qty}")
        ...
        if fa_order:
            ...
            fa_order.delivered_amount = to_decimal(
                fa_order.delivered_amount) + to_decimal(op_list.warenwert)
            fa_order.last_id = user_init
            # pass
            # print(f"[LOG] update fa_order.delivery_qty: {fa_order.delivered_qty}")
            # db_session.commit()
