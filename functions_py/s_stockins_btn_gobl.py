...
def s_stockins_btn_gobl(curr_pos:int, s_artnr:int, price:Decimal, t_amount:Decimal, curr_lager:int, op_list_data:[Op_list], pvilanguage:int, l_out_stornogrund:string, lief_nr:int, billdate:date, fb_closedate:date, m_closedate:date, m_endkum:int, f_endkum:int, b_endkum:int, avail_l_out:bool, lscheinnr:string, rcv_type:int, qty:Decimal, user_init:string):
    ...
    def create_l_op():
        ...
        if ((l_artikel.endkum == f_endkum or l_artikel.endkum == b_endkum) and billdate <= fb_closedate) or (l_artikel.endkum >= m_endkum and billdate <= m_closedate):
            ...
            l_bestand = db_session.query(L_bestand).filter((L_bestand.lager_nr == 0) & (L_bestand.artnr == s_artnr)).with_for_update().first()
            ...
            l_bestand = db_session.query(L_bestand).filter((L_bestand.lager_nr == op_list.lager_nr) & (L_bestand.artnr == s_artnr)).with_for_update().first()
        ...
        if l_lieferant:
            ...
            l_liefumsatz = db_session.query(L_liefumsatz).filter((L_liefumsatz.lief_nr == lief_nr) & (L_liefumsatz.datum == billdate)).with_for_update().first()
    ...
    def create_purchase_book(s_artnr:int, price:Decimal, qty:Decimal, datum:date, lief_nr:int):
        ...
        if curr_anz >= max_anz:
            ...
            l_price1 = db_session.query(L_price1).filter((L_price1.artnr == s_artnr) & (L_price1.counter == 1)).with_for_update().first()
            ...
            for i in range(2,curr_anz + 1) :
                ...
                l_pprice = db_session.query(L_pprice).filter((L_pprice.artnr == s_artnr) & (L_pprice.counter == i)).first()
    ...
    for op_list in query(op_list_data):
        ...
        l_artikel = db_session.query(L_artikel).filter(L_artikel.artnr == s_artnr).first()
