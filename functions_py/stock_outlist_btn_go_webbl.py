...
from functions.stock_outlist_btn_go1_cldbl import stock_outlist_btn_go1_cldbl
...
def stock_outlist_btn_go_webbl(trans_code: str, from_grp: int, mi_alloc: bool, mi_article: bool, mi_docu: bool, mi_date: bool, mattype: int, from_lager: int, to_lager: int, from_date: date, to_date: date, from_art: int, to_art: int, show_price: bool, cost_acct: str, deptno: int, payload_list_data: [Payload_list]):
    it_exist = False
    tot_anz = to_decimal("0.0")
    tot_amount = to_decimal("0.0")
    stock_outlist_data = []

    str_list = stock_outlist = payload_list = None
    ...
    stock_outlist_data, Stock_outlist = create_model(
        "Stock_outlist",
        {
            "datum": date,
            "lager": str,
    ...
    def generate_output():
        ...
        nonlocal str_list, stock_outlist, payload_list
        ...
        return {
            ...
            "stock-outlist": stock_outlist_data
        }
    payload_list = query(payload_list_data, first=True)
    it_exist, tot_anz, tot_amount, str_list_data = get_output(stock_outlist_btn_go1_cldbl(
        trans_code, from_grp, mi_alloc, mi_article, mi_docu, mi_date, mattype, from_lager, to_lager, from_date, to_date, from_art, to_art, show_price, cost_acct, deptno, payload_list.user_number))
    ...
    for str_list in query(str_list_data):
        ...
        stock_outlist.usrname = str_list.usrname
        stock_outlist.fusername = str_list.fusername
