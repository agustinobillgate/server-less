...
from models import Queasy, Artikel, Htparam, Res_line, Arrangement, Reslin_queasy, Bill, Counters, Reservation, Guest, Bediener, Bill_line, Debitor, Billjournal, Umsatz, Gl_jouhdr, Gl_journal


def leasing_create_journal_print_proforma_chgratebl(qrecid: int, pinvoice_no: string, user_init: string):

    prepare_cache([Queasy, Artikel, Htparam, Res_line, Arrangement, Reslin_queasy, Bill, Counters,Reservation, Guest, Bediener, Bill_line, Debitor, Billjournal, Umsatz, Gl_jouhdr, Gl_journal])
    ...
    queasy = artikel = htparam = res_line = arrangement = reslin_queasy = bill = counters = reservation = guest = bediener = bill_line = debitor = billjournal = umsatz = gl_jouhdr = gl_journal = None

    periode_list = bqueasy = bartikel = None
    ...
    def generate_output():
        nonlocal success_flag, log_artnr, ar_ledger, divered_rental, bill_date, tot_amount, tot_nettamount, tot_serv, tot_tax, datum, netto, service, tax, tax2, serv, vat, vat2, fact, loopi, serv_acctno, vat_acctno, vat_fibu, vat2_fibu, serv_fibu, div_fibu, rechnr, diff_amount, prev_tax, prev_serv, prev_amount_debit, prev_amount_credit, month_str1, month_str2, queasy, artikel, htparam, res_line, arrangement, reslin_queasy, bill, counters, reservation, guest, bediener, bill_line, debitor, billjournal, umsatz, gl_jouhdr, gl_journal
        nonlocal qrecid, pinvoice_no, user_init
        nonlocal bqueasy, bartikel
        nonlocal periode_list, bqueasy, bartikel
    ...
    def create_bill_line(billno: int):
        ...
        if bartikel:
            ...
            bill_line.anzahl = 1
            bill_line.betrag = - to_decimal(diff_amount)
            ...
            #     "[" + "Payment Leasing #" + to_string(queasy.number1) + "]"
            bill_line.bezeich = f"{bill_line.bezeich}[Payment Leasing #{to_string(queasy.number1)}]"
    ...
    def create_ar():
        ...
        if not debitor:
            ...
            debitor.saldo = - to_decimal(diff_amount)
            debitor.vesrdep = - to_decimal(diff_amount)
        ...
        billjournal.fremdwaehrng = to_decimal(diff_amount)
        billjournal.betrag = to_decimal(diff_amount)
        ...
        umsatz.betrag = to_decimal(umsatz.betrag) + to_decimal(diff_amount)
    ...
    def calc_periode():
        nonlocal success_flag, log_artnr, ar_ledger, divered_rental, bill_date, tot_amount, tot_nettamount, tot_serv, tot_tax, datum, netto, service, tax, tax2, serv, vat, vat2, fact, serv_acctno, vat_acctno, vat_fibu, vat2_fibu, serv_fibu, div_fibu, rechnr, diff_amount, prev_tax, prev_serv, prev_amount_debit, prev_amount_credit, month_str1, month_str2, queasy, artikel, htparam, res_line, arrangement, reslin_queasy, bill, counters, reservation, guest, bediener, bill_line, debitor, billjournal, umsatz, gl_jouhdr, gl_journal
        nonlocal qrecid, pinvoice_no, user_init
        nonlocal bqueasy, bartikel
        nonlocal periode_list, bqueasy, bartikel
        nonlocal periode_list_data
        ...
        if res_line:
            ...
            if bqueasy:
                for bqueasy in db_session.query(Bqueasy).filter(
                    ...
                    diff_amount = to_decimal(
                        tot_amount - bqueasy.deci1)
