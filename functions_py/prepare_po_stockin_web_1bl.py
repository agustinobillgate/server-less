...
def prepare_po_stockin_web_1bl(lief_nr:int, docu_nr:string, user_init:string):
    ...
    t_l_order_data, T_l_order = create_model_like(L_order, {"rec_id":int, "art_bezeich":string, "jahrgang":int, "alkoholgrad":Decimal, "lief_einheit":Decimal, "curr_disc":int, "curr_disc2":int, "curr_vat":int, "add_vat":Decimal, "addvat_no":int, "addvat_value":Decimal, "amount":Decimal, "masseinheit":string, "traubensorte":string})
    ...
    while None != l_order:
        ...
        t_l_order.curr_disc = to_int(substring(l_order.quality, 0, 2)) * 100 +\
                to_int(substring(l_order.quality, 3, 2))
        t_l_order.curr_vat = to_int(substring(l_order.quality, 6, 2)) * 100 +\
                to_int(substring(l_order.quality, 9, 2))

        if length(t_l_order.quality) >= 17:
            t_l_order.curr_disc2 = to_int(substring(l_order.quality, 12, 2)) * 100 + to_int(substring(l_order.quality, 15, 2))

        if t_l_order.lief_einheit > 1 and t_l_order.geliefert > 0:
            t_l_order.angebot_lief[0] = t_l_order.geliefert % t_l_order.lief_einheit
            t_l_order.geliefert = ( to_decimal(t_l_order.geliefert) - to_decimal(t_l_order.angebot_lief [0])) / to_decimal(t_l_order.lief_einheit)
