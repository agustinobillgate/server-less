...
from models import Printer, Htparam, Artikel, H_bill, H_artikel, Queasy, H_queasy, Kellner, Hoteldpt, H_bill_line, Paramtext, Bediener, Tisch, Res_line, Mc_guest, H_mjourn, Printcod, H_journal, Waehrung

def print_hbill1_phbl(pvilanguage:int, session_parameter:string, user_init:string, hbrecid:int, printnr:int, use_h_queasy:bool, print_all:bool):

    prepare_cache ([Htparam, Artikel, H_bill, Queasy, H_queasy, Kellner, Hoteldpt, H_bill_line, Paramtext, Bediener, Tisch, Res_line, Mc_guest, H_mjourn, Printcod, H_journal, Waehrung])
    ...
    subtotal2:Decimal = to_decimal("0.0")
    ...
    f_discart:int = -1
    is_scd:bool = False
    scd_val:Decimal = to_decimal("0.0")
    sub_scd_val:Decimal = to_decimal("0.0")
    serv_scd_val:Decimal = to_decimal("0.0")
    serv_art:int = 0
    vat_art:int = 0
    sc_art:int = 0
    sc_art1:int = 0
    ...
    f_discart1:int = 0
    f_discart2:int = 0
    scd_perc:Decimal = to_decimal("0.0")
    disc:Decimal = to_decimal("0.0")
    disc_sc:Decimal = to_decimal("0.0")
    str451:string = ""
    str452:string = ""
    t_str:string = ""
    art_str:string = ""
    ...
    is_bev:bool = False
    is_food:bool = False
    printer = htparam = artikel = h_bill = h_artikel = queasy = h_queasy = kellner = hoteldpt = h_bill_line = paramtext = bediener = tisch = res_line = mc_guest = h_mjourn = printcod = h_journal = waehrung = None

    art_list = t_printer = vat_list = output_list = fb_vatlist = t_htp = artbuff = abuff = None

    art_list_data, Art_list = create_model("Art_list", {"printed":bool, "disc_flag":bool, "dept":int, "artnr":int, "bezeich":string, "artart":int, "zwkum":int, "qty":int, "price":Decimal, "amount":Decimal, "betrag":Decimal, "happyhr":string, "datum":date, "sysdate":date, "zeit":int, "condiment":int, "i_group":int})
    t_printer_data, T_printer = create_model_like(Printer)
    vat_list_data, Vat_list = create_model("Vat_list", {"vat_amt":Decimal, "betrag_amt":Decimal})
    output_list_data, Output_list = create_model("Output_list", {"str":string, "pos":int, "flag_popup":bool, "npause":int, "sort_i":int})
    fb_vatlist_data, Fb_vatlist = create_model("Fb_vatlist", {"i_type":int, "artno":int, "vatno":int, "bezeich":string, "vatvalue":Decimal, "vatamt":Decimal})
    t_htp_data, T_htp = create_model_like(Htparam)
    ...
    def generate_output():
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, loctax_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, vat2, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, h_vat2, serv_perc, mwst_perc, vat2_perc, fact, mwst1, subtotal, subtotal2, bline_exist, qty1000, i, n, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, is_scd, scd_val, sub_scd_val, serv_scd_val, serv_art, vat_art, sc_art, sc_art1, sc_art2, vat_str, oth_art, disc_art, f_discart1, f_discart2, scd_perc, disc, disc_sc, str451, str452, t_str, art_str, loopi, bartnr, is_bev, is_food, printer, htparam, artikel, h_bill, h_artikel, queasy, h_queasy, kellner, hoteldpt, h_bill_line, paramtext, bediener, tisch, res_line, mc_guest, h_mjourn, printcod, h_journal, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all
        nonlocal artbuff, abuff


        nonlocal art_list, t_printer, vat_list, output_list, fb_vatlist, t_htp, artbuff, abuff
        nonlocal art_list_data, t_printer_data, vat_list_data, output_list_data, fb_vatlist_data, t_htp_data

        return {"print_all": print_all, "filename": filename, "msg_str": msg_str, "winprinterflag": winprinterflag, "output-list": output_list_data, "t-printer": t_printer_data}
    ...
    def cal_servat(depart:int, h_artnr:int, service_code:int, mwst_code:int, inpdate:date):
        ...
        def generate_inner_output():
            return (serv_perc, mwst_perc, vat2_perc, servat)
        ...
        serv_perc, mwst_perc, vat2_perc, servat = get_output(calc_servtaxesbl(1, abuff.artnr, abuff.departement, inpdate))
        mwst_perc =  to_decimal(mwst_perc)
        vat2_perc =  to_decimal(vat2_perc)
        serv_perc =  to_decimal(serv_perc)
    ...
    def add_unitprice_text():
        ...
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, loctax_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, vat2, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, h_vat2, serv_perc, mwst_perc, vat2_perc, fact, mwst1, subtotal, subtotal2, bline_exist, qty1000, i, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, is_scd, scd_val, sub_scd_val, serv_scd_val, serv_art, vat_art, sc_art, sc_art1, sc_art2, vat_str, oth_art, disc_art, f_discart1, f_discart2, scd_perc, disc, disc_sc, str451, str452, t_str, art_str, loopi, bartnr, is_bev, is_food, printer, htparam, artikel, h_bill, h_artikel, queasy, h_queasy, kellner, hoteldpt, h_bill_line, paramtext, bediener, tisch, res_line, mc_guest, h_mjourn, printcod, h_journal, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all
        nonlocal artbuff, abuff
    ...
    def print_overhead1():
        ...
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, loctax_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, vat2, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, h_vat2, serv_perc, mwst_perc, vat2_perc, fact, mwst1, subtotal, subtotal2, bline_exist, qty1000, n, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, is_scd, scd_val, sub_scd_val, serv_scd_val, serv_art, vat_art, sc_art, sc_art1, sc_art2, vat_str, oth_art, disc_art, f_discart1, f_discart2, scd_perc, disc, disc_sc, str451, str452, t_str, art_str, loopi, bartnr, is_bev, is_food, printer, htparam, artikel, h_bill, h_artikel, queasy, h_queasy, kellner, hoteldpt, h_bill_line, paramtext, bediener, tisch, res_line, mc_guest, h_mjourn, printcod, h_journal, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all
        nonlocal artbuff, abuff
    ...
    def print_billine():
        ...
        ct:string = ""
        bezeich:string = ""
        is_str451:bool = False
        is_str452:bool = False
        h_art = None
        H_art =  create_buffer("H_art",H_artikel)
        t_htp_data = get_output(read_htparambl(5, None, None))

        t_htp = query(t_htp_data, filters=(lambda t_htp: t_htp.paramnr == 10), first=True)
        serv_perc =  to_decimal(t_htp.fdeci)
        ...
        if prtwoline:
            ...
            if art_list.artnr == serv_art:
                serv_scd_val = ( to_decimal((serv_perc) / to_decimal(100)) * to_decimal(subtotal))

                if price_decimal == 0:

                    if not long_digit:
                        output_list.str = output_list.str + to_string(serv_scd_val, "->>>,>>>,>>9")
                    else:
                        output_list.str = output_list.str + to_string(serv_scd_val, "->>>,>>>,>>9")
                else:
                    output_list.str = output_list.str + to_string(serv_scd_val, "->>>,>>>,>>9.99")
            else:
    ...
    def print_overhead2(prall_flag:int):
        ...
        for art_list in query(art_list_data):
            ...
            if art_list.artnr == f_discart or art_list.artnr == f_discart1 or art_list.artnr == f_discart2:
                for i in range(1,pos + 1) :
                    output_list.str = output_list.str + to_string(" ")

                h_bill_line = get_cache (H_bill_line, {"rechnr": [(eq, h_bill.rechnr)],"artnr": [(eq, f_discart)]})
                for i in range(1,nbez1 + 1) :

                    if i > length(h_bill_line.bezeich):
                        output_list.str = output_list.str + to_string(" ")
                    else:
                        output_list.str = output_list.str + to_string(substring(h_bill_line.bezeich, i - 1, 1) , "x(1)")
                disc =  to_decimal(art_list.price)

                if price_decimal == 0:

                    if not long_digit:
                        output_list.str = output_list.str + to_string(art_list.price, "->>>,>>>,>>9")
                        output_list = Output_list()
                        output_list_data.append(output_list)
                    ...
                    output_list.str = output_list.str + to_string(art_list.price, "->>>,>>>,>>9.99")
                    output_list = Output_list()
                    output_list_data.append(output_list)
        ...
        if disc != 0:
            subtotal =  to_decimal(subtotal) + to_decimal(disc)
        ...
        if sc_art != 0:
            ...
            if h_bill_line:
                scd_val =  to_decimal(subtotal) * to_decimal((scd_perc) / to_decimal(100)) * -1
        for i in range(1,pos + 1) :
            output_list.str = output_list.str + to_string(" ")
        ...
        if service != 0 or mwst != 0:
            ...
            if sc_art != 0:
                ...
                if h_bill_line:
                    ...
                    art_list = query(art_list_data, filters=(lambda art_list: art_list.i_group == 2), first=True)
                    mwst =  to_decimal(art_list.amount)
            s = translateExtended ("VAT exempt sale", lvcarea, "")
            output_list.str = fill(" ", pos)
            output_list.str = output_list.str +\
                    substring(s, 0, nbez1) +\
                    fill(" ", nbez1 - length(s))
            ...
            for fb_vatlist in query(fb_vatlist_data, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.i_type >= 3), sort_by=[("i_type",False)]):

                if fb_vatlist.vatAmt != 0:
                    s = fb_vatlist.bezeich
                    output_list.str = fill(" ", pos)
                    output_list.str = output_list.str +\
                            substring(s, 0, nbez1) +\
                            fill(" ", nbez1 - length(s))
                    tot_amount =  to_decimal(tot_amount) + to_decimal(fb_vatlist.vatAmt)

                    if price_decimal == 0:

                        if not long_digit:
                            output_list.str = output_list.str + to_string(fb_vatlist.vatAmt, "->>>,>>>,>>9")
                            output_list = Output_list()
                            output_list_data.append(output_list)
                        ...
                        output_list.str = output_list.str + to_string(fb_vatlist.vatAmt, "->>>,>>>,>>9.99")
                        output_list = Output_list()
                        output_list_data.append(output_list)
            ...
            art_list = query(art_list_data, filters=(lambda art_list: art_list.i_group == 1), first=True)

            if art_list:
                output_list.str = fill(" ", pos)
                output_list.str = output_list.str +\
                        substring(art_list.bezeich, 0, nbez1) +\
                        fill(" ", nbez1 - length(art_list.bezeich))

                if price_decimal == 0:

                    if not long_digit:
                        output_list.str = output_list.str + to_string(scd_val, "->>>,>>>,>>9")
                        output_list = Output_list()
                        output_list_data.append(output_list)
                    ...
                    output_list.str = output_list.str + to_string(scd_val, "->>>,>>>,>>9.99")
                    output_list = Output_list()
                    output_list_data.append(output_list)
            ...
            output_list.str = output_list.str +\
                    substring(s, 0, nbez1) +\
                    fill(" ", nbez1 - length(s))
            subtotal2 =  to_decimal(subtotal) + to_decimal(tot_amount) + to_decimal(scd_val)

            if price_decimal == 0:

                if not long_digit:
                    output_list.str = output_list.str + to_string(subtotal2, "->>>,>>>,>>9")
                    output_list = Output_list()
                    output_list_data.append(output_list)
                ...
                output_list.str = output_list.str + to_string(subtotal2, "->>>,>>>,>>9.99")
                output_list = Output_list()
                output_list_data.append(output_list)
            ...
            if prall_flag == 0:
                ...
                return
            tot_amount =  to_decimal(subtotal2)
        output_list = Output_list()
        output_list_data.append(output_list)
        ...
        if price_decimal == 0:
            ...
            if not long_digit:
                output_list.str = output_list.str + to_string(round(tot_amount, 2) , "->>>,>>>,>>9")
                output_list = Output_list()
                output_list_data.append(output_list)
            ...
            output_list.str = output_list.str + to_string(round(tot_amount, 2) , "->>>,>>>,>>9.99")
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    def print_overhead3():
        ...
        if h_bill.bilname != "":
            ...
            if qty1000:
                ...
                output_list.str = output_list.str + to_string("", "x(5)")
            output_list.str = output_list.str + to_string(translateExtended ("Guest :", lvcarea, "")) + to_string(" ") + to_string(h_bill.bilname, "x(20)")
            output_list = Output_list()
            output_list_data.append(output_list)
    ...
    def cal_totalfb():
        ...
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, loctax_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, vat2, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, h_vat2, serv_perc, mwst_perc, vat2_perc, mwst1, subtotal, subtotal2, bline_exist, qty1000, i, n, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, is_scd, scd_val, sub_scd_val, serv_scd_val, serv_art, vat_art, sc_art, sc_art1, sc_art2, vat_str, oth_art, disc_art, f_discart1, f_discart2, scd_perc, disc, disc_sc, str451, str452, t_str, art_str, loopi, bartnr, is_bev, is_food, printer, htparam, artikel, h_bill, h_artikel, queasy, h_queasy, kellner, hoteldpt, h_bill_line, paramtext, bediener, tisch, res_line, mc_guest, h_mjourn, printcod, h_journal, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all
        nonlocal artbuff, abuff
    ...
    def multi_currency():
        ...
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, loctax_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, vat2, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, h_vat2, serv_perc, mwst_perc, vat2_perc, fact, mwst1, subtotal, subtotal2, bline_exist, qty1000, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, is_scd, scd_val, sub_scd_val, serv_scd_val, serv_art, vat_art, sc_art, sc_art1, sc_art2, vat_str, oth_art, disc_art, f_discart1, f_discart2, scd_perc, disc, disc_sc, str451, str452, t_str, art_str, loopi, bartnr, is_bev, is_food, printer, htparam, artikel, h_bill, h_artikel, queasy, h_queasy, kellner, hoteldpt, h_bill_line, paramtext, bediener, tisch, res_line, mc_guest, h_mjourn, printcod, h_journal, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all
        nonlocal artbuff, abuff
    ...
    def print_net_vat():
        ...
        nonlocal filename, msg_str, winprinterflag, output_list_data, t_printer_data, lvcarea, disc_bezeich, amount, sort_i, order_id, disc_zwkum, print_balance, disc_art1, disc_art2, disc_art3, incl_service, incl_mwst, service_taxable, print_fbtotal, prdisc_flag, mwst_str, service_str, loctax_str, hmargin, bmargin, lpage, nbezeich, nwidth, npause, bill_date110, bill_date, price_decimal, n11, long_digit, prtwoline, printed_line, last_amount, zeit, comp_flag, service, mwst, vat2, tot_amount, comp_taxserv, tot_sales, new_item, printed, qty, do_it, rm_transfer, new_fbart, tot_line, h_service, h_mwst, h_vat2, serv_perc, mwst_perc, vat2_perc, fact, mwst1, subtotal, subtotal2, bline_exist, qty1000, i, n, curr_j, npage, tot_ndisc_line, tot_disc_line, buttom_lines, prtabledesc, header1, header2, foot1, foot2, anz_foot, overhead1, overhead2, overhead3, overhead4, total_food, total_bev, total_other, total_fdisc, total_bdisc, total_odisc, gst_logic, serv_disc, vat_disc, f_discart, is_scd, scd_val, sub_scd_val, serv_scd_val, serv_art, vat_art, sc_art, sc_art1, sc_art2, oth_art, disc_art, f_discart1, f_discart2, scd_perc, disc, disc_sc, str451, str452, t_str, art_str, loopi, bartnr, is_bev, is_food, printer, htparam, artikel, h_bill, h_artikel, queasy, h_queasy, kellner, hoteldpt, h_bill_line, paramtext, bediener, tisch, res_line, mc_guest, h_mjourn, printcod, h_journal, waehrung
        nonlocal pvilanguage, session_parameter, user_init, hbrecid, printnr, use_h_queasy, print_all
        nonlocal artbuff, abuff
    ...
    def inv_cat(artnr:int):
        ...
        def generate_inner_output():
            return (is_bev, is_food)


        h_artikel = get_cache (H_artikel, {"artnr": [(eq, artnr)]})

        if h_artikel:

            if h_artikel.endkum == 1:
                is_food = True
                is_bev = False

        if h_artikel.endkum == 2:
            is_bev = True
            is_food = False
    ...
    sc_art1 = to_int(entry(1, str451, ";"))
    sc_art2 = to_int(entry(2, str451, ";"))


    str452 = get_output(htpchar(452))
    str452 = replace_str(str452, "," , ";")
    str452 = replace_str(str452, "-" , ";")
    vat_str = trim(str452)

    h_bill = get_cache (H_bill, {"_recid": [(eq, hbrecid)]})
    for loopi in range(1,num_entries(str452, ";")  + 1) :
        bartnr = 0
        bartnr = to_int(entry(loopi - 1, str452, ";"))

        if bartnr != 0:

            h_artikel = get_cache (H_artikel, {"artnr": [(eq, bartnr)],"departement": [(eq, h_bill.departement)]})

            htparam = get_cache (Htparam, {"paramgruppe": [(eq, 4)],"bezeichnung": [(eq, h_artikel.bezeich)]})
            fb_vatlist = Fb_vatlist()
            fb_vatlist_data.append(fb_vatlist)

            fb_vatlist.i_type = loopi + 1
            fb_vatlist.artno = bartnr
            fb_vatlist.vatno = htparam.paramnr
            fb_vatlist.bezeich = h_artikel.bezeich
            fb_vatlist.vatvalue =  to_decimal(htparam.fdecimal) * to_decimal(0.01)

            if num_entries(htparam.fchar, " ") >= 2:
                fb_vatlist.vatvalue =  to_decimal(to_decimal(entry(1 , htparam.fchar , " "))) * to_decimal(0.0001)
    scd_perc = get_output(htpdec(462))

    htparam = get_cache (Htparam, {"paramnr": [(eq, 556)]})

    if htparam:
        f_discart2 = htparam.finteger
    art_str = trim(art_str) + ";" + trim(to_string(f_discart2))

    htparam = get_cache (Htparam, {"paramnr": [(eq, 557)]})

    if htparam:
        f_discart = htparam.finteger
    art_str = trim(art_str) + ";" + trim(to_string(f_discart))

    htparam = get_cache (Htparam, {"paramnr": [(eq, 596)]})

    if htparam:
        f_discart1 = htparam.finteger
    art_str = trim(art_str) + ";" + trim(to_string(f_discart1))
    ...
    if printnr != 0:
        ...
        for h_bill_line in db_session.query(H_bill_line).filter(
            ...
            if not use_h_queasy:
                ...
                queasy = get_cache (Queasy, {"key": [(eq, 4)],"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],
                                             "number2": [(eq, to_int(h_bill_line._recid))],"deci2": [(eq, h_bill.billnr)]})
                ...
                h_queasy = query(H_queasy, filters=(lambda h_queasy: h_queasy.number1 == (h_bill.departement + h_bill.rechnr * 100) and 
                                                    h_queasy.number2 == to_int(h_bill_line._recid) and h_queasy.billno == h_bill.billnr), first=True)
                if not h_queasy:
                    new_item = True
            ...
            if do_it:
                ...
                art_list = query(art_list_data, filters=(lambda art_list: art_list.artnr == h_bill_line.artnr and 
                                                         art_list.dept == h_bill_line.departement and art_list.bezeich == h_bill_line.bezeich and 
                                                         art_list.price == h_bill_line.epreis and art_list.condiment == 0 and not art_list.printed), first=True)
            ...
            if not art_list or h_bill_line.betriebsnr == 1:
                ...
                if prdisc_flag:
                    ...
                    art_list.disc_flag = (art_list.zwkum == disc_zwkum)

                if h_artikel and h_artikel.artart == 0:

                    if h_artikel.artnr == sc_art:
                        art_list.i_group = 1


                    else:

                        fb_vatlist = query(fb_vatlist_data, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.artNo == art_list.artnr), first=True)

                        if fb_vatlist:
                            art_list.i_group = fb_vatlist.i_type
                            fb_vatlist.vatamt =  to_decimal(fb_vatlist.vatamt) + to_decimal(h_bill_line.betrag)
            ...
            if art_list.artart == 0:
                serv_perc, mwst_perc, vat2_perc, fact = cal_servat(h_artikel.departement, h_artikel.artnr, h_artikel.service_code, h_artikel.mwst_code, h_bill_line.bill_datum)

                if art_list.i_group == 0:
                    is_bev, is_food = inv_cat(h_bill_line.artnr)

                    fb_vatlist = query(fb_vatlist_data, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.i_type == 2), first=True)

                    if is_food:
                        fb_vatlist.vatamt = to_decimal(fb_vatlist.vatamt +\
                            round(h_bill_line.betrag / fact * mwst_perc , price_decimal))

                    fb_vatlist = query(fb_vatlist_data, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.i_type == 3), first=True)
                    fb_vatlist.vatamt = to_decimal(fb_vatlist.vatamt +\
                            round(h_bill_line.betrag / fact * serv_perc , price_decimal))

                    artikel = get_cache (Artikel, {"artnr": [(eq, h_artikel.artnrfront)],"departement": [(eq, h_artikel.departement)]})

                    if artikel.prov_code != 0:

                        htparam = get_cache (Htparam, {"paramgruppe": [(eq, 4)],"paramnr": [(eq, artikel.prov_code)]})

                        fb_vatlist = query(fb_vatlist_data, filters=(lambda fb_vatlist: fb_vatlist.fb_vatlist.bezeich == htparam.bezeich), first=True)
                        fb_vatlist.vatamt = to_decimal(fb_vatlist.vatamt + round(h_bill_line.betrag / fact * vat2_perc , price_decimal))
                ...
                if not vat_disc and h_bill_line.artnr == f_discart:
                    ...
                    h_vat2 =  to_decimal(h_bill_line.betrag) / to_decimal(fact) * to_decimal(vat2_perc)
                    h_vat2 = to_decimal(round(h_vat2 , 2))
                ...
                if not incl_mwst:
                    amount =  to_decimal(amount) - to_decimal(h_mwst) - to_decimal(h_vat2)
                    mwst =  to_decimal(mwst) + to_decimal(h_mwst)
                    mwst1 =  to_decimal(mwst1) + to_decimal(h_mwst)
                    vat2 =  to_decimal(vat2) + to_decimal(h_vat2)
        ...
        if prdisc_flag and tot_ndisc_line >= 1 and tot_disc_line >= 1:
            ...
            art_list = query(art_list_data, filters=(lambda art_list: art_list.artart == 0 and not art_list.disc_flag and art_list.i_group == 0 and not art_list.printed), first=True)
            n = 0
            curr_j = printed_line
            for i in range(1,npage + 1) :
                ...
                while None != art_list and (curr_j <= lpage) :
                    ...
                    art_list = query(art_list_data, filters=(lambda art_list: art_list.artart == 0 and not art_list.disc_flag and art_list.i_group == 0 and not art_list.printed), next=True)
                ...
                if i == npage:
                    ...
                    for art_list in query(art_list_data, filters=(lambda art_list: art_list.artart == 0 and not art_list.disc_flag and art_list.i_group == 0 and not art_list.printed)):
                        subtotal =  to_decimal(subtotal) + to_decimal(art_list.amount)
                        print_billine()
        ...
        if printnr >= 0:
            ...
            if not use_h_queasy:
                ...
                queasy = db_session.query(Queasy).filter(
                         (Queasy.key == 4) & (Queasy.number1 == (h_bill.departement + h_bill.rechnr * 100)) & (Queasy.number2 == 0) & 
                         (Queasy.deci2 == to_decimal(h_bill.billnr))).with_for_update().first()
                ...
                # h_queasy = get_cache (H_queasy, {"number1": [(eq, (h_bill.departement + h_bill.rechnr * 100))],"number2": [(eq, 0)],"billno": [(eq, hbill.billnr)]})
                h_queasy = db_session.query(H_queasy).filter(
                         (H_queasy.number1 == (h_bill.departement + h_bill.rechnr * 100)) & (H_queasy.number2 == 0) & (H_queasy.billno == h_bill.billnr)).with_for_update().first()
