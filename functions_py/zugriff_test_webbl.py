...
def zugriff_test_webbl(user_init:string, array_nr:int, expected_nr:int):
    ...
    def generate_output():
        nonlocal zugriff, epoch_signature, mess_str, signature_list_data, mail_exist, logical_flag, n, perm, s1, s2, mn_date, anz, username, bediener, messages, paramtext
        nonlocal user_init, array_nr, expected_nr


        nonlocal value_list, signature_list, tp_bediener, t_messages
        nonlocal value_list_data, signature_list_data, tp_bediener_data, t_messages_data

        return {"zugriff": zugriff, "epoch_signature": epoch_signature, "mess_str": mess_str, "signature-list": signature_list_data}
    ...
    def create_signature(user_name:string, value_list_data:[Value_list]):
        ...
        nonlocal value_list, signature_list, tp_bediener, t_messages
        nonlocal signature_list_data, tp_bediener_data, t_messages_data
    ...
    if user_init == "":
        ...
        epoch_signature, signature_list_data = create_signature(username, value_list_data)
