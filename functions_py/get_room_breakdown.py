...
def get_room_breakdown(recid_resline:int, datum:date, curr_i:int, curr_date:date):
    ...
    def calc_lodging2():
        ...
        if arrangement:
            ...
            reslin_queasy = db_session.query(Reslin_queasy).filter((Reslin_queasy.key == "arrangement") & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.date1 <= datum) & (Reslin_queasy.date2 >= datum)).first()
        ...
        if argtnr == 0:
            tmp_bezeich = res_line.arrangement.strip()
        ...
        if arrangement:
            ...
            for argt_line._recid, argt_line.betriebsnr, argt_line.betrag, argt_line.argtnr, argt_line.vt_percnt, artikel.departement, artikel.artnr, artikel.service_code, artikel.mwst_code, artikel.umsatzart, artikel.zwkum, artikel._recid in db_session.query(Argt_line._recid, Argt_line.betriebsnr, Argt_line.betrag, Argt_line.argtnr, Argt_line.vt_percnt, Artikel.departement, Artikel.artnr, Artikel.service_code, Artikel.mwst_code, Artikel.umsatzart, Artikel.zwkum, Artikel._recid).join(Artikel,(Artikel.artnr == Argt_line.argt_artnr) & (Artikel.departement == Argt_line.departement)).filter((Argt_line.argtnr == arrangement.argtnr) & not_ (Argt_line.kind2) & (Argt_line.kind1)).order_by(Argt_line._recid).all():
    ...
    def get_lodging(argtnr:int, zipreis:Decimal, bill_date:date):
        ...
        if argtnr == 0:
            tmp_bez = res_line.arrangement.strip()
