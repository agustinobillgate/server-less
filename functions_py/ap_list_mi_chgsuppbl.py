...
def ap_list_mi_chgsuppbl(ap_recid:int, lief_nr:int, bediener_nr:int):
    ...
    l_kredit = db_session.query(L_kredit).filter(L_kredit._recid == ap_recid).first()
    ...
    for l_op in db_session.query(L_op).filter(
        ...
        opbuff = db_session.query(Opbuff).filter(Opbuff._recid == l_op._recid).with_for_update().first()
    ...
    if l_kredit.counter != 0:
        ...
        for l_ap in db_session.query(L_ap).filter(
            (L_ap.counter == l_kredit.counter) & 
            (L_ap.opart >= 1) & 
            (L_ap._recid != l_kredit._recid)).with_for_update().order_by(L_ap._recid).all():
