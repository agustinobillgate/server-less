...
def fo_invoice_close_billbl(bil_recid:int, user_init:string, bill_date:date):
    ...
    if bill_anzahl != curr_billnr:
        ...
        bill1 = db_session.query(Bill).filter(
            (Bill.resnr == bill.resnr) & (Bill.parent_nr == bill.parent_nr) & (Bill.billnr == bill_anzahl) & (Bill.flag == 0) & (Bill.zinr == bill.zinr)
    ...
    res_line = db_session.query(Res_line).filter(
            (Res_line.resnr == bill.resnr) & (Res_line.reslinnr == bill.reslinnr) & (Res_line.zinr == bill.zinr)
        ).with_for_update().first()
    if res_line:
