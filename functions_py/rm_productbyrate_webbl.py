...
#---------------------------------------
# Rulita, 28/08/2025
# Modify : Added calculate total arr mtd
# issue scenario 44
#---------------------------------------
...
def rm_productbyrate_webbl(from_date:date, to_date:date):
    ...
    tlist_data, Tlist = create_model("Tlist", {"nr":int, "ratecode":string, "bezeich":string, "rm_rev":Decimal, "arr":Decimal, "rm_sold":Decimal, "fbrev":Decimal, "proz":Decimal, "mrm_rev":Decimal, "marr":Decimal, "mrm_sold":Decimal, "mfbrev":Decimal, "mproz":Decimal, "yrm_rev":Decimal, "yarr":Decimal, "yrm_sold":Decimal, "yfbrev":Decimal, "yproz":Decimal})
    tmp_room_data, Tmp_room = create_model("Tmp_room", {"gastnr":int, "zinr":string, "flag":int})

    Bqueasy = create_buffer("Bqueasy",Queasy)

    set_cache(Queasy, (Queasy.key.in_([212,211])),[["key", "number1"], ["key", "number3"]], True,[],[])

    db_session = local_storage.db_session

    def generate_output():
        nonlocal tlist_data, s, curr_code, do_it, counter, tot_rmrev, tot_arr, tot_rmsold, tot_fbrev, tot_proz, tot_mrmrev, tot_marr, tot_mrmsold, tot_mfbrev, tot_mproz, tot_yrmrev, tot_yarr, tot_yrmsold, tot_yfbrev, tot_yproz, queasy, guest, genstat
        nonlocal from_date, to_date
        nonlocal bqueasy


        nonlocal tlist, tmp_room, bqueasy
        nonlocal tlist_data, tmp_room_data

        return {"tlist": tlist_data}
    ...
    for genstat.logis, genstat.gastnr, genstat.zinr, genstat.res_deci, genstat.datum, genstat._recid, guest.gastnr, guest._recid in db_session.query(Genstat.logis, Genstat.gastnr, Genstat.zinr, Genstat.res_deci, Genstat.datum, Genstat._recid, Guest.gastnr, Guest._recid).join(Guest,(Guest.gastnr == Genstat.gastnr)).filter(
             (Genstat.datum >= from_date) & (Genstat.datum <= to_date) & (Genstat.resstatus != 13) & (Genstat.segmentcode != 0) & (Genstat.nationnr != 0) & (Genstat.zinr != "") & (Genstat.res_logic[inc_value(1)])).order_by(Genstat.gastnr).all():
        ...
        if do_it :
            ...
            tlist = query(tlist_data, filters=(lambda tlist: tlist.ratecode.lower()  == (curr_code).lower()), first=True)
    ...
    for tlist in query(tlist_data):
        ...
        if to_decimal(tlist.rm_sold) != 0 and to_decimal(tlist.rm_sold) != None:
        ...
        if tlist.marr == None:
            ...
            tlist.marr   = safe_divide(tlist.mrm_rev , tlist.mrm_sold)      #Rulita added calculate total arr mtd
        ...
        if to_decimal(tot_rmrev) != 0 and to_decimal(tot_rmrev) != None:
            tlist.proz = ( to_decimal(tlist.rm_rev) / to_decimal(tot_rmrev)) * to_decimal("100")

        if to_decimal(tot_mrmrev) != 0 and to_decimal(tot_mrmrev) != None:
            tlist.mproz = ( to_decimal(tlist.mrm_rev) / to_decimal(tot_mrmrev)) * to_decimal("100")

        if to_decimal(tot_yrmrev) != 0 and to_decimal(tot_yrmrev) != None:
    ...
    for tlist in query(tlist_data, sort_by=[("bezeich",False)]):
        counter = counter + 1
        tlist.nr = counter
