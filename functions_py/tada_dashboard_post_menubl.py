...
def tada_dashboard_post_menubl(user_init:string, tablenumber:int, outletnumber:int, guestname:string, pax:int, orderid:int, menu_list_data:[Menu_list]):
    ...
    if doit :
        ...
        orderhdr = db_session.query(Queasy).filter(
                 (Queasy.key == 271) & (Queasy.betriebsnr == 1) & (Queasy.number1 == deptno) & (Queasy.number2 == orderid) ).with_for_update().first()
        ...
        for menu_list in query(menu_list_data):
            ...
            orderline = db_session.query(Queasy).filter(
                     (Queasy.key == 271) & (Queasy.betriebsnr == 2) & (Queasy.number2 == orderid) & (Queasy.number1 == menu_list.artnr) ).with_for_update().first()
        ...
        guest = db_session.query(Guest).filter(
                 (Guest.mobil_telefon == order_phone) & (Guest.karteityp == 0) ).with_for_update().first()

        if not guest:

            # guest = get_cache (Guest, {"telefon": [(eq, order_phone)],"karteityp": [(eq, 0)]})
            guest = db_session.query(Guest).filter(
                     (Guest.telefon == order_phone) & (Guest.karteityp == 0) ).with_for_update().first()
