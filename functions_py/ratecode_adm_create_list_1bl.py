...
# yusufwijasena, 21/11/2025
# - fix python indentation
# - comment this following line because it couses error "flag_modified(pr_list, "flag")"
# ------------------------------------------
...
def ratecode_adm_create_list_1bl(prcode: string, market_nr: int):
    ...
    pr_list_data, Pr_list = create_model(
        "Pr_list",
        {
            "cstr": [string, 2],
            "prcode": string,
    ...
    def create_list():
        ...
        if market_nr > 0:
            prtable0 = get_cache(
                Prtable, {"marknr": [(eq, market_nr)], "prcode": [(eq, "")]})

            if prtable0:
                prtable = get_cache(
                    Prtable, {"marknr": [(eq, market_nr)], "prcode": [(eq, prcode)]})
                for i in range(1, 99 + 1):
                    if prtable0.zikatnr[i - 1] != 0:
                        zimkateg = get_cache(
                            Zimkateg, {"zikatnr": [(eq, prtable0.zikatnr[i - 1])]})

                        if zimkateg:
                            for j in range(1, 99 + 1):
                                if prtable0.argtnr[j - 1] != 0:
                                    arrangement = get_cache(
                                        Arrangement, {"argtnr": [(eq, prtable0.argtnr[j - 1])]})
                                    found1 = False
                                    k = 1
                                    ...
                                    if arrangement:
                                        ...
                                        product_len = len(
                                            prtable.product) if prtable and prtable.product else 0
    ...
    if ratecode:
        queasy = get_cache(
            Queasy, {"key": [(eq, 2)], "char1": [(eq, ratecode.code)]})
