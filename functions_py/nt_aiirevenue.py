...
from models import Zimkateg, Arrangement, Queasy, Prmarket, Ratecode, Waehrung, Genstat, Artikel, Nightaudit, Nitehist, Htparam, Paramtext, Res_line, Reslin_queasy, Reservation, Segment, Zkstat, Zinrstat, Segmentstat, Umsatz, Budget, Guest, Guestseg, Sourccod, Guest_pr, Nation, exrate, Gl_acct, Billjournal
...
def nt_aiirevenue():
    ...
    def datetime2char(datum:date, zeit:int):
        ...
        nonlocal ci_date, fr_date, fr_date1, to_date, htl_no, loop_datum, co_date, rsv_date, birthdate, canceldate, expdate, address, flight1, flight2, eta, etd, str_rsv, purpose, ankunft, abreise, hharr, mmarr, hhdep, mmdep, ci_time, co_time, pickup, voucherno, contcode, bookdate, dropoff, memozinr, rsv_time, outstr, segm__purcode, loop_i, curr_i, price_decimal, i_counter, droom, mroom, mbroom, flodging, lodging, breakfast, lunch, dinner, others, rmrev, vat, service, tot_lodging, tot_breakfast, tot_lunch, tot_dinner, tot_others, tot_rmrev, tot_vat, tot_service, payable_sum, taxed_payable_sum, drev, mrev, dtrev, mtrev, otherrev, ebdisc_flag, kbdisc_flag, backdate_flag, reihenfolge, progname, exrate, frate, anz, anz0, gastnr_wi, gastnr_ind, cat_flag, fact, n_betrag, n1_betrag, foreign_flag, rev_gross_day, rev_gross_bud, rev_gross_mtd, rev_gross_ytd, zimkateg, arrangement, queasy, prmarket, ratecode, waehrung, genstat, artikel, nightaudit, nitehist, htparam, paramtext, res_line, reslin_queasy, reservation, segment, zkstat, zinrstat, segmentstat, umsatz, budget, guest, guestseg, sourccod, guest_pr, nation, exrate, gl_acct, billjournal
        nonlocal wrung, gstat, buffart
    ...
    def create_rsv_list(i_resnr:int, i_reslinnr:int, i_date:date, i_time:int):
        ...
        if not rate_list and contcode != "":
            ...
            queasy = db_session.query(Queasy).filter(
                     (Queasy.key == 2) & (Queasy.char1 == (contcode).lower())).first()
        ...
        if res_line.resstatus != 9 and res_line.resstatus != 99 and res_line.resstatus != 10:
            ...
            for datum in date_range(res_line.ankunft,co_date) :
                ...
                if reslin_queasy:
                    ...
                    reslin_queasy = db_session.query(Reslin_queasy).filter(
                             (Reslin_queasy.key == ("arrangement").lower()) & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr) & (Reslin_queasy.date1 == res_line.ankunft) & (Reslin_queasy.date2 == res_line.abreise)).first()
        ...
        reslin_queasy = db_session.query(Reslin_queasy).filter(
                 (Reslin_queasy.key == ("arrangement").lower()) & (Reslin_queasy.resnr == res_line.resnr) & (Reslin_queasy.reslinnr == res_line.reslinnr)).first()
    ...
    def find_payable(fib:string, curr_val:Decimal, curr_taxed_val:Decimal, prev_val:Decimal):
        ...
        gl_acct = db_session.query(Gl_acct).filter(
                 (Gl_acct.fibukonto == (fib).lower()) & (Gl_acct.acc_type != 4)).first()
    ...
    def find_gross(fib:string, n1_betrag:Decimal, prev_betrag:Decimal):
        ...
        nonlocal ci_date, fr_date, fr_date1, to_date, htl_no, datum, loop_datum, co_date, rsv_date, birthdate, canceldate, expdate, address, flight1, flight2, eta, etd, str_rsv, purpose, ankunft, abreise, hharr, mmarr, hhdep, mmdep, ci_time, co_time, pickup, voucherno, contcode, bookdate, dropoff, memozinr, rsv_time, outstr, segm__purcode, loop_i, curr_i, price_decimal, i_counter, droom, mroom, mbroom, flodging, lodging, breakfast, lunch, dinner, others, rmrev, vat, service, tot_lodging, tot_breakfast, tot_lunch, tot_dinner, tot_others, tot_rmrev, tot_vat, tot_service, payable_sum, taxed_payable_sum, drev, mrev, dtrev, mtrev, otherrev, ebdisc_flag, kbdisc_flag, backdate_flag, reihenfolge, progname, exrate, frate, anz, anz0, gastnr_wi, gastnr_ind, cat_flag, fact, n_betrag, foreign_flag, rev_gross_day, rev_gross_bud, rev_gross_mtd, rev_gross_ytd, zimkateg, arrangement, queasy, prmarket, ratecode, waehrung, genstat, artikel, nightaudit, nitehist, htparam, paramtext, res_line, reslin_queasy, reservation, segment, zkstat, zinrstat, segmentstat, umsatz, budget, guest, guestseg, sourccod, guest_pr, nation, exrate, gl_acct, billjournal
        nonlocal wrung, gstat, buffart
    ...
    def fill_persocc():
        ...
        nonlocal ci_date, fr_date, fr_date1, htl_no, datum, loop_datum, co_date, rsv_date, birthdate, canceldate, expdate, address, flight1, flight2, eta, etd, str_rsv, purpose, ankunft, abreise, hharr, mmarr, hhdep, mmdep, ci_time, co_time, pickup, voucherno, contcode, bookdate, dropoff, memozinr, rsv_time, outstr, segm__purcode, loop_i, curr_i, price_decimal, i_counter, droom, mroom, mbroom, flodging, lodging, breakfast, lunch, dinner, others, rmrev, vat, service, tot_lodging, tot_breakfast, tot_lunch, tot_dinner, tot_others, tot_rmrev, tot_vat, tot_service, payable_sum, taxed_payable_sum, drev, mrev, dtrev, mtrev, otherrev, ebdisc_flag, kbdisc_flag, backdate_flag, reihenfolge, progname, exrate, frate, anz, anz0, gastnr_wi, gastnr_ind, cat_flag, fact, n_betrag, n1_betrag, foreign_flag, rev_gross_day, rev_gross_bud, rev_gross_mtd, rev_gross_ytd, zimkateg, arrangement, queasy, prmarket, ratecode, waehrung, genstat, artikel, nightaudit, nitehist, htparam, paramtext, res_line, reslin_queasy, reservation, segment, zkstat, zinrstat, segmentstat, umsatz, budget, guest, guestseg, sourccod, guest_pr, nation, exrate, gl_acct, billjournal
        nonlocal wrung, gstat, buffart
    ...
    def find_backdated_fotransaction():
        ...
        def generate_inner_output():
            return (backdate_flag)

        maxtime = 24 * 3600 - 1
        backdate_flag = False

        for billjournal in db_session.query(Billjournal).filter(
                 (Billjournal.bill_datum < ci_date) & (Billjournal.sysdate == ci_date) & (Billjournal.zeit <= maxtime) & (Billjournal.anzahl != 0) & (Billjournal.betrag != 0)).order_by(Billjournal.rechnr).all():
            do_it = True

            artikel = db_session.query(Artikel).filter(
                     (Artikel.artnr == billjournal.artnr) & (Artikel.departement == billjournal.departement)).first()
            do_it = None != artikel and (artikel.artart == 0 or artikel.artart == 8)
    ...
    nightaudit = db_session.query(Nightaudit).filter(
             (Nightaudit.programm == (progname).lower())).first()
    ...
    fill_persocc()
    backdate_flag = False
    backdate_flag = find_backdated_fotransaction()
