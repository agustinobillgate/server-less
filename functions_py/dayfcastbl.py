...
def dayfcastbl(pvilanguage:int, inhouse_flag:bool, disp_flag:bool, lessoo:bool, incl_tent:bool, curr_date:date, printer_nr:int, call_from:int, txt_file:string, vhp_limited:bool):
    ...
    room_list_data, Room_list = create_model("Room_list", {"nr":int, "flag":string, "bez1":string, "bezeich":string, "room":[Decimal,19], "coom":[string,19]})
    sum_list_data, Sum_list = create_model("Sum_list", {"bezeich":string, "summe":[int,19]})
    ...
    def generate_output():
        ...
        nonlocal room_list_data, sum_list_data, segm_list_data

        return {"room-list": room_list_data, "sum-list": sum_list_data, "segm-list": segm_list_data}
    ...
    def create_browse():
        ...
        nonlocal room_list_data, sum_list_data, segm_list_data, tab_label, datum, ci_date, wlist, dlist, curr_day, week_list, i, inhouse, tot_room, inactive, pax, from_date, tmp_date, lvcarea, htparam, zimkateg, zimmer, res_line, reservation, segment, kontline, reslin_queasy, queasy, guestseg, resplan, outorder
        nonlocal pvilanguage, inhouse_flag, disp_flag, lessoo, incl_tent, curr_date, printer_nr, call_from, txt_file, vhp_limited
            ...
            if do_it:
                ...
                for datum in date_range(datum1,datum2) :
                    ...
                    if res_line.abreise > datum and do_it:
                        ...
                        segm_list = query(segm_list_data, filters=(lambda segm_list: segm_list.segmentcode == reservation.segmentcode), first=True)
        ...
        for datum in date_range(curr_date,to_date) :
            ...
            for queasy.number1, queasy.number3, queasy._recid, room.sleeping, room._recid in db_session.query(Queasy.number1, Queasy.number3, Queasy._recid, Room.sleeping, Room._recid).join(Room,(Room.zinr == Queasy.char1) & (Room.sleeping)).filter(
                ...
                if guestseg:
                    ...
                    segm_list = query(segm_list_data, filters=(lambda segm_list: segm_list.segmentcode == guestseg.segmentcode), first=True)
        ...
        for datum in date_range(curr_date,to_date) :
            ...
            for kontline in db_session.query(Kontline).filter(
                ...
                segm_list = query(segm_list_data, filters=(lambda segm_list: segm_list.segmentcode == 99999), first=True)
        ...
        room_list = query(room_list_data, filters=(lambda room_list: room_list.bez1.lower()  == ("Resident").lower()), first=True)
        for i in range(1,19 + 1) :
        ...
        for room_list in query(room_list_data, filters=(lambda room_list: room_list.bezeich != ""), sort_by=[("nr",False)]):
            for i in range(1,19 + 1) :
