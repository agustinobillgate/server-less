...
input_list_data, Input_list = create_model("Input_list", {"guestno":int, "first_name":string, "last_name":string, "country":string, "city":string, "segment":string, "keyaccount":string})

def read_keyaccount_webbl(input_list_data:[Input_list]):
    ...
    ilist = query(ilist_data, first=True)

    if ilist:

        for input_list in query(input_list_data, filters=(lambda input_list: input_list.keyaccount != "")):
            ...
            if queasy:
                ...
                queasy = db_session.query(Queasy).filter(
                     (Queasy.key == 212) & (Queasy.number3 == input_list.guestno)).with_for_update().first()
