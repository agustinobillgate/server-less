...
def prepare_fo_transdetail_webbl(bill_no:int, billdate:date, artno:int, amount:Decimal, systdate:date, systtime:int):
    ...
    def transdetail():
        ...
        for billjournal in db_session.query(Billjournal).filter(
                 (func.substring(Billjournal.bezeich.collate("C"), 1, 1) == ("*").lower()) & (Billjournal.bill_datum == billdate) & (Billjournal.artnr == artno) & (Billjournal.anzahl == 0) & (Billjournal.betrag == amount) & (((Billjournal.sysdate == systdate) & (Billjournal.zeit > systtime)) | ((Billjournal.sysdate > systdate)))).order_by(Billjournal.sysdate, Billjournal.zeit).all():
            

            if to_int(replace_str(billjournal.bezeich.split(";")[0]) , "*", "") == bill_no:
                err = 5
    ...
    if bill and billdate == None:
        ...
        if ci_dt != None:
            ...
            for billjournal in db_session.query(Billjournal).filter(
                     (func.left(Billjournal.bezeich, 1) == ("*")) & (func.replace(func.split_part(Billjournal.bezeich.collate("C"), ";", 1), '*', '') == to_string(bill_no))).order_by(Billjournal._recid).all():
