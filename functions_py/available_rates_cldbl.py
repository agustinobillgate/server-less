...
# - fix rate_list.rateCode to rate_list.ratecode
# - fix created_list.rateCode to created_list.ratecode
...
def available_rates_cldbl(frdate: date, todate: date, i_zikatnr: int, i_counter: int, adult_child_str: string, ind_gastno: int, created_list_data: list[Created_list]):
    ...
    for guest_pr.code, guest_pr._recid, bqueasy.char3, bqueasy._recid, bqueasy.char1, bqueasy.number1, bqueasy.number2, bqueasy.deci2, bqueasy.number3, bqueasy.deci3, bqueasy.date1, bqueasy.date2, bqueasy.logi1 in db_session.query(Guest_pr.code, Guest_pr._recid, Bqueasy.char3, Bqueasy._recid, Bqueasy.char1, Bqueasy.number1, Bqueasy.number2, Bqueasy.deci2, Bqueasy.number3, Bqueasy.deci3, Bqueasy.date1, Bqueasy.date2, Bqueasy.logi1).join(Bqueasy, (Bqueasy.key == 2) & (Bqueasy.char1 == Guest_pr.code)).filter(
        ...
        # elif bqueasy.deci3 > 0 and bqueasy.deci3 < (ankunft - timedelta(days=ci_date)):
            ...
            pqueasy = get_cache(
                Queasy, {"key": [(eq, 264)], "char1": [(eq, bqueasy.char1)]})
                ...
                for bratecode in db_session.query(Bratecode).filter(
                    ...
                    if do_it:
                        ...
                        if not rate_list.expired:
                            ...
                            total_days = (todate - frdate).days
                            for offset in range(total_days + 1):
                                curr_i += 1
                                curr_date = frdate + timedelta(days=offset)
                                allotment_ok = True

                                if allotment_ok:
                                    rate_found, rm_rate, restricted, kback_flag = get_output(
                                        ratecode_rate(
                                            False, False, 0, 1,
                                            "!" + bqueasy.char1,
                                            ci_date,
            ...
            if rate_list and not rate_created:
                ...
                for curr_date in (frdate - todate).days:
                    ...
                    if dynarate_list:
                        ...
                        if not global_occ:
                            queasy = get_cache(
                                Queasy, {"key": [(eq, 145)], "char1": [(eq, guest_pr.code)], "char2": [(eq, mapcode)], "number1": [(eq, i_zikatnr)], "deci1": [(eq, dynarate_list.w_day)], "deci2": [(eq, dynarate_list.counter)], "date1": [(eq, curr_date)]})
                        else:
                            queasy = get_cache(
                                Queasy, {"key": [(eq, 145)], "char1": [(eq, guest_pr.code)], "char2": [(eq, mapcode)], "number1": [(eq, 0)], "deci1": [(eq, dynarate_list.w_day)], "deci2": [(eq, dynarate_list.counter)], "date1": [(eq, curr_date)]})
                ...
                if not_found_flag:
                    for curr_date in (frdate - todate).days:
                        ...
                        if dynarate_list:
                            ...
                            if not global_occ:
                                queasy = get_cache(
                                    Queasy, {"key": [(eq, 145)], "char1": [(eq, guest_pr.code)], "char2": [(eq, mapcode)], "number1": [(eq, i_zikatnr)], "deci1": [(eq, 0)], "deci2": [(eq, dynarate_list.counter)], "date1": [(eq, curr_date)]})
                            else:
                                queasy = get_cache(
                                    Queasy, {"key": [(eq, 145)], "char1": [(eq, guest_pr.code)], "char2": [(eq, mapcode)], "number1": [(eq, 0)], "deci1": [(eq, dynarate_list.w_day)], "deci2": [(eq, dynarate_list.counter)], "date1": [(eq, curr_date)]})
