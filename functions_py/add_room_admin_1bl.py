...
zimmer_list_data, Zimmer_list = create_model("Zimmer_list", {"bediener_nr_stat":int, "betriebsnr":int, "bezeich":string, "build":string, "code":string, "etage":int, "features":string, "fixpreis":bool, "flag1":int, "flag2":int, "flag3":int, "flag4":int, "himmelsr":string, "house_status":int, "kbezeich":str, "nebenstelle":str, "nebstflag":int, "owner_nr":int, "personal":bool, "personen":int, "prioritaet":int, "reihenfolge":int, "setup":int, "sleeping":bool, "sollflag":int, "typ":int, "verbindung":[string, 3], "vid_actuel":int, "vid_request":int, "wertigkeit":int, "zikatnr":int, "zikennz":string, "zinr":string, "zistatus":int})
...
def add_room_admin_1bl(pvilanguage:int, case_type:int, zimmer_list_data:[Zimmer_list], rm_feature:string, curr_mode:string, rmno:string, rmcatbez:string, user_init:string):
    ...
    if type(zimmer_list.reihenfolge) == str:
        zimmer_list.reihenfolge = to_int(zimmer_list.reihenfolge.strip())
    if type(zimmer_list.typ) == str:
        zimmer_list.typ = to_int(zimmer_list.typ.strip())
