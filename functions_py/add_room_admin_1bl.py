...
zimmer_list_data, Zimmer_list = create_model(
    "Zimmer_list",
    {
        "bediener_nr_stat": int,
        "betriebsnr": int,
        "bezeich": string,
        "build": string,
        "code": string,
...
def add_room_admin_1bl(pvilanguage: int, case_type: int, zimmer_list_data: [Zimmer_list], rm_feature: string, curr_mode: string, rmno: string, rmcatbez: string, user_init: string):
    ...
    def generate_output():
        ...
        return {
            ...
            "dynaRate-list": dynarate_list_data
        }
    ...
    def fill_zimmer():
        ...
        if (case_type != 1) and (zimmer_list.sleeping != zimmer.sleeping):
            ...
            res_history.aenderung = "RoomNo: " + zimmer.zinr + " change room active " +\
                to_string(zimmer.sleeping) + " to " + \
                to_string(zimmer_list.sleeping)
    ...
    def check_dynarate():
        ...
        for dynarate_list in query(dynarate_list_data, filters=(lambda dynarate_list: dynarate_list.dynarate_list.to_room < zimkateg.maxzimanz)):
            ct = ct + dynarate_list.prcode + " " + \
                to_string(dynarate_list.to_room) + "; "

        if ct != "":
            msg_str = msg_str + chr_unicode(2) + translateExtended("The MAX To-Room of following Ratecode(s) < Room QTY of the related Room Type",
                                                                   lvcarea, "") + " (= " + to_string(zimkateg.maxzimanz) + ")." + chr_unicode(10) + ct
    ...
    def update_queasy():
        ...
        zimkateg = get_cache(
            Zimkateg, {"zikatnr": [(eq, zimmer_list.zikatnr)]})
        ...
        queasy = get_cache(Queasy, {"key": [(eq, 171)], "date1": [
                           (ge, get_current_date())], "number1": [(eq, zikatnr)]})
