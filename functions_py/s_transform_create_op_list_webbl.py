...
"""_yusufwijasena_21/10/2025
    ...
    TicketID: 6B68DB
        _issue_:    - update from FDL: 174FEB, MALIK: 07D1DD, OSCAR: D2BFCE
"""
...
def s_transform_create_op_list_webbl(op_list_data:[Op_list], qty:Decimal, price:Decimal, curr_lager:int, transdate:date, s_artnr:int, bediener_nr:string, lscheinnr:string, t_amount:Decimal):
    ...
    for op_list in query(op_list_data, filters=(lambda op_list: op_list.userinit.lower()  != (bediener_nr).lower())):
        op_list.userinit = bediener_nr # MALIK: 07D1DD

    l_artikel_obj_list = {}
    for l_artikel, sys_user in db_session.query(L_artikel, Sys_user).join(Sys_user,(Sys_user.userinit == op_list.userinit)).filter(
        ...
        if l_artikel_obj_list.get(l_artikel._recid):
            ...
            l_artikel_obj_list[l_artikel._recid] = True # OSCAR: D2BFCE
        ...
        t_op_list.bezeich = l_artikel.bezeich
        t_op_list.username = sys_user.username # OSCAR: D2BFCE
