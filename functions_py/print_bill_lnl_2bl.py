...
from functions.prepare_fo_transdetail_lnlbl import prepare_fo_transdetail_lnlbl
from models import Billjournal, Guest, Res_line, Htparam, Paramtext, Bill, Mc_guest, Artikel, Bill_line, Reservation, Bediener, Arrangement, Waehrung, Zimmer, Queasy, Exrate

t_spbill_list_data, T_spbill_list = create_model("T_spbill_list", {"selected":bool, "bl_recid":int}, {"selected": True})

def print_bill_lnl_2bl(t_spbill_list_data:[T_spbill_list], pvilanguage:int, curr_status:string, briefnr:int, resnr:int, reslinnr:int, inv_type:int, rechnr:int, curr_program:string, gastnr:int, spbill_flag:bool, user_init:string, lnldelimeter:string, for_curr:Decimal, conv_wabkurz:string, curr_bezeich:string, disp_transfer:bool):

    prepare_cache ([Billjournal, Guest, Res_line, Htparam, Paramtext, Bill, Mc_guest, Artikel, Reservation, Bediener, Arrangement, Waehrung, Zimmer, Queasy, Exrate])
    ...
    nopd:string = ""
    amt_beftax_round0:Decimal = to_decimal("0.0")
    netto_amt:Decimal = to_decimal("0.0")
    netto_amount_r0:Decimal = to_decimal("0.0")
    vat_amt_r0:Decimal = to_decimal("0.0")
    serv_amt_r0:Decimal = to_decimal("0.0")
    netto_amount:Decimal = to_decimal("0.0")
    vat_amt:Decimal = to_decimal("0.0")
    serv_amt:Decimal = to_decimal("0.0")
    ...
    systtime:int = 0
    created:bool = False
    billjournal = guest = res_line = htparam = paramtext = bill = mc_guest = artikel = bill_line = reservation = bediener = arrangement = waehrung = zimmer = queasy = exrate = None

    bline_list = temp_bjournal = sum_tbl = t_str3 = t_spbill_list = bl_guest = bline_vatlist = guest1 = resline = None
    ...
    def generate_output():
        nonlocal str1, str2, str3, t_str3_data, lvcarea, briefnr2, briefnr21, htl_name, htl_adr1, htl_adr2, htl_adr3, htl_tel, htl_fax, htl_email, resv_name, username, bill_recv, bill_no, bl_descript, bl_descript0, bl_voucher, bl0_balance, bl0_balance1, bl_balance, bl_balance1, sum_anz, address1, address2, address3, email, hp_no, acc, adult, child1, child2, complgst, room_no, room_price, arrival, departure, bl_guest, l_guest, bl_instruct, resno, in_word, room_cat, city, country, zip, hp_guest, phone, bl_accom, sstr1, sstr2, sstr3, w_length, progname, temp_amt, wi_gastnr, ind_gastnr, select_flag, g_comment, bill_comment, ma_gst_amount, ma_gst_tot_sales_artikel, ma_gst_tot_non_taxable, ma_gst_tot_taxable, ma_gst_gtot_tax, curr_guest, tot_service_code, serv1, vat1, vat3, fact1, netto, guest_taxcode, nopd, amt_beftax_round0, netto_amt, netto_amount_r0, vat_amt_r0, serv_amt_r0, netto_amount, vat_amt, serv_amt, tot_inclvat, net_amount, serv_code, acc_tax, vat_cam, do_it, membernumber, mgst, artnr_1001, selected_room, paidout, frate, rm_serv, rm_vat, service, vat, amount_bef_tax, vat2, fact, billdate, artno, amount, systdate, systtime, created, billjournal, guest, res_line, htparam, paramtext, bill, mc_guest, artikel, bill_line, reservation, bediener, arrangement, waehrung, zimmer, queasy, exrate
        nonlocal pvilanguage, curr_status, briefnr, resnr, reslinnr, inv_type, rechnr, curr_program, gastnr, spbill_flag, user_init, lnldelimeter, for_curr, conv_wabkurz, curr_bezeich, disp_transfer
        nonlocal guest1, resline


        nonlocal bline_list, temp_bjournal, sum_tbl, t_str3, t_spbill_list, bl_guest, bline_vatlist, guest1, resline
        nonlocal bline_list_data, temp_bjournal_data, sum_tbl_data, t_str3_data, bl_guest_data, bline_vatlist_data
    ...
    def calc_bl_balance1(datum:date, betrag:Decimal, fremdwbetrag:Decimal, fbetrag:Decimal):
        ...
        nonlocal str1, str2, str3, t_str3_data, lvcarea, briefnr2, briefnr21, htl_name, htl_adr1, htl_adr2, htl_adr3, htl_tel, htl_fax, htl_email, resv_name, username, bill_recv, bill_no, bl_descript, bl_descript0, bl_voucher, bl0_balance, bl0_balance1, bl_balance, bl_balance1, sum_anz, address1, address2, address3, email, hp_no, acc, adult, child1, child2, complgst, room_no, room_price, arrival, departure, bl_guest, l_guest, bl_instruct, resno, in_word, room_cat, city, country, zip, hp_guest, phone, bl_accom, sstr1, sstr2, sstr3, w_length, progname, temp_amt, wi_gastnr, ind_gastnr, select_flag, g_comment, bill_comment, ma_gst_amount, ma_gst_tot_sales_artikel, ma_gst_tot_non_taxable, ma_gst_tot_taxable, ma_gst_gtot_tax, curr_guest, tot_service_code, serv1, vat1, vat3, fact1, netto, guest_taxcode, nopd, amt_beftax_round0, netto_amt, netto_amount_r0, vat_amt_r0, serv_amt_r0, netto_amount, vat_amt, serv_amt, tot_inclvat, net_amount, serv_code, acc_tax, vat_cam, do_it, membernumber, mgst, artnr_1001, selected_room, paidout, frate, rm_serv, rm_vat, service, vat, amount_bef_tax, vat2, fact, artno, amount, systdate, systtime, created, billjournal, guest, res_line, htparam, paramtext, bill, mc_guest, artikel, bill_line, reservation, bediener, arrangement, waehrung, zimmer, queasy, exrate
        nonlocal pvilanguage, curr_status, briefnr, resnr, reslinnr, inv_type, rechnr, curr_program, gastnr, spbill_flag, user_init, lnldelimeter, for_curr, conv_wabkurz, curr_bezeich, disp_transfer
        nonlocal guest1, resline
    ...
    if curr_program.lower()  == ("ns-invoice").lower() :
        ...
        if disp_transfer:
            created, temp_bjournal_data = get_output(prepare_fo_transdetail_lnlbl(rechnr, billdate, artno, amount, systdate, systtime))
        ...
        for bill_line, artikel in db_session.query(Bill_line, Artikel).join(Artikel,(Artikel.artnr == Bill_line.artnr) & (Artikel.departement == Bill_line.departement)).filter(
            ...
            if artikel.artart == 0 or artikel.artart == 1 or artikel.artart == 8 or artikel.artart == 9:
                ...
                serv1, vat1, vat3, fact1 = get_output(calc_servtaxesbl(1, artikel.artnr, artikel.departement, bill_line.bill_datum))
                netto_amt =  to_decimal(bill_line.betrag) / to_decimal(fact1)
                netto_amt = to_decimal(round(netto_amt , 2))
                serv_amt =  to_decimal(netto_amt) * to_decimal(serv1)
                serv_amt = to_decimal(round(serv_amt , 2))
                vat_amt =  to_decimal(netto_amt) * to_decimal(vat1)
                vat_amt = to_decimal(round(vat_amt , 2))
                netto_amount_r0 =  to_decimal(netto_amount_r0) + to_decimal(netto_amt)
                serv_amt_r0 =  to_decimal(serv_amt_r0) + to_decimal(serv_amt)
                vat_amt_r0 =  to_decimal(vat_amt_r0) + to_decimal(vat_amt)


        netto_amount_r0 = to_decimal(round(netto_amount_r0 , 0))
        serv_amt_r0 = to_decimal(round(serv_amt_r0 , 0))
        vat_amt_r0 = to_decimal(round(vat_amt_r0 , 0))
        ...
        if res_line:
            ...
            resline = get_cache (Res_line, {"resnr": [(eq, res_line.resnr)],"l_zuordnung[2]": [(ne, 0)]})

            if resline:

                guest = get_cache (Guest, {"gastnr": [(eq, resline.gastnrmember)]})
        ...
        if curr_status.lower()  == ("design").lower() :
            ...
            if not bline_list and not created:
            ...
            if bline_list:
                ...
                amt_beftax_round0 = to_decimal(round(amount_bef_tax , 2))
                bl_balance =  to_decimal(bl_balance) + to_decimal(bline_list.saldo)
                ma_gst_amount =  to_decimal("0")
                str3 = to_string(bline_list.datum) + lnldelimeter + bl_descript + lnldelimeter + to_string(bline_list.anzahl, "->>>") + lnldelimeter + bline_list.zinr + lnldelimeter + to_string(bline_list.saldo, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + bline_list.userinit + lnldelimeter + bl_guest + lnldelimeter + bl_descript0 + lnldelimeter + bl_voucher + lnldelimeter + to_string(amount_bef_tax, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(bline_list.epreis, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(bl_balance, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + in_word + lnldelimeter + to_string(ma_gst_amount, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(for_curr, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + conv_wabkurz + lnldelimeter + curr_bezeich + lnldelimeter + to_string(amt_beftax_round0, "->>>,>>>,>>>,>>>,>>9")

            elif created:

                temp_bjournal = query(temp_bjournal_data, first=True)
                bl_descript = to_string(temp_bjournal.item_name)
                bl_descript0 = entry(0, bl_descript, "/")
                ...
                if num_entries(bl_descript, "/") > 1:
                    ...
                    bl_voucher = ""
                amount_bef_tax =  to_decimal(temp_bjournal.betrag)
                amount_bef_tax =  to_decimal(amount_bef_tax) / to_decimal((1) + to_decimal(service) + to_decimal(vat))
                bl_balance =  to_decimal(bl_balance) + to_decimal(temp_bjournal.betrag)
                ma_gst_amount =  to_decimal("0")
                str3 = to_string(temp_bjournal.bill_datum) + lnldelimeter + bl_descript + lnldelimeter + to_string(temp_bjournal.anzahl, "->>>") + lnldelimeter + to_string(temp_bjournal.zinr) + lnldelimeter + to_string(temp_bjournal.betrag, "->>,>>>,>>>,>>9.99") + lnldelimeter + to_string(temp_bjournal.userinit) + lnldelimeter + bl_guest + lnldelimeter + bl_descript0 + lnldelimeter + bl_voucher + lnldelimeter + to_string(amount_bef_tax, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(temp_bjournal.epreis, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(bl_balance, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + in_word + lnldelimeter + to_string(ma_gst_amount, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + to_string(for_curr, "->>>,>>>,>>>,>>>,>>9.99") + lnldelimeter + conv_wabkurz + lnldelimeter + curr_bezeich + lnldelimeter + to_string(" ")
            t_str3 = T_str3()
            t_str3_data.append(t_str3)
            ...
            if created:
                ...
                for temp_bjournal in query(temp_bjournal_data):
                    ...
                    billjournal = get_cache (Billjournal, {"_recid": [(eq, temp_bjournal.bj_recid)]})

                    artikel = get_cache (Artikel, {"artnr": [(eq, billjournal.artnr)],"departement": [(eq, billjournal.departement)]})

                    if artikel.mwst_code != 0 or artikel.service_code != 0 or artikel.prov_code != 0:
                        service, vat, vat2, fact = get_output(calc_servtaxesbl(1, artikel.artnr, artikel.departement, billjournal.bill_datum))
                    vat =  to_decimal(vat) + to_decimal(vat2)
        ...
        if res_line:
            ...
            for res_line in db_session.query(Res_line).filter(
                     (Res_line.resnr == resnr) & (Res_line.resstatus != 12) & (Res_line.resstatus != 9) & (Res_line.resstatus != 10) & (Res_line.resstatus != 13) & (Res_line.l_zuordnung[inc_value(2)] != 1)).order_by(Res_line.name, Res_line.zinr).all():
        ...
        if curr_status.lower()  == ("design").lower() :
            ...
            str1 = "$bill-recv" + bill_recv + lnldelimeter + "$bill-no" + bill_no + lnldelimeter + "$address1" + address1 + lnldelimeter + "$address2" + address2 + lnldelimeter + "$address3" + address3 + lnldelimeter + "$hp-no" + hp_no + lnldelimeter + "$room-no" + room_no + lnldelimeter + "$arrival" + arrival + lnldelimeter + "$arrival0" + arrival + lnldelimeter + "$departure" + departure + lnldelimeter + "$departure0" + departure + lnldelimeter + "$bl-guest" + bl_guest + lnldelimeter + "$resno" + resno + lnldelimeter + "$bl-id" + user_init + lnldelimeter + "$Date" + to_string(get_current_date(), "99/99/9999") + lnldelimeter + "$bl-time" + to_string(get_current_time_in_seconds(), "HH:MM") + lnldelimeter + "$hp-guest" + hp_guest + lnldelimeter + "$phone" + phone + lnldelimeter + "$memberno" + membernumber + lnldelimeter + "$resv-name" + resv_name + lnldelimeter + "$acc-guest" + bl_accom + lnldelimeter + "$bl-username" + username + lnldelimeter + "$guest-taxcode" + guest_taxcode + lnldelimeter + "$nopd" + nopd
            ...
            if not spbill_flag:
                ...
                if inv_type == 2:
                    ...
                    t_spbill_list = query(t_spbill_list_data, filters=(lambda t_spbill_list: t_spbill_list.selected), first=True)
