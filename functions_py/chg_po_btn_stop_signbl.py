...
def chg_po_btn_stop_signbl(docu_nr:string, q245_data:[Q245]):
    ...
    for queasy in db_session.query(Queasy).filter(
             (Queasy.key == 245) & (Queasy.char1 == (docu_nr).lower())).order_by(Queasy._recid).with_for_update().all():
    ...
    for q245 in query(q245_data):
        ...
        queasy = db_session.query(Queasy).filter(
             (Queasy.key == q245.key) & (Queasy.char1 == (q245.docu_nr)) &
             (Queasy.number1 == q245.app_no)).with_for_update().first() 
