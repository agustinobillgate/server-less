...
from models import Paramtext, Zimmer, Queasy, Zimkateg, Res_line

input_list_data, Input_list = create_model("Input_list", {"case_type":int, "rmno":string, "rmcatbez":string, "user_init":string})
input_zimmer_data, Input_zimmer = create_model("Input_zimmer", {"zinr":string, "view_type":int, "bed_type":int, "etage":int, "zikatnr":int, "prev_zinr":string, "prev_etage":int})

def add_room_admin_setup_wizardbl(input_list_data:[Input_list], input_zimmer_data:[Input_zimmer]):
    ...
    bed_default:int = 0
    floor_mode:string = ""
    paramtext = zimmer = queasy = zimkateg = res_line = None

    input_list = input_zimmer = output_zimmer = output_list = bf_paramtext = bf_zimmer = bf_queasy = None

    output_zimmer_data, Output_zimmer = create_model("Output_zimmer", {"zinr":string, "view_type":string, "bed_type":string, "etage":int, "zikatnr":int, "sleeping":bool})
    ...
    def generate_output():
        nonlocal output_zimmer_data, output_list_data, bed_default, floor_mode, paramtext, zimmer, queasy, zimkateg, res_line
    ...
    def create_section():
        ...
        queasy = get_cache (Queasy, {"key": [(eq, 357)],"number1": [(eq, 3)],"deci1": [(eq, 3.4)],"char1": [(eq, "room setup")],"char2": [(eq, "room admin")],"logi1": [(eq, True)]})
    ...
    def create_floor():
        ...
        if floor_mode.lower()  == ("add").lower() :
        ...
        elif floor_mode.lower()  == ("chg").lower() :
            ...
            if input_zimmer.prev_etage != input_zimmer.etage:
                ...
                queasy = get_cache (Queasy, {"key": [(eq, 25)],"number2": [(eq, input_zimmer.prev_etage)],"betriebsnr": [(ne, 0)]})
                ...
                queasy = get_cache (Queasy, {"key": [(eq, 25)],"number2": [(eq, input_zimmer.etage)],"betriebsnr": [(ne, 0)]})
        ...
        elif floor_mode.lower()  == ("modify/create").lower() :
    ...
    if input_list.case_type == 1:
        ...
        for input_zimmer in query(input_zimmer_data, sort_by=[("zinr",True)]):
            ...
            zimkateg = get_cache (Zimkateg, {"zikatnr": [(eq, input_zimmer.zikatnr)]})
            ...
            if not zimmer:
                ...
                output_list.success_flag = False
                output_list.msg_str = "Create room setup number of " + to_string(input_zimmer.zinr) + " already exist. Please create another room number"
        ...
        if zimmer:
            ...
            if trim(input_zimmer.prev_zinr) != trim(input_zimmer.zinr):
                ...
                output_list.success_flag = False
                output_list.msg_str = "Room number " + to_string(input_zimmer.zinr) + " already exist. Please change another room number."
            ...
            output_list.success_flag = True
            output_list.msg_str = "Update room setup number of " + to_string(input_zimmer.zinr) + " successfully."
            ...
            output_list.success_flag = False
            output_list.msg_str = "Room number " + to_string(input_zimmer.zinr) + " not found. Please change another room number."
