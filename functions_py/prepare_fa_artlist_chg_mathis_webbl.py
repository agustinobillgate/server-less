...
def prepare_fa_artlist_chg_mathis_webbl(recid_mathis: int, recid_fa_artikel: int):
    ...
    def generate_output():
        nonlocal spec, locate, curr_location, curr_gnr, curr_subgrp, curr_asset, fibukonto, credit_fibu, debit_fibu, upgrade_part, grp_bez, sgrp_bez, rate, rate_bez, fa_grup_fibukonto, fa_grup_credit_fibu, fa_grup_debit_fibu, m_list_data, fa_art_data, mathis, fa_artikel, queasy, fa_grup, fa_kateg
        ...
        return {
            ...
            "curr_gnr": curr_gnr,
            "curr_subgrp": curr_subgrp,
            "curr_asset": curr_asset,
            "fibukonto": fibukonto,
            "credit_fibu": credit_fibu,
            "debit_fibu": debit_fibu,
            "upgrade_part": upgrade_part,
            "grp_bez": grp_bez,
            "sgrp_bez": sgrp_bez,
            "rate": rate,
            "rate_bez": rate_bez,
            "fa_grup_fibukonto": fa_grup_fibukonto,
            "fa_grup_credit_fibu": fa_grup_credit_fibu,
            "fa_grup_debit_fibu": fa_grup_debit_fibu,
            "m-list": m_list_data,
            "fa-art": fa_art_data
        }
    ...
    queasy = get_cache(
        Queasy, {"key": [(eq, 314)], "number1": [(eq, fa_artikel.nr)]})
    ...
    fa_grup = get_cache(
        Fa_grup, {"flag": [(eq, 1)], "gnr": [(eq, fa_artikel.subgrp)]})
    if fa_grup:
