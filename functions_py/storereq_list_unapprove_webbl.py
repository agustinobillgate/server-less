...
def storereq_list_unapprove_webbl(payload_list_data):
        ...
        if l_op:
            ...
            if l_op.datum <= close_date:
                ...
                l_ophdr = db_session.query(L_ophdr).filter(
                         (L_ophdr.op_typ == "req") & (L_ophdr.lscheinnr == l_op.lscheinnr) & (L_ophdr.docu_nr == l_op.lscheinnr)).with_for_update().first()
