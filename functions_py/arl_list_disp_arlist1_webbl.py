...
def arl_list_disp_arlist1_webbl(show_rate:bool, last_sort:int, lresnr:int, long_stay:int, ci_date:date, grpflag:bool, room:string, lname:string, sorttype:int, fdate1:date, fdate2:date, fdate:date):
    ...
    def generate_output():
        nonlocal rmlen, arl_list_data, vipnr1, vipnr2, vipnr3, vipnr4, vipnr5, vipnr6, vipnr7, vipnr8, vipnr9, done_flag, curr_resnr, curr_resline, today_str, reserve_str, created_time, do_it, loop_i, comment_str, all_inclusive, res_mode, checkin_flag, stay, resbemerk, rescomment, guest, paramtext, htparam, res_line, waehrung, reservation, zimkateg, master, messages, kontline, gentable, mc_guest, reslin_queasy, zimmer, queasy
    ...
    def fixing_blank_resname():
        ...
        while None != res_line:
            ...
            rline = db_session.query(Res_line).filter(
                         (Res_line._recid == res_line._recid)).with_for_update().first()
    ...
    def disp_arlist_group():
        ...
        if sorttype == 1:
            ...
            for res_line, gbuff, waehrung, reservation, zimkateg in db_session.query(Res_line, Gbuff, Waehrung, Reservation, Zimkateg).join(Gbuff,(Gbuff.gastnr == Res_line.gastnr)).join(Waehrung,(Waehrung.waehrungsnr == Res_line.betriebsnr)).join(Reservation,(Reservation.resnr == Res_line.resnr) & (matches(Reservation.groupname,(lname)))).join(Zimkateg,(Zimkateg.zikatnr == Res_line.zikatnr)).filter(
                ...
                arlbuff = query(arlbuff_data, filters=(lambda arlbuff: arlbuff.resnr == res_line.resnr and arlbuff.reslinnr == res_line.reslinnr), first=True)
