...
DEFINE TEMP-TABLE copyRequest
    ...
    FIELD avail-image   AS LOGICAL INIT NO
...
DEFINE VARIABLE type-str AS CHAR EXTENT 6
...
RUN create-request1.
RUN open-query1.
RUN eg-repmaintain-open-querybl.p (TODAY, TODAY, OUTPUT TABLE t-eg-maintain).

FOR EACH t-eg-maintain NO-LOCK ,
    FIRST eg-location WHERE eg-location.nr = t-eg-maintain.location NO-LOCK, 
    FIRST eg-property WHERE eg-property.nr = t-eg-maintain.propertynr NO-LOCK,
    FIRST eg-staff WHERE eg-staff.nr = t-eg-maintain.pic NO-LOCK:
    RUN create-temp.
END.

FOR EACH smaintain NO-LOCK:
    FOR EACH eg-mdetail WHERE KEY = 1 AND eg-mdetail.maintainnr = smaintain.maintainnr NO-LOCK,
        FIRST eg-action WHERE eg-action.actionnr = eg-mdetail.nr NO-LOCK:
        CREATE action.
        ASSIGN
            action.actionnr = eg-action.actionnr
            action.bezeich = eg-action.bezeich
            action.maintask = eg-action.maintask
...
PROCEDURE create-temp:
    ...
    ASSIGN  
        ...
        smaintain.freq          = type-str[t-eg-maintain.typework]  
        smaintain.location      = eg-location.bezeich /*t-eg-maintain.location tproperty.ploc*/     
        smaintain.zinr          = t-eg-maintain.zinr  /*tproperty.pzinr */       
        smaintain.property      = eg-property.bezeich + "(" + trim(string(eg-property.nr , ">>>>>>9")) + ")"   
        smaintain.comments      = t-eg-maintain.comments     
        smaintain.pic           = eg-staff.NAME
    .
END.
        ...
        DO: 
            ...
            DO:
                FOR EACH eg-request WHERE eg-request.zinr = rmno 
                    ...
                    FIRST eg-property WHERE eg-property.nr = eg-request.propertynr NO-LOCK 
...
PROCEDURE create-copy1 :
    ...
    ASSIGN 
        ...
        copyRequest.property       = eg-request.propertynr
        /*copyRequest.property-nm    = tproperty.prop-nm + "(" + trim(string(eg-request.propertynr , ">>>>>>9")) + ")"*/
    ...
    pic-number =  INTEGER("-8" + STRING(eg-request.reqnr)).
    FIND FIRST guestbook WHERE guestbook.gastnr EQ pic-number 
        AND guestbook.reserve-int[1] EQ eg-request.reqnr EXCLUSIVE-LOCK NO-ERROR.
    IF AVAILABLE guestbook THEN copyRequest.avail-image = YES.
    
    FIND FIRST eg-property WHERE eg-property.nr = eg-request.propertynr NO-LOCK NO-ERROR.
    IF AVAILABLE eg-property THEN
    DO:
        ASSIGN copyRequest.property-nm  = eg-property.bezeich + "(" + trim(string(eg-request.propertynr , ">>>>>>9")) + ")".
    END.
    ELSE
